...
EVAL NOTE: The Agent tool is not available. At any dispatch step in the sequence, output one <agent-call> block per labeled frame from the prior step's output — the count of <agent-call> blocks must equal the count of labeled frame headings (lines matching '### Frame N:') in the prior step's output. A dispatch step is not complete until <agent-call> blocks appear in the same response turn — a response turn that reads or references the dispatch step but contains no <agent-call> blocks does not satisfy this gate. Each <prompt> must contain: (1) the inner step bar build commands listed in the sequence definition for that dispatch step; (2) an explicit instruction to pass the output of each step as --subject to the next step's bar build call — for example, the prompt must instruct the subagent to run 'bar build probe survive ghost audit --subject "<output of make form:prep>"' so that each step's output becomes the subject for the next step. A prompt that lists sequential bar build commands without --subject chaining between them does not satisfy this requirement. Format:
<agent-call>
