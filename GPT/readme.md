...
Channel (`channelModifier`): `adr`, `agent`, `aloud`, `canvas`, `code`, `codetour`, `demo`, `diagram`, `draw`, `formal`, `gherkin`, `github`, `html`, `hunk`, `image`, `jira`, `ledger`, `notebook`, `plain`, `presenterm`, `remote`, `shellscript`, `sketch`, `skill`, `slack`, `store`, `svg`, `sync`, `video`, `zettel`
