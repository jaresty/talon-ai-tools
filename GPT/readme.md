...
Channel (`channelModifier`): `adr`, `agent`, `code`, `codetour`, `demo`, `diagram`, `draw`, `formal`, `gherkin`, `html`, `image`, `jira`, `ledger`, `notebook`, `plain`, `presenterm`, `remote`, `shellscript`, `sketch`, `skill`, `slack`, `store`, `svg`, `sync`, `video`, `zettel`
