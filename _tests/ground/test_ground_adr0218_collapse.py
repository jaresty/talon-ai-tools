...
class TestADR0218Collapse(GroundADRTestBase):
    ...
    def test_observable_evidence_required(self):
        self.assertDetects("Pre/post change states visible through actual tool output", self.core)
    ...
    def test_completeness_slice(self):
        self.assertDetects("One independently testable behavior per thread per cycle", self.core)

    # --- ADR-0235: artifact type definitions ---

    def test_validation_defined(self):
        self.assertDetects("executable validation artifact is a file whose sole purpose is to assert behavioral properties", self.core)
    ...
    def test_validation_not_imported(self):
        self.assertDetects("validation files may not be imported by implementation files", self.core)

    def test_implementation_defined(self):
        self.assertDetects("executable implementation artifact is a file that produces behavior directly", self.core)
    ...
    def test_classification_by_content(self):
        self.assertDetects("classification is determined by file content, not file path or naming convention", self.core)

    # --- ADR-0236/0237: live-code observation ---

    def test_observation_means_direct_invocation(self):
        self.assertDetects("Observing running behavior means invoking the system directly", self.core)
    ...
    def test_test_suite_excluded(self):
        self.assertDetects("a test suite run does not satisfy this requirement", self.core)
    ...
    def test_ground_complete_only_from_loop(self):
        self.assertDetects("\u2705 Ground complete may only be emitted as the outcome of the observation loop", self.core)

    def test_emitting_outside_loop_is_violation(self):
        self.assertDetects("emitting it outside the loop is a protocol violation", self.core)
    ...
    def test_write_auth_type_crosscheck(self):
        self.assertDetects("cited artifact type must match", self.core)

    # --- ADR-0230/0231: upward return ---

    def test_upward_return_trigger_from_above(self):
        self.assertDetects("trigger for an upward return must originate at the rung being revised or above", self.core)

    def test_lower_rung_pressure_prohibited(self):
        self.assertDetects("difficulty, failure, or constraint pressure from any lower rung is not a valid trigger", self.core)

    def test_observation_loop_return(self):
        self.assertDetects("Returning to the session observation loop is valid", self.core)
