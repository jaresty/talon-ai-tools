...
class TestThread1_P21UpwardReturn(GroundADRTestBase):
    ...
    def test_p21_permits_upward_return_on_derivation_error(self):
        self.assertDetects("derivation error", self.core)
    ...
    def test_p21_lower_rung_hard_to_produce_voids(self):
        self.assertDetects("returning upward because a lower rung", self.core)

    def test_p21_revised_rung_re_derived_from_input(self):
        self.assertDetects("re-derived from its input rung", self.core)

    def test_p21_descent_resumes_from_revised_rung(self):
        self.assertDetects("descent resumes downward", self.core)

    def test_p21_propagated_to_prompt(self):
        self.assertDetects("P19 (Upward return)", build_ground_prompt())
