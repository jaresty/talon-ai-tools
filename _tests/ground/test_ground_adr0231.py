...
class TestThread1_P21ObservationLoopReturn(GroundADRTestBase):
    ...
    def test_trigger_must_be_tool_executed_observation(self):
        self.assertDetects("tool-executed observation reveals the declared gap is incorrect", self.core)
    ...
    def test_new_observation_required_before_new_gap(self):
        self.assertDetects("new observation must be made before any new gap may be declared", self.core)

    def test_propagated_to_prompt(self):
        self.assertDetects("Returning to the session observation loop is valid", build_ground_prompt())
