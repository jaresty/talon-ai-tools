...
class TestMinimalGroundParts(unittest.TestCase):
    ...
    def test_total_chars_under_3000(self):
        ...
        self.assertLess(
            total,
            31500,
            f"GROUND_PARTS_MINIMAL total {total} chars; expected < 31500 (raised after ADR-0203 escape-route closures + vitest browser mode: +5 additions)",
        )
    ...
    def test_all_rung_names_present_in_order(self):
        ...
        for name in CANONICAL_RUNG_NAMES:
            ...
            self.assertGreater(
                new_pos, pos, f"Rung name {name!r} missing or out of order"
            )
            pos = new_pos
    ...
    def test_all_sentinel_format_strings_present(self):
        for key, value in SENTINEL_TEMPLATES.items():
            self.assertIn(
                value, self.prompt, f"Sentinel {key!r} missing from minimal prompt"
            )
    ...
    def test_vro_recurring_gap_returns_to_formal_notation(self):
        self.assertIn(
            "observed gap matches the prior cycle's gap",
            self.prompt,
    ...
    def test_vro_rung_stop_after_first_failure(self):
        self.assertIn(
            "do not enumerate multiple failures",
            self.prompt,
    ...
    def test_ev_rung_requires_prior_red_run(self):
        self.assertIn(
            "if it passes, the artifact is vacuous",
            self.prompt,
    ...
    def test_gap_names_behavior_not_artifact_absence(self):
        self.assertIn(
            "need to implement a landing page",
            self.prompt,
    ...
    def test_criterion_may_not_use_conjunction(self):
        self.assertIn(
            "if the criterion contains the word",
            self.prompt,
    ...
    def test_ev_green_first_run_return_to_gap(self):
        self.assertIn(
            "if the artifact asserts runtime behavior, perturb the implementation",
            self.prompt,
    ...
    def test_obs_rung_required_every_cycle(self):
        self.assertIn(
            "including the observed running behavior rung",
            self.prompt,
    ...
    def test_manifest_exhausted_when_no_gap_remains(self):
        self.assertIn(
            "if no currently-false behavior remains, emit",
            self.prompt,
    ...
    def test_final_report_includes_prose_criteria_notation(self):
        self.assertIn(
            "prose, criteria, and formal notation for each thread",
            self.prompt,
    ...
    def test_sentinel_placement_at_defining_rung_only(self):
        self.assertIn(
            "valid only at its defining rung",
            self.prompt,
    ...
    def test_thread_complete_references_prose_rung(self):
        ...
        self.assertIn(
            "prose", segment, "Manifest declaration scan must reference the prose"
        )
    ...
    def test_c10_reread_formal_notation_before_writing_test(self):
        ev_idx = self.prompt.index(
            "each test function asserts exactly one behavioral property"
        )
        v_complete_idx = self.prompt.index(
            "\u2705 Validation artifact V complete must be emitted"
        )
    ...
    def test_c11_one_assertion_per_test_function(self):
        self.assertIn(
            "one behavioral property per test function",
            self.prompt,
    ...
    def test_c13_harness_error_is_not_red_run(self):
        self.assertIn(
            "harness error (import failure, syntax error, missing file) is not a red run",
            self.prompt,
    ...
    def test_c13_test_logic_must_have_run_and_failed(self):
        self.assertIn(
            "its assertions must have run, and they must have failed",
            self.prompt,
    ...
    def test_c14_behavioral_predicate_coverage_check(self):
        ...
        thread_end = self.prompt.index(
            "Outputting a rung label is what begins that rung"
        )
