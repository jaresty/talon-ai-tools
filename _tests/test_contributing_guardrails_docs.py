    ...
    class ContributingGuardrailsDocsTests(unittest.TestCase):
        def test_contributing_mentions_guardrail_targets(self) -> None:
            ...
            self.assertIn(
                "not tracked",
                text,
                "Expected CONTRIBUTING to state axis/static prompt Talon lists are untracked",
            )
            self.assertIn("request-history-guardrails", text)
            self.assertIn("request-history-guardrails-fast", text)
            self.assertIn("manual telemetry", text)
