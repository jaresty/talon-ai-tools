...
### ~~Grammar: Eliminate duplicate definition storage in axisConfig.py~~ ✓ done 2026-03-18
`AXIS_KEY_TO_VALUE` is the SSOT. The generator now strips `definition` from the stored
`AXIS_TOKEN_METADATA` body; `axis_token_metadata()` derives it at runtime from `AXIS_KEY_TO_VALUE`.
Schema conformance tests updated to use the accessor. Gap check added to `test_axis_config_generator.py`.
