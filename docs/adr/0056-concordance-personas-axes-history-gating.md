...
2. **Persona & Intent Presets**
   ...
   - (Completed 2025-12-19) Refactored `_validated_persona_value`, `_canonical_persona_value`, and `_build_persona_intent_docs` to rely on catalog-backed `persona_intent_maps`, keeping persona/intents aligned with axis metadata.
    ...
    - (Completed 2025-12-22) Added a Talon guardrail command (`user.model_export_telemetry`) that snapshots suggestion skip telemetry alongside history gating so CLI artefacts stay accurate.
    - (Completed 2025-12-22) Routed the Talon exporter to the shared `artifacts/telemetry/` directory and updated Make/CI guardrails (scripts, tests, CI workflow) to read from that location. The Talon command runs with its baked-in defaults; CLI helpers accept optional flags for offline testing only.
    - (Completed 2025-12-22) Updated CLI guardrail scripts to consume the Talon-generated JSON snapshots (instead of querying live state) via `--summarize-json` / `--counts-json` so they operate solely on the exported files.
...
3. **Request Gating & Streaming Lifecycle**
    ...
    - (Completed 2025-12-22) Added a Talon guardrail command that snapshots history/gating and suggestion-skip telemetry into `artifacts/telemetry/` before CLI helpers run so the exported JSON reflects live Talon state.
    - (Completed 2025-12-22) Added a CLI freshness check (`scripts/tools/check-telemetry-export-marker.py`) that request-history guardrail targets invoke before running, failing fast when Talon telemetry hasn’t been exported recently.
    - (Completed 2025-12-23) Added `scripts/tools/scheduler_source.py` and wired guardrail CLI/Make targets to label the scheduler telemetry source so I can see whether stats come from the telemetry export, summary, marker, or defaults.
    ...
    - (Completed 2025-12-23) Guardrail CLI now tracks consecutive `(missing)` / `(non-default)` telemetry marker warnings, persists streak metadata to `artifacts/telemetry/cli-warning-streak.json`, and prints the streak alongside the last guardrail command once the escalation threshold is reached.
      - Threshold: trigger the prompt after two consecutive warnings (matching the manual escalation guidance) and reset the counter once telemetry is refreshed successfully.
      - Reporting: emit the streak count, reason, and last guardrail command in CLI output so I can copy the details into `docs/adr/0056-concordance-personas-axes-history-gating.work-log.md` without re-running.
      - Persistence: streak JSON entries store `streak`, `last_reason`, `last_command`, and `updated_at`, giving auditors a machine-readable trail for repeated defaults warnings.
    - (Completed 2025-12-23) `scripts/tools/run_guardrails_ci.sh` now reads `artifacts/telemetry/cli-warning-streak.json`, prints the telemetry export streak lines in CLI output, and appends the same summary block to GitHub step summaries so CI logs retain escalation context.
    - (Completed 2025-12-23) The guardrail runner highlights non-zero telemetry export streaks with a `Streak alert` line in both CLI output and GitHub step summaries so consecutive warnings stand out immediately.
    - (Completed 2025-12-23) GitHub guardrail summaries now include a `Telemetry export streak` row so non-zero streaks surface alongside gating totals without scanning the narrative.
...
- Optional guardrail: from within Talon, run `user.model_export_telemetry` (or the `model export telemetry` voice command) to snapshot telemetry into `artifacts/telemetry/` before invoking CLI guardrails; subsequent CLI helpers consume the exported JSON (via `--summarize-json` / `--counts-json`) rather than querying live state. The guardrail targets now call `scripts/tools/check-telemetry-export-marker.py` to enforce freshness, so workstations running guardrails without Talon must set `ALLOW_STALE_TELEMETRY=1` explicitly.
- CLI guardrail tip: when the export marker is missing or stale, `check-telemetry-export-marker.py` now prints a `TIP:` reminder telling me to run `model export telemetry` before continuing, and supports `--wait` / `--wait-seconds` to pause until telemetry is fresh.
- Defaults escalation: Guardrail CLI/Make targets must auto-trigger `model export telemetry` after two consecutive `(missing)` or `(non-default)` scheduler sources so Concordance checks reuse fresh telemetry without manual intervention. The helper logs the auto-invocation (command, timestamp, source) and respects `ALLOW_AUTO_TELEMETRY=0` when I need to disable the behaviour for dry runs.
- Streak logging: `check-telemetry-export-marker.py` now persists streak metadata to `artifacts/telemetry/cli-warning-streak.json` and prints `Telemetry export warning streak:` plus the last guardrail command once the escalation threshold is crossed. Copy those lines into the work log entry and reset the escalation trail after the export succeeds; when the auto-trigger runs, the helper records the invocation alongside the streak reset.
- Guardrail runner output: `scripts/tools/run_guardrails_ci.sh` emits the telemetry export streak state in both its CLI output and GitHub step summaries, adds a `Streak alert` line whenever the streak is non-zero, and writes a `Telemetry export streak` table row so CI transcripts expose consecutive warnings alongside gating totals. Telemetry guardrail metadata now uses `history-telemetry-inspect.py --format summary`, so CLI/Make runs show a single `summary: guardrail_target=…` line alongside streaming status and last-drop details, plus the default `summary_path`, `total_entries`, and `streaming_status` fields without extra `--field` flags.
- CLI streak counter behaviour: The helper still prompts and records streak context, but it now auto-invokes Talon exports after the configured threshold while honouring `ALLOW_AUTO_TELEMETRY=0` for manual-only sessions.
- Talon cadence updates: changing the `guardrail_telemetry_export_interval_minutes` setting now reschedules the background exporter immediately, so I can tighten or loosen the cadence without restarting Talon.
- Scheduler telemetry: each export now records scheduler reschedule stats in the telemetry payload and marker, so CLI guardrails can surface cadence drift and last reschedule reasons. Guardrail CLI output now prints the reschedule count, last interval, reason, timestamp, and the data source (telemetry, summary, marker, or defaults), highlighting non-default sources with a `(non-default)` suffix and stale readings with `(stale)`; job summaries embed both the JSON snapshot and the bullet summary.
...
- Guardrail simulation: set `HISTORY_AXIS_VALIDATE_SIMULATE_GATING_DROP=__DEFAULT__` (or assign a custom message string) before running `python3 scripts/tools/history-axis-validate.py` in CI to synthesize a gating drop; leave the variable unset during production guardrail runs so real telemetry remains authoritative.
- Telemetry inspection: guardrail targets now print the telemetry metadata automatically; run `python3 scripts/tools/history-telemetry-inspect.py artifacts/telemetry/history-validation-summary.telemetry.json --format summary` (or pass another telemetry path) when you want the same concise guardrail target, drop totals, and last-drop messages during manual comparisons.
