...
3. **Request Gating & Streaming Lifecycle**
    ...
    - (Planned 2025-12-23) Instrument guardrail CLI output to track consecutive `(missing)` / `(non-default)` scheduler warnings and surface a prompt when the streak exceeds the manual escalation threshold; backlog ties to Loop 361.
      ...
      - Reporting: emit the streak count and last guardrail command in CLI output so operators can copy the details into `docs/adr/0056-concordance-personas-axes-history-gating.work-log.md` without re-running.
...
- Manual tracking: Until CLI instrumentation lands, record each `(missing)` or `(non-default)` warning with its streak length inside `docs/adr/0056-concordance-personas-axes-history-gating.work-log.md` so the escalation trail stays auditable.
- CLI streak counter design: When the counter lands, print the streak count alongside the existing `TIP:` guidance without auto-invoking Talon commands, and note the threshold crossing so operators can log the event verbatim.
