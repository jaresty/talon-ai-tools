...
- Defaults escalation: After reviewing automation options, we will not auto-trigger `model export telemetry` from the CLI when defaults warnings repeat; doing so could emit Talon voice commands without user context. Instead, treat two consecutive `(missing)` or `(non-default)` scheduler sources as a manual escalation: invoke `model export telemetry` (or `--wait` / `--wait-seconds`) before retrying, and capture the incident in the work log.
- Streak logging: `check-telemetry-export-marker.py` now persists streak metadata to `artifacts/telemetry/cli-warning-streak.json` and prints `Telemetry export warning streak:` plus the last guardrail command once the escalation threshold is crossed. Copy those lines into the work log entry and reset the escalation trail after the export succeeds.
- Guardrail runner output: `scripts/tools/run_guardrails_ci.sh` emits the telemetry export streak state in both its CLI output and GitHub step summaries, adds a `Streak alert` line whenever the streak is non-zero, and writes a `Telemetry export streak` table row so CI transcripts expose consecutive warnings alongside gating totals.
- CLI streak counter behaviour: The helper never auto-invokes Talon commands; it only prints streak and command context, keeping me in control while still delivering auditable output.
