...
- Guardrail runner output: `scripts/tools/run_guardrails_ci.sh` emits the telemetry export streak state in both its CLI output and GitHub step summaries, adds a `Streak alert` line whenever the streak is non-zero, and writes a `Telemetry export streak` table row so CI transcripts expose consecutive warnings alongside gating totals. When the CLI auto-trigger fires, the summary highlights the refreshed export and the guardrail command that issued it.
...
- Telemetry inspection: guardrail targets now print the telemetry metadata automatically; run `python3 scripts/tools/history-telemetry-inspect.py artifacts/telemetry/history-validation-summary.telemetry.json` (or pass another telemetry path) when you want the same guardrail target, drop totals, and last-drop messages during manual comparisons.
