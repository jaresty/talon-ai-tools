...
- Telemetry inspection: run `python3 scripts/tools/history-telemetry-inspect.py artifacts/telemetry/history-validation-summary.telemetry.json` (or pass another telemetry path) to print the guardrail target, drop totals, and last-drop messages when comparing exports manually.
