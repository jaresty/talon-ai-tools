...
- Guardrail runner output: `scripts/tools/run_guardrails_ci.sh` emits the telemetry export streak state in both its CLI output and GitHub step summaries and adds a `Streak alert` line whenever the streak is non-zero so CI transcripts expose consecutive warnings without scanning the detailed block.
