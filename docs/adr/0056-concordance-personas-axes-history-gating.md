...
- Guardrail runner output: `scripts/tools/run_guardrails_ci.sh` emits the telemetry export streak state in both its CLI output and GitHub step summaries so CI transcripts include the escalation count, reason, last command, and updated timestamp.
