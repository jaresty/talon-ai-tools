...
- Optional guardrail: from within Talon, run `user.model_export_telemetry` (or the `model export telemetry` voice command) to snapshot telemetry into `artifacts/telemetry/` before invoking CLI guardrails; subsequent CLI helpers consume the exported JSON (via `--summarize-json` / `--counts-json`) rather than querying live state. The guardrail targets now call `scripts/tools/check-telemetry-export-marker.py` to enforce freshness, and when `ALLOW_STALE_TELEMETRY=1` is set for offline shells the helper now logs a warning reminding you to refresh telemetry and clear the override promptly. The CI runner now echoes that warning to stdout and adds a `Telemetry Export Check` section to GitHub summaries so bypassed freshness checks stay visible to reviewers.
