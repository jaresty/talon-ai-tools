...
- Optional guardrail: from within Talon, run `user.history_export_telemetry` (or the `history export telemetry` voice command) to snapshot telemetry into `artifacts/telemetry/` before invoking CLI guardrails; subsequent CLI helpers consume the exported JSON (via `--summarize-json` / `--counts-json`) rather than querying live state. Consider wiring this command into recurring guardrail runbooks so Concordance snapshots aren’t forgotten between loops.
