...
3. **Request Gating & Streaming Lifecycle**
    ...
    - (Completed 2025-12-23) Added `scripts/tools/scheduler_source.py` and wired guardrail CLI/Make targets to label the scheduler telemetry source so operators can see whether stats come from the telemetry export, summary, marker, or defaults.
    ...
    - (Completed 2025-12-23) Guardrail CLI now tracks consecutive `(missing)` / `(non-default)` telemetry marker warnings, persists streak metadata to `artifacts/telemetry/cli-warning-streak.json`, and prints the streak alongside the last guardrail command once the escalation threshold is reached.
      ...
      - Reporting: emit the streak count, reason, and last guardrail command in CLI output so operators can copy the details into `docs/adr/0056-concordance-personas-axes-history-gating.work-log.md` without re-running.
...
- CLI guardrail tip: when the export marker is missing or stale, `check-telemetry-export-marker.py` now prints a `TIP:` reminder telling operators to run `model export telemetry` before continuing, and supports `--wait` / `--wait-seconds` to pause until telemetry is fresh.
...
- CLI streak counter behaviour: The helper never auto-invokes Talon commands; it only prints streak and command context, keeping operators in control while still delivering auditable output.
- Talon cadence updates: changing the `guardrail_telemetry_export_interval_minutes` setting now reschedules the background exporter immediately, so operators can tighten or loosen the cadence without restarting Talon.
