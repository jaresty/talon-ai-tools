...
- CLI guardrail tip: when the export marker is missing or stale, `check-telemetry-export-marker.py` now prints a `TIP:` reminder telling operators to run `model export telemetry` before continuing.
...
- Scheduler telemetry: each export now records scheduler reschedule stats in the telemetry payload and marker, so CLI guardrails can surface cadence drift and last reschedule reasons.
