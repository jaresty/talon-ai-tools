...
The GPT integration exposes a rich, orthogonal prompt grammar (static prompts, directional modifiers, completeness/method/scope/style axes, and system-level defaults). This enables very fine-grained control but is hard to remember and discover in day-to-day use, especially in a voice-driven workflow.
...
2. **Recipe recap in the confirmation GUI for passive reinforcement**
   ...
   - The recap is derived from the same resolved axis values applied to `GPTState.system_prompt` (static prompt plus effective completeness/scope/method/style).
   - This line appears consistently but unobtrusively, reinforcing axis names and values every time the model is used.
   - The underlying axis semantics (from the Talon lists) are expressed both in the system prompt contract and as explicit `Constraints:` lines in the user prompt; this duplication is intentional to make the grammar visible in logs and confirmation views without relying solely on system metadata.
...
3. **Global “what can I say?” help modal for exploration**
   - Add a separate help GUI (for example, `modelHelpGUI`) opened with `model help` that:
     - Briefly explains each axis (goal, directional lens, completeness, scope, method, style).
...
- Quick help GUI variants are available:
  ...
  - `model quick help completeness` / `scope` / `method` / `style` open focused views on individual axes.
...
- When you’re exploring or teaching yourself the grammar:
  ...
  - Use `model quick help completeness` / `scope` / `method` / `style` to drill into a single axis.
