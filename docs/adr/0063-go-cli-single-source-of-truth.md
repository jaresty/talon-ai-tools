...
- **Live provider transport staging**: introduce a provider client interface with fixture-backed default, enforce feature-flag gating for network usage, and require deterministic recordings with schema tagging, automated redaction, and freshness checks before parity adopts live data. Dual-path evidence (fixture + live) and telemetry/SLO enforcement land before the flag flips for Talon operators, `BAR_PROVIDER_COMMAND_MODE` keeps shell-outs disabled when parity or fixtures need hermetic behaviour, fixtures-only mode fails fast with a structured error when recordings are missing, and `BAR_PROVIDER_HTTP_ENDPOINT` enables hermetic HTTP transports that reuse recorded transcripts when the endpoint is unavailable while annotating responses with fallback metadata; bearer-token headers, retry budgets, and timeout controls remain environment-configurable so Talon can delegate to credentialled providers without bespoke shell scripts, and HTTP telemetry (`http_status`, `http_latency_ms`, fallback reason/attempts) flows back into Talon parity fixtures.
