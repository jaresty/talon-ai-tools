...
- Maintain the shared payload helper (`_parse_bar_cli_payload`) and its `BarCliPayload` dataclass as the canonical decoder for CLI stdout; future telemetry fields must extend this helper so adapters and tests stay aligned.
...
- **Payload helper parity**: keep `_parse_bar_cli_payload` and the `BarCliPayload` dataclass under unit tests covering success and failure cases; extend the helper before introducing new telemetry fields.
