...
- **Log truncation**: cap CLI stdout/stderr and debug log values at 512 characters with an `...(truncated)` suffix and attach indicator metadata (for example, `(original length 2048 chars)`); guardrail tests cover error, breadcrumb, stdout, stderr, and payload logging so runaway payloads cannot overwhelm Talon consoles. Configure the cap via the Talon setting `user.bar_cli_debug_log_limit` because Talon cannot read environment variables.
- **Residual monitoring**: record `user.bar_cli_debug_log_limit` usage via settings telemetry, alert when values exceed 4096, and flag when the override deviates from defaults in support bundles so operators can correlate truncation anomalies during incident response.
- **Telemetry note**: emit a structured telemetry event when truncation occurs, including the post-truncation length, indicator metadata, and current `user.bar_cli_debug_log_limit` value so dashboards can surface chronic truncation scenarios. Events populate the `truncation_events` array with objects shaped as `{ "field": str, "action": str, "limit": int, "timestamp": iso8601, "original_length": int, "truncated_length": int }` for string payloads and `{ "field": str, "action": str, "limit": int, "timestamp": iso8601, "truncated_items": int, "max_original_length": int }` for list payloads.
- **Troubleshooting**: when truncation telemetry stops flowing, export snapshots via `python3 -m talon_user.lib.telemetryExport` and inspect `history-validation-summary.telemetry.json` plus `truncation_events`; if empty, ensure provider truncation hooks still call `record_truncation_event` and that dashboards ingest the artefacts.
