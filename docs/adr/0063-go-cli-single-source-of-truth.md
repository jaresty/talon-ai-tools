...
- **Live provider transport staging**: introduce a provider client interface with fixture-backed default, enforce feature-flag gating for network usage, and require deterministic recordings with schema tagging, automated redaction, and freshness checks before parity adopts live data. Dual-path evidence (fixture + live) and telemetry/SLO enforcement land before the flag flips for Talon operators, and `BAR_PROVIDER_COMMAND_MODE` keeps shell-outs disabled when parity or fixtures need hermetic behaviour.
