...
- **Telemetry note**: emit a structured telemetry event when truncation occurs, including the post-truncation length, indicator metadata, and current `user.bar_cli_debug_log_limit` value so dashboards can surface chronic truncation scenarios.
