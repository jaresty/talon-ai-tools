...
- Initial loops (001-004) highlighted gaps; follow-up audits (Loops 005-006) added shared hydration helper and passive guard semantics as explicit deliverables, both implemented in Loops 007-008 before acceptance.
...
- **Deliver a shared hydration helper.** Ship a single helper (e.g., `hydrate_intent_token`) that all surfaces call before rendering labels so quick-help, prompt session, Help Hub, and suggestion UIs stay consistent. (Loop 007.)
...
## Implementation Plan
1. Add the shared `hydrate_intent_token` helper and wire it into the suggest pipeline, prompt session, Help Hub, and quick-help rendering so every surface hydrates canonical tokens the same way. ✅ (Loop 007)
2. Update guard utilities to expose backwards-compatible attributes (or provide documented replacements), keep `close_common_overlays(..., passive=True)` side-effect free, and adjust tests to tolerate the new kwargs while still verifying suppression cleanup. ✅ (Loop 008)
3. Tweak quick-help drawing to rely on the helper for alias presentation and keep canonical spoken shortcuts first in the rendered list. ✅ (Loops 002 & 007)
4. Re-run the failing test modules, then the full suite, to confirm coverage remains green. ✅ (Loops 004, 007, 008)
