...
## 2026-01-10 — loop 062
- helper_version: helper:v20251223.1
- focus: Decision § subject import/export — keep palette filter status surfacing copy command hint after typing
- active_constraint: Palette filter status guidance could drop the copy command hint once operators typed a filter, leaving the ADR’s single-command workflow without discoverability; go test ./internal/bartui lacked an assertion to catch the regression.
- validation_targets:
  - go test ./internal/bartui
  - go test ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
- evidence:
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-062.md#loop-062-green--go-test-internal-bartui
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-062.md#loop-062-green--go-test-cmd-bar
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-062.md#loop-062-green--python3--m-pytest-_tests-test_bar_completion_cli.py
- rollback_plan: `git restore --source=HEAD -- internal/bartui/program_test.go docs/adr/0070-bubble-tea-prompt-editor-tui.work-log.md docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-062.md`
- delta_summary: helper:diff-snapshot=git diff --stat | internal/bartui/program_test.go (+copy-hint assertion for palette filter status)
- loops_remaining_forecast: 0 loops — palette status copy hint now guarded; continue monitoring pilot feedback for additional UX adjustments.
- residual_constraints:
...
## 2026-01-10 — loop 063
- helper_version: helper:v20251223.1
- focus: Decision § subject import/export — keep filtered palette status surfacing copy command hint and filter text
- active_constraint: After typing into the palette filter, the status reverted to the generic prompt and dropped the copy-command hint required by the ADR, leaving pilots without discoverability for the CLI fallback; go test ./internal/bartui lacked coverage for this regression.
- validation_targets:
  - go test ./internal/bartui
  - go test ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
- evidence:
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-063.md#loop-063-green--go-test-internal-bartui
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-063.md#loop-063-green--go-test-cmd-bar
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-063.md#loop-063-green--python3--m-pytest-_tests-test_bar_completion_cli.py
- rollback_plan: `git restore --source=HEAD -- internal/bartui/program.go internal/bartui/program_test.go docs/adr/0070-bubble-tea-prompt-editor-tui.work-log.md docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-063.md`
- delta_summary: helper:diff-snapshot=git diff --stat | internal/bartui/program.go (+copy-hint filter status update), internal/bartui/program_test.go (+applyKey helper covering filtered hint)
- loops_remaining_forecast: 0 loops — palette status now preserves copy command guidance under filtering; continue monitoring pilot feedback for new constraints.
- residual_constraints:
  - Pilot feedback on the new token controls remains pending (severity: medium; mitigation: gather pilot notes after rollout; monitoring: rerun go test ./internal/bartui and python3 -m pytest _tests/test_bar_completion_cli.py if adjustments are required).
- next_work:
  - Behaviour: Monitor pilot telemetry for token editing and palette hints; rerun go test ./internal/bartui, go test ./cmd/bar/..., and python3 -m pytest _tests/test_bar_completion_cli.py if follow-up changes are needed.
...
## 2026-01-10 — loop 064
- helper_version: helper:v20251223.1
- focus: Decision § subject import/export — keep copy command hint visible after clearing palette filter
- active_constraint: Clearing the token palette filter with Ctrl+W replaced the status message with “Token filter cleared.” and removed the required copy-command hint, reducing discoverability of the CLI fallback demanded by the ADR; go test ./internal/bartui lacked coverage to prevent the regression.
- validation_targets:
  - go test ./internal/bartui
  - go test ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
- evidence:
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-064.md#loop-064-green--go-test-internal-bartui
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-064.md#loop-064-green--go-test-cmd-bar
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-064.md#loop-064-green--python3--m-pytest-_tests-test_bar_completion_cli.py
- rollback_plan: `git restore --source=HEAD -- internal/bartui/program.go internal/bartui/program_test.go docs/adr/0070-bubble-tea-prompt-editor-tui.work-log.md docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-064.md`
- delta_summary: helper:diff-snapshot=git diff --stat | internal/bartui/program.go (+copy-hint status when clearing filter), internal/bartui/program_test.go (+coverage for cleared/empty filter hints)
- loops_remaining_forecast: 0 loops — palette copy hint now persists after filter clears; continue monitoring pilot feedback for additional UX adjustments.
- residual_constraints:
  - Pilot feedback on the new token controls remains pending (severity: medium; mitigation: gather pilot notes after rollout; monitoring: rerun go test ./internal/bartui and python3 -m pytest _tests/test_bar_completion_cli.py if adjustments are required).
- next_work:
  - Behaviour: Monitor pilot telemetry for token editing and palette hints; rerun go test ./internal/bartui, go test ./cmd/bar/..., and python3 -m pytest _tests/test_bar_completion_cli.py if follow-up changes are needed.
...
## 2026-01-10 — loop 065
- helper_version: helper:v20251223.1
- focus: Decision § subject import/export — ensure palette no-match status retains copy command hint
- active_constraint: When the palette filter produced no matching options and focus moved to the options pane, the status message omitted the copy-command hint mandated by the ADR, leaving pilots without visible guidance on the CLI fallback; go test ./internal/bartui lacked a regression test for this scenario.
- validation_targets:
  - go test ./internal/bartui
  - go test ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
- evidence:
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-065.md#loop-065-green--go-test-internal-bartui
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-065.md#loop-065-green--go-test-cmd-bar
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-065.md#loop-065-green--python3--m-pytest-_tests-test_bar_completion_cli.py
- rollback_plan: `git restore --source=HEAD -- internal/bartui/program.go internal/bartui/program_test.go docs/adr/0070-bubble-tea-prompt-editor-tui.work-log.md docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-065.md`
- delta_summary: helper:diff-snapshot=git diff --stat | internal/bartui/program.go (+copy-hint helper ensuring options status retains guidance), internal/bartui/program_test.go (+no-match options test covering copy hint)
- loops_remaining_forecast: 0 loops — palette copy hint now persists even when no options match; continue monitoring for new pilot feedback.
- residual_constraints:
  - Pilot feedback on the new token controls remains pending (severity: medium; mitigation: gather pilot notes after rollout; monitoring: rerun go test ./internal/bartui and python3 -m pytest _tests/test_bar_completion_cli.py if adjustments are required).
- next_work:
  - Behaviour: Monitor pilot telemetry for token editing and palette hints; rerun go test ./internal/bartui, go test ./cmd/bar/..., and python3 -m pytest _tests/test_bar_completion_cli.py if follow-up changes are needed.
...
## 2026-01-10 — loop 066
- helper_version: helper:v20251223.1
- focus: Decision § subject import/export — keep copy command hint visible when palette focuses categories
- active_constraint: Tab-switching to the palette’s category column replaced the status with navigation guidance lacking the copy-command hint required by the ADR, leaving the CLI fallback undiscoverable after filtering; go test ./internal/bartui did not protect this path.
- validation_targets:
  - go test ./internal/bartui
  - go test ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
...
## 2026-01-10 — loop 069
- helper_version: helper:v20251223.1
- focus: Decision § subject import/export — keep copy command hint visible while palette commands run and finish
- active_constraint: Starting palette commands replaced the status with “Running …” messages that dropped the copy-command hint, hiding the ADR-required CLI fallback during long-running operations; go test ./internal/bartui lacked regression coverage for this path.
- validation_targets:
  - go test ./internal/bartui
  - go test ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
- evidence:
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-069.md#loop-069-green--go-test-internal-bartui
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-069.md#loop-069-green--go-test-cmd-bar
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-069.md#loop-069-green--python3--m-pytest-_tests/test_bar_completion_cli.py
- rollback_plan: `git restore --source=HEAD -- internal/bartui/program.go internal/bartui/program_test.go docs/adr/0070-bubble-tea-prompt-editor-tui.work-log.md docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-069.md`
- delta_summary: helper:diff-snapshot=git diff --stat | internal/bartui/program.go (+ensure copy hint persists during command runs and completion), internal/bartui/program_test.go (+command status copy hint tests)
- loops_remaining_forecast: 0 loops — copy guidance now persists through command execution; continue monitoring pilot feedback for additional UX adjustments.
- residual_constraints:
  - Pilot feedback on the new token controls remains pending (severity: medium; mitigation: gather pilot notes after rollout; monitoring: rerun go test ./internal/bartui and python3 -m pytest _tests/test_bar_completion_cli.py if adjustments are required).
- next_work:
  - Behaviour: Monitor pilot telemetry for token editing and palette hints; rerun go test ./internal/bartui, go test ./cmd/bar/..., and python3 -m pytest _tests/test_bar_completion_cli.py if follow-up changes are needed.
...
## 2026-01-12 — loop 084
- helper_version: helper:v20251223.1
- focus: Salient Tasks — subject import/export command piping ensures subprocess workflows stay usable
- active_constraint: Subject-mode command execution skipped piping the current subject into subprocess stdin, breaking ADR 0070 Decision § subject import/export by preventing command outputs from reflecting the authored subject during reinsertion flows.
- validation_targets:
  - go test -count=1 ./internal/bartui
  - go test -count=1 ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
- evidence:
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-084.md#loop-084-green--helper-diff-snapshot-git-diff--stat-head
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-084.md#loop-084-green--go-test--count1--internal-bartui
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-084.md#loop-084-green--go-test--count1--cmd-bar
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-084.md#loop-084-green--python3--m-pytest-_tests-test_bar_completion_cli.py
- rollback_plan: `git restore --source=HEAD -- internal/bartui/program.go internal/bartui/program_test.go docs/adr/0070-bubble-tea-prompt-editor-tui.work-log.md docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-084.md`
- delta_summary: helper:diff-snapshot=git diff --stat | internal/bartui/program.go (subject stdin piping) · internal/bartui/program_test.go (command stdin regression coverage)
- loops_remaining_forecast: 1 loop (revisit condensed status messaging) — medium confidence while outstanding UX messaging constraints remain.
- residual_constraints:
...
- active_constraint: Operators running Ctrl+Y after a stdout-free command (for example `cat` without preview piping) still hit "Last command stdout was empty", leaving the ADR’s reinsertion shortcut unusable during preview copy workflows.
- validation_targets:
  - go test -count=1 ./internal/bartui
  - go test -count=1 ./cmd/bar/...
  - python3 -m pytest _tests/test_bar_completion_cli.py
- evidence:
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-086.md#loop-086-green--helper-diff-snapshot-git-diff--stat-head
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-086.md#loop-086-green--go-test--count1--internal-bartui
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-086.md#loop-086-green--go-test--count1--cmd-bar
  - green: docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-086.md#loop-086-green--python3--m-pytest-_tests-test_bar_completion_cli.py
- rollback_plan: `git restore --source=HEAD -- internal/bartui/program.go internal/bartui/program_test.go docs/adr/0070-bubble-tea-prompt-editor-tui.work-log.md docs/adr/evidence/0070-bubble-tea-prompt-editor-tui/loop-086.md`
- delta_summary: helper:diff-snapshot=git diff --stat | internal/bartui/program.go (Ctrl+Y falls back to preview text when stdout is empty) · internal/bartui/program_test.go (preview fallback + undo regression coverage)
- loops_remaining_forecast: 1 loop (status message length trim) — medium confidence while UX messaging work remains.
- residual_constraints:
  - Status line still exceeds comfortable length when commands/palette emit long guidance (severity: medium; mitigation: explore contextual status truncation; monitoring trigger: pilot feedback that status messages wrap or obscure viewport content).
  - Help overlay remains the sole location for full shortcut documentation (severity: low; mitigation: leave as-is but monitor onboarding feedback).
