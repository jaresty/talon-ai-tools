...
- Add focus breadcrumbs, toast-style transient feedback, and a single-step undo stack tailored to token changes so pilots can experiment quickly without losing context. Favor simple Lip Gloss styling unless a Charm “skill” directly reinforces the learning objective.
- Surface token telemetry beneath the grammar composer with a compact sparkline so operators can see recent token counts without leaving the compose flow.
