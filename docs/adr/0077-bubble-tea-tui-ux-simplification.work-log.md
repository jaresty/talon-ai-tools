...
focus: ADR 0077 Decision bullet 5 → highlight the active history row alongside the existing metadata so operators can identify the latest action without scanning every entry.

active_constraint: History entries rendered as uniform text even after metadata landed, forcing pilots to read each line to learn which action just fired (validation: `scripts/tools/run-tui-expect.sh --all`).
...
| Probability | High | Updating the bartui renderer and tests directly addresses the missing styling. |
...
rollback_plan: `<VCS_REVERT>` = `git restore --source=HEAD -- internal/bartui/program.go internal/bartui/program_test.go`; rerun `scripts/tools/run-tui-expect.sh --all` to confirm the history list reverts to the pre-highlight layout before reapplying the loop diff.

delta_summary: helper:diff-snapshot=2 files changed, 115 insertions(+), 2 deletions(-) — adds a Lip Gloss highlight style, applies it to the latest history row, and covers the behaviour with dedicated unit tests.

loops_remaining_forecast: 0 loops. Confidence: high — history typography now meets ADR 0077’s guidance; only monitoring remains.

residual_constraints:
- Low — Continue monitoring new history event kinds for highlight coverage. Mitigation: rerun `scripts/tools/run-tui-expect.sh --all` when additional history behaviours land; owner: ADR 0077 follow-up.
