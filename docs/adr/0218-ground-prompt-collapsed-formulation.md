...
14. **Evidential authority**: Only tool-executed events have standing; evidence scoped to the cycle and gap in which it was produced; a prior cycle, different thread, or different gap does not satisfy any gate. The impl_gate requires at least one assertion-level failure string in verbatim exec_observed output — infrastructure failure, import error, process crash, or model-described summary does not satisfy this gate.
