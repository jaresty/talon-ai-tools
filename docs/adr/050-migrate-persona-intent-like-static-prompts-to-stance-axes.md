...
- Existing Persona/Intent axes (ADR 015/040/042): voice, audience, tone, purpose, persona presets, intent presets. These represent **who is speaking** and **why they are acting**.
...
## Definitions
- **Persona/Intent (Who/Why)**: voice/audience/tone/purpose tokens and persona/intent presets that shape stance, empathy, and rhetorical posture.
...
- Form/channel tokens with stance flavour:
  - `announce` (rhetorical tone + audience); consider tone/purpose + destination instead.
...
- **Re-home stance-like static prompts into Persona/Intent axes or presets**, removing them from `staticPrompt`:
  - `jobs`, `value`, `pain`, `team`, `question`, `facilitate`, `done` -> add as `purpose`/intent tokens or persona presets where appropriate; drop from static prompt catalog.
...
- **Revisit form/channel stance tokens**:
  - Move `announce` to tone/purpose + explicit channel destination; keep channel axis for pure destinations.
...
1) **Add stance tokens/presets**  
   - Add new tokens to `GPT/lists/modelVoice|modelAudience|modelTone|modelPurpose.talon-list` as needed.  
...
3) **Adjust form/channel axis entries**  
   - Remove `announce` from `form/channel` axes; express via tone/purpose + destination.  
...
- `jobs` → Prefer intent/purpose token (Why); alternate: persona preset (“jobs lens”)  
...
- `value` → Intent/purpose token (Why); alternate: method `analysis` + intent  
  Outcome/impact framing. Intent keeps it portable; pairing with method `analysis` is fine, but the Why should live in intent.
- `pain` → Intent/purpose token (Why); alternate: scope `actions` + intent  
...
- `facilitate` → Persona preset (facilitator) + Method `liberating`/`plan` + Purpose (session outcome); alternate: scope `activities`  
  Facilitator stance plus process. Method + persona captures it; scope `activities` can narrow to agenda. Drop static prompt.
- `done` → Intent/purpose token (Definition of Done); alternate: completeness axis presets if we add “acceptance” flavour  
...
- `announce` (form/channel) → Tone/purpose + Channel destination; alternate: method `direct` + audience token  
