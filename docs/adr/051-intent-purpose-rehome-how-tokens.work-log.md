...
- Changes:
  - Removed non-Why intent tokens from intent SSOT and Talon list (`lib/personaConfig.py`, `GPT/lists/modelIntent.talon-list`); dropped the `collaborate` intent preset/bucket entry to keep presets aligned with the trimmed intent axis and refreshed `get_persona_intent_orchestrator()` cache consumers.
  ...
  - Added a guardrail to block reintroduction of how/artifact tokens on the intent axis (`_tests/test_axis_family_token_guardrails.py`) and updated intent/preset expectation tests (`_tests/test_voice_audience_tone_purpose_lists.py`, `_tests/test_persona_presets.py`); adjusted help fallback text to the new preset set (`lib/helpHub.py`).
