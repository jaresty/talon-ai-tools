...
2. **Run one loop / execute iteration for that ADR**
   ...
   - **Work-log location and convention:** for each ADR, use a dedicated work-log file alongside the primary ADR document, following a convention such as `<ADR-NUMBER>-<slug>.work-log.md` (for example, `0118-example-adr.work-log.md`). If no such file exists yet for the chosen ADR, the assistant **must create one on first use** (for example, `docs/adr/0118-example-adr.work-log.md`) and record slices there rather than appending large change histories directly to the primary ADR document.
