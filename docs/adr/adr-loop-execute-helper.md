...
| `active_constraint` | Highest-impact repository-controlled bottleneck (including unresolved assumptions) that currently limits the targeted behaviour, citing the validation command or blocker evidence; include deferrals; external blockers move to `residual_constraints` with their owning ADR reference | **Loop Contract → Focus declared** |
...
- A constraint is the specific factor inside this repository that currently limits progress toward that objective—the bottleneck that, unless relieved (or stabilized) by code or artefact changes under version control, prevents the targeted behaviour from landing green. When the limiting factor sits outside this repository, the helper treats it as out of scope; the loop records the blocker in `residual_constraints` and a separate ADR manages the external work.
...
**Next work queued**
- `next_work` contains `Behaviour:` bullets tied to salient task IDs, each citing the validation command or blocker evidence expected to relieve or stabilize the next active constraint and the future-shaping action that keeps the canonical intent aligned.
