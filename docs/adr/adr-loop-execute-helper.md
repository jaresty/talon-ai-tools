...
- **Active constraint** – the repository-controlled, falsifiable bottleneck that currently limits the targeted behaviour; the loop states it as a validation-backed statement. Limiting factors that sit outside this repository (for example, Launch configuration, vendor approval, infrastructure access) or no longer outrank other medium/high constraints appear in `residual_constraints` with mitigation notes. A compliant active-constraint statement names at least one alternative candidate that was considered and states why it ranks lower on the expected-value scale.
...
- **Expected value** – estimated benefit of relieving the constraint now, calculated as Impact × Probability × Time Sensitivity. Document each factor explicitly.
    ...
    - **Expected value table (example snippet)**
        ...
        ```
    Expected value is computed by assigning H=3, M=2, L=1 to each factor and multiplying. A compliant entry records the numeric product alongside the table (e.g., H×M×H = 18). Ties resolve toward the action with higher Time Sensitivity.
...
- **Constraint palate cleansers** – the probabilistic reviews triggered at reflection points; each reflection point runs this palate cleanser at every fifth reflection point (i.e., reflection points 5, 10, 15, …) or after any constraint pivot, whichever comes first; it re-evaluates the active constraint against all recorded medium/high candidates via the helper’s expected-value rubric, and records the outcome in the work-log while updating `active_constraint`/`residual_constraints` accordingly.
...
- **Canonical intent** – the minimal, shared representation of a behaviour outcome that loops converge on (e.g., a single helper, type, or invariant). The canonical intent for a behaviour outcome MUST be authoritatively expressed by its specifying validation; implementation code is considered conforming, not defining.
...
- `<SALIENT_TASK_LIST>` – the numbered list of open behaviour outcomes maintained in the ADR body. A salient task ID has the form `T-<N>` (e.g., `T-1`, `T-12`); loop entries cite these IDs when identifying the targeted behaviour.
...
**Focus declared**
- Red evidence is recorded before edits land and green/removal evidence after, keeping guardrail commands limited to the validation channel for that behaviour. "Edits land" means changes to executable artefacts are present in the working tree (staged or committed). The canonical VCS state is specified in the ADR header; absent that override, `git add` constitutes the boundary.
...
- Compliance requires the `helper:wip-preserve` safeguard: each agent session begins with an initial `helper:diff-snapshot` (an agent session is any new context window: a new conversation, a new CI job, or a new human work session; the session boundary is established by the first `helper:diff-snapshot` call in that context); when it reports non-zero while the latest work-log entry lacks green evidence, the loop is classified as interrupted and remains compliant only when the diff is archived under `<EVIDENCE_ROOT>`, cited in the work-log, and the audited baseline is restored through `<VCS_REVERT>` before edits proceed. The interrupted classification applies when the diff includes files touched by the active constraint's validation target. Modifications confined to directories unrelated to the validation target may be excluded with an explicit note.
...
**Slice qualifies**
- A loop is documentation-only when no executable artefacts change in the slice. Executable artefacts are code files, test files, scripts, schemas, configuration files that alter runtime behaviour, and automation templates.
...
- Removal records document the same command pairing, the `<VCS_REVERT>` invocation, and the re-run failure snippet. Documentation-only loops that do not touch executable artefacts may omit this removal record once blocker evidence is captured. If any required step fails (e.g., revert blocked), log the blocker evidence immediately. When revert does not re-fail (i.e., the original validation did not fully specify the behaviour), a compliant tightening record identifies: (a) which assertion was strengthened, (b) the behaviour the original validation did not cover, and (c) the assertion that would fail on revert of the implementation.
- Removal records keep the specifying validation in place and revert only the implementation under test; the re-run failure demonstrates that the validation fully specifies the behaviour change.
...
- Work-log entries for a given ADR cite a single helper version; when the version changes, a reconciliation entry records the migration outcome before new loops land. A compliant reconciliation entry identifies: (a) the rules that changed between versions, (b) any validation mappings that require updating, (c) evidence that existing loops remain compliant or loops queued to address gaps, and (d) any requirements introduced by the new version that were absent in the prior one.
...
Completion is eligible once every loop satisfies this contract under a single helper version, outstanding tasks are closed or parked with evidence, a final adversarial entry restates motivations and residual gaps, and repository metadata reflects the updated ADR state. Completion requires the targeted behaviour to land (code, tests, docs, automation) with green evidence; planning-only loops must keep the series open until the behaviour ships or is explicitly deferred with blocker evidence. An adversarial completion entry argues against completion being warranted: it names at least one residual gap, assesses whether it represents an undiscovered failure mode, and records the grounds on which the gap is accepted rather than kept open.
