...
2. **Run one loop / execute iteration for that ADR**
   - **Open or create the ADR work-log:**
     - Derive the expected work-log filename from the ADR file (for example, `docs/adr/0118-example-adr.md` → `docs/adr/0118-example-adr.work-log.md`).
   ...
   - **Work-log location and convention:** for each ADR, use a dedicated work-log file alongside the primary ADR document, following a convention such as `<ADR-NUMBER>-<slug>.work-log.md` (for example, `0118-example-adr.work-log.md`). Use this derived filename consistently when creating or opening the work-log.
