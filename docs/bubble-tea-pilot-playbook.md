...
- History is always a keystroke away—press `Ctrl+H` to expand or collapse the palette history and review recent token tweaks or CLI invocations. Press `Ctrl+Shift+H` while history is open to copy the most recent CLI command. `Ctrl+W` clears the composer filter text, and `Ctrl+Z` undoes the most recent token change.
