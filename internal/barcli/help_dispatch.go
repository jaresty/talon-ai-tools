...
func writeDispatchStepBlock(w io.Writer, step SequenceStep, _ int, _ *Grammar) {
	...
	if step.DuringDispatch != "" {
		fmt.Fprintf(w, "          4. [DISPATCH GATE] A response turn at this dispatch step satisfies this step only when it contains at least one Agent tool call. Text about dispatch is permitted only when Agent tool calls appear in the same turn. Spawn one Agent tool call per item — all in this same response turn. Set run_in_background: true on every Agent tool call so the during_dispatch command can run in the foreground concurrently. The number of Agent tool calls in this turn must equal the number of items in the enumerated list. Each Agent tool call prompt must contain the `## Agent Configuration` block from step 0c. A response turn where Agent tool call count is less than item count is non-compliant. A response turn where any Agent tool call prompt is missing the `## Agent Configuration` block is non-compliant. Use subagent_type: general-purpose (has Bash access and can run bar commands). A dispatch step is complete only when Agent tool calls appear in the transcript response turn that reads the dispatch step specification. A response turn that reads or references the dispatch step but contains no Agent tool calls does not satisfy this step — proceed to the Agent tool calls without announcing them first. In the same response turn as the Agent tool calls, also run `%s` as a Bash tool call — this must appear in the same turn as the Agent calls, not before them. Do not wait for agents to return before running this command. Wait for all background agent completion notifications and for this command to finish before proceeding to step 6.\n", step.DuringDispatch)
	} else {
