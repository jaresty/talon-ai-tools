...
func writeDispatchStepBlock(w io.Writer, step SequenceStep, _ int, _ *Grammar) {
	...
	if step.DuringDispatch != "" {
		fmt.Fprintf(w, "          4. [DISPATCH GATE] A response turn at this dispatch step satisfies this step only when it contains at least one Agent tool call. An evaluator determines whether a turn is the dispatch turn by locating the literal string `[DISPATCH GATE]` in the assistant's text output in that turn. Before spawning agents, emit a line of the form `Dispatching N agents:` where N equals the count of items in the enumerated list. Spawn one Agent tool call per item — all in this same response turn. Set run_in_background: true on every Agent tool call. The number of Agent tool calls in this turn must equal N. An evaluator determines required Agent call count by reading N from the first `Dispatching N agents:` line in the turn. Each Agent tool call prompt must contain the `## Agent Configuration` block from step 0c. Use subagent_type: general-purpose (has Bash access and can run bar commands). Proceed to the Agent tool calls without announcing them first. In the same response turn as the Agent tool calls, also run `%s` as a Bash tool call. An evaluator determines compliance by locating a Bash tool call containing the literal string `%s` in the same turn as at least one Agent tool call. Do not proceed to step 6 until every background agent has returned a result — an evaluator determines compliance by confirming that a result block for each Agent tool call spawned in this step appears in the transcript before step 6 output.\n", step.DuringDispatch, step.DuringDispatch)
	} else {
