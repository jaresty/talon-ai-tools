...
func writeDispatchStepBlock(w io.Writer, step SequenceStep, _ int, _ *Grammar) {
	...
	if step.DuringDispatch != "" {
		fmt.Fprintf(w, "          during_dispatch: immediately after spawning all agents in step 4, run `%s` as a bar build Bash tool call in the foreground — do not wait for agents to return first. Wait for all background agent completion notifications and for this command to finish before proceeding to the join in step 6. Do not proceed to step 6 until every background agent has sent a completion notification.\n", step.DuringDispatch)
	}
