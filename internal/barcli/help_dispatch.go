...
func writeDispatchStepBlock(w io.Writer, step SequenceStep, _ int, _ *Grammar) {
	...
	fmt.Fprintf(w, "          0a. Use the `/bar-autopilot` skill to select tokens appropriate to this dispatch step's role and task domain, then run `bar build [selected-tokens]` as a Bash tool call (no channel token — do not append `agent` or `skill`). Read the bar build output and use it to write the `## Agent Configuration` block. The first Agent tool call must appear in the same response turn as that Bash result block — an Agent tool call appearing in a response turn that contains no bar build Bash result block does not satisfy this step.\n")
