...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "1. Run `bar build [task-appropriate tokens] agent` — choose tokens based on the dispatch step's role and the current task domain. Read the TASK, CONSTRAINTS, and PERSONA sections of the output. Write a `## Agent Configuration` block containing task-specific context for this agent: the assigned item, any domain constraints, and relevant background the orchestrator has. Do not include persona, approach, or behavioral goal — those come from the agent's own bar build invocations. Pass this block inline in each Agent tool call prompt. A transcript where the first Agent tool call appears without a preceding `## Agent Configuration` block in the same response is non-compliant.\n\n   Example `## Agent Configuration` block:\n   ```\n   ## Agent Configuration\n   Assigned frame: Connection layer — connection pooling, timeout config, HTTP client behavior.\n   Context: Errors appear under load in the payment path; recent dependency update is the suspected trigger.\n   ```\n")
