...
func renderNavigationGuide(w io.Writer, grammar *Grammar) {
	...
	fmt.Fprintf(w, "\n")
	fmt.Fprintf(w, "# Intent-to-token discovery — returns tokens, starter packs (kind=pack), and sequences (kind=sequence):\n")
	fmt.Fprintf(w, "bar lookup \"<intent>\"               # e.g. bar lookup \"compare options\"\n")
	fmt.Fprintf(w, "bar lookup \"<intent>\" --axis method  # restrict to one axis (tokens only)\n")
...
func renderQuickStart(w io.Writer, compact bool) {
	...
	fmt.Fprintf(w, "# Intent-to-token lookup: find tokens, starter packs, and sequences by goal\n")
	fmt.Fprintf(w, "# Results include tokens (kind=token), starter packs (kind=pack), and sequences (kind=sequence).\n")
