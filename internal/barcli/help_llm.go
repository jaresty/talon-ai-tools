...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "1. Before writing any `## Agent Configuration` block: run `bar build [task-appropriate tokens]` as a Bash tool call and wait for its result — the result containing `=== TASK` is the gate event. Do not write the block until that result appears in the transcript. Choose tokens based on the dispatch step's role and the current task domain. Do NOT append a channel token (`agent` or `skill`) — those produce YAML spec output with a disk-write instruction, not prose the orchestrator can use inline. Read the bar build output and use it to write the `## Agent Configuration` block. The block may contain: the assigned item, any domain constraints, and relevant background. Do not include persona, approach, or behavioral goal — those come from each agent's own bar build invocations. Pass this block inline in each Agent tool call prompt. A transcript where the first Agent tool call appears without a preceding `## Agent Configuration` block in the same response is non-compliant.\n\n   Example `## Agent Configuration` block:\n   ```\n   ## Agent Configuration\n   Assigned frame: Connection layer — connection pooling, timeout config, HTTP client behavior.\n   Context: Errors appear under load in the payment path; recent dependency update is the suspected trigger.\n   ```\n")
