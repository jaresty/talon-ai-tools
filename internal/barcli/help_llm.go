...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "1. Run `bar build [task-appropriate tokens] agent` — choose tokens based on the dispatch step's role and the current task domain. Read the TASK, CONSTRAINTS, and PERSONA sections of the output. Write a `## Agent Configuration` block in the transcript containing: a persona statement derived from the PERSONA section, a method statement derived from the CONSTRAINTS section, and a behavioral goal derived from the TASK section. Do not copy bar build output verbatim — derive each statement from it. Pass this `## Agent Configuration` block inline in each Agent tool call prompt. A transcript where the first Agent tool call appears without a preceding `## Agent Configuration` block in the same response is non-compliant.\n\n   Example `## Agent Configuration` block (derive content from bar build output, do not copy this example verbatim):\n   ```\n   ## Agent Configuration\n   **Persona:** Investigator tracing failure modes from a single structural vantage point.\n   **Method:** Cycle through hypotheses — form prep to frame the hypothesis, probe to gather evidence, vet to assess — until the goal condition is met or exhausted.\n   **Behavioral goal:** Identify whether [frame-specific root cause] explains the observed symptom; return a confirmed or ruled-out verdict with supporting evidence.\n   ```\n")
