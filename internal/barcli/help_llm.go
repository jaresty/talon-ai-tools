...
func renderTokenSelectionHeuristics(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "**Automation-class methods:**\n")
	fmt.Fprintf(w, "- Prefer automated over manual steps → `automate` (model what can be expressed as automatic, repeatable operations)\n\n")
	fmt.Fprintf(w, "**Specification-class methods** (TDD / governing intent):\n")
	fmt.Fprintf(w, "- Test-driven specification / spec-first → `ground`\n")
	fmt.Fprintf(w, "- `ground` declares I, builds falsifiable V at minimal-sufficient strength, runs V if executable, then produces O\n\n")
