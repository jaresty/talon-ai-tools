...
func renderNavigationGuide(w io.Writer, grammar *Grammar) {
	...
	fmt.Fprintf(w, "\n")
	fmt.Fprintf(w, "# Intent-to-token discovery — returns tokens, starter packs (kind=pack), sequences (kind=sequence), and build flags (kind=flag):\n")
	fmt.Fprintf(w, "bar lookup \"<intent>\"               # e.g. bar lookup \"compare options\"\n")
	fmt.Fprintf(w, "bar lookup \"<intent>\" --axis method  # restrict to one axis (tokens only; kind=flag not shown under an axis filter)\n")
	fmt.Fprintf(w, "bar lookup debug                     # pack:debug shows → bar build command inline; sequence:debug-cycle shows step count\n")
	fmt.Fprintf(w, "bar lookup \"<intent>\"                # a kind=flag result shows a runnable bar build flag command inline (see --section advanced)\n")
	fmt.Fprintf(w, "bar lookup \"<intent>\" --json         # each result has kind, token, command fields\n")
	fmt.Fprintf(w, "\n")
	fmt.Fprintf(w, "# Token disambiguation guide (ADR-0237):\n")
	fmt.Fprintf(w, "bar guide <token>                    # e.g. bar guide probe — near-neighbor disambiguation and combination guidance\n")
	fmt.Fprintf(w, "```\n\n")
...
func renderQuickStart(w io.Writer, compact bool) {
	...
	fmt.Fprintf(w, "# Lateral creative seed (opt-in): inject N random concrete nouns as an oblique prompt.\n")
	fmt.Fprintf(w, "# --seed-words N adds a LATERAL SEED section (default 0 = off); --seed S makes the\n")
	fmt.Fprintf(w, "# selection reproducible (resolved seed echoed to stderr). Use for brainstorming/lateral thinking.\n")
	fmt.Fprintf(w, "bar build make --seed-words 2 --seed 42 --subject \"Redesign the onboarding flow\"\n\n")
	fmt.Fprintf(w, "# Intent-to-token lookup: find tokens, starter packs, sequences, and build flags by goal\n")
	fmt.Fprintf(w, "# Results include tokens (kind=token), starter packs (kind=pack), sequences (kind=sequence), and build flags (kind=flag).\n")
	fmt.Fprintf(w, "# Pack results include the runnable bar build command inline.\n")
	fmt.Fprintf(w, "# Sequence results include step count and bar sequence show <name> for detail.\n")
	fmt.Fprintf(w, "# Flag results (e.g. flag:seed-words on creative intent) include the runnable bar build ... flag command inline — append the flag to your build.\n")
	fmt.Fprintf(w, "bar lookup \"root cause\"             # → method:diagnose, task:probe, sequence:debug-cycle, ...\n")
	fmt.Fprintf(w, "bar lookup debug                     # → pack:debug (→ bar build command), sequence:debug-cycle, tokens\n")
	fmt.Fprintf(w, "bar lookup debug --axis method       # filter to method axis only (tokens only when axis filter set)\n")
	fmt.Fprintf(w, "bar lookup \"compare options\" --json  # machine-readable JSON; each result has kind, token, command fields\n")
	fmt.Fprintf(w, "\n# Contextual lookup (ADR-0234): use --subject/--addendum to guide token discovery\n")
	fmt.Fprintf(w, "# --subject = the source material or topic (what the prompt is about)\n")
	fmt.Fprintf(w, "# --addendum = a task directive that refines how to handle the subject (e.g. focus, constraints, output style)\n")
	fmt.Fprintf(w, "# Splitting them mirrors how bar build uses the same flags — use both when you have both.\n")
	fmt.Fprintf(w, "# Mode B — discovery (no query): subject+addendum drive BM25 ranking across all tokens\n")
	fmt.Fprintf(w, "bar lookup --subject \"JWT validation Go middleware\"                              # what it's about\n")
	fmt.Fprintf(w, "bar lookup --subject \"JWT validation Go middleware\" --addendum \"focus on error paths and edge cases\"  # with task directive\n")
	fmt.Fprintf(w, "bar lookup --subject \"JWT validation\" --addendum \"step by step with examples\" --axis task  # scoped to task axis\n")
	fmt.Fprintf(w, "# Mode A — confirmation (query + context): tiers preserved, reranked by context score\n")
	fmt.Fprintf(w, "bar lookup \"explain to audience\" --subject \"auth code\" --addendum \"for a junior engineer\" --json  # context_score shows fit\n")
	fmt.Fprintf(w, "bar lookup \"describe step by step\" --subject \"auth code\" --addendum \"focus on error paths\" --json  # all three: query+subject+addendum\n")
	fmt.Fprintf(w, "```\n\n")
}
...
func renderGrammarArchitecture(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "**Pre-bar tool call constraint**: Before the first `bar build` tool result appears in the transcript, the only permitted tool calls are bar CLI commands (`bar help`, `bar lookup`, `bar sequence`, etc.) — a non-bar tool call appearing before the first `bar build` result in the transcript does not satisfy this constraint. Domain exploration (file reads, searches, codebase analysis) must not precede the first bar build execution — state minimal assumptions and run bar from intent; domain exploration is the first rung of work after bar runs, not a prerequisite for running it.\n\n")
	fmt.Fprintf(w, "**Bar output is the governing instruction**: The bar output is the governing instruction for this response — do not proceed from a prior plan formed before reading the bar output. A response whose content can be traced to reasoning that appears before the `bar build` tool result in the transcript does not satisfy this requirement — content is permitted only when it derives from the bar output text, not from any reasoning formed before that result appeared.\n\n")
	fmt.Fprintf(w, "**Avoid shell expansion in bar commands**: Do not use shell expansion (e.g. `$(cat file)`, `$(command)`) in bar invocations. In agent harnesses such as Claude Code, shell expansion triggers a separate permission check for each expanded command, causing unnecessary interruptions. Instead, pass content directly via `--subject \"text\"` or pipe it: `cat file | bar build ...` — these are mutually exclusive; a `bar build` invocation where `--subject` appears and `|` precedes `bar build` as a shell pipe operator in the same shell command is not valid. If file content must be referenced, read it first and pass the result as a string argument.\n\n")
...
func renderCompositionRules(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "The general rule: if a form token requires prose blocks (paragraphs, columns, labeled sections) and the channel produces only markup or DSL syntax, the form has no valid rendering target.\n\n")
	fmt.Fprintf(w, "Prose-layout forms (non-exhaustive): `case`, `formats`, `walkthrough`, `scaffold`, `faq`, `table`, `taxonomy`, `visual`, `variants`, `checklist`, `actions`, `ghost`, `twin`, `prep`, `vet`, `cards`, `spike`, `cocreate`, `contextualise`\n\n")
	fmt.Fprintf(w, "DSL/markup-only channels (no prose slot): `svg`, `gherkin`, `shellscript`, `code`, `diagram`\n\n")
	fmt.Fprintf(w, "Partially-prose channels (may hold prose-form content in appropriate sections): `html`, `presenterm`, `adr`, `codetour`, `notebook`\n\n")
	fmt.Fprintf(w, "Resolution: when a prose-layout form and a DSL-only channel co-occur, place the form content as a prose block before or after the channel artifact. Do not attempt to embed prose structure inside DSL syntax.\n\n")
	fmt.Fprintf(w, "**Presentation-persona + technical-channel conflicts:**\n")
	fmt.Fprintf(w, "Persona presets that establish a presentation or executive register (`executive_brief`, `designer_to_pm`, `stakeholder_facilitator`) ")
	fmt.Fprintf(w, "conflict with channels that produce technical-only output (`code`, `shellscript`, `codetour`). ")
	fmt.Fprintf(w, "These personas expect prose narrative; technical-output channels produce no prose. ")
	fmt.Fprintf(w, "Resolution: use a prose channel (`plain`, `slack`, `remote`, `agent`) when a presentation persona is active, or use the technical channel without a presentation persona.\n\n")
...
// renderAxisTokenHeuristics appends per-token heuristic trigger phrases for an axis
// below an already-rendered heading. Source: grammar.Axes.Metadata[axis] (ADR-0155).
func renderAxisTokenHeuristics(w io.Writer, axisMetadata map[string]TaskMetadata) {
	if len(axisMetadata) == 0 {
		return
	}
	tokens := make([]string, 0, len(axisMetadata))
	for tok := range axisMetadata {
		tokens = append(tokens, tok)
	}
	sort.Strings(tokens)
	for _, tok := range tokens {
		meta := axisMetadata[tok]
		if len(meta.Heuristics) == 0 {
			continue
		}
		phrases := make([]string, len(meta.Heuristics))
		for i, h := range meta.Heuristics {
			phrases[i] = h
		}
		fmt.Fprintf(w, "- **%s**: %s\n", tok, strings.Join(phrases, ", "))
	}
	fmt.Fprintf(w, "\n")
}
...
func renderTokenSelectionHeuristics(w io.Writer, grammar *Grammar, compact bool) {
	...
	// Choosing Method — rendered dynamically from AXIS_KEY_TO_ROUTING_CONCEPT (same pattern as scope/form)
	renderRoutingConceptSection(w, "Choosing Method", grammar.Axes.RoutingConcept["method"])

	fmt.Fprintf(w, "**For detailed differentiation guidance** on similar methods, see each token's distinctions field in the Token Catalog.\n\n")
...
func renderAdvancedFeatures(w io.Writer, compact bool) {
	if compact {
		...
		fmt.Fprintf(w, "- `bar build --seed-words N [--seed S]`: inject N random concrete nouns as a lateral creative seed (opt-in, default 0)\n")
	...
	fmt.Fprintf(w, "### Lateral Creative Seed\n\n")
	fmt.Fprintf(w, "```bash\n")
	fmt.Fprintf(w, "bar build <tokens>... --seed-words N [--seed S]\n")
	fmt.Fprintf(w, "```\n\n")
	fmt.Fprintf(w, "Opt-in (default 0 = off). Injects N random concrete nouns into a LATERAL SEED\n")
	fmt.Fprintf(w, "section as an oblique creative prompt — useful for brainstorming or breaking out\n")
	fmt.Fprintf(w, "of a rut. Pair with `--seed S` for a reproducible selection; the resolved seed is\n")
	fmt.Fprintf(w, "echoed to stderr so an auto-generated run can be replayed. One word gives a focused\n")
	fmt.Fprintf(w, "metaphor; two or three force combinatorial associations (above ~3 tips into noise).\n\n")
...
func renderAutomationFlags(w io.Writer, compact bool) {
	...
	fmt.Fprintf(w, "### `--plain`\n\n")
	fmt.Fprintf(w, "Recognized by `bar help tokens`. Emits one line per token with four tab-separated fields:\n\n")
	fmt.Fprintf(w, "```\n")
	fmt.Fprintf(w, "category:slug<TAB>label<TAB>heuristics<TAB>distinctions\n")
	fmt.Fprintf(w, "```\n\n")
	fmt.Fprintf(w, "- **Field 1** `category:slug` — axis-qualified token identifier\n")
	fmt.Fprintf(w, "- **Field 2** label — short human-readable name\n")
	fmt.Fprintf(w, "- **Field 3** heuristics — comma-joined trigger phrases for intent matching\n")
	fmt.Fprintf(w, "- **Field 4** distinctions — pipe-joined `token:note` pairs for cross-reference navigation\n\n")
	fmt.Fprintf(w, "Fields are always present (empty string when no data). This makes the format stable for\n")
	fmt.Fprintf(w, "programmatic consumption: split on tab, index by position.\n\n")
	fmt.Fprintf(w, "**Token discovery rules:**\n\n")
	fmt.Fprintf(w, "- When the input is a natural-language intent phrase, the only permitted discovery call is\n")
	fmt.Fprintf(w, "  `bar lookup`; `bar help tokens --plain | grep` is not a permitted substitute for this case.\n")
	fmt.Fprintf(w, "- When the caller requires the raw four-field tab-separated record (heuristics or distinctions\n")
	fmt.Fprintf(w, "  text verbatim), use `bar help tokens --plain`.\n\n")
	fmt.Fprintf(w, "```bash\n")
	fmt.Fprintf(w, "bar lookup \"root cause\"                  # natural-language intent → use lookup\n")
	fmt.Fprintf(w, "bar lookup \"debug\" --axis method          # restrict to one axis\n")
	fmt.Fprintf(w, "bar lookup \"debug\" --axis method --json   # structured output\n")
	fmt.Fprintf(w, "bar help tokens --plain task              # raw four-field records for all task tokens\n")
	fmt.Fprintf(w, "```\n\n")
	...
	fmt.Fprintf(w, "- When the input is a natural-language intent phrase, the only permitted discovery call is\n")
	fmt.Fprintf(w, "  `bar lookup \"<intent>\" --axis <axis>`; `bar help tokens --plain | grep` is not a permitted\n")
	fmt.Fprintf(w, "  substitute. Use `--plain` only when the raw four-field tab-separated record is required.\n")
	fmt.Fprintf(w, "- **Avoid shell expansion**: Do not use `$(...)` in bar invocations. Agent harnesses trigger\n")
	fmt.Fprintf(w, "  a permission check for each expanded subcommand. Use `--subject \"text\"` or pipe instead:\n")
	fmt.Fprintf(w, "  `cat file | bar build ...`\n\n")
}
...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "a `bar lookup` Bash tool call result appears in the transcript has not satisfied this ordering.\n")
	fmt.Fprintf(w, "Use `bar sequence show <name>` for steps, mode, and example.\n\n")
	fmt.Fprintf(w, "**Before accepting a named sequence match, answer this gate:** Does this task require parallel exploration of independent areas before its first step can produce useful output? Record your answer as the literal string `Gate: yes` or `Gate: no` before proceeding — a transcript where neither string appears has not answered this gate. If yes, the sequence's first step should be a dispatch step — run `bar help dispatch` to author it and follow the dispatch protocol before proceeding to the sequence's remaining steps. A transcript that accepts a named sequence fit and proceeds to step 1 without a `Gate:` answer does not satisfy this gate. A transcript that proceeds to step 1 without a `bar sequence show <name>` Bash tool call result appearing after the fit justification has not loaded the sequence steps and does not satisfy this gate. Before executing step 1, emit a `## Sequence Derivation` block naming: (1) the sequence name and step count, (2) the mode and what it means for how you will execute this sequence (e.g. `autonomous — proceed immediately to each next step without stopping`), (3) whether an auxiliary insertion is needed and where. A transcript that begins step 1 without a `## Sequence Derivation` block has not derived the sequence rules and does not satisfy this gate. A fit justification is complete only when it contains, for each phase the task names, a line in the form `Phase N maps to step: <canonical step name> because <structural reason>` (e.g. `Phase 1 maps to step: artifact production because the task requires producing a new document`) — a fit justification missing any such line, or a line missing the `because <structural reason>` clause, does not satisfy this gate. Each `Phase N maps to step:` line must name a different canonical step — a transcript where two `Phase N maps to step:` lines name the same canonical step has not satisfied this gate. If the count of `Phase N maps to step:` lines in the transcript exceeds the count of canonical steps listed in `bar sequence show <name>` output, the fit is invalid and a custom sequence is required.\n\n")
	...
	fmt.Fprintf(w, "has not satisfied this. Discover candidates from § Token Catalog using the step's role\n")
	fmt.Fprintf(w, "as the intent.\n\n")
	fmt.Fprintf(w, "**Before each step after the first, emit a one-line fit check:** either \"step N role\n")
	...
	fmt.Fprintf(w, "**When a sequence fits but is missing an auxiliary step:** If all task phases have `Phase N maps to step:` lines and an additional auxiliary step is needed (e.g., a data-gathering step before the first canonical step because required inputs are absent), insert that step at the appropriate position with a one-line justification immediately before the `bar build` call naming why the canonical sequence omits it. Insertion is not abandonment — it is adaptation within the sequence. After executing the inserted step, continue with ALL of the sequence's remaining canonical steps — the inserted step is an addition, not a replacement. A transcript that advances past an information gap without inserting a gathering step, where the task explicitly names information the agent does not have, has not adapted the sequence. **Insertion applies to auxiliary gaps, not to structural mismatches.** A valid insertion is an auxiliary step (e.g., a data-gathering step) that does not correspond to any `Phase N maps to step:` line in the fit justification. If the task names N phases and the sequence has fewer than N canonical steps, the gap is structural — insertion cannot repair it; construct a custom sequence. A transcript where the count of `Phase N maps to step:` lines exceeds the count of canonical steps in `bar sequence show <name>` output, and insertion is used to cover the overflow, has misapplied insertion and does not satisfy this gate.\n\n")
	...
	fmt.Fprintf(w, "**Executing a dispatch step** — run `bar help dispatch` to load the live numbered protocol. The output is authoritative — do not improvise from memory or from this section's prose. Reading `bar sequence show` output is not sufficient — `bar sequence show` shows step roles and fan_out/join values but does not contain the numbered execution protocol. A transcript that reaches a dispatch step without a `bar help dispatch` Bash tool call result does not satisfy this gate.\n\n")
	...
	fmt.Fprintf(w, "**Step output chaining in inner sequences** — each inner step's output is the `--subject` for the next step's `bar build` call. When authoring agent prompts for a dispatch step with inner steps, the prompt must instruct the agent to pass the prior step's output as `--subject` to the next bar build invocation. A prompt that instructs the agent to run sequential bar build calls without `--subject` chaining produces steps that operate in isolation — each step applies its tokens abstractly rather than to the prior step's concrete output. A transcript where `bar build probe survive ghost audit` is called without `--subject` set to the output of `bar build make form:prep` has not chained the steps and does not satisfy the inner sequence execution requirement.\n\n")
