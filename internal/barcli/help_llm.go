...
func renderQuickStart(w io.Writer, compact bool) {
	...
	fmt.Fprintf(w, "bar lookup explain --subject \"auth code to my manager\"          # rerank 'explain' matches by context\n")
	fmt.Fprintf(w, "bar lookup explain --subject \"auth code\" --addendum \"for a non-technical audience\" --json  # context_score shows fit\n")
	fmt.Fprintf(w, "bar lookup describe --subject \"auth code\" --addendum \"focus on error paths\" --json  # all three: query+subject+addendum\n")
	fmt.Fprintf(w, "```\n\n")
}
