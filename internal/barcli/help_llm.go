...
func renderTokenSelectionHeuristics(w io.Writer, compact bool) {
	...
	fmt.Fprintf(w, "- **Channel × audience compatibility**: technical output channels (`code`, `shellscript`, `codetour`) are incompatible with non-technical audiences (to-CEO, to-managers, to-stakeholders, to-team). When audience is non-technical, prefer `diagram`, `presenterm`, `sketch`, or `plain` over code-oriented channels.\n")
