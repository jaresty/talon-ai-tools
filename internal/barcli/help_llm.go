...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "1. **Run bar build.** Issue `bar build [tokens matching the dispatch step's role]` as a Bash tool call. Wait for the result — `=== TASK` in the result is the gate event. Do not write any agent prompts until that string appears. Omit channel tokens (`agent`, `skill`) — they produce YAML, not prose.\n")
	fmt.Fprintf(w, "2. Write one `## Agent Configuration` block per dispatch item, placed as the first line of that item's agent prompt (before any other content). The block names the assigned item and its concern. Example:\n   ```\n   ## Agent Configuration\n   Assigned item: AuthMiddleware — security: JWT validation gaps, token replay vectors, privilege escalation paths.\n   ```\n   (One bar build call covers all items — no second call needed.)\n")
	fmt.Fprintf(w, "3. Read `fan_out`: `enumerate` — the prior step's output is a list of distinct inputs; each agent receives a different item. `replicate` — all agents receive the same full input; differentiation is in each agent's configuration (its framing, hypothesis, or approach), not in what input it receives. Use replicate when named items in the task are approach variants on shared input, not input partitions.\n")
