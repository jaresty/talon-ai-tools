...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "1. Run `bar build [task-appropriate tokens]` as a Bash tool call — choose tokens based on the dispatch step's role and the current task domain. Do NOT append a channel token (`agent` or `skill`) — those produce YAML spec output with a disk-write instruction, not prose the orchestrator can use inline. Read the bar build output and use it to write a `## Agent Configuration` block. A `## Agent Configuration` block appearing in a response that contains no bar build Bash tool call result does not satisfy this step — writing the block from prior knowledge without running bar build is non-compliant. The block may contain: the assigned item, any domain constraints, and relevant background. Do not include persona, approach, or behavioral goal — those come from each agent's own bar build invocations. Pass this block inline in each Agent tool call prompt. A transcript where the first Agent tool call appears without a preceding `## Agent Configuration` block in the same response is non-compliant.\n\n   Example `## Agent Configuration` block:\n   ```\n   ## Agent Configuration\n   Assigned frame: Connection layer — connection pooling, timeout config, HTTP client behavior.\n   Context: Errors appear under load in the payment path; recent dependency update is the suspected trigger.\n   ```\n")
	fmt.Fprintf(w, "2. Do NOT run an additional `bar build` for this step's content. The run-one-command rule does not apply beyond the pre-dispatch agent config call above.\n")
	fmt.Fprintf(w, "3. Read `fan_out`: `enumerate` treats the prior step's output as a list and sends one item per agent. `replicate` sends the full prior output to every agent unchanged.\n")
	fmt.Fprintf(w, "4. Read `isolation`: when `true`, each agent receives only its assigned item and the step's prompt_hint (plus inner steps and stop_when if present) — no shared conversation history. When `false`, agents share context.\n")
	fmt.Fprintf(w, "5. Spawn one agent per item using the Agent tool. Do not batch items into a single agent. Spawn all agents in a single response — a transcript where agents for the same dispatch step appear across multiple responses is non-compliant.\n")
	fmt.Fprintf(w, "6. Read `join`: `all` — wait for every agent to complete (fail if any fail); `first` — take the first successful result (remaining agents may still complete); `merge` — collect all results into an array.\n")
	fmt.Fprintf(w, "7. Pass the join result as `--subject` to the next step. Do not synthesize or interpret before passing forward.\n\n")
	fmt.Fprintf(w, "**Authoring a custom dispatch step** — when you want fan-out in a workflow that doesn't match a named sequence:\n")
	fmt.Fprintf(w, "A dispatch step is just a regular sequence step with `type: dispatch` in the config. You can plan one ad hoc without a named sequence. Declare the following before spawning agents:\n")
	fmt.Fprintf(w, "- `fan_out`: `enumerate` (one agent per item in prior output) or `replicate` (same input to all agents)\n")
	fmt.Fprintf(w, "- `join`: `all` (wait for all), `first` (take first success), or `merge` (collect all results)\n")
	fmt.Fprintf(w, "- `isolation`: whether each agent gets only its own item (`true`) or shared context (`false`)\n")
	fmt.Fprintf(w, "- `inner`: optional — an inline sub-sequence each agent executes (list of token steps + stop_when). When absent, agents act on prompt_hint alone.\n")
	fmt.Fprintf(w, "Use `bar sequence show <name> --json` to inspect any named sequence's dispatch step structure as a reference model.\n\n")
