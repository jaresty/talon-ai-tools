...
func renderAutomationFlags(w io.Writer, compact bool) {
	...
	fmt.Fprintf(w, "**Token discovery workflow using `--plain`:**\n\n")
	fmt.Fprintf(w, "1. Grep field 3 (heuristics) to find tokens matching an intent phrase\n")
	fmt.Fprintf(w, "2. Read field 4 (distinctions) of the match to find related tokens worth comparing\n")
	fmt.Fprintf(w, "3. Use those token names to look up full descriptions in `bar help llm`\n\n")
	fmt.Fprintf(w, "```bash\n")
	fmt.Fprintf(w, "bar help tokens --plain method | grep 'debug'     # find method tokens for debugging intent\n")
	fmt.Fprintf(w, "bar help tokens --plain task                       # all tasks with heuristics and distinctions\n")
	fmt.Fprintf(w, "bar help tokens scope --plain                      # scope axis: slug, label, heuristics, distinctions\n")
	fmt.Fprintf(w, "bar help tokens --plain | grep '^task:'            # just task lines\n")
	fmt.Fprintf(w, "```\n\n")
	...
	fmt.Fprintf(w, "- Use `--plain` for targeted token discovery: grep field 3 (heuristics) to match intent phrases,\n")
	fmt.Fprintf(w, "  then read field 4 (distinctions) for cross-reference navigation. Complements `bar help llm`\n")
	fmt.Fprintf(w, "  for targeted axis scans without re-reading the full reference.\n")
