...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "**When no named sequence fits:** If `bar sequence list` reveals no sequence whose canonical step structure matches your task's phases, construct a custom ad hoc chain — name each step explicitly by role, one `bar build` per step. A fit justification is complete only when it contains one sentence per task phase explicitly mapping that phase to a named step in the sequence — a justification that names fewer phases than the task has does not satisfy this gate. A transcript that selects a named sequence without such a justification has not satisfied this gate.\n\n")
