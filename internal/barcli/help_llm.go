...
func renderLLMSequencesSection(w io.Writer, grammar *Grammar, compact bool) {
	...
	fmt.Fprintf(w, "**When a sequence fits but is missing a step:** If a named sequence covers the task's overall shape but the task requires a step the canonical list omits, insert that step at the appropriate position with a one-line justification immediately before the `bar build` call naming why the canonical sequence omits it. Insertion is not abandonment — it is adaptation within the sequence. After executing the inserted step, continue with ALL of the sequence's remaining canonical steps — the inserted step is an addition, not a replacement. A transcript that advances past an information gap without inserting a gathering step, where the task explicitly names information the agent does not have, has not adapted the sequence.\n\n")
