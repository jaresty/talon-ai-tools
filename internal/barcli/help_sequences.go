...
func renderSequencesHelp(w io.Writer, grammar *Grammar) {
	...
	fmt.Fprintln(w, "## How sequences are designed")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "A pairing warrants a named sequence when all three hold:")
	fmt.Fprintln(w, "  1. Step B is measurably better after step A than running B cold — not just compatible,")
	fmt.Fprintln(w, "     but genuinely uplifted. A→B and B→A should not uplift equally.")
	fmt.Fprintln(w, "  2. The pattern recurs across different subjects and users.")
	fmt.Fprintln(w, "  3. The mode is non-obvious — autonomous, linear, or cycle — and worth declaring")
	fmt.Fprintln(w, "     explicitly rather than leaving to ad hoc judgment.")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "Common structural patterns:")
	fmt.Fprintln(w, "  frame → act      Prepare before executing (experiment-cycle, plan-and-retrospect)")
	fmt.Fprintln(w, "  simulate → check Anticipate before reviewing actual outcomes (simulate-and-review)")
	fmt.Fprintln(w, "  gather → show    Scope collection before synthesizing (gather-and-synthesize)")
	fmt.Fprintln(w, "  make → vet       Produce an artifact before reviewing it (make-and-review)")
	fmt.Fprintln(w, "  probe → fix      Surface root causes before repairing (debug-cycle)")
	fmt.Fprintln(w)
