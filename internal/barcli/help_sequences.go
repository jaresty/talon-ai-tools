...
func renderSequencesHelp(w io.Writer, grammar *Grammar) {
	...
	fmt.Fprintln(w, "  frame → act      Prepare or scope before executing in the real world")
	fmt.Fprintln(w, "  simulate → check Anticipate outcomes before reviewing what actually happened")
	fmt.Fprintln(w, "  gather → show    Scope what to collect before synthesizing the findings")
	fmt.Fprintln(w, "  make → vet       Produce an artifact before reviewing it critically")
	fmt.Fprintln(w, "  diagnose → fix   Surface root causes before repairing")
	fmt.Fprintln(w)
