...
func LookupTokens(query string, g *Grammar, axisFilter string) []LookupResult {
	...
	tryToken := func(axis, token, label, definition string, heuristics []string, distinctions []string) {
		...
		for _, word := range words {
			...
			// Tier 0a: axis heuristics substring (categorical routing signal, ranks above definition)
			if bestTier < 0 {
				for _, ah := range g.AxisLevelHeuristics(axis) {
					if strings.Contains(strings.ToLower(ah), wordLower) {
						bestTier = 0
						bestField = "axis_heuristics"
						bestText = ah
						break
					}
				}
			}
			// Tier 0b: definition substring
...
func LookupTokensWithEmbedder(query string, g *Grammar, axisFilter string, _ interface{}) []LookupResult {
	return LookupTokens(query, g, axisFilter)
}
