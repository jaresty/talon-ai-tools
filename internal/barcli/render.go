...
func RenderPlainText(result *BuildResult) string {
	...
	if len(result.HydratedConstraints) == 0 && result.Persona == (PersonaResult{}) {
		...
		b.WriteString("For each token line below (excluding persona = (none)), run `bar help token <slug>` as a Bash tool call, where <slug> is the slug shown after →. The next non-empty line of assistant output after that tool-result block must begin with `Loaded: <slug> (when: \"<verbatim heuristic phrase from the Heuristics line that best matches this request>\" — not: \"<one other verbatim phrase from the same Heuristics line>\" — because: [one clause naming the property of the current request that the selected phrase matches and the rejected phrase does not])` — a Loaded: line is valid only when a tool-result block containing \"# Token: <slug>\" appears above it and the when: phrase must appear verbatim as a complete semicolon-delimited phrase in the Heuristics line in that tool-result block — partial fragments do not satisfy this requirement — and the not: phrase must appear verbatim as a complete semicolon-delimited phrase in the same Heuristics line. Skip the tool call only if a compliant `Loaded: <slug> (when: \"...\" — not: \"...\" — because: ...)` already appears verbatim in the transcript above this TOKENS section, immediately following a tool-result block containing \"# Token: <slug>\" — if the prior `Loaded: <slug>` line is not visible verbatim in the current transcript window, treat it as absent and re-run the tool call; do not skip based on memory or inference; in that case, write `Loaded: <slug> (skip confirmed — when: \"<phrase>\" — not: \"<phrase>\" — because: <reason>)` where both phrase values must be verbatim copies of the corresponding phrases in the prior `Loaded:` line for the same slug. Once a `Loaded: <slug>` line exists for every slug listed above, write the literal line `Token loads complete.` — a Token derivations block appearing before `Token loads complete.` does not satisfy this requirement; `Token loads complete.` appearing before all required `Loaded:` lines does not satisfy this requirement. Before writing `Token loads complete.`, write one line of the form `Loads verified: <slug1>, <slug2>, ... (<N> of <N>)` where the slugs are copied verbatim from the token list above and N is the count of that list. Then write the Token derivations block. The when: phrase is a provenance anchor confirming the token was loaded from the tool result — the behavioral claim belongs in the effect description after →. In the Token derivations block, for each active token write one line of the form `[slug]: \"[verbatim phrase from the Heuristics line in the `bar help token <slug>` tool-result block immediately above the `Loaded: <slug>` line for this token]\" → \"[verbatim text of the Description field up to the first ' —' or first period, whichever comes first; if the Description field contains neither ' —' nor '.', use the full Description text]\" as applied here: [how that clause manifests in this specific response]` — a derivations line whose → quoted clause does not appear verbatim in the `bar help token <slug>` tool-result block above does not satisfy this requirement. The next assistant text after any `bar help token` tool-result block must be either a `Loaded: <slug>` line for that result or another `bar help token` tool call — any other assistant text between two tool-result blocks renders the load sequence non-compliant. Where the model supports parallel tool calls, all outstanding `bar help token` calls for slugs not yet loaded may be issued as a single simultaneous batch. `Token loads complete.` is not a turn-end signal and no user message may appear between it and the Token derivations block.\n")
		for _, constraint := range result.HydratedConstraints {
			axisKey := strings.ToLower(strings.TrimSpace(constraint.Axis))
