...
func RenderPlainText(result *BuildResult) string {
	...
	if len(result.LateralSeed) > 0 {
		writeSection(&b, sectionLateralSeed, lateralSeedBody(result.LateralSeedBodySingular, result.LateralSeedBodyPlural, result.LateralSeed))
	}
...
// lateralSeedBody renders the seed words by substituting the comma-joined quoted
// word list into the grammar-sourced template (SSOT: LATERAL_SEED_BODY_SINGULAR /
// LATERAL_SEED_BODY_PLURAL in lib/metaPromptConfig.py). Singular framing is used
// for one word, plural for several. Every "{words}" placeholder is replaced.
// Mirrored by lateralSeedBody() in web/src/lib/renderPrompt.ts.
func lateralSeedBody(singular, plural string, words []string) string {
	...
	return strings.ReplaceAll(template, "{words}", joined)
}
