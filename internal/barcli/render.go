...
func RenderPlainText(result *BuildResult) string {
	...
	if len(result.HydratedConstraints) == 0 && result.Persona == (PersonaResult{}) {
		...
		b.WriteString("For each token line below (excluding persona = (none)), run `bar help token <slug>` as a Bash tool call, where <slug> is the slug shown after →. The first assistant text after that tool-result block must begin with `Loaded: <slug> (when: \"<verbatim heuristic phrase from the Heuristics line that best matches this request>\" — not: \"<one other verbatim phrase from the same Heuristics line>\" — because: [one clause naming the property of the current request that the selected phrase matches and the rejected phrase does not])` — a Loaded: line not preceded by a tool-result block containing \"# Token: <slug>\", or whose when: phrase does not appear verbatim in that tool-result block, or whose not: phrase does not appear verbatim in that tool-result block, does not satisfy this requirement. Skip the tool call only if a compliant `Loaded: <slug> (when: \"...\" — not: \"...\" — because: ...)` already appears verbatim in the transcript above this TOKENS section, immediately following a tool-result block containing \"# Token: <slug>\"; in that case, write `Loaded: <slug> (skip confirmed — when: \"<phrase>\" — not: \"<phrase>\" — because: <reason>)` where both phrase values are copied verbatim from the prior `Loaded:` line — a skip-confirmed line whose quoted phrase does not match the quoted phrase in the prior `Loaded:` line does not satisfy this requirement. Once a `Loaded: <slug>` line exists for every slug listed above, write the literal line `Token loads complete.` — a Token derivations block appearing before `Token loads complete.` does not satisfy this requirement; `Token loads complete.` appearing before all required `Loaded:` lines does not satisfy this requirement. Then write the Token derivations block. The when: phrase is a provenance anchor confirming the token was loaded from the tool result — the behavioral claim belongs in the effect description after →. In the Token derivations block, for each active token write one line of the form `[slug]: \"[quoted trigger phrase from the Loaded: line]\" → \"[verbatim text of the Description field up to the first ' —' or first period, whichever comes first]\" as applied here: [how that clause manifests in this specific response]` — a derivations line whose → quoted clause does not appear verbatim in the `bar help token <slug>` tool-result block above does not satisfy this requirement.\n")
		for _, constraint := range result.HydratedConstraints {
			axisKey := strings.ToLower(strings.TrimSpace(constraint.Axis))
