	...
	for i, step := range seq.Steps {
		...
		if step.Type == "dispatch" {
			...
			if step.Inner != nil {
				...
				fmt.Fprintf(stdout, "          5. Each agent prompt must include the literal bar build command the orchestrator provides — do not discover tokens. The orchestrator must construct and include this exact command: `bar build <step-token-string> --subject '<assigned-item>' --addendum '<prompt_hint>'`. After running bar build, the agent response must be grounded in the TASK, CONSTRAINTS, and PERSONA sections of that output — a response element not traceable to one of those three sections does not satisfy this requirement. Each agent must return a ## Derivation block naming: bar tokens applied, governing goal, behavioral dimensions. The orchestrator must preserve all Derivation blocks from every agent in the join result — do not strip or summarize them.\n")
			}
			joinDesc := step.Join
			switch step.Join {
			case "all":
