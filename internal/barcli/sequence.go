...
func runSequenceList(g *Grammar, asJSON bool, stdout, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "  autonomous  all steps run without stopping — autonomous sequences contain no requires_user_input steps; proceed to the next step in the same response turn immediately after completing each step's substantive response. A response turn satisfies autonomous mode for a non-final step only when it contains, in order: the literal line \"Step N complete.\" (where N is the current step number), then a bar build call whose token string exactly matches the token string shown for step N+1 in the sequence definition — a bar build call that does not contain that token string does not satisfy this requirement. Exception: dispatch steps suspend the response turn while background agents run; before proceeding, write the literal line \"Agents complete: N of N.\" (where N is the count of agents spawned) immediately after reproducing all agent Derivation blocks — this line is the transcript-observable that marks all agents returned; the bar build call for the step following the dispatch step must appear in the same response turn as that line, before any other content.\n")
	fmt.Fprintf(stdout, "  linear      pause after each step for user input before continuing — may contain requires_user_input steps that require real-world action\n")
...
func runSequenceShow(g *Grammar, name string, asJSON bool, stdout, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "Execution modes:\n")
	fmt.Fprintf(stdout, "  autonomous  all steps run without stopping — autonomous sequences contain no requires_user_input steps; proceed to the next step in the same response turn immediately after completing each step's substantive response. A response turn satisfies autonomous mode for a non-final step only when it contains, in order: the literal line \"Step N complete.\" (where N is the current step number), then a bar build call whose token string exactly matches the token string shown for step N+1 in the sequence definition — a bar build call that does not contain that token string does not satisfy this requirement. Exception: dispatch steps suspend the response turn while background agents run; before proceeding, write the literal line \"Agents complete: N of N.\" (where N is the count of agents spawned) immediately after reproducing all agent Derivation blocks — this line is the transcript-observable that marks all agents returned; the bar build call for the step following the dispatch step must appear in the same response turn as that line, before any other content.\n")
	fmt.Fprintf(stdout, "  linear      pause after each step for user input before continuing — linear sequences may contain requires_user_input steps that require real-world action by the user before the sequence can continue\n")
	fmt.Fprintf(stdout, "  cycle       repeat the step sequence until a stop condition is met\n")
	fmt.Fprintln(stdout)
	...
	for i, step := range seq.Steps {
		...
		if step.Type != "dispatch" && step.Type != "action" {
			...
			fmt.Fprintf(stdout, "          3. This step is complete only when a bar build tool-call result block for this step appears in the transcript — prose that mentions bar build, or a response written before that result block appears, does not satisfy this gate.\n")
		}
		if step.Type == "dispatch" {
...
func (g *Grammar) SequencesForToken(axisSlug string) []SequenceMembership {
	...
	for name, seq := range g.Sequences {
		for i, step := range seq.Steps {
			if step.Token == axisSlug || strings.HasSuffix(step.Token, " "+axisSlug) || strings.HasPrefix(step.Token, axisSlug+" ") || strings.Contains(step.Token, " "+axisSlug+" ") {
