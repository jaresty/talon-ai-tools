...
func runSequenceList(g *Grammar, asJSON bool, stdout, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "  autonomous  all steps run without stopping — no requires_user_input steps; proceed to the next step immediately without waiting for user input\n")
...
func runSequenceShow(g *Grammar, name string, asJSON bool, stdout, stderr io.Writer) int {
	...
	fmt.Fprintf(stdout, "Execution modes:\n")
	fmt.Fprintf(stdout, "  autonomous  all steps run without stopping — autonomous sequences contain no requires_user_input steps and no during_dispatch steps; proceed to the next step immediately after completing each step, without waiting for user input. A response turn that completes a non-final step without a bar build call for the next step does not satisfy autonomous mode.\n")
