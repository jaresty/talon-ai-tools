...
func TestHistoryHighlightRecentEntry(t *testing.T) {
	...
	m.now = func() time.Time { return base.Add(historyHighlightLifetime / 2) }
	...
	highlighted := composerTheme.historyActiveStyle.Render(entry)
	if !strings.Contains(section, highlighted) {
		t.Fatalf("expected recent history entry to include highlight style, got section:\n%s", section)
	}
}
...
func TestHistoryHighlightExpires(t *testing.T) {
	...
	m.now = func() time.Time { return base.Add(historyHighlightLifetime + 500*time.Millisecond) }
	section := m.renderHistorySection()
	entry := fmt.Sprintf("  • %s", formatHistoryEvent(m.paletteHistory[0]))
	highlighted := composerTheme.historyActiveStyle.Render(entry)
	if strings.Contains(section, highlighted) {
		t.Fatalf("expected highlight to expire for older entries, got section:\n%s", section)
	}
