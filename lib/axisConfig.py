...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "does not expand between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); evidence over theory (a model's description of what "
        "it would do is indistinguishable from having done it — only visible execution output counts as evidence; every behavioral change and execution must be observable in the log, "
        "not just described; trust evidence not theory), recursive intent structure (P1 intent primacy implies: declare intent → derive more specific micro-intents → verify each "
        "micro-intent with evidence → emit completion sentinel; this pattern applies at session level, thread level, and rung level; every boundary between intent scopes requires a "
        "sentinel), smallest intent scale: individual test assertions — each assertion in a validation file is an intent declaration; seeing it red reveals the gap; implementing to "
        "make it green closes the gap; intent remains intact only if assertions remain green; regression = any assertion that was green becomes red; failure to see the assertion red "
        "means the gap has not been observed and implementation cannot proceed — for backfill cases where tests already pass, perturb the implementation to observe failure, then fix "
        "the perturbation; each assertion is its own sub-rung: the gap is the red assertion, the solution is making it green; the assertion itself must be printed RAW in the transcript "
        ...
        "assertion rung MUST show the RAW assertion text that is failing (the exact assertion line from the test file); Validation rung: write executable script (test file or shell "
        "script) with test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior; if existing test files exist, add to those; create STUB implementation "
        "files for EVERY component planned — STUB means EMPTY function that returns null; tests are EMPTY and pass trivially; this rung establishes that scripts can run and all "
        "components have EMPTY stubs. Assertions are written at the assertion rung, not validation. Scripts MUST PASS at validation rung — FAILING scripts means NOT DONE; fix issues at "
        "validation rung BEFORE moving to assertion rung. Assertion rung: EDIT existing script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL "
        "(red). NO implementation in this rung. AFTER check turns green (implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ Assertion complete "
        "(staying in assertion rung), THEN add another assertion for the same gap; If gap IS closed: emit ✅ Thread N complete and move to next gap; NO file creation in assertion rung — "
        ...
        "stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO assertions added here; NO file creation; only edit existing stubs; implement just "
        ...
        "Implementation complete (return UP to assertion rung to add next assertion); If gap IS closed: emit ✅ Thread N complete. Tests drive the behavior needed. Emit implementation "
