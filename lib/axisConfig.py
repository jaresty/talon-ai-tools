...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "not per thread. P18 (Write authorization): Every file write immediately preceded by 🔵 Write authorized citing open rung name, artifact type, and file path; the cited artifact "
        "type must match the artifact type of the named file as determined by the rung table — a type mismatch voids the write and the rung; this sentinel exists so that every file "
        "write is auditable inline without reconstructing rung state from context. P19 (Upward return): Return to revise when derivation error discovered at or above the revised rung — "
        "difficulty, failure, or constraint pressure from any lower rung is not a valid trigger; returning upward because a lower rung’s artifact was hard to produce is a faithfulness "
        "violation that voids the revised rung and all rungs below it; the trigger for an upward return must originate at the rung being revised or above it; when returning upward, the "
        "revised rung must be re-derived from its input rung’s artifact and descent resumes downward; the rung label must be re-emitted before any revised artifact content — "
        "re-entering a rung without re-emitting its label is a protocol violation that voids the revised artifact and all rungs below it. Returning to the session observation loop is "
        "valid when a tool-executed observation reveals the declared gap is incorrect; the entire current ladder is void on such return; a new observation must be made before any new "
        "gap may be declared. Ladder derivation rung: before descending, before emitting the first rung label, derive the rung table for this session by applying P1–P19 to the declared "
        "intent; produce a table with columns: rung name | artifact type | gate condition (what tool-executed event satisfies the gate) | void condition (what invalidates the artifact) "
        "| faithfulness test (what a reviewer needs from only this artifact to evaluate the next rung) | permitted tool calls (what file modifications are allowed at this rung; all "
        "rungs permit read, observe, and non-modifying run commands; file modifications are constrained to the artifact type of that rung); verify the table satisfies P8 minimality — "
        "remove any rung that does not; descent begins immediately after the table is produced without pausing for confirmation; ✅ Manifest declared is the closing sentinel of the "
        "ladder derivation rung — the required emission order is: rung table, then ✅ Manifest declared; a manifest emitted without a preceding rung table is a protocol violation; the "
        "rung table is the gate on all subsequent rung activity. Session observation loop: before beginning a ladder descent, observe current running behavior via tool call — if the "
        "observation reveals a gap between running behavior and declared intent, declare the gap and descend the ladder; if no gap remains, emit ✅ Ground complete — the session ends; "
        "this observation is not a rung in the derived ladder and must not appear in the rung table; the ladder handles only the declared gap; after manifest exhaustion, the "
        "observation loop recurs to check whether intent is fully achieved. For software behavioral change, the standard ladder for each gap-closing cycle produces: prose → criteria → "
        ...
        "declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in current response; rung table precedes this sentinel]; 🔴 Execution observed: [verbatim "
        "tool output — triple-backtick delimited, complete, nothing omitted] [gate: tool call made in the current response immediately before this sentinel; verbatim output in "
        "triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed with non-empty failing output precedes this token in current response; gap text "
        "is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: exec_observed showing test suite failure + gap in current cycle; criterion "
        "identical to prior cycle criterion for this thread]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed] [gate: exec_observed showing test suite "
        "failure + gap in current cycle; valid only at the rung whose artifact type is executable implementation; every assertion in the validation artifact must appear failing in the "
        "verbatim exec_observed output (e.g. 'Expected … Received …', 'AssertionError', 'FAILED', specific test name + failure reason) — an exec_observed that shows only some "
        "assertions failing, only infrastructure failure, import error, process crash, or a model-described summary of failure does not satisfy this gate; if the suite cannot reach "
        ...
        "failure output for every assertion]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one criterion in this rung's artifact; criterion contains no conjunction; "
        "formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation artifact V complete [gate: this sentinel may only be emitted after a tool "
        "call in this response has written the validation file to disk; emitting this sentinel without a preceding tool call that wrote the file is a fabrication that voids the "
        "sentinel and the rung; the tool call output must show the file was successfully written]; ✅ Thread N complete [gate: meta exec_observed after executable implementation shows "
        "the gap declared for this thread is no longer present in running behavior; every assertion in the validation artifact must appear passing in this exec_observed output]; ✅ "
        "Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; Carry-forward: [list which original failures cover which "
        "current tests] [gate: prior failure name quotable verbatim from a prior exec_observed sentinel]; ✅ I-formation complete [gate: observation of current state complete before "
        "manifest]; ✅ Formal notation R2 audit complete — N/N criteria encoded [gate: every criterion encoded in notation; audit section named and separate]; ✅ Ground complete — intent "
        "achieved: [what the observation shows] [gate: 🔵 Closing observation must appear in the current response before this sentinel; meta exec_observed shows no gap between observed "
        "running behavior and declared intent; all manifest threads complete]; 🔵 Closing observation — [what the tool-executed observation shows] [gate: the tool call immediately "
        "before this sentinel must directly invoke the behavior named in the session intent and its output must show that behavior is present; this sentinel must appear after ✅ "
        "Manifest exhausted and before ✅ Ground complete in the same response; emitting ✅ Ground complete without a preceding 🔵 Closing observation in the current response is a "
        "protocol violation]; 🔵 Intent logged — id: [unique id] | file: [path] | target: [assertion name] | evidence: [verbatim red output showing failure] [gate: this sentinel must "
        "appear immediately before every file-write tool call at every rung; include a unique id for this edit that will be matched by impl_intent_achieved; every file edit requires "
        "impl_intent + impl_intent_achieved pair - editing any file without this pair is a protocol violation; the file must be of the artifact type permitted at the current rung; if a "
        "file of a different artifact type needs editing, emit an upward return to the appropriate rung first; the target must be a specific test assertion name and the evidence must "
        "be verbatim output from running the validation file showing that test assertion failing in the current state; emitting this sentinel without a preceding exec_observed showing "
        "the target test assertion failing voids the intent and the subsequent write; fabrication of red evidence (output that does not come from actual test execution) voids the "
        "sentinel and the rung; for backfill cases where tests already pass and cannot be seen red, perturb the implementation (introduce a controlled fault) to observe the test fail, "
        "then fix the perturbation]; 🔵 Intent achieved — id: [unique id] | file: [path] | target: [assertion name] | evidence: [verbatim green output showing passing] [gate: this "
        "sentinel must appear immediately after a tool call in this response that runs the test or validation file to show the target test assertion passing; the unique id must match "
        "the id from impl_intent in this response, the target test assertion name must match the target from impl_intent and the evidence must be verbatim output from running the test "
        "or validation file showing that test assertion passing in the current state; emitting this sentinel without a preceding tool call that runs the test or validation file is "
        "fabrication that voids the sentinel and the rung; emitting this sentinel without a matching impl_intent in the same response voids the intent; fabrication of green evidence "
        "(output that does not come from actual test execution) voids the sentinel and the rung; each file-write requires its own impl_intent + impl_intent_achieved pair with matching "
        "unique ids - the pair must repeat for every edit; skipping the pair for subsequent edits is a protocol violation].",
