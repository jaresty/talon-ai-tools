...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "appear before a valid '§2 check:' line in the transcript; (2b) derive formalization: '## Formalization' must not appear before '§2 dimensions closed' in the transcript; the ## "
        "Formalization section must contain at least one expression matching one of: a typed function signature, a logical predicate (∀/∃ notation), or a concrete input/output example "
        "(f(v) = w or equivalent); immediately after that expression, apply the ambiguity test: write '§ ambiguity test:' followed by one quoted phrase from the expression and whether "
        "it could be satisfied by two non-equivalent outputs; if yes, that phrase names the subject of the next rung and ## Formalization must not close; when no phrase in the "
        "expression admits two non-equivalent outputs, write 'alternative satisfier:' followed by a space and at least one non-whitespace character that is not '[' — a bracket "
        "character as the first non-whitespace character after 'alternative satisfier: ' signals an unfilled template and does not satisfy this requirement; the text names a concrete "
        "thing that the expression would admit but that the governing goal would reject; a formalization that cannot name such a satisfier is not yet precise enough to close; '§ "
        "formalization complete' must not appear before '§ ambiguity test:' in the transcript — a '§ formalization complete' that appears without a preceding '§ ambiguity test:' does "
        "not satisfy this requirement; '§ formalization complete' must not appear before a valid 'alternative satisfier:' line (one whose first non-whitespace character after "
        "'alternative satisfier: ' is not '[') in the transcript — a '§ formalization complete' with no preceding valid 'alternative satisfier:' line does not satisfy this requirement; "
        "write '§ formalization complete' only after both '§ ambiguity test:' and a valid 'alternative satisfier:' line have appeared; '## Enforcement sequence' must not appear before "
        "'§ formalization complete' in the transcript; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the transcript, "
        "and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a literal substring what the "
        "prior sub-heading's passing state admits that this sub-heading rejects; (4) verify coverage: '## Completion check' must not appear before '## Enforcement sequence' in the "
        "transcript and must not appear before '§ implementation permitted' in the transcript; each covered dimension cites a verbatim string that appears either as a substring of a "
        "tool-result block produced after '§ implementation permitted' and visible above '## Completion check', or, when no tool-result blocks are present after '§ implementation "
        "permitted', as a substring of the response text produced in this turn; when every covered dimension cites such a substring, write '§4 coverage verified'; '## Completion check' "
        ...
        "turn must continue to the next derived step rather than ending — the first non-blank line after the '## Completion check' heading must be 'Gate condition:'; a turn whose final "
        "non-blank content line is '§4 coverage verified' is exempt from the resume phrase requirement; when a turn is non-exempt, its final non-blank content line, after trimming "
        'trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates still apply" to proceed under the same protocol. (5) enumerate escape paths: \'## Path '
        "enumeration' must not appear before '## Enforcement sequence' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal "
        ...
        "appear before a valid '§5 check:' line in the transcript; any referent named in the '[text]' from §1 not already present as a literal string in the transcript above §1 must be "
        "fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted' followed by a space and a decimal integer "
        "(e.g. '§ implementation permitted 1') must appear in the transcript after '§5 enumeration complete'; a sentinel line containing the literal bracket characters '[' or ']' does "
        "not satisfy this requirement — only a decimal integer satisfies the index requirement; the integer must equal the 1-based ordinal count of file-modifying tool calls that will "
        "have appeared through and including the tool call immediately following this sentinel — '§ implementation permitted 1' must precede the first file-modifying tool call, '§ "
        ...
        "complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant message block in which it is written — a checker "
        "verifies this by locating the sentinel line, scanning all subsequent lines in the same assistant message block, and confirming every such line is blank (contains only "
        "whitespace or is empty); any non-blank line after the sentinel in the same assistant message block is a violation; no exceptions are permitted — if additional content must "
        "appear, it must precede the sentinel, not follow it. Continuation invariant: when the final non-blank content line of a turn contains or follows one of the rung-completion "
        "strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage verified', or '§5 enumeration complete', that turn must end with one of: '§ blocked: <text>', '§ "
        "awaiting: <text>', or '§ no-next-action: <text>', where '<text>' is non-empty. A turn whose final non-blank content line contains or follows one of these strings and does not "
        "end with one of the three exit strings does not satisfy ground.",
