...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the other way around. Bootstrap exception: when no tool-result block in the transcript before the Falsify derivation block contains the governed symbol name as a verbatim "
        ...
        "the named executor invocation — non-executor actions do not break immediacy. Artifact identifier must differ: the artifact identifier of the named governing artifact must "
        "differ from the artifact identifier of the artifact being modified. Semantically-equivalent refactor exception: (g) is also satisfied when an executor result block containing "
        "(a) before (c) appears in the transcript before the governed artifact-producing action, produced by a named executor invocation occurring in the current session after the "
        "last governed artifact-producing action, against a state where the refactored form was modified to violate the invariant the governing assertion checks — the perturbation "
        "result must appear before the revert action. Revert is complete when the pre-perturbation executor run result is quoted verbatim in the transcript before the revert executor "
        ...
        "derived exclusively from the executor-produced result entries (c), (d), and (g); the allow-list is derived from the full content of the (g) tool-result block named in the "
        "active Falsify derivation block; an identifier that does not appear as a verbatim substring of that tool-result block may not be introduced; test description lines, docstring "
        "echo lines, parametrize label lines, and summary lines in that block are valid allow-list sources — the restriction on these line types applies only to (c) derivation, not to "
        "allow-list sourcing; a construct whose introduction makes a reachable execution outcome possible that does not appear as a verbatim substring of (g) requires its own "
        "governing FAIL before it can be introduced, even if its identifier appears in (g) — the path constraint is satisfied only when every execution outcome the introduced "
        "construct can produce appears as a verbatim substring of the (g) tool-result block; during the creation-step exception, the allow-list is (d) only; a reference to an "
        "identifier not on the allow-list within a stub or partial implementation is not exempt; a construct whose complete identifier does not appear as a verbatim substring of the "
        "active allow-list entries must not be introduced in the same action and requires its own governing FAIL before it can be introduced. Scope-governing entries: only (c), (d), "
        "and allow-list identifiers from (g) are scope-governing entries — entries (a), (b), (e), (f), and (f2) are not scope-governing entries and do not contribute to the "
        "allow-list. Artifact gap: when the Falsify derivation block contains the line '(g) observed FAIL result: unavailable' and no tool-result block in this transcript produced by "
        "a named executor invocation contains (c) as a substring, the derivation is incomplete. A derivation is complete only when a line beginning 'Gap artifact:' appears in the "
        "transcript immediately above the Falsify derivation block, naming the executor invocation designated to produce a result containing (a) before (c). When 'Gap artifact:' is "
        "present, the tool call that produces that artifact satisfies the creation-step exception; no file-modifying tool call may appear before the tool call that produces the named "
        "artifact. A governed symbol renamed by a refactor is a new governed symbol requiring a new derivation block before the rename action. If no named executor exists for the "
        "governed artifact type, this token does not apply.",
