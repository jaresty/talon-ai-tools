...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "start. Protocol derivation: before ladder derivation, derive the complete protocol rules by applying P1-P6 — Session observation loop: when to observe vs descend; derive: "
        "Session observation loop (when to observe vs descend, what observation means vs does NOT mean), ladder derivation format (table columns, columns meaning), rung-specific "
        "behaviors (when to emit each sentinel, gate blocks sentinel emission until gate satisfied), upward return conditions (what triggers return, what does NOT trigger return and "
        ...
        "reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived "
        "are protocol violations; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: "
        "a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder: prose → criteria → formal notation → executable validation → "
        "executable implementation. EV rung: edits only validation files. EI rung: edits only implementation files. An executable validation artifact is a file whose sole purpose is to "
