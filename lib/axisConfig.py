...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ambiguity test:' and a valid 'alternative satisfier:' line have appeared; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§ properties "
        "complete' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a "
        "literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; when all sub-headings in '## Enforcement sequence' have been written, write "
        "'§ enforcement check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the sub-heading lines written above in this section — a '§ enforcement "
        "check:' line whose quoted phrase does not appear verbatim in a sub-heading line above it does not satisfy this requirement; then write '§ enforcement complete'; '§ enforcement "
        "complete' is valid only after a valid '§ enforcement check:' line has appeared; After '§ formalization complete', immediately write the label line 'Formalized properties:' — "
        ...
        "list, or f(v)=w pattern} present as a substring, does not satisfy this requirement; for each 'property [N]:' line, write one or more atomic sub-property lines of the form "
        "'property [Na]:', 'property [Nb]:', etc., each containing a formal expression — a sub-property line whose content is prose only does not satisfy this requirement; the "
        "conjunction of all sub-property expressions for a given 'property [N]:' must be logically equivalent to the expression in 'property [N]:' — a sub-property line introducing a "
        "symbol, predicate, or clause absent from 'property [N]:' does not satisfy this requirement; if the property is already atomic (no strict sub-expression exists that is "
        "independently falsifiable), before writing 'property [Na]: atomic — ', write '§ atomicity test: \"[quoted sub-expression]\"' where [quoted sub-expression] is a verbatim "
        "phrase from the parent 'property [N]:' expression, followed by a sentence naming why that sub-expression is not independently falsifiable from the remainder — a 'property "
        "[Na]: atomic —' line that does not have a '§ atomicity test:' line with a verbatim quoted phrase above it does not satisfy this requirement; 'property [Na]: atomic —' is valid "
        "only after '§ atomicity test:' has appeared; the restatement following 'atomic —' must use the same notation class as the parent 'property [N]:' expression — if the parent "
        "contains '∀' or '∃', the restatement must contain '∀' or '∃'; if the parent contains a typed function signature, the restatement must contain a typed function signature; if "
        "the parent contains 'f(' followed by ')=', the restatement must contain 'f(' followed by ')='; a restatement that changes notation class does not satisfy the same notation "
        "class requirement; when all 'property [N]:' and sub-property lines are written, write '§ properties check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring "
        ...
        "satisfy this requirement; then write '§ properties complete'; '§ properties complete' is valid only after a valid '§ properties check:' line has appeared; (4) verify coverage: "
        ...
        "escape paths: '## Path enumeration' must not appear before '§ enforcement complete' in the transcript; enumerate every path by which §1–§4 headings could be present without "
        "the governing goal identified, dimensions derived, or dimensions covered; for each path, name the literal string whose presence closes it; when no further open path remains, "
        "write '§5 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the escape path closing strings named in the enumeration above — a '§5 check:' "
        "line whose quoted phrase does not appear verbatim in the path enumeration above it does not satisfy this requirement; then write '§5 enumeration complete'; '§5 enumeration "
        "complete' must not appear before a valid '§5 check:' line in the transcript; '§5 enumeration complete' must not appear before '§ enforcement complete' in the transcript; any "
        "referent named in the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§5 enumeration "
        "complete' appears exactly once; the string '§ implementation permitted' followed by a space and a decimal integer (e.g. '§ implementation permitted 1') must appear in the "
        "transcript after '§5 enumeration complete'; a sentinel line containing the literal bracket characters '[' or ']' does not satisfy this requirement — only a decimal integer "
        "satisfies the index requirement; the integer must equal the 1-based ordinal count of file-modifying tool calls that will have appeared through and including the tool call "
        "immediately following this sentinel — '§ implementation permitted 1' must precede the first file-modifying tool call, '§ implementation permitted 2' must precede the second, "
        "and so on; a checker verifies this by scanning the transcript for every file-modifying tool call, locating the final non-blank line of the immediately preceding assistant "
        "message block, and confirming it matches '§ implementation permitted N' where N is the call's ordinal position; no file-modifying tool call may appear before the first such "
        "sentinel; the permit sentinel must appear as the final non-blank line of the assistant message block immediately before the assistant message block containing the "
        ...
        "toward compliance — an occurrence inside a code fence or on a line beginning with '>' does not satisfy any rung or ordering requirement. Rung-completion sentinel finality: a "
        "turn is defined as all content within a single assistant message block; a line is non-blank if it contains at least one non-whitespace character; each of the strings '§0 "
        "observed', '§1 goal derived', '§1a decomposed', '§1b candidates', '§2 dimensions closed', '§ formalization complete', '§ properties complete', '§ test suite complete', '§4 "
        "coverage verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant message block in which it is written — the sentinel line must begin "
        "with the literal first character of the sentinel string ('§' or '#'); a sentinel line beginning with '**' or any other markdown formatting character does not satisfy this "
        "requirement; a checker verifies this by locating the sentinel line, scanning all subsequent lines in the same assistant message block, and confirming every such line is blank "
        "(contains only whitespace or is empty); any non-blank line after the sentinel in the same assistant message block is a violation; no exceptions are permitted — if additional "
        "content must appear, it must precede the sentinel, not follow it. Completion check ordering: the '## Completion check' block must appear after '§5 enumeration complete' in the "
