...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "cleared is the next valid token; a green exec_observed at the VRO rung means there is no failing assertion — 🔴 Gap: may not be emitted after a green exec_observed; a green "
        ...
        "cite a failing assertion from the exec_observed output; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — "
        ...
        "and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion. A file read never "
        ...
        "directly invocable. Before any tool call at this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — the provenance statement "
        ...
        "acknowledged in the transcript with a written reason. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal "
        ...
        "displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read the prose rung output via a tool "
        "call to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry containing a "
        "verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the criteria rung, not the manifest; ✅ "
        "Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not "
        "been output has not begun and no artifact for it may exist. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without "
