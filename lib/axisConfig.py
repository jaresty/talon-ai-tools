...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "appear until a tool call exists in the transcript after the observed running behavior label in the current cycle; then immediately invoke the implemented artifact via a tool "
        "call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a "
        "non-technical observer of the running system would see — not the test suite, not process state unless the criterion is specifically about process state; a build output or "
        "compilation result never satisfies this gate regardless of what it names; prefer the most realistic invocation — for a web UI, start the dev server and emit the HTML response "
        "body (e.g., via curl or browser); for a CLI, invoke it directly and emit its output; in-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when "
        "no runnable artifact exists; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared "
        "behavior; a 🔴 Execution observed: block that is empty or blank, or contains only static-analysis output (file reads, grep results, directory listings), does not satisfy this "
        "gate at any rung — the block must contain output produced by running the artifact; if the declared gap names a real network endpoint, the OBS must include verbatim output "
        "showing a real HTTP request or response — a test run whose network calls are mocked does not satisfy this gate; before any tool call touching the filesystem at this rung, emit "
        "a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the rung label cannot be located in the transcript, the tool call is prohibited; the "
        "provenance statement does not replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: "
        "with non-empty verbatim output appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution "
        ...
        "test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — dismissing a failing test as unrelated "
        ...
        "validation and validation run observation to have fired for the current gap, including after an upward return. One edit per re-run cycle; after each edit re-run the validation "
        "artifact before any further edit. Upward returns follow the failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did "
        ...
        "intent is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before producing the final report. After emitting ✅ Manifest "
        ...
        "then reconcile any documents the implementation affects. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → "
        "executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ Manifest declared — N threads: [numbered list of behavioral "
        "gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🛑 HARD STOP — upward "
        "return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation artifact V complete; ✅ Thread N complete; ✅ Manifest "
        "exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current tests]; ✅ I-formation complete; ✅ Formal notation R2 audit complete — N/N "
        "criteria encoded.",
