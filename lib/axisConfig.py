...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "regardless of intent — a tool-result block in this transcript containing the FAIL is the only evidence that the artifact encodes the governed behavior. The response treats "
        "implementation derivation as constrained exclusively to observed governing failures — the only evidence that a governing artifact encodes the governed behavior is an "
        "executed-artifact result (a tool-result block in this transcript) in which (a) precedes (c) and the failure line contains a substring from the governed assertion call "
        "expression. The response establishes, before any governed artifact-producing action, that the named governing artifact has been observed to detect the absence of each "
        "governed behavior. A governed artifact-producing action is any action that produces or modifies a persistent artifact — including file writes, document edits, configuration "
        "changes, and policy commits — whose absence of a governed behavior can be detected by a named executor invocation; an action whose output does not persist beyond the current "
        "session, or for which no named executor exists, is not a governed artifact-producing action. A named governing artifact is one whose file path — the path passed as an "
        "argument to the executor invocation, not a package name or directory — appears as a literal string in the named executor invocation that produces a result from it; an inline "
        "inspection (grep, curl, read, shell pipeline, verbal check) whose output is not produced by invoking a named artifact does not satisfy this requirement regardless of whether "
        "it produces a failure result. If no governing artifact exists for a governed behavior, writing that artifact is the first governed artifact-producing action; the assertion "
        "text and the governed symbol must reside in separate artifacts — a governed artifact-producing action that produces both assertion text and the governed symbol in the same "
        "action does not satisfy the creation-step exception for either; (a) through (g) apply to the named executor invocation immediately following that creation — the creation "
        "action itself satisfies the creation-step exception below. Derivation block temporal gate: the literal string named in (c) must appear as a verbatim substring of at least one "
        "tool-result block before the Falsify derivation block in the transcript; a (c) value that does not appear as a verbatim substring of any tool-result block preceding it does "
        "not satisfy the derivation phase regardless of whether the derivation block is structurally complete. Derivation phase (required before any governed artifact-producing action "
        "— complete only when a literal block labeled 'Falsify derivation:' containing entries for (a) through (g) and (f2) appears in the transcript; prose does not satisfy this "
        "requirement): Name — (a) the assertion-failure marker: the literal string in an executed-artifact result signaling the artifact detected behavior absence — (a) must be a "
        "string that appears in executor output when the governed artifact is absent; specifically, a string that the executor produces when running the named executor against a state "
        "where the governed artifact has not yet been created; a string that requires the governed artifact to have executed in order to appear in the executor output is not a valid "
        "(a); (b) the literal string signaling the governed behavior is present; (c) the literal string identifying each governed behavior in an executed-artifact result — (c) must be "
        "derived from a direct callee of the assertion call expression as it appears on the FAIL line; a value derived from a test-identifier echo line, non-FAIL line, summary line, "
        "or test description in executor output is not a valid (c); the governed symbol must appear as a call expression reachable from the assertion call expression by following the "
        "execution call chain — the chain may pass through the test body, test setup, or the governed artifact; a callee whose name contains the governed symbol name as a substring "
        "but through which the governed symbol does not appear as a call expression in the reachable execution chain is a naming proxy and does not satisfy this requirement; a call "
        "expression that satisfies (c) must name a symbol whose callable definition does not appear in the content of any Write or Edit tool call whose path argument matches the "
        "governing artifact file path — if the identifier that appears as (c) on the failure line also appears as the name of a callable definition in the content of any Write or Edit "
        "tool call whose path argument matches the governing artifact, that identifier is a governing-artifact proxy and does not satisfy (c); (d) the governed symbol: the literal "
        "string naming the governed behavior — (d) must be derived from the direct callee of the assertion call expression in the governing artifact, where the call expression is not "
        "used only as an argument to a call expression whose callee does not contain the governed symbol name; (d) must be the identifier that appears as a substring of the failure "
        "line in (g) — not a symbol that contains or is associated with that identifier; if the failure line names a narrower identifier, that narrower identifier is (d) regardless of "
        "which broader symbol was invoked to reach it; (d) is derived from the failure line, not chosen independently — the failure line names (d), not the other way around. "
        "Assertion-target provenance exception: when the following conditions hold in the governing artifact's source text, the restriction that (d) must appear as a direct callee of "
        "the assertion call expression is suspended for this governed call: (1) an identifier is assigned exactly once in the governing artifact, and its initializer's outermost "
        "non-structural callee — after stripping any leading await and trailing property-access or index-access chains — is the governed symbol; Returned-object invocation provenance: "
        "condition (1) also permits a chain of single-assignment identifiers when each identifier in the chain satisfies conditions (1)–(3) with the immediately preceding identifier "
        "in the chain substituting for the governed symbol in condition (1) — the first identifier in the chain must have the governed symbol as its outermost non-structural callee; "
        "each subsequent identifier must have the immediately preceding chain identifier as the outermost non-structural callee of its initializer after stripping any leading await "
        "and trailing property-access or index-access chains; the same no-reassignment and structural-operations constraints from condition (3) apply to each intermediate identifier; "
        "(2) that identifier appears as an argument to an assertion call expression; (3) the identifier does not appear on the left-hand side of any assignment expression between its "
