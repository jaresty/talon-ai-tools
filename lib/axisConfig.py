...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "assertions that will fail; these assertions define the intent that implementation must satisfy. Create STUB implementation files so tests can RUN (imports resolve) but "
        "assertions FAIL because implementation is stubbed. Tests must PASS at validation rung (no failing assertions) — only infrastructure should be resolved. The assertions in "
        "validation rung tests are FIXED intent declarations — they cannot be changed in later rungs; implementation must make them green, not make tests match implementation. "
        "Assertion rung is per assertion: each individual assertion is its own rung. Implement ONLY enough to make THAT specific assertion green. Tests are IMMUTABLE once written at "
        "validation rung — implementation must satisfy tests, never modify tests to match implementation. If tests are wrong, return UP to validation rung to fix them; modifying tests "
        "in later rungs voids the rung. Infrastructure failures (import errors, missing files, syntax errors) are NOT assertion failures. If infrastructure fails, return UP to "
        "validation rung to fix the infrastructure issue before continuing. A test that cannot even START due to import error has NOT yet reached the assertion phase. The assertion "
        "rung only begins AFTER tests run successfully to the point of failing assertions - not import errors. ASSERTIONS are test code that verifies behavior, NOT import errors or "
        "infrastructure failures. An import error means the file doesn't exist - fix the file first, not the test. Only test assertions with expect() count as assertions. Rung "
        "completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; No implementation "
        "is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed sentinel must include the verbatim failing "
        "assertion text; Evidence before claims: for every claim about code being correct, there must be tool execution output proving it; describing what you would do is not evidence; "
        "ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: a rung is valid iff a "
        "human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → validation → "
        "assertion → implementation — the assertion rung is REQUIRED in this order; the assertion rung verifies each individual assertion red → green; emit the rung label before "
