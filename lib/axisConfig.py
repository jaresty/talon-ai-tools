...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "by applying P1-P6 — observe at every feasible rung before proceeding; Protocol derivation checklist: AFTER deriving rules from P1-P6, BEFORE proceeding, MAKE VISIBLE the "
        "derived rules in the conversation — show what you derived about file-edit justification, stub emptiness, validation-then-assertion order, rung sequence, and gap closure — the "
        "derivation results must be in the chat, not just in your head. This checklist must appear BEFORE the first ladder rung begins. derive: Sentinels from recursive intent "
        ...
        "when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return upward), rung completion order (each rung must produce its "
        ...
        "return to that rung rather than restarting from the top; continue descending from that rung to close the remaining gap; after all gaps are closed, return to the observation "
        "loop above the ladder to verify closure and emit Ground complete), observation loop corollary (if opening observation shows no gap exists, emit Ground complete immediately — "
        "no ladder descent needed), upward return conditions (what triggers return, what does NOT trigger return and why excluded), evidential scoping (evidence is scoped to current "
        "cycle and gap, not prior cycles), scope preservation (scope does not expand between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the "
        "rule is); evidence over theory (a model's description of what it would do is indistinguishable from having done it — only visible execution output counts as evidence; every "
        "behavioral change and execution must be observable in the log, not just described; trust evidence not theory), recursive intent structure (P1 intent primacy implies: declare "
        "intent → derive more specific micro-intents → verify each micro-intent with evidence → emit completion sentinel; this pattern applies at session level, thread level, and rung "
        "level; every boundary between intent scopes requires a sentinel), smallest intent scale: individual test assertions — each assertion in a validation file is an intent "
        "declaration; seeing it red reveals the gap; implementing to make it green closes the gap; intent remains intact only if assertions remain green; regression = any assertion "
        "that was green becomes red; failure to see the assertion red means the gap has not been observed and implementation cannot proceed — for backfill cases where tests already "
        "pass, perturb the implementation to observe failure, then fix the perturbation; each assertion is its own sub-rung: the gap is the red assertion, the solution is making it "
        "green; the assertion itself must be printed RAW in the transcript — only the surrounding test context may be paraphrased), each derived rule must cite its source principle(s) "
        "from P1-P6; rules that cannot be derived are protocol violations; Protocol derived MUST be emitted as the exact sentinel: 🔵 Protocol derived — and MUST be emitted AFTER "
        "deriving all protocol rules from P1-P6 — omitting derivation or emitting before derivation is complete voids the session; Ladder derived MUST be emitted as the exact sentinel: "
        "🔵 Ladder derived — and MUST be emitted AFTER producing the rung table — the rung table MUST have columns: rung name | artifact type | gate condition | void condition | "
        "faithfulness test | permitted tool calls; the table is NOT narrative text; No ladder descent (prose, criteria, etc.) is permitted before both 🔵 Protocol derived and 🔵 Ladder "
        "derived sentinels — these are gates on all subsequent work; After validation rung, you MUST descend to the assertion rung — validation shows tests exist, assertion rung shows "
        "which specific assertions are red; the assertion rung is NOT optional; the assertion rung MUST show the RAW assertion text that is failing (the exact assertion line from the "
        "test file); Validation rung: write test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior; create STUB implementation files for EVERY "
        "component planned — STUB means EMPTY function that returns null, like `export default function EditDialog() { return null; }`; STUB is NOT implementation — it is literally no "
        "code; tests are EMPTY and pass trivially because they have no code; this rung establishes that tests can run and all components have EMPTY stubs. Assertions are written at the "
        "assertion rung, not validation. Tests MUST PASS at validation rung — FAILING tests means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. Assertion "
        "rung: EDIT existing test file to add ONE specific expect() assertion; run tests to see it FAIL (red). AFTER making assertion green (implementation succeeded), CHECK if gap is "
        "closed — if gap still exists, add another assertion; if gap is closed, emit completion sentinel and move to next gap. NO file creation in assertion rung — stubs created at "
        "validation rung; only edit existing test file to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one "
        "rung at a time; completion before continuation. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make "
        "that specific assertion green — NO file creation in implementation rung, only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, "
        ...
        "the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, return UP to find next gap (ascend ladder); if gap is closed, "
        "emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. ASSERTIONS are test code that "
