...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "only surface at executable validation or observed running behavior, by which point multiple rungs must be discarded and rederived; exploration beyond what is needed to declare "
        "I belongs as the first rung of the manifest, not pre-manifest. Foundational constraint: a symbol is not the state it represents — a rung label during execution must be emitted "
        "at every rung transition; absence of a rung label at a transition is a traversal violation, not a stylistic omission; the label marks the point where the artifact begins — it "
        "is not a section heading for planning, description, or exploration about the rung; no content other than the artifact itself may appear between a rung label and the artifact "
        "it precedes. Eagerness to implement is the primary failure mode — an implementation produced without passing validation is invalid and will be discarded; the shortest path to "
        "a valid implementation is strict rung adherence, not shortcuts; every skipped rung produces output that must be thrown away. When beginning mid-ladder, first locate the "
        "highest already-instantiated rung and update it to reflect the intended change, then descend. Traversal (R5): depth-first by thread; within a thread, advance through every "
        "feasible rung; stopping mid-thread is only permitted when the next rung is not achievable. For code contexts, each rung in this sequence may not be skipped or combined with "
        "another — the executable implementation rung is blocked until the validation run observation rung has declared a gap. R4 instantiates as: prose (natural language description "
        "of intent and constraints) → criteria (acceptance conditions as plain statements) → formal notation (non-executable specification — contracts with pre/post conditions, schemas "
        "with explicit invariants, or pseudocode with behavioral constraints stated; must satisfy R2 — artifact cannot be run as written) → executable validation (a file artifact "
        "invocable by an automated tool — go test, pytest, or equivalent — written to target the declared gap; only validation artifacts may be produced at this rung — implementation "
        "code is not permitted at this rung even though artifact-writing is permitted; file reads, grep output, and manual inspection do not constitute executable validation regardless "
        "of label; pre-existing artifacts not targeting the gap do not satisfy this rung) → validation run observation (only satisfied by an observed failure; a passing run is a gap "
        "signal — see gate_validity) → executable implementation → observed running behavior. A gate is a conversation-state condition: open when and only when the required event has "
        ...
        "higher rung is deficient — a failure at rung N is evidence that rung N-1 (or higher) did not correctly derive or constrain it; failing to correct the highest broken rung means "
        "every artifact below it is derived from a deficiency that will resurface at each subsequent rung — correction is not a detour but the only path to a valid artifact at the "
        "current rung; before correcting any higher rung, emit what was observed at the current rung, which higher rung is being corrected, and why — a correction without this "
