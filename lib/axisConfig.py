...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "UnambiguousRequest | SplitDecision | SplitCandidate | Property | PlanningComplete. Type dispatch table — each type has exactly one permitted transition; any other transition "
        ...
        "Materialize. Property → SplitCheck. PlanningComplete → emit 'Planning complete.' Trace grammar — every state must have exactly this form, nothing else:   Active: "
        "<id>:<type>    <exactly one transition line for that type>    Pending: <id:type list, or 'empty'>  Any text between Pending: and the next Active: is a syntax error. A node may "
        "not be described, explained, or elaborated until after 'Planning complete.' A newly created node is inert until it appears in a future Active: line — a transition may never "
        "inspect or process a node it just created. Transitions: Interpret(R:Request): write 'Interpret: R → IP0'; add IP0:InterpretationPair. Expand(IP:InterpretationPair): write "
        "'Expand: IP → I1 / I2'; add I1:Interpretation, I2:Interpretation. Select(I:Interpretation): write 'Select: I → S'; add S:Selected.   (Select may also write 'Discard: <sibling> "
        "→ X' and add X:Discarded in the same transition line,    because the sibling's fate is determined by selection — this is the only permitted two-output exception.) "
        "Consume(X:Discarded): write 'Consume: X'; add nothing. GroundCheck(S:Selected): write 'GroundCheck: S → C'; add C:AmbiguityCheck. GroundResolve(C:AmbiguityCheck): if ambiguity "
        "remains, write 'GroundResolve: C → R\\'' and add R\\':Request; otherwise write 'GroundResolve: C → F' and add F:FormalismChoice. Formalize(F:FormalismChoice): write "
        "'Formalism: <language>' on one line (first-order predicate logic | set-builder notation | lambda calculus | typed function signatures); write 'Formalize: F → U' where U is the "
        "formal expression — no natural language; add U:UnambiguousRequest. SplitCheck(N:UnambiguousRequest or Property): write 'SplitCheck: N → D'; add D:SplitDecision. "
        "AnalyzeSplit(D:SplitDecision): if D's source expression splits into two independent sub-expressions E1/E2, write 'AnalyzeSplit: D → SC1 / SC2', then 'SC1.expression=<E1 "
        "verbatim>', then 'SC2.expression=<E2 verbatim>'; add SC1:SplitCandidate, SC2:SplitCandidate. Otherwise write 'AnalyzeSplit: D → terminal'; add nothing. "
        "Materialize(SC:SplitCandidate): write 'Materialize: SC → P'; write 'P.source=<SC id>'; add P:Property.   (P.source=<SC id> is the only valid content line for P — any following "
        "line before Pending: is a syntax error.) When Pending becomes empty: add PC:PlanningComplete. PlanningComplete: write 'Planning complete.'; add nothing. Task content and "
        "explanation of properties begin only after 'Planning complete.' Validity: every identifier introduced exactly once; every Active: names a node currently in Pending; every "
        "iteration emits exactly one Active:, one transition block, one Pending:; 'Planning complete.' emitted only by PlanningComplete; SC1.expression and SC2.expression are verbatim "
        "sub-expressions of D's source expression.",
