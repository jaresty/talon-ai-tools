...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "why Baseline establishes evaluation structure with minimal scaffolding; what GOES WRONG if Baseline IS the solution: no criteria structure exists to prove gaps; RULE: Baseline "
        "= evaluation structure (may include minimal scaffolding to hold criteria, but scaffolding is not solution) [ ] Structure before baseline: derive from P1,P2 — organization must "
        ...
        "Derivation: from criteria, derive evaluation structure. Evidence: evaluation structure produced and verified to work. IMPORTANT: Baseline establishes the STRUCTURE for "
        "evaluation - this may include minimal scaffolding to hold criteria. The scaffolding is NOT the solution - it's just where criteria live. IMPORTANT: Baseline evaluation must be "
        "SELF-CONTAINED. If it fails due to missing prerequisites, that's a broken framework, not evidence of gaps. Only NEW criteria added at Challenge can expose gaps in the "
        "solution. 5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, add NEW evaluation criteria that FAIL against the solution. Evidence: failing output from "
        "NEW criteria proves gaps exist, NOT failing because framework is broken. IMPORTANT: Work on the smallest verifiable thing at a time — each check must be satisfied before the "
        "next. 6. Refinement — How is the artifact improved? Derivation: from challenge, derive targeted changes. Evidence: addresses specific gaps. 7. Verification — How is completion "
        ...
        "is vacuous (Texas sharpshooter). Baseline defines evaluation framework; Challenge applies it to show gaps exist; Refinement produces solution that passes evaluation. for "
