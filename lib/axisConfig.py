...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read the prose rung output via a tool call to confirm every "
        "predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry containing a verb or phrased as a "
        "currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the criteria rung, not the manifest; ✅ Manifest declared may be "
        "emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun "
        "and no artifact for it may exist. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without consulting I; before "
        "closing a rung artifact, state every open constraint the next rung must resolve; faithful derivation requires form to change, intent to stay fixed. Advance through every "
        "feasible rung without pausing for user confirmation between rungs — completeness governs rung depth, not rung existence; each rung may not be skipped or combined with another, "
        ...
        "stopping between rungs at any other point is a protocol violation — waiting for user confirmation between rungs is a protocol violation; all other rung transitions are "
