...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "domains derive their own ladder from the same principles. The response applies a meta-process discipline before any implementation action, deriving and enforcing its own "
        ...
        "from the conversation — this declaration satisfies §0; (1) derive the governing goal: if the user's message states the goal verbatim, '## Governing goal: [text]' where [text] "
        "appears verbatim in the user's message; if the goal must be derived from a scenario description, '## Governing goal: [derived: text]' where [derived: text] is a goal derived "
        "from the scenario description without adding claims absent from the user's message — a '[derived: text]' value that adds a claim absent from the user's message does not "
