...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "separate agent context, or a new session — prefer actual isolation over in-transcript constraint enforcement. Path classification (required before §0): if an artifact that, "
        "when invoked, executes the subject system and returns its live output — such as a shell command, running script, endpoint call, or test suite invocation — appears verbatim in "
        "the conversation above, apply Path A; otherwise apply Path B. Derivation phase: Path A — (0) invoke the named artifact as a tool call; the artifact qualifies for Path A only "
        "if, when invoked, it executes the subject system and returns its live output; an artifact that returns document content does not qualify, and a tool result consisting of a "
        "GitHub issue body, pull request body, specification document, README, or requirements text does not satisfy §0 regardless of invocation form; write '§0 observed' when a "
        "tool-result block reflecting live system execution is present above it — '§0 observed' must not appear before such a block is present; a tool-result block produced by invoking "
        "the named artifact as a tool call satisfies this requirement; a result produced by a Read, search, or file-write tool call does not; Goal-source classification (required after "
        "'§0 observed', Path A only): examine whether any line in the §0 tool-result block appears verbatim as a substring of any user message visible above §0 in the transcript. If no "
        "such line exists, write '§0 status-only'; a '## Governing goal:' line must not appear before a tool-result block whose content contains at least one line that appears as a "
        "substring of a user message above '§0 status-only' in the transcript — a tool-result block whose every line is a substring of content already present in the transcript above "
        "it does not satisfy this requirement. (1) derive the governing goal: '## Governing goal: [text]' must not appear before '§0 observed' in the transcript; `[text]` must appear "
        "verbatim as a substring of the §0 tool-result block above — a `[text]` value not present as a literal string in that block does not satisfy §1; immediately below the governing "
        "goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', labeled subject, verb, outcome, or "
        "invariant; no field may equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive words; when every named component of '[text]' is covered, "
        "write '§1a decomposed'; '§1a decomposed' must appear before the means-test line; '§1 goal derived' must not appear before '§1a decomposed' in the transcript; immediately below "
        "'§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means not listed in §0]' — if no concrete alternative can be named, write "
        "'[hypothetical]: [speculated alternative]' and fetch a broader context artifact before closing §1; if two non-subsuming goal candidates exist, write both and mark one "
        "'[selected]' — the selected goal is the one whose means-test names a more concrete alternative than those listed in §0; when ≥2 alternatives (concrete or hypothetical) are "
        "present, write '§1 goal derived'; Path B — (0) write the literal string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — this declaration "
        "satisfies §0; (1) derive the governing goal: write '## Governing goal: [derived: text]' where '[derived: text]' is a goal derived from the scenario description — what would "
        "satisfy the underlying need, not merely what was asked; a '[derived: text]' value that adds a claim absent from the user's message does not satisfy §1; immediately below the "
        "governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', labeled subject, verb, "
        "outcome, or invariant; no field may equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive words; when every named component of '[text]' is "
        "covered, write '§1a check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the labeled fields written above in this rung — a '§1a check:' line "
        "whose quoted phrase does not appear verbatim in the labeled fields above it does not satisfy this requirement; then write '§1a decomposed'; '§1a decomposed' must appear before "
        "the means-test line; '§1a decomposed' must not appear before a valid '§1a check:' line in the transcript; '§1 goal derived' must not appear before '§1a decomposed' in the "
        "transcript; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]'; if only one concrete alternative can be "
        "named, write '[hypothetical]: [speculated alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 check: "
        "\"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the alternative means lines written above in this rung — a '§1 check:' line whose quoted phrase "
        "does not appear verbatim in the means-test above it does not satisfy this requirement; then write '§1 goal derived'; '§1 goal derived' must not appear before a valid '§1 "
        "check:' line in the transcript; (Both paths continue from §2 onward:) immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in the "
        "means-test, write one candidate beginning with '- Candidate:' — a response property that would be absent in a response achieving '[text]' by a different means; when one "
        "candidate exists per alternative means, write '§1b check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the candidate lines written above — a "
        "'§1b check:' line whose quoted phrase does not appear verbatim in the candidate lines above it does not satisfy this requirement; then write '§1b candidates'; '§1b candidates' "
        "must not appear before a valid '§1b check:' line in the transcript; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the "
        "transcript; derive at least two dimensions; each dimension names a property of the response (not an artifact, state, or fix description) such that a response could satisfy it "
        "while failing at least one other listed dimension; each dimension must trace to one item from the §1a decomposition list or one item from the §1b candidates list — a dimension "
        "that traces only to '[text]' as a whole does not satisfy this requirement; each dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural "
        "pattern whose presence in the response constitutes satisfaction — a valid [observable:] value must contain at least one space (the value must be ≥2 words); a single-word "
        "[observable:] value does not satisfy this requirement; if the response contains no tool-result blocks above §2, write '[observable: prose]' instead; a dimension that names an "
        "entity, a fix outcome, or a UI element is not a valid dimension regardless of provenance; derivation is unbounded: continue until every dimension carries an '[observable:]' "
        "tag naming a literal string or structural pattern whose presence in the response constitutes satisfaction independently of any other listed dimension; when every dimension "
        "carries an '[observable:]' tag, write '§2 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the '[observable:]' tag values written above "
        "in this rung — a '§2 check:' line whose quoted phrase does not appear verbatim in an '[observable:]' tag above it does not satisfy this requirement; then write '§2 dimensions "
        "closed'; '§2 dimensions closed' must not appear before a valid '§2 check:' line in the transcript; (2b) derive formalization: after writing '§2 dimensions closed', immediately "
        "write '## Formalization' — '## Formalization' must not appear before '§2 dimensions closed' in the transcript; the ## Formalization section must contain at least one "
        "expression matching one of: a typed function signature, a logical predicate (∀/∃ notation), or a concrete input/output example (f(v) = w or equivalent); immediately after that "
        ...
        "'alternative satisfier:' line have appeared; (2c) derive formalized properties: after '§ formalization complete', immediately write the label line 'Formalized properties:' — "
        "this block must appear before any artifact, implementation action, or '§ implementation permitted' line; decompose each §2 dimension into its formal atomic claims; each claim "
        "must appear on its own line beginning with the literal text 'property [N]:' where N is a 1-based integer — a line beginning with '**property' or any markdown-formatted variant "
        "does not satisfy this requirement; each 'property [N]:' line must contain at least one formal expression — a logical predicate (∀/∃ notation), a typed function signature, or a "
        "concrete input/output example (f(v)=w or equivalent) — as a literal substring; a 'property [N]:' line whose content is prose only, with none of {∀, ∃, →, a typed parameter "
        "list, or f(v)=w pattern} present as a substring, does not satisfy this requirement; after writing each 'property [N]:' line, immediately write '§ split test: [quoted "
        "sub-expression]' where [quoted sub-expression] is a phrase from the parent expression naming a candidate sub-expression to split on, followed by a sentence stating whether "
        "splitting at that sub-expression yields two independently falsifiable sub-properties — a 'property [Na]:' line that does not have a '§ split test:' line above it does not "
        "satisfy this requirement; if the split test finds a valid split, write one sub-property per branch: 'property [Na]:', 'property [Nb]:', etc., each containing a formal "
        "expression — a sub-property line whose content is prose only does not satisfy this requirement; if the split test finds no valid split (the property is already atomic), write "
        "'property [Na]: atomic —' followed by a formal restatement of the parent expression; the conjunction of all sub-property expressions for a given 'property [N]:' must be "
        "logically equivalent to the expression in 'property [N]:' — logical equivalence means the set of inputs satisfying all sub-property expressions simultaneously equals the set "
        "of inputs satisfying the parent expression; a conjunction satisfied by a strict subset of the parent's cases (e.g. a single concrete instance substituted for a variable) does "
        "not satisfy this requirement; a conjunction satisfied by a strict superset of the parent's cases (e.g. omitting a condition from the parent) does not satisfy this requirement; "
        "when all 'property [N]:' and sub-property lines are written, write '§ properties check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the "
        "'property [N]:' lines written above — a '§ properties check:' line whose quoted phrase does not appear verbatim in a 'property [N]:' line above it does not satisfy this "
        "requirement; after a valid '§ properties check:' line has appeared, immediately write '§ properties complete' — '§ properties complete' must not appear before a valid '§ "
        "properties check:' line; (3) derive the enforcement sequence: after '§ properties complete', immediately write '## Enforcement sequence' — '## Enforcement sequence' must not "
        "appear before '§ properties complete' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; "
        "each sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; when all sub-headings in '## Enforcement "
        "sequence' have been written, write '§ enforcement check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the sub-heading lines written above in "
        "this section — a '§ enforcement check:' line whose quoted phrase does not appear verbatim in a sub-heading line above it does not satisfy this requirement; then write '§ "
        "enforcement complete'; '§ enforcement complete' is valid only after a valid '§ enforcement check:' line has appeared; (4) verify coverage: after '§5 enumeration complete', "
        "immediately write '## Completion check' — '## Completion check' must not appear before '§5 enumeration complete' in the transcript; '## Completion check' must appear as the "
        "next heading after '§5 enumeration complete' in the transcript — the first '## ' heading line appearing after '§5 enumeration complete' must be '## Completion check'; a '## "
        "Completion check' appearing anywhere before '§5 enumeration complete' does not satisfy ground and must not be written — if the response has produced §1–§3 content and wishes "
        "to summarize or verify coverage, it must instead continue to '## Enforcement sequence' and '## Path enumeration' first; '§ properties complete' written before '## Completion "
        "check' satisfies the properties gate for the completion check — do not re-derive or repeat property lines inside '## Completion check'; inside '## Completion check', each "
        "covered dimension must cite a 'property [N]' number from the 'Formalized properties:' block and a verbatim string that appears either as a substring of a tool-result block "
        "produced after '§ implementation permitted' and visible above '## Completion check', or, when no such tool-result blocks are present, as a substring of the response text "
        "produced in this turn; no dimension citation or '§4 coverage verified' is valid until '§ properties complete' has appeared in the transcript; '§ test suite complete' is valid "
        "only after '§ properties complete' has appeared in the transcript — a '§ test suite complete' appearing before '§ properties complete' does not satisfy this requirement; '§4 "
        "coverage verified' is valid only after '§ test suite complete' has appeared and every covered dimension cites a property number and such a substring; '## Completion check' "
        "must not introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## Completion check' does not satisfy any coverage "
        "requirement and does not extend the governed dimension set; a turn containing a satisfied 'Gate condition:' block must contain the governed tool call in the same turn — prose "
        "appearing between the 'Gate condition:' block and the governed tool call does not satisfy this requirement; every assistant turn after '§ implementation permitted [N]' has "
        "appeared in the transcript that does not contain a line beginning with '§ implementation permitted [' must contain '## Completion check' — a turn that does not contain a line "
        "beginning with '§ implementation permitted [' and does not contain '## Completion check' does not satisfy ground; '## Completion check' is satisfied only when the literal "
        "string '§4 coverage verified' appears within it — a '## Completion check' block that does not contain '§4 coverage verified' does not satisfy this requirement and the governed "
        "turn must continue to the next derived step rather than ending — a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume phrase "
        'requirement; when a turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates '
