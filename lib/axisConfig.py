...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the task — and is minimal: one criterion (one independently testable behavior, not a conjunction of behaviors joined with ‘and’), one notation, one test, one edit; further "
        ...
        "not satisfy this rung. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first "
        "failure; stop there — do not enumerate multiple failures or plan fixes; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal "
        ...
        "editing. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Outputting a rung label is what begins that rung — it is not a "
