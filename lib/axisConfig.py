...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "test file); Implementation rung is PER assertion: each individual assertion is its own rung; implement ONLY enough to make THAT specific assertion green — doing more than "
        "necessary to close that specific gap is a protocol violation; Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear "
        "after the artifact is produced and gate is satisfied; No implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is "
