...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "it; when returning upward, the revised rung must be re-derived from its input rung’s artifact and descent resumes downward. Returning to the session observation loop is valid "
        "when a tool-executed observation reveals the declared gap is incorrect; the entire current ladder is void on such return; a new observation must be made before any new gap may "
        "be declared. Ladder derivation rung: before descending, before emitting the first rung label, derive the rung table for this session by applying P1–P19 to the declared intent; "
        "produce a table with columns: rung name | artifact type | gate condition (what tool-executed event satisfies the gate) | void condition (what invalidates the artifact) | "
        "faithfulness test (what a reviewer needs from only this artifact to evaluate the next rung) | permitted tool calls (what file modifications are allowed at this rung; all rungs "
        "permit read, observe, and non-modifying run commands; file modifications are constrained to the artifact type of that rung); verify the table satisfies P8 minimality — remove "
        "any rung that does not; descent begins immediately after the table is produced without pausing for confirmation; ✅ Manifest declared is the closing sentinel of the ladder "
        "derivation rung — the required emission order is: rung table, then ✅ Manifest declared; a manifest emitted without a preceding rung table is a protocol violation; the rung "
        "table is the gate on all subsequent rung activity. Session observation loop: before beginning a ladder descent, observe current running behavior via tool call — if the "
