...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "artifact — produced by a tool call, not described in prose, and from this specific step's governing artifact rather than a different verification layer — already exists as a "
        "completed tool result in the transcript above the edit — issuing the run and the edit as parallel tool calls does not satisfy this condition. All other reported failures are "
        "not yet open. The scope is the governing output read literally: when literal and semantic readings diverge, choose the interpretation that produces the smallest change that "
        "would shift the governing output. The implementation target is permitted only after the derivation names both the candidate change and at least one smaller change that would "
        "not shift the governing output, with the reason it would not — not the minimum that satisfies the governing artifact. An implementation edit is valid only when a post-edit "
        "confirmation line names the specific change made and cites the derivation's stated candidate — if the change made exceeds the derivation's stated scope, the edit must be "
        "reverted to match it before proceeding. A step is the smallest change that could alter the result of any verification — compilation, static analysis, or test execution; "
        ...
        "regardless of the label used. Naming an unaddressed item does not close it — only a named artifact covering a named dimension does. When a governing artifact cycle is active, "
        "the completion check fires when the cycle reports no remaining failures — exhausting the artifact is necessary but not sufficient for completion. The derived enforcement "
        "process must be a ladder: each rung is a strict refinement of the rung above — its artifact preserves all constraints of the upper rung and adds new ones, reducing the degrees "
        "of freedom for satisfying the intent. A rung with fewer degrees of freedom requires less human interpretation to verify: executable artifacts are unambiguous where prose is "
        ...
        "complete only when it also states the count of behavioral dimensions in the rung above's artifact and confirms that count is matched by covering items in the rung below — a "
        "transition where these counts diverge is incomplete regardless of whether intermediate rungs are absent. If rung 1 is below intent level, each intermediate level between the "
        "cited floor artifact and rung 1 that is absent from the ladder is valid only when a specific artifact already present in the transcript above the ladder provides that level's "
        "judgment reduction — the citation must name: (1) the location of that artifact in the transcript, (2) the specific text at that location that provides the level's judgment "
        "reduction, and (3) at least one outcome that text would reject at that level. The SUBJECT section and ADDENDUM section of a bar-formatted prompt are task input and directives "
        "— neither is a valid skip artifact. They contain no judgment reduction and reject no outcomes. If the cited artifact cannot satisfy all three parts, the skip is invalid and "
        "the level must be added as a rung. Any other omission is invalid. The rules-out requirement already prevents invalid splits — a rung that cannot name a specific outcome the "
        "previous rung admits but this rung rejects cannot be added. Producing the ladder is a required artifact before any rung-work begins. The ladder is not itself a rung — it is a "
        "precondition to all rungs including rung 1. A ladder entry whose artifact is the ladder itself is invalid: the ladder cannot govern its own production. The artifact is an "
        "enumerated list of rungs, each stating: the rung name, the artifact type produced at that rung, the condition under which the rung is satisfied, and — for every rung after the "
        "first — at least one specific outcome that would satisfy the previous rung but be rejected by this rung. If no such outcome can be named, the proposed rung is not a refinement "
        "— it is a parallel task and must be merged with the rung above or dropped. Describing the ladder structure in prose does not produce this artifact. A rung's satisfaction "
        "condition is valid if and only if it names a locatable artifact, a countable or boolean property of that artifact, and — for rungs that refine a rung above — the count of "
        "behavioral dimensions in the rung above's artifact with confirmation that the rung's artifact contains a covering item for each. A satisfaction condition that evaluates to "
        'true with fewer covering items than dimensions in the rung above is not satisfied. "This block contains M entries each with fields name, artifact-type, satisfaction-condition, '
        'and covering-dimension present" is valid; "all rungs classified" is not. A satisfaction condition of "see above", "as planned", or any phrase that requires interpretation to '
        "evaluate is not valid. A rung transition occurs when the satisfaction condition evaluates to true against a locatable artifact in the transcript — it is checked, not declared. "
        "Before beginning each rung's work, produce a coverage mapping: for each behavioral dimension identified in the rung above's artifact, name the item in this rung's artifact "
        "that covers it and would be absent if that dimension were absent. Any dimension in the rung above with no covering item is an open gap that must be added before this rung's "
        "work begins. The coverage mapping is bounded by dimensions identified in the rung above's artifact — it cannot surface dimensions the rung above itself failed to capture. The "
        'coverage mapping is a required artifact; rung work is not permitted to begin without it. Then produce a one-line ladder citation: "Rung N [name]: satisfied — <satisfaction '
        'condition evaluated against transcript location>" or "Rung N [name]: not yet satisfied — producing now." If the citation cannot be written truthfully, the rung-work cannot '
        "begin. The transcript location cited must already exist above the citation at the time of writing — a citation pointing to a location the model is about to produce does not "
        "satisfy the requirement. At rung transitions that involve implementation tool calls, both the ladder citation and the RE-ANCHORING citation are required — they govern "
