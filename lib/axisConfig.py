...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "check: if no 🔴 Gap: sentinel exists in output yet, emit it now; before emitting 🟢 Implementation gate cleared, check: does a 🔴 Execution observed: sentinel exist in "
        "conversation output? if not, do not emit it — produce the executable validation artifact instead. Before beginning each rung, emit 🔴 Gap: naming what the prior rung’s artifact "
        "leaves unresolved, then immediately produce the rung artifact — do not pause after the gap sentinel; only the validation run observation rung stops after naming the gap. From "
        ...
        "valid; “need to implement a landing page” is not; naming an absent artifact is not a valid gap. Each rung’s artifact addresses only that gap — not all known requirements of "
        "the task — and is minimal: one criterion (one independently testable behavior, not a conjunction of behaviors joined with ‘and’ — ‘the table renders Display Name’ and ‘the "
        "search bar filters results’ are two criteria, not one), one notation, one test, one edit; further artifacts emerge from subsequent gaps after each cycle completes. Formal "
        "notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only validation artifacts may be produced at the executable "
        "validation rung — implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must "
        "be invocable by an automated tool; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be newly produced for the declared gap; the "
        "validation artifact must fail before any implementation edit — a green run with no prior red run does not satisfy this rung. Before writing the validation run observation "
        "label, check: has a test file been written and its path stated? If not, write it now. At the validation run observation rung, run the validation artifact; emit 🔴 Execution "
        ...
        "cycle's gap, the edit did not address it — return to formal notation before any further edit. Before each edit, check: does a 🔴 Execution observed: sentinel exist for the "
        ...
        "writing the observed running behavior label, immediately run the tool and emit its output; then before emitting ✅ Thread N complete, check: does the declared intent have any "
        "behaviors not yet observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and begin the next cycle at the criteria rung. Outputting a rung label is what "
