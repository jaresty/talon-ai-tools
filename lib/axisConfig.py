...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "trust. P15 (Failure-first guard trust): each guard earns trust independently only after it has visibly failed under a controlled violation while active, then visibly passed "
        "under correction; failure/pass evidence for one guard does not transfer to adjacent guards, and observing only success weakens confidence in both guard and result.P16 "
        "(Observable-minimal gap): a gap exists only when the smallest externally observable behavior fails under applied evaluation; internal absence or unstated expectation does not "
        "constitute a valid gap. P17 (No guard–implementation interleaving): guards and implementation must change in separate rungs; no refinement may alter both simultaneously "
        "because hidden causality destroys trust in observed closure. P18 (Consequence Materialization Principle): When derivation identifies what goes wrong under violation, the "
        "protocol must immediately materialize that consequence as an active cost, invalidation, blockage, or trust reduction at the point of violation; a failure mode that produces no "
        "present consequence will not reliably shape behavior. P19 — Prerequisite Minimality Principle: When a challenge fails because a prerequisite artifact is absent, refinement may "
        "add only the smallest form necessary to allow the original guard to execute; prerequisite creation must not satisfy downstream behavior beyond restoring evaluability.Protocol "
        "derivation: before work begins, derive protocol rules by applying P1-P19 — observe at every feasible rung before proceeding. Protocol derivation checklist: AFTER deriving "
        "rules from P1-P19, BEFORE proceeding, MAKE VISIBLE in the conversation: [ ] The invariant: claim vs reality — what GOES WRONG if you claim without evidence: false completion; "
        "RULE: evidence before claims [ ] Optimization pressure: the model will seek shortcuts — what GOES WRONG if unaddressed: steps skipped, unverifiable results; RULE: make "
        "compliance easier than deviation and deviation detectable [ ] The ladder structure: 6 steps — Intent → Criteria → Structure → Challenge → Refinement → Verification; what GOES "
        "WRONG if skipped: gaps disappear, completion becomes false; RULE: one rung at a time [ ] Challenge: derive from P3,P5,P6,P8,P13 — instantiate a mechanism that detects gaps, "
        "APPLY it in the same layer as the artifact, and SHOW failure; a gap exists only when mechanism visibly fails [ ] Single demonstrated gap only: derive from P5,P12,P16 — select "
        "exactly one observed failure as the active gap; refinement targets one gap only [ ] Meta-loop required: derive from P3,P5,P6 — after EVERY Verification, Challenge runs again; "
        "loop until zero-gap evidence appears [ ] Zero-gap evidence required: derive from P3,P16 — absence of failure must be evidenced, never asserted [ ] Full intent coverage "
        "required: derive from P1,P4,P6 — every criterion must be evaluated at least once before completion [ ] Meta-loop continuation rule: derive from P5,P6 — if any criterion "
        "remains unevaluated, continue Challenge with new evaluation basis [ ] Structure before Challenge: derive from P1,P3 — without structure nothing exists to evaluate [ ] Criteria "
        "before Structure: derive from P1 — without criteria evaluation has no target [ ] Intent before Criteria: derive from P1 — without intent criteria lose meaning [ ] Evidence at "
        "proximity: derive from P3 — evidence must appear immediately after result [ ] Evidence echoed: derive from P3,P4 — explain how evidence proves compliance [ ] No contradiction "
        "in completion: derive from P3,P5 — no completion claim while known gaps remain [ ] Rule citation at action: derive from P1,P3,P4 — cite governing rule before action [ ] "
        "Guard–task separation enforced: derive from P9 — evaluation mechanisms cannot change during solution refinement [ ] Enforcement validity: derive from P8,P15 — every guard must "
        "fail visibly before pass is trusted [ ] Behavioral unit enforcement: derive from P12,P16 — checks target smallest observable behavior only [ ] Failure-first demonstration: "
        "derive from P15 — pass without prior visible fail weakens trust [ ] No guard–implementation interleaving: derive from P9,P17 — guards and implementation change in separate "
        "rungs only [ ] No anticipatory completion: derive from P14,P5 — do not add beyond what closes the active gap [ ] Incremental minimality required: derive from P14,P5 — "
        ...
        'P7,P8,P18 — every stated "what GOES WRONG" must create an immediate protocol consequence (artifact invalidation, rung rejection, guard replay, or trust downgrade); explanation '
        "without cost does not constrain behavior [ ] Prerequisite minimality: derive from P14,P16,P19 — if failure is prerequisite absence, refinement may only restore guard "
        "executability; do not implement downstream behavior during prerequisite repair [ ] File-instantiation minimality: derive from P12,P14,P16,P19 — if the active gap is file "
        "absence, refinement may create only the smallest valid file needed to restore the interrupted guard (e.g. export symbol / minimal stub) and must immediately rerun the same "
        "challenge before any behavioral implementation This checklist must appear BEFORE the first ladder rung begins. The derivation results must be in the chat, not only internal. "
