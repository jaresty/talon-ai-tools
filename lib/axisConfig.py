...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "re-emission is required: emitting the OBR label without immediately following it with the criterion text is a protocol violation; ✅ Thread N complete may not appear without "
        "both the criterion re-emission and a 🔴 Execution observed: in the current OBR section; then immediately invoke the implemented artifact via a tool call and emit 🔴 Execution "
        "observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a non-technical observer of "
        "the running system would see — not the test suite, not process state unless the criterion is specifically about process state; a build output or compilation result never "
        "satisfies this gate regardless of what it names; for a React or web UI component, invoke the render utility (e.g., renderToStaticMarkup or the testing framework’s render "
        "function reading container.innerHTML) and emit the resulting HTML fragment showing browser-visible content — starting a dev server does not satisfy this rung; if the "
        "implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared behavior; a 🔴 Execution observed: block "
        "that is empty or blank, or contains only static-analysis output (file reads, grep results, directory listings), does not satisfy this gate at any rung — the block must contain "
        "output produced by running the artifact; if the declared gap names a real network endpoint, the OBS must include verbatim output showing a real HTTP request or response — a "
        "test run whose network calls are mocked does not satisfy this gate; before any tool call touching the filesystem at this rung, emit a provenance statement: ‘Invoking "
        "[artifact] produced at [rung name] in this thread’ — if the rung label cannot be located in the transcript, the tool call is prohibited; the provenance statement does not "
        "replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: with non-empty verbatim output "
        "appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol "
        "violation; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label "
        "has been written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it that directly "
        "demonstrates the criterion — a test pass is not a demonstration; if the 🔴 Execution observed: at the observed running behavior rung contains test runner output (pass counts, "
        "test names, duration summary), it does not satisfy the OBR gate — re-invoke the implemented artifact directly before continuing; ✅ Thread N complete may not be emitted after "
        "an OBR 🔴 Execution observed: that shows test runner output; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already closed "
        "(return to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — dismissing a "
        "failing test as unrelated without an explicit written acknowledgment is a protocol violation; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: from a full "
        "test suite run exists in the transcript after the most recent OBR tool call in this thread — the only valid next action if no such result exists is the tool call that runs the "
