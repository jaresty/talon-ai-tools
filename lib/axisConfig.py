...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "belongs in formal notation instead; if the criterion contains the word ‘and’ it is a conjunction (‘the table renders Display Name’ and ‘the search bar filters results’ are two "
        "criteria, not one); a criterion that names both a structural element and its data source is a conjunction; asserting a column header is present does not satisfy this rung — "
        "add a criterion asserting its content or effect; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread "
        ...
        "an easier implementation is a protocol violation; each criterion must state (a) observable behavior and (b) a falsifying condition statable without source files, function "
        "names, or variable names — if part (b) references implementation internals, the criterion is structural and must be rewritten before this rung closes; this is a content gate, "
        "not a self-check; after emitting the criteria artifact, proceed immediately to formal notation — enumerating remaining criteria or planning future gap cycles before the "
        "current cycle completes is a protocol violation; the next criterion for this thread may not be named until ✅ Thread N complete has been emitted for the current criterion’s "
        "cycle — ✅ Thread N complete is the criterion-emergence gate; the first criteria rung after ✅ Manifest declared emits exactly one criterion for Thread 1 — emitting criteria for "
        "multiple threads at the first criteria rung is a protocol violation; the manifest declared the thread list; the criteria rung instantiates the first thread’s gap assertion "
        "only. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases; it must encode all structural constraints the "
        "criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe what notation looks like, not what it may omit; selecting a "
        "subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type signatures, interfaces, pseudocode, and structural invariants — the test "
        "of valid formal notation is that it cannot be directly compiled or executed without modification; any construct that compiles or runs as-is is implementation code and voids "
        "the rung; formal notation describes what must be true, not how to implement it — permitted: type signatures without bodies, interface shapes, structural invariants, "
        "pre/postconditions; prohibited: complete function bodies, variable assignments, constant declarations (const, let, var assignments) regardless of what they assign — the "
        "presence of an assignment operator outside a type definition indicates implementation code, working logic sequences; labeling prohibited content as pseudocode does not exempt "
        "it from this prohibition; if the criterion is already a complete behavioral specification — naming observable behavior and a falsifying condition without referencing function "
        "names, field names, or implementation internals — the formal notation artifact may restate it verbatim with an explicit declaration that no elaboration is needed; this is not "
        "a skipped rung; the artifact must appear in the transcript and the audit section still applies; the audit section is separate and named; the rung is incomplete until all "
        ...
        "type); before writing the test, re-read the formal notation and assert each structural constraint it encodes; a pre-existing test that happens to pass does not satisfy this "
        "rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must be invocable by an automated tool; when the declared intent is "
        "to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the validation artifact must be a meta-test — an executable artifact "
        "that fails when the test-under-modification does not exhibit the criterion; before writing the test, check for an existing test file covering the same module — add there if "
        ...
        "Before writing the validation run observation rung label, invoke the validation artifact via a test runner — file reads, grep, directory listings, and static analysis produce "
        ...
        "demonstrates the criterion — a test pass is not a demonstration; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already "
