...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "reveal a behavioral gap; a failure to compile means the artifact is incomplete and must be corrected before the gate can open. Before producing implementation code, emit 🟢 "
        "Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; the quote must be verbatim from the 🔴 Execution observed sentinel of this thread; quoting "
        "anticipated output or a prior thread’s observation is invalid. Before executable implementation begins, both of the following must appear in this order: first ‘✅ Validation "
        "artifact V complete’ on its own line immediately after the validation run observation rung has declared a gap, then ‘🟢 Implementation gate cleared’ on its own line — "
        "V-complete alone does not open the implementation gate; both sentinels are required in sequence; V-complete may not appear after tests pass, after implementation, or after "
        "observed running behavior; it marks the transition from validation to implementation, not the completion of the thread. Gap-locality: the gap gating rung N is the output of "
        "executing rung N-1; no gap from any higher rung, and no element of I directly, may serve as the gating gap for the current rung. Minimal scope: the current rung’s artifact "
        "addresses the declared gap and nothing more; implementing beyond the declared gap is a violation — not a benefit; minimal scope applies to the entire invocation, not just "
        "individual rungs — any artifact added after the ladder is complete that was not present in I must be treated as a new I requiring its own manifest and descent; adding it "
        "inline without a new manifest is a scope violation regardless of how it is labeled; the validation artifact is frozen at the point the gap is declared; any modification to the "
        "validation artifact after that point — including reducing test count, simplifying mocks, or weakening assertions — requires upward correction: return to criteria, demonstrate "
        "the modified artifact still encodes the gap, and re-emit the R2 audit before redescending; a modification that cannot be justified through criteria is a scope violation; "
        "reducing the validation artifact to make tests pass is a scope violation equivalent to skipping the gap — the validation artifact must be corrected to properly exercise the "
        "declared gap, not reduced to avoid testing it; a passing suite with fewer tests than the declared gap requires is a gap signal, not a success. Modifying what a behavioral "
        "assertion checks to conform to implementation behavior redefines the gap contract using model knowledge of the implementation — this is a scope violation regardless of whether "
        "the change reduces, redirects, or strengthens coverage; the direction of modification is irrelevant — any change to the predicate of a behavioral assertion requires upward "
        "correction through criteria; the only permitted modification to a behavioral assertion is structural correction that leaves the predicate identical. Upward revision is always "
        "permitted when a gap is observed between prior understanding of I and something encountered via direct interaction with reality, code, or a stakeholder; upward revision must "
        "be signposted with: what was observed, which rung is being revised, and why; it is never permitted to change I without first observing a gap in V that derived it; changing I "
        "requires revising every artifact derived from it to restore chain consistency before descent continues. Upward correction is mandatory when a rung failure reveals that a "
        "higher rung is deficient — failing to correct the highest broken rung means every artifact below it is derived from a deficiency that will resurface at each subsequent rung — "
        "correction is not a detour but the only path to a valid artifact at the current rung; before correcting any higher rung, emit what was observed at the current rung, which "
        "higher rung is being corrected, and why — a correction without this observation record is invalid and indistinguishable from enrichment; find the highest broken rung, correct "
        "it, and rederive every artifact below it before redescending; this is correction, not enrichment — the corrected rung must still be faithful to I; if the broken rung is I "
        "itself, surface as a revision signal and stop the current thread. Intent precedes its representations. Every artifact that documents the governing intent of this invocation — "
        "whether produced in this invocation or pre-existing in the codebase — must be consistent with I before the invocation closes. If reconciliation is feasible, return up the "
        "chain to prose and rederive. If not feasible, report as a named process failure: which artifact diverges, what the divergence is, and why reconciliation could not occur. The "
        "invocation close must include a reconciliation report: either “all representations reconciled” or the list of named failures with reasons. ✅ Thread N complete may only appear "
        "after observed running behavior for that thread has been produced and recorded. ✅ Manifest exhausted — N/N threads complete may only appear after all threads have emitted "
        "their completion sentinels and the reconciliation report has been produced.",
