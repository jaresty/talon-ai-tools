...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "form": {
        ...
        "quiz": "The response creates genuine retrieval demand for every concept by requiring the reader to commit to an explicit expectation before the answer is available. Before generating any "
        "question, name the concepts to be covered as a numbered list, then enumerate every named structural path by which a question could appear without creating genuine retrieval demand "
        ...
        "path (b) is closed globally by the Predict: requirement; paths (a) and (c) require a per-question check — before each question, write 'Path (a) check: [reason the phrasing does "
        "not reveal the answer]' and 'Path (c) check: [what the question demands that has not yet appeared in the transcript]'; proceed to writing the question only after both check "
        "strings appear above it. Before writing any question, name the concept that will carry the Misconception:/Why: pair in this quiz — this commitment must appear in the transcript "
        "before the first question is written; if no concept naturally carries a common misconception, choose the concept whose incorrect prediction would be most damaging. For each "
        "concept after the first, begin with a Following from: line quoting a specific phrase from the preceding answer, making explicit what the previous reveal established. Then write "
        "the question, require a Predict: line (the reader names their expectation before the answer is shown), then write the answer. After each answer: a gap-reveal sentence comparing "
        "what the Predict: line stated against what is correct or complete; then for every concept except the first and the last, a Hook: line quoting a specific phrase from the current "
        "answer and naming the implication it leaves unresolved — the question the next concept answers (evaluator check: the quoted phrase appears verbatim in the current answer); a Hook: "
        "line must not appear after the last concept's answer; then for at least one concept per quiz, a Misconception: line naming the specific incorrect belief that would lead a reader "
        "to predict the wrong answer, and a Why: line stating the structural reason that belief is wrong; then an association sentence — before writing it, name the numbers already used in "
        "prior association sentences — using the format '#N (Name): [relationship]' where N is the opening-list number of a prior concept whose number has not appeared in any earlier "
        "association sentence in this quiz and Name is that concept's name from the opening list (for the first concept, use the format '#N (Name): [relationship]' naming the opening-list "
        "number and name of the first concept whose understanding it enables). emit '✓ [concept name]' only when the Predict: line states the complete and correct answer with no "
        "correction, addition, or nuance needed — if any gap exists between the prediction and the answer, write a gap-reveal sentence instead of '✓'. Continue until every concept in the "
        "opening list has either a '✓ [concept name]' marker or a gap-reveal sentence in the transcript, or the user explicitly declines to continue; then close with a terminal declaration "
        "listing all confirmed concepts. With an output-exclusive channel, structure the output as a quiz document with each question immediately before its answer per topic.",
