...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "errors, missing files, syntax errors) are NOT assertion failures. If infrastructure fails, return UP to validation rung to fix the infrastructure issue before continuing. A "
        "test that cannot even START due to import error has NOT yet reached the assertion phase. The assertion rung only begins AFTER tests run successfully to the point of failing "
        "assertions - not import errors. ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. An import error means the file doesn't exist - "
        "fix the file first, not the test. Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; "
        "the sentinel must appear after the artifact is produced and gate is satisfied; No implementation is permitted before the assertion rung shows red assertions AND the specific "
        "RAW assertion text is displayed; the exec_observed sentinel must include the verbatim failing assertion text; Evidence before claims: for every claim about code being correct, "
