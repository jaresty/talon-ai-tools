...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation files so tests can RUN (imports resolve); include COMMENTS or formal notation describing the assertions that will be tested — do NOT include actual expect() "
        "assertions at this rung; assertions are written at the assertion rung, not validation. Tests should PASS at validation rung because implementations are stubs. Assertion rung "
        "is per assertion: each individual assertion is its own rung. Implement ONLY enough to make THAT specific assertion green. Tests are IMMUTABLE once written at validation rung — "
        "implementation must satisfy tests, never modify tests to match implementation. If tests are wrong, return UP to validation rung to fix them; modifying tests in later rungs "
        "voids the rung. Infrastructure failures (import errors, missing files, syntax errors) are NOT assertion failures. If infrastructure fails, return UP to validation rung to fix "
        "the infrastructure issue before continuing. A test that cannot even START due to import error has NOT yet reached the assertion phase. The assertion rung only begins AFTER "
        "tests run successfully to the point of failing assertions - not import errors. ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. "
        "An import error means the file doesn't exist - fix the file first, not the test. Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ "
        "[Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; No implementation is permitted before the "
        "assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed sentinel must include the verbatim failing assertion text; Evidence "
