...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "test file); Implementation rung scope: implement ONLY enough to make THAT specific assertion green — doing more than necessary to close that specific gap is a protocol "
        "violation; each assertion is its own sub-rung with its own gap; Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear "
