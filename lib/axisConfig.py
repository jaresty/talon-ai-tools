...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "creation in assertion rung — stubs created at validation rung; only edit existing test file to add assertions. Every file change requires justification: AFTER creating or "
        "editing any file, EXPLAIN WHY this change satisfies the requirements of the current rung; if you cannot justify why it satisfies the rung, ITERATE until it does; file changes "
        "without justification are protocol violations. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make "
