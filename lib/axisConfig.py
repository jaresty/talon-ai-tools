...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only validation artifacts may be produced at the executable validation rung — "
        "implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must be invocable by "
        "an automated tool; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be newly produced for the declared gap; the validation artifact must "
        "fail before any implementation edit — a green run with no prior red run does not satisfy this rung; if the artifact passes on its first run, perturb the implementation to "
        "force a red run — if the artifact still passes after perturbation, the test is vacuous and must be rewritten; if the artifact goes red after perturbation, restore the "
        "perturbation and return to the gap rung, as the declared gap names a behavior that already exists. ✅ Validation artifact V complete must be emitted at the executable "
        "validation rung before the validation run observation label is written. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with "
        "verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; if the observed gap matches the prior cycle's gap, the "
        "edit did not address it — return to formal notation before any further edit. Upward returns follow the failure class: if the implementation did not close the gap, loop within "
        ...
        "cannot be observed as described — return to the prose rung and update it; observation cannot be produced is a prose-description failure, not an implementation failure. Before "
        "each edit, check: does a 🔴 Execution observed: sentinel exist for the current gap? If not, run the validation artifact now before editing. One edit per re-run cycle; after "
        "each edit re-run the validation artifact before any further edit. Upon writing the observed running behavior label, immediately invoke a tool and emit 🔴 Execution observed: "
        "with its output — prose description of tool output does not satisfy this rung; implementation edits, new files, and code changes are not permitted at this rung — if "
        "observation reveals a defect, emit the gap and return upward. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written in the current "
        "cycle and a 🔴 Execution observed: sentinel appears immediately after it; once ✅ Thread N complete is emitted no further output in that cycle is valid. Before emitting ✅ Thread "
        "N complete, check: does the declared intent have any behaviors not yet observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and begin the next cycle at "
        "the criteria rung. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact "
        "for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every descent through "
        "executable implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward return. Reconciliation "
        "gate: at every upward rung transition — whether the scheduled formal-notation → executable-validation gate or any upward return triggered by new information — reconcile all "
        "artifacts at that rung against I before redescending; documentation is a reconciliation artifact, not a thread. After emitting ✅ Manifest exhausted, produce a final report "
        "containing prose, criteria, and formal notation for each thread in order; then reconcile any documents (README, specs, ADRs) the implementation affects — this reconciliation "
        "is part of the final report, not a separate thread. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable "
