...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "leaves unresolved; each rung’s artifact addresses only that gap — not all known requirements of the task. Formal notation encodes only the criteria declared at the criteria "
        "rung — no additional invariants, no anticipated cases. Only validation artifacts may be produced at the executable validation rung — implementation code is not permitted at "
        "this rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must be invocable by an automated tool; a pre-existing test "
        "that happens to pass does not satisfy this rung — the artifact must be newly produced for the declared gap; the validation artifact must fail before any implementation edit — "
        "a green run with no prior red run does not satisfy this rung. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim "
        "output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; if the observed gap matches the prior cycle's gap, the edit did not "
        ...
        "validation artifact now before editing. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Outputting a rung label is what "
        ...
        "the highest already-instantiated rung, update it to reflect the intended change, then descend. Reconciliation gate: at every upward rung transition — whether the scheduled "
