...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "type. Node types: Request | Interpretation | Selected | AmbiguityCheck | FormalismChoice | UnambiguousRequest | SplitDecision | SplitCandidate | Property | PlanningComplete. "
        "Type dispatch table — each type has exactly one permitted consumer transition; applying any other transition to a node is a validity violation: Request → Interpret only. "
        "Interpretation → Select only. Selected → GroundCheck only. AmbiguityCheck → GroundResolve only. FormalismChoice → Formalize only. UnambiguousRequest → SplitCheck only. "
        "SplitDecision → AnalyzeSplit only. SplitCandidate → MaterializeProperty only. Property → SplitCheck only. PlanningComplete → write 'Planning complete.' only. Execution loop: "
        "Pending := {R0:Request} where R0 is a Request node representing the task. Each iteration must write: 'Active: <id>:<type>' naming the single node removed from Pending, then "
        "exactly one transition determined by <type> from the dispatch table above, then 'Pending: <remaining id:type pairs or empty>'. Transitions: Interpret(R:Request) [→2]: write "
        "'Interpret: R → I1 / I2'; add I1:Interpretation, I2:Interpretation to Pending. Select(I1:Interpretation, I2:Interpretation) [→1]: write 'Select: I1 / I2 → S'; add S:Selected "
        "to Pending. GroundCheck(S:Selected) [→1]: write 'GroundCheck: S → C'; add C:AmbiguityCheck to Pending. GroundResolve(C:AmbiguityCheck) [→1]: if a second non-equivalent "
        "interpretation exists, write 'GroundResolve: C → R\\'' and add R\\':Request to Pending; otherwise write 'GroundResolve: C → F' and add F:FormalismChoice to Pending. "
        "Formalize(F:FormalismChoice) [→1]: write 'Formalism: <language>' choosing one (allowed: first-order predicate logic | set-builder notation | lambda calculus | typed function "
        "signatures); write 'Formalize: F → U' where U expresses the selected interpretation entirely in the chosen formalism — natural-language sentences are not valid U content; add "
        "U:UnambiguousRequest to Pending. SplitCheck(N: UnambiguousRequest or Property) [→1]: write 'SplitCheck: N → D' where D records D.source=N; add D:SplitDecision to Pending. "
        "AnalyzeSplit(D:SplitDecision) [→0 or 2]: examine the formal expression of D.source; if it contains two independent syntactic sub-expressions E1 and E2, write 'AnalyzeSplit: D "
        "→ SC1 / SC2' then on the next two lines write 'SC1.expression=<E1 verbatim>' and 'SC2.expression=<E2 verbatim>' — E1 and E2 must be quoted from the formal notation, not "
        "described in prose; add SC1:SplitCandidate, SC2:SplitCandidate to Pending; otherwise write 'AnalyzeSplit: D → terminal' — nothing added to Pending, D.source is consumed. "
        "MaterializeProperty(SC:SplitCandidate) [→1]: write 'MaterializeProperty: SC → P'; on the next line write 'P.source=<SC id>'; P.source is the sole content of P during planning "
        "— no paraphrase or explanation; add P:Property to Pending. When Pending is empty, add PC:PlanningComplete to Pending. PlanningComplete [→0]: write 'Planning complete.' — task "
        "content begins. After 'Planning complete.' the response may interpret properties by name. Validity: every identifier introduced exactly once; every identifier has exactly one "
        "creator transition and exactly one consumer transition per the dispatch table; every iteration writes exactly one 'Active:', one transition, one 'Pending:' line; 'Planning "
        "complete.' written only by PlanningComplete; SC1.expression and SC2.expression must be verbatim sub-expressions of D.source's formal notation.",
