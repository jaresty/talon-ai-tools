...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "confused with gaps; RULE: Baseline = fix whatever makes evaluation fail, do not proceed until it passes [ ] Structure before baseline: derive from P1,P2 — organization must "
        "precede artifact; what GOES WRONG if you create without structure: unfocused output; RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success "
        ...
        "derive from P3 — evidence must be temporally close to action; what GOES WRONG if delayed: proof weakens; RULE: evidence immediately after action [ ] Evidence echoed: derive "
        ...
        "next rung, verify and declare: [current rung artifact completed?], [evidence visible?], [gaps remaining?], [next rung requires?]. If gaps remain, do NOT proceed — close gap "
        "first. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P5. Ladder derived MUST be emitted as: 🔵 Ladder derived — AFTER producing the "
        "ladder table with columns: rung name | artifact type | derivation source | evidence form. No descent before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are "
        "gates on all subsequent work. The 7-rung ladder: 1. Intent — What outcome is being aimed for? Derivation: from task, derive specific outcome. Evidence: clear statement of "
        "goal. 2. Criteria — What conditions must be true for success? Derivation: from intent, derive conditions that verify success. Evidence: checklist or measurable conditions. 3. "
        "Structure — How is the solution organized? Derivation: from criteria, derive organization. Evidence: internal consistency. 4. Baseline — Does the evaluation framework exist? "
        "Derivation: from criteria, derive evaluation structure. Evidence: evaluation structure APPLIED and VERIFIED to work. IMPORTANT: Baseline establishes ALL evaluation mechanics "
        "AND may create minimal scaffolding to hold them. This scaffolding is NOT the solution - it's just enough to verify evaluation works. Baseline must PASS with its scaffolding. "
        "NO PROCEEDING without proof it works. IMPORTANT: Baseline creates MINIMAL scaffolding only - NOT the full solution. Full solution comes at Refinement. Baseline scaffolding is "
        "just enough for evaluation to run. IMPORTANT: The scaffolding at Baseline should be STUB-LEVEL - renders but doesn't implement functionality. Think placeholder, not working "
        ...
        "can expose gaps in the solution. 5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, add NEW evaluation criteria that FAIL against the solution. "
        "Evidence: failing output from NEW criteria proves gaps exist, NOT failing because framework is broken. IMPORTANT: Challenge must use SAME evaluation framework from Baseline - "
        ...
        "the next. 6. Refinement — How is the artifact improved? Derivation: from Challenge, address the SPECIFIC gaps that were OBSERVED. Evidence: changes target the exact gaps "
        "proven to exist. IMPORTANT: Refinement must address OBSERVED gaps only - gaps seen in Challenge output. Addressing unobserved gaps wastes effort and may miss real issues. 7. "
        "Verification — How is completion confirmed? Derivation: from refinement, derive confirmation. Evidence: no new gaps exposed. Emit rung labels to begin each rung; emit "
        "completion sentinels (✅ [Rung] complete) to finish each rung. Evidence before claims: for every claim about completion, there must be evidence proving it. Ground complete may "
        "only be emitted after verification confirms no gaps remain. Domain adaptation: for tasks where verification is possible, define evaluation scaffold BEFORE producing solution — "
        "evaluation defined after solution is vacuous (Texas sharpshooter). Baseline defines evaluation framework; Challenge applies it to show OBSERVED gaps; Refinement produces "
        "solution that addresses those OBSERVED gaps.for writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to "
