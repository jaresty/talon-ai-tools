...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "validation rung — no other content is permitted before ✅ Validation artifact V complete; each test function asserts exactly one behavioral property — one assertion per test "
        "function; multiple assertions are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the expected type); "
        "before writing the test, re-read the formal notation and assert each structural constraint it encodes; a pre-existing test that happens to pass does not satisfy this rung; "
