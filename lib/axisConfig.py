...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "content. Any deviation voids the sentinel; fix by re-invoking the tool and re-emitting the block. Once ✅ Ground entered has been emitted in this session, every subsequent "
        ...
        "active until the session ends or the user explicitly exits. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a "
        "narrative summary is a protocol violation. The gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not "
        "render” is valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be emitted unless "
        "a ✅ Thread N complete sentinel exists for each thread declared in ✅ Manifest declared — before emitting ✅ Manifest exhausted, locate the N in the ✅ Manifest declared sentinel "
        "and count ✅ Thread N complete sentinels — if the count does not equal the declared N, the manifest is not exhausted. one independently testable behavior derived from the prose "
        "alone — if stating it requires names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is a conjunction — "
        "split before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a structural element and "
        "its data source is a conjunction — split before continuing; asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect; "
        "before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a citable "
        "manifest excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; "
        "each criterion states (a) one observable behavior and (b) a falsifying condition referencing no implementation internals (no file names, function names, or variable names); "
        "this is a content gate, not a self-check; exactly one criterion may be written per thread per cycle — after the first criterion is written, the only valid next token is the "
        "formal notation rung label; a second criterion for the same thread is a protocol violation — stop, return to the prose rung, and split the thread before descending again. The "
        "formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be "
        "true — the executable or testable part. Natural language labels, introduces, and explains the specification — prose adds context but the behavioral constraints must still be "
        "encoded in notation.it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) "
        "describe what notation looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type "
        "signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is that it cannot be directly compiled or executed without modification; any "
        "construct that compiles or runs as-is is implementation code and voids the rung; formal notation describes what must be true, not how to implement it — permitted: type "
        "signatures without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: complete function bodies, variable assignments, constant declarations "
        "(const, let, var assignments) regardless of what they assign — the presence of an assignment operator outside a type definition indicates implementation code, working logic "
        "sequences; labeling prohibited content as pseudocode does not exempt it from this prohibition; if the criterion is already a complete behavioral specification — naming "
        "observable behavior and a falsifying condition without referencing function names, field names, or implementation internals — the formal notation artifact may restate it "
        "verbatim with an explicit declaration that no elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript and the audit section still "
        ...
        "test-under-modification does not exhibit the criterion; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is "
        ...
        "re-statement, 🟢 Implementation gate cleared is the next valid token; a green exec_observed at the VRO rung means there is no failing assertion — 🔴 Gap: may not be emitted "
        "after a green exec_observed; a green exec_observed without a prior red run means the test is vacuous — return to the executable validation rung and rewrite the test to fail; a "
        "valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed output; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only "
        "valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation run observation rung "
        "in the current cycle — emitting it at any other position is a protocol violation; prose-only exec_observed sentinel does not satisfy this gate — the exec_observed block must "
        "contain non-empty verbatim tool output in a triple-backtick block; HARD STOP may not be emitted at the executable validation rung — a harness error at EV requires fixing the "
        "harness, not an upward return; When 🔴 Execution observed: at the executable validation rung shows a harness error (import failure, syntax error, missing file), the only valid "
        "next token is a tool call that repairs the harness — not a gap statement, not HARD STOP, not a rung label; HARD STOP may not appear in the same response as a harness-error "
        ...
        "does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread; the prose "
        "re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running behavior label, re-emit the criterion for this cycle — criterion "
        "re-emission is required: emitting the OBR label without immediately following it with the criterion text is a protocol violation; the tool call is the only valid next action "
        "after criterion re-emission; planning statements, diagnostic narration, and commentary between criterion re-emission and the tool call are protocol violations; ✅ Thread N "
        "complete may not appear until a tool call exists in the transcript after the observed running behavior label in the current cycle; then immediately invoke the implementation "
        "as a live running process via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named "
        "in the criterion. A file read never satisfies this rung — the block must contain output produced by running the artifact. If the artifact requires a start step before querying "
        ...
        "artifact directly invocable. Before any tool call at this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — the provenance "
        ...
        "explicitly acknowledged in the transcript with a written reason. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, "
        "formal notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared gap — what the prior rung’s artifact "
        "leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared is the first "
        "token of the executable implementation rung — no tool call, no prose, and no file modification may appear before it in the current response; emitting a file-write tool call "
        "before 🟢 Implementation gate cleared is a protocol violation regardless of whether impl_gate is emitted later in the same response. it does not complete the thread; after "
        "emitting it, the executable implementation rung, the observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread N complete is "
        "valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may not appear until the observed running behavior "
        "rung has fired for this thread. Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed inline by "
        "a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly "
        ...
        "separate thread; re-read the prose rung output via a tool call to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, "
        "not a behavioral assertion; a manifest entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the "
        "first time at the criteria rung, not the manifest; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not "
        ...
        "artifact must be self-sufficient to derive the next without consulting I; before closing a rung artifact, state every open constraint the next rung must resolve; faithful "
        "derivation requires form to change, intent to stay fixed. Advance through every feasible rung without pausing for user confirmation between rungs — completeness governs rung "
        "depth, not rung existence; each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle; the only "
        ...
        "resolved is a final report regardless of its heading; Such prose may not appear until ✅ Manifest exhausted exists in the transcript for this invocation; if no ✅ Manifest "
        ...
        "the implementation affects. Rung-entry gate: before producing content at any rung, state (a) the rung name, (b) the current gap as a currently-false behavioral assertion, (c) "
        "the artifact type this rung requires, and (d) whether a 🔴 Execution observed: sentinel exists in the current cycle and, if so, whether its output is of type (c) — if no "
        "exec_observed exists yet in this cycle, state that explicitly rather than treating prior-cycle output as satisfying this check; if any of (a)–(d) cannot be stated from the "
        "current-cycle transcript, produce it before any other content at this rung; if (d) reveals that no valid exec_observed exists in the current cycle, the only permitted next "
        "token is a tool call — producing any other content when (d) is false is a protocol violation. Seven rungs in order: prose → criteria → formal notation → executable validation "
        ...
        "notation R2 audit complete — N/N criteria encoded.",
