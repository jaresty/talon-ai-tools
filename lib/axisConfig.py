...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "produced tool-executed run result — a run result is immediately preceding a tool call when no other tool-executed run result appears between that result and the tool call; "
        "non-run tool calls do not break this recency. Derivation phase (required before the first file-modifying tool call): name the root criterion — before each file-modifying tool "
        "call, the scope commitment text must appear as a quoted string above that call, where the quoted scope text must be a substring of the immediately preceding tool-executed run "
        "result (scope commitment); every symbol the call will add or modify must be named above that call, derived from the same run result that produced the scope commitment (symbol "
        "commitment); each named symbol must appear as a literal substring of the quoted scope text or be named in the failing-item line from the most recent tool-executed run result — "
        "a symbol not present in either does not satisfy this requirement; the quoted scope text must be from a test assertion whose pass/fail state is determined solely by the symbols "
        "named in the symbol commitment — a test that passes or fails independently of those symbols does not satisfy this requirement; a statement in the derivation phase is a "
        "commitment only if its truth value can be determined from tool-executed results already present above it in the transcript — a statement whose truth value requires evaluating "
        "a result not yet present is not a commitment; after the call, a tool-executed run result must appear in the transcript in which the quoted scope text is absent, every other "
        "line from the immediately preceding pre-edit run result is present, and a result line appears for every test name that appeared in that pre-edit run result — the pre-edit run "
        "result for a given call is the run result that produced that call's scope commitment; a failing-item line from the pre-edit run result may be absent from the post-call run "
        "result only if its literal text appeared as a quoted scope string above this call and no other failing-item line from the same pre-edit run result also appeared as a quoted "
        "scope string above this call. Then enumerate the following four escape categories — each must have a named closure string in the transcript before any file-modifying tool "
        "call: (i) scope inflation: name the string that prevents the quoted scope text from covering more than one independently testable symbol; (ii) stale quote: name the string "
        "that confirms no tool-executed run result intervenes between the source of the quoted scope text and this call; (iii) symbol under-declaration: name the string that confirms "
        "no symbol modified by this call is absent from the symbol commitment; (iv) post-call line mismatch: name the string whose absence from the post-call result confirms the scope "
        "text was resolved and no other line was inadvertently removed. Proceed to file-modifying tool calls only when the derivation and all four enumeration categories are present as "
        "text above this point in the transcript.",
        ...
        "ground": "The response applies a meta-process discipline before any implementation action. Derivation phase (required before any file-modifying tool call): name the root criterion — (0) "
        "before naming the governing goal, run the subject under observation and record its output under a literal heading — the governing goal heading must contain as a literal "
        "substring a string present verbatim in that tool-executed result; a governing goal heading with no such substring has not been derived from observed state and does not satisfy "
        "this requirement; for non-software subjects, observing the subject as it currently stands satisfies the §0 run requirement — record what is observed under a literal heading "
        "and derive the governing goal from a literal substring of that observation; (1) the governing goal must appear under a literal heading in the transcript before any "
        "implementation action, identified as the goal no other stated goal subsumes; (2) behavioral dimensions must appear under a literal heading, each naming as a literal substring "
        "the specific clause in the governing goal it derives from; (3) each artifact in the enforcement sequence must appear under a literal heading naming, as a literal substring, "
        "what the prior artifact's passing state admits that this artifact rejects; (4) each behavioral dimension must be covered by a verbatim string from a tool-executed result "
        "appearing under a literal completion-check heading; (5) enumerate every path by which a transcript could satisfy the §1, §2, §3, and §4 heading requirements without the "
        "governing goal having actually been identified, dimensions actually derived, or dimensions actually covered — the enumeration must include at least one path entry for each of "
        "§1, §2, §3, and §4; for each path, name the literal string in the transcript that closes it — a path is closed only when naming that string makes the non-compliant transcript "
        "distinguishable from the compliant one without assessing intent; a path that cannot be closed by naming a string must be eliminated by ensuring the required structural "
        "property is present before the heading is written; no file-modifying tool call may appear before this enumeration is present in the transcript; the literal strings named "
        "during enumeration as path closures are binding — a heading or structural property that does not match the committed string verbatim does not satisfy the corresponding "
        "requirement. Proceed to implementation only when §0–§5 are present as text above this point in the transcript. Any noun phrase in the governing goal that names a file path, "
        "URL, ticket number, or document title not already present in the transcript must be fetched before the governing goal heading is written — derivation is blocked until the "
        "referent is resolved. No file-modifying tool call may appear before the completion-check heading. The response is not permitted to end until the completion-check heading "
        "appears in the transcript and every covered dimension must cite a verbatim string from a tool-executed result whose invocation names the subject under observation directly — a "
        "tool-executed result whose invocation names an artifact that evaluates or reports on the subject rather than the subject itself does not satisfy this requirement.",
