...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "artifact whose modification satisfies property [N]; depending on the property, this may be the implementation, the regression guard, a configuration file, or another "
        "explicitly governed artifact; modify the governed artifact to satisfy property [N], then immediately emit 'Quoted implementation: <smallest contiguous verbatim text uniquely "
        "identifying the modification to the governed artifact>'; if step (4) required constructing a temporary violating state because the governed artifact already satisfied "
        "property [N], step (5) is satisfied by confirming the original state is restored — emit 'Quoted implementation: <verbatim text identifying the restored governed artifact>' to "
        "confirm; no subsequent step may occur until this sentinel has been emitted; (6) attempt to construct a minimal modification to the quoted implementation that preserves the "
        "success of the quoted guard while still changing the governed artifact, and attempt to construct a concrete execution path through the quoted implementation that is not "
        ...
        "retained property, and for every existing property whose quoted implementation or quoted guard is affected by the new properties repeat steps (5) and (6) before resuming the "
        "audit; repeat until a full audit pass emits zero gaps, then emit 'Audit: implementation complete'; 'Coverage: complete' may not be emitted before 'Audit: implementation "
        "complete'. A response that omits any required sentinel, performs a step before emitting its required quoted artifact, modifies the governed artifact to satisfy property [N] "
        "before a quoted Failure line appears, omits any executable assertion from the assertion inventory, or leaves a property without an automatic regression guard does not satisfy "
        "this token.",
        ...
        "out-of-scope constraints, including internal mechanism constraints not explicitly required by the chosen interpretation. When all conditions are met, emit '§ ground complete' "
        "as the final line of the Ground properties block — this sentinel may not appear before '§ properties complete? yes' in the transcript. '§ ground complete' is a phase-boundary "
        "sentinel, not a response-termination condition. After emitting '§ ground complete', continue with the requested task using the retained atomic properties as the governing "
        "constraints for all subsequent task reasoning, tests, implementation steps, and tool calls. The response must not terminate solely because '§ ground complete' has been "
        "emitted. All subsequent task work must remain consistent with the completed Ground properties; the Ground properties block is not itself the requested task unless the original "
        "request explicitly asks only for property derivation. Any test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does not satisfy this "
        "protocol; if task reasoning begins and '§ ground complete' has not yet appeared, emit the Ground properties block in full and '§ ground complete' before continuing. After '§ "
        "ground complete', continue with the requested task; do not treat the sentinel as terminating the response.",
