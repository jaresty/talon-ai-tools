...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "interpretation requires. Step 2 — Introduce provisional sub-properties. Introduce provisional candidate sub-properties whose conjunction is proposed to represent the current "
        "property: 'property [Na]: <definition>' and 'property [Nb]: <definition>'. Apply the canonicalization procedure (Step 1) to each provisional sub-property before proceeding. "
        "The resulting canonical definitions replace the provisional definitions for all subsequent split evaluation. The labels [Na] and [Nb] continue to identify the resulting "
        "canonical definitions after replacement. Split evidence is attached to the canonical definitions of the current property and its provisional sub-properties, and to the "
        "falsifying constructions emitted in the split test, at the moment the split test is emitted. Any modification to any of those definitions or constructions after the split test "
        "is emitted voids that evidence; the split evidence must then be regenerated against the current canonical state before the split may be accepted. Step 3 — Evaluate split "
        "validity. Before evaluating the split, verify that the chosen interpretation determines an accessible-observation boundary: the set of outputs, behaviors, states, and "
        "interactions that the party responsible for determining whether the request was satisfied can access without inspecting the implementation. If the chosen interpretation does "
        "not determine that boundary, emit 'Interpretation refinement required: <what the boundary must specify>' and emit or refine the Interpretation: line to supply it. An "
        "interpretation refinement voids the entire current retained-property tree, all previously derived properties, and all split evidence, and restarts property derivation and "
        "recursive decomposition from the request constraints under the refined interpretation; any previously emitted split test is void and does not constitute a completed split test "
        "for the current canonical state. After the accessible-observation boundary has been established and the current property, its provisional sub-properties, and their split "
        "evidence have been regenerated under the current interpretation, emit exactly one non-void candidate split test for the current canonical state: '§ split test: property [N]: "
        '"<verbatim canonical definition>" ⇔ (property [Na]: "<verbatim canonical definition>" ∧ property [Nb]: "<verbatim canonical definition>"); property [Na] — falsified by '
        "<construction>; property [Nb] — falsified by <construction>; these falsifiers are independent.' The quoted definitions for [N], [Na], and [Nb] must match the verbatim "
        "canonical definitions confirmed in Step 1 for each respective property. A split may be accepted only if its split test, quoted definitions, falsifying constructions, and "
        "interpretation are identical to the non-void state against which the split test was emitted. A split succeeds only if: for every admissible construction x, P(x) ⇔ (PNa(x) ∧ "
        "PNb(x)) — the original property is logically equivalent to the conjunction of the accepted sub-properties over all admissible constructions; each quoted falsifier violates "
        "exactly its quoted property while satisfying the other; both falsifiers are admissible constructions within the chosen interpretation; the candidate split is nontrivial: "
        "neither accepted sub-property is logically equivalent to the original property over all admissible constructions, and neither accepted sub-property logically entails the other "
        "over all admissible constructions; each accepted sub-property corresponds to a distinct constraint on the admissible construction under the chosen interpretation that is "
        "independently required by the explicit request constraints, and the request itself distinguishes the two sub-constraints at its own granularity — meaning the request "
        "separately names, or would be independently judged as failing on, each sub-constraint on its own; a candidate split is not valid merely because it decomposes the mathematical "
        "or definitional structure of a predicate, is not valid merely because each sub-property is independently testable, assertable, or useful as verification evidence, and is not "
        "valid merely because the two sub-constraints are logically distinct when the request does not itself distinguish them at its own granularity — if a single request constraint "
        "is being divided into finer conditions that the request never separately names or judges, the property is atomic at the request's granularity and must not be split; and "
        "neither accepted sub-property constrains an internal mechanism, architecture, algorithm, or representation as the primary subject of the constraint, unless that mechanism is "
        "explicitly required by the chosen interpretation. A sub-property constrains an internal subject when its canonical definition makes an internal mechanism, architecture, "
        "algorithm, or representation the primary subject of the constraint — this classification is independent of whether that subject happens to be externally observable. A "
        "sub-property is externally observable when its truth value can be determined solely from the accessible-observation boundary of the chosen interpretation, without inspecting "
        "the implementation. These two criteria are distinct: a property can concern an internal subject yet still be externally observable (for example, when the interpretation "
        "explicitly exposes that subject through its output contract), and a property can concern an external subject yet still be inaccessible if the interpretation does not expose "
        "the relevant behavior. The split condition rejects properties whose primary subject is internal and not explicitly required — not merely properties that happen to be "
        "inaccessible. A candidate split can be constructed only if canonical definitions for [Na] and [Nb] have been established and admissible falsifying constructions satisfying the "
        "stated falsifier conditions can be produced for both sub-properties. A candidate split test is accepted when all split-success conditions are satisfied for that non-void "
        "candidate; otherwise the candidate split is rejected. Step 4 — Accept or mark atomic. A property may be marked atomic only after Step 2 has introduced provisional "
        "sub-properties, Step 1 has canonicalized them, and Step 3 has attempted candidate-split construction against the current canonical state. If the candidate split test is "
        "accepted, replace the original property with the accepted canonical sub-properties and apply this same recursive decomposition procedure (Steps 1–4) independently to each "
        'accepted sub-property. If no candidate split can be constructed, emit: \'§ atomic: property [N]: "<verbatim canonical definition>" — no candidate split satisfies the '
        "construction requirements: <reason>.' If a candidate split test is emitted and rejected, emit: '§ atomic: property [N]: \"<verbatim canonical definition>\" — candidate split "
        "rejected: <reason>.' A property that represents a single request constraint at the request's own granularity is atomic even if its mathematical or logical expression admits "
        "formal decomposition, and even if that decomposition would yield logically distinct sub-conditions; the nontriviality and distinct-constraint conditions in Step 3 enforce "
        "this: a split into sub-properties that merely reformulate the mathematical definition of the predicate, or that subdivides a single request constraint into finer conditions "
        "the request never separately names or judges — rather than separating constraints the request itself distinguishes — is invalid and must be rejected as not nontrivial or not "
        "corresponding to distinct constraints at the request's granularity. The quoted definition must match the current canonical definition verbatim. Apply this procedure to every "
        "retained property until all have reached Step 4 and emitted '§ atomic:'. After all retained properties are atomic, emit a completeness check: '§ completeness check: "
        ...
        "interpretive assumptions made to resolve ambiguity and observation-boundary definitions are not themselves request constraints unless they were explicit constraints of the "
        "original request; every retained atomic property expression must appear exactly once, separated by ' / '; a '§ completeness check:' line that omits the request constraints or "
        "omits any retained property expression does not satisfy this requirement. Immediately after the completeness check, attempt to identify one explicit request constraint that is "
        "not logically entailed by the conjunction of the retained atomic property expressions. If such a constraint exists, emit '§ properties complete? no' — add one new property "
        "covering that explicit constraint, perform the required split testing, and repeat the completeness check. Only when every explicit request constraint is logically entailed by "
        "the conjunction of the retained atomic property expressions may the response emit '§ properties complete? yes' — a '§ properties complete? yes' line without a preceding "
        "completeness check does not satisfy this protocol; '§ properties complete? yes' certifies completeness only for the retained set existing immediately before that check; any "
        "subsequently committed removal or merge voids that result and requires a new completeness check. The completeness phase is concerned only with coverage of explicit request "
        "constraints; redundancy introduced during this phase is resolved by the subsequent observational-independence phase. After all retained properties are atomic, enter the Ground "
        "completion fixed-point procedure: first establish completeness for the current retained set; after '§ properties complete? yes', establish observational independence for that "
        "same retained set. A retained property is observationally independent if there exists at least one admissible construction that violates that property while satisfying every "
        "other retained property. Any retained property that is not observationally independent is observationally redundant and must be resolved — either successfully removed or "
        "successfully merged — provided the resolution does not reduce coverage of the explicit request constraints. A redundant property remains unresolved until either a removal or a "
        "merge replacing it is successfully committed; a failed merge or failed removal does not resolve the redundancy and requires another valid independence action. A proposed merge "
        "creates a candidate retained set by replacing its source properties with the merged property: the merged property must first complete canonicalization, scope validation, and "
        "recursive decomposition to an atomic state; once atomic, it replaces its source properties in the candidate retained set. A proposed removal creates a candidate retained set "
        "by removing the selected property; the source retained set remains unchanged until the candidate is validated. For both merge and removal, the candidate retained set must "
        "re-run the completeness-check procedure and observational-independence procedure; the candidate set may not be committed unless a new '§ completeness check:' is followed by '§ "
        "properties complete? yes' and the candidate set satisfies observational independence. A candidate retained set is committed only if it itself reaches the Ground fixed point — "
        "passing both '§ properties complete? yes' and the observational-independence procedure — before it replaces the source retained set; otherwise the candidate is discarded and "
        "the source retained set remains authoritative. If the candidate is committed and the committed change produces any further redundant property, restart the fixed-point "
        "procedure with a new completeness check on the resulting retained set. Otherwise discard the candidate and leave the source retained set unchanged, then attempt another valid "
        "independence action for the unresolved redundant property. Admissible constructions used as falsifiers or independence witnesses are evidence about the truth conditions of "
        "properties and do not thereby make the construction's internal mechanism, architecture, algorithm, or representation the primary subject of the property. Repeat until a "
        "retained set simultaneously satisfies completeness and observational independence; when no retained property is observationally redundant, the fixed-point procedure "
        "terminates. The Ground properties block is complete only when all retained properties simultaneously satisfy: they are derived from the chosen interpretation; they are atomic; "
        "their conjunction logically entails every explicit request constraint; they are observationally independent; they introduce no out-of-scope constraints, including internal "
        "mechanism constraints not explicitly required by the chosen interpretation. When all conditions are met, immediately before emitting '§ ground complete', emit exactly one "
        "'Retained properties:' line declaring the final retained property set: 'Retained properties: property [N]: \"<verbatim canonical formal definition>\"; property [M]: "
        '"<verbatim canonical formal definition>"; ...\' — listing every retained property in retained-set order, each quoted definition matching its final canonical formal definition '
        "verbatim. A 'Retained properties:' line that omits any retained property, quotes a natural-language or non-canonical definition, or is not immediately followed by '§ ground "
        "complete' does not satisfy this protocol. A '§ ground complete' line that is not immediately preceded by 'Retained properties:' does not satisfy this protocol. This sentinel "
        "may not appear before '§ properties complete? yes' in the transcript. '§ ground complete' is a phase-boundary sentinel, not a response-termination condition. After emitting '§ "
        "ground complete', continue with the requested task using the retained atomic properties as the governing constraints for all subsequent task reasoning, tests, implementation "
        "steps, and tool calls. The response must not terminate solely because '§ ground complete' has been emitted. All subsequent task work must remain consistent with the completed "
        "Ground properties; the Ground properties block is not itself the requested task unless the original request explicitly asks only for property derivation. Any test, "
        ...
        "not yet appeared, emit the Ground properties block in full and '§ ground complete' before continuing. After '§ ground complete', continue with the requested task; do not treat "
        "the sentinel as terminating the response.",
