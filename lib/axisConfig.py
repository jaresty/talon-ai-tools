...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "WHAT the rule is); evidence over theory (a model's description of what it would do is indistinguishable from having done it — only visible execution output counts as evidence; "
        "every behavioral change and execution must be observable in the log, not just described; trust evidence not theory), each derived rule must cite its source principle(s) from "
        "P1-P6; rules that cannot be derived are protocol violations; emit Protocol derived after deriving all rules; emit Ladder derived after producing the rung table; ground "
        "complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: a rung is valid iff a human "
        "reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → executable validation "
