...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "which specific assertions are red; the assertion rung is NOT optional; Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must "
        "appear after the artifact is produced and gate is satisfied; No implementation is permitted before the assertion rung shows red assertions — the exec_observed sentinel must "
        "appear with verbatim test output showing failure BEFORE any implementation code is written; the gap is the red assertion, the solution is making it green; Evidence before "
