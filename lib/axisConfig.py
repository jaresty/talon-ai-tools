...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "satisfied[ ] Baseline: derive from P2,P3 — Baseline defines evaluation structure not solution; RULE: Baseline = evaluation structure must PASS; Gap failures at Challenge only "
        ...
        "Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create without structure: unfocused output; RULE: structure before "
        "baseline [ ] Criteria before structure: derive from P1 — success conditions before organization; what GOES WRONG if you organize without criteria: no way to evaluate; RULE: "
        "criteria before structure [ ] Intent before criteria: derive from P1 — outcome before success conditions; what GOES WRONG if you define success without outcome: criteria "
        "irrelevant; RULE: intent before criteria [ ] Evidence at proximity: derive from P3 — evidence must be temporally close to action; what GOES WRONG if delayed: proof weakens; "
        "RULE: evidence immediately after action [ ] Evidence echoed: derive from P3,P4 — explain how evidence proves rules satisfied; what GOES WRONG if unexplained: description "
        "indistinguishable from compliance; RULE: explain evidence connection [ ] No contradiction in completion: derive from P3,P5 — claiming complete while acknowledging gaps is "
        "void; what GOES WRONG if you write 'complete' with 'gaps', 'caveats', 'needs': claim invalid; RULE: scan ENTIRE message for contradiction before 'complete' [ ] Rule citation "
        "at action: derive from P1,P3,P4 — cite specific derived rule before each action; what GOES WRONG if uncited: protocol compliance invisible; RULE: cite rule before action This "
        ...
        "rung requires?]. If gaps remain, do NOT proceed — close gap first. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P5. Ladder "
        "derived MUST be emitted as: 🔵 Ladder derived — AFTER producing the ladder table with columns: rung name | artifact type | derivation source | evidence form. No descent before "
        "both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent work. The 7-rung ladder: 1. Intent — What outcome is being aimed for? Derivation: "
        "from task, derive specific outcome. Evidence: clear statement of goal. 2. Criteria — What conditions must be true for success? Derivation: from intent, derive conditions that "
        "verify success. Evidence: checklist or measurable conditions. 3. Structure — How is the solution organized? Derivation: from criteria, derive organization. Evidence: internal "
        "consistency. 4. Baseline — Does the evaluation framework exist? Derivation: from criteria, derive evaluation structure. Evidence: evaluation structure APPLIED and VERIFIED to "
        "work. IMPORTANT: Baseline establishes evaluation STRUCTURE only - defines WHAT will be evaluated, NOT the solution itself. Baseline = criteria definition and scaffolding, not "
        "implementation. IMPORTANT: Baseline MUST PASS. If it fails, that's a broken framework - fix it. Gaps/failures are introduced at Challenge, not Baseline. IMPORTANT: Baseline "
        "creates MINIMAL scaffolding only - NOT the full solution. Full solution comes at Refinement. Baseline scaffolding is just enough for evaluation to run. ADVERSARIAL = minimal "
        "scaffolding to make evaluation run, nothing more. IMPORTANT: Baseline establishes evaluation framework at Baseline. Challenge adds NEW criteria to the SAME framework - "
        "creating NEW evaluation at Challenge is SCOPE EXPANSION, not valid Challenge. Challenge must extend existing evaluation, not create new ones. 5. Challenge — What evidence "
        "proves gaps exist? Derivation: from Baseline, add N new criteria to existing evaluation that FAIL. Evidence: N failing outputs prove N gaps exist. IMPORTANT: Challenge must "
        "use SAME evaluation framework from Baseline - creating new evaluation at Challenge is scope expansion, not valid Challenge. IMPORTANT: Work on the smallest verifiable thing at "
        "a time — each check must be satisfied before the next. 6. Refinement — How is the artifact improved? Derivation: from Challenge, for each failing criterion, add ONLY enough to "
        "satisfy that criterion. Evidence: modifications target the exact gaps proven to exist. IMPORTANT: Take ADVERSARIAL posture - try to satisfy verification with MINIMAL effort. "
        "Ask: what's the least I can do to make these N criteria pass? Do not add extra features or over-engineer. IMPORTANT: Refinement must address OBSERVED gaps only - gaps seen in "
        "Challenge output. Do NOT modify evaluation logic - only add solution content. 7. Verification — How is completion confirmed? Derivation: from refinement, run full evaluation "
        "from baseline. Evidence: all criteria satisfied = no gaps remain. IMPORTANT: After Refinement, if gaps still exist, iterate: Challenge → Refinement → Challenge → ... until no "
        "gaps remain. This is gap-driven iteration. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to finish each rung. Evidence before claims: for "
        "every claim about completion, there must be evidence proving it. Ground complete may only be emitted after verification confirms no gaps remain. Domain adaptation: for tasks "
        "where verification is possible, define evaluation scaffold BEFORE producing solution — evaluation defined after solution is vacuous (Texas sharpshooter). Baseline defines "
        "evaluation framework; Challenge adds N failing criteria; Refinement adds only enough to satisfy each; Verification runs same framework.for writing tasks, it maps to "
