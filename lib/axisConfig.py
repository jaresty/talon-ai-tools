...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "first explicitly selected demonstrated gap. Other observed failures remain recorded but inactive until a new Challenge rung begins. [ ] Structure before Challenge: derive from "
        "P1,P3 — solution organization must precede gap detection; what GOES WRONG if you skip structure: unclear what to evaluate; RULE: structure before challenge [ ] Criteria before "
        ...
        "rung. BEFORE transitioning to next rung, verify and declare: [current rung artifact completed?], [evidence visible?], [gaps remaining?], [intent still aligned?], [next rung "
        "requires?]. If gaps remain or verification not shown, do NOT proceed. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P6. Ladder "
        ...
        "defined after solution is vacuous (Texas sharpshooter). Challenge produces, applies, and shows the evaluative artifact failing; Refinement adds only enough to change that "
        "already-observed failing result; Verification re-applies the SAME evaluative artifact and shows that the previously observed failure no longer occurs, without introducing new "
        "drift. IMPORTANT: When deriving ladder artifact types, use MINIMAL names: - For Refinement, use 'Partial Implementation' or 'Minimal Code Changes' — NOT 'Implementation' or "
        "'full solution' - Artifact type names that imply completeness ('Implementation', 'Full Solution') contradict the gap-driven approach. If output diverges from protocol intent "
        "(e.g., producing full implementation instead of addressing specific gaps), reflect on: Which rung first diverged? What artifact type name drove this? What would a minimal "
        "change look like? Refinement should address ONE gap at a time with minimal changes — not build the entire feature. If you find yourself creating a complete feature, you have "
        "deviated from gap-driven refinement. for writing tasks, it maps to intent→criteria→structure→challenge→refinement→verification; for decision-making, it maps to "
