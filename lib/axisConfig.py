...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "be closed by the next rung) — if the observation reveals no gap, stop and fix the rung before proceeding; these two sentinels are required before any artifact for the next "
        "rung appears, regardless of whether the output shows success or failure; a rung is not achievable only when the domain lacks a standard artifact type, not because execution "
