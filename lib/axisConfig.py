...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "change is insufficient and the candidate is permitted; the only permitted evidence that the smaller change is insufficient is a tool-executed run result in which the governing "
        "failure signal text appears verbatim; a Smaller line is valid only when the immediately preceding tool-executed result contains the governing failure signal text verbatim — a "
        "Smaller line that appears without such a result, or whose immediately preceding tool-executed result does not contain the governing failure signal text, does not satisfy this "
        "requirement; no other form of argument — including prose reasoning about other tests or failure modes — constitutes valid insufficiency evidence; second, in a separate entry "
        "below, quote the specific text from the governing failure signal that shows the governed behavior absent — the quote must appear in a tool-executed run result above the first "
        "entry; a candidate that is larger than the smallest change that resolves the quoted failure text is not permitted regardless of what else it also resolves; a failure not yet "
        "observed in a prior tool-executed run result has not been shown to govern, and a change that fixes it cannot be derived from the current governing artifact; (3) line manifest "
        "listing every line to be added; for each line, quote the specific assertion string from the governing failure signal that would be absent from the run result when the line is "
        "present and would appear in the run result when the line is absent — an entry whose quoted string would remain in the run result when the line is present does not satisfy this "
        "requirement. Immediately before each tool call that modifies a file, produce: 'Scope: <verbatim quoted failure signal> | Candidate: <named candidate change> | Smaller: <named "
        ...
        "file in the transcript, the edit is not permitted; this line is not permitted unless a tool-executed result showing the smaller change applied appears above it in the "
        "transcript — a Scope line that appears without such a result above it does not satisfy this requirement. After the edit, produce a post-edit run result by tool call, then "
        ...
        "what must be present; where a clause names an action, the allow-list must name the conditions that permit that action), deny-list clause (names what must be absent without "
        "naming the present-state observable), prescription clause (names a step without naming the criterion the step serves), or prescription cluster (a set of clauses none of which "
        "derives from a criterion stated within the instruction). For each invalid clause: name it and its type; name the specific string whose removal would leave the "
