...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rung is incomplete until all criteria are encoded. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ "
        "Validation artifact V complete; each test function asserts exactly one behavioral property — one behavioral property per test function; multiple assertions are permitted only "
        ...
        "test-under-modification does not exhibit the criterion; before writing the test, check whether an existing test file covers the same module and add there if so; when the "
        "validation artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure "
        ...
        "until carry-forward has been emitted. before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — "
        "V-complete may not be emitted without this tool-executed result in the transcript; the validation artifact must reside within the project’s version-controlled file tree; if "
        "the artifact asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the "
        "implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten; implementation edits may not begin until a red run exists "
        "in the transcript for the current cycle; ✅ Validation artifact V complete may not be emitted unless a 🔴 Execution observed: showing the test failing exists in the transcript "
        "for the current cycle; emitting V-complete before seeing a red run is a protocol violation regardless of whether perturbation was attempted. ✅ Validation artifact V complete "
        "must be emitted at the executable validation rung before the validation run observation label is written; if the observed running behavior label is written without a ✅ "
        "Validation artifact V complete appearing in the transcript for the current cycle, the OBR rung is void — return to the executable validation rung and produce the artifact. "
        "Before writing the validation run observation rung label, invoke the validation artifact via a tool call; a harness error (import failure, syntax error, missing file) is not a "
        ...
        "fixing the harness, not an upward return; only a post-VRO Gap justifies HARD STOP; if the observed gap matches the prior cycle's gap, the edit did not address it — return to "
        "formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria "
        "rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running behavior label, re-emit "
        "the criterion for this cycle — criterion re-emission is required: emitting the OBR label without immediately following it with the criterion text is a protocol violation; ✅ "
        "Thread N complete may not appear without both the criterion re-emission and a 🔴 Execution observed: in the current OBR section; then immediately invoke the implemented "
        "artifact via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; "
        "it must be what a non-technical observer of the running system would see — not the test suite, not process state unless the criterion is specifically about process state; a "
        "build output or compilation result never satisfies this gate regardless of what it names; for a React or web UI component, the OBR invocation renders the component to DOM "
        "using the project’s render utility (e.g., renderToStaticMarkup, or the testing framework’s render function reading container.innerHTML) and emits the resulting HTML or text "
        "fragment showing browser-visible content — this is not a test assertion; it is direct DOM output showing the criterion’s named content is present in the rendered tree; "
        "starting a dev server does not satisfy this rung — server process state is not rendered content; if the implementation has no directly invocable artifact, invoke any non-test "
        "consumer of the changed artifact that demonstrates the declared behavior; a 🔴 Execution observed: whose verbatim block is empty or blank does not satisfy this gate; if the "
        "declared gap names a real network endpoint, the OBS must include verbatim output showing a real HTTP request or response — a test run whose network calls are mocked does not "
        "satisfy this gate; before any tool call touching the filesystem at this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — if "
        "the rung label cannot be located in the transcript, the tool call is prohibited; the provenance statement does not replace the tool call — it gates the tool call; the tool "
        "call must follow the provenance statement in the same response; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: with non-empty verbatim output appears in "
        "the transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol violation; "
        "implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label has been "
        "written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it that directly demonstrates the "
        "criterion — a test pass is not a demonstration; if the 🔴 Execution observed: at the observed running behavior rung contains test runner output (pass counts, test names, "
        "duration summary), it does not satisfy the OBR gate — re-invoke the implemented artifact directly before continuing; ✅ Thread N complete may not be emitted after an OBR 🔴 "
        "Execution observed: that shows test runner output; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already closed (return "
        "to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — dismissing a failing "
        "test as unrelated without an explicit written acknowledgment is a protocol violation; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: from a full test "
        "suite run exists in the transcript after the most recent OBR tool call in this thread — the only valid next action if no such result exists is the tool call that runs the "
        "suite; once ✅ Thread N complete is emitted no further output in that cycle is valid; at most one thread is in progress at a time — no output for thread N+1 may appear until ✅ "
        "Thread N complete has been emitted for thread N; this gate applies to all rungs, not only the prose rung. Before writing the criteria, formal notation, executable validation, "
        ...
        "rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared licenses the first edit — it does not complete the thread; "
        "after emitting it, the executable implementation rung, the observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread N complete "
        "is valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may not appear until the observed running "
        "behavior rung has fired for this thread. Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed "
        "inline by a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain "
        "exactly one entry per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at "
        "the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted "
        "prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — not from a prior cycle — to exist in output; "
        "the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. Before "
        ...
        "executable implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward return. One edit per "
        "re-run cycle; after each edit re-run the validation artifact before any further edit. Upward returns follow the failure class: if the implementation did not close the gap, "
        "loop within executable implementation; if the spec did not model the criterion, return to formal notation; observation cannot be produced is a prose-description failure, not "
        "an implementation failure — return to the prose rung and update it. After every upward return, a new ✅ Validation artifact V complete must be emitted for the current cycle’s "
        "gap — an artifact written in a prior cycle does not satisfy this gate. Reconciliation gate: at every upward rung transition, reconcile all artifacts against I and update any "
        "pre-existing document that records I (ADR, README, spec) before redescending. Before producing the final report, invoke the delivered system end-to-end via a tool call and "
        "emit 🔴 Execution observed: showing the primary behavioral intent is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before "
        "producing the final report. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order — before "
        "writing each section, locate the artifact in the prior transcript; if it cannot be found verbatim in the transcript, it may not appear in the final report; no new behavioral "
        "claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Seven rungs in order: prose → criteria → formal notation → executable "
        ...
        "Formal notation R2 audit complete — N/N criteria encoded.",
