...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "form": {
        ...
        "constraints governing this response; in that case the answer follows immediately. When not in channel mode, write the answer in the response turn after the user has provided their "
        "prediction. After each answer: a gap-reveal sentence comparing what the Predict: line stated against what is correct or complete; then for every concept except the first and the "
        "last, a Hook: line quoting a specific phrase from the current answer and naming the implication it leaves unresolved — the question the next concept answers; a Hook: line is "
        "compliant only when it appears after the gap-reveal sentence for this concept and before the Following from: line of the next concept (evaluator check: the quoted phrase appears "
        "verbatim in the current answer); a Hook: line must not appear after the last concept's answer; then a Misconception: line and Why: line — compliant only when they appear in the "
        "answer block of that named concept (the concept named in the pre-quiz commitment); a Misconception: line names the specific incorrect belief that would lead a reader to predict "
        "the wrong answer; a Why: line states the structural reason that belief is wrong; then an association sentence — before writing it, name the numbers already used in prior "
        "association sentences — using the format '#N (Name): [relationship]' where N is the opening-list number of a prior concept whose number has not appeared in any earlier association "
        "sentence in this quiz and Name is that concept's name from the opening list (for the first concept, use the format '#N (Name): [relationship]' naming the opening-list number and "
        "name of the first concept whose understanding it enables). emit '✓ [concept name]' only when the Predict: line states the complete and correct answer with no correction, addition, "
        "or nuance needed — if any gap exists between the prediction and the answer, write a gap-reveal sentence instead of '✓'. Continue until every concept in the opening list has either "
        "a '✓ [concept name]' marker or a gap-reveal sentence in the transcript, or the user explicitly declines to continue; then close with a terminal declaration containing the verbatim "
        "name of every concept from the opening list — an evaluator determines completeness by comparing the terminal declaration against the opening list and confirming every name "
        "appears. With an output-exclusive channel, structure the output as a quiz document with each question immediately before its answer per topic.",
