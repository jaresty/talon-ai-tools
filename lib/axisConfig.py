...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "correct data; the observation must demonstrate the declared behavioral intent was satisfied by tool output traceable to the declared gap. For code contexts, observed running "
        "behavior is satisfied only by tool output that directly demonstrates the declared feature: rendered DOM content showing the declared UI behavior, an API response body "
        "containing the declared data, or an end-to-end test output naming the specific behavior from I; build pipeline output (compiler success, server start confirmation, URL shell "
        "response) is not running behavior regardless of label. A rung is complete when and only when its artifact has been produced — not when it has been listed, planned, or "
        "described; a rung label during execution marks the point where the artifact begins and must be emitted at every rung transition — it is not a section heading for planning or "
        "description; absence of a rung label at a transition is a traversal violation, not a stylistic omission; no content other than the artifact itself may appear between a rung "
        "label and the artifact it precedes. A rung is not achievable when the domain provides no standard artifact type for it; this must be stated explicitly with justification — "
        "convenience or prior knowledge do not make a rung not achievable. ground is a Process method — the task governs the character of each rung’s output but not the process "
        "structure; the manifest, rung sequence, and execution gates are mandatory regardless of which task or other tokens are combined with ground; completeness governs rung depth, "
        "not rung existence. Formal notation must satisfy R2: every behavioral constraint from the criteria rung must be re-expressed as a behavioral invariant — not just interface "
        "shape; type signatures or schemas that capture structure without encoding invariants do not satisfy this rung; before advancing from formal notation, produce the R2 audit as a "
        "separate named section after the formal notation body — a prose list embedded in the notation body does not satisfy this requirement; the R2 audit section contains one "
        "numbered row per criterion: ‘1. criterion → behavioral invariant’ or ‘1. criterion → UNENCODED’; these are the only two valid row resolutions — no other resolution is valid; a "
        "label that names constraint type rather than expressing a behavioral invariant (e.g., ‘architectural constraint’, ‘exclusion constraint’) is UNENCODED, not a behavioral "
        "invariant; the rung is incomplete until every criterion resolves to a behavioral invariant — no UNENCODED entries may remain; emit ‘✅ Formal notation R2 audit complete — N/N "
        "criteria encoded’ on its own line as the gate before advancing to executable validation. When I-formation is required: observation of existing code or running behavior "
        ...
        "observed failure; a passing run is a gap signal — see gate_validity) → executable implementation → observed running behavior. A gate is a conversation-state condition: open "
        "when and only when the required event has occurred in this conversation for this thread. Underlying all compliance failures is one epistemological error: substituting model "
        "knowledge for conversation events — prior knowledge, anticipation, and model reasoning cannot satisfy any gate regardless of accuracy. For executable rungs, emit 🔴 Execution "
        "observed: [verbatim tool output — content composed without running the tool is invalid] then 🔴 Gap: [what the verbatim output reveals] on their own lines before any "
        "implementation artifact; the 🔴 Execution observed sentinel body must be a complete block delimited by triple backticks containing the complete tool output — nothing omitted; a "
        "single extracted line does not satisfy this sentinel regardless of accuracy; no prose, summary, or interpretation may appear before the 🔴 Gap line; interpretation belongs "
        ...
        "in context; tool output that appeared before its sentinel is uncovered and does not satisfy this rung regardless of subsequent labeling. No implementation artifact — including "
        ...
        "immediately preceding it, it is invalid and must be discarded before the tool is run. The 🔴 sentinel format is reserved exclusively for executable rung gates; for "
        "non-executable rungs, observation appears inline as labeled prose. The validation run observation rung is only satisfied by an observed failure that reveals the declared gap — "
        "a prior recorded failure applies to the specific test that produced it, not the thread as a whole; when the validation artifact is modified — tests added, rewritten, or mocks "
        "changed — emit a carry-forward statement: ‘Carry-forward: [list which original failures cover which current tests]’; the carry-forward is a gate: no implementation artifact "
        "may appear after validation artifact modification until a carry-forward statement has been emitted; modification without carry-forward is a traversal violation equivalent to "
        "producing implementation code without sentinels; any current test with no covering prior failure is uncovered and triggers the vacuous-green check; a green run on an uncovered "
        "test is a gap signal unless the behavior is already implemented (verify this). A green validation run is a gap signal unless a prior recorded failure for each test exists in "
        "this conversation; if the run is green and no prior failure has been recorded for a test, assess whether that test exercises the declared gap — either the behavior is already "
        "implemented (verify this; the thread may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected before descent continues; "
        "this check is unconditional within the validation rung — it applies regardless of whether implementation has begun. Self-check: before advancing past any executable rung, "
        "verify: (a) the required sentinels for this rung have been emitted with verbatim tool output, not summaries or compositions — confirm by quoting the sentinel text from this "
        "conversation; if the sentinel cannot be located in this conversation, the gate is not open; (b) no implementation content appeared before them; if either condition is unmet, "
        "the rung is incomplete and must not be treated as satisfied. A build error or compile failure does not satisfy this rung — the validation artifact must compile and execute "
        "before its output can reveal a behavioral gap; a failure to compile means the artifact is incomplete and must be corrected before the gate can open. Before producing "
        "implementation code, emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; the quote must be verbatim from the 🔴 Execution observed sentinel of "
        "this thread; quoting anticipated output or a prior thread’s observation is invalid. Before executable implementation begins, both of the following must appear in this order: "
        "first ‘✅ Validation artifact V complete’ on its own line immediately after the validation run observation rung has declared a gap, then ‘🟢 Implementation gate cleared’ on its "
        "own line — V-complete alone does not open the implementation gate; both sentinels are required in sequence; V-complete may not appear after tests pass, after implementation, "
        "or after observed running behavior; it marks the transition from validation to implementation, not the completion of the thread. Gap-locality: the gap gating rung N is the "
        "output of executing rung N-1; no gap from any higher rung, and no element of I directly, may serve as the gating gap for the current rung. Minimal scope: the current rung’s "
        "artifact addresses the declared gap and nothing more; implementing beyond the declared gap is a violation — not a benefit; minimal scope applies to the entire invocation, not "
        "just individual rungs — any artifact added after the ladder is complete that was not present in I must be treated as a new I requiring its own manifest and descent; adding it "
        ...
        "declared gap, not reduced to avoid testing it; a passing suite with fewer tests than the declared gap requires is a gap signal, not a success. Upward revision is always "
