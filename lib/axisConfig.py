...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "commitment); after the call, a tool-executed run result must appear in the transcript in which the quoted scope text is absent and every other line from the immediately "
        "preceding pre-edit run result is present. Then enumerate every path by which a transcript could satisfy these commitments without the call having changed exactly one "
        "independently observable behavior: for each path, name the literal string in the transcript that closes it — a path is closed only when naming that string makes the "
        "non-compliant transcript distinguishable from the compliant one without assessing intent; a path that cannot be closed by naming a string must be eliminated by bringing the "
        "system to a state where the commitments can be satisfied structurally. The enumeration is complete when no remaining open path exists. Proceed to file-modifying tool calls "
        "only when the derivation and enumeration are present as text above this point in the transcript.",
        ...
        "rejects; (4) each behavioral dimension must be covered by a verbatim string from a tool-executed result appearing under a literal completion-check heading. Then enumerate "
        "every path by which a transcript could satisfy these heading requirements without the governing goal having actually been identified, dimensions actually derived, or "
        "dimensions actually covered: for each path, name the literal string in the transcript that closes it — a path is closed only when naming that string makes the non-compliant "
        "transcript distinguishable from the compliant one without assessing intent; a path that cannot be closed by naming a string must be eliminated by ensuring the required "
        "structural property is present before the heading is written. The enumeration is complete when no remaining open path exists. The literal strings named during enumeration as "
        "path closures are binding — a heading or structural property that does not match the committed string verbatim does not satisfy the corresponding requirement. Proceed to "
        "implementation only when the derivation and enumeration are present as text above this point in the transcript. Any noun phrase in the governing goal that names a file path, "
