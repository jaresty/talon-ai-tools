...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "observation as a tool call and record its output; the artifact named in that invocation is the §0 identifier, fixed for all steps; for non-software subjects, recording an "
        "observation of the subject as it currently stands under a literal heading satisfies §0 — the §0 identifier is the name used in that observation; (1) write '## Governing goal: "
        "[text]' where '[text]' is a verbatim substring of the §0 tool result; if two non-subsuming goal candidates exist, write both on separate lines under §1 and mark one "
        "'[selected]' — the selected goal is the one whose non-achievement leaves the task incomplete regardless of other goals; (2) write '## Behavioral dimensions'; each listed "
        "dimension quotes a verbatim substring of the '[text]' string from §1; (3) write '## Enforcement sequence'; each sub-heading names as a literal substring what the prior "
        "sub-heading's passing state admits that this sub-heading rejects; (4) write '## Completion check'; each covered dimension cites a verbatim string from a tool call whose "
        "invocation names the §0 identifier directly; (5) write '## Path enumeration'; enumerate every path by which §1–§4 headings could be present without the governing goal "
