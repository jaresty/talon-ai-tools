...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "read literally: when literal and semantic readings diverge, choose the interpretation that produces the smallest change that would shift the governing output. The permitted "
        ...
        "of how compact it is. New failure signals become new governing outputs for new steps. The implementation target is permitted only after the derivation names both the candidate "
        "change and at least one smaller change that would not shift the governing output, with a citation to a specific location already in the transcript — a prior run result or "
        "structural observation — that supports the claim that the smaller change would not shift the governing output; a reason that could be stated without inspecting any prior "
        "transcript artifact has not been derived. When an implementation step adds lines to any file — whether creating a new file or editing an existing one — the derivation must "
        "produce a line manifest before the edit: an enumerated list of every line (or logical block) to be added, each entry stating (a) the specific text in the governing failure "
        "signal that names the behavior this line implements — quoted verbatim from the governing failure signal — and (b) the reason removing this line would leave the governing "
        "failure signal unshifted or would reintroduce it. A manifest entry whose behavioral connection is stated without quoting the governing failure signal text has not been "
        "derived. A manifest entry citing a behavior not explicitly named in the governing failure signal is not valid. The manifest must exist as a completed transcript artifact "
        "before the edit tool call — a manifest generated in the same act as the edit does not satisfy this requirement. For new files, each entry must name the specific text in the "
        "governing failure signal that names the behavior this line implements or enables — compilation dependencies between lines within the new file are not sufficient justification; "
        "a line justified only by being required for another line to compile has not been derived against the failure signal. If a line is required for another line to execute, that "
        "other line's manifest entry must itself name governing failure signal text; the dependency chain must terminate at a quoted governing failure signal text, not at another "
        "manifest entry. After every edit tool call, the derivation must produce a post-edit manifest check as a transcript artifact: enumerate every line actually added (from the edit "
        "diff), verify each appears in the pre-edit manifest, and name any line present in the edit but absent from the manifest — if any such line exists, the edit must be reverted "
        "before the step can continue. A post-edit manifest check that does not enumerate the diff lines explicitly has not been derived. An implementation edit closes the step only "
        "when a post-edit run result from the governing artifact — produced by a tool call after the edit — exists in the transcript and names a failure set F' satisfying: (a) the "
        "governed failure is absent from F'; (b) every failure signal in F' whose governing artifact produced no failure in F is absent — a failure signal in F' whose governing "
        "artifact had no failure in F is a regression and requires revert regardless of any judgment about latency; (c) failure signals in F' from governing artifacts that were already "
        "producing failures in F are progression failures, each becoming a new governing output for a new step. A failure in F' is a progression failure if and only if the governing "
        "artifact that produced it also appears in F with at least one failure — checkable against the pre-edit run result without interpretation; a model assertion that a failure is "
        "latent does not satisfy this condition. If (a) or (b) is not satisfied, the edit must be reverted. A post-edit confirmation line names the specific change made, cites the "
        "derivation's stated candidate, and cites the pre-edit and post-edit run results by transcript location — if the change made exceeds the derivation's stated scope, the edit "
        "must be reverted to match it before proceeding. A step is the smallest change that could alter the result of any verification — compilation, static analysis, or test "
        "execution; changes that address different verification layers are always separate steps even when they feel like parts of the same fix. The governing artifact must be the "
        "verification layer with scope to detect a failure if this step's change is incorrect — running a layer that cannot see the specific files, call sites, or concerns this step "
        "touches does not satisfy the governing artifact requirement even if that layer passes. An artifact is a valid governing artifact for this step only when a derivation exists in "
        "the transcript naming the specific FAIL output it would produce if this step's change were absent. Implementing beyond the current governing output is a violation regardless "
        "of how many other failures exist. Analysis and classification steps have a text artifact as their governing artifact — a complete classification block or derivation section; "
        "no tool call is required to open or close these steps. A step qualifies as an analysis step when and only when its governing artifact's content, if applied verbatim to the "
        "system, would produce no change in system behavior, and names at least one specific behavioral invariant that would remain unchanged if the artifact were discarded. "
        "Implementation steps have a FAIL tool result as their governing artifact. These are distinct step types with distinct governing artifacts — analysis steps may precede the "
        "first tool call without violating atomic.",
