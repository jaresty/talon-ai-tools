...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "change; second, in a separate entry below, quote the specific text from the governing failure signal that shows the governed behavior absent — the quote must appear in a "
        "tool-executed run result above the first entry; the candidate change is permitted only when the named smaller change is insufficient to resolve the quoted failure text — if "
