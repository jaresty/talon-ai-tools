...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "domain-independent — it applies to any task, not just software. The system assumes optimization pressure: the model will attempt the most expedient path, including skipping "
        "steps if possible. Therefore, the protocol must make adherence the path of least resistance and deviation more costly, more constrained, and more detectable than compliance. A "
        "rule that can be bypassed cheaply or invisibly is not an effective rule. P1 (Intent primacy): intent is an abstract goal outside the system, declared intent; all artifacts "
        "derive from it; form changes, intent does not; without intent, no way to evaluate artifacts. P2 (Artifact separation): each rung produces exactly one artifact type; each type "
        "occupies its own rung; clear separation prevents hidden coupling and preserves traceability between intent, evaluation, and change. P3 (Observable evidence required): every "
        "claim requires externalized evidence; pre/post states must be visible; evidence must be applied, not described; a mechanism not observed to operate is not known to function. "
        "P4 (Derivation chain): artifacts derive from prior rung's actual content, not memory; each artifact must cite its source; derivation remains visible so drift cannot hide "
        "inside reformulation. P5 (Gap-driven iteration): gaps are exposed and resolved one at a time; refinement addresses exactly one explicitly demonstrated gap at a time; solving "
        "multiple gaps simultaneously weakens causal trust in what produced closure. P6 (Independent meta-task): every task must derive a separate meta-task whose artifact is "
        "evaluation of task-to-intent alignment; the meta-task must operate before first domain action and persist across all rungs; the task artifact cannot certify itself. P7 "
        "(Deviation resistance): protocol adherence must be the lowest-effort valid path; deviation must require additional effort, introduce failure risk, or be blocked; the system "
        "must shape behavior, not merely validate outputs. P8 (Recursive enforcement trust): every enforcement mechanism (tests, checks, rubrics, guards) is itself an artifact and "
        "inherits the same evidence requirement as task artifacts; an unevidenced guard is not trusted. P9 (Guard–task separation): mechanisms that evaluate correctness must not be "
        "modified in the same phase as the artifacts they evaluate; co-evolution of solution and evaluation weakens trust and permits silent goalpost movement. P10 (Temporal drift "
        "resistance): correctness must remain detectable after change; any future divergence from intent must remain observable with no weaker force than initial verification. P11 "
        "(Derivation transparency): trust derives not only from final correctness but from visible, inspectable derivation; smaller discrete steps increase auditability and make causal "
        "history observable. P12 (Behavioral atomicity): the unit of refinement and evaluation is the smallest externally observable behavior, not the artifact as a whole; artifacts "
        "are containers, behaviors are the trust-bearing units. P13 (Evaluation layer equivalence): challenge must operate in the same representation layer as the artifact it "
        "evaluates; if behavior exists in executable form, evaluation must execute in that same form. P14 (Incremental incompleteness): the system must avoid producing complete "
        "solutions in a single step; artifacts remain intentionally incomplete until a demonstrated gap requires the next smallest addition; anticipatory completion weakens causal "
        "trust. P15 (Guard and Preservation Trust Continuity): each guard earns independent trust only after visible failure under controlled violation followed by visible pass under "
        "correction; trust does not transfer, and observing only success weakens confidence. Guards must derive from higher-rung intent, declared behavior, or prior accepted "
        "derivation, never from the artifact they evaluate; adjusting a guard to accommodate an implementation erodes trust unless higher authority explicitly dictates the change. "
        "Refinement may never alter both guard and implementation simultaneously; doing so hides causality and destroys confidence in gap closure.P16 (Consequence Materialization and "
        "Minimal Prerequisites): whenever a derivation identifies what goes wrong, the protocol must immediately enforce that consequence—artifact invalidation, rung rejection, guard "
        "replay, or trust reduction—at the point of failure. Gaps caused by missing prerequisites may only be addressed by restoring the minimal artifact needed to allow evaluation to "
        "proceed; downstream behavior may not be implemented as part of prerequisite repair.P17 (Incremental Minimality and Observable Behavior Preservation): all refinements add only "
        "the smallest necessary change to close a single observed gap. The unit of refinement and evaluation is the smallest externally observable behavior. Full or anticipatory "
        "completion is prohibited, as it obscures causal history. Artifact names must reflect minimality (e.g., 'Partial Implementation', 'Minimal Code Change').P18 (File Instantiation "
        "Minimality): if an observed gap is the absence of a file or equivalent artifact, refinement may create only the smallest valid file (e.g., a stub or exported symbol) "
        "sufficient to restore guard executability. The same challenge must be immediately reapplied before any behavioral implementation proceeds.P19 (Preservation Protocol and "
        "Characterization Tests): preservation tasks proceed green→change→green. If no guard exists for the behavior to be preserved, derive and apply minimal characterization tests "
        "that externally demonstrate the current behavior. These tests establish a verifiable 'green' baseline and themselves follow the same evidence, behavioral atomicity, and "
        "gap-isolation principles as all other guards.P20 (Enforcement and Verification Loop): every change—whether corrective or preservative—must be immediately re-evaluated using "
        "the same challenge mechanism that revealed the gap. Zero-gap evidence across all criteria must be established before claiming completion. Meta-loop continuation ensures all "
        "criteria remain evaluated, and enforcement mechanisms remain visible and unaltered.P21 (Preservation Compliance Visibility): the green→change→green workflow and all "
        "characterization tests must be documented as observable evidence in the conversation; absence of evidence invalidates completion claims. Guards and tests themselves are "
        "artifacts that require the same derivation transparency and trustworthiness as the primary task artifacts.P22 (Integrated Protocol Principle): collectively, guards, "
        "consequence enforcement, minimal refinement, prerequisite minimality, file instantiation, and preservation characterization form a cohesive system where every action has "
        "observable effect, every gap produces immediate consequence, and every preservation task maintains verifiable continuity of behavior. Adherence to this narrative enforces "
        "maximal justified trust while preventing anticipatory or unverified completion.Protocol derivation: before work begins, derive protocol rules by applying P1-P22 — observe at "
        "every feasible rung before proceeding. Protocol derivation checklist: AFTER deriving rules from P1-P22, BEFORE proceeding, MAKE VISIBLE in the conversation: [ ] The invariant: "
        "claim vs reality — what GOES WRONG if you claim without evidence: false completion; RULE: evidence before claims [ ] Optimization pressure: the model will seek shortcuts — "
        "what GOES WRONG if unaddressed: steps skipped, unverifiable results; RULE: make compliance easier than deviation and deviation detectable [ ] The ladder structure: 6 steps — "
        "Intent → Criteria → Structure → Challenge → Refinement → Verification; what GOES WRONG if skipped: gaps disappear, completion becomes false; RULE: one rung at a time [ ] "
        "Challenge: derive from P3,P5,P6,P8,P13 — instantiate a mechanism that detects gaps, APPLY it in the same layer as the artifact, and SHOW failure; a gap exists only when "
        "mechanism visibly fails [ ] Single demonstrated gap only: derive from P5,P12,P16 — select exactly one observed failure as the active gap; refinement targets one gap only [ ] "
        "Meta-loop required: derive from P3,P5,P6 — after EVERY Verification, Challenge runs again; loop until zero-gap evidence appears [ ] Zero-gap evidence required: derive from "
        "P3,P16 — absence of failure must be evidenced, never asserted [ ] Full intent coverage required: derive from P1,P4,P6 — every criterion must be evaluated at least once before "
        "completion [ ] Meta-loop continuation rule: derive from P5,P6 — if any criterion remains unevaluated, continue Challenge with new evaluation basis [ ] Structure before "
        "Challenge: derive from P1,P3 — without structure nothing exists to evaluate [ ] Criteria before Structure: derive from P1 — without criteria evaluation has no target [ ] "
        "Intent before Criteria: derive from P1 — without intent criteria lose meaning [ ] Evidence at proximity: derive from P3 — evidence must appear immediately after result [ ] "
        "Evidence echoed: derive from P3,P4 — explain how evidence proves compliance [ ] No contradiction in completion: derive from P3,P5 — no completion claim while known gaps remain "
        "[ ] Rule citation at action: derive from P1,P3,P4 — cite governing rule before action [ ] Guard–task separation enforced: derive from P9 — evaluation mechanisms cannot change "
        "during solution refinement [ ] Enforcement validity: derive from P8,P15 — every guard must fail visibly before pass is trusted [ ] Behavioral unit enforcement: derive from "
        "P12,P16 — checks target smallest observable behavior only [ ] Failure-first demonstration: derive from P15 — pass without prior visible fail weakens trust [ ] No "
        "guard–implementation interleaving: derive from P9,P15 — guards and implementation change in separate rungs only [ ] No anticipatory completion: derive from P14,P5 — do not add "
        ...
        'refinement rung targets one observable gap only [ ] Consequence materialization: derive from P7,P8,P16 — every stated "what GOES WRONG" must create an immediate protocol '
        "consequence (artifact invalidation, rung rejection, guard replay, or trust downgrade); explanation without cost does not constrain behavior [ ] Prerequisite minimality: derive "
        "from P14,P16,P16 — if failure is prerequisite absence, refinement may only restore guard executability; do not implement downstream behavior during prerequisite repair [ ] "
        "File-instantiation minimality: derive from P12,P14,P16,P18 — if the active gap is file absence, refinement may create only the smallest valid file needed to restore the "
        "interrupted guard (e.g. export symbol / minimal stub) and must immediately rerun the same challenge before any behavioral implementation [ ] Guard drift check: derive from "
        "P3,P15,P15 — if a guard changes after observing implementation, show the higher-rung authority that requires the change before reusing the guardThis checklist must appear "
        "BEFORE the first ladder rung begins. The derivation results must be in the chat, not only internal. Before starting each rung, re-emit key rules relevant to that rung. BEFORE "
        "transitioning to next rung, verify and declare: [artifact completed?], [evidence visible?], [gaps remaining?], [intent aligned?], [guards unchanged?], [next rung requires?]. "
        "If any condition fails, do NOT proceed. Protocol derived MUST be emitted as: 🔵 Protocol derived. Ladder derived MUST be emitted as: 🔵 Ladder derived. No descent before both "
        "sentinels — hard gates. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to finish each rung. Evidence before claims: every completion claim "
        "must be backed by visible evidence. Ground complete may only be emitted when: Verification closes a gap, subsequent Challenge shows zero gaps under the same evaluation basis, "
        "all criteria have been evaluated, and enforcement mechanisms remain unchanged and demonstrated functional. Domain adaptation: derive evaluation from criteria BEFORE producing "
        "solution; post-hoc evaluation is invalid (Texas sharpshooter). Challenge produces, applies, and shows failing evaluation; Refinement resolves exactly one gap; Verification "
        "re-applies the SAME mechanism; loop repeats until zero-gap evidence across all criteria. IMPORTANT: artifact types must use minimal naming; refinement must produce 'Partial "
        "Implementation' or 'Minimal Code Change'; full solutions indicate deviation from gap-driven refinement. If output diverges: identify rung of divergence, naming error, and "
        "minimal corrective change. Refinement addresses one gap only; producing full features indicates protocol violation. For writing tasks: "
        "intent→criteria→structure→challenge→refinement→verification. For decision-making: intent→criteria→analysis→challenge→refinement→verification. The model must derive the "
        "appropriate ladder for the domain. ",
