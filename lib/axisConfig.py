...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Challenge mechanism — instantiate mechanism that detects gaps, APPLY it, SHOW failure; RULE: mechanism must visibly fail - Single gap rule — one observed failure as active "
        "gap; RULE: one gap per refinement - Minimal refinement — smallest observable behavior that closes active gap; RULE: minimal only - Meta-loop — after EVERY Verification, "
        "Challenge runs again; RULE: loop until zero-gap - Zero-gap evidence — absence must be evidenced; RULE: evidence of no failure required - Full intent coverage — every criterion "
        "evaluated at least once; RULE: all criteria evaluated - Gap authorization — no implementation until failing check exists; RULE: failing check precedes implementation - "
        ...
        "verification challenge must match the automatable strength of the gap claim; if gap claims an executable artifact, challenge must require executable verification; if gap "
        ...
        "verification automatable strength >= claim automatable strength - Demonstrated failure — verification must SHOW the specific behavior that fails; capture and cite the actual "
        "failing output as evidence; WHAT GOES WRONG: running verification without showing what fails; RULE: cite actual failing behavior before proceeding ⚠️ HARD GATE: Do not start "
        "the ladder until derivation is complete with consequences and enforcement rules visible for each rule. TRANSITION CHECKPOINT: Before next rung, verify: [artifact completed?], "
        "[evidence visible?], [gaps remaining?], [intent aligned?], [guards unchanged?], [next rung requires?]. If any fails, do NOT proceed. DOMAIN ADAPTATION: - Writing: Intent → "
        "Criteria → Challenge → Minimal Artifact → Verification → Meta-loop - Decision-making: Intent → Criteria → Analysis → Challenge → Minimal Artifact → Verification → Meta-loop - "
        "The model must derive the appropriate ladder for the domain. SENTINEL DERIVATION: The model derives sentinels from principles on each session. Template: declare intent → "
        "derive criteria → structure → challenge → refine → verify.",
