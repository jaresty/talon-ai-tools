...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "(1) the first text emitted for property [N] must be 'Observing: property [N] \"<verbatim property definition>\" — <what the artifact observes>' — no other text, sentinel, or "
        ...
        "(2) write a concrete automatic regression guard for property [N] via a tool call and wait for the tool-result block confirming it was written; planning or describing a guard "
        "does not satisfy this token; for every executable assertion in the written guard emit exactly one 'Property [N] assertion [M]: <quoted assertion text>' line — every "
        ...
        "the assertion inventory; no subsequent step may occur until 'Assertion inventory: complete' has been emitted; then emit 'Quoted test: <smallest contiguous verbatim executable "
        "assertion or guard uniquely identifying the written guard>'; no subsequent step may occur until this sentinel has been emitted; (3) attempt to falsify the quoted guard by "
        ...
        "<construction>' or 'Test blind-spot: not found'; if found, strengthen the guard and repeat from step (2); (4) execute the quoted guard in a state where property [N] is "
        "violated and emit 'Failure: property [N] \"<verbatim property definition>\" — <verbatim quoted failure line from a tool-result block>' or 'Unobservable: property [N] "
        '"<verbatim property definition>" — <reason>\'; emit Unobservable only when no executable guard, assertion, constraint, or rule could ever distinguish satisfaction from '
        "violation regardless of the governed artifact; if a guard can exist, execute it against a violating state and use the resulting failure line instead; before modifying the "
        "governed artifact to satisfy property [N], a verbatim Failure line from a tool-result block must appear; if the failure line does not specifically witness the violation of "
