...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "you are at rung N until you emit completion sentinel for rung N), rung completion order (each rung must produce its artifact before emitting completion sentinel; sequential "
        "descent through prose → criteria → formal notation → validation → implementation; skipping a rung is a protocol violation), P5 implies sequential descent (since each artifact "
        "derives from prior artifact per P5, you must produce them in order — treating the ladder as a planning document rather than strict sequence violates derivation), ladder is "
        "strict execution sequence (emit rung label, produce artifact, emit completion sentinel — repeat for each rung in order; you may return upward freely, but only when higher rung "
        "artifact is discovered to be malformed — NOT to make lower rung implementation easier; difficulty, failure, or constraint pressure at a lower rung is not a valid upward return "
        "trigger), upward return conditions (what triggers return, what does NOT trigger return and why excluded), evidential scoping (evidence is scoped to current cycle and gap, not "
        "prior cycles), scope preservation (scope does not expand between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); each "
