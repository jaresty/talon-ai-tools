...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "creates or modifies a file — narrating a change without a file-write tool call is not an edit. Rung table — name | artifact type | gate condition | void condition: prose: "
        "artifact=“natural language description of intent and constraints”, gate=“I declared”, voids_if=“skipped”; criteria: artifact=“falsifiable behavioral acceptance conditions”, "
        "gate=“prose complete”, voids_if=“criterion is structural/implementation rather than behavioral”; formal notation: artifact=“non-executable specification with behavioral "
        ...
        "tree; pre-existing artifact”; validation run observation: artifact=“exec_observed sentinel with verbatim failure naming declared gap”, gate=“executable validation artifact "
        "runs”, voids_if=“build/compile error; green run without prior failure; infrastructure-only output”; executable implementation: artifact=“implementation source edits; one edit "
        "per re-run cycle”, gate=“exec_observed + gap declared; impl_gate sentinel emitted”, voids_if=“multiple edits without intervening re-run; newly-passing test has no prior "
        "failure”; observed running behavior: artifact=“live-process output — output produced by a tool call that starts or queries a running process; reading any file does not satisfy "
        "this type regardless of content”, gate=“implementation complete; exec_observed + gap for behavior from I”, voids_if=“new files created; output names only infrastructure state; "
        "file read used as evidence; test runner output — a test-suite pass is validation-run-observation-type output, not observed-running-behavior-type output, and voids this rung”. "
        ...
        "again; before writing the test, check for an existing test file covering the same module — add there if so; when the artifact is modified, invoke a tool call to read the "
        "current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution observed "
        "sentinel] covers current test [verbatim test name as written in the artifact]’ — each prior-failure name must be quotable verbatim from a 🔴 Execution observed sentinel; "
        "modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until carry-forward has been emitted. when the declared "
        "intent is to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the validation artifact must be a meta-test — an executable "
        "artifact that fails when the test-under-modification does not exhibit the criterion; before emitting ✅ Validation artifact V complete, the artifact content must appear in the "
        "current response — emitting the sentinel without quoting the artifact in the transcript is a protocol violation; before emitting ✅ Validation artifact V complete, confirm via "
        "tool call that the artifact path does not pre-exist or is already failing — if that tool call has not been made in the current response, the only valid next action is that "
        "tool call — V-complete may not be emitted without this tool-executed result in the transcript; asserting confirmation without showing the exec_observed block does not satisfy "
        "this gate — the exec_observed block must appear in the current response; the validation artifact must reside within the project’s version-controlled file tree; if the artifact "
        ...
        "assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness; a green 🔴 Execution observed: is only valid as the "
        ...
        "repairs the harness — not a gap statement, not HARD STOP, not a rung label; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. "
        "Before emitting 🛑 HARD STOP, locate the criterion written in the most recent prior HARD STOP cycle for this thread — if it is textually identical to the current criterion, "
        "HARD STOP is a protocol violation: an identical criterion means the upward return accomplished nothing; if the implementation did not close the gap, loop within executable "
        "implementation; if the spec did not model the criterion, return to formal notation before issuing HARD STOP. if the observed gap matches the prior cycle's gap, the edit did "
        "not address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the "
        "next token is the criteria rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running "
        "behavior label, re-emit the criterion for this cycle — criterion re-emission is required: emitting the OBR label without immediately following it with the criterion text is a "
        "protocol violation; the tool call is the only valid next action after criterion re-emission; planning statements, diagnostic narration, and commentary between criterion "
        "re-emission and the tool call are protocol violations; ✅ Thread N complete may not appear until a tool call exists in the transcript after the observed running behavior label "
        "in the current cycle; then immediately invoke the implementation as a live running process via a tool call and emit 🔴 Execution observed: with its verbatim output — the "
        "verbatim output must directly demonstrate the specific behavior named in the criterion. a test runner invocation does not satisfy this gate — running tests produces "
        "validation-run-observation-type output, not observed-running-behavior-type output; the invocation target must be the implementation artifact itself, not a test harness "
        "wrapping it; A file read never satisfies this rung — the block must contain output produced by running the artifact. If the artifact requires a start step before querying "
        "(e.g., a web server), two tool calls are required in sequence: process-start then query — the 🔴 Execution observed: sentinel may not be emitted until both complete. In-process "
        ...
        "artifact directly invocable. Before any tool call at this rung, emit a provenance statement naming the invocation target: ‘Invoking [artifact] produced at [rung name] in this "
        "thread’ — if the target is a test runner, stop: the invocation target must be the implementation artifact itself running as a live process, not a test harness; the provenance "
        "statement does not replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not be emitted unless all of the following exist in this "
        "cycle's transcript: (1) a tool call exists in the transcript after the observed running behavior label in the current cycle; (2) observed running behavior label has been "
        "written after the most recent 🟢 Implementation gate cleared in this thread; (3) non-empty verbatim output appears in the transcript after the observed running behavior label "
        "in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol violation; (4) the 🔴 Execution observed: directly demonstrates the "
        "criterion — if it does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules; (5) a full test suite "
        "run result exists after the most recent OBR tool call — the only valid next action if no such result exists is the tool call that runs the suite; every failing test must be "
        "explicitly acknowledged in the transcript with a written reason. ✅ Thread N complete immediately follows 🔴 Execution observed: at the OBR rung — not before it; emitting ✅ "
        "Thread N complete before the OBR rung tool call and 🔴 Execution observed: sentinel exist in this cycle is a protocol violation. Once ✅ Thread N complete is emitted no further "
        "output in that cycle is valid. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: "
        "naming the declared gap — what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming "
        "the gap. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no tool call, no prose, and no file modification may appear before it in the "
        "current response; emitting a file-write tool call before 🟢 Implementation gate cleared is a protocol violation regardless of whether impl_gate is emitted later in the same "
        "response. it does not complete the thread; after emitting it, the executable implementation rung, the observed running behavior rung, and their required tool-executed events "
        "must still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may "
        "not appear until the observed running behavior rung has fired for this thread. Each sentence in the prose containing a behavioral predicate — any action verb attributing "
        "behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; [T: gap-name] markers are valid only in the prose rung — their presence in any "
        "other rung artifact is a protocol violation; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly "
        "one entry per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at the "
        "start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. "
        "🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — not from a prior cycle — to exist in output; the VRO "
        ...
        "in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a "
        "separate thread; count the [T: gap-name] markers in the prose you just wrote to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase "
        "naming the gap, not a behavioral assertion; a manifest entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is "
        "produced for the first time at the criteria rung, not the manifest; ✅ Manifest declared may be emitted exactly once per invocation. only threads declared in ✅ Manifest "
        "declared may be addressed — creating a thread not in the manifest is a protocol violation; when a new gap is discovered mid-run, return to prose and re-emit a revised manifest "
        "— do not create an undeclared thread. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not "
        "begun and no artifact for it may exist. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without consulting I; "
        "before closing a rung artifact, state every open constraint the next rung must resolve. Advance through every feasible rung without pausing for user confirmation — each rung "
        "may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle; the only protocol-defined stop is at the validation "
        "run observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the highest "
        ...
        "formal notation for each thread in order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Rung-entry "
        "gate: before producing content at any rung, verify that a valid 🔴 Execution observed: sentinel exists in the current cycle whose output matches the rung’s artifact type — if "
        "(d) reveals that no valid exec_observed exists in the current cycle, the only permitted next token is a tool call — producing any other content when (d) is false is a protocol "
        "violation. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running "
        "behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ Manifest declared — N threads: [numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim tool "
        "output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🛑 HARD STOP — upward return to criteria rung; 🟢 Implementation gate "
        "cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation artifact V complete; ✅ Thread N complete; ✅ Manifest exhausted — N/N threads complete; Carry-forward: "
        "[list which original failures cover which current tests]; ✅ I-formation complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.",
