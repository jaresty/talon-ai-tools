...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "next token is the criteria rung label for Thread 1 — no criteria for other threads, no planning narration, no other content. No criterion for Thread N+1 may appear until ✅ "
        "Thread N complete has been emitted for Thread N’s current cycle. no rung content of any type for Thread N+1 may appear before ✅ Thread N complete — the sequential constraint "
        "covers all seven rungs, not only criteria. no anticipation of future gaps, no additional invariants, no coverage beyond the declared gap; one edit means exactly one tool call "
        ...
        "table does not render” is valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be "
        "emitted unless a ✅ Thread N complete sentinel exists for each thread declared in ✅ Manifest declared — before emitting ✅ Manifest exhausted, locate the N in the ✅ Manifest "
        "declared sentinel and count ✅ Thread N complete sentinels — if the count does not equal the declared N, the manifest is not exhausted. one independently testable behavior "
        "derived from the prose alone — if stating it requires names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it "
        "is a conjunction — split before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a "
        "structural element and its data source is a conjunction — split before continuing; asserting a column header is present does not satisfy this rung — add a criterion asserting "
        "its content or effect; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a criterion written "
        "without a citable manifest excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a "
        "protocol violation; each criterion states (a) one observable behavior and (b) a falsifying condition referencing no implementation internals (no file names, function names, or "
        "variable names); this is a content gate, not a self-check; exactly one criterion may be written per thread per cycle — after the first criterion is written, the only valid "
        "next token is the formal notation rung label; a second criterion for the same thread is a protocol violation — stop, return to the prose rung, and split the thread before "
        ...
        "violation. The formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes "
        "what must be true — the executable or testable part. Natural language labels, introduces, and explains the specification — prose adds context but the behavioral constraints "
        "must still be encoded in notation.it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural "
        "invariants) describe what notation looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces "
        "only type signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is that it cannot be directly compiled or executed without "
        "modification; any construct that compiles or runs as-is is implementation code and voids the rung; formal notation describes what must be true, not how to implement it — "
        "permitted: type signatures without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: complete function bodies, variable assignments, constant "
        "declarations (const, let, var assignments) regardless of what they assign — the presence of an assignment operator outside a type definition indicates implementation code, "
        "working logic sequences; labeling prohibited content as pseudocode does not exempt it from this prohibition; if the criterion is already a complete behavioral specification — "
        "naming observable behavior and a falsifying condition without referencing function names, field names, or implementation internals — the formal notation artifact may restate "
        "it verbatim with an explicit declaration that no elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript and the audit section still "
        "applies; the audit section is separate and named; the rung is incomplete until all criteria are encoded. Natural language may appear as section labels or introductions (e.g., "
        "‘This invariant captures the rendering constraint:’) but may not substitute for encoding a constraint in notation — if a constraint can be stated in notation, it must be. each "
        "test function asserts exactly one behavioral property — one behavioral property per test function; multiple assertions are permitted only when they jointly constitute a single "
        "indivisible check (e.g., asserting a value is both non-null and of the expected type); before writing the test, re-read the formal notation and assert each structural "
        "constraint it encodes; confirm via tool call that the criteria artifact for this cycle was produced in this cycle's transcript; if the criteria artifact is identical to a "
        "prior cycle’s criteria artifact, the criterion has not changed — return to the prose rung and re-emit with an updated gap before descending again; before writing the test, "
        "check for an existing test file covering the same module — add there if so; when the artifact is modified, invoke a tool call to read the current test file before emitting "
        "carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim "
        "test name as written in the artifact]’ — each prior-failure name must be quotable verbatim from a 🔴 Execution observed sentinel; modification without carry-forward is a "
        "traversal violation; no implementation artifact may appear after modification until carry-forward has been emitted. when the declared intent is to modify a test artifact, the "
        "test-under-modification is the implementation artifact for this cycle and the validation artifact must be a meta-test — an executable artifact that fails when the "
        "test-under-modification does not exhibit the criterion; before emitting ✅ Validation artifact V complete, the artifact content must appear in the current response — emitting "
        "the sentinel without quoting the artifact in the transcript is a protocol violation; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact "
        "path does not pre-exist or is already failing — V-complete may not be emitted without this tool-executed result in the transcript; asserting confirmation without showing the "
        "exec_observed block does not satisfy this gate — the exec_observed block must appear in the current response; the validation artifact must reside within the project’s "
        ...
        "test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness; a green 🔴 Execution "
        ...
        "first failure; stop there — do not enumerate multiple failures or plan fixes; after a red VRO that is not a harness error, the criterion re-statement is an anchor only — "
        ...
        "the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running behavior label, re-emit the criterion for this cycle — "
        "criterion re-emission is required: emitting the OBR label without immediately following it with the criterion text is a protocol violation; the tool call is the only valid "
        "next action after criterion re-emission; planning statements, diagnostic narration, and commentary between criterion re-emission and the tool call are protocol violations; ✅ "
        "Thread N complete may not appear until a tool call exists in the transcript after the observed running behavior label in the current cycle; then immediately invoke the "
        "implementation as a live running process via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific "
        "behavior named in the criterion. a test runner invocation does not satisfy this gate — running tests produces validation-run-observation-type output, not "
        "observed-running-behavior-type output; the invocation target must be the implementation artifact itself, not a test harness wrapping it; A file read never satisfies this rung "
        "— the block must contain output produced by running the artifact. If the artifact requires a start step before querying (e.g., a web server), two tool calls are required in "
        "sequence: process-start then query — the 🔴 Execution observed: sentinel may not be emitted until both complete. In-process rendering (renderToStaticMarkup, test-framework "
        "render) is acceptable only when no runnable artifact exists. If no invocable process exists, open a gap cycle to make the artifact directly invocable. Before any tool call at "
        "this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — the provenance statement does not replace the tool call — the tool call "
        "must follow it in the same response; ✅ Thread N complete may not be emitted unless all of the following exist in this cycle's transcript: (1) a tool call exists in the "
        "transcript after the observed running behavior label in the current cycle; (2) observed running behavior label has been written after the most recent 🟢 Implementation gate "
        "cleared in this thread; (3) non-empty verbatim output appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no "
        "following 🔴 Execution observed: is a protocol violation; (4) the 🔴 Execution observed: directly demonstrates the criterion — if it does not directly demonstrate the criterion, "
        "emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules; (5) a full test suite run result exists after the most recent OBR tool call — the "
        "only valid next action if no such result exists is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written "
        "reason. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal notation, executable validation, executable "
        ...
        "artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no "
        "tool call, no prose, and no file modification may appear before it in the current response; emitting a file-write tool call before 🟢 Implementation gate cleared is a protocol "
        "violation regardless of whether impl_gate is emitted later in the same response. it does not complete the thread; after emitting it, the executable implementation rung, the "
        "observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool call "
        "that creates or modifies an implementation file; ✅ Thread N complete may not appear until the observed running behavior rung has fired for this thread. Each sentence in the "
        "prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; [T: "
        ...
        "declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been "
        "output has not begun and no artifact for it may exist. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without "
        "consulting I; before closing a rung artifact, state every open constraint the next rung must resolve; faithful derivation requires form to change, intent to stay fixed. "
        "Advance through every feasible rung without pausing for user confirmation between rungs — completeness governs rung depth, not rung existence; each rung may not be skipped or "
        "combined with another, including the observed running behavior rung — it is required in every cycle; the only protocol-defined stop is at the validation run observation rung "
        "after emitting 🔴 Gap:; stopping between rungs at any other point is a protocol violation — waiting for user confirmation between rungs is a protocol violation; all other rung "
        ...
        "describes files created or modified, or names gaps as resolved is a final report regardless of its heading; Such prose may not appear until ✅ Manifest exhausted exists in the "
        "transcript for this invocation; if no ✅ Manifest exhausted exists, stop: emit ✅ Manifest exhausted if all threads are complete, or open the next gap cycle if they are not. "
        "After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order — no new behavioral claims, no coverage "
        "summaries, no suggestions; then reconcile any documents the implementation affects. Rung-entry gate: before producing content at any rung, state (a) the rung name, (b) the "
        "current gap as a currently-false behavioral assertion, (c) the artifact type this rung requires, and (d) whether a 🔴 Execution observed: sentinel exists in the current cycle "
        "and, if so, whether its output is of type (c) — if no exec_observed exists yet in this cycle, state that explicitly rather than treating prior-cycle output as satisfying this "
        "check; if any of (a)–(d) cannot be stated from the current-cycle transcript, produce it before any other content at this rung; if (d) reveals that no valid exec_observed "
