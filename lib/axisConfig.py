...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "(1) 'Observing: property [N] \"<verbatim property definition>\" — <what the artifact observes>' — a property with no Observing: line is ungoverned and does not satisfy this "
        "token; (2) write a concrete automatic regression guard for property [N] via a tool call and wait for the tool-result block confirming it was written; planning or describing a "
        "guard does not satisfy this token; for every executable assertion in the written guard emit exactly one 'Property [N] assertion [M]: <quoted assertion text>' line — every "
        "executable assertion must appear exactly once and only once; omitting, merging, or inventing an assertion does not satisfy this token; after the complete assertion inventory, "
        "emit 'Quoted test: <smallest contiguous verbatim executable assertion or guard uniquely identifying the written guard>'; no subsequent step may occur until this sentinel has "
        "been emitted; (3) attempt to falsify the quoted guard by constructing a concrete scenario in which property [N] is absent while every listed 'Property [N] assertion [M]:' "
        "nevertheless succeeds; emit 'Test blind-spot: found — <construction>' or 'Test blind-spot: not found'; if found, strengthen the guard and repeat from step (2); (4) execute "
        'the quoted guard in a state where property [N] is violated and emit \'Failure: property [N] "<verbatim property definition>" — <verbatim quoted failure line from a '
        "tool-result block>' or 'Unobservable: property [N] \"<verbatim property definition>\" — <reason>'; emit Unobservable only when no executable guard, assertion, constraint, or "
        "rule could ever distinguish satisfaction from violation regardless of the governed artifact; if a guard can exist, execute it against a violating state and use the resulting "
        "failure line instead; before modifying the governed artifact to satisfy property [N], a verbatim Failure line from a tool-result block must appear; if the failure line does "
        "not specifically witness the violation of property [N], modify the violating state and repeat from step (4); (5) modify the governed artifact to satisfy property [N], then "
        "immediately emit 'Quoted implementation: <smallest contiguous verbatim text uniquely identifying the modified artifact>'; no subsequent step may occur until this sentinel has "
        "been emitted; (6) attempt to construct a minimal modification to the quoted implementation that preserves the success of the quoted guard while changing the governed "
        ...
        "'Implementation overreach: found — <modification or execution path>' or 'Implementation overreach: not found'; if found, minimally revise the implementation to eliminate the "
        "overreach, emit a new 'Quoted implementation:', and repeat from step (6); every implementation overreach analysis must reason only from the quoted implementation and quoted "
        "guard emitted earlier in the transcript, not from an imagined or intended version. After all properties complete these steps, emit 'Coverage: complete' if all properties are "
        "resolved, or 'Coverage: gap — property [N] \"<verbatim property definition>\"' for any unresolved property. A response that omits any required sentinel, performs a step "
        "before emitting its required quoted artifact, modifies the governed artifact to satisfy property [N] before a quoted Failure line appears, omits any executable assertion from "
        "the assertion inventory, or leaves a property without an automatic regression guard does not satisfy this token.",
        ...
        "are derived exclusively from this interpretation. Every property must be derivable from the explicit constraints of the chosen interpretation. A property that introduces a "
        "constraint not implied by those constraints is out of scope and must not appear in the block. Each property is written on its own line beginning 'property [N]:' followed by a "
        "formal definition having a single unambiguous denotation. The definition must be expressed as one of: a typed mathematical expression, a logical predicate (for example using "
        "∀, ∃, ⇒, ⇔), or a formally defined relation. Two independent readers applying the same interpretation must reach the same conclusion about whether a construction satisfies the "
        "property. A property is atomic when no pair of strictly simpler properties satisfies both of the following: (1) their conjunction has identical coverage to the original "
        "property; (2) each part admits at least one admissible construction that falsifies that part without falsifying the other. To test atomicity, provisional candidate "
        "sub-properties may be introduced immediately before the split test using: 'property [Na]: <definition>' and 'property [Nb]: <definition>' — these provisional lines exist only "
        "for evaluating the proposed split and are not members of the property set unless the split succeeds. Immediately after the provisional lines, emit exactly one split test: '§ "
        'split test: property [Na]: "<verbatim definition>" — falsified by <construction>; property [Nb]: "<verbatim definition>" — falsified by <construction>; these falsifiers are '
        "independent.' The quoted definitions must match the immediately preceding provisional definitions verbatim. A split succeeds only if: the conjunction of the accepted "
        ...
        "admissible constructions within the chosen interpretation. If the split succeeds, replace the original property with the accepted sub-properties and continue recursively "
        "testing each accepted property. If no valid split exists, emit: '§ split test: property [N]: \"<verbatim definition>\" — atomic, no valid split: <reason>.' The quoted "
        "definition must match the retained property verbatim. Continue recursively until every retained property has a split test concluding 'atomic, no valid split.' After all "
        ...
        "to identify one explicit request constraint that is not represented by any retained property. If such a constraint exists, emit '§ properties complete? no' — add one new "
        "property covering that explicit constraint, perform the required split testing, and repeat the completeness check. Only when every explicit request constraint is represented "
        "by at least one retained property may the response emit '§ properties complete? yes' — a '§ properties complete? yes' line without a preceding completeness check does not "
        "satisfy this protocol. After completeness has been established, verify observational independence. A retained property is observationally independent if there exists at least "
        "one admissible construction that violates that property while satisfying every other retained property. Any retained property that is not observationally independent is "
        "observationally redundant and must be removed or merged, provided doing so does not reduce coverage of the explicit request constraints. If a merge or removal changes the "
        "retained property set, repeat the atomicity and completeness procedures. The Ground properties block is complete only when all retained properties simultaneously satisfy: they "
        "are derived from the chosen interpretation; they are atomic; they collectively cover every explicit request constraint; they are observationally independent; they introduce no "
        "out-of-scope constraints. Beginning any test, implementation step, tool call, or task reasoning before the Ground properties block is complete does not satisfy this protocol.",
