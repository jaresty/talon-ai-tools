...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "concrete scenario in which property [N] is absent while every listed 'Property [N] assertion [M]:' nevertheless succeeds; emit 'Test blind-spot: found — <construction>' or "
        "'Test blind-spot: not found'; if found, strengthen the guard and repeat from step (2); (4) the text immediately following 'Test blind-spot: not found' must be the tool call "
        "that executes the quoted guard against a deliberately constructed violating state — no narrative, planning, implementation change, 'Quoted implementation:', 'Implementation "
        "overreach:', or other sentinel may appear between 'Test blind-spot: not found' and that execution tool call; no transcript content may appear between the execution tool call "
        "and the corresponding tool-result block — the execution tool call and tool result are an atomic pair; execute the quoted guard against a deliberately constructed violating "
        "state — if the governed artifact already satisfies property [N], first construct a minimal temporary modification to the artifact that violates property [N], execute the "
        "guard against that temporary state, emit the required Failure: line from the tool-result block, then restore the original state before continuing; the text immediately "
        "following the execution tool-result block must be the Failure: line for property [N] — no narrative, causal argument, prose explanation, reference to a prior failure, or "
        "other content may appear between the tool-result block and the Failure: line; a Failure: line that does not immediately follow a tool-result block does not satisfy this "
        "token; emit 'Failure: property [N] \"<verbatim property definition>\" — <verbatim quoted failure line from a tool-result block>'; emit 'Unobservable: property [N] \"<verbatim "
        "property definition>\" — <reason>' only after demonstrating that no temporary violating state can be constructed within the governed artifact on which the guard could "
        "distinguish satisfaction from violation — Unobservable: is the last resort, not the first; successful compilation, build output, or unrelated verification does not satisfy "
        "this step — the violating state must exist solely to witness that the guard detects the absence of property [N]; before modifying the governed artifact to satisfy property "
        "[N], a verbatim Failure line from a tool-result block must appear; if the failure line does not specifically witness the violation of property [N], modify the violating state "
        "and repeat from step (4); before emitting any witness classification, determine whether every inventoried assertion can be classified mechanically from the immediately "
        "preceding execution tool-result block — a witness is mechanically derivable iff the tool-result (i) identifies exactly one assertion, (ii) identifies its outcome (pass or "
        "fail), and (iii) no other assertion satisfies the same identifier; if any witness is not mechanically derivable by this operational definition, temporarily instrument the "
        "regression guard or construct an equivalent observation harness that executes exactly the inventoried assertions without strengthening, weakening, removing, or reordering "
        "them, while emitting observable per-assertion outcomes; before executing the harness, emit 'Observation correspondence: assertion [M] -> <observation point>' for every "
        "inventoried assertion, establishing a declared one-to-one mapping from every inventoried assertion to its corresponding observation point; if a one-to-one mapping cannot be "
        "established, revise the harness; execute the instrumented guard or harness against the same violating state via a tool call and wait for the tool-result block; after "
        "execution, verify that every declared observation point produced exactly one observation in the tool-result — if any observation point produced zero or more than one "
