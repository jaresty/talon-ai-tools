...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "violation regardless of whether 🔵 Closing observation is present. For software behavioral change, the standard ladder for each gap-closing cycle produces: prose → criteria → "
        "formal notation → executable validation → executable implementation. A derived ladder that omits any of these rungs for a software behavioral change task must cite which "
        ...
        "begun. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend. Sentinel formats — ✅ Ground entered — prose rung begins [gate: first "
        "content emitted in this response; no artifact, code, prose, or reasoning may precede this sentinel — preceding content voids the session]; ✅ Manifest declared — N threads: "
        "[numbered list of behavioral gaps] [gate: rung table produced in current response; rung table precedes this sentinel]; 🔴 Execution observed: [verbatim tool output — "
        "triple-backtick delimited, complete, nothing omitted] [gate: tool call made in the current response immediately before this sentinel; verbatim output in triple-backtick block "
        "follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed with non-empty failing output precedes this token in current response; gap text is a currently-false "
        "behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: exec_observed showing test suite failure + gap in current cycle; criterion identical to prior cycle "
        "criterion for this thread]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed] [gate: exec_observed showing test suite failure + gap in current "
        "cycle; valid only at the rung whose artifact type is executable implementation; the verbatim exec_observed output must contain at least one assertion-level failure string "
        "(e.g. 'Expected … Received …', 'AssertionError', 'FAILED', specific test name + failure reason) — an exec_observed whose output shows only infrastructure failure, import "
        "error, process crash, or a model-described summary of failure does not satisfy this gate; if the suite cannot reach assertions, the implementation artifact for that cycle "
        "resolves only the infrastructure gap, and this gate cannot be emitted until a subsequent cycle produces assertion-level failure output]; ✅ Criteria complete — 1 criterion: "
        "[gap-name] [gate: exactly one criterion in this rung's artifact; criterion contains no conjunction; formal notation rung label may not be emitted until this sentinel has been "
        "emitted]; ✅ Validation artifact V complete [gate: pre-existence check tool call result present; tool call in this response wrote the file to disk]; ✅ Thread N complete [gate: "
        "meta exec_observed after executable implementation shows the gap declared for this thread is no longer present in running behavior]; ✅ Manifest exhausted — N/N threads "
        "complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; Carry-forward: [list which original failures cover which current tests] [gate: prior "
        "failure name quotable verbatim from a prior exec_observed sentinel]; ✅ I-formation complete [gate: observation of current state complete before manifest]; ✅ Formal notation R2 "
        "audit complete — N/N criteria encoded [gate: every criterion encoded in notation; audit section named and separate]; ✅ Ground complete — intent achieved: [what the observation "
