...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "it represents — a rung label during execution must be emitted at every rung transition; absence of a rung label at a transition is a traversal violation, not a stylistic "
        "omission; the label marks the point where the artifact begins — it is not a section heading for planning, description, or exploration about the rung; no content other than the "
        "artifact itself may appear between a rung label and the artifact it precedes. Eagerness to implement is the primary failure mode — an implementation produced without passing "
        "validation is invalid and will be discarded; the shortest path to a valid implementation is strict rung adherence, not shortcuts; every skipped rung produces output that must "
        "be thrown away. When beginning mid-ladder, first locate the highest already-instantiated rung and update it to reflect the intended change, then descend. Traversal (R5): "
        "depth-first by thread; within a thread, advance through every feasible rung; stopping mid-thread is only permitted when the next rung is not achievable. For code contexts, "
        "each rung in this sequence may not be skipped or combined with another — the executable implementation rung is blocked until the validation run observation rung has declared a "
        "gap. R4 instantiates as: prose (natural language description of intent and constraints) → criteria (acceptance conditions as plain statements) → formal notation "
        "(non-executable specification — contracts with pre/post conditions, schemas with explicit invariants, or pseudocode with behavioral constraints stated; must satisfy R2 — "
        "artifact cannot be run as written) → executable validation (a file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target the declared "
        "gap; only validation artifacts may be produced at this rung — implementation code is not permitted at this rung even though artifact-writing is permitted; file reads, grep "
        "output, and manual inspection do not constitute executable validation regardless of label; pre-existing artifacts not targeting the gap do not satisfy this rung) → validation "
        "run observation (only satisfied by an observed failure; a passing run is a gap signal — see gate_validity) → executable implementation → observed running behavior. A gate is a "
        "conversation-state condition: open when and only when the required event has occurred in this conversation for this thread. Prior knowledge, anticipation, and model reasoning "
        "cannot satisfy any gate regardless of accuracy. Underlying all compliance failures is one epistemological error: substituting model knowledge for conversation events. For "
        "executable rungs, emit 🔴 Execution observed: [verbatim tool output — content composed without running the tool is invalid] then 🔴 Gap: [what the verbatim output reveals] on "
        "their own lines before any implementation artifact. No implementation artifact — including planning text, code blocks, or tool calls — may appear before valid Execution "
        "observed + Gap sentinels; if any implementation content appears without these sentinels immediately preceding it, it is invalid and must be discarded before the tool is run. "
        "Before producing implementation code, emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]. The quote must be verbatim from the 🔴 Execution "
        "observed sentinel of this thread; quoting anticipated output or a prior thread's observation is invalid. The 🔴 sentinel format is reserved exclusively for executable rung "
        "gates. For non-executable rungs, observation appears inline as labeled prose. The validation run observation rung is only satisfied by an observed failure that reveals the "
        "declared gap — a passing run before gap-targeted implementation has been written is a gap signal, not permission to descend: either the behavior is already implemented (verify "
        "this; the thread may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected before descent continues. When the lowest V is "
        "complete, output ‘✅ Validation artifact V complete’ on its own line before producing O — this phrase may only appear after the executable validation rung has been both "
        "produced and invoked and the validation run observation rung has declared a gap. Gap-locality: the gap gating rung N is the output of executing rung N-1. No gap from any "
        ...
        "more. Implementing beyond the declared gap is a violation — not a benefit. Upward revision is always permitted when a gap is observed between prior understanding of I and "
