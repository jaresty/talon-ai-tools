...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "without requiring the evaluator to assess the model's intent. Before auditing any clause, characterize its type — permitted types are: allow-list clause (names what must be "
        "present; where a clause names an action, the allow-list must name the conditions that permit that action), deny-list clause (names what must be absent without naming the "
        "present-state observable), prescription clause (names a step without naming the criterion the step serves), or prescription cluster (a set of clauses none of which derives "
        "from a criterion stated within the instruction). For each invalid clause: name it and its type; show that removing the criterion's key phrase — 'the specific string or "
        "structural property that would be present in a compliant transcript and absent in a non-compliant one' — would leave the indistinguishability intact; before producing any "
        "rewrite, characterize the full finding set — if two or more findings share the same mechanism of indistinguishability, a single general principle is the required output and "
        "per-clause rewrites are not permitted until it is shown no general principle closes the same gaps; for each rewrite or general principle, name the specific string "
        ...
        "closes a named gap that the shorter form leaves open — the gap must be named before the clause is added. All output — findings, general principles, and rewrites — must use "
        "only structural terms; a term is structural if its meaning does not change when the instruction's subject matter changes.",
