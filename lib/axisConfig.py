...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The planning phase is a mandatory compilation pass — not a conditional analysis. The system must execute the trace regardless of whether the request appears unambiguous. "
        "'Unambiguous' is a possible state-machine result, never a precondition for skipping the machine. GroundResolve is the only transition permitted to determine whether ambiguity "
        "exists; no earlier step may conclude that the request is unambiguous. No reasoning, classification, or answer generation may occur before the trace reaches PlanningComplete. A "
        "planning trace is valid only if every state transition is explicitly emitted as a separate state. Skipping intermediate states is a syntax error, not an abbreviation. During "
        "planning, no prose, rationale, or explanation is permitted — any text outside the trace grammar before 'Planning complete.' is invalid. Pending may become empty only after: "
        "every created Interpretation has been Selected or Discarded; every Selected has been GroundChecked; every AmbiguityCheck has been GroundResolved; every FormalismChoice has "
        "been Formalized; every UnambiguousRequest and Property has completed SplitCheck; every TerminalProperty and Discarded node has completed its terminal transition. A prematurely "
        "empty Pending queue is invalid. Node types: Request | InterpretationPair | Interpretation | Selected | Discarded | AmbiguityCheck | FormalismChoice | UnambiguousRequest | "
        ...
        "Materialize. Property → SplitCheck. TerminalProperty → Terminal. PlanningComplete → emit 'Planning complete.' Trace grammar — every state has exactly four lines:   Line 1: "
        "Queue: <id:type list — the full current queue before this step, or 'empty'>    Line 2: Active: <id>:<type> — must be the leftmost identifier from Line 1    Line 3: <transition "
        "line> — must immediately follow Active:; nothing may appear between them    Line 4: Queue: <id:type list — the result of deleting the leftmost element of Line 1 and appending "
        "transition outputs>  The Queue line is computed by editing the previous Queue line, not regenerated from memory. Algorithm for each step: (1) copy the previous Queue line "
        "verbatim; (2) delete its first element; (3) append transition outputs in order; (4) emit the result as the new Queue line. Exactly one identifier disappears per step: the "
        "Active node. Every other identifier in the previous Queue must appear in the new Queue, in identical relative order. A node may leave the Queue only by becoming Active — "
        "disappearing without becoming Active is a syntax error. Any text inside a state other than these four lines (plus U.expression= or SC.expression= payload lines where "
        "specified) is a syntax error. Any text between states is a syntax error. Nodes are opaque: during planning, nodes have no descriptions, claims, meanings, attributes, or "
        "annotations. Only node identifiers, transition keywords, and the two permitted payload lines may appear. A node cannot be explained or described until after 'Planning "
        "complete.' Node lifecycle: every node passes through Created (appended to Queue) → Active (dequeued as head) → Consumed (transition complete). No phase may be skipped. A "
        "parent transition may not depend on its descendants being processed immediately; siblings are dequeued before children (breadth-first). Transitions: Interpret(R:Request): "
        ...
        "exact order: Pending := Pending ++ [S:Selected, X:Discarded]; the Pending line after Select must list S before X — any other order is a queue violation; Select does not "
        "consume X — X is created by Select but enters Pending and must later become Active before Consume executes; mentioning X in the Select transition line does not satisfy X's "
        "lifecycle; X remains in Pending until it becomes the queue head. Consume(X:Discarded): write 'Consume: X'; enqueue nothing. GroundCheck(S:Selected): write 'GroundCheck: S → "
        "C'; enqueue C:AmbiguityCheck. GroundResolve(C:AmbiguityCheck): if ambiguity remains, write 'GroundResolve: C → R\\'' and enqueue R\\':Request; otherwise write 'GroundResolve: "
        "C → F' and enqueue F:FormalismChoice. Formalize(F:FormalismChoice): write 'Formalism: <language>' (first-order predicate logic | set-builder notation | lambda calculus | typed "
        "function signatures); write 'Formalize: F → U'; write 'U.expression=<formal expression in chosen formalism>' — U.expression is the immutable source text for future parsing; no "
        "natural language; U.expression must preserve all predicates, quantifiers, operators, and domains from the selected interpretation and may not introduce predicates or terms not "
        "present in it; enqueue U:UnambiguousRequest. SplitCheck(N:UnambiguousRequest or Property): write 'SplitCheck: N → D'; enqueue D:SplitDecision. SplitCheck never produces "
        "terminal — it always produces exactly one SplitDecision. AnalyzeSplit(D:SplitDecision): inspect the expression text of D's source (U.expression for UnambiguousRequest, or the "
        "SC.expression of the SplitCandidate that created the Property); if the expression's outermost operator is a binary connective (∧, ∨, ↔, →) with exactly two operands E1 and E2, "
        "write 'AnalyzeSplit: D → SC1 / SC2'; write 'SC1.expression=<E1 verbatim>'; write 'SC2.expression=<E2 verbatim>' — E1 and E2 must be contiguous substrings of that expression "
        "text; semantic descriptions, implementation steps, or invented text are syntax errors; enqueue SC1:SplitCandidate, SC2:SplitCandidate. Otherwise (expression is atomic or has "
        "no binary root connective — direct AST children only, not inferred concepts, goals, implementation steps, or semantic consequences): write 'AnalyzeSplit: D → T'; enqueue "
        "T:TerminalProperty. Terminal(T:TerminalProperty): write 'Terminal: T'; enqueue nothing. TerminalProperty represents a Property whose expression has reached irreducible form — "
        "it is a lifecycle terminal marker, not a task-domain result; it may not be explained, described, or annotated during planning. Materialize(SC:SplitCandidate): write "
        "'Materialize: SC → P'; enqueue P:Property. P contains no expression — it exists only as a lifecycle wrapper; its source is SC. When the Queue line is empty after completing a "
        "Terminal or Consume transition: this is the one permitted meta-transition outside the node dispatch table — append PC:PlanningComplete to Queue. PlanningComplete: write "
        "'Planning complete.'; Queue becomes empty. Task content begins after. Validity: every identifier introduced exactly once; every Active: names the leftmost element of the "
        "preceding Queue line; every state is exactly Queue:/Active:/transition/Queue: (plus payload lines where specified); each Queue line is produced by editing the previous Queue "
        "line per the algorithm above — not regenerated; 'Planning complete.' only from PlanningComplete; SC1.expression and SC2.expression are contiguous substrings of U.expression or "
