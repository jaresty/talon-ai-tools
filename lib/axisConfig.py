...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "atomic": "The response enforces one governing output per step: take the first failure signal from a tool-executed run result appearing above this line in the transcript as the step's "
        "scope — 'first failure signal' means the first line in the run output that names a specific behavior absent, not a summary line that aggregates multiple such absences; a line "
        "that reports a count of failures, a test-case name, or a grouping header is a summary line and does not govern; the governing line is the earliest line whose text names the "
        "specific condition that was not met — a grouping that reports multiple distinct failure signals contains multiple governing failures; each must appear as a separate Scope line "
        "in the transcript before the implementation that closes it — a single step that closes two failure signals without a separate Scope line for each does not satisfy this "
        "requirement — the candidate is limited to the change that resolves the first failure signal only; other failures visible in the same result are not in scope for this step — "
        "they may not be cited to expand the candidate and become the governing failures for subsequent steps. Before any edit, produce three artifacts as separate completed blocks — "
        "each block must end before the next opens, and no tool call that modifies a file may appear in the same block as any artifact: (1) scope declaration quoting verbatim the "
        "failure signal from the prior run result — the quoted text must appear in a tool result above this line; (2) derivation block as two entries: first, name the candidate change "
        "and at least one smaller change; the smaller change must be the minimal stub — a function body containing only the correct type signature and, for non-void return types, a "
        "return statement of the correct type; no other statements are permitted in a minimal stub; a body containing any statement other than the return statement (or any statement at "
        "all for void functions) is not a minimal stub regardless of whether it performs the governed transformation; a change that produces a type error, a crash, or a failure mode "
        "different from the governing failure signal at the call site is not a valid smaller change; then run the smaller change and produce a tool-executed result — if the governing "
        "failure signal text is absent from that result, the smaller change is sufficient and must become the candidate; if the governing failure signal text is present, the smaller "
        "change is insufficient and the candidate is permitted; an insufficiency claim is not permitted until a tool-executed result showing the smaller change appears above it in the "
        "transcript; an insufficiency argument that names or references any failure string other than the governing failure signal text does not satisfy this requirement — the "
        "insufficiency must be derivable from the governing failure signal text alone; second, in a separate entry below, quote the specific text from the governing failure signal that "
        "shows the governed behavior absent — the quote must appear in a tool-executed run result above the first entry; a candidate that is larger than the smallest change that "
        "resolves the quoted failure text is not permitted regardless of what else it also resolves; a failure not yet observed in a prior tool-executed run result has not been shown "
        "to govern, and a change that fixes it cannot be derived from the current governing artifact; (3) line manifest listing every line to be added; for each line, quote the "
        "specific assertion string from the governing failure signal that would be absent from the run result when the line is present and would appear in the run result when the line "
        "is absent — an entry whose quoted string would remain in the run result when the line is present does not satisfy this requirement. Immediately before each tool call that "
        "modifies a file, produce: 'Scope: <verbatim quoted failure signal> | Candidate: <named candidate change> | Smaller: <named smaller change whose insufficiency is demonstrated "
        "above> | Lines: <count of lines in manifest>' — if this line does not appear immediately above the tool call that modifies a file in the transcript, the edit is not permitted; "
        "this line is not permitted unless a tool-executed result showing the smaller change applied appears above it in the transcript — a Scope line that appears without such a "
        "result above it does not satisfy this requirement. After the edit, produce a post-edit run result by tool call, then verify: (a) the assertion text from the scope declaration "
        ...
        "result exists in the transcript above that tool call, the tool call is not permitted; if execution results exist, a file is governed only if a prior tool-executed run result "
        "in this transcript names it or its output as part of a failure — governance may not be asserted in prose; a file not named in any such result is not governed for this step; A "
        "FAIL that appears after any such modification is not valid regardless of its position relative to the governing artifact citation. Before the run tool call, produce a "
        "derivation block as three entries: first, name at least one slower check that would catch this behavior but takes longer to run locally — name it by file path or command; do "
        "not run this check; name what specific test case in that check covers this behavior — the named entity must be a runnable test case that produces a named failure message (a "
        "test name or assertion string) when the governed behavior is absent; a compiler check, type annotation, or linter rule does not satisfy this requirement regardless of whether "
        "it runs on the same source files; a description that states or implies the test does not cover the governed behavior — including qualifications such as 'but not [behavior]', "
        "'does not cover', or 'cannot distinguish' — is self-refuting; when a description is self-refuting, the first entry is not complete — no candidate check may be named until a "
        "non-self-refuting slower check appears; a candidate check that appears in the transcript after a self-refuting description is invalid regardless of any intervening text; a "
        "claim that the named test cannot catch this behavior must be grounded in a prior execution result showing the test passing while the behavior was absent — quote that result; "
        "a reasoned claim of incapability without such a quote is not valid and the slower check stands as derived; if no capable test can be named and no such quote can be produced, "
        "the slower check is not derived and the candidate check must not be named — choose a different candidate for which a nameable test exists; second, name the candidate check — "
        "the specific artifact to run, naming its relative speed compared to alternatives; third, name at least one insufficient check — a faster check that exercises the same "
        "triggering event as the candidate check but at a resolution that does not distinguish the governed behavior from its absence; name the specific shared triggering event and "
        "why the resolution is coarser; a check that does not share the triggering event is irrelevant, not insufficient, and must not be named here; then, before closing this entry, "
        ...
        "candidate is not derived and a different check must be chosen. Before running the candidate check, run it against a minimal stub — a function with the correct type signature "
        "that returns a value of the correct type without performing the governed transformation; if the candidate check passes against the minimal stub, the check does not govern the "
        "implementation and a different assertion must be written before proceeding; a tool-executed run result showing the candidate check against the minimal stub must appear in the "
        "transcript before the governing FAIL result — a FAIL result that appears before the minimal-stub run result does not satisfy this requirement. Then run the candidate check. "
        "The FAIL must be produced by running an artifact that (a) contains an assertion whose output string appears in the run result when the behavior is absent and is absent from "
        ...
        "costly to maintain. Before acting, state the boundary searched — the content at the role=user position of the most recent user turn in the transcript; any noun or noun phrase "
        "in that content that resolves to an artifact not already present in the transcript must be fetched before deriving desired end states; if the referent cannot be resolved, "
        ...
        "are not valid at any stage. After each of the four artifacts (anchoring citation, intent artifact, ladder derivation, completion check), consider: would a brief visual summary "
        "communicate the artifact's current state more efficiently than the prose already does? If yes, produce one in whatever format conveys the state most directly. This display has "
        "no effect on artifact validity and does not substitute for it.",
        ...
        "requiring the evaluator to assess the model's intent — this criterion applies in any domain where instructions govern model behavior. For each clause in the subject "
        "instruction that violates the root criterion: state the root criterion as a standalone sentence before deriving anything — the sentence must be extractable without surrounding "
        "context; a sentence that references prior prose to be interpretable is not standalone; name the specific text; derive the mechanism of indistinguishability from the stated "
        "criterion — the mechanism statement must quote the specific phrase from the stated criterion and show that removing that phrase from the criterion would eliminate the "
        "indistinguishability — a mechanism statement that quotes the phrase but does not show this counterfactual does not satisfy this requirement; give a concrete example naming a "
        "specific transcript state satisfying the instruction nominally but not genuinely — name the specific string or field in that state that satisfies the instruction nominally, "
        "and state explicitly that this string is absent from any state satisfying it genuinely. Any rewrite produced is permitted only when it is no longer than the original, or when "
        "each added clause closes a named gap that the shorter form leaves open — the gap must be named before the clause is added; permitted rewrites must name the specific string or "
        "structural property that distinguishes the two compliance states before claiming the rewrite closes the gap; where a clause names an action, state exactly what conditions "
        "permit that action (allow-list); where a condition gates an action, add a post-condition naming the specific observable that would be absent if the condition were not "
        "satisfied. A clause that lists excluded behaviors rather than permitted ones is a deny-list clause; a clause that names a step without naming the criterion the step serves is "
        "a prescription clause; a set of clauses that each stand independently — none deriving from a criterion stated elsewhere in the instruction — is a prescription cluster; a "
        "deny-list clause, prescription clause, or prescription cluster must be characterized (type named for a clause; standalone count stated for a cluster) before any rewrite or "
        "audit of it appears in the transcript — a rewrite or audit preceding this characterization does not satisfy this requirement. Before producing any per-clause rewrite, "
        "characterize the finding set: if two or more findings share the same mechanism of indistinguishability, a single general principle closing all of them is the required output — "
        "per-clause rewrites for those findings are not permitted until the transcript contains a statement that no general principle closes the same gaps; the general principle is "
        "preferred when substituting it for the per-clause rewrites leaves no finding in the set unclosed — verify this by stating which gap each finding addresses and confirming the "
        "principle closes it; a per-clause rewrite that appears before this verification does not satisfy this requirement. All findings and rewrites must use only terms whose meaning "
        "is determined by the structure of instructions and the properties of clauses — a term is structural if its meaning does not change when the instruction's subject matter "
        "changes; a term whose meaning shifts with the instruction's subject matter must not appear.",
