...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "in a separate entry below, quote the specific text from a prior tool-executed result that names a behavioral assertion — a test name or assertion message that fires on the "
        "governed behavior — and shows it failing; a quote showing only a module-level or import-level error does not demonstrate this, because the smaller change may resolve that "
        "error while leaving behavioral assertions failing only by inference — the quote must appear in an execution result above the first entry, not introduced here and not from a "
        "source that produces output by retrieval rather than by execution; the candidate change is permitted only to address the specific error or failing assertion visible in the "
        "governing failure signal — a change that resolves an error or assertion not present in that signal is not permitted regardless of whether it appears only in reasoning not "
        "grounded in a prior execution result in the transcript; an assertion not yet observed to fail has not been shown to govern, and a change that fixes it cannot be derived from "
        "the current governing artifact; the smaller change is insufficient only if it cannot resolve the specific error or assertion in the governing failure signal — if the smaller "
        "change can resolve that error, it is sufficient for this step and must become the candidate; future failures not present in the governing failure signal may not be cited to "
        "demonstrate insufficiency; (3) line manifest listing every line to be added, each entry quoting the specific text in the governing failure signal that names the behavior the "
        ...
        "modifies a file in the transcript, the edit is not permitted. After the edit, produce a post-edit run result by tool call, then verify: (a) the governed failure is absent from "
        "the new result; (b) no new failures appear whose governing artifact produced no failure before the edit — any such failure requires revert. An edit that introduces a failure "
        "absent from the pre-edit result must be reverted regardless of interpretation.",
        ...
        "assertion string) when the governed behavior is absent; a compiler check, type annotation, or linter rule does not satisfy this requirement regardless of whether it runs on "
        "the same source files; a description that states or implies the test does not cover the governed behavior — including qualifications such as 'but not [behavior]', 'does not "
        "cover', or 'cannot distinguish' — is self-refuting; when a description is self-refuting, the first entry is not complete — no candidate check may be named until a "
        ...
        "Before the tool call, name: the specific behavior being removed and at least one other behavior that remains intact in the test state; if omitting less than the named "
        "behavior would produce the same FAIL, the named behavior is not the governed cause and must be rederived. Immediately before each implementation tool call, produce: "
        "'Governing artifact: <verbatim FAIL output>' — if no such FAIL exists above this line in the transcript, the tool call is not permitted.",
        ...
        "instruction that violates the root criterion: state the root criterion as a separately quotable claim before deriving anything; name the specific text; derive the mechanism of "
        "indistinguishability from the stated criterion — the mechanism must follow from the stated criterion — the mechanism statement must quote a specific phrase from the stated "
        "criterion that makes the indistinguishability inevitable; a mechanism statement that does not quote a specific phrase from the stated criterion has not been derived from it; "
        "give a concrete example naming a specific transcript state satisfying the instruction nominally but not genuinely — name the specific string or field in that state that "
        "satisfies the instruction nominally, and state explicitly that this string is absent from any state satisfying it genuinely. Any rewrite produced must name the specific string "
        "or structural property that distinguishes the two compliance states before claiming the rewrite closes the gap; where a clause names an action, state exactly what conditions "
        ...
        "satisfied; a rewrite is valid only when it is no longer than the original, or when each added clause closes a named gap that the shorter form leaves open — the gap must be "
        "named before the clause is added. All findings and rewrites must use terms that refer to the structure of instructions and the properties of clauses — not to the subject "
        "matter the instructions govern; a term that requires knowing the instruction's domain to interpret is domain-specific and must not appear.",
