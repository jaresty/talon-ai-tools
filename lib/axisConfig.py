...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "a time — the prose rung of thread N+1 may not begin until ✅ Thread N complete has been emitted for thread N. Before writing the criteria, formal notation, executable "
        ...
        "followed inline by a bracketed thread marker: [T: gap-name]. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any "
        "cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from "
        "the current cycle’s validation run observation rung — not from a prior cycle — to exist in output. Before emitting ✅ Manifest declared, scan every sentence in the prose for "
        "behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; "
        "re-read the prose rung output via a tool call to confirm every predicate is covered; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is "
