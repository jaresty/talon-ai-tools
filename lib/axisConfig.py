...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "of any tool-executed result block above this call, the symbol is new; a new-symbol call is compliant only if the introduced code body for that symbol contains exactly one "
        "statement — a no-op, empty return, panic, or language-idiomatic not-implemented marker; a body containing more than one statement does not satisfy this requirement; the quoted "
        "scope text must be from a test assertion whose pass/fail state is determined solely by the named symbol — a test that passes or fails independently of that symbol does not "
        "satisfy this requirement; a statement in the derivation phase is a commitment only if its truth value can be determined from tool-executed results already present above it in "
        "the transcript — a statement whose truth value requires evaluating a result not yet present is not a commitment; after the call, a tool-executed run result must appear in the "
