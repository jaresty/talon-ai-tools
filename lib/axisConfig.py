...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "coverage is a structural scope violation that gate does not permit. Second, one assertion per behavior: an assertion covering multiple behaviors cannot be verified in isolation; "
        "each assertion must cover exactly one observable outcome — a governing artifact that checks multiple outcomes in a single verification step covers multiple behaviors and is a "
