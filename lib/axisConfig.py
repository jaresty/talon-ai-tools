...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "of any tool-executed result block above this call, the symbol is new; a new-symbol call is compliant only if the introduced symbol satisfies the stub requirement for its kind "
        "— for a callable (function, method, or constructor), the body contains no logic: an empty return, panic, throw, or language-idiomatic not-implemented marker; for a "
        "non-callable (variable, constant, exported value, or class field), the initializer is a single primitive or zero-value expression with no sub-expressions — nil, null, "
        ...
        "falsify": "The response establishes, before any governed action, that the governing artifact has been observed to detect the absence of each governed behavior. If no governing artifact "
        "exists for a governed behavior, writing that artifact is the first governed action; writing the artifact means producing (c) in a test or specification file — no modification "
        "to the implementation file is permitted before (c) appears in such a file; (a) through (g) apply to the executor invocation immediately following that creation — the creation "
        "action itself satisfies the creation-step exception below. Derivation phase (required before any governed action — complete only when a literal block labeled 'Falsify "
        "derivation:' containing entries for (a) through (g) appears in the transcript; prose does not satisfy this requirement): Name — (a) the assertion-failure marker: the literal "
        "string in an executed-artifact result signaling the artifact detected behavior absence; (b) the literal string signaling the governed behavior is present; (c) the literal "
        "string identifying each governed behavior in an executed-artifact result; (d) the literal string identifying the governed action, which must appear as a substring in the name "
        "of the symbol added or modified; (e) the layer classification — if the governed symbol name appears as a direct call in the test body, the layer is 'unit'; if the governed "
        ...
        "assertion reached execution at the layer named in (e); a governing artifact is disposable if removing the governed symbol from the implementation without changing the test "
        "leaves the artifact result unchanged — a disposable artifact does not satisfy (f); (g) the observed FAIL result: the content of a tool call result block appearing in the "
        "transcript after the artifact was applied to the subject and before the first governed action that modifies the implementation, where (a) precedes (c), the executor named in "
        "(f) produced the result, and the failure line contains a string present in the assertion body. Layer gate: if (e) is 'unit', the governed symbol name must appear in the "
        ...
        "from (c) in (g) by any line whose content contains (c) for a different governed behavior; and a subsequent executed-artifact result contains (b) at the position where (a) was "
        "absent. A result satisfying (g) must contain a failure line whose content includes a substring from the assert statement of the governed assertion; a failure line that "
        ...
        "governing artifact's assertion text, per governed behavior — the creation step is that action only; (g) is required for every action that modifies the implementation, "
        "including in the same session — a creation step is that action only if (c) is absent before the action and present after it, and an executor invocation against the governing "
        "artifact immediately following the action produces a tool-executed result containing (c) and not (a). Semantically-equivalent refactor exception: (g) is also satisfied when a "
        "tool call result block containing (a) before (c) appears in the transcript before the file-modifying call, produced by an executor invocation after the refactored form was "
        "modified to violate the invariant the governing assertion checks. The layer gate applies to this perturbation executor invocation. Revert is complete when the "
        "pre-perturbation executor run result is quoted verbatim in the transcript before the revert executor invocation, and a subsequent executor invocation produces (b) at the "
        "position where (a) was absent.",
        ...
        "gates regardless of accuracy. All compliance headings must be markdown '## ' headings — a line not beginning with '## ' does not satisfy any heading requirement. Each rung "
        ...
        "execution is present above it — '§0 observed' must not appear before such a block is present; (1) derive the governing goal: '## Governing goal: [text]' must not appear before "
        "'§0 observed' in the transcript; `[text]` must appear verbatim as a substring of the §0 tool-result block above — a `[text]` value not present as a literal string in that "
        "block does not satisfy §1; immediately below the governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping "
        ...
        "derive a means-test: write 'The goal [text] could be achieved by: [alternative means not listed in §0]' — if no concrete alternative can be named, write '[hypothetical]: "
        "[speculated alternative]' and fetch a broader context artifact before closing §1; if two non-subsuming goal candidates exist, write both and mark one '[selected]' — the "
        "selected goal is the one whose means-test names a more concrete alternative than those listed in §0; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 "
        "goal derived'; Path B — (0) write the literal string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — this declaration satisfies §0; (1) "
        "derive the governing goal: if the user's message states the goal verbatim, '## Governing goal: [text]' where [text] appears verbatim in the user's message; if the goal must be "
        "derived from a scenario description, '## Governing goal: [derived: text]' where [derived: text] is a goal derived from the scenario description without adding claims absent "
        "from the user's message — a '[derived: text]' value that adds a claim absent from the user's message does not satisfy §1; immediately below the governing goal heading, before "
        "the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', labeled subject, verb, outcome, or invariant; no field may "
        ...
        "if only one concrete alternative can be named, write '[hypothetical]: [speculated alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or "
        "hypothetical) are present, write '§1 goal derived'; (Both paths continue from §2 onward:) immediately below '§1 goal derived', derive dimension candidates: for each "
        "alternative means named in the means-test, write one candidate — a response property that would be absent in a response achieving '[text]' by a different means; when one "
        "candidate exists per alternative means, write '§1b candidates'; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the "
        ...
        "pattern whose presence in the response constitutes satisfaction — if the response contains no tool-result blocks above §2, write '[observable: prose]' instead; a dimension "
        "that names an entity, a fix outcome, or a UI element is not a valid dimension regardless of provenance; derivation is unbounded: add dimensions, challenge each against the "
        "reader-uncertainty test, and revise until every dimension is independently falsifiable; when every dimension carries an '[observable:]' tag, write '§2 dimensions closed'; (3) "
