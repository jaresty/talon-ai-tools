...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "skipped tests are not observations; observed running behavior must be traceable to the behavioral gap declared in the validation run observation — verifying that a process "
        "started or a URL responded is not sufficient; for UI artifacts, a server response does not confirm rendering; for APIs, an HTTP 200 does not confirm correct data; the "
        "observation must demonstrate the declared behavioral intent was satisfied. A rung is complete when and only when its artifact has been produced — not when it has been listed, "
        "planned, or described; a rung label during execution marks the point where the artifact begins and must be emitted at every rung transition — it is not a section heading for "
        "planning or description; absence of a rung label at a transition is a traversal violation, not a stylistic omission; no content other than the artifact itself may appear "
        "between a rung label and the artifact it precedes. A rung is not achievable when the domain provides no standard artifact type for it; this must be stated explicitly with "
        "justification — convenience or prior knowledge do not make a rung not achievable. ground is a Process method — the task governs the character of each rung’s output but not the "
        "process structure; the manifest, rung sequence, and execution gates are mandatory regardless of which task or other tokens are combined with ground; completeness governs rung "
        "depth, not rung existence. Formal notation must satisfy R2: every behavioral constraint from the criteria rung must be re-expressed as a behavioral invariant — not just "
        "interface shape; type signatures or schemas that capture structure without encoding invariants do not satisfy this rung; before advancing from formal notation, enumerate each "
        "criterion from the criteria rung and confirm it is encoded as a behavioral invariant — list any unencoded criteria explicitly; the rung is incomplete until all are addressed. "
        "When I-formation is required: observation of existing code or running behavior sufficient to establish I is required before the manifest when I cannot be declared from context "
        "alone — skipping I-formation when context is insufficient is an error, not a permitted shortcut — it produces a specification derived from assumed rather than actual "
        "constraints, and every downstream artifact will be wrong in ways that only surface at executable validation or observed running behavior, by which point multiple rungs must be "
        "discarded and rederived; output ‘✅ I-formation complete’ on its own line after declaring I and before the rung manifest — the manifest may not appear before this sentinel; "
        "exploration beyond what is needed to declare I belongs as the first rung of the manifest, not pre-manifest. Boundary: no rung work, planning text, or content of any kind may "
        "appear before the manifest-complete sentinel. Eagerness to implement is the primary failure mode — the shortest path to a valid implementation is strict rung adherence; every "
        "skipped rung produces output that must be discarded and rederived. When beginning mid-ladder, first locate the highest already-instantiated rung and update it to reflect the "
        "intended change, then descend. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; stopping mid-thread is only permitted when the next "
        "rung is not achievable. For code contexts, each rung in this sequence may not be skipped or combined with another — the executable implementation rung is blocked until the "
        "validation run observation rung has declared a gap. R4 instantiates as: prose (natural language description of intent and constraints) → criteria (acceptance conditions as "
        "plain statements) → formal notation (non-executable specification — contracts with pre/post conditions, schemas with explicit invariants, or pseudocode with behavioral "
        "constraints stated; must satisfy R2 — artifact cannot be run as written) → executable validation (a file artifact invocable by an automated tool — go test, pytest, or "
        "equivalent — written to target the declared gap; only validation artifacts may be produced at this rung — implementation code is not permitted at this rung even though "
        "artifact-writing is permitted; file reads, grep output, and manual inspection do not constitute executable validation regardless of label; pre-existing artifacts not targeting "
        "the gap do not satisfy this rung) → validation run observation (only satisfied by an observed failure; a passing run is a gap signal — see gate_validity) → executable "
        "implementation → observed running behavior. A gate is a conversation-state condition: open when and only when the required event has occurred in this conversation for this "
        "thread. Underlying all compliance failures is one epistemological error: substituting model knowledge for conversation events — prior knowledge, anticipation, and model "
        ...
        "invalid] then 🔴 Gap: [what the verbatim output reveals] on their own lines before any implementation artifact. No implementation artifact — including planning text, code "
        ...
        "inline as labeled prose. The validation run observation rung is only satisfied by an observed failure that reveals the declared gap — a passing run before gap-targeted "
        "implementation has been written is a gap signal: either the behavior is already implemented (verify this; the thread may be complete) or the validation artifact is vacuous "
        "with respect to the declared gap and must be corrected before descent continues. A build error or compile failure does not satisfy this rung — the validation artifact must "
        "compile and execute before its output can reveal a behavioral gap; a failure to compile means the artifact is incomplete and must be corrected before the gate can open. Before "
        "producing implementation code, emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; the quote must be verbatim from the 🔴 Execution observed "
        "sentinel of this thread; quoting anticipated output or a prior thread’s observation is invalid. Output ‘✅ Validation artifact V complete’ on its own line immediately after the "
        "validation run observation rung has declared a gap and before the executable implementation rung begins — this sentinel may not appear after tests pass, after implementation, "
        ...
        "inline without a new manifest is a scope violation regardless of how it is labeled. Upward revision is always permitted when a gap is observed between prior understanding of I "
