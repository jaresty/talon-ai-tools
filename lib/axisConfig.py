...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "criterion — the mechanism statement must name the specific phrase from the stated criterion that is violated by the clause and state the consequence that phrase produces — a "
        "phrase that appears in the mechanism statement but is not named as the source of the indistinguishability does not satisfy this requirement; give a concrete example naming a "
        ...
        "and state explicitly that this string is absent from any state satisfying it genuinely. Any rewrite produced must name the specific string or structural property that "
        "distinguishes the two compliance states before claiming the rewrite closes the gap; where a clause names an action, state exactly what conditions permit that action "
        "(allow-list); where a condition gates an action, add a post-condition naming the specific observable that would be absent if the condition were not satisfied; a rewrite is "
        "valid only when it is no longer than the original, or when each added clause closes a named gap that the shorter form leaves open — the gap must be named before the clause is "
        "added. A clause that lists excluded behaviors rather than permitted ones is a deny-list clause; a clause that names a step without naming the criterion the step serves is a "
        "prescription clause; a deny-list or prescription clause must be named and its type stated before any rewrite of that clause is produced — a rewrite that appears before the "
        "type is named in the transcript does not satisfy this requirement. If the subject instruction contains clauses that each stand independently — none derives from a criterion "
        "stated elsewhere in the instruction — name the set as a prescription cluster and count the standalone clauses before producing any individual clause audit — an audit that "
        "appears before the cluster count appears in the transcript does not satisfy this requirement. All findings and rewrites must use terms that refer to the structure of "
        "instructions and the properties of clauses — not to the subject matter the instructions govern; a term that requires knowing the instruction's domain to interpret is "
        "domain-specific and must not appear.",
