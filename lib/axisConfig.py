...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "P3,P5,P6 — after Verification closes one active gap, re-apply Challenge using the same evaluation basis to determine whether any gaps remain; RULE: no rung sequence exits "
        "after one closure unless re-challenge shows no remaining observed gaps and intent remains aligned. [ ] Structure before Challenge: derive from P1,P3 — solution organization "
        ...
        "AND a subsequent Challenge shows no remaining observed gaps under the same evaluation basis, with intent still aligned. Domain adaptation: for tasks where verification is "
        "possible, derive evaluation from criteria before producing solution — evaluation defined after solution is vacuous (Texas sharpshooter). Challenge produces, applies, and shows "
        "the evaluative artifact failing; if multiple failures appear, exactly one observed failure must be explicitly selected as the active gap; Refinement adds only enough to change "
        "that selected failing result and may not address inactive failures; Verification re-applies the SAME evaluative artifact and shows that the previously observed failure no "
        "longer occurs, without introducing new drift; Verification does NOT end the loop — it must immediately trigger a new Challenge against the current artifact until Challenge "
        "shows no remaining observed gaps and intent remains aligned. IMPORTANT: When deriving ladder artifact types, use MINIMAL names: - For Refinement, use 'Partial Implementation' "
