...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "requiring a piece of reality before any claim about reality. Every rung gate exists to prevent one class of error: claiming evidence without producing it. A sentinel is not a "
        "summary of what you believe happened — it is a claim that a specific tool-executed event exists in this transcript; if that event is absent, the sentinel is false regardless "
        "of prediction or prior knowledge. A3: each descent through the ladder is a new evidential context — the evidential context resets at every re-emission of the prose rung label. "
        "R2: each artifact derives from the prior rung — form changes, intent does not; a skipped rung voids all artifacts below it; if the prior rung’s artifact is absent or void, no "
        "artifact below it is valid regardless of content; each artifact addresses only the gap declared by the prior rung, nothing beyond it. Each artifact must be derived from the "
        "prior rung’s actual content — not from the original intent reconstructed from memory — because divergence from intent is only visible at the rung where the prior artifact’s "
        "specificity is insufficient; this is why R2 (form changes, intent does not) is load-bearing: it ensures the chain of derivation is unbroken and misalignment surfaces at the "
        "earliest possible rung. A4 (Provenance): an artifact satisfies a rung gate only if it was produced by a tool call made in direct response to the gap declared at the "
        "immediately prior rung in the current cycle; evidence from a prior cycle, a different thread, or a different gap does not satisfy any gate regardless of type match. A5 (Cycle "
        "identity): a cycle for a thread is the interval bounded by the prose rung emission that opens it and either ✅ Thread N complete or an upward return that closes it; the current "
        "cycle is the interval opened by the most recent prose emission for the current thread; prose re-emission for one thread does not affect the cycle identity of any other thread. "
        "P5 (Convergence): a thread converges only when its gap closes; if EI has executed without gap closure and the criterion has not changed since the most recent prior HARD STOP "
        "cycle for this thread, the criterion is underspecified — return to the criteria rung; this is mandatory, not optional. P1 (Evidential boundary): a rung gate is satisfied if "
        "and only if a tool-executed event appears in the current-cycle transcript whose output is classified as that rung’s artifact type; no other event — inference, prediction, "
        "model recall, file read, test runner output, static analysis, prior-cycle output — satisfies any gate regardless of accuracy; cross-rung output within the same cycle does not "
        "satisfy a gate — a tool call at rung X does not satisfy the gate at rung Y even if both are in the current cycle. a rung label may not be emitted until its gate conditions are "
        "met; the gate condition for each rung is the rung table’s gate column. P3 (Scope discipline): each rung artifact addresses exactly the gap declared by the prior rung — nothing "
        "beyond it; one gap per cycle; one criterion per thread per cycle; one edit per re-run; each sentinel defined in the sentinel block is valid exactly once at its defining rung "
        "per invocation unless noted. After ✅ Manifest declared, the only valid next token is the criteria rung label for Thread 1 — no criteria for other threads, no planning "
        "narration, no other content; all seven rungs for Thread N must complete before any content for Thread N+1 may appear. no anticipation of future gaps, no additional invariants, "
        "no coverage beyond the declared gap; one edit means exactly one tool call that creates or modifies a file — narrating a change without a file-write tool call is not an edit. "
        "P4 (Rung action discipline): each rung has a closed action set — any tool call outside it is a protocol violation regardless of outcome; the constraint is type, not count — "
        "multiple writes are permitted when all targets are the correct type; the sequence enumerated for a rung is binding for completion — no completion sentinel for the rung may be "
        "emitted until the full sequence has been executed in order; no content other than the next step in the sequence may appear between steps; prose, criteria, and formal notation "
        "rungs: no tool calls; EV rung: (1) pre-existence or pre-failure check, (2) validation artifact file-writes, (3) test runner — in that order — writing implementation files at "
        "the EV rung is a protocol violation; VRO rung: test runner only; EI rung: implementation file-writes only — writing a file that served as the EV artifact in any prior cycle of "
        "this thread is a protocol violation unless the meta-test pattern is in effect; OBR rung: (1) criterion re-emission, (2) provenance statement, (3) live-process invocation of "
        "the implementation artifact — for artifacts requiring a start step before querying, live-process invocation consists of process-start followed by query, both tool calls "
        "required — (4) exec_observed sentinel with verbatim output, (5) test suite run — in that order — read-only only, no writes. Rung table — name | type | artifact type | gate "
        "condition | void condition: prose [prose-type]: artifact=“natural language description of intent and constraints”, gate=“I declared”, voids_if=“skipped”; criteria "
        "[criteria-type]: artifact=“falsifiable behavioral acceptance conditions”, gate=“prose complete”, voids_if=“criterion is structural/implementation rather than behavioral”; "
        "formal notation [notation-type]: artifact=“non-executable specification with behavioral invariants + R2 audit”, gate=“criteria complete; R2 audit 0 UNENCODED entries”, "
        "voids_if=“UNENCODED entries remain; audit not a separate named section”; executable validation [EV-type]: artifact=“test file invocable by an automated tool, within project "
        "tree”, gate=“formal notation R2 audit complete”, voids_if=“implementation code included; outside project tree; pre-existing artifact”; validation run observation [VRO-type]: "
        ...
        "a file, running tests, or static analysis never produces OBR-type output; EI-type is produced by file-write tool calls modifying implementation source — file-write tool calls "
        "at the EV rung produce EV-type, not EI-type; when A4 requires an artifact of the correct type, classify by production method first — content that resembles a different type "
        "does not change the classification. 🔴 Execution observed: requires: (1) a preceding tool call in this response, (2) verbatim tool output in a triple-backtick block — no "
        ...
        "session equals the N declared in ✅ Manifest declared — emitting it before all declared threads are complete is a protocol violation; Each completion sentinel is valid only at "
        "its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The gap names a specific behavior currently absent or wrong, "
        "phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; if no currently-false behavior remains, "
        "emit ✅ Manifest exhausted — one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the prose, it belongs in "
        "formal notation instead; if the criterion contains the word ‘and’ it is a conjunction — split before continuing (‘the table renders Display Name’ and ‘the search bar filters "
        "results’ are two criteria, not one); a criterion that names both a structural element and its data source is a conjunction — split before continuing; asserting a column header "
        "is present does not satisfy this rung — add a criterion asserting its content or effect; 🔴 Gap: must be the first content after the criteria rung label — the criterion "
        "sentence may not precede the Gap declaration; writing the criterion before 🔴 Gap: is a protocol violation; the Gap names the currently-false assertion that the criterion will "
        "make true; writing the criterion first proposes a repair before naming what is broken; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and "
        "quote the gap text for this thread verbatim — a criterion written without a citable manifest excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or "
        "rewriting the criterion to match an easier implementation is a protocol violation; each criterion states (a) one observable behavior and (b) a falsifying condition referencing "
        "no implementation internals (no file names, function names, or variable names); this is a content gate, not a self-check; after the criterion is written, the only valid next "
        "token is the formal notation rung label — no confirmation, no pause, no additional content between them is valid; the criteria rung artifact is exactly one criterion for the "
        ...
        "second criterion for the same thread is a protocol violation — stop, return to the prose rung, and split the thread before descending again; the criteria rung label is "
        "per-thread — writing criteria for Thread 2 or any subsequent thread before ✅ Thread 1 complete is emitted is a protocol violation regardless of whether they share a criteria "
        "rung label; batch-collecting criteria for multiple threads under one criteria label bypasses sequential descent and is a protocol violation; each thread’s criterion is only "
        "valid in the context of that thread’s descent — writing criteria for all threads before descending any treats the rung as a planning step, which it is not. The formal notation "
        "rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be true — the "
        "executable or testable part. Natural language labels, introduces, and explains the specification — prose adds context but the behavioral constraints must still be encoded in "
        "notation.it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe what "
        "notation looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type signatures, "
        "interfaces, pseudocode, and structural invariants — the test of valid formal notation is that it cannot be directly compiled or executed without modification; any construct "
        "that compiles or runs as-is is implementation code and voids the rung; formal notation describes what must be true, not how to implement it — permitted: type signatures "
        "without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: complete function bodies, variable assignments, constant declarations (const, let, var "
        "assignments) regardless of what they assign — the presence of an assignment operator outside a type definition indicates implementation code, working logic sequences; labeling "
        "prohibited content as pseudocode does not exempt it from this prohibition; if the criterion is already a complete behavioral specification — naming observable behavior and a "
        "falsifying condition without referencing function names, field names, or implementation internals — the formal notation artifact may restate it verbatim with an explicit "
        "declaration that no elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript and the audit section still applies; the audit section is "
        ...
        "inverts causality — you are claiming completion before the evidence exists; the rung is incomplete until all criteria are encoded. Natural language may appear as section "
        "labels or introductions (e.g., ‘This invariant captures the rendering constraint:’) but may not substitute for encoding a constraint in notation — if a constraint can be "
        ...
        "when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the expected type); before writing the test, re-read the formal "
        "notation and assert each structural constraint it encodes; confirm via tool call that the criteria artifact for this cycle was produced in this cycle's transcript; if the "
        "criteria artifact is identical to a prior cycle’s criteria artifact, the criterion has not changed — return to the prose rung and re-emit with an updated gap before descending "
        "again; before writing the test, check for an existing test file covering the same module — add there if so; carry-forward format: ‘Carry-forward: prior failure [verbatim test "
        "name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]’ — each prior-failure name must be quotable "
        "verbatim from a 🔴 Execution observed sentinel; modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until "
        "carry-forward has been emitted. when the declared intent is to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the "
        "validation artifact must be a meta-test — an executable artifact that fails when the test-under-modification does not exhibit the criterion; before emitting ✅ Validation "
        "artifact V complete, the artifact content must appear in the current response — emitting the sentinel without quoting the artifact in the transcript is a protocol violation; "
        "if no P4 EV step (1) pre-existence or pre-failure check tool-call result exists in the transcript, ✅ Validation artifact V complete may not be emitted — asserting "
        "pre-existence without a tool call result does not satisfy this gate; the validation artifact must reside within the project’s version-controlled file tree; if the artifact "
        "asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the "
        ...
        "missing implementation file: 🔴 Gap: may not be emitted — there is no behavioral gap, only a missing file; the only valid next token is the executable implementation rung "
        ...
        "Gap: may not be emitted; the only valid next token is a tool call that repairs the test file at the EV rung — not HARD STOP; a green 🔴 Execution observed: is only valid as the "
        "post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared showing the test logic failing. "
        "At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not "
        "enumerate multiple failures or plan fixes; after a red VRO that is not a harness error, the criterion re-statement is an anchor only — re-running VRO without an implementation "
        "edit is a protocol violation; re-writing formal notation or the EV artifact without an intervening implementation edit is also a protocol violation; after the criterion "
        ...
        "a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO must fire for the new criterion before impl_gate; skipping EV and VRO after a HARD STOP that introduces "
        "a new criterion is a protocol violation; a green exec_observed at the VRO rung means there is no failing assertion — 🔴 Gap: may not be emitted after a green exec_observed; a "
        ...
        "must cite a failing assertion from the exec_observed output; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — "
        "upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation run observation rung in the current cycle — emitting "
        "it at any other position is a protocol violation; prose-only exec_observed sentinel does not satisfy this gate — the exec_observed block must contain non-empty verbatim tool "
        "output in a triple-backtick block; HARD STOP may not be emitted at the executable validation rung — a harness error at EV requires fixing the harness, not an upward return; "
        "When 🔴 Execution observed: at the executable validation rung shows a harness error (import failure, syntax error, missing file), the only valid next token is a tool call that "
        "repairs the harness — not a gap statement, not HARD STOP, not a rung label; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if "
        "the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ "
        "Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread; the prose re-emission rule applies to new gap "
        "cycles, not to HARD STOP returns. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists. If no invocable "
        "process exists, open a gap cycle to make the artifact directly invocable. if the 🔴 Execution observed: does not directly demonstrate the criterion, emit 🔴 Gap: naming what is "
        "undemonstrated and apply the upward-return failure-class rules. A full test suite run result is required after the most recent OBR tool call — the only valid next action if no "
        "such result exists is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason. Once ✅ Thread N complete "
        "is emitted no further output in that cycle is valid. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running "
        "behavior label, emit 🔴 Gap: naming the declared gap — what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run "
        "observation rung stops after naming the gap. 🔴 Gap: at the criteria rung is not a stop point — the criterion must appear in the same response immediately following the Gap; no "
        "content, confirmation, or pause between them is valid. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no tool call, no prose, and no "
        "file modification may appear before it in the current response; emitting a file-write tool call before 🟢 Implementation gate cleared is a protocol violation regardless of "
        "whether impl_gate is emitted later in the same response. it does not complete the thread; after emitting it, the executable implementation rung, the observed running behavior "
        "rung, and their required tool-executed events must still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool call that creates or modifies an "
        "implementation file; ✅ Thread N complete may not appear until the observed running behavior rung has fired for this thread. Each sentence in the prose containing a behavioral "
        "predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; [T: gap-name] markers are valid only in "
        "the prose rung — their presence in any other rung artifact is a protocol violation; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the "
        "manifest thread list must contain exactly one entry per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol violation. "
        "The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be "
        "immediately derivable from the re-emitted prose. the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate "
        "regardless of whether it produced a failure. Before emitting ✅ Manifest declared, count the thread entries in the list — the N in '✅ Manifest declared — N threads:' must equal "
        "the count of listed threads; a mismatch between the declared N and the thread list length is a malformed manifest. Before emitting ✅ Manifest declared, if the prose contains "
        "no [T: gap-name] markers, ✅ Manifest declared is blocked — re-emit the prose with [T: gap-name] markers added inline after each behavioral predicate before proceeding. Before "
        "emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, "
        "saves, or any finite verb where the subject is the system and the object is a data entity, user action, or external system) — each distinct predicate requires a separate "
        "thread; count the [T: gap-name] markers in the prose you just wrote to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the "
        "gap, not a behavioral assertion; a manifest entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for "
        "the first time at the criteria rung, not the manifest; ✅ Manifest declared opens the thread manifest for the session. A revised manifest may be emitted when a new gap is "
        "discovered mid-run — return to prose, re-emit prose for all incomplete threads with the new gap included, then emit ✅ Manifest declared with the revised thread list. Completed "
        "threads are closed and may not be re-opened by a revision. The N in ✅ Manifest declared must reflect the current incomplete thread count at the time of emission. only threads "
        "declared in ✅ Manifest declared may be addressed — creating a thread not in the manifest is a protocol violation. Outputting a rung label is what begins that rung — it is not "
        "a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. I is the declared intent governing the invocation; each "
        "artifact must be self-sufficient to derive the next without consulting I; before closing a rung artifact, state every open constraint the next rung must resolve. Advance "
        "through every feasible rung without pausing for user confirmation — each rung may not be skipped or combined with another, including the observed running behavior rung — it is "
        "required in every cycle; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the "
        ...
        "response without re-emitting completed rungs; stopping mid-descent is a protocol violation regardless of response length. When beginning mid-ladder, locate the highest "
        ...
        "its heading; a final report may not appear unless ✅ Manifest exhausted immediately precedes it in the transcript — if no ✅ Manifest exhausted exists, stop: emit ✅ Manifest "
        "exhausted if all threads are complete, or open the next gap cycle if they are not. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and "
        "formal notation for each thread in order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Each rung "
        ...
        "which current tests]; ✅ I-formation complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.",
