...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "resembles output do not satisfy this requirement at any rung. Rule 2: each artifact derives from the prior rung — form changes, intent does not; a skipped rung voids all "
        "artifacts below it; each artifact addresses only the gap declared by the prior rung — nothing beyond it. Rule 3: every thread ends when tool output names the declared gap; "
        "stop there. I is the declared intent governing the invocation; I precedes and is not itself an artifact; each artifact must be self-sufficient to derive the next without "
        "consulting I; faithful derivation requires form to change, intent to stay fixed. Advance through every feasible rung; completeness governs rung depth, not rung existence; each "
        "rung may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle. Before any tool call or file read, check: if no "
        "🔴 Gap: sentinel exists in output yet, emit it now; before emitting 🟢 Implementation gate cleared, check: does a 🔴 Execution observed: sentinel from the current cycle's "
        "validation run observation rung — not from a prior cycle — exist in output? if not, do not emit it — produce the executable validation artifact instead. 🟢 Implementation gate "
        ...
        "file — ✅ Thread N complete, a new criteria cycle, or any other sentinel may not appear until at least one edit and its post-edit re-run have occurred and the observed running "
        "behavior rung has fired for this thread. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running behavior label, "
        "emit 🔴 Gap: naming what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the "
        "gap. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be "
        "immediately derivable from the re-emitted prose. From the criteria rung onward, the gap names a specific behavior currently absent or wrong, phrased as a currently-false "
        "assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — ✅ "
        "Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists in the transcript for each thread declared in ✅ Manifest declared — check the thread count "
        "before emitting. Each rung’s artifact addresses only that gap — not all known requirements of the task — and is minimal: one independently testable behavior derived from the "
        "prose alone — if stating it requires names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is invalid — "
        "split it before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a structural element "
        "and its data source is a conjunction — ‘renders columns with data fetched from the endpoint’ must be split into ‘columns render’ and ‘data is fetched from the endpoint’ before "
        "continuing; a criterion that asserts only that a structural element exists is not behavioral — asserting a column header is present does not satisfy this rung; add a criterion "
        "asserting the element’s content or effect (the row displays the value from the data source); the criterion for thread N must name the same behavior as the gap declared for "
        "thread N in ✅ Manifest declared — narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol "
        "violation; if the declared gap cannot be satisfied, return to the prose rung and revise the manifest. Formal notation encodes only the criteria declared at the criteria rung — "
        "no additional invariants, no anticipated cases; it produces only type signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is that "
        "it cannot be directly compiled or executed without modification; if placing the notation verbatim in the source tree would produce a runnable artifact, it is implementation "
        "code and voids the rung; annotation-decorated classes, repository interfaces, and any construct that compiles as-is are implementation code regardless of whether they contain "
        "imperative logic. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ Validation artifact V complete; the "
        "artifact must be invocable by an automated tool; before writing the test, re-read the formal notation and assert each structural constraint it encodes; each test function "
        "contains exactly one assertion — one assertion per test function; a pre-existing test that happens to pass does not satisfy this rung — add to an existing test file covering "
        "the same module if one exists, otherwise create a new file; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist "
        "or is already failing — V-complete may not be emitted without this tool-executed result in the transcript; the validation artifact must reside within the project’s "
        ...
        "edits may not begin until a red run exists in the transcript for the current cycle — if the validation artifact passes before any implementation edit, either the test is "
        "vacuous (rewrite it) or the gap is already closed (return to criteria); ✅ Validation artifact V complete may not be emitted unless a 🔴 Execution observed: showing the test "
        "failing exists in the transcript for the current cycle — write the test, run it, confirm it fails, then emit V-complete; emitting V-complete before seeing a red run is a "
        ...
        "run observation label is written. Before writing the validation run observation rung label, invoke the validation artifact via a tool call — predicting what the run would show "
        "does not satisfy this gate regardless of accuracy; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same "
        "artifact appeared after the most recent 🟢 Implementation gate cleared for this thread showing the test logic failing — a harness error (import failure, syntax error, missing "
        ...
        "witness — do not proceed to the implementation gate until a loadable run with failing assertions exists in the transcript for the current cycle. At the validation run "
        ...
        "failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token in the response — any other content before the "
        "criteria label is a protocol violation; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation "
        "run observation rung in the current cycle — emitting it after a criteria label, after a formal notation label, or at any other position is a protocol violation; if the "
        "observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Upward returns follow the failure class: if the "
        ...
        "is a prose-description failure, not an implementation failure — return to the prose rung and update it. One edit per re-run cycle; after each edit re-run the validation "
        "artifact before any further edit. After every upward return, a new ✅ Validation artifact V complete must be emitted for the current cycle’s gap before the implementation gate "
        "may fire — an artifact written in a prior cycle does not satisfy this gate even if it covers the same path. Upon writing the observed running behavior label, re-emit the "
        ...
        "state (server started, build succeeded, exit code 0) unless the criterion is specifically about process state; a build output, bundle manifest, or compilation result never "
        "satisfies this gate regardless of what it names — direct demonstration means the running feature produced the output, not the build tool; for a UI component, this means "
        "browser-visible text and values — not a test runner’s DOM query result; if the output does not directly demonstrate the criterion, emit the gap and return upward; a 🔴 "
        "Execution observed: whose verbatim block is empty or blank does not satisfy this gate — a verification script, console log, or tool call that produces no visible output is not "
        "valid OBS evidence; invoke a tool call that produces non-empty output demonstrating the behavior; if the declared gap names a real network endpoint (fetches from, calls, "
        "integrates with), the OBS must include verbatim output showing a real HTTP request or response — a test run whose network calls are mocked does not satisfy this gate; if the "
        "implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared behavior; implementation edits, new "
        ...
        "recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it and the OBS output directly demonstrates each criterion "
        "declared in this thread — a test pass is not a demonstration; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already "
        "closed (return to criteria) or the test is vacuous (rewrite it) — Thread N complete may not be emitted when no implementation edit was made in this cycle; before emitting ✅ "
        "Thread N complete or ✅ Manifest exhausted, run the full test suite; every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — "
        "dismissing a failing test as unrelated without an explicit written acknowledgment is a protocol violation; once ✅ Thread N complete is emitted no further output in that cycle "
        "is valid. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, "
        "authenticates, loads, saves, or any similar action verb) — each distinct predicate requires a separate thread; a manifest that omits a behavioral predicate from the prose is "
        "incomplete and may not be emitted. ✅ Manifest declared may be emitted exactly once per invocation — at the prose rung, after the first gap is named; re-emitting it at any "
        "other rung is a protocol violation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun "
        "and no artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every "
