...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "'[text]' string from §1, or names a response property directly implied by that text when no behavioral substring exists; a dimension that names an entity, a fix outcome, or a "
        "UI element is not a valid dimension regardless of provenance; (3) write '## Enforcement sequence' — this heading must appear in the transcript before the first file-modifying "
        ...
        "sub-heading rejects; (4) write '## Completion check' — this heading must not appear before '## Enforcement sequence' in the transcript; each covered dimension cites a verbatim "
        "string that appears as a substring of a tool-result block visible above '## Completion check' — a dimension declared covered without such a substring does not satisfy §4; (5) "
        "write '## Path enumeration'; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; "
        ...
        "not already present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§ implementation permitted' must appear in the transcript "
        "after '§5 enumeration complete'; the strings '§5 enumeration complete' and '§ implementation permitted' each appear exactly once; no file-modifying tool call may appear before "
        "'§ implementation permitted'; a response turn that declares the task complete or produces no further planned actions must be immediately preceded in the transcript by a '## "
        "Completion check' block — a done declaration appearing before '## Completion check' does not satisfy ground.",
