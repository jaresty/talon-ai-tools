...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "output — as the current step's scope. An implementation edit is permitted only when a tool-executed run result from this step's governing artifact — produced by a tool call, "
        "not described in prose, and from this specific step's governing artifact rather than a different verification layer — already exists as a completed tool result in the "
        "transcript above the edit — issuing the run and the edit as parallel tool calls does not satisfy this condition. That pre-edit run result names the failure set F for this "
        "step. The only behaviors in scope for the current step are those whose absence is explicitly stated in the current governing failure signal — a behavior is explicitly named "
        "only when the failure signal contains text that identifies that specific behavior's absence as the cause of the signal. Behaviors whose absence is implied by, related to, or "
        "co-located with the named absence are not in scope; implication is not naming. Behaviors governed by the same artifact that have not yet produced a failure signal, and "
        "behaviors anticipated but not yet observed as failures, are not in scope regardless of how many behaviors share the same governing artifact. The scope is the governing output "
        "read literally: when literal and semantic readings diverge, choose the interpretation that produces the smallest change that would shift the governing output.Before any "
        "implementation edit, produce a one-line scope declaration as a completed transcript artifact: 'Governing this step: [verbatim governing failure signal]. All other signals from "
        "the same governing artifact are not yet open.' The edit is not permitted until this declaration exists above it in the transcript — a scope declaration written in the same "
        "generative act as the edit does not satisfy this requirement. A scope declaration that names more than one governing failure signal has not been derived. The permitted "
        "implementation contains only lines whose behavioral effect is required to shift the governing failure signal — a line is permitted only when removing it would leave the "
        "governing failure signal unshifted or would reintroduce it; a line that addresses a behavior not explicitly named in the governing failure signal is not permitted regardless "
        "of how compact it is. New failure signals become new governing outputs for new steps. Before any implementation edit, produce a derivation block as a completed transcript "
        "artifact: (1) the verbatim governing failure signal, (2) the candidate change, (3) at least one smaller change that would not shift the governing output, with a quoted "
        "citation to a specific prior transcript location naming the specific text at that location that makes the smaller-change claim checkable. The derivation block must be closed "
        "before the edit tool call — a derivation block generated in the same generative act as the edit does not satisfy this requirement. A citation to a prior transcript location is "
        "valid only when the specific text at that location demonstrates, for the current governing failure signal and the current candidate change, that the named smaller change would "
        "not shift the current signal — a citation that demonstrates this for a different signal, a different change, or a different step is not valid. A statement of the form 'this is "
        "analogous to [prior step]' is not a citation. produce a line manifest before the edit: an enumerated list of every line (or logical block) to be added, each entry stating (a) "
        "the specific text in the governing failure signal that names the behavior this line implements — quoted verbatim from the governing failure signal — and (b) the reason "
        "removing this line would leave the governing failure signal unshifted or would reintroduce it. A manifest entry whose behavioral connection is stated without quoting the "
        "governing failure signal text has not been derived. A manifest entry citing a behavior not explicitly named in the governing failure signal is not valid. The manifest must "
        "exist as a completed transcript artifact before the edit tool call — a manifest generated in the same act as the edit does not satisfy this requirement. For new files, each "
        "entry must name the specific text in the governing failure signal that names the behavior this line implements or enables — compilation dependencies between lines within the "
        "new file are not sufficient justification; a line justified only by being required for another line to compile has not been derived against the failure signal. If a line is "
        "required for another line to execute, that other line's manifest entry must itself name governing failure signal text; the dependency chain must terminate at a quoted "
        "governing failure signal text, not at another manifest entry. After every edit tool call, the derivation must produce a post-edit manifest check as a transcript artifact: "
        "enumerate every line actually added (from the edit diff), verify each appears in the pre-edit manifest, and name any line present in the edit but absent from the manifest — if "
        "any such line exists, the edit must be reverted before the step can continue. A post-edit manifest check that does not enumerate the diff lines explicitly has not been "
        "derived. An implementation edit closes the step only when a post-edit run result from the governing artifact — produced by a tool call after the edit — exists in the "
        "transcript and names a failure set F' satisfying: (a) the governed failure is absent from F'; (b) every failure signal in F' whose governing artifact produced no failure in F "
        "is absent — a failure signal in F' whose governing artifact had no failure in F is a regression and requires revert regardless of any judgment about latency; (c) failure "
        "signals in F' from governing artifacts that were already producing failures in F are progression failures, each becoming a new governing output for a new step. A failure in F' "
        "is a progression failure if and only if the governing artifact that produced it also appears in F with at least one failure — checkable against the pre-edit run result without "
        "interpretation; a model assertion that a failure is latent does not satisfy this condition. If (a) or (b) is not satisfied, the edit must be reverted. A post-edit confirmation "
        "line names the specific change made, cites the derivation's stated candidate, and cites the pre-edit and post-edit run results by transcript location — if the change made "
        "exceeds the derivation's stated scope, the edit must be reverted to match it before proceeding. A step is the smallest change that could alter the result of any verification — "
        "compilation, static analysis, or test execution; changes that address different verification layers are always separate steps even when they feel like parts of the same fix. "
        "The governing artifact must be the verification layer with scope to detect a failure if this step's change is incorrect — running a layer that cannot see the specific files, "
        "call sites, or concerns this step touches does not satisfy the governing artifact requirement even if that layer passes. An artifact is a valid governing artifact for this "
        "step only when a derivation exists in the transcript naming the specific FAIL output it would produce if this step's change were absent. Implementing beyond the current "
        "governing output is a violation regardless of how many other failures exist. Analysis and classification steps have a text artifact as their governing artifact — a complete "
        "classification block or derivation section; no tool call is required to open or close these steps. A step may be classified as an analysis step only when a prior transcript "
        "artifact already present above the classification statement demonstrates, by naming a specific governing artifact and a specific property of that artifact, that running the "
        "governing artifact against this step's change would produce no different output. The classification statement must cite: (a) the specific governing artifact, (b) the specific "
        "property of that artifact that would be unchanged by this step's change, and (c) the specific change being made. A classification that cannot name (a), (b), and (c) with "
        "citations to prior transcript locations has not been derived. A label such as 'structural', 'untestable', 'wiring', or any equivalent term is not a valid classification — the "
        "classification is valid only when the cited artifact and property support it. Implementation steps have a FAIL tool result as their governing artifact. These are distinct step "
        "types with distinct governing artifacts — analysis steps may precede the first tool call without violating atomic.",
        ...
        "cannot name a specific outcome the previous rung admits but this rung rejects cannot be added. Producing the ladder is a required artifact before any rung-work begins. The "
        "ladder is not itself a rung — it is a precondition to all rungs including rung 1. A ladder entry whose artifact is the ladder itself is invalid: the ladder cannot govern its "
        "own production. The rung 1 coverage mapping is a separate required artifact produced after the ladder is closed — producing both in the same generative act does not satisfy "
        "this requirement. The ladder must be a completed artifact in the transcript before the rung 1 coverage mapping begins. Rung 0 is the intent artifact — a representation of "
        ...
        "valid ladder entry — and, for every rung after the first, at least one specific outcome that would satisfy the previous rung but be rejected by this rung, naming both (a) the "
        "specific artifact state that satisfies the previous rung's satisfaction condition and (b) the behavioral dimension that state fails to constrain; a rejects entry that names "
        "only a dimension label or boolean property without naming a specific satisfying artifact state is not valid. If no such outcome can be named, the proposed rung is not a "
        "refinement — it is a parallel task and must be merged with the rung above or dropped. Describing the ladder structure in prose does not produce this artifact. A rung's "
        "satisfaction condition is valid if and only if it names: (a) a locatable artifact, and (b) a property of that artifact that would evaluate differently if any behavioral "
        "dimension of the artifact were absent — a property that evaluates the same regardless of which behavioral dimensions are present is not valid. A count of items is not a valid "
        "property unless each counted item is itself required to name a specific behavioral dimension such that removing any one item would change the count and leave a dimension "
        "uncovered; a bare item count without this per-item behavioral binding is not valid. For rungs that refine a rung above, the satisfaction condition must additionally confirm "
        "that the rung's artifact contains a covering item for each behavioral dimension in the rung above's artifact — a satisfaction condition that evaluates to true with fewer "
        "covering items than dimensions in the rung above is not satisfied. When stating a satisfaction condition, the model must also state the specific property value that would "
        'cause the condition to evaluate to false — a satisfaction condition whose false-evaluating state cannot be named has not been derived. A satisfaction condition of "see above", '
        '"as planned", or any phrase that requires interpretation to evaluate is not valid. A rung transition occurs when the satisfaction condition evaluates to true against a '
