...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response Rule 1: every claim about system state or artifact completeness is valid only if a tool-executed event in this conversation produced it. Rule 2: each artifact "
        ...
        "if not, do not emit it — produce the executable validation artifact instead. Before writing the criteria, formal notation, executable validation, executable implementation, or "
        "observed running behavior label, emit 🔴 Gap: naming what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run "
        "observation rung stops after naming the gap. Before writing the criteria rung artifact in any cycle after the first, re-emit the prose rung verbatim — the criterion must be "
        "immediately derivable from the re-emitted prose, making derivability locally auditable. From the criteria rung onward, the gap names a specific behavior currently absent or "
        "wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; naming an absent artifact is not a "
        "valid gap; if no currently-false behavior remains, emit ✅ Manifest exhausted — do not emit 🔴 Gap:. Each rung’s artifact addresses only that gap — not all known requirements of "
        "the task — and is minimal: one criterion (one independently testable behavior derived from the prose alone — if stating it requires knowledge of function names, hook names, "
        "API endpoints, or data structures not in the prose, it belongs in the formal notation rung instead; ‘the table displays attribute data’ derives from the prose, ‘the useFoo "
        "hook calls /bar’ does not; if the criterion contains the word ‘and’ it is invalid — split it before continuing; ‘the table renders Display Name’ and ‘the search bar filters "
        "results’ are two criteria, not one), one notation, one test, one edit; further artifacts emerge from subsequent gaps after each cycle completes. Formal notation encodes only "
        "the criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only validation artifacts may be produced at the executable validation rung — no "
        "other content is permitted at the executable validation rung before ✅ Validation artifact V complete: not implementation notes, not planning prose, not file reads, not ‘let me "
        "fix’ or ‘I need to’; implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact "
        "must be invocable by an automated tool; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be newly produced for the declared gap; the "
        "validation artifact must fail before any implementation edit — a green run with no prior red run does not satisfy this rung; if the artifact passes on its first run, perturb "
        "the implementation to force a red run — if the artifact still passes after perturbation, the test is vacuous and must be rewritten; if the artifact goes red after "
        "perturbation, restore the perturbation and return to the gap rung, as the declared gap names a behavior that already exists. ✅ Validation artifact V complete must be emitted "
        "at the executable validation rung before the validation run observation label is written. At the validation run observation rung, run the validation artifact; emit 🔴 Execution "
        "observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; if the observed gap matches the prior "
        "cycle's gap, the edit did not address it — return to formal notation before any further edit. Upward returns follow the failure class: if the implementation did not close the "
        "gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; if the prose description is insufficient — incomplete, "
        "inaccurate, or the system cannot be observed as described — return to the prose rung and update it; observation cannot be produced is a prose-description failure, not an "
        "implementation failure. Before each edit, check: does a 🔴 Execution observed: sentinel exist for the current gap? If not, run the validation artifact now before editing. One "
        "edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upon writing the observed running behavior label, immediately invoke a tool and "
        "emit 🔴 Execution observed: with its output — prose description of tool output does not satisfy this rung; implementation edits, new files, and code changes are not permitted "
        "at this rung — if observation reveals a defect, emit the gap and return upward. ✅ Thread N complete may not be emitted unless the observed running behavior label has been "
        "written in the current cycle and a 🔴 Execution observed: sentinel appears immediately after it; once ✅ Thread N complete is emitted no further output in that cycle is valid. "
        "Before emitting ✅ Thread N complete, check: does the declared intent have any behaviors not yet observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and "
        "begin the next cycle at the criteria rung. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not "
        ...
        "reconcile all artifacts at that rung against I before redescending; documentation is a reconciliation artifact, not a thread. Each completion sentinel is valid only at its "
        ...
        "executable validation → validation run observation → executable implementation → observed running behavior. Sentinel formats — 🔴 Execution observed: [verbatim tool output — "
        "triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution "
