...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation rung to make that assertion pass. One assertion at a time: write assertion → see red → implement → see green → move to next assertion. ASSERTIONS are test code "
        "that verifies behavior, NOT import errors or infrastructure failures. Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ [Rung name] "
        "complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; No implementation is permitted before the assertion rung shows "
