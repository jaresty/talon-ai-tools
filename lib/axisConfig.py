...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "adequacy the work of the adequacy check; falsify only constructs the apparatus and concrete state needed to attempt falsification of the already-established properties. A "
        "guard is an observation-producing procedure: a repeatable procedure that, applied to the artifact, yields an observation distinguishing the property's satisfying and "
        "violating states — a software test, a map or route check, a source inspection, a measurement, a trial, or a comparison are all instances. An observation witnesses an "
        "assertion only when the procedure evaluates that assertion and produces an assertion-specific result: Retrieving evidence for an assertion is not the same operation as "
        "witnessing the assertion. Information merely retrieved, inferred, or cited by the procedure is not itself the assertion's result — establishing that a procedure or its "
        "evidence is appropriate to the property is the work of ground and verify; falsify, once an assertion and its procedure are in play, obtains the procedure's own discriminating "
        "result for that assertion. What makes an observation a witness is that the procedure could have produced the violating result: it is capable of yielding either the satisfying "
        "or the violating observation, and is not constructed from the conclusion it is meant to evaluate. This is not asserted — it is shown, by perturbing the relevant state and "
        "observing the result flip (the satisfying observation on one state, the violating observation on a perturbed state); a procedure that yields the same observation regardless "
        "of the perturbed state does not distinguish the assertion and is not a witness. A guard is established by an observation-producing act whose result record contains the "
        "guard's procedure verbatim; every assertion referenced later must be copied verbatim from such a result record. What witnesses an assertion is that the procedure is "
        "assertion-bound and counterfactually independent; regression detection additionally requires the procedure to re-apply on its own. Regression-persistence is a separable "
        "capability that may be present or absent: a one-time application that is assertion-bound and counterfactually independent still witnesses its assertion now, and is a valid "
        "Failure or pass observation, even when it cannot re-fire later (a measurement taken once, an expert inspection) — only the claim that the guard will detect a future "
        "regression depends on persistence. When a Failure observation is intended as a standing regression guard, it must be produced by a committed procedure persisted to a location "
        "that re-applies without human initiation, not an inline or ephemeral one-off. Repeatable is not the same as deterministic: a guard whose observation depends on external or "
        "changing state (a live lookup, a measurement, a poll) is still a valid witness at the moment it is applied, but its observation and provenance must be retained rather than "
        ...
        "path>', trim to the boundary the guards require, and repeat. A bare 'Overreach: not found' with no attempted reduction does not satisfy this token. Both 'Overreach: found' "
        "and 'Overreach: not found' are observation-dependent; when no qualifying observation is available, emit 'Overreach: untested — observation unavailable' rather than either "
        "verdict — untested is never 'not found'. Witness every assertion (Gate 2). Enumerate the assertions: emit one 'Assertion: <verbatim assertion text>' line for each assertion "
        "of the established guard, quoted verbatim from the guard's result record. The verbatim lines are the observable; no count or completeness sentinel is used, and a "
        "self-declared count does not satisfy it. Every enumerated 'Assertion:' line must then receive its own 'Failure: assertion \"<verbatim assertion>\" — \"<verbatim failure "
        "text>\"' or 'Unobservable: assertion \"<verbatim assertion>\" — structural' before 'Coverage: complete'; the binding is by verbatim identity — the assertion text in the "
        "Failure or Unobservable line must match an enumerated Assertion line exactly, and an enumerated assertion with no matching outcome line leaves coverage incomplete. After each "
        "Failure or Unobservable outcome, if any enumerated 'Assertion:' line still lacks a verbatim-matching outcome, return to the witness step for the next unwitnessed assertion "
        "and repeat; do not leave Gate 2 until every enumerated assertion has a matching outcome. Each enumerated assertion is either behavioral or structural. A behavioral assertion "
        "is witnessed only by a pair of A-observations: an application in which A's own guard-defined result is a failure, and one in which A's own result is a pass. An A-observation "
        "is an application of the committed guard in which assertion A is evaluated and produces its own guard-defined result. The guard defines the observable result for A: an A-pass "
        ...
        "reached (a software build or compilation error when the governed symbol is undefined; a route check that cannot run because no lodging has been chosen yet; a source lookup "
        "that fails because no source is cited) is tempting to read as 'the property is absent, so this is the failure'; it is not an A-fail, because A never ran — the application "
        "disqualified itself as an observation of A and fails identically no matter which property is absent. The A-fail you need is A's own assertion applied and reporting failure "
        "against a present-but-wrong artifact, which the subject's absence cannot produce. Both A-observations must be produced by the committed guard artifact, and the guard's output "
        "must identify A (by a name, path, or message that distinguishes A) so the pair is bound to this enumerated assertion rather than another. Provenance closure: any transcript "
        "reference that functions as evidence — the assertion identity, the A-fail result, and the A-pass result — must quote verbatim the text of the qualifying observation record "
        "that establishes it, and counts as evidence only if that exact text occurs in a referenced qualifying observation record; a result or identity typed as prose that does not "
        "appear verbatim in a cited qualifying observation record is not evidence and does not witness A. For a behavioral assertion whose artifact is present and perturbable, the "
        "absence of an A-fail does not terminate the branch; it triggers construction: construct an admissible minimal perturbation of the current artifact that makes A alone fail "
        "while every other assertion still passes, apply the guard to that perturbed state to obtain the A-fail, emit the Failure from that result record, then restore; a modification "
        "that fails every assertion at once discriminates none. This construction is mandatory whenever the artifact is present and perturbable — it is not an optional convenience for "
        "artifacts that already pass A, and ambiguity in the satisfying artifact is resolved by constructing a definite violating state, not cited as observation unavailable: "
        "'observation unavailable' is unreachable for a behavioral assertion whose artifact is present and perturbable. Constructing this violating state does not select which "
        "assertion is required or judge its adequacy — property selection remains the work of ground and adequacy the work of verify; falsify only constructs the concrete perturbed "
        "state that produces A's own discriminating result. Emit 'Failure: assertion \"<verbatim assertion>\" — <A-fail result> / <A-pass result>' quoting both observed results from "
        "their result records; a 'witness:' line, if added, only projects those two results and is not itself evidence. A structural assertion — one whose property has the artifact's "
        'own text or structure as its primary subject, so no behavioral failing application exists — is witnessed instead by \'Unobservable: assertion "<verbatim assertion>" — '
        ...
        "perturbations, and expected outcomes and identify what remains unverified, but it may not represent any observation-dependent verdict as observed.",
