...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation; skipping a rung is a protocol violation; each rung may not be skipped or combined with another), rung-specific gaps (each rung addresses a specific gap that "
        "only that rung can solve; the gap must be explicitly expressed IN THE CONVERSATION with evidence appropriate to the rung level; for validation assertions, the gap is the "
        "individual assertion being red; gaps are not implicit — they must be written down in the conversation with supporting evidence; fabricated test runs or falsified output void "
        "all evidence and the rung), P5 implies sequential descent (since each artifact derives from prior artifact per P5, you must produce them in order — treating the ladder as a "
        ...
        "for each rung in order; when resuming mid-ladder, locate the highest already-instantiated rung, update it if needed, then descend; you may return upward freely, but only when "
        ...
        "upward return trigger), upward gap scan (after completing the final rung (implementation), if gaps remain, scan upward to find the nearest rung above that still has a gap to "
        "close; return to that rung rather than restarting from the top; continue descending from that rung to close the remaining gap; after all gaps are closed, return to the "
        "observation loop above the ladder to verify closure and emit Ground complete), observation loop corollary (if opening observation shows no gap exists, emit Ground complete "
        ...
        "every behavioral change and execution must be observable in the log, not just described; trust evidence not theory), recursive intent structure (P1 intent primacy implies: "
        "declare intent → derive more specific micro-intents → verify each micro-intent with evidence → emit completion sentinel; this pattern applies at session level, thread level, "
        "and rung level; every boundary between intent scopes requires a sentinel), smallest intent scale: individual test assertions — each assertion in a validation file is an intent "
        ...
        "test file); Validation rung: create stub test structure that encodes intent from formal notation — write test file with test function names that describe expected behavior but "
        "the assertions are not yet implemented (placeholder assertions that will fail); create stub implementation files with function signatures but no implementation details; these "
        "stubs provide the structure that the assertion rung will exercise; Assertion rung is per assertion: each individual assertion is its own rung. Implement ONLY enough to make "
        "THAT specific assertion green. Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and "
