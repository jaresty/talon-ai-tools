...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "expectation was incorrect. before emitting ✅ Validation artifact V complete, the artifact content must appear in the current response; artifact content means each test block "
        "(name + assertion body) — imports, describe wrappers, and setup blocks are not required; emitting the sentinel without any test block in the transcript is a protocol "
        "violation; if no P4 EV step (1) pre-existence check tool-call result exists, ✅ Validation artifact V complete may not be emitted; a file-write tool call result showing the "
        "artifact was written must appear before V complete — artifact content appearing in prose without a file-write tool call does not satisfy this gate; the validation artifact "
        "must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be "
        "rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be "
        "rewritten. Before writing the validation run observation rung label, invoke the validation artifact via a test runner; a harness error (import failure, syntax error, missing "
        "file) is not a red run — the test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating any run as a red "
        "witness. when VRO exec_observed is a harness error caused by a missing implementation file: 🔴 Gap: and 🟢 Implementation gate cleared may not be emitted after a harness error; "
        "the only valid next token is the executable implementation rung label; HARD STOP is not valid. when VRO exec_observed is a harness error caused by a test file error (syntax "
        "error, import of a non-existent component in the test itself): same block rule applies; the only valid next token is a tool call that repairs the test file at the EV rung — "
        "not HARD STOP. when VRO exec_observed is a test-interaction failure — the tests loaded and ran but could not interact with the component under test (e.g., pointer-events: "
        "none, role not found, element not accessible) — 🔴 Gap: may not be emitted and 🟢 Implementation gate cleared is not valid; the only valid next token is a tool call that repairs "
        "the component's accessibility or the test's interaction strategy at the EV rung; this is a harness error class, not a behavioral failure. a green 🔴 Execution observed: is only "
