...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "minimal-step cycle: each edit implements only enough to advance past the current observed failure and nothing more; after each edit, re-run the validation artifact before "
        "producing any further implementation change; a single undifferentiated implementation pass that produces a passing suite without intermediate re-runs is a traversal violation "
        "— it is indistinguishable from anticipating the result; if a re-run within this cycle produces a green result and any newly-passing test has no prior recorded failure from "
        "this cycle, that test is uncovered and the vacuous-green check applies unconditionally — either the behavior was already implemented (verify; the thread may be complete) or "
        "the validation artifact is vacuous with respect to the declared gap and must be corrected] → observed running behavior (observation rung — no new files may be created at this "
        "rung; OBR is satisfied by invoking artifacts already produced by this ladder and recording their output; a screenshot, preview panel, server-running confirmation, URL "
        "response, or build success does not satisfy this rung regardless of label; for UI artifacts, a server response does not confirm rendering; for APIs, an HTTP 200 does not "
        "confirm correct data; for code contexts, observed running behavior is satisfied only by tool output that directly demonstrates the declared feature: rendered DOM content "
        "showing the declared UI behavior, an API response body containing the declared data, or an end-to-end test output naming the specific behavior from I; the observation must "
        "demonstrate the declared behavioral intent was satisfied by tool output traceable to the declared gap; R3 validity is determined by output content — output naming only "
