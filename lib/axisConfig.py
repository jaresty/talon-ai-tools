...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "criteria rung instantiates the first thread’s gap assertion only. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no "
        "anticipated cases; it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) "
        ...
        "compilation result never satisfies this gate regardless of what it names; for a React or web UI component, invoke the render utility (e.g., renderToStaticMarkup or the testing "
        "framework’s render function reading container.innerHTML) and emit the resulting HTML fragment showing browser-visible content — starting a dev server does not satisfy this "
        "rung; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared behavior; a 🔴 Execution "
        "observed: block that is empty or blank, or contains only static-analysis output (file reads, grep results, directory listings), does not satisfy this gate at any rung — the "
        "block must contain output produced by running the artifact; if the declared gap names a real network endpoint, the OBS must include verbatim output showing a real HTTP request "
        "or response — a test run whose network calls are mocked does not satisfy this gate; before any tool call touching the filesystem at this rung, emit a provenance statement: "
        "‘Invoking [artifact] produced at [rung name] in this thread’ — if the rung label cannot be located in the transcript, the tool call is prohibited; the provenance statement "
        "does not replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: with non-empty "
        "verbatim output appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a "
        "protocol violation; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running "
        "behavior label has been written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it that "
        "directly demonstrates the criterion — a test pass is not a demonstration; if the 🔴 Execution observed: at the observed running behavior rung contains test runner output (pass "
        "counts, test names, duration summary), it does not satisfy the OBR gate — re-invoke the implemented artifact directly before continuing; ✅ Thread N complete may not be emitted "
        "after an OBR 🔴 Execution observed: that shows test runner output; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already "
        "closed (return to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — "
        "dismissing a failing test as unrelated without an explicit written acknowledgment is a protocol violation; ✅ Thread N complete may not be emitted unless a 🔴 Execution "
        "observed: from a full test suite run exists in the transcript after the most recent OBR tool call in this thread — the only valid next action if no such result exists is the "
        "tool call that runs the suite; once ✅ Thread N complete is emitted no further output in that cycle is valid; Before writing the criteria, formal notation, executable "
        ...
        "produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared licenses the first edit — it does not complete "
        "the thread; after emitting it, the executable implementation rung, the observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread "
        "N complete is valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may not appear until the observed "
        "running behavior rung has fired for this thread. Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior to the system — must be "
        "followed inline by a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must "
        "contain exactly one entry per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol violation. The prose rung must be "
        "re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the "
        "re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — not from a prior cycle — to exist "
        "in output; the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a "
        "failure. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, "
        "authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read the prose rung output via a tool call to confirm every predicate is "
        "covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry containing a verb or phrased as a "
        ...
        "all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every "
        ...
        "One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upward returns follow the failure class: if the implementation did not close "
        ...
        "before producing the final report. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order — no "
        "new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Seven rungs in order: prose → criteria → formal notation "
        "→ executable validation → validation run observation → executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ "
        "Manifest declared — N threads: [numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: "
        "[what the verbatim output reveals]; 🛑 HARD STOP — upward return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation "
        "artifact V complete; ✅ Thread N complete; ✅ Manifest exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current tests]; ✅ I-formation "
        "complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.",
