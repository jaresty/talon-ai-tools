...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "why evaluation scaffold must be produced and verified PASSING before Challenge; what GOES WRONG if scaffold fails at Baseline: cannot distinguish broken framework from actual "
        "gaps; RULE: Baseline evaluation scaffold must PASS before Challenge [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if "
        "you create without structure: unfocused output; RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success conditions before organization; what "
        "GOES WRONG if you organize without criteria: no way to evaluate; RULE: criteria before structure [ ] Intent before criteria: derive from P1 — outcome before success "
        "conditions; what GOES WRONG if you define success without outcome: criteria irrelevant; RULE: intent before criteria [ ] Evidence at proximity: derive from P3 — evidence must "
        "be temporally close to action; what GOES WRONG if delayed: proof weakens; RULE: evidence immediately after action [ ] Evidence echoed: derive from P3,P4 — explain how evidence "
        ...
        "solution organized? Derivation: from criteria, derive organization. Evidence: internal consistency. 4. Baseline — Does the evaluation scaffold exist? Derivation: from "
        "criteria, derive evaluation framework. Evidence: evaluation structure produced and verified to work. IMPORTANT: Baseline evaluation scaffold must WORK before Challenge can add "
        "new criteria. A scaffold that fails at Baseline means evaluation framework is broken, not that gaps exist. 5. Challenge — What evidence proves gaps exist? Derivation: from "
