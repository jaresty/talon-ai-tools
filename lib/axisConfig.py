...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "appears in the transcript; each listed dimension quotes a verbatim substring of the '[text]' string from §1; (3) write '## Enforcement sequence' — this heading must appear in "
        "the transcript before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a literal substring what the "
        "prior sub-heading's passing state admits that this sub-heading rejects; (4) write '## Completion check' — this heading must not appear before '## Enforcement sequence' in the "
        "transcript; each covered dimension cites a verbatim string that appears as a substring of a tool-result block visible above '## Completion check' — a dimension declared "
        "covered without such a substring does not satisfy §4; (5) write '## Path enumeration'; enumerate every path by which §1–§4 headings could be present without the governing goal "
