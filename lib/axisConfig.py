...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "artifact, proceed immediately to formal notation — enumerating remaining criteria or planning future gap cycles before the current cycle completes is a protocol violation; "
        "subsequent criteria emerge only after the current cycle closes. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no "
        ...
        "signatures without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: complete function bodies, variable assignments, working logic sequences; "
        "labeling prohibited content as pseudocode does not exempt it from this prohibition; if the criterion is already a complete behavioral specification — naming observable "
        "behavior and a falsifying condition without referencing function names, field names, or implementation internals — the formal notation artifact may restate it verbatim with an "
        "explicit declaration that no elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript and the audit section still applies; the audit "
        "section is separate and named; the rung is incomplete until all criteria are encoded. Only validation artifacts may be produced at the executable validation rung — no other "
        "content is permitted before ✅ Validation artifact V complete; each test function asserts exactly one behavioral property — one behavioral property per test function; multiple "
        ...
        "test, re-read the formal notation and assert each structural constraint it encodes; a pre-existing test that happens to pass does not satisfy this rung; when the declared "
        ...
        "artifact that fails when the test-under-modification does not exhibit the criterion; before writing the test, check whether an existing test file covers the same module and "
        "add there if so; when the validation artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: "
        ...
        "attempted. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is written. Before writing the "
        "validation run observation rung label, invoke the validation artifact via a tool call; a harness error (import failure, syntax error, missing file) is not a red run — the test "
        "must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness; a green 🔴 Execution observed: "
        "is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared showing the "
        "test logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; "
        "stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP "
        ...
        "emitting it at any other position is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before "
        "any further edit. Upon writing the observed running behavior label, re-emit the criterion for this cycle, then immediately invoke the implemented artifact via a tool call and "
        "emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a "
        "non-technical observer of the running system would see — not the test suite, not process state unless the criterion is specifically about process state; a build output, bundle "
        "manifest, or compilation result never satisfies this gate regardless of what it names; for a UI component, this means rendered browser-visible text and values; if the "
        "implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared behavior; a 🔴 Execution observed: whose "
        "verbatim block is empty or blank does not satisfy this gate; if the declared gap names a real network endpoint, the OBS must include verbatim output showing a real HTTP "
        "request or response — a test run whose network calls are mocked does not satisfy this gate; before any tool call touching the filesystem at this rung, emit a provenance "
        "statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the rung label cannot be located in the transcript, the tool call is prohibited; implementation "
        "edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written after the "
        "most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it that directly demonstrates the criterion — a test "
        "pass is not a demonstration; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already closed (return to criteria) or the "
        ...
        "complete is emitted no further output in that cycle is valid; at most one thread is in progress at a time — the prose rung of thread N+1 may not begin until ✅ Thread N "
        "complete has been emitted for thread N. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running behavior label, "
        "emit 🔴 Gap: naming the declared gap — what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops "
        "after naming the gap. 🟢 Implementation gate cleared licenses the first edit — it does not complete the thread; after emitting it, the executable implementation rung, the "
        ...
        "prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]. The prose "
        "rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately "
        "derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — not from a prior "
        "cycle — to exist in output. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, "
        ...
        "predicate is covered; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a "
        ...
        "intent to stay fixed. Advance through every feasible rung without pausing for user confirmation between rungs — each rung may not be skipped or combined with another, "
