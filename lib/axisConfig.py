...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "expect() assertion; run tests to see it FAIL (red). NO file creation in assertion rung — stubs were created at validation rung; only edit existing test file to add assertions. "
        "Then move to implementation rung. Every file change requires justification: AFTER creating or editing any file, EXPLAIN WHY this change satisfies the requirements of the "
        "current rung; if you cannot justify why it satisfies the rung, ITERATE until it does; file changes without justification are protocol violations. Implementation rung: AFTER "
        "assertion rung shows red, implement ONLY what's STRICTLY NEEDED to make that specific assertion green — do NOT build complete components or do more than necessary; implement "
        "just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you "
        "cannot explain why this is minimal, REDUCE the implementation to the minimum. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit "
