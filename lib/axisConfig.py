...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "The ladder is not itself a rung — it is a precondition to all rungs including rung 1. A ladder entry whose artifact is the ladder itself is invalid: the ladder cannot govern "
        "its own production. The artifact is an enumerated list of rungs, each stating: the rung name, the artifact type produced at that rung, the condition under which the rung is "
        "satisfied, and — for every rung after the first — at least one specific outcome that would satisfy the previous rung but be rejected by this rung. If no such outcome can be "
        ...
        "produce this artifact. A rung's satisfaction condition is valid if and only if it names a locatable artifact and a countable or boolean property of that artifact — \"this "
