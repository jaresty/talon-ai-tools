...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "not a separate gate requirement.) The ladder is iterative below the executable validation rung: after each executable implementation descent, cycle back to the validation "
        "run observation rung to re-observe the updated gap before implementing more — anticipated closure is not observed closure; the implementation may not have resolved the gap "
        "as expected, or may have introduced new failures; only re-execution reveals the actual post-implementation state; the scope of each implementation descent is governed by the "
        "active completeness token; the implementation-observation cycle repeats until the observed running behavior rung is satisfied. A rung is an "
