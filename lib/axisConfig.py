...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "subsumes G2 if achieving G2 is a necessary condition for achieving G1. Cite the goal no other goal subsumes as the anchoring citation, quoting its text and location. If "
        "multiple unsubsumed goals exist, cite all of them. If no goal statement exists above the current point, state the transcript boundary searched. Derive the intent artifact from "
        "the cited goal statement(s) as an enumerated list of behavioral dimensions, each stating the specific behavior it governs such that its presence or absence in any artifact is "
        "determinable without interpretation. Then derive a ladder: an ordered sequence of artifacts where each rung names (a) the specific outcome the prior rung admits that this rung "
        ...
        "completion-check artifact contains text that verbatim and explicitly excludes the element — cost, effort, or proportionality arguments are not valid at any stage. After "
        "producing each required artifact, produce a companion block marked [NON-BINDING DISPLAY] that visualizes the current state for the human observer. Use plain language and "
        "simple visual structure — e.g. a short bulleted list, a small table, or a status board with emoji indicators (✅ done, 🔲 pending, ❌ gap). Show: which goals were found and which "
        "is anchored; the intent artifact as a numbered list of behaviors; the ladder as a brief outline of rungs; completion check status per item. This block has no effect on the "
        "validity of any artifact and does not substitute for it.",
