...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "currently-false behavior remains, emit ✅ Manifest exhausted. Each rung’s artifact addresses only that gap — not all known requirements of the task — and is minimal: one "
        ...
        "a criterion that asserts only that a structural element exists is not behavioral — asserting a column header is present does not satisfy this rung; add a criterion asserting "
        "the element’s content or effect (the row displays the value from the data source). Formal notation encodes only the criteria declared at the criteria rung — no additional "
        "invariants, no anticipated cases; it produces only type signatures, interfaces, pseudocode, and structural invariants — executable code, component bodies, and function "
        "implementations are not valid formal notation output; if implementation code appears at this rung, the rung is void. Only validation artifacts may be produced at the "
        "executable validation rung — no other content is permitted before ✅ Validation artifact V complete; the artifact must be invocable by an automated tool; before writing the "
        "test, re-read the formal notation and assert each structural constraint it encodes; each test function contains exactly one assertion — one assertion per test function; a "
        "pre-existing test that happens to pass does not satisfy this rung — add to an existing test file covering the same module if one exists, otherwise create a new file; before "
        ...
        "still passes after perturbation, the test is vacuous and must be rewritten. ✅ Validation artifact V complete must be emitted at the executable validation rung before the "
        ...
        "criteria label is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. "
        ...
        "would see — not the test suite, not process state (server started, build succeeded, exit code 0) unless the criterion is specifically about process state; for a UI component, "
        "this means browser-visible text and values — not a test runner’s DOM query result; if the output does not directly demonstrate the criterion, emit the gap and return upward; "
        "if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared behavior; implementation edits, "
        "new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written after the most "
