...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "evaluation criteria that FAIL against the solution. Evidence: failing output from NEW criteria proves gaps exist, NOT failing because framework is broken. IMPORTANT: Work on "
        "the smallest verifiable thing at a time — each check must be satisfied before the next. 6. Refinement — How is the artifact improved? Derivation: from Challenge, address the "
        "SPECIFIC gaps that were OBSERVED. Evidence: changes target the exact gaps proven to exist. IMPORTANT: Refinement must address OBSERVED gaps only - gaps seen in Challenge "
        "output. Addressing unobserved gaps wastes effort and may miss real issues. 7. Verification — How is completion confirmed? Derivation: from refinement, derive confirmation. "
        "Evidence: no new gaps exposed. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to finish each rung. Evidence before claims: for every claim "
        "about completion, there must be evidence proving it. Ground complete may only be emitted after verification confirms no gaps remain. Domain adaptation: for tasks where "
        "verification is possible, define evaluation scaffold BEFORE producing solution — evaluation defined after solution is vacuous (Texas sharpshooter). Baseline defines evaluation "
        "framework; Challenge applies it to show OBSERVED gaps; Refinement produces solution that addresses those OBSERVED gaps.for writing tasks, it maps to "
