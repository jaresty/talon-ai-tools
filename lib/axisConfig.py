...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "— a manifest omitting a required rung is malformed before descent begins. I is the declared intent governing the invocation; I precedes and is not itself an artifact. V is a "
        "constraint artifact self-contained to evaluate the next artifact without consulting I. O is the output evaluated against V. Three rules govern every thread: R1 (I is fixed): "
        "every artifact derives from I through faithful derivation of the prior rung — form changes, intent does not. R2 (rung criterion): an artifact is a rung iff upward-faithful "
        "(derived from the prior rung without adding unstated constraints) and downward-sufficient (self-contained to evaluate the next artifact without consulting I); domain phases "
        "are not downward-sufficient — writing tests for ‘API layer’ requires knowing what the API must do, which comes from I, not the phase name. R3 (observation terminates): every "
        "thread descends to observed running behavior; observed running behavior is an executable rung — it requires 🔴 Execution observed (verbatim tool output) and 🔴 Gap (naming the "
        "specific behavior from I the output demonstrates) before ✅ Thread N complete may appear; only tool-executed output with a declared gap satisfies the execution gate; skipped "
        "tests are not observations; observed running behavior must reference the specific behavior declared in the gap — a screenshot, preview panel, server-running confirmation, URL "
        "response, or build success does not satisfy this rung regardless of label; system-state events (server running, build succeeded, tests passing) confirm infrastructure, not "
        "behavior; for UI artifacts, a server response does not confirm rendering; for APIs, an HTTP 200 does not confirm correct data; the observation must demonstrate the declared "
        "behavioral intent was satisfied by tool output traceable to the declared gap. For code contexts, observed running behavior is satisfied only by tool output that directly "
        ...
        "naming the specific behavior from I; build pipeline output (compiler success, server start confirmation, URL shell response) is not running behavior regardless of label; R3 "
        "validity is determined by output content, not script type — the specific behavior named in the gap must appear as a verifiable event in the tool output; output naming only "
        "infrastructure state (‘route loads successfully’, ‘server running’, ‘200 OK’) does not satisfy R3 regardless of what tool or script produced it. A rung is complete when and "
        "only when its artifact has been produced — not when it has been listed, planned, or described; a rung label during execution marks the point where the artifact begins and must "
        "be emitted at every rung transition — it is not a section heading for planning or description; absence of a rung label at a transition is a traversal violation, not a "
        "stylistic omission; no content other than the artifact itself may appear between a rung label and the artifact it precedes. A rung is not achievable when the domain provides "
        "no standard artifact type for it; this must be stated explicitly with justification — convenience or prior knowledge do not make a rung not achievable. ground is a Process "
        "method — the task governs the character of each rung’s output but not the process structure; the manifest, rung sequence, and execution gates are mandatory regardless of which "
        "task or other tokens are combined with ground; completeness governs rung depth, not rung existence. Formal notation must satisfy R2: every behavioral constraint from the "
        ...
        "constraint’, ‘exclusion constraint’) is UNENCODED, not a behavioral invariant; the rung is incomplete until every criterion resolves to a behavioral invariant — no UNENCODED "
        "entries may remain; emit ‘✅ Formal notation R2 audit complete — N/N criteria encoded’ on its own line as the gate before advancing to executable validation. When I-formation "
        "is required: observation of existing code or running behavior sufficient to establish I is required before the manifest when I cannot be declared from context alone — skipping "
        ...
        "satisfy this rung) → validation run observation (only satisfied by an observed failure; a passing run is a gap signal — see gate_validity) → executable implementation → "
        "observed running behavior. A gate is a conversation-state condition: open when and only when the required event has occurred in this conversation for this thread. Underlying "
        "all compliance failures is one epistemological error: substituting model knowledge for conversation events — prior knowledge, anticipation, and model reasoning cannot satisfy "
        "any gate regardless of accuracy. For executable rungs, emit 🔴 Execution observed: [verbatim tool output — content composed without running the tool is invalid] then 🔴 Gap: "
        "[what the verbatim output reveals] on their own lines before any implementation artifact; the 🔴 Execution observed sentinel body must be a complete block delimited by triple "
        "backticks containing the complete tool output — nothing omitted; a single extracted line does not satisfy this sentinel regardless of accuracy; no prose, summary, or "
        ...
        "immediately before it. No implementation artifact — including planning text, code blocks, or tool calls — may appear before valid Execution observed + Gap sentinels; if any "
        "implementation content appears without these sentinels immediately preceding it, it is invalid and must be discarded before the tool is run. The 🔴 sentinel format is reserved "
        "exclusively for executable rung gates; for non-executable rungs, observation appears inline as labeled prose. The validation run observation rung is only satisfied by an "
        "observed failure that reveals the declared gap — a prior recorded failure applies to the specific test that produced it, not the thread as a whole; when the validation "
        ...
        "compositions — confirm by quoting the sentinel text from this conversation; if the sentinel cannot be located in this conversation, the gate is not open; (b) no implementation "
        "content appeared before them; if either condition is unmet, the rung is incomplete and must not be treated as satisfied. A build error or compile failure does not satisfy this "
        "rung — the validation artifact must compile and execute before its output can reveal a behavioral gap; a failure to compile means the artifact is incomplete and must be "
        "corrected before the gate can open. Before producing implementation code, emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; the quote must "
        "be verbatim from the 🔴 Execution observed sentinel of this thread; quoting anticipated output or a prior thread’s observation is invalid. Before executable implementation "
        "begins, both of the following must appear in this order: first ‘✅ Validation artifact V complete’ on its own line immediately after the validation run observation rung has "
        "declared a gap, then ‘🟢 Implementation gate cleared’ on its own line — V-complete alone does not open the implementation gate; both sentinels are required in sequence; "
        "V-complete may not appear after tests pass, after implementation, or after observed running behavior; it marks the transition from validation to implementation, not the "
        "completion of the thread. Gap-locality: the gap gating rung N is the output of executing rung N-1; no gap from any higher rung, and no element of I directly, may serve as the "
        "gating gap for the current rung. Minimal scope: the current rung’s artifact addresses the declared gap and nothing more; implementing beyond the declared gap is a violation — "
        "not a benefit; minimal scope applies to the entire invocation, not just individual rungs — any artifact added after the ladder is complete that was not present in I must be "
        "treated as a new I requiring its own manifest and descent; adding it inline without a new manifest is a scope violation regardless of how it is labeled; the validation "
        "artifact is frozen at the point the gap is declared; any modification to the validation artifact after that point — including reducing test count, simplifying mocks, or "
        "weakening assertions — requires upward correction: return to criteria, demonstrate the modified artifact still encodes the gap, and re-emit the R2 audit before redescending; a "
        ...
        "gap requires is a gap signal, not a success. Upward revision is always permitted when a gap is observed between prior understanding of I and something encountered via direct "
        "interaction with reality, code, or a stakeholder; upward revision must be signposted with: what was observed, which rung is being revised, and why; it is never permitted to "
        "change I without first observing a gap in V that derived it; changing I requires revising every artifact derived from it to restore chain consistency before descent continues. "
        "Upward correction is mandatory when a rung failure reveals that a higher rung is deficient — failing to correct the highest broken rung means every artifact below it is "
        "derived from a deficiency that will resurface at each subsequent rung — correction is not a detour but the only path to a valid artifact at the current rung; before correcting "
        "any higher rung, emit what was observed at the current rung, which higher rung is being corrected, and why — a correction without this observation record is invalid and "
        "indistinguishable from enrichment; find the highest broken rung, correct it, and rederive every artifact below it before redescending; this is correction, not enrichment — the "
        "corrected rung must still be faithful to I; if the broken rung is I itself, surface as a revision signal and stop the current thread. Intent precedes its representations. "
