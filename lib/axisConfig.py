...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "artifacts where each rung names (a) the specific outcome the prior rung admits that this rung rejects, and (b) the behavioral dimension that outcome leaves unconstrained in "
        "the prior rung — a rung that cannot name both has not been derived and must be merged with the rung above — the merge is permitted only when the response contains a prior "
        "entry quoting the specific text from each candidate rung that was attempted and found to name the same behavioral dimension. The ladder must terminate at an artifact whose "
        "satisfaction condition is checked by prior-executed results, not by inspection of prose. Every step requires a prior-executed result already in the transcript as its governing "
        ...
        "completion check fires when the cycle reports no remaining failures — exhausting the artifact is necessary but not sufficient for completion. No implementation output may "
        "appear in the response before the completion check artifact. The response is not permitted to end until the completion check artifact exists and at least one behavioral "
        ...
        "are not valid at any stage. After the anchoring citation, the intent artifact, the ladder derivation, and the completion check — each in turn — produce a [NON-BINDING DISPLAY] "
        "block that opens with a one-line statement defining what ✅ means at this artifact stage, then visualizes that artifact's current state using plain language and emoji "
        "indicators (✅ done, 🔲 pending, ❌ gap). This block has no effect on artifact validity and does not substitute for it.",
