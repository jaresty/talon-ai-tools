...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "target must be the implementation artifact itself, not a test harness wrapping it; A file read never satisfies this rung — the block must contain output produced by running "
        "the artifact. If the artifact requires a start step before querying (e.g., a web server), two tool calls are required in sequence: process-start then query — the 🔴 Execution "
        "observed: sentinel may not be emitted until both complete. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact "
        "exists. If no invocable process exists, open a gap cycle to make the artifact directly invocable. Before any tool call at this rung, emit a provenance statement naming the "
        "invocation target: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the target is a test runner, stop: the invocation target must be the implementation "
        "artifact itself running as a live process, not a test harness; the provenance statement does not replace the tool call — the tool call must follow it in the same response; ✅ "
        "Thread N complete may not be emitted unless all of the following exist in this cycle's transcript: (1) a tool call exists in the transcript after the observed running behavior "
        "label in the current cycle; (2) observed running behavior label has been written after the most recent 🟢 Implementation gate cleared in this thread; (3) non-empty verbatim "
        "output appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol "
        "violation; (4) the 🔴 Execution observed: directly demonstrates the criterion — if it does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and "
        "apply the upward-return failure-class rules; (5) a full test suite run result exists after the most recent OBR tool call — the only valid next action if no such result exists "
        "is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason. ✅ Thread N complete immediately follows 🔴 "
        "Execution observed: at the OBR rung — not before it; emitting ✅ Thread N complete before the OBR rung tool call and 🔴 Execution observed: sentinel exist in this cycle is a "
        "protocol violation. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal notation, executable validation, "
        "executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared gap — what the prior rung’s artifact leaves unresolved; immediately produce the "
        "rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared is the first token of the executable implementation rung — "
        "no tool call, no prose, and no file modification may appear before it in the current response; emitting a file-write tool call before 🟢 Implementation gate cleared is a "
        "protocol violation regardless of whether impl_gate is emitted later in the same response. it does not complete the thread; after emitting it, the executable implementation "
        "rung, the observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a "
        "tool call that creates or modifies an implementation file; ✅ Thread N complete may not appear until the observed running behavior rung has fired for this thread. Each sentence "
        "in the prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; [T: "
        "gap-name] markers are valid only in the prose rung — their presence in any other rung artifact is a protocol violation; before emitting ✅ Manifest declared, count every [T: "
        "gap-name] marker in the prose — the manifest thread list must contain exactly one entry per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding "
        "manifest entry is a protocol violation. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an "
        "upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s "
        "validation run observation rung — not from a prior cycle — to exist in output; the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung "
        "does not satisfy the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, count the thread entries in the list — the N in '✅ Manifest "
        "declared — N threads:' must equal the count of listed threads; a mismatch between the declared N and the thread list length is a malformed manifest. Before emitting ✅ Manifest "
        "declared, if the prose contains no [T: gap-name] markers, ✅ Manifest declared is blocked — re-emit the prose with [T: gap-name] markers added inline after each behavioral "
        "predicate before proceeding. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, "
        "validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; count the [T: gap-name] markers in the prose you just wrote to "
        "confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry containing a verb or "
        "phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the criteria rung, not the manifest; ✅ Manifest "
        "declared may be emitted exactly once per invocation. only threads declared in ✅ Manifest declared may be addressed — creating a thread not in the manifest is a protocol "
        "violation; when a new gap is discovered mid-run, return to prose and re-emit a revised manifest — do not create an undeclared thread. Outputting a rung label is what begins "
        "that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. I is the declared intent governing the "
        ...
        "behavior rung — it is required in every cycle; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; all other rung transitions are "
        "continuous within the same response. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through executable "
