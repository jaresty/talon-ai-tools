...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "NOT sufficient; WHAT GOES WRONG: diagnostic commands replace actual tests; RULE: write the test, run it, show the failing output - Single gap rule — one observed failure as "
        "active gap; RULE: one gap per refinement - Minimal refinement — smallest observable behavior that exposes the next failure in the sequence; do NOT try to close the gap "
        "entirely — only produce an artifact sufficient to see the next failure; WHAT GOES WRONG: over-engineering, solving too much at once; RULE: minimal artifact to reveal next "
        "failure point - Meta-loop — after EVERY Verification, Challenge runs again; RULE: loop until zero-gap - Zero-gap evidence — absence must be evidenced; RULE: evidence of no "
        "failure required - Full intent coverage — every criterion evaluated at least once; RULE: all criteria evaluated - Gap authorization — no implementation until failing check "
        "exists; RULE: failing check precedes implementation - Enforcement validity — every guard must fail visibly before pass; RULE: fail-first then pass - Guard-implementation "
        "separation — guards and implementation change in separate rungs; RULE: separate rungs - Consequence materialization — every 'what GOES WRONG' creates visible consequence; "
        "RULE: immediate visible consequence - Preservation verification — pre/post characterization tests; RULE: green→change→green required - Hard gate — no rung until sentinels "
        "visible; RULE: sentinels required before descent - Proximal enforcement — rules and consequences re-emitted at each rung; WHAT GOES WRONG: rules forgotten, enforcement "
        "distant; RULE: re-emit relevant rules at each rung start - Self-referential derivation — when deriving gaps from this protocol itself, you MUST cite the governing rule, show "
        "the mechanism that would detect failure, and demonstrate why the gap would cause the protocol to fail; WHAT GOES WRONG: gap claims without evidence; RULE: claims require "
        "visible mechanism failure - Challenge commensurability — the verification challenge must match the automatable strength of the gap claim; if gap claims an executable artifact, "
        "challenge must REQUIRE EXECUTABLE VERIFICATION (tests, scripts, runtime checks); if gap claims text, challenge must require text verification (criteria sheet, derivation "
        "template); WHAT GOES WRONG: weak verification (grep) for automatable claim; RULE: verification automatable strength >= claim automatable strength; WHEN IMPLEMENTATION MODIFIES "
        "BEHAVIOR, ONLY EXECUTABLE VALIDATION COUNTS — no amount of code inspection satisfies the challenge - Demonstrated failure — verification must SHOW the specific behavior that "
        "fails via EXECUTION; capture and cite the actual failing test output, runtime error, or observable misbehavior as evidence; WHAT GOES WRONG: running verification without "
        "showing what fails; RULE: cite actual failing execution output before proceeding; FOR BEHAVIORAL GAPS, VERIFICATION MUST EXECUTE — assertions without running code are "
        "insufficient ⚠️ HARD GATE: Protocol derivation is MANDATORY FIRST STEP — you MUST derive all protocol rules with their consequences and enforcement mechanisms BEFORE emitting "
        "🔵 Protocol derived. For EACH rule, you MUST explicitly state: (1) THE RULE — what must be done, (2) WHAT GOES WRONG NOW — the immediate consequence of violation (not vague "
        "future harm, but what breaks NOW), (3) THE ENFORCEMENT — what mechanism catches the violation. Do NOT emit the sentinel until every rule has this three-part derivation. The "
        "ladder cannot begin until this derivation is complete and the sentinel is visible. TRANSITION CHECKPOINT: Before next rung, verify: [artifact completed?], [evidence visible?], "
        "[gaps remaining?], [intent aligned?], [guards unchanged?], [next rung requires?]. If any fails, do NOT proceed. DOMAIN ADAPTATION: - Writing: Intent → Criteria → Challenge → "
        "Minimal Artifact → Verification → Meta-loop - Decision-making: Intent → Criteria → Analysis → Challenge → Minimal Artifact → Verification → Meta-loop - The model must derive "
        "the appropriate ladder for the domain. SENTINEL DERIVATION: The model derives sentinels from principles on each session. Template: declare intent → derive criteria → structure "
        "→ challenge → refine → verify.",
