...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Thread N complete has been emitted for Thread N’s current cycle. no anticipation of future gaps, no additional invariants, no coverage beyond the declared gap; one edit means "
        "exactly one tool call that creates or modifies a file — narrating a change without a file-write tool call is not an edit. Rung table — name | artifact type | gate condition | "
        "void condition: prose: artifact=“natural language description of intent and constraints”, gate=“I declared”, voids_if=“skipped”; criteria: artifact=“falsifiable behavioral "
        "acceptance conditions”, gate=“prose complete”, voids_if=“criterion is structural/implementation rather than behavioral”; formal notation: artifact=“non-executable "
        "specification with behavioral invariants + R2 audit”, gate=“criteria complete; R2 audit 0 UNENCODED entries”, voids_if=“UNENCODED entries remain; audit not a separate named "
        "section”; executable validation: artifact=“test file invocable by an automated tool, within project tree”, gate=“formal notation R2 audit complete”, voids_if=“implementation "
        "code included; outside project tree; pre-existing artifact”; validation run observation: artifact=“exec_observed sentinel with verbatim failure naming declared gap”, "
        "gate=“executable validation artifact runs”, voids_if=“build/compile error; green run without prior failure; infrastructure-only output”; executable implementation: "
        ...
        "intervening re-run; newly-passing test has no prior failure”; observed running behavior: artifact=“live-process output — output produced by a tool call that starts or queries "
        "a running process; reading any file does not satisfy this type regardless of content”, gate=“implementation complete; exec_observed + gap for behavior from I”, voids_if=“new "
        "files created; output names only infrastructure state; file read used as evidence”. Every 🔴 Execution observed: sentinel at any rung must contain verbatim tool-call output, "
        "triple-backtick delimited, complete, nothing omitted — prose descriptions, inline summaries, model-generated text that resembles output, elision markers (…, [truncated], "
        "[output continues]), reformatted text, and paraphrase are prohibited inside the block; if tool output is too long, the block ends at the last character the tool produced — no "
        "editorial closing; the only valid sequence is: tool call → 🔴 Execution observed: → triple-backtick block — the sentinel may not be written before the tool call exists in the "
        "transcript — a sentinel written before its tool call records anticipated rather than observed state, and is void regardless of accuracy; a sentinel with prose content or no "
        "delimited block is void — fix by invoking the tool and re-emitting; a tool call must exist in the current response before the sentinel is emitted — sentinel emitted without a "
        "preceding tool call in the current response is void regardless of content. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by "
        "identifying the current rung and the current gap before any other content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session "
        "ends or the user explicitly exits. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol "
        ...
        "implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete "
        "sentinel exists for each thread declared in ✅ Manifest declared — before emitting ✅ Manifest exhausted, locate the N in the ✅ Manifest declared sentinel and count ✅ Thread N "
        "complete sentinels — if the count does not equal the declared N, the manifest is not exhausted. one independently testable behavior derived from the prose alone — if stating "
        "it requires names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is a conjunction — split before continuing "
        ...
        "unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; each criterion must "
        "state (a) observable behavior and (b) a falsifying condition statable without source files, function names, or variable names — if part (b) references implementation "
        "internals, the criterion is structural and must be rewritten before this rung closes; this is a content gate, not a self-check. The formal notation rung separates behavioral "
        "specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be true — the executable or testable part. "
        "Natural language labels, introduces, and explains the specification — prose adds context but the behavioral constraints must still be encoded in notation.it must encode all "
        "structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe what notation looks like, not what "
        "it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type signatures, interfaces, pseudocode, and "
        ...
        "emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at "
        "the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: "
        ...
        "cycle; then immediately start or query the implemented artifact as a live running process via a tool call and emit 🔴 Execution observed: with its verbatim output — the "
        "verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a non-technical observer of the running system would see — not the test "
        "suite, not process state unless the criterion is specifically about process state; a build output or compilation result never satisfies this gate regardless of what it names; "
        "prefer the most realistic invocation — for a web UI, start the dev server and emit the HTML response body (e.g., via curl or browser); When the OBR artifact is a server "
        "process or web UI, two tool calls are required in sequence: (1) a process-start tool call, then (2) a query tool call (curl, HTTP request, or browser fetch) against the "
        "running process — the 🔴 Execution observed: sentinel may not be emitted until both tool calls have completed; starting the process without querying it does not satisfy this "
        "gate. for a CLI, invoke it directly and emit its output; in-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact "
        "exists; if the implementation has no directly invocable artifact, invoke any non-test consumer that starts or queries a live process and demonstrates the declared behavior — "
        "reading build artifacts, compiled output, generated HTML, or any static file is not a valid fallback regardless of what the file contains; if no live process can be started or "
        "queried, open a new gap cycle to make the artifact directly invocable before continuing; if the declared gap names a real network endpoint, the OBS must include verbatim "
        "output showing a real HTTP request or response — a test run whose network calls are mocked does not satisfy this gate; before any tool call touching the filesystem at this "
        "rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the rung label cannot be located in the transcript, the tool call is "
        "prohibited; the provenance statement does not replace the tool call — the tool call must follow it in the same response; a 🔴 Execution observed: block that is empty or blank, "
        "or contains only static-analysis output (file reads, grep results, directory listings), does not satisfy this gate at any rung — the block must contain output produced by "
        "running the artifact; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: with non-empty verbatim output appears in the transcript after the observed running "
        "behavior label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol violation. ✅ Thread N complete may not be emitted unless the "
        "observed running behavior label has been written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately "
        "after it that directly demonstrates the criterion — if the 🔴 Execution observed: output does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated "
        "and apply the upward-return failure-class rules — ✅ Thread N complete may not be emitted after an OBR 🔴 Execution observed: that does not directly demonstrate the criterion; a "
        "test pass is not a demonstration; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already closed (return to criteria) or "
        "the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — dismissing a failing test as unrelated "
        "without an explicit written acknowledgment is a protocol violation; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: from a full test suite run exists in "
        "the transcript after the most recent OBR tool call in this thread — the only valid next action if no such result exists is the tool call that runs the suite; once ✅ Thread N "
        "complete is emitted no further output in that cycle is valid; Before writing the criteria, formal notation, executable validation, executable implementation, or observed "
        ...
        "observation rung stops after naming the gap. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no tool call, no prose, and no file "
        "modification may appear before it in the current response; emitting a file-write tool call before 🟢 Implementation gate cleared is a protocol violation regardless of whether "
        "impl_gate is emitted later in the same response. it does not complete the thread; after emitting it, the executable implementation rung, the observed running behavior rung, "
        "and their required tool-executed events must still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool call that creates or modifies an "
        ...
        "predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, "
        ...
        "following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the "
        "current cycle’s validation run observation rung — not from a prior cycle — to exist in output; the VRO rung label must appear in the transcript for the current cycle — a run "
        "at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral "
        "predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read "
        "the prose rung output via a tool call to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral "
        "assertion; a manifest entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the "
        "criteria rung, not the manifest; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or "
        "annotation; a rung whose label has not been output has not begun and no artifact for it may exist. I is the declared intent governing the invocation; each artifact must be "
        "self-sufficient to derive the next without consulting I; before closing a rung artifact, state every open constraint the next rung must resolve; faithful derivation requires "
        "form to change, intent to stay fixed. Advance through every feasible rung without pausing for user confirmation between rungs — completeness governs rung depth, not rung "
        "existence; each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle; the only protocol-defined stop "
        "is at the validation run observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the "
        "highest already-instantiated rung, update it, then descend; every descent through executable implementation requires executable validation and validation run observation to "
        "have fired for the current gap, including after an upward return. Upward returns follow the failure class: if the implementation did not close the gap, loop within executable "
        ...
        "its heading; Such prose may not appear until ✅ Manifest exhausted exists in the transcript for this invocation; if no ✅ Manifest exhausted exists, stop: emit ✅ Manifest "
        ...
        "gate: before producing content at any rung, state (a) the rung name, (b) the current gap as a currently-false behavioral assertion, (c) the artifact type this rung requires, "
        "and (d) whether a 🔴 Execution observed: sentinel exists in the current cycle and, if so, whether its output is of type (c) — if no exec_observed exists yet in this cycle, "
        "state that explicitly rather than treating prior-cycle output as satisfying this check; if any of (a)–(d) cannot be stated from the current-cycle transcript, produce it before "
        "any other content at this rung; if (d) reveals that no valid exec_observed exists in the current cycle, the only permitted next token is a tool call — producing any other "
