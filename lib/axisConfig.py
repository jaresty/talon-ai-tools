...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "an assertion and its procedure are in play, obtains the procedure's own discriminating result for that assertion. A guard is established by an observation-producing act whose "
        "result record contains the guard's procedure verbatim; every assertion referenced later must be copied verbatim from such a result record. Because a guard detects regression "
        "only if it re-applies on its own, a Failure observation is valid only when it is produced by applying a committed, re-runnable guard artifact — a procedure persisted to a "
        "location that re-applies without human initiation — never from an inline or ephemeral one-off. Repeatable is not the same as deterministic: a guard whose observation depends "
        "on external or changing state (a live lookup, a measurement, a poll) is still a valid witness at the moment it is applied, but its observation and provenance must be retained "
        "rather than treated as reproducible on demand. Capability is not evidence: what witnesses a behavioral observation is an available qualifying observation record, not the mere "
        "fact that an act did or did not occur this turn. A qualifying observation record is a transcript-present record produced by the committed guard artifact that carries the "
        "guard's procedure identity, the assertion's identity, and the relevant A-result. A fresh application of the guard produces one; a prior result record already present in the "
        "transcript is one; model prose is never one. When the observation capability is available the response must apply the committed guard to obtain new observations; when it is "
        "unavailable it may cite qualifying observation records already present but must not synthesize, infer, or narrate an observation. Before declaring an observation unavailable, "
        "inspect the available observation records for a qualifying record; only when no qualifying record is found may the unavailable state be emitted — the unavailable state is the "
        "residue of that inspection, not a first-move declaration. A behavioral assertion for which no qualifying observation record exists is neither Failure nor structural "
        "Unobservable — emit 'Unwitnessed: assertion \"<verbatim assertion>\" — observation unavailable'; it does not contribute to 'Coverage: complete', and it is a witnessing gap, "
        "not a structural exemption. Before acting, observe the gap: emit 'Observing gap: <what is currently absent or failing>'. Shrink the artifact to the guard (Gate 1). Always "
        "attempt the simpler artifact: construct a smaller modification to the artifact and apply the guards to it. 'Overreach: not found' is a claim that no such reduction exists, so "
        "it is valid only after an attempted reduction — a qualifying observation record showing a smaller artifact that the guards reject (or a smaller artifact they accept, which "
        "you then keep). If a reduction the guards still accept exists, or a path through the artifact is not exercised or distinguished by any guard, emit 'Overreach: found — "
        "<modification or path>', trim to the boundary the guards require, and repeat. A bare 'Overreach: not found' with no attempted reduction does not satisfy this token. Both "
        "'Overreach: found' and 'Overreach: not found' are observation-dependent; when no qualifying observation is available, emit 'Overreach: untested — observation unavailable' "
        "rather than either verdict — untested is never 'not found'. Witness every assertion (Gate 2). Enumerate the assertions: emit one 'Assertion: <verbatim assertion text>' line "
        "for each assertion of the established guard, quoted verbatim from the guard's result record. The verbatim lines are the observable; no count or completeness sentinel is used, "
        "and a self-declared count does not satisfy it. Every enumerated 'Assertion:' line must then receive its own 'Failure: assertion \"<verbatim assertion>\" — \"<verbatim failure "
        ...
        "appear verbatim in a cited qualifying observation record is not evidence and does not witness A. If the artifact already passes A, construct a temporary modification that "
        "makes A alone fail while every other assertion still passes, apply the guard, emit the Failure from that result record, then restore; a modification that fails every "
        "assertion at once discriminates none. Emit 'Failure: assertion \"<verbatim assertion>\" — <A-fail result> / <A-pass result>' quoting both observed results from their result "
        "records; a 'witness:' line, if added, only projects those two results and is not itself evidence. A structural assertion — one whose property has the artifact's own text or "
        "structure as its primary subject, so no behavioral failing application exists — is witnessed instead by 'Unobservable: assertion \"<verbatim assertion>\" — structural'; "
        "Unobservable is never satisfied by having searched for a failing application and found none. An assertion that is behavioral yet no admissible modification can make fail is a "
        "guard defect: strengthen the guard and repeat. A guard is a blind spot whenever an admissible state exists in which its pass-or-fail outcome is not equivalent to whether the "
        "property holds — check three: (a) proxy or non-exercising guard checking a surrogate; (b) durability — a guard gone at the moment it would need to fire; (c) ephemeral guard "
        "not committed to a location that re-applies. Verdict-follows-observation governs both gates: when the observation capability is available, every 'Overreach:' verdict and "
        "every 'Failure:' observation is valid only when it immediately follows a result record that mechanically produces it; a verdict emitted from description or analysis alone "
        "does not satisfy this token. A structural Unobservable assertion is exempt, having no application to anchor to. 'Coverage: complete' is valid only when every enumerated "
        "assertion has either a 'Failure:' line backed by its A-fail/A-pass application pair or an 'Unobservable: … — structural' line, and no 'Overreach: found' remains open; an "
        "'Unwitnessed: … — observation unavailable' assertion leaves coverage incomplete and does not satisfy this token. When qualifying observation is unavailable for one or more "
        "behavioral assertions, the honest terminal state is 'Coverage: incomplete — observation unavailable', not a claimed completion; the protocol may construct guards, "
