...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the pre-canonicalization subject-scope check must already have passed before Step 1 is entered. A property is atomic when the recursive decomposition procedure reaches Step 4 "
        "and no valid split exists. Step 1 — Canonicalize the property. Emit: '§ formality check: property [N]: \"<verbatim definition>\" — [formal | informal: <reason>]'. If informal, "
        ...
        "constructions; each accepted sub-property corresponds to a distinct constraint on the admissible construction under the chosen interpretation — a candidate split is not valid "
        "merely because it decomposes the mathematical or definitional structure of a predicate; and neither accepted sub-property constrains an internal mechanism, architecture, "
        "algorithm, or representation as the primary subject of the constraint, unless that mechanism is explicitly required by the chosen interpretation. A sub-property constrains an "
        "internal subject when its canonical definition makes an internal mechanism, architecture, algorithm, or representation the primary subject of the constraint — this "
        "classification is independent of whether that subject happens to be externally observable. A sub-property is externally observable when its truth value can be determined "
