...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "derived ladder that omits any of these rungs for a software behavioral change task must cite which principle makes that rung unnecessary. A structural failure (import error, "
        "compilation error, missing module) at the validation-run-observation rung is not evidence the criterion is unmet — it is a gap in test infrastructure; the "
        "executable-implementation artifact for a structural failure resolves only the structural gap (minimum stub sufficient to allow the suite to run); the criterion has not been "
        "exercised until individual test assertions fail in a subsequent cycle. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label "
        "has not been output has not begun. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend. Sentinel formats — ✅ Ground entered — "
        "prose rung begins [gate: first content emitted in this response; no artifact, code, prose, or reasoning may precede this sentinel — preceding content voids the session]; ✅ "
        "Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in current response; rung table precedes this sentinel]; 🔴 Execution observed: "
        "[verbatim tool output — triple-backtick delimited, complete, nothing omitted] [gate: tool call made in the current response immediately before this sentinel; verbatim output "
        "in triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed with non-empty failing output precedes this token in current response; gap "
        "text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: exec_observed + gap at the validation-run-observation rung in current "
        "cycle; criterion identical to prior cycle criterion for this thread]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed] [gate: exec_observed + "
        "gap at the validation-run-observation rung in current cycle; valid only at the executable-implementation rung]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly "
        "one criterion in this rung's artifact; criterion contains no conjunction; formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation "
        "artifact V complete [gate: test file written via tool call in current response; pre-existence check tool call result present]; ✅ Thread N complete [gate: exec_observed at the "
        "observed-running-behavior rung directly demonstrating criterion in current cycle; passing test suite run after that tool call; valid only at the observed-running-behavior "
        "rung]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; Carry-forward: [list which original failures "
        "cover which current tests] [gate: prior failure name quotable verbatim from a prior exec_observed sentinel]; ✅ I-formation complete [gate: observation of current state "
        "complete before manifest]; ✅ Formal notation R2 audit complete — N/N criteria encoded [gate: every criterion encoded in notation; audit section named and separate].",
