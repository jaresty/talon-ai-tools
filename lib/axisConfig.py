...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "upward return trigger), upward gap scan (after completing a rung, if gap remains unresolved, scan upward to find the nearest rung above that still has a gap to close; return "
        "to that rung rather than restarting from the top; continue descending from that rung to close the remaining gap), observation loop corollary (if opening observation shows no "
        "gap exists, emit Ground complete immediately — no ladder descent needed), upward return conditions (what triggers return, what does NOT trigger return and why excluded), "
        ...
        "derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; Protocol derived must be emitted AFTER deriving all protocol "
        "rules from P1-P6 — omitting derivation or emitting before derivation is complete voids the session; Ladder derived must be emitted AFTER producing the rung table — emitting "
        "before rung table is complete voids the session; No ladder descent (prose, criteria, etc.) is permitted before Protocol derived and Ladder derived sentinels — these are gates "
        "on all subsequent work; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: "
        "a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal "
        "notation → validation → assertion → implementation — each rung must be completed in this order; the assertion rung verifies each individual assertion red → green; emit the "
