...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "following recursive decomposition procedure to every retained property. Initially, the retained properties are the property [N] entries in the Ground properties block. Every "
        "property that can enter the retained set — including each initial property [N], each property added during the completeness phase, and each property produced by a merge — must "
        "be observable under the chosen interpretation after canonicalization, unless the chosen interpretation explicitly requires an internal mechanism, architecture, algorithm, or "
        "representation as its subject; a property that constrains an internal subject without that explicit requirement is out of scope and must not appear in the retained set. "
        "Observability classification is applied to the canonical definition produced by Step 1, not to the provisional surface form. A property is atomic when the recursive "
        'decomposition procedure reaches Step 4 and no valid split exists. Step 1 — Canonicalize the property. Emit: \'§ formality check: property [N]: "<verbatim definition>" — '
        "[formal | informal: <reason>]'. If informal, rewrite as a typed mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, and emit '§ "
        'rewritten: property [N]: "<new definition>"\', then repeat the formality check on the rewritten definition. Continue until the formality check concludes formal. The definition '
        "confirmed formal in the final check is the canonical definition for this property — no subsequent step may quote a definition not yet confirmed formal. After canonicalization, "
        "verify that the canonical definition is observable under the chosen interpretation. If the canonical definition constrains an internal mechanism, architecture, algorithm, or "
        "representation without that subject being explicitly required by the chosen interpretation, reject the property as out of scope and do not retain it. Step 2 — Introduce "
        "provisional sub-properties. Introduce provisional candidate sub-properties whose conjunction is proposed to represent the current property: 'property [Na]: <definition>' and "
        "'property [Nb]: <definition>'. Apply the canonicalization procedure (Step 1) to each provisional sub-property before proceeding. The resulting canonical definitions replace "
        "the provisional definitions for all subsequent split evaluation. The labels [Na] and [Nb] continue to identify the resulting canonical definitions after replacement. Split "
        "evidence is attached to the canonical definitions of the current property and its provisional sub-properties, and to the falsifying constructions emitted in the split test, at "
        "the moment the split test is emitted. Any modification to any of those definitions or constructions after the split test is emitted voids that evidence; the split evidence "
        "must then be regenerated against the current canonical state before the split may be accepted. Step 3 — Evaluate split validity. Before evaluating the split, verify that the "
        "chosen interpretation determines an accessible-observation boundary: the set of outputs, behaviors, states, and interactions that the party responsible for determining whether "
        "the request was satisfied can access without inspecting the implementation. If the chosen interpretation does not determine that boundary, emit 'Interpretation refinement "
        "required: <what the boundary must specify>' and emit or refine the Interpretation: line to supply it. An interpretation refinement voids the entire current retained-property "
        "tree, all previously derived properties, and all split evidence, and restarts property derivation and recursive decomposition from the request constraints under the refined "
        "interpretation; any previously emitted split test is void and does not constitute a completed split test for the current canonical state. After the accessible-observation "
        "boundary has been established and the current property, its provisional sub-properties, and their split evidence have been regenerated under the current interpretation, emit "
        'exactly one non-void split test for the current canonical state: \'§ split test: property [Na]: "<verbatim canonical definition>" — falsified by <construction>; property [Nb]: '
        '"<verbatim canonical definition>" — falsified by <construction>; these falsifiers are independent.\' The quoted definitions must match the immediately preceding canonical '
        "provisional definitions verbatim. A split may be accepted only if its split test, quoted definitions, falsifying constructions, and interpretation are identical to the "
        "non-void state against which the split test was emitted. A split succeeds only if: the conjunction of the accepted sub-properties has identical coverage to the original "
        "property; each quoted falsifier violates exactly its quoted property while satisfying the other; both falsifiers are admissible constructions within the chosen interpretation; "
        "and neither accepted sub-property constrains an internal mechanism, architecture, algorithm, or representation as the primary subject of the constraint, unless that mechanism "
        "is explicitly required by the chosen interpretation. A sub-property constrains an internal subject when its canonical definition makes an internal mechanism, architecture, "
        ...
        "inaccessible. Step 4 — Recurse or mark atomic. If the split succeeds, replace the original property with the accepted canonical sub-properties and apply this same recursive "
        "decomposition procedure (Steps 1–4) independently to each accepted sub-property. If no valid split exists, emit: '§ split test: property [N]: \"<verbatim canonical "
        ...
        "verbatim>\" / <P1 expression> / <P2 expression> / ...' — the request constraints must be quoted verbatim from the chosen interpretation; every retained atomic property "
        ...
        "redundant and must be removed or merged, provided doing so does not reduce coverage of the explicit request constraints. Any property produced by a merge is a new property and "
        "must enter the recursive decomposition procedure (Steps 1–4) as a newly derived property before being added to the retained set. If a merge or removal changes the retained "
        "property set, repeat the atomicity, completeness, and observational-independence procedures. The Ground properties block is complete only when all retained properties "
