...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "provenance; derivation is unbounded: add dimensions, challenge each against the reader-uncertainty test, and revise until every dimension is independently falsifiable; when "
        "every dimension carries an '[observable:]' tag, write '§2 dimensions closed'; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 "
        "dimensions closed' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading "
        "names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; (4) verify coverage: '## Completion check' must not appear before "
        "'## Enforcement sequence' in the transcript and must not appear before '§ implementation permitted' in the transcript; each covered dimension cites a verbatim string that "
        "appears either as a substring of a tool-result block produced after '§ implementation permitted' and visible above '## Completion check', or, when no tool-result blocks are "
        "present after '§ implementation permitted', as a substring of the response text produced in this turn; when every covered dimension cites such a substring, write '§4 coverage "
        "verified'; '## Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## Completion check' does "
        "not satisfy any coverage requirement and does not extend the governed dimension set; a turn containing a satisfied 'Gate condition:' block must contain the governed tool call "
        "in the same turn — prose appearing between the 'Gate condition:' block and the governed tool call does not satisfy this requirement; every assistant turn after '§ "
        "implementation permitted [N]' has appeared in the transcript that does not contain a line beginning with '§ implementation permitted [' must contain '## Completion check' — a "
        "turn that does not contain a line beginning with '§ implementation permitted [' and does not contain '## Completion check' does not satisfy ground; '## Completion check' is "
        "satisfied only when the literal string '§4 coverage verified' appears within it — a '## Completion check' block that does not contain '§4 coverage verified' does not satisfy "
        "this requirement and the governed turn must continue to the next derived step rather than ending — the first non-blank line after the '## Completion check' heading must be "
        "'Gate condition:'; a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume phrase requirement; when a turn is non-exempt, its final "
        'non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates still apply" to proceed under the same protocol. (5) '
        "enumerate escape paths: '## Path enumeration' must not appear before '## Enforcement sequence' in the transcript; enumerate every path by which §1–§4 headings could be present "
        "without the governing goal identified, dimensions derived, or dimensions covered; for each path, name the literal string whose presence closes it; write '§5 enumeration "
        "complete' when no further open path remains; any referent named in the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched "
        "before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted [N]' where N is the 1-based index of the file-modifying "
        "tool call this sentinel governs must appear in the transcript after '§5 enumeration complete'; no file-modifying tool call may appear before the first '§ implementation "
        "permitted [N]' sentinel; the literal string '§ implementation permitted [N]' with N matching the 1-based index of the governed tool call must appear immediately before the "
        "`(i)` line of the five-line block in the assistant text block immediately before each file-modifying tool call, in the same response turn as that tool call — a user message "
        "appearing between '§ implementation permitted [N]' and the tool call does not satisfy this requirement; a file-modifying tool call not immediately preceded by assistant text "
        "whose final non-blank content ends with the `(iv)` line of the five-line block does not satisfy this requirement. Quoted-span exclusion: an occurrence of any named sentinel or "
