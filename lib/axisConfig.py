...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation internals, the criterion is structural and must be rewritten before this rung closes; this is a content gate, not a self-check. Formal notation encodes only the "
        "criteria declared at the criteria rung — no additional invariants, no anticipated cases; it produces only type signatures, interfaces, pseudocode, and structural invariants — "
        "the test of valid formal notation is that it cannot be directly compiled or executed without modification; any construct that compiles or runs as-is is implementation code and "
        "voids the rung; formal notation describes what must be true, not how to implement it — permitted: type signatures without bodies, interface shapes, structural invariants, "
