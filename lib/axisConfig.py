...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "search bar filters results’ are two criteria, not one). Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated "
        "cases. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ Validation artifact V complete; the artifact must "
        "be invocable by an automated tool; before writing the test, re-read the formal notation and assert each structural constraint it encodes; each test function contains exactly "
        "one assertion — one assertion per test function; a pre-existing test that happens to pass does not satisfy this rung — add to an existing test file covering the same module if "
        "one exists, otherwise create a new file; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already "
        "failing — V-complete may not be emitted without this tool-executed result in the transcript; the validation artifact must reside within the project’s version-controlled file "
        "tree; if the artifact asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, "
        "perturb the implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten. ✅ Validation artifact V complete must be "
        ...
        "post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared in the current cycle showing the test logic failing — a harness error (import "
        "failure, syntax error, missing file) is not a red run; the test must have been loadable, its assertions must have run, and they must have failed. At the validation run "
        ...
        "cycle; after each edit re-run the validation artifact before any further edit. Upon writing the observed running behavior label, re-emit the criterion for this cycle, then "
        ...
        "specific behavior named in the criterion; it must be what a non-technical observer of the running system would see — not the test suite, not process state (server started, "
        "build succeeded, exit code 0) unless the criterion is specifically about process state; if the output does not directly demonstrate the criterion, emit the gap and return "
        "upward; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared behavior; implementation "
        "edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written in the "
        "current cycle and a 🔴 Execution observed: sentinel appears immediately after it and the OBS output directly demonstrates each criterion declared in this thread; once ✅ Thread "
        "N complete is emitted no further output in that cycle is valid. For each sentence in the prose that contains a behavioral predicate (fetches, requires, displays, filters, "
        "renders, validates, authenticates, loads, saves, or any similar action verb), check that a completed gap cycle exists whose criterion directly names that behavior — implicit "
        "or incidental coverage does not satisfy this check; if any behavioral sentence has no directly-named cycle, emit a new 🔴 Gap: for it before emitting ✅ Thread N complete. "
        "Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. "
        "When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every descent through executable "
        ...
        "a valid emission. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order; the final report copies "
        "artifacts already emitted in the thread rungs — no new behavioral claims, no coverage summaries, no suggestions may appear in the final report; then reconcile any documents "
        "(README, specs, ADRs) the implementation affects — this reconciliation is part of the final report, not a separate thread. Seven rungs in order: prose → criteria → formal "
