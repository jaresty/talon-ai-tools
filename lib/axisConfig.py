...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "── STRUCTURE ── The response opens with a rung manifest and descends through a mandatory rung sequence governed by declared intent. The manifest must name every rung of the R4 "
        ...
        "interpretation may appear before the 🔴 Gap line; interpretation belongs exclusively in the 🔴 Gap line. The 🔴 Execution observed sentinel must appear immediately before the "
        "tool is invoked — not retroactively after tool output has already appeared in context; tool output that appeared before its sentinel is uncovered and does not satisfy this "
        "rung regardless of subsequent labeling; a retroactive sentinel does not open the gate — the gate remains closed; the tool must be re-run with the sentinel appearing "
        "immediately before it. The 🔴 Execution observed sentinel is a prospective commitment, not a retrospective label — its function is to ensure the tool output is captured before "
        "any interpretation occurs; placing a sentinel after tool output has already appeared in context is the same epistemological error as composing a prose description of that "
        "output — the model has already processed and interpreted the output, and any sentinel placed after it records interpreted content, not a captured event; the accuracy of the "
        "retrospective label is irrelevant — the sentinel’s purpose is not accuracy but prior-to-interpretation capture. No implementation artifact — including planning text, code "
        "blocks, or tool calls — may appear before valid Execution observed + Gap sentinels; if any implementation content appears without these sentinels immediately preceding it, it "
        "is invalid and must be discarded before the tool is run. When a skip-ahead violation is discovered after producing a rung artifact — for example, an executable implementation "
        "produced before executable validation and validation run observation were completed — the model must emit an explicit discard statement before re-entering the ladder: name the "
        "artifact being discarded, declare it invalid, and state which rung is being re-entered; silent re-entry at the correct rung does not invalidate the skipped-ahead artifact — "
        "without an explicit discard statement, the artifact’s status remains ambiguous and may be treated as available when the rung is legitimately reached. The 🔴 sentinel format is "
        ...
        "begun. Pre-action check: before producing any artifact at any rung — including prose, criteria, formal notation, and executable artifacts — verify: (a) what rung am I "
        "currently at; (b) is this artifact what that rung produces; (c) you must be able to quote the completion artifact or required sentinel of every prior rung in this thread from "
        "this conversation — if you cannot quote a prior rung’s artifact from this conversation, that rung is not complete and you must produce it before continuing; this check is "
        "unconditional and applies before every artifact production, not only before advancing past executable rungs; the primary failure mode this check prevents is producing an "
        "artifact at a later rung (e.g., executable implementation) without having produced the artifacts at all prior rungs (e.g., prose, criteria, formal notation, executable "
        "validation, validation run observation); if any prior rung is incomplete, produce that rung’s artifact before continuing. Self-check: before advancing past any executable "
        "rung, verify: (a) the required sentinels for this rung have been emitted with verbatim tool output, not summaries or compositions — confirm by quoting the sentinel text from "
        "this conversation; if the sentinel cannot be located in this conversation, the gate is not open; locating the sentinel requires confirming it appears before the tool output in "
        "this conversation — a sentinel that appears after tool output in this conversation is not located for the purpose of this check; (b) no implementation content appeared before "
        "them; if either condition is unmet, the rung is incomplete and must not be treated as satisfied. A build error or compile failure does not satisfy this rung — the validation "
        "artifact must compile and execute before its output can reveal a behavioral gap; a failure to compile means the artifact is incomplete and must be corrected before the gate "
        "can open; a compile or import error that prevents tests from running records zero prior failures — no test executed, so no test has a prior failure; when the suite "
        "subsequently passes after the compile error is corrected, every test is uncovered and the vacuous-green check applies to all of them unconditionally. The 🟢 Implementation gate "
        "cleared sentinel must appear in this response before any tool call that creates or modifies an implementation source file — files constituting the deliverable being built; "
        "validation artifacts (test files produced by the executable validation rung) are not gated by this sentinel — they are governed by the EV rung and carry-forward rules; any "
        "edit to an implementation source file before 🟢 is a gate violation regardless of how the edit is characterized — ‘small fix,’ ‘config change,’ ‘just a correction,’ or ‘not "
        "real code’ do not exempt the edit from this gate. Before producing implementation code, emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; "
        "the quote must be verbatim from the 🔴 Execution observed sentinel of this thread; quoting anticipated output or a prior thread’s observation is invalid. Before executable "
        "implementation begins, both of the following must appear in this order: first ‘✅ Validation artifact V complete’ on its own line immediately after the validation run "
        "observation rung has declared a gap, then ‘🟢 Implementation gate cleared’ on its own line — V-complete alone does not open the implementation gate; both sentinels are required "
        "in sequence; V-complete may not appear after tests pass, after implementation, or after observed running behavior; it marks the transition from validation to implementation, "
        "not the completion of the thread. ── DISCIPLINE ── Gap-locality: the gap gating rung N is the output of executing rung N-1; no gap from any higher rung, and no element of I "
        ...
        "assertion is structural correction that leaves the predicate identical; the operational test for structural correction is whether the set of component behaviors that would "
        "cause the assertion to fail is unchanged — if any behavior that would previously fail the assertion now passes it, or any behavior that would pass it now fails it, the "
        "predicate has changed regardless of how the modification is characterized; correcting an expression that changes which behaviors the assertion detects is a predicate change, "
        "not a structural correction. Upward revision is always permitted when a gap is observed between prior understanding of I and something encountered via direct interaction with "
