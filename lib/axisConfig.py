...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle; the only protocol-defined stop is at the "
        "validation run observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the highest "
        ...
        "for the current gap, including after an upward return. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upward returns follow "
        "the failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; "
        ...
        "Validation artifact V complete must be emitted for the current cycle’s gap — an artifact written in a prior cycle does not satisfy this gate. Reconciliation gate: at every "
        "upward rung transition, reconcile all artifacts against I and update any pre-existing document that records I (ADR, README, spec) before redescending. Before producing the "
        "final report, invoke the delivered system end-to-end via a tool call and emit 🔴 Execution observed: showing the primary behavioral intent is satisfied — this is the "
        "post-delivery integration observation; if it fails, open a new gap cycle before producing the final report. After emitting ✅ Manifest exhausted, produce a final report "
        "containing prose, criteria, and formal notation for each thread in order — before writing each section, locate the artifact in the prior transcript; if it cannot be found "
        "verbatim in the transcript, it may not appear in the final report; no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the "
        "implementation affects. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed "
