...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "manifest-complete sentinel — no rung work, planning text, or content of any kind may appear before it; when I cannot be declared from context alone, observation of existing "
        "code or running behavior sufficient to establish I is required before the manifest — this is I-formation, not rung work; skipping I-formation when context is insufficient is "
        "an error, not a permitted shortcut; exploration beyond what is needed to declare I belongs as the first rung of the manifest, not pre-manifest. Foundational constraint: a "
        "symbol is not the state it represents — a rung label during execution must be emitted at every rung transition; absence of a rung label at a transition is a traversal "
        "violation, not a stylistic omission; the label marks the point where the artifact begins — it is not a section heading for planning, description, or exploration about the "
        "rung; no content other than the artifact itself may appear between a rung label and the artifact it precedes. Eagerness to implement is the primary failure mode — an "
        ...
        "descent continues. When the lowest V is complete, output ‘✅ Validation artifact V complete’ on its own line before producing O — this phrase may only appear after the "
        "executable validation rung has been both produced and invoked and the validation run observation rung has declared a gap. Gap-locality: the gap gating rung N is the output of "
        ...
        "addresses the declared gap and nothing more. Implementing beyond the declared gap is a violation — not a benefit. Upward revision is always permitted when a gap is observed "
        "between prior understanding of I and something encountered via direct interaction with reality, code, or a stakeholder. Upward revision must be signposted with: what was "
        "observed, which rung is being revised, and why. It is never permitted to change I without first observing a gap in V that derived it. Changing I requires revising every "
        "artifact derived from it to restore chain consistency before descent continues. Upward correction is mandatory when a rung failure reveals that a higher rung is deficient — a "
        "failure at rung N is evidence that rung N-1 (or higher) did not correctly derive or constrain it; before correcting any higher rung, emit what was observed at the current "
        "rung, which higher rung is being corrected, and why — a correction without this observation record is invalid and indistinguishable from enrichment; find the highest broken "
        "rung, correct it, and rederive every artifact below it before redescending; this is correction, not enrichment — the corrected rung must still be faithful to I; if the broken "
        "rung is I itself, surface as a revision signal and stop the current thread. Intent precedes its representations. Every artifact that documents the governing intent of this "
        "invocation — whether produced in this invocation or pre-existing in the codebase — must be consistent with I before the invocation closes. If reconciliation is feasible, "
