...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "line when the request admits multiple readings does not satisfy this protocol. Every property must be derivable from the explicit constraints of the chosen interpretation. A "
        "property that introduces a constraint not implied by those constraints is out of scope and must not appear in the block. Each property is written on its own line beginning "
        "'property [N]:' followed by a formal definition having a single unambiguous denotation. The definition must be expressed as one of: a typed mathematical expression, a logical "
        "predicate (for example using ∀, ∃, ⇒, ⇔), or a formally defined relation. Two independent readers applying the same interpretation must reach the same conclusion about whether "
        "a construction satisfies the property. Apply the following recursive decomposition procedure to every retained property. Initially, the retained properties are the property "
        "[N] entries in the Ground properties block. A property is atomic when the recursive decomposition procedure reaches Step 4 and no valid split exists. Step 1 — Canonicalize the "
        "property. Emit: '§ formality check: property [N]: \"<verbatim definition>\" — [formal | informal: <reason>]'. If informal, rewrite as a typed mathematical expression, a "
        "logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, and emit '§ rewritten: property [N]: \"<new definition>\"', then repeat the formality check on the rewritten "
        "definition. Continue until the formality check concludes formal. The definition confirmed formal in the final check is the canonical definition for this property — no "
        "subsequent step may quote a definition not yet confirmed formal. Step 2 — Introduce provisional sub-properties. Introduce provisional candidate sub-properties whose "
        ...
        "the Interpretation: line to supply it. An interpretation refinement restarts property derivation and recursive decomposition from the request constraints under the refined "
        "interpretation; all previously derived properties and split evidence are void; any previously emitted split test derived under the prior interpretation is void and does not "
        "constitute a completed split test for the current canonical state. After the accessible-observation boundary has been established and all affected definitions have been "
        'regenerated under the current interpretation, emit exactly one split test: \'§ split test: property [Na]: "<verbatim canonical definition>" — falsified by <construction>; '
        'property [Nb]: "<verbatim canonical definition>" — falsified by <construction>; these falsifiers are independent.\' The quoted definitions must match the immediately preceding '
        "canonical provisional definitions verbatim. A split succeeds only if: the conjunction of the accepted sub-properties has identical coverage to the original property; each "
        "quoted falsifier violates exactly its quoted property while satisfying the other; both falsifiers are admissible constructions within the chosen interpretation; and neither "
        "accepted sub-property constrains an internal mechanism, architecture, algorithm, or representation as the primary subject of the constraint, unless that mechanism is "
        "explicitly required by the chosen interpretation. For this condition, a sub-property constrains an internal subject when determining whether the property is satisfied or "
        "falsified requires observing state, structure, or mechanism not accessible under the chosen interpretation's accessible-observation boundary. A sub-property is observable "
        "under the interpretation when its truth value can be determined solely from the outputs, behaviors, states, and interactions that the interpretation makes accessible to the "
        "evaluating party without inspecting the implementation. Step 4 — Recurse or mark atomic. If the split succeeds, replace the original property with the accepted canonical "
        "sub-properties and apply this same recursive decomposition procedure (Steps 1–4) independently to each accepted sub-property. If no valid split exists, emit: '§ split test: "
        'property [N]: "<verbatim canonical definition>" — atomic, no valid split: <reason>.\' The quoted definition must match the retained canonical property verbatim. Apply this '
        "procedure to every retained property until all have reached Step 4 with no valid split. After all retained properties are atomic, emit a completeness check: '§ completeness "
        'check: "<request constraints verbatim>" / <P1 expression> / <P2 expression> / ...\' — the request constraints must be quoted verbatim from the chosen interpretation; every '
        ...
        "property expression does not satisfy this requirement. Immediately after the completeness check, attempt to identify one explicit request constraint that is not represented by "
        "any retained property. If such a constraint exists, emit '§ properties complete? no' — add one new property covering that explicit constraint, perform the required split "
        "testing, and repeat the completeness check. Only when every explicit request constraint is represented by at least one retained property may the response emit '§ properties "
        "complete? yes' — a '§ properties complete? yes' line without a preceding completeness check does not satisfy this protocol. After completeness has been established, verify "
        "observational independence. A retained property is observationally independent if there exists at least one admissible construction that violates that property while "
        ...
        "doing so does not reduce coverage of the explicit request constraints. If a merge or removal changes the retained property set, repeat the atomicity, completeness, and "
