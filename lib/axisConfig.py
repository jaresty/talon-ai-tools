...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "set of outputs, behaviors, states, and interactions that a party evaluating satisfaction of the request can access without inspecting the implementation. If the chosen "
        "interpretation does not determine that boundary, emit 'Interpretation refinement required: <what the boundary must specify>' and refine the Interpretation: line to supply it "
        "before proceeding. An interpretation refinement invalidates all properties and split evidence derived under the prior interpretation; those properties and evidence must be "
        "regenerated under the refined interpretation before any split acceptance continues. Immediately after the canonical provisional definitions, emit exactly one split test: '§ "
        'split test: property [Na]: "<verbatim canonical definition>" — falsified by <construction>; property [Nb]: "<verbatim canonical definition>" — falsified by <construction>; '
        "these falsifiers are independent.' The quoted definitions must match the immediately preceding canonical provisional definitions verbatim. A split succeeds only if: the "
        "conjunction of the accepted sub-properties has identical coverage to the original property; each quoted falsifier violates exactly its quoted property while satisfying the "
        ...
        "sub-property constrains an internal subject when determining whether the property is satisfied or falsified requires observing state, structure, or mechanism not accessible to "
        "a party evaluating the result under the chosen interpretation's accessible-observation boundary. A sub-property is observable under the interpretation when its truth value can "
        "be determined solely from the outputs, behaviors, states, and interactions that the interpretation makes accessible to an evaluating party without inspecting the "
        ...
        "coverage of the explicit request constraints. If a merge or removal changes the retained property set, repeat the atomicity and completeness procedures. The Ground properties "
        "block is complete only when all retained properties simultaneously satisfy: they are derived from the chosen interpretation; they are atomic; they collectively cover every "
        "explicit request constraint; they are observationally independent; they introduce no out-of-scope constraints. When all conditions are met, emit '§ ground complete' as the "
        "final line of the Ground properties block — this sentinel may not appear before '§ properties complete? yes' in the transcript. Any test, implementation step, tool call, or "
        "task reasoning that appears before '§ ground complete' does not satisfy this protocol; if task reasoning begins and '§ ground complete' has not yet appeared, emit the Ground "
