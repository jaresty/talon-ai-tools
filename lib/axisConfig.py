...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "same gap; the completion sentinel MUST explicitly declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied]; If gap IS closed: emit ✅ "
        ...
        "are NOT yet satisfied]; if gap IS closed: emit ✅ Thread N complete. If gap IS closed: emit ✅ Thread N complete — all gaps closed. Tests drive the behavior needed. Emit "
        "implementation rung label first, then implement, then emit completion sentinel. ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. "
        "Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the "
        "artifact is produced and gate is satisfied; Every rung must emit its label BEFORE producing its artifact and emit its completion sentinel AFTER. No implementation is permitted "
        "before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed sentinel must include the verbatim failing assertion text; "
        "Evidence before claims: for every claim about code being correct, there must be tool execution output proving it; describing what you would do is not evidence; ground complete "
        ...
        "implementation — the assertion rung is REQUIRED in this order; the assertion rung verifies each individual assertion red → green; emit the rung label before producing its "
