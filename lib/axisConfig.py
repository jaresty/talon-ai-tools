...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "of the immediately preceding rung's output does not satisfy this requirement. The behavioral observation — the executed FAIL result showing the system's actual behavior "
        "against the absent governed behavior — is the rung where reality enters the chain; every rung before it is agent-authored artifact, and every rung after it derives from what "
        "that observation actually produced, not from what the agent wrote before it. Memory is not evidence — what a model recalls about a prior step carries the same confabulation "
        "risk as any other model output, so only what is visible in the transcript counts as having happened. A shorter ladder is not more efficient — each collapsed step is ambiguity "
        "that was not closed, and ambiguity that was not closed remains available as an escape route. §0 is a starting gate, not a bypass — description, analysis, and planning tasks "
        "run §1–§5 in full after writing '§0: [scenario]'; the declaration opens the ladder, it does not close it; every rung sentinel is satisfied by the literal string appearing in "
        "the response text when no executable artifact is present — no tool call is required or expected to satisfy any rung when §0 observed is absent. Ladder depth: a ladder is "
        ...
        "and ask whether that phrase could be satisfied by two non-equivalent outputs given the governing goal as context — the test is decidable per rung because the governing goal "
        "provides a fixed reference that bounds what counts as non-equivalent. If such a phrase exists, that phrase names the subject of the next rung. The ladder is complete only when "
        "the ambiguity test finds no such phrase. A ladder that reaches implementation before passing the ambiguity test has collapsed a step and must be re-derived. Rung-type "
        "homogeneity: rung types must stay homogeneous across the ladder — all rungs before the behavioral observation must be successive precision-narrowings of the same governed "
        "behavior; introducing a new kind of artifact, deliverable, preflight check, or task category as a rung is not a narrowing and does not satisfy the ladder. A ladder that begins "
        "with intent → criteria → specification and then inserts a preflight check, implementation step, or presenter-support deliverable as a rung has switched dimensions — those "
        "items belong outside the ladder as governed actions following it, not inside it as rungs. Intent cannot be read directly from a request — every governing goal must be derived "
        "by inferring what would satisfy the underlying need from the scenario's subject matter; derivation means selecting the most precise formulation of what the scenario implies, "
        "not adding requirements absent from it; a goal that introduces a new constraint not implied by the scenario has overreached, and a goal that merely restates the request has "
        "not derived anything. The response applies a meta-process discipline before any implementation action, deriving and enforcing its own correctness conditions as "
        "transcript-inspectable strings. A rung is satisfied when and only when a transcript-visible event matching its definition appears — when §0 observed is present, rungs require "
        "tool-executed events and the behavioral observation is where reality enters the chain; when §0 observed is absent, no executable artifact exists and rungs are satisfied by the "
        "literal sentinel string appearing in the response text — this is an acknowledged weaker form of evidence that enables the protocol to run on non-executable subjects "
        "(specifications, contracts, analyses) while preserving the derivation structure; the protocol's preference for execution over description holds, but does not block the "
        "protocol when execution is unavailable; inference, prediction, and prior knowledge do not satisfy rung gates regardless of accuracy. All compliance headings must be markdown "
        "'## ' headings — a line not beginning with '## ' does not satisfy any heading requirement. Invocation scope: a sentinel written in a prior ground invocation does not satisfy "
        "any rung requirement for the current invocation — a sentinel satisfies its rung only when it appears in the transcript after the most recent '=== TOKENS' section. Each "
        ...
        "Derivation phase: (0) after 'Derived stance complete.', immediately write '§0: [scenario]' where [scenario] names the subject from the conversation — this declaration "
        "satisfies §0 and opens the ladder; if an artifact that executes the subject system and returns its live output appears verbatim in the conversation above, invoke it as a tool "
        "call immediately after writing '§0: [scenario]'; after that tool-result block appears and reflects live system execution, immediately write '§0 observed' — a tool-result block "
