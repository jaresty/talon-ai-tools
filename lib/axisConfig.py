...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "🔵 Ladder derived — and MUST be emitted AFTER producing the rung table — emitting before rung table is complete voids the session; No ladder descent (prose, criteria, etc.) is "
        "permitted before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent work; Rung completion MUST be emitted as: ✅ [Rung name] complete — "
        "not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; No implementation is permitted before validation shows red assertions — "
        "the exec_observed sentinel must appear with verbatim test output showing failure BEFORE any implementation code is written; the gap is the red assertion, the solution is "
        "making it green; Evidence before claims: for every claim about code being correct, there must be tool execution output proving it; describing what you would do is not "
        "evidence; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: a rung is "
        "valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → "
        "validation → assertion → implementation — each rung must be completed in this order; the assertion rung verifies each individual assertion red → green; emit the rung label "
