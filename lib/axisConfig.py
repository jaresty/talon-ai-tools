...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "green), each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; Protocol derived must be emitted AFTER deriving "
        "all protocol rules from P1-P6 — omitting derivation or emitting before derivation is complete voids the session; Ladder derived must be emitted AFTER producing the rung table "
        "— emitting before rung table is complete voids the session; No ladder descent (prose, criteria, etc.) is permitted before Protocol derived and Ladder derived sentinels — these "
        "are gates on all subsequent work; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung "
