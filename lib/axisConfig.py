...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "session — prefer actual isolation over in-transcript constraint enforcement. Derivation phase: (0) '§0: [scenario]' is the opening sentinel of the derivation, where [scenario] "
        "names the subject from the conversation — the first sentinel appearing in the response must be '§0:'; a response whose first sentinel is not '§0:' does not satisfy this "
        "requirement; when tools are available in this execution environment, invoke the governing test artifact as a tool call immediately after writing '§0: [scenario]' — '§0 "
        ...
        "requirement; (1) derive the governing goal: '## Governing goal:' is the first '## ' heading in the transcript after '§0:' — a '## ' heading other than '## Governing goal:' "
        "appearing first after '§0:' does not satisfy this requirement; if '§0 observed' is present, '[text]' must appear verbatim as a substring of the §0 tool-result block above; if "
        "'§0 observed' is absent, '[text]' is derived from the scenario description — what would satisfy the underlying need; a '[text]' value that adds a claim absent from the "
        "scenario does not satisfy §1; immediately below the governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping "
        "substring of '[text]', labeled subject, verb, outcome, or invariant; no field may equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive "
        "words; when every named component of '[text]' is covered, write '§1a check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the labeled fields "
        "written above in this rung — a '§1a check:' line whose quoted phrase does not appear verbatim in the labeled fields above it does not satisfy this requirement; after a valid "
        "'§1a check:' line has appeared, immediately write '§1a decomposed'; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: "
        "[alternative means]' — when '§0 observed' is present, alternative means must not duplicate approaches already listed in §0; if only one concrete alternative can be named, "
        "write '[hypothetical]: [speculated alternative]' before closing §1; if two non-subsuming goal candidates exist, write both and mark one '[selected]' — the selected goal is the "
        "one whose means-test names a more concrete alternative; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 check: \"[quoted phrase]\"' where [quoted "
        "phrase] is a verbatim substring of one of the alternative means lines written above in this rung — a '§1 check:' line whose quoted phrase does not appear verbatim in the "
        "means-test above it does not satisfy this requirement; after a valid '§1 check:' line has appeared, immediately write '§1 goal derived'; immediately below '§1 goal derived', "
        "derive dimension candidates: for each alternative means named in the means-test, write one candidate beginning with '- Candidate:' — a response property that would be absent "
        "in a response achieving '[text]' by a different means; when one candidate exists per alternative means, write '§1b check: \"[quoted phrase]\"' where [quoted phrase] is a "
        "verbatim substring of one of the candidate lines written above — a '§1b check:' line whose quoted phrase does not appear verbatim in the candidate lines above it does not "
        "satisfy this requirement; then write '§1b candidates'; '§1b candidates' must not appear before a valid '§1b check:' line in the transcript; (2) derive behavioral dimensions: "
        "'## Behavioral dimensions' must not appear before '§1b candidates' in the transcript; derive at least two dimensions; each dimension names a property of the response (not an "
        "artifact, state, or fix description) such that a response could satisfy it while failing at least one other listed dimension; each dimension must trace to one item from the "
        "§1a decomposition list or one item from the §1b candidates list — a dimension that traces only to '[text]' as a whole does not satisfy this requirement; each dimension ends "
        "with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose presence in the response constitutes satisfaction — a valid [observable:] value "
        "must contain at least one space (the value must be ≥2 words); a single-word [observable:] value does not satisfy this requirement; if the response contains no tool-result "
        "blocks above §2, write '[observable: prose]' instead; a dimension that names an entity, a fix outcome, or a UI element is not a valid dimension regardless of provenance; "
        "derivation is unbounded: continue until every dimension carries an '[observable:]' tag naming a literal string or structural pattern whose presence in the response constitutes "
        "satisfaction independently of any other listed dimension; when every dimension carries an '[observable:]' tag, write '§2 check: \"[quoted phrase]\"' where [quoted phrase] is "
        "a verbatim substring of one of the '[observable:]' tag values written above in this rung — a '§2 check:' line whose quoted phrase does not appear verbatim in an "
        "'[observable:]' tag above it does not satisfy this requirement; then write '§2 dimensions closed'; '§2 dimensions closed' must not appear before a valid '§2 check:' line in "
        "the transcript; (2b) derive formalization: after writing '§2 dimensions closed', immediately write '## Formalization' — '## Formalization' must not appear before '§2 "
        "dimensions closed' in the transcript; the ## Formalization section must contain at least one expression matching one of: a typed function signature, a logical predicate (∀/∃ "
        "notation), or a concrete input/output example (f(v) = w or equivalent); immediately after that expression, apply the ambiguity test: write '§ ambiguity test:' followed by one "
        "quoted phrase from the expression and whether it could be satisfied by two non-equivalent outputs; if yes, that phrase names the subject of the next rung and ## Formalization "
        "must not close; when no phrase in the expression admits two non-equivalent outputs, write 'alternative satisfier:' followed by a space and at least one non-whitespace "
        "character that is not '[' — a bracket character as the first non-whitespace character after 'alternative satisfier: ' signals an unfilled template and does not satisfy this "
        "requirement; the text names a concrete thing that the expression would admit but that the governing goal would reject; a formalization that cannot name such a satisfier is not "
        "yet precise enough to close; '§ formalization complete' must not appear before '§ ambiguity test:' in the transcript — a '§ formalization complete' that appears without a "
        "preceding '§ ambiguity test:' does not satisfy this requirement; '§ formalization complete' must not appear before a valid 'alternative satisfier:' line (one whose first "
        "non-whitespace character after 'alternative satisfier: ' is not '[') in the transcript — a '§ formalization complete' with no preceding valid 'alternative satisfier:' line "
        "does not satisfy this requirement; after '§ ambiguity test:' and a valid 'alternative satisfier:' line have appeared, immediately write '§ formalization complete' — '§ "
        "formalization complete' must not appear before both '§ ambiguity test:' and a valid 'alternative satisfier:' line have appeared; (2c) derive formalized properties: after '§ "
        "formalization complete', derive the notation form for this domain: examine the §2 dimensions and the governing goal to identify the mathematical notation that best represents "
        "the governed behaviors — write '§ notation derived: [form]' where [form] is a literal notation template containing at least one bracketed placeholder — a substring of the form "
        "'[' followed by a label and ']' (e.g. 'f([arg]) → [outcome]', '∀[var]: [type], [predicate]([var])', 'check([input]) → PASS | FAIL([reason])')); a '§ notation derived:' line "
        "whose [form] field contains no '[' character does not satisfy this requirement; immediately after '§ notation derived:', write the label line 'Formalized properties:' — this "
        "block must appear before any artifact, implementation action, or '§ implementation permitted' line; decompose each §2 dimension into its formal atomic claims; each claim must "
        "appear on its own line beginning with the literal text 'property [N]:' where N is a 1-based integer — a line beginning with '**property' or any markdown-formatted variant does "
        "not satisfy this requirement; each 'property [N]:' line must contain at least one mathematical expression — any notation in which every term has a fixed denotation independent "
        "of natural-language context (e.g. ∀/∃ predicates, typed function signatures, relational expressions, or interface-shape patterns such as check(G) → PASS | FAIL(reason)) — as a "
        "literal substring; a 'property [N]:' line containing no mathematical expression does not satisfy this requirement; after writing each 'property [N]:' line, immediately write "
        "'§ split test: [quoted sub-expression]' where [quoted sub-expression] is a phrase from the parent expression naming a candidate sub-expression to split on, followed by a "
        "sentence stating whether splitting at that sub-expression yields two independently falsifiable sub-properties — a 'property [Na]:' line that does not have a '§ split test:' "
        "line above it does not satisfy this requirement; if the split test finds a valid split, write one sub-property per branch: 'property [Na]:', 'property [Nb]:', etc., each "
        "containing a formal expression — a sub-property line whose content is prose only does not satisfy this requirement; if the split test finds no valid split (the property is "
        "already atomic), write 'property [Na]: atomic —' followed by a restatement of the parent expression in a notation where every term has an unambiguous denotation — a "
        "restatement that two independent agents could interpret differently does not satisfy this requirement; the conjunction of all sub-property expressions for a given 'property "
        ...
        "phrase requirement; a turn whose final non-blank content line begins with '§ blocked:', '§ awaiting:', or '§ no-next-action:' is exempt from the resume phrase requirement — "
        "the continuation invariant exit string is the terminal constraint for that turn; when a turn is non-exempt, its final non-blank content line, after trimming trailing "
        'whitespace, must equal exactly: Resume: say "Continue autonomously — gates still apply" to proceed under the same protocol. (5) enumerate escape paths: after \'§ enforcement '
        "complete', immediately write '## Path enumeration' — '## Path enumeration' must not appear before '§ enforcement complete' in the transcript; enumerate every path by which "
        "§1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each path, name the literal string whose presence closes "
        "it; when no further open path remains, write '§5 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the escape path closing strings named in "
        "the enumeration above — a '§5 check:' line whose quoted phrase does not appear verbatim in the path enumeration above it does not satisfy this requirement; then write '§5 "
        "enumeration complete'; '§5 enumeration complete' must not appear before a valid '§5 check:' line in the transcript; '§5 enumeration complete' must not appear before '§ "
        "enforcement complete' in the transcript; any referent named in the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched before "
        "§1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted' followed by a space and a decimal integer (e.g. '§ "
        "implementation permitted 1') must appear in the transcript after '§5 enumeration complete'; a sentinel line containing the literal bracket characters '[' or ']' does not "
        "satisfy this requirement — only a decimal integer satisfies the index requirement; the integer must equal the 1-based ordinal count of file-modifying tool calls that will have "
        "appeared through and including the tool call immediately following this sentinel — '§ implementation permitted 1' must precede the first file-modifying tool call, '§ "
        "implementation permitted 2' must precede the second, and so on; a checker verifies this by scanning the transcript for every file-modifying tool call, locating the final "
        "non-blank line of the immediately preceding assistant message block, and confirming it matches '§ implementation permitted N' where N is the call's ordinal position; no "
        "file-modifying tool call may appear before the first such sentinel; the permit sentinel must appear as the final non-blank line of the assistant message block immediately "
        "before the assistant message block containing the file-modifying tool call — a permit sentinel in the same message block as the tool call satisfies this requirement only when "
        "it is the final non-blank line before the tool call within that block; § implementation permitted [N] must appear immediately before the `(i)` line of each file-modifying tool "
        "call — no intervening blank lines, prose, tool-call blocks, or other content may appear between the sentinel and the `(i)` line; no user message appearing between § "
        "implementation permitted [N] and the governed tool call satisfies this requirement — a sentinel written in one turn and a tool call appearing in a later turn after a user "
        "message does not satisfy the same-turn anchor; a blank line, horizontal rule, or any other content between them does not satisfy this requirement; a file-modifying tool call "
        "not immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the five-line block does not satisfy this requirement. Quoted-span "
        ...
        "or ordering requirement. Sentinel formatting: a sentinel line must begin with the literal first character of the sentinel string ('§' or '#'); a sentinel line beginning with "
        "'**' or any other markdown formatting character does not satisfy this requirement. Completion check ordering: the '## Completion check' block must appear after '§5 enumeration "
        "complete' in the transcript; a '## Completion check' block that appears before '§5 enumeration complete' does not satisfy this requirement even if its contents are otherwise "
        "correct; a response that contains '§5 enumeration complete' and does not produce a subsequent '## Completion check' block containing 'Formalized properties:', at least one "
        "'property [N]:' line satisfying all property requirements, '§ properties complete', and '§4 coverage verified' does not satisfy ground. Continuation invariant: when the final "
        "non-blank content line of a turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage verified', or "
        "'§5 enumeration complete', that turn must end with one of: '§ blocked: <text>', '§ awaiting: <text>', or '§ no-next-action: <text>', where '<text>' is non-empty. A turn whose "
