...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "🔴 Execution observed: requires: (1) a preceding tool call in this response, (2) verbatim tool output in a triple-backtick block — complete, nothing omitted, no editorial "
        "content. Any deviation voids the sentinel; fix by re-invoking the tool and re-emitting the block. If no tool call has been made in the current response, 🔴 Execution observed: "
        "is a protocol violation — stop, make the tool call, then write the sentinel with the verbatim output; generated text that resembles tool output does not satisfy this sentinel "
        "under any circumstances. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current gap "
        "before any other content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. Each completion "
        "sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The gap names a specific behavior "
        "currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; if no "
        "currently-false behavior remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists for each thread declared in "
        "✅ Manifest declared — before emitting ✅ Manifest exhausted, locate the N in the ✅ Manifest declared sentinel and count ✅ Thread N complete sentinels — if the count does not "
        "equal the declared N, the manifest is not exhausted. one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the "
        ...
        "asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect; before writing the criteria artifact, locate ✅ Manifest "
        "declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a citable manifest excerpt is unanchored; narrowing the criterion, "
        "substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; each criterion states (a) one observable behavior and (b) "
        "a falsifying condition referencing no implementation internals (no file names, function names, or variable names); this is a content gate, not a self-check; exactly one "
        "criterion may be written per thread per cycle — after the first criterion is written, the only valid next token is the formal notation rung label; a second criterion for the "
        "same thread is a protocol violation — stop, return to the prose rung, and split the thread before descending again. The formal notation rung separates behavioral specification "
        ...
        "Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — V-complete may not be emitted without this "
        "tool-executed result in the transcript; asserting confirmation without showing the exec_observed block does not satisfy this gate — the exec_observed block must appear in the "
        "current response; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it "
        ...
        "harness error before treating any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the "
        "same artifact appeared after the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; "
        "emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after a red VRO that is "
        "not a harness error, the criterion re-statement is an anchor only — re-running VRO without an implementation edit is a protocol violation; re-writing formal notation or the EV "
        "artifact without an intervening implementation edit is also a protocol violation; after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token; a "
        "green exec_observed at the VRO rung means there is no failing assertion — 🔴 Gap: may not be emitted after a green exec_observed; a green exec_observed without a prior red run "
        "means the test is vacuous — return to the executable validation rung and rewrite the test to fail; a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the "
        "exec_observed output; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is "
        ...
        "block; HARD STOP may not be emitted at the executable validation rung — a harness error at EV requires fixing the harness, not an upward return; When 🔴 Execution observed: at "
        "the executable validation rung shows a harness error (import failure, syntax error, missing file), the only valid next token is a tool call that repairs the harness — not a "
        "gap statement, not HARD STOP, not a rung label; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the observed gap matches the "
        "prior cycle's gap, the edit did not address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest "
        "declared are not re-emitted; the next token is the criteria rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. "
        "Upon writing the observed running behavior label, re-emit the criterion for this cycle — criterion re-emission is required: emitting the OBR label without immediately "
        "following it with the criterion text is a protocol violation; the tool call is the only valid next action after criterion re-emission; planning statements, diagnostic "
        "narration, and commentary between criterion re-emission and the tool call are protocol violations; ✅ Thread N complete may not appear until a tool call exists in the "
        "transcript after the observed running behavior label in the current cycle; then immediately invoke the implementation as a live running process via a tool call and emit 🔴 "
