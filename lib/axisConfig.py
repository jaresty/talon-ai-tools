...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "line is '§4 coverage verified' is exempt from the resume phrase requirement; (5) enumerate escape paths: '## Path enumeration' must not appear before '## Enforcement sequence' "
        "in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each "
        "path, name the literal string whose presence closes it; write '§5 enumeration complete' when no further open path remains; any referent named in the '[text]' from §1 not "
