...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "checkable values > prose descriptions of what should be validated. Code displayed in chat without being written to disk is prose description, not executable code. A "
        "validation artifact is complete when it can be applied to determine whether the conditions are satisfied without requiring additional interpretation or implementation. Using "
        "a weaker mechanism when a stronger one is available violates this constraint.",
