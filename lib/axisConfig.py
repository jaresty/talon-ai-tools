...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "exists for a governed behavior, writing that artifact is the first governed action; (a) through (g) apply to the executor invocation immediately following that creation — the "
        "creation action itself satisfies the creation-step exception below. Derivation phase (required before any governed action — complete only when a literal block labeled "
        "'Falsify derivation:' containing entries for (a) through (g) appears in the transcript; prose does not satisfy this requirement): Name — (a) the assertion-failure marker: the "
        "literal string in an executed-artifact result signaling the artifact detected behavior absence; (b) the literal string signaling the governed behavior is present; (c) the "
        "literal string identifying each governed behavior in an executed-artifact result; (d) the literal string identifying the governed action, which must appear as a substring in "
        "the name of the symbol added or modified; (e) the layer classification — if the governed symbol name appears as a direct call in the test body, the layer is 'unit'; if the "
        "governed symbol name does not appear in the test body, the layer is 'integration'; (f) the executor class — the executor must produce output that could only exist if the "
        "governed assertion reached execution at the layer named in (e); a governing artifact is disposable if removing the governed symbol from the implementation without changing "
        "the test leaves the artifact result unchanged — a disposable artifact does not satisfy (f); (g) the observed FAIL result: the content of a tool call result block appearing in "
        "the transcript after the artifact was applied to the subject and before the first governed action that modifies the implementation, where (a) precedes (c), the executor named "
        "in (f) produced the result, and the failure line contains a string present in the assertion body. Layer gate: if (e) is 'unit', the governed symbol name must appear in the "
