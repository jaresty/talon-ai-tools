...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "cross-thread argument lifts this block; only a subsequent passing test run does. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing "
        "the criteria, formal notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared gap — what the prior "
        "rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. Outputting a rung label is what "
        "begins that rung — it is not a heading or annotation. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without "
        "consulting I; before closing a rung artifact, state every open constraint the next rung must resolve. Advance through every feasible rung without pausing for user confirmation "
        "— each rung may not be skipped or combined with another, including the observed running behavior rung; the only protocol-defined stop is at the validation run observation rung "
        "after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. Response length is never a valid reason to stop between rungs — if the full descent "
        "cannot fit in one response, continue from the current rung in the next response without re-emitting completed rungs. The prose rung must be re-emitted at the start of every "
        "cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. When beginning "
