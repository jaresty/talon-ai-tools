...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Then move to implementation rung. Implementation rung: AFTER assertion rung shows red, implement ONLY what's STRICTLY NEEDED to make that specific assertion green — do NOT "
        "build complete components or do more than necessary; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this "
        "change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. Tests drive the behavior needed. Emit "
