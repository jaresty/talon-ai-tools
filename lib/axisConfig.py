...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "violation; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, "
        "the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, "
        "the test is vacuous and must be rewritten. Before writing the validation run observation rung label, invoke the validation artifact via a test runner; a harness error (import "
        "failure, syntax error, missing file) is not a red run — the test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before "
        "treating any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared "
        "after the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution "
        "observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after a red VRO that is not a harness "
        "error, the criterion re-statement is an anchor only — re-running VRO without an implementation edit is a protocol violation; re-writing formal notation or the EV artifact "
        "without an intervening implementation edit is also a protocol violation; after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token — this "
        ...
        "must fire for the new criterion before impl_gate; skipping EV and VRO after a HARD STOP that introduces a new criterion is a protocol violation; a green exec_observed at the "
        "VRO rung means there is no failing assertion — 🔴 Gap: may not be emitted after a green exec_observed; a green exec_observed without a prior red run means the test is vacuous — "
        "return to the executable validation rung and rewrite the test to fail; a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed output; after "
        "emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution "
        "observed: and 🔴 Gap: have been emitted at the validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; prose-only "
        "exec_observed sentinel does not satisfy this gate — the exec_observed block must contain non-empty verbatim tool output in a triple-backtick block; HARD STOP may not be "
        "emitted at the executable validation rung — a harness error at EV requires fixing the harness, not an upward return; When 🔴 Execution observed: at the executable validation "
        "rung shows a harness error (import failure, syntax error, missing file), the only valid next token is a tool call that repairs the harness — not a gap statement, not HARD "
        "STOP, not a rung label; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the "
        ...
        "re-emitted; the next token is the criteria rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. In-process "
        "rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists. If no invocable process exists, open a gap cycle to make the "
        "artifact directly invocable. if the 🔴 Execution observed: does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return "
        "failure-class rules. A full test suite run result is required after the most recent OBR tool call — the only valid next action if no such result exists is the tool call that "
        "runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason. Once ✅ Thread N complete is emitted no further output in that cycle "
        "is valid. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared "
        "gap — what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🔴 Gap: at "
        "the criteria rung is not a stop point — the criterion must appear in the same response immediately following the Gap; no content, confirmation, or pause between them is valid. "
        ...
        "response; emitting a file-write tool call before 🟢 Implementation gate cleared is a protocol violation regardless of whether impl_gate is emitted later in the same response. "
        "it does not complete the thread; after emitting it, the executable implementation rung, the observed running behavior rung, and their required tool-executed events must still "
        "fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may not appear "
        "until the observed running behavior rung has fired for this thread. Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior to the "
        "system — must be followed inline by a bracketed thread marker: [T: gap-name]; [T: gap-name] markers are valid only in the prose rung — their presence in any other rung "
        "artifact is a protocol violation; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly one entry "
        "per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at the start of "
        "every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. the VRO "
        "rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. Before "
        "emitting ✅ Manifest declared, count the thread entries in the list — the N in '✅ Manifest declared — N threads:' must equal the count of listed threads; a mismatch between the "
        "declared N and the thread list length is a malformed manifest. Before emitting ✅ Manifest declared, if the prose contains no [T: gap-name] markers, ✅ Manifest declared is "
        "blocked — re-emit the prose with [T: gap-name] markers added inline after each behavioral predicate before proceeding. Before emitting ✅ Manifest declared, scan every sentence "
        "in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or any finite verb where the subject is the "
        "system and the object is a data entity, user action, or external system) — each distinct predicate requires a separate thread; count the [T: gap-name] markers in the prose you "
        "just wrote to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry "
        "containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the criteria rung, not the "
        "manifest; ✅ Manifest declared opens the thread manifest for the session. A revised manifest may be emitted when a new gap is discovered mid-run — return to prose, re-emit "
        "prose for all incomplete threads with the new gap included, then emit ✅ Manifest declared with the revised thread list. Completed threads are closed and may not be re-opened "
        "by a revision. The N in ✅ Manifest declared must reflect the current incomplete thread count at the time of emission. only threads declared in ✅ Manifest declared may be "
        "addressed — creating a thread not in the manifest is a protocol violation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose "
        "label has not been output has not begun and no artifact for it may exist. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive "
        "the next without consulting I; before closing a rung artifact, state every open constraint the next rung must resolve. Advance through every feasible rung without pausing for "
        "user confirmation — each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle; the only "
        "protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. Response length is "
        "never a valid reason to stop between rungs — if the full descent cannot fit in one response, continue from the current rung in the next response without re-emitting completed "
        "rungs; stopping mid-descent is a protocol violation regardless of response length. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then "
        ...
        "appear unless ✅ Manifest exhausted immediately precedes it in the transcript — if no ✅ Manifest exhausted exists, stop: emit ✅ Manifest exhausted if all threads are complete, "
        "or open the next gap cycle if they are not. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in "
        "order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Seven rungs in order: prose → criteria → "
        "formal notation → executable validation → validation run observation → executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung "
        "begins; ✅ Manifest declared — N threads: [numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing "
        "omitted]; 🔴 Gap: [what the verbatim output reveals]; 🛑 HARD STOP — upward return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution "
        "observed]; ✅ Validation artifact V complete; ✅ Thread N complete; ✅ Manifest exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current "
        "tests]; ✅ I-formation complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.",
