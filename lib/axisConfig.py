...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "voids the rung; the audit section is separate and named; the rung is incomplete until all criteria are encoded. Only validation artifacts may be produced at the executable "
        "validation rung — no other content is permitted before ✅ Validation artifact V complete; each test function asserts exactly one behavioral property — one behavioral property "
        ...
        "rung; when the declared intent is to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the validation artifact must be a "
        "meta-test — an executable artifact that fails when the test-under-modification does not exhibit the criterion; before writing the test, check whether an existing test file "
        "covers the same module and add there if so; when the validation artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — "
        ...
        "writing the validation run observation rung label, invoke the validation artifact via a tool call; a harness error (import failure, syntax error, missing file) is not a red "
        ...
        "current cycle — emitting it at any other position is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal "
        "notation before any further edit. Upon writing the observed running behavior label, re-emit the criterion for this cycle, then immediately invoke the implemented artifact via "
        "a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be "
        "what a non-technical observer of the running system would see — not the test suite, not process state unless the criterion is specifically about process state; a build output, "
        "bundle manifest, or compilation result never satisfies this gate regardless of what it names; for a UI component, this means rendered browser-visible text and values; if the "
        ...
        "gap-name]. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must "
        ...
        "not from a prior cycle — to exist in output. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, "
        ...
        "confirm every predicate is covered; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or "
        ...
        "form to change, intent to stay fixed. Advance through every feasible rung; each rung may not be skipped or combined with another, including the observed running behavior rung "
        "— it is required in every cycle. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through executable "
        "implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward return. One edit per re-run cycle; "
        "after each edit re-run the validation artifact before any further edit. Upward returns follow the failure class: if the implementation did not close the gap, loop within "
        ...
        "— an artifact written in a prior cycle does not satisfy this gate. Reconciliation gate: at every upward rung transition, reconcile all artifacts against I and update any "
