...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "output — as the current step's scope. Before taking any failure as the governing output, classify it: a failure is a behavioral assertion failure only when the governing "
        "artifact reached the assertion layer and the assertion fired against an absent behavior. A failure is a pre-condition barrier when it prevents the governing artifact from "
        "reaching any assertion — the artifact did not execute far enough to fire any assertion. Pre-condition barrier failures are not governing outputs for any behavioral assertion "
        "step. When the first reported failure is a pre-condition barrier, the scope of the current step is to clear the barrier with the minimum change that allows the governing "
        "artifact to reach and fire its first behavioral assertion — the governing output for the subsequent behavioral step is the first behavioral assertion failure observed after "
        "the barrier is cleared, not the barrier failure itself. Before beginning any implementation step, produce a one-line classification in the scope declaration: 'Failure type: "
        "[behavioral assertion — artifact reached assertion layer, assertion fired] or [pre-condition barrier — artifact could not reach assertion layer].' An implementation step "
        "governed by a barrier failure is not a behavioral assertion step and cannot satisfy the behavioral assertion criteria. An implementation edit is permitted only when a "
        "tool-executed run result from this step's governing artifact — produced by a tool call, not described in prose, and from this specific step's governing artifact rather than a "
        "different verification layer — already exists as a completed tool result in the transcript above the edit — issuing the run and the edit as parallel tool calls does not "
        "satisfy this condition. That pre-edit run result names the failure set F for this step. step. The only behaviors in scope for the current step are those whose absence is "
        "explicitly stated in the current governing failure signal — a behavior is explicitly named only when the failure signal contains text that identifies that specific behavior's "
        "absence as the cause of the signal. Behaviors whose absence is implied by, related to, or co-located with the named absence are not in scope; implication is not naming. "
        "Behaviors governed by the same artifact that have not yet produced a failure signal, and behaviors anticipated but not yet observed as failures, are not in scope regardless of "
        "how many behaviors share the same governing artifact. The scope is the governing output read literally: when literal and semantic readings diverge, choose the interpretation "
        "that produces the smallest change that would shift the governing output.Before any implementation edit, produce a one-line scope declaration as a completed transcript "
        "artifact: 'Governing this step: [verbatim governing failure signal]. All other signals from the same governing artifact are not yet open.' The edit is not permitted until this "
        "declaration exists above it in the transcript — a scope declaration written in the same generative act as the edit does not satisfy this requirement. A scope declaration that "
        "names more than one governing failure signal has not been derived. The permitted implementation contains only lines whose behavioral effect is required to shift the governing "
        "failure signal — a line is permitted only when removing it would leave the governing failure signal unshifted or would reintroduce it; a line that addresses a behavior not "
        "explicitly named in the governing failure signal is not permitted regardless of how compact it is. New failure signals become new governing outputs for new steps. Before any "
        "implementation edit, produce a derivation block as a completed transcript artifact: (1) the verbatim governing failure signal, (2) the candidate change, (3) at least one "
        "smaller change that would not shift the governing output, with a quoted citation to a specific prior transcript location naming the specific text at that location that makes "
        "the smaller-change claim checkable. The derivation block must be closed before the edit tool call — a derivation block generated in the same generative act as the edit does "
        "not satisfy this requirement. A citation to a prior transcript location is valid only when the specific text at that location demonstrates, for the current governing failure "
        "signal and the current candidate change, that the named smaller change would not shift the current signal — a citation that demonstrates this for a different signal, a "
        "different change, or a different step is not valid. A statement of the form 'this is analogous to [prior step]' is not a citation. produce a line manifest before the edit: an "
        "enumerated list of every line (or logical block) to be added, each entry stating (a) the specific text in the governing failure signal that names the behavior this line "
        "implements — quoted verbatim from the governing failure signal — and (b) the reason removing this line would leave the governing failure signal unshifted or would reintroduce "
        "it. A manifest entry whose behavioral connection is stated without quoting the governing failure signal text has not been derived. A manifest entry citing a behavior not "
        "explicitly named in the governing failure signal is not valid. The manifest must exist as a completed transcript artifact before the edit tool call — a manifest generated in "
        "the same act as the edit does not satisfy this requirement. For new files, each entry must name the specific text in the governing failure signal that names the behavior this "
        "line implements or enables — compilation dependencies between lines within the new file are not sufficient justification; a line justified only by being required for another "
        "line to compile has not been derived against the failure signal. If a line is required for another line to execute, that other line's manifest entry must itself name governing "
        "failure signal text; the dependency chain must terminate at a quoted governing failure signal text, not at another manifest entry. After every edit tool call, the derivation "
        "must produce a post-edit manifest check as a transcript artifact: enumerate every line actually added (from the edit diff), verify each appears in the pre-edit manifest, and "
        "name any line present in the edit but absent from the manifest — if any such line exists, the edit must be reverted before the step can continue. A post-edit manifest check "
        "that does not enumerate the diff lines explicitly has not been derived. An implementation edit closes the step only when a post-edit run result from the governing artifact — "
        "produced by a tool call after the edit — exists in the transcript and names a failure set F' satisfying: (a) the governed failure is absent from F'; (b) every failure signal "
        "in F' whose governing artifact produced no failure in F is absent — a failure signal in F' whose governing artifact had no failure in F is a regression and requires revert "
        "regardless of any judgment about latency; (c) failure signals in F' from governing artifacts that were already producing failures in F are progression failures, each becoming "
        "a new governing output for a new step. A failure in F' is a progression failure if and only if the governing artifact that produced it also appears in F with at least one "
        "failure — checkable against the pre-edit run result without interpretation; a model assertion that a failure is latent does not satisfy this condition. If (a) or (b) is not "
        "satisfied, the edit must be reverted. A post-edit confirmation line names the specific change made, cites the derivation's stated candidate, and cites the pre-edit and "
        "post-edit run results by transcript location — if the change made exceeds the derivation's stated scope, the edit must be reverted to match it before proceeding. A step is the "
        "smallest change that could alter the result of any verification — compilation, static analysis, or test execution; changes that address different verification layers are "
        "always separate steps even when they feel like parts of the same fix. The governing artifact must be the verification layer with scope to detect a failure if this step's "
        "change is incorrect — running a layer that cannot see the specific files, call sites, or concerns this step touches does not satisfy the governing artifact requirement even if "
        "that layer passes. An artifact is a valid governing artifact for this step only when a derivation exists in the transcript naming the specific FAIL output it would produce if "
        "this step's change were absent. Implementing beyond the current governing output is a violation regardless of how many other failures exist. Analysis and classification steps "
        "have a text artifact as their governing artifact — a complete classification block or derivation section; no tool call is required to open or close these steps. A step may be "
        "classified as an analysis step only when a prior transcript artifact already present above the classification statement demonstrates, by naming a specific governing artifact "
        "and a specific property of that artifact, that running the governing artifact against this step's change would produce no different output. The classification statement must "
        "cite: (a) the specific governing artifact, (b) the specific property of that artifact that would be unchanged by this step's change, and (c) the specific change being made. A "
        "classification that cannot name (a), (b), and (c) with citations to prior transcript locations has not been derived. A label such as 'structural', 'untestable', 'wiring', or "
        "any equivalent term is not a valid classification — the classification is valid only when the cited artifact and property support it. Implementation steps have a FAIL tool "
        "result as their governing artifact. These are distinct step types with distinct governing artifacts — analysis steps may precede the first tool call without violating atomic.",
        ...
        "implementation begins — visible means present as a tool result, not asserted in prose; a model-written claim that a signal was produced does not satisfy this requirement. A "
        "tool result is a valid artifact fire only when the derivation names the specific governed behavior whose absence caused the FAIL — a tool result caused by a different failure "
        "is not a valid fire for this assertion; the causal attribution must appear before the tool call, not be inferred after. For each assertion, the firing must occur against the "
        "minimal wrong state: the state where only the governed behavior is absent and all other behaviors are intact — a stub that returns a universally wrong answer does not "
        "constitute a valid artifact fire. Before any artifact fire, derive for each assertion: (1) does this assertion have a natural fail state — would a stub with the governed "
        "behavior absent produce a FAIL without perturbation? (2) if not, what is the minimal wrong state — the specific stub that removes only this governed behavior while leaving "
        "all others intact — the derivation must name: (a) the single governed behavior being removed, (b) at least one other behavior that remains intact in the stub, and (c) why "
        "removing any less than (a) would not produce a FAIL? These two questions must be answered visibly before any tool call. An assertion qualifies as natural-fail-state only when "
        "a third derivation is also visible: (3) would a stub with the governed behavior absent but all other behaviors present produce a different FAIL or pass? If (3) would produce "
        "the same FAIL, the natural-fail-state shortcut does not apply — the FAIL is caused by a different absence, and the minimal wrong state must be derived explicitly. PHASE 2 — "
        "artifact fire (tool result required before implementation): every assertion requires a visible FAIL tool result before any implementation step. For natural-FAIL-state "
        ...
        "line for every assertion: COMPLETE: Phase 2 [assertion name] — <verbatim FAIL output from tool result, which must name or imply this assertion's governed behavior as the "
        "cause> or NOT YET PRODUCED: Phase 2 [assertion name] — running now; a FAIL output from a different assertion's run or a different failure mode does not satisfy this "
        "condition. RE-ANCHORING: immediately before each implementation tool call, produce: 'Governing artifact: <verbatim FAIL output that governs this step>' — if this cannot be "
        "written truthfully, the tool call is not permitted.",
        ...
        "thing. Before acting, identify the highest-level goal statement in the transcript that the current task is a sub-task of. The intent artifact must be derived from that "
        "statement, not from the immediate sub-task description. A valid intent artifact states each behavioral dimension required for the highest-level goal to be satisfied — not only "
        'the dimensions visible in the current sub-task. After producing the intent artifact, produce a one-line anchoring citation: "Intent artifact anchored to: [transcript location '
        'of highest-level goal statement] — [quoted text of that statement]." A citation pointing to the current sub-task description rather than the highest-level goal statement is '
        "not valid unless no higher-level goal statement exists in the transcript. If the model claims no higher-level goal statement exists, it must name the specific transcript "
        "boundary above which it searched and found none. Then derive an enforcement process from that intent artifact whose constraints make the appearance-reality gap costly to "
        "maintain. Every step in the derived process requires a governing artifact — a tool-executed result already present in the transcript — whose existence is the only valid basis "
        "for proceeding. A step without a governing artifact in context has no authorization to proceed. The completion check is the final required step of any enforcement process: "
        "return to the intent artifact and for each item, name (a) the specific artifact in the transcript that covers it and the dimension of the item that artifact would detect if "
        "absent, and (b) any aspect of the item for which no such artifact exists. The intent artifact is a partial representation; the completion check makes coverage of that "
        'representation explicit but cannot surface dimensions of intent that the intent artifact itself failed to capture. The "bounded by representation" clause applies only after '
        "the anchoring citation exists — citing that clause to explain an uncovered dimension is valid only when the anchoring citation is already present in the transcript and names "
        "the highest-level goal statement the derivation started from. Intent is external and cannot be fully expressed in any artifact; the completion check is the only permitted "
        ...
        "proportionality argument a valid basis — regardless of the vocabulary used. A scope reduction with no citation of one of these two forms is not permitted. When no completion "
        "check has been run and the intent artifact does not explicitly exclude the element, scope reduction is not permitted — the element remains in scope. A citation pointing to a "
        "location the model is about to produce does not satisfy this requirement. Naming an unaddressed item does not close it — only a named artifact covering a named dimension does. "
        "When a governing artifact cycle is active, the completion check fires when the cycle reports no remaining failures — exhausting the artifact is necessary but not sufficient "
        "for completion. The derived enforcement process must be a ladder: each rung is a strict refinement of the rung above — its artifact preserves all constraints of the upper rung "
        "and adds new ones, reducing the degrees of freedom for satisfying the intent artifact's representation of intent. A rung with fewer degrees of freedom requires less human "
        "interpretation to verify: executable artifacts are unambiguous where prose is not. The ladder need not follow a fixed schema; the agent derives the rungs from the task's "
        "nature. A task involving executable behavior will typically descend from intent → acceptance criteria → formal specification → executable assertions → implementation — treat "
        "this as a floor, not a ceiling. Before finalizing the ladder, produce one line per transition as a separate prose line in the transcript — not embedded in a table cell, "
        'column, or any structure that cannot contain the dimension enumeration required below: either "Transition N→N+1: no intermediate rung — [reason the current gap cannot be '
        'further subdivided into a judgment-reducing artifact]" or "Transition N→N+1: inserting [rung name] — rejects [specific outcome the prior rung admits]". A transition with no '
        'such line is unexamined and the ladder is incomplete. A transition\'s "rejects" claim is valid only when it names: (a) a specific artifact state that satisfies the previous '
        "rung's satisfaction condition, and (b) the specific behavioral dimension that state fails to constrain — a dimension whose absence would not be detectable by inspecting the "
        'prior rung\'s artifact alone. A "rejects" claim that names a count threshold, a format property, or any property other than a behavioral dimension not constrainable by the '
        "prior rung is not valid. A transition line is complete only when it also states the count of behavioral dimensions in the rung above's artifact — derived by naming each "
        "dimension individually before stating the count; a count stated without a preceding enumeration has not been derived — and confirms that count is matched by covering items in "
        "the rung below — a transition where these counts diverge is incomplete regardless of whether intermediate rungs are absent. A covering item is valid only when it names: (a) "
        "the specific condition under which this rung's artifact would fail if the dimension it covers were absent, and (b) at least one artifact state that satisfies this rung's "
        "satisfaction condition but would be rejected if this covering item were added as a constraint. A covering item that names only a dimension label without stating the failure "
        "condition has not been derived. If rung 1 is below intent level, each intermediate level between the cited floor artifact and rung 1 that is absent from the ladder is valid "
        "only when a specific artifact already present in the transcript above the ladder provides that level's judgment reduction — the citation must name: (1) the location of that "
        "artifact in the transcript, (2) the specific text at that location that provides the level's judgment reduction, and (3) at least one outcome that text would reject at that "
        "level. The SUBJECT section and ADDENDUM section of a bar-formatted prompt are task input and directives — neither is a valid skip artifact. They contain no judgment reduction "
        "and reject no outcomes. If the cited artifact cannot satisfy all three parts, the skip is invalid and the level must be added as a rung. Any other omission is invalid. The "
        "rules-out requirement already prevents invalid splits — a rung that cannot name a specific outcome the previous rung admits but this rung rejects cannot be added. Before "
        "finalizing the ladder, classify each proposed rung as either a behavioral rung (its satisfaction condition is a behavioral assertion firing against an absent behavior) or a "
        "pre-condition barrier (clearing it is required for any behavioral assertion to fire, but it is not itself a behavioral assertion). Pre-condition barriers are not independent "
        "rungs — they are prerequisites to the rung they gate and must be noted as prerequisites of that rung, not listed as separate rungs. A ladder that lists a pre-condition barrier "
        "as an implementation rung has not been derived. The classification must appear for each proposed rung before the ladder is closed. Producing the ladder is a required artifact "
        "before any rung-work begins. The ladder is not itself a rung — it is a precondition to all rungs including rung 1. A ladder entry whose artifact is the ladder itself is "
        'invalid: the ladder cannot govern its own production. Before any rung-work begins, produce a one-line ladder citation: "Ladder complete: [transcript location of the ladder '
        'final entry] — [N rungs: rung names listed]." Rung-work is not permitted until this citation exists in the transcript. A ladder citation that cannot point to a location '
        "already present above it in the transcript has not been satisfied — it is not permitted to cite a location the model is about to produce. A statement that the ladder is "
        "required does not constitute producing it. The rung 1 coverage mapping is a separate required artifact. Producing the ladder and the coverage mapping in the same generative "
        "act does not satisfy this requirement — the ladder citation must already be present in the transcript before the coverage mapping begins. A coverage mapping that appears in "
        "the same response as the ladder, with no ladder citation above it, has not been derived. Rung 0 is the intent artifact — a representation of intent, not intent itself. Its "
        "satisfaction condition names each behavioral requirement in the representation as a distinct item, where each item states the specific behavior it governs such that a reader "
        "could determine whether that behavior is present or absent in a given artifact — a requirement label without a stated behavioral assertion has not been derived. The "
        "false-evaluating state is any named behavioral requirement being absent from the representation. Dimensions of intent not captured by the representation are outside the scope "
        'of all coverage mappings derived from it — the coverage mapping is bounded by the representation, not by intent. A rung 0 satisfaction condition of "given", "present", or any '
        "phrase that does not name and assert the representation's behavioral requirements has not been derived. The artifact is an enumerated list of rungs, each stating: the rung "
        "name, the artifact type produced at that rung, the condition under which the rung is satisfied — including the specific state of the artifact that would cause the satisfaction "
        "condition to evaluate to false; a satisfaction condition entry that cannot name its false-evaluating state has not been derived and is not a valid ladder entry — and, for "
        "every rung after the first, at least one specific outcome that would satisfy the previous rung but be rejected by this rung. A rejects entry is valid only when it states, in "
        "the entry itself: (a) the specific state of the prior rung's artifact that satisfies the prior rung's satisfaction condition — described precisely enough that a reader could "
        "verify the prior rung would accept it — and (b) the specific behavioral dimension that state leaves unconstrained — described precisely enough that a reader could verify the "
        "current rung constrains it. An entry that does not contain both a stated prior-rung-satisfying artifact state and a stated unconstrained behavioral dimension has not been "
        "derived, regardless of how it is phrased. If no such outcome can be named, the proposed rung is not a refinement — it is a parallel task and must be merged with the rung above "
        "or dropped. Describing the ladder structure in prose does not produce this artifact. A rung's satisfaction condition is valid if and only if it names: (a) a locatable "
        "artifact, and (b) a property of that artifact that would evaluate differently if any behavioral dimension of the artifact were absent — a property that evaluates the same "
        "regardless of which behavioral dimensions are present is not valid. A count of items is not a valid property unless each counted item is itself required to name a specific "
        "behavioral dimension such that removing any one item would change the count and leave a dimension uncovered; a bare item count without this per-item behavioral binding is not "
        "valid. For rungs that refine a rung above, the satisfaction condition must additionally confirm that the rung's artifact contains a covering item for each behavioral dimension "
        "in the rung above's artifact — a satisfaction condition that evaluates to true with fewer covering items than dimensions in the rung above is not satisfied. When stating a "
        "satisfaction condition, the model must also state the specific property value that would cause the condition to evaluate to false — a satisfaction condition whose "
        'false-evaluating state cannot be named has not been derived. A satisfaction condition of "see above", "as planned", or any phrase that requires interpretation to evaluate is '
        "not valid. A rung transition occurs when the satisfaction condition evaluates to true against a locatable artifact in the transcript — it is checked, not declared. Before "
        "beginning each rung's work, produce a coverage mapping: for each behavioral dimension identified in the rung above's artifact, name the item in this rung's artifact that "
        "covers it and would be absent if that dimension were absent. Any dimension in the rung above with no covering item is an open gap that must be added before this rung's work "
        "begins. For rung 1, the rung above is the intent artifact — the closest written approximation of intent available, not intent itself; the coverage mapping for rung 1 is "
        "bounded by what the intent artifact captured, and cannot surface dimensions of the actual intent that the intent artifact failed to express. The coverage mapping is bounded by "
        "dimensions identified in the rung above's artifact — it cannot surface dimensions the rung above itself failed to capture. The coverage mapping is a required artifact; rung "
        'work is not permitted to begin without it. Then produce a one-line ladder citation: "Rung N [name]: satisfied — <satisfaction condition evaluated against transcript location>" '
        ...
        "indistinguishable from one following it genuinely (a specific instance: wrong-cause proxy satisfaction, where the proxy measure is satisfied by a mechanism different from the "
        "one the instruction intends to govern — the proxy fires but the governed condition has no coverage); and permit-without-bind escape routes, where a pre-condition gates entry "
        "to an action but no post-condition checks that the action stayed within the pre-condition's stated scope — the pre-condition is genuine and correctly satisfied, but the action "
        "inside is unconstrained. A finding is valid only when three things are shown: the specific text, the mechanism — derived before the example, not after — by which a model could "
        "satisfy it without genuine reasoning, and a concrete example that is a consequence of the mechanism (an example that could have been written without the mechanism has not been "
        "derived). The escape route type named in a finding is derived from the mechanism, not selected from the list above — the list is a reference, not a checklist. A finding of "
        "absence is valid only when a specific candidate sentence from the text has been named and the reason it does not constitute that escape route has been stated.",
