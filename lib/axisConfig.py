...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "not a separate gate requirement.) The ladder is iterative below the executable validation rung: each executable implementation produces the minimum change that resolves "
        "at least one unresolved assertion from the preceding gap declaration — not the full solution; after each implementation, cycle back to the validation run observation rung "
        "to re-observe the updated gap before implementing more; the implementation-observation cycle repeats until the observed running behavior rung is satisfied. A rung is an "
