...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current gap before any other "
        "content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. A rung is satisfied when and only "
        "when a tool-executed event appears in the transcript whose output is of that rung’s artifact type — inference, prediction, and prior knowledge do not satisfy rung gates "
        "regardless of accuracy; output from a different rung’s artifact type does not satisfy the gate regardless of what it implies (running the test suite at the observed running "
        "behavior rung produces validation-run-observation-type output, not observed-running-behavior-type output, and does not satisfy that gate). Every 🔴 Execution observed: sentinel "
        "at any rung must contain verbatim tool-call output, triple-backtick delimited, complete, nothing omitted — prose descriptions, inline summaries, and model-generated text that "
        "resembles output do not satisfy this requirement at any rung. The triple-backtick block constitutes the sentinel body; no content may appear inside the block that was not "
        "produced by the tool call — elision markers (…, [truncated], [output continues], [remaining output omitted]), reformatted text, and paraphrase are prohibited inside the block; "
        "if tool output is too long to include in full, the block must end at a natural output boundary with the last character the tool produced — no editorial closing. The "
        "execution-observed sentinel gates the tool invocation — the only valid next action after emitting this sentinel is the tool call it names; any response content between this "
        "sentinel and the tool call converts the sentinel to retroactive regardless of sequence, because the model will have already interpreted that content before the tool runs; a "
        "sentinel-to-tool sequence with intervening content must be treated as if the sentinel was not emitted — re-emit and re-run. Rule 2: each artifact derives from the prior rung — "
        "form changes, intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap declared by the prior rung — nothing beyond it. Rule 3: every "
        "thread ends when tool output names the declared gap; stop there. I is the declared intent governing the invocation; I precedes and is not itself an artifact; each artifact "
        "must be self-sufficient to derive the next without consulting I; before closing a rung artifact, identify every decision the next rung must make that is not fully determined "
        "by this artifact’s explicit content — state each such decision as an open constraint in the artifact; an artifact with no stated open constraints claims to be fully "
        "downward-sufficient; faithful derivation requires form to change, intent to stay fixed. Advance through every feasible rung; completeness governs rung depth, not rung "
        "existence; each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle. Before any tool call or file "
        "read, check: if no 🔴 Gap: sentinel exists in output yet, emit it now; before emitting 🟢 Implementation gate cleared, check: does a 🔴 Execution observed: sentinel from the "
        "current cycle's validation run observation rung — not from a prior cycle — exist in output? if not, do not emit it — produce the executable validation artifact instead. 🟢 "
        ...
        "implementation file — ✅ Thread N complete, a new criteria cycle, or any other sentinel may not appear until at least one edit and its post-edit re-run have occurred and the "
        "observed running behavior rung has fired for this thread. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running "
        "behavior label, emit 🔴 Gap: naming what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops "
        "after naming the gap. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the "
        "criterion must be immediately derivable from the re-emitted prose. From the criteria rung onward, the gap names a specific behavior currently absent or wrong, phrased as a "
        ...
        "exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists in the transcript for each thread declared in ✅ Manifest declared — check the "
        "thread count before emitting. Each rung’s artifact addresses only that gap — not all known requirements of the task — and is minimal: one independently testable behavior "
        ...
        "is invalid — split it before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a "
        "structural element and its data source is a conjunction — ‘renders columns with data fetched from the endpoint’ must be split into ‘columns render’ and ‘data is fetched from "
        "the endpoint’ before continuing; a criterion that asserts only that a structural element exists is not behavioral — asserting a column header is present does not satisfy this "
        "rung; add a criterion asserting the element’s content or effect (the row displays the value from the data source); before writing the criteria artifact, locate ✅ Manifest "
        "declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a citable manifest excerpt in this cycle is unanchored and the "
        "criteria rung has not been satisfied; the criterion for thread N must name the same behavior as the gap declared for thread N in ✅ Manifest declared — narrowing the criterion, "
        "substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; if the declared gap cannot be satisfied, return to the "
        "prose rung and revise the manifest. Each criterion must be written in two parts: (a) observable behavior — what a non-technical observer of the running system would see; (b) "
        "falsifying condition — what behavioral gap would cause this criterion to fail; the falsifying condition is valid only if part (b) names a behavioral gap statable without "
        "naming source files, function names, or variable names — if part (b) references implementation internals, the criterion is structural and must be rewritten before this rung "
        "closes; this is a content gate, not a self-check. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases; it "
        ...
        "modification; if placing the notation verbatim in the source tree would produce a runnable artifact, it is implementation code and voids the rung; annotation-decorated "
        "classes, repository interfaces, and any construct that compiles as-is are implementation code regardless of whether they contain imperative logic. Only validation artifacts "
        "may be produced at the executable validation rung — no other content is permitted before ✅ Validation artifact V complete; the artifact must be invocable by an automated tool; "
        "before writing the test, re-read the formal notation and assert each structural constraint it encodes; each test function contains exactly one assertion — one assertion per "
        "test function; a pre-existing test that happens to pass does not satisfy this rung — add to an existing test file covering the same module if one exists, otherwise create a "
        "new file; when the validation artifact is modified — tests added, rewritten, or mocks changed — invoke a tool call to read the current test file before emitting carry-forward "
        "rows — each row must cite a prior-failure name quotable verbatim from that read result in the transcript; a carry-forward row with no citable read-tool result in the "
        "transcript for this cycle is uncovered regardless of model recall; emit a carry-forward statement before any implementation artifact: carry-forward format: ‘Carry-forward: "
        "prior failure [verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]’ — one row per current "
        "test; each prior-failure name must be quotable verbatim from a 🔴 Execution observed sentinel in this conversation; any current test whose name does not appear in a "
        "carry-forward row is unconditionally uncovered; modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until "
        "carry-forward has been emitted. before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — "
        ...
        "in the transcript for the current cycle — if the validation artifact passes before any implementation edit, either the test is vacuous (rewrite it) or the gap is already "
        "closed (return to criteria); ✅ Validation artifact V complete may not be emitted unless a 🔴 Execution observed: showing the test failing exists in the transcript for the "
        "current cycle — write the test, run it, confirm it fails, then emit V-complete; emitting V-complete before seeing a red run is a protocol violation regardless of whether "
        "perturbation was attempted. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is written. Before "
        "writing the validation run observation rung label, invoke the validation artifact via a tool call — a green 🔴 Execution observed: is only valid as the post-implementation "
        "green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared for this thread showing the test logic failing — a "
        "harness error (import failure, syntax error, missing file) is not a red run; the test must have been loadable, its assertions must have run, and they must have failed; fix the "
        "harness error before treating any run as a red witness — do not proceed to the implementation gate until a loadable run with failing assertions exists in the transcript for "
        "the current cycle. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; "
        "stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token in the "
        "response — any other content before the criteria label is a protocol violation; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 "
        "Gap: have been emitted at the validation run observation rung in the current cycle — emitting it after a criteria label, after a formal notation label, or at any other "
        "position is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Upward "
        ...
        "notation; observation cannot be produced is a prose-description failure, not an implementation failure — return to the prose rung and update it. One edit per re-run cycle; "
        "after each edit re-run the validation artifact before any further edit. After every upward return, a new ✅ Validation artifact V complete must be emitted for the current "
        "cycle’s gap before the implementation gate may fire — an artifact written in a prior cycle does not satisfy this gate even if it covers the same path. Upon writing the "
        "observed running behavior label, re-emit the criterion for this cycle, then immediately invoke the implemented artifact via a tool call and emit 🔴 Execution observed: with its "
        "verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a non-technical observer of the running system "
        "would see — not the test suite, not process state (server started, build succeeded, exit code 0) unless the criterion is specifically about process state; a build output, "
        "bundle manifest, or compilation result never satisfies this gate regardless of what it names — direct demonstration means the running feature produced the output, not the "
        "build tool; for a UI component, this means browser-visible text and values — not a test runner’s DOM query result; if the output does not directly demonstrate the criterion, "
        "emit the gap and return upward; a 🔴 Execution observed: whose verbatim block is empty or blank does not satisfy this gate — a verification script, console log, or tool call "
        "that produces no visible output is not valid OBS evidence; invoke a tool call that produces non-empty output demonstrating the behavior; if the declared gap names a real "
        "network endpoint (fetches from, calls, integrates with), the OBS must include verbatim output showing a real HTTP request or response — a test run whose network calls are "
        "mocked does not satisfy this gate; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the "
        "declared behavior; before any tool call touching the filesystem at this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — the "
        "cited rung label must appear verbatim in this thread’s prior transcript output — if the rung label cannot be located in the transcript, the tool call is prohibited; model "
        "recall of a rung that fired does not satisfy this gate; a tool call with no citable provenance is creating a new artifact regardless of characterization and is prohibited at "
        "this rung; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label "
        "has been written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it and the OBS output "
        "directly demonstrates each criterion declared in this thread — a test pass is not a demonstration; no implementation file was created or modified for this thread’s gap in this "
        "cycle means the gap was already closed (return to criteria) or the test is vacuous (rewrite it) — Thread N complete may not be emitted when no implementation edit was made in "
        "this cycle; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: from a full test suite run exists in the transcript after the most recent OBR tool call in "
        "this thread — the only valid next action if no such result exists is the tool call that runs the suite; a model summary of expected suite results does not satisfy this gate; "
        "every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — dismissing a failing test as unrelated without an explicit written "
        "acknowledgment is a protocol violation; once ✅ Thread N complete is emitted no further output in that cycle is valid. Each sentence in the prose containing a behavioral "
        "predicate — any action verb attributing behavior to the system (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — must be "
        "followed inline by a bracketed thread marker: [T: gap-name]; these thread markers become the authoritative enumeration for the manifest and the Thread N complete check; a "
        "prose rung with unmarked behavioral predicates is incomplete at time of production. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral "
        "predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or any similar action verb) — each distinct predicate requires a separate "
        "thread; a manifest that omits a behavioral predicate from the prose is incomplete and may not be emitted. Before emitting ✅ Manifest declared, re-read the prose rung output "
        "via a tool call; every behavioral predicate in the verbatim read result must have a [T: gap-name] marker — ✅ Manifest declared may not be emitted until this read exists in the "
        "transcript and every predicate is covered. ✅ Manifest declared may be emitted exactly once per invocation — at the prose rung, after the first gap is named; re-emitting it at "
        "any other rung is a protocol violation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not "
        "begun and no artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every "
        ...
        "Reconciliation gate: at every upward rung transition, reconcile all artifacts at that rung against I before redescending; if prose has evolved, any pre-existing document that "
        "records I (ADR, README, spec) must be updated before redescending — this is a gate precondition, not a final-report step; documentation is a reconciliation artifact, not a "
        "thread. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside the rung where it is defined is a protocol violation; a sentinel used as a "
        "narrative summary or wrap-up is not a valid emission. Before producing the final report, invoke the delivered system end-to-end via a tool call and emit 🔴 Execution observed: "
        "showing the primary behavioral intent from the prose is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before producing the "
        "final report. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order; the final report copies "
        "artifacts already emitted in the thread rungs — before writing each section, locate the artifact in the prior transcript; if it cannot be found verbatim in the transcript, it "
        "may not appear in the final report; no new behavioral claims, no coverage summaries, no suggestions may appear in the final report; then reconcile any documents (README, "
        "specs, ADRs) the implementation affects — this reconciliation is part of the final report, not a separate thread. Seven rungs in order: prose → criteria → formal notation → "
        "executable validation → validation run observation → executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ Manifest "
        "declared — N threads: [numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the "
        "verbatim output reveals]; 🛑 HARD STOP — upward return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation artifact "
        "V complete; ✅ Thread N complete; ✅ Manifest exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current tests]; ✅ I-formation complete; "
