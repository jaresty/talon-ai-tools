...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "tool-executed run result above the first entry; the candidate change is permitted only to address the specific failure text visible in the governing failure signal; a failure "
        "not yet observed in a prior tool-executed run result has not been shown to govern, and a change that fixes it cannot be derived from the current governing artifact; an "
