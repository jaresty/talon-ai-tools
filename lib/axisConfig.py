...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "every path by which a transcript could satisfy the §1, §2, §3, and §4 heading requirements without the governing goal having actually been identified, dimensions actually "
        "derived, or dimensions actually covered — the enumeration must include at least one path entry for each of §1, §2, §3, and §4; for each path, name the literal string in the "
        "transcript that closes it — a path is closed only when naming that string makes the non-compliant transcript distinguishable from the compliant one without assessing intent; a "
        "path that cannot be closed by naming a string must be eliminated by ensuring the required structural property is present before the heading is written. No file-modifying tool "
        "call may appear before this enumeration is present in the transcript. The literal strings named during enumeration as path closures are binding — a heading or structural "
        "property that does not match the committed string verbatim does not satisfy the corresponding requirement. Proceed to implementation only when the derivation and enumeration "
        "are present as text above this point in the transcript. Any noun phrase in the governing goal that names a file path, URL, ticket number, or document title not already present "
        "in the transcript must be fetched before the governing goal heading is written — derivation is blocked until the referent is resolved. No file-modifying tool call may appear "
        "before the completion-check heading. The response is not permitted to end until the completion-check heading appears in the transcript and every covered dimension must cite a "
        "verbatim string from a tool-executed result whose invocation names the subject under observation directly — a tool-executed result whose invocation names an artifact that "
        "evaluates or reports on the subject rather than the subject itself does not satisfy this requirement.",
