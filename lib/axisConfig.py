...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "until you emit completion sentinel for rung N or return upward), rung completion order (each rung must produce its artifact before emitting completion sentinel; sequential "
        "descent through prose → criteria → formal notation → validation → implementation; skipping a rung is a protocol violation; each rung may not be skipped or combined with "
        "another), formal notation is NOT a file artifact — it is a mental exercise, not a written document; produce the specification in your head, not on disk; the formal notation "
        "rung verifies your understanding, not a file; rung-specific gaps (each rung addresses a specific gap that only that rung can solve; the gap must be explicitly expressed IN THE "
        "CONVERSATION with evidence appropriate to the rung level; for validation assertions, the gap is the individual assertion being red; gaps are not implicit — they must be "
        "written down in the conversation with supporting evidence; fabricated test runs or falsified output void all evidence and the rung), P5 implies sequential descent (since each "
        "artifact derives from prior artifact per P5, you must produce them in order — treating the ladder as a planning document rather than strict sequence violates derivation), "
        "ladder is strict execution sequence (emit rung label, produce artifact, emit completion sentinel — repeat for each rung in order; when resuming mid-ladder, locate the highest "
        ...
        "complete; UPWARD SCAN MUST examine ALL prior rungs (prose, criteria, formal notation, validation) for unaddressed criteria — not just the immediately prior rung; each "
        "criterion from earlier rungs must have corresponding assertions and implementation; if criteria from prose/criteria/formal notation rungs are not satisfied by "
        ...
        "rung: EDIT existing script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL (red). NO implementation in this rung. AFTER check turns "
        "green (implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ Assertion complete (staying in assertion rung), THEN add another assertion for the "
        "same gap; the completion sentinel MUST explicitly declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied] — evidence: [reference "
        ...
        "implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO assertions added here; NO file creation; only edit existing stubs; implement just enough to "
        "pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why "
        "this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if gap is closed — If gap still exists: FIRST emit ✅ Implementation complete "
        "(return UP to assertion rung to add next assertion); if gaps remain after implementation, the completion sentinel MUST explicitly declare: ✅ Implementation complete — gaps "
        "remaining: [list which prose/criteria/formal notation criteria are NOT yet satisfied] — evidence of gap verification: [reference the specific criteria checked, or if gaps "
        "listed, explain why return upward is needed]; if gap IS closed: emit ✅ Thread N complete — all gaps closed. If gap IS closed: emit ✅ Thread N complete — all gaps closed. Tests "
        "drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. ASSERTIONS are test code that verifies behavior, NOT import "
        "errors or infrastructure failures. Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; "
        "the sentinel must appear after the artifact is produced and gate is satisfied; Every rung must emit its label BEFORE producing its artifact and emit its completion sentinel "
        "AFTER. No implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed sentinel must include "
        "the verbatim failing assertion text; Evidence before claims: for every claim about code being correct, there must be tool execution output proving it; describing what you "
        "would do is not evidence; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity "
        "test: a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal "
        "notation → validation → assertion → implementation — the assertion rung is REQUIRED in this order; the assertion rung verifies each individual assertion red → green; emit the "
        "rung label before producing its artifact; emit completion sentinel after artifact is produced and gate is satisfied. EV rung: edits only validation files. EI rung: edits only "
