...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "declared N and the thread list length is a malformed manifest. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, "
        "requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read the prose rung output "
        "via a tool call to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry "
        ...
        "manifest; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose "
        ...
        "the next without consulting I; before closing a rung artifact, state every open constraint the next rung must resolve; faithful derivation requires form to change, intent to "
        "stay fixed. Advance through every feasible rung without pausing for user confirmation between rungs — completeness governs rung depth, not rung existence; each rung may not be "
        "skipped or combined with another, including the observed running behavior rung — it is required in every cycle; the only protocol-defined stop is at the validation run "
        "observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the highest already-instantiated "
        ...
        "implemented behavior, lists acceptance criteria met, describes files created or modified, or names gaps as resolved is a final report regardless of its heading; Such prose may "
        "not appear until ✅ Manifest exhausted exists in the transcript for this invocation; if no ✅ Manifest exhausted exists, stop: emit ✅ Manifest exhausted if all threads are "
        ...
        "thread in order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Rung-entry gate: before producing "
        "content at any rung, state (a) the rung name, (b) the current gap as a currently-false behavioral assertion, (c) the artifact type this rung requires, and (d) whether a 🔴 "
        "Execution observed: sentinel exists in the current cycle and, if so, whether its output is of type (c) — if no exec_observed exists yet in this cycle, state that explicitly "
        "rather than treating prior-cycle output as satisfying this check; if any of (a)–(d) cannot be stated from the current-cycle transcript, produce it before any other content at "
        "this rung; if (d) reveals that no valid exec_observed exists in the current cycle, the only permitted next token is a tool call — producing any other content when (d) is false "
        "is a protocol violation. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed "
