...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "written; otherwise, identify the existing guard via a tool call that locates or retrieves it and wait for the tool-result block confirming the identified guard; in either "
        "case, exactly one regression guard shall govern property [N] for the remainder of the cycle; every subsequent reference to 'the guard' — including every 'Property [N] "
        "assertion', 'Quoted test:', 'Test blind-spot:', and 'Failure:' — refers exclusively to this established guard until step (2) is repeated; every property shall establish "
        ...
        "tool-result block must appear; if the failure line does not specifically witness the violation of property [N], modify the violating state and repeat from step (4); after the "
        "Failure: line has been emitted, emit exactly one witness line for every previously inventoried executable assertion, in inventory order; each witness line takes the form: "
        '\'Property [N] assertion [M] witness: Failure — assertion: "<verbatim assertion text>" — failure: "<verbatim tool-result failure text>"\' if the assertion produced the '
        "observed failure, 'Property [N] assertion [M] witness: Success — assertion: \"<verbatim assertion text>\"' if the assertion passed, or 'Property [N] assertion [M] witness: "
        'Unreached — assertion: "<verbatim assertion text>" — <reason execution did not reach this assertion>\' if execution could not reach the assertion; the verbatim assertion text '
        "in each witness line must match the assertion text from the corresponding inventory line exactly; no witness may be emitted for an assertion not present in the assertion "
        "inventory, and no inventoried assertion may lack a witness; at least one inventoried assertion must receive witness classification 'Failure' — a guard execution in which no "
        "inventoried assertion is classified 'Failure' does not satisfy step (4), regardless of whether a Failure: line was produced; a 'Failure' witness whose verbatim failure text "
        "is not present in the corresponding Failure: tool-result block does not satisfy this token; an 'Unreached' witness without a stated reason does not satisfy this token; if no "
        "inventoried assertion can be attributed to the observed failure, revise the violating state or the regression guard and repeat from step (2); after all assertions are "
        "witnessed, emit 'Assertion witnesses: complete — <count> assertions witnessed' where <count> must equal the count from 'Assertion inventory: complete'; a witness count that "
        ...
        "violating state or guard and repeat from step (4); no subsequent step may occur until 'Assertion witnesses: complete' has been emitted; (5) identify the governed artifact for "
        "property [N] — the artifact whose modification satisfies property [N]; depending on the property, this may be the implementation, the regression guard, a configuration file, "
        "or another explicitly governed artifact; modify the governed artifact to satisfy property [N], then immediately emit 'Quoted implementation: <smallest contiguous verbatim "
        "text uniquely identifying the modification to the governed artifact>'; if step (4) required constructing a temporary violating state because the governed artifact already "
        ...
        "emitted while any property [N] lacks a witnessed Failure: or Unobservable: line. A response that omits any required sentinel, performs a step before emitting its required "
