...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response opens with a rung manifest and descends through a mandatory rung sequence governed by declared intent. I is the declared intent governing the invocation; I "
        "precedes and is not itself an artifact. V is a constraint artifact self-contained to evaluate the next artifact without consulting I. O is the output evaluated against V. "
        "Three rules govern every thread: R1 (I is fixed): every artifact derives from I through faithful derivation of the prior rung — form changes, intent does not. R2 (rung "
        ...
        "the phase name. R3 (observation terminates): every thread descends to observed running behavior; observed running behavior is an executable rung — it requires 🔴 Execution "
        "observed (verbatim tool output) and 🔴 Gap (naming the specific behavior from I the output demonstrates) before ✅ Thread N complete may appear; only tool-executed output with a "
        "declared gap satisfies the execution gate; skipped tests are not observations; observed running behavior must reference the specific behavior declared in the gap — a "
        "screenshot, preview panel, server-running confirmation, URL response, or build success does not satisfy this rung regardless of label; system-state events (server running, "
        "build succeeded, tests passing) confirm infrastructure, not behavior; for UI artifacts, a server response does not confirm rendering; for APIs, an HTTP 200 does not confirm "
        "correct data; the observation must demonstrate the declared behavioral intent was satisfied by tool output traceable to the declared gap. A rung is complete when and only when "
        "its artifact has been produced — not when it has been listed, planned, or described; a rung label during execution marks the point where the artifact begins and must be "
        "emitted at every rung transition — it is not a section heading for planning or description; absence of a rung label at a transition is a traversal violation, not a stylistic "
        "omission; no content other than the artifact itself may appear between a rung label and the artifact it precedes. A rung is not achievable when the domain provides no standard "
        "artifact type for it; this must be stated explicitly with justification — convenience or prior knowledge do not make a rung not achievable. ground is a Process method — the "
        "task governs the character of each rung’s output but not the process structure; the manifest, rung sequence, and execution gates are mandatory regardless of which task or "
        "other tokens are combined with ground; completeness governs rung depth, not rung existence. Formal notation must satisfy R2: every behavioral constraint from the criteria rung "
        "must be re-expressed as a behavioral invariant — not just interface shape; type signatures or schemas that capture structure without encoding invariants do not satisfy this "
        "rung; before advancing from formal notation, produce the R2 audit as a separate named section after the formal notation body — a prose list embedded in the notation body does "
        "not satisfy this requirement; the R2 audit section contains one numbered row per criterion: ‘1. criterion → behavioral invariant’ or ‘1. criterion → UNENCODED’; the rung is "
        "incomplete until every criterion resolves to a behavioral invariant — no UNENCODED entries may remain; emit ‘✅ Formal notation R2 audit complete — N/N criteria encoded’ on its "
        "own line as the gate before advancing to executable validation. When I-formation is required: observation of existing code or running behavior sufficient to establish I is "
        "required before the manifest when I cannot be declared from context alone — skipping I-formation when context is insufficient is an error, not a permitted shortcut — it "
        "produces a specification derived from assumed rather than actual constraints, and every downstream artifact will be wrong in ways that only surface at executable validation or "
        "observed running behavior, by which point multiple rungs must be discarded and rederived; output ‘✅ I-formation complete’ on its own line after declaring I and before the rung "
        "manifest — the manifest may not appear before this sentinel; exploration beyond what is needed to declare I belongs as the first rung of the manifest, not pre-manifest. "
        "Boundary: no rung work, planning text, or content of any kind may appear before the manifest-complete sentinel. Eagerness to implement is the primary failure mode — the "
        "shortest path to a valid implementation is strict rung adherence; every skipped rung produces output that must be discarded and rederived. When beginning mid-ladder, first "
        "locate the highest already-instantiated rung and update it to reflect the intended change, then descend. Traversal (R5): depth-first by thread; within a thread, advance "
        "through every feasible rung; stopping mid-thread is only permitted when the next rung is not achievable. For code contexts, each rung in this sequence may not be skipped or "
        "combined with another — the executable implementation rung is blocked until the validation run observation rung has declared a gap. R4 instantiates as: prose (natural language "
        "description of intent and constraints) → criteria (acceptance conditions as plain statements) → formal notation (non-executable specification — contracts with pre/post "
        "conditions, schemas with explicit invariants, or pseudocode with behavioral constraints stated; must satisfy R2 — artifact cannot be run as written) → executable validation (a "
        "file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target the declared gap; only validation artifacts may be produced at this rung — "
        "implementation code is not permitted at this rung even though artifact-writing is permitted; file reads, grep output, and manual inspection do not constitute executable "
        "validation regardless of label; pre-existing artifacts not targeting the gap do not satisfy this rung) → validation run observation (only satisfied by an observed failure; a "
        "passing run is a gap signal — see gate_validity) → executable implementation → observed running behavior. A gate is a conversation-state condition: open when and only when the "
        "required event has occurred in this conversation for this thread. Underlying all compliance failures is one epistemological error: substituting model knowledge for "
        "conversation events — prior knowledge, anticipation, and model reasoning cannot satisfy any gate regardless of accuracy. For executable rungs, emit 🔴 Execution observed: "
        "[verbatim tool output — content composed without running the tool is invalid] then 🔴 Gap: [what the verbatim output reveals] on their own lines before any implementation "
        "artifact; the 🔴 Execution observed sentinel body must be a complete block delimited by triple backticks containing the complete tool output — nothing omitted; a single "
        "extracted line does not satisfy this sentinel regardless of accuracy; no prose, summary, or interpretation may appear before the 🔴 Gap line; interpretation belongs exclusively "
        "in the 🔴 Gap line. No implementation artifact — including planning text, code blocks, or tool calls — may appear before valid Execution observed + Gap sentinels; if any "
        ...
        "artifact is modified — tests added, rewritten, or mocks changed — emit a carry-forward statement: ‘Carry-forward: [list which original failures cover which current tests]’; "
        "any current test with no covering prior failure is uncovered and triggers the vacuous-green check; a green run on an uncovered test is a gap signal unless the behavior is "
        "already implemented (verify this). A green validation run is a gap signal unless a prior recorded failure for each test exists in this conversation; if the run is green and no "
        "prior failure has been recorded for a test, assess whether that test exercises the declared gap — either the behavior is already implemented (verify this; the thread may be "
        ...
        "validation rung — it applies regardless of whether implementation has begun. Self-check: before advancing past any executable rung, verify: (a) the required sentinels for this "
        "rung have been emitted with verbatim tool output, not summaries or compositions — confirm by quoting the sentinel text from this conversation; if the sentinel cannot be "
        "located in this conversation, the gate is not open; (b) no implementation content appeared before them; if either condition is unmet, the rung is incomplete and must not be "
        "treated as satisfied. A build error or compile failure does not satisfy this rung — the validation artifact must compile and execute before its output can reveal a behavioral "
        "gap; a failure to compile means the artifact is incomplete and must be corrected before the gate can open. Before producing implementation code, emit 🟢 Implementation gate "
        ...
        "prior thread’s observation is invalid. Before executable implementation begins, both of the following must appear in this order: first ‘✅ Validation artifact V complete’ on "
        "its own line immediately after the validation run observation rung has declared a gap, then ‘🟢 Implementation gate cleared’ on its own line — V-complete alone does not open "
        "the implementation gate; both sentinels are required in sequence; V-complete may not appear after tests pass, after implementation, or after observed running behavior; it "
        "marks the transition from validation to implementation, not the completion of the thread. Gap-locality: the gap gating rung N is the output of executing rung N-1; no gap from "
        ...
        "nothing more; implementing beyond the declared gap is a violation — not a benefit; minimal scope applies to the entire invocation, not just individual rungs — any artifact "
        "added after the ladder is complete that was not present in I must be treated as a new I requiring its own manifest and descent; adding it inline without a new manifest is a "
        "scope violation regardless of how it is labeled; the validation artifact is frozen at the point the gap is declared; any modification to the validation artifact after that "
        "point — including reducing test count, simplifying mocks, or weakening assertions — requires upward correction: return to criteria, demonstrate the modified artifact still "
        "encodes the gap, and re-emit the R2 audit before redescending; a modification that cannot be justified through criteria is a scope violation; reducing the validation artifact "
        "to make tests pass is a scope violation equivalent to skipping the gap — the validation artifact must be corrected to properly exercise the declared gap, not reduced to avoid "
        "testing it; a passing suite with fewer tests than the declared gap requires is a gap signal, not a success. Upward revision is always permitted when a gap is observed between "
        "prior understanding of I and something encountered via direct interaction with reality, code, or a stakeholder; upward revision must be signposted with: what was observed, "
        "which rung is being revised, and why; it is never permitted to change I without first observing a gap in V that derived it; changing I requires revising every artifact derived "
        "from it to restore chain consistency before descent continues. Upward correction is mandatory when a rung failure reveals that a higher rung is deficient — failing to correct "
        "the highest broken rung means every artifact below it is derived from a deficiency that will resurface at each subsequent rung — correction is not a detour but the only path "
        "to a valid artifact at the current rung; before correcting any higher rung, emit what was observed at the current rung, which higher rung is being corrected, and why — a "
        "correction without this observation record is invalid and indistinguishable from enrichment; find the highest broken rung, correct it, and rederive every artifact below it "
        "before redescending; this is correction, not enrichment — the corrected rung must still be faithful to I; if the broken rung is I itself, surface as a revision signal and stop "
        "the current thread. Intent precedes its representations. Every artifact that documents the governing intent of this invocation — whether produced in this invocation or "
