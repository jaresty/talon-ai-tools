...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "skip: hidden gaps persist; RULE: explicit verification before completion [ ] Challenge: derive from P3,P5 — Challenge MUST extend existing evaluation not create new; RULE: "
        "Challenge = add N new criteria to baseline that FAIL; Gap failures happen at Challenge not Baseline [ ] Refinement: derive from P3,P5 — why Refinement addresses OBSERVED gaps "
        "only; what GOES WRONG if you modify evaluation logic: changes what Challenge proved; RULE: Refinement = for each failing criterion add ONLY enough to satisfy it ADVERSARIAL = "
        "minimal effort to pass verification [ ] Verification: derive from P1,P3 — why Verification uses SAME evaluation framework; what GOES WRONG if you add manual verification: "
        "undermines evidence authority; RULE: Verification = run full evaluation from baseline evidence = all criteria satisfied; If gaps remain iterate Challenge→Refinement until "
        "satisfied[ ] Baseline: derive from P2,P3 — Baseline defines evaluation structure not solution; RULE: Baseline = evaluation structure functions; PROVE necessity for each "
        "element or remove it [ ] Baseline scaffolding: derive from P2,P3 — Baseline scaffolding is placeholder not solution; RULE: Baseline scaffolding = empty shell, Refinement adds "
        ...
        "remaining?], [next rung requires?]. If gaps remain or verification not shown, do NOT proceed.Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules "
        "from P1-P5. Ladder derived MUST be emitted as: 🔵 Ladder derived — AFTER producing the ladder table with columns: rung name | artifact type | derivation source | evidence form. "
        "No descent before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent work. The 7-rung ladder: 1. Intent — What outcome is being aimed "
        "for? Derivation: from task, derive specific outcome. Evidence: clear statement of goal. 2. Criteria — What conditions must be true for success? Derivation: from intent, derive "
        ...
        "APPLIED and VERIFIED to work. IMPORTANT: Baseline establishes evaluation STRUCTURE only - defines WHAT will be evaluated, NOT the solution itself. Baseline = criteria "
        "definition and scaffolding, not implementation. IMPORTANT: Baseline PASS means evaluation structure FUNCTIONS, not that task is complete. Baseline passes if evaluation works "
        "without errors - nothing more. Solution comes later through iteration. IMPORTANT: DO NOT proceed to Challenge until Baseline PASSES. Show evidence that evaluation works "
        "without errors. If you cannot show passing evidence, fix Baseline first. IMPORTANT: ADVERSARIAL posture required. For each element you add to Baseline, ask: Would removing "
        "this break evaluation? If NO, remove it. Only keep what is STRICTLY NECESSARY. Prove necessity or remove. IMPORTANT: Baseline establishes evaluation framework at Baseline. "
        "Challenge adds NEW criteria to the SAME framework - creating NEW evaluation at Challenge is SCOPE EXPANSION, not valid Challenge. Challenge must extend existing evaluation, "
        "not create new ones. 5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, add N new criteria to existing evaluation that FAIL. Evidence: N failing "
        "outputs prove N gaps exist. IMPORTANT: Challenge must use SAME evaluation framework from Baseline - creating new evaluation at Challenge is scope expansion, not valid "
        ...
        "Verification — How is completion confirmed? Derivation: from refinement, run full evaluation from baseline. Evidence: all criteria satisfied = no gaps remain. IMPORTANT: After "
        "Refinement, if gaps still exist, iterate: Challenge → Refinement → Challenge → ... until no gaps remain. This is gap-driven iteration. Emit rung labels to begin each rung; "
        "emit completion sentinels (✅ [Rung] complete) to finish each rung. Evidence before claims: for every claim about completion, there must be evidence proving it. Ground complete "
        "may only be emitted after verification confirms no gaps remain. Domain adaptation: for tasks where verification is possible, define evaluation scaffold BEFORE producing "
        "solution — evaluation defined after solution is vacuous (Texas sharpshooter). Baseline defines evaluation framework; Challenge adds N failing criteria; Refinement adds only "
        "enough to satisfy each; Verification runs same framework.for writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to "
        "intent→criteria→analysis→option→stakeholder review→recommendation→decision. The model should derive the appropriate ladder for the task domain. ",
