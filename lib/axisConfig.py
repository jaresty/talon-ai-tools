...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "assertion call expression is one of: property access, index access, await, grouping, or argument separator; (4) the failure line in (g) contains (a) before any line "
        "containing (c), and contains either (d) or the identifier from condition (1) as a substring, or a string literal that appears verbatim as the message argument to the "
        "assertion call expression in the governing artifact's source text — the message argument satisfies this only when it appears as a string literal (not a variable or computed "
        "expression) in the content of a Write or Edit tool call whose path argument matches the governing artifact file path. Bootstrap exception: when no tool-result block in the "
