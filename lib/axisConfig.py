...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "another's next pass. Emit one line 'Message <N>: <from-label> to <to-label> — <content>' per exchange, where <N> increments by one per Message line in transcript order across "
        "the whole dialogue and each Message names exactly one <from-label> and one <to-label>; a message intended for more than one recipient is emitted as multiple Message lines, "
        "one per recipient, each independently Forwarded:-audited. Replies are not required: a Message obligates no answering Message, and one-directional or asymmetric exchange is "
