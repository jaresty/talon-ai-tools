...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "changes, intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap declared by the prior rung, nothing beyond it. Rung-entry gate: "
        "before producing content at any rung, state (a) the rung name, (b) the current gap as a currently-false behavioral assertion, (c) the artifact type this rung requires, and (d) "
        "whether a 🔴 Execution observed: sentinel exists in the current cycle and, if so, whether its output is of type (c) — if no exec_observed exists yet in this cycle, state that "
        "explicitly rather than treating prior-cycle output as satisfying this check; if any of (a)–(d) cannot be stated from the current-cycle transcript, produce it before any other "
        "content at this rung. A rung is satisfied when and only when a tool-executed event appears in the transcript whose output is of that rung’s artifact type. Every 🔴 Execution "
        ...
        "triple-backtick block — the sentinel may not be written before the tool call exists in the transcript; a sentinel with prose content or no delimited block is void — fix by "
        "invoking the tool and re-emitting. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current "
        "gap before any other content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. Each "
        ...
        "equal the declared N, the manifest is not exhausted. From the criteria rung onward, the gap names a specific behavior currently absent or wrong, phrased as a currently-false "
        "assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not. Each rung’s artifact addresses only that gap — not all known "
        "requirements of the task — and is minimal: one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the prose, it "
        "belongs in formal notation instead; if the criterion contains the word ‘and’ it is a conjunction — split before continuing (‘the table renders Display Name’ and ‘the search "
        "bar filters results’ are two criteria, not one); a criterion that names both a structural element and its data source is a conjunction — split before continuing; asserting a "
        "column header is present does not satisfy this rung — add a criterion asserting its content or effect; before writing the criteria artifact, locate ✅ Manifest declared in the "
        "transcript and quote the gap text for this thread verbatim — a criterion written without a citable manifest excerpt is unanchored; narrowing the criterion, substituting a "
        "weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; each criterion must state (a) observable behavior and (b) a falsifying "
        "condition statable without source files, function names, or variable names — if part (b) references implementation internals, the criterion is structural and must be rewritten "
        "before this rung closes; this is a content gate, not a self-check; after emitting the criteria artifact, proceed immediately to formal notation — enumerating remaining "
        "criteria or planning future gap cycles before the current cycle completes is a protocol violation; the next criterion for this thread may not be named until ✅ Thread N "
        "complete has been emitted for the current criterion’s cycle — ✅ Thread N complete is the criterion-emergence gate; the first criteria rung after ✅ Manifest declared emits "
        "exactly one criterion for Thread 1 — emitting criteria for multiple threads at the first criteria rung is a protocol violation; the manifest declared the thread list; the "
        "criteria rung instantiates the first thread’s gap assertion only. The formal notation rung separates behavioral specification from explanation. Formal notation (type "
        ...
        "specification — prose adds context but the behavioral constraints must still be encoded in notation.Formal notation encodes only the criteria declared at the criteria rung — "
        "no additional invariants, no anticipated cases; it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, "
        ...
        "constraint it encodes; a pre-existing test that happens to pass does not satisfy this rung; file reads, grep output, and manual inspection do not constitute executable "
        "validation — the artifact must be invocable by an automated tool; when the declared intent is to modify a test artifact, the test-under-modification is the implementation "
        "artifact for this cycle and the validation artifact must be a meta-test — an executable artifact that fails when the test-under-modification does not exhibit the criterion; "
        "before writing the test, check for an existing test file covering the same module — add there if so; when the artifact is modified, invoke a tool call to read the current test "
        "file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers "
        "current test [verbatim test name as written in the artifact]’ — each prior-failure name must be quotable verbatim from a 🔴 Execution observed sentinel; modification without "
        "carry-forward is a traversal violation; no implementation artifact may appear after modification until carry-forward has been emitted. before emitting ✅ Validation artifact V "
        "complete, confirm via tool call that the artifact path does not pre-exist or is already failing — V-complete may not be emitted without this tool-executed result in the "
        "transcript; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, "
        ...
        "the test is vacuous and must be rewritten; implementation edits may not begin until a red run exists in the transcript for the current cycle; ✅ Validation artifact V complete "
        "may not be emitted unless a 🔴 Execution observed: showing the test failing exists in the transcript for the current cycle; emitting V-complete before seeing a red run is a "
        "protocol violation regardless of whether perturbation was attempted. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation "
        "run observation label is written; if the observed running behavior label is written without a ✅ Validation artifact V complete appearing in the transcript for the current "
        "cycle, the OBR rung is void — return to the executable validation rung and produce the artifact. Before writing the validation run observation rung label, invoke the "
        "validation artifact via a test runner — file reads, grep, directory listings, and static analysis produce static-analysis-type output, not validation-run-observation-type "
        "output, and do not satisfy this gate; a harness error (import failure, syntax error, missing file) is not a red run — the test must have been loadable, its assertions must "
        "have run, and they must have failed; fix the harness error before treating any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation "
        ...
        "failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is "
        ...
        "protocol violation; HARD STOP may not be emitted at the executable validation rung — a harness error at EV requires fixing the harness, not an upward return; if the observed "
        "gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered "
        "and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD "
        "STOP returns. Upon writing the observed running behavior label, re-emit the criterion for this cycle — criterion re-emission is required: emitting the OBR label without "
        "immediately following it with the criterion text is a protocol violation; the tool call is the only valid next action after criterion re-emission; ✅ Thread N complete may not "
        "appear until a tool call exists in the transcript after the observed running behavior label in the current cycle; then immediately start or query the implemented artifact as a "
        "live running process via a tool call and emit 🔴 Execution observed: with its verbatim output — reading a file is not invoking a live process and does not satisfy this rung "
        "regardless of the file's content; the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a non-technical observer of the "
        "running system would see — not the test suite, not process state unless the criterion is specifically about process state; a build output or compilation result never satisfies "
        "this gate regardless of what it names; prefer the most realistic invocation — for a web UI, start the dev server and emit the HTML response body (e.g., via curl or browser); "
        "for a CLI, invoke it directly and emit its output; in-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists; if "
        "the implementation has no directly invocable artifact, invoke any non-test consumer that starts or queries a live process and demonstrates the declared behavior — reading "
        "build artifacts, compiled output, generated HTML, or any static file is not a valid fallback regardless of what the file contains; if no live process can be started or "
        "queried, open a new gap cycle to make the artifact directly invocable before continuing; a 🔴 Execution observed: block that is empty or blank, or contains only static-analysis "
        "output (file reads, grep results, directory listings), does not satisfy this gate at any rung — the block must contain output produced by running the artifact; if the declared "
        "gap names a real network endpoint, the OBS must include verbatim output showing a real HTTP request or response — a test run whose network calls are mocked does not satisfy "
        "this gate; before any tool call touching the filesystem at this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the rung "
        "label cannot be located in the transcript, the tool call is prohibited; the provenance statement does not replace the tool call — the tool call must follow it in the same "
        "response; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: with non-empty verbatim output appears in the transcript after the observed running behavior "
        "label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol violation; implementation edits, new files, and code changes are not "
        "permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written after the most recent 🟢 Implementation gate cleared "
        "in this thread and a 🔴 Execution observed: sentinel appears immediately after it that directly demonstrates the criterion — a test pass is not a demonstration; if the 🔴 "
        "Execution observed: output does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules — ✅ Thread N "
        "complete may not be emitted after an OBR 🔴 Execution observed: that does not directly demonstrate the criterion; if the 🔴 Execution observed: at the observed running behavior "
        "rung contains test runner output (pass counts, test names, duration summary), it does not satisfy the OBR gate — re-invoke the implemented artifact directly before continuing; "
        "✅ Thread N complete may not be emitted after an OBR 🔴 Execution observed: that shows test runner output; no implementation file was created or modified for this thread’s gap "
        "in this cycle means the gap was already closed (return to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the "
        "transcript with a written reason — dismissing a failing test as unrelated without an explicit written acknowledgment is a protocol violation; ✅ Thread N complete may not be "
        "emitted unless a 🔴 Execution observed: from a full test suite run exists in the transcript after the most recent OBR tool call in this thread — the only valid next action if "
        "no such result exists is the tool call that runs the suite; once ✅ Thread N complete is emitted no further output in that cycle is valid; Before writing the criteria, formal "
        ...
        "unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared licenses the first edit "
        "— it does not complete the thread; after emitting it, the executable implementation rung, the observed running behavior rung, and their required tool-executed events must "
        "still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may not "
        "appear until the observed running behavior rung has fired for this thread. Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior "
        "to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the "
        ...
        "immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — not "
        "from a prior cycle — to exist in output; the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless "
        "of whether it produced a failure. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, "
        "renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read the prose rung output via a tool call to confirm "
        "every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry containing a verb or phrased "
        "as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the criteria rung, not the manifest; ✅ Manifest declared may "
        "be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not "
        ...
        "🔴 Gap:; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; "
        "every descent through executable implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward "
        "return. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upward returns follow the failure class: if the implementation did "
        "not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; observation cannot be produced is a "
        ...
        "open a new gap cycle before producing the final report. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each "
