...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "exactly one witness classification for every previously inventoried executable assertion: 'Property [N] assertion [M] witness: Failure — <verbatim failure text>' if the "
        "assertion produced the observed failure, 'Property [N] assertion [M] witness: Success' if the assertion passed, or 'Property [N] assertion [M] witness: Unreached' if "
        "execution did not reach the assertion; every inventoried assertion must receive exactly one witness classification; no witness may be emitted for an assertion not present in "
        "the assertion inventory, and no inventoried assertion may lack a witness; at least one inventoried assertion must receive witness classification 'Failure' — a guard execution "
        "in which no inventoried assertion is classified 'Failure' does not satisfy step (4), regardless of whether a Failure: line was produced; the quoted failure text for every "
        "'Failure' witness must be a verbatim substring of the corresponding Failure: tool-result line; if the observed failure cannot be attributed to one or more inventoried "
        "assertions, revise the violating state or the regression guard and repeat from step (2); after all assertions are witnessed, emit 'Assertion witnesses: complete — <count> "
        "assertions witnessed' or 'Assertion witnesses: incomplete — <reason>'; if incomplete, revise the violating state or guard and repeat from step (4); no subsequent step may "
        "occur until 'Assertion witnesses: complete' has been emitted; (5) identify the governed artifact for property [N] — the artifact whose modification satisfies property [N]; "
        "depending on the property, this may be the implementation, the regression guard, a configuration file, or another explicitly governed artifact; modify the governed artifact "
        ...
        "Unobservable: line. A response that omits any required sentinel, performs a step before emitting its required quoted artifact, modifies the governed artifact to satisfy "
        "property [N] before a quoted Failure line appears, omits any executable assertion from the assertion inventory, or leaves a property without an automatic regression guard "
