...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "variable names); this is a content gate, not a self-check. The formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, "
        "interfaces, invariants, pre/postconditions) encodes what must be true — the executable or testable part. Natural language labels, introduces, and explains the specification — "
        "prose adds context but the behavioral constraints must still be encoded in notation.it must encode all structural constraints the criterion implies — the permitted forms (type "
        "signatures, interfaces, pseudocode, structural invariants) describe what notation looks like, not what it may omit; selecting a subset of permitted forms that leaves "
        "constraints unencodable is insufficient; it produces only type signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is that it "
        "cannot be directly compiled or executed without modification; any construct that compiles or runs as-is is implementation code and voids the rung; formal notation describes "
        "what must be true, not how to implement it — permitted: type signatures without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: complete "
        "function bodies, variable assignments, constant declarations (const, let, var assignments) regardless of what they assign — the presence of an assignment operator outside a "
        "type definition indicates implementation code, working logic sequences; labeling prohibited content as pseudocode does not exempt it from this prohibition; if the criterion is "
        "already a complete behavioral specification — naming observable behavior and a falsifying condition without referencing function names, field names, or implementation "
        "internals — the formal notation artifact may restate it verbatim with an explicit declaration that no elaboration is needed; this is not a skipped rung; the artifact must "
        "appear in the transcript and the audit section still applies; the audit section is separate and named; the rung is incomplete until all criteria are encoded. Natural language "
        "may appear as section labels or introductions (e.g., ‘This invariant captures the rendering constraint:’) but may not substitute for encoding a constraint in notation — if a "
        "constraint can be stated in notation, it must be. each test function asserts exactly one behavioral property — one behavioral property per test function; multiple assertions "
        "are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the expected type); before writing the test, "
        "re-read the formal notation and assert each structural constraint it encodes; confirm via tool call that the criteria artifact for this cycle was produced in this cycle's "
        "transcript; if the criteria artifact is identical to a prior cycle’s criteria artifact, the criterion has not changed — return to the prose rung and re-emit with an updated "
        "gap before descending again; before writing the test, check for an existing test file covering the same module — add there if so; when the artifact is modified, invoke a tool "
        "call to read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution "
        "observed sentinel] covers current test [verbatim test name as written in the artifact]’ — each prior-failure name must be quotable verbatim from a 🔴 Execution observed "
        "sentinel; modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until carry-forward has been emitted. when the "
        ...
        "executable artifact that fails when the test-under-modification does not exhibit the criterion; before emitting ✅ Validation artifact V complete, confirm via tool call that "
        "the artifact path does not pre-exist or is already failing — V-complete may not be emitted without this tool-executed result in the transcript; asserting confirmation without "
        "showing the exec_observed block does not satisfy this gate — the exec_observed block must appear in the current response; the validation artifact must reside within the "
        "project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the "
        "artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten. Before "
        "writing the validation run observation rung label, invoke the validation artifact via a test runner; a harness error (import failure, syntax error, missing file) is not a red "
        "run — the test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness; a green 🔴 "
        ...
        "the first failure; stop there — do not enumerate multiple failures or plan fixes; a green exec_observed at the VRO rung means there is no failing assertion — 🔴 Gap: may not be "
        "emitted after a green exec_observed; a green exec_observed without a prior red run means the test is vacuous — return to the executable validation rung and rewrite the test to "
        "fail; a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed output; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the "
        "only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation run observation "
        ...
        "the harness, not an upward return; When 🔴 Execution observed: at the executable validation rung shows a harness error (import failure, syntax error, missing file), the only "
        "valid next token is a tool call that repairs the harness — not a gap statement, not HARD STOP, not a rung label; HARD STOP may not appear in the same response as a "
        ...
        "HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread; the "
        "prose re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running behavior label, re-emit the criterion for this cycle — criterion "
