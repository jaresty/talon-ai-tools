...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "appears. P7 (Upward faithfulness): any artifact at any rung must be faithful to the rung above it; the space of valid implementations permitted by a lower rung may only be "
        "equal to or smaller than the space permitted by the rung above it; expanding the permitted space is a faithfulness violation; tool execution at any rung is permitted. P8 (Rung "
        "validity test): a rung is valid if and only if a human reviewer, given only that rung’s artifact and no other context, can determine whether the next rung’s artifact is "
        "faithful to the intent without consulting any prior rung; a rung that fails this test is narration, not a rung; the ladder must be minimal — removing any rung must leave some "
        "faithfulness evaluation impossible without consulting prior rungs; a rung that can be removed without loss must be removed. P9 (Information density preservation): each rung "
        "must encode the same or higher quality of information as the rung above it — sufficient to represent the intent as fully as possible in its artifact type; a rung whose "
        "artifact loses information present in the rung above it is a protocol violation. P10 (Three-part completeness): a session is complete only when it has produced: (a) a manual "
        "observation confirming the behavior is present, (b) automation that verifies the behavior is consistently maintained, and (c) for behavior-change sessions, an artifact that "
        "directly implements the change; all three must be present. P11 (Immediate lowest-rung observation): whenever a behavioral change is made that would be visible at a higher "
        "rung, it must be observed immediately at the lowest rung that allows observation of that change; deferring observation to a higher rung when a lower rung could have surfaced "
        "it first is a protocol violation. P12 (Completeness slice): after declaring intent, a slice of behavior is chosen to carry through the ladder; the slice must be declared "
        "explicitly before descent begins; descending without a declared slice is a protocol violation; the slice is exactly one independently testable behavior per thread per cycle — "
        "a criteria artifact asserting more than one behavior is a conjunction and is a protocol violation; a criterion containing the word “and” is presumptively a conjunction unless "
        "both halves jointly constitute a single indivisible observable state. P13 (Observation-first, observation-last): a session begins by observing current behavior manually and "
        "declaring what the intent is believed to be; a session ends by repeating that observation to confirm intent has been met; declaring completion without a closing observation is "
        "a protocol violation; the opening observation must be tool-executed — the intent declaration must be derivable from that tool-executed output; intent declared without a "
        "tool-executed opening observation is an evidential violation under P14. P14 (Evidential authority): only tool-executed events have evidential standing; inference, prediction, "
        "prior-cycle output, and model-generated descriptions of tool output have none, regardless of accuracy; a rung gate is satisfied if and only if a tool-executed event appears in "
        "the current-cycle transcript whose output is of that rung’s artifact type; a rung gate is not a description of evidence — it is a blocker: no completion sentinel for that rung "
        "and no label for the subsequent rung may appear while the gate is unsatisfied; before emitting a completion sentinel or the next rung label, verify the gate condition is met — "
        "emitting either before the gate condition is met is a protocol violation that voids the rung; a completion sentinel is a closing marker — it appears after the rung’s artifact "
        "is complete and its gate condition is verified; emitting a completion sentinel before the artifact it closes is a protocol violation that voids both the sentinel and the rung; "
        "after a rung label is emitted, the only valid content before the completion sentinel is the rung’s artifact — prose commentary, planning text, debugging narration, and content "
        "of any other artifact type between the rung label and its completion sentinel is a protocol violation that voids the rung; 🔴 Execution observed: is only valid when a tool call "
        "was made in the current response immediately before it — a sentinel emitted without a preceding tool call is a fabrication — it voids the rung in which it appears regardless "
        "of whether its text resembles tool output; ✅ Ground entered must be the first emitted content of any response that begins a ground session — any artifact, reasoning, or "
        "content of any type produced before ✅ Ground entered is a pre-entry violation — it voids the sentinel and all work that follows in that response. P15 (Cycle identity): "
        "evidence is valid only within the cycle in which it was produced; a cycle opens when ✅ Ground entered is emitted for a given thread and closes at Thread N complete or an "
        "upward return; evidence from a prior cycle, a different thread, or a different gap does not satisfy any gate in the current cycle regardless of type match; re-emitting ✅ "
        "Ground entered for one thread does not affect the cycle identity of any other thread; an upward return is valid only when a prior cycle for the current thread exists in the "
        "current session — a comparison gate requires a prior cycle artifact to compare against; invoking an upward return in a thread’s first cycle is a protocol violation. P16 "
        "(Provenance): a gate is satisfied only by a tool call made in direct response to the gap declared at the immediately prior rung in the current cycle; a tool call made for a "
        "different purpose, targeting a different gap, or produced in a prior cycle does not satisfy any gate regardless of type match. P17 (Derivation chain): each artifact must be "
        "derived from the prior rung’s actual content — not from the original intent reconstructed from memory; a skipped rung voids all artifacts below it; form changes between rungs, "
        "intent does not; divergence from intent is only detectable at the rung where the prior artifact’s specificity was insufficient — breaking the chain makes that detection "
        "impossible; an artifact at any rung addresses only the gap declared by the prior rung — scope does not expand between rungs; an artifact that addresses behaviors beyond the "
        "declared gap is a derivation violation that voids the rung; an artifact may not retrieve specificity from a rung above its immediate predecessor — if a lower rung requires "
        "more specificity than the rung above it provides, the remedy is an upward return to that rung, not retrieval from a higher rung. P18 (Continuous descent): once descent begins, "
        "advance through every feasible rung without pausing for user confirmation; each rung may not be skipped or combined with another; all rung transitions are continuous within "
        "the same response unless the derived ladder specifies a protocol-defined stop at that rung; response length is never a valid reason to stop between rungs — if the full descent "
        "cannot fit in one response, continue from the current rung in the next response without re-emitting completed rungs. P19 (Thread sequencing): when multiple behavioral gaps are "
        "declared, they form a manifest; each gap is a thread; all rungs for Thread N must complete before any content for Thread N+1 may appear; threads not declared in the manifest "
        "may not be created mid-session; a thread is complete only when all its rungs have fired in the current cycle and a completion sentinel has been emitted. Ladder derivation "
        "rung: before descending, derive the rung table for this session by applying P1–P19 to the declared intent; produce a table with columns: rung name | artifact type | gate "
        "condition (what tool-executed event satisfies the gate) | void condition (what invalidates the artifact) | faithfulness test (what a reviewer needs from only this artifact to "
        "evaluate the next rung) | permitted tool calls (what file modifications are allowed at this rung; all rungs permit read, observe, and non-modifying run commands; file "
        "modifications are constrained to the artifact type of that rung); verify the table satisfies P8 minimality — remove any rung that does not; descent begins immediately after "
        "the table is produced without pausing for confirmation; ✅ Manifest declared is the closing sentinel of the ladder derivation rung — the required emission order is: rung table, "
        "then ✅ Manifest declared; a manifest emitted without a preceding rung table is a protocol violation; the rung table is the gate on all subsequent rung activity. For software "
        "behavioral change, the standard derivation begins with a tool-executed observation of current behavior from which intent is declared, then produces: observed running behavior "
        "(baseline) → prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running behavior (confirmation). A "
        "derived ladder that omits any of these rungs for a software behavioral change task must cite which principle makes that rung unnecessary. The criterion is exercised only when "
        "the automated validation suite runs to completion and individual assertions fail; execution halted before reaching the assertions (by infrastructure failure, import error, or "
        "any other cause) does not exercise the criterion — the executable-implementation artifact for such a halt resolves only the infrastructure gap (minimum change sufficient to "
        "allow the suite to reach the assertions); the criterion has not been exercised until individual assertions fail in a subsequent cycle. Outputting a rung label is what begins "
        "that rung — it is not a heading or annotation; a rung whose label has not been output has not begun. When beginning mid-ladder, locate the highest already-instantiated rung, "
        "update it, then descend. Sentinel formats — ✅ Ground entered — prose rung begins [gate: first content emitted in this response; no artifact, code, prose, or reasoning may "
        "precede this sentinel — preceding content voids the session]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in current "
        "response; rung table precedes this sentinel]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] [gate: tool call made in the "
        "current response immediately before this sentinel; verbatim output in triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed with "
        "non-empty failing output precedes this token in current response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: "
        "exec_observed showing test suite failure + gap in current cycle; criterion identical to prior cycle criterion for this thread]; 🟢 Implementation gate cleared — gap cited: "
        "[verbatim from 🔴 Execution observed] [gate: exec_observed showing test suite failure + gap in current cycle; valid only at the rung whose artifact type is executable "
        "implementation]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one criterion in this rung's artifact; criterion contains no conjunction; formal notation rung "
        "label may not be emitted until this sentinel has been emitted]; ✅ Validation artifact V complete [gate: test file written via tool call in current response; pre-existence "
        "check tool call result present]; ✅ Thread N complete [gate: exec_observed at the observed-running-behavior rung directly demonstrating criterion in current cycle; passing test "
        "suite run after that tool call; valid only at the observed-running-behavior rung]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels "
        "equals N in Manifest declared]; Carry-forward: [list which original failures cover which current tests] [gate: prior failure name quotable verbatim from a prior exec_observed "
