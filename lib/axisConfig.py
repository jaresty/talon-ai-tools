...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "executable validation (a file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target the declared gap and reveal whether it has been closed; file reads, grep output, and manual inspection do not constitute executable validation regardless of label; pre-existing artifacts not targeting the gap do not satisfy this rung) → "
