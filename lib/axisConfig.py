...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "emitting it.  Selection may be enforced by delegation: when the inadmissible frames are separable from the minimal statement of the task — accumulated reasoning that can be "
        "left unforwarded — invoke a subagent-spawning tool now to reason under the disposition in a context that never received the inadmissible frames; the invocation appears as a "
        "tool-use block and the reconcile step cites its result block. Decide separability by stating the task minimally and checking whether an inadmissible frame still appears: if "
        "omitted, separable, delegate; if still present, briefing the delegate re-imports it, so reason in-thread. When delegating, emit one 'Forwarded: <item> — [requirement | "
        "inadmissible-frame]' line per item entering the delegate's prompt; an inadmissible-frame item must be absent from that input or appear only as an explicit question, and one "
        "appearing declaratively is passed as an assumption.  (3) Reason: reason about the subject using the default and admit frames, excluding the inadmissible frames.  (4) "
