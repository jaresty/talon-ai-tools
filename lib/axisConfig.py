...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "interpretation. For this condition, a sub-property constrains an internal subject when determining whether the property is satisfied or falsified requires observing state, "
        "structure, or mechanism not accessible under the chosen interpretation's accessible-observation boundary. A sub-property is observable under the interpretation when its truth "
        "value can be determined solely from the outputs, behaviors, states, and interactions that the interpretation makes accessible to the evaluating party without inspecting the "
        "implementation. Step 4 — Recurse or mark atomic. If the split succeeds, replace the original property with the accepted canonical sub-properties and apply this same recursive "
        ...
        "satisfy this requirement. Immediately after the completeness check, attempt to identify one explicit request constraint that is not represented by any retained property. If "
        "such a constraint exists, emit '§ properties complete? no' — add one new property covering that explicit constraint, perform the required split testing, and repeat the "
        "completeness check. Only when every explicit request constraint is logically entailed by the conjunction of the retained atomic property expressions may the response emit '§ "
        "properties complete? yes' — a '§ properties complete? yes' line without a preceding completeness check does not satisfy this protocol. After completeness has been established, "
        "verify observational independence. A retained property is observationally independent if there exists at least one admissible construction that violates that property while "
        "satisfying every other retained property. Any retained property that is not observationally independent is observationally redundant and must be removed or merged, provided "
        "doing so does not reduce coverage of the explicit request constraints. Any property produced by a merge is a new property and must enter the recursive decomposition procedure "
        "(Steps 1–4) as a newly derived property before being added to the retained set. If a merge or removal changes the retained property set, repeat the atomicity, completeness, "
        "and observational-independence procedures. The Ground properties block is complete only when all retained properties simultaneously satisfy: they are derived from the chosen "
        "interpretation; they are atomic; they collectively cover every explicit request constraint; they are observationally independent; they introduce no out-of-scope constraints, "
        "including internal mechanism constraints not explicitly required by the chosen interpretation. When all conditions are met, emit '§ ground complete' as the final line of the "
        "Ground properties block — this sentinel may not appear before '§ properties complete? yes' in the transcript. Any test, implementation step, tool call, or task reasoning that "
        "appears before '§ ground complete' does not satisfy this protocol; if task reasoning begins and '§ ground complete' has not yet appeared, emit the Ground properties block in "
        "full and '§ ground complete' before continuing.",
