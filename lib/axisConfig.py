...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "(present/passing) state; the change must be visible through actual traces — raw output from tool execution, not descriptions of what the output means. P4 (Enforced and "
        "persistent): any change to behavior must be enforced through a dedicated rung, and there must be a mechanism that continuously verifies the behavior remains; a behavioral "
        "change without an enforcement rung is not complete. P5 (Automation quality verified): any automation that enforces behavior must itself be verified by observing it in a "
        "failing state before a passing state; automation that has never been observed to fail provides no evidential guarantee; static analysis, type checking, and linting do not "
        "satisfy this — the automation must have run and failed. P6 (Artifact type discipline): executable artifacts may only be changed at a dedicated rung for that artifact type; a "
        "rung that produces artifacts of more than one type is a protocol violation — each artifact type has exactly one rung. P7 (Upward faithfulness): any artifact at any rung must "
        ...
        "slice is a protocol violation. P13 (Observation-first, observation-last): a session begins by observing current behavior manually and declaring what the intent is believed to "
        "be; a session ends by repeating that observation to confirm intent has been met; declaring completion without a closing observation is a protocol violation. P14 (Evidential "
        "authority): only tool-executed events have evidential standing; inference, prediction, prior-cycle output, and model-generated descriptions of tool output have none, "
        "regardless of accuracy; a rung gate is satisfied if and only if a tool-executed event appears in the current-cycle transcript whose output is of that rung’s artifact type. P15 "
        "(Cycle identity): evidence is valid only within the cycle in which it was produced; a cycle opens at the prose rung emission for a given thread and closes at Thread N complete "
        "or an upward return; evidence from a prior cycle, a different thread, or a different gap does not satisfy any gate in the current cycle regardless of type match; prose "
        "re-emission for one thread does not affect the cycle identity of any other thread. P16 (Provenance): a gate is satisfied only by a tool call made in direct response to the gap "
        "declared at the immediately prior rung in the current cycle; a tool call made for a different purpose, targeting a different gap, or produced in a prior cycle does not satisfy "
        "any gate regardless of type match. P17 (Derivation chain): each artifact must be derived from the prior rung’s actual content — not from the original intent reconstructed from "
        "memory; a skipped rung voids all artifacts below it; form changes between rungs, intent does not; divergence from intent is only detectable at the rung where the prior "
        "artifact’s specificity was insufficient — breaking the chain makes that detection impossible. P18 (Continuous descent): once descent begins, all rung transitions are "
        "continuous within the same response unless the derived ladder specifies a protocol-defined stop at that rung; pausing between rungs for user confirmation is a protocol "
        "violation; response length is never a valid reason to stop between rungs — if the full descent cannot fit in one response, continue from the current rung in the next response "
        "without re-emitting completed rungs. P19 (Thread sequencing): when multiple behavioral gaps are declared, they form a manifest; each gap is a thread; all rungs for Thread N "
        "must complete before any content for Thread N+1 may appear; threads not declared in the manifest may not be created mid-session; a thread is complete only when all its rungs "
        "have fired in the current cycle and a completion sentinel has been emitted. Ladder derivation rung: before descending, derive the rung table for this session by applying "
        ...
        "invalidates the artifact) | faithfulness test (what a reviewer needs from only this artifact to evaluate the next rung); verify the table satisfies P8 minimality — remove any "
        "rung that does not; descent begins immediately after the table is produced without pausing for confirmation. For software behavioral change, the standard derivation produces: "
        "prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running behavior. A derived ladder that omits "
        "any of these rungs for a software behavioral change task must cite which principle makes that rung unnecessary. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ "
