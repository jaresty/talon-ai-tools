...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Failure or Unobservable line must match an enumerated Assertion line exactly, and an enumerated assertion with no matching outcome line leaves coverage incomplete. To witness "
        "an assertion, construct an admissible state in which the property it guards is genuinely absent and execute the guard against it. A Failure observation is valid only when the "
        "state satisfies all of: the governed symbol is present and executes; that assertion's property is violated; every other assertion's property could simultaneously hold; and "
        "the observed failure cause is attributable to that assertion alone, distinct from every other assertion's cause. A successful compilation, a build succeeding, an unrelated "
        "verification, or a whole-symbol or undefined error does not witness an assertion, because such an outcome fails identically regardless of which property is absent. If the "
        "artifact already satisfies the property, construct a temporary violating modification, execute the guard against it, emit the Failure from that tool-result, then restore the "
        "original state. The temporary violating modification must violate exactly one assertion's property while the others still pass; a single modification that breaks the whole "
        "artifact and fails every assertion at once does not discriminate them, so it witnesses none. For every executable assertion the tool-result must produce an outcome "
        "attributable to that assertion alone; a whole-guard execution that does not yield per-assertion outcomes is insufficient regardless of whether the model recognizes the "
        "insufficiency — sufficiency is judged from the tool-result, not from the model's assessment. A guard whose framework already reports per-assertion results satisfies this "
        "directly; otherwise any execution mechanism that yields per-assertion outcomes — for example a temporary observation harness executing each enumerated assertion — may be "
        "used, and each Failure is derived from that tool-result. An assertion that no admissible perturbation can make fail is a guard defect: strengthen the guard and repeat. "
        "'Unobservable: assertion … — structural' is permitted only when the property has the artifact's own text or structure as its primary subject, so no behavioral violating state "
        "exists; it is never satisfied by having searched for a violating state and found none. A guard is a blind spot whenever an admissible state exists in which its pass-or-fail "
        "outcome is not equivalent to whether the property holds — check three: (a) proxy or non-exercising guard checking a surrogate; (b) durability — a guard gone at the moment it "
        "would need to fire; (c) ephemeral guard not committed to a location that re-runs. Verdict-follows-execution governs both gates: when tool calls are available, every "
        "'Overreach:' verdict and every 'Failure:' observation is valid only when it immediately follows a tool-result block that mechanically produces it; a verdict emitted from "
        "description or analysis alone — reasoning about what a probe or run would show, without a preceding tool-result — does not satisfy this token. A structural Unobservable "
        "assertion is exempt, because a property about the artifact's own text or structure has no execution to anchor to. Each 'Failure: assertion' line must be accompanied by a "
        "witness line that makes its validity checkable rather than merely asserted: 'witness: symbol present; property [<this assertion>] violated; other assertions pass; cause is a "
        "genuine failure' — each clause naming the observed evidence for it. A 'Failure:' line whose observed cause is an observed outcome that indicates success — text such as 'ok', "
        "'PASS', 'passed', or 'succeeded' — does not count as a Failure, because a passing outcome does not witness the absence of the property. A 'Failure:' line without a passing "
        "witness line does not count toward Coverage. 'Coverage: complete' is valid only when every enumerated assertion has a verbatim-matching observed 'Failure:' line with its "
        "witness line, or an 'Unobservable: … — structural' line, and no 'Overreach: found' remains open.",
