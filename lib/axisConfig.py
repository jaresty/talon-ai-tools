...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "form": {
        ...
        "answer and naming the implication it leaves unresolved — the question the next concept answers (evaluator check: the quoted phrase appears verbatim in the current answer); then "
        "for at least one concept per quiz, a Misconception: line naming the specific incorrect belief that would lead a reader to predict the wrong answer, and a Why: line stating the "
        "structural reason that belief is wrong; then an association sentence — before writing it, name the numbers already used in prior association sentences — using the format '#N "
        ...
        "understanding it enables). If a Predict: line is present and no gap-reveal sentence was written for this concept, emit '✓ [concept name]' to mark that concept confirmed. Continue "
        "until every concept in the opening list has either a '✓ [concept name]' marker or a gap-reveal sentence in the transcript, or the user explicitly declines to continue; then close "
        "with a terminal declaration listing all confirmed concepts. With an output-exclusive channel, structure the output as a quiz document with each question immediately before its "
        "answer per topic.",
