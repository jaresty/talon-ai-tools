...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "write '§0 observed' when a tool-result block reflecting live system execution is present above it — '§0 observed' must not appear before such a block is present; (1) derive "
        "the governing goal: '## Governing goal: [text]' must not appear before '§0 observed' in the transcript; `[text]` must appear verbatim as a substring of the §0 tool-result "
        "block above — a `[text]` value not present as a literal string in that block does not satisfy §1; immediately below the governing goal heading, before the means-test, extract "
        ...
        "'§1 goal derived', derive dimension candidates: for each alternative means named in the means-test, write one candidate — a response property that would be absent in a "
        "response achieving '[text]' by a different means; when one candidate exists per alternative means, write '§1b candidates'; (2) derive behavioral dimensions: '## Behavioral "
        "dimensions' must not appear before '§1b candidates' in the transcript; derive at least two dimensions; each dimension names a property of the response (not an artifact, state, "
        "or fix description) such that a response could satisfy it while failing at least one other listed dimension; each dimension must trace to one item from the §1a decomposition "
        "list or one item from the §1b candidates list — a dimension that traces only to '[text]' as a whole does not satisfy this requirement; each dimension ends with '[observable: "
        "<string>]' where '<string>' is a literal string or structural pattern whose presence in the response constitutes satisfaction — if the response contains no tool-result blocks "
        "above §2, write '[observable: prose]' instead; a dimension that names an entity, a fix outcome, or a UI element is not a valid dimension regardless of provenance; derivation "
        "is unbounded: add dimensions, challenge each against the reader-uncertainty test, and revise until every dimension is independently falsifiable; when every dimension carries "
        "an '[observable:]' tag, write '§2 dimensions closed'; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the "
        ...
        "permitted', as a substring of the response text produced in this turn; when every covered dimension cites such a substring, write '§4 coverage verified'; any assistant turn "
        "that contains a question directed at the user or no further planned tool calls, after '§ implementation permitted' has appeared in the transcript, must contain '## Completion "
        "check' before that question or closing text — a turn whose question or closing text appears before '## Completion check' does not satisfy ground; (5) enumerate escape paths: "
        "'## Path enumeration' must not appear before '§4 coverage verified' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal "
