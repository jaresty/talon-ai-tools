...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "precedes and is not itself an artifact. Every artifact derives from I through the prior rung — form changes, intent does not. V is a constraint artifact self-contained to "
        ...
        "faithful derivation of the prior rung — the form changes, the intent does not. R2 (rung criterion): an artifact is a rung iff upward-faithful (derived from the prior rung "
        "without adding unstated constraints) and downward-sufficient (self-contained to evaluate the next artifact without consulting I); domain phases are not downward-sufficient — "
        "writing tests for 'API layer' requires knowing what the API must do, which comes from I, not the phase name. R3 (observation terminates): every thread descends to observed "
        "running behavior; only tool-executed output with a declared gap satisfies the execution gate; skipped tests are not observations; observed running behavior must be traceable "
        "to the behavioral gap declared in the validation run observation — verifying that a process started or a URL responded is not sufficient; for UI artifacts, a server response "
        "does not confirm rendering; for APIs, an HTTP 200 does not confirm correct data; the observation must demonstrate the declared behavioral intent was satisfied. A rung is "
        "complete when and only when its artifact has been produced — not when it has been listed, planned, or described. A rung is not achievable when the domain provides no standard "
        "artifact type for it; this must be stated explicitly with justification — convenience, anticipated outcome, or prior knowledge do not make a rung not achievable. ground is a "
        "Process method — the task governs the character of each rung's output but not the process structure; the manifest, rung sequence, and execution gates are mandatory regardless "
        "of which task or other tokens are combined with ground; completeness governs rung depth, not rung existence. Executable verification is required only for executable artifacts; "
        "prose artifacts do not require execution to be complete. Formal notation must satisfy R2: every behavioral constraint from the criteria rung must be re-expressed in the "
        "notation — not just interface shape; type signatures or schemas that capture structure without encoding invariants do not satisfy this rung; before advancing from formal "
        "notation, enumerate each criterion from the criteria rung and confirm it is encoded as a behavioral invariant — list any unencoded criteria explicitly; the rung is incomplete "
        "until all are addressed. Boundary: the manifest is the first and only output before the manifest-complete sentinel — no rung work, planning text, or content of any kind may "
        "appear before it; when I cannot be declared from context alone, observation of existing code or running behavior sufficient to establish I is required before the manifest — "
        "this is I-formation, not rung work; skipping I-formation when context is insufficient is an error, not a permitted shortcut — it produces a specification derived from assumed "
        "rather than actual constraints, and every downstream artifact will be wrong in ways that only surface at executable validation or observed running behavior, by which point "
        "multiple rungs must be discarded and rederived; when I-formation is required, output ‘✅ I-formation complete’ on its own line after declaring I and before the rung manifest — "
        "the manifest may not appear before this sentinel; exploration beyond what is needed to declare I belongs as the first rung of the manifest, not pre-manifest. Foundational "
        "constraint: a symbol is not the state it represents — a rung label during execution must be emitted at every rung transition; absence of a rung label at a transition is a "
        "traversal violation, not a stylistic omission; the label marks the point where the artifact begins — it is not a section heading for planning, description, or exploration "
        "about the rung; no content other than the artifact itself may appear between a rung label and the artifact it precedes. Eagerness to implement is the primary failure mode — an "
        "implementation produced without passing validation is invalid and will be discarded; the shortest path to a valid implementation is strict rung adherence, not shortcuts; every "
        "skipped rung produces output that must be thrown away. When beginning mid-ladder, first locate the highest already-instantiated rung and update it to reflect the intended "
        ...
        "observed running behavior. A gate is a conversation-state condition: open when and only when the required event has occurred in this conversation for this thread. Prior "
        "knowledge, anticipation, and model reasoning cannot satisfy any gate regardless of accuracy. Underlying all compliance failures is one epistemological error: substituting "
        "model knowledge for conversation events. For executable rungs, emit 🔴 Execution observed: [verbatim tool output — content composed without running the tool is invalid] then 🔴 "
        "Gap: [what the verbatim output reveals] on their own lines before any implementation artifact. No implementation artifact — including planning text, code blocks, or tool calls "
        "— may appear before valid Execution observed + Gap sentinels; if any implementation content appears without these sentinels immediately preceding it, it is invalid and must be "
        "discarded before the tool is run. Before producing implementation code, emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]. The quote must be "
        "verbatim from the 🔴 Execution observed sentinel of this thread; quoting anticipated output or a prior thread's observation is invalid. The 🔴 sentinel format is reserved "
        ...
        "observed failure that reveals the declared gap — a passing run before gap-targeted implementation has been written is a gap signal, not permission to descend: either the "
        "behavior is already implemented (verify this; the thread may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected before "
        "descent continues. A build error or compile failure does not satisfy this rung — the validation artifact must compile and execute before its output can reveal a behavioral "
        "gap; a failure to compile means the artifact is incomplete and must be corrected before the gate can open. When the lowest V is complete, output ‘✅ Validation artifact V "
        "complete’ on its own line immediately after the validation run observation rung has declared a gap and before the executable implementation rung begins — this sentinel may not "
        ...
        "requiring its own manifest and descent; adding it inline without a new manifest is a scope violation regardless of how it is labeled. Upward revision is always permitted when "
        "a gap is observed between prior understanding of I and something encountered via direct interaction with reality, code, or a stakeholder. Upward revision must be signposted "
        "with: what was observed, which rung is being revised, and why. It is never permitted to change I without first observing a gap in V that derived it. Changing I requires "
        ...
        "deficient — a failure at rung N is evidence that rung N-1 (or higher) did not correctly derive or constrain it; failing to correct the highest broken rung means every artifact "
        "below it is derived from a deficiency that will resurface at each subsequent rung — correction is not a detour but the only path to a valid artifact at the current rung; "
        "before correcting any higher rung, emit what was observed at the current rung, which higher rung is being corrected, and why — a correction without this observation record is "
        "invalid and indistinguishable from enrichment; find the highest broken rung, correct it, and rederive every artifact below it before redescending; this is correction, not "
        "enrichment — the corrected rung must still be faithful to I; if the broken rung is I itself, surface as a revision signal and stop the current thread. Intent precedes its "
        ...
        "Manifest exhausted — N/N threads complete may only appear after all threads have emitted their completion sentinels and the reconciliation report has been produced.",
