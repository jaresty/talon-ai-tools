...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "is witnessed only by a pair of A-observations: an execution in which A's own guard-defined result is a failure, and one in which A's own result is a pass. An A-observation is "
        "an execution of the committed guard in which assertion A is evaluated and produces its own guard-defined result. The guard defines the observable predicate for A: an A-pass "
        "result witnesses the property present, an A-fail result witnesses it absent — so there is no separate model claim that a state 'violates' or 'satisfies' the property; the "
        "guard is the measurement mechanism and its A result is the observation. The failure (A-fail) execution must immediately precede the 'Failure:' line (it is the producer); the "
        "passing (A-pass) execution is the contrast evidence and may appear elsewhere in the transcript. Any execution that does not produce an A result is not an observation of A and "
        "cannot witness A, regardless of the cause or form of the execution's outcome — this covers execution errors, unrelated assertions failing, a harness that exits without "
        "reporting A, and any other run in which A itself was not evaluated; only an A-fail result can supply the violating observation, and only an A-pass result the satisfying "
        "contrast. Both A-observations must be produced by the committed guard artifact, and the guard's output must identify A (by test name, subtest path, or assertion message) so "
        "the pair is bound to this enumerated assertion rather than another. If the artifact already passes A, construct a temporary modification that makes A alone fail while every "
        "other assertion still passes, execute, emit the Failure from that tool-result, then restore; a modification that fails every assertion at once discriminates none. Emit "
        ...
        "A-fail/A-pass execution pair or an 'Unobservable: … — structural' line, and no 'Overreach: found' remains open.",
