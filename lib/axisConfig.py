...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rung; before writing the test, check whether an existing test file covers the same module and add there if so; when the validation artifact is modified, invoke a tool call to "
        "read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution "
        ...
        "sentinel; modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until carry-forward has been emitted. before "
        "emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — V-complete may not be emitted without this "
