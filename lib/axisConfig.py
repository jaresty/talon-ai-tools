...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "result (scope commitment); exactly one symbol the call will add or modify must be named above that call, derived from the same run result that produced the scope commitment "
        "(symbol commitment) — a symbol commitment naming more than one symbol does not satisfy this requirement; the named symbol must appear as a literal substring of the quoted "
        "scope text or be named in the failing-item line from the most recent tool-executed run result — a symbol not present in either does not satisfy this requirement; the quoted "
        ...
        "line from the same pre-edit run result also appeared as a quoted scope string above this call. Then enumerate the following four escape categories — each must have a named "
        "closure string in the transcript before any file-modifying tool call: (i) scope inflation: name the string that prevents the quoted scope text from covering more than one "
        "independently testable symbol; (ii) stale quote: name the string that confirms no tool-executed run result intervenes between the source of the quoted scope text and this "
        "call; (iii) symbol cardinality: name the string that confirms exactly one symbol is named in the symbol commitment and no other symbol is modified by this call; (iv) post-call "
        "line mismatch: name the string whose absence from the post-call result confirms the scope text was resolved and no other line was inadvertently removed. Proceed to "
        "file-modifying tool calls only when the derivation and all four enumeration categories are present as text above this point in the transcript.",
