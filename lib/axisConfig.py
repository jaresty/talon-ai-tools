...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "planned artifacts, rung labels, and derivation sources. The rung manifest does not instantiate any rung — listing a rung in the manifest is a plan, not a production; no rung "
        "is complete until its artifact has been produced and its execution gate satisfied. Gate: output 'Rung manifest complete' on its own line before any artifact. Boundary: no "
        "artifact appears before this phrase; after this phrase, execution must begin at Rung 1 of Thread 1 — no rung may be skipped on the grounds that its content is described in or "
        ...
        "the phase name. R3 (observation terminates): every thread descends to observed running behavior; anticipated outcomes, asserted expectations, or conversational acknowledgment "
        "never satisfy the execution gate; only tool-executed output with a declared gap does; skipped tests are not observations. Execution gate (R3): before each rung descent output "
        "'Execution observed: [verbatim tool output — content composed without running the tool is invalid]' then 'Gap: [what the verbatim output reveals — gaps inferred from reading "
        "source or test descriptions without running the tool are invalid]' on their own lines — these sentinels gate the immediately following rung only; they do not permit skipping "
        "intervening rungs; a rung manifest or verbal description of expected behavior is never a valid Execution observed; for executable rungs, observation means invoking the "
        "artifact as a running process and recording the terminal output — writing a test file is not observation; the test runner must be invoked; required regardless of whether tests "
        "pass or fail; if tests do not run, fix the rung before descending; if any implementation artifact is produced without valid Execution observed + Gap sentinels immediately "
        "preceding it, discard the implementation and run the tool first. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; stopping "
        "mid-thread is only permitted when the next rung is not achievable (the domain provides no standard artifact type for it); the active completeness token governs how many "
        "threads are batched per descent; when a thread reaches observed running behavior, return to the top for the next thread. Ground applies regardless of perceived task "
        "complexity. When a rung reveals the rung above needs correction — a constraint conflict, a pivot, or new information — correct the rung above to restore faithfulness to I, "
        "then re-derive the current rung; this is correction, not enrichment; if the rung requiring correction is I, surface as a revision signal and stop the current thread. When "
        "beginning mid-ladder, first locate the highest already-instantiated rung and update it to reflect the intended change, then descend. For code contexts, R4 instantiates as: "
        ...
        "must reveal the gap — tests covering behavior that does not yet exist must fail; if all tests pass before new implementation has been written, either the intended behavior is "
        "already implemented (verify this; the thread may be complete) or the tests are not testing the intended behavior and must be corrected; the executable implementation rung is "
        "blocked until the gap has been observed; the executable implementation rung is scoped to the declared gap — implement only what closing the observed gap requires; anticipated "
        "future behavior belongs to a future gap declaration; if tests skip, fix the executable validation rung first.) When the lowest V is complete, output 'Validation artifact V "
        "complete' on its own line before producing O. Non-executable rungs appear inline as labeled log entries; executable rungs are placed where they can be executed — in an "
        "existing file where a natural home exists, or a new file where none does.",
