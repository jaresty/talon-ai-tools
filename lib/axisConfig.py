...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "were sufficient first [ ] Smallest intent at executable level: derive from P1,P3 — the smallest unit of intent is the individual failing assertion, not the test file; each "
        "assertion is its own intent declaration; regression = assertion that was green becomes red; RULE: implement one assertion at a time, not batch of assertions [ ] File edit "
        "declarations: derive from P4 — what to declare before edits; RULE: before any edit, declare 'no new files' at assertion/implementation rungs [ ] Evidence: derive from P3 — "
        "what evidence for completion sentinels; RULE: cite specific assertion that was red [ ] File manifest: derive from P4,P6 — why validation must list files created; RULE: "
        "validation rung lists every file before creating it [ ] Upward scan: derive from P5,P6 — how to verify all criteria addressed; RULE: after implementation, scan up with ✅/❌ "
        "checklist for each criterion from prose/criteria/formal notation [ ] Rung context: derive from P1,P6 — why re-emit rules at each rung; RULE: before each new rung label, "
        "re-emit key rules from derivation [ ] Rule citation at action: derive from P1,P3,P5 — why you must cite the specific derived rule before each action; what GOES WRONG if you "
        "don't cite rules at point of action: rule compliance becomes invisible, cannot verify protocol was followed, description of compliance is indistinguishable from compliance; "
        "RULE: BEFORE each file edit, rung transition, and completion emission, cite the specific derived rule that permits it [ ] Evidence at proximity: derive from P3,P5 — why "
        "evidence must be temporally close to action; what GOES WRONG if evidence is delayed: proof of rule compliance weakens, gaps in evidence allow violations to go undetected; "
        "RULE: evidence of rule compliance must appear IMMEDIATELY after each action, not in later summary — delayed evidence is weaker proof, proximity in transcript strengthens proof "
        "of rule adherence [ ] Evidence echoed in transcript: derive from P1,P3 — why you must explain how evidence proves rules were satisfied; what GOES WRONG if you don't explain: "
        "description of compliance is indistinguishable from compliance, evidence without explanation is just theory; RULE: after each completion sentinel, explain which evidence "
        "proves which rules were satisfied — make the connection visible, not implicit This checklist must appear BEFORE the first ladder rung begins. The derivation results must be in "
        "the chat, not just in your head. Before starting each rung, re-emit key rules from your derivation relevant to that rung. BEFORE transitioning to next rung, verify and "
        "declare: [current rung artifact completed?], [evidence visible?], [gaps remaining?], [next rung requires?], [files permitted?]. If gaps remain, do NOT proceed downward — "
