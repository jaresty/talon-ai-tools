...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target the declared gap) → validation run observation → executable implementation → "
        ...
        "knowledge, anticipation, and model reasoning cannot satisfy any gate regardless of accuracy. For executable rungs, emit 🔴 Execution observed: [verbatim tool output — content "
        "composed without running the tool is invalid] then 🔴 Gap: [what the verbatim output reveals] on their own lines before any implementation artifact. Before producing "
        ...
        "this thread; quoting anticipated output or a prior thread's observation is invalid. The 🔴 sentinel format is reserved exclusively for executable rung gates. For non-executable "
        "rungs, observation appears inline as labeled prose. No implementation artifact may appear before the 🔴 sentinels for the current thread. When the lowest V is complete, output "
        "‘✅ Validation artifact V complete’ on its own line before producing O — this phrase may only appear after the executable validation rung has been both produced and invoked and "
        "the validation run observation rung has declared a gap. Gap-locality: the gap gating rung N is the output of executing rung N-1. No gap from any higher rung, and no element of "
        "I directly, may serve as the gating gap for the current rung. Minimal scope: the current rung's artifact addresses the declared gap and nothing more. Implementing beyond the "
        "declared gap is a violation — not a benefit. Upward revision is always permitted when a gap is observed between prior understanding of I and something encountered via direct "
