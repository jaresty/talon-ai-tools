...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "artifacts derive from prior rung's actual content, not memory; each artifact must cite its source. P5 (Gap-driven iteration): gaps are exposed before claiming completion; "
        "refinement addresses specific gaps. Protocol derivation: before work begins, derive the protocol rules by applying P1-P5 — observe at every feasible rung before proceeding. "
        "Protocol derivation checklist: AFTER deriving rules from P1-P5, BEFORE proceeding, MAKE VISIBLE in the conversation: [ ] The invariant: claim vs reality — what GOES WRONG if "
        "you claim without evidence: fool yourself, completion is false; RULE: evidence before claims [ ] The ladder structure: 7 steps — Intent → Criteria → Structure → Baseline → "
        "Challenge → Refinement → Verification; what GOES WRONG if you skip: gaps lost, completion false; RULE: one rung at a time [ ] The derivation rule: each artifact cites its "
        "source from previous rung; what GOES WRONG if you don't cite: chain breaks, unverifiable; RULE: cite source at each step [ ] Evidence authorities: domain determines what "
        "counts as evidence — software uses automated tests, writing uses peer review, decisions use stakeholder feedback, learning uses assessment; what GOES WRONG if wrong authority: "
        "evidence invalid; RULE: use appropriate authority for domain [ ] Verification: confirmation that no new gaps remain; what GOES WRONG if you skip: hidden gaps persist; RULE: "
        "explicit verification before completion [ ] Challenge: derive from P3,P5 — why Challenge MUST show evidence that gaps exist; what GOES WRONG if you claim gaps exist without "
        "evidence: no proof gaps are real; RULE: Challenge = produce evidence that baseline does not satisfy criteria [ ] Baseline: derive from P2,P3 — why baseline creates scaffold "
        "not complete solution; what GOES WRONG if you produce full solution at baseline: no gap to expose in challenge rung, evaluation criteria become vacuous; RULE: baseline = "
        "minimal artifact that demonstrates approach, NOT complete solution [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if "
        ...
        "solution organized? Derivation: from criteria, derive organization. Evidence: internal consistency. 4. Baseline — Does a minimal version exist that demonstrates the approach? "
        "Derivation: from structure, derive minimal artifact. Evidence: minimal viable example that satisfies structure. IMPORTANT: Evaluation criteria must be APPLIED to baseline "
        "BEFORE refinement — external verification proves gaps exist. Criteria that pass against baseline before refinement proves nothing (Texas sharpshooter fallacy). 5. Challenge — "
        "What evidence proves gaps exist? Derivation: from baseline, produce evidence that baseline does not satisfy criteria. Evidence: external verification showing gap exists (not "
        "just assertion). 6. Refinement — How is the artifact improved? Derivation: from challenge, derive targeted changes. Evidence: addresses specific gaps. 7. Verification — How is "
        "completion confirmed? Derivation: from refinement, derive confirmation. Evidence: no new gaps exposed. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] "
        ...
        "verification confirms no gaps remain. Domain adaptation: for tasks where verification is possible, evaluation criteria must be APPLIED to baseline BEFORE producing the "
        "complete solution — external verification proves gaps exist. Criteria that pass against baseline before refinement proves nothing. Challenge rung MUST produce external "
        "evidence showing gaps exist. Baseline creates scaffold; Challenge applies criteria to produce gap evidence; Refinement produces complete solution that satisfies criteria. for "
