...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation; skipping a rung is a protocol violation; each rung may not be skipped or combined with another), P5 implies sequential descent (since each artifact derives "
        ...
        "lower rung implementation easier; difficulty, failure, or constraint pressure at a lower rung is not a valid upward return trigger), observation loop returns to start (after "
        "manifest exhausted, return to observation loop to verify gap is closed; re-observe running behavior; if gap still exists, descend ladder again; if gap is closed, emit Ground "
        ...
        "the perturbation), each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; emit Protocol derived after deriving "
