...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "non-run tool calls do not break this recency. Derivation phase (required before the first file-modifying tool call): name the root criterion — The last assistant text line "
        "immediately before each file-modifying tool call must be the quoted scope commitment, where the quoted scope text must be a literal substring of the immediately preceding "
        "tool-executed run result; exactly one symbol the call will add or modify must be named above that call, derived from the same run result that produced the scope commitment "
