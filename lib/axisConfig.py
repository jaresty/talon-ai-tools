...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "compliance headings must be markdown '## ' headings — a line not beginning with '## ' does not satisfy any heading requirement. Derivation phase: (0) invoke the subject under "
        "observation as a tool call and record its output; the artifact named in that invocation is the §0 identifier, fixed for all steps; for non-software subjects, record an "
        "observation of the subject under a literal '## ' heading naming the subject — an observation not under such a heading does not satisfy §0; the §0 identifier is the heading "
        "text used; (1) write '## Governing goal: [text]' where '[text]' is a verbatim substring of the §0 tool result — a '## Governing goal:' line appearing before any tool-result "
        "block above it does not satisfy §1; immediately below the heading, write one sentence of the form 'The goal [text] could be achieved by: [alternative means not listed in §0]' "
        "— if no concrete alternative can be named, write '[hypothetical]: [speculated alternative]' and fetch a broader context artifact before proceeding to §2; a '## Behavioral "
        "dimensions' heading appearing without this sentence above it does not satisfy §1; if two non-subsuming goal candidates exist, write both on separate lines under §1 and mark "
        "one '[selected]' — the selected goal is the one whose means-test sentence names a more concrete alternative than those listed in §0; (2) write '## Behavioral dimensions' — "
        "this heading must not appear before '## Governing goal:' appears in the transcript; list at least two dimensions; each dimension names a property of the response (not an "
        "artifact, state, or fix description) such that a response could satisfy it while failing at least one other listed dimension; each dimension quotes a verbatim substring of the "
        "'[text]' string from §1, or names a response property whose absence would leave a reader who accepted all other dimensions still uncertain whether '[text]' was achieved; each "
        "dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose presence in the response constitutes satisfaction — if the "
        "response contains no tool-result blocks above §2, write '[observable: prose]' instead; a '## Enforcement sequence' heading appearing before every dimension carries an "
        "'[observable:]' tag does not satisfy §2; a dimension that names an entity, a fix outcome, or a UI element is not a valid dimension regardless of provenance; (3) write '## "
        "Enforcement sequence' — this heading must appear in the transcript before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each "
        "sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; (4) write '## Completion check' — this heading must "
        "not appear before '## Enforcement sequence' in the transcript; each covered dimension cites a verbatim string that appears as a substring of a tool-result block visible above "
        "'## Completion check' — a dimension declared covered without such a substring does not satisfy §4; (5) write '## Path enumeration'; enumerate every path by which §1–§4 "
        ...
        "§1 must be fetched before §1 is written; the string '§ implementation permitted' must appear in the transcript after '§5 enumeration complete'; the strings '§5 enumeration "
        "complete' and '§ implementation permitted' each appear exactly once; no file-modifying tool call may appear before '§ implementation permitted'; a response turn that declares "
        "the task complete or produces no further planned actions must be immediately preceded in the transcript by a '## Completion check' block — a done declaration appearing before "
