...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "acting, observe the gap: name what is currently absent or failing. For each property [N] to be governed, emit exactly these steps in order. 'Observing: property [N]' may be "
        "emitted only after every property [K] where K < N has already emitted 'Implementation overreach: not found' for property [K] in the transcript above that point; if any "
        "earlier property lacks that sentinel, do not emit 'Observing: property [N]' — instead, resume the earliest incomplete property immediately after its last emitted sentinel and "
        "continue until 'Implementation overreach: not found' has been emitted for it, then re-evaluate whether 'Observing: property [N]' may be emitted. 'Coverage:' may not be "
        "emitted until every property has emitted 'Implementation overreach: not found'. (1) the first text emitted for property [N] must be 'Observing: property [N] \"<verbatim "
        "property definition>\" — <what the artifact observes>' beginning in column 1 — no other text, sentinel, or tool call may appear for property [N] before this line has been "
        "emitted; a property cycle whose first emitted content is not the Observing: line does not satisfy this token; Every property — including preconditions, invariants, absence "
        "properties, boundary conditions, already-satisfied properties, and no-op properties — shall execute the complete six-step cycle without abbreviation. No class of property may "
        "omit or abbreviate any step on the basis that the property is already satisfied, vacuously satisfied, structural, informational, observational, or requires no implementation "
        "change. Step (2) is mandatory for every property: before any reasoning, verification, implementation, or execution for property [N], exactly one regression guard shall be "
        "established (written or identified). The existence, triviality, or pre-satisfied status of the property does not eliminate this requirement. Every property requires a "
        "regression guard because the guard governs future regressions, not the current edit — whether the current response changes the governed artifact is irrelevant to the "
        "requirement to write and execute the guard. (2) the text immediately following the 'Observing: property [N]' line must be the tool call that establishes the regression guard "
        "— no narrative, planning, explanation, analysis, verification, implementation step, sentinel, or unrelated tool call may appear between the Observing: line and the "
        "guard-establishing tool invocation; no other transcript content may appear until the corresponding tool-result block has been emitted — the tool call and tool result are an "
        "atomic pair; establishing the guard means: if no suitable regression guard already exists, write one via a tool call and wait for the tool-result block confirming it was "
        "written; otherwise, identify the existing guard via a tool call that locates or retrieves sufficient artifact text to enumerate every executable assertion — if the retrieved "
        "text does not permit full enumeration, the existing guard is not suitable and shall be rewritten or replaced; wait for the tool-result block confirming the identified guard; "
        "in either case, exactly one regression guard shall govern property [N] for the remainder of the cycle; every subsequent reference to 'the guard' — including every 'Property "
        "[N] assertion', 'Quoted test:', 'Test blind-spot:', and 'Failure:' — refers exclusively to this established guard until step (2) is repeated; every property shall establish "
        "exactly one governing regression guard during step (2); proceeding without an established governing guard does not satisfy this token; build commands, compilation runs, or "
        "unrelated verification commands are not regression guards unless they directly execute the established guard itself; when the governed artifact has executable behavior, the "
        "established guard should prefer a guard that executes that behavior and derives its assertions from the observed output over a guard that inspects the artifact's source text, "
        "structure, or metadata without executing it — a static (non-executing) guard is appropriate only when the governed property is itself about that text, structure, or metadata; "
        "when a static guard is chosen while the governed behavior was executable, emit a one-line justification for why a static guard was chosen; repeating step (2) replaces the "
        "previous regression guard; after replacement, every subsequent assertion inventory, 'Quoted test:', 'Failure:', and implementation overreach analysis must derive exclusively "
        "from the replacement guard; references to earlier guards no longer satisfy this token; until the guard-establishing tool-result block has appeared, no 'Property [N] "
        "assertion', 'Assertion inventory:', 'Quoted test:', 'Test blind-spot:', 'Failure:', 'Quoted implementation:', 'Implementation overreach:', implementation change, or artifact "
        "modification may occur; Guard artifact provenance: the established regression guard is an already-confirmed artifact state, not a planned or anticipated test. Before any "
        "'Property [N] assertion [M]:', 'Assertion inventory:', or 'Quoted test:' line may be emitted, the guard's executable text must have appeared verbatim in the qualifying "
        "guard-establishment tool-result block produced in step (2). Every assertion inventory entry must be copied verbatim from guard text confirmed in that tool-result block — an "
        "assertion entry whose text does not appear verbatim in the qualifying prior tool-result block is invalid. A 'Quoted test:' line must quote verbatim text that appeared in the "
        "qualifying guard-establishment tool-result block; a 'Quoted test:' line that introduces, predicts, paraphrases, or quotes test text not present verbatim in that prior "
        "tool-result does not satisfy this token. For every executable assertion in the confirmed guard text emit exactly one 'Property [N] assertion [M]: <quoted assertion text>' "
        "line — every executable assertion must appear exactly once and only once; omitting, merging, or inventing an assertion does not satisfy this token; after the assertion "
        "inventory emit 'Assertion inventory: complete — <count> assertions for property [N]' or 'Assertion inventory: incomplete — <reason>'; if incomplete, revise the guard or "
        "inventory and repeat the assertion inventory; no subsequent step may occur until 'Assertion inventory: complete' has been emitted; the text immediately following 'Assertion "
        "inventory: complete' must be the 'Quoted test:' line — no tool call, Failure: line, Test blind-spot: attempt, or other content may appear for property [N] between 'Assertion "
        "inventory: complete' and the 'Quoted test:' line; emit 'Quoted test: <smallest contiguous verbatim executable assertion or guard uniquely identifying the written guard>'; a "
        "'Quoted test:' line that does not immediately follow 'Assertion inventory: complete' does not satisfy this token; (3) attempt to falsify the quoted guard by constructing a "
        "concrete scenario in which property [N] is absent while every listed 'Property [N] assertion [M]:' nevertheless succeeds; more generally, a guard is a blind spot whenever "
        "there exists an admissible state in which the guard's pass or fail outcome is not equivalent to whether property [N] actually holds — in either direction: the guard passes "
        "while the property is absent, or the guard fails or is absent while the property holds. Three named constructions must be checked as instances of this: (a) proxy or "
        "non-exercising guard — the guard checks a surrogate (that a substring is present, that an artifact exists, that a build succeeds) rather than executing the governed behavior, "
        "so it can pass while the property is absent; treat such a guard as a found blind spot unless the surrogate's truth is logically equivalent to property [N] holding. (b) "
        "durability — a guard less durable than the property it governs, invalidated or removed by the same change that regresses the property, so the guard is gone at the exact "
        "moment it would need to fire and cannot detect the regression; treat any guard whose continued ability to exercise property [N] depends on the property itself remaining "
        "unchanged as a found blind spot. (c) ephemeral or non-persistent guard — a one-off script, a temporary test, or any guard not committed to a location that re-runs without "
        "human initiation; it is a found blind spot because it is absent on the next run while property [N] still holds, so it cannot detect a later regression; this holds regardless "
        "of whether the property itself changed, which distinguishes it from (b), whose loss of durability is coupled to the property changing. emit 'Test blind-spot: found — "
        "<construction>' or 'Test blind-spot: not found'; if found, strengthen the guard and repeat from step (2); (4) the text immediately following 'Test blind-spot: not found' must "
        "be the tool call that executes the quoted guard against a deliberately constructed violating state — no narrative, planning, implementation change, 'Quoted implementation:', "
        "'Implementation overreach:', or other sentinel may appear between 'Test blind-spot: not found' and that execution tool call; no transcript content may appear between the "
        "execution tool call and the corresponding tool-result block — the execution tool call and tool result are an atomic pair; execute the quoted guard against a deliberately "
        "constructed violating state — if the governed artifact already satisfies property [N], first construct a minimal temporary modification to the artifact that violates property "
        "[N], execute the guard against that temporary state, emit the required Failure: line from the tool-result block, then restore the original state before continuing; the text "
        "immediately following the execution tool-result block must be the Failure: line for property [N] — no narrative, causal argument, prose explanation, reference to a prior "
        "failure, or other content may appear between the tool-result block and the Failure: line; a Failure: line that does not immediately follow a tool-result block does not "
        "satisfy this token; emit 'Failure: property [N] \"<verbatim property definition>\" — <verbatim quoted failure line from a tool-result block>'; emit 'Unobservable: property "
        '[N] "<verbatim property definition>" — <reason>\' only after demonstrating that no temporary violating state can be constructed within the governed artifact on which the '
        "guard could distinguish satisfaction from violation — Unobservable: is the last resort, not the first; successful compilation, build output, or unrelated verification does "
        "not satisfy this step — the violating state must exist solely to witness that the guard detects the absence of property [N]; before modifying the governed artifact to satisfy "
        "property [N], a verbatim Failure line from a tool-result block must appear; if the failure line does not specifically witness the violation of property [N], modify the "
        "violating state and repeat from step (4); before emitting any witness classification, determine whether every inventoried assertion can be classified mechanically from the "
        "immediately preceding execution tool-result block — a witness is mechanically derivable iff the tool-result (i) identifies exactly one assertion, (ii) identifies its outcome "
        "(pass or fail), and (iii) no other assertion satisfies the same identifier; if any witness is not mechanically derivable by this operational definition, temporarily "
        "instrument the regression guard or construct an equivalent observation harness that executes exactly the inventoried assertions without strengthening, weakening, removing, or "
        "reordering them, while emitting observable per-assertion outcomes; before executing the harness, emit 'Observation correspondence: assertion [M] -> <observation point>' for "
        "every inventoried assertion, establishing a declared one-to-one mapping from every inventoried assertion to its corresponding observation point; if a one-to-one mapping "
        "cannot be established, revise the harness; execute the instrumented guard or harness against the same violating state via a tool call and wait for the tool-result block; "
        "after execution, verify that every declared observation point produced exactly one observation in the tool-result — if any observation point produced zero or more than one "
        "observation, emit 'Observation correspondence: incomplete — <reason>', revise the harness, and repeat from step (4); also verify that every assertion observed in the "
        "tool-result output appears in the 'Observation correspondence:' mapping — if any observed assertion is absent, the harness is non-equivalent and must be revised; derive every "
        "witness classification exclusively from the verified tool-result; the temporary instrumentation exists solely to observe execution and must not become part of the regression "
        "guard or governed artifact — discard it after witness generation; the text immediately following the Failure: line (or the observation harness tool-result block if "
        "instrumentation was required) must be the first witness line — no narrative, causal argument, prose explanation, or other content may appear before the first 'Property [N] "
        "assertion [M] witness:' line; emit exactly one witness line for every previously inventoried executable assertion, in ascending assertion inventory order without omission or "
        "reordering; each witness line takes one of these forms, derived exclusively from the verified tool-result block: 'Property [N] assertion [M] witness: Failure — assertion: "
        '"<verbatim assertion text>" — observed: "<verbatim tool-result text uniquely identifying this assertion as the failure by assertion text, source location, or execution '
        "identifier>\"' if and only if the tool-result uniquely identifies this assertion as a failing outcome — when identical assertion texts appear more than once, the failure must "
        'identify the specific occurrence by source location or execution identifier; \'Property [N] assertion [M] witness: Success — assertion: "<verbatim assertion text>" — '
        'observed: "<verbatim tool-result text confirming this assertion passed>"\' if the tool-result contains a directly observed passing outcome for this assertion; if an assertion '
        "cannot be classified Success or Failure from the current tool-result, it is not yet observable — before accepting that conclusion, determine whether it is due to dead code, "
        "unreachability, mis-inventory, or harness omission; refine the observation harness or violating state to resolve the cause and repeat the instrumented execution; only after "
        "no executable instrumentation or violating state modification can make the assertion observable may 'Property [N] assertion [M] witness: Unobservable — assertion: \"<verbatim "
        "assertion text>\" — <observed reason quoted from tool-result>' be emitted; Unobservable is the last resort and must be justified by a verbatim tool-result excerpt; an "
        "Unobservable witness is treated as a guard defect — revise the guard to make the assertion observable and repeat from step (2); no assertion may permanently carry "
        "Unobservable status; after all witness lines are emitted, verify set equality between the assertion inventory and observed assertions: (i) every assertion observed in the "
        "harness or execution output must appear exactly once in the assertion inventory — if any observed assertion is absent from the inventory, emit 'Assertion inventory: "
        "incomplete — missing assertion: <text>' and repeat from step (2); (ii) every inventoried assertion must appear exactly once in the observation output — if any inventoried "
        "assertion is absent from the observation output, treat it as unresolved and refine the harness or violating state and repeat the instrumented execution; at least one "
        "inventoried assertion must receive witness classification Failure — a guard execution in which no inventoried assertion is classified Failure does not satisfy step (4), "
        "regardless of whether a Failure: line was produced; a Failure witness whose observed text is not present verbatim in the corresponding tool-result block does not satisfy this "
        "token; if no inventoried assertion can be attributed to the observed failure, revise the violating state or the regression guard and repeat from step (2); after all "
        "assertions are witnessed and set equality is verified, emit 'Assertion witnesses: complete — <count> assertions witnessed' where <count> must equal the count from 'Assertion "
        "inventory: complete'; a witness count that differs from the inventory count does not satisfy this token; emit 'Assertion witnesses: incomplete — <reason>' if any assertion "
        "lacks a witness; if incomplete, revise the violating state, observation harness, or guard and repeat from step (4); no 'Quoted implementation:', implementation modification, "
        "or 'Implementation overreach:' may appear before 'Assertion witnesses: complete' has been emitted; no subsequent step may occur until 'Assertion witnesses: complete' has been "
        "emitted; (5) identify the governed artifact for property [N] — the artifact whose modification satisfies property [N]; no behavior-altering modification to any governed "
        "artifact may occur before 'Assertion witnesses: complete' has been emitted for property [N] — a modification that occurs before a qualifying Failure witness for property [N] "
        "has been established does not satisfy this token; depending on the property, the governed artifact is the artifact that the established regression guard governs — the "
        "artifact whose behavior that guard constrains, in whatever form that artifact takes; it is never the guard itself, since the guard and the artifact it governs are necessarily "
        "distinct; make the behavior-altering modification via a tool call and wait for the tool-result block confirming it; after that tool-result block, emit 'Quoted implementation: "
        "property [N]: <smallest contiguous verbatim text uniquely identifying the modification just written to the governed artifact>' — this sentinel must appear in the transcript "
        "after the tool-result block confirming the modification, and must name the retained property [N] it closes; a 'Quoted implementation:' line that appears before the "
        "modification tool call does not satisfy this token; the 'Quoted implementation:' content must be a contiguous fragment of the literal text the modification tool call wrote "
        "into the governed artifact — a description of the change's purpose, effect, or plan (for example imperative phrasing such as 'replace X with Y', 'record A separately', "
        "'retain B', or 'permit C') does not satisfy this token; a 'Quoted implementation:' whose content does not appear verbatim as literal artifact text in the immediately "
        "preceding modification tool-result block does not satisfy this token; moreover, a comment, docstring, or other non-executable annotation does not by itself satisfy this token "
        "— the quoted text must be the behavior-bearing artifact text whose introduction causes the guard that produced this property's Failure: witness to succeed; text whose removal "
        "would leave the outcome of the guard that produced this property's Failure: witness unchanged does not satisfy this token; if step (4) required constructing a temporary "
        "violating state because the governed artifact already satisfied property [N], step (5) is satisfied by confirming the original state is restored — after the restoration "
        "tool-result block, emit 'Quoted implementation: property [N]: <verbatim text identifying the restored governed artifact>'; no subsequent step may occur until this sentinel "
        "has been emitted after its corresponding modification or restoration tool-result block has appeared;(6) attempt to construct a minimal modification to the quoted "
        "implementation that preserves the success of the quoted guard while still changing the governed artifact, and attempt to construct a concrete execution path through the "
        "quoted implementation that is not exercised or distinguished by the quoted guard; emit 'Implementation overreach: found — <modification or execution path>' or 'Implementation "
        "overreach: not found'; if found, minimally revise the governed artifact to eliminate the overreach, emit a new 'Quoted implementation:', and repeat from step (6); every "
        "implementation overreach analysis must reason only from the quoted implementation and quoted guard emitted earlier in the transcript, not from an imagined or intended "
        "version. After all current properties complete their six-step cycles, begin the implementation audit. 'Coverage: complete' is the global fixed-point sentinel and is valid iff "
        "all of the following hold simultaneously: (1) every retained property completed steps (1)–(6); (2) every retained property has a witnessed Failure or justified Unobservable; "
        "(3) assertion inventory equals observed assertions for every property (set equality verified in both directions); (4) no implementation gap exists in the final audit pass; "
        "(5) Ground reached a fixed point (§ ground complete was emitted for the final retained property set); (6) the implementation audit reached a fixed point (Audit: "
        "implementation complete was emitted after a pass that discovered no new gaps). The implementation audit is a convergence loop: repeat — (a) enumerate every modified artifact; "
        "(b) enumerate every externally callable entry point of the governed artifact; (c) enumerate every distinct execution branch; (d) enumerate every caller or consumer whose "
        "observable output changed; (e) enumerate every externally visible output or state transition; enumerate all observable behaviors and emit one 'Audit: implementation gap — "
        "<description>' for each behavior not governed by any retained property; if any gaps were emitted, restart the Ground protocol from the full retained property set "
        "incorporating all newly discovered properties — including atomicity, completeness, and observational independence checks until a new '§ ground complete' is reached — then "
        "execute the full six-step falsify cycle for every newly retained property; for every existing property whose canonical definition, quoted implementation, or quoted guard is "
        "affected by the new properties, determine the impact scope: if only the quoted implementation is affected, repeat steps (5) and (6) for that property; if the guard or "
        "observation mechanism is affected — meaning the assertion inventory, quoted test, or failure evidence may no longer validly witness the property under the new retained set — "
        "repeat steps (2) through (6) for that property; a prior falsify result for a property whose guard or observation was affected does not govern the resulting state and must not "
        "be treated as satisfying this token; resume the audit after all affected properties have completed their required cycle; repeat until a full audit pass emits zero gaps, then "
        "emit 'Audit: implementation complete'; 'Coverage: complete' may not be emitted before 'Audit: implementation complete'. A response that omits any required sentinel, performs "
        "a step before emitting its required quoted artifact, modifies the governed artifact to satisfy property [N] before a qualifying Failure witness has been established, omits "
        "any executable assertion from the assertion inventory, or leaves a property without an automatic regression guard does not satisfy this token.",
        ...
        'specific sub-term: "<verbatim sub-term of this property\'s canonical definition>"; candidate relaxation: <admissible construction that weakens or drops that sub-term>; '
        "rejected: <reason that construction violates the chosen interpretation>' when none does. The minimal form must exhibit this witnessed relaxation attempt: the most specific "
        "sub-term is the narrowest literal, constant, named value, fixed set, or bound appearing in the canonical definition; the candidate relaxation is a concrete admissible "
        "construction that generalizes or removes exactly that sub-term; and the rejection reason shows that construction fails the chosen interpretation, which is what justifies "
        "retaining the sub-term. A bare 'minimal' verdict — one that asserts no construction falsifies the property without quoting a most specific sub-term, without naming a candidate "
        "relaxation construction, and without showing that construction violates the interpretation — does not satisfy this protocol; if the candidate relaxation instead satisfies the "
        "interpretation, the property is over-specific and the over-specific form must be emitted instead. The minimality check runs on every property without exception — including "
        "properties whose primary constraint appears obviously general, properties already in formal notation, and properties introduced during the completeness phase or produced by a "
        "merge; no property may be exempted on the ground that it is evidently minimal. A property that reaches Step 2 without an emitted '§ minimality:' line for that property, in one "
        "of these two forms, does not satisfy this protocol; if the '§ minimality:' line for a property has not appeared, emit it before introducing any provisional sub-property. Step "
        "2 — Introduce provisional sub-properties. Introduce provisional candidate sub-properties whose conjunction is proposed to represent the current property: 'property [Na]: "
        "<definition>' and 'property [Nb]: <definition>'. Apply the canonicalization procedure (Step 1) to each provisional sub-property before proceeding. The resulting canonical "
        "definitions replace the provisional definitions for all subsequent split evaluation. The labels [Na] and [Nb] continue to identify the resulting canonical definitions after "
        "replacement. Split evidence is attached to the canonical definitions of the current property and its provisional sub-properties, and to the falsifying constructions emitted in "
        "the split test, at the moment the split test is emitted. Any modification to any of those definitions or constructions after the split test is emitted voids that evidence; the "
        "split evidence must then be regenerated against the current canonical state before the split may be accepted. Step 3 — Evaluate split validity. Before evaluating the split, "
        "verify that the chosen interpretation determines an accessible-observation boundary: the set of outputs, behaviors, states, and interactions that the party responsible for "
        "determining whether the request was satisfied can access without inspecting the implementation. If the chosen interpretation does not determine that boundary, emit "
        "'Interpretation refinement required: <what the boundary must specify>' and emit or refine the Interpretation: line to supply it. An interpretation refinement voids the entire "
        "current retained-property tree, all previously derived properties, and all split evidence, and restarts property derivation and recursive decomposition from the request "
        "constraints under the refined interpretation; any previously emitted split test is void and does not constitute a completed split test for the current canonical state. After "
        "the accessible-observation boundary has been established and the current property, its provisional sub-properties, and their split evidence have been regenerated under the "
        'current interpretation, emit exactly one non-void candidate split test for the current canonical state: \'§ split test: property [N]: "<verbatim canonical definition>" ⇔ '
        '(property [Na]: "<verbatim canonical definition>" ∧ property [Nb]: "<verbatim canonical definition>"); property [Na] — falsified by <construction>; property [Nb] — falsified '
        "by <construction>; these falsifiers are independent.' The quoted definitions for [N], [Na], and [Nb] must match the verbatim canonical definitions confirmed in Step 1 for each "
        "respective property. A split may be accepted only if its split test, quoted definitions, falsifying constructions, and interpretation are identical to the non-void state "
        "against which the split test was emitted. A split succeeds only if: for every admissible construction x, P(x) ⇔ (PNa(x) ∧ PNb(x)) — the original property is logically "
        "equivalent to the conjunction of the accepted sub-properties over all admissible constructions; each quoted falsifier violates exactly its quoted property while satisfying the "
        "other; both falsifiers are admissible constructions within the chosen interpretation; the candidate split is nontrivial: neither accepted sub-property is logically equivalent "
        "to the original property over all admissible constructions, and neither accepted sub-property logically entails the other over all admissible constructions; each accepted "
        "sub-property corresponds to a distinct constraint on the admissible construction under the chosen interpretation that is independently required by the explicit request "
        "constraints, and the request itself distinguishes the two sub-constraints at its own granularity — meaning the request separately names, or would be independently judged as "
        "failing on, each sub-constraint on its own; a candidate split is not valid merely because it decomposes the mathematical or definitional structure of a predicate, is not valid "
        "merely because each sub-property is independently testable, assertable, or useful as verification evidence, and is not valid merely because the two sub-constraints are "
        "logically distinct when the request does not itself distinguish them at its own granularity — if a single request constraint is being divided into finer conditions that the "
        "request never separately names or judges, the property is atomic at the request's granularity and must not be split; and neither accepted sub-property constrains an internal "
        "mechanism, architecture, algorithm, or representation as the primary subject of the constraint, unless that mechanism is explicitly required by the chosen interpretation. A "
        "sub-property constrains an internal subject when its canonical definition makes an internal mechanism, architecture, algorithm, or representation the primary subject of the "
        "constraint — this classification is independent of whether that subject happens to be externally observable. A sub-property is externally observable when its truth value can "
        "be determined solely from the accessible-observation boundary of the chosen interpretation, without inspecting the implementation. These two criteria are distinct: a property "
        "can concern an internal subject yet still be externally observable (for example, when the interpretation explicitly exposes that subject through its output contract), and a "
        "property can concern an external subject yet still be inaccessible if the interpretation does not expose the relevant behavior. The split condition rejects properties whose "
        "primary subject is internal and not explicitly required — not merely properties that happen to be inaccessible. A candidate split can be constructed only if canonical "
        "definitions for [Na] and [Nb] have been established and admissible falsifying constructions satisfying the stated falsifier conditions can be produced for both sub-properties. "
        "A candidate split test is accepted when all split-success conditions are satisfied for that non-void candidate; otherwise the candidate split is rejected. Step 4 — Accept or "
        "mark atomic. A property may be marked atomic only after Step 2 has introduced provisional sub-properties, Step 1 has canonicalized them, and Step 3 has attempted "
        "candidate-split construction against the current canonical state. If the candidate split test is accepted, replace the original property with the accepted canonical "
        ...
        "found' (5). A corrective protocol finding must name the governing clause verbatim and state which of the five properties is absent.",
