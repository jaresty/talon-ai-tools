...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        'refinement rung targets one observable gap only[ ] Consequence materialization: derive from A3 (optimization pressure), A1 (evidence primacy) — every stated "what GOES WRONG" '
        "must create an immediate, visible protocol consequence; explanation without cost does not constrain behavior[ ] Prerequisite minimality: derive from A6 (execution discipline) "
        "— if failure is prerequisite absence, refinement may only restore guard executability; do not implement downstream behavior during prerequisite repair[ ] File-instantiation "
        "minimality: derive from A6 (execution discipline) — if the active gap is file absence, refinement may create only the smallest valid file needed to restore the interrupted "
        "guard (e.g. export symbol / minimal stub) and must immediately rerun the same challenge before any behavioral implementation[ ] Guard drift check: derive from A1 (evidence "
        "primacy), A5 (independent evaluation) — if a guard changes after observing implementation, show the higher-rung authority that requires the change before reusing the guard[ ] "
        "Preservation characterization: derive from A1 (evidence primacy), A4 (causal traceability), A5 (independent evaluation) — all preservation changes must have pre/post "
        ...
        "A3 (optimization pressure), A1 (evidence primacy) — what GOES WRONG if proportional enforcement is not applied: gaps may be ignored, refinements may overreach, trust is "
        'compromised, meta-loop may fail; for every stated "what GOES WRONG" (observable failure or gap), an immediate, visible protocol consequence must be applied scaled to severity; '
        "minor gaps trigger guard replay with visible evidence in transcript, moderate gaps trigger rung rejection (block rung output and prevent downstream use) plus guard replay with "
        "evidence, major gaps trigger artifact invalidation (mark current artifact invalid), trust downgrade (reset confidence/trust state), and replay of affected meta-loop steps, all "
        "with immediately visible evidence; consequences must always be applied visibly and immediately in the conversation; explanation without enforced action or visible evidence "
        'does not constrain behavior; severity must be justified by the observable impact described in the "what GOES WRONG" statement.[ ] Hard gate enforcement: Derive from A3 '
        "(optimization pressure) — no rung may begin until both Protocol and Ladder sentinels are visible; noncompliance triggers immediate protocol consequence.[ ] Rung labeling: "
        "Derive from A6 (execution discipline) — emit rung labels at beginning, completion sentinels (✅ [Rung] complete) at end; skipping labels produces immediate enforcement "
        "consequence.[ ] Evidence before claims: Derive from A1 (evidence primacy) — every completion claim requires visible evidence; claiming without evidence triggers rung "
        "invalidation.[ ] Ground completion conditions: Derive from A6 (execution discipline), A1 (evidence primacy), A4 (causal traceability) — Verification must close gap, Challenge "
        "confirms zero gaps, all criteria evaluated, guards unchanged and functional; otherwise, halt.[ ] Domain adaptation: Derive from A2 (intent anchoring), A4 (causal "
        "traceability), A5 (independent evaluation) — derive evaluation from criteria **before** producing solution; post-hoc evaluation is invalid and triggers consequence.[ ] "
        "Challenge/Refinement/Verification loop: Derive from A1 (evidence primacy), A6 (execution discipline), A5 (independent evaluation) — Challenge produces, applies, and shows "
        "failing evaluation; Refinement resolves exactly one gap; Verification re-applies same mechanism; loop repeats until zero-gap evidence.[ ] Minimal artifact naming: Derive from "
        "A6 (execution discipline) — all refinement artifacts must be 'Partial Implementation' or 'Minimal Code Change'; full solutions indicate protocol violation.[ ] Divergence "
        "detection: Derive from A6 (execution discipline) — if output diverges, identify rung of divergence, naming error, and minimal corrective change.[ ] Refinement scope "
        "enforcement: Derive from A6 (execution discipline) — each refinement addresses one gap only; producing full features triggers immediate enforcement consequence.[ ] Ladder "
        "derivation by domain: Derive from A2 (intent anchoring) — for each domain (writing, decision-making, etc.), derive ladder structure appropriate for that domain; failure to do "
        "so blocks rung start.[ ] Failure-frontier rule: derive from A1 (evidence primacy), A6 (execution discipline) — one refinement artifact may change the active failing "
        "observation only once; the artifact may leave the check failing, but must stop immediately after producing a new observable failure state or a pass; WHAT GOES WRONG if absent: "
        "multiple hidden fixes accumulate inside one rung, stubs silently become implementations, gap boundaries disappear; CONSEQUENCE: additional changes beyond the first new failure "
        "state are surplus and invalid.[ ] Validation linkage: derive from A1 (evidence primacy), A6 (execution discipline) — each implementation artifact must produce at least one "
        "observable check/validation artifact that directly drives the active gap; WHAT GOES WRONG if ignored: gaps remain untested, meta-loop falsely succeeds, refinement "
        "unconstrained; CONSEQUENCE: immediate rung rejection, artifact invalidation, and meta-loop replay with visible evidence.[ ] Direct artifact coverage: derive from A1 (evidence "
        "primacy), A4 (causal traceability) — each implementation artifact introduced in this task must be the named subject of at least one evaluation that targets it directly; "
        "coverage only through a higher-level artifact that depends on it does not satisfy this requirement, because the higher-level artifact controls inputs and cannot expose the "
        "direct artifact's own edge-case contract; WHAT GOES WRONG if absent: edge cases isolated to the artifact are never exercised, failures may be masked by the higher-level "
        "artifact's behavior, and the causal chain from intent to evaluation is broken; CONSEQUENCE: rung rejection until a direct evaluation exists in transcript for each introduced "
        "artifact.[ ] Pre-rung rule re-emission: Derive from A3 (optimization pressure), A1 (evidence primacy) — before starting any rung, emit all key rules and consequences of rule "
        "violations relevant to that rung. Observable violation: missing rules or consequences at rung start. Immediate enforcement: block rung start, trigger artifact invalidation or "
        "rung rejection until rule emission is visible.[ ] Gap authorization: derive from A1 (evidence primacy), A6 (execution discipline), A5 (independent evaluation) — no "
        "implementation artifact may begin until a failing check/validation artifact already exists in transcript and that exact artifact names the active gap; WHAT GOES WRONG if "
        "absent: implementation is inferred from described intent rather than forced by observed failure, structure leaks ahead of proof, guards become retrospective; CONSEQUENCE: "
        "immediate rung rejection, artifact invalidation, and replay from Challenge using the failing artifact as the sole authorization source.This checklist must appear BEFORE the "
        "first ladder rung begins. Each item must include its full derivation, including: source principles, observable failure if skipped, and immediate protocol consequence. All "
        "derivations must be emitted visibly in the conversation; internal-only derivations are insufficient.⚠️ HARD GATE: Do not start the ladder until every checklist item above has "
        "been fully emitted with derivation. Skipping or partially emitting any derivation triggers immediate rung rejection and blocks progress.Before starting each rung, re-emit key "
        "rules and consequences of rule violations relevant to that rung.BEFORE transitioning to next rung, verify and declare: [artifact completed?], [evidence visible?], [gaps "
        "remaining?], [intent aligned?], [guards unchanged?], [next rung requires?]. If any condition fails, do NOT proceed.Protocol derived MUST be emitted as: 🔵 Protocol "
        "derived.Ladder derived MUST be emitted as: 🔵 Ladder derived.No descent before both sentinels — hard gates.Emit rung labels to begin each rung and at each upward return; emit "
        "completion sentinels (✅ [Rung] complete) to finish each rung.For writing tasks: intent→criteria→challenge→minimal artifact→verification→meta-loop.For decision-making: "
