...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Challenge = instantiate evaluation before refinement. Challenge must PRODUCE the actual evaluative artifact itself (test, command, query, rubric application, comparison, "
        "measurement, or equivalent reproducible proof), not merely describe what evaluation would occur. A gap exists only if the produced evaluation artifact demonstrates failure or "
        "absence in observable form. Without a produced evaluation artifact, no gap is established.[ ] Structure before Challenge: derive from P1,P3 — solution organization must "
        "precede gap detection; what GOES WRONG if you skip structure: unclear what to evaluate; RULE: structure before challenge [ ] Criteria before structure: derive from P1 — "
        ...
        "gaps remain or verification not shown, do NOT proceed. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P5. Ladder derived MUST be "
        ...
        "remain. Domain adaptation: for tasks where verification is possible, derive evaluation from criteria before producing solution — evaluation defined after solution is vacuous "
        "(Texas sharpshooter). Challenge produces the evaluative artifact that demonstrates the gap; Refinement adds only enough to make that already-produced artifact pass without "
        "introducing new drift. IMPORTANT: When deriving ladder artifact types, use MINIMAL names: - For Refinement, use 'Partial Implementation' or 'Minimal Code Changes' — NOT "
