...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation. If the chosen interpretation does not determine that boundary, emit 'Interpretation refinement required: <what the boundary must specify>' and refine the "
        "Interpretation: line to supply it. An interpretation refinement restarts property derivation and recursive decomposition from the initial retained properties under the refined "
        "interpretation; any previously emitted split test derived under the prior interpretation is void and does not constitute a completed split test for the current canonical "
        "state. After the accessible-observation boundary has been established and all affected definitions have been regenerated under the current interpretation, emit exactly one "
        'split test: \'§ split test: property [Na]: "<verbatim canonical definition>" — falsified by <construction>; property [Nb]: "<verbatim canonical definition>" — falsified by '
        "<construction>; these falsifiers are independent.' The quoted definitions must match the immediately preceding canonical provisional definitions verbatim. A split succeeds "
        "only if: the conjunction of the accepted sub-properties has identical coverage to the original property; each quoted falsifier violates exactly its quoted property while "
        "satisfying the other; both falsifiers are admissible constructions within the chosen interpretation; and neither accepted sub-property constrains an internal mechanism, "
        "architecture, algorithm, or representation as the primary subject of the constraint, unless that mechanism is explicitly required by the chosen interpretation. For this "
        "condition, a sub-property constrains an internal subject when determining whether the property is satisfied or falsified requires observing state, structure, or mechanism not "
        "accessible under the chosen interpretation's accessible-observation boundary. A sub-property is observable under the interpretation when its truth value can be determined "
        "solely from the outputs, behaviors, states, and interactions that the interpretation makes accessible to the evaluating party without inspecting the implementation. Step 4 — "
        "Recurse or mark atomic. If the split succeeds, replace the original property with the accepted canonical sub-properties and apply this same recursive decomposition procedure "
        '(Steps 1–4) independently to each accepted sub-property. If no valid split exists, emit: \'§ split test: property [N]: "<verbatim canonical definition>" — atomic, no valid '
        "split: <reason>.' The quoted definition must match the retained canonical property verbatim. Apply this procedure to every retained property until all have reached Step 4 with "
        'no valid split. After all retained properties are atomic, emit a completeness check: \'§ completeness check: "<request constraints verbatim>" / <P1 expression> / <P2 '
        "expression> / ...' — the request constraints must be quoted verbatim from the chosen interpretation; every retained atomic property expression must appear exactly once, "
        ...
        "after the completeness check, attempt to identify one explicit request constraint that is not represented by any retained property. If such a constraint exists, emit '§ "
        ...
        "explicit request constraint is represented by at least one retained property may the response emit '§ properties complete? yes' — a '§ properties complete? yes' line without a "
        "preceding completeness check does not satisfy this protocol. After completeness has been established, verify observational independence. A retained property is observationally "
        ...
        "merge or removal changes the retained property set, repeat the atomicity and completeness procedures. The Ground properties block is complete only when all retained properties "
        "simultaneously satisfy: they are derived from the chosen interpretation; they are atomic; they collectively cover every explicit request constraint; they are observationally "
        "independent; they introduce no out-of-scope constraints. When all conditions are met, emit '§ ground complete' as the final line of the Ground properties block — this sentinel "
        "may not appear before '§ properties complete? yes' in the transcript. Any test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does "
        "not satisfy this protocol; if task reasoning begins and '§ ground complete' has not yet appeared, emit the Ground properties block in full and '§ ground complete' before "
        "continuing.",
