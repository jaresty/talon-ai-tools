...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "requiring a piece of reality before any claim about reality. P1 (Intent primacy): intent exists; all artifacts derive from it; when intent changes, refine affected rungs "
        "top-down; completed rungs may not be re-opened except by this mechanism. P2 (Behavioral change isolation): One rung per artifact type; changes only at dedicated rung; a rung "
        "that changes behavior and also produces another artifact type must be split. P3 (Observable evidence required): Pre/post change states visible through actual tool output; the "
        "observation must directly demonstrate the behavior named in the criterion — infrastructure evidence does not satisfy this unless the criterion explicitly asserts "
        "infrastructure state. P4 (Enforced and persistent): Behavioral changes require a dedicated verification rung; a behavioral change without one is not complete. P5 (Automation "
        "quality verified): Automation must fail before passing; the failure output is part of the verification artifact, not a separate rung; a reviewer cannot determine from the test "
        "file alone what the implementation must do — the failure output is required; the transcript must also contain green-state evidence — every assertion in the validation artifact "
        "must be seen passing after implementation. P6 (Artifact type discipline): One rung per type; text rungs produce no files; each rung may not be skipped or combined with "
        "another; frozen artifacts may not be modified at subsequent rungs; every file write must occur at the dedicated rung for that artifact type. An executable validation artifact "
        "is a file whose sole purpose is to assert behavioral properties of another artifact — it contains no behavior of its own; it must be invocable by an automated tool; validation "
        ...
        "determined by file content, not file path or naming convention — path and naming are evidence, not authority. P7 (Upward faithfulness): Lower rungs narrow what upper rungs "
        "permit; expanding the permitted space is a faithfulness violation. P8 (Rung validity test): Human reviewer with only this rung’s artifact can evaluate next rung; ladder is "
        "minimal — remove any rung that does not affect faithfulness evaluation. P9 (Information density preservation): Each rung encodes same/higher quality than above; a rung that "
        "loses information is a protocol violation. P10 (Three-part completeness): Observation + automation + implementation; all three must be present. P11 (Immediate lowest-rung "
        "observation): Observe at every feasible rung, beginning at the lowest; deferring is a protocol violation. P12 (Completeness slice): One independently testable behavior per "
        "thread per cycle; criterion is a falsifiable behavioral assertion — a feature name is not a criterion. P13 (Observation-first, observation-last): Open and close with "
        "tool-executed observation of live running code. Observing running behavior means invoking the system directly in a manner that exercises the behavior named in the intent — the "
        "output must be produced by the behavior itself, not by a test framework asserting properties of it; the tool call must execute live running code — reading files, grepping "
        ...
        "whether the tests pass. The intent declaration must be derivable from the tool-executed opening observation. ✅ Ground complete may only be emitted as the outcome of the "
        "observation loop — emitting it outside the loop is a protocol violation regardless of whether 🔵 Closing observation is present. 🔵 Closing observation must appear after ✅ "
        "Manifest exhausted and before ✅ Ground complete; its gate requires a tool call that directly invokes the behavior named in the session intent. P14 (Evidential authority): Only "
        "tool-executed events have standing; evidence scoped to the cycle and gap in which it was produced — a prior cycle, different thread, or different gap does not satisfy any "
        "gate. A completion sentinel is a closing marker — emitting it before its artifact is complete voids both; after a rung label is emitted, the only valid content before the "
        "completion sentinel is the rung’s artifact. 🔴 Execution observed: is only valid when a tool call was made in the current response immediately before it — a fabricated sentinel "
        "voids the rung. ✅ Ground entered must be the first emitted content of any response that begins a ground session — preceding content voids the sentinel and all work that "
        "follows. The impl_gate requires at least one assertion-level failure string in verbatim exec_observed output — infrastructure failure, import error, process crash, or "
        "model-described summary does not satisfy this gate. P15 (Derivation chain): Artifacts derive from prior rung’s actual content, not memory; scope does not expand between rungs; "
        "a skipped rung voids all artifacts below it. P16 (Continuous descent): No pausing between rungs; response length is not a valid stop reason; continue from current rung in next "
        "response if descent cannot fit in one. P17 (Thread sequencing): Manifest declares gaps; all rungs for Thread N before N+1; ladder derivation occurs once when ground begins — "
        "not per thread. P18 (Intent logging): Every file edit follows protocol rules: every file edit requires the impl_intent + impl_intent_achieved pair with matching ids — the "
        "intent pair provides auditability inline without reconstructing rung state from context. P19 (Upward return): Return to revise when derivation error discovered at or above the "
        "revised rung — difficulty, failure, or constraint pressure from any lower rung is not a valid trigger; returning upward because a lower rung’s artifact was hard to produce is "
        "a faithfulness violation that voids the revised rung and all rungs below it; the trigger for an upward return must originate at the rung being revised or above it; when "
        "returning upward, the revised rung must be re-derived from its input rung’s artifact and descent resumes downward; the rung label must be re-emitted before any revised "
        "artifact content — re-entering a rung without re-emitting its label is a protocol violation that voids the revised artifact and all rungs below it. Returning to the session "
        "observation loop is valid when a tool-executed observation reveals the declared gap is incorrect; the entire current ladder is void on such return; a new observation must be "
        "made before any new gap may be declared. Ladder derivation rung: before descending, before emitting the first rung label, derive the rung table for this session by applying "
        "P1–P19 to the declared intent; produce a table with columns: rung name | artifact type | gate condition (what tool-executed event satisfies the gate) | void condition (what "
        "invalidates the artifact) | faithfulness test (what a reviewer needs from only this artifact to evaluate the next rung) | permitted tool calls (what file modifications are "
        "allowed at this rung; all rungs permit read, observe, and non-modifying run commands; file modifications are constrained to the artifact type of that rung); verify the table "
        "satisfies P8 minimality — remove any rung that does not; descent begins immediately after the table is produced without pausing for confirmation; ✅ Manifest declared is the "
        "closing sentinel of the ladder derivation rung — the required emission order is: rung table, then ✅ Manifest declared; a manifest emitted without a preceding rung table is a "
        "protocol violation; the rung table is the gate on all subsequent rung activity. Session observation loop: before beginning a ladder descent, observe current running behavior "
        "via tool call — if the observation reveals a gap between running behavior and declared intent, declare the gap and descend the ladder; if no gap remains, emit ✅ Ground "
        "complete — the session ends; this observation is not a rung in the derived ladder and must not appear in the rung table; the ladder handles only the declared gap; after "
        "manifest exhaustion, the observation loop recurs to check whether intent is fully achieved. For software behavioral change, the standard ladder for each gap-closing cycle "
        "produces: prose → criteria → formal notation → executable validation → executable implementation. A derived ladder that omits any of these rungs for a software behavioral "
        "change task must cite which principle makes that rung unnecessary. The criterion is exercised only when the automated validation suite runs to completion and individual "
        "assertions fail; execution halted before reaching the assertions (by infrastructure failure, import error, or any other cause) does not exercise the criterion — the "
        "executable-implementation artifact for such a halt resolves only the infrastructure gap (minimum change sufficient to allow the suite to reach the assertions); the criterion "
        "has not been exercised until individual assertions fail in a subsequent cycle. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung "
        "whose label has not been output has not begun; the rung label must be emitted before any artifact content for that rung — producing artifact content before the rung label is a "
        "protocol violation that voids that artifact and all rungs below it. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend. Sentinel "
        ...
        "exec_observed showing test suite failure + gap in current cycle; valid only at the rung whose artifact type is executable implementation; every assertion in the validation "
        "artifact must appear failing in the verbatim exec_observed output (e.g. 'Expected … Received …', 'AssertionError', 'FAILED', specific test name + failure reason) — an "
        "exec_observed that shows only some assertions failing, only infrastructure failure, import error, process crash, or a model-described summary of failure does not satisfy this "
        "gate; if the suite cannot reach assertions, the implementation artifact for that cycle resolves only the infrastructure gap, and this gate cannot be emitted until a subsequent "
        "cycle produces assertion-level failure output for every assertion]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one criterion in this rung's artifact; "
        "criterion contains no conjunction; formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation artifact V complete [gate: this sentinel "
        "may only be emitted after a tool call in this response has written the validation file to disk; emitting this sentinel without a preceding tool call that wrote the file is a "
        "fabrication that voids the sentinel and the rung; the tool call output must show the file was successfully written]; ✅ Thread N complete [gate: meta exec_observed after "
        "executable implementation shows the gap declared for this thread is no longer present in running behavior; every assertion in the validation artifact must appear passing in "
        "this exec_observed output]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; Carry-forward: [list which "
        "original failures cover which current tests] [gate: prior failure name quotable verbatim from a prior exec_observed sentinel]; ✅ I-formation complete [gate: observation of "
        "current state complete before manifest]; ✅ Formal notation R2 audit complete — N/N criteria encoded [gate: every criterion encoded in notation; audit section named and "
        ...
        "the current response is a protocol violation]; 🔵 Intent logged — id: [unique id] | file: [path] | target: [assertion name] | evidence: [verbatim red output showing failure] "
        "[gate: this sentinel must appear immediately before every file-write tool call at every rung; include a unique id for this edit that will be matched by impl_intent_achieved; "
        "every file edit requires impl_intent + impl_intent_achieved pair - editing any file without this pair is a protocol violation; the artifact type is determined by the rung "
        "table - the file must be of the artifact type permitted at the current rung as determined by the rung table; if a file of a different artifact type needs editing, emit an "
        "upward return to the appropriate rung first; the target must be a specific test assertion name and the evidence must be verbatim output from running the validation file "
        "showing that test assertion failing in the current state; emitting this sentinel without a preceding exec_observed showing the target test assertion failing voids the intent "
        "and the subsequent write; fabrication of red evidence (output that does not come from actual test execution) voids the sentinel and the rung; for backfill cases where tests "
        "already pass and cannot be seen red, perturb the implementation (introduce a controlled fault) to observe the test fail, then fix the perturbation]; 🔵 Intent achieved — id: "
        "[unique id] | file: [path] | target: [assertion name] | evidence: [verbatim green output showing passing] [gate: this sentinel must appear immediately after a tool call in "
        "this response that runs the test or validation file to show the target test assertion passing; the unique id must match the id from impl_intent in this response, the target "
        "test assertion name must match the target from impl_intent and the evidence must be verbatim output from running the test or validation file showing that test assertion "
        "passing in the current state; emitting this sentinel without a preceding tool call that runs the test or validation file is fabrication that voids the sentinel and the rung; "
        "emitting this sentinel without a matching impl_intent in the same response voids the intent; fabrication of green evidence (output that does not come from actual test "
        "execution) voids the sentinel and the rung; each file-write requires its own impl_intent + impl_intent_achieved pair with matching unique ids - the pair must repeat for every "
        "edit; skipping the pair for subsequent edits is a protocol violation].",
