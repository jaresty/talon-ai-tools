...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "interpretation. Step 2 — Introduce provisional sub-properties. Introduce provisional candidate sub-properties whose conjunction is proposed to represent the current property: "
        "'property [Na]: <definition>' and 'property [Nb]: <definition>'. Apply the canonicalization procedure (Step 1) to each provisional sub-property before proceeding. The "
        "resulting canonical definitions replace the provisional definitions for all subsequent split evaluation. The labels [Na] and [Nb] continue to identify the resulting canonical "
        "definitions after replacement. Split evidence is attached to the canonical definitions of the current property and its provisional sub-properties, and to the falsifying "
        "constructions emitted in the split test, at the moment the split test is emitted. Any modification to any of those definitions or constructions after the split test is emitted "
        "voids that evidence; the split evidence must then be regenerated against the current canonical state before the split may be accepted. Step 3 — Evaluate split validity. Before "
        "evaluating the split, verify that the chosen interpretation determines an accessible-observation boundary: the set of outputs, behaviors, states, and interactions that the "
        "party responsible for determining whether the request was satisfied can access without inspecting the implementation. If the chosen interpretation does not determine that "
        "boundary, emit 'Interpretation refinement required: <what the boundary must specify>' and emit or refine the Interpretation: line to supply it. An interpretation refinement "
        "voids the entire current retained-property tree, all previously derived properties, and all split evidence, and restarts property derivation and recursive decomposition from "
        "the request constraints under the refined interpretation; any previously emitted split test is void and does not constitute a completed split test for the current canonical "
        "state. After the accessible-observation boundary has been established and the current property, its provisional sub-properties, and their split evidence have been regenerated "
        "under the current interpretation, emit exactly one non-void candidate split test for the current canonical state: '§ split test: property [N]: \"<verbatim canonical "
        'definition>" ⇔ (property [Na]: "<verbatim canonical definition>" ∧ property [Nb]: "<verbatim canonical definition>"); property [Na] — falsified by <construction>; property '
        "[Nb] — falsified by <construction>; these falsifiers are independent.' The quoted definitions for [N], [Na], and [Nb] must match the verbatim canonical definitions confirmed "
        "in Step 1 for each respective property. A split may be accepted only if its split test, quoted definitions, falsifying constructions, and interpretation are identical to the "
        "non-void state against which the split test was emitted. A split succeeds only if: for every admissible construction x, P(x) ⇔ (PNa(x) ∧ PNb(x)) — the original property is "
        "logically equivalent to the conjunction of the accepted sub-properties over all admissible constructions; each quoted falsifier violates exactly its quoted property while "
        "satisfying the other; both falsifiers are admissible constructions within the chosen interpretation; the candidate split is nontrivial: neither accepted sub-property is "
        "logically equivalent to the original property over all admissible constructions, and neither accepted sub-property logically entails the other over all admissible "
        "constructions; each accepted sub-property corresponds to a distinct constraint on the admissible construction under the chosen interpretation that is independently required by "
        "the explicit request constraints, and the request itself distinguishes the two sub-constraints at its own granularity — meaning the request separately names, or would be "
        "independently judged as failing on, each sub-constraint on its own; a candidate split is not valid merely because it decomposes the mathematical or definitional structure of a "
        "predicate, is not valid merely because each sub-property is independently testable, assertable, or useful as verification evidence, and is not valid merely because the two "
        "sub-constraints are logically distinct when the request does not itself distinguish them at its own granularity — if a single request constraint is being divided into finer "
        "conditions that the request never separately names or judges, the property is atomic at the request's granularity and must not be split; and neither accepted sub-property "
        "constrains an internal mechanism, architecture, algorithm, or representation as the primary subject of the constraint, unless that mechanism is explicitly required by the "
        "chosen interpretation. A sub-property constrains an internal subject when its canonical definition makes an internal mechanism, architecture, algorithm, or representation the "
        "primary subject of the constraint — this classification is independent of whether that subject happens to be externally observable. A sub-property is externally observable "
        "when its truth value can be determined solely from the accessible-observation boundary of the chosen interpretation, without inspecting the implementation. These two criteria "
        "are distinct: a property can concern an internal subject yet still be externally observable (for example, when the interpretation explicitly exposes that subject through its "
        "output contract), and a property can concern an external subject yet still be inaccessible if the interpretation does not expose the relevant behavior. The split condition "
        ...
        "Step 4 — Accept or mark atomic. A property may be marked atomic only after Step 2 has introduced provisional sub-properties, Step 1 has canonicalized them, and Step 3 has "
