...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "property that is not observationally independent is observationally redundant and must be resolved — either successfully removed or successfully merged — provided the "
        "resolution does not reduce coverage of the explicit request constraints. A redundant property remains unresolved until either a removal or a merge replacing it is successfully "
        "committed; a failed merge or failed removal does not resolve the redundancy and requires another valid independence action. A proposed merge creates a candidate retained set "
        "by replacing its source properties with the merged property: the merged property must first complete canonicalization, scope validation, and recursive decomposition to an "
        "atomic state; once atomic, it replaces its source properties in the candidate retained set. A proposed removal creates a candidate retained set by removing the selected "
        "property; the source retained set remains unchanged until the candidate is validated. For both merge and removal, the candidate retained set must re-run the completeness-check "
        "procedure and observational-independence procedure; the candidate set may not be committed unless a new '§ completeness check:' is followed by '§ properties complete? yes' and "
        "the candidate set satisfies observational independence. If those conditions are met, commit the change; otherwise discard it and leave the source retained set unchanged, then "
        "attempt another valid independence action for the unresolved redundant property. The Ground properties block is complete only when all retained properties simultaneously "
        "satisfy: they are derived from the chosen interpretation; they are atomic; their conjunction logically entails every explicit request constraint; they are observationally "
        "independent; they introduce no out-of-scope constraints, including internal mechanism constraints not explicitly required by the chosen interpretation. When all conditions are "
        "met, emit '§ ground complete' as the final line of the Ground properties block — this sentinel may not appear before '§ properties complete? yes' in the transcript. Any test, "
