...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "prerequisite absence, refinement may only restore guard executability; do not implement downstream behavior during prerequisite repairThis checklist must appear BEFORE the "
        "first ladder rung begins. The derivation results must be in the chat, not only internal. Before starting each rung, re-emit key rules relevant to that rung. BEFORE "
