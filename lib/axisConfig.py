...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "break immediacy. Artifact identifier must differ: the artifact identifier of the named governing artifact must differ from the artifact identifier of the artifact being "
        "modified. Semantically-equivalent refactor exception: (g) is also satisfied when an executor result block containing (a) before (c) appears in the transcript before the "
        ...
        "produces (b) at the position where (a) was absent. Per-behavior scoping: a governed artifact-producing action is compliant when (g) is satisfied for the behavior named in the "
        "Falsify derivation block's (d) entry — the behavior name is fixed at derivation time and may not be restated or narrowed after (g) is observed; the action is not required to "
