...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the task — and is minimal: one criterion, one notation, one test, one edit; further artifacts emerge from subsequent gaps after each cycle completes. Formal notation encodes "
        "only the criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only validation artifacts may be produced at the executable validation rung — "
        ...
        "fail before any implementation edit — a green run with no prior red run does not satisfy this rung. At the validation run observation rung, run the validation artifact; emit 🔴 "
        "Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; if the observed gap matches the "
        "prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Before each edit, check: does a 🔴 Execution observed: sentinel exist for "
        "the current gap? If not, run the validation artifact now before editing. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. "
        ...
        "When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend. Reconciliation gate: at every upward rung "
        "transition — whether the scheduled formal-notation → executable-validation gate or any upward return triggered by new information — reconcile all artifacts at that rung "
        "against I before redescending; documentation is a reconciliation artifact, not a thread. Seven rungs in order: prose → criteria → formal notation → executable validation → "
        "validation run observation → executable implementation → observed running behavior. Sentinel formats — 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, "
        "complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation artifact V "
