...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "sentinels enforce one gap at a time [ ] File edit declarations: derive from P4 (file edit protocol) — what must you declare before edits at each rung to prove you are "
        "following protocol; why is explicit declaration necessary; what happens if you don't declare [ ] Evidence requirements: derive from P3 (evidence required) — what evidence must "
        "accompany completion sentinels; why raw assertion text matters; how to cite evidence properly [ ] File manifest: derive from P4/P6 — why must validation rung list all files "
        "created; what happens if you skip this [ ] Upward scan: derive from P5/P6 — how to verify all criteria are addressed; why explicit checklist format (✅/❌) matters; what "
        "contradicts 'complete' claims This checklist must appear BEFORE the first ladder rung begins. The derivation results must be in the chat, not just in your head. derive: "
