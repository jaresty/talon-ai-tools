...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "behavior — the named test must be capable of catching this specific behavior; a claim that the named test cannot catch this behavior must be grounded in a prior execution "
        "result showing the test passing while the behavior was absent — quote that result; a reasoned claim of incapability without such a quote is not valid and the slower check "
        "stands as derived; if no capable test can be named and no such quote can be produced, the slower check is not derived and the candidate check must not be named — choose a "
        "different candidate for which a nameable test exists; second, name the candidate check — the specific artifact to run, naming its relative speed compared to alternatives; "
        ...
        "irrelevant, not insufficient, and must not be named here — and quote the specific output from a prior run result showing the insufficient check passes when the behavior is "
        "absent; if no such insufficient check output exists above this line in the transcript, the candidate is not derived and a different check must be chosen. Then run the "
