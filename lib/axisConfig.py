...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "whether intent is fully achieved. For software behavioral change, the standard ladder for each gap-closing cycle produces: prose → criteria → formal notation → executable "
        "validation → executable implementation. A derived ladder that omits any of these rungs for a software behavioral change task must cite which principle makes that rung "
        "unnecessary. The criterion is exercised only when the automated validation suite runs to completion and individual assertions fail; execution halted before reaching the "
        "assertions (by infrastructure failure, import error, or any other cause) does not exercise the criterion — the executable-implementation artifact for such a halt resolves only "
        "the infrastructure gap (minimum change sufficient to allow the suite to reach the assertions); the criterion has not been exercised until individual assertions fail in a "
