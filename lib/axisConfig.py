...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation files so tests can RUN (imports resolve) — stubs must be MINIMAL: just function signatures returning null or empty values, NOT complete implementations; include "
        "ONLY COMMENTS using GIVEN-WHEN-THEN format describing the assertions that will be tested — do NOT include actual expect() assertions at this rung; writing actual test "
        "assertions (expect() calls) at validation rung is a protocol violation; tests should be EMPTY skeletons that pass because they have no assertions. Assertions are written at "
        "the assertion rung, not validation. Tests MUST PASS at validation rung to move to assertion rung. Assertion rung: write ONE specific assertion using expect(), run tests to see "
        "it FAIL (red), then proceed to implementation rung to make that assertion pass. One assertion at a time: write assertion → see red → implement → see green → move to next "
        "assertion. Implementation rung: implement ONLY enough to make the CURRENT assertion green — take the perspective of an adversarial implementer; implement what the test "
        "demands, nothing more; tests drive the behavior needed, not your assumptions about what the feature should do. ASSERTIONS are test code that verifies behavior, NOT import "
