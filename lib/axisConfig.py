...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "earlier rungs violates this constraint regardless of whether the earlier content is visible in the transcript. The behavioral observation — the executed FAIL result showing "
        "the system's actual behavior against the absent governed behavior — is the rung where reality enters the chain; every rung before it is agent-authored artifact, and every rung "
        ...
        "satisfaction independently of any other listed dimension; when every dimension carries an '[observable:]' tag, write '§2 dimensions closed'; (3) derive the enforcement "
        "sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the transcript, and must appear before the first file-modifying tool call regardless of "
        ...
        "(4) verify coverage: '## Completion check' must not appear before '## Enforcement sequence' in the transcript and must not appear before '§ implementation permitted' in the "
        "transcript; each covered dimension cites a verbatim string that appears either as a substring of a tool-result block produced after '§ implementation permitted' and visible "
        "above '## Completion check', or, when no tool-result blocks are present after '§ implementation permitted', as a substring of the response text produced in this turn; when "
        "every covered dimension cites such a substring, write '§4 coverage verified'; '## Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a "
        ...
        "name the literal string whose presence closes it; write '§5 enumeration complete' when no further open path remains; any referent named in the '[text]' from §1 not already "
        ...
        "implementation permitted [N]' where N is the 1-based index of the file-modifying tool call this sentinel governs must appear in the transcript after '§5 enumeration complete'; "
        "no file-modifying tool call may appear before the first '§ implementation permitted [N]' sentinel; the literal string '§ implementation permitted [N]' with N matching the "
        "1-based index of the governed tool call must appear immediately before the `(i)` line of the five-line block in the assistant text block immediately before each file-modifying "
        "tool call, in the same response turn as that tool call — a user message appearing between '§ implementation permitted [N]' and the tool call does not satisfy this requirement; "
        "a file-modifying tool call not immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the five-line block does not satisfy this "
        "requirement. Quoted-span exclusion: an occurrence of any named sentinel or heading string inside a markdown code fence (a span beginning with ``` or ~~~ on its own line) does "
        "not satisfy any rung or ordering requirement — only occurrences outside code fences count toward compliance. Continuation invariant: when the final non-blank content line of a "
        "turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage verified', or '§5 enumeration complete', "
        "that turn must end with one of: '§ blocked: <text>', '§ awaiting: <text>', or '§ no-next-action: <text>', where '<text>' is non-empty. A turn whose final non-blank content "
        "line contains or follows one of these strings and does not end with one of the three exit strings does not satisfy ground.",
