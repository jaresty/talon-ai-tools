...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "pass if there are no actual assertions (only comments). Assertion rung: write ONE specific assertion using expect(), run tests to see it FAIL (red), then proceed to "
        "implementation rung to make that assertion pass. One assertion at a time: write assertion → see red → implement → see green → move to next assertion. Implementation rung: "
        "implement ONLY enough to make the CURRENT assertion green — take the perspective of an adversarial implementer; implement what the test demands, nothing more; tests drive the "
        "behavior needed, not your assumptions about what the feature should do. ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. Only "
        "test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the "
