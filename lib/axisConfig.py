...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect. the criteria rung artifact is exactly one criterion for the "
        ...
        "writing two criteria under one label is a protocol violation; The formal notation rung separates behavioral specification from explanation — formal notation encodes what must "
        "be true in a form that is executable or testable; valid notation forms include type signatures, interfaces, pseudocode, and invariant expressions; verbatim restatement of the "
        "criterion is permitted when the criterion is already a complete behavioral specification; the test of valid formal notation is that it cannot be directly compiled or executed "
        "without modification; prohibited: complete function bodies, variable assignments, constant declarations (const, let, var assignments) regardless of what they assign; labeling "
        "content as pseudocode does not exempt it from the prohibition if it is implementation-shaped. it must encode all structural constraints the criterion implies — natural "
        "language may add context but may not substitute for notation; prose adds context and introduces the specification — it does not encode the constraints. the audit section is "
        "separate and named and must appear before the completion sentinel — the sentinel closes the audit section; it does not constitute it; emitting it before the audit section "
        "inverts causality; the rung is incomplete until all criteria are encoded. Natural language may appear as section labels but may not substitute for encoding a constraint in "
        "notation — if a constraint can be stated in notation, it must be. each test function asserts exactly one behavioral property — one behavioral property per test function; "
        "multiple assertions are permitted only when they jointly constitute a single indivisible check. before writing the test, check for an existing test file covering the same "
        "module — add there if so. confirm via tool call that the criteria artifact for this cycle was produced in this cycle's transcript; if the criteria artifact is identical to a "
        "prior cycle’s criteria artifact, the criterion has not changed — return to the prose rung and re-emit with an updated gap. carry-forward format: 'Carry-forward: prior failure "
        "[verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]'; modification without carry-forward "
        "is a traversal violation. when the declared intent is to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the validation "
        "artifact must be a meta-test — an executable artifact that fails when the test-under-modification does not exhibit the criterion. when a test assertion is changed to match "
        "implementation output rather than the declared criterion, this constitutes a criterion change — return to the prose rung, re-emit with the revised gap, and descend; a test "
        "expectation update made without re-emitting prose is a protocol violation. before emitting ✅ Validation artifact V complete, the artifact content must appear in the current "
        "response; artifact content means each test block (name + assertion body) — imports, describe wrappers, and setup blocks are not required; emitting the sentinel without any "
        "test block in the transcript is a protocol violation; if no P4 EV step (1) pre-existence check tool-call result exists, ✅ Validation artifact V complete may not be emitted; a "
        "file-write tool call result showing the artifact was written must appear before V complete — artifact content appearing in prose without a file-write tool call does not "
        "satisfy this gate; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it "
        "passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes after "
        ...
        "harness error before treating any run as a red witness. Harness error routing (all three block 🔴 Gap: and 🟢 Implementation gate cleared; HARD STOP is not valid): "
        "missing-implementation-file → EI directly; test-file-error (syntax error, import of non-existent component) → EV repair; test-interaction-failure (pointer-events: none, role "
        "not found, element not accessible) → EV repair; this is a harness error class, not a behavioral failure. a green 🔴 Execution observed: is only valid as the post-implementation "
        "green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run "
        "observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple "
        "failures or plan fixes. re-running VRO without an implementation edit is a protocol violation — every re-run of the validation artifact must be preceded by an EI rung edit. "
        "after a red VRO that is not a harness error, after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token — this shortcut applies only when a red "
        "VRO already exists in the current cycle for the current criterion; a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO must fire before impl_gate; skipping "
        "EV and VRO for a new criterion introduced by HARD STOP is a protocol violation. a green exec_observed without a prior red run means the test is vacuous — return to the "
        "executable validation rung and rewrite the test to fail. a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed output. 🛑 HARD STOP — upward "
        "return to criteria rung is valid in exactly one case: the current criterion for this thread is identical to the criterion from the most recent prior cycle for this thread, "
        "confirming the criterion is underspecified (P5 condition); all other failure classes have defined routes — missing implementation file routes to EI directly; test-interaction "
        "failure routes to EV repair; implementation gap that changes between cycles loops within EI; spec gap returns to formal notation; using HARD STOP for any failure class other "
        "than criterion underspecification is a protocol violation. 🛑 HARD STOP position gate: valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation "
        "run observation rung in the current cycle — emitting it at any other position is a protocol violation; prose-only exec_observed sentinel does not satisfy this gate — the "
        "exec_observed block must contain non-empty verbatim tool output in a triple-backtick block; HARD STOP may not be emitted at the executable validation rung — a harness error at "
        "EV requires fixing the harness, not an upward return; when EV shows a harness error that is an import failure, the repair must re-enter at step (2a) of the current EV cycle — "
        "a freeform stub write that bypasses the import-check gate is a protocol violation; the only valid next action when EV shows a harness error is a tool call that fixes the "
        "harness; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the edit did not "
        "address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next "
        "token is the criteria rung label for the current thread. the VRO rung label must appear in the transcript for the current cycle. 🟢 Implementation gate cleared is the first "
        "token of the executable implementation rung — no tool call, no prose, and no file modification may appear before it in the current response; the next required action after 🟢 "
        "is a tool call that creates or modifies an implementation file; the implementation gate does not complete the thread; ✅ Thread N complete may not appear until the observed "
        "running behavior rung has fired for this thread; the OBR rung label must appear in the transcript for the current thread's current cycle before ✅ Thread N complete may be "
        "emitted — output produced before the OBR rung label may not be reused to satisfy the OBR exec_observed gate; its absence is a protocol violation, not a voiding condition. "
        "declaring a thread complete by similarity to another thread's failure is not valid — each thread requires its own full descent through OBR with a passing run; similarity does "
        "not substitute for a tool-executed result in the current cycle. prior evidence — passing tests, complete implementation, or prior cycles — does not satisfy the OBR gate; only "
        "a live-process invocation in the current cycle does. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists. If "
        "no invocable process exists, open a gap cycle to make the artifact directly invocable. if the 🔴 Execution observed: does not directly demonstrate the criterion, emit 🔴 Gap: "
        "naming what is undemonstrated and apply the upward-return failure-class rules; the exec_observed output must show the specific behavior asserted in the criterion — server "
        "liveness (a 200 OK, a page title, a process start message) does not satisfy this gate unless the criterion asserts server liveness; the query must exercise the criterion’s "
        "behavior directly, not merely confirm the server is running. ✅ Thread N complete is blocked until a live-process tool call result appears in the current thread’s current cycle "
        "— a test runner invocation, even in browser mode, does not satisfy this gate; the test suite run at step 5 presupposes the live-process invocation at step 3 has already "
        "produced an exec_observed result; if no live-process tool call result exists, the only valid next action is to start the process and query it. A full test suite run result is "
        "required after the most recent OBR tool call — the only valid next action if no such result exists is the tool call that runs the suite; every failing test must be explicitly "
