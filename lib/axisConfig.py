...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response applies a meta-process discipline before any implementation action, deriving and enforcing its own correctness conditions as transcript-inspectable strings. All "
        "compliance headings must be markdown '## ' headings — a line not beginning with '## ' does not satisfy any heading requirement. Each rung derives from the prior rung's output; "
        "no rung has a fixed step count. Derivation phase: (0) invoke the subject as a tool call; the artifact named in that invocation is the §0 identifier, fixed for all rungs; for "
        "non-software subjects, record an observation under a markdown '## ' heading naming the subject — an observation not under such a heading does not satisfy §0; write '§0 "
        "observed' when either a tool-result block or a compliant '## ' observation heading is present above it — '§0 observed' must not appear before one of these is present; (1) "
        "derive the governing goal: '## Governing goal: [text]' must not appear before '§0 observed' in the transcript; `[text]` must appear verbatim as a substring of the §0 "
        "tool-result block above — a `[text]` value not present as a literal string in that block does not satisfy §1; immediately below the heading, derive a means-test: write 'The "
        ...
        "names a more concrete alternative than those listed in §0; when goal and means-test are both present, write '§1 goal derived'; (2) derive behavioral dimensions: '## Behavioral "
        "dimensions' must not appear before '§1 goal derived' in the transcript; derive at least two dimensions; each dimension names a property of the response (not an artifact, "
        "state, or fix description) such that a response could satisfy it while failing at least one other listed dimension; each dimension quotes a verbatim substring of the '[text]' "
        "string from §1, or names a response property whose absence would leave a reader who accepted all other dimensions still uncertain whether '[text]' was achieved; each dimension "
        "ends with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose presence in the response constitutes satisfaction — if the response "
        "contains no tool-result blocks above §2, write '[observable: prose]' instead; a dimension that names an entity, a fix outcome, or a UI element is not a valid dimension "
        "regardless of provenance; derivation is unbounded: add dimensions, challenge each against the reader-uncertainty test, and revise until every dimension is independently "
        "falsifiable; when every dimension carries an '[observable:]' tag, write '§2 dimensions closed'; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear "
        ...
        "appear before '## Enforcement sequence' in the transcript; each covered dimension cites a verbatim string that appears as a substring of a tool-result block visible above '## "
        "Completion check'; when every covered dimension cites such a substring, write '§4 coverage verified'; (5) enumerate escape paths: '## Path enumeration' must not appear before "
        "'§4 coverage verified' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or "
        ...
        "the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§ implementation permitted' must "
        "appear in the transcript after '§5 enumeration complete'; the strings '§5 enumeration complete' and '§ implementation permitted' each appear exactly once; no file-modifying "
        "tool call may appear before '§ implementation permitted'; a response turn that declares the task complete or produces no further planned actions must be immediately preceded "
        "in the transcript by a '## Completion check' block — a done declaration appearing before '## Completion check' does not satisfy ground.",
