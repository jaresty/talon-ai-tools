...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "guard as written by constructing a concrete scenario in which property [N] is absent while the quoted guard nevertheless succeeds; emit 'Test blind-spot: found — "
        "<construction>' or 'Test blind-spot: not found'; if found, rewrite the guard and repeat from step (2); (4) execute the quoted guard against a violating stub — an "
        "implementation intentionally lacking property [N] — and emit 'Failure: property [N] — <verbatim quoted failure line from a tool-result block>' or 'Unobservable: property [N] "
        "— <reason>'; emit Unobservable only when no executable guard could ever distinguish satisfaction from violation regardless of implementation; if any executable guard can "
        "exist, it must be written, quoted, executed against a violating stub, and its failure line quoted instead; no target implementation may be written before a quoted Failure: "
        "line appears; (5) write the target implementation, then immediately emit 'Quoted implementation: <smallest contiguous verbatim text uniquely identifying the implementation>'; "
        "no subsequent step may occur until this sentinel has been emitted; (6) attempt to falsify the quoted implementation by constructing a concrete implementation or execution "
        "that satisfies the quoted guard while property [N] remains false; emit 'Implementation blind-spot: found — <construction>' or 'Implementation blind-spot: not found'; if "
        "found, strengthen the guard or rewrite the implementation and repeat from step (2). Every blind-spot analysis must reason only from the quoted artifact(s) emitted earlier in "
        "the transcript, not from an imagined or intended version. After all properties complete these steps, emit 'Coverage: complete' if all properties are resolved, or 'Coverage: "
        "gap — property [N]' for any unresolved property. A response that omits any required sentinel, performs a step before emitting its required quoted artifact, writes the target "
        "implementation before a quoted Failure: line, or leaves a property without a concrete automatic regression guard does not satisfy this token.",
