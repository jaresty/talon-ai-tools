...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "formalization complete', immediately write the label line 'Formalized properties:' — this block must appear before any artifact, implementation action, or '§ implementation "
        "permitted' line; decompose each §2 dimension into its formal atomic claims; each claim must appear on its own line beginning with the literal text 'property [N]:' where N is a "
        "1-based integer — a line beginning with '**property' or any markdown-formatted variant does not satisfy this requirement; each 'property [N]:' line must contain at least one "
        "mathematical expression — any notation in which every term has a fixed denotation independent of natural-language context (e.g. ∀/∃ predicates, typed function signatures, "
        "relational expressions, or interface-shape patterns such as check(G) → PASS | FAIL(reason)) — as a literal substring; a 'property [N]:' line containing no mathematical "
        "expression does not satisfy this requirement; after writing each 'property [N]:' line, immediately write '§ split test: [quoted sub-expression]' where [quoted sub-expression] "
        "is a phrase from the parent expression naming a candidate sub-expression to split on, followed by a sentence stating whether splitting at that sub-expression yields two "
        "independently falsifiable sub-properties — a 'property [Na]:' line that does not have a '§ split test:' line above it does not satisfy this requirement; if the split test "
        "finds a valid split, write one sub-property per branch: 'property [Na]:', 'property [Nb]:', etc., each containing a formal expression — a sub-property line whose content is "
        "prose only does not satisfy this requirement; if the split test finds no valid split (the property is already atomic), write 'property [Na]: atomic —' followed by a "
