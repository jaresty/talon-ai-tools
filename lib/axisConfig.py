...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "this documents the intent without writing assertions; create STUB implementation files for every component planned (empty functions returning null); no expect() assertions, no "
        "mock setup, no render; tests are EMPTY and pass trivially because they have no code; this rung establishes that tests can run and all components have stubs. Assertions are "
        "written at the assertion rung, not validation. Tests MUST PASS at validation rung to move to assertion rung. Assertion rung: EDIT existing test file to add ONE specific "
        "expect() assertion; run tests to see it FAIL (red). AFTER making assertion green (implementation succeeded), CHECK if gap is closed — if gap still exists, add another "
        "assertion; if gap is closed, emit completion sentinel and move to next gap. NO file creation in assertion rung — stubs created at validation rung; only edit existing test file "
        "to add assertions. Every file change requires justification: AFTER creating or editing any file, EXPLAIN WHY this change satisfies the requirements of the current rung; if you "
        "cannot justify why it satisfies the rung, ITERATE until it does; file changes without justification are protocol violations. Implementation rung: AFTER assertion rung shows "
        "red, implement ONLY what's STRICTLY NEEDED to make that specific assertion green — do NOT build complete components or do more than necessary; implement just enough to pass "
        "that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this "
        "is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, return UP to find "
        "next gap (ascend ladder); if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit "
