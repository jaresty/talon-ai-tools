...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "file alone what the implementation must do — the failure output is required. P6 (Artifact type discipline): One rung per type; text rungs produce no files; each rung may not "
        "be skipped or combined with another; frozen artifacts may not be modified at subsequent rungs; every file write must occur at the dedicated rung for that artifact type. An "
        ...
        "voids the rung; classification is determined by file content, not file path or naming convention — path and naming are evidence, not authority. P7 (Upward faithfulness): Lower "
        "rungs narrow what upper rungs permit; expanding the permitted space is a faithfulness violation. P8 (Rung validity test): Human reviewer with only this rung’s artifact can "
        "evaluate next rung; ladder is minimal — remove any rung that does not affect faithfulness evaluation. P9 (Information density preservation): Each rung encodes same/higher "
        "quality than above; a rung that loses information is a protocol violation. P10 (Three-part completeness): Observation + automation + implementation; all three must be present. "
        "P11 (Immediate lowest-rung observation): Observe at every feasible rung, beginning at the lowest; deferring is a protocol violation. P12 (Completeness slice): One "
        "independently testable behavior per thread per cycle; criterion is a falsifiable behavioral assertion — a feature name is not a criterion. P13 (Observation-first, "
        "observation-last): Open and close with tool-executed observation of live running code. Observing running behavior means invoking the system directly in a manner that exercises "
        "the behavior named in the intent — the output must be produced by the behavior itself, not by a test framework asserting properties of it; the tool call must execute live "
        "running code — reading files, grepping source, or inspecting static artifacts does not satisfy this requirement regardless of what the output shows; a test suite run does not "
        "satisfy this requirement regardless of whether the tests pass. The intent declaration must be derivable from the tool-executed opening observation. ✅ Ground complete may only "
        "be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation regardless of whether 🔵 Closing observation is present. 🔵 Closing "
        "observation must appear after ✅ Manifest exhausted and before ✅ Ground complete; its gate requires a tool call that directly invokes the behavior named in the session intent. "
        "P14 (Evidential authority): Only tool-executed events have standing; evidence scoped to the cycle and gap in which it was produced — a prior cycle, different thread, or "
        "different gap does not satisfy any gate. A completion sentinel is a closing marker — emitting it before its artifact is complete voids both; after a rung label is emitted, the "
        "only valid content before the completion sentinel is the rung’s artifact. 🔴 Execution observed: is only valid when a tool call was made in the current response immediately "
        "before it — a fabricated sentinel voids the rung. ✅ Ground entered must be the first emitted content of any response that begins a ground session — preceding content voids the "
        "sentinel and all work that follows. The impl_gate requires at least one assertion-level failure string in verbatim exec_observed output — infrastructure failure, import error, "
        "process crash, or model-described summary does not satisfy this gate. P15 (Derivation chain): Artifacts derive from prior rung’s actual content, not memory; scope does not "
        "expand between rungs; a skipped rung voids all artifacts below it. P16 (Continuous descent): No pausing between rungs; response length is not a valid stop reason; continue "
        "from current rung in next response if descent cannot fit in one. P17 (Thread sequencing): Manifest declares gaps; all rungs for Thread N before N+1; ladder derivation occurs "
        "once when ground begins — not per thread. P18 (Write authorization): Every file write immediately preceded by 🔵 Write authorized citing open rung name, artifact type, and file "
        "path; the cited artifact type must match the artifact type of the named file as determined by the rung table — a type mismatch voids the write and the rung; this sentinel "
        "exists so that every file write is auditable inline without reconstructing rung state from context. P19 (Upward return): Return to revise when derivation error discovered at "
        "or above the revised rung — difficulty, failure, or constraint pressure from any lower rung is not a valid trigger; returning upward because a lower rung’s artifact was hard "
        "to produce is a faithfulness violation that voids the revised rung and all rungs below it; the trigger for an upward return must originate at the rung being revised or above "
        "it; when returning upward, the revised rung must be re-derived from its input rung’s artifact and descent resumes downward; the rung label must be re-emitted before any "
        "revised artifact content — re-entering a rung without re-emitting its label is a protocol violation that voids the revised artifact and all rungs below it. Returning to the "
        "session observation loop is valid when a tool-executed observation reveals the declared gap is incorrect; the entire current ladder is void on such return; a new observation "
        "must be made before any new gap may be declared. Ladder derivation rung: before descending, before emitting the first rung label, derive the rung table for this session by "
        "applying P1–P19 to the declared intent; produce a table with columns: rung name | artifact type | gate condition (what tool-executed event satisfies the gate) | void condition "
        "(what invalidates the artifact) | faithfulness test (what a reviewer needs from only this artifact to evaluate the next rung) | permitted tool calls (what file modifications "
        "are allowed at this rung; all rungs permit read, observe, and non-modifying run commands; file modifications are constrained to the artifact type of that rung); verify the "
        "table satisfies P8 minimality — remove any rung that does not; descent begins immediately after the table is produced without pausing for confirmation; ✅ Manifest declared is "
        "the closing sentinel of the ladder derivation rung — the required emission order is: rung table, then ✅ Manifest declared; a manifest emitted without a preceding rung table is "
        "a protocol violation; the rung table is the gate on all subsequent rung activity. Session observation loop: before beginning a ladder descent, observe current running behavior "
        ...
        "Ground complete — intent achieved: [what the observation shows] [gate: 🔵 Closing observation must appear in the current response before this sentinel; meta exec_observed shows "
        "no gap between observed running behavior and declared intent; all manifest threads complete]; 🔵 Closing observation — [what the tool-executed observation shows] [gate: the "
        "tool call immediately before this sentinel must directly invoke the behavior named in the session intent and its output must show that behavior is present; this sentinel must "
        "appear after ✅ Manifest exhausted and before ✅ Ground complete in the same response; emitting ✅ Ground complete without a preceding 🔵 Closing observation in the current "
        "response is a protocol violation]; 🔵 Write authorized — rung: [rung name] | artifact type: [type] | file: [path] [gate: currently open rung (most recently emitted rung label "
        "with no completion sentinel yet) has this file's artifact type in its permitted-tool-calls column; no open rung → gate unsatisfied; artifact type mismatch → gate unsatisfied; "
        "the cited artifact type must match the artifact type of the named file as determined by the rung table's artifact-type column — a cited artifact type that does not match the "
        "file being written is a fabrication that voids the sentinel and the rung regardless of whether the open rung permits that type; a file write without a preceding 🔵 Write "
        "authorized voids the write and the rung in which it appears; a 🔵 Write authorized whose cited rung is not currently open is a fabrication that voids the write and the rung; "
        "this sentinel must appear immediately before the file-write tool call — intervening content between this sentinel and the tool call voids both].",
