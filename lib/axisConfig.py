...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "container (the file, module, or resource that would be created or modified as a whole) does not satisfy this requirement; the quoted scope text must be from a test assertion "
        "whose pass/fail state is determined solely by the named symbol — a test that passes or fails independently of that symbol does not satisfy this requirement; a statement in the "
        "derivation phase is a commitment only if its truth value can be determined from tool-executed results already present above it in the transcript — a statement whose truth "
        "value requires evaluating a result not yet present is not a commitment; after the call, a tool-executed run result must appear in the transcript in which the quoted scope text "
        "is absent, every other line from the immediately preceding pre-edit run result is present, and a result line appears for every test name that appeared in that pre-edit run "
        "result — the pre-edit run result for a given call is the run result that produced that call's scope commitment; a failing-item line from the pre-edit run result may be absent "
        "from the post-call run result only if its literal text appeared as a quoted scope string above this call and no other failing-item line from the same pre-edit run result also "
        "appeared as a quoted scope string above this call. Each file-modifying tool call's scope line must satisfy these requirements independently — a scope line from a prior call "
        "does not carry forward. Then, immediately before each file-modifying tool call — as the last assistant text block before that call — write a block containing exactly these "
        "four lines in order: `(i) scope inflation: <string>`, `(ii) stale quote: <string>`, `(iii) symbol cardinality: <string>`, `(iv) post-call line mismatch: <string>`. A "
        "file-modifying tool call not immediately preceded by a block whose last four content lines begin with `(i)`, `(ii)`, `(iii)`, `(iv)` in that order does not satisfy this "
        ...
        "derives from the prior rung's output; no rung has a fixed step count. Path classification (required before §0): if a named artifact — file path, repo URL, endpoint, or shell "
        "command — appears verbatim in the conversation above, apply Path A; otherwise apply Path B. Derivation phase: Path A — (0) invoke the named artifact as a tool call; a tool "
        "call whose result contains at least one line not present in the invocation argument satisfies §0; write '§0 observed' when that tool-result block is present above it — '§0 "
        "observed' must not appear before such a block is present; (1) derive the governing goal: '## Governing goal: [text]' must not appear before '§0 observed' in the transcript; "
        "`[text]` must appear verbatim as a substring of the §0 tool-result block above — a `[text]` value not present as a literal string in that block does not satisfy §1; "
        "immediately below the heading, derive a means-test: write 'The goal [text] could be achieved by: [alternative means not listed in §0]' — if no concrete alternative can be "
        "named, write '[hypothetical]: [speculated alternative]' and fetch a broader context artifact before closing §1; if two non-subsuming goal candidates exist, write both and mark "
        "one '[selected]' — the selected goal is the one whose means-test names a more concrete alternative than those listed in §0; when goal and means-test are both present, write "
        "'§1 goal derived'; Path B — (0) write the literal string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — this declaration satisfies §0; (1) "
        ...
        "from the user's message — a '[derived: text]' value that adds a claim absent from the user's message does not satisfy §1; immediately below the heading, derive a means-test: "
        "write 'The goal [text] could be achieved by: [alternative means]'; when goal and means-test are both present, write '§1 goal derived'; (Both paths continue from §2 onward:) "
        "(2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1 goal derived' in the transcript; derive at least two dimensions; each dimension names a "
        ...
        "quotes a verbatim substring of the '[text]' string from §1, or names a response property whose absence would leave a reader who accepted all other dimensions still uncertain "
        "whether '[text]' was achieved; each dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose presence in the response "
        "constitutes satisfaction — if the response contains no tool-result blocks above §2, write '[observable: prose]' instead; a dimension that names an entity, a fix outcome, or a "
        "UI element is not a valid dimension regardless of provenance; derivation is unbounded: add dimensions, challenge each against the reader-uncertainty test, and revise until "
        "every dimension is independently falsifiable; when every dimension carries an '[observable:]' tag, write '§2 dimensions closed'; (3) derive the enforcement sequence: '## "
        ...
        "coverage: '## Completion check' must not appear before '## Enforcement sequence' in the transcript; each covered dimension cites a verbatim string that appears as a substring "
        "of a tool-result block visible above '## Completion check'; when every covered dimension cites such a substring, write '§4 coverage verified'; (5) enumerate escape paths: '## "
        ...
        "exactly once; no file-modifying tool call may appear before '§ implementation permitted'; a response turn that declares the task complete or produces no further planned "
        "actions must be immediately preceded in the transcript by a '## Completion check' block — a done declaration appearing before '## Completion check' does not satisfy ground.",
