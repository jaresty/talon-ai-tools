...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "character the tool produced — no editorial closing; if the 🔴 Execution observed: sentinel is not immediately followed by a triple-backtick delimited block containing verbatim "
        "tool output, the sentinel is void — emitting the sentinel with prose content or no delimited block is a protocol violation equivalent to not emitting the sentinel; fix by "
        "invoking the tool and re-emitting. The execution-observed sentinel gates the tool invocation — the only valid next action after emitting this sentinel is the tool call it "
        "names; intervening content converts the sentinel to retroactive — re-emit and re-run. Once ✅ Ground entered has been emitted in this session, every subsequent response must "
        "begin by identifying the current rung and the current gap before any other content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the "
        "session ends or the user explicitly exits. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a "
        "protocol violation. The gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; "
        "“need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N "
        "complete sentinel exists for each thread declared in ✅ Manifest declared — before emitting ✅ Manifest exhausted, locate the N in the ✅ Manifest declared sentinel and count ✅ "
        "Thread N complete sentinels — if the count does not equal the declared N, the manifest is not exhausted. From the criteria rung onward, the gap names a specific behavior "
        "currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not. Each rung’s "
        "artifact addresses only that gap — not all known requirements of the task — and is minimal: one independently testable behavior derived from the prose alone — if stating it "
        "requires names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is invalid — split it before continuing (‘the "
        "table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a structural element and its data source is a "
        ...
        "internals, the criterion is structural and must be rewritten before this rung closes; this is a content gate, not a self-check; after emitting the criteria artifact, proceed "
        "immediately to formal notation — enumerating remaining criteria or planning future gap cycles before the current cycle completes is a protocol violation; the next criterion "
        "for this thread may not be named until ✅ Thread N complete has been emitted for the current criterion’s cycle — ✅ Thread N complete is the criterion-emergence gate; the first "
        "criteria rung after ✅ Manifest declared emits exactly one criterion for Thread 1 — emitting criteria for multiple threads at the first criteria rung is a protocol violation; "
        "the manifest declared the thread list; the criteria rung instantiates the first thread’s gap assertion only. Formal notation encodes only the criteria declared at the criteria "
        "rung — no additional invariants, no anticipated cases; it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, "
        ...
        "and the audit section still applies; the audit section is separate and named; the rung is incomplete until all criteria are encoded. Only validation artifacts may be produced "
        "at the executable validation rung — no other content is permitted before ✅ Validation artifact V complete; each test function asserts exactly one behavioral property — one "
        "behavioral property per test function; multiple assertions are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null "
        "and of the expected type); before writing the test, re-read the formal notation and assert each structural constraint it encodes; a pre-existing test that happens to pass does "
        "not satisfy this rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must be invocable by an automated tool; when the "
        ...
        "executable artifact that fails when the test-under-modification does not exhibit the criterion; before writing the test, check whether an existing test file covers the same "
        "module and add there if so; when the validation artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward "
        ...
        "implementation artifact may appear after modification until carry-forward has been emitted. before emitting ✅ Validation artifact V complete, confirm via tool call that the "
        "artifact path does not pre-exist or is already failing — V-complete may not be emitted without this tool-executed result in the transcript; the validation artifact must reside "
        ...
        "implementation edits may not begin until a red run exists in the transcript for the current cycle; ✅ Validation artifact V complete may not be emitted unless a 🔴 Execution "
        "observed: showing the test failing exists in the transcript for the current cycle; emitting V-complete before seeing a red run is a protocol violation regardless of whether "
        "perturbation was attempted. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is written; if the "
        "observed running behavior label is written without a ✅ Validation artifact V complete appearing in the transcript for the current cycle, the OBR rung is void — return to the "
        "executable validation rung and produce the artifact. Before writing the validation run observation rung label, invoke the validation artifact via a tool call; a harness error "
        "(import failure, syntax error, missing file) is not a red run — the test must have been loadable, its assertions must have run, and they must have failed; fix the harness "
        "error before treating any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same "
        "artifact appeared after the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; emit "
        "🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the "
        ...
        "executable validation rung — a harness error at EV requires fixing the harness, not an upward return; only a post-VRO Gap justifies HARD STOP; if the observed gap matches the "
        "prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Upon writing the observed running behavior label, re-emit the criterion for "
        "this cycle — criterion re-emission is required: emitting the OBR label without immediately following it with the criterion text is a protocol violation; ✅ Thread N complete "
        "may not appear without both the criterion re-emission and a 🔴 Execution observed: in the current OBR section; then immediately invoke the implemented artifact via a tool call "
        "and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a "
        ...
        "compilation result never satisfies this gate regardless of what it names; for a React or web UI component, the OBR invocation renders the component to DOM using the project’s "
        "render utility (e.g., renderToStaticMarkup, or the testing framework’s render function reading container.innerHTML) and emits the resulting HTML or text fragment showing "
        "browser-visible content — this is not a test assertion; it is direct DOM output showing the criterion’s named content is present in the rendered tree; starting a dev server "
        "does not satisfy this rung — server process state is not rendered content; if the implementation has no directly invocable artifact, invoke any non-test consumer of the "
        "changed artifact that demonstrates the declared behavior; a 🔴 Execution observed: whose verbatim block is empty or blank does not satisfy this gate; if the declared gap names "
        "a real network endpoint, the OBS must include verbatim output showing a real HTTP request or response — a test run whose network calls are mocked does not satisfy this gate; "
        ...
        "be located in the transcript, the tool call is prohibited; the provenance statement does not replace the tool call — it gates the tool call; the tool call must follow the "
        "provenance statement in the same response; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: with non-empty verbatim output appears in the transcript after "
        "the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol violation; implementation edits, new "
        "files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written after the most "
        "recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it that directly demonstrates the criterion — a test pass is "
        "not a demonstration; if the 🔴 Execution observed: at the observed running behavior rung contains test runner output (pass counts, test names, duration summary), it does not "
        "satisfy the OBR gate — re-invoke the implemented artifact directly before continuing; ✅ Thread N complete may not be emitted after an OBR 🔴 Execution observed: that shows test "
        "runner output; no implementation file was created or modified for this thread’s gap in this cycle means the gap was already closed (return to criteria) or the test is vacuous "
        "(rewrite it); every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — dismissing a failing test as unrelated without an explicit "
        "written acknowledgment is a protocol violation; ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: from a full test suite run exists in the transcript after "
        "the most recent OBR tool call in this thread — the only valid next action if no such result exists is the tool call that runs the suite; once ✅ Thread N complete is emitted no "
        "further output in that cycle is valid; at most one thread is in progress at a time — no output for thread N+1 may appear until ✅ Thread N complete has been emitted for thread "
        "N; this gate applies to all rungs, not only the prose rung. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running "
        ...
        "satisfy this gate. Reconciliation gate: at every upward rung transition, reconcile all artifacts against I and update any pre-existing document that records I (ADR, README, "
        "spec) before redescending. Before producing the final report, invoke the delivered system end-to-end via a tool call and emit 🔴 Execution observed: showing the primary "
        "behavioral intent is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before producing the final report. After emitting ✅ "
        "Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order — before writing each section, locate the artifact in the "
        "prior transcript; if it cannot be found verbatim in the transcript, it may not appear in the final report; no new behavioral claims, no coverage summaries, no suggestions; "
