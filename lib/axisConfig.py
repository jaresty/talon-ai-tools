...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "witnessed failing when its property is absent. Guard and artifact may be established in any order. A guard is an observation-producing procedure: a repeatable procedure that, "
        "applied to the artifact, yields an observation distinguishing the property's satisfying and violating states — a software test, a map or route check, a source inspection, a "
        "measurement, a trial, or a comparison are all instances. An observation witnesses an assertion only when the procedure evaluates that assertion and produces an "
        "assertion-specific result: Retrieving evidence for an assertion is not the same operation as witnessing the assertion. Information merely retrieved, inferred, or cited by the "
        "procedure is not itself the assertion's result — establishing that a procedure or its evidence is appropriate to the property is the work of ground and verify; falsify, once "
        "an assertion and its procedure are in play, obtains the procedure's own discriminating result for that assertion. What makes an observation a witness is that the procedure "
        "could have produced the violating result: it is capable of yielding either the satisfying or the violating observation, and is not constructed from the conclusion it is meant "
        "to evaluate. This is not asserted — it is shown, by perturbing the relevant state and observing the result flip (the satisfying observation on one state, the violating "
        "observation on a perturbed state); a procedure that yields the same observation regardless of the perturbed state does not distinguish the assertion and is not a witness. A "
        "guard is established by an observation-producing act whose result record contains the guard's procedure verbatim; every assertion referenced later must be copied verbatim "
        "from such a result record. What witnesses an assertion is that the procedure is assertion-bound and counterfactually independent; regression detection additionally requires "
        "the procedure to re-apply on its own. Regression-persistence is a separable capability that may be present or absent: a one-time application that is assertion-bound and "
        "counterfactually independent still witnesses its assertion now, and is a valid Failure or pass observation, even when it cannot re-fire later (a measurement taken once, an "
        "expert inspection) — only the claim that the guard will detect a future regression depends on persistence. When a Failure observation is intended as a standing regression "
        "guard, it must be produced by a committed procedure persisted to a location that re-applies without human initiation, not an inline or ephemeral one-off. Repeatable is not "
        "the same as deterministic: a guard whose observation depends on external or changing state (a live lookup, a measurement, a poll) is still a valid witness at the moment it is "
        "applied, but its observation and provenance must be retained rather than treated as reproducible on demand. Capability is not evidence: what witnesses a behavioral "
        "observation is an available qualifying observation record, not the mere fact that an act did or did not occur this turn. A qualifying observation record is a "
        "transcript-present record produced by the committed guard artifact that carries the guard's procedure identity, the assertion's identity, and the relevant A-result. A fresh "
        "application of the guard produces one; a prior result record already present in the transcript is one; model prose is never one. Observation availability is "
        "capability-relative, not tool-relative: whether the observation capability is available depends on whether the committed guard can be applied with the acting party's current "
        "capabilities, not on whether an external tool is involved — an inspection or derivation over the available artifact is itself an applicable observation procedure, and "
        "applying it is producing the observation. It counts as an observation only when that inspection or derivation is the assertion's qualifying procedure and produces an "
        "assertion-specific result, not because content was read or noticed: reading the artifact and finding text that resembles the property is not an observation until the guard "
        "evaluates the assertion against it. When the observation capability is available the response must apply the committed guard to obtain new observations; when it is "
        "unavailable it may cite qualifying observation records already present but must not synthesize, infer, or narrate an observation. A guard that is applicable but not yet "
        "applied is not unavailable — it is unobserved, and unobserved is a do-the-work state that requires application, not the honest terminal 'observation unavailable' state; "
        "'observation unavailable' is reserved for the case where no qualifying guard can be applied with current capabilities, never for a guard that is merely unapplied. Before "
        "declaring an observation unavailable, inspect the available observation records for a qualifying record and confirm no applicable guard remains unapplied; only when no "
        ...
        "\"<verbatim assertion>\" — observation unavailable'; it does not contribute to 'Coverage: complete', and it is a witnessing gap, not a structural exemption. Before acting, "
        "observe the gap: emit 'Observing gap: <what is currently absent or failing>'. Shrink the artifact to the guard (Gate 1). Always attempt the simpler artifact: construct a "
        "smaller modification to the artifact and apply the guards to it. 'Overreach: not found' is a claim that no such reduction exists, so it is valid only after an attempted "
        "reduction — a qualifying observation record showing a smaller artifact that the guards reject (or a smaller artifact they accept, which you then keep). If a reduction the "
        "guards still accept exists, or a path through the artifact is not exercised or distinguished by any guard, emit 'Overreach: found — <modification or path>', trim to the "
        "boundary the guards require, and repeat. A bare 'Overreach: not found' with no attempted reduction does not satisfy this token. Both 'Overreach: found' and 'Overreach: not "
        "found' are observation-dependent; when no qualifying observation is available, emit 'Overreach: untested — observation unavailable' rather than either verdict — untested is "
        "never 'not found'. Witness every assertion (Gate 2). Enumerate the assertions: emit one 'Assertion: <verbatim assertion text>' line for each assertion of the established "
        "guard, quoted verbatim from the guard's result record. The verbatim lines are the observable; no count or completeness sentinel is used, and a self-declared count does not "
        "satisfy it. Every enumerated 'Assertion:' line must then receive its own 'Failure: assertion \"<verbatim assertion>\" — \"<verbatim failure text>\"' or 'Unobservable: "
        "assertion \"<verbatim assertion>\" — structural' before 'Coverage: complete'; the binding is by verbatim identity — the assertion text in the Failure or Unobservable line "
        "must match an enumerated Assertion line exactly, and an enumerated assertion with no matching outcome line leaves coverage incomplete. After each Failure or Unobservable "
        "outcome, if any enumerated 'Assertion:' line still lacks a verbatim-matching outcome, return to the witness step for the next unwitnessed assertion and repeat; do not leave "
        "Gate 2 until every enumerated assertion has a matching outcome. Each enumerated assertion is either behavioral or structural. A behavioral assertion is witnessed only by a "
        "pair of A-observations: an application in which A's own guard-defined result is a failure, and one in which A's own result is a pass. An A-observation is an application of "
        "the committed guard in which assertion A is evaluated and produces its own guard-defined result. The guard defines the observable result for A: an A-pass is the guard's "
        "reported pass result for A and an A-fail is the guard's reported failure result for A. These results are observations of the guard, not independent claims about the truth of "
        "any property — whether A's result corresponds to the property A is intended to govern is not established by this token; that correspondence is established separately by the "
        ...
        "is evaluated and reports pass. The failure (A-fail) application must immediately precede the 'Failure:' line (it is the producer); the passing (A-pass) application is the "
        "contrast evidence and may appear elsewhere in the transcript. Any application that does not produce an A result is not an observation of A and cannot witness A, regardless of "
        "the cause or form of the application's outcome — this covers procedure errors, unrelated assertions failing, a guard that returns without reporting A, and any other "
        "application in which A itself was not evaluated; only an A-fail result can supply the violating observation, and only an A-pass result the satisfying contrast. In particular "
        "— as an illustration of this rule, not an exhaustive list — when the subject the assertion governs does not yet exist, an error that halts before A is reached (a software "
        "build or compilation error when the governed symbol is undefined; a route check that cannot run because no lodging has been chosen yet; a source lookup that fails because no "
        "source is cited) is tempting to read as 'the property is absent, so this is the failure'; it is not an A-fail, because A never ran — the application disqualified itself as an "
        "observation of A and fails identically no matter which property is absent. The A-fail you need is A's own assertion applied and reporting failure against a present-but-wrong "
        "artifact, which the subject's absence cannot produce. Both A-observations must be produced by the committed guard artifact, and the guard's output must identify A (by a name, "
        "path, or message that distinguishes A) so the pair is bound to this enumerated assertion rather than another. Provenance closure: any transcript reference that functions as "
        "evidence — the assertion identity, the A-fail result, and the A-pass result — must quote verbatim the text of the qualifying observation record that establishes it, and "
        "counts as evidence only if that exact text occurs in a referenced qualifying observation record; a result or identity typed as prose that does not appear verbatim in a cited "
        "qualifying observation record is not evidence and does not witness A. If the artifact already passes A, construct a temporary modification that makes A alone fail while every "
        "other assertion still passes, apply the guard, emit the Failure from that result record, then restore; a modification that fails every assertion at once discriminates none. "
        "Emit 'Failure: assertion \"<verbatim assertion>\" — <A-fail result> / <A-pass result>' quoting both observed results from their result records; a 'witness:' line, if added, "
        "only projects those two results and is not itself evidence. A structural assertion — one whose property has the artifact's own text or structure as its primary subject, so no "
        "behavioral failing application exists — is witnessed instead by 'Unobservable: assertion \"<verbatim assertion>\" — structural'; Unobservable is never satisfied by having "
        "searched for a failing application and found none. An assertion that is behavioral yet no admissible modification can make fail is a guard defect: strengthen the guard and "
        "repeat. A guard is a blind spot whenever an admissible state exists in which its pass-or-fail outcome is not equivalent to whether the property holds — check three: (a) proxy "
        "or non-exercising guard checking a surrogate; (b) durability — a guard gone at the moment it would need to fire; (c) ephemeral guard not committed to a location that "
        "re-applies. Verdict-follows-observation governs both gates: when the observation capability is available, every 'Overreach:' verdict and every 'Failure:' observation is valid "
        "only when it immediately follows a result record that mechanically produces it; a verdict emitted from description or analysis alone does not satisfy this token. A structural "
        "Unobservable assertion is exempt, having no application to anchor to. 'Coverage: complete' is valid only when every enumerated assertion has either a 'Failure:' line backed "
        "by its A-fail/A-pass application pair or an 'Unobservable: … — structural' line, and no 'Overreach: found' remains open; an 'Unwitnessed: … — observation unavailable' "
        "assertion leaves coverage incomplete and does not satisfy this token. When qualifying observation is unavailable for one or more behavioral assertions, the honest terminal "
        "state is 'Coverage: incomplete — observation unavailable', not a claimed completion; the protocol may construct guards, perturbations, and expected outcomes and identify what "
        "remains unverified, but it may not represent any observation-dependent verdict as observed.",
