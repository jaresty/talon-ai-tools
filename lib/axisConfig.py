...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "retained property — including each initial property [N] and each property added during the completeness phase — must be observable under the chosen interpretation unless the "
        "chosen interpretation explicitly requires an internal mechanism, architecture, algorithm, or representation as its subject; a property that constrains an internal subject "
        "without that explicit requirement is out of scope and must not appear in the retained set. A property is atomic when the recursive decomposition procedure reaches Step 4 and "
        "no valid split exists. Step 1 — Canonicalize the property. Emit: '§ formality check: property [N]: \"<verbatim definition>\" — [formal | informal: <reason>]'. If informal, "
        ...
        "canonical definition for this property — no subsequent step may quote a definition not yet confirmed formal. Step 2 — Introduce provisional sub-properties. Introduce "
        ...
        "the current property, its provisional sub-properties, and their split evidence have been regenerated under the current interpretation, emit exactly one split test: '§ split "
        ...
        "falsifiers are independent.' The quoted definitions must match the immediately preceding canonical provisional definitions verbatim. A split may be accepted only while its "
        "split test, quoted definitions, falsifying constructions, and interpretation are all unchanged from the state in which the split test was emitted. A split succeeds only if: "
        "the conjunction of the accepted sub-properties has identical coverage to the original property; each quoted falsifier violates exactly its quoted property while satisfying the "
        "other; both falsifiers are admissible constructions within the chosen interpretation; and neither accepted sub-property constrains an internal mechanism, architecture, "
        "algorithm, or representation as the primary subject of the constraint, unless that mechanism is explicitly required by the chosen interpretation. For this condition, a "
        "sub-property constrains an internal subject when determining whether the property is satisfied or falsified requires observing state, structure, or mechanism not accessible "
        "under the chosen interpretation's accessible-observation boundary. A sub-property is observable under the interpretation when its truth value can be determined solely from the "
        "outputs, behaviors, states, and interactions that the interpretation makes accessible to the evaluating party without inspecting the implementation. Step 4 — Recurse or mark "
        "atomic. If the split succeeds, replace the original property with the accepted canonical sub-properties and apply this same recursive decomposition procedure (Steps 1–4) "
        'independently to each accepted sub-property. If no valid split exists, emit: \'§ split test: property [N]: "<verbatim canonical definition>" — atomic, no valid split: '
        "<reason>.' The quoted definition must match the retained canonical property verbatim. Apply this procedure to every retained property until all have reached Step 4 with no "
        'valid split. After all retained properties are atomic, emit a completeness check: \'§ completeness check: "<request constraints verbatim>" / <P1 expression> / <P2 expression> '
        "/ ...' — the request constraints must be quoted verbatim from the chosen interpretation; every retained atomic property expression must appear exactly once, separated by ' / "
        "'; a '§ completeness check:' line that omits the request constraints or omits any retained property expression does not satisfy this requirement. Immediately after the "
        "completeness check, attempt to identify one explicit request constraint that is not represented by any retained property. If such a constraint exists, emit '§ properties "
        ...
        "constraint is represented by at least one retained property may the response emit '§ properties complete? yes' — a '§ properties complete? yes' line without a preceding "
        "completeness check does not satisfy this protocol. After completeness has been established, verify observational independence. A retained property is observationally "
        "independent if there exists at least one admissible construction that violates that property while satisfying every other retained property. Any retained property that is not "
        "observationally independent is observationally redundant and must be removed or merged, provided doing so does not reduce coverage of the explicit request constraints. If a "
        "merge or removal changes the retained property set, repeat the atomicity, completeness, and observational-independence procedures. The Ground properties block is complete only "
        "when all retained properties simultaneously satisfy: they are derived from the chosen interpretation; they are atomic; they collectively cover every explicit request "
        "constraint; they are observationally independent; they introduce no out-of-scope constraints, including internal mechanism constraints not explicitly required by the chosen "
        "interpretation. When all conditions are met, emit '§ ground complete' as the final line of the Ground properties block — this sentinel may not appear before '§ properties "
        "complete? yes' in the transcript. Any test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does not satisfy this protocol; if task "
        "reasoning begins and '§ ground complete' has not yet appeared, emit the Ground properties block in full and '§ ground complete' before continuing.",
