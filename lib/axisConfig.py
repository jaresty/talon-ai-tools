...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "satisfied for that non-void candidate; otherwise the candidate split is rejected. Step 4 — Accept or mark atomic. A property may be marked atomic only after Step 2 has "
        "introduced provisional sub-properties, Step 1 has canonicalized them, and Step 3 has attempted candidate-split construction against the current canonical state. If the "
        "candidate split test is accepted, replace the original property with the accepted canonical sub-properties and apply this same recursive decomposition procedure (Steps 1–4) "
        'independently to each accepted sub-property. If no candidate split can be constructed, emit: \'§ atomic: property [N]: "<verbatim canonical definition>" — no candidate split '
        "satisfies the construction requirements: <reason>.' If a candidate split test is emitted and rejected, emit: '§ atomic: property [N]: \"<verbatim canonical definition>\" — "
        "candidate split rejected: <reason>.' A property that represents a single request constraint at the request's own granularity is atomic even if its mathematical or logical "
        "expression admits formal decomposition, and even if that decomposition would yield logically distinct sub-conditions; the nontriviality and distinct-constraint conditions in "
        "Step 3 enforce this: a split into sub-properties that merely reformulate the mathematical definition of the predicate, or that subdivides a single request constraint into "
        "finer conditions the request never separately names or judges — rather than separating constraints the request itself distinguishes — is invalid and must be rejected as not "
        "nontrivial or not corresponding to distinct constraints at the request's granularity. The quoted definition must match the current canonical definition verbatim. Apply this "
        "procedure to every retained property until all have reached Step 4 and emitted '§ atomic:'. After all retained properties are atomic, emit a completeness check: '§ "
        'completeness check: "<request constraints verbatim>" / <P1 expression> / <P2 expression> / ...\' — the request constraints must be quoted verbatim from the explicit '
        "constraints of the original request; the chosen interpretation determines the meaning of those constraints where the request is ambiguous but does not add, remove, or alter "
        "explicit constraints; interpretive assumptions made to resolve ambiguity and observation-boundary definitions are not themselves request constraints unless they were explicit "
        "constraints of the original request; every retained atomic property expression must appear exactly once, separated by ' / '; a '§ completeness check:' line that omits the "
        "request constraints or omits any retained property expression does not satisfy this requirement. Immediately after the completeness check, attempt to identify one explicit "
        "request constraint that is not logically entailed by the conjunction of the retained atomic property expressions. If such a constraint exists, emit '§ properties complete? no' "
        "— add one new property covering that explicit constraint, perform the required split testing, and repeat the completeness check. Only when every explicit request constraint is "
        "logically entailed by the conjunction of the retained atomic property expressions may the response emit '§ properties complete? yes' — a '§ properties complete? yes' line "
        "without a preceding completeness check does not satisfy this protocol; '§ properties complete? yes' certifies completeness only for the retained set existing immediately "
        "before that check; any subsequently committed removal or merge voids that result and requires a new completeness check. The completeness phase is concerned only with coverage "
        "of explicit request constraints; redundancy introduced during this phase is resolved by the subsequent observational-independence phase. After all retained properties are "
        "atomic, enter the Ground completion fixed-point procedure: first establish completeness for the current retained set; after '§ properties complete? yes', establish "
        "observational independence for that same retained set. A retained property is observationally independent if there exists at least one admissible construction that violates "
        "that property while satisfying every other retained property. Any retained property that is not observationally independent is observationally redundant and must be resolved — "
        "either successfully removed or successfully merged — provided the resolution does not reduce coverage of the explicit request constraints. A redundant property remains "
        "unresolved until either a removal or a merge replacing it is successfully committed; a failed merge or failed removal does not resolve the redundancy and requires another "
        "valid independence action. A proposed merge creates a candidate retained set by replacing its source properties with the merged property: the merged property must first "
        "complete canonicalization, scope validation, and recursive decomposition to an atomic state; once atomic, it replaces its source properties in the candidate retained set. A "
        "proposed removal creates a candidate retained set by removing the selected property; the source retained set remains unchanged until the candidate is validated. For both merge "
        "and removal, the candidate retained set must re-run the completeness-check procedure and observational-independence procedure; the candidate set may not be committed unless a "
        "new '§ completeness check:' is followed by '§ properties complete? yes' and the candidate set satisfies observational independence. A candidate retained set is committed only "
        "if it itself reaches the Ground fixed point — passing both '§ properties complete? yes' and the observational-independence procedure — before it replaces the source retained "
        "set; otherwise the candidate is discarded and the source retained set remains authoritative. If the candidate is committed and the committed change produces any further "
        "redundant property, restart the fixed-point procedure with a new completeness check on the resulting retained set. Otherwise discard the candidate and leave the source "
        "retained set unchanged, then attempt another valid independence action for the unresolved redundant property. Admissible constructions used as falsifiers or independence "
        "witnesses are evidence about the truth conditions of properties and do not thereby make the construction's internal mechanism, architecture, algorithm, or representation the "
        "primary subject of the property. Repeat until a retained set simultaneously satisfies completeness and observational independence; when no retained property is observationally "
        "redundant, the fixed-point procedure terminates. The Ground properties block is complete only when all retained properties simultaneously satisfy: they are derived from the "
        "chosen interpretation; they are atomic; their conjunction logically entails every explicit request constraint; they are observationally independent; they introduce no "
        "out-of-scope constraints, including internal mechanism constraints not explicitly required by the chosen interpretation. When all conditions are met, immediately before "
        "emitting '§ ground complete', emit exactly one 'Retained properties:' line declaring the final retained property set: 'Retained properties: property [N]: \"<verbatim "
        'canonical formal definition>"; property [M]: "<verbatim canonical formal definition>"; ...\' — listing every retained property in retained-set order, each quoted definition '
        "matching its final canonical formal definition verbatim. A 'Retained properties:' line that omits any retained property, quotes a natural-language or non-canonical definition, "
        "or is not immediately followed by '§ ground complete' does not satisfy this protocol. A '§ ground complete' line that is not immediately preceded by 'Retained properties:' "
        "does not satisfy this protocol. This sentinel may not appear before '§ properties complete? yes' in the transcript. '§ ground complete' is a phase-boundary sentinel, not a "
        "response-termination condition. After emitting '§ ground complete', continue with the requested task using the retained atomic properties as the governing constraints for all "
        "subsequent task reasoning, tests, implementation steps, and tool calls. The response must not terminate solely because '§ ground complete' has been emitted. All subsequent "
        "task work must remain consistent with the completed Ground properties; the Ground properties block is not itself the requested task unless the original request explicitly asks "
        "only for property derivation. Any test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does not satisfy this protocol; if task "
        "reasoning begins and '§ ground complete' has not yet appeared, emit the Ground properties block in full and '§ ground complete' before continuing. After '§ ground complete', "
        "continue with the requested task; do not treat the sentinel as terminating the response.",
