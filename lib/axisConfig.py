...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "atomic": "The response enforces one scope commitment per file-modifying tool call. A scope commitment names the first line beginning with the FAIL signal prefix in the most recently "
        "produced tool-executed run result, where a tool-executed run result is a result block from a test runner, build command, or linter — Read, Edit, Write, and search results do "
        "not qualify and do not break recency. A run result is immediately preceding a tool call when no other tool-executed run result appears between that result and the tool call; "
        "non-run tool calls do not break this recency. Derivation phase (required before the first file-modifying tool call): name the root criterion — the line immediately above the "
        "`(i)` escape category line must be the quoted scope commitment — a literal substring of the immediately preceding tool-executed run result; a file-modifying tool call whose "
        "`(i)` line is not immediately preceded by such a quoted string does not satisfy this requirement; the quoted scope text must be a literal substring of the immediately "
        "preceding tool-executed run result; exactly one symbol the call will add or modify must be named above that call, derived from the same run result that produced the scope "
        "commitment (symbol commitment) — a symbol commitment naming more than one symbol does not satisfy this requirement; the named symbol must appear as a literal substring of the "
        "quoted scope text or be named in the failing-item line from the most recent tool-executed run result — a symbol not present in either does not satisfy this requirement; a "
        "symbol is a named member within a container — it names an entity whose existence is established within the container, not the container itself; a commitment naming the "
        "container (the file, module, or resource that would be created or modified as a whole) does not satisfy this requirement; when the symbol name does not appear as a substring "
        "of the immediately preceding tool-executed run result, the symbol is new; a new-symbol call is compliant only if the introduced symbol satisfies the stub requirement for its "
        "kind — for a callable (function, method, or constructor), the body contains no logic: an empty return or a language-idiomatic zero-value return for the declared return type; "
        "for a non-callable (variable, constant, exported value, or class field), the initializer is a single primitive or zero-value expression with no sub-expressions — nil, null, "
        "undefined, a typed zero, or an empty literal; an initializer containing any call expression, struct literal, array literal, object literal, or nested expression in the symbol "
        "source language does not satisfy this requirement; a body or initializer not matching its kind's stub form does not satisfy this requirement; the quoted scope text must be "
        "from a test assertion whose pass/fail state is determined solely by the named symbol — a test that passes or fails independently of that symbol does not satisfy this "
        "requirement; a statement in the derivation phase is a commitment only if its truth value can be determined from tool-executed results already present above it in the "
        "transcript — a statement whose truth value requires evaluating a result not yet present is not a commitment; after the call, a tool-executed run result must appear in the "
        "transcript in which the quoted scope text is absent, every other line from the immediately preceding pre-edit run result is present, and a result line appears for every test "
        ...
        "call — write a block containing exactly these five lines in order: `(i) scope inflation: <string>`, `(ii) stale quote: <string>`, `(iii) symbol cardinality: <string>`, `(iv) "
        "post-call line mismatch: <string>`, `(v) implementation depth: <string>`. A file-modifying tool call not immediately preceded by a block whose last five content lines begin "
        "with `(i)`, `(ii)`, `(iii)`, `(iv)`, `(v)` in that order does not satisfy this requirement. Proceed to file-modifying tool calls only when the derivation and all five "
        "enumeration categories are present as the last five content lines above this point in the transcript.",
        ...
        "block produced by invoking the named artifact as a tool call satisfies this requirement; a result produced by a Read, search, or file-write tool call does not; (1) derive the "
        ...
        "dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose presence in the response constitutes satisfaction — an "
        "[observable:] value that is only an adjective or category name does not satisfy this requirement; if the response contains no tool-result blocks above §2, write '[observable: "
        "prose]' instead; a dimension that names an entity, a fix outcome, or a UI element is not a valid dimension regardless of provenance; derivation is unbounded: add dimensions, "
        "challenge each against the reader-uncertainty test, and revise until every dimension is independently falsifiable; when every dimension carries an '[observable:]' tag, write "
        "'§2 dimensions closed'; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the transcript, and must appear before "
        "the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a literal substring what the prior sub-heading's "
        "passing state admits that this sub-heading rejects; (4) verify coverage: '## Completion check' must not appear before '## Enforcement sequence' in the transcript and must not "
        "appear before '§ implementation permitted' in the transcript; each covered dimension cites a verbatim string that appears either as a substring of a tool-result block produced "
        "after '§ implementation permitted' and visible above '## Completion check', or, when no tool-result blocks are present after '§ implementation permitted', as a substring of "
        "the response text produced in this turn; when every covered dimension cites such a substring, write '§4 coverage verified'; any assistant turn that contains a question "
        "directed at the user or no further planned tool calls, after '§ implementation permitted' has appeared in the transcript, must contain '## Completion check' before that "
        "question or closing text — a turn whose question or closing text appears before '## Completion check' does not satisfy ground; (5) enumerate escape paths: '## Path "
        "enumeration' must not appear before '§4 coverage verified' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal "
        ...
        "string '§ implementation permitted [N]' with N matching the 1-based index of the governed tool call must appear as the final non-blank content in the assistant text block "
        "immediately before each file-modifying tool call — a file-modifying tool call not immediately preceded by assistant text whose final non-blank content is the literal string '§ "
        "implementation permitted [N]' does not satisfy this requirement. Quoted-span exclusion: an occurrence of any named sentinel or heading string inside a markdown code fence (a "
        "span beginning with ``` or ~~~ on its own line) does not satisfy any rung or ordering requirement — only occurrences outside code fences count toward compliance.",
