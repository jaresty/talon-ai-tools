...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "challenge → refine → verify → next gap [ ] Three-part completeness: derive from P2,P3 — baseline demonstrates approach, challenge exposes gaps, refinement addresses them; what "
        "GOES WRONG if you skip challenge: gaps invisible; RULE: always challenge before claiming done [ ] Structure before baseline: derive from P1,P2 — organization must precede "
        ...
        "the solution organized? Derivation: from criteria, derive organization. Evidence: internal consistency. 4. Baseline — Does a minimal version exist? Derivation: from structure, "
        "derive minimal artifact. Evidence: minimal viable example. 5. Challenge — What checks expose gaps? Derivation: from baseline, derive gap-exposing checks. Evidence: "
        "falsification attempt or external review. 6. Refinement — How is the artifact improved? Derivation: from challenge, derive targeted changes. Evidence: addresses specific gaps. "
        "7. Verification — How is completion confirmed? Derivation: from refinement, derive confirmation. Evidence: no new gaps exposed. Emit rung labels to begin each rung; emit "
        ...
        "only be emitted after verification confirms no gaps remain. Domain adaptation: for software tasks, the ladder maps to "
        "prose→criteria→formal→validation→implementation→observation; for writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to "
