...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Node types: Request | Interpretation | Selected | AmbiguityCheck | FormalismChoice | UnambiguousRequest | SplitDecision | SplitCandidate | Property | PlanningComplete. "
        "Execution loop: Pending := {R0} where R0 is a Request node representing the task. Each iteration must write: 'Active: <id>' naming the single node removed from Pending, then "
        "exactly one transition applied to Active, then 'Pending: <remaining ids or empty>'. Each transition consumes exactly one node. Decision nodes (AmbiguityCheck, SplitDecision) "
        "never create task-domain nodes directly — only non-decision transitions may create Request, UnambiguousRequest, SplitCandidate, or Property nodes. Transitions and their output "
        "arities: Interpret(Request R) [→2]: write 'Interpret: R → I1 / I2' naming two more-specific interpretations; add I1, I2 to Pending. Select(Interpretation I1, Interpretation "
        "I2) [→1]: write 'Select: I1 / I2 → S'; add S (Selected) to Pending. GroundCheck(Selected S) [→1]: write 'GroundCheck: S → C'; add C (AmbiguityCheck) to Pending. "
        "GroundResolve(AmbiguityCheck C) [→1]: if a second non-equivalent interpretation exists, write 'GroundResolve: C → R'' and add R' (Request) to Pending; otherwise write "
        "'GroundResolve: C → F' and add F (FormalismChoice) to Pending. Formalize(FormalismChoice F) [→1]: write 'Formalism: <language>' choosing one (allowed: first-order predicate "
        "logic | set-builder notation | lambda calculus | typed function signatures); write 'Formalize: F → U' where U expresses the selected interpretation entirely in that formalism "
        "— natural-language sentences are not valid U nodes; add U (UnambiguousRequest) to Pending. SplitCheck(UnambiguousRequest U or Property P) [→1]: write 'SplitCheck: <id> → D' "
        "where D records its source as the consumed node; add D (SplitDecision) to Pending. Every UnambiguousRequest and every Property node must enter Pending and then be consumed by "
        "SplitCheck — there is no other valid transition for these node types; implicit closure, early termination, or any transition other than SplitCheck consuming a Property or "
        "UnambiguousRequest node is a validity violation. AnalyzeSplit(SplitDecision D) [→0 or 2]: examine the formal expression of D.source; if it contains two independent syntactic "
        "sub-expressions, write 'AnalyzeSplit: D → A / B' where A and B are the exact formal sub-expressions of D.source — A and B must be quoted from the formal notation, not "
        "described in prose; add A, B (SplitCandidate) to Pending; otherwise write 'AnalyzeSplit: D → terminal' — nothing is added to Pending, D.source is consumed. "
        "ResolveSplit(SplitCandidate A or B) [→1]: write 'Divide: <id> → P' where P captures the formal expression of its SplitCandidate source; add P (Property) to Pending. When "
        "Pending becomes empty after all Property nodes have been consumed by SplitCheck and reached terminal, add a single PlanningComplete node to Pending. PlanningComplete [→0]: "
        "write 'Planning complete.' — task content may now begin. Validity: every referenced identifier must already exist; every identifier is introduced exactly once; every iteration "
        "writes exactly one Active:, one transition, one Pending: line; 'Planning complete.' may only be written by the PlanningComplete transition; A and B in AnalyzeSplit must be "
        "verbatim sub-expressions of D.source's formal notation — prose descriptions of what the sub-expressions mean do not satisfy this requirement.",
