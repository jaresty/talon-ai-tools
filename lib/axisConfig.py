...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "step's scope. No two Scope lines may quote the same string from the same run result — each must appear as a separate Scope line in the transcript before the implementation "
        "that closes it; the candidate is limited to the change that resolves the first item only; resolving more than one item in one tool call is not permitted. Before any edit, "
        "produce three artifacts as separate completed blocks — each block must end before the next opens, and no tool call that modifies a file may appear in the same block as any "
        "artifact: (1) scope declaration: quote verbatim the first item from the most recent tool-executed run result above this line — the quoted text must appear in a tool result "
        "above this line; name the failure-line pattern as the literal prefix or suffix the governing artifact uses to mark a failing item, as observed in that result (for example, "
        "'FAILED', 'not ok', 'error['); (2) derivation block as two entries: first, name the candidate change and the minimal stub — the minimal stub is a function with the correct "
        "type signature whose body contains only a return of the zero value of the return type; then apply the minimal stub and execute the governing artifact — if the quoted scope "
        "text is absent from that result, the stub is sufficient and must become the candidate; if the quoted scope text is present, the stub is insufficient and the candidate is "
        "permitted — the only permitted evidence of insufficiency is a tool-executed run result in which the scope text appears verbatim; a Smaller line is valid only when the "
        "file-modifying tool call that applied the stub appears in the transcript above the run result and the most recent tool-executed result above it contains the scope text "
        "verbatim; second, quote the specific text from the scope declaration that would be absent from the run result when the candidate change is present and would appear when it is "
        "absent; (3) line manifest listing every line to be added; for each line, quote the specific string from the scope declaration that would be absent from the run result when the "
        "line is present and would appear when it is absent. Immediately before each tool call that modifies a file, produce: 'Scope: <verbatim quoted item> | Pattern: <failure-line "
        "pattern> | Candidate: <named candidate change> | Smaller: <named stub whose insufficiency is demonstrated above> | Symbols: <every function or symbol name added or modified by "
        "this tool call, comma-separated> | Lines: <count of lines in manifest>' — this line must appear immediately above the tool call; the tool call is not permitted if any symbol "
        "in the diff is absent from the Symbols list; a Symbols list with more than one entry requires a separate Scope line and separate tool call for each entry; this line is not "
        "permitted unless a tool-executed result showing the stub applied appears above it. After the edit, produce a post-edit run result by tool call, then verify: (a) the verbatim "
        "scope text is absent from the new run result; (b) no line matching the declared Pattern appears in the post-edit result that was absent from the immediately preceding pre-edit "
        "run result; (c) every line matching the declared Pattern that appeared in the immediately preceding pre-edit run result, other than the line matching the verbatim scope text, "
        "must still appear in the post-edit result — a line matching the declared Pattern that was present in the pre-edit result and is absent from the post-edit result and does not "
        "match the verbatim scope text requires revert regardless of interpretation.",
        ...
        "structural property of the code, not a property of the test suite. Exception — one pre-edit requirement only is waived for governing artifact creation: a tool-executed FAIL "
        ...
        "complete. All other pre-edit requirements remain in force: the scope declaration, derivation block, line manifest, and behavior-absent state for the insufficient-check run "
        "must appear before the governing artifact edit. A throwaway scaffold — one whose assertion will not persist as a re-executable record after implementation is complete — is "
        "not a governing artifact; no relief applies to it. Produce the assertion in the form it will take permanently from the start. The FAIL gate applies only to non-governing "
        "artifact edits (implementation). A prose statement that names a file-modifying action as the next step is permitted only when a tool-executed FAIL result appears above it in "
        "the transcript whose failure signal names the behavior the action governs — a prose statement announcing an implementation step where no such FAIL result exists above it does "
        "not satisfy this requirement and must be replaced with the FAIL-producing run first.",
