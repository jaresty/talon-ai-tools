...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "exactly its quoted property while satisfying the other; both falsifiers are admissible constructions within the chosen interpretation; and neither accepted sub-property "
        ...
        "rejected, emit: '§ atomic: property [N]: \"<verbatim canonical definition>\" — candidate split rejected: <reason>.' The quoted definition must match the current canonical "
        "definition verbatim. Apply this procedure to every retained property until all have reached Step 4 and emitted '§ atomic:'. After all retained properties are atomic, emit a "
        "completeness check: '§ completeness check: \"<request constraints verbatim>\" / <P1 expression> / <P2 expression> / ...' — the request constraints must be quoted verbatim from "
        "the explicit constraints of the original request; the chosen interpretation determines the meaning of those constraints where the request is ambiguous but does not add, "
        "remove, or alter explicit constraints; interpretive assumptions made to resolve ambiguity and observation-boundary definitions are not themselves request constraints unless "
        "they were explicit constraints of the original request; every retained atomic property expression must appear exactly once, separated by ' / '; a '§ completeness check:' line "
        "that omits the request constraints or omits any retained property expression does not satisfy this requirement. Immediately after the completeness check, attempt to identify "
        "one explicit request constraint that is not logically entailed by the conjunction of the retained atomic property expressions. If such a constraint exists, emit '§ properties "
        "complete? no' — add one new property covering that explicit constraint, perform the required split testing, and repeat the completeness check. Only when every explicit request "
        "constraint is logically entailed by the conjunction of the retained atomic property expressions may the response emit '§ properties complete? yes' — a '§ properties complete? "
        "yes' line without a preceding completeness check does not satisfy this protocol; '§ properties complete? yes' certifies completeness only for the retained set existing "
        "immediately before that check; any subsequently committed removal or merge voids that result and requires a new completeness check. The completeness phase is concerned only "
        "with coverage of explicit request constraints; redundancy introduced during this phase is resolved by the subsequent observational-independence phase. After all retained "
        "properties are atomic, enter the Ground completion fixed-point procedure: first establish completeness for the current retained set; after '§ properties complete? yes', "
        "establish observational independence for that same retained set. A retained property is observationally independent if there exists at least one admissible construction that "
        "violates that property while satisfying every other retained property. Any retained property that is not observationally independent is observationally redundant and must be "
        "resolved — either successfully removed or successfully merged — provided the resolution does not reduce coverage of the explicit request constraints. A redundant property "
        "remains unresolved until either a removal or a merge replacing it is successfully committed; a failed merge or failed removal does not resolve the redundancy and requires "
        "another valid independence action. A proposed merge creates a candidate retained set by replacing its source properties with the merged property: the merged property must "
        "first complete canonicalization, scope validation, and recursive decomposition to an atomic state; once atomic, it replaces its source properties in the candidate retained "
        "set. A proposed removal creates a candidate retained set by removing the selected property; the source retained set remains unchanged until the candidate is validated. For "
        "both merge and removal, the candidate retained set must re-run the completeness-check procedure and observational-independence procedure; the candidate set may not be "
        "committed unless a new '§ completeness check:' is followed by '§ properties complete? yes' and the candidate set satisfies observational independence. If those conditions are "
        "met, commit the change; if the committed change produces any further redundant property, restart the fixed-point procedure with a new completeness check on the resulting "
        "retained set. Otherwise discard the change and leave the source retained set unchanged, then attempt another valid independence action for the unresolved redundant property. "
        "Repeat until a retained set simultaneously satisfies completeness and observational independence and no further removal or merge is required. The Ground properties block is "
        ...
        "by the chosen interpretation. When all conditions are met, emit '§ ground complete' as the final line of the Ground properties block — this sentinel may not appear before '§ "
        "properties complete? yes' in the transcript. Any test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does not satisfy this "
        "protocol; if task reasoning begins and '§ ground complete' has not yet appeared, emit the Ground properties block in full and '§ ground complete' before continuing.",
