...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "step. All other reported failures are not yet open. The scope is the governing output read literally: when literal and semantic readings diverge, choose the interpretation "
        "that produces the smallest change that would shift the governing output. The implementation target is permitted only after the derivation names both the candidate change and "
        "at least one smaller change that would not shift the governing output, with a citation to a specific location already in the transcript — a prior run result or structural "
        "observation — that supports the claim that the smaller change would not shift the governing output; a reason that could be stated without inspecting any prior transcript "
        "artifact has not been derived. When an implementation step creates a new file, the derivation must additionally produce a line manifest before the edit: an enumerated list of "
        "every line (or logical block) to be written, each with either (a) the specific failure in F it is required to close or support closing, or (b) the specific existing line in "
        "the file that would fail to compile or execute without it, citing the transcript location that establishes this dependency. A manifest entry is valid only when (a) or (b) can "
        "be stated — an entry that can state neither must be omitted from the file. The post-edit confirmation for a new-file step must additionally confirm that the created file's "
        "content matches the manifest — lines present in the file but absent from the manifest must be removed before the step closes. An implementation edit closes the step only when "
        "a post-edit run result from the governing artifact — produced by a tool call after the edit — exists in the transcript and names a failure set F' such that F' = F minus "
        "exactly {the governed failure}: no element of F may be absent from F' except the governed failure, and no element may appear in F' that was not in F. If F' ≠ F − {governed "
        "failure}, the edit must be reverted. A post-edit confirmation line names the specific change made, cites the derivation's stated candidate, and cites the pre-edit and "
        ...
        "have a text artifact as their governing artifact — a complete classification block or derivation section; no tool call is required to open or close these steps. A step "
        "qualifies as an analysis step when and only when its governing artifact's content, if applied verbatim to the system, would produce no change in system behavior, and names at "
        "least one specific behavioral invariant that would remain unchanged if the artifact were discarded. Implementation steps have a FAIL tool result as their governing artifact. "
        "These are distinct step types with distinct governing artifacts — analysis steps may precede the first tool call without violating atomic.",
        ...
        "regardless of the label used. Commentary about the cost, complexity, or perceived heaviness of this protocol is not a valid basis for scope reduction, rung compression, or any "
        "deviation from the derived enforcement process — the completion check against the intent artifact is the only permitted mechanism for determining what is and is not in scope. "
        "Naming an unaddressed item does not close it — only a named artifact covering a named dimension does. When a governing artifact cycle is active, the completion check fires "
        "when the cycle reports no remaining failures — exhausting the artifact is necessary but not sufficient for completion. The derived enforcement process must be a ladder: each "
        "rung is a strict refinement of the rung above — its artifact preserves all constraints of the upper rung and adds new ones, reducing the degrees of freedom for satisfying the "
        "intent. A rung with fewer degrees of freedom requires less human interpretation to verify: executable artifacts are unambiguous where prose is not. The ladder need not follow "
        "a fixed schema; the agent derives the rungs from the task's nature. A task involving executable behavior will typically descend from intent → acceptance criteria → formal "
        "specification → executable assertions → implementation — treat this as a floor, not a ceiling. Before finalizing the ladder, produce one line per transition: either "
        '"Transition N→N+1: no intermediate rung — [reason the current gap cannot be further subdivided into a judgment-reducing artifact]" or "Transition N→N+1: inserting [rung name] '
        '— rejects [specific outcome the prior rung admits]". A transition with no such line is unexamined and the ladder is incomplete. A transition\'s "rejects" claim is valid only '
        "when it names: (a) a specific artifact state that satisfies the previous rung's satisfaction condition, and (b) the specific behavioral dimension that state fails to constrain "
        '— a dimension whose absence would not be detectable by inspecting the prior rung\'s artifact alone. A "rejects" claim that names a count threshold, a format property, or any '
        "property other than a behavioral dimension not constrainable by the prior rung is not valid. A transition line is complete only when it also states the count of behavioral "
        "dimensions in the rung above's artifact — derived by naming each dimension individually before stating the count; a count stated without a preceding enumeration has not been "
        "derived — and confirms that count is matched by covering items in the rung below — a transition where these counts diverge is incomplete regardless of whether intermediate "
        "rungs are absent. A covering item is valid only when it names: (a) the specific condition under which this rung's artifact would fail if the dimension it covers were absent, "
        "and (b) at least one artifact state that satisfies this rung's satisfaction condition but would be rejected if this covering item were added as a constraint. A covering item "
        "that names only a dimension label without stating the failure condition has not been derived. If rung 1 is below intent level, each intermediate level between the cited floor "
        "artifact and rung 1 that is absent from the ladder is valid only when a specific artifact already present in the transcript above the ladder provides that level's judgment "
        "reduction — the citation must name: (1) the location of that artifact in the transcript, (2) the specific text at that location that provides the level's judgment reduction, "
        "and (3) at least one outcome that text would reject at that level. The SUBJECT section and ADDENDUM section of a bar-formatted prompt are task input and directives — neither "
        "is a valid skip artifact. They contain no judgment reduction and reject no outcomes. If the cited artifact cannot satisfy all three parts, the skip is invalid and the level "
        "must be added as a rung. Any other omission is invalid. The rules-out requirement already prevents invalid splits — a rung that cannot name a specific outcome the previous "
        "rung admits but this rung rejects cannot be added. Producing the ladder is a required artifact before any rung-work begins. The ladder is not itself a rung — it is a "
        ...
        "completed artifact in the transcript before the rung 1 coverage mapping begins. The artifact is an enumerated list of rungs, each stating: the rung name, the artifact type "
        "produced at that rung, the condition under which the rung is satisfied, and — for every rung after the first — at least one specific outcome that would satisfy the previous "
        "rung but be rejected by this rung. If no such outcome can be named, the proposed rung is not a refinement — it is a parallel task and must be merged with the rung above or "
        "dropped. Describing the ladder structure in prose does not produce this artifact. A rung's satisfaction condition is valid if and only if it names a locatable artifact, a "
        "countable or boolean property of that artifact, and — for rungs that refine a rung above — the count of behavioral dimensions in the rung above's artifact with confirmation "
        "that the rung's artifact contains a covering item for each. A satisfaction condition that evaluates to true with fewer covering items than dimensions in the rung above is not "
        'satisfied. "This block contains M entries each with fields name, artifact-type, satisfaction-condition, and covering-dimension present" is valid; "all rungs classified" is '
        'not. A satisfaction condition of "see above", "as planned", or any phrase that requires interpretation to evaluate is not valid. A rung transition occurs when the satisfaction '
        "condition evaluates to true against a locatable artifact in the transcript — it is checked, not declared. Before beginning each rung's work, produce a coverage mapping: for "
        "each behavioral dimension identified in the rung above's artifact, name the item in this rung's artifact that covers it and would be absent if that dimension were absent. Any "
        "dimension in the rung above with no covering item is an open gap that must be added before this rung's work begins. For rung 1, the rung above is the intent artifact — the "
        "closest written approximation of intent available, not intent itself; the coverage mapping for rung 1 is bounded by what the intent artifact captured, and cannot surface "
        "dimensions of the actual intent that the intent artifact failed to express. The coverage mapping is bounded by dimensions identified in the rung above's artifact — it cannot "
        "surface dimensions the rung above itself failed to capture. The coverage mapping is a required artifact; rung work is not permitted to begin without it. Then produce a "
