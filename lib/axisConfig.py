...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "token; (2) write a concrete automatic regression guard for property [N]; planning or describing a guard does not satisfy this token; for each assertion in the guard emit "
        "'Property [N] assertion [M]: <quoted assertion text>' — the complete assertion inventory must appear before the next sentinel; then immediately emit 'Quoted test: <smallest "
        "contiguous verbatim executable assertion or guard uniquely identifying the written guard>'; no subsequent step may occur until this sentinel has been emitted; (3) attempt to "
        "falsify the quoted guard by constructing a concrete scenario in which property [N] is absent while every listed 'Property [N] assertion [M]:' nevertheless passes — quote each "
        "assertion label being tested (e.g. a guard that reads a static artifact rather than invoking the governed behavior succeeds whenever the artifact contains the expected text, "
        "even if the behavior never executes); emit 'Test blind-spot: found — <construction>' or 'Test blind-spot: not found'; if found, fix the guard and repeat from step (2); (4) "
        'execute the quoted guard in a state where property [N] is violated and emit \'Failure: property [N] "<verbatim property definition>" — <verbatim quoted failure line from a '
        ...
        "rule could ever distinguish satisfaction from violation regardless of what tests are written; if a guard can exist, execute it against a violating state and use the resulting "
        "failure line instead; before writing any implementation, a verbatim failure line from a tool-result block must appear; if the failure line does not specifically witness the "
        "violation of property [N], fix the violating state or implementation and repeat from step (4); (5) write the implementation, then immediately emit 'Quoted implementation: "
        "<smallest contiguous verbatim text uniquely identifying the implementation>'; no subsequent step may occur until this sentinel has been emitted; (6) for each 'Property [N] "
        "assertion [M]:' line in the transcript, ask: does a 'Failure:' line from a tool-result block appear that specifically witnesses that assertion failing? emit 'Implementation "
        "overreach: all assertions witnessed — <Property [N] assertion [M]: text for every assertion in the transcript, listed in full>' — omitting any assertion label from this "
        "sentinel means that assertion is not witnessed; or emit 'Implementation overreach: assertion unwitnessed — Property [N] assertion [M]: <quoted text>'; if any assertion is "
        ...
        "sentinel and proceed. After all properties complete these steps, emit 'Coverage: complete' if all properties are resolved, or 'Coverage: gap — property [N] \"<verbatim "
        "property definition>\"' for any unresolved property. A response that omits any required sentinel, performs a step before emitting its required quoted artifact, writes an "
        "implementation before a quoted failure line appears, or leaves a property without an automatic regression guard does not satisfy this token.",
        ...
        "ground": "The response must produce a 'Ground properties:' block before any test, implementation step, tool call, or task reasoning begins. The block contains one line per property, "
        "each beginning with 'property [N]:' followed by a formal definition in mathematical notation — a typed expression, logical predicate (∀/∃), or relational formula — with a "
        "single unambiguous denotation: two independent readers must reach the same conclusion about what satisfies it. Each property must be atomic. To test atomicity, write candidate "
        "sub-properties first: write 'property [Na]: <definition of part a>' and 'property [Nb]: <definition of part b>' before the split test. Then immediately emit '§ split test: "
        'property [Na]: "<verbatim definition>" — falsified by <construction that violates [Na] but not [Nb]>; property [Nb]: "<verbatim definition>" — falsified by <construction that '
        "violates [Nb] but not [Na]>; these falsifiers are independent' — a split test whose quoted definitions do not appear verbatim in the sub-property lines immediately above it "
        'does not satisfy this requirement. If no valid independent falsifier pair exists, emit \'§ split test: property [N]: "<verbatim definition>" — atomic, no valid split: '
        "<reason>' and retain the original property [N] line; discard the candidate sub-property lines. A split is valid only when both (a) each part can be falsified by a construction "
        "that does not falsify the other, and (b) the parts have identical coverage to the original. A property that admits a valid split is not yet atomic and must be divided before "
        "the block is complete. Continue splitting until every property [N] has a '§ split test:' line showing no valid split. The block is complete when all properties are atomic. "
        "Properties must be observationally independent: for each property [N], there must exist at least one construction that violates property [N] without violating any other "
        "property in the block. A property for which every violation also witnesses another property in the block is observationally redundant and must be removed or merged before the "
        "block is complete. Every property must be derivable from the request's stated constraints — a property that introduces a constraint not present in the request is out of scope "
        "and does not belong in the block. If the request admits more than one interpretation, the chosen reading must be named on a line beginning 'Interpretation:' before the first "
        "'property [N]:' line. A response that begins any test, implementation, tool call, or task reasoning before the 'Ground properties:' block is complete does not satisfy this "
        "token. ",
