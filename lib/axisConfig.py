...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "acting, observe the gap: name what is currently absent or failing. For each property [N] to be governed, emit exactly these steps in order. "
        "'Observing: property [N]' may be emitted only after every property [K] where K < N has already emitted 'Implementation overreach: not found' for property [K] in the transcript above that point; "
        "if any earlier property lacks that sentinel, do not emit 'Observing: property [N]' — instead, resume the earliest incomplete property immediately after its last emitted sentinel and continue until 'Implementation overreach: not found' has been emitted for it, then re-evaluate whether 'Observing: property [N]' may be emitted. "
        "'Coverage:' may not be emitted until every property has emitted 'Implementation overreach: not found'. "
        "(1) the first text emitted for property [N] must be 'Observing: property [N] \"<verbatim property definition>\" — <what the artifact observes>' beginning in column 1 — no other text, sentinel, or "
        ...
        "verification, implementation step, sentinel, or unrelated tool call may appear between the Observing: line and the guard-establishing tool invocation; "
        "no other transcript content may appear until the corresponding tool-result block has been emitted — the tool call and tool result are an atomic pair; "
        "establishing the guard means: if no suitable regression guard already exists, write one via a tool call and wait for the tool-result block confirming it was written; "
        "otherwise, identify the existing guard via a tool call that locates or retrieves it and wait for the tool-result block confirming the identified guard; "
        "in either case, exactly one regression guard shall govern property [N] for the remainder of the cycle; "
        "every subsequent reference to 'the guard' — including every 'Property [N] assertion', 'Quoted test:', 'Test blind-spot:', and 'Failure:' — refers exclusively to this established guard until step (2) is repeated; "
        "every property shall establish exactly one governing regression guard during step (2); proceeding without an established governing guard does not satisfy this token; "
        "build commands, compilation runs, or unrelated verification commands are not regression guards unless they directly execute the established guard itself; "
        "repeating step (2) replaces the previous regression guard; after replacement, every subsequent assertion inventory, 'Quoted test:', 'Failure:', and implementation overreach analysis must derive exclusively from the replacement guard; references to earlier guards no longer satisfy this token; "
        "until the guard-establishing tool-result block has appeared, no 'Property [N] assertion', 'Assertion inventory:', 'Quoted test:', 'Test blind-spot:', 'Failure:', 'Quoted implementation:', 'Implementation overreach:', implementation change, or artifact modification may occur; "
        "for every executable assertion in the written guard emit exactly one 'Property [N] assertion [M]: <quoted assertion text>' line — every "
        ...
        "the assertion inventory; no subsequent step may occur until 'Assertion inventory: complete' has been emitted; "
        "the text immediately following 'Assertion inventory: complete' must be the 'Quoted test:' line — no tool call, Failure: line, Test blind-spot: attempt, or other content may appear for property [N] between 'Assertion inventory: complete' and the 'Quoted test:' line; "
        "emit 'Quoted test: <smallest contiguous verbatim executable "
        "assertion or guard uniquely identifying the written guard>'; a 'Quoted test:' line that does not immediately follow 'Assertion inventory: complete' does not satisfy this token; (3) attempt to falsify the quoted guard by "
        "constructing a concrete scenario in which property [N] is absent while every listed 'Property [N] assertion [M]:' nevertheless succeeds; emit 'Test blind-spot: found — "
        "<construction>' or 'Test blind-spot: not found'; if found, strengthen the guard and repeat from step (2); "
        "(4) the text immediately following 'Test blind-spot: not found' must be the tool call that executes the quoted guard against a deliberately constructed violating state — "
        "no narrative, planning, implementation change, 'Quoted implementation:', 'Implementation overreach:', or other sentinel may appear between 'Test blind-spot: not found' and that execution tool call; "
        "no transcript content may appear between the execution tool call and the corresponding tool-result block — the execution tool call and tool result are an atomic pair; "
        "execute the quoted guard against a deliberately constructed violating state — if the governed artifact already satisfies property [N], first construct a minimal temporary modification "
        "to the artifact that violates property [N], execute the guard against that temporary state, emit the required Failure: line from the tool-result block, then restore the original state before continuing; "
        "emit 'Failure: property [N] \"<verbatim property definition>\" — <verbatim quoted failure line from a tool-result block>'; "
