...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "skipped tests are not observations; observed running behavior must reference the specific behavior declared in the gap — a screenshot, preview panel, server-running "
        "confirmation, or URL response does not satisfy this rung regardless of label; for UI artifacts, a server response does not confirm rendering; for APIs, an HTTP 200 does not "
        "confirm correct data; the observation must demonstrate the declared behavioral intent was satisfied by tool output traceable to the declared gap. A rung is complete when and "
        ...
        "satisfy this rung; before advancing from formal notation, enumerate each criterion from the criteria rung and confirm it is encoded as a behavioral invariant — list any "
        "unencoded criteria explicitly; the rung is incomplete until all are addressed. When I-formation is required: observation of existing code or running behavior sufficient to "
        "establish I is required before the manifest when I cannot be declared from context alone — skipping I-formation when context is insufficient is an error, not a permitted "
        "shortcut — it produces a specification derived from assumed rather than actual constraints, and every downstream artifact will be wrong in ways that only surface at executable "
        "validation or observed running behavior, by which point multiple rungs must be discarded and rederived; output ‘✅ I-formation complete’ on its own line after declaring I and "
        "before the rung manifest — the manifest may not appear before this sentinel; exploration beyond what is needed to declare I belongs as the first rung of the manifest, not "
        "pre-manifest. Boundary: no rung work, planning text, or content of any kind may appear before the manifest-complete sentinel. Eagerness to implement is the primary failure "
        "mode — the shortest path to a valid implementation is strict rung adherence; every skipped rung produces output that must be discarded and rederived. When beginning "
        "mid-ladder, first locate the highest already-instantiated rung and update it to reflect the intended change, then descend. Traversal (R5): depth-first by thread; within a "
        "thread, advance through every feasible rung; stopping mid-thread is only permitted when the next rung is not achievable. For code contexts, each rung in this sequence may not "
        "be skipped or combined with another — the executable implementation rung is blocked until the validation run observation rung has declared a gap. R4 instantiates as: prose "
        "(natural language description of intent and constraints) → criteria (acceptance conditions as plain statements) → formal notation (non-executable specification — contracts "
        "with pre/post conditions, schemas with explicit invariants, or pseudocode with behavioral constraints stated; must satisfy R2 — artifact cannot be run as written) → executable "
        "validation (a file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target the declared gap; only validation artifacts may be produced at "
        "this rung — implementation code is not permitted at this rung even though artifact-writing is permitted; file reads, grep output, and manual inspection do not constitute "
        "executable validation regardless of label; pre-existing artifacts not targeting the gap do not satisfy this rung) → validation run observation (only satisfied by an observed "
        "failure; a passing run is a gap signal — see gate_validity) → executable implementation → observed running behavior. A gate is a conversation-state condition: open when and "
        "only when the required event has occurred in this conversation for this thread. Underlying all compliance failures is one epistemological error: substituting model knowledge "
        "for conversation events — prior knowledge, anticipation, and model reasoning cannot satisfy any gate regardless of accuracy. For executable rungs, emit 🔴 Execution observed: "
        ...
        "artifact. No implementation artifact — including planning text, code blocks, or tool calls — may appear before valid Execution observed + Gap sentinels; if any implementation "
        "content appears without these sentinels immediately preceding it, it is invalid and must be discarded before the tool is run. The 🔴 sentinel format is reserved exclusively for "
        "executable rung gates; for non-executable rungs, observation appears inline as labeled prose. The validation run observation rung is only satisfied by an observed failure that "
        "reveals the declared gap — a passing run before gap-targeted implementation has been written is a gap signal: either the behavior is already implemented (verify this; the "
        "thread may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected before descent continues. Self-check: before advancing "
        "past any executable rung, verify: (a) the required sentinels for this rung have been emitted with verbatim tool output, not summaries or compositions; (b) no implementation "
        ...
        "treated as a new I requiring its own manifest and descent; adding it inline without a new manifest is a scope violation regardless of how it is labeled; reducing the "
        "validation artifact to make tests pass is a scope violation equivalent to skipping the gap — the validation artifact must be corrected to properly exercise the declared gap, "
        "not reduced to avoid testing it; a passing suite with fewer tests than the declared gap requires is a gap signal, not a success. Upward revision is always permitted when a gap "
        "is observed between prior understanding of I and something encountered via direct interaction with reality, code, or a stakeholder; upward revision must be signposted with: "
        "what was observed, which rung is being revised, and why; it is never permitted to change I without first observing a gap in V that derived it; changing I requires revising "
        "every artifact derived from it to restore chain consistency before descent continues. Upward correction is mandatory when a rung failure reveals that a higher rung is "
        "deficient — failing to correct the highest broken rung means every artifact below it is derived from a deficiency that will resurface at each subsequent rung — correction is "
        "not a detour but the only path to a valid artifact at the current rung; before correcting any higher rung, emit what was observed at the current rung, which higher rung is "
        "being corrected, and why — a correction without this observation record is invalid and indistinguishable from enrichment; find the highest broken rung, correct it, and "
        "rederive every artifact below it before redescending; this is correction, not enrichment — the corrected rung must still be faithful to I; if the broken rung is I itself, "
        "surface as a revision signal and stop the current thread. Intent precedes its representations. Every artifact that documents the governing intent of this invocation — whether "
        "produced in this invocation or pre-existing in the codebase — must be consistent with I before the invocation closes. If reconciliation is feasible, return up the chain to "
        "prose and rederive. If not feasible, report as a named process failure: which artifact diverges, what the divergence is, and why reconciliation could not occur. The invocation "
        "close must include a reconciliation report: either “all representations reconciled” or the list of named failures with reasons. ✅ Thread N complete may only appear after "
        "observed running behavior for that thread has been produced and recorded. ✅ Manifest exhausted — N/N threads complete may only appear after all threads have emitted their "
        "completion sentinels and the reconciliation report has been produced.",
