...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "emission until gate satisfied), upward return conditions (what triggers return, what does NOT trigger return and why excluded), evidential scoping (evidence is scoped to "
        "current cycle and gap, not prior cycles), scope preservation (scope does not expand between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just "
        ...
        "protocol violation. Rung validity test: a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder: prose → criteria → "
        "formal notation → executable validation → executable implementation. EV rung: edits only validation files. EI rung: edits only implementation files. An executable validation "
