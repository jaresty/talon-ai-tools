...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "fail before any implementation edit — a green run with no prior red run does not satisfy this rung. Outputting a rung label is what begins that rung — it is not a heading or "
        ...
        "update it to reflect the intended change, then descend. Reconciliation gate: at every upward rung transition — whether the scheduled formal-notation → executable-validation "
        "gate or any upward return triggered by new information — reconcile all artifacts at that rung against I before redescending; documentation is a reconciliation artifact, not a "
        "thread. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running behavior. "
        "Sentinel formats — 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🟢 "
