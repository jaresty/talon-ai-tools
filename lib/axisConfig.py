...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "above that call, where the quoted scope text must be a substring of the tool-executed run result most recently produced before that specific call — a quote whose source result "
        "has an intervening tool-executed run result between it and the call does not satisfy this requirement; (scope commitment); a statement in the derivation phase is a commitment "
        "only if its truth value can be determined from tool-executed results already present above it in the transcript — a statement whose truth value requires evaluating a result "
        ...
        "quoted scope string above this call and no other failing-item line from the same pre-edit run result also appeared as a quoted scope string above this call. Then enumerate "
        "every path by which a transcript could satisfy these commitments without the call having changed exactly one independently observable behavior: for each path, name the literal "
        "string in the transcript that closes it — a path is closed only when naming that string makes the non-compliant transcript distinguishable from the compliant one without "
        "assessing intent; a path that cannot be closed by naming a string must be eliminated by bringing the system to a state where the commitments can be satisfied structurally. The "
        "enumeration is complete when no remaining open path exists. Proceed to file-modifying tool calls only when the derivation and enumeration are present as text above this point "
        "in the transcript.",
