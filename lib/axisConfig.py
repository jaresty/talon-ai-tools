...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "through every feasible rung; completeness governs rung depth, not rung existence; each rung may not be skipped or combined with another. The executable implementation rung is "
        "blocked until the validation run observation rung has declared a gap. Only validation artifacts may be produced at the executable validation rung — implementation code is not "
        "permitted at this rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must be invocable by an automated tool. A rung "
        "label must be emitted at every rung transition; a rung label during execution marks the point where the artifact begins, not a section heading for planning or description. "
