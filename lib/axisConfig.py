...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response opens with a rung manifest to make rung membership legible before descent begins — a short document that first decomposes I into threads (discrete units "
        "of intent each of which can be independently descended and validated), then for each thread lists the planned artifacts, their rung labels, and their derivation sources. Gate: output the exact phrase 'Rung manifest complete' on its own line before producing any other artifact. Boundary: no artifact from any rung may appear "
        ...
        "or asserted expectations do not satisfy this and do not permit descent; observation requires the artifact to execute and produce output that reveals the gap — an output of "
        "'all tests skipped' or 'no tests run' reveals nothing about the gap and does not permit descent; resolve the reason tests do not execute before descending; output the exact "
        "phrase 'Execution observed: [actual tool output]' on its own line (replacing [actual tool output] with the real output), then output the exact phrase 'Gap: [what the "
        "observation reveals about what the next rung must produce]' on its own line (replacing the bracketed clause with a specific description of what remains unresolved and must "
        "be closed by the next rung) — 'skipped' is not a test outcome; if tests do not run, stop and fix the executable validation rung before descending; these two sentinels are "
        "required before any artifact for the next rung appears, regardless of whether tests pass or fail (either outcome is a valid observation); a rung is not achievable only when the domain lacks a standard artifact type, not because execution "
        "is inconvenient or anticipated; there are no documentation-based exemptions; human confirmation, user approval, or asking whether to proceed does not satisfy the execution "
        "gate — observation requires tool-based execution, not conversational acknowledgment. Ground applies regardless of perceived task complexity — simplicity does not permit skipping rungs or the execution gate. The response treats the declared intent (I) as fixed. At each step, the response produces a "
        ...
        "types, contracts, schemas, property checks) → validation run observation → executable implementation → observed running behavior. (The validation run observation rung records "
        "the actual output of executing the preceding executable validation rung and declares the gap it reveals — tests may fail at this point; that is expected and correct before "
        "implementation exists; the gap declaration for this rung states which assertions fail, meaning the implementation rung must make those assertions pass; if tests skip rather "
        "than fail or pass, the executable validation rung is incomplete — fix it before descending to this rung; descending to this rung satisfies that rung's execution gate; it is "
        "not a separate gate requirement.) The ladder is traversed depth-first by thread: for each thread, descend through all rungs until observed running behavior is satisfied — "
        "anticipated closure is not observed closure; observation is the terminal rung of every thread, not a special cycle between two adjacent rungs; the active completeness token "
        "governs how many threads are batched per descent; when a thread is complete, return to the top and begin the next thread. When any rung reveals that the rung above needs "
        "revision — a constraint conflict, a pivot forced by implementation, or new information discovered during descent — correct the rung above to accurately reflect what I requires "
        "at that rung, then re-derive the current rung from the corrected one; this is not enrichment (adding constraints from below); it is correction (restoring faithfulness to I); "
        "if the rung requiring correction is I itself, do not correct it unilaterally — surface the conflict as a revision signal and stop the current thread; the ladder must remain "
        "consistent at every boundary. A rung is an "
        "artifact that is both upward-faithful (derived from the prior rung without adding unstated constraints) and downward-sufficient (self-contained enough to evaluate the next "
        "artifact without re-consulting I). Domain phases fail this test — to produce the next artifact from a domain phase, you must first decide what constraints it imposes, which "
        "requires consulting I; a domain phase like 'API layer' is not downward-sufficient because writing tests for it requires knowing what the API must do, and that knowledge comes "
        "from I, not from the phase name. "
        "The ladder's tightening is heuristic — each rung localizes where interpretive judgment enters rather than eliminating it. Lower "
        "rungs do not guarantee monotonic reduction of ambiguity — a test suite encoding only typical cases may be more executable than prose while less faithful to intent. Descent "
        "presumes tightening; it does not prove it. The "
        ...
        "In code contexts, I precedes the prose rung; if the only artifact available is a written spec, treat it as V₁ (the prose rung) and extract I from its governing "
        ...
        "should be), then descend. Within a thread, advance through every feasible rung; stopping mid-thread is only permitted when the next rung is not achievable — a rung is not "
        "achievable when the domain provides no standard artifact type for it. Present V (the lowest constraint artifact reached), then the exact phrase 'Validation artifact V complete' on its own line (this "
