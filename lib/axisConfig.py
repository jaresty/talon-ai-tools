...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "file alone what the implementation must do — the failure output is required. P6 (Artifact type discipline): One rung per type; text rungs produce no files; frozen artifacts "
        "may not be modified at subsequent rungs. An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another artifact — it contains no "
        "behavior of its own; validation files may not be imported by implementation files. An executable implementation artifact is a file that produces behavior directly — it "
        ...
        "P11 (Immediate lowest-rung observation): Observe at lowest feasible rung; deferring is a protocol violation. P12 (Completeness slice): One independently testable behavior per "
        ...
        "revised rung must be re-derived from its input rung’s artifact and descent resumes downward. Returning to the session observation loop is valid when a tool-executed "
        ...
        "derivation rung: before descending, derive the rung table for this session by applying P1–P19 to the declared intent; produce a table with columns: rung name | artifact type | "
        "gate condition (what tool-executed event satisfies the gate) | void condition (what invalidates the artifact) | faithfulness test (what a reviewer needs from only this "
        "artifact to evaluate the next rung) | permitted tool calls (what file modifications are allowed at this rung; all rungs permit read, observe, and non-modifying run commands; "
        "file modifications are constrained to the artifact type of that rung); verify the table satisfies P8 minimality — remove any rung that does not; descent begins immediately "
        "after the table is produced without pausing for confirmation; ✅ Manifest declared is the closing sentinel of the ladder derivation rung — the required emission order is: rung "
        "table, then ✅ Manifest declared; a manifest emitted without a preceding rung table is a protocol violation; the rung table is the gate on all subsequent rung activity. Session "
        "observation loop: before beginning a ladder descent, observe current running behavior via tool call — if the observation reveals a gap between running behavior and declared "
        "intent, declare the gap and descend the ladder; if no gap remains, emit ✅ Ground complete — the session ends; this observation is not a rung in the derived ladder and must not "
        "appear in the rung table; the ladder handles only the declared gap; after manifest exhaustion, the observation loop recurs to check whether intent is fully achieved. For "
        "software behavioral change, the standard ladder for each gap-closing cycle produces: prose → criteria → formal notation → executable validation → executable implementation. A "
        ...
        "implementation; the verbatim exec_observed output must contain at least one assertion-level failure string (e.g. 'Expected … Received …', 'AssertionError', 'FAILED', specific "
        "test name + failure reason) — an exec_observed whose output shows only infrastructure failure, import error, process crash, or a model-described summary of failure does not "
        "satisfy this gate; if the suite cannot reach assertions, the implementation artifact for that cycle resolves only the infrastructure gap, and this gate cannot be emitted until "
        "a subsequent cycle produces assertion-level failure output]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one criterion in this rung's artifact; criterion "
        "contains no conjunction; formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation artifact V complete [gate: pre-existence check tool "
        "call result present; tool call in this response wrote the file to disk]; ✅ Thread N complete [gate: meta exec_observed after executable implementation shows the gap declared "
        "for this thread is no longer present in running behavior]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest "
        "declared]; Carry-forward: [list which original failures cover which current tests] [gate: prior failure name quotable verbatim from a prior exec_observed sentinel]; ✅ "
        ...
        "in notation; audit section named and separate]; ✅ Ground complete — intent achieved: [what the observation shows] [gate: 🔵 Closing observation must appear in the current "
        "response before this sentinel; meta exec_observed shows no gap between observed running behavior and declared intent; all manifest threads complete]; 🔵 Closing observation — "
        "[what the tool-executed observation shows] [gate: the tool call immediately before this sentinel must directly invoke the behavior named in the session intent and its output "
        "must show that behavior is present; this sentinel must appear after ✅ Manifest exhausted and before ✅ Ground complete in the same response; emitting ✅ Ground complete without "
        "a preceding 🔵 Closing observation in the current response is a protocol violation]; 🔵 Write authorized — rung: [rung name] | artifact type: [type] | file: [path] [gate: "
