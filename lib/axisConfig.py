...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "constraint it encodes; a pre-existing test that happens to pass does not satisfy this rung; when the declared intent is to modify a test artifact, the test-under-modification "
        ...
        "exhibit the criterion; before writing the test, check whether an existing test file covers the same module and add there if so; when the validation artifact is modified, "
        ...
        "before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — V-complete may not be emitted without "
        "this tool-executed result in the transcript; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run "
        "it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if "
        "it still passes after perturbation, the test is vacuous and must be rewritten; implementation edits may not begin until a red run exists in the transcript for the current "
        "cycle; ✅ Validation artifact V complete may not be emitted unless a 🔴 Execution observed: showing the test failing exists in the transcript for the current cycle; emitting "
        "V-complete before seeing a red run is a protocol violation regardless of whether perturbation was attempted. ✅ Validation artifact V complete must be emitted at the executable "
        "validation rung before the validation run observation label is written. Before writing the validation run observation rung label, invoke the validation artifact via a tool "
        "call; a harness error (import failure, syntax error, missing file) is not a red run — the test must have been loadable, its assertions must have run, and they must have "
        "failed; fix the harness error before treating any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution "
        "observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the "
        "validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after "
        ...
        "observed: and 🔴 Gap: have been emitted at the validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; if the observed "
        "gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Upon writing the observed running behavior label, re-emit "
        "the criterion for this cycle, then immediately invoke the implemented artifact via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output "
        "must directly demonstrate the specific behavior named in the criterion; it must be what a non-technical observer of the running system would see — not the test suite, not "
        "process state unless the criterion is specifically about process state; a build output, bundle manifest, or compilation result never satisfies this gate regardless of what it "
        "names; for a UI component, this means rendered browser-visible text and values; if the implementation has no directly invocable artifact, invoke any non-test consumer of the "
        ...
        "be located in the transcript, the tool call is prohibited; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be "
        ...
        "distinct predicate requires a separate thread; re-read the prose rung output via a tool call to confirm every predicate is covered; ✅ Manifest declared may be emitted exactly "
        "once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact "
        "for it may exist. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without consulting I; before closing a rung "
        "artifact, state every open constraint the next rung must resolve; faithful derivation requires form to change, intent to stay fixed. Advance through every feasible rung "
        "without pausing for user confirmation between rungs — each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in "
