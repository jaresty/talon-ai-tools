...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "planned artifacts, rung labels, and derivation sources — rung labels and artifact types only, no rung content appears in the manifest. The rung sequence must follow the ladder order — executable validation and validation run observation must both precede executable implementation; a manifest that places implementation before validation or before validation run observation is malformed. Each thread must include an executable "
        ...
        "Rung manifest complete' on its own line before any artifact — this sentinel marks the end of the manifest document and may only appear after a manifest containing at least one thread with labeled rungs; an empty manifest, a stated intention to write one, or planning notes do not satisfy this requirement. Boundary: the manifest is the first and only output before this phrase — no rung work, planning text, or content of any kind may appear before it; "
        "observation of existing code or running behavior sufficient to establish I is permitted before the manifest when I cannot be declared from context alone — this is I-formation, not rung work; "
        "exploration beyond what is needed to declare I belongs as the first rung of the manifest, not pre-manifest; after this phrase, execution must begin at Rung 1 of Thread 1 — no rung may be skipped on "
        "grounds of perceived simplicity, prior knowledge, other active constraint tokens, or "
        "content described in or implied by the manifest. I is the declared intent (the fixed governing authority); V is a constraint artifact derived from I through the ladder; O is "
        "the output evaluated against V. Three rules govern every thread. R1 (I is fixed): every artifact derives faithfully from I and the prior rung — the form changes, the intent "
        "does not. R2 (rung criterion): an artifact is a rung iff upward-faithful (derived from the prior rung without adding unstated constraints) and downward-sufficient "
        "(self-contained to evaluate the next artifact without consulting I); domain phases are not downward-sufficient — writing tests for 'API layer' requires knowing what the API "
        "must do, which comes from I, not the phase name. R3 (observation terminates): every thread descends to observed running behavior; only tool-executed output with a declared gap satisfies the execution gate; skipped tests are not observations. Single-step derivation: each rung "
        "addresses only the gap declared by the immediately preceding rung; using gaps or intent from any rung higher than the prior rung to drive the current rung is an error. Gap-locality: a gap is local to the rung that produced it — "
        "the gap that gates descent to rung N is the output of executing rung N-1; no other rung’s observation, including I, may serve as the gating gap. Foundational constraint: a symbol is not "
        "the state it represents — writing a sentinel does not instantiate the observation it marks; a manifest entry does not complete the rung it describes; understanding how to "
        "produce an artifact does not produce it; a rung label during execution marks the point where the artifact begins — it is not a section heading for planning, description, or exploration about the rung; "
        "no content other than the artifact itself may appear between a rung label and the artifact it precedes. Eagerness to implement is the primary failure mode — an implementation produced without passing validation is invalid and will be discarded; the shortest path to a valid implementation is strict rung adherence, not shortcuts; every skipped rung produces output that must be thrown away, leaving the agent further from completion than if it had followed the ladder. Execution gate (R3): "
        "one principle governs all gates — every rung advancement requires an externally received result; nothing self-produced by the model (planning text, anticipated output, "
        "composed code blocks, or inline expansions) counts as permission to advance. Before descending to an executable rung, output '🔴 Execution observed: [verbatim tool output — content composed "
        "without running the tool is invalid]' then '🔴 Gap: [what the verbatim output reveals]' on their own lines; non-executable rung observations appear inline as prose — the 🔴 sentinel format is reserved exclusively for executable rung gates and must not be used as a general "
        "observation marker, gap-discovery record, or preamble; place sentinels immediately before the executable rung artifact they gate, not at the manifest level or in any "
        "earlier position — these sentinels apply only to executable rungs and gate the immediately "
        "following executable rung only; they do not permit skipping intervening rungs; a rung manifest, verbal description, or code block containing a command and its expected output "
        "is never a valid Execution observed — only output received as a distinct tool call result in the conversation satisfies R3; for executable rungs, observation means invoking "
        "the artifact as a running process and recording the terminal output — required regardless of whether "
        "tests pass or fail; if tests do not run, fix the rung before descending; no implementation artifact — including planning text, code blocks, or tool calls — may appear before "
        ...
        "the tool is run. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; "
        "stopping mid-thread is only permitted when the next rung is not achievable (the domain provides no standard artifact type for it); the active completeness token governs how "
        "many threads are batched per descent; when a thread reaches observed running behavior, return to the top for the next thread. Ground applies regardless of perceived task "
        "complexity — the rung manifest and all execution gates are mandatory even for tasks that appear straightforward; other constraint tokens operate within each rung of the "
        "process, not on the process structure itself — they govern depth, scope, and style at each step but cannot eliminate steps or the manifest; completeness tokens govern how deeply each rung is executed — a minimal completeness token produces a shallower rung, not a skipped one. When a rung reveals the rung above "
        ...
        "highest already-instantiated rung and update it to reflect the intended change, then descend. For code contexts, each rung in this sequence may not be skipped or combined with another — "
        "the executable implementation rung is blocked until the validation run observation rung has declared a gap; announcing two rungs together or labeling one rung with another's name does not satisfy either. "
        "R4 instantiates as: prose (natural language description of intent and constraints) → criteria (acceptance conditions as plain statements) → formal notation (non-executable specification — type signatures, schemas, pseudocode, or contracts; may use code syntax to describe interfaces but the artifact cannot be run as written) → "
        "executable validation (a file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target the declared gap and reveal whether it has been closed; only validation artifacts may be produced at this rung — implementation code is not permitted at this rung even though artifact-writing is permitted; file reads, grep output, and manual inspection do not constitute executable validation regardless of label; pre-existing artifacts not targeting the gap do not satisfy this rung) → "
        "[GATE: validation run observation — executable implementation is blocked until this rung has been executed and declared a gap] → executable implementation → observed running "
        ...
        "may be complete) or the tests are not testing the intended behavior and must be corrected; the executable implementation rung is blocked until the gap has been observed; if tests skip, fix the executable validation rung first.) When the lowest V is complete, output '✅ Validation artifact V complete' on its own line before "
