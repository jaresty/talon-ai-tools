...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    "channel": {
        ...
        "formal": "The response separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be true — "
        "the executable or testable part. Natural language labels, introduces, and explains the specification — prose adds context but does not substitute for formal encoding where "
        "precision matters. The notation describes what must be true rather than how to implement it — permitted forms include type signatures without bodies, interface shapes, "
        "protocol definitions, and logical assertions; prohibited forms include complete function implementations, variable assignments, or working code sequences. When "
        "domain-specific notation exists (e.g., Z notation, TLA+, Alloy), the response may use it if appropriate; otherwise uses pseudocode-style type annotations and constraint "
    ...
    "form": {
        ...
        "quiz": "The response creates genuine retrieval demand for every concept by requiring the reader to commit to an explicit expectation before the answer is available — the goal is a cycle "
        "where each prediction creates a gap to resolve, each reveal closes it, and where a Hook: is present, it opens the next gap rather than announcing the next concept. Epistemic "
        "constraints on valid questions: (1) Source material grounding — a question whose expected answer is not supported by the source material does not constitute retrieval demand; it "
        "constitutes invention; questions must be answerable from the source material and reveals must cite where. (2) Derived-framework guardrail — a facilitator-created model is not "
        "domain knowledge; scoring recall of it as knowledge is an epistemic error; test whether the learner can evaluate or use the model, not whether they can reproduce it. (3) "
        "Information-gain gate — a concept's quiz value is proportional to its consequence, uncertainty, and source quality, not its position in an enumeration; defer low-value taxonomy "
        "and details recoverable through tools. (4) LLM-augmented relevance (conditional) — when LLM assistance will be available in the target context, knowing means recognizing and "
        "evaluating rather than unaided production; in that context, test source selection, hallucination detection, prioritization, and recommendation review rather than unaided recall of "
        'templates or checklists. (5) Learner-safety — a human message turn containing "pass", "skip", or "I don\'t know" in response to a question is treated as a diagnostic response '
        "equivalent to an incorrect prediction; the reveal is produced in the next response turn without gap-reveal comparison; no additional questions on that concept are required; a pass "
        'or unknown response satisfies the prediction requirement for that concept. (6) Bounded-round — the response begins with a literal "Quiz purpose:" line and a literal "Stopping '
        'condition:" line before the first question; the stopping condition names the transcript-inspectable state whose presence closes the round, not a question count; when every concept '
        "in the opening list has either a ✓ marker, a gap-reveal sentence, or a pass/unknown response in the transcript, the round is complete; the terminal declaration may be omitted when "
        'the stopping condition has been satisfied and a "Round complete:" line appears naming that condition. Before generating any question, name the concepts to be covered as a numbered '
        "list, then enumerate every named structural path by which a question could appear without creating genuine retrieval demand — path (a): question phrasing reveals the answer; path "
        "(b): question is immediately followed by its answer with no structural gap; path (c): question recaps rather than elicits — path (b) is closed globally by the Predict: "
        "requirement; paths (a) and (c) are closed by the upfront enumeration and the root criterion of genuine retrieval demand. Before writing any question, name the concept that will "
        "carry the Misconception:/Why: pair in this quiz — this commitment must appear in the transcript before the first question is written; if no concept naturally carries a common "
        "misconception, choose the concept whose incorrect prediction would be most damaging. For each concept after the first, begin with a Following from: line containing (1) a verbatim "
        "quoted phrase from the preceding answer and (2) a one-sentence restatement of what that reveal established, making the line self-contained without requiring the reader to scroll "
        "back. Then write the question, require a Predict: line (the reader names their expectation before the answer is shown). An answer is permitted in the same response turn as the "
        "question only when the current response is producing a static document or terminal sequence artifact — meaning no human turn is pending and no Predict: line is expected from the "
        "reader before this response ends; in that case the answer follows the question immediately and the Predict: requirement is suspended. When the current response is not in static "
        "document or terminal artifact mode: write only the question followed by the literal line 'Predict: [your expectation before reading the answer]'. Do not write the answer in this "
        "turn. A Predict: line satisfies the gate only when it appears in a human message turn — a Predict: line appearing in the model's own response text in the same turn as the "
        "question, or appearing inside a tool result block, does not satisfy this gate. The answer appears in the response turn after a human message containing a Predict: line. After each "
        "answer: a gap-reveal sentence comparing what the Predict: line stated against what is correct or complete; then for every concept except the first and the last, a Hook: line "
        "quoting a specific phrase from the current answer and naming the implication it leaves unresolved — the question the next concept answers; a Hook: line is compliant only when it "
        "appears after the gap-reveal sentence for this concept and before the Following from: line of the next concept (evaluator check: the quoted phrase appears verbatim in the current "
        "answer); a Hook: line must not appear after the last concept's answer; then a Misconception: line and Why: line — compliant only when they appear in the answer block of that named "
        "concept (the concept named in the pre-quiz commitment); a Misconception: line names the specific incorrect belief that would lead a reader to predict the wrong answer; a Why: line "
        "states the structural reason that belief is wrong; then an association sentence — before writing it, name the numbers already used in prior association sentences — using the "
        "format '#N (Name): [relationship]' where N is the opening-list number of a prior concept whose number has not appeared in any earlier association sentence in this quiz and Name is "
        "that concept's name from the opening list (for the first concept, use the format '#N (Name): [relationship]' naming the opening-list number and name of the first concept whose "
        "understanding it enables). emit '✓ [concept name]' only when the Predict: line states the complete and correct answer with no correction, addition, or nuance needed — if any gap "
        "exists between the prediction and the answer, write a gap-reveal sentence instead of '✓'. Continue until every concept in the opening list has either a '✓ [concept name]' marker "
        ...
        "that it targets. This response contains no declarative sentence asserting the agent's position on the subject — withholding judgment in favor of questions directed at the "
        "user's claim. The response ends with an explicit invitation for the user to respond, not with any conclusion, summary, verdict, or recommendation.",
    ...
    "method": {
        ...
        "falsify": "The response obtains an attributable observation of each gap between intent and current state, and leaves a mechanism that will detect regression without human initiation. "
        "falsify shrinks three things into agreement: the artifact shrinks to exactly what the guards require, the guards shrink or expand to exactly the properties, and each "
        "assertion is witnessed failing when its property is absent. The relative order of observation, guard establishment, and artifact implementation does not affect whether the "
        "assertion is witnessed. Guard and artifact may be established in any order, but establishment is an obligation, not merely a permission: if a required guard or artifact is "
        "absent but constructible from the already-established intent, properties, and constraints, establish it before declaring any observation unavailable; absence of a supplied "
        "guard or artifact is not itself evidence of unavailability, and a missing-but-constructible prerequisite is a construction state, not a terminal excuse. This establishment "
        "does not determine or revise which properties are required or whether the guard is adequate to them — property semantics remain the work of ground and adequacy the work of "
        "the adequacy check; falsify only constructs the apparatus and concrete state needed to attempt falsification of the already-established properties. A guard is an "
        "observation-producing procedure: a repeatable procedure that, applied to the artifact, yields an observation distinguishing the property's satisfying and violating states — a "
        "software test, a map or route check, a source inspection, a measurement, a trial, or a comparison are all instances. An observation witnesses an assertion only when the "
        "procedure evaluates that assertion and produces an assertion-specific result: Retrieving evidence for an assertion is not the same operation as witnessing the assertion. "
        "Information merely retrieved, inferred, or cited by the procedure is not itself the assertion's result — establishing that a procedure or its evidence is appropriate to the "
        "property is the work of ground and verify; falsify, once an assertion and its procedure are in play, obtains the procedure's own discriminating result for that assertion. "
        "What makes an observation a witness is that the procedure could have produced the violating result: it is capable of yielding either the satisfying or the violating "
        "observation, and is not constructed from the conclusion it is meant to evaluate. This is not asserted — it is shown, by perturbing the relevant state and observing the result "
        "flip (the satisfying observation on one state, the violating observation on a perturbed state); a procedure that yields the same observation regardless of the perturbed state "
        "does not distinguish the assertion and is not a witness. A guard is established by an observation-producing act whose result record contains the guard's procedure verbatim; "
        "every assertion referenced later must be copied verbatim from such a result record. What witnesses an assertion is that the procedure is assertion-bound and counterfactually "
        "independent; regression detection additionally requires the procedure to re-apply on its own. Regression-persistence is a separable capability that may be present or absent: "
        "a one-time application that is assertion-bound and counterfactually independent still witnesses its assertion now, and is a valid Failure or pass observation, even when it "
        "cannot re-fire later (a measurement taken once, an expert inspection) — only the claim that the guard will detect a future regression depends on persistence. When a Failure "
        "observation is intended as a standing regression guard, it must be produced by a committed procedure persisted to a location that re-applies without human initiation, not an "
        "inline or ephemeral one-off. Repeatable is not the same as deterministic: a guard whose observation depends on external or changing state (a live lookup, a measurement, a "
        "poll) is still a valid witness at the moment it is applied, but its observation and provenance must be retained rather than treated as reproducible on demand. Capability is "
        "not evidence: what witnesses a behavioral observation is an available qualifying observation record, not the mere fact that an act did or did not occur this turn. A "
        "qualifying observation record is a transcript-present record produced by the committed guard artifact that carries the guard's procedure identity, the assertion's identity, "
        "and the relevant A-result. A fresh application of the guard produces one; a prior result record already present in the transcript is one; model prose is never one. A "
        "qualifying observation record may originate in the current execution or an addressable parent, child, or sibling execution; its validity depends on retained procedure, "
        "assertion, result, and execution provenance, not its position relative to implementation. Observation availability is capability-relative, not tool-relative: whether the "
        "observation capability is available depends on whether the committed guard can be applied with the acting party's current capabilities, not on whether an external tool is "
        "involved — an inspection or derivation over the available artifact is itself an applicable observation procedure, and applying it is producing the observation. It counts as "
        "an observation only when that inspection or derivation is the assertion's qualifying procedure and produces an assertion-specific result, not because content was read or "
        "noticed: reading the artifact and finding text that resembles the property is not an observation until the guard evaluates the assertion against it. A qualifying observation "
        "record already present may be cited whether or not the observation capability is currently available. When no qualifying record exists and the observation capability is "
        "available, the response must apply the committed guard to obtain new observations; it must never synthesize, infer, or narrate an observation. A guard that is applicable but "
        "not yet applied is not unavailable — it is unobserved, and unobserved is a do-the-work state that requires application, not the honest terminal 'observation unavailable' "
        "state; 'observation unavailable' is reserved for the case where no qualifying guard can be applied with current capabilities, never for a guard that is merely unapplied. "
        "Before declaring an observation unavailable, inspect the available observation records for a qualifying record and confirm no applicable guard remains unapplied; only when no "
        "qualifying record is found and no applicable guard remains may the unavailable state be emitted — the unavailable state is the residue of that inspection, not a first-move "
        "declaration. A behavioral assertion for which no qualifying observation record exists is neither Failure nor structural Unobservable — emit 'Unwitnessed: assertion "
        "\"<verbatim assertion>\" — observation unavailable'; it does not contribute to 'Coverage: complete', and it is a witnessing gap, not a structural exemption. Record the "
        "observed gap: emit 'Observing gap: <what is currently absent or failing>'. Shrink the artifact to the guard (Gate 1). Always attempt the simpler artifact: construct a smaller "
        "modification to the artifact and apply the guards to it. 'Overreach: not found' is a claim that no such reduction exists, so it is valid only after an attempted reduction — a "
        "qualifying observation record showing a smaller artifact that the guards reject (or a smaller artifact they accept, which you then keep). If a reduction the guards still "
        "accept exists, or a path through the artifact is not exercised or distinguished by any guard, emit 'Overreach: found — <modification or path>', trim to the boundary the "
        "guards require, and repeat. A bare 'Overreach: not found' with no attempted reduction does not satisfy this token. Both 'Overreach: found' and 'Overreach: not found' are "
        "observation-dependent; when no qualifying observation is available, emit 'Overreach: untested — observation unavailable' rather than either verdict — untested is never 'not "
        "found'. Witness every assertion (Gate 2). Enumerate the assertions: emit one 'Assertion: <verbatim assertion text>' line for each assertion of the established guard, quoted "
        "verbatim from the guard's result record. The verbatim lines are the observable; no count or completeness sentinel is used, and a self-declared count does not satisfy it. "
        "Every enumerated 'Assertion:' line must then receive its own 'Failure: assertion \"<verbatim assertion>\" — \"<verbatim failure text>\"' or 'Unobservable: assertion "
        "\"<verbatim assertion>\" — structural' before 'Coverage: complete'; the binding is by verbatim identity — the assertion text in the Failure or Unobservable line must match "
        "an enumerated Assertion line exactly, and an enumerated assertion with no matching outcome line leaves coverage incomplete. After each Failure or Unobservable outcome, if any "
        "enumerated 'Assertion:' line still lacks a verbatim-matching outcome, return to the witness step for the next unwitnessed assertion and repeat; do not leave Gate 2 until "
        "every enumerated assertion has a matching outcome. Each enumerated assertion is either behavioral or structural. A behavioral assertion is witnessed only by a pair of "
        "A-observations: an application in which A's own guard-defined result is a failure, and one in which A's own result is a pass. An A-observation is an application of the "
        "committed guard in which assertion A is evaluated and produces its own guard-defined result. The guard defines the observable result for A: an A-pass is the guard's reported "
        "pass result for A and an A-fail is the guard's reported failure result for A. These results are observations of the guard, not independent claims about the truth of any "
        "property — whether A's result corresponds to the property A is intended to govern is not established by this token; that correspondence is established separately by the "
        "adequacy check. What this token establishes is only that A distinguishes its own two results: an application in which A is evaluated and reports failure, and one in which A "
        "is evaluated and reports pass. The failure (A-fail) and passing (A-pass) applications may appear anywhere in the addressable execution lineage; the 'Failure:' line must cite "
        "both qualifying result records by execution identity and quote their assertion-specific results verbatim. Any application that does not produce an A result is not an "
        "observation of A and cannot witness A, regardless of the cause or form of the application's outcome — this covers procedure errors, unrelated assertions failing, a guard that "
        "returns without reporting A, and any other application in which A itself was not evaluated; only an A-fail result can supply the violating observation, and only an A-pass "
        "result the satisfying contrast. In particular — as an illustration of this rule, not an exhaustive list — when the subject the assertion governs does not yet exist, an error "
        "that halts before A is reached (a software build or compilation error when the governed symbol is undefined; a route check that cannot run because no lodging has been chosen "
        "yet; a source lookup that fails because no source is cited) is tempting to read as 'the property is absent, so this is the failure'; it is not an A-fail, because A never ran "
        "— the application disqualified itself as an observation of A and fails identically no matter which property is absent. The A-fail you need is A's own assertion applied and "
        "reporting failure against a present-but-wrong artifact, which the subject's absence cannot produce. Both A-observations must be produced by the committed guard artifact, and "
        "the guard's output must identify A (by a name, path, or message that distinguishes A) so the pair is bound to this enumerated assertion rather than another. Provenance "
        "closure: any transcript reference that functions as evidence — the assertion identity, the A-fail result, and the A-pass result — must quote verbatim the text of the "
        "qualifying observation record that establishes it, and counts as evidence only if that exact text occurs in a referenced qualifying observation record; a result or identity "
        "typed as prose that does not appear verbatim in a cited qualifying observation record is not evidence and does not witness A. For a behavioral assertion whose artifact is "
        "present and perturbable, the absence of an A-fail does not terminate the branch; it triggers construction: construct an admissible minimal perturbation of the current "
        "artifact that makes A alone fail while every other assertion still passes, apply the guard to that perturbed state to obtain the A-fail, emit the Failure from that result "
        "record, then restore; a modification that fails every assertion at once discriminates none. This construction is mandatory whenever the artifact is present and perturbable — "
        "it is not an optional convenience for artifacts that already pass A, and ambiguity in the satisfying artifact is resolved by constructing a definite violating state, not "
        "cited as observation unavailable: 'observation unavailable' is unreachable for a behavioral assertion whose artifact is present and perturbable. Constructing this violating "
        "state does not select which assertion is required or judge its adequacy — property selection remains the work of ground and adequacy the work of verify; falsify only "
        "constructs the concrete perturbed state that produces A's own discriminating result. Emit 'Failure: assertion \"<verbatim assertion>\" — <A-fail result> / <A-pass result>' "
        "quoting both observed results from their result records; a 'witness:' line, if added, only projects those two results and is not itself evidence. A structural assertion — one "
        "whose property has the artifact's own text or structure as its primary subject, so no behavioral failing application exists — is witnessed instead by 'Unobservable: assertion "
        '"<verbatim assertion>" — structural\'; Unobservable is never satisfied by having searched for a failing application and found none. An assertion that is behavioral yet no '
        "admissible modification can make fail is a guard defect: strengthen the guard and repeat. A guard is a blind spot whenever an admissible state exists in which its "
        "pass-or-fail outcome is not equivalent to whether the property holds — check three: (a) proxy or non-exercising guard checking a surrogate; (b) durability — a guard gone at "
        "the moment it would need to fire; (c) ephemeral guard not committed to a location that re-applies. Provenance-bound verdicts govern both gates: every 'Overreach:' verdict and "
        "every 'Failure:' observation is valid only when it cites a qualifying result record that mechanically produces it by execution identity and quotes its result verbatim; a "
        "verdict emitted from description or analysis alone does not satisfy this token. A structural Unobservable assertion is exempt, having no application to anchor to. 'Coverage: "
        "complete' is valid only when every enumerated assertion has either a 'Failure:' line backed by its A-fail/A-pass application pair or an 'Unobservable: … — structural' line, "
        "and no 'Overreach: found' remains open; an 'Unwitnessed: … — observation unavailable' assertion leaves coverage incomplete and does not satisfy this token. When qualifying "
        "observation is unavailable for one or more behavioral assertions, the honest terminal state is 'Coverage: incomplete — observation unavailable', not a claimed completion; the "
        "protocol may construct guards, perturbations, and expected outcomes and identify what remains unverified, but it may not represent any observation-dependent verdict as "
        "observed.",
        ...
        "both hold simultaneously, and the possibility that neither holds. Each quadrant must appear in the response with at least one named claim about what would be true if that "
        ...
        "ground": "The response must produce a 'Ground properties:' block before any test, implementation step, tool call, or task reasoning begins. If the request admits more than one "
        "reasonable interpretation, the chosen interpretation must first be declared on a line beginning exactly: 'Interpretation: <chosen interpretation>' — all subsequent properties "
        "are derived exclusively from this interpretation. Before writing the first 'property [N]:' line, verify that an 'Interpretation:' line appears above it if the request admits "
        "multiple readings; if the request is ambiguous and no Interpretation: line is present, emit one now — a 'property [N]:' line that appears without a preceding Interpretation: "
        "line when the request admits multiple readings does not satisfy this protocol. If the request is unambiguous and no Interpretation: line is required, the request itself is the "
        "chosen interpretation. Every property must be derivable from the explicit constraints of the chosen interpretation. A property that introduces a constraint not implied by "
        "those constraints is out of scope and must not appear in the block. Each property is written on its own line beginning 'property [N]:' followed by a candidate definition. Step "
        "1 establishes the canonical formal definition; a property need not already be expressed formally when first written. The canonical definition must be expressed in formal "
        "notation: a typed mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation. Natural-language prose alone is not a canonical definition, "
        "regardless of how precisely it is phrased. The canonical definition must have a single unambiguous denotation such that two independent readers applying the same "
        "interpretation reach the same conclusion about whether any given construction satisfies the property. Every property entering the retained set must have a formal canonical "
        "definition before it can be retained, quoted in 'Retained properties:', or governed by any downstream token. Apply the following recursive decomposition procedure to every "
        "retained property. Initially, the candidate properties are the property [N] entries in the Ground properties block; a property enters the retained set only after "
        "canonicalization and scope validation. Every property that can enter the retained set — including each initial property [N], each property added during the completeness phase, "
        "each property produced by a merge, and each property introduced during audit-driven Ground re-entry — must have as its primary subject the requested artifact or outcome "
        "itself: a required behavior, state, output, interface, or other condition explicitly imposed by the chosen interpretation. A property must not have as its primary subject the "
        "process used to produce, implement, test, review, validate, inspect, measure, or verify the requested artifact or outcome, unless that process is itself an explicit constraint "
        "of the request. Evidence about a property is not the property itself. Tests, assertions, review artifacts, checklists, tool results, validation files, implementation-process "
        "artifacts, and other verification evidence may establish or falsify a property but must not be promoted into a Ground property merely because they are necessary to verify it. "
        "A property must not be retained merely because it is a conjunction of in-scope and out-of-scope conditions: a conjunction property is subject to scope validation of each "
        "conjunct independently; any conjunct whose primary subject is a process, evidence artifact, or mechanism not explicitly required by the interpretation renders the entire "
        "conjunction out of scope unless the offending conjunct is removed before retention. A property must not constrain an internal mechanism, architecture, algorithm, or "
        "representation as its primary subject unless the chosen interpretation explicitly requires that subject. Boundary construction: before the boundary is used to admit any "
        "property, determine what the accessible-observation boundary contains. A state belongs in the accessible-observation boundary only if it is part of what the requester asked to "
        "exist — the outcome — and not part of how the artifact was made to exist — the means of producing or delivering it. The deciding test is indifference across means: ask whether "
        "the requester would still want this state to hold if the artifact could be delivered by any other means. If the requester is indifferent between alternative means that yield "
        "the same delivered artifact — the same artifact reached by a different production or delivery path — then the state is a means and does not belong in the boundary. If the "
        "requester wants exactly this state and no substitute means would satisfy them, because the state is itself the outcome and not a way of reaching it, then it belongs in the "
        "boundary. This is stated as what the boundary must contain, not what it must exclude: apply the indifference test to each candidate state and admit only those the requester "
        "wants for their own sake. A record of how the artifact was produced, delivered, or verified does not enter the boundary merely because the interpretation names it as part of "
        "the artifact or the boundary; such a record enters only if it independently passes the indifference test — that is, only when the requester wants that record itself as the "
        "outcome rather than as evidence of how the work arrived. This admission test is stated as what a property must be, not what it must not be: run it by holding the observed "
        "delivered artifact fixed and varying the entire process that produced, delivered, reviewed, or verified it; the property is admissible exactly when its truth-value does not "
        "change under that variation, because only then does it constrain the artifact the requester receives rather than the means of producing or handing it over. A candidate whose "
        "truth-value can change under that variation while the observed delivered artifact is unchanged has not met the admission principle and does not enter the property set — this "
        "holds even when the chosen interpretation has named that process element, because naming a means of production or delivery in the interpretation does not convert it into a "
        "state of the delivered artifact. Apply the admission principle before the subject-scope check below. Scope classification of the primary subject must occur before "
        "canonicalization can proceed: if the candidate property's primary subject is not the requested artifact or outcome, the property is out of scope and must be rejected before "
        "Step 1 is entered. Canonicalization may formalize a property's definition but may not change its primary subject from the requested artifact or outcome to a process, "
        "mechanism, or evidence artifact. Subject-stability test: replace the implementation and verification process completely while preserving the same requested artifact or "
        "outcome. If the property can become false solely because the implementation, test, review, validation, or verification process changed while the requested artifact or outcome "
        "remains the same, the property is out of scope unless that process is explicitly required by the request. This scope test applies recursively to every property and "
        "sub-property produced during decomposition, completeness, merging, or audit-driven Ground re-entry. Externally observable status is evaluated independently of internal-subject "
        "status and is not itself a gate for retained-set entry. Scope validation is applied to the canonical definition produced by Step 1, not to the provisional surface form — but "
        "the pre-canonicalization subject-scope check must already have passed before Step 1 is entered. A property may contain multiple logical predicates, verification conditions, "
        "evidence requirements, or procedural checks without being non-atomic. Recursive decomposition does not decompose a property's formal definition into its logical conjuncts "
        "merely because those conjuncts are independently testable or formally expressible. Recursive decomposition is concerned only with independently meaningful request constraints, "
        "not with decomposition of the evidence, verification, or implementation conditions used to establish that the property holds. A candidate sub-property is not a distinct "
        "constraint merely because it is: (a) an independently testable predicate; (b) an assertion in a regression guard; (c) a verification step or review check; (d) evidence used to "
        "establish the parent property; (e) a condition required by the implementation or test harness; or (f) a logical conjunct of the parent's formal definition. Such a predicate "
        "remains part of the parent property unless the explicit request constraints independently require it. In particular, if a property P can be verified by checking Q ∧ R ∧ S, "
        "this does not make Q, R, and S separate Ground properties unless the original request independently requires Q, R, or S — Q, R, and S are verification conditions of P, not "
        "properties of the requested outcome. A property is atomic when the recursive decomposition procedure reaches Step 4 and no valid split into independently meaningful request "
        "constraints exists. Step 1 — Canonicalize the property. Emit: '§ formality check: property [N]: \"<verbatim definition>\" — [formal | informal: <reason>]'. If informal, "
        "rewrite as a typed mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, and emit '§ rewritten: property [N]: \"<new definition>\"', then "
        "repeat the formality check on the rewritten definition. Continue until the formality check concludes formal. The definition confirmed formal in the final check is the "
        "canonical definition for this property — no subsequent step may quote a definition not yet confirmed formal. After canonicalization, verify that the chosen interpretation has "
        "established an accessible-observation boundary. If it has not, emit 'Interpretation refinement required: <what the boundary must specify>' and refine the Interpretation: line "
        "before evaluating scope. Any interpretation refinement at this point voids the current canonical definition and all downstream scope, split, and retained-set state for this "
        "and any previously processed property under the prior interpretation, and restarts property derivation from the request constraints under the refined interpretation. Then "
        "verify scope: if the canonical definition constrains an internal mechanism, architecture, algorithm, or representation as its primary subject without that subject being "
        "explicitly required by the chosen interpretation, reject the property as out of scope and do not retain it. Externally observable status is evaluated independently of "
        "internal-subject status; a property is not rejected merely because its subject is inaccessible, only because its primary subject is an internal mechanism not required by the "
        "interpretation. After scope validation, apply the minimality check: if the canonical definition is over-specific — meaning there exists an admissible construction that "
        "satisfies the chosen interpretation yet falsifies the property — the property is over-specific and must be weakened to the weakest condition the chosen interpretation entails "
        "before it can be retained. A property is over-specific when it excludes an admissible construction that the chosen interpretation would nonetheless accept as satisfying the "
        "request: the property names a particular way of satisfying a request constraint as though the request required that particular way, when the interpretation would accept other "
        "constructions equally. To weaken, replace the canonical definition with the least restrictive predicate that is still logically entailed by the explicit constraints of the "
        "chosen interpretation, such that no admissible construction satisfying the interpretation falsifies the weakened property; then re-run the formality check on the weakened "
        "definition. The chosen interpretation, not the raw request, is the weakening floor: specificity the interpretation explicitly requires is entailed and must be preserved, while "
        "specificity present only as an incidental feature of one satisfying construction must be removed. This minimality check is the dual of the later completeness check: "
        "completeness ensures the retained properties are not collectively weaker than the interpretation requires; minimality ensures no individual property is stronger than the "
        "interpretation requires. For each property, immediately after the minimality check and before Step 2, emit a line beginning '§ minimality: property [N]:' recording the outcome "
        "of that check, in one of exactly two forms: '§ minimality: property [N]: over-specific — weakened to \"<new canonical definition>\"' when an admissible construction satisfying "
        "the chosen interpretation falsifies the property, followed by re-running the formality check on the weakened definition; or '§ minimality: property [N]: minimal — most "
        'specific sub-term: "<verbatim sub-term of this property\'s canonical definition>"; candidate relaxation: <admissible construction that quotes the most specific sub-term '
        "verbatim and then weakens or drops it>; rejected: <reason that construction violates the chosen interpretation>' when none does. The minimal form must exhibit this witnessed "
        "relaxation attempt: the most specific sub-term is the narrowest literal, constant, named value, fixed set, or bound appearing in the canonical definition; the candidate "
        "relaxation is a concrete admissible construction that quotes the most specific sub-term verbatim, then generalizes or removes exactly that quoted sub-term; and the rejection "
        "reason shows that construction fails the chosen interpretation, which is what justifies retaining the sub-term. The verbatim quote is what makes the sub-term to relaxation "
        "linkage checkable from the transcript alone: the sub-term string named in the sub-term field must appear again inside the candidate relaxation; a candidate relaxation that "
        "does not contain that verbatim string does not satisfy this protocol. A bare 'minimal' verdict — one that asserts no construction falsifies the property without quoting a most "
        "specific sub-term, without naming a candidate relaxation construction that contains that sub-term verbatim, and without showing that construction violates the interpretation — "
        "does not satisfy this protocol; if the candidate relaxation instead satisfies the interpretation, the property is over-specific and the over-specific form must be emitted "
        "instead. The minimality check runs on every property without exception — including properties whose primary constraint appears obviously general, properties already in formal "
        "notation, and properties introduced during the completeness phase or produced by a merge; no property may be exempted on the ground that it is evidently minimal. A property "
        "that reaches Step 2 without an emitted '§ minimality:' line for that property, in one of these two forms, does not satisfy this protocol; if the '§ minimality:' line for a "
        "property has not appeared, emit it before introducing any provisional sub-property. Step 2 — Introduce provisional sub-properties. Introduce provisional candidate "
        "sub-properties whose conjunction is proposed to represent the current property: 'property [Na]: <definition>' and 'property [Nb]: <definition>'. Apply the canonicalization "
        "procedure (Step 1) to each provisional sub-property before proceeding. The resulting canonical definitions replace the provisional definitions for all subsequent split "
        "evaluation. The labels [Na] and [Nb] continue to identify the resulting canonical definitions after replacement. Split evidence is attached to the canonical definitions of the "
        "current property and its provisional sub-properties, and to the falsifying constructions emitted in the split test, at the moment the split test is emitted. Any modification "
        "to any of those definitions or constructions after the split test is emitted voids that evidence; the split evidence must then be regenerated against the current canonical "
        "state before the split may be accepted. Step 3 — Evaluate split validity. Before evaluating the split, verify that the chosen interpretation determines an "
        "accessible-observation boundary: the set of outputs, behaviors, states, and interactions that the party responsible for determining whether the request was satisfied can "
        "access without inspecting the implementation. If the chosen interpretation does not determine that boundary, emit 'Interpretation refinement required: <what the boundary must "
        "specify>' and emit or refine the Interpretation: line to supply it. An interpretation refinement voids the entire current retained-property tree, all previously derived "
        "properties, and all split evidence, and restarts property derivation and recursive decomposition from the request constraints under the refined interpretation; any previously "
        "emitted split test is void and does not constitute a completed split test for the current canonical state. After the accessible-observation boundary has been established and "
        "the current property, its provisional sub-properties, and their split evidence have been regenerated under the current interpretation, emit exactly one non-void candidate "
        'split test for the current canonical state: \'§ split test: property [N]: "<verbatim canonical definition>" ⇔ (property [Na]: "<verbatim canonical definition>" ∧ property '
        '[Nb]: "<verbatim canonical definition>"); property [Na] — falsified by <construction>; property [Nb] — falsified by <construction>; these falsifiers are independent.\' The '
        "quoted definitions for [N], [Na], and [Nb] must match the verbatim canonical definitions confirmed in Step 1 for each respective property. A split may be accepted only if its "
        "split test, quoted definitions, falsifying constructions, and interpretation are identical to the non-void state against which the split test was emitted. A split succeeds "
        "only if: for every admissible construction x, P(x) ⇔ (PNa(x) ∧ PNb(x)) — the original property is logically equivalent to the conjunction of the accepted sub-properties over "
        "all admissible constructions; each quoted falsifier violates exactly its quoted property while satisfying the other; both falsifiers are admissible constructions within the "
        "chosen interpretation; the candidate split is nontrivial: neither accepted sub-property is logically equivalent to the original property over all admissible constructions, and "
        "neither accepted sub-property logically entails the other over all admissible constructions; each accepted sub-property corresponds to a distinct constraint on the admissible "
        "construction under the chosen interpretation that is independently required by the explicit request constraints, and the request itself distinguishes the two sub-constraints "
        "at its own granularity — meaning the request separately names, or would be independently judged as failing on, each sub-constraint on its own; a candidate split is not valid "
        "merely because it decomposes the mathematical or definitional structure of a predicate, is not valid merely because each sub-property is independently testable, assertable, or "
        "useful as verification evidence, and is not valid merely because the two sub-constraints are logically distinct when the request does not itself distinguish them at its own "
        "granularity — if a single request constraint is being divided into finer conditions that the request never separately names or judges, the property is atomic at the request's "
        "granularity and must not be split; and neither accepted sub-property constrains an internal mechanism, architecture, algorithm, or representation as the primary subject of the "
        "constraint, unless that mechanism is explicitly required by the chosen interpretation. A sub-property constrains an internal subject when its canonical definition makes an "
        "internal mechanism, architecture, algorithm, or representation the primary subject of the constraint — this classification is independent of whether that subject happens to be "
        "externally observable. A sub-property is externally observable when its truth value can be determined solely from the accessible-observation boundary of the chosen "
        "interpretation, without inspecting the implementation. These two criteria are distinct: a property can concern an internal subject yet still be externally observable (for "
        "example, when the interpretation explicitly exposes that subject through its output contract), and a property can concern an external subject yet still be inaccessible if the "
        "interpretation does not expose the relevant behavior. The split condition rejects properties whose primary subject is internal and not explicitly required — not merely "
        "properties that happen to be inaccessible. A candidate split can be constructed only if canonical definitions for [Na] and [Nb] have been established and admissible falsifying "
        "constructions satisfying the stated falsifier conditions can be produced for both sub-properties. A candidate split test is accepted when all split-success conditions are "
        "satisfied for that non-void candidate; otherwise the candidate split is rejected. Step 4 — Accept or mark atomic. Before a property may be marked atomic, its "
        "request-constraint coverage must be made explicit and every covered boundary must have been attempted as a candidate split. Emit a line beginning '§ constraint coverage: "
        "property [N]: covers {<constraint>, <constraint>, ...}' enumerating each separately-stated explicit request constraint that the property's canonical definition covers — a "
        "constraint is separately stated when the request names it, or would independently be judged as failing on it, on its own, at the request's granularity. For every enumerated "
        "constraint boundary — every partition of the covered constraint set into two non-empty groups that the request itself distinguishes at its own granularity — a Step 3 '§ split "
        "test:' must have been emitted and evaluated against the current canonical state for a candidate split whose two sub-properties fall on opposite sides of that boundary. "
        "Rejecting an arbitrary input partition, a logical-conjunct reformulation of the predicate, a per-element or per-record decomposition, or any candidate that does not fall on a "
        "request-distinguished boundary does not discharge this obligation: such a rejection is not a split test for any enumerated constraint boundary. A property whose '§ constraint "
        "coverage:' line enumerates two or more separately-stated request constraints may not be marked atomic while any request-distinguished boundary between them has no emitted and "
        "evaluated Step 3 split test — if such a boundary remains unattempted, decomposition is incomplete and the candidate split along that boundary must be constructed and evaluated "
        "before Step 4 may conclude. A property may be marked atomic only after its '§ constraint coverage:' line has been emitted, every request-distinguished boundary among the "
        "enumerated constraints has an emitted and evaluated Step 3 split test, Step 2 has introduced provisional sub-properties, Step 1 has canonicalized them, and Step 3 has "
        "attempted candidate-split construction against the current canonical state. If the candidate split test is accepted, replace the original property with the accepted canonical "
        "sub-properties and apply this same recursive decomposition procedure (Steps 1–4) independently to each accepted sub-property. If no candidate split can be constructed, emit: "
        "'§ atomic: property [N]: \"<verbatim canonical definition>\" — no candidate split satisfies the construction requirements: <reason>.' If a candidate split test is emitted and "
        "rejected, emit: '§ atomic: property [N]: \"<verbatim canonical definition>\" — candidate split rejected: <reason>.' A property that represents a single request constraint at "
        "the request's own granularity is atomic even if its mathematical or logical expression admits formal decomposition, and even if that decomposition would yield logically "
        "distinct sub-conditions; the nontriviality and distinct-constraint conditions in Step 3 enforce this: a split into sub-properties that merely reformulate the mathematical "
        "definition of the predicate, or that subdivides a single request constraint into finer conditions the request never separately names or judges — rather than separating "
        "constraints the request itself distinguishes — is invalid and must be rejected as not nontrivial or not corresponding to distinct constraints at the request's granularity. The "
        "quoted definition must match the current canonical definition verbatim. Apply this procedure to every retained property until all have reached Step 4 and emitted '§ atomic:'. "
        "After all retained properties are atomic, emit a completeness check: '§ completeness check: \"<request constraints verbatim>\" / <P1 expression> / <P2 expression> / ...' — the "
        "request constraints must be quoted verbatim from the explicit constraints of the original request; the chosen interpretation determines the meaning of those constraints where "
        "the request is ambiguous but does not add, remove, or alter explicit constraints; interpretive assumptions made to resolve ambiguity and observation-boundary definitions are "
        "not themselves request constraints unless they were explicit constraints of the original request; every retained atomic property expression must appear exactly once, separated "
        "by ' / '; a '§ completeness check:' line that omits the request constraints or omits any retained property expression does not satisfy this requirement. Immediately after the "
        "completeness check, attempt to identify one explicit request constraint that is not logically entailed by the conjunction of the retained atomic property expressions. If such "
        "a constraint exists, emit '§ properties complete? no' — add one new property covering that explicit constraint, perform the required split testing, and repeat the completeness "
        "check. Only when every explicit request constraint is logically entailed by the conjunction of the retained atomic property expressions may the response emit '§ properties "
        "complete? yes' — a '§ properties complete? yes' line without a preceding completeness check does not satisfy this protocol; '§ properties complete? yes' certifies completeness "
        "only for the retained set existing immediately before that check; any subsequently committed removal or merge voids that result and requires a new completeness check. The "
        "completeness phase is concerned only with coverage of explicit request constraints; redundancy introduced during this phase is resolved by the subsequent "
        "observational-independence phase. After all retained properties are atomic, enter the Ground completion fixed-point procedure: first establish completeness for the current "
        "retained set; after '§ properties complete? yes', establish observational independence for that same retained set. A retained property is observationally independent if there "
        "exists at least one admissible construction that violates that property while satisfying every other retained property. Any retained property that is not observationally "
        "independent is observationally redundant and must be resolved — either successfully removed or successfully merged — provided the resolution does not reduce coverage of the "
        "explicit request constraints. A redundant property remains unresolved until either a removal or a merge replacing it is successfully committed; a failed merge or failed "
        "removal does not resolve the redundancy and requires another valid independence action. A proposed merge creates a candidate retained set by replacing its source properties "
        "with the merged property: the merged property must first complete canonicalization, scope validation, and recursive decomposition to an atomic state; once atomic, it replaces "
        "its source properties in the candidate retained set. A proposed removal creates a candidate retained set by removing the selected property; the source retained set remains "
        "unchanged until the candidate is validated. For both merge and removal, the candidate retained set must re-run the completeness-check procedure and observational-independence "
        "procedure; the candidate set may not be committed unless a new '§ completeness check:' is followed by '§ properties complete? yes' and the candidate set satisfies "
        "observational independence. A candidate retained set is committed only if it itself reaches the Ground fixed point — passing both '§ properties complete? yes' and the "
        "observational-independence procedure — before it replaces the source retained set; otherwise the candidate is discarded and the source retained set remains authoritative. If "
        "the candidate is committed and the committed change produces any further redundant property, restart the fixed-point procedure with a new completeness check on the resulting "
        "retained set. Otherwise discard the candidate and leave the source retained set unchanged, then attempt another valid independence action for the unresolved redundant "
        "property. Admissible constructions used as falsifiers or independence witnesses are evidence about the truth conditions of properties and do not thereby make the "
        "construction's internal mechanism, architecture, algorithm, or representation the primary subject of the property. Repeat until a retained set simultaneously satisfies "
        "completeness and observational independence; when no retained property is observationally redundant, the fixed-point procedure terminates. The Ground properties block is "
        "complete only when all retained properties simultaneously satisfy: they are derived from the chosen interpretation; they are atomic; their conjunction logically entails every "
        "explicit request constraint; they are observationally independent; they introduce no out-of-scope constraints, including internal mechanism constraints not explicitly required "
        "by the chosen interpretation. When all conditions are met, immediately before emitting '§ ground complete', emit exactly one 'Retained properties:' line declaring the final "
        'retained property set: \'Retained properties: property [N]: "<verbatim canonical formal definition>"; property [M]: "<verbatim canonical formal definition>"; ...\' — listing '
        "every retained property in retained-set order, each quoted definition matching its final canonical formal definition verbatim. A 'Retained properties:' line that omits any "
        "retained property, quotes a natural-language or non-canonical definition, or is not immediately followed by '§ ground complete' does not satisfy this protocol. A '§ ground "
        "complete' line that is not immediately preceded by 'Retained properties:' does not satisfy this protocol. This sentinel may not appear before '§ properties complete? yes' in "
        "the transcript. '§ ground complete' is a phase-boundary sentinel, not a response-termination condition. After emitting '§ ground complete', continue with the requested task "
        "using the retained atomic properties as the governing constraints for all subsequent task reasoning, tests, implementation steps, and tool calls. The response must not "
        "terminate solely because '§ ground complete' has been emitted. All subsequent task work must remain consistent with the completed Ground properties; the Ground properties "
        "block is not itself the requested task unless the original request explicitly asks only for property derivation. Any test, implementation step, tool call, or task reasoning "
        "that appears before '§ ground complete' does not satisfy this protocol; if task reasoning begins and '§ ground complete' has not yet appeared, emit the Ground properties block "
        "in full and '§ ground complete' before continuing. After '§ ground complete', continue with the requested task; do not treat the sentinel as terminating the response.",
        ...
        "classify each clause: a clause governs model behavior if compliance requires a specific string or structural property to be present or absent in the transcript; a "
        "concept-description clause names what the token means, the reasoning move it performs, or the output shape it produces and does not require any transcript state — apply the "
        "root criterion only to governing clauses, skip concept-description clauses. A governing clause is valid only if it names, within its own text, the specific string or "
        "structural property that would be present in a compliant transcript and absent in a non-compliant one, without requiring the evaluator to assess the model's intent. The "
        "stronger form of this criterion is addressability: a clause is addressable only if the named string or structural property allows the evaluator to determine — from the "
        "transcript alone, without semantic inference — the role's boundary (where the role-instance begins and ends), its count (how many instances exist), and its attachment (which "
        "structural position owns the instance); naming a specific string is insufficient for a clause that constrains cardinality or structural attachment — the named string must also "
        "allow the evaluator to determine boundary, count, and attachment from the transcript alone without semantic inference; a clause demonstrates the addressability gap only when "
        "it names a specific sentence from the subject instruction that would produce equal evaluator-observable transcripts with different evaluator-agreement rates on count or "
        "attachment. Before auditing any clause, characterize its type — permitted types are: allow-list clause (names what must be present; where a clause names an action, the "
        "allow-list must name the conditions that permit that action), prescription clause (names a step without naming the criterion the step serves), or prescription cluster (a set "
        "of clauses none of which derives from a criterion stated within the instruction). Any clause not matching a permitted type is invalid. The permitted-type list above is "
        "exhaustive. A deny-list clause (names what must be absent without naming the present-state observable) is invalid and must be converted: name the allow-list condition that "
        "permits the behavior before treating the clause as closed. For each invalid clause: name it and its type; name the specific string whose removal would leave the "
        ...
        "different evaluator-agreement rates on count or attachment (the addressability gap); before producing any rewrite, list each finding and its indistinguishability mechanism — "
        "two findings share a mechanism when the same string removal leaves both gaps open — if two or more findings share a mechanism, a single general principle is the required "
        "output and per-clause rewrites are not permitted until it is shown no general principle closes the same gaps; for each rewrite or general principle, name the specific string "
        "distinguishing the two compliance states before claiming the gap is closed; a rewrite is permitted only when it is no longer than the original, or when each added clause "
        "closes a named gap that the shorter form leaves open — the gap must appear as a labeled finding in the same response before the clause that closes it is added. All output — "
        "findings, general principles, and rewrites — must use only structural terms; a term is structural if its meaning does not change when the instruction's subject matter changes. "
        "When two strings must co-occur in the same response turn, numbered step, or delimited block — where the block delimiters are named within the governing instruction — to "
        "jointly satisfy a requirement, the clause must name both strings and the co-occurrence condition explicitly — a clause that names only one string of a co-occurrence pair "
        "leaves the same-unit condition unevaluable by transcript inspection alone. Corrective protocol check: after the clause-type audit, classify the subject token as an artifact "
        "token (output has a fixed schema; structural check sufficient) or a process token (output is a sequence of steps where deviation is possible at each step). For each governing "
        "clause in a process token, check whether the definition contains a corrective protocol for that clause. A corrective protocol is a named sequence that: (1) quotes the specific "
        "string from the governed clause it monitors; (2) attempts to construct a concrete violation of that clause; (3) classifies the outcome using a vocabulary defined within the "
        "same definition (e.g. 'found' / 'not found'); (4) routes each classification to a named adjustment (e.g. 'if found, fix the guard and repeat from step (2)'); (5) reaches a "
        "terminal state that ends iteration. A governing clause with no corresponding corrective protocol is unenforced — name it as a finding. Example of a corrective protocol "
        "satisfying all five properties: falsify's test blind-spot step — 'attempt to falsify the quoted guard by constructing a concrete scenario in which property [N] is absent while "
        "every listed assertion nevertheless passes; emit Test blind-spot: found — <construction> or Test blind-spot: not found; if found, fix the guard and repeat from step (2)' — "
        "this cites 'the quoted guard' (1), constructs a violation scenario (2), classifies with found/not found (3), routes 'if found' to a named fix (4), and terminates when 'not "
        "found' (5). A corrective protocol finding must name the governing clause verbatim and state which of the five properties is absent. Prior-state check: a governing clause is "
        "prior-state-dependent when its satisfying condition is the presence or absence of some earlier transcript content rather than a property of the current output — its compliance "
        "cannot be decided from the clause's own turn alone. For each prior-state-dependent governing clause, in addition to the corrective-protocol check, verify the clause names "
        "within its own text both: (a) the qualifying-result-class — which class of earlier transcript content counts as satisfying (e.g. a command-execution result versus a file read, "
        "a specific block header, a named tool's output) rather than treating any earlier content as sufficient; and (b) the transcript-position relation — the required ordering "
        "between the qualifying content and the clause (e.g. the content must appear at an earlier transcript position than the clause). A prior-state-dependent clause naming neither, "
        "or only one, of (a) and (b) is a prior-state escape route: name it as a finding, quoting the clause verbatim and stating which of (a) or (b) is unnamed — a clause that accepts "
        "'some earlier content exists' without naming the result-class permits any earlier content to satisfy it (a measurable-proxy escape route), and a clause that names the "
        "result-class without the position relation cannot distinguish content appearing before the clause from content appearing after it.",
        ...
        "interfaces, and documented invariants present: high; scattered helpers or implicit state with no explicit contract: low. Scope: how widely a change at this seam propagates "
        "across domains or layers — confined to a single bounded context: low; ripples across multiple services, teams, or abstraction layers: high. Volatility: how tightly temporal or "
        "structural coupling forces synchronised change sets — tight timing dependencies, shared schemas, shared algorithms, or positional interfaces present: high. For each dimension, "
        "the response names which of the listed structural properties are present in the seam and states the resulting classification. All three pressures must appear in the output; "
        ...
        "prism": "The response enhances the task by first enumerating N named vantage-point frames as a governing artifact, where N is stated as a numeral in the enumeration artifact — each "
        "frame is introduced by a named label that appears as a heading in the transcript, and each frame output block is headed by that label; a frame without a heading label or an "
        "output block not headed by its label does not satisfy this requirement. Each frame names a single structural vantage point — a frame body that enumerates alternative "
        "approaches, methods, or investigation paths does not satisfy this requirement. The enumeration is a precondition: no frame may be applied until all N frames are named. A "
        "response that applies frames without first enumerating them, or that produces a count of headed output blocks not equal to N, does not satisfy this requirement.",
        ...
        "constrain reasoning.",
        "release": "The response reduces distortion by de-weighting contingent or temporary features of the situation — asking which concerns would dissolve if the current state changed; "
        "concerns identified as contingent are absent from the response's constraint set and do not appear as premises in any argument or as conditions on any recommendation.",
        ...
        "rigor": "The response enhances the task by making its reasoning explicit such that each claim names the specific premise or evidence it follows from.",
        ...
        "robust": "The response enhances the task by reasoning under deep uncertainty, favoring options where the response names at least one future scenario in which each option fails and at "
        "least one in which an alternative succeeds, rather than optimizing for a single forecast.",
        ...
        "is not a candidate frame; it is the ground the sort sits on.  The disposition lines are emitted before anything else this pass does — every invocation emits them, and any "
        "delegation tool-use block must appear at a later transcript position than the disposition lines it enforces; delegation enforces the disposition and never substitutes for "
        "emitting it.  Orchestration: the disposition may be run under one of four named shapes. Emit one line 'Orchestration: [in-thread | delegate | parallel | dialogue]' at an "
        "earlier transcript position than the first '## shoshin pass N' or '## shoshin context <label>' header, and at an earlier position than the first disposition block — the "
        "Orchestration line precedes the disposition it governs, because it fixes how many disposition blocks there will be. - in-thread: reason under the disposition in this thread. "
        "One pass, one disposition block. - delegate: one isolated subagent pass, chosen when the inadmissible frames are separable from the minimal task statement. The invocation "
        "appears as a subagent tool-use block and Reconcile cites its result block. - parallel: two or more isolated contexts each reason under their own disposition with no exchange "
        "between them; their outputs are compared only after each has produced its result. Emit one disposition block per context under its own '## shoshin context <label>' header at "
        "an earlier position than that context runs. - dialogue: two or more isolated contexts exchange outputs across rounds — one context's output is admitted as a fact into "
        "another's next pass. dialogue is chosen when the contexts are interdependent rather than merely comparable: one context's finding would change how another reasons, so the "
        "shapes must exchange outputs as admitted facts rather than only compare final results — routing to parallel instead whenever no context's output needs to enter another's "
        "reasoning. Emit one line 'Message <N>: <from-label> to <to-label> — <content>' per exchange, where <N> increments by one per Message line in transcript order across the whole "
        "dialogue and each Message names exactly one <from-label> and one <to-label>; a message intended for more than one recipient is emitted as multiple Message lines, one per "
        "recipient, each independently Forwarded:-audited. Replies are not required: a Message obligates no answering Message, and one-directional or asymmetric exchange is "
        "conformant. Each Message line appears at a later transcript position than the receiving context's disposition block and at an earlier position than that context's next '## "
        "shoshin pass'/'context' header, so every forwarded item has a disposition to be audited against. Each Message is Forwarded:-audited exactly as a delegate briefing is: one "
        "'Forwarded: <item> — [requirement | inadmissible-frame]' line per item in the message; an inadmissible-frame item must be absent from the message or appear only as an "
        "explicit question. The participant set is exactly the set of labels appearing in any Message line's <from-label> or <to-label> field. Termination is per-context: every label "
        "in the participant set emits its own 'Terminal: <label-of-context> — [converged | deadlock | divergence | withdrawn]' at a later transcript position than that context's own "
        "last disposition block and last Message line it sent or received, whichever is latest, and at an earlier position than Reconcile. converged = this context reached a claim "
        "shared with a context it exchanged with; deadlock = this context holds a claim incompatible with a context it exchanged with and neither drops it; divergence = this context "
        "produced a different claim, reported as the finding; withdrawn = this context declines a claim and must name what it declines, as 'Terminal: <ctx> — withdrawn: no claim on "
        "<the exchanged question it exits>'. The labels of different contexts need not match; convergence across contexts is neither the goal nor required — a productive deadlock, a "
        "reported divergence, or a mix is a valid terminal state, the value of the exchange being in the differences it surfaces, not in agreement. Termination check: for each label "
        "in the participant set, confirm exactly one Terminal line owns it (a line whose <label-of-context> equals that label); a participant label owning no Terminal line leaves the "
        "dialogue incomplete — emit the missing Terminal, or record that context as 'withdrawn: no claim on <the question of its last received Message, or, if it received none, the "
        "content of its own last sent Message>'; a participant label owning more than one Terminal line is a violation — keep the last such line in transcript order and remove the "
        "earlier ones; the check terminates when every participant label owns exactly one Terminal line. A Terminal line is valid only when at least one Message line appears at an "
        "earlier transcript position. Isolation mode (parallel and dialogue), block decides — the same observable that resolves delegate: if a subagent tool-use result block is "
        "present for a context, that context's isolation is ENFORCED and its 'Leak: not found' for inadmissible frames is guaranteed by construction; if no such block is present, the "
        "context is a DECLARED in-thread simulation (reasoning under withheld frames without a separate process) and isolation rests on the leak check alone, with the same honest "
        "ceiling as single-thread delegation — declared, not enforced. Separability governs parallel and dialogue as it governs delegate: a context is isolated only when the frames "
        "withheld from it are separable from the minimal statement of its task; a frame that re-enters through a message is a leak, caught by that message's Forwarded: audit and by "
        "the leak check. Every item entering an isolated context — the brief that spawns a delegate, parallel, or dialogue context, and every subsequent dialogue message — is "
        "Forwarded:-audited before that context's tool-use call: emit one 'Forwarded: <item> — [requirement | inadmissible-frame]' line per item, each at an earlier transcript "
        "position than the tool-use block it enters; an inadmissible-frame item must be absent from that input or appear only as an explicit question. The leak check runs over every "
        "context's conclusions AND every Message's forwarded content.  (3) Reason: reason about the subject using the default and admit frames, excluding the inadmissible frames.  (4) "
        "Reconcile: compare the selected-frame result against the current-frame view and either quote a specific differing claim or state no claim differs. Before Reconcile, run the "
        "leak check: for each conclusion, identify any inadmissible frame it relied on — reused in-thread or passed as an assumption in a delegate's tool-use input; emit 'Leak: found' "
        "with the frame and redo excluding it, or 'Leak: not found'; iteration terminates at 'Leak: not found'. When the pass ran in a genuinely isolating subagent that never received "
        "the inadmissible frames, 'Leak: not found' for those is guaranteed by construction. A shoshin-eligible response that produced no disposition lines has not established that "
        "its frame set was chosen rather than inherited; a response containing a delegation tool-use block with no disposition lines at an earlier transcript position does not satisfy "
        "this token; and a response containing a delegation tool-use block with no 'Forwarded:' line at an earlier transcript position than that block does not satisfy this token, "
        "since the items entering the delegate's prompt were not audited for inherited framing before the handoff.",
...
AXIS_KEY_TO_LABEL: Dict[str, Dict[str, str]] = {
    "channel": {
        ...
        "browse": "Drive a browser via agent-browser CLI",
        ...
        "image": "Generated image only, no prose",
        "jira": "Jira markup formatting",
        "ledger": "Write to persistent categorized ledger file",
        "notebook": "Jupyter notebook (.ipynb) only",
        "notion": "Deliver to Notion via ntn CLI",
        ...
        "video": "Generated video only, no prose",
    ...
    "method": {
        ...
        "atomic": {
            "distinctions": [
                ...
                {
                    "note": "ground = recursive protocol that resolves ambiguity by deriving formal atomic properties (§ ground complete sentinel); atomic = one observable "
                    "change per step. use ground when the request needs disambiguation and formal property derivation; use atomic to control step size.",
                    "token": "ground",
                },
            ],
            "heuristics": [
        ...
        "enforce": {
            "distinctions": [
                {
                    "note": "enforce converts procedural requirements into structural dependencies; gate blocks action on unmet conditions; falsify requires attributable "
                    "failing and passing observations",
                    "token": "gate",
                },
                {
        ...
        "falsify": {
            "distinctions": [
                {
                    "note": "falsify requires attributable observations that distinguish an assertion's violating and satisfying states; gate constrains whether later action is "
                    "blocked on a condition.",
                    "token": "gate",
                },
                {
                    "note": "verify applies falsification pressure to claims; falsify requires assertion-bound, mechanically observed failing and passing results with retained "
                    "provenance.",
                    "token": "verify",
                },
                {
        ...
        "ground": {
            "distinctions": [
                {
                    "note": "gate blocks the next action until a condition is met; falsify requires attributable discriminating observations; ground recursively derives an "
                    "unambiguous request and its governing properties.",
                    "token": "gate",
                },
                {
                ...
                {
                    "note": "atomic = one observable change per step; ground = recursive protocol that resolves ambiguity by deriving formal atomic properties (§ ground complete "
                    "sentinel). ground handles disambiguation and formal property derivation; atomic governs step size within each part.",
                    "token": "atomic",
                },
                {
        ...
        "mint": {
            ...
            "heuristics": [
                ...
                "don't reproduce source structure",
                "anti-mirroring",
        ...
        "verify": {
            "distinctions": [
                {
                    "note": "gate blocks action until a condition is met; falsify requires assertion-bound failing and passing observations; verify applies falsification "
                    "pressure to claims without requiring an observation pair.",
                    "token": "gate",
                },
                {
