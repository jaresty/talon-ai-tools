...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "descent through prose → criteria → formal notation → validation → implementation; skipping a rung is a protocol violation), upward return conditions (what triggers return, "
        "what does NOT trigger return and why excluded), evidential scoping (evidence is scoped to current cycle and gap, not prior cycles), scope preservation (scope does not expand "
        "between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); each derived rule must cite its source principle(s) from P1-P6; "
        "rules that cannot be derived are protocol violations; emit Protocol derived after deriving all rules; emit Ladder derived after producing the rung table; ground complete may "
        "only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: a rung is valid iff a human reviewer with "
        "only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → executable validation → executable "
