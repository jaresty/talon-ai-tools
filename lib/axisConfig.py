...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "form": {
        ...
        "damaging. For each concept after the first, begin with a Following from: line quoting a specific phrase from the preceding answer, making explicit what the previous reveal "
        "established. Then write the question, require a Predict: line (the reader names their expectation before the answer is shown), then write the answer. After each answer: a "
        ...
        "specific phrase from the current answer and naming the implication it leaves unresolved — the question the next concept answers (evaluator check: the quoted phrase appears "
        "verbatim in the current answer); a Hook: line must not appear after the last concept's answer; then for at least one concept per quiz, a Misconception: line naming the specific "
        "incorrect belief that would lead a reader to predict the wrong answer, and a Why: line stating the structural reason that belief is wrong; then an association sentence — before "
        "writing it, name the numbers already used in prior association sentences — using the format '#N (Name): [relationship]' where N is the opening-list number of a prior concept whose "
        "number has not appeared in any earlier association sentence in this quiz and Name is that concept's name from the opening list (for the first concept, use the format '#N (Name): "
        ...
        "then close with a terminal declaration listing all confirmed concepts. With an output-exclusive channel, structure the output as a quiz document with each question immediately "
        "before its answer per topic.",
