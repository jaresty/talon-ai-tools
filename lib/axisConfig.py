...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "cycle. R2: each artifact derives from the prior rung — form changes, intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap "
        ...
        "output — satisfies any gate regardless of accuracy. a rung label may not be emitted until its gate conditions are met; the gate condition for each rung is the rung table’s "
        "gate column. P3 (Scope discipline): each rung artifact addresses exactly the gap declared by the prior rung — nothing beyond it; one gap per cycle; one criterion per thread "
        "per cycle; one edit per re-run; After ✅ Manifest declared, the only valid next token is the criteria rung label for Thread 1 — no criteria for other threads, no planning "
        "narration, no other content; all seven rungs must complete for Thread N before any rung content for Thread N+1 may appear. no anticipation of future gaps, no additional "
        ...
        "not an edit. Rung table — name | artifact type | gate condition | void condition: prose: artifact=“natural language description of intent and constraints”, gate=“I declared”, "
        "voids_if=“skipped”; criteria: artifact=“falsifiable behavioral acceptance conditions”, gate=“prose complete”, voids_if=“criterion is structural/implementation rather than "
        "behavioral”; formal notation: artifact=“non-executable specification with behavioral invariants + R2 audit”, gate=“criteria complete; R2 audit 0 UNENCODED entries”, "
        "voids_if=“UNENCODED entries remain; audit not a separate named section”; executable validation: artifact=“test file invocable by an automated tool, within project tree”, "
        "gate=“formal notation R2 audit complete”, voids_if=“implementation code included; outside project tree; pre-existing artifact”; validation run observation: "
        ...
        "failure; infrastructure-only output”; executable implementation: artifact=“implementation source edits; one edit per re-run cycle”, gate=“exec_observed + gap declared; "
        "impl_gate sentinel emitted”, voids_if=“multiple edits without intervening re-run; newly-passing test has no prior failure”; observed running behavior: artifact=“live-process "
        "output — output produced by a tool call that starts or queries a running process; reading any file does not satisfy this type regardless of content”, gate=“implementation "
        "complete; exec_observed + gap for behavior from I”, voids_if=“new files created; output names only infrastructure state; file read used as evidence; test runner output — a "
        "test-suite pass is validation-run-observation-type output, not observed-running-behavior-type output, and voids this rung”. 🔴 Execution observed: requires: (1) a preceding "
        "tool call in this response, (2) verbatim tool output in a triple-backtick block — no editorial content, no paraphrasing. If the output is long, include the first failure and "
        "the final summary line, then insert exactly one '[... N lines omitted ...]' marker with the actual count — omitting content without a count marker voids the sentinel. Any "
        "other deviation voids the sentinel; fix by re-invoking the tool and re-emitting the block. If no tool call has been made in the current response, 🔴 Execution observed: is a "
        "protocol violation — stop, make the tool call, then write the sentinel with the verbatim output; generated text that resembles tool output does not satisfy this sentinel under "
        "any circumstances. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current gap before any "
        ...
        "complete is a protocol violation; Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol "
        ...
        "implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — one independently testable behavior derived from the prose alone — if "
        ...
        "source is a conjunction — split before continuing; asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect; before "
        "writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a citable manifest "
        "excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; each "
        "criterion states (a) one observable behavior and (b) a falsifying condition referencing no implementation internals (no file names, function names, or variable names); this is "
        "a content gate, not a self-check; exactly one criterion may be written per thread per cycle — after the first criterion is written, the only valid next token is the formal "
        "notation rung label; a second criterion for the same thread is a protocol violation — stop, return to the prose rung, and split the thread before descending again; the "
        "criteria rung label is per-thread — writing criteria for Thread 2 or any subsequent thread before ✅ Thread 1 complete is emitted is a protocol violation regardless of whether "
        "they share a criteria rung label; batch-collecting criteria for multiple threads under one criteria label bypasses sequential descent and is a protocol violation. The formal "
        "notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be true — "
        "the executable or testable part. Natural language labels, introduces, and explains the specification — prose adds context but the behavioral constraints must still be encoded "
        "in notation.it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe "
        "what notation looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type signatures, "
        ...
        "separate and named; the rung is incomplete until all criteria are encoded. Natural language may appear as section labels or introductions (e.g., ‘This invariant captures the "
        "rendering constraint:’) but may not substitute for encoding a constraint in notation — if a constraint can be stated in notation, it must be. each test function asserts "
        "exactly one behavioral property — one behavioral property per test function; multiple assertions are permitted only when they jointly constitute a single indivisible check "
        "(e.g., asserting a value is both non-null and of the expected type); before writing the test, re-read the formal notation and assert each structural constraint it encodes; "
        "confirm via tool call that the criteria artifact for this cycle was produced in this cycle's transcript; if the criteria artifact is identical to a prior cycle’s criteria "
        "artifact, the criterion has not changed — return to the prose rung and re-emit with an updated gap before descending again; before writing the test, check for an existing test "
        "file covering the same module — add there if so; when the artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — "
        ...
        "in the artifact]’ — each prior-failure name must be quotable verbatim from a 🔴 Execution observed sentinel; modification without carry-forward is a traversal violation; no "
        ...
        "artifact in the transcript is a protocol violation; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is "
        "already failing — V-complete may not be emitted without this tool-executed result in the transcript; asserting confirmation without showing the exec_observed block does not "
        "satisfy this gate — the exec_observed block must appear in the current response; the validation artifact must reside within the project’s version-controlled file tree; if the "
        ...
        "cycles, not to HARD STOP returns. Upon writing the observed running behavior label, re-emit the criterion for this cycle — criterion re-emission is required: emitting the OBR "
        "label without immediately following it with the criterion text is a protocol violation; the tool call is the only valid next action after criterion re-emission; planning "
        "statements, diagnostic narration, and commentary between criterion re-emission and the tool call are protocol violations; ✅ Thread N complete may not appear until a tool call "
        "exists in the transcript after the observed running behavior label in the current cycle; then immediately invoke the implementation as a live running process via a tool call "
        "and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion. a test runner invocation "
        "does not satisfy this gate — running tests produces validation-run-observation-type output, not observed-running-behavior-type output; the invocation target must be the "
        "implementation artifact itself, not a test harness wrapping it; A file read never satisfies this rung — the block must contain output produced by running the artifact. If the "
        "artifact requires a start step before querying (e.g., a web server), two tool calls are required in sequence: process-start then query — the 🔴 Execution observed: sentinel may "
        "not be emitted until both complete. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists. If no invocable "
        "process exists, open a gap cycle to make the artifact directly invocable. Before any tool call at this rung, emit a provenance statement naming the invocation target: "
        "‘Invoking [artifact] produced at [rung name] in this thread’ — if the target is a test runner, stop: the invocation target must be the implementation artifact itself running "
        "as a live process, not a test harness; the provenance statement does not replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not "
        "be emitted unless all of the following exist in this cycle's transcript: (1) a tool call exists in the transcript after the observed running behavior label in the current "
        "cycle; (2) observed running behavior label has been written after the most recent 🟢 Implementation gate cleared in this thread; (3) non-empty verbatim output appears in the "
        "transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution observed: is a protocol violation; (4) the 🔴 "
        "Execution observed: directly demonstrates the criterion — if it does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the "
        "upward-return failure-class rules; (5) a full test suite run result exists after the most recent OBR tool call — the only valid next action if no such result exists is the "
        "tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason. ✅ Thread N complete immediately follows 🔴 Execution "
        "observed: at the OBR rung — not before it; emitting ✅ Thread N complete before the OBR rung tool call and 🔴 Execution observed: sentinel exist in this cycle is a protocol "
        "violation. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal notation, executable validation, executable "
        ...
        "resolve; faithful derivation requires form to change, intent to stay fixed. Advance through every feasible rung without pausing for user confirmation between rungs — "
        "completeness governs rung depth, not rung existence; each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in "
        "every cycle; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; stopping between rungs at any other point is a protocol violation "
        "— waiting for user confirmation between rungs is a protocol violation; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate "
        "the highest already-instantiated rung, update it, then descend; every descent through executable implementation requires executable validation and validation run observation "
        "to have fired for the current gap, including after an upward return. Upward returns follow the failure class: if the implementation did not close the gap, loop within "
        "executable implementation; if the spec did not model the criterion, return to formal notation; observation cannot be produced is a prose-description failure, not an "
        "implementation failure — return to the prose rung and update it. After every upward return, a new ✅ Validation artifact V complete must be emitted for the current cycle’s gap "
        "— an artifact written in a prior cycle does not satisfy this gate. Before producing the final report, invoke the delivered system end-to-end via a tool call and emit 🔴 "
        "Execution observed: showing the primary behavioral intent is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before producing "
        "the final report. Any prose that summarizes implemented behavior, lists acceptance criteria met, describes files created or modified, or names gaps as resolved is a final "
        "report regardless of its heading; a final report may not appear unless ✅ Manifest exhausted immediately precedes it in the transcript — if no ✅ Manifest exhausted exists, "
        "stop: emit ✅ Manifest exhausted if all threads are complete, or open the next gap cycle if they are not. After emitting ✅ Manifest exhausted, produce a final report containing "
        "prose, criteria, and formal notation for each thread in order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the "
        "implementation affects. Rung-entry gate: before producing content at any rung, verify that a valid 🔴 Execution observed: sentinel exists in the current cycle whose output "
        "matches the rung’s artifact type — if (d) reveals that no valid exec_observed exists in the current cycle, the only permitted next token is a tool call — producing any other "
        "content when (d) is false is a protocol violation. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable "
