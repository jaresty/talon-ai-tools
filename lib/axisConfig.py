...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "unrelated verification commands are not regression guards unless they directly execute the established guard itself; repeating step (2) replaces the previous regression "
        "guard; after replacement, every subsequent assertion inventory, 'Quoted test:', 'Failure:', and implementation overreach analysis must derive exclusively from the replacement "
        "guard; references to earlier guards no longer satisfy this token; until the guard-establishing tool-result block has appeared, no 'Property [N] assertion', 'Assertion "
        "inventory:', 'Quoted test:', 'Test blind-spot:', 'Failure:', 'Quoted implementation:', 'Implementation overreach:', implementation change, or artifact modification may occur; "
        "Guard artifact provenance: the established regression guard is an already-confirmed artifact state, not a planned or anticipated test. Before any 'Property [N] assertion "
        "[M]:', 'Assertion inventory:', or 'Quoted test:' line may be emitted, the guard's executable text must have appeared verbatim in the qualifying guard-establishment "
        "tool-result block produced in step (2). Every assertion inventory entry must be copied verbatim from guard text confirmed in that tool-result block — an assertion entry whose "
        "text does not appear verbatim in the qualifying prior tool-result block is invalid. A 'Quoted test:' line must quote verbatim text that appeared in the qualifying "
        "guard-establishment tool-result block; a 'Quoted test:' line that introduces, predicts, paraphrases, or quotes test text not present verbatim in that prior tool-result does "
        "not satisfy this token. For every executable assertion in the confirmed guard text emit exactly one 'Property [N] assertion [M]: <quoted assertion text>' line — every "
        "executable assertion must appear exactly once and only once; omitting, merging, or inventing an assertion does not satisfy this token; after the assertion inventory emit "
        "'Assertion inventory: complete — <count> assertions for property [N]' or 'Assertion inventory: incomplete — <reason>'; if incomplete, revise the guard or inventory and repeat "
        "the assertion inventory; no subsequent step may occur until 'Assertion inventory: complete' has been emitted; the text immediately following 'Assertion inventory: complete' "
        "must be the 'Quoted test:' line — no tool call, Failure: line, Test blind-spot: attempt, or other content may appear for property [N] between 'Assertion inventory: complete' "
        "and the 'Quoted test:' line; emit 'Quoted test: <smallest contiguous verbatim executable assertion or guard uniquely identifying the written guard>'; a 'Quoted test:' line "
        "that does not immediately follow 'Assertion inventory: complete' does not satisfy this token; (3) attempt to falsify the quoted guard by constructing a concrete scenario in "
        "which property [N] is absent while every listed 'Property [N] assertion [M]:' nevertheless succeeds; emit 'Test blind-spot: found — <construction>' or 'Test blind-spot: not "
        "found'; if found, strengthen the guard and repeat from step (2); (4) the text immediately following 'Test blind-spot: not found' must be the tool call that executes the "
        "quoted guard against a deliberately constructed violating state — no narrative, planning, implementation change, 'Quoted implementation:', 'Implementation overreach:', or "
        "other sentinel may appear between 'Test blind-spot: not found' and that execution tool call; no transcript content may appear between the execution tool call and the "
        "corresponding tool-result block — the execution tool call and tool result are an atomic pair; execute the quoted guard against a deliberately constructed violating state — if "
        "the governed artifact already satisfies property [N], first construct a minimal temporary modification to the artifact that violates property [N], execute the guard against "
        "that temporary state, emit the required Failure: line from the tool-result block, then restore the original state before continuing; the text immediately following the "
        "execution tool-result block must be the Failure: line for property [N] — no narrative, causal argument, prose explanation, reference to a prior failure, or other content may "
        "appear between the tool-result block and the Failure: line; a Failure: line that does not immediately follow a tool-result block does not satisfy this token; emit 'Failure: "
        'property [N] "<verbatim property definition>" — <verbatim quoted failure line from a tool-result block>\'; emit \'Unobservable: property [N] "<verbatim property definition>" '
        "— <reason>' only after demonstrating that no temporary violating state can be constructed within the governed artifact on which the guard could distinguish satisfaction from "
        "violation — Unobservable: is the last resort, not the first; successful compilation, build output, or unrelated verification does not satisfy this step — the violating state "
        "must exist solely to witness that the guard detects the absence of property [N]; before modifying the governed artifact to satisfy property [N], a verbatim Failure line from "
        "a tool-result block must appear; if the failure line does not specifically witness the violation of property [N], modify the violating state and repeat from step (4); before "
        "emitting any witness classification, determine whether every inventoried assertion can be classified mechanically from the immediately preceding execution tool-result block — "
        "a witness is mechanically derivable iff the tool-result (i) identifies exactly one assertion, (ii) identifies its outcome (pass or fail), and (iii) no other assertion "
        "satisfies the same identifier; if any witness is not mechanically derivable by this operational definition, temporarily instrument the regression guard or construct an "
        "equivalent observation harness that executes exactly the inventoried assertions without strengthening, weakening, removing, or reordering them, while emitting observable "
        "per-assertion outcomes; before executing the harness, emit 'Observation correspondence: assertion [M] -> <observation point>' for every inventoried assertion, establishing a "
        "declared one-to-one mapping from every inventoried assertion to its corresponding observation point; if a one-to-one mapping cannot be established, revise the harness; "
        "execute the instrumented guard or harness against the same violating state via a tool call and wait for the tool-result block; after execution, verify that every declared "
        "observation point produced exactly one observation in the tool-result — if any observation point produced zero or more than one observation, emit 'Observation correspondence: "
        "incomplete — <reason>', revise the harness, and repeat from step (4); also verify that every assertion observed in the tool-result output appears in the 'Observation "
        "correspondence:' mapping — if any observed assertion is absent, the harness is non-equivalent and must be revised; derive every witness classification exclusively from the "
        "verified tool-result; the temporary instrumentation exists solely to observe execution and must not become part of the regression guard or governed artifact — discard it "
        "after witness generation; the text immediately following the Failure: line (or the observation harness tool-result block if instrumentation was required) must be the first "
        "witness line — no narrative, causal argument, prose explanation, or other content may appear before the first 'Property [N] assertion [M] witness:' line; emit exactly one "
        "witness line for every previously inventoried executable assertion, in ascending assertion inventory order without omission or reordering; each witness line takes one of "
        'these forms, derived exclusively from the verified tool-result block: \'Property [N] assertion [M] witness: Failure — assertion: "<verbatim assertion text>" — observed: '
        '"<verbatim tool-result text uniquely identifying this assertion as the failure by assertion text, source location, or execution identifier>"\' if and only if the tool-result '
        "uniquely identifies this assertion as a failing outcome — when identical assertion texts appear more than once, the failure must identify the specific occurrence by source "
        'location or execution identifier; \'Property [N] assertion [M] witness: Success — assertion: "<verbatim assertion text>" — observed: "<verbatim tool-result text confirming '
        "this assertion passed>\"' if the tool-result contains a directly observed passing outcome for this assertion; if an assertion cannot be classified Success or Failure from the "
        "current tool-result, it is not yet observable — before accepting that conclusion, determine whether it is due to dead code, unreachability, mis-inventory, or harness "
        "omission; refine the observation harness or violating state to resolve the cause and repeat the instrumented execution; only after no executable instrumentation or violating "
        'state modification can make the assertion observable may \'Property [N] assertion [M] witness: Unobservable — assertion: "<verbatim assertion text>" — <observed reason quoted '
        "from tool-result>' be emitted; Unobservable is the last resort and must be justified by a verbatim tool-result excerpt; an Unobservable witness is treated as a guard defect — "
        "revise the guard to make the assertion observable and repeat from step (2); no assertion may permanently carry Unobservable status; after all witness lines are emitted, "
        "verify set equality between the assertion inventory and observed assertions: (i) every assertion observed in the harness or execution output must appear exactly once in the "
        "assertion inventory — if any observed assertion is absent from the inventory, emit 'Assertion inventory: incomplete — missing assertion: <text>' and repeat from step (2); "
        "(ii) every inventoried assertion must appear exactly once in the observation output — if any inventoried assertion is absent from the observation output, treat it as "
        "unresolved and refine the harness or violating state and repeat the instrumented execution; at least one inventoried assertion must receive witness classification Failure — a "
        "guard execution in which no inventoried assertion is classified Failure does not satisfy step (4), regardless of whether a Failure: line was produced; a Failure witness whose "
        "observed text is not present verbatim in the corresponding tool-result block does not satisfy this token; if no inventoried assertion can be attributed to the observed "
        "failure, revise the violating state or the regression guard and repeat from step (2); after all assertions are witnessed and set equality is verified, emit 'Assertion "
        "witnesses: complete — <count> assertions witnessed' where <count> must equal the count from 'Assertion inventory: complete'; a witness count that differs from the inventory "
        "count does not satisfy this token; emit 'Assertion witnesses: incomplete — <reason>' if any assertion lacks a witness; if incomplete, revise the violating state, observation "
        "harness, or guard and repeat from step (4); no 'Quoted implementation:', implementation modification, or 'Implementation overreach:' may appear before 'Assertion witnesses: "
        "complete' has been emitted; no subsequent step may occur until 'Assertion witnesses: complete' has been emitted; (5) identify the governed artifact for property [N] — the "
        "artifact whose modification satisfies property [N]; no behavior-altering modification to any governed artifact may occur before 'Assertion witnesses: complete' has been "
        "emitted for property [N] — a modification that occurs before a qualifying Failure witness for property [N] has been established does not satisfy this token; depending on the "
        "property, the governed artifact is the artifact that the established regression guard governs — the artifact whose behavior that guard constrains, in whatever form that "
        "artifact takes; it is never the guard itself, since the guard and the artifact it governs are necessarily distinct; before making any behavior-altering modification, emit "
        "'Quoted implementation: <smallest contiguous verbatim text uniquely identifying the intended modification to the governed artifact>' — this sentinel must appear in the "
        "transcript before the tool call that makes the modification; a modification tool call that appears before the 'Quoted implementation:' sentinel does not satisfy this token; "
        "the 'Quoted implementation:' content must be a contiguous fragment of the literal text the modification tool call writes into the governed artifact — a description of the "
        "change's purpose, effect, or plan (for example imperative phrasing such as 'replace X with Y', 'record A separately', 'retain B', or 'permit C') does not satisfy this token; "
        "a 'Quoted implementation:' whose content does not appear verbatim as literal artifact text in the immediately following modification tool call does not satisfy this token; "
        "moreover, a comment, docstring, or other non-executable annotation does not by itself satisfy this token — the quoted text must be the behavior-bearing artifact text whose "
        "introduction causes the guard that produced this property's Failure: witness to succeed; text whose removal would leave the outcome of the guard that produced this property's "
        "Failure: witness unchanged does not satisfy this token; after the 'Quoted implementation:' sentinel, make the modification via a tool call and wait for the tool-result block "
        "confirming it; if step (4) required constructing a temporary violating state because the governed artifact already satisfied property [N], step (5) is satisfied by confirming "
        "the original state is restored — emit 'Quoted implementation: <verbatim text identifying the restored governed artifact>' before the restoration tool call; no subsequent step "
        "may occur until this sentinel has been emitted and the corresponding modification or restoration tool call has completed;(6) attempt to construct a minimal modification to "
        "the quoted implementation that preserves the success of the quoted guard while still changing the governed artifact, and attempt to construct a concrete execution path "
        "through the quoted implementation that is not exercised or distinguished by the quoted guard; emit 'Implementation overreach: found — <modification or execution path>' or "
        "'Implementation overreach: not found'; if found, minimally revise the governed artifact to eliminate the overreach, emit a new 'Quoted implementation:', and repeat from step "
        "(6); every implementation overreach analysis must reason only from the quoted implementation and quoted guard emitted earlier in the transcript, not from an imagined or "
        "intended version. After all current properties complete their six-step cycles, begin the implementation audit. 'Coverage: complete' is the global fixed-point sentinel and is "
        "valid iff all of the following hold simultaneously: (1) every retained property completed steps (1)–(6); (2) every retained property has a witnessed Failure or justified "
        "Unobservable; (3) assertion inventory equals observed assertions for every property (set equality verified in both directions); (4) no implementation gap exists in the final "
        "audit pass; (5) Ground reached a fixed point (§ ground complete was emitted for the final retained property set); (6) the implementation audit reached a fixed point (Audit: "
