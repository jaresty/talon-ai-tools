...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        'outcome the prior rung admits]". A transition with no such line is unexamined and the ladder is incomplete. If rung 1 is below intent level, name the artifact already present '
        "in the transcript that covers intent and acceptance criteria — that citation is required before rung 1 can begin. The rules-out requirement already prevents invalid splits — a "
        "rung that cannot name a specific outcome the previous rung admits but this rung rejects cannot be added. Producing the ladder is a required artifact before any rung-work "
