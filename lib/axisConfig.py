...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "costly to maintain. Before acting, state the boundary searched (the first message of the current session, or the specific prior message where the search began), then list "
        "every statement within that boundary that describes a desired end state. For each, state whether it subsumes the others — a goal G1 subsumes G2 if achieving G2 is a necessary "
