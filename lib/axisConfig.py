...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "requires the governed artifact to have executed in order to appear in the executor output is not a valid (a); (b) the literal string signaling the governed behavior is "
        "present; (c) the literal string identifying each governed behavior in an executed-artifact result — (c) must be derived from a direct callee of the assertion call expression "
        "as it appears on the FAIL line; a value derived from a test-identifier echo line, non-FAIL line, summary line, or test description in executor output is not a valid (c); the "
        "governed symbol must appear as a call expression reachable from the assertion call expression by following the execution call chain — the chain may pass through the test "
        "body, test setup, or the governed artifact; a callee whose name contains the governed symbol name as a substring but through which the governed symbol does not appear as a "
        "call expression in the reachable execution chain is a naming proxy and does not satisfy this requirement; a call expression that satisfies (c) must name a symbol whose "
        "callable definition does not appear in the content of any Write or Edit tool call whose path argument matches the governing artifact file path — if the identifier that "
        "appears as (c) on the failure line also appears as the name of a callable definition in the content of any Write or Edit tool call whose path argument matches the governing "
        "artifact, that identifier is a governing-artifact proxy and does not satisfy (c); (d) the governed symbol: the literal string naming the governed behavior — (d) must be "
        "derived from the direct callee of the assertion call expression in the governing artifact, where the call expression is not used only as an argument to a call expression "
        "whose callee does not contain the governed symbol name; (d) must be the identifier that appears as a substring of the failure line in (g) — not a symbol that contains or is "
        "associated with that identifier; if the failure line names a narrower identifier, that narrower identifier is (d) regardless of which broader symbol was invoked to reach it; "
        "(d) is derived from the failure line, not chosen independently — the failure line names (d), not the other way around. Assertion-target provenance exception: when the "
        "following conditions hold in the governing artifact's source text, the restriction that (d) must appear as a direct callee of the assertion call expression is suspended for "
        "this governed call: (1) an identifier is assigned exactly once in the governing artifact, and its initializer's outermost non-structural callee — after stripping any leading "
        "await and trailing property-access or index-access chains — is the governed symbol; Returned-object invocation provenance: condition (1) also permits a chain of "
        "single-assignment identifiers when each identifier in the chain satisfies conditions (1)–(3) with the immediately preceding identifier in the chain substituting for the "
        "governed symbol in condition (1) — the first identifier in the chain must have the governed symbol as its outermost non-structural callee; each subsequent identifier must "
        "have the immediately preceding chain identifier as the outermost non-structural callee of its initializer after stripping any leading await and trailing property-access or "
        "index-access chains; the same no-reassignment and structural-operations constraints from condition (3) apply to each intermediate identifier; (2) that identifier appears as "
        "an argument to an assertion call expression; (3) the identifier does not appear on the left-hand side of any assignment expression between its initial assignment and the "
        "assertion call expression, and every operation on the identifier between the assignment statement and the assertion call expression is one of: property access, index access, "
        "await, grouping, or argument separator; (4) the executor result block satisfies the base (a) and (c) requirements — no additional requirement that the failure line contain "
        "(d) or the chain identifier applies under this exception. Prose-origin gate: a string does not satisfy any derivation gate — including (a), (c), (d), the allow-list, and the "
        "failure line — if it appears verbatim in the content of a Write or Edit tool call in this transcript that precedes the first executor result block in this transcript "
        "containing that string; exceptions: (1) strings governed by the Bootstrap exception; (2) strings that satisfy the creation-step exception's absent/present condition — where "
        "(c) is absent before the Write action and present after it in executor output immediately following the Write. Bootstrap exception: when no tool-result block in the "
        "transcript before the Falsify derivation block contains the governed symbol name as a verbatim substring, a single governed artifact-producing action may introduce an "
        "artifact containing the governed symbol name — this action may not produce a result in which (b) appears; the named executor must be invoked immediately after this action (no "
        "executor result block between the action and the invocation), and the result must contain (a) before (c) with (d) appearing as a substring of the failure line; no further "
        "governed artifact-producing action is permitted until this executor result appears in the transcript. (e) the layer classification — if the governed symbol name appears as a "
        "token in a call expression in the test body — where the call expression is not nested inside another call expression that names a different symbol — the layer is 'unit'; if "
        "the governed symbol name does not appear in the test body, the layer is 'integration'; (f) the executor class — the executor must produce output that could only exist if the "
        "governed assertion reached execution at the layer named in (e); a governing artifact is disposable if removing the governed symbol from the governed artifact — without "
        "modifying the named governing artifact — produces an executor result in which (c) does not appear; a governing artifact is non-disposable only when (c) appears in the "
        "executor result after removal — a disposable artifact does not satisfy (f); (f2) the serialization gate — classify the assertion target: the value produced by the expression "
        "that is the direct argument to the assertion call. A governing assertion satisfies (f2) if and only if its assertion target is a behavioral value: a value whose content is "
        "determined by whether the governed behavior is correctly implemented and that could not be produced by an arbitrary implementation that merely serializes the governed "
        "artifact's output correctly. A governing assertion fails (f2) if its assertion target is a serialization value: a value produced by converting the governed artifact's output "
        "to a string/byte/text representation — regardless of what the conversion expression is named — such that a correct implementation and an incorrect implementation would "
        "produce the same assertion target value if their outputs serialize identically. The Falsify derivation block must contain the literal line '(f2) serialization gate: "
        "[assertion target] — targets: [behavioral property/state/outcome | string/byte/text representation] — gate: [pass | fail]'; a derivation block that does not contain this line "
        "does not satisfy (f2). The creation-step exception does not apply when the derivation block contains '(f2) serialization gate:' with 'gate: fail'; (g) the observed FAIL "
        "result: the content of an executor result block appearing in the transcript after the artifact was applied to the subject and before the first governed artifact-producing "
        "action for this behavior, where (a) precedes (c), the named executor produced the result, and the failure line contains (d) as a substring. A result satisfies this token if "
        "and only if: (g) is present at the required position; (a) is not separated from (c) in (g) by any line whose content contains (c) for a different governed behavior; and a "
        "subsequent executed-artifact result contains (b) at the position where (a) was absent. A result satisfying (g) must contain a failure line whose content includes a substring "
        "from the governed assertion call expression — where the call expression is a syntactic call in the test body whose callee name contains the governed symbol name as a "
        "substring; a failure line that contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) in its output; a valid (g) failure line is a line on "
        "which (a), (c), and a direct callee name containing (d) co-occur — an executor result that contains (a) before (c) but where no single line satisfies this co-occurrence does "
        "not satisfy the creation-step exception. Exception: (g) is not required for the single action that produces the governing artifact's assertion text, per governed behavior — "
        "the creation step is that action only; (g) is required for every governed artifact-producing action in the current session — a creation step is that action only if (c) is "
        "absent before the action and present after it, and a named executor invocation against the governing artifact immediately following the action produces a tool-executed result "
        "containing (c) and not (a); immediacy is defined as: no executor result block appears between that action and the named executor invocation — non-executor actions do not "
        "break immediacy. When (c) for a different behavior of the same governed symbol is already present in the transcript, the creation-step exception applies per governed behavior "
        "— the absent/present condition is evaluated for the specific (c) of the behavior being added, not for (c) of any other behavior; prior occurrences of (c) for other behaviors "
        "of the same governed symbol do not prevent the creation-step exception from applying to a new behavior. Artifact identifier must differ: the artifact identifier of the named "
        "governing artifact must differ from the artifact identifier of the artifact being modified. Semantically-equivalent refactor exception: (g) is also satisfied when an executor "
        "result block containing (a) before (c) appears in the transcript before the governed artifact-producing action, produced by a named executor invocation occurring in the "
        "current session after the last governed artifact-producing action, against a state where the refactored form was modified to violate the invariant the governing assertion "
        "checks — the perturbation result must appear before the revert action. Revert is complete when the pre-perturbation executor run result is quoted verbatim in the transcript "
        "before the revert executor invocation, and a subsequent executor invocation produces (b) at the position where (a) was absent. Per-behavior scoping: a governed "
        "artifact-producing action is compliant when (g) is satisfied for the behavior named in the Falsify derivation block's (d) entry — the behavior name is fixed at derivation "
        "time and may not be restated or narrowed after (g) is observed; the action is not required to implement any behavior for which (g) has not yet been established; unguarded "
        "behaviors require separate governing FAILs in subsequent steps and must not be treated as a blocking condition on the current action. Implementation scope constraint: a "
        "governed artifact-producing action may introduce only constructs whose complete identifier — the full name as it appears in the governed artifact, not any component or prefix "
        "— appears as a verbatim substring of (c), (d), or (g) in the Falsify derivation block active at the time of the action; a construct is introduced when its identifier appears "
        "in the governed artifact in any position — defined, declared, imported, or referenced — that was absent before the action; the governing artifact's content is not a valid "
        "source for the allow-list; the allow-list is derived exclusively from the executor-produced result entries (c), (d), and (g); the allow-list is derived from the full content "
        "of the (g) tool-result block named in the active Falsify derivation block; an identifier that does not appear as a verbatim substring of that tool-result block may not be "
        "introduced; test description lines, docstring echo lines, parametrize label lines, and summary lines in that block are valid allow-list sources — the restriction on these "
        "line types applies only to (c) derivation, not to allow-list sourcing; a construct whose introduction makes a reachable execution outcome possible that does not appear as a "
        "verbatim substring of (g) requires its own governing FAIL before it can be introduced, even if its identifier appears in (g) — the path constraint is satisfied only when "
        "every execution outcome the introduced construct can produce appears as a verbatim substring of the (g) tool-result block; during the creation-step exception, the allow-list "
        "is (d) only; a reference to an identifier not on the allow-list within a stub or partial implementation is not exempt; a construct whose complete identifier does not appear "
        "as a verbatim substring of the active allow-list entries must not be introduced in the same action and requires its own governing FAIL before it can be introduced. "
        "Scope-governing entries: only (c), (d), and allow-list identifiers from (g) are scope-governing entries — entries (a), (b), (e), (f), and (f2) are not scope-governing entries "
        "and do not contribute to the allow-list. Only identifiers that appear on a line in (g) that also contains (a) are valid allow-list entries — an identifier that appears in (g) "
        "only on lines that do not contain (a) is not a valid allow-list entry regardless of whether it appears in expected values, diff output, assertion messages, test descriptions, "
        "or source excerpts. Each line in (g) that contains (a) authorizes at most one new allow-list entry — the identifier that appears as a substring of that line and also "
        "satisfies (d)'s specificity constraint; if no such identifier appears on that line beyond (a) itself, that line authorizes no new allow-list entry. Imported-contract "
        "reference exception: an identifier that appears as a declared member name in the content of a Read tool-result block appearing before the current Pre-edit block in the "
        "transcript is an imported-contract identifier and is not subject to the allow-list source constraint for that action alone; this exception applies to the allow-list "
        "constraint only and does not exempt a branch construct whose condition or guard contains only imported-contract identifiers from the branch constraint. Artifact gap: when the "
        "Falsify derivation block contains the line '(g) observed FAIL result: unavailable' and no tool-result block in this transcript produced by a named executor invocation "
        "contains (c) as a substring, the derivation is incomplete. A derivation is complete only when a line beginning 'Gap artifact:' appears in the transcript immediately above the "
        "Falsify derivation block, naming the executor invocation designated to produce a result containing (a) before (c). When 'Gap artifact:' is present, the tool call that "
        "produces that artifact satisfies the creation-step exception; no file-modifying tool call may appear before the tool call that produces the named artifact. A governed symbol "
        "renamed by a refactor is a new governed symbol requiring a new derivation block before the rename action. If no named executor exists for the governed artifact type, this "
        ...
        "description, analysis, and planning tasks run §1–§5 in full after writing '§0 Path B: [scenario]'; the declaration opens the ladder, it does not close it. One example ladder "
        "(software domain — not prescriptive): intent → criteria → formal specification (invariants the artifact must satisfy, independent of implementation) → executable verification "
        "→ observed result → implementation; other domains derive their own ladder from the same principles. Rung-type homogeneity: rung types must stay homogeneous across the ladder — "
        "all rungs before the behavioral observation must be successive precision-narrowings of the same governed behavior; introducing a new kind of artifact, deliverable, preflight "
        "check, or task category as a rung is not a narrowing and does not satisfy the ladder. A ladder that begins with intent → criteria → specification and then inserts a preflight "
        "check, implementation step, or presenter-support deliverable as a rung has switched dimensions — those items belong outside the ladder as governed actions following it, not "
        "inside it as rungs. Intent cannot be read directly from a request — every governing goal must be derived, because the request describes what was asked for, not what would "
        "satisfy the underlying need. The response applies a meta-process discipline before any implementation action, deriving and enforcing its own correctness conditions as "
        "transcript-inspectable strings. A rung is satisfied when and only when a tool-executed event matching its definition appears in the transcript — inference, prediction, and "
        "prior knowledge do not satisfy rung gates regardless of accuracy. All compliance headings must be markdown '## ' headings — a line not beginning with '## ' does not satisfy "
        "any heading requirement. Invocation scope: a sentinel written in a prior ground invocation does not satisfy any rung requirement for the current invocation — a sentinel "
        "satisfies its rung only when it appears in the transcript after the most recent '=== TOKENS' section. Each rung derives exclusively from the immediately preceding rung's "
        "output; no rung has a fixed step count. A rung that references content from any rung earlier than its immediate predecessor does not satisfy this requirement — isolation is "
        "enforced at each step; backward reference to earlier rungs is not permitted even when the content appears in the transcript. When the execution environment supports literal "
        "isolation — a fresh invocation, a separate agent context, or a new session — prefer actual isolation over in-transcript constraint enforcement. Path classification (required "
        "before §0): if an artifact that, when invoked, executes the subject system and returns its live output — such as a shell command, running script, endpoint call, or test suite "
        "invocation — appears verbatim in the conversation above, apply Path A; otherwise apply Path B. Derivation phase: Path A — (0) invoke the named artifact as a tool call; the "
        "artifact qualifies for Path A only if, when invoked, it executes the subject system and returns its live output; an artifact that returns document content does not qualify, "
        "and a tool result consisting of a GitHub issue body, pull request body, specification document, README, or requirements text does not satisfy §0 regardless of invocation form; "
        "write '§0 observed' when a tool-result block reflecting live system execution is present above it — '§0 observed' must not appear before such a block is present; a tool-result "
        "block produced by invoking the named artifact as a tool call satisfies this requirement; a result produced by a Read, search, or file-write tool call does not; Goal-source "
        "classification (required after '§0 observed', Path A only): examine whether any line in the §0 tool-result block appears verbatim as a substring of any user message visible "
        "above §0 in the transcript. If no such line exists, write '§0 status-only'; a '## Governing goal:' line must not appear before a tool-result block whose content contains at "
        "least one line that appears as a substring of a user message above '§0 status-only' in the transcript — a tool-result block whose every line is a substring of content already "
        "present in the transcript above it does not satisfy this requirement. (1) derive the governing goal: '## Governing goal: [text]' must not appear before '§0 observed' in the "
        "transcript; `[text]` must appear verbatim as a substring of the §0 tool-result block above — a `[text]` value not present as a literal string in that block does not satisfy "
        "§1; immediately below the governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', "
        "labeled subject, verb, outcome, or invariant; no field may equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive words; when every named "
        "component of '[text]' is covered, write '§1a decomposed'; '§1a decomposed' must appear before the means-test line; immediately below '§1a decomposed', derive a means-test: "
        "write 'The goal [text] could be achieved by: [alternative means not listed in §0]' — if no concrete alternative can be named, write '[hypothetical]: [speculated alternative]' "
        "and fetch a broader context artifact before closing §1; if two non-subsuming goal candidates exist, write both and mark one '[selected]' — the selected goal is the one whose "
        "means-test names a more concrete alternative than those listed in §0; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal derived'; Path B — (0) write "
        "the literal string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — this declaration satisfies §0; (1) derive the governing goal: write '## "
        "Governing goal: [derived: text]' where '[derived: text]' is a goal derived from the scenario description — what would satisfy the underlying need, not merely what was asked; a "
        ...
        "before the means-test line; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]'; if only one concrete "
        "alternative can be named, write '[hypothetical]: [speculated alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or hypothetical) are present, write "
        "'§1 goal derived'; (Both paths continue from §2 onward:) immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in the means-test, "
        "write one candidate beginning with '- Candidate:' — a response property that would be absent in a response achieving '[text]' by a different means; when one candidate exists "
        "per alternative means, write '§1b candidates'; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the transcript; derive "
        "at least two dimensions; each dimension names a property of the response (not an artifact, state, or fix description) such that a response could satisfy it while failing at "
        "least one other listed dimension; each dimension must trace to one item from the §1a decomposition list or one item from the §1b candidates list — a dimension that traces only "
        "to '[text]' as a whole does not satisfy this requirement; each dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose "
        ...
        "'[observable:]' tag, write '§2 dimensions closed'; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the "
        ...
        "'§5 enumeration complete' appears exactly once; the string '§ implementation permitted [N]' where N is the 1-based index of the file-modifying tool call this sentinel governs "
        "must appear in the transcript after '§5 enumeration complete'; no file-modifying tool call may appear before the first '§ implementation permitted [N]' sentinel; the literal "
        "string '§ implementation permitted [N]' with N matching the 1-based index of the governed tool call must appear immediately before the `(i)` line of the five-line block in the "
        "assistant text block immediately before each file-modifying tool call, in the same response turn as that tool call — a user message appearing between '§ implementation "
        "permitted [N]' and the tool call does not satisfy this requirement; a file-modifying tool call not immediately preceded by assistant text whose final non-blank content ends "
        "with the `(iv)` line of the five-line block does not satisfy this requirement. Quoted-span exclusion: an occurrence of any named sentinel or heading string inside a markdown "
        "code fence (a span beginning with ``` or ~~~ on its own line) does not satisfy any rung or ordering requirement — only occurrences outside code fences count toward compliance. "
        "Continuation invariant: when the final non-blank content line of a turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions "
        "closed', '§4 coverage verified', or '§5 enumeration complete', that turn must end with one of: '§ blocked: <text>', '§ awaiting: <text>', or '§ no-next-action: <text>', where "
        "'<text>' is non-empty. A turn whose final non-blank content line contains or follows one of these strings and does not end with one of the three exit strings does not satisfy "
        "ground.",
