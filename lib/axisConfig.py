...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "concrete scenario in which property [N] is absent while every listed 'Property [N] assertion [M]:' nevertheless succeeds; a guard that is less durable than the property it "
        "governs is inherently a blind spot, because the same change that regresses the property also invalidates or removes the guard, so the guard is gone at the exact moment it "
        "would need to fire and cannot detect the regression — treat any guard whose continued ability to exercise property [N] depends on the property itself remaining unchanged as a "
        "found blind spot; emit 'Test blind-spot: found — <construction>' or 'Test blind-spot: not found'; if found, strengthen the guard and repeat from step (2); (4) the text "
        "immediately following 'Test blind-spot: not found' must be the tool call that executes the quoted guard against a deliberately constructed violating state — no narrative, "
        "planning, implementation change, 'Quoted implementation:', 'Implementation overreach:', or other sentinel may appear between 'Test blind-spot: not found' and that execution "
        "tool call; no transcript content may appear between the execution tool call and the corresponding tool-result block — the execution tool call and tool result are an atomic "
        "pair; execute the quoted guard against a deliberately constructed violating state — if the governed artifact already satisfies property [N], first construct a minimal "
