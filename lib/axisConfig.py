...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "voids the rung; if the criterion is already a complete behavioral specification — naming observable behavior and a falsifying condition without referencing function names, "
        "field names, or implementation internals — the formal notation artifact may restate it verbatim with an explicit declaration that no elaboration is needed; this is not a "
        "skipped rung; the artifact must appear in the transcript and the audit section still applies; the audit section is separate and named; the rung is incomplete until all "
        "criteria are encoded. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ Validation artifact V complete; each "
        ...
        "to the system — must be followed inline by a bracketed thread marker: [T: gap-name]. The prose rung must be re-emitted at the start of every cycle — the first cycle, any "
        ...
        "Execution observed: from the current cycle’s validation run observation rung — not from a prior cycle — to exist in output. Before emitting ✅ Manifest declared, scan every "
        "sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate "
        "requires a separate thread; re-read the prose rung output via a tool call to confirm every predicate is covered; ✅ Manifest declared may be emitted exactly once per "
        ...
        "state every open constraint the next rung must resolve; faithful derivation requires form to change, intent to stay fixed. Advance through every feasible rung; each rung may "
        "not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle. When beginning mid-ladder, locate the highest "
