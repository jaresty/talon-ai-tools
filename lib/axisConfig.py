...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "this rung — add a criterion asserting its content or effect. after the criterion is written, the only valid next token is the formal notation rung label. the criteria rung "
        "artifact is exactly one criterion for the current thread only — not one criterion per thread, not criteria for all threads collected under one label; exactly one criterion may "
        "be written per thread per cycle — a second criterion for the same thread requires a split: return to prose, re-emit for the new gap, emit ✅ Manifest declared with both gaps, "
        "then descend each thread separately; writing two criteria under one label is a protocol violation; the criteria rung label is per-thread — writing criteria for Thread 2 or any "
        "subsequent thread before ✅ Thread 1 complete is a protocol violation; batch-collecting criteria for multiple threads under one criteria label bypasses sequential descent and "
        "is a protocol violation; each thread’s criterion is only valid in the context of that thread’s descent — writing criteria for all threads before descending any treats the rung "
        "as a planning step, which it is not. The formal notation rung separates behavioral specification from explanation — formal notation encodes what must be true in a form that is "
        ...
        "re-emitting prose is a protocol violation regardless of whether the prior expectation was incorrect. before emitting ✅ Validation artifact V complete, the artifact content "
        "must appear in the current response; emitting the sentinel without artifact content is a protocol violation; if no P4 EV step (1) pre-existence or pre-failure check tool-call "
        "result exists in the transcript, ✅ Validation artifact V complete may not be emitted — asserting pre-existence without a tool call result does not satisfy this gate; the "
        "validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, the artifact is "
        "vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the test is "
        "vacuous and must be rewritten. Before writing the validation run observation rung label, invoke the validation artifact via a test runner; a harness error (import failure, "
        "syntax error, missing file) is not a red run — the test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating "
        "any run as a red witness. when VRO exec_observed is a harness error caused by a missing implementation file: 🔴 Gap: may not be emitted; the only valid next token is the "
        ...
        "component in the test itself): 🔴 Gap: may not be emitted; the only valid next token is a tool call that repairs the test file at the EV rung — not HARD STOP. when VRO "
        "exec_observed is a test-interaction failure — the tests loaded and ran but could not interact with the component under test (e.g., pointer-events: none, role not found, "
        "element not accessible) — 🔴 Gap: may not be emitted and 🟢 Implementation gate cleared is not valid; the only valid next token is a tool call that repairs the component's "
        "accessibility or the test's interaction strategy at the EV rung; this is a harness error class, not a behavioral failure. a green 🔴 Execution observed: is only valid as the "
        ...
        "enumerate multiple failures or plan fixes. re-running VRO without an implementation edit is a protocol violation — every re-run of the validation artifact must be preceded by "
        "an EI rung edit. after a red VRO that is not a harness error, after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token — this shortcut applies "
        "only when a red VRO already exists in the current cycle for the current criterion; a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO must fire before "
        "impl_gate; skipping EV and VRO for a new criterion introduced by HARD STOP is a protocol violation. a green exec_observed without a prior red run means the test is vacuous — "
        "return to the executable validation rung and rewrite the test to fail. a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed output. 🛑 HARD STOP "
        "— upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation run observation rung in the current cycle — "
        "emitting it at any other position is a protocol violation; prose-only exec_observed sentinel does not satisfy this gate — the exec_observed block must contain non-empty "
        "verbatim tool output in a triple-backtick block; HARD STOP may not be emitted at the executable validation rung — a harness error at EV requires fixing the harness, not an "
        "upward return; the only valid next action when EV shows a harness error is a tool call that fixes the harness; HARD STOP may not appear in the same response as a harness-error "
        "exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. HARD STOP "
        "does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread. the VRO rung "
        "label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. 🟢 Implementation "
        "gate cleared is the first token of the executable implementation rung — no tool call, no prose, and no file modification may appear before it in the current response; the next "
        "required action after 🟢 is a tool call that creates or modifies an implementation file; the implementation gate does not complete the thread — EI and OBR rungs must still fire "
        "before ✅ Thread N complete; ✅ Thread N complete may not appear until the observed running behavior rung has fired for this thread; the OBR rung label must appear in the "
        "transcript for the current thread's current cycle before ✅ Thread N complete may be emitted — its absence is a protocol violation, not a voiding condition; the void conditions "
        "for OBR handle the case where OBR fires with invalid output; this rule handles the case where OBR never fires at all. In-process rendering (renderToStaticMarkup, "
        ...
        "Execution observed: does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules. A full test suite "
        "run result is required after the most recent OBR tool call — the only valid next action if no such result exists is the tool call that runs the suite; every failing test must "
        "be explicitly acknowledged in the transcript with a written reason; acknowledging failures does not permit ✅ Thread N complete — the step-5 run must show zero failures for the "
        "criterion declared in this thread before ✅ Thread N complete may be emitted. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the "
        "criteria, formal notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared gap — what the prior rung’s "
        "artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. Outputting a rung label is what begins "
        "that rung — it is not a heading or annotation. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without consulting "
        "I; before closing a rung artifact, state every open constraint the next rung must resolve. Advance through every feasible rung without pausing for user confirmation — each "
        "rung may not be skipped or combined with another, including the observed running behavior rung; the only protocol-defined stop is at the validation run observation rung after "
        "emitting 🔴 Gap:; all other rung transitions are continuous within the same response. Response length is never a valid reason to stop between rungs — if the full descent cannot "
        "fit in one response, continue from the current rung in the next response without re-emitting completed rungs. The prose rung must be re-emitted at the start of every cycle — "
        "the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. When beginning "
