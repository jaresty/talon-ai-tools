...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "call): name the root criterion — the line immediately above the `(i)` escape category line must be the quoted scope commitment — a literal substring of the immediately "
        "preceding run result; a file-modifying tool call whose `(i)` line is not immediately preceded by such a quoted string does not satisfy this requirement; the quoted scope text "
        "must be a literal substring of the immediately preceding run result; exactly one symbol the call will add or modify must be named above that call (symbol commitment) — a "
        "symbol commitment naming more than one symbol does not satisfy this requirement; the symbol commitment must appear as a line matching `symbol: <identifier>` where identifier "
        "matches `[A-Za-z_][A-Za-z0-9_]*` (no spaces); the named symbol must appear as a literal substring of the quoted scope text or of the immediately preceding run result — a "
        "symbol not present in either does not satisfy this requirement; a symbol is a named member within a container — it names an entity whose existence is established within the "
        "container, not the container itself; a commitment naming the container (the file, module, or resource that would be created or modified as a whole) does not satisfy this "
        "requirement; when the symbol name does not appear as a substring of the immediately preceding run result, the symbol is new; a new-symbol call is compliant only if the "
        "introduced symbol satisfies the stub requirement for its kind — for a callable (function, method, or constructor), the body contains no logic: an empty return or a "
        "language-idiomatic zero-value return for the declared return type; for a non-callable (variable, constant, exported value, or class field), the initializer is a single "
        "primitive or zero-value expression with no sub-expressions — body/initializer contains only one of nil, null, undefined, 0, 0.0, false, empty string literal, or empty return, "
        "and contains no `(`, `[`, `{`, or operator character; a body or initializer not matching its kind's stub form does not satisfy this requirement; a statement in the derivation "
        "phase is a commitment only if its truth value can be determined from run results already present above it in the transcript — a statement whose truth value requires evaluating "
        "a result not yet present is not a commitment; after the call, a run result must appear in the transcript in which the quoted scope text is absent and every other line from the "
        "immediately preceding pre-edit run result is present — the pre-edit run result for a given call is the run result that produced that call's scope commitment. Each "
        "file-modifying tool call's scope line must satisfy these requirements independently — a scope line from a prior call does not carry forward. Then, immediately before each "
        "file-modifying tool call — as the last assistant text block before that call — write a block containing exactly these five lines in order: `(i) scope inflation: <string>`, "
        "`(ii) stale quote: <string>`, `(iii) symbol cardinality: <string>`, `(iv) post-call line mismatch: <string>`, `(v) implementation depth: <string>`. The five lines must appear "
        "consecutively with no intervening lines. A content line is any non-blank line. A file-modifying tool call not immediately preceded by a block whose last five content lines "
        "begin with `(i)`, `(ii)`, `(iii)`, `(iv)`, `(v)` in that order does not satisfy this requirement. Proceed to file-modifying tool calls only when the derivation and all five "
