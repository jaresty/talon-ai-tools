...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "works.' This is domain-independent — it applies to any task, not just software. P1 (Intent primacy): intent is an abstract goal outside the system; all artifacts derive from "
        "it; form changes, intent does not; without intent, no way to evaluate artifacts. P2 (One artifact per type): each rung produces exactly one artifact type; dedicated rung for "
        "each type. P3 (Observable evidence required): every claim requires externalized evidence; pre/post states must be visible. P4 (Derivation chain): artifacts derive from prior "
        "rung's actual content, not memory; each artifact must cite its source. P5 (Gap-driven iteration): gaps are exposed before claiming completion; refinement addresses specific "
        "gaps. Protocol derivation: before work begins, derive the protocol rules by applying P1-P5 — observe at every feasible rung before proceeding. Protocol derivation checklist: "
        "AFTER deriving rules from P1-P5, BEFORE proceeding, MAKE VISIBLE in the conversation: [ ] The invariant: claim vs reality — what GOES WRONG if you claim without evidence: fool "
        "yourself, completion is false; RULE: evidence before claims [ ] The ladder structure: 7 steps — Intent → Criteria → Structure → Baseline → Challenge → Refinement → "
        "Verification; what GOES WRONG if you skip: gaps lost, completion false; RULE: one rung at a time [ ] The derivation rule: each artifact cites its source from previous rung; "
        "what GOES WRONG if you don't cite: chain breaks, unverifiable; RULE: cite source at each step [ ] Evidence authorities: domain determines what counts as evidence — software "
        "uses automated tests, writing uses peer review, decisions use stakeholder feedback, learning uses assessment; what GOES WRONG if wrong authority: evidence invalid; RULE: use "
        "appropriate authority for domain [ ] Verification: confirmation that no new gaps remain; what GOES WRONG if you skip: hidden gaps persist; RULE: explicit verification before "
        "completion [ ] Gap cycle: derive from P1,P3,P5 — one gap at a time ensures clear signal; what GOES WRONG if you batch: unclear what changed; RULE: challenge → refine → verify "
        "→ next gap [ ] Three-part completeness: derive from P2,P3 — baseline demonstrates approach, challenge exposes gaps, refinement addresses them; what GOES WRONG if you skip "
        "challenge: gaps invisible; RULE: always challenge before claiming done [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG "
        "if you create without structure: unfocused output; RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success conditions before organization; what "
        ...
        "solution organized? Derivation: from criteria, derive organization. Evidence: internal consistency. 4. Baseline — Does a minimal version exist? Derivation: from structure, "
