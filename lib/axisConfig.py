...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "and has been verified; the cycle continues until every behavior the task requires the system to produce has a corresponding verified assertion — this is a hard termination "
        "condition, not a probabilistic goal; scope is determined by the task's stated requirements, not by what is convenient to assert; excluding a required behavior from assertion "
        "coverage is a unilateral change to the task's intent. Second, one assertion per behavior: an assertion covering multiple behaviors cannot be verified in isolation; each "
        "assertion must cover exactly one observable outcome — a governing artifact that checks multiple outcomes in a single verification step covers multiple behaviors and is a "
        "structural violation of this requirement; the unit of assertion is a single check that fails independently when its one target behavior is absent; more assertions than behaviors "
        "is acceptable, any behavior without its own independently-failing assertion is a structural violation. Before writing the assertion, verify it is a minimal, direct observable "
        "for exactly the intended behavior: the test must pass only when the specific behavior exists, not when any adjacent implementation satisfies it by coincidence — an assertion "
        ...
        "test does not substitute for same-layer coverage — both must exist.",
        ...
        "mu": "The response produces one or more objects — questions, scenarios, or imperatives — that enact the central irresolution of the subject rather than describing it. Each object must "
        "trap the reader inside the tension: a question whose answer requires abandoning the question's own frame, a scenario where every move is structurally wrong, or an imperative that "
        "collapses the distinction it invokes. The irresolution must arise from the subject's own internal structure, not imported from general philosophy or common discourse about the "
        "subject's domain. A tension is valid only if removing the subject would make it disappear. Do not name the tension. Do not state that something cannot be resolved. The object "
        "succeeds only if the reader cannot answer it without first becoming stuck.",
