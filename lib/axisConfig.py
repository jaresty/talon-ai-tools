...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "no two fields may share a common phrase of 3 or more consecutive words; when every named component of '[text]' is covered, write '§1a decomposed'; '§1a decomposed' must appear "
        "before the means-test line; '§1 goal derived' must not appear before '§1a decomposed' in the transcript; immediately below '§1a decomposed', derive a means-test: write 'The "
        "goal [text] could be achieved by: [alternative means]'; if only one concrete alternative can be named, write '[hypothetical]: [speculated alternative]' before writing '§1 goal "
        "derived'; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal derived'; (Both paths continue from §2 onward:) immediately below '§1 goal derived', "
        ...
        "in a response achieving '[text]' by a different means; when one candidate exists per alternative means, write '§1b candidates'; (2) derive behavioral dimensions: '## "
        ...
        "satisfaction independently of any other listed dimension; when every dimension carries an '[observable:]' tag, write '§2 dimensions closed'; (2b) derive formalization: '## "
        ...
        "satisfier:' line have appeared; '## Enforcement sequence' must not appear before '§ formalization complete' in the transcript; (3) derive the enforcement sequence: '## "
        "Enforcement sequence' must not appear before '§2 dimensions closed' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ "
        "implementation permitted' appears; each sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; (4) verify "
        "coverage: '## Completion check' must not appear before '## Enforcement sequence' in the transcript and must not appear before '§ implementation permitted' in the transcript; "
        "each covered dimension cites a verbatim string that appears either as a substring of a tool-result block produced after '§ implementation permitted' and visible above '## "
        "Completion check', or, when no tool-result blocks are present after '§ implementation permitted', as a substring of the response text produced in this turn; when every covered "
        "dimension cites such a substring, write '§4 coverage verified'; '## Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a dimension named "
        "for the first time inside '## Completion check' does not satisfy any coverage requirement and does not extend the governed dimension set; a turn containing a satisfied 'Gate "
        ...
        "'§4 coverage verified' does not satisfy this requirement and the governed turn must continue to the next derived step rather than ending — the first non-blank line after the "
        "'## Completion check' heading must be 'Gate condition:'; a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume phrase requirement; when "
        'a turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates still apply" to '
        ...
        "closes it; write '§5 enumeration complete' when no further open path remains; any referent named in the '[text]' from §1 not already present as a literal string in the "
        ...
        "this requirement only when it is the final non-blank line before the tool call within that block; the permit sentinel must be immediately followed by the `(i)` line of the "
        "five-line block with no intervening blank lines or other content between them — a blank line, horizontal rule, or any other content between them does not satisfy this "
        "requirement; a file-modifying tool call not immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the five-line block does not "
        "satisfy this requirement. Quoted-span exclusion: only an occurrence of a named sentinel or heading string on a line that is not inside a markdown code fence (a span beginning "
        "with ``` or ~~~ on its own line) and does not begin with '>' (a markdown block-quote prefix) counts toward compliance — an occurrence inside a code fence or on a line "
        "beginning with '>' does not satisfy any rung or ordering requirement. Rung-completion sentinel finality: a turn is defined as all content within a single assistant message "
        "block; a line is non-blank if it contains at least one non-whitespace character; each of the strings '§0 observed', '§1 goal derived', '§1a decomposed', '§1b candidates', '§2 "
        "dimensions closed', '§ formalization complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant message block in "
        "which it is written — a checker verifies this by locating the sentinel line, scanning all subsequent lines in the same assistant message block, and confirming every such line "
        "is blank (contains only whitespace or is empty); any non-blank line after the sentinel in the same assistant message block is a violation; no exceptions are permitted — if "
        "additional content must appear, it must precede the sentinel, not follow it. Continuation invariant: when the final non-blank content line of a turn contains or follows one of "
