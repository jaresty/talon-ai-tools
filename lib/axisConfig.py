...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "not satisfy this requirement; (d) the governed symbol: the literal string naming the governed behavior — (d) must be derived from the direct callee of the assertion call "
        "expression in the governing artifact, where the call expression is not used only as an argument to a call expression whose callee does not contain the governed symbol name; "
        "(d) is the literal string which must appear as a substring of the failure line in (g); (e) the layer classification — if the governed symbol name appears as a token in a call "
        "expression in the test body — where the call expression is not nested inside another call expression that names a different symbol — the layer is 'unit'; if the governed "
        "symbol name does not appear in the test body, the layer is 'integration'; (f) the executor class — the executor must produce output that could only exist if the governed "
        "assertion reached execution at the layer named in (e); a governing artifact is disposable if removing the governed symbol from the governed artifact — without modifying the "
        "named governing artifact — produces an executor result in which (c) does not appear; a governing artifact is non-disposable only when (c) appears in the executor result after "
        "removal — a disposable artifact does not satisfy (f); (f2) the serialization gate — classify the assertion target: the value produced by the expression that is the direct "
        "argument to the assertion call. A governing assertion satisfies (f2) if and only if its assertion target is a behavioral value: a value whose content is determined by whether "
        "the governed behavior is correctly implemented and that could not be produced by an arbitrary implementation that merely serializes the governed artifact's output correctly. "
        "A governing assertion fails (f2) if its assertion target is a serialization value: a value produced by converting the governed artifact's output to a string/byte/text "
        "representation — regardless of what the conversion expression is named — such that a correct implementation and an incorrect implementation would produce the same assertion "
        "target value if their outputs serialize identically. The Falsify derivation block must contain the literal line '(f2) serialization gate: [assertion target] — targets: "
        "[behavioral property/state/outcome | string/byte/text representation] — gate: [pass | fail]'; a derivation block that does not contain this line does not satisfy (f2). The "
        "creation-step exception does not apply when the derivation block contains '(f2) serialization gate:' with 'gate: fail'; (g) the observed FAIL result: the content of an "
        "executor result block appearing in the transcript after the artifact was applied to the subject and before the first governed artifact-producing action for this behavior, "
        "where (a) precedes (c), the named executor produced the result, and the failure line contains (d) as a substring. A result satisfies this token if and only if: (g) is present "
        "at the required position; (a) is not separated from (c) in (g) by any line whose content contains (c) for a different governed behavior; and a subsequent executed-artifact "
        "result contains (b) at the position where (a) was absent. A result satisfying (g) must contain a failure line whose content includes a substring from the governed assertion "
        "call expression — where the call expression is a syntactic call in the test body whose callee name contains the governed symbol name as a substring; a failure line that "
        "contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) in its output; a valid (g) failure line is a line on which (a), (c), and a direct "
        "callee name containing (d) co-occur — an executor result that contains (a) before (c) but where no single line satisfies this co-occurrence does not satisfy the creation-step "
        "exception. Exception: (g) is not required for the single action that produces the governing artifact's assertion text, per governed behavior — the creation step is that "
        "action only; (g) is required for every governed artifact-producing action in the current session — a creation step is that action only if (c) is absent before the action and "
        "present after it, and a named executor invocation against the governing artifact immediately following the action produces a tool-executed result containing (c) and not (a); "
        "immediacy is defined as: no executor result block appears between that action and the named executor invocation — non-executor actions do not break immediacy. Artifact "
        ...
        "(a), (b), (e), (f), and (f2) are not scope-governing entries and do not contribute to the allow-list. Artifact gap: when the Falsify derivation block contains the line '(g) "
        "observed FAIL result: unavailable' and no tool-result block in this transcript produced by a named executor invocation contains (c) as a substring, the derivation is "
        "incomplete. A derivation is complete only when a line beginning 'Gap artifact:' appears in the transcript immediately above the Falsify derivation block, naming the executor "
        "invocation designated to produce a result containing (a) before (c). When 'Gap artifact:' is present, the tool call that produces that artifact satisfies the creation-step "
        "exception; no file-modifying tool call may appear before the tool call that produces the named artifact. A governed symbol renamed by a refactor is a new governed symbol "
        "requiring a new derivation block before the rename action. If no named executor exists for the governed artifact type, this token does not apply.",
