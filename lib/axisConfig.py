...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "this documents the intent without writing assertions; no expect() assertions, no mock setup, no render; stub implementation files return null; tests are EMPTY and pass "
        "trivially because they have no code; this rung establishes only that tests can be imported and run without errors. Assertions are written at the assertion rung, not "
        "validation. Tests MUST PASS at validation rung to move to assertion rung. Assertion rung: write ONE specific assertion using expect(), run tests to see it FAIL (red). NO "
        "implementation in assertion rung — only write assertion, run tests, observe failure. Then move to implementation rung. Implementation rung: AFTER assertion rung shows red, "
        "implement ONLY enough to make that specific assertion green — take the perspective of an adversarial implementer; implement what the test demands, nothing more; tests drive "
        "the behavior needed, not your assumptions about what the feature should do. Emit implementation rung label first, then implement, then emit completion sentinel. ASSERTIONS are "
        "test code that verifies behavior, NOT import errors or infrastructure failures. Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ "
        "[Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; Every rung must emit its label BEFORE producing "
        "its artifact and emit its completion sentinel AFTER. No implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is "
