...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ambiguity test:' and a valid 'alternative satisfier:' line have appeared; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§ "
        "formalization complete' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each "
        "sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; After '§ formalization complete', immediately write "
        ...
        "'**property' or any markdown-formatted variant does not satisfy this requirement; each 'property [N]:' line must contain at least one formal expression — a logical predicate "
        "(∀/∃ notation), a typed function signature, or a concrete input/output example (f(v)=w or equivalent) — as a literal substring; a 'property [N]:' line whose content is prose "
        "only, with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} present as a substring, does not satisfy this requirement; for each 'property [N]:' line, write one or "
        "more atomic sub-property lines of the form 'property [Na]:', 'property [Nb]:', etc., each containing a formal expression — a sub-property line whose content is prose only does "
        "not satisfy this requirement; if the property is already atomic (no strict sub-expression exists that is independently falsifiable), write 'property [Na]: atomic — ' followed "
        "by a formal restatement of the same expression; when all 'property [N]:' and sub-property lines are written, write '§ properties check: \"[quoted phrase]\"' where [quoted "
        "phrase] is a verbatim substring of one of the 'property [N]:' lines written above — a '§ properties check:' line whose quoted phrase does not appear verbatim in a 'property "
        "[N]:' line above it does not satisfy this requirement; then write '§ properties complete'; '§ properties complete' is valid only after a valid '§ properties check:' line has "
        "appeared; (4) verify coverage: '## Completion check' must appear as the next heading after '§5 enumeration complete' in the transcript — the first '## ' heading line appearing "
        ...
        "appear before '## Enforcement sequence' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions "
        ...
        "check:' line in the transcript; any referent named in the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched before §1 is "
        "written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted' followed by a space and a decimal integer (e.g. '§ implementation "
        "permitted 1') must appear in the transcript after '§5 enumeration complete'; a sentinel line containing the literal bracket characters '[' or ']' does not satisfy this "
        "requirement — only a decimal integer satisfies the index requirement; the integer must equal the 1-based ordinal count of file-modifying tool calls that will have appeared "
        "through and including the tool call immediately following this sentinel — '§ implementation permitted 1' must precede the first file-modifying tool call, '§ implementation "
        "permitted 2' must precede the second, and so on; a checker verifies this by scanning the transcript for every file-modifying tool call, locating the final non-blank line of "
        "the immediately preceding assistant message block, and confirming it matches '§ implementation permitted N' where N is the call's ordinal position; no file-modifying tool call "
        "may appear before the first such sentinel; the permit sentinel must appear as the final non-blank line of the assistant message block immediately before the assistant message "
        "block containing the file-modifying tool call — a permit sentinel in the same message block as the tool call satisfies this requirement only when it is the final non-blank "
        "line before the tool call within that block; § implementation permitted [N] must appear immediately before the `(i)` line of each file-modifying tool call — no intervening "
        "blank lines, prose, tool-call blocks, or other content may appear between the sentinel and the `(i)` line; no user message appearing between § implementation permitted [N] and "
        "the governed tool call satisfies this requirement — a sentinel written in one turn and a tool call appearing in a later turn after a user message does not satisfy the "
        "same-turn anchor; a blank line, horizontal rule, or any other content between them does not satisfy this requirement; a file-modifying tool call not immediately preceded by "
        "assistant text whose final non-blank content ends with the `(iv)` line of the five-line block does not satisfy this requirement. Quoted-span exclusion: only an occurrence of a "
        "named sentinel or heading string on a line that is not inside a markdown code fence (a span beginning with ``` or ~~~ on its own line) and does not begin with '>' (a markdown "
        "block-quote prefix) counts toward compliance — an occurrence inside a code fence or on a line beginning with '>' does not satisfy any rung or ordering requirement. "
        "Rung-completion sentinel finality: a turn is defined as all content within a single assistant message block; a line is non-blank if it contains at least one non-whitespace "
        "character; each of the strings '§0 observed', '§1 goal derived', '§1a decomposed', '§1b candidates', '§2 dimensions closed', '§ formalization complete', '§ properties "
        "complete', '§ test suite complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant message block in which it is "
        "written — the sentinel line must begin with the literal first character of the sentinel string ('§' or '#'); a sentinel line beginning with '**' or any other markdown "
        "formatting character does not satisfy this requirement; a checker verifies this by locating the sentinel line, scanning all subsequent lines in the same assistant message "
        "block, and confirming every such line is blank (contains only whitespace or is empty); any non-blank line after the sentinel in the same assistant message block is a "
        "violation; no exceptions are permitted — if additional content must appear, it must precede the sentinel, not follow it. Completion check ordering: the '## Completion check' "
        "block must appear after '§5 enumeration complete' in the transcript; a '## Completion check' block that appears before '§5 enumeration complete' does not satisfy this "
        "requirement even if its contents are otherwise correct; a response that contains '§5 enumeration complete' and does not produce a subsequent '## Completion check' block "
        "containing 'Formalized properties:', at least one 'property [N]:' line satisfying all property requirements, '§ properties complete', and '§4 coverage verified' does not "
