...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "asserting its content or effect; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a "
        "criterion written without a citable manifest excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier "
        "implementation is a protocol violation; each criterion states (a) one observable behavior and (b) a falsifying condition referencing no implementation internals (no file "
        "names, function names, or variable names); this is a content gate, not a self-check; after the criterion is written, the only valid next token is the formal notation rung "
