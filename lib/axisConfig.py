...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "skip: hidden gaps persist; RULE: explicit verification before completion [ ] Challenge: derive from P3,P5 — why Challenge MUST use SAME evaluation framework from Baseline (not "
        "create new structures); what GOES WRONG if you create new evaluation: scope expansion, not valid Challenge; RULE: Challenge = extend existing evaluation with NEW criteria, not "
        "create new evaluation structures [ ] Baseline: derive from P2,P3 — why Baseline MUST work (not just exist); what GOES WRONG if you rationalize failure: broken framework "
        "confused with gaps; RULE: Baseline = fix whatever makes evaluation fail, do not proceed until it passes [ ] Baseline scaffolding: derive from P2,P3 — why Baseline scaffolding "
        ...
        "only - NOT the full solution. Full solution comes at Refinement. Baseline scaffolding is just enough for evaluation to run. IMPORTANT: The scaffolding at Baseline should be "
        "PLACEHOLDER-LEVEL - has the structure but not the functionality. Think empty shell, not working feature. IMPORTANT: Baseline establishes evaluation framework at Baseline. "
        ...
        "not create new ones. IMPORTANT: Baseline evaluation must be SELF-CONTAINED. If it FAILS for ANY reason, that's a broken framework, not evidence of gaps. Fix the framework to "
        "make it work - do not rationalize why failure is acceptable. Only NEW criteria added at Challenge can expose gaps in the solution. 5. Challenge — What evidence proves gaps "
        "exist? Derivation: from Baseline, add NEW evaluation criteria that FAIL against the solution. Evidence: failing output from NEW criteria proves gaps exist, NOT failing because "
        "framework is broken. IMPORTANT: Challenge must use SAME evaluation framework from Baseline - creating new evaluation at Challenge is scope expansion, not valid Challenge. "
        "IMPORTANT: Work on the smallest verifiable thing at a time — each check must be satisfied before the next. 6. Refinement — How is the artifact improved? Derivation: from "
        "Challenge, address the SPECIFIC gaps that were OBSERVED. Evidence: changes target the exact gaps proven to exist. IMPORTANT: Refinement must address OBSERVED gaps only - gaps "
        "seen in Challenge output. Addressing unobserved gaps wastes effort and may miss real issues. 7. Verification — How is completion confirmed? Derivation: from refinement, derive "
        "confirmation. Evidence: no new gaps exposed. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to finish each rung. Evidence before claims: "
        "for every claim about completion, there must be evidence proving it. Ground complete may only be emitted after verification confirms no gaps remain. Domain adaptation: for "
        "tasks where verification is possible, define evaluation scaffold BEFORE producing solution — evaluation defined after solution is vacuous (Texas sharpshooter). Baseline "
        "defines evaluation framework; Challenge applies it to show OBSERVED gaps; Refinement produces solution that addresses those OBSERVED gaps.for writing tasks, it maps to "
