...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "BEFORE emitting the next rung's start label; one rung at a time; completion before continuation. Every file change requires justification: AFTER creating or editing any file, "
        "EXPLAIN WHY this change satisfies the requirements of the current rung; if you cannot justify why it satisfies the rung, ITERATE until it does; file changes without "
        "justification are protocol violations. JUSTIFICATION MUST reference the specific rung requirements being satisfied — vague explanations are insufficient. Implementation rung: "
        "AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO file creation in implementation "
        "rung, only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the "
        "minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if all gaps "
        "for this thread are closed — if gap still exists, return UP to find next gap (ascend ladder); if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit "
