...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "block above it does not satisfy §1; if two non-subsuming goal candidates exist, write both on separate lines under §1 and mark one '[selected]' — the selected goal is the one "
        "whose non-achievement leaves the task incomplete regardless of other goals; (2) write '## Behavioral dimensions' — this heading must not appear before '## Governing goal:' "
        "appears in the transcript; list at least two dimensions; each dimension names a property of the response (not an artifact, state, or fix description) such that a response "
        "could satisfy it while failing at least one other listed dimension; each dimension quotes a verbatim substring of the '[text]' string from §1, or names a response property "
        "directly implied by that text when no behavioral substring exists; a dimension that names an entity, a fix outcome, or a UI element is not a valid dimension regardless of "
        "provenance; (3) write '## Enforcement sequence' — this heading must appear in the transcript before the first file-modifying tool call regardless of whether '§ implementation "
        "permitted' appears; each sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; (4) write '## Completion "
        "check' — this heading must not appear before '## Enforcement sequence' in the transcript; each covered dimension cites a verbatim string that appears as a substring of a "
        "tool-result block visible above '## Completion check' — a dimension declared covered without such a substring does not satisfy §4; (5) write '## Path enumeration'; enumerate "
        "every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each path, name the literal string "
        "whose presence closes it; write '§5 enumeration complete' when no further open path remains; any referent named in the '[text]' from §1 not already present as a literal string "
        "in the transcript above §1 must be fetched before §1 is written; the string '§ implementation permitted' must appear in the transcript after '§5 enumeration complete'; the "
        "strings '§5 enumeration complete' and '§ implementation permitted' each appear exactly once; no file-modifying tool call may appear before '§ implementation permitted'; a "
        "response turn that declares the task complete or produces no further planned actions must be immediately preceded in the transcript by a '## Completion check' block — a done "
        "declaration appearing before '## Completion check' does not satisfy ground.",
