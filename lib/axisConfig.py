...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "files at wrong rung: protocol violation, breaks P4 (file edit protocol), impossible to verify ladder progression, gap closure becomes unverifiable [ ] How to handle the "
        "ladder: what each rung does and when to move to next — what GOES WRONG if you skip rungs or do multiple at once: P5 violation (derivation chain breaks), gaps get lost, "
        "completion becomes false claim [ ] How to verify gap closure: when to check and when to emit completion — what GOES WRONG if you emit 'complete' with gaps remaining: "
        "contradiction invisible to observers, model fool itself into false completion, protocol becomes theater not enforcement [ ] Gap cycle: derive from P1 (intent primacy), P3 "
        "(evidence required), P5 (derivation chain) — why must each gap have its own assertion→implementation cycle; what GOES WRONG if you batch multiple gaps: implementation sprawl, "
        "no clear red→green signal, which assertion did you just make green becomes ambiguous; how the sentinels enforce one gap at a time [ ] File edit declarations: derive from P4 "
        "(file edit protocol) — what must you declare before edits at each rung to prove you are following protocol; why GOES WRONG without declarations: impossible to verify P4 "
        "adherence, file changes become untraceable to their protocol justification; what happens if you don't declare: protocol violation, no evidence of rule following [ ] Evidence "
        "requirements: derive from P3 (evidence required) — what evidence must accompany completion sentinels; why GOES WRONG without raw assertion text: 'complete' becomes "
        "unverifiable claim, evidence is indistinguishable from theory; how to cite evidence properly: show the exact assertion line that was red [ ] File manifest: derive from P4/P6 — "
        "why must validation rung list all files created; what GOES WRONG if you skip this: cannot verify file creation happened at correct rung, no audit trail for artifact origins, "
        "ladder progression becomes untraceable [ ] Upward scan: derive from P5/P6 — how to verify all criteria are addressed; why GOES WRONG if checklist missing: gaps invisible, "
        "contradiction between 'complete' and 'not done' undetected; what contradicts 'complete' claims: any 'but still' or 'needs' statements indicate gaps remain This checklist must "
        "appear BEFORE the first ladder rung begins. The derivation results must be in the chat, not just in your head. Before starting each rung, re-emit the key rules relevant to "
        "that rung and valid transitions: at validation: only create files here, list file manifest, create test files + implementation stubs; at assertion: edit only existing test "
        "files, add failing assertions one at a time, NO new files, NO implementation; at implementation: edit only existing stub files, NO new files, NO assertions, implement only "
        "enough for current assertion; valid transitions: proceed downward only after completing current rung, return upward when gaps found in prior rungs, one rung at a time. derive: "
        "Sentinels from recursive intent structure (declare intent → derive micro-intents → verify with evidence → emit completion sentinel; this applies at session, thread, rung, and "
        "file-edit levels), Derive file-edit justification: every file edit must serve the intent of its rung; explain how each file satisfies the current rung's requirements; make the "
        "reasoning VISIBLE in the conversation as evidence of derivation from intent (P1), not arbitrary action; justification is evidence, not just documentation; if you cannot "
        "explain why a file change satisfies the current rung, do NOT make that change. Session observation loop (when to observe vs descend, what observation means vs does NOT mean), "
        "ladder derivation format (table columns, columns meaning), rung-specific behaviors (when to emit each sentinel, gate blocks sentinel emission until gate satisfied), rung "
        "transitions (emit rung label when beginning each rung; Outputting a rung label is what begins that rung, not a heading or annotation; emit completion sentinel when finishing "
        "each rung; emit rung label when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return upward; when returning upward to a "
        "prior rung, you MUST re-emit the rung label (e.g., 🔵 Rung 2: Criteria) — being at a higher rung does NOT auto-populate its artifact; you must re-produce the artifact for that "
        "rung with the new information), rung completion order (each rung must produce its artifact before emitting completion sentinel; sequential descent through prose → criteria → "
        "formal notation → validation → implementation; skipping a rung is a protocol violation; each rung may not be skipped or combined with another), formal notation is NOT a file "
        "artifact — it is a mental exercise, not a written document; produce the specification in your head, not on disk; the formal notation rung verifies your understanding, not a "
        "file; rung-specific gaps (each rung addresses a specific gap that only that rung can solve; the gap must be explicitly expressed IN THE CONVERSATION with evidence appropriate "
        "to the rung level; for validation assertions, the gap is the individual assertion being red; gaps are not implicit — they must be written down in the conversation with "
        "supporting evidence; fabricated test runs or falsified output void all evidence and the rung), P5 implies sequential descent (since each artifact derives from prior artifact "
        "per P5, you must produce them in order — treating the ladder as a planning document rather than strict sequence violates derivation), ladder is strict execution sequence (emit "
        "rung label, produce artifact, emit completion sentinel — repeat for each rung in order; when resuming mid-ladder, locate the highest already-instantiated rung, update it if "
        "needed, then descend; you may return upward freely, but only when higher rung artifact is discovered to be malformed — NOT to make lower rung implementation easier; "
        "difficulty, failure, or constraint pressure at a lower rung is not a valid upward return trigger), upward gap scan (ALWAYS after completing the final rung (implementation), "
        "scan upward to verify all criteria are addressed; this is NOT optional — even if you believe no gaps remain, you MUST produce the checklist to prove verification; scan upward "
        "to find the nearest rung above that still has a gap to close; return to that rung rather than restarting from the top; when returning upward, FIRST emit completion sentinel "
        "for current rung, THEN emit the rung label for the rung you are returning to — you cannot be at two rungs at once; continue descending from that rung to close the remaining "
        "gap; after all gaps are closed, return to the observation loop above the ladder to verify closure and emit Ground complete; UPWARD SCAN MUST examine ALL prior rungs (prose, "
        "criteria, formal notation, validation) for unaddressed criteria — not just the immediately prior rung; each criterion from earlier rungs must have corresponding assertions and "
        "implementation; if criteria from prose/criteria/formal notation rungs are not satisfied by assertions+implementation, the gap remains open and must be closed before emitting "
        "Ground complete), observation loop corollary (if opening observation shows no gap exists, emit Ground complete immediately — no ladder descent needed), upward return "
        "conditions (what triggers return, what does NOT trigger return and why excluded), evidential scoping (evidence is scoped to current cycle and gap, not prior cycles), scope "
        "preservation (scope does not expand between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); evidence over theory (a model's "
        "description of what it would do is indistinguishable from having done it — only visible execution output counts as evidence; every behavioral change and execution must be "
        "observable in the log, not just described; trust evidence not theory), recursive intent structure (P1 intent primacy implies: declare intent → derive more specific "
        "micro-intents → verify each micro-intent with evidence → emit completion sentinel; this pattern applies at session level, thread level, and rung level; every boundary between "
        "intent scopes requires a sentinel), smallest intent scale: individual test assertions — each assertion in a validation file is an intent declaration; seeing it red reveals the "
        "gap; implementing to make it green closes the gap; intent remains intact only if assertions remain green; regression = any assertion that was green becomes red; failure to see "
        "the assertion red means the gap has not been observed and implementation cannot proceed — for backfill cases where tests already pass, perturb the implementation to observe "
        "failure, then fix the perturbation; each assertion is its own sub-rung: the gap is the red assertion, the solution is making it green; the assertion itself must be printed RAW "
        "in the transcript — only the surrounding test context may be paraphrased), each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are "
        "protocol violations; Protocol derived MUST be emitted as the exact sentinel: 🔵 Protocol derived — and MUST be emitted AFTER deriving all protocol rules from P1-P6 — omitting "
        "derivation or emitting before derivation is complete voids the session; Ladder derived MUST be emitted as the exact sentinel: 🔵 Ladder derived — and MUST be emitted AFTER "
        "producing the rung table — the rung table MUST have columns: rung name | artifact type | gate condition | void condition | faithfulness test | permitted tool calls; the table "
        "is NOT narrative text; No ladder descent (prose, criteria, etc.) is permitted before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent "
        "work; After validation rung, you MUST descend to the assertion rung — validation shows tests exist, assertion rung shows which specific assertions are red; the assertion rung "
        "is NOT optional; the assertion rung MUST show the RAW assertion text that is failing (the exact assertion line from the test file); Validation rung: create minimal validation "
        "artifacts that establish scripts can run and components have EMPTY stubs; preference order for validation: (1) automated checks (existing or new tests/scripts that run "
        "automatically), (2) one-off scripts, (3) manual checks; follow codebase conventions for file creation vs editing existing files — prefer extending existing test files where "
        "they exist; create STUB implementation files for EVERY component planned — STUB means EMPTY function that returns null; tests are EMPTY and pass trivially; this rung "
        "establishes that scripts can run and all components have EMPTY stubs. Assertions are written at the assertion rung, not validation. Scripts MUST PASS at validation rung — "
        "FAILING scripts means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. Assertion rung: EDIT existing script to add ONE specific check (expect() "
        "assertion or shell script check); run to see it FAIL (red). NO implementation in this rung. NO file creation — edit existing test files only; AFTER check turns green "
        "(implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ Assertion complete (staying in assertion rung), THEN add another assertion for the same "
        "gap; the completion sentinel MUST explicitly declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied]; If gap IS closed: emit ✅ "
        "Thread N complete — all gaps closed; move to next gap; NO file creation in assertion rung — edit existing test files if they exist; stubs created at validation rung; only edit "
        "existing scripts to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one rung at a time; completion "
        "before continuation. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion "
        "green — NO assertions added here; NO file creation; only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER "
        "implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER "
        "completing implementation, CHECK if gap is closed — If gap still exists: FIRST emit ✅ Implementation complete (return UP to assertion rung to add next assertion); if gaps "
        "remain after implementation, the completion sentinel MUST explicitly declare: ✅ Implementation complete — gaps remaining: [list which prose/criteria/formal notation criteria "
        "are NOT yet satisfied]; if gap IS closed: emit ✅ Thread N complete — all gaps closed. If gap IS closed: emit ✅ Thread N complete — all gaps closed. Tests drive the behavior "
        "needed. Emit implementation rung label first, then implement, then emit completion sentinel. ASSERTIONS are test code that verifies behavior, NOT import errors or "
        "infrastructure failures. Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the "
        "sentinel must appear after the artifact is produced and gate is satisfied; Every rung must emit its label BEFORE producing its artifact and emit its completion sentinel AFTER. "
        "No implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed sentinel must include the "
        "verbatim failing assertion text; Evidence before claims: for every claim about code being correct, there must be tool execution output proving it; describing what you would do "
        "is not evidence; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: a rung "
        "is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → "
        "validation → assertion → implementation — the assertion rung is REQUIRED in this order; the assertion rung verifies each individual assertion red → green; emit the rung label "
        "before producing its artifact; emit completion sentinel after artifact is produced and gate is satisfied. EV rung: edits only validation files. EI rung: edits only "
        "implementation files. An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another artifact — it contains no behavior of its "
        "own; it must be invocable by an automated tool; validation files may not be imported by implementation files. An executable implementation artifact is a file that produces "
        ...
        "type-discipline violation that voids the rung; classification is determined by file content, not file path or naming convention — path and naming are evidence, not authority. ",
