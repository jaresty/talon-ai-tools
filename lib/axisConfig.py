...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "delegation is preferred; if the minimal statement still contains it the frame is inseparable and in-thread enumeration is used. A shoshin-eligible response that produced "
        "neither a delegation tool-use block nor an 'Assumed frame:' enumeration has not established that no trigger was missed. (2) Inadmissible: lists the specific prior conclusions "
        "or framings withheld for this pass; this line must appear at an earlier transcript position than the pass it governs. (3) A fresh-pass block reasons about the subject using "
        ...
        "quotes a specific claim that differs between them or states no claim differs. Before Reconcile, a leak check runs: for each conclusion in the fresh-pass block, attempt to "
        "identify an item named in Inadmissible that the conclusion depended on; emit 'Leak: found' with the item and redo the pass excluding that item, or 'Leak: not found'; "
        "iteration terminates when Leak: not found. The withheld context is not erased — it is declared inadmissible, and a fresh-pass conclusion that reuses an Inadmissible item "
        "without a resolved leak check does not satisfy this token. The admissibility change may be enforced by an isolating topology when available (a separate window or subprocess "
        "that genuinely cannot see the withheld context), in which case Leak: not found is guaranteed by construction rather than checked. This move is re-entrant: it may be invoked "
        "any number of times in a single response, each invocation self-contained with its own four parts. A response in which no point meets the trigger emits no invocations and is "
        "compliant; excluding context never licenses contradicting an established fact.",
