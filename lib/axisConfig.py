...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "independently-failing assertion that produces a meaningful FAIL when its own contract is violated, regardless of other layers. PHASE 1 — classification (text artifact, required "
        "before any process derivation): produce a classification block for each assertion using this exact structure: Assertion: <name or description> | Principle: <which principle "
        "governs it> | Natural FAIL state: <yes, or no — answer no if the assertion checks that something does NOT occur, because such assertions pass trivially when the behavior is "
        "absent> | Coverage artifact: <if Natural FAIL state is no, describe the specific incorrect behavior to introduce to produce a FAIL tool result; otherwise n/a>. Every assertion "
        "must have its own block. Phase 1 is complete when every assertion has a block. PHASE 2 — coverage artifact (tool result required before implementation): every assertion requires "
        "a FAIL tool result before any implementation step — Principle 1 applies to all assertions without exception. For natural-FAIL-state assertions (Natural FAIL state: yes), the "
        "FAIL comes from the initial red-state test run. For no-FAIL-state assertions (Natural FAIL state: no), the FAIL requires perturbation. Before any implementation step, produce a "
        "Phase 2 status line for every assertion using exactly one of these two forms — COMPLETE: Phase 2 [assertion name] — <verbatim FAIL output from tool result> or NOT YET PRODUCED: "
        "Phase 2 [assertion name] — running now. The NOT YET PRODUCED form names a missing predecessor. The implementation step's correctness criterion is: change the system from the "
        "observed wrong state to the correct state. Without the FAIL tool result, the wrong state has not been observed — the correctness criterion is undefined, not merely unmet. Chain "
        "requires every step to reproduce its predecessor before proceeding, and the predecessor named here does not yet exist as a tool result. The agent derives from this that no "
        "implementation step is definable until the tool call is made and the COMPLETE line can be written from actual output. There is no path between NOT YET PRODUCED and "
        "implementation that does not require fabricating a predecessor — and fabrication is not reproduction. The COMPLETE form requires the quoted FAIL output to be present verbatim in "
        "a prior tool result in this conversation — a fabricated or paraphrased output in a COMPLETE line is a chain violation. A compilation error is not a valid FAIL artifact for any "
        "assertion — natural-FAIL-state or otherwise. Principle 5 derives this: a compile error is a build failure, not a behavioral observation; the assertion has not run and has not "
        "observed the wrong state. When a test does not compile, the required action is to write a stub — a minimal implementation with the correct signatures and no-op behavior — so the "
        "test compiles and runs, then collect the behavioral FAIL from the running test. The behavioral FAIL (e.g. 'key missing', 'unexpected value') is the only valid predecessor for an "
