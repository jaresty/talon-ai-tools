...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "falsify": "The response treats implementation derivation as constrained exclusively to observed governing failures — the only evidence that a governing artifact encodes the governed "
        "behavior is an executed-artifact result (a tool-result block in this transcript) in which (a) precedes (c) and the failure line contains a substring from the governed "
        "assertion call expression. The response establishes, before any governed artifact-producing action, that the named governing artifact has been observed to detect the absence "
        "of each governed behavior. A governed artifact-producing action is any action that produces or modifies a persistent artifact — including file writes, document edits, "
        "configuration changes, and policy commits — whose absence of a governed behavior can be detected by a named executor invocation; an action whose output does not persist "
        "beyond the current session, or for which no named executor exists, is not a governed artifact-producing action. A named governing artifact is one whose file path — the path "
        "passed as an argument to the executor invocation, not a package name or directory — appears as a literal string in the named executor invocation that produces a result from "
        "it; an inline inspection (grep, curl, read, shell pipeline, verbal check) whose output is not produced by invoking a named artifact does not satisfy this requirement "
        "regardless of whether it produces a failure result. If no governing artifact exists for a governed behavior, writing that artifact is the first governed artifact-producing "
        "action; the assertion text and the governed symbol must reside in separate artifacts — a governed artifact-producing action that produces both assertion text and the governed "
        "symbol in the same action does not satisfy the creation-step exception for either; (a) through (g) apply to the named executor invocation immediately following that creation "
        "— the creation action itself satisfies the creation-step exception below. Derivation block temporal gate: the literal string named in (c) must appear as a verbatim substring "
        "of at least one tool-result block before the Falsify derivation block in the transcript; a (c) value that does not appear as a verbatim substring of any tool-result block "
        "preceding it does not satisfy the derivation phase regardless of whether the derivation block is structurally complete. Derivation phase (required before any governed "
        "artifact-producing action — complete only when a literal block labeled 'Falsify derivation:' containing entries for (a) through (g) and (f2) appears in the transcript; prose "
        "does not satisfy this requirement): Name — (a) the assertion-failure marker: the literal string in an executed-artifact result signaling the artifact detected behavior "
        "absence — (a) must be a string that appears in executor output when the governed artifact is absent; specifically, a string that the executor produces when running the named "
        ...
        "executor output is not a valid (a); (b) the literal string signaling the governed behavior is present; (c) the literal string identifying each governed behavior in an "
        "executed-artifact result — (c) must be derived from a direct callee of the assertion call expression as it appears on the FAIL line; a value derived from a test-identifier "
        "echo line, non-FAIL line, summary line, or test description in executor output is not a valid (c); the governed symbol must appear as a call expression reachable from the "
        "assertion call expression by following the execution call chain — the chain may pass through the test body, test setup, or the governed artifact; a callee whose name contains "
        "the governed symbol name as a substring but through which the governed symbol does not appear as a call expression in the reachable execution chain is a naming proxy and does "
        "not satisfy this requirement; a call expression that satisfies (c) must name a symbol whose callable definition does not appear in the content of any Write or Edit tool call "
        "whose path argument matches the governing artifact file path — if the identifier that appears as (c) on the failure line also appears as the name of a callable definition in "
        "the content of any Write or Edit tool call whose path argument matches the governing artifact, that identifier is a governing-artifact proxy and does not satisfy (c); (d) the "
        "governed symbol: the literal string naming the governed behavior — (d) must be derived from the direct callee of the assertion call expression in the governing artifact, "
        "where the call expression is not used only as an argument to a call expression whose callee does not contain the governed symbol name; (d) must be the identifier that appears "
        "as a substring of the failure line in (g) — not a symbol that contains or is associated with that identifier; if the failure line names a narrower identifier, that narrower "
        "identifier is (d) regardless of which broader symbol was invoked to reach it; (d) is derived from the failure line, not chosen independently — the failure line names (d), not "
        "the other way around. Assertion-target provenance exception: when the following conditions hold in the governing artifact's source text, the restriction that (d) must appear "
        "as a direct callee of the assertion call expression is suspended for this governed call: (1) an identifier is assigned exactly once in the governing artifact, and its "
        "initializer's outermost non-structural callee — after stripping any leading await and trailing property-access or index-access chains — is the governed symbol; "
        "Returned-object invocation provenance: condition (1) also permits a chain of single-assignment identifiers when each identifier in the chain satisfies conditions (1)–(3) with "
        "the immediately preceding identifier in the chain substituting for the governed symbol in condition (1) — the first identifier in the chain must have the governed symbol as "
        "its outermost non-structural callee; each subsequent identifier must have the immediately preceding chain identifier as the outermost non-structural callee of its initializer "
        "after stripping any leading await and trailing property-access or index-access chains; the same no-reassignment and structural-operations constraints from condition (3) apply "
        "to each intermediate identifier; (2) that identifier appears as an argument to an assertion call expression; (3) the identifier does not appear on the left-hand side of any "
        "assignment expression between its initial assignment and the assertion call expression, and every operation on the identifier between the assignment statement and the "
        "assertion call expression is one of: property access, index access, await, grouping, or argument separator; (4) the executor result block satisfies the base (a) and (c) "
        "requirements — no additional requirement that the failure line contain (d) or the chain identifier applies under this exception. Prose-origin gate: a string does not satisfy "
        "any derivation gate — including (a), (c), (d), the allow-list, and the failure line — if it appears verbatim in the content of a Write or Edit tool call in this transcript "
        "that precedes the first executor result block in this transcript containing that string; exceptions: (1) strings governed by the Bootstrap exception; (2) strings that satisfy "
        "the creation-step exception's absent/present condition — where (c) is absent before the Write action and present after it in executor output immediately following the Write. "
        "Bootstrap exception: when no tool-result block in the transcript before the Falsify derivation block contains the governed symbol name as a verbatim substring, a single "
        "governed artifact-producing action may introduce an artifact containing the governed symbol name — this action may not produce a result in which (b) appears; the named "
        "executor must be invoked immediately after this action (no executor result block between the action and the invocation), and the result must contain (a) before (c) with (d) "
        "appearing as a substring of the failure line; no further governed artifact-producing action is permitted until this executor result appears in the transcript. (e) the layer "
        "classification — if the governed symbol name appears as a token in a call expression in the test body — where the call expression is not nested inside another call expression "
        "that names a different symbol — the layer is 'unit'; if the governed symbol name does not appear in the test body, the layer is 'integration'; (f) the executor class — the "
        "executor must produce output that could only exist if the governed assertion reached execution at the layer named in (e); a governing artifact is disposable if removing the "
        "governed symbol from the governed artifact — without modifying the named governing artifact — produces an executor result in which (c) does not appear; a governing artifact "
        "is non-disposable only when (c) appears in the executor result after removal — a disposable artifact does not satisfy (f); (f2) the serialization gate — classify the "
        "assertion target: the value produced by the expression that is the direct argument to the assertion call. A governing assertion satisfies (f2) if and only if its assertion "
        "target is a behavioral value: a value whose content is determined by whether the governed behavior is correctly implemented and that could not be produced by an arbitrary "
        "implementation that merely serializes the governed artifact's output correctly. A governing assertion fails (f2) if its assertion target is a serialization value: a value "
        "produced by converting the governed artifact's output to a string/byte/text representation — regardless of what the conversion expression is named — such that a correct "
        "implementation and an incorrect implementation would produce the same assertion target value if their outputs serialize identically. The Falsify derivation block must contain "
        "the literal line '(f2) serialization gate: [assertion target] — targets: [behavioral property/state/outcome | string/byte/text representation] — gate: [pass | fail]'; a "
        "derivation block that does not contain this line does not satisfy (f2). The creation-step exception does not apply when the derivation block contains '(f2) serialization "
        "gate:' with 'gate: fail'; (g) the observed FAIL result: the content of an executor result block appearing in the transcript after the artifact was applied to the subject and "
        "before the first governed artifact-producing action for this behavior, where (a) precedes (c), the named executor produced the result, and the failure line contains (d) as a "
        "substring. A result satisfies this token if and only if: (g) is present at the required position; (a) is not separated from (c) in (g) by any line whose content contains (c) "
        "for a different governed behavior; and a subsequent executed-artifact result contains (b) at the position where (a) was absent. A result satisfying (g) must contain a failure "
        "line whose content includes a substring from the governed assertion call expression — where the call expression is a syntactic call in the test body whose callee name "
        "contains the governed symbol name as a substring; a failure line that contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) in its output; a "
        "valid (g) failure line is a line on which (a), (c), and a direct callee name containing (d) co-occur — an executor result that contains (a) before (c) but where no single "
        "line satisfies this co-occurrence does not satisfy the creation-step exception. Exception: (g) is not required for the single action that produces the governing artifact's "
