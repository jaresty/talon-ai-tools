...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "that is failing (the exact assertion line from the test file); Validation rung: write test file with test function names that describe expected behavior; create STUB "
        "implementation files so tests can RUN (imports resolve) — NO implementation whatsoever at validation rung, just wiring (empty functions returning null); tests may RENDER and "
        "MOCK dependencies but must have NO actual expect() assertions — writing actual test assertions (expect() calls) at validation rung is a protocol violation; tests should be "
        "EMPTY skeletons that pass because they have no assertions. FIX mock issues at validation rung, don't skip them. Assertions are written at the assertion rung, not validation. "
        "Tests MUST PASS at validation rung to move to assertion rung. Assertion rung: write ONE specific assertion using expect(), run tests to see it FAIL (red). NO implementation in "
        "assertion rung — only write assertion, run tests, observe failure. Then move to implementation rung. Implementation rung: AFTER assertion rung shows red, implement ONLY enough "
        "to make that specific assertion green — take the perspective of an adversarial implementer; implement what the test demands, nothing more; tests drive the behavior needed, not "
        "your assumptions about what the feature should do. Emit implementation rung label first, then implement, then emit completion sentinel. ASSERTIONS are test code that verifies "
        "behavior, NOT import errors or infrastructure failures. Only test assertions with expect() count as assertions. Rung completion MUST be emitted as: ✅ [Rung name] complete — "
        "not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; Every rung must emit its label BEFORE producing its artifact and emit its "
        "completion sentinel AFTER. No implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed "
        "sentinel must include the verbatim failing assertion text; Evidence before claims: for every claim about code being correct, there must be tool execution output proving it; "
        "describing what you would do is not evidence; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol "
        "violation. Rung validity test: a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: "
        "prose → criteria → formal notation → validation → assertion → implementation — the assertion rung is REQUIRED in this order; the assertion rung verifies each individual "
        "assertion red → green; emit the rung label before producing its artifact; emit completion sentinel after artifact is produced and gate is satisfied. EV rung: edits only "
