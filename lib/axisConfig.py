...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "atomic": "The response enforces one governing output per step: take the first failure from a tool-executed run result already in the transcript as the step's scope — the candidate is "
        "limited to the change that resolves the first failure only; other failures visible in the same result are not in scope for this step — they may not be cited to expand the "
        "candidate and become the governing failures for subsequent steps; a scope that names a failure not present in a prior tool result has not been derived. Before any edit, "
        ...
        "artifact: (1) scope declaration quoting verbatim the failure signal from the prior run result — the quoted text must appear in a tool result above this line; (2) derivation "
        "block as two entries: first, name the candidate change and at least one smaller change; second, in a separate entry below, quote the specific text from a prior tool-executed "
        "result that names a behavioral assertion — a test name or assertion message that fires on the governed behavior — and shows it failing; a quote showing only a module-level or "
        "import-level error does not demonstrate this, because the smaller change may resolve that error while leaving behavioral assertions failing only by inference — the quote must "
