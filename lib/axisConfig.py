...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "executable assertions → implementation — treat this as a floor, not a ceiling. Before finalizing the ladder, ask at each transition: is there an intermediate artifact that "
        "would further reduce human judgment? If yes, insert it as a rung. The rules-out requirement already prevents invalid splits — a rung that cannot name a specific outcome the "
        ...
        'artifact-type, and satisfaction-condition present" is valid; "all rungs classified" is not. A satisfaction condition of "see above", "as planned", or any phrase that requires '
        ...
        "checked, not declared. Before beginning each rung's work, produce a one-line ladder citation: \"Rung N [name]: satisfied — <satisfaction condition evaluated against transcript "
        'location>" or "Rung N [name]: not yet satisfied — producing now." If the citation cannot be written truthfully, the rung-work cannot begin. The transcript location cited must '
        "already exist above the citation at the time of writing — a citation pointing to a location the model is about to produce does not satisfy the requirement. At rung transitions "
        "that involve implementation tool calls, both the ladder citation and the RE-ANCHORING citation are required — they govern different things and neither substitutes for the "
        "other.",
