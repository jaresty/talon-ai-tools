...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "content is prose only, with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} present as a substring, does not satisfy this requirement; after writing each "
        "'property [N]:' line, immediately write '§ split test: [quoted sub-expression]' where [quoted sub-expression] is a phrase from the parent expression naming a candidate "
        "sub-expression to split on, followed by a sentence stating whether splitting at that sub-expression yields two independently falsifiable sub-properties — a 'property [Na]:' "
        "line that does not have a '§ split test:' line above it does not satisfy this requirement; if the split test finds a valid split, write one sub-property per branch: 'property "
        "[Na]:', 'property [Nb]:', etc., each containing a formal expression — a sub-property line whose content is prose only does not satisfy this requirement; if the split test "
        "finds no valid split (the property is already atomic), write 'property [Na]: atomic —' followed by a formal restatement of the parent expression; the conjunction of all "
        ...
        "properties complete' must not appear before a valid '§ properties check:' line; (4) verify coverage: after '§5 enumeration complete', immediately write '## Completion check' — "
        "'## Completion check' must not appear before '§5 enumeration complete' in the transcript; '## Completion check' must appear as the next heading after '§5 enumeration complete' "
        "in the transcript — the first '## ' heading line appearing after '§5 enumeration complete' must be '## Completion check'; a '## Completion check' appearing anywhere before '§5 "
        "enumeration complete' does not satisfy ground and must not be written — if the response has produced §1–§3 content and wishes to summarize or verify coverage, it must instead "
        "continue to '## Enforcement sequence' and '## Path enumeration' first; '§ properties complete' written before '## Completion check' satisfies the properties gate for the "
        "completion check — do not re-derive or repeat property lines inside '## Completion check'; inside '## Completion check', each covered dimension must cite a 'property [N]' "
        "number from the 'Formalized properties:' block and a verbatim string that appears either as a substring of a tool-result block produced after '§ implementation permitted' and "
        "visible above '## Completion check', or, when no such tool-result blocks are present, as a substring of the response text produced in this turn; no dimension citation or '§4 "
        "coverage verified' is valid until '§ properties complete' has appeared in the transcript; '§ test suite complete' is valid only after '§ properties complete' has appeared in "
        "the transcript — a '§ test suite complete' appearing before '§ properties complete' does not satisfy this requirement; '§4 coverage verified' is valid only after '§ test suite "
        ...
        "ending — a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume phrase requirement; when a turn is non-exempt, its final non-blank "
        'content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates still apply" to proceed under the same protocol. (5) enumerate '
        "escape paths: after '§ enforcement complete', immediately write '## Path enumeration' — '## Path enumeration' must not appear before '§ enforcement complete' in the "
        "transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each path, name "
        "the literal string whose presence closes it; when no further open path remains, write '§5 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of "
        "the escape path closing strings named in the enumeration above — a '§5 check:' line whose quoted phrase does not appear verbatim in the path enumeration above it does not "
        "satisfy this requirement; then write '§5 enumeration complete'; '§5 enumeration complete' must not appear before a valid '§5 check:' line in the transcript; '§5 enumeration "
        "complete' must not appear before '§ enforcement complete' in the transcript; any referent named in the '[text]' from §1 not already present as a literal string in the "
        "transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted' followed by a "
        "space and a decimal integer (e.g. '§ implementation permitted 1') must appear in the transcript after '§5 enumeration complete'; a sentinel line containing the literal bracket "
        "characters '[' or ']' does not satisfy this requirement — only a decimal integer satisfies the index requirement; the integer must equal the 1-based ordinal count of "
        ...
        "does not satisfy any rung or ordering requirement. Rung-completion sentinel finality: a turn is defined as all content within a single assistant message block; a line is "
        "non-blank if it contains at least one non-whitespace character; each of the strings '§0 observed', '§1 goal derived', '§1a decomposed', '§1b candidates', '§2 dimensions "
        "closed', '§ formalization complete', '§ properties complete', '§ test suite complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank "
        "line of the assistant message block in which it is written — the sentinel line must begin with the literal first character of the sentinel string ('§' or '#'); a sentinel line "
        "beginning with '**' or any other markdown formatting character does not satisfy this requirement; a checker verifies this by locating the sentinel line, scanning all "
        "subsequent lines in the same assistant message block, and confirming every such line is blank (contains only whitespace or is empty); any non-blank line after the sentinel in "
        "the same assistant message block is a violation; no exceptions are permitted — if additional content must appear, it must precede the sentinel, not follow it. Completion check "
        "ordering: the '## Completion check' block must appear after '§5 enumeration complete' in the transcript; a '## Completion check' block that appears before '§5 enumeration "
        "complete' does not satisfy this requirement even if its contents are otherwise correct; a response that contains '§5 enumeration complete' and does not produce a subsequent "
        "'## Completion check' block containing 'Formalized properties:', at least one 'property [N]:' line satisfying all property requirements, '§ properties complete', and '§4 "
        "coverage verified' does not satisfy ground. Continuation invariant: when the final non-blank content line of a turn contains or follows one of the rung-completion strings '§0 "
        "observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage verified', or '§5 enumeration complete', that turn must end with one of: '§ blocked: <text>', '§ awaiting: "
        "<text>', or '§ no-next-action: <text>', where '<text>' is non-empty. A turn whose final non-blank content line contains or follows one of these strings and does not end with "
        "one of the three exit strings does not satisfy ground.",
