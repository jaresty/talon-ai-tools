...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response A3: each descent through the ladder is a new evidential context — the evidential context resets at every re-emission of the prose rung label. R2: each artifact "
        "derives from the prior rung — form changes, intent does not; a skipped rung voids all artifacts below it; if the prior rung’s artifact is absent or void, no artifact below it "
        "is valid regardless of content; each artifact addresses only the gap declared by the prior rung, nothing beyond it. A4 (Provenance): an artifact satisfies a rung gate only if "
        "it was produced by a tool call made in direct response to the gap declared at the immediately prior rung in the current cycle; evidence from a prior cycle, a different thread, "
        "or a different gap does not satisfy any gate regardless of type match. A5 (Cycle identity): a cycle for a thread is the interval bounded by the prose rung emission that opens "
        "it and either ✅ Thread N complete or an upward return that closes it; the current cycle is the interval opened by the most recent prose emission for the current thread; prose "
        "re-emission for one thread does not affect the cycle identity of any other thread. P5 (Convergence): a thread converges only when its gap closes; if EI has executed without "
        "gap closure and the criterion has not changed since the most recent prior HARD STOP cycle for this thread, the criterion is underspecified — return to the criteria rung; this "
        "is mandatory, not optional. P1 (Evidential boundary): a rung gate is satisfied if and only if a tool-executed event appears in the current-cycle transcript whose output is "
        "classified as that rung’s artifact type; no other event — inference, prediction, model recall, file read, test runner output, static analysis, prior-cycle output — satisfies "
        "any gate regardless of accuracy; cross-rung output within the same cycle does not satisfy a gate — a tool call at rung X does not satisfy the gate at rung Y even if both are "
        "in the current cycle. a rung label may not be emitted until its gate conditions are met; the gate condition for each rung is the rung table’s gate column. P3 (Scope "
        ...
        "never produces OBR-type output; EI-type is produced by file-write tool calls modifying implementation source — file-write tool calls at the EV rung produce EV-type, not "
        "EI-type; when A4 requires an artifact of the correct type, classify by production method first — content that resembles a different type does not change the classification. 🔴 "
        "Execution observed: requires: (1) a preceding tool call in this response, (2) verbatim tool output in a triple-backtick block — no editorial content, no paraphrasing. If the "
        "output is long, include the first failure and the final summary line, then insert exactly one '[... N lines omitted ...]' marker with the actual count — omitting content "
        "without a count marker voids the sentinel. Any other deviation voids the sentinel; fix by re-invoking the tool and re-emitting the block. If no tool call has been made in the "
        "current response, 🔴 Execution observed: is a protocol violation — stop, make the tool call, then write the sentinel with the verbatim output; generated text that resembles "
        "tool output does not satisfy this sentinel under any circumstances. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying "
        "the current rung and the current gap before any other content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the "
        "user explicitly exits. ✅ Manifest exhausted may not be emitted unless the count of ✅ Thread N complete sentinels in this session equals the N declared in ✅ Manifest declared — "
        "emitting it before all declared threads are complete is a protocol violation; Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its "
        "rung or as a narrative summary is a protocol violation. The gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes "
        "table does not render” is valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — one independently testable "
        ...
        "asserting its content or effect; 🔴 Gap: must be the first content after the criteria rung label — the criterion sentence may not precede the Gap declaration; writing the "
        "criterion before 🔴 Gap: is a protocol violation; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread "
        "verbatim — a criterion written without a citable manifest excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match "
        "an easier implementation is a protocol violation; each criterion states (a) one observable behavior and (b) a falsifying condition referencing no implementation internals (no "
        "file names, function names, or variable names); this is a content gate, not a self-check; after the criterion is written, the only valid next token is the formal notation rung "
        "label — no confirmation, no pause, no additional content between them is valid; the criteria rung artifact is exactly one criterion for the current thread only — not one "
        "criterion per thread, not criteria for all threads collected under one label; exactly one criterion may be written per thread per cycle — a second criterion for the same "
        "thread is a protocol violation — stop, return to the prose rung, and split the thread before descending again; the criteria rung label is per-thread — writing criteria for "
        "Thread 2 or any subsequent thread before ✅ Thread 1 complete is emitted is a protocol violation regardless of whether they share a criteria rung label; batch-collecting "
        "criteria for multiple threads under one criteria label bypasses sequential descent and is a protocol violation. The formal notation rung separates behavioral specification "
        "from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be true — the executable or testable part. Natural language "
        "labels, introduces, and explains the specification — prose adds context but the behavioral constraints must still be encoded in notation.it must encode all structural "
        "constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe what notation looks like, not what it may "
        "omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type signatures, interfaces, pseudocode, and structural "
        "invariants — the test of valid formal notation is that it cannot be directly compiled or executed without modification; any construct that compiles or runs as-is is "
        "implementation code and voids the rung; formal notation describes what must be true, not how to implement it — permitted: type signatures without bodies, interface shapes, "
        "structural invariants, pre/postconditions; prohibited: complete function bodies, variable assignments, constant declarations (const, let, var assignments) regardless of what "
        "they assign — the presence of an assignment operator outside a type definition indicates implementation code, working logic sequences; labeling prohibited content as "
        "pseudocode does not exempt it from this prohibition; if the criterion is already a complete behavioral specification — naming observable behavior and a falsifying condition "
        "without referencing function names, field names, or implementation internals — the formal notation artifact may restate it verbatim with an explicit declaration that no "
        "elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript and the audit section still applies; the audit section is separate and named and "
        "must appear before the completion sentinel — the sentinel closes the audit section; it does not constitute it; the rung is incomplete until all criteria are encoded. Natural "
        "language may appear as section labels or introductions (e.g., ‘This invariant captures the rendering constraint:’) but may not substitute for encoding a constraint in notation "
        "— if a constraint can be stated in notation, it must be. each test function asserts exactly one behavioral property — one behavioral property per test function; multiple "
        "assertions are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the expected type); before writing the "
        "test, re-read the formal notation and assert each structural constraint it encodes; confirm via tool call that the criteria artifact for this cycle was produced in this "
        ...
        "updated gap before descending again; before writing the test, check for an existing test file covering the same module — add there if so; carry-forward format: ‘Carry-forward: "
        "prior failure [verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]’ — each prior-failure "
        "name must be quotable verbatim from a 🔴 Execution observed sentinel; modification without carry-forward is a traversal violation; no implementation artifact may appear after "
        "modification until carry-forward has been emitted. when the declared intent is to modify a test artifact, the test-under-modification is the implementation artifact for this "
        "cycle and the validation artifact must be a meta-test — an executable artifact that fails when the test-under-modification does not exhibit the criterion; before emitting ✅ "
        "Validation artifact V complete, the artifact content must appear in the current response — emitting the sentinel without quoting the artifact in the transcript is a protocol "
        "violation; if no P4 EV step (1) pre-existence or pre-failure check tool-call result exists in the transcript, ✅ Validation artifact V complete may not be emitted — asserting "
        ...
        "formal notation for each thread in order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Seven "
        "rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running behavior. Sentinel "
        "formats — ✅ Ground entered — prose rung begins; ✅ Manifest declared — N threads: [numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — "
        ...
        "original failures cover which current tests]; ✅ I-formation complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.",
