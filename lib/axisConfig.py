...
AXIS_KEY_TO_USE_WHEN: Dict[str, Dict[str, str]] = {
    ...
    "form": {
        ...
        "contextualise": "Wrapping content with a contextual preamble or frame: user "
        "wants output that includes surrounding background, not just the core "
        "content. The form adds framing without rewriting the substance. "
        "Heuristic: 'with context', 'add background before', 'frame this "
        "for', 'include context about why', 'give me X with background', "
        "'contextualize this for the team', 'prepend context' → "
        "contextualise. Distinct from mean scope (mean = focus the response "
        "on conceptual meaning; contextualise = wrap the response with a "
        "contextual preamble regardless of topic).",
