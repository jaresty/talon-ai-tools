...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "a running system; this is why test runner output is VRO-type and cannot satisfy the OBR gate regardless of the result; EI-type is produced by file-write tool calls modifying "
        ...
        "N complete sentinels in this session equals the N declared in ✅ Manifest declared — emitting it before all declared threads are complete is a protocol violation; Each "
        "completion sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The gap names a specific "
        "behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; if no "
        "currently-false behavior remains, emit ✅ Manifest exhausted — one independently testable behavior derived from the prose alone — if stating it requires names or structures not "
        "in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is a conjunction — split before continuing (‘the table renders Display Name’ "
        "and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a structural element and its data source is a conjunction — split before "
        "continuing; asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect; 🔴 Gap: must be the first content after the "
        "criteria rung label — the criterion sentence may not precede the Gap declaration; writing the criterion before 🔴 Gap: is a protocol violation; the Gap names the "
        "currently-false assertion that the criterion will make true; writing the criterion first proposes a repair before naming what is broken; before writing the criteria artifact, "
        "locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a citable manifest excerpt is unanchored; narrowing "
        "the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; each criterion states (a) one observable "
        ...
        "after the criterion is written, the only valid next token is the formal notation rung label — no confirmation, no pause, no additional content between them is valid; the "
        ...
        "criterion may be written per thread per cycle — a second criterion for the same thread is a protocol violation — stop, return to the prose rung, and split the thread before "
        "descending again; the criteria rung label is per-thread — writing criteria for Thread 2 or any subsequent thread before ✅ Thread 1 complete is emitted is a protocol violation "
        "regardless of whether they share a criteria rung label; batch-collecting criteria for multiple threads under one criteria label bypasses sequential descent and is a protocol "
        "violation; each thread’s criterion is only valid in the context of that thread’s descent — writing criteria for all threads before descending any treats the rung as a planning "
        "step, which it is not. The formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, "
        "pre/postconditions) encodes what must be true — the executable or testable part. Natural language labels, introduces, and explains the specification — prose adds context but "
        "the behavioral constraints must still be encoded in notation.it must encode all structural constraints the criterion implies — the permitted forms (type signatures, "
        "interfaces, pseudocode, structural invariants) describe what notation looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints "
        "unencodable is insufficient; it produces only type signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is that it cannot be "
        "directly compiled or executed without modification; any construct that compiles or runs as-is is implementation code and voids the rung; formal notation describes what must be "
        "true, not how to implement it — permitted: type signatures without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: complete function bodies, "
        "variable assignments, constant declarations (const, let, var assignments) regardless of what they assign — the presence of an assignment operator outside a type definition "
        "indicates implementation code, working logic sequences; labeling prohibited content as pseudocode does not exempt it from this prohibition; if the criterion is already a "
        "complete behavioral specification — naming observable behavior and a falsifying condition without referencing function names, field names, or implementation internals — the "
        "formal notation artifact may restate it verbatim with an explicit declaration that no elaboration is needed; this is not a skipped rung; the artifact must appear in the "
        "transcript and the audit section still applies; the audit section is separate and named and must appear before the completion sentinel — the sentinel closes the audit section; "
        "it does not constitute it; emitting it before the audit section inverts causality — you are claiming completion before the evidence exists; the rung is incomplete until all "
        "criteria are encoded. Natural language may appear as section labels or introductions (e.g., ‘This invariant captures the rendering constraint:’) but may not substitute for "
        "encoding a constraint in notation — if a constraint can be stated in notation, it must be. each test function asserts exactly one behavioral property — one behavioral property "
        "per test function; multiple assertions are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the expected "
        "type); before writing the test, re-read the formal notation and assert each structural constraint it encodes; confirm via tool call that the criteria artifact for this cycle "
        "was produced in this cycle's transcript; if the criteria artifact is identical to a prior cycle’s criteria artifact, the criterion has not changed — return to the prose rung "
        "and re-emit with an updated gap before descending again; before writing the test, check for an existing test file covering the same module — add there if so; carry-forward "
        "format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the "
        "artifact]’ — each prior-failure name must be quotable verbatim from a 🔴 Execution observed sentinel; modification without carry-forward is a traversal violation; no "
        "implementation artifact may appear after modification until carry-forward has been emitted. when the declared intent is to modify a test artifact, the test-under-modification "
        ...
        "exhibit the criterion; before emitting ✅ Validation artifact V complete, the artifact content must appear in the current response — emitting the sentinel without quoting the "
        "artifact in the transcript is a protocol violation; if no P4 EV step (1) pre-existence or pre-failure check tool-call result exists in the transcript, ✅ Validation artifact V "
        "complete may not be emitted — asserting pre-existence without a tool call result does not satisfy this gate; the validation artifact must reside within the project’s "
        "version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact "
        "asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten. Before writing the "
        "validation run observation rung label, invoke the validation artifact via a test runner; a harness error (import failure, syntax error, missing file) is not a red run — the "
        "test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness; when VRO exec_observed "
        "is a harness error caused by a missing implementation file: 🔴 Gap: may not be emitted — there is no behavioral gap, only a missing file; the only valid next token is the "
        "executable implementation rung label; HARD STOP is not valid; when VRO exec_observed is a harness error caused by a test file error (syntax error, import of a non-existent "
        "component in the test itself): 🔴 Gap: may not be emitted; the only valid next token is a tool call that repairs the test file at the EV rung — not HARD STOP; a green 🔴 "
        "Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate "
        "cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming "
        "the first failure; stop there — do not enumerate multiple failures or plan fixes; after a red VRO that is not a harness error, the criterion re-statement is an anchor only — "
        "re-running VRO without an implementation edit is a protocol violation; re-writing formal notation or the EV artifact without an intervening implementation edit is also a "
        "protocol violation; after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token — this shortcut applies only when a red VRO already exists in the "
        "current cycle for the current criterion; a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO must fire for the new criterion before impl_gate; skipping EV "
        "and VRO after a HARD STOP that introduces a new criterion is a protocol violation; a green exec_observed at the VRO rung means there is no failing assertion — 🔴 Gap: may not "
        "be emitted after a green exec_observed; a green exec_observed without a prior red run means the test is vacuous — return to the executable validation rung and rewrite the test "
        "to fail; a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed output; after emitting 🔴 Gap: at the observation rung, the criteria rung label is "
        "the only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation run "
        "observation rung in the current cycle — emitting it at any other position is a protocol violation; prose-only exec_observed sentinel does not satisfy this gate — the "
        ...
        "EV requires fixing the harness, not an upward return; When 🔴 Execution observed: at the executable validation rung shows a harness error (import failure, syntax error, missing "
        "file), the only valid next token is a tool call that repairs the harness — not a gap statement, not HARD STOP, not a rung label; HARD STOP may not appear in the same response "
        "as a harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further "
        "edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread; "
        "the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no "
        ...
        "the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules. A full test suite run result is required after the most recent OBR "
        "tool call — the only valid next action if no such result exists is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with "
        "a written reason. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal notation, executable validation, executable "
        ...
        "artifact — only the validation run observation rung stops after naming the gap. 🔴 Gap: at the criteria rung is not a stop point — the criterion must appear in the same "
        "response immediately following the Gap; no content, confirmation, or pause between them is valid. 🟢 Implementation gate cleared is the first token of the executable "
        "implementation rung — no tool call, no prose, and no file modification may appear before it in the current response; emitting a file-write tool call before 🟢 Implementation "
        "gate cleared is a protocol violation regardless of whether impl_gate is emitted later in the same response. it does not complete the thread; after emitting it, the executable "
        "implementation rung, the observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread N complete is valid; the next required action "
        "after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may not appear until the observed running behavior rung has fired for this thread. "
        "Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: "
        "gap-name]; [T: gap-name] markers are valid only in the prose rung — their presence in any other rung artifact is a protocol violation; before emitting ✅ Manifest declared, "
        "count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly one entry per distinct gap-name; a [T: gap-name] marker in the prose with no "
        "corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle "
        "following an upward return; the criterion must be immediately derivable from the re-emitted prose. the VRO rung label must appear in the transcript for the current cycle — a "
        "run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, count the thread entries in the list — the N "
        "in '✅ Manifest declared — N threads:' must equal the count of listed threads; a mismatch between the declared N and the thread list length is a malformed manifest. Before "
        "emitting ✅ Manifest declared, if the prose contains no [T: gap-name] markers, ✅ Manifest declared is blocked — re-emit the prose with [T: gap-name] markers added inline after "
        "each behavioral predicate before proceeding. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, "
        ...
        "— each distinct predicate requires a separate thread; count the [T: gap-name] markers in the prose you just wrote to confirm every predicate is covered; each manifest entry is "
        "a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest entry containing a verb or phrased as a currently-false assertion is malformed; the "
        "behavioral assertion for a gap is produced for the first time at the criteria rung, not the manifest; ✅ Manifest declared opens the thread manifest for the session. A revised "
        "manifest may be emitted when a new gap is discovered mid-run — return to prose, re-emit prose for all incomplete threads with the new gap included, then emit ✅ Manifest "
        "declared with the revised thread list. Completed threads are closed and may not be re-opened by a revision. The N in ✅ Manifest declared must reflect the current incomplete "
        "thread count at the time of emission. only threads declared in ✅ Manifest declared may be addressed — creating a thread not in the manifest is a protocol violation. Outputting "
        "a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. I is the "
        ...
        "including the observed running behavior rung — it is required in every cycle; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; "
        "all other rung transitions are continuous within the same response. Response length is never a valid reason to stop between rungs — if the full descent cannot fit in one "
        "response, continue from the current rung in the next response without re-emitting completed rungs; stopping mid-descent is a protocol violation regardless of response length. "
        "When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through executable implementation requires executable "
        "validation and validation run observation to have fired for the current gap, including after an upward return. Upward returns follow the failure class: if the implementation "
        "did not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; observation cannot be produced is a "
        "prose-description failure, not an implementation failure — return to the prose rung and update it. After every upward return, a new ✅ Validation artifact V complete must be "
        "emitted for the current cycle’s gap — an artifact written in a prior cycle does not satisfy this gate. Before producing the final report, invoke the delivered system "
        "end-to-end via a tool call and emit 🔴 Execution observed: showing the primary behavioral intent is satisfied — this is the post-delivery integration observation; if it fails, "
        "open a new gap cycle before producing the final report. Any prose that summarizes implemented behavior, lists acceptance criteria met, describes files created or modified, or "
        "names gaps as resolved is a final report regardless of its heading; a final report may not appear unless ✅ Manifest exhausted immediately precedes it in the transcript — if no "
        "✅ Manifest exhausted exists, stop: emit ✅ Manifest exhausted if all threads are complete, or open the next gap cycle if they are not. After emitting ✅ Manifest exhausted, "
        "produce a final report containing prose, criteria, and formal notation for each thread in order — no new behavioral claims, no coverage summaries, no suggestions; then "
        "reconcile any documents the implementation affects. Each rung forces a kind of specificity the prior rung cannot enforce: prose makes intent legible, criteria makes it "
        "falsifiable, formal notation makes it precise, EV makes it executable, VRO confirms it fails, EI produces the fix, OBR confirms it runs; skipping a rung leaves open the escape "
        "route that rung closes. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed "
        "running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ Manifest declared — N threads: [numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim "
