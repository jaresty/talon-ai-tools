...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ambiguity test:' and a valid 'alternative satisfier:' line have appeared; (2c) derive formalized properties: after writing '§ formalization complete', immediately write '## "
        "Formalized properties' — '## Formalized properties' must not appear before '§ formalization complete' in the transcript; decompose each §2 dimension into its atomic formal "
        "claims, one claim per line prefixed 'property [N]:' where N is a 1-based index; for each property write '§ property [N] atomic: [sub-claim that would be false while the "
        "property as a whole holds]' — if no such sub-claim exists the property is already atomic; write '§ properties complete' after all properties and atomicity checks appear; '## "
        "Enforcement sequence' must not appear before '§ properties complete' in the transcript; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before "
        "'§2 dimensions closed' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each "
        "sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; (4) verify coverage: '## Completion check' must not "
        "appear before '## Enforcement sequence' in the transcript and must not appear before '§ implementation permitted' in the transcript; each covered dimension cites a verbatim "
        ...
        "blocks are present after '§ implementation permitted', as a substring of the response text produced in this turn; when every covered dimension cites such a substring, before "
        "writing '§4 coverage verified', count the number of distinct 'Falsify derivation:' blocks that appear in the transcript above '## Completion check'; if at least one such block "
        "exists, write '§ test suite complete: [N] derivation(s) verified' where [N] is that count; if no such block exists, write '§ test suite complete: (none)'; '§4 coverage "
        "verified' must not appear before '§ test suite complete' in the transcript; '## Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a "
        "dimension named for the first time inside '## Completion check' does not satisfy any coverage requirement and does not extend the governed dimension set; a turn containing a "
        "satisfied 'Gate condition:' block must contain the governed tool call in the same turn — prose appearing between the 'Gate condition:' block and the governed tool call does "
        "not satisfy this requirement; every assistant turn after '§ implementation permitted [N]' has appeared in the transcript that does not contain a line beginning with '§ "
        "implementation permitted [' must contain '## Completion check' — a turn that does not contain a line beginning with '§ implementation permitted [' and does not contain '## "
        "Completion check' does not satisfy ground; '## Completion check' is satisfied only when the literal string '§4 coverage verified' appears within it — a '## Completion check' "
        "block that does not contain '§4 coverage verified' does not satisfy this requirement and the governed turn must continue to the next derived step rather than ending — the "
        "first non-blank line after the '## Completion check' heading must be 'Gate condition:'; a turn whose final non-blank content line is '§4 coverage verified' is exempt from the "
        'resume phrase requirement; when a turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue '
        "autonomously — gates still apply\" to proceed under the same protocol. (5) enumerate escape paths: '## Path enumeration' must not appear before '## Enforcement sequence' in "
        "the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each path, "
        "name the literal string whose presence closes it; when no further open path remains, write '§5 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one "
        "of the escape path closing strings named in the enumeration above — a '§5 check:' line whose quoted phrase does not appear verbatim in the path enumeration above it does not "
        "satisfy this requirement; then write '§5 enumeration complete'; '§5 enumeration complete' must not appear before a valid '§5 check:' line in the transcript; any referent named "
        "in the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears "
        ...
        "verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant message block in which it is written — a checker verifies this by locating "
        "the sentinel line, scanning all subsequent lines in the same assistant message block, and confirming every such line is blank (contains only whitespace or is empty); any "
        "non-blank line after the sentinel in the same assistant message block is a violation; no exceptions are permitted — if additional content must appear, it must precede the "
        "sentinel, not follow it. Continuation invariant: when the final non-blank content line of a turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal "
