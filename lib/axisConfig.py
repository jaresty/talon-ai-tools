...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "moving to assertion rung. At validation rung, you MUST explicitly list all files being created in the conversation — this is your file manifest. Assertion rung: EDIT existing "
        "script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL (red). NO implementation in this rung. NO file creation — edit existing test "
        "files only; before editing, declare 'no new files' as verification that no new files are being created; AFTER check turns green (implementation succeeded), CHECK if gap is "
        ...
        "declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied] — evidence: [reference assertions checked or gap reason]; If gap IS closed: "
        "emit ✅ Thread N complete — all gaps closed; move to next gap; NO file creation in assertion rung — edit existing test files if they exist; stubs created at validation rung; "
        "only edit existing scripts to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one rung at a time; "
        "completion before continuation. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific "
        "assertion green — NO assertions added here; NO file creation; only edit existing stubs; before editing, declare 'no new files' as verification that no new files are being "
        "created; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy "
        "the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if gap is closed — If gap still exists: "
        "FIRST emit ✅ Implementation complete (return UP to assertion rung to add next assertion); if gaps remain after implementation, the completion sentinel MUST explicitly declare: "
