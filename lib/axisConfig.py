...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "action produces a tool-executed result containing (c). Semantically-equivalent refactor exception: (g) is also satisfied when a tool-executed result containing (a) before (c) "
        "appears in the transcript before the file-modifying call, produced by an executor invocation after the refactored form was modified to violate the invariant the governing "
        "assertion checks. Revert is complete when a subsequent executor invocation produces (b) at the position where (a) was absent and every assertion line from the "
        "pre-perturbation run result is present.",
