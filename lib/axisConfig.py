...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "highest already-instantiated rung and update it to reflect the intended change, then descend. For code contexts, R4 instantiates as: prose (natural language description of intent and constraints) → criteria (acceptance conditions as plain statements) → formal notation (non-executable specification — type signatures, schemas, pseudocode, or contracts; may use code syntax to describe interfaces but the artifact cannot be run as written) → "
