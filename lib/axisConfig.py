...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; (4) verify coverage: '## Completion check' is valid "
        "only after '§5 enumeration complete' has appeared in the transcript; immediately after '## Completion check', write the label line 'Formalized properties:' — this label line "
        "is valid only after '§ formalization complete' has appeared in the transcript; '§ properties complete' is valid only after 'Formalized properties:' has appeared in the current "
        "'## Completion check' block; decompose each §2 dimension into its atomic formal claims; each claim must appear on its own line beginning with the literal text 'property [N]:' "
        "where N is a 1-based integer — a line beginning with '**property' or any markdown-formatted variant does not satisfy this requirement; each 'property [N]:' line must contain "
        "at least one formal expression — a logical predicate (∀/∃ notation), a typed function signature, or a concrete input/output example (f(v)=w or equivalent) — as a literal "
        "substring; a 'property [N]:' line whose content is prose only, with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} present as a substring, does not satisfy this "
        "requirement; for each property write '§ property [N] atomic: [sub-claim that would be false while the property as a whole holds]' — if no such sub-claim exists the property is "
        "already atomic; when all 'property [N]:' lines and atomicity checks are written, write '§ properties check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim "
        "substring of one of the 'property [N]:' lines written above — a '§ properties check:' line whose quoted phrase does not appear verbatim in a 'property [N]:' line above it does "
        "not satisfy this requirement; then write '§ properties complete'; '§ properties complete' is valid only after a valid '§ properties check:' line has appeared; no dimension "
        "citation, '§ test suite complete', or '§4 coverage verified' is valid until '§ properties complete' has appeared in the current '## Completion check' block; dimension "
        "citations are valid only after '§ properties complete' has appeared; each covered dimension cites a verbatim string that appears either as a substring of a tool-result block "
        ...
        "produced in this turn; '§ test suite complete' is valid only after '§ properties complete' has appeared in the current '## Completion check' block — a '§ test suite complete' "
        "appearing before '§ properties complete' or before '## Completion check' does not satisfy this requirement; '§4 coverage verified' is valid only after '§ properties complete' "
        "has appeared in the current '## Completion check' block, '§ test suite complete' has appeared, and every covered dimension cites such a substring; '## Completion check' must "
        "not introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## Completion check' does not satisfy any coverage requirement "
        "and does not extend the governed dimension set; a turn containing a satisfied 'Gate condition:' block must contain the governed tool call in the same turn — prose appearing "
        ...
        "proceed under the same protocol. (5) enumerate escape paths: '## Path enumeration' must not appear before '## Enforcement sequence' in the transcript; enumerate every path by "
