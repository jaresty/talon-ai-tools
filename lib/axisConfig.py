...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "relative to the governing artifact citation. Before the run tool call, produce a derivation block as two entries: first, name the candidate check — the specific artifact to "
        "run, naming its relative speed compared to alternatives; second, name at least one insufficient check — a faster or lighter check that would not catch this behavior — and "
