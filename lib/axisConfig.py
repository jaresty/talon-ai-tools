...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "transcript artifact has not been derived. When an implementation step creates a new file, the derivation must additionally produce a line manifest before the edit: an "
        "enumerated list of every line (or logical block) to be written, each with either (a) the specific failure in F it is required to close or support closing, or (b) the specific "
        "existing line in the file that would fail to compile or execute without it, citing the transcript location that establishes this dependency. A manifest entry is valid only "
        "when (a) or (b) can be stated — an entry that can state neither must be omitted from the file. The post-edit confirmation for a new-file step must additionally confirm that "
        "the created file's content matches the manifest — lines present in the file but absent from the manifest must be removed before the step closes. An implementation edit closes "
        "the step only when a post-edit run result from the governing artifact — produced by a tool call after the edit — exists in the transcript and names a failure set F' "
        "satisfying: (a) the governed failure is absent from F'; (b) every failure signal in F' whose governing artifact produced no failure in F is absent — a failure signal in F' "
        "whose governing artifact had no failure in F is a regression and requires revert regardless of any judgment about latency; (c) failure signals in F' from governing artifacts "
        "that were already producing failures in F are progression failures, each becoming a new governing output for a new step. A failure in F' is a progression failure if and only "
        "if the governing artifact that produced it also appears in F with at least one failure — checkable against the pre-edit run result without interpretation; a model assertion "
        "that a failure is latent does not satisfy this condition. If (a) or (b) is not satisfied, the edit must be reverted. A post-edit confirmation line names the specific change "
        "made, cites the derivation's stated candidate, and cites the pre-edit and post-edit run results by transcript location — if the change made exceeds the derivation's stated "
        "scope, the edit must be reverted to match it before proceeding. A step is the smallest change that could alter the result of any verification — compilation, static analysis, "
        "or test execution; changes that address different verification layers are always separate steps even when they feel like parts of the same fix. The governing artifact must be "
        "the verification layer with scope to detect a failure if this step's change is incorrect — running a layer that cannot see the specific files, call sites, or concerns this "
        "step touches does not satisfy the governing artifact requirement even if that layer passes. An artifact is a valid governing artifact for this step only when a derivation "
        "exists in the transcript naming the specific FAIL output it would produce if this step's change were absent. Implementing beyond the current governing output is a violation "
        "regardless of how many other failures exist. Analysis and classification steps have a text artifact as their governing artifact — a complete classification block or derivation "
        "section; no tool call is required to open or close these steps. A step qualifies as an analysis step when and only when its governing artifact's content, if applied verbatim "
        "to the system, would produce no change in system behavior, and names at least one specific behavioral invariant that would remain unchanged if the artifact were discarded. "
        ...
        "be fully expressed in any artifact; the completion check is the only permitted mechanism for determining what is in scope against the intent artifact. An item may only be "
        "removed from scope when the original intent artifact explicitly excludes it — any reduction in scope not present in the original intent artifact is a unilateral change "
        "regardless of the label used. The only valid basis for scope reduction, rung compression, or any deviation from the derived enforcement process is the completion check against "
        "the intent artifact explicitly excluding the item. Any other basis — including commentary about cost, complexity, heaviness, resource constraints, task simplicity, or "
        "proportionality — is not valid. Naming an unaddressed item does not close it — only a named artifact covering a named dimension does. When a governing artifact cycle is "
        "active, the completion check fires when the cycle reports no remaining failures — exhausting the artifact is necessary but not sufficient for completion. The derived "
        "enforcement process must be a ladder: each rung is a strict refinement of the rung above — its artifact preserves all constraints of the upper rung and adds new ones, reducing "
        "the degrees of freedom for satisfying the intent artifact's representation of intent. A rung with fewer degrees of freedom requires less human interpretation to verify: "
        "executable artifacts are unambiguous where prose is not. The ladder need not follow a fixed schema; the agent derives the rungs from the task's nature. A task involving "
        "executable behavior will typically descend from intent → acceptance criteria → formal specification → executable assertions → implementation — treat this as a floor, not a "
        "ceiling. Before finalizing the ladder, produce one line per transition as a separate prose line in the transcript — not embedded in a table cell, column, or any structure that "
        'cannot contain the dimension enumeration required below: either "Transition N→N+1: no intermediate rung — [reason the current gap cannot be further subdivided into a '
        'judgment-reducing artifact]" or "Transition N→N+1: inserting [rung name] — rejects [specific outcome the prior rung admits]". A transition with no such line is unexamined and '
        "the ladder is incomplete. A transition's \"rejects\" claim is valid only when it names: (a) a specific artifact state that satisfies the previous rung's satisfaction "
        "condition, and (b) the specific behavioral dimension that state fails to constrain — a dimension whose absence would not be detectable by inspecting the prior rung's artifact "
        'alone. A "rejects" claim that names a count threshold, a format property, or any property other than a behavioral dimension not constrainable by the prior rung is not valid. A '
        "transition line is complete only when it also states the count of behavioral dimensions in the rung above's artifact — derived by naming each dimension individually before "
        "stating the count; a count stated without a preceding enumeration has not been derived — and confirms that count is matched by covering items in the rung below — a transition "
        "where these counts diverge is incomplete regardless of whether intermediate rungs are absent. A covering item is valid only when it names: (a) the specific condition under "
        "which this rung's artifact would fail if the dimension it covers were absent, and (b) at least one artifact state that satisfies this rung's satisfaction condition but would "
        "be rejected if this covering item were added as a constraint. A covering item that names only a dimension label without stating the failure condition has not been derived. If "
        "rung 1 is below intent level, each intermediate level between the cited floor artifact and rung 1 that is absent from the ladder is valid only when a specific artifact already "
        "present in the transcript above the ladder provides that level's judgment reduction — the citation must name: (1) the location of that artifact in the transcript, (2) the "
        "specific text at that location that provides the level's judgment reduction, and (3) at least one outcome that text would reject at that level. The SUBJECT section and "
        "ADDENDUM section of a bar-formatted prompt are task input and directives — neither is a valid skip artifact. They contain no judgment reduction and reject no outcomes. If the "
        "cited artifact cannot satisfy all three parts, the skip is invalid and the level must be added as a rung. Any other omission is invalid. The rules-out requirement already "
        "prevents invalid splits — a rung that cannot name a specific outcome the previous rung admits but this rung rejects cannot be added. Producing the ladder is a required "
        "artifact before any rung-work begins. The ladder is not itself a rung — it is a precondition to all rungs including rung 1. A ladder entry whose artifact is the ladder itself "
        "is invalid: the ladder cannot govern its own production. The rung 1 coverage mapping is a separate required artifact produced after the ladder is closed — producing both in "
        "the same generative act does not satisfy this requirement. The ladder must be a completed artifact in the transcript before the rung 1 coverage mapping begins. Rung 0 is the "
        "intent artifact — a representation of intent, not intent itself. Its satisfaction condition checks the representation's content, not intent: it names which behavioral "
        "requirements appear in this representation, with a false-evaluating state of any named behavioral requirement being absent from the representation. Dimensions of intent not "
        "captured by the representation are outside the scope of all coverage mappings derived from it — the coverage mapping is bounded by the representation, not by intent. A rung 0 "
        'satisfaction condition of "given", "present", or any phrase that does not name the representation\'s behavioral requirements has not been derived. The artifact is an '
        "enumerated list of rungs, each stating: the rung name, the artifact type produced at that rung, the condition under which the rung is satisfied — including the specific state "
        "of the artifact that would cause the satisfaction condition to evaluate to false; a satisfaction condition entry that cannot name its false-evaluating state has not been "
        "derived and is not a valid ladder entry — and, for every rung after the first, at least one specific outcome that would satisfy the previous rung but be rejected by this rung, "
        "naming both (a) the specific artifact state that satisfies the previous rung's satisfaction condition and (b) the behavioral dimension that state fails to constrain; a rejects "
        "entry that names only a dimension label or boolean property without naming a specific satisfying artifact state is not valid. If no such outcome can be named, the proposed "
        "rung is not a refinement — it is a parallel task and must be merged with the rung above or dropped. Describing the ladder structure in prose does not produce this artifact. A "
        "rung's satisfaction condition is valid if and only if it names: (a) a locatable artifact, and (b) a property of that artifact that would evaluate differently if any behavioral "
