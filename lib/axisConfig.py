...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "subsequent rung activity. Session observation loop: before beginning a ladder descent, observe current running behavior via tool call — if the observation reveals a gap "
        "between running behavior and declared intent, declare the gap and descend the ladder; if no gap remains, emit ✅ Ground complete — the session ends; this observation is not a "
        "rung in the derived ladder and must not appear in the rung table; the ladder handles only the declared gap; after manifest exhaustion, the observation loop recurs to check "
        "whether intent is fully achieved; ✅ Ground complete may only be emitted as the outcome of the observation loop — emitting it outside the observation loop is a protocol "
        "violation regardless of whether 🔵 Closing observation is present. An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another "
        "artifact — it contains no behavior of its own; validation files may not be imported by implementation files. An executable implementation artifact is a file that produces "
        "behavior directly — it contains no assertions about other artifacts; implementation files may not contain assertions. A file that both asserts and implements is a "
        "type-discipline violation that voids the rung at which it was written; classification is determined by file content, not file path or naming convention — path and naming are "
        "evidence, not authority. For software behavioral change, the standard ladder for each gap-closing cycle produces: prose → criteria → formal notation → executable validation → "
        "executable implementation. A derived ladder that omits any of these rungs for a software behavioral change task must cite which principle makes that rung unnecessary. The "
        "criterion is exercised only when the automated validation suite runs to completion and individual assertions fail; execution halted before reaching the assertions (by "
        "infrastructure failure, import error, or any other cause) does not exercise the criterion — the executable-implementation artifact for such a halt resolves only the "
        "infrastructure gap (minimum change sufficient to allow the suite to reach the assertions); the criterion has not been exercised until individual assertions fail in a "
        "subsequent cycle. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun. When beginning "
        "mid-ladder, locate the highest already-instantiated rung, update it, then descend. Sentinel formats — ✅ Ground entered — prose rung begins [gate: first content emitted in this "
        "response; no artifact, code, prose, or reasoning may precede this sentinel — preceding content voids the session]; ✅ Manifest declared — N threads: [numbered list of "
        ...
        "HARD STOP — upward return to criteria rung [gate: exec_observed showing test suite failure + gap in current cycle; criterion identical to prior cycle criterion for this "
        "thread]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed] [gate: exec_observed showing test suite failure + gap in current cycle; valid only at "
        "the rung whose artifact type is executable implementation; the verbatim exec_observed output must contain at least one assertion-level failure string (e.g. 'Expected … "
        "Received …', 'AssertionError', 'FAILED', specific test name + failure reason) — an exec_observed whose output shows only infrastructure failure, import error, process crash, "
        "or a model-described summary of failure does not satisfy this gate; if the suite cannot reach assertions, the implementation artifact for that cycle resolves only the "
        "infrastructure gap, and this gate cannot be emitted until a subsequent cycle produces assertion-level failure output]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: "
        "exactly one criterion in this rung's artifact; criterion contains no conjunction; formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ "
        "Validation artifact V complete [gate: pre-existence check tool call result present; tool call in this response wrote the file to disk]; ✅ Thread N complete [gate: meta "
        "exec_observed after executable implementation shows the gap declared for this thread is no longer present in running behavior]; ✅ Manifest exhausted — N/N threads complete "
        ...
        "intent; all manifest threads complete]; 🔵 Closing observation — [what the tool-executed observation shows] [gate: the tool call immediately before this sentinel must directly "
        "invoke the behavior named in the session intent and its output must show that behavior is present; this sentinel must appear after ✅ Manifest exhausted and before ✅ Ground "
        "complete in the same response; emitting ✅ Ground complete without a preceding 🔵 Closing observation in the current response is a protocol violation]; 🔵 Write authorized — "
        "rung: [rung name] | artifact type: [type] | file: [path] [gate: currently open rung (most recently emitted rung label with no completion sentinel yet) has this file's artifact "
        "type in its permitted-tool-calls column; no open rung → gate unsatisfied; artifact type mismatch → gate unsatisfied; the cited artifact type must match the artifact type of "
        "the named file as determined by the rung table's artifact-type column — a cited artifact type that does not match the file being written is a fabrication that voids the "
        ...
        "file-write tool call — intervening content between this sentinel and the tool call voids both].",
