...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; before "
        "emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly one entry per distinct gap-name; a [T: gap-name] "
        "marker in the prose with no corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at the start of every cycle — the first cycle, any "
        "subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 "
        "Execution observed: from the current cycle’s validation run observation rung — not from a prior cycle — to exist in output; the VRO rung label must appear in the transcript "
        "for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, count the thread "
        "entries in the list — the N in '✅ Manifest declared — N threads:' must equal the count of listed threads; a mismatch between the declared N and the thread list length is a "
        "malformed manifest. Before emitting ✅ Manifest declared, if the prose contains no [T: gap-name] markers, ✅ Manifest declared is blocked — re-emit the prose with [T: gap-name] "
        "markers added inline after each behavioral predicate before proceeding. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates "
        "(fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read the prose "
        "rung output via a tool call to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a "
        "manifest entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the criteria "
        "rung, not the manifest; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a "
        "rung whose label has not been output has not begun and no artifact for it may exist. I is the declared intent governing the invocation; each artifact must be self-sufficient "
        "to derive the next without consulting I; before closing a rung artifact, state every open constraint the next rung must resolve; faithful derivation requires form to change, "
        "intent to stay fixed. Advance through every feasible rung without pausing for user confirmation between rungs — completeness governs rung depth, not rung existence; each rung "
        ...
        "run observation rung after emitting 🔴 Gap:; stopping between rungs at any other point is a protocol violation — waiting for user confirmation between rungs is a protocol "
        "violation; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then "
        ...
        "behavior, lists acceptance criteria met, describes files created or modified, or names gaps as resolved is a final report regardless of its heading; Such prose may not appear "
        "until ✅ Manifest exhausted exists in the transcript for this invocation; if no ✅ Manifest exhausted exists, stop: emit ✅ Manifest exhausted if all threads are complete, or "
        "open the next gap cycle if they are not. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order — "
        "no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Rung-entry gate: before producing content at any "
        "rung, state (a) the rung name, (b) the current gap as a currently-false behavioral assertion, (c) the artifact type this rung requires, and (d) whether a 🔴 Execution observed: "
        "sentinel exists in the current cycle and, if so, whether its output is of type (c) — if no exec_observed exists yet in this cycle, state that explicitly rather than treating "
        "prior-cycle output as satisfying this check; if any of (a)–(d) cannot be stated from the current-cycle transcript, produce it before any other content at this rung; if (d) "
        "reveals that no valid exec_observed exists in the current cycle, the only permitted next token is a tool call — producing any other content when (d) is false is a protocol "
