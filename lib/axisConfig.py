...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "produce an artifact does not produce it. Execution gate (R3): "
        ...
        "tests pass or fail; if tests do not run, fix the rung before descending; no file-creating or file-editing tool call for an implementation rung may be invoked before these "
        "sentinels appear in output; if any implementation artifact is produced without valid Execution observed + Gap sentinels immediately preceding it, discard the implementation "
        "and run the tool first. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; "
