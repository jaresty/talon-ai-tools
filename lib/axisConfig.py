...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "tests to match implementation. If tests are wrong, return UP to validation rung to fix them; modifying tests in later rungs voids the rung. ASSERTIONS are test code that "
        "verifies behavior, NOT import errors or infrastructure failures. An import error means the file doesn't exist - fix the file first, not the test. Only test assertions with "
        ...
        "gate is satisfied; No implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed sentinel "
        "must include the verbatim failing assertion text; Evidence before claims: for every claim about code being correct, there must be tool execution output proving it; describing "
        "what you would do is not evidence; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung "
        ...
        "→ formal notation → validation → assertion → implementation — the assertion rung is REQUIRED in this order; the assertion rung verifies each individual assertion red → green; "
        "emit the rung label before producing its artifact; emit completion sentinel after artifact is produced and gate is satisfied. EV rung: edits only validation files. EI rung: "
        "edits only implementation files. An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another artifact — it contains no "
        "behavior of its own; it must be invocable by an automated tool; validation files may not be imported by implementation files. An executable implementation artifact is a file "
        "that produces behavior directly — it contains no assertions about other artifacts; implementation files may not contain assertions. A file that both asserts and implements is "
        "a type-discipline violation that voids the rung; classification is determined by file content, not file path or naming convention — path and naming are evidence, not "
        "authority. ",
