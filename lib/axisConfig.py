...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Derivation phase: (0) write '§0: [scenario]' as the first content line of the ground derivation, where [scenario] names the subject from the conversation — this declaration "
        "satisfies §0 and opens the ladder; '§0:' must not appear before the '=== TOKENS' section of the current invocation; if an artifact that executes the subject system and returns "
        "its live output appears verbatim in the conversation above, invoke it as a tool call immediately after writing '§0: [scenario]'; after that tool-result block appears and "
        "reflects live system execution, immediately write '§0 observed' — a tool-result block produced by invoking the named artifact as a tool call satisfies this requirement; a "
        "result produced by a Read, search, or file-write tool call does not; an artifact that returns document content does not qualify — a tool result consisting of a GitHub issue "
        "body, pull request body, specification document, README, or requirements text does not satisfy §0 regardless of invocation form; goal-source classification (required when '§0 "
        "observed' is present): examine whether any line in the §0 tool-result block appears verbatim as a substring of any user message visible above §0 in the transcript; if no such "
        "line exists, write '§0 status-only'; after '§0 status-only', fetch a broader context artifact before writing '## Governing goal:' — the fetch tool-result block must contain at "
        "least one line that appears as a substring of a user message above it in the transcript; a tool-result block whose every line is a substring of content already present in the "
        "transcript above it does not satisfy this requirement; (1) derive the governing goal: after '§0: [scenario]', write '## Governing goal: [text]' — if '§0 observed' is present, "
        "'[text]' must appear verbatim as a substring of the §0 tool-result block above; if '§0 observed' is absent, '[text]' is derived from the scenario description — what would "
        "satisfy the underlying need; a '[text]' value that adds a claim absent from the scenario does not satisfy §1; immediately below the governing goal heading, before the "
        "means-test, extract labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', labeled subject, verb, outcome, or invariant; no field may "
        "equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive words; when every named component of '[text]' is covered, write '§1a check: "
        "\"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the labeled fields written above in this rung — a '§1a check:' line whose quoted phrase does not "
        "appear verbatim in the labeled fields above it does not satisfy this requirement; after a valid '§1a check:' line has appeared, immediately write '§1a decomposed'; immediately "
        "below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]' — when '§0 observed' is present, alternative means must not "
        "duplicate approaches already listed in §0; if only one concrete alternative can be named, write '[hypothetical]: [speculated alternative]' before closing §1; if two "
        "non-subsuming goal candidates exist, write both and mark one '[selected]' — the selected goal is the one whose means-test names a more concrete alternative; when ≥2 "
        "alternatives (concrete or hypothetical) are present, write '§1 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the alternative means lines "
        "written above in this rung — a '§1 check:' line whose quoted phrase does not appear verbatim in the means-test above it does not satisfy this requirement; after a valid '§1 "
        "check:' line has appeared, immediately write '§1 goal derived'; immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in the "
