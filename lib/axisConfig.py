...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "line, or test description in executor output is not a valid (c); the governed symbol must appear as a call expression reachable from the assertion call expression by "
        "following the execution call chain — the chain may pass through the test body, test setup, or the governed artifact; a callee whose name contains the governed symbol name as "
        "a substring but through which the governed symbol does not appear as a call expression in the reachable execution chain is a naming proxy and does not satisfy this "
        "requirement; (d) the governed symbol: the literal string naming the governed behavior — (d) must be derived from the direct callee of the assertion call expression in the "
        "governing artifact, where the call expression is not used only as an argument to a call expression whose callee does not contain the governed symbol name; (d) is the literal "
        "string which must appear as a substring of the failure line in (g); (e) the layer classification — if the governed symbol name appears as a token in a call expression in the "
        "test body — where the call expression is not nested inside another call expression that names a different symbol — the layer is 'unit'; if the governed symbol name does not "
        "appear in the test body, the layer is 'integration'; (f) the executor class — the executor must produce output that could only exist if the governed assertion reached "
