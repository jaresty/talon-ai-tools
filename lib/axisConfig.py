...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "check: if no 🔴 Gap: sentinel exists in output yet, emit it now; the 🟢 Implementation gate sentinel may only be emitted if a 🔴 Execution observed: sentinel already exists in "
        "the conversation output — if absent, this sentinel is prohibited; produce the executable validation artifact instead. Only validation artifacts may be produced at the "
        "executable validation rung — implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not constitute executable validation — the "
        "artifact must be invocable by an automated tool. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output "
        "has not begun and no artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then "
