...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "guard in which assertion A is evaluated and produces its own guard-defined result. The guard defines the observable predicate for A: an A-pass result witnesses the property "
        "present, an A-fail result witnesses it absent — so there is no separate model claim that a state 'violates' or 'satisfies' the property; the guard is the measurement "
        "mechanism and its A result is the observation. The failure (A-fail) execution must immediately precede the 'Failure:' line (it is the producer); the passing (A-pass) "
        "execution is the contrast evidence and may appear elsewhere in the transcript. Any execution that does not produce an A result is not an observation of A and cannot witness "
        "A, regardless of the cause or form of the execution's outcome — this covers execution errors, unrelated assertions failing, a harness that exits without reporting A, and any "
        "other run in which A itself was not evaluated; only an A-fail result can supply the violating observation, and only an A-pass result the satisfying contrast. Both "
        "A-observations must be produced by the committed guard artifact, and the guard's output must identify A (by test name, subtest path, or assertion message) so the pair is "
        "bound to this enumerated assertion rather than another. Provenance closure: any transcript reference that functions as evidence — the assertion identity, the A-fail result, "
        "and the A-pass result — must quote verbatim the text of the qualifying execution record that establishes it, and counts as evidence only if that exact text occurs in a "
        "referenced qualifying execution record; a result or identity typed as prose that does not appear verbatim in a cited qualifying execution record is not evidence and does not "
        "witness A. If the artifact already passes A, construct a temporary modification that makes A alone fail while every other assertion still passes, execute, emit the Failure "
        'from that tool-result, then restore; a modification that fails every assertion at once discriminates none. Emit \'Failure: assertion "<verbatim assertion>" — <A-fail result> '
        "/ <A-pass result>' quoting both observed results from their tool-results; a 'witness:' line, if added, only projects those two results and is not itself evidence. A "
        "structural assertion — one whose property has the artifact's own text or structure as its primary subject, so no behavioral failing execution exists — is witnessed instead by "
        "'Unobservable: assertion \"<verbatim assertion>\" — structural'; Unobservable is never satisfied by having searched for a failing execution and found none. An assertion that "
        "is behavioral yet no admissible modification can make fail is a guard defect: strengthen the guard and repeat. A guard is a blind spot whenever an admissible state exists in "
        "which its pass-or-fail outcome is not equivalent to whether the property holds — check three: (a) proxy or non-exercising guard checking a surrogate; (b) durability — a guard "
        "gone at the moment it would need to fire; (c) ephemeral guard not committed to a location that re-runs. Verdict-follows-execution governs both gates: when tool calls are "
        "available, every 'Overreach:' verdict and every 'Failure:' observation is valid only when it immediately follows a tool-result block that mechanically produces it; a verdict "
        "emitted from description or analysis alone does not satisfy this token. A structural Unobservable assertion is exempt, having no execution to anchor to. 'Coverage: complete' "
        "is valid only when every enumerated assertion has either a 'Failure:' line backed by its A-fail/A-pass execution pair or an 'Unobservable: … — structural' line, and no "
        "'Overreach: found' remains open; an 'Unwitnessed: … — execution unavailable' assertion leaves coverage incomplete and does not satisfy this token. When qualifying execution "
        "evidence is unavailable for one or more behavioral assertions, the honest terminal state is 'Coverage: incomplete — execution unavailable', not a claimed completion; the "
        "protocol may construct guards, perturbations, and expected outcomes and identify what remains unverified, but it may not represent any execution-dependent verdict as "
        "observed.",
