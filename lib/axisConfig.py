...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "script) with test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior; create STUB implementation files for EVERY component planned — STUB means "
        "EMPTY function that returns null; tests are EMPTY and pass trivially; this rung establishes that scripts can run and all components have EMPTY stubs. Assertions are written at "
        "the assertion rung, not validation. Scripts MUST PASS at validation rung — FAILING scripts means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. "
        "Assertion rung: EDIT existing script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL (red). NO implementation in this rung. AFTER "
        "check turns green (implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ Assertion complete (staying in assertion rung), THEN add another "
        "assertion for the same gap; If gap IS closed: emit ✅ Thread N complete and move to next gap; NO file creation in assertion rung — stubs created at validation rung; only edit "
        "existing test file to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one rung at a time; completion "
        ...
        "completing implementation, CHECK if gap is closed — If gap still exists: FIRST emit ✅ Implementation complete (return UP to assertion rung to add next assertion); If gap IS "
        "closed: emit ✅ Thread N complete. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. ASSERTIONS are test "
