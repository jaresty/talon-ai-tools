...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "satisfy rung gates regardless of accuracy; the tool call(s) at a rung must produce output whose artifact type matches that rung’s definition — output from a different rung’s "
        "artifact type does not satisfy the gate regardless of what it implies (running the test suite at the observed running behavior rung produces validation-run-observation-type "
        "output, not observed-running-behavior-type output, and does not satisfy that gate). Every 🔴 Execution observed: sentinel at any rung must contain verbatim tool-call output, "
        "triple-backtick delimited, complete, nothing omitted — prose descriptions, inline summaries, and model-generated text that resembles output do not satisfy this requirement at "
        "any rung. Rule 2: each artifact derives from the prior rung — form changes, intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap "
        ...
        "leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. The prose rung must be re-emitted at the start "
        "of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. From "
        "the criteria rung onward, the gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is "
        "valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — do not emit 🔴 Gap:. Each rung’s artifact addresses only "
        ...
        "Display Name’ and ‘the search bar filters results’ are two criteria, not one); further artifacts emerge from subsequent gaps after each cycle completes. Formal notation "
        "encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only validation artifacts may be produced at the executable "
        "validation rung — no other content is permitted before ✅ Validation artifact V complete; the artifact must be invocable by an automated tool; before writing the test, re-read "
        "the formal notation and identify each structural constraint it encodes — value, position, ordering, co-presence — the test must assert each of those constraints, not only the "
        "criterion’s surface text; each test function contains exactly one assertion — one assertion per test function — this enforces per-assertion red witness structurally: a "
        "single-assertion function that passes before any edit is unambiguously vacuous; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be "
        ...
        "when no existing file covers the concern or when the concern warrants explicit isolation; before emitting ✅ Validation artifact V complete, confirm via tool call that the "
        ...
        "within the project’s version-controlled file tree — artifacts written to /tmp or outside the repository do not satisfy this rung; if the artifact asserts static state (file "
        "contents, schema structure, attribute presence, file existence, or any property checkable without executing the implementation), run it against the current codebase before any "
        "edit — if it passes, the artifact is vacuous and must be rewritten with a target that is currently absent; if the artifact asserts runtime behavior, perturb the implementation "
        "to force a red run — if the artifact still passes after perturbation, the test is vacuous and must be rewritten; if the artifact goes red after perturbation, restore the "
        "perturbation and return to the gap rung. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is "
        "written. Before writing the validation run observation rung label, invoke the validation artifact via a tool call — the rung label may not be written until tool output exists "
        "in this conversation for this thread; predicting what the run would show does not satisfy this gate regardless of accuracy; a green 🔴 Execution observed: at the validation run "
        "observation rung is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared in the current cycle showing the test logic "
        "failing — a harness error (import failure, syntax error, missing file) is not a red run; the test must have been loadable, its assertions must have run, and they must have "
        "failed. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — "
        "do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token in the response — any "
        "other content before the criteria label is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation "
        "before any further edit. Upward returns follow the failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did not model "
        "the criterion, return to formal notation; observation cannot be produced is a prose-description failure, not an implementation failure — return to the prose rung and update "
        "it. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upon writing the observed running behavior label, re-emit the criterion "
        "for this cycle, then immediately invoke the implemented artifact via a tool call and emit 🔴 Execution observed: with its verbatim output — the criterion re-emission makes "
        "satisfaction locally auditable: the verbatim output must directly demonstrate the specific behavior named in the criterion; the verbatim output at this rung must be what a "
        "non-technical observer of the running system would see — rendered DOM content, an API response body, CLI output, or equivalent direct artifact output; process state output "
        "(server started, build succeeded, exit code 0) does not satisfy this rung unless the criterion is specifically about process state; if the output does not directly demonstrate "
        "the criterion, emit the gap and return upward; invoke the implemented artifact directly — the binary, command, script, or rendered output produced by the implementation as a "
        "user or caller would observe it — not the test suite; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that "
        "demonstrates the declared behavior; implementation edits, new files, and code changes are not permitted at this rung — if observation reveals a defect, emit the gap and return "
        "upward. Before emitting ✅ Thread N complete, verify that the observed running behavior output directly demonstrates each criterion declared in this thread; if any criterion "
        "has not been directly demonstrated, invoke the artifact again targeting that criterion first. ✅ Thread N complete may not be emitted unless the observed running behavior label "
        "has been written in the current cycle and a 🔴 Execution observed: sentinel appears immediately after it; once ✅ Thread N complete is emitted no further output in that cycle is "
        "valid. For each sentence in the prose that contains a behavioral predicate (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or any "
        "similar action verb), check that a completed gap cycle exists whose criterion directly names that behavior — implicit or incidental coverage does not satisfy this check; if "
        "any behavioral sentence has no directly-named cycle, emit a new 🔴 Gap: for it before emitting ✅ Thread N complete. Outputting a rung label is what begins that rung — it is not "
        "a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. When beginning mid-ladder, locate the highest "
        ...
        "validation run observation to have fired for the current gap, including after an upward return. Reconciliation gate: at every upward rung transition — whether the scheduled "
        "formal-notation → executable-validation gate or any upward return triggered by new information — reconcile all artifacts at that rung against I before redescending; if prose "
        "has evolved on this upward return, any pre-existing document that records I (ADR, README, spec) must be updated to match the evolved prose before redescending — this is a gate "
        "precondition, not a final-report step; documentation is a reconciliation artifact, not a thread. Each completion sentinel is valid only at its defining rung — emitting a "
        "sentinel outside the rung where it is defined is a protocol violation; a sentinel used as a narrative summary or wrap-up is not a valid emission. After emitting ✅ Manifest "
        "exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order; the final report copies artifacts already emitted in the thread "
        "rungs — no new behavioral claims, no coverage summaries, no suggestions may appear in the final report; then reconcile any documents (README, specs, ADRs) the implementation "
