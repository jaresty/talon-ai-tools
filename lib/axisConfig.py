...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "following recursive decomposition procedure to every retained property. Initially, the candidate retained properties are the property [N] entries in the Ground properties "
        "block; a property enters the retained set only after canonicalization and scope validation. Every property that can enter the retained set — including each initial property "
        "[N], each property added during the completeness phase, and each property produced by a merge — must not constrain an internal mechanism, architecture, algorithm, or "
        "representation as its primary subject unless the chosen interpretation explicitly requires that subject; a property that constrains an internal subject without that explicit "
        "requirement is out of scope and must not appear in the retained set. Externally observable status is evaluated independently of internal-subject status and is not itself a "
        "gate for retained-set entry. Scope validation is applied to the canonical definition produced by Step 1, not to the provisional surface form. A property is atomic when the "
        "recursive decomposition procedure reaches Step 4 and no valid split exists. Step 1 — Canonicalize the property. Emit: '§ formality check: property [N]: \"<verbatim "
        "definition>\" — [formal | informal: <reason>]'. If informal, rewrite as a typed mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, and "
        "emit '§ rewritten: property [N]: \"<new definition>\"', then repeat the formality check on the rewritten definition. Continue until the formality check concludes formal. The "
        "definition confirmed formal in the final check is the canonical definition for this property — no subsequent step may quote a definition not yet confirmed formal. After "
        "canonicalization, verify that the chosen interpretation has established an accessible-observation boundary. If it has not, emit 'Interpretation refinement required: <what the "
        "boundary must specify>' and refine the Interpretation: line before evaluating scope. Any interpretation refinement at this point voids the current canonical definition and all "
        "downstream scope, split, and retained-set state for this and any previously processed property under the prior interpretation, and restarts property derivation from the "
        "request constraints under the refined interpretation. Then verify scope: if the canonical definition constrains an internal mechanism, architecture, algorithm, or "
        "representation as its primary subject without that subject being explicitly required by the chosen interpretation, reject the property as out of scope and do not retain it. "
        "Externally observable status is evaluated independently of internal-subject status; a property is not rejected merely because its subject is inaccessible, only because its "
        "primary subject is an internal mechanism not required by the interpretation. Step 2 — Introduce provisional sub-properties. Introduce provisional candidate sub-properties "
        "whose conjunction is proposed to represent the current property: 'property [Na]: <definition>' and 'property [Nb]: <definition>'. Apply the canonicalization procedure (Step 1) "
        "to each provisional sub-property before proceeding. The resulting canonical definitions replace the provisional definitions for all subsequent split evaluation. The labels "
        "[Na] and [Nb] continue to identify the resulting canonical definitions after replacement. Split evidence is attached to the canonical definitions of the current property and "
        "its provisional sub-properties, and to the falsifying constructions emitted in the split test, at the moment the split test is emitted. Any modification to any of those "
        "definitions or constructions after the split test is emitted voids that evidence; the split evidence must then be regenerated against the current canonical state before the "
        "split may be accepted. Step 3 — Evaluate split validity. Before evaluating the split, verify that the chosen interpretation determines an accessible-observation boundary: the "
        "set of outputs, behaviors, states, and interactions that the party responsible for determining whether the request was satisfied can access without inspecting the "
        "implementation. If the chosen interpretation does not determine that boundary, emit 'Interpretation refinement required: <what the boundary must specify>' and emit or refine "
        "the Interpretation: line to supply it. An interpretation refinement voids the entire current retained-property tree, all previously derived properties, and all split evidence, "
        "and restarts property derivation and recursive decomposition from the request constraints under the refined interpretation; any previously emitted split test is void and does "
        "not constitute a completed split test for the current canonical state. After the accessible-observation boundary has been established and the current property, its provisional "
        "sub-properties, and their split evidence have been regenerated under the current interpretation, emit exactly one non-void split test for the current canonical state: '§ split "
        'test: property [N]: "<verbatim canonical definition>" ⇔ (property [Na]: "<verbatim canonical definition>" ∧ property [Nb]: "<verbatim canonical definition>"); property [Na] — '
        "falsified by <construction>; property [Nb] — falsified by <construction>; these falsifiers are independent.' The quoted definitions for [N], [Na], and [Nb] must match the "
        "verbatim canonical definitions confirmed in Step 1 for each respective property. A split may be accepted only if its split test, quoted definitions, falsifying constructions, "
        "and interpretation are identical to the non-void state against which the split test was emitted. A split succeeds only if: for every admissible construction x, P(x) ⇔ (PNa(x) "
        ...
        "redundant and must be removed or merged, provided doing so does not reduce coverage of the explicit request constraints. A merge replaces its source properties with the newly "
        "derived merged property; the source properties are removed from the retained set before the merged property is evaluated for retention. The merged property is a new property "
        "and must enter the recursive decomposition procedure (Steps 1–4) as a newly derived property before being added to the retained set. If a merge or removal changes the retained "
        "property set, the current completeness and independence results are void; re-run the recursive decomposition procedure for every newly introduced property, then re-establish "
        "completeness and observational independence over the resulting retained set. The Ground properties block is complete only when all retained properties simultaneously satisfy: "
        "they are derived from the chosen interpretation; they are atomic; their conjunction logically entails every explicit request constraint; they are observationally independent; "
        "they introduce no out-of-scope constraints, including internal mechanism constraints not explicitly required by the chosen interpretation. When all conditions are met, emit '§ "
