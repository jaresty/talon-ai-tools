...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "run, naming its speed tier; second, name at least one insufficient check — a faster or lighter check that would not catch this behavior — and quote the specific output from a "
        "prior run result showing the insufficient check passes when the behavior is absent; if no such insufficient check output exists above this line in the transcript, the "
