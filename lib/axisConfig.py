...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "output that must be thrown away, leaving the agent further from completion than if it had followed the ladder. Known bypass patterns (all invalid): (1) Treating implementation "
        "as validation — writing tests and running them after implementing is not the validation run observation rung; the validation rung must produce a failing artifact before any "
        "implementation exists. (2) Composing an 'Execution observed' sentinel from anticipated output, test structure, or prior knowledge rather than from an actual tool call result "
        "in the conversation. (3) Recognizing a violation only after producing the invalid artifact — post-hoc recognition does not retroactively validate the output; the artifact must "
        "be discarded and the ladder re-entered from the correct rung. Execution gate (R3): one principle governs all gates — every rung advancement requires an externally received "
        "result; nothing self-produced by the model (planning text, anticipated output, composed code blocks, or inline expansions) counts as permission to advance. Before descending "
        "to an executable rung, output '🔴 Execution observed: [verbatim tool output — content composed without running the tool is invalid]' then '🔴 Gap: [what the verbatim output "
        "reveals]' on their own lines; non-executable rung observations appear inline as prose — the 🔴 sentinel format is reserved exclusively for executable rung gates and must not be "
        "used as a general observation marker, gap-discovery record, or preamble; place sentinels immediately before the executable rung artifact they gate, not at the manifest level "
        "or in any earlier position — these sentinels apply only to executable rungs and gate the immediately following executable rung only; they do not permit skipping intervening "
        "rungs; a rung manifest, verbal description, or code block containing a command and its expected output is never a valid Execution observed — only output received as a distinct "
        "tool call result in the conversation satisfies R3; for executable rungs, observation means invoking the artifact as a running process and recording the terminal output — "
        "required regardless of whether tests pass or fail; if tests do not run, fix the rung before descending; no implementation artifact — including planning text, code blocks, or "
        "tool calls — may appear before valid Execution observed + Gap sentinels; if any implementation content appears without these sentinels immediately preceding it, it is invalid "
        "and must be discarded before the tool is run. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; stopping mid-thread is only "
        "permitted when the next rung is not achievable (the domain provides no standard artifact type for it); the active completeness token governs how many threads are batched per "
        "descent; when a thread reaches observed running behavior, return to the top for the next thread. Ground applies regardless of perceived task complexity — the rung manifest and "
        "all execution gates are mandatory even for tasks that appear straightforward; other constraint tokens operate within each rung of the process, not on the process structure "
        "itself — they govern depth, scope, and style at each step but cannot eliminate steps or the manifest; completeness tokens govern how deeply each rung is executed — a minimal "
        "completeness token produces a shallower rung, not a skipped one. When a rung reveals the rung above needs correction — a constraint conflict, a pivot, or new information — "
        "correct the rung above to restore faithfulness to I, then re-derive the current rung; this is correction, not enrichment; if the rung requiring correction is I, surface as a "
        "revision signal and stop the current thread. When beginning mid-ladder, first locate the highest already-instantiated rung and update it to reflect the intended change, then "
        "descend. For code contexts, each rung in this sequence may not be skipped or combined with another — the executable implementation rung is blocked until the validation run "
        "observation rung has declared a gap; announcing two rungs together or labeling one rung with another's name does not satisfy either. R4 instantiates as: prose (natural "
        "language description of intent and constraints) → criteria (acceptance conditions as plain statements) → formal notation (non-executable specification — type signatures, "
        "schemas, pseudocode, or contracts; may use code syntax to describe interfaces but the artifact cannot be run as written) → executable validation (a file artifact invocable by "
        "an automated tool — go test, pytest, or equivalent — written to target the declared gap and reveal whether it has been closed; only validation artifacts may be produced at "
        ...
        "executable validation regardless of label; pre-existing artifacts not targeting the gap do not satisfy this rung) → [GATE: validation run observation — executable "
        "implementation is blocked until this rung has been executed and declared a gap; before producing implementation code, output '🟢 Implementation gate cleared — gap cited: [quote "
        "the gap from the validation run observation verbatim]' on its own line; this sentinel requires verbatim citation from the actual conversation — a gap composed from anticipated "
        "output, test structure, or prior knowledge is invalid; if no validation run observation exists for this thread in the conversation, the gate is not open and this sentinel may "
        "not be produced] → executable implementation → observed running behavior. (Validation run observation records the actual output of the executable validation rung and declares "
        "the gap — tests must reveal the gap — tests covering behavior that does not yet exist must fail; if all tests pass before new implementation has been written, either the "
        "intended behavior is already implemented (verify this; the thread may be complete) or the tests are not testing the intended behavior and must be corrected; the executable "
        "implementation rung is blocked until the gap has been observed; if tests skip, fix the executable validation rung first.) When the lowest V is complete, output '✅ Validation "
        "artifact V complete' on its own line before producing O — this phrase may only appear after the executable validation rung has been both produced and invoked and the "
        "validation run observation rung has declared a gap; it may not be placed after planning, criteria, file exploration, or any non-executable rung alone. Non-executable rungs "
        "appear inline as labeled log entries; executable rungs are placed where they can be executed — in an existing file where a natural home exists, or a new file where none does.",
