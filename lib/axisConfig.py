...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "while the property is absent, or the guard fails or is absent while the property holds. Two named constructions must be checked as instances of this: (a) proxy or "
        ...
        "unchanged as a found blind spot. emit 'Test blind-spot: found — <construction>' or 'Test blind-spot: not found'; if found, strengthen the guard and repeat from step (2); (4) "
        "the text immediately following 'Test blind-spot: not found' must be the tool call that executes the quoted guard against a deliberately constructed violating state — no "
        "narrative, planning, implementation change, 'Quoted implementation:', 'Implementation overreach:', or other sentinel may appear between 'Test blind-spot: not found' and that "
        "execution tool call; no transcript content may appear between the execution tool call and the corresponding tool-result block — the execution tool call and tool result are an "
        "atomic pair; execute the quoted guard against a deliberately constructed violating state — if the governed artifact already satisfies property [N], first construct a minimal "
        "temporary modification to the artifact that violates property [N], execute the guard against that temporary state, emit the required Failure: line from the tool-result block, "
        "then restore the original state before continuing; the text immediately following the execution tool-result block must be the Failure: line for property [N] — no narrative, "
        "causal argument, prose explanation, reference to a prior failure, or other content may appear between the tool-result block and the Failure: line; a Failure: line that does "
        'not immediately follow a tool-result block does not satisfy this token; emit \'Failure: property [N] "<verbatim property definition>" — <verbatim quoted failure line from a '
        "tool-result block>'; emit 'Unobservable: property [N] \"<verbatim property definition>\" — <reason>' only after demonstrating that no temporary violating state can be "
        "constructed within the governed artifact on which the guard could distinguish satisfaction from violation — Unobservable: is the last resort, not the first; successful "
        "compilation, build output, or unrelated verification does not satisfy this step — the violating state must exist solely to witness that the guard detects the absence of "
        "property [N]; before modifying the governed artifact to satisfy property [N], a verbatim Failure line from a tool-result block must appear; if the failure line does not "
        "specifically witness the violation of property [N], modify the violating state and repeat from step (4); before emitting any witness classification, determine whether every "
        "inventoried assertion can be classified mechanically from the immediately preceding execution tool-result block — a witness is mechanically derivable iff the tool-result (i) "
        "identifies exactly one assertion, (ii) identifies its outcome (pass or fail), and (iii) no other assertion satisfies the same identifier; if any witness is not mechanically "
        "derivable by this operational definition, temporarily instrument the regression guard or construct an equivalent observation harness that executes exactly the inventoried "
        "assertions without strengthening, weakening, removing, or reordering them, while emitting observable per-assertion outcomes; before executing the harness, emit 'Observation "
        "correspondence: assertion [M] -> <observation point>' for every inventoried assertion, establishing a declared one-to-one mapping from every inventoried assertion to its "
        "corresponding observation point; if a one-to-one mapping cannot be established, revise the harness; execute the instrumented guard or harness against the same violating state "
        "via a tool call and wait for the tool-result block; after execution, verify that every declared observation point produced exactly one observation in the tool-result — if any "
        "observation point produced zero or more than one observation, emit 'Observation correspondence: incomplete — <reason>', revise the harness, and repeat from step (4); also "
        "verify that every assertion observed in the tool-result output appears in the 'Observation correspondence:' mapping — if any observed assertion is absent, the harness is "
        "non-equivalent and must be revised; derive every witness classification exclusively from the verified tool-result; the temporary instrumentation exists solely to observe "
        "execution and must not become part of the regression guard or governed artifact — discard it after witness generation; the text immediately following the Failure: line (or "
        "the observation harness tool-result block if instrumentation was required) must be the first witness line — no narrative, causal argument, prose explanation, or other content "
        "may appear before the first 'Property [N] assertion [M] witness:' line; emit exactly one witness line for every previously inventoried executable assertion, in ascending "
        "assertion inventory order without omission or reordering; each witness line takes one of these forms, derived exclusively from the verified tool-result block: 'Property [N] "
        'assertion [M] witness: Failure — assertion: "<verbatim assertion text>" — observed: "<verbatim tool-result text uniquely identifying this assertion as the failure by '
        "assertion text, source location, or execution identifier>\"' if and only if the tool-result uniquely identifies this assertion as a failing outcome — when identical assertion "
        "texts appear more than once, the failure must identify the specific occurrence by source location or execution identifier; 'Property [N] assertion [M] witness: Success — "
        'assertion: "<verbatim assertion text>" — observed: "<verbatim tool-result text confirming this assertion passed>"\' if the tool-result contains a directly observed passing '
        "outcome for this assertion; if an assertion cannot be classified Success or Failure from the current tool-result, it is not yet observable — before accepting that conclusion, "
        "determine whether it is due to dead code, unreachability, mis-inventory, or harness omission; refine the observation harness or violating state to resolve the cause and "
        "repeat the instrumented execution; only after no executable instrumentation or violating state modification can make the assertion observable may 'Property [N] assertion [M] "
        'witness: Unobservable — assertion: "<verbatim assertion text>" — <observed reason quoted from tool-result>\' be emitted; Unobservable is the last resort and must be justified '
        "by a verbatim tool-result excerpt; an Unobservable witness is treated as a guard defect — revise the guard to make the assertion observable and repeat from step (2); no "
        "assertion may permanently carry Unobservable status; after all witness lines are emitted, verify set equality between the assertion inventory and observed assertions: (i) "
        "every assertion observed in the harness or execution output must appear exactly once in the assertion inventory — if any observed assertion is absent from the inventory, emit "
        "'Assertion inventory: incomplete — missing assertion: <text>' and repeat from step (2); (ii) every inventoried assertion must appear exactly once in the observation output — "
        "if any inventoried assertion is absent from the observation output, treat it as unresolved and refine the harness or violating state and repeat the instrumented execution; at "
        "least one inventoried assertion must receive witness classification Failure — a guard execution in which no inventoried assertion is classified Failure does not satisfy step "
        "(4), regardless of whether a Failure: line was produced; a Failure witness whose observed text is not present verbatim in the corresponding tool-result block does not satisfy "
        "this token; if no inventoried assertion can be attributed to the observed failure, revise the violating state or the regression guard and repeat from step (2); after all "
