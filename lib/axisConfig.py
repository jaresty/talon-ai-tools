...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "absence; (b) the literal string signaling the governed behavior is present; (c) the literal string identifying each governed behavior in an executed-artifact result — (c) "
        "must be derived from a direct callee of the assertion call expression as it appears on the FAIL line; a value derived from a test-identifier echo line, non-FAIL line, summary "
        "line, or test description in executor output is not a valid (c); (d) the governed symbol: the literal string naming the governed behavior — (d) must be derived from the "
        "direct callee of the assertion call expression in the governing artifact, where the call expression is not used only as an argument to a call expression whose callee does not "
        "contain the governed symbol name; (d) is the literal string which must appear as a substring of the failure line in (g); (e) the layer classification — if the governed symbol "
        "name appears as a token in a call expression in the test body — where the call expression is not nested inside another call expression that names a different symbol — the "
        "layer is 'unit'; if the governed symbol name does not appear in the test body, the layer is 'integration'; (f) the executor class — the executor must produce output that "
        ...
        "executor invocation — non-executor actions do not break immediacy. Artifact identifier must differ: the artifact identifier of the named governing artifact must differ from "
        "the artifact identifier of the artifact being modified. Semantically-equivalent refactor exception: (g) is also satisfied when an executor result block containing (a) before "
        "(c) appears in the transcript before the governed artifact-producing action, produced by a named executor invocation occurring in the current session after the last governed "
        "artifact-producing action, against a state where the refactored form was modified to violate the invariant the governing assertion checks — the perturbation result must "
        "appear before the revert action. Revert is complete when the pre-perturbation executor run result is quoted verbatim in the transcript before the revert executor invocation, "
        "and a subsequent executor invocation produces (b) at the position where (a) was absent. Per-behavior scoping: a governed artifact-producing action is compliant when (g) is "
        "satisfied for the behavior named in the Falsify derivation block's (d) entry — the behavior name is fixed at derivation time and may not be restated or narrowed after (g) is "
        "observed; the action is not required to implement any behavior for which (g) has not yet been established; unguarded behaviors require separate governing FAILs in subsequent "
        "steps and must not be treated as a blocking condition on the current action. Implementation scope constraint: a governed artifact-producing action may introduce only "
        "constructs whose complete identifier — the full name as it appears in the governed artifact, not any component or prefix — appears as a verbatim substring of (c), (d), or (g) "
        "in the Falsify derivation block active at the time of the action; a construct is introduced when its identifier appears in the governed artifact in any position — defined, "
        "declared, imported, or referenced — that was absent before the action; the governing artifact's content is not a valid source for the allow-list; the allow-list is derived "
        "exclusively from the executor-produced result entries (c), (d), and (g); the allow-list is derived from the full content of the (g) tool-result block named in the active "
        "Falsify derivation block; an identifier that does not appear as a verbatim substring of that tool-result block may not be introduced; test description lines, docstring echo "
        "lines, parametrize label lines, and summary lines in that block are valid allow-list sources — the restriction on these line types applies only to (c) derivation, not to "
