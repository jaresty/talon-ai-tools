...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "requirement; the symbol commitment must appear as a line matching `symbol: <identifier>` where identifier matches `[A-Za-z_][A-Za-z0-9_]*` (no spaces); the named symbol must "
        "appear as a literal substring of the quoted scope text or of the immediately preceding run result — a symbol not present in either does not satisfy this requirement; a symbol "
        "is a named member within a container — it names an entity whose existence is established within the container, not the container itself; a commitment naming the container (the "
        "file, module, or resource that would be created or modified as a whole) does not satisfy this requirement; when the symbol name does not appear as a substring of the "
        "immediately preceding run result, the symbol is new; a new-symbol call is compliant only if the introduced symbol satisfies the stub requirement for its kind — for a callable "
        "(function, method, or constructor), the body contains no logic: an empty return or a language-idiomatic zero-value return for the declared return type; for a non-callable "
        "(variable, constant, exported value, or class field), the initializer is a single primitive or zero-value expression with no sub-expressions — body/initializer contains only "
        "one of nil, null, undefined, 0, 0.0, false, empty string literal, or empty return, and contains no `(`, `[`, `{`, or operator character; a body or initializer not matching its "
        "kind's stub form does not satisfy this requirement; a statement in the derivation phase is a commitment only if its truth value can be determined from run results already "
        ...
        "regardless of intent — a tool-result block in this transcript containing the FAIL is the only evidence that the artifact encodes the governed behavior. Persistence rationale: "
        "falsify's purpose is ongoing governance and future regression detection, not point-in-time confirmation — a governing artifact must persist in the repository as a named "
        "artifact invocable by the executor on future runs; an ephemeral governing artifact that fires (g) in session but is deleted, written to a temporary path, or otherwise absent "
        "from future executor runs does not satisfy falsify regardless of whether (g) was observed; this is what distinguishes falsify from verify — verify checks a claim now, but "
        "falsify requires evidence that continues detecting regressions after this session ends. The response treats implementation derivation as constrained exclusively to observed "
        "governing failures — the only evidence that a governing artifact encodes the governed behavior is an executed-artifact result (a tool-result block in this transcript) in "
        "which (a) precedes (c) and the failure line contains a substring from the governed assertion call expression. The response establishes, before any governed artifact-producing "
        "action, that the named governing artifact has been observed to detect the absence of each governed behavior. A governed artifact-producing action is any action that produces "
        "or modifies a persistent artifact — including file writes, document edits, configuration changes, and policy commits — whose absence of a governed behavior can be detected by "
        "a named executor invocation; an action whose output does not persist beyond the current session, or for which no named executor exists, is not a governed artifact-producing "
        "action. A named governing artifact is one whose file path — the path passed as an argument to the executor invocation, not a package name or directory — appears as a literal "
        "string in the named executor invocation that produces a result from it; an inline inspection (grep, curl, read, shell pipeline, verbal check) whose output is not produced by "
        "invoking a named artifact does not satisfy this requirement regardless of whether it produces a failure result. If no governing artifact exists for a governed behavior, "
        "writing that artifact is the first governed artifact-producing action; the assertion text and the governed symbol must reside in separate artifacts — a governed "
        "artifact-producing action that produces both assertion text and the governed symbol in the same action does not satisfy the creation-step exception for either; (a) through "
        "(g) apply to the named executor invocation immediately following that creation — the creation action itself satisfies the creation-step exception below. Derivation block "
        "temporal gate: the literal string named in (c) must appear as a verbatim substring of at least one tool-result block before the Falsify derivation block in the transcript; a "
        "(c) value that does not appear as a verbatim substring of any tool-result block preceding it does not satisfy the derivation phase regardless of whether the derivation block "
        "is structurally complete. Derivation phase (required before any governed artifact-producing action — complete only when a literal block labeled 'Falsify derivation:' "
        "containing entries for (a) through (g) and (f2) appears in the transcript; prose does not satisfy this requirement): Before naming (a) and (c), identify the failure line: the "
        "single line in the executor output that the executor produced by transforming the artifact's structure — interpreting identifiers, resolving symbols, or computing results — "
        "rather than by echoing content supplied as a string literal argument to an output-producing call; this is the line from which both (a) and (c) are read; a line whose content "
        "the executor produced by echoing a string literal argument is not the failure line regardless of how much failure information it contains. Name — (a) the assertion-failure "
        "marker: the substring of the failure line that signals the artifact detected behavior absence; specifically, a string that the executor produces when running the named "
        "executor against a state where the governed artifact has not yet been created; a string that requires the governed artifact to have executed in order to appear in the "
        "executor output is not a valid (a); (b) the literal string signaling the governed behavior is present; (c) the literal string identifying the governed behavior — (c) is read "
        "from the failure line, not chosen independently; to derive (c), first identify the failure line in (g): the line on which (a) appears; then read (c) as the substring of that "
        "line that identifies the governed behavior; (c) must be derived from a direct callee of the assertion call expression as it appears on that line; the governed symbol must "
        "appear as a call expression reachable from the assertion call expression by following the execution call chain — the chain may pass through the test body, test setup, or the "
        "governed artifact; a callee whose name contains the governed symbol name as a substring but through which the governed symbol does not appear as a call expression in the "
        "reachable execution chain is a naming proxy and does not satisfy this requirement; a call expression that satisfies (c) must name a symbol whose callable definition does not "
        "appear in the content of any Write or Edit tool call whose path argument matches the governing artifact file path — if the identifier that appears as (c) on the failure line "
        "also appears as the name of a callable definition in the content of any Write or Edit tool call whose path argument matches the governing artifact, that identifier is a "
        "governing-artifact proxy and does not satisfy (c); (d) the governed symbol: the literal string naming the governed behavior — (d) must be derived from the direct callee of "
        "the assertion call expression in the governing artifact, where the call expression is not used only as an argument to a call expression whose callee does not contain the "
        "governed symbol name; (d) must be the identifier that appears as a substring of the failure line in (g) — not a symbol that contains or is associated with that identifier; if "
        "the failure line names a narrower identifier, that narrower identifier is (d) regardless of which broader symbol was invoked to reach it; (d) is derived from the failure "
        "line, not chosen independently — the failure line names (d), not the other way around. Assertion-target provenance exception: when the following conditions hold in the "
        "governing artifact's source text, the restriction that (d) must appear as a direct callee of the assertion call expression is suspended for this governed call: (1) an "
        "identifier is assigned exactly once in the governing artifact, and its initializer's outermost non-structural callee — after stripping any leading await and trailing "
        "property-access or index-access chains — is the governed symbol; Returned-object invocation provenance: condition (1) also permits a chain of single-assignment identifiers "
        "when each identifier in the chain satisfies conditions (1)–(3) with the immediately preceding identifier in the chain substituting for the governed symbol in condition (1) — "
        "the first identifier in the chain must have the governed symbol as its outermost non-structural callee; each subsequent identifier must have the immediately preceding chain "
        "identifier as the outermost non-structural callee of its initializer after stripping any leading await and trailing property-access or index-access chains; the same "
        "no-reassignment and structural-operations constraints from condition (3) apply to each intermediate identifier; (2) that identifier appears as an argument to an assertion "
        "call expression; (3) the identifier does not appear on the left-hand side of any assignment expression between its initial assignment and the assertion call expression, and "
        "every operation on the identifier between the assignment statement and the assertion call expression is one of: property access, index access, await, grouping, or argument "
        "separator; (4) the executor result block satisfies the base (a) and (c) requirements — no additional requirement that the failure line contain (d) or the chain identifier "
        "applies under this exception. Prose-origin gate: a string does not satisfy any derivation gate — including (a), (c), (d), the allow-list, and the failure line — if it appears "
        "verbatim in the content of a Write or Edit tool call in this transcript that precedes the first executor result block in this transcript containing that string; exceptions: "
        "(1) strings governed by the Bootstrap exception; (2) strings that satisfy the creation-step exception's absent/present condition — where (c) is absent before the Write action "
        "and present after it in executor output immediately following the Write, and (c) appears on a line the executor produced by transforming the artifact's structure — "
        "interpreting identifiers, resolving symbols, or computing results — rather than by echoing content the agent supplied as a string literal argument to an output-producing "
        "call; a string that appears only on lines whose content the executor produced by echoing a string literal argument does not satisfy exception (2) regardless of the "
        "absent/present condition. Bootstrap exception: when no tool-result block in the transcript before the Falsify derivation block contains the governed symbol name as a verbatim "
        "substring, a single governed artifact-producing action may introduce an artifact containing the governed symbol name — this action may not produce a result in which (b) "
        "appears; the named executor must be invoked immediately after this action (no executor result block between the action and the invocation), and the result must contain (a) "
        "before (c) with (d) appearing as a substring of the failure line; no further governed artifact-producing action is permitted until this executor result appears in the "
        "transcript. (e) the layer classification — if the governed symbol name appears as a token in a call expression in the test body — where the call expression is not nested "
        "inside another call expression that names a different symbol — the layer is 'unit'; if the governed symbol name does not appear in the test body, the layer is 'integration'; "
        "(f) the executor class — the executor must produce output that could only exist if the governed assertion reached execution at the layer named in (e); a governing artifact is "
        "disposable if removing the governed symbol from the governed artifact — without modifying the named governing artifact — produces an executor result in which (c) does not "
        "appear; a governing artifact is non-disposable only when (c) appears in the executor result after removal — a disposable artifact does not satisfy (f); (f2) the serialization "
        "gate — classify the assertion target: the value produced by the expression that is the direct argument to the assertion call. A governing assertion satisfies (f2) if and only "
        "if its assertion target is a behavioral value: a value whose content is determined by whether the governed behavior is correctly implemented and that could not be produced by "
        "an arbitrary implementation that merely serializes the governed artifact's output correctly. A governing assertion fails (f2) if its assertion target is a serialization "
        "value: a value produced by converting the governed artifact's output to a string/byte/text representation — regardless of what the conversion expression is named — such that "
        "a correct implementation and an incorrect implementation would produce the same assertion target value if their outputs serialize identically. The Falsify derivation block "
        "must contain the literal line '(f2) serialization gate: [assertion target] — targets: [behavioral property/state/outcome | string/byte/text representation] — gate: [pass | "
        "fail]'; a derivation block that does not contain this line does not satisfy (f2). The creation-step exception does not apply when the derivation block contains '(f2) "
        "serialization gate:' with 'gate: fail'; (g) the observed FAIL result: the content of an executor result block appearing in the transcript after the artifact was applied to "
        "the subject and before the first governed artifact-producing action for this behavior, where (a) precedes (c), the named executor produced the result, and the failure line "
        "contains (d) as a substring. A result satisfies this token if and only if: (g) is present at the required position; (a) is not separated from (c) in (g) by any line whose "
        "content contains (c) for a different governed behavior; and a subsequent executed-artifact result contains (b) at the position where (a) was absent. A result satisfying (g) "
        "must contain a failure line whose content includes a substring from the governed assertion call expression — where the call expression is a syntactic call in the test body "
        "whose callee name contains the governed symbol name as a substring; a failure line that contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) "
        "in its output; a valid (g) failure line is a line on which (a), (c), and a direct callee name containing (d) co-occur — an executor result that contains (a) before (c) but "
        "where no single line satisfies this co-occurrence does not satisfy the creation-step exception. Exception: (g) is not required for the single action that produces the "
        "governing artifact's assertion text, per governed behavior — the creation step is that action only; (g) is required for every governed artifact-producing action in the "
        "current session — a creation step is that action only if (c) is absent before the action and present after it, and a named executor invocation against the governing artifact "
        "immediately following the action produces a tool-executed result containing (c) and not (a); immediacy is defined as: no executor result block appears between that action and "
        "the named executor invocation — non-executor actions do not break immediacy. When (c) for a different behavior of the same governed symbol is already present in the "
        "transcript, the creation-step exception applies per governed behavior — the absent/present condition is evaluated for the specific (c) of the behavior being added, not for "
        "(c) of any other behavior; prior occurrences of (c) for other behaviors of the same governed symbol do not prevent the creation-step exception from applying to a new "
        "behavior. Artifact identifier must differ: the artifact identifier of the named governing artifact must differ from the artifact identifier of the artifact being modified. "
        "Semantically-equivalent refactor exception: (g) is also satisfied when an executor result block containing (a) before (c) appears in the transcript before the governed "
        "artifact-producing action, produced by a named executor invocation occurring in the current session after the last governed artifact-producing action, against a state where "
        "the refactored form was modified to violate the invariant the governing assertion checks — the perturbation result must appear before the revert action. Revert is complete "
        "when the pre-perturbation executor run result is quoted verbatim in the transcript before the revert executor invocation, and a subsequent executor invocation produces (b) at "
        "the position where (a) was absent. Per-behavior scoping: a governed artifact-producing action is compliant when (g) is satisfied for the behavior named in the Falsify "
        "derivation block's (d) entry — the behavior name is fixed at derivation time and may not be restated or narrowed after (g) is observed; the action is not required to "
        "implement any behavior for which (g) has not yet been established; unguarded behaviors require separate governing FAILs in subsequent steps and must not be treated as a "
        "blocking condition on the current action. Implementation scope constraint: a governed artifact-producing action may introduce only constructs whose complete identifier — the "
        "full name as it appears in the governed artifact, not any component or prefix — appears as a verbatim substring of (c), (d), or (g) in the Falsify derivation block active at "
        "the time of the action; a construct is introduced when its identifier appears in the governed artifact in any position — defined, declared, imported, or referenced — that was "
        "absent before the action; the governing artifact's content is not a valid source for the allow-list; the allow-list is derived exclusively from the executor-produced result "
        "entries (c), (d), and (g); the allow-list is derived from the full content of the (g) tool-result block named in the active Falsify derivation block; an identifier that does "
        "not appear as a verbatim substring of that tool-result block may not be introduced; test description lines, docstring echo lines, parametrize label lines, and summary lines "
        "in that block are valid allow-list sources — the restriction on these line types applies only to (c) derivation, not to allow-list sourcing; a construct whose introduction "
        "makes a reachable execution outcome possible that does not appear as a verbatim substring of (g) requires its own governing FAIL before it can be introduced, even if its "
        "identifier appears in (g) — the path constraint is satisfied only when every execution outcome the introduced construct can produce appears as a verbatim substring of the (g) "
        "tool-result block; during the creation-step exception, the allow-list is (d) only; a reference to an identifier not on the allow-list within a stub or partial implementation "
        "is not exempt; a construct whose complete identifier does not appear as a verbatim substring of the active allow-list entries must not be introduced in the same action and "
        "requires its own governing FAIL before it can be introduced. Scope-governing entries: only (c), (d), and allow-list identifiers from (g) are scope-governing entries — entries "
        "(a), (b), (e), (f), and (f2) are not scope-governing entries and do not contribute to the allow-list. Only identifiers that appear on a line in (g) that also contains (a) are "
        "valid allow-list entries — an identifier that appears in (g) only on lines that do not contain (a) is not a valid allow-list entry regardless of whether it appears in "
        "expected values, diff output, assertion messages, test descriptions, or source excerpts. Each line in (g) that contains (a) authorizes at most one new allow-list entry — the "
        "identifier that appears as a substring of that line and also satisfies (d)'s specificity constraint; if no such identifier appears on that line beyond (a) itself, that line "
        "authorizes no new allow-list entry. Imported-contract reference exception: an identifier that appears as a declared member name in the content of a Read tool-result block "
        "appearing before the current Pre-edit block in the transcript is an imported-contract identifier and is not subject to the allow-list source constraint for that action alone; "
        "this exception applies to the allow-list constraint only and does not exempt a branch construct whose condition or guard contains only imported-contract identifiers from the "
        "branch constraint. Artifact gap: when the Falsify derivation block contains the line '(g) observed FAIL result: unavailable' and no tool-result block in this transcript "
        "produced by a named executor invocation contains (c) as a substring, the derivation is incomplete. A derivation is complete only when a line beginning 'Gap artifact:' appears "
        "in the transcript immediately above the Falsify derivation block, naming the executor invocation designated to produce a result containing (a) before (c). When 'Gap "
        "artifact:' is present, the tool call that produces that artifact satisfies the creation-step exception; no file-modifying tool call may appear before the tool call that "
        "produces the named artifact. A governed symbol renamed by a refactor is a new governed symbol requiring a new derivation block before the rename action. If no named executor "
        "exists for the governed artifact type, this token does not apply.",
        ...
        "ground": "The response runs a meta-process protocol that exists because a description of completed work is indistinguishable from actually completing it — every gate enforces the "
        "distinction by requiring a piece of reality before any claim about reality. The protocol is a discipline against self-deception: it prevents 'I think it works' from replacing "
        "'I proved it works.' Trust must come from enforced evidence, not narrative claims. The intent behind any task is an abstract goal outside the system — all artifacts derive "
        "from it, and without a named intent, there is no basis for evaluating whether an artifact satisfies it. Every task must derive a separate meta-task whose artifact is "
        "evaluation of task-to-intent alignment; the meta-task must operate before the first domain action, and the task artifact cannot certify itself. Protocol adherence must be the "
        "lowest-effort valid path — deviation must require additional effort, introduce failure risk, or be blocked. Each derivation-phase rung derives exclusively from the immediately "
        "preceding rung's output — this progressively narrows the interpretation space so that no rung can claim more than its inputs justify; a derivation-phase rung that reaches back "
        "past its immediate predecessor violates this constraint regardless of whether the earlier content is visible in the transcript. Verification phases (## Enforcement sequence, "
        "## Path enumeration, ## Completion check) are explicitly permitted to reference earlier derivation outputs by property number or sentinel name — they are not derivation rungs "
        "and the isolation constraint does not apply to them. Rung isolation enforcement: before writing each derivation-phase rung after §0, write '§ rung rejects: [quoted phrase]' "
        "where [quoted phrase] is a verbatim substring of the immediately preceding rung's output that this rung eliminates as insufficiently precise; a derivation-phase rung whose "
        "first non-blank line does not begin with '§ rung rejects:' does not satisfy the isolation requirement; a '§ rung rejects:' value that does not appear verbatim as a substring "
        "of the immediately preceding rung's output does not satisfy this requirement. When tools are available (§0 observed is present): the behavioral observation — the executed FAIL "
        "result showing the system's actual behavior against the absent governed behavior — is the rung where reality enters the chain; every rung before it is agent-authored artifact, "
        "and every rung after it derives from what that observation actually produced, not from what the agent wrote before it. When tools are unavailable (§0 observed is absent): the "
        "derivation structure itself is the primary artifact — the ladder runs to completion using prose-satisfiable sentinels, and this is a fully legitimate execution mode, not a "
        "degraded one. Memory is not evidence — what a model recalls about a prior step carries the same confabulation risk as any other model output, so only what is visible in the "
        "transcript counts as having happened. A shorter ladder is not more efficient — each collapsed step is ambiguity that was not closed, and ambiguity that was not closed remains "
        "available as an escape route. §0 is a starting gate, not a bypass — description, analysis, and planning tasks run §1–§5 in full after writing '§0: [scenario]'; the declaration "
        "opens the ladder, it does not close it; every rung sentinel is satisfied by the literal string appearing in the response text when no executable artifact is present — no tool "
        "call is required or expected to satisfy any rung when §0 observed is absent. Ladder depth: a ladder is complete only when its final pre-implementation rung contains at least "
        "one expression of the form: a typed function signature, a logical predicate (∀/∃ notation), or a concrete input/output example (f(v) = w or equivalent); other domains use the "
        "equivalent terminal form — a notation where every term has an unambiguous denotation such that two independent agents given only that rung would produce functionally "
        "equivalent outputs. After writing each rung, apply the ambiguity test: quote one phrase from the rung and ask whether that phrase could be satisfied by two non-equivalent "
        "outputs given the governing goal as context — the test is decidable per rung because the governing goal provides a fixed reference that bounds what counts as non-equivalent. "
        "If such a phrase exists, that phrase names the subject of the next rung. The ladder is complete only when the ambiguity test finds no such phrase. A ladder that reaches "
        "implementation before passing the ambiguity test has collapsed a step and must be re-derived. Rung-type homogeneity: rung types must stay homogeneous across the ladder — all "
        "derivation-phase rungs must be successive precision-narrowings of the same governed behavior; introducing a new kind of artifact, deliverable, preflight check, or task "
        "category as a rung is not a narrowing and does not satisfy the ladder. A ladder that begins with intent → criteria → specification and then inserts a preflight check, "
        "implementation step, or presenter-support deliverable as a rung has switched dimensions — those items belong outside the ladder as governed actions following it, not inside it "
        "as rungs. Intent cannot be read directly from a request — every governing goal must be derived by inferring what would satisfy the underlying need from the scenario's subject "
        "matter; derivation means selecting the most precise formulation of what the scenario implies, not adding requirements absent from it; a goal that introduces a new constraint "
        "not implied by the scenario has overreached, and a goal that merely restates the request has not derived anything. The response applies a meta-process discipline before any "
        "implementation action, deriving and enforcing its own correctness conditions as transcript-inspectable strings. A rung is satisfied when and only when a transcript-visible "
        "event matching its definition appears — when §0 observed is present, rungs require tool-executed events and the behavioral observation is where reality enters the chain; when "
        "§0 observed is absent, no executable artifact exists and rungs are satisfied by the literal sentinel string appearing in the response text — this is an acknowledged weaker "
        "form of evidence that enables the protocol to run on non-executable subjects (specifications, contracts, analyses) while preserving the derivation structure; the protocol's "
        "preference for execution over description holds, but does not block the protocol when execution is unavailable; inference, prediction, and prior knowledge do not satisfy rung "
        "gates regardless of accuracy. All compliance headings must be markdown '## ' headings — a line not beginning with '## ' does not satisfy any heading requirement. Invocation "
        "scope: a sentinel written in a prior ground invocation does not satisfy any rung requirement for the current invocation — a sentinel satisfies its rung only when it appears in "
        "the transcript after the most recent '=== TOKENS' section. Each derivation-phase rung (§0 through § properties complete) derives exclusively from the immediately preceding "
        "rung's output; no rung has a fixed step count. A derivation-phase rung that references content from any rung earlier than its immediate predecessor does not satisfy this "
        "requirement — isolation is enforced at each derivation step; backward reference to earlier derivation rungs is not permitted even when the content appears in the transcript. "
        "Verification phases (## Enforcement sequence, ## Path enumeration, ## Completion check) are not derivation rungs — they are permitted and required to reference earlier "
        "derivation outputs by property number or sentinel name. When the execution environment supports literal isolation — a fresh invocation, a separate agent context, or a new "
        "session — prefer actual isolation over in-transcript constraint enforcement. Derivation phase: (0) before writing any task content, write '§0: [scenario]' where [scenario] "
        "names the subject from the conversation — this is the required opening line of the ground derivation and satisfies §0; '§0:' must not appear before the '=== TOKENS' section of "
        "the current invocation; when tools are available in this execution environment, invoke the governing test artifact as a tool call immediately after writing '§0: [scenario]' — "
        "'§0 observed' is required when tools are available and is not optional; after that tool-result block appears and reflects live system execution, immediately write '§0 "
        "observed' — a tool-result block produced by invoking the named artifact as a tool call satisfies this requirement; a result produced by a Read, search, or file-write tool call "
        "does not; an artifact that returns document content does not qualify — a tool result consisting of a GitHub issue body, pull request body, specification document, README, or "
        "requirements text does not satisfy §0 regardless of invocation form; goal-source classification (required when '§0 observed' is present): examine whether any line in the §0 "
        "tool-result block appears verbatim as a substring of any user message visible above §0 in the transcript; if no such line exists, write '§0 status-only'; after '§0 "
        "status-only', fetch a broader context artifact before writing '## Governing goal:' — the fetch tool-result block must contain at least one line that appears as a substring of "
        "a user message above it in the transcript; a tool-result block whose every line is a substring of content already present in the transcript above it does not satisfy this "
        "requirement; (1) derive the governing goal: '## Governing goal: [text]' is valid only after '§0: [scenario]' has appeared earlier in the current response — a '## Governing "
        "goal:' line with no preceding '§0:' line in the current response does not satisfy this requirement; write '## Governing goal: [text]' — if '§0 observed' is present, '[text]' "
        "must appear verbatim as a substring of the §0 tool-result block above; if '§0 observed' is absent, '[text]' is derived from the scenario description — what would satisfy the "
        "underlying need; a '[text]' value that adds a claim absent from the scenario does not satisfy §1; immediately below the governing goal heading, before the means-test, extract "
        "labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', labeled subject, verb, outcome, or invariant; no field may equal '[text]' in full; "
        "no two fields may share a common phrase of 3 or more consecutive words; when every named component of '[text]' is covered, write '§1a check: \"[quoted phrase]\"' where "
        "[quoted phrase] is a verbatim substring of one of the labeled fields written above in this rung — a '§1a check:' line whose quoted phrase does not appear verbatim in the "
        "labeled fields above it does not satisfy this requirement; after a valid '§1a check:' line has appeared, immediately write '§1a decomposed'; immediately below '§1a "
        "decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]' — when '§0 observed' is present, alternative means must not duplicate "
        "approaches already listed in §0; if only one concrete alternative can be named, write '[hypothetical]: [speculated alternative]' before closing §1; if two non-subsuming goal "
        "candidates exist, write both and mark one '[selected]' — the selected goal is the one whose means-test names a more concrete alternative; when ≥2 alternatives (concrete or "
        "hypothetical) are present, write '§1 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the alternative means lines written above in this rung "
        "— a '§1 check:' line whose quoted phrase does not appear verbatim in the means-test above it does not satisfy this requirement; after a valid '§1 check:' line has appeared, "
        "immediately write '§1 goal derived'; immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in the means-test, write one candidate "
        "beginning with '- Candidate:' — a response property that would be absent in a response achieving '[text]' by a different means; when one candidate exists per alternative "
        "means, write '§1b check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the candidate lines written above — a '§1b check:' line whose quoted "
        "phrase does not appear verbatim in the candidate lines above it does not satisfy this requirement; then write '§1b candidates'; '§1b candidates' must not appear before a valid "
        "'§1b check:' line in the transcript; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the transcript; derive at least "
        "two dimensions; each dimension names a property of the response (not an artifact, state, or fix description) such that a response could satisfy it while failing at least one "
        "other listed dimension; each dimension must trace to one item from the §1a decomposition list or one item from the §1b candidates list — a dimension that traces only to "
        "'[text]' as a whole does not satisfy this requirement; each dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose "
        "presence in the response constitutes satisfaction — a valid [observable:] value must contain at least one space (the value must be ≥2 words); a single-word [observable:] value "
        "does not satisfy this requirement; if the response contains no tool-result blocks above §2, write '[observable: prose]' instead; a dimension that names an entity, a fix "
        "outcome, or a UI element is not a valid dimension regardless of provenance; derivation is unbounded: continue until every dimension carries an '[observable:]' tag naming a "
        "literal string or structural pattern whose presence in the response constitutes satisfaction independently of any other listed dimension; when every dimension carries an "
        "'[observable:]' tag, write '§2 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the '[observable:]' tag values written above in this "
        "rung — a '§2 check:' line whose quoted phrase does not appear verbatim in an '[observable:]' tag above it does not satisfy this requirement; then write '§2 dimensions closed'; "
        "'§2 dimensions closed' must not appear before a valid '§2 check:' line in the transcript; (2b) derive formalization: after writing '§2 dimensions closed', immediately write "
        "'## Formalization' — '## Formalization' must not appear before '§2 dimensions closed' in the transcript; the ## Formalization section must contain at least one expression "
        "matching one of: a typed function signature, a logical predicate (∀/∃ notation), or a concrete input/output example (f(v) = w or equivalent); immediately after that "
        "expression, apply the ambiguity test: write '§ ambiguity test:' followed by one quoted phrase from the expression and whether it could be satisfied by two non-equivalent "
        "outputs; if yes, that phrase names the subject of the next rung and ## Formalization must not close; when no phrase in the expression admits two non-equivalent outputs, write "
        "'alternative satisfier:' followed by a space and at least one non-whitespace character that is not '[' — a bracket character as the first non-whitespace character after "
        "'alternative satisfier: ' signals an unfilled template and does not satisfy this requirement; the text names a concrete thing that the expression would admit but that the "
        "governing goal would reject; a formalization that cannot name such a satisfier is not yet precise enough to close; '§ formalization complete' must not appear before '§ "
        "ambiguity test:' in the transcript — a '§ formalization complete' that appears without a preceding '§ ambiguity test:' does not satisfy this requirement; '§ formalization "
        "complete' must not appear before a valid 'alternative satisfier:' line (one whose first non-whitespace character after 'alternative satisfier: ' is not '[') in the transcript "
        "— a '§ formalization complete' with no preceding valid 'alternative satisfier:' line does not satisfy this requirement; after '§ ambiguity test:' and a valid 'alternative "
        "satisfier:' line have appeared, immediately write '§ formalization complete' — '§ formalization complete' must not appear before both '§ ambiguity test:' and a valid "
        "'alternative satisfier:' line have appeared; (2c) derive formalized properties: after '§ formalization complete', derive the notation form for this domain: examine the §2 "
        "dimensions and the governing goal to identify the mathematical notation that best represents the governed behaviors — write '§ notation derived: [form]' where [form] is a "
        "literal notation template containing at least one bracketed placeholder — a substring of the form '[' followed by a label and ']' (e.g. 'f([arg]) → [outcome]', '∀[var]: "
        "[type], [predicate]([var])', 'check([input]) → PASS | FAIL([reason])')); a '§ notation derived:' line whose [form] field contains no '[' character does not satisfy this "
        "requirement; immediately after '§ notation derived:', write the label line 'Formalized properties:' — this block must appear before any artifact, implementation action, or '§ "
        "implementation permitted' line; decompose each §2 dimension into its formal atomic claims; each claim must appear on its own line beginning with the literal text 'property "
        "[N]:' where N is a 1-based integer — a line beginning with '**property' or any markdown-formatted variant does not satisfy this requirement; each 'property [N]:' line must "
        "contain at least one mathematical expression — any notation in which every term has a fixed denotation independent of natural-language context (e.g. ∀/∃ predicates, typed "
        "function signatures, relational expressions, or interface-shape patterns such as check(G) → PASS | FAIL(reason)) — as a literal substring; a 'property [N]:' line containing no "
        "mathematical expression does not satisfy this requirement; after writing each 'property [N]:' line, immediately write '§ split test: [quoted sub-expression]' where [quoted "
        "sub-expression] is a phrase from the parent expression naming a candidate sub-expression to split on, followed by a sentence stating whether splitting at that sub-expression "
        "yields two independently falsifiable sub-properties — a 'property [Na]:' line that does not have a '§ split test:' line above it does not satisfy this requirement; if the "
        "split test finds a valid split, write one sub-property per branch: 'property [Na]:', 'property [Nb]:', etc., each containing a formal expression — a sub-property line whose "
        "content is prose only does not satisfy this requirement; if the split test finds no valid split (the property is already atomic), write 'property [Na]: atomic —' followed by a "
        "restatement of the parent expression in a notation where every term has an unambiguous denotation — a restatement that two independent agents could interpret differently does "
        "not satisfy this requirement; the conjunction of all sub-property expressions for a given 'property [N]:' must be logically equivalent to the expression in 'property [N]:' — "
        "logical equivalence means the set of inputs satisfying all sub-property expressions simultaneously equals the set of inputs satisfying the parent expression; a conjunction "
        "satisfied by a strict subset of the parent's cases (e.g. a single concrete instance substituted for a variable) does not satisfy this requirement; a conjunction satisfied by a "
        "strict superset of the parent's cases (e.g. omitting a condition from the parent) does not satisfy this requirement; when all 'property [N]:' and sub-property lines are "
        "written, write '§ properties check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the 'property [N]:' lines written above — a '§ properties "
        "check:' line whose quoted phrase does not appear verbatim in a 'property [N]:' line above it does not satisfy this requirement; after a valid '§ properties check:' line has "
        "appeared, immediately write '§ properties complete' — '§ properties complete' must not appear before a valid '§ properties check:' line; (3) derive the enforcement sequence: "
        "after '§ properties complete', immediately write '## Enforcement sequence' — '## Enforcement sequence' must not appear before '§ properties complete' in the transcript, and "
        "must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a literal substring what the prior "
        "sub-heading's passing state admits that this sub-heading rejects; when all sub-headings in '## Enforcement sequence' have been written, write '§ enforcement check: "
        "\"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the sub-heading lines written above in this section — a '§ enforcement check:' line whose quoted "
        "phrase does not appear verbatim in a sub-heading line above it does not satisfy this requirement; then write '§ enforcement complete'; '§ enforcement complete' is valid only "
        "after a valid '§ enforcement check:' line has appeared; (4) verify coverage: after '§5 enumeration complete', immediately write '## Completion check' — '## Completion check' "
        "must not appear before '§5 enumeration complete' in the transcript; '## Completion check' must appear as the next heading after '§5 enumeration complete' in the transcript — "
        "the first '## ' heading line appearing after '§5 enumeration complete' must be '## Completion check'; a '## Completion check' appearing anywhere before '§5 enumeration "
        "complete' does not satisfy ground and must not be written — if the response has produced §1–§3 content and wishes to summarize or verify coverage, it must instead continue to "
        "'## Enforcement sequence' and '## Path enumeration' first; '§ properties complete' written before '## Completion check' satisfies the properties gate for the completion check "
        "— do not re-derive or repeat property lines inside '## Completion check'; inside '## Completion check', each covered dimension must cite a 'property [N]' number from the "
        "'Formalized properties:' block and a verbatim string that appears either as a substring of a tool-result block produced after '§ implementation permitted' and visible above "
        "'## Completion check', or, when no such tool-result blocks are present, as a substring of the response text produced in this turn; no dimension citation or '§4 coverage "
        "verified' is valid until '§ properties complete' has appeared in the transcript; '§ test suite complete' is valid only after '§ properties complete' has appeared in the "
        "transcript — a '§ test suite complete' appearing before '§ properties complete' does not satisfy this requirement; '§4 coverage verified' is valid only after '§ test suite "
        "complete' has appeared and every covered dimension cites a property number and such a substring; '## Completion check' must not introduce a dimension absent from '## "
        "Behavioral dimensions' — a dimension named for the first time inside '## Completion check' does not satisfy any coverage requirement and does not extend the governed dimension "
        "set; a turn containing a satisfied 'Gate condition:' block must contain the governed tool call in the same turn — prose appearing between the 'Gate condition:' block and the "
        "governed tool call does not satisfy this requirement; every assistant turn after '§ implementation permitted [N]' has appeared in the transcript that does not contain a line "
        "beginning with '§ implementation permitted [' must contain '## Completion check' — a turn that does not contain a line beginning with '§ implementation permitted [' and does "
        "not contain '## Completion check' does not satisfy ground; '## Completion check' is satisfied only when the literal string '§4 coverage verified' appears within it — a '## "
        "Completion check' block that does not contain '§4 coverage verified' does not satisfy this requirement and the governed turn must continue to the next derived step rather than "
        "ending — a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume phrase requirement; a turn whose final non-blank content line begins "
        "with '§ blocked:', '§ awaiting:', or '§ no-next-action:' is exempt from the resume phrase requirement — the continuation invariant exit string is the terminal constraint for "
        'that turn; when a turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates '
        "still apply\" to proceed under the same protocol. (5) enumerate escape paths: after '§ enforcement complete', immediately write '## Path enumeration' — '## Path "
        "enumeration' must not appear before '§ enforcement complete' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal "
        "identified, dimensions derived, or dimensions covered; for each path, name the literal string whose presence closes it; when no further open path remains, write '§5 check: "
        "\"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the escape path closing strings named in the enumeration above — a '§5 check:' line whose quoted "
        "phrase does not appear verbatim in the path enumeration above it does not satisfy this requirement; then write '§5 enumeration complete'; '§5 enumeration complete' must not "
        "appear before a valid '§5 check:' line in the transcript; '§5 enumeration complete' must not appear before '§ enforcement complete' in the transcript; any referent named in "
        "the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears "
        "exactly once; the string '§ implementation permitted' followed by a space and a decimal integer (e.g. '§ implementation permitted 1') must appear in the transcript after '§5 "
        "enumeration complete'; a sentinel line containing the literal bracket characters '[' or ']' does not satisfy this requirement — only a decimal integer satisfies the index "
        "requirement; the integer must equal the 1-based ordinal count of file-modifying tool calls that will have appeared through and including the tool call immediately following "
        "this sentinel — '§ implementation permitted 1' must precede the first file-modifying tool call, '§ implementation permitted 2' must precede the second, and so on; a checker "
        "verifies this by scanning the transcript for every file-modifying tool call, locating the final non-blank line of the immediately preceding assistant message block, and "
        "confirming it matches '§ implementation permitted N' where N is the call's ordinal position; no file-modifying tool call may appear before the first such sentinel; the permit "
        "sentinel must appear as the final non-blank line of the assistant message block immediately before the assistant message block containing the file-modifying tool call — a "
        "permit sentinel in the same message block as the tool call satisfies this requirement only when it is the final non-blank line before the tool call within that block; § "
        "implementation permitted [N] must appear immediately before the `(i)` line of each file-modifying tool call — no intervening blank lines, prose, tool-call blocks, or other "
        "content may appear between the sentinel and the `(i)` line; no user message appearing between § implementation permitted [N] and the governed tool call satisfies this "
        "requirement — a sentinel written in one turn and a tool call appearing in a later turn after a user message does not satisfy the same-turn anchor; a blank line, horizontal "
        "rule, or any other content between them does not satisfy this requirement; a file-modifying tool call not immediately preceded by assistant text whose final non-blank content "
        "ends with the `(iv)` line of the five-line block does not satisfy this requirement. Quoted-span exclusion: only an occurrence of a named sentinel or heading string on a line "
        "that is not inside a markdown code fence (a span beginning with ``` or ~~~ on its own line) and does not begin with '>' (a markdown block-quote prefix) counts toward "
        "compliance — an occurrence inside a code fence or on a line beginning with '>' does not satisfy any rung or ordering requirement. Sentinel formatting: a sentinel line must "
        "begin with the literal first character of the sentinel string ('§' or '#'); a sentinel line beginning with '**' or any other markdown formatting character does not satisfy "
        "this requirement. Completion check ordering: the '## Completion check' block must appear after '§5 enumeration complete' in the transcript; a '## Completion check' block that "
        "appears before '§5 enumeration complete' does not satisfy this requirement even if its contents are otherwise correct; a response that contains '§5 enumeration complete' and "
        "does not produce a subsequent '## Completion check' block containing 'Formalized properties:', at least one 'property [N]:' line satisfying all property requirements, '§ "
        "properties complete', and '§4 coverage verified' does not satisfy ground. Continuation invariant: when the final non-blank content line of a turn contains or follows one of "
        "the rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage verified', or '§5 enumeration complete', that turn must end with one of: '§ "
        "blocked: <text>', '§ awaiting: <text>', or '§ no-next-action: <text>', where '<text>' is non-empty. A turn whose final non-blank content line contains or follows one of these "
        "strings and does not end with one of the three exit strings does not satisfy ground.",
        ...
        "leaves the same-unit condition unevaluable by transcript inspection alone.",
