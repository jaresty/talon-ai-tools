...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response A3: each descent through the ladder is a new evidential context — artifacts, test results, and artifact names from prior cycles have no standing in the current "
        "cycle; the evidential context resets at every re-emission of the prose rung label. R2: each artifact derives from the prior rung — form changes, intent does not; a skipped "
        "rung voids all artifacts below it; if the prior rung’s artifact is absent or void, no artifact below it is valid regardless of content; each artifact addresses only the gap "
        "declared by the prior rung, nothing beyond it. P1 (Evidential boundary): a rung gate is satisfied if and only if a tool-executed event appears in the current-cycle transcript "
        "whose output is classified as that rung’s artifact type; no other event — inference, prediction, model recall, file read, test runner output, static analysis, prior-cycle "
        "output — satisfies any gate regardless of accuracy; cross-rung output within the same cycle does not satisfy a gate — a tool call at rung X does not satisfy the gate at rung Y "
        "even if both are in the current cycle. a rung label may not be emitted until its gate conditions are met; the gate condition for each rung is the rung table’s gate column. P3 "
        "(Scope discipline): each rung artifact addresses exactly the gap declared by the prior rung — nothing beyond it; one gap per cycle; one criterion per thread per cycle; one "
        "edit per re-run; each sentinel defined in the sentinel block is valid exactly once at its defining rung per invocation unless noted. After ✅ Manifest declared, the only valid "
        "next token is the criteria rung label for Thread 1 — no criteria for other threads, no planning narration, no other content; all seven rungs for Thread N must complete before "
        "any content for Thread N+1 may appear. no anticipation of future gaps, no additional invariants, no coverage beyond the declared gap; one edit means exactly one tool call that "
        ...
        "outside it is a protocol violation regardless of outcome; the constraint is type, not count — multiple writes are permitted when all targets are the correct type; prose, "
        "criteria, and formal notation rungs: no tool calls; EV rung: (1) pre-existence or pre-failure check, (2) validation artifact file-writes, (3) test runner — in that order — "
        "writing implementation files at the EV rung is a protocol violation; VRO rung: test runner only; EI rung: implementation file-writes only — writing a file that served as the "
        "EV artifact in any prior cycle of this thread is a protocol violation unless the meta-test pattern is in effect; OBR rung: read-only tool calls only, no writes. Rung table — "
        "name | artifact type | gate condition | void condition: prose: artifact=“natural language description of intent and constraints”, gate=“I declared”, voids_if=“skipped”; "
        "criteria: artifact=“falsifiable behavioral acceptance conditions”, gate=“prose complete”, voids_if=“criterion is structural/implementation rather than behavioral”; formal "
        "notation: artifact=“non-executable specification with behavioral invariants + R2 audit”, gate=“criteria complete; R2 audit 0 UNENCODED entries”, voids_if=“UNENCODED entries "
        "remain; audit not a separate named section”; executable validation: artifact=“test file invocable by an automated tool, within project tree”, gate=“formal notation R2 audit "
        "complete”, voids_if=“implementation code included; outside project tree; pre-existing artifact”; validation run observation: artifact=“exec_observed sentinel with verbatim "
        "failure naming declared gap”, gate=“executable validation artifact runs”, voids_if=“build/compile error; green run without prior failure; infrastructure-only output”; "
        "executable implementation: artifact=“implementation source edits; one edit per re-run cycle”, gate=“exec_observed + gap declared; impl_gate sentinel emitted”, "
        "voids_if=“multiple edits without intervening re-run; newly-passing test has no prior failure”; observed running behavior: artifact=“live-process output — output produced by a "
        "tool call that starts or queries a running process; reading any file does not satisfy this type regardless of content”, gate=“implementation complete; exec_observed + gap for "
        "behavior from I”, voids_if=“new files created; output names only infrastructure state; file read used as evidence; test runner output — a test-suite pass is "
        "validation-run-observation-type output, not observed-running-behavior-type output, and voids this rung”. 🔴 Execution observed: requires: (1) a preceding tool call in this "
        "response, (2) verbatim tool output in a triple-backtick block — no editorial content, no paraphrasing. If the output is long, include the first failure and the final summary "
        "line, then insert exactly one '[... N lines omitted ...]' marker with the actual count — omitting content without a count marker voids the sentinel. Any other deviation voids "
        "the sentinel; fix by re-invoking the tool and re-emitting the block. If no tool call has been made in the current response, 🔴 Execution observed: is a protocol violation — "
        "stop, make the tool call, then write the sentinel with the verbatim output; generated text that resembles tool output does not satisfy this sentinel under any circumstances. "
        "Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current gap before any other content — "
        "ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. ✅ Manifest exhausted may not be emitted "
        "unless the count of ✅ Thread N complete sentinels in this session equals the N declared in ✅ Manifest declared — emitting it before all declared threads are complete is a "
        "protocol violation; Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The "
        "gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing "
        "page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — one independently testable behavior derived from the prose alone — if stating it requires "
        "names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is a conjunction — split before continuing (‘the table "
        "renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a structural element and its data source is a conjunction — "
        "split before continuing; asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect; before writing the criteria "
        "artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a citable manifest excerpt is unanchored; "
        "narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation; each criterion states (a) one "
        "observable behavior and (b) a falsifying condition referencing no implementation internals (no file names, function names, or variable names); this is a content gate, not a "
        "self-check; exactly one criterion may be written per thread per cycle — after the first criterion is written, the only valid next token is the formal notation rung label; a "
        ...
        "rung label; batch-collecting criteria for multiple threads under one criteria label bypasses sequential descent and is a protocol violation. The formal notation rung separates "
        "behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be true — the executable or testable "
        "part. Natural language labels, introduces, and explains the specification — prose adds context but the behavioral constraints must still be encoded in notation.it must encode "
        "all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe what notation looks like, not "
        "what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type signatures, interfaces, pseudocode, and "
        "structural invariants — the test of valid formal notation is that it cannot be directly compiled or executed without modification; any construct that compiles or runs as-is is "
        ...
        "elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript and the audit section still applies; the audit section is separate and named; the "
        "rung is incomplete until all criteria are encoded. Natural language may appear as section labels or introductions (e.g., ‘This invariant captures the rendering constraint:’) "
        "but may not substitute for encoding a constraint in notation — if a constraint can be stated in notation, it must be. each test function asserts exactly one behavioral "
        "property — one behavioral property per test function; multiple assertions are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value "
        "is both non-null and of the expected type); before writing the test, re-read the formal notation and assert each structural constraint it encodes; confirm via tool call that "
        ...
        "changed — return to the prose rung and re-emit with an updated gap before descending again; before writing the test, check for an existing test file covering the same module — "
        "add there if so; when the artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: "
        ...
        "violation; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — if that tool call has not "
        "been made in the current response, the only valid next action is that tool call — V-complete may not be emitted without this tool-executed result in the transcript; asserting "
        "confirmation without showing the exec_observed block does not satisfy this gate — the exec_observed block must appear in the current response; the validation artifact must "
        "reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be "
        ...
        "witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 "
        ...
        "output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after a red VRO that is not a harness error, the criterion "
        "re-statement is an anchor only — re-running VRO without an implementation edit is a protocol violation; re-writing formal notation or the EV artifact without an intervening "
        "implementation edit is also a protocol violation; after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token — this shortcut applies only when a "
        "red VRO already exists in the current cycle for the current criterion; a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO must fire for the new criterion "
        "before impl_gate; skipping EV and VRO after a HARD STOP that introduces a new criterion is a protocol violation; a green exec_observed at the VRO rung means there is no "
        "failing assertion — 🔴 Gap: may not be emitted after a green exec_observed; a green exec_observed without a prior red run means the test is vacuous — return to the executable "
        "validation rung and rewrite the test to fail; a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed output; after emitting 🔴 Gap: at the "
        "observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: "
        "have been emitted at the validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; prose-only exec_observed sentinel "
        "does not satisfy this gate — the exec_observed block must contain non-empty verbatim tool output in a triple-backtick block; HARD STOP may not be emitted at the executable "
        "validation rung — a harness error at EV requires fixing the harness, not an upward return; When 🔴 Execution observed: at the executable validation rung shows a harness error "
        "(import failure, syntax error, missing file), the only valid next token is a tool call that repairs the harness — not a gap statement, not HARD STOP, not a rung label; HARD "
        "STOP may not appear in the same response as a harness-error exec_observed at the EV rung. Before emitting 🛑 HARD STOP, locate the criterion written in the most recent prior "
        "HARD STOP cycle for this thread — if it is textually identical to the current criterion, HARD STOP is a protocol violation: an identical criterion means the upward return "
        "accomplished nothing; if the implementation did not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation "
        "before issuing HARD STOP. if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. HARD STOP does "
        "not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread; the prose re-emission "
        "rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running behavior label, re-emit the criterion for this cycle — criterion re-emission is "
        "required: emitting the OBR label without immediately following it with the criterion text is a protocol violation; the tool call is the only valid next action after criterion "
        "re-emission; planning statements, diagnostic narration, and commentary between criterion re-emission and the tool call are protocol violations; ✅ Thread N complete may not "
        "appear until a tool call exists in the transcript after the observed running behavior label in the current cycle; then immediately invoke the implementation as a live running "
        "process via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion. a "
        "test runner invocation does not satisfy this gate — running tests produces validation-run-observation-type output, not observed-running-behavior-type output; the invocation "
        "target must be the implementation artifact itself, not a test harness wrapping it; if the most recent tool call at this rung is a test runner, ✅ Thread N complete is blocked — "
        "the only valid next action is a live-process invocation of the implementation artifact; the live-process invocation must directly demonstrate every criterion declared for this "
        "thread, not only the primary one — a live-process invocation that demonstrates fewer than all declared criteria is a gap, not a completion; the test suite run required by "
        "condition (5) may follow the live-process invocation but may not precede or replace it; A file read never satisfies this rung — the block must contain output produced by "
        "running the artifact. If the artifact requires a start step before querying (e.g., a web server), two tool calls are required in sequence: process-start then query — the 🔴 "
        "Execution observed: sentinel may not be emitted until both complete. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable "
        "artifact exists. If no invocable process exists, open a gap cycle to make the artifact directly invocable. Before any tool call at this rung, emit a provenance statement "
        "naming the invocation target: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the target is a test runner, stop: the invocation target must be the "
        "implementation artifact itself running as a live process, not a test harness; the provenance statement does not replace the tool call — the tool call must follow it in the "
        "same response; ✅ Thread N complete may not be emitted unless all of the following exist in this cycle's transcript: (1) a tool call exists in the transcript after the observed "
        "running behavior label in the current cycle; (2) observed running behavior label has been written after the most recent 🟢 Implementation gate cleared in this thread; (3) "
        "non-empty verbatim output appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 Execution "
        "observed: is a protocol violation; (4) the 🔴 Execution observed: directly demonstrates the criterion — if it does not directly demonstrate the criterion, emit 🔴 Gap: naming "
        "what is undemonstrated and apply the upward-return failure-class rules; (5) a full test suite run result exists after the most recent OBR tool call — the only valid next "
        "action if no such result exists is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason. ✅ Thread N "
        "complete immediately follows 🔴 Execution observed: at the OBR rung — not before it; emitting ✅ Thread N complete before the OBR rung tool call and 🔴 Execution observed: "
        "sentinel exist in this cycle is a protocol violation. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal "
        ...
        "unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared is the first token of "
        "the executable implementation rung — no tool call, no prose, and no file modification may appear before it in the current response; emitting a file-write tool call before 🟢 "
        "Implementation gate cleared is a protocol violation regardless of whether impl_gate is emitted later in the same response. it does not complete the thread; after emitting it, "
        "the executable implementation rung, the observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread N complete is valid; the next "
        "required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ Thread N complete may not appear until the observed running behavior rung has fired "
        "for this thread. Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior to the system — must be followed inline by a bracketed "
        "thread marker: [T: gap-name]; [T: gap-name] markers are valid only in the prose rung — their presence in any other rung artifact is a protocol violation; before emitting ✅ "
        "Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly one entry per distinct gap-name; a [T: gap-name] marker in the "
        "prose with no corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and "
        "any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: "
        "from the current cycle’s validation run observation rung — not from a prior cycle — to exist in output; the VRO rung label must appear in the transcript for the current cycle "
        "— a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, count the thread entries in the list — "
        "the N in '✅ Manifest declared — N threads:' must equal the count of listed threads; a mismatch between the declared N and the thread list length is a malformed manifest. "
        "Before emitting ✅ Manifest declared, if the prose contains no [T: gap-name] markers, ✅ Manifest declared is blocked — re-emit the prose with [T: gap-name] markers added inline "
        "after each behavioral predicate before proceeding. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, "
        "displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; count the [T: gap-name] markers in the "
        "prose you just wrote to confirm every predicate is covered; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; a manifest "
        "entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a gap is produced for the first time at the criteria rung, not the "
        "manifest; ✅ Manifest declared may be emitted exactly once per invocation. only threads declared in ✅ Manifest declared may be addressed — creating a thread not in the manifest "
        "is a protocol violation; when a new gap is discovered mid-run, return to prose and re-emit a revised manifest — do not create an undeclared thread. Outputting a rung label is "
        "what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. I is the declared intent "
        ...
        "running behavior rung — it is required in every cycle; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; all other rung "
        "transitions are continuous within the same response. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through "
        "executable implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward return. Upward returns "
        "follow the failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal "
        "notation; observation cannot be produced is a prose-description failure, not an implementation failure — return to the prose rung and update it. After every upward return, a "
        "new ✅ Validation artifact V complete must be emitted for the current cycle’s gap — an artifact written in a prior cycle does not satisfy this gate. Before producing the final "
        ...
        "immediately precedes it in the transcript — if no ✅ Manifest exhausted exists, stop: emit ✅ Manifest exhausted if all threads are complete, or open the next gap cycle if they "
        "are not. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order — no new behavioral claims, no "
        "coverage summaries, no suggestions; then reconcile any documents the implementation affects. Rung-entry gate: before producing content at any rung, verify that a valid 🔴 "
        "Execution observed: sentinel exists in the current cycle whose output matches the rung’s artifact type — if (d) reveals that no valid exec_observed exists in the current "
        "cycle, the only permitted next token is a tool call — producing any other content when (d) is false is a protocol violation. Seven rungs in order: prose → criteria → formal "
        "notation → executable validation → validation run observation → executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; "
        "✅ Manifest declared — N threads: [numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 "
        "Gap: [what the verbatim output reveals]; 🛑 HARD STOP — upward return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ "
        "Validation artifact V complete; ✅ Thread N complete; ✅ Manifest exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current tests]; ✅ "
        "I-formation complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.",
