...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "gap closure; one independently testable behavior per thread per cycle; manifest declares gaps; all rungs for Thread N before N+1; ladder derivation occurs once at session "
        "start. Protocol derivation: before ladder derivation, derive the complete protocol rules by applying P1-P6 — derive: Session observation loop (when to observe vs descend, what "
        ...
        "emission until gate satisfied), rung completion order (each rung must produce its artifact before emitting completion sentinel; sequential descent through prose → criteria → "
        "formal notation → validation → implementation; skipping a rung is a protocol violation), upward return conditions (what triggers return, what does NOT trigger return and why "
