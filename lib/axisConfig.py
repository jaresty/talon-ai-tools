...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "names, function names, or variable names); this is a content gate, not a self-check; exactly one criterion may be written per thread per cycle — after the first criterion is "
        "written, the only valid next token is the formal notation rung label; a second criterion for the same thread is a protocol violation — stop, return to the prose rung, and "
        "split the thread before descending again; the criteria rung label is per-thread — writing criteria for Thread 2 or any subsequent thread before ✅ Thread 1 complete is emitted "
        "is a protocol violation regardless of whether they share a criteria rung label; batch-collecting criteria for multiple threads under one criteria label bypasses sequential "
        "descent and is a protocol violation. The formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, "
        ...
        "transcript and the audit section still applies; the audit section is separate and named; the rung is incomplete until all criteria are encoded. Natural language may appear as "
        "section labels or introductions (e.g., ‘This invariant captures the rendering constraint:’) but may not substitute for encoding a constraint in notation — if a constraint can "
        "be stated in notation, it must be. each test function asserts exactly one behavioral property — one behavioral property per test function; multiple assertions are permitted "
        "only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the expected type); before writing the test, re-read the formal "
        ...
        "the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, the "
        "artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the "
        "test is vacuous and must be rewritten. Before writing the validation run observation rung label, invoke the validation artifact via a test runner; a harness error (import "
