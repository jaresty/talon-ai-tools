...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "validation artifact must fail before any implementation edit — a green run with no prior red run does not satisfy this rung. At the validation run observation rung, run the "
        "validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; if "
        "the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Before each edit, check: does a 🔴 Execution "
        "observed: sentinel exist for the current gap? If not, run the validation artifact now before editing. One edit per re-run cycle; after each edit re-run the validation artifact "
        "before any further edit. At the observed running behavior rung, emit tool output demonstrating the declared behavior — a ✅ Thread N complete sentinel with no preceding tool "
        "output does not satisfy this rung. Before emitting ✅ Thread N complete, check: does the declared intent have any behaviors not yet observed running? If so, emit a new 🔴 Gap: "
        "naming the next unmet behavior and begin the next cycle at the criteria rung. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung "
