...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation artifact V complete [gate: pre-existence check tool call result present; tool "
        "call in this response wrote the file to disk]; ✅ Thread N complete [gate: meta exec_observed after executable implementation shows the gap declared for this thread is no "
        "longer present in running behavior; every assertion in the validation artifact must appear passing in this exec_observed output]; ✅ Manifest exhausted — N/N threads complete "
        ...
        "complete in the same response; emitting ✅ Ground complete without a preceding 🔵 Closing observation in the current response is a protocol violation]; 🔵 Intent logged — file: "
        "[path] | target: [assertion name] | evidence: [verbatim red output showing failure] [gate: this sentinel must appear immediately before each file-write tool call to "
        "implementation code; the target must be a specific test assertion name and the evidence must be verbatim output from running the validation file showing that test assertion "
        "failing in the current state; emitting this sentinel without a preceding exec_observed showing the target test assertion failing voids the intent and the subsequent write; "
        "fabrication of red evidence (output that does not come from actual test execution) voids the sentinel and the rung; for backfill cases where tests already pass and cannot be "
        "seen red, perturb the implementation (introduce a controlled fault) to observe the test fail, then fix the perturbation]; 🔵 Intent achieved — file: [path] | target: [assertion "
        "name] | evidence: [verbatim green output showing passing] [gate: this sentinel must appear after each file-write tool call to implementation code; the target test assertion "
        "name must match the target from impl_intent and the evidence must be verbatim output from running the validation file showing that test assertion passing in the current state; "
