...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "guard as written by constructing a concrete scenario in which property [N] is absent while the quoted guard nevertheless succeeds (e.g. a guard that reads a static artifact "
        "rather than invoking the governed behavior succeeds whenever the artifact contains the expected text, even if the behavior never executes); emit 'Test blind-spot: found — "
        "<construction>' or 'Test blind-spot: not found'; if found, fix the guard and repeat from step (2); (4) execute the quoted guard in a state where property [N] is violated and "
        "emit 'Failure: property [N] — <verbatim quoted failure line from a tool-result block>' or 'Unobservable: property [N] — <reason>'; emit Unobservable only when no executable "
        "guard, assertion, constraint, or rule could ever distinguish satisfaction from violation regardless of what tests are written; if a guard can exist, execute it against a "
