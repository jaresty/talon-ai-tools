...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "verified'; every assistant turn after '§ implementation permitted [N]' has appeared in the transcript that does not contain a line beginning with '§ implementation permitted "
        ...
        "transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted [N]' where N is the "
        "1-based index of the file-modifying tool call this sentinel governs must appear in the transcript after '§5 enumeration complete'; no file-modifying tool call may appear "
        "before the first '§ implementation permitted [N]' sentinel; the literal string '§ implementation permitted [N]' with N matching the 1-based index of the governed tool call "
        "must appear immediately before the `(i)` line of the five-line block in the assistant text block immediately before each file-modifying tool call, in the same response turn as "
        "that tool call — a user message appearing between '§ implementation permitted [N]' and the tool call does not satisfy this requirement; a file-modifying tool call not "
        "immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the five-line block does not satisfy this requirement. Quoted-span exclusion: "
        "an occurrence of any named sentinel or heading string inside a markdown code fence (a span beginning with ``` or ~~~ on its own line) does not satisfy any rung or ordering "
        "requirement — only occurrences outside code fences count toward compliance.",
