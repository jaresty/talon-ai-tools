...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "completion sentinel, THEN add another assertion; if gap is closed, emit completion sentinel and move to next gap. NO file creation in assertion rung — stubs created at "
        ...
        "the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, FIRST emit completion sentinel, THEN return UP to assertion "
        "rung to add more assertions; if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit "
