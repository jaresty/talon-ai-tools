...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "quoted scope text or be named in the failing-item line from the most recent tool-executed run result — a symbol not present in either does not satisfy this requirement; the "
        "quoted scope text must be from a test assertion whose pass/fail state is determined solely by the named symbol — a test that passes or fails independently of that symbol does "
        "not satisfy this requirement; a statement in the derivation phase is a commitment only if its truth value can be determined from tool-executed results already present above it "
        "in the transcript — a statement whose truth value requires evaluating a result not yet present is not a commitment; after the call, a tool-executed run result must appear in "
        "the transcript in which the quoted scope text is absent, every other line from the immediately preceding pre-edit run result is present, and a result line appears for every "
        "test name that appeared in that pre-edit run result — the pre-edit run result for a given call is the run result that produced that call's scope commitment; a failing-item "
        "line from the pre-edit run result may be absent from the post-call run result only if its literal text appeared as a quoted scope string above this call and no other "
        "failing-item line from the same pre-edit run result also appeared as a quoted scope string above this call. Each file-modifying tool call's scope line must satisfy these "
        "requirements independently — a scope line from a prior call does not carry forward. Then, immediately before each file-modifying tool call — as the last assistant text block "
        "before that call — write a block containing exactly these four lines in order: `(i) scope inflation: <string>`, `(ii) stale quote: <string>`, `(iii) symbol cardinality: "
        "<string>`, `(iv) post-call line mismatch: <string>`. A file-modifying tool call not immediately preceded by a block whose last four content lines begin with `(i)`, `(ii)`, "
        "`(iii)`, `(iv)` in that order does not satisfy this requirement. Proceed to file-modifying tool calls only when the derivation and all four enumeration categories are present "
        "as the last four content lines above this point in the transcript.",
