...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "to see it FAIL (red). NO implementation in this rung. AFTER check turns green (implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ Assertion "
        "complete (staying in assertion rung), THEN add another assertion for the same gap; If gap IS closed: emit ✅ Thread N complete and move to next gap; NO file creation in "
        ...
        "stubs; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy "
        ...
        "FIRST emit ✅ Implementation complete (return UP to assertion rung to add next assertion); If gap IS closed: emit ✅ Thread N complete. Tests drive the behavior needed. Emit "
