...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "(d) must be the identifier that appears as a substring of the failure line in (g) — not a symbol that contains or is associated with that identifier; if the failure line "
        "names a narrower identifier, that narrower identifier is (d) regardless of which broader symbol was invoked to reach it; (d) is derived from the failure line, not chosen "
        "independently — the failure line names (d), not the other way around; (e) the layer classification — if the governed symbol name appears as a token in a call expression in "
        "the test body — where the call expression is not nested inside another call expression that names a different symbol — the layer is 'unit'; if the governed symbol name does "
        "not appear in the test body, the layer is 'integration'; (f) the executor class — the executor must produce output that could only exist if the governed assertion reached "
        "execution at the layer named in (e); a governing artifact is disposable if removing the governed symbol from the governed artifact — without modifying the named governing "
        "artifact — produces an executor result in which (c) does not appear; a governing artifact is non-disposable only when (c) appears in the executor result after removal — a "
        "disposable artifact does not satisfy (f); (f2) the serialization gate — classify the assertion target: the value produced by the expression that is the direct argument to the "
        "assertion call. A governing assertion satisfies (f2) if and only if its assertion target is a behavioral value: a value whose content is determined by whether the governed "
        "behavior is correctly implemented and that could not be produced by an arbitrary implementation that merely serializes the governed artifact's output correctly. A governing "
        "assertion fails (f2) if its assertion target is a serialization value: a value produced by converting the governed artifact's output to a string/byte/text representation — "
        "regardless of what the conversion expression is named — such that a correct implementation and an incorrect implementation would produce the same assertion target value if "
        "their outputs serialize identically. The Falsify derivation block must contain the literal line '(f2) serialization gate: [assertion target] — targets: [behavioral "
        "property/state/outcome | string/byte/text representation] — gate: [pass | fail]'; a derivation block that does not contain this line does not satisfy (f2). The creation-step "
        "exception does not apply when the derivation block contains '(f2) serialization gate:' with 'gate: fail'; (g) the observed FAIL result: the content of an executor result "
        "block appearing in the transcript after the artifact was applied to the subject and before the first governed artifact-producing action for this behavior, where (a) precedes "
        "(c), the named executor produced the result, and the failure line contains (d) as a substring. A result satisfies this token if and only if: (g) is present at the required "
        "position; (a) is not separated from (c) in (g) by any line whose content contains (c) for a different governed behavior; and a subsequent executed-artifact result contains "
        "(b) at the position where (a) was absent. A result satisfying (g) must contain a failure line whose content includes a substring from the governed assertion call expression — "
        "where the call expression is a syntactic call in the test body whose callee name contains the governed symbol name as a substring; a failure line that contains no such "
        "substring does not satisfy (g) regardless of whether (a) precedes (c) in its output; a valid (g) failure line is a line on which (a), (c), and a direct callee name containing "
        "(d) co-occur — an executor result that contains (a) before (c) but where no single line satisfies this co-occurrence does not satisfy the creation-step exception. Exception: "
        "(g) is not required for the single action that produces the governing artifact's assertion text, per governed behavior — the creation step is that action only; (g) is "
        "required for every governed artifact-producing action in the current session — a creation step is that action only if (c) is absent before the action and present after it, "
        "and a named executor invocation against the governing artifact immediately following the action produces a tool-executed result containing (c) and not (a); immediacy is "
        "defined as: no executor result block appears between that action and the named executor invocation — non-executor actions do not break immediacy. Artifact identifier must "
        "differ: the artifact identifier of the named governing artifact must differ from the artifact identifier of the artifact being modified. Semantically-equivalent refactor "
        "exception: (g) is also satisfied when an executor result block containing (a) before (c) appears in the transcript before the governed artifact-producing action, produced by "
        "a named executor invocation occurring in the current session after the last governed artifact-producing action, against a state where the refactored form was modified to "
        "violate the invariant the governing assertion checks — the perturbation result must appear before the revert action. Revert is complete when the pre-perturbation executor run "
        "result is quoted verbatim in the transcript before the revert executor invocation, and a subsequent executor invocation produces (b) at the position where (a) was absent. "
        "Per-behavior scoping: a governed artifact-producing action is compliant when (g) is satisfied for the behavior named in the Falsify derivation block's (d) entry — the "
        "behavior name is fixed at derivation time and may not be restated or narrowed after (g) is observed; the action is not required to implement any behavior for which (g) has "
        "not yet been established; unguarded behaviors require separate governing FAILs in subsequent steps and must not be treated as a blocking condition on the current action. "
        "Implementation scope constraint: a governed artifact-producing action may introduce only constructs whose complete identifier — the full name as it appears in the governed "
        "artifact, not any component or prefix — appears as a verbatim substring of (c), (d), or (g) in the Falsify derivation block active at the time of the action; a construct is "
        "introduced when its identifier appears in the governed artifact in any position — defined, declared, imported, or referenced — that was absent before the action; the "
        "governing artifact's content is not a valid source for the allow-list; the allow-list is derived exclusively from the executor-produced result entries (c), (d), and (g); the "
        "allow-list is derived from the full content of the (g) tool-result block named in the active Falsify derivation block; an identifier that does not appear as a verbatim "
        "substring of that tool-result block may not be introduced; test description lines, docstring echo lines, parametrize label lines, and summary lines in that block are valid "
        "allow-list sources — the restriction on these line types applies only to (c) derivation, not to allow-list sourcing; a construct whose introduction makes a reachable "
        "execution outcome possible that does not appear as a verbatim substring of (g) requires its own governing FAIL before it can be introduced, even if its identifier appears in "
        "(g) — the path constraint is satisfied only when every execution outcome the introduced construct can produce appears as a verbatim substring of the (g) tool-result block; "
        "during the creation-step exception, the allow-list is (d) only; a reference to an identifier not on the allow-list within a stub or partial implementation is not exempt; a "
        "construct whose complete identifier does not appear as a verbatim substring of the active allow-list entries must not be introduced in the same action and requires its own "
        "governing FAIL before it can be introduced. Scope-governing entries: only (c), (d), and allow-list identifiers from (g) are scope-governing entries — entries (a), (b), (e), "
        "(f), and (f2) are not scope-governing entries and do not contribute to the allow-list. Artifact gap: when the Falsify derivation block contains the line '(g) observed FAIL "
        "result: unavailable' and no tool-result block in this transcript produced by a named executor invocation contains (c) as a substring, the derivation is incomplete. A "
