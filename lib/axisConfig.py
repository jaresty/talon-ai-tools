...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "action; justification is evidence, not just documentation; if you cannot explain why a file change satisfies the current rung, do NOT make that change. Session observation "
        "loop (when to observe vs descend, what observation means vs does NOT mean), ladder derivation format (table columns, columns meaning), rung-specific behaviors (when to emit "
        "each sentinel, gate blocks sentinel emission until gate satisfied), rung transitions (emit rung label when beginning each rung; Outputting a rung label is what begins that "
        "rung, not a heading or annotation; emit completion sentinel when finishing each rung; emit rung label when returning upward to a prior rung; you are at rung N until you emit "
        "completion sentinel for rung N or return upward), rung completion order (each rung must produce its artifact before emitting completion sentinel; sequential descent through "
        "prose → criteria → formal notation → validation → implementation; skipping a rung is a protocol violation; each rung may not be skipped or combined with another), formal "
        "notation is NOT a file artifact — it is a mental exercise, not a written document; produce the specification in your head, not on disk; the formal notation rung verifies your "
        "understanding, not a file; rung-specific gaps (each rung addresses a specific gap that only that rung can solve; the gap must be explicitly expressed IN THE CONVERSATION with "
        "evidence appropriate to the rung level; for validation assertions, the gap is the individual assertion being red; gaps are not implicit — they must be written down in the "
        "conversation with supporting evidence; fabricated test runs or falsified output void all evidence and the rung), P5 implies sequential descent (since each artifact derives "
        "from prior artifact per P5, you must produce them in order — treating the ladder as a planning document rather than strict sequence violates derivation), ladder is strict "
        "execution sequence (emit rung label, produce artifact, emit completion sentinel — repeat for each rung in order; when resuming mid-ladder, locate the highest "
