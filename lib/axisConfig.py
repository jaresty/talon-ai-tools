...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "not satisfy this requirement; each 'property [N]:' line must contain as a literal substring the longest contiguous non-bracket portion of the template in '§ notation derived:' "
        "that is at least 3 characters long — where the non-bracket portion is any maximal substring of the template that contains no '[' or ']' character; a 'property [N]:' line that "
        "does not contain such a substring does not satisfy this requirement; after writing each 'property [N]:' line, immediately write '§ split test: [quoted sub-expression]' where "
        "[quoted sub-expression] is a phrase from the parent expression naming a candidate sub-expression to split on, followed by a sentence stating whether splitting at that "
        "sub-expression yields two independently falsifiable sub-properties — a 'property [Na]:' line that does not have a '§ split test:' line above it does not satisfy this "
        "requirement; if the split test finds a valid split, write one sub-property per branch: 'property [Na]:', 'property [Nb]:', etc., each containing a formal expression — a "
        "sub-property line whose content is prose only does not satisfy this requirement; if the split test finds no valid split (the property is already atomic), write 'property [Na]: "
        "atomic —' followed by a restatement of the parent expression in a notation where every term has an unambiguous denotation — a restatement that two independent agents could "
        "interpret differently does not satisfy this requirement; the conjunction of all sub-property expressions for a given 'property [N]:' must be logically equivalent to the "
        "expression in 'property [N]:' — logical equivalence means the set of inputs satisfying all sub-property expressions simultaneously equals the set of inputs satisfying the "
        "parent expression; a conjunction satisfied by a strict subset of the parent's cases (e.g. a single concrete instance substituted for a variable) does not satisfy this "
