...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "guards against it. 'Overreach: not found' is a claim that no such reduction exists, so it is valid only after an executed attempt — a tool-result showing a smaller artifact "
        "that the guards reject (or a smaller artifact they accept, which you then keep). If a reduction the guards still accept exists, or an execution path through the artifact is "
        "not exercised or distinguished by any guard, emit 'Overreach: found — <modification or path>', trim to the boundary the guards require, and repeat. A bare 'Overreach: not "
        "found' with no executed reduction attempt does not satisfy this token. Witness every assertion (Gate 2). Enumerate the executable assertions: emit one 'Assertion: <verbatim "
        "assertion text>' line for each executable assertion of the established guard, quoted verbatim from the guard's tool-result. The verbatim lines are the observable; no count or "
        "completeness sentinel is used, and a self-declared count does not satisfy it. Every enumerated 'Assertion:' line must then receive its own 'Failure: assertion \"<verbatim "
        "assertion>\" — \"<verbatim failure text>\"' or 'Unobservable: assertion \"<verbatim assertion>\" — structural' before 'Coverage: complete'; the binding is by verbatim "
        "identity — the assertion text in the Failure or Unobservable line must match an enumerated Assertion line exactly, and an enumerated assertion with no matching outcome line "
        "leaves coverage incomplete. After each Failure or Unobservable outcome, if any enumerated 'Assertion:' line still lacks a verbatim-matching outcome, return to the witness "
        "step for the next unwitnessed assertion and repeat; do not leave Gate 2 until every enumerated assertion has a matching outcome. Each enumerated assertion is either "
        "behavioral or structural. A behavioral assertion is witnessed only by a pair of A-observations: an execution in which A's own guard-defined result is a failure, and one in "
        "which A's own result is a pass. An A-observation is an execution of the committed guard in which assertion A is evaluated and produces its own guard-defined result. The guard "
        "defines the observable predicate for A: an A-pass result witnesses the property present, an A-fail result witnesses it absent — so there is no separate model claim that a "
        "state 'violates' or 'satisfies' the property; the guard is the measurement mechanism and its A result is the observation. The failure (A-fail) execution must immediately "
        "precede the 'Failure:' line (it is the producer); the passing (A-pass) execution is the contrast evidence and may appear elsewhere in the transcript. Any execution that does "
        "not produce an A result is not an observation of A and cannot witness A, regardless of the cause or form of the execution's outcome — this covers execution errors, unrelated "
        "assertions failing, a harness that exits without reporting A, and any other run in which A itself was not evaluated; only an A-fail result can supply the violating "
        "observation, and only an A-pass result the satisfying contrast. Both A-observations must be produced by the committed guard artifact, and the guard's output must identify A "
        "(by test name, subtest path, or assertion message) so the pair is bound to this enumerated assertion rather than another. Provenance closure: any transcript reference that "
        "functions as evidence — the assertion identity, the A-fail result, and the A-pass result — must quote verbatim the text of the qualifying execution record that establishes "
        "it, and counts as evidence only if that exact text occurs in a referenced qualifying execution record; a result or identity typed as prose that does not appear verbatim in a "
        "cited qualifying execution record is not evidence and does not witness A. If the artifact already passes A, construct a temporary modification that makes A alone fail while "
        "every other assertion still passes, execute, emit the Failure from that tool-result, then restore; a modification that fails every assertion at once discriminates none. Emit "
        "'Failure: assertion \"<verbatim assertion>\" — <A-fail result> / <A-pass result>' quoting both observed results from their tool-results; a 'witness:' line, if added, only "
        "projects those two results and is not itself evidence. A structural assertion — one whose property has the artifact's own text or structure as its primary subject, so no "
        "behavioral failing execution exists — is witnessed instead by 'Unobservable: assertion \"<verbatim assertion>\" — structural'; Unobservable is never satisfied by having "
        "searched for a failing execution and found none. An assertion that is behavioral yet no admissible modification can make fail is a guard defect: strengthen the guard and "
        ...
        "or non-exercising guard checking a surrogate; (b) durability — a guard gone at the moment it would need to fire; (c) ephemeral guard not committed to a location that re-runs. "
        "Verdict-follows-execution governs both gates: when tool calls are available, every 'Overreach:' verdict and every 'Failure:' observation is valid only when it immediately "
        "follows a tool-result block that mechanically produces it; a verdict emitted from description or analysis alone does not satisfy this token. A structural Unobservable "
        "assertion is exempt, having no execution to anchor to. 'Coverage: complete' is valid only when every enumerated assertion has either a 'Failure:' line backed by its "
        "A-fail/A-pass execution pair or an 'Unobservable: … — structural' line, and no 'Overreach: found' remains open; an 'Unwitnessed: … — execution unavailable' assertion leaves "
        "coverage incomplete and does not satisfy this token.",
