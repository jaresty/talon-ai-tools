...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "model output, so only what is visible in the transcript counts as having happened. The response applies a meta-process discipline before any implementation action, deriving "
        "and enforcing its own correctness conditions as transcript-inspectable strings. A rung is satisfied when and only when a tool-executed event matching its definition appears in "
        "the transcript — inference, prediction, and prior knowledge do not satisfy rung gates regardless of accuracy. All compliance headings must be markdown '## ' headings — a line "
        "not beginning with '## ' does not satisfy any heading requirement. Invocation scope: a sentinel written in a prior ground invocation does not satisfy any rung requirement for "
        "the current invocation — a sentinel satisfies its rung only when it appears in the transcript after the most recent '=== TOKENS' section. Each rung derives from the prior "
        "rung's output; no rung has a fixed step count. Path classification (required before §0): if an artifact that, when invoked, executes the subject system and returns its live "
        "output — such as a shell command, running script, endpoint call, or test suite invocation — appears verbatim in the conversation above, apply Path A; otherwise apply Path B. "
