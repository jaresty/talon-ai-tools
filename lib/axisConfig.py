...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "independently falsifiable), write 'property [Na]: atomic — ' followed by a formal restatement of the same expression; when all 'property [N]:' and sub-property lines are "
        ...
        "check:' line whose quoted phrase does not appear verbatim in a 'property [N]:' line above it does not satisfy this requirement; then write '§ properties complete'; '§ "
        "properties complete' is valid only after a valid '§ properties check:' line has appeared; (4) verify coverage: '## Completion check' must not appear before '§5 enumeration "
        ...
        "line is '§4 coverage verified' is exempt from the resume phrase requirement; when a turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, "
        "must equal exactly: Resume: say \"Continue autonomously — gates still apply\" to proceed under the same protocol. (5) enumerate escape paths: '## Path enumeration' must not "
        "appear before '§ enforcement complete' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions "
        "derived, or dimensions covered; for each path, name the literal string whose presence closes it; when no further open path remains, write '§5 check: \"[quoted phrase]\"' where "
        "[quoted phrase] is a verbatim substring of one of the escape path closing strings named in the enumeration above — a '§5 check:' line whose quoted phrase does not appear "
        "verbatim in the path enumeration above it does not satisfy this requirement; then write '§5 enumeration complete'; '§5 enumeration complete' must not appear before a valid '§5 "
        "check:' line in the transcript; '§5 enumeration complete' must not appear before '§ enforcement complete' in the transcript; any referent named in the '[text]' from §1 not "
        "already present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ "
