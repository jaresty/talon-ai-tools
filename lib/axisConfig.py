...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "caller would observe it — not the test suite; running tests at this rung is a protocol violation; if the implementation has no directly invocable artifact, invoke any consumer "
        "of the changed artifact that demonstrates the declared behavior — not the test that gates it; implementation edits, new files, and code changes are not permitted at this rung "
        "— if observation reveals a defect, emit the gap and return upward. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written in the "
        "current cycle and a 🔴 Execution observed: sentinel appears immediately after it; once ✅ Thread N complete is emitted no further output in that cycle is valid. Before emitting "
        "✅ Thread N complete, check: does the declared intent have any behaviors not yet observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and begin the next "
        "cycle at the criteria rung. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no "
        "artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every descent "
        "through executable implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward return. "
        "Reconciliation gate: at every upward rung transition — whether the scheduled formal-notation → executable-validation gate or any upward return triggered by new information — "
        "reconcile all artifacts at that rung against I before redescending; if prose has evolved on this upward return, any pre-existing document that records I (ADR, README, spec) "
        "must be updated to match the evolved prose before redescending — this is a gate precondition, not a final-report step; documentation is a reconciliation artifact, not a "
        ...
        "narrative summary or wrap-up is not a valid emission. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each "
        "thread in order; the final report copies artifacts already emitted in the thread rungs — no new behavioral claims, no coverage summaries, no suggestions may appear in the "
        "final report; then reconcile any documents (README, specs, ADRs) the implementation affects — this reconciliation is part of the final report, not a separate thread. Seven "
