...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "after the 'Quoted implementation:' sentinel, make the modification via a tool call and wait for the tool-result block confirming it; if step (4) required constructing a "
        "temporary violating state because the governed artifact already satisfied property [N], step (5) is satisfied by confirming the original state is restored — emit 'Quoted "
        "implementation: <verbatim text identifying the restored governed artifact>' before the restoration tool call; no subsequent step may occur until this sentinel has been "
        "emitted and the corresponding modification or restoration tool call has completed;(6) attempt to construct a minimal modification to the quoted implementation that preserves "
        "the success of the quoted guard while still changing the governed artifact, and attempt to construct a concrete execution path through the quoted implementation that is not "
        "exercised or distinguished by the quoted guard; emit 'Implementation overreach: found — <modification or execution path>' or 'Implementation overreach: not found'; if found, "
        "minimally revise the governed artifact to eliminate the overreach, emit a new 'Quoted implementation:', and repeat from step (6); every implementation overreach analysis must "
        "reason only from the quoted implementation and quoted guard emitted earlier in the transcript, not from an imagined or intended version. After all current properties complete "
        "their six-step cycles, begin the implementation audit. 'Coverage: complete' is the global fixed-point sentinel and is valid iff all of the following hold simultaneously: (1) "
        "every retained property completed steps (1)–(6); (2) every retained property has a witnessed Failure or justified Unobservable; (3) assertion inventory equals observed "
        "assertions for every property (set equality verified in both directions); (4) no implementation gap exists in the final audit pass; (5) Ground reached a fixed point (§ ground "
        "complete was emitted for the final retained property set); (6) the implementation audit reached a fixed point (Audit: implementation complete was emitted after a pass that "
        "discovered no new gaps). The implementation audit is a convergence loop: repeat — (a) enumerate every modified artifact; (b) enumerate every externally callable entry point "
        "of the governed artifact; (c) enumerate every distinct execution branch; (d) enumerate every caller or consumer whose observable output changed; (e) enumerate every "
        "externally visible output or state transition; enumerate all observable behaviors and emit one 'Audit: implementation gap — <description>' for each behavior not governed by "
        "any retained property; if any gaps were emitted, restart the Ground protocol from the full retained property set incorporating all newly discovered properties — including "
        "atomicity, completeness, and observational independence checks until a new '§ ground complete' is reached — then execute the full six-step falsify cycle for every newly "
        "retained property; for every existing property whose canonical definition, quoted implementation, or quoted guard is affected by the new properties, determine the impact "
        "scope: if only the quoted implementation is affected, repeat steps (5) and (6) for that property; if the guard or observation mechanism is affected — meaning the assertion "
        "inventory, quoted test, or failure evidence may no longer validly witness the property under the new retained set — repeat steps (2) through (6) for that property; a prior "
        "falsify result for a property whose guard or observation was affected does not govern the resulting state and must not be treated as satisfying this token; resume the audit "
        "after all affected properties have completed their required cycle; repeat until a full audit pass emits zero gaps, then emit 'Audit: implementation complete'; 'Coverage: "
        "complete' may not be emitted before 'Audit: implementation complete'. A response that omits any required sentinel, performs a step before emitting its required quoted "
        "artifact, modifies the governed artifact to satisfy property [N] before a qualifying Failure witness has been established, omits any executable assertion from the assertion "
        "inventory, or leaves a property without an automatic regression guard does not satisfy this token.",
