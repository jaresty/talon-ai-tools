...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "change; second, in a separate entry below, quote the specific text from a prior tool-executed result that names a behavioral assertion — a test name or assertion message that "
        "fires on the governed behavior — and shows it failing; the quote must appear in an execution result above the first entry, not introduced here and not from a source that "
        "produces output by retrieval rather than by execution; the candidate change is permitted only to address the specific error or failing assertion visible in the governing "
        "failure signal; an assertion not yet observed to fail has not been shown to govern, and a change that fixes it cannot be derived from the current governing artifact; an "
        "insufficiency claim is permitted only when it quotes specific text from the governing failure signal that the smaller change leaves unresolved — no other basis for an "
        "insufficiency claim is valid; if the smaller change resolves all quoted text in the governing failure signal, it is sufficient and must become the candidate; (3) line manifest "
        "listing every line to be added, each entry quoting the specific text in the governing failure signal that names the behavior the line implements. Immediately before each tool "
        "call that modifies a file, produce: 'Scope: <verbatim quoted failure signal> | Candidate: <named candidate change> | Smaller: <named smaller change whose insufficiency is "
        "demonstrated above> | Lines: <count of lines in manifest>' — if this line does not appear immediately above the tool call that modifies a file in the transcript, the edit is "
        "not permitted. After the edit, produce a post-edit run result by tool call, then verify: (a) the assertion text from the scope declaration is absent from the new run result; "
        "(b) no assertion text appears in the new run result that was absent from all tool-executed run results appearing above this edit in the transcript — any such new assertion "
        "text requires revert. An edit that introduces a failure absent from the pre-edit result must be reverted regardless of interpretation.",
        ...
        "audit of it appears in the transcript — a rewrite or audit preceding this characterization does not satisfy this requirement. All findings and rewrites must use only terms "
        "whose meaning is determined by the structure of instructions and the properties of clauses — a term is structural if its meaning does not change when the instruction's subject "
        "matter changes; a term whose meaning shifts with the instruction's subject matter must not appear.",
