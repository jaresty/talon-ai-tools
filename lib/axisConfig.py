...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "a construction satisfies the property. A property is atomic when no pair of strictly simpler properties satisfies both of the following: (1) their conjunction has identical "
        "coverage to the original property; (2) each part admits at least one admissible construction that falsifies that part without falsifying the other. To test atomicity, first "
        'verify that the property definition to be split satisfies the formality requirement. Emit: \'§ formality check: property [N]: "<verbatim definition>" — [formal | informal: '
        "<reason>]'. If the check concludes informal, rewrite the definition as a typed mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, and "
        ...
        "definition confirmed formal in the final check is the canonical definition for all subsequent steps — a provisional line or split test that quotes a definition not yet "
        "confirmed formal does not satisfy this protocol. Provisional candidate sub-properties may then be introduced immediately before the split test using: 'property [Na]: "
        "<definition>' and 'property [Nb]: <definition>' — these provisional lines exist only for evaluating the proposed split and are not members of the property set unless the split "
        'succeeds. Immediately after the provisional lines, emit exactly one split test: \'§ split test: property [Na]: "<verbatim definition>" — falsified by <construction>; property '
        '[Nb]: "<verbatim definition>" — falsified by <construction>; these falsifiers are independent.\' The quoted definitions must match the immediately preceding provisional '
        "definitions verbatim. A split succeeds only if: the conjunction of the accepted sub-properties has identical coverage to the original property; each quoted falsifier violates "
        "exactly its quoted property while satisfying the other; both falsifiers are admissible constructions within the chosen interpretation. If the split succeeds, replace the "
        "original property with the accepted sub-properties and continue recursively testing each accepted property. If no valid split exists, emit: '§ split test: property [N]: "
        '"<verbatim definition>" — atomic, no valid split: <reason>.\' The quoted definition must match the retained property verbatim. Continue recursively until every retained '
        "property has a split test concluding 'atomic, no valid split.' After all retained properties are atomic, emit a completeness check: '§ completeness check: \"<request "
        "constraints verbatim>\" / <P1 expression> / <P2 expression> / ...' — the request constraints must be quoted verbatim from the chosen interpretation; every retained atomic "
        ...
        "does not satisfy this requirement. Immediately after the completeness check, attempt to identify one explicit request constraint that is not represented by any retained "
        "property. If such a constraint exists, emit '§ properties complete? no' — add one new property covering that explicit constraint, perform the required split testing, and "
        "repeat the completeness check. Only when every explicit request constraint is represented by at least one retained property may the response emit '§ properties complete? yes' "
        "— a '§ properties complete? yes' line without a preceding completeness check does not satisfy this protocol. After completeness has been established, verify observational "
