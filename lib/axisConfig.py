...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "precondition to all rungs including rung 1. A ladder entry whose artifact is the ladder itself is invalid: the ladder cannot govern its own production. The rung 1 coverage "
        "mapping is a separate required artifact produced after the ladder is closed — producing both in the same generative act does not satisfy this requirement. The ladder must be a "
        "completed artifact in the transcript before the rung 1 coverage mapping begins. Rung 0 is the intent artifact — a representation of intent, not intent itself. Its satisfaction "
        "condition names each behavioral requirement in the representation as a distinct item, where each item states the specific behavior it governs such that a reader could "
        "determine whether that behavior is present or absent in a given artifact — a requirement label without a stated behavioral assertion has not been derived. The false-evaluating "
        "state is any named behavioral requirement being absent from the representation. Dimensions of intent not captured by the representation are outside the scope of all coverage "
        'mappings derived from it — the coverage mapping is bounded by the representation, not by intent. A rung 0 satisfaction condition of "given", "present", or any phrase that does '
        "not name and assert the representation's behavioral requirements has not been derived. The artifact is an enumerated list of rungs, each stating: the rung name, the artifact "
        "type produced at that rung, the condition under which the rung is satisfied — including the specific state of the artifact that would cause the satisfaction condition to "
        "evaluate to false; a satisfaction condition entry that cannot name its false-evaluating state has not been derived and is not a valid ladder entry — and, for every rung after "
        "the first, at least one specific outcome that would satisfy the previous rung but be rejected by this rung. A rejects entry is valid only when it states, in the entry itself: "
        "(a) the specific state of the prior rung's artifact that satisfies the prior rung's satisfaction condition — described precisely enough that a reader could verify the prior "
        "rung would accept it — and (b) the specific behavioral dimension that state leaves unconstrained — described precisely enough that a reader could verify the current rung "
        "constrains it. An entry that does not contain both a stated prior-rung-satisfying artifact state and a stated unconstrained behavioral dimension has not been derived, "
        "regardless of how it is phrased. If no such outcome can be named, the proposed rung is not a refinement — it is a parallel task and must be merged with the rung above or "
        "dropped. Describing the ladder structure in prose does not produce this artifact. A rung's satisfaction condition is valid if and only if it names: (a) a locatable artifact, "
        "and (b) a property of that artifact that would evaluate differently if any behavioral dimension of the artifact were absent — a property that evaluates the same regardless of "
        "which behavioral dimensions are present is not valid. A count of items is not a valid property unless each counted item is itself required to name a specific behavioral "
        "dimension such that removing any one item would change the count and leave a dimension uncovered; a bare item count without this per-item behavioral binding is not valid. For "
        "rungs that refine a rung above, the satisfaction condition must additionally confirm that the rung's artifact contains a covering item for each behavioral dimension in the "
        "rung above's artifact — a satisfaction condition that evaluates to true with fewer covering items than dimensions in the rung above is not satisfied. When stating a "
