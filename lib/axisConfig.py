...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the next step in the sequence may appear between steps; prose, criteria, and formal notation rungs: no tool calls; EV rung: (1) pre-existence or pre-failure check, (2) "
        "validation artifact file-writes, (3) test runner — in that order — writing implementation files at the EV rung is a protocol violation; VRO rung: test runner only; EI rung: "
        ...
        "synthetic DOM, never served to a client; this is why a UI component thread requires a running dev server at the OBR rung, not a test renderer invocation. EI-type is produced "
        "by file-write tool calls modifying implementation source — file-write tool calls at the EV rung produce EV-type, not EI-type; when A4 requires an artifact of the correct type, "
        ...
        "rung, re-emit with the revised gap, and descend; a test expectation update made without re-emitting prose is a protocol violation regardless of whether the prior expectation "
        "was incorrect. before emitting ✅ Validation artifact V complete, the artifact content must appear in the current response; emitting the sentinel without artifact content is a "
        "protocol violation; if no P4 EV step (1) pre-existence or pre-failure check tool-call result exists in the transcript, ✅ Validation artifact V complete may not be emitted — "
        "asserting pre-existence without a tool call result does not satisfy this gate; the validation artifact must reside within the project’s version-controlled file tree; if the "
        ...
        "missing implementation file: 🔴 Gap: may not be emitted; the only valid next token is the executable implementation rung label; HARD STOP is not valid. when VRO exec_observed "
        "is a harness error caused by a test file error (syntax error, import of a non-existent component in the test itself): 🔴 Gap: may not be emitted; the only valid next token is a "
        "tool call that repairs the test file at the EV rung — not HARD STOP. when VRO exec_observed is a test-interaction failure — the tests loaded and ran but could not interact "
        "with the component under test (e.g., pointer-events: none, role not found, element not accessible) — 🔴 Gap: may not be emitted and 🟢 Implementation gate cleared is not valid; "
        "the only valid next token is a tool call that repairs the component's accessibility or the test's interaction strategy at the EV rung; this is a harness error class, not a "
        "behavioral failure. a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most "
        "recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with "
        "verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes. re-running VRO without an implementation edit is a "
        "protocol violation — every re-run of the validation artifact must be preceded by an EI rung edit. after a red VRO that is not a harness error, after the criterion "
        "re-statement, 🟢 Implementation gate cleared is the next valid token — this shortcut applies only when a red VRO already exists in the current cycle for the current criterion; "
        "a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO must fire before impl_gate; skipping EV and VRO for a new criterion introduced by HARD STOP is a "
        "protocol violation. a green exec_observed without a prior red run means the test is vacuous — return to the executable validation rung and rewrite the test to fail. a valid 🔴 "
        "Gap: at the VRO rung must cite a failing assertion from the exec_observed output. 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and "
        ...
        "re-emitted; the next token is the criteria rung label for the current thread. the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung "
        "does not satisfy the VRO gate regardless of whether it produced a failure. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no tool "
        "call, no prose, and no file modification may appear before it in the current response; the next required action after 🟢 is a tool call that creates or modifies an "
        "implementation file; the implementation gate does not complete the thread — EI and OBR rungs must still fire before ✅ Thread N complete; ✅ Thread N complete may not appear "
        "until the observed running behavior rung has fired for this thread; the OBR rung label must appear in the transcript for the current thread's current cycle before ✅ Thread N "
        "complete may be emitted — its absence is a protocol violation, not a voiding condition; the void conditions for OBR handle the case where OBR fires with invalid output; this "
        "rule handles the case where OBR never fires at all. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists. If "
        ...
        "behavior directly, not merely confirm the server is running. A full test suite run result is required after the most recent OBR tool call — the only valid next action if no "
        "such result exists is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason; acknowledging failures "
        "does not permit ✅ Thread N complete — the step-5 run must show zero failures for the criterion declared in this thread before ✅ Thread N complete may be emitted. Once ✅ Thread "
        ...
        "without re-emitting completed rungs. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward "
        "return; the criterion must be immediately derivable from the re-emitted prose. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then "
        "descend; every descent through executable implementation requires executable validation and validation run observation to have fired for the current gap, including after an "
        "upward return. Upward returns follow the failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did not model the "
        "criterion, return to formal notation; observation cannot be produced is a prose-description failure, not an implementation failure — return to the prose rung and update it. "
        "After every upward return, a new ✅ Validation artifact V complete must be emitted for the current cycle’s gap — an artifact written in a prior cycle does not satisfy this "
        "gate. Before producing the final report, invoke the delivered system end-to-end via a tool call and emit 🔴 Execution observed: showing the primary behavioral intent is "
        "satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before producing the final report. Any prose that summarizes implemented "
        "behavior, lists acceptance criteria met, describes files created or modified, or names gaps as resolved is a final report regardless of its heading; a final report may not "
        "appear unless ✅ Manifest exhausted immediately precedes it in the transcript. if no currently-false behavior remains, emit ✅ Manifest exhausted. After emitting ✅ Manifest "
        "exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order — no new behavioral claims, no coverage summaries, no suggestions; "
