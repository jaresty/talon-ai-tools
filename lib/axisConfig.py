...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "detect drift. The task artifact cannot certify itself; alignment evidence must come from the independent meta-task.Protocol derivation: before work begins, derive the protocol "
        "rules by applying P1-P5 — observe at every feasible rung before proceeding. Protocol derivation checklist: AFTER deriving rules from P1-P5, BEFORE proceeding, MAKE VISIBLE in "
        "the conversation: [ ] The invariant: claim vs reality — what GOES WRONG if you claim without evidence: fool yourself, completion is false; RULE: evidence before claims [ ] The "
        "ladder structure: 6 steps — Intent → Criteria → Structure → Challenge → Refinement → Verification; what GOES WRONG if you skip: gaps lost, completion false; RULE: one rung at "
        "a time [ ] Challenge: derive from P3,P5 — Challenge derives evaluation from Structure to expose gaps AND independently test continued alignment to declared intent; RULE: "
        "Challenge = apply evaluation showing (1) the smallest observable gap exists and (2) the current artifact still matches intent. IMPORTANT: A gap indicates something is MISSING "
        "- could be a whole thing, structure, or behavior. It does NOT imply creating the complete thing - just addressing what is missing. Challenge must produce MECHANICAL "
        "VERIFICATION for both: show the specific test/command/query/evidence that proves the gap exists, AND show the specific reproducible proof that the current artifact still "
        "aligns to intent. Without both, closure cannot be confirmed later.[ ] Structure before Challenge: derive from P1,P3 — solution organization must precede gap detection; what "
        "GOES WRONG if you skip structure: unclear what to evaluate; RULE: structure before challenge [ ] Criteria before structure: derive from P1 — success conditions before "
        ...
        "evidence must be temporally close to result; what GOES WRONG if delayed: proof weakens; RULE: evidence immediately after result[ ] Evidence echoed: derive from P3,P4 — explain "
        "how evidence proves rules satisfied; what GOES WRONG if unexplained: description indistinguishable from compliance; RULE: explain evidence connection [ ] No contradiction in "
        "completion: derive from P3,P5 — claiming complete while acknowledging gaps is void; what GOES WRONG if you write 'complete' with 'gaps', 'caveats', 'needs': claim invalid; "
        "RULE: scan ENTIRE message for contradiction before 'complete' [ ] Rule citation at action: derive from P1,P3,P4 — cite specific derived rule before each action; what GOES "
        "WRONG if uncited: protocol compliance invisible; RULE: cite rule before action This checklist must appear BEFORE the first ladder rung begins. The derivation results must be "
        "in the chat, not just in your head. Before starting each rung, re-emit key rules from your derivation relevant to that rung. BEFORE transitioning to next rung, verify and "
        "declare: [current rung artifact completed?], [evidence visible?], [gaps remaining?], [intent still aligned?], [next rung requires?]. If gaps remain or verification not shown, "
        "do NOT proceed. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P5. Ladder derived MUST be emitted as: 🔵 Ladder derived — AFTER "
        "producing the ladder table with columns: rung name | artifact type | derivation source | evidence form. No descent before both 🔵 Protocol derived and 🔵 Ladder derived "
        "sentinels — these are gates on all subsequent work. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to finish each rung. Evidence before "
        "claims: for every claim about completion, there must be evidence proving it. Ground complete may only be emitted after verification confirms no gaps remain. Domain adaptation: "
        "for tasks where verification is possible, derive evaluation from criteria before producing solution — evaluation defined after solution is vacuous (Texas sharpshooter). "
        "Challenge exposes gaps and tests intent alignment; Refinement adds only enough to satisfy each proven gap without breaking alignment; Verification confirms both gap closure "
        "and preserved intent. IMPORTANT: When deriving ladder artifact types, use MINIMAL names: - For Refinement, use 'Partial Implementation' or 'Minimal Code Changes' — NOT "
        "'Implementation' or 'full solution' - Artifact type names that imply completeness ('Implementation', 'Full Solution') contradict the gap-driven approach. If output diverges "
        "from protocol intent (e.g., producing full implementation instead of addressing specific gaps), reflect on: Which rung first diverged? What artifact type name drove this? What "
        "would a minimal change look like? Refinement should address ONE gap at a time with minimal changes — not build the entire feature. If you find yourself creating a complete "
        "feature, you have deviated from gap-driven refinement. for writing tasks, it maps to intent→criteria→structure→challenge→refinement→verification; for decision-making, it maps "
        "to intent→criteria→analysis→challenge→refinement→verification. The model should derive the appropriate ladder for the task domain. ",
