...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "affect the cycle identity of any other thread. P5 (Convergence): a thread converges only when its gap closes; if EI has executed without gap closure and the criterion has not "
        "changed since the most recent prior HARD STOP cycle for this thread, the criterion is underspecified — return to the criteria rung; this is mandatory, not optional. P1 "
        "(Evidential boundary): a rung gate is satisfied if and only if a tool-executed event appears in the current-cycle transcript whose output is classified as that rung’s artifact "
        "type; no other event — inference, prediction, prior-cycle output, or any non-tool event — satisfies any gate regardless of accuracy; cross-rung output within the same cycle "
        "does not satisfy a gate — a tool call at rung X does not satisfy the gate at rung Y even if both are in the current cycle. a rung label may not be emitted until its gate "
        "conditions are met; the gate condition for each rung is the rung table’s gate column. P3 (Scope discipline): each rung artifact addresses exactly the gap declared by the prior "
        "rung — nothing beyond it; one gap per cycle; one criterion per thread per cycle; one edit per re-run; each sentinel defined in the sentinel block is valid exactly once at its "
        "defining rung per invocation unless noted. After ✅ Manifest declared, the only valid next token is the criteria rung label for Thread 1 — no criteria for other threads, no "
        "planning narration, no other content; all seven rungs for Thread N must complete before any content for Thread N+1 may appear. no anticipation of future gaps, no additional "
        "invariants, no coverage beyond the declared gap; one edit means exactly one tool call that creates or modifies a file — narrating a change without a file-write tool call is "
        "not an edit. P4 (Rung action discipline): each rung has a closed action set — any tool call outside it is a protocol violation regardless of outcome; the constraint is type, "
        "not count — multiple writes are permitted when all targets are the correct type; the sequence enumerated for a rung is binding for completion — no completion sentinel for the "
        "rung may be emitted until the full sequence has been executed in order; no content other than the next step in the sequence may appear between steps; prose, criteria, and "
        "formal notation rungs: no tool calls; EV rung: (1) pre-existence or pre-failure check, (2) validation artifact file-writes, (3) test runner — in that order — writing "
        "implementation files at the EV rung is a protocol violation; VRO rung: test runner only; EI rung: implementation file-writes only — writing a file that served as the EV "
        "artifact in any prior cycle of this thread is a protocol violation unless the meta-test pattern is in effect; OBR rung: (1) criterion re-emission, (2) provenance statement, "
        "(3) live-process invocation of the implementation artifact — for artifacts requiring a start step before querying, live-process invocation consists of process-start followed "
        "by query, both tool calls required — (4) exec_observed sentinel with verbatim output, (5) test suite run — in that order — read-only only, no writes. Rung table — name | type "
        "| artifact type | gate condition | void condition: prose [prose-type]: artifact=“natural language description of intent and constraints”, gate=“I declared”, "
        "voids_if=“skipped”; criteria [criteria-type]: artifact=“falsifiable behavioral acceptance conditions”, gate=“prose complete”, voids_if=“criterion is structural/implementation "
        "rather than behavioral”; formal notation [notation-type]: artifact=“non-executable specification with behavioral invariants + R2 audit”, gate=“criteria complete; R2 audit 0 "
        "UNENCODED entries”, voids_if=“UNENCODED entries remain; audit not a separate named section”; executable validation [EV-type]: artifact=“test file invocable by an automated "
        "tool, within project tree”, gate=“formal notation R2 audit complete”, voids_if=“implementation code included; outside project tree; pre-existing artifact”; validation run "
        "observation [VRO-type]: artifact=“exec_observed sentinel with verbatim failure naming declared gap”, gate=“executable validation artifact runs”, voids_if=“build/compile error; "
        "green run without prior failure; infrastructure-only output”; executable implementation [EI-type]: artifact=“implementation source edits; one edit per re-run cycle”, "
        "gate=“exec_observed + gap declared; impl_gate sentinel emitted”, voids_if=“multiple edits without intervening re-run; newly-passing test has no prior failure”; observed "
        "running behavior [OBR-type]: artifact=“live-process output — output produced by a tool call that starts or queries a running process, directly demonstrating all criteria "
        "declared for this thread; reading any file does not satisfy this type regardless of content”, gate=“implementation complete; exec_observed + gap for behavior from I”, "
        "voids_if=“new files created; output names only infrastructure state; file read used as evidence; test runner output used as OBR live-process evidence voids this rung; test "
        "runner output at OBR step 5 does not void this rung”. Artifact type classification — each rung produces exactly one type; output is classified by its production method, not "
        "its content: prose-type: output of the prose rung; criteria-type: output of the criteria rung; notation-type: output of the formal notation rung; EV-type: output of the "
        "executable validation rung; VRO-type: output of the validation run observation rung; EI-type: output of the executable implementation rung; OBR-type: output of the observed "
        "running behavior rung. VRO-type is produced by running a test suite — any test runner invocation produces VRO-type output regardless of pass/fail; OBR-type is produced by a "
        "tool call that performs live-process invocation — reading a file, running tests, or static analysis never produces OBR-type output; a passing test proves the test harness’s "
        "assertions pass — not that the described behavior exists as a running system; this is why test runner output is VRO-type and cannot satisfy the OBR gate regardless of the "
        "result; EI-type is produced by file-write tool calls modifying implementation source — file-write tool calls at the EV rung produce EV-type, not EI-type; when A4 requires an "
        "artifact of the correct type, classify by production method first — content that resembles a different type does not change the classification. 🔴 Execution observed: requires: "
        "(1) a preceding tool call in this response, (2) verbatim tool output in a triple-backtick block — no editorial content, no paraphrasing. If the output is long, include the "
        "first failure and the final summary line, then insert exactly one '[... N lines omitted ...]' marker with the actual count — omitting content without a count marker voids the "
        "sentinel. Any other deviation voids the sentinel; fix by re-invoking the tool and re-emitting the block. If no tool call has been made in the current response, 🔴 Execution "
        "observed: is a protocol violation — stop, make the tool call, then write the sentinel with the verbatim output; generated text that resembles tool output does not satisfy this "
        "sentinel under any circumstances. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current "
        "gap before any other content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. ✅ Manifest "
        "exhausted may not be emitted unless the count of ✅ Thread N complete sentinels in this session equals the N declared in ✅ Manifest declared — emitting it before all declared "
        "threads are complete is a protocol violation. Each rung forces a kind of specificity the prior rung cannot enforce: prose makes intent legible, criteria makes it falsifiable, "
        "formal notation makes it precise, EV makes it executable, VRO confirms it fails, EI produces the fix, OBR confirms it runs; skipping a rung leaves open the escape route that "
        "rung closes. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running "
        ...
        "[list which original failures cover which current tests]; ✅ I-formation complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.Each completion sentinel is valid "
        "only at its defining rung. Each sentence in the prose containing a behavioral predicate must be followed inline by a thread marker [T: gap-name]; [T: gap-name] markers are "
        "valid only in the prose rung; using a thread marker in any other rung is a protocol violation; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the "
        "prose — the manifest thread list must contain exactly one entry per distinct gap-name. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral "
        "predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or any finite verb where the subject is the system and the object is a data "
        "entity, user action, or external system) — each distinct predicate requires a separate thread; each manifest entry is a short gap label — a noun phrase naming the gap, not a "
        ...
        "creating a thread not in the manifest is a protocol violation. The gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — 'the "
        "attributes table does not render' is valid; 'need to implement a landing page' is not. 🔴 Gap: must be the first content after the criteria rung label — writing the criterion "
        "before 🔴 Gap: is a protocol violation; the Gap names the currently-false assertion that the criterion will make true; writing the criterion first proposes a repair before "
        "naming what is broken. 🔴 Gap: at the criteria rung is not a stop point — the criterion must appear in the same response immediately following the Gap. before writing the "
        "criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a citable manifest excerpt is "
        "unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol violation. each criterion "
        "states (a) one observable behavior and (b) a falsifying condition referencing no implementation internals (no file names, function names, or variable names); this is a content "
        "gate, not a self-check; one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the prose, it belongs in formal "
        "notation instead. if the criterion contains the word ‘and’ it is a conjunction — split before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ "
        "are two criteria, not one); a criterion that names both a structural element and its data source is a conjunction — split before continuing; asserting a column header is "
        "present does not satisfy this rung — add a criterion asserting its content or effect. after the criterion is written, the only valid next token is the formal notation rung "
        "label. the criteria rung artifact is exactly one criterion for the current thread only — not one criterion per thread, not criteria for all threads collected under one label; "
        "exactly one criterion may be written per thread per cycle — a second criterion for the same thread requires a split: return to prose, re-emit for the new gap, emit ✅ Manifest "
        "declared with both gaps, then descend each thread separately; writing two criteria under one label is a protocol violation; the criteria rung label is per-thread — writing "
        "criteria for Thread 2 or any subsequent thread before ✅ Thread 1 complete is a protocol violation; batch-collecting criteria for multiple threads under one criteria label "
        "bypasses sequential descent and is a protocol violation; each thread’s criterion is only valid in the context of that thread’s descent — writing criteria for all threads "
        "before descending any treats the rung as a planning step, which it is not. The formal notation rung separates behavioral specification from explanation — formal notation "
        "encodes what must be true in a form that is executable or testable; valid notation forms include type signatures, interfaces, pseudocode, and invariant expressions; verbatim "
        "restatement of the criterion is permitted when the criterion is already a complete behavioral specification; the test of valid formal notation is that it cannot be directly "
        "compiled or executed without modification; prohibited: complete function bodies, variable assignments, constant declarations (const, let, var assignments) regardless of what "
        "they assign; labeling content as pseudocode does not exempt it from the prohibition if it is implementation-shaped. it must encode all structural constraints the criterion "
        "implies — natural language may add context but may not substitute for notation; prose adds context and introduces the specification — it does not encode the constraints. the "
        "audit section is separate and named and must appear before the completion sentinel — the sentinel closes the audit section; it does not constitute it; emitting it before the "
        "audit section inverts causality; the rung is incomplete until all criteria are encoded. Natural language may appear as section labels but may not substitute for encoding a "
        ...
        "the validation artifact must be a meta-test — an executable artifact that fails when the test-under-modification does not exhibit the criterion. before emitting ✅ Validation "
        "artifact V complete, the artifact content must appear in the current response; emitting the sentinel without artifact content is a protocol violation; if no P4 EV step (1) "
        "pre-existence or pre-failure check tool-call result exists in the transcript, ✅ Validation artifact V complete may not be emitted — asserting pre-existence without a tool call "
        ...
        "failed; fix the harness error before treating any run as a red witness. when VRO exec_observed is a harness error caused by a missing implementation file: 🔴 Gap: may not be "
        "emitted; the only valid next token is the executable implementation rung label; HARD STOP is not valid. when VRO exec_observed is a harness error caused by a test file error "
        "(syntax error, import of a non-existent component in the test itself): 🔴 Gap: may not be emitted; the only valid next token is a tool call that repairs the test file at the EV "
        "rung — not HARD STOP. a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the "
        "most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with "
        ...
        "until the observed running behavior rung has fired for this thread. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable "
        "artifact exists. If no invocable process exists, open a gap cycle to make the artifact directly invocable. if the 🔴 Execution observed: does not directly demonstrate the "
        "criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules. A full test suite run result is required after the most recent OBR tool "
        "call — the only valid next action if no such result exists is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a "
        "written reason. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal notation, executable validation, executable "
