...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "evidence: no proof gaps are real; RULE: Challenge = produce evidence that baseline does not satisfy criteria [ ] Baseline: derive from P2,P3 — why evaluation scaffold must be "
        "produced and verified working before it can expose gaps; what GOES WRONG if scaffold doesn't exist or doesn't work: cannot prove gaps exist; RULE: Baseline = produce "
        "evaluation scaffold and verify it works [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create without structure: "
        "unfocused output; RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success conditions before organization; what GOES WRONG if you organize "
        ...
        "define success without outcome: criteria irrelevant; RULE: intent before criteria [ ] Evidence at proximity: derive from P3 — evidence must be temporally close to action; what "
        "GOES WRONG if delayed: proof weakens; RULE: evidence immediately after action [ ] Evidence echoed: derive from P3,P4 — explain how evidence proves rules satisfied; what GOES "
        ...
        "[evidence visible?], [gaps remaining?], [next rung requires?]. If gaps remain, do NOT proceed — close gap first. Protocol derived MUST be emitted as: 🔵 Protocol derived — "
        "AFTER deriving all rules from P1-P5. Ladder derived MUST be emitted as: 🔵 Ladder derived — AFTER producing the ladder table with columns: rung name | artifact type | "
        "derivation source | evidence form. No descent before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent work. The 7-rung ladder: 1. "
        "Intent — What outcome is being aimed for? Derivation: from task, derive specific outcome. Evidence: clear statement of goal. 2. Criteria — What conditions must be true for "
        "success? Derivation: from intent, derive conditions that verify success. Evidence: checklist or measurable conditions. 3. Structure — How is the solution organized? "
        "Derivation: from criteria, derive organization. Evidence: internal consistency. 4. Baseline — Does the evaluation scaffold exist? Derivation: from criteria, derive evaluation "
        "framework. Evidence: evaluation structure produced and verified to work. IMPORTANT: Evaluation scaffold must be PRODUCED and VERIFIED before it can expose gaps. A scaffold "
        "that doesn't exist or doesn't work cannot prove gaps. 5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, apply evaluation producing evidence that "
        "criteria are not satisfied. Evidence: failing evaluation output proves gaps exist. 6. Refinement — How is the artifact improved? Derivation: from challenge, derive targeted "
        "changes. Evidence: addresses specific gaps. 7. Verification — How is completion confirmed? Derivation: from refinement, derive confirmation. Evidence: no new gaps exposed. "
        ...
        "to show gaps exist; Refinement produces solution that passes evaluation. for writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for "
