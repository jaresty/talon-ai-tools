...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the subject of the next rung and ## Formalization must not close; when no phrase in the expression admits two non-equivalent outputs, write 'alternative satisfier: [text]' "
        "where [text] names a concrete thing that the expression would admit but that the governing goal would reject — a formalization that cannot name such a satisfier is not yet "
        "precise enough to close; '§ formalization complete' must not appear before '§ ambiguity test:' in the transcript — a '§ formalization complete' that appears without a "
        "preceding '§ ambiguity test:' does not satisfy this requirement; '§ formalization complete' must not appear before 'alternative satisfier:' followed by non-empty text in the "
        "transcript — a '§ formalization complete' with no preceding 'alternative satisfier:' disclosure does not satisfy this requirement; write '§ formalization complete' only after "
        "both '§ ambiguity test:' and 'alternative satisfier:' with content have appeared; '## Enforcement sequence' must not appear before '§ formalization complete' in the "
        "transcript; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the transcript, and must appear before the first "
        "file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a literal substring what the prior sub-heading's passing state "
        "admits that this sub-heading rejects; (4) verify coverage: '## Completion check' must not appear before '## Enforcement sequence' in the transcript and must not appear before "
        "'§ implementation permitted' in the transcript; each covered dimension cites a verbatim string that appears either as a substring of a tool-result block produced after '§ "
        "implementation permitted' and visible above '## Completion check', or, when no tool-result blocks are present after '§ implementation permitted', as a substring of the "
        "response text produced in this turn; when every covered dimension cites such a substring, write '§4 coverage verified'; '## Completion check' must not introduce a dimension "
        "absent from '## Behavioral dimensions' — a dimension named for the first time inside '## Completion check' does not satisfy any coverage requirement and does not extend the "
        "governed dimension set; a turn containing a satisfied 'Gate condition:' block must contain the governed tool call in the same turn — prose appearing between the 'Gate "
        "condition:' block and the governed tool call does not satisfy this requirement; every assistant turn after '§ implementation permitted [N]' has appeared in the transcript that "
        "does not contain a line beginning with '§ implementation permitted [' must contain '## Completion check' — a turn that does not contain a line beginning with '§ implementation "
        "permitted [' and does not contain '## Completion check' does not satisfy ground; '## Completion check' is satisfied only when the literal string '§4 coverage verified' appears "
        "within it — a '## Completion check' block that does not contain '§4 coverage verified' does not satisfy this requirement and the governed turn must continue to the next "
        "derived step rather than ending — the first non-blank line after the '## Completion check' heading must be 'Gate condition:'; a turn whose final non-blank content line is '§4 "
        ...
        "dimensions covered; for each path, name the literal string whose presence closes it; write '§5 enumeration complete' when no further open path remains; any referent named in "
        ...
        "exactly once; the string '§ implementation permitted' followed by a space and a decimal integer (e.g. '§ implementation permitted 1') where the integer is the 1-based index of "
        "the file-modifying tool call this sentinel governs must appear in the transcript after '§5 enumeration complete'; a sentinel line of the form '§ implementation permitted [N]' "
        "containing the literal bracket characters '[' and ']' does not satisfy this requirement — only a decimal integer satisfies the index requirement; no file-modifying tool call "
        "may appear before the first such sentinel; the literal string '§ implementation permitted' followed by a space and a decimal integer with the integer matching the 1-based "
        "index of the governed tool call must appear immediately before the `(i)` line of the five-line block with no intervening blank lines or other content between the permit "
        "sentinel and the `(i)` line — a blank line, horizontal rule, or any other content between them does not satisfy this requirement; in the assistant text block immediately "
        "before each file-modifying tool call, in the same response turn as that tool call — a user message appearing between '§ implementation permitted [N]' and the tool call does "
        "not satisfy this requirement; a file-modifying tool call not immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the five-line "
        "block does not satisfy this requirement. Quoted-span exclusion: only an occurrence of a named sentinel or heading string on a line that is not inside a markdown code fence (a "
        "span beginning with ``` or ~~~ on its own line) and does not begin with '>' (a markdown block-quote prefix) counts toward compliance — an occurrence inside a code fence or on "
        "a line beginning with '>' does not satisfy any rung or ordering requirement. Rung-completion sentinel finality: each of the strings '§0 observed', '§1 goal derived', '§1a "
        "decomposed', '§1b candidates', '§2 dimensions closed', '§ formalization complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank "
        "line of the turn in which it is written — no prose, heading, or non-sentinel content may follow a rung-completion sentinel in the same turn; a turn that contains a "
        "rung-completion sentinel followed by any non-blank content that is not itself a rung-completion sentinel or exit string does not satisfy ground. Continuation invariant: when "
        "the final non-blank content line of a turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage "
