...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "and each property produced by a merge — must not constrain an internal mechanism, architecture, algorithm, or representation as its primary subject unless the chosen "
        "interpretation explicitly requires that subject; a property that constrains an internal subject without that explicit requirement is out of scope and must not appear in the "
        "retained set. Externally observable status is evaluated independently of internal-subject status and is not itself a gate for retained-set entry. Scope validation is applied "
        "to the canonical definition produced by Step 1, not to the provisional surface form. A property is atomic when the recursive decomposition procedure reaches Step 4 and no "
        "valid split exists. Step 1 — Canonicalize the property. Emit: '§ formality check: property [N]: \"<verbatim definition>\" — [formal | informal: <reason>]'. If informal, "
