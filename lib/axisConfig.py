...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "property, the governed artifact may be the implementation, the regression guard, a configuration file, or another explicitly governed artifact; before making any "
        "behavior-altering modification, emit 'Quoted implementation: <smallest contiguous verbatim text uniquely identifying the intended modification to the governed artifact>' — "
        "this sentinel must appear in the transcript before the tool call that makes the modification; a modification tool call that appears before the 'Quoted implementation:' "
        "sentinel does not satisfy this token; after the 'Quoted implementation:' sentinel, make the modification via a tool call and wait for the tool-result block confirming it; if "
        "step (4) required constructing a temporary violating state because the governed artifact already satisfied property [N], step (5) is satisfied by confirming the original "
        "state is restored — emit 'Quoted implementation: <verbatim text identifying the restored governed artifact>' before the restoration tool call; no subsequent step may occur "
        "until this sentinel has been emitted and the corresponding modification or restoration tool call has completed;(6) attempt to construct a minimal modification to the quoted "
