...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "absent passes trivially when the behavior is absent — structural test: does the assertion pass when the implementation does not exist? If yes, it has no natural FAIL state and "
        "requires perturbation: run it against a state where the behavior exists but is deliberately wrong. Principle 5 (type matching): Assertion type must match behavior type — a "
        "static check cannot govern executable behavior; a compiler-enforced constraint needs no additional assertion. Principle 6 (proximity): prefer the assertion layer closest to the "
        "behavior that can observe a FAIL — the further the assertion from the behavior, the weaker the attribution to the governed contract (Principle 1 corollary). In multi-layer work, "
        "proximity requires an independently-failing per-layer assertion: a cross-layer FAIL cannot isolate which layer's contract was violated. Principle 7 (durability): assertion "
        "durability must match behavior durability — every permanent behavioral change requires a committed, automatically re-executable unit assertion in the test suite. A session-only "
        "invocation, manual check, or one-off script is not durable: it demonstrates current state but will not re-fire when behavior regresses. A FAIL tool result from a session-only "
        "script satisfies the Phase 2 observation requirement but does not close the implementation step; the committed assertion must exist before the step closes. Named exceptions "
        "(structural test required to invoke — self-assessment is not sufficient): Exception A (structural floor): when the behavior produces no observable output of any kind — no file, "
        "no parsed structure, no exit code, no return value — behavioral assertions are impossible and structural floor assertions (file existence, identifier presence) are required "
        "instead; these do not satisfy gate; a manual verification protocol must be declared: who verifies, by what procedure, and what binary pass/fail condition is defined in advance "
        "and checkable by someone other than the author. Structural test to invoke: would the behavior produce any observable output if it ran? If yes, Exception A does not apply. "
        "Exception B (exploratory): when the task has no predetermined correct output — the goal is to discover what is measurable or detectable, not to implement a specified behavior — "
        "governing assertions must be observability assertions (claims about what can be detected or measured), not outcome assertions that encode a direction before exploration has "
        "occurred. Structural test to invoke: is there a specified correct output? If yes, Exception B does not apply. PHASE 1 — classification (text artifact, required before any "
        "process derivation): produce a classification block for each assertion using this exact structure: Assertion: <name or description> | Principle: <which principle governs it> | "
        "Natural FAIL state: <yes, or no — structural test: does this assertion pass when the implementation does not exist?> | Coverage artifact: <if Natural FAIL state is no, describe "
        "the specific incorrect behavior to introduce to produce a FAIL tool result; otherwise n/a>. Every assertion must have its own block. Phase 1 is complete when every assertion has "
        "a block. PHASE 2 — coverage artifact (tool result required before implementation): every assertion requires a FAIL tool result before any implementation step. For "
        "natural-FAIL-state assertions (Natural FAIL state: yes), the FAIL comes from the initial red-state test run. For no-FAIL-state assertions (Natural FAIL state: no), the FAIL "
        ...
        "these principles require and follow it — the derivation must be visible.",
