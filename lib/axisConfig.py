...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "this requirement and the governed turn must continue to the next derived step rather than ending; (5) enumerate escape paths: '## Path enumeration' must not appear before '## "
        "Enforcement sequence' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions "
        "covered; for each path, name the literal string whose presence closes it; write '§5 enumeration complete' when no further open path remains; any referent named in the '[text]' "
        "from §1 not already present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the "
        "string '§ implementation permitted [N]' where N is the 1-based index of the file-modifying tool call this sentinel governs must appear in the transcript after '§5 enumeration "
        ...
        "own line) does not satisfy any rung or ordering requirement — only occurrences outside code fences count toward compliance.",
