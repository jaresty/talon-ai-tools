...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "exclusively from the executor-produced result entries (c), (d), and (g); within a tool-result block, only lines on which (a) and (c) co-occur are valid (g) source lines — "
        "test description lines, docstring echo lines, parametrize label lines, and summary lines in executor output are not valid sources for (c) or (g) even when they appear in the "
        "same tool-result block as a FAIL line; during the creation-step exception, the allow-list is (d) only; a reference to an identifier not on the allow-list within a stub or "
        "partial implementation is not exempt; a construct whose complete identifier does not appear as a verbatim substring of the active allow-list entries must not be introduced in "
        "the same action and requires its own governing FAIL before it can be introduced. Scope-governing entries: only (c), (d), and allow-list identifiers from (g) are "
        "scope-governing entries — entries (a), (b), (e), (f), and (f2) are not scope-governing entries and do not contribute to the allow-list. Artifact gap: when the Falsify "
        "derivation block contains the line '(g) observed FAIL result: unavailable' and no tool-result block in this transcript produced by a named executor invocation contains (c) as "
        "a substring, the derivation is incomplete. A derivation is complete only when a line beginning 'Gap artifact:' appears in the transcript immediately above the Falsify "
        "derivation block, naming the executor invocation designated to produce a result containing (a) before (c). When 'Gap artifact:' is present, the tool call that produces that "
        "artifact satisfies the creation-step exception; no file-modifying tool call may appear before the tool call that produces the named artifact. A governed symbol renamed by a "
        "refactor is a new governed symbol requiring a new derivation block before the rename action. If no named executor exists for the governed artifact type, this token does not "
        "apply.",
