...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "failure, or constraint pressure at a lower rung is not a valid upward return trigger), upward return conditions (what triggers return, what does NOT trigger return and why "
        ...
        "(explain WHY each rule follows from P1-P6, not just WHAT the rule is); each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are "
        "protocol violations; emit Protocol derived after deriving all rules; emit Ladder derived after producing the rung table; ground complete may only be emitted as the outcome of "
        "the observation loop — emitting it outside the loop is a protocol violation. Rung validity test: a rung is valid iff a human reviewer with only that rung's artifact can "
        "evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → executable validation → executable implementation — each rung must "
        "be completed in this order; emit the rung label before producing its artifact; emit completion sentinel after artifact is produced and gate is satisfied. EV rung: edits only "
        "validation files. EI rung: edits only implementation files. An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another "
        ...
        "and naming are evidence, not authority. Sentinel formats — ✅ Ground entered — prose rung begins [gate: first content emitted in this response; no artifact, code, prose, or "
        "reasoning may precede this sentinel — preceding content voids the session]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in "
        "current response; rung table precedes this sentinel]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] [gate: tool call made "
        "in the current response immediately before this sentinel; verbatim output in triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed "
        "with non-empty failing output precedes this token in current response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: "
        ...
        "implementation; every assertion in the validation artifact must appear failing in the verbatim exec_observed output (e.g. 'Expected … Received …', 'AssertionError', 'FAILED', "
        "specific test name + failure reason) — an exec_observed that shows only some assertions failing, only infrastructure failure, import error, process crash, or a model-described "
        "summary of failure does not satisfy this gate; if the suite cannot reach assertions, the implementation artifact for that cycle resolves only the infrastructure gap, and this "
        "gate cannot be emitted until a subsequent cycle produces assertion-level failure output for every assertion]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one "
        "criterion in this rung's artifact; criterion contains no conjunction; formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation "
        "artifact V complete [gate: this sentinel may only be emitted after a tool call in this response has written the validation file to disk; emitting this sentinel without a "
        "preceding tool call that wrote the file is a fabrication that voids the sentinel and the rung; the tool call output must show the file was successfully written]; ✅ Thread N "
        "complete [gate: meta exec_observed after executable implementation shows the gap declared for this thread is no longer present in running behavior; every assertion in the "
        "validation artifact must appear passing in this exec_observed output]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in "
        ...
        "a preceding 🔵 Closing observation in the current response is a protocol violation]; 🔵 Intent logged — id: [unique id] | file: [path] | target: [assertion name] | evidence: "
        "[verbatim red output showing failure] [gate: this sentinel must appear immediately before every file-write tool call at every rung; include a unique id for this edit that will "
        "be matched by impl_intent_achieved; every file edit requires impl_intent + impl_intent_achieved pair - editing any file without this pair is a protocol violation; the artifact "
        "type is determined by the rung table - the file must be of the artifact type permitted at the current rung as determined by the rung table; if a file of a different artifact "
        "type needs editing, emit an upward return to the appropriate rung first; the target must be a specific test assertion name and the evidence must be verbatim output from "
        "running the validation file showing that test assertion failing in the current state; emitting this sentinel without a preceding exec_observed showing the target test "
        "assertion failing voids the intent and the subsequent write; fabrication of red evidence (output that does not come from actual test execution) voids the sentinel and the "
        "rung; for backfill cases where tests already pass and cannot be seen red, perturb the implementation (introduce a controlled fault) to observe the test fail, then fix the "
        "perturbation]; 🔵 Intent achieved — id: [unique id] | file: [path] | target: [assertion name] | evidence: [verbatim green output showing passing] [gate: this sentinel must "
