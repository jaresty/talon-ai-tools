...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "Before producing any artifact: open with a rung manifest — a short document listing each planned artifact, its rung label, and its derivation source — and output the "
        "exact phrase 'Rung manifest complete' on its own line before descending. No artifact from any lower rung may appear before it. Execution gate: before descending to any rung, "
        "the gap at the current rung must be confirmed by actual observation — for executable rungs (tests, type checks, schema validators, running programs, passing validation runs), "
        "observation means executing the artifact and recording its output; anticipated outcomes, asserted expectations, or documented rationales for skipping execution do not satisfy "
        "this requirement and do not permit descent. A rung is not achievable only when the domain lacks a standard artifact type — not because execution is inconvenient or its outcome "
        "is anticipated. There are no documentation-based exemptions from this evidence-driven flow. The response treats the declared intent (I) as fixed. At each step, the response "
        "produces a representation V derived faithfully from I and the prior rung (or from I alone, at the first rung — I is not an artifact; artifacts are derived from I) — the form "
        "changes, the intent does not — such that any proposed output O can be evaluated against V without consulting I again. V is a reliable rejection filter: one violated constraint "
        "suffices to reject O. Accepting O additionally requires trusting that V captured all governing constraints from I, which V cannot prove from the inside. Faithful derivation "
        "means every constraint V imposes is either directly stated in the prior rung or strictly implied by it, with no additional judgment about I required. Call such a representation "
        "a constraint artifact. Note the asymmetry: producing V requires I; evaluating O against V does not — once produced, V is self-contained as a check. The primary failure mode is "
        ...
        "identifying its rung and what it was derived from. File naming conventions are left to domain context; where none exists, a name that reflects the artifact's rung "
        "(e.g., validate_*, *_spec, *_test) aids legibility without being required.",
...
AXIS_TOKEN_METADATA: dict[str, dict[str, AxisTokenMetadata]] = {
    ...
    "method": {
        ...
        "ground": {
            "definition": "Before producing any artifact: open with a rung manifest — a short document listing each planned artifact, its rung label, and its derivation source — "
            "and output the exact phrase 'Rung manifest complete' on its own line before descending. No artifact from any lower rung may appear before it. Execution gate: "
            "before descending to any rung, the gap at the current rung must be confirmed by actual observation — for executable rungs (tests, type checks, schema validators, "
            "running programs, passing validation runs), observation means executing the artifact and recording its output; anticipated outcomes, asserted expectations, or "
            "documented rationales for skipping execution do not satisfy this requirement and do not permit descent. A rung is not achievable only when the domain lacks a "
            "standard artifact type — not because execution is inconvenient or its outcome is anticipated. There are no documentation-based exemptions from this evidence-driven "
            "flow. The response treats the declared intent (I) as fixed. At each step, the response produces a representation V derived faithfully from I and the prior rung (or "
            ...
            "when the domain provides no standard artifact type for it. Present V (the lowest constraint artifact reached), then the exact phrase 'Validation artifact V "
            "complete' on its own line (this phrase marks V as complete and self-contained — what follows is evaluated against V, not against I), then O satisfying V. Each "
            "artifact should carry a header comment identifying its rung and what it was derived from. File naming conventions are left to domain context; where none exists, "
            "a name that reflects the artifact's rung (e.g., validate_*, *_spec, *_test) aids legibility without being required.",
            "distinctions": [
                {
