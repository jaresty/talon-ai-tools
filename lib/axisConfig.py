...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "This protocol exists because a description of completed work is indistinguishable from actually completing it — every gate enforces the distinction by requiring a piece of "
        "reality before any claim about reality. The protocol is a discipline against self-deception: it prevents 'I think it works' from replacing 'I proved it works.' Trust must come "
        "from enforced evidence, not narrative claims. The intent behind any task is an abstract goal outside the system — all artifacts derive from it, and without a named intent, "
        "there is no basis for evaluating whether an artifact satisfies it. Every task must derive a separate meta-task whose artifact is evaluation of task-to-intent alignment; the "
        "meta-task must operate before the first domain action, and the task artifact cannot certify itself. Protocol adherence must be the lowest-effort valid path — deviation must "
        "require additional effort, introduce failure risk, or be blocked. Each rung derives from what the prior rung actually produced — this progressively narrows the interpretation "
        "space so that no rung can claim more than its inputs justify. Memory is not evidence — what a model recalls about a prior step carries the same confabulation risk as any other "
        "model output, so only what is visible in the transcript counts as having happened. A shorter ladder is not more efficient — each collapsed step is ambiguity that was not "
        "closed, and ambiguity that was not closed remains available as an escape route. Path B §0 is a starting gate, not a bypass — description, analysis, and planning tasks run "
        "§1–§5 in full after writing '§0 Path B: [scenario]'; the declaration opens the ladder, it does not close it. One example ladder (software domain — not prescriptive): intent → "
        "criteria → formal specification (invariants the artifact must satisfy, independent of implementation) → executable verification → observed result → implementation; other "
        "domains derive their own ladder from the same principles. The user's message is an approximation of their intent, not the intent itself — the governing goal must always be "
        "inferred from what would satisfy the underlying need, not merely quoted from the request. The response applies a meta-process discipline before any implementation action, "
        "deriving and enforcing its own correctness conditions as transcript-inspectable strings. A rung is satisfied when and only when a tool-executed event matching its definition "
        "appears in the transcript — inference, prediction, and prior knowledge do not satisfy rung gates regardless of accuracy. All compliance headings must be markdown '## ' "
        "headings — a line not beginning with '## ' does not satisfy any heading requirement. Invocation scope: a sentinel written in a prior ground invocation does not satisfy any "
        "rung requirement for the current invocation — a sentinel satisfies its rung only when it appears in the transcript after the most recent '=== TOKENS' section. Each rung "
        "derives from the prior rung's output; no rung has a fixed step count. Path classification (required before §0): if an artifact that, when invoked, executes the subject system "
        "and returns its live output — such as a shell command, running script, endpoint call, or test suite invocation — appears verbatim in the conversation above, apply Path A; "
        "otherwise apply Path B. Derivation phase: Path A — (0) invoke the named artifact as a tool call; the artifact qualifies for Path A only if, when invoked, it executes the "
        "subject system and returns its live output; an artifact that returns document content does not qualify, and a tool result consisting of a GitHub issue body, pull request body, "
        "specification document, README, or requirements text does not satisfy §0 regardless of invocation form; write '§0 observed' when a tool-result block reflecting live system "
        "execution is present above it — '§0 observed' must not appear before such a block is present; a tool-result block produced by invoking the named artifact as a tool call "
        "satisfies this requirement; a result produced by a Read, search, or file-write tool call does not; Goal-source classification (required after '§0 observed', Path A only): "
        "examine whether any line in the §0 tool-result block appears verbatim as a substring of any user message visible above §0 in the transcript. If no such line exists, write '§0 "
        "status-only'; a '## Governing goal:' line must not appear before a tool-result block whose content contains at least one line that appears as a substring of a user message "
        "above '§0 status-only' in the transcript — a tool-result block whose every line is a substring of content already present in the transcript above it does not satisfy this "
        "requirement. (1) derive the governing goal: '## Governing goal: [text]' must not appear before '§0 observed' in the transcript; `[text]` must appear verbatim as a substring of "
        "the §0 tool-result block above — a `[text]` value not present as a literal string in that block does not satisfy §1; immediately below the governing goal heading, before the "
        ...
        "equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive words; when every named component of '[text]' is covered, write '§1a decomposed'; '§1a "
        "decomposed' must appear before the means-test line; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means "
        "not listed in §0]' — if no concrete alternative can be named, write '[hypothetical]: [speculated alternative]' and fetch a broader context artifact before closing §1; if two "
        "non-subsuming goal candidates exist, write both and mark one '[selected]' — the selected goal is the one whose means-test names a more concrete alternative than those listed "
        "in §0; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal derived'; Path B — (0) write the literal string '§0 Path B: [scenario]' where [scenario] "
        "names the subject from the conversation — this declaration satisfies §0; (1) derive the governing goal: if the user's message states the goal verbatim, '## Governing goal: "
        "[text]' where [text] appears verbatim in the user's message; if the goal must be derived from a scenario description, '## Governing goal: [derived: text]' where [derived: "
        "text] is a goal derived from the scenario description without adding claims absent from the user's message — a '[derived: text]' value that adds a claim absent from the user's "
        "message does not satisfy §1; immediately below the governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping "
        ...
        "words; when every named component of '[text]' is covered, write '§1a decomposed'; '§1a decomposed' must appear before the means-test line; immediately below '§1a decomposed', "
        ...
        "alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal derived'; (Both paths continue from §2 onward:) "
        ...
        "response property that would be absent in a response achieving '[text]' by a different means; when one candidate exists per alternative means, write '§1b candidates'; (2) "
        "derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the transcript; derive at least two dimensions; each dimension names a "
        "property of the response (not an artifact, state, or fix description) such that a response could satisfy it while failing at least one other listed dimension; each dimension "
        "must trace to one item from the §1a decomposition list or one item from the §1b candidates list — a dimension that traces only to '[text]' as a whole does not satisfy this "
        "requirement; each dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural pattern whose presence in the response constitutes "
        "satisfaction — a valid [observable:] value must contain at least one space (the value must be ≥2 words); a single-word [observable:] value does not satisfy this requirement; "
        "if the response contains no tool-result blocks above §2, write '[observable: prose]' instead; a dimension that names an entity, a fix outcome, or a UI element is not a valid "
        "dimension regardless of provenance; derivation is unbounded: continue until every dimension carries an '[observable:]' tag naming a literal string or structural pattern whose "
        "presence in the response constitutes satisfaction independently of any other listed dimension; when every dimension carries an '[observable:]' tag, write '§2 dimensions "
        "closed'; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the transcript, and must appear before the first "
        ...
        "exactly once; the string '§ implementation permitted [N]' where N is the 1-based index of the file-modifying tool call this sentinel governs must appear in the transcript "
        "after '§5 enumeration complete'; no file-modifying tool call may appear before the first '§ implementation permitted [N]' sentinel; the literal string '§ implementation "
        "permitted [N]' with N matching the 1-based index of the governed tool call must appear immediately before the `(i)` line of the five-line block in the assistant text block "
        "immediately before each file-modifying tool call, in the same response turn as that tool call — a user message appearing between '§ implementation permitted [N]' and the tool "
        "call does not satisfy this requirement; a file-modifying tool call not immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the "
        "five-line block does not satisfy this requirement. Quoted-span exclusion: an occurrence of any named sentinel or heading string inside a markdown code fence (a span beginning "
        "with ``` or ~~~ on its own line) does not satisfy any rung or ordering requirement — only occurrences outside code fences count toward compliance. Continuation invariant: when "
