...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "changes, intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap declared by the prior rung, nothing beyond it. A rung is satisfied "
        "when and only when a tool-executed event appears in the transcript whose output is of that rung’s artifact type. Every 🔴 Execution observed: sentinel at any rung must contain "
        "verbatim tool-call output, triple-backtick delimited, complete, nothing omitted — prose descriptions, inline summaries, model-generated text that resembles output, elision "
        "markers (…, [truncated], [output continues]), reformatted text, and paraphrase are prohibited inside the block; if tool output is too long, the block ends at the last "
        "character the tool produced — no editorial closing; the only valid sequence is: tool call → 🔴 Execution observed: → triple-backtick block — the sentinel may not be written "
        "before the tool call exists in the transcript; a sentinel with prose content or no delimited block is void — fix by invoking the tool and re-emitting. Once ✅ Ground entered "
        ...
        "terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. Each completion sentinel is valid only at its defining rung — "
        "emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The gap names a specific behavior currently absent or wrong, phrased as a "
        ...
        "exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists for each thread declared in ✅ Manifest declared — before emitting ✅ Manifest "
        "exhausted, locate the N in the ✅ Manifest declared sentinel and count ✅ Thread N complete sentinels — if the count does not equal the declared N, the manifest is not "
        "exhausted. From the criteria rung onward, the gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not "
        "render” is valid; “need to implement a landing page” is not. Each rung’s artifact addresses only that gap — not all known requirements of the task — and is minimal: one "
        "independently testable behavior derived from the prose alone — if stating it requires names or structures not in the prose, it belongs in formal notation instead; if the "
        "criterion contains the word ‘and’ it is invalid — split it before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); "
        "a criterion that names both a structural element and its data source is a conjunction — split before continuing; asserting a column header is present does not satisfy this "
        "rung — add a criterion asserting its content or effect; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this "
        ...
        "match an easier implementation is a protocol violation; each criterion must state (a) observable behavior and (b) a falsifying condition statable without source files, "
        "function names, or variable names — if part (b) references implementation internals, the criterion is structural and must be rewritten before this rung closes; this is a "
        "content gate, not a self-check; after emitting the criteria artifact, proceed immediately to formal notation — enumerating remaining criteria or planning future gap cycles "
        "before the current cycle completes is a protocol violation; the next criterion for this thread may not be named until ✅ Thread N complete has been emitted for the current "
        "criterion’s cycle — ✅ Thread N complete is the criterion-emergence gate; the first criteria rung after ✅ Manifest declared emits exactly one criterion for Thread 1 — emitting "
        "criteria for multiple threads at the first criteria rung is a protocol violation; the manifest declared the thread list; the criteria rung instantiates the first thread’s gap "
        "assertion only. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases; it must encode all structural "
        ...
        "but may not substitute for encoding a constraint in notation — if a constraint can be stated in notation, it must be. Only validation artifacts may be produced at the "
        "executable validation rung — no other content is permitted before ✅ Validation artifact V complete; each test function asserts exactly one behavioral property — one behavioral "
        "property per test function; multiple assertions are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the "
        "expected type); before writing the test, re-read the formal notation and assert each structural constraint it encodes; a pre-existing test that happens to pass does not "
        "satisfy this rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must be invocable by an automated tool; when the "
        ...
        "executable artifact that fails when the test-under-modification does not exhibit the criterion; before writing the test, check for an existing test file covering the same "
        "module — add there if so; when the artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: "
        "‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]’ — "
        "each prior-failure name must be quotable verbatim from a 🔴 Execution observed sentinel; modification without carry-forward is a traversal violation; no implementation artifact "
        "may appear after modification until carry-forward has been emitted. before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not "
        "pre-exist or is already failing — V-complete may not be emitted without this tool-executed result in the transcript; the validation artifact must reside within the project’s "
        ...
        "asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten; implementation "
        "edits may not begin until a red run exists in the transcript for the current cycle; ✅ Validation artifact V complete may not be emitted unless a 🔴 Execution observed: showing "
        "the test failing exists in the transcript for the current cycle; emitting V-complete before seeing a red run is a protocol violation regardless of whether perturbation was "
        "attempted. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is written; if the observed running "
        "behavior label is written without a ✅ Validation artifact V complete appearing in the transcript for the current cycle, the OBR rung is void — return to the executable "
        "validation rung and produce the artifact. Before writing the validation run observation rung label, invoke the validation artifact via a test runner — file reads, grep, "
        "directory listings, and static analysis produce static-analysis-type output, not validation-run-observation-type output, and do not satisfy this gate; a harness error (import "
        ...
        "observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the observation "
        "rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been "
        "emitted at the validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; HARD STOP may not be emitted at the executable "
        "validation rung — a harness error at EV requires fixing the harness, not an upward return; if the observed gap matches the prior cycle's gap, the edit did not address it — "
        ...
        "violation; ✅ Thread N complete may not appear without both the criterion re-emission and a 🔴 Execution observed: in the current OBR section; then immediately invoke the "
        "implemented artifact via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the "
        "criterion; it must be what a non-technical observer of the running system would see — not the test suite, not process state unless the criterion is specifically about process "
        "state; a build output or compilation result never satisfies this gate regardless of what it names; for a React or web UI component, invoke the render utility (e.g., "
        "renderToStaticMarkup or the testing framework’s render function reading container.innerHTML) and emit the resulting HTML fragment showing browser-visible content — starting a "
        "dev server does not satisfy this rung; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the "
        "declared behavior; a 🔴 Execution observed: block that is empty or blank, or contains only static-analysis output (file reads, grep results, directory listings), does not "
        "satisfy this gate at any rung — the block must contain output produced by running the artifact; if the declared gap names a real network endpoint, the OBS must include "
        ...
        "prohibited; the provenance statement does not replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not be emitted unless a 🔴 "
        "Execution observed: with non-empty verbatim output appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no "
        "following 🔴 Execution observed: is a protocol violation; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be "
        "emitted unless the observed running behavior label has been written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel "
        "appears immediately after it that directly demonstrates the criterion — a test pass is not a demonstration; if the 🔴 Execution observed: at the observed running behavior rung "
        "contains test runner output (pass counts, test names, duration summary), it does not satisfy the OBR gate — re-invoke the implemented artifact directly before continuing; ✅ "
        "Thread N complete may not be emitted after an OBR 🔴 Execution observed: that shows test runner output; no implementation file was created or modified for this thread’s gap in "
        "this cycle means the gap was already closed (return to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the "
        ...
        "no such result exists is the tool call that runs the suite; once ✅ Thread N complete is emitted no further output in that cycle is valid; at most one thread is in progress at "
        "a time — no output for thread N+1 may appear until ✅ Thread N complete has been emitted for thread N; this gate applies to all rungs, not only the prose rung. Before writing "
        ...
        "rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate cleared "
        "licenses the first edit — it does not complete the thread; after emitting it, the executable implementation rung, the observed running behavior rung, and their required "
        "tool-executed events must still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file; ✅ "
        "Thread N complete may not appear until the observed running behavior rung has fired for this thread. Each sentence in the prose containing a behavioral predicate — any action "
        "verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] "
        "marker in the prose — the manifest thread list must contain exactly one entry per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry "
        "is a protocol violation. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the "
        "criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s validation run "
        "observation rung — not from a prior cycle — to exist in output; the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy "
        "the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, "
        ...
        "an upward return. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upward returns follow the failure class: if the "
        "implementation did not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; observation cannot be produced "
        "is a prose-description failure, not an implementation failure — return to the prose rung and update it. After every upward return, a new ✅ Validation artifact V complete must "
        "be emitted for the current cycle’s gap — an artifact written in a prior cycle does not satisfy this gate. Reconciliation gate: at every upward rung transition, reconcile all "
        "artifacts against I and update any pre-existing document that records I (ADR, README, spec) before redescending. Before producing the final report, invoke the delivered system "
        ...
        "thread in order — before writing each section, locate the artifact in the prior transcript; if it cannot be found verbatim in the transcript, it may not appear in the final "
        "report; no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. Seven rungs in order: prose → criteria → "
