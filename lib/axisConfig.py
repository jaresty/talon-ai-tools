...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "by applying P1-P6 — observe at every feasible rung before proceeding; derive: Sentinels from recursive intent structure (declare intent → derive micro-intents → verify with "
        "evidence → emit completion sentinel; this applies at session, thread, rung, and file-edit levels), Session observation loop (when to observe vs descend, what observation means "
        "vs does NOT mean), ladder derivation format (table columns, columns meaning), rung-specific behaviors (when to emit each sentinel, gate blocks sentinel emission until gate "
        "satisfied), rung transitions (emit rung label when beginning each rung; Outputting a rung label is what begins that rung, not a heading or annotation; emit completion sentinel "
        "when finishing each rung; emit rung label when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return upward), rung "
        "completion order (each rung must produce its artifact before emitting completion sentinel; sequential descent through prose → criteria → formal notation → validation → "
        "implementation; skipping a rung is a protocol violation; each rung may not be skipped or combined with another), formal notation is NOT a file artifact — it is a mental "
        "exercise, not a written document; produce the specification in your head, not on disk; the formal notation rung verifies your understanding, not a file; rung-specific gaps "
        "(each rung addresses a specific gap that only that rung can solve; the gap must be explicitly expressed IN THE CONVERSATION with evidence appropriate to the rung level; for "
        "validation assertions, the gap is the individual assertion being red; gaps are not implicit — they must be written down in the conversation with supporting evidence; "
        ...
        "constraint pressure at a lower rung is not a valid upward return trigger), upward gap scan (after completing the final rung (implementation), if gaps remain, scan upward to "
        "find the nearest rung above that still has a gap to close; return to that rung rather than restarting from the top; continue descending from that rung to close the remaining "
        "gap; after all gaps are closed, return to the observation loop above the ladder to verify closure and emit Ground complete), observation loop corollary (if opening observation "
        "shows no gap exists, emit Ground complete immediately — no ladder descent needed), upward return conditions (what triggers return, what does NOT trigger return and why "
        "excluded), evidential scoping (evidence is scoped to current cycle and gap, not prior cycles), scope preservation (scope does not expand between rungs), derivation reasoning "
        "(explain WHY each rule follows from P1-P6, not just WHAT the rule is); evidence over theory (a model's description of what it would do is indistinguishable from having done it "
        "— only visible execution output counts as evidence; every behavioral change and execution must be observable in the log, not just described; trust evidence not theory), "
        "recursive intent structure (P1 intent primacy implies: declare intent → derive more specific micro-intents → verify each micro-intent with evidence → emit completion sentinel; "
        "this pattern applies at session level, thread level, and rung level; every boundary between intent scopes requires a sentinel), smallest intent scale: individual test "
        "assertions — each assertion in a validation file is an intent declaration; seeing it red reveals the gap; implementing to make it green closes the gap; intent remains intact "
        "only if assertions remain green; regression = any assertion that was green becomes red; failure to see the assertion red means the gap has not been observed and implementation "
        "cannot proceed — for backfill cases where tests already pass, perturb the implementation to observe failure, then fix the perturbation; each assertion is its own sub-rung: the "
        "gap is the red assertion, the solution is making it green; the assertion itself must be printed RAW in the transcript — only the surrounding test context may be paraphrased), "
        "each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; Protocol derived MUST be emitted as the exact sentinel: 🔵 "
        ...
        "that is failing (the exact assertion line from the test file); Validation rung: write test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior; "
        "create STUB implementation files for EVERY component planned (empty functions returning null) — this creates all files needed for the entire feature, not just some; no "
        "expect() assertions, no mock setup, no render; tests are EMPTY and pass trivially because they have no code; this rung establishes that tests can run and all components have "
        "stubs. Assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung — FAILING tests means NOT DONE; fix issues at validation rung BEFORE "
        "moving to assertion rung. Assertion rung: EDIT existing test file to add ONE specific expect() assertion; run tests to see it FAIL (red). AFTER making assertion green "
        "(implementation succeeded), CHECK if gap is closed — if gap still exists, add another assertion; if gap is closed, emit completion sentinel and move to next gap. NO file "
        "creation in assertion rung — stubs created at validation rung; only edit existing test file to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete "
        "BEFORE emitting the next rung's start label; one rung at a time; completion before continuation. Every file change requires justification: AFTER each file edit, do NOTHING "
        "else until you have provided justification IN THE CONVERSATION; write NO code, make NO tool calls, issue NO commands until you have explained in the chat why this change "
        "satisfies the current rung requirements; if you cannot justify why it satisfies the rung, ITERATE until it does; file changes without justification are protocol violations. "
        "JUSTIFICATION MUST reference the specific rung requirements being satisfied — vague explanations are insufficient. Implementation rung: AFTER assertion rung shows red, EDIT "
        "existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO file creation in implementation rung, only edit existing stubs; "
        "implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; "
        "if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap "
        "still exists, return UP to find next gap (ascend ladder); if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, "
        "then implement, then emit completion sentinel. ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. Only test assertions with "
