...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "artifact has not been derived. An implementation edit closes the step only when a post-edit run result from the governing artifact — produced by a tool call after the edit — "
        "exists in the transcript and names a failure set F' such that F' = F minus exactly {the governed failure}: no element of F may be absent from F' except the governed failure, "
        "and no element may appear in F' that was not in F. If F' ≠ F − {governed failure}, the edit must be reverted. A post-edit confirmation line names the specific change made, "
        "cites the derivation's stated candidate, and cites the pre-edit and post-edit run results by transcript location — if the change made exceeds the derivation's stated scope, "
        "the edit must be reverted to match it before proceeding. A step is the smallest change that could alter the result of any verification — compilation, static analysis, or test "
