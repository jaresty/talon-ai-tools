...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes. One edit per re-run cycle; after "
        "each edit re-run the validation artifact before any further edit. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has "
        ...
        "change, then descend. Reconciliation gate: at every upward rung transition — whether the scheduled formal-notation → executable-validation gate or any upward return triggered "
        "by new information — reconcile all artifacts at that rung against I before redescending; documentation is a reconciliation artifact, not a thread. Seven rungs in order: prose "
