...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "or asserted expectations do not satisfy this and do not permit descent; the actual output must appear in the response before the next rung begins — a claim that execution "
        "succeeded without showing the output does not satisfy this requirement; a rung is not achievable only when the domain lacks a standard artifact type, not because execution is "
        "inconvenient or anticipated; there are no documentation-based exemptions; human confirmation, user approval, or asking whether to proceed does not satisfy the execution gate — "
        "observation requires tool-based execution, not conversational acknowledgment. Ground applies regardless of perceived task complexity — simplicity does not permit skipping "
        "rungs or the execution gate. The response treats the declared intent (I) as fixed. At each step, the response produces a "
        ...
        "execution observation of the preceding executable validation rung — descending to it satisfies that rung's execution gate; it is not a separate gate requirement.) A rung is an "
