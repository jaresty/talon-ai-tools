...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "name appears as a direct call in the test body, the layer is 'unit'; if the governed symbol name does not appear in the test body, the layer is 'integration'; (f) the "
        "executor class — the executor must produce output that could only exist if the governed assertion reached execution at the layer named in (e); a governing artifact is "
        "disposable if removing the governed symbol from the governed artifact without changing the named governing artifact leaves the executor result unchanged — a disposable "
        "artifact does not satisfy (f); (g) the observed FAIL result: the content of an executor result block appearing in the transcript after the artifact was applied to the subject "
        "and before the first governed artifact-producing action for this behavior, where (a) precedes (c), the named executor produced the result, and the failure line contains (d) "
        "as a substring. A result satisfies this token if and only if: (g) is present at the required position; (a) is not separated from (c) in (g) by any line whose content contains "
        "(c) for a different governed behavior; and a subsequent executed-artifact result contains (b) at the position where (a) was absent. A result satisfying (g) must contain a "
        "failure line whose content includes a substring from the assert statement of the governed assertion; a failure line that contains no such substring does not satisfy (g) "
        "regardless of whether (a) precedes (c) in its output. Exception: (g) is not required for the single action that produces the governing artifact's assertion text, per governed "
        "behavior — the creation step is that action only; (g) is required for every governed artifact-producing action in the current session — a creation step is that action only if "
        "(c) is absent before the action and present after it, and a named executor invocation against the governing artifact immediately following the action produces a tool-executed "
        "result containing (c) and not (a); immediacy is defined as: no executor result block appears between that action and the named executor invocation — non-executor actions do "
        "not break immediacy. Artifact identifier must differ: the artifact identifier of the named governing artifact must differ from the artifact identifier of the artifact being "
        ...
        "not be treated as a blocking condition on the current action. Artifact gap: when the Falsify derivation block contains the line '(g) observed FAIL result: unavailable' and no "
        ...
        "outcome, or a UI element is not a valid dimension regardless of provenance; derivation is unbounded: add dimensions, challenge each against the reader-uncertainty test, and "
        "revise until every dimension is independently falsifiable; when every dimension carries an '[observable:]' tag, write '§2 dimensions closed'; (3) derive the enforcement "
