...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "and at least one smaller change; the smaller change must be the minimal stub — the smallest change that satisfies the structural constraints of the candidate without "
        "implementing the governed behavior; a minimal stub exists structurally but does not perform the transformation; a change that produces a failure mode different from the "
        "governing failure signal is not a valid smaller change; then apply the smaller change and execute the governing artifact to produce a tool-executed result — if the governing "
        ...
        "smaller change and executing the governing artifact against the minimal wrong state, and that result contains the governing failure signal text verbatim — a tool-executed "
        "result produced before the smaller change was applied does not satisfy this gate regardless of its content; no other form of argument — including prose reasoning about other "
        "tests or failure modes — constitutes valid insufficiency evidence; second, in a separate entry below, quote the specific text from the governing failure signal that shows the "
        "governed behavior absent — the quote must appear in a tool-executed run result above the first entry; a candidate that is larger than the smallest change that resolves the "
        "quoted failure text is not permitted regardless of what else it also resolves; a failure not yet observed in a prior tool-executed run result has not been shown to govern, and "
        "a change that fixes it cannot be derived from the current governing artifact; (3) line manifest listing every line to be added; for each line, quote the specific assertion "
        ...
        "such new assertion text requires revert. An edit that introduces a failure absent from the pre-edit result must be reverted regardless of interpretation.",
