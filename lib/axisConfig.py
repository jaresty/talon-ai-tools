...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "proven in Challenge output [ ] Baseline: derive from P2,P3 — why Baseline must prove evaluation works before proceeding; what GOES WRONG if you skip proving it works: no proof "
        "framework is valid; RULE: Baseline = prove evaluation works (apply and show passing output), cannot proceed without this proof[ ] Structure before baseline: derive from P1,P2 "
        ...
        "Baseline must PASS with its scaffolding. NO PROCEEDING without proof it works. IMPORTANT: Baseline establishes mechanics at Baseline (may create them). Challenge adds NEW "
        "criteria but uses EXISTING mechanics - adding new mechanics at Challenge is scope expansion. IMPORTANT: Baseline evaluation must be SELF-CONTAINED. If it fails due to missing "
        "prerequisites, that's a broken framework, not evidence of gaps. Only NEW criteria added at Challenge can expose gaps in the solution. 5. Challenge — What evidence proves gaps "
        ...
        "framework is broken. IMPORTANT: Work on the smallest verifiable thing at a time — each check must be satisfied before the next. 6. Refinement — How is the artifact improved? "
        "Derivation: from Challenge, address the SPECIFIC gaps that were OBSERVED. Evidence: changes target the exact gaps proven to exist. IMPORTANT: Refinement must address OBSERVED "
        "gaps only - gaps seen in Challenge output. Addressing unobserved gaps wastes effort and may miss real issues. 7. Verification — How is completion confirmed? Derivation: from "
        "refinement, derive confirmation. Evidence: no new gaps exposed. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to finish each rung. "
        "Evidence before claims: for every claim about completion, there must be evidence proving it. Ground complete may only be emitted after verification confirms no gaps remain. "
        "Domain adaptation: for tasks where verification is possible, define evaluation scaffold BEFORE producing solution — evaluation defined after solution is vacuous (Texas "
        "sharpshooter). Baseline defines evaluation framework; Challenge applies it to show OBSERVED gaps; Refinement produces solution that addresses those OBSERVED gaps.for writing "
