...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the gap declared by the prior rung — nothing beyond it; one gap per cycle; one criterion per thread per cycle; one edit per re-run; no anticipation of future gaps, no "
        "additional invariants, no coverage beyond the declared gap; one edit means exactly one tool call that creates or modifies a file — narrating a change without a file-write tool "
        "call is not an edit. Rung table — name | artifact type | gate condition | void condition: prose: artifact=“natural language description of intent and constraints”, gate=“I "
        "declared”, voids_if=“skipped”; criteria: artifact=“falsifiable behavioral acceptance conditions”, gate=“prose complete”, voids_if=“criterion is structural/implementation "
        "rather than behavioral”; formal notation: artifact=“non-executable specification with behavioral invariants + R2 audit”, gate=“criteria complete; R2 audit 0 UNENCODED "
        "entries”, voids_if=“UNENCODED entries remain; audit not a separate named section”; executable validation: artifact=“test file invocable by an automated tool, within project "
        "tree”, gate=“formal notation R2 audit complete”, voids_if=“implementation code included; outside project tree; pre-existing artifact”; validation run observation: "
        ...
        "complete; exec_observed + gap for behavior from I”, voids_if=“new files created; output names only infrastructure state; file read used as evidence”. Every 🔴 Execution "
        "observed: sentinel at any rung must contain verbatim tool-call output, triple-backtick delimited, complete, nothing omitted — prose descriptions, inline summaries, "
        "model-generated text that resembles output, elision markers (…, [truncated], [output continues]), reformatted text, and paraphrase are prohibited inside the block; if tool "
        "output is too long, the block ends at the last character the tool produced — no editorial closing; the only valid sequence is: tool call → 🔴 Execution observed: → "
        "triple-backtick block — the sentinel may not be written before the tool call exists in the transcript — a sentinel written before its tool call records anticipated rather than "
        "observed state, and is void regardless of accuracy; a sentinel with prose content or no delimited block is void — fix by invoking the tool and re-emitting; a tool call must "
        "exist in the current response before the sentinel is emitted — sentinel emitted without a preceding tool call in the current response is void regardless of content. Once ✅ "
        "Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current gap before any other content — ground "
        "protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. Each completion sentinel is valid only at its "
        "defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The gap names a specific behavior currently absent or wrong, phrased as "
        "a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ "
        "Manifest exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists for each thread declared in ✅ Manifest declared — before emitting ✅ "
        "Manifest exhausted, locate the N in the ✅ Manifest declared sentinel and count ✅ Thread N complete sentinels — if the count does not equal the declared N, the manifest is not "
        ...
        "criterion to match an easier implementation is a protocol violation; each criterion must state (a) observable behavior and (b) a falsifying condition statable without source "
        "files, function names, or variable names — if part (b) references implementation internals, the criterion is structural and must be rewritten before this rung closes; this is "
        "a content gate, not a self-check. The formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, "
        ...
        "notation and assert each structural constraint it encodes; before writing the test, check for an existing test file covering the same module — add there if so; when the "
        "artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test "
        ...
        "artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — V-complete may not be emitted without this tool-executed result in "
        "the transcript; asserting confirmation without showing the exec_observed block does not satisfy this gate — the exec_observed block must appear in the current response; the "
        ...
        "any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after "
        "the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: "
        "with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the "
        "criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the "
        "validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; prose-only exec_observed sentinel does not satisfy this gate "
        "— the exec_observed block must contain non-empty verbatim tool output in a triple-backtick block; HARD STOP may not be emitted at the executable validation rung — a harness "
        "error at EV requires fixing the harness, not an upward return; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation "
        "before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for "
        "the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running behavior label, re-emit the criterion for "
        "this cycle — criterion re-emission is required: emitting the OBR label without immediately following it with the criterion text is a protocol violation; the tool call is the "
        "only valid next action after criterion re-emission; planning statements, diagnostic narration, and commentary between criterion re-emission and the tool call are protocol "
        "violations; ✅ Thread N complete may not appear until a tool call exists in the transcript after the observed running behavior label in the current cycle; then immediately "
        "start or query the implemented artifact as a live running process via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly "
        "demonstrate the specific behavior named in the criterion; it must be what a non-technical observer of the running system would see — not the test suite, not process state "
        "unless the criterion is specifically about process state; a build output or compilation result never satisfies this gate regardless of what it names; prefer the most realistic "
        "invocation — for a web UI, start the dev server and emit the HTML response body (e.g., via curl or browser); for a CLI, invoke it directly and emit its output; in-process "
        "rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists; if the implementation has no directly invocable artifact, invoke "
        "any non-test consumer that starts or queries a live process and demonstrates the declared behavior — reading build artifacts, compiled output, generated HTML, or any static "
        "file is not a valid fallback regardless of what the file contains; if no live process can be started or queried, open a new gap cycle to make the artifact directly invocable "
        "before continuing; if the declared gap names a real network endpoint, the OBS must include verbatim output showing a real HTTP request or response — a test run whose network "
        "calls are mocked does not satisfy this gate; before any tool call touching the filesystem at this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung "
        "name] in this thread’ — if the rung label cannot be located in the transcript, the tool call is prohibited; the provenance statement does not replace the tool call — the tool "
        "call must follow it in the same response; a 🔴 Execution observed: block that is empty or blank, or contains only static-analysis output (file reads, grep results, directory "
        "listings), does not satisfy this gate at any rung — the block must contain output produced by running the artifact; ✅ Thread N complete may not be emitted unless a 🔴 Execution "
        "observed: with non-empty verbatim output appears in the transcript after the observed running behavior label in the current cycle — a provenance statement with no following 🔴 "
        "Execution observed: is a protocol violation. ✅ Thread N complete may not be emitted unless the observed running behavior label has been written after the most recent 🟢 "
        "Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it that directly demonstrates the criterion — if the 🔴 Execution "
        "observed: output does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules — ✅ Thread N complete "
        "may not be emitted after an OBR 🔴 Execution observed: that does not directly demonstrate the criterion; a test pass is not a demonstration; no implementation file was created "
        "or modified for this thread’s gap in this cycle means the gap was already closed (return to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or "
        "explicitly acknowledged in the transcript with a written reason — dismissing a failing test as unrelated without an explicit written acknowledgment is a protocol violation; ✅ "
        "Thread N complete may not be emitted unless a 🔴 Execution observed: from a full test suite run exists in the transcript after the most recent OBR tool call in this thread — "
        "the only valid next action if no such result exists is the tool call that runs the suite; once ✅ Thread N complete is emitted no further output in that cycle is valid; Before "
        "writing the criteria, formal notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared gap — what the "
        "prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 Implementation gate "
        "cleared licenses the first edit — it does not complete the thread; after emitting it, the executable implementation rung, the observed running behavior rung, and their "
        ...
        "action verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: "
        ...
        "does not satisfy the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates "
