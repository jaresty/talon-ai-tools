...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "by applying P1-P6 — observe at every feasible rung before proceeding. Derive from P2 (one artifact per type) that each artifact type requires its own rung; derive from P3 "
        "(evidence required) that validation must be executable (tool-run); derive from P5 (derivation chain) that validation artifact derives from what it validates; derive from P2/P3 "
        "that validation artifact contains NO implementation behavior — it only asserts, it does not produce; this is called 'executable validation' — a file that asserts behavioral "
        "properties without containing behavior of its own. Similarly, from P2/P3 derive 'executable implementation' — a file that produces behavior directly without asserting about "
        "other artifacts. Protocol derivation checklist: AFTER deriving rules from P1-P6, BEFORE proceeding, MAKE VISIBLE in the conversation: [ ] How to handle files: what creates at "
        "validation, what edits at assertion/implementation — what GOES WRONG if you create files at wrong rung: audit trail breaks, protocol unverifiable; RULE: validation creates ALL "
        "stubs, assertion adds assertions to existing test files, implementation edits existing stub files only, NO new files after validation [ ] How to handle the ladder: what each "
        "rung does and when to move to next — what GOES WRONG if you skip rungs: gaps lost, completion becomes false claim; RULE: one rung at a time, complete before next [ ] How to "
        "verify gap closure: when to check and emit completion — what GOES WRONG if you emit 'complete' with gaps remaining: fool yourself, contradictions invisible; RULE: only emit "
        "'complete' after upward scan confirms no gaps, look for 'but still' or 'needs' as contradiction signals [ ] Gap cycle: derive from P1,P3,P5 — why one gap at a time; what GOES "
        "WRONG if you batch: no clear red→green signal; RULE: one gap at a time, assertion→implement→verify→complete→next gap [ ] Validation before assertion: derive from P3,P5 — why "
        "validation tests must pass (scripts run, stubs exist) before assertion rung: assertion rung failures must be due to actual gaps, not infrastructure issues; RULE: validation "
        "tests must pass before moving to assertion rung [ ] Smallest intent at executable level: derive from P1,P3 — the smallest unit of intent is the individual failing assertion, "
        "not the test file; each assertion is its own intent declaration; regression = assertion that was green becomes red; RULE: implement one assertion at a time, not batch of "
        "assertions [ ] File edit declarations: derive from P4 — what to declare before edits; RULE: before any edit, declare 'no new files' at assertion/implementation rungs [ ] "
        "Evidence: derive from P3 — what evidence for completion sentinels; RULE: cite specific assertion that was red [ ] File manifest: derive from P4,P6 — why validation must list "
        "files created; RULE: validation rung lists every file before creating it [ ] Upward scan: derive from P5,P6 — how to verify all criteria addressed; RULE: after implementation, "
        "scan up with ✅/❌ checklist for each criterion from prose/criteria/formal notation [ ] Rung context: derive from P1,P6 — why re-emit rules at each rung; RULE: before each new "
        "rung label, re-emit key rules from derivation This checklist must appear BEFORE the first ladder rung begins. The derivation results must be in the chat, not just in your "
        "head. Before starting each rung, re-emit key rules from your derivation relevant to that rung. BEFORE transitioning to next rung, verify and declare: [current rung artifact "
        ...
        "and 🔵 Ladder derived sentinels — these are gates on all subsequent work. Derive the Session observation loop from P6 (thread sequencing) — the loop that observes current state "
        "before descending into ladder; observation must complete before manifest declares gaps. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to "
        "finish each rung. Evidence before claims: for every claim about code being correct, there must be tool execution output proving it. Ground complete may only be emitted after "
        "upward scan confirms all gaps closed. ",
