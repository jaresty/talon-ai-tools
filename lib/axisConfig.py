...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "validation run observation (only satisfied by an observed failure; a passing run is a gap signal — see gate_validity) → executable implementation → observed running behavior. "
        "A gate is a conversation-state condition: open when and only when the required event has occurred in this conversation for this thread. Prior knowledge, anticipation, and "
        "model reasoning cannot satisfy any gate regardless of accuracy. Underlying all compliance failures is one epistemological error: substituting model knowledge for conversation "
        "events. For executable rungs, emit 🔴 Execution observed: [verbatim tool output — content composed without running the tool is invalid] then 🔴 Gap: [what the verbatim output "
        "reveals] on their own lines before any implementation artifact. No implementation artifact — including planning text, code blocks, or tool calls — may appear before valid "
        "Execution observed + Gap sentinels; if any implementation content appears without these sentinels immediately preceding it, it is invalid and must be discarded before the tool "
        "is run. Before producing implementation code, emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]. The quote must be verbatim from the 🔴 "
        "Execution observed sentinel of this thread; quoting anticipated output or a prior thread's observation is invalid. The 🔴 sentinel format is reserved exclusively for executable "
        "rung gates. For non-executable rungs, observation appears inline as labeled prose. The validation run observation rung is only satisfied by an observed failure that reveals "
        "the declared gap — a passing run before gap-targeted implementation has been written is a gap signal, not permission to descend: either the behavior is already implemented "
        "(verify this; the thread may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected before descent continues. When the "
        "lowest V is complete, output ‘✅ Validation artifact V complete’ on its own line before producing O — this phrase may only appear after the executable validation rung has been "
        "both produced and invoked and the validation run observation rung has declared a gap. Gap-locality: the gap gating rung N is the output of executing rung N-1. No gap from any "
