...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "component of '[text]' is covered, write '§1a decomposed'; '§1a decomposed' must appear before the means-test line; '§1 goal derived' must not appear before '§1a decomposed' in "
        "the transcript; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means not listed in §0]' — if no concrete "
        "alternative can be named, write '[hypothetical]: [speculated alternative]' and fetch a broader context artifact before closing §1; if two non-subsuming goal candidates exist, "
        "write both and mark one '[selected]' — the selected goal is the one whose means-test names a more concrete alternative than those listed in §0; when ≥2 alternatives (concrete "
        "or hypothetical) are present, write '§1 goal derived'; Path B — (0) write the literal string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — "
        "this declaration satisfies §0; (1) derive the governing goal: write '## Governing goal: [derived: text]' where '[derived: text]' is a goal derived from the scenario "
        "description — what would satisfy the underlying need, not merely what was asked; a '[derived: text]' value that adds a claim absent from the user's message does not satisfy "
        "§1; immediately below the governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', "
        "labeled subject, verb, outcome, or invariant; no field may equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive words; when every named "
        "component of '[text]' is covered, write '§1a decomposed'; '§1a decomposed' must appear before the means-test line; '§1 goal derived' must not appear before '§1a decomposed' in "
        "the transcript; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]'; if only one concrete alternative "
        "can be named, write '[hypothetical]: [speculated alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal "
        "derived'; (Both paths continue from §2 onward:) immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in the means-test, write one "
        "candidate beginning with '- Candidate:' — a response property that would be absent in a response achieving '[text]' by a different means; when one candidate exists per "
        "alternative means, write '§1b candidates'; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the transcript; derive at "
        "least two dimensions; each dimension names a property of the response (not an artifact, state, or fix description) such that a response could satisfy it while failing at least "
        "one other listed dimension; each dimension must trace to one item from the §1a decomposition list or one item from the §1b candidates list — a dimension that traces only to "
