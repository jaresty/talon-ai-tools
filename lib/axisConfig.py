...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "artifact already exists as a completed tool result in the transcript above the edit — issuing the run and the edit as parallel tool calls does not satisfy this condition. All "
        "other reported failures are not yet open. The scope is the governing output read literally: when literal and semantic readings diverge, choose the interpretation that produces "
        "the smallest change that would shift the governing output. The implementation target is permitted only after the derivation names both the candidate change and at least one "
        "smaller change that would not shift the governing output, with the reason it would not — not the minimum that satisfies the governing artifact. A step is the smallest change "
        "that could alter the result of any verification — compilation, static analysis, or test execution; changes that address different verification layers are always separate steps "
        "even when they feel like parts of the same fix. The governing artifact must be the verification layer with scope to detect a failure if this step's change is incorrect — "
        "running a layer that cannot see the specific files, call sites, or concerns this step touches does not satisfy the governing artifact requirement even if that layer passes. An "
        "artifact is a valid governing artifact for this step only when a derivation exists in the transcript naming the specific FAIL output it would produce if this step's change "
        "were absent. Implementing beyond the current governing output is a violation regardless of how many other failures exist. Analysis and classification steps have a text "
        "artifact as their governing artifact — a complete classification block or derivation section; no tool call is required to open or close these steps. A step qualifies as an "
        "analysis step when and only when its governing artifact's content, if applied verbatim to the system, would produce no change in system behavior, and names at least one "
        "specific behavioral invariant that would remain unchanged if the artifact were discarded. Implementation steps have a FAIL tool result as their governing artifact. These are "
        "distinct step types with distinct governing artifacts — analysis steps may precede the first tool call without violating atomic.",
