...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "output. 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation run observation rung in the "
        "current cycle — emitting it at any other position is a protocol violation; prose-only exec_observed sentinel does not satisfy this gate — the exec_observed block must contain "
        "non-empty verbatim tool output in a triple-backtick block; HARD STOP may not be emitted at the executable validation rung — a harness error at EV requires fixing the harness, "
        "not an upward return; the only valid next action when EV shows a harness error is a tool call that fixes the harness; HARD STOP may not appear in the same response as a "
        "harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. "
        "HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread. the VRO "
        "rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. 🟢 "
        ...
        "response; the next required action after 🟢 is a tool call that creates or modifies an implementation file; the implementation gate does not complete the thread — EI and OBR "
        "rungs must still fire before ✅ Thread N complete; ✅ Thread N complete may not appear until the observed running behavior rung has fired for this thread; the OBR rung label "
        "must appear in the transcript for the current thread's current cycle before ✅ Thread N complete may be emitted — its absence is a protocol violation, not a voiding condition; "
        "the void conditions for OBR handle the case where OBR fires with invalid output; this rule handles the case where OBR never fires at all. declaring a thread complete by "
        "similarity to another thread's failure is not valid — each thread requires its own full descent through OBR with a passing run; cross-thread similarity reasoning does not "
        "substitute for a tool-executed result in the current thread's current cycle. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no "
        "runnable artifact exists. If no invocable process exists, open a gap cycle to make the artifact directly invocable. if the 🔴 Execution observed: does not directly demonstrate "
        "the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules; the exec_observed output must show the specific behavior asserted in "
        "the criterion — server liveness (a 200 OK, a page title, a process start message) does not satisfy this gate unless the criterion asserts server liveness; the query must "
        "exercise the criterion’s behavior directly, not merely confirm the server is running. A full test suite run result is required after the most recent OBR tool call — the only "
        ...
        "prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately "
        "derivable from the re-emitted prose. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through executable "
        "implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward return. Upward returns follow the "
        "failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; "
        "observation cannot be produced is a prose-description failure, not an implementation failure — return to the prose rung and update it. After every upward return, a new ✅ "
        "Validation artifact V complete must be emitted for the current cycle’s gap — an artifact written in a prior cycle does not satisfy this gate. Before producing the final "
        "report, invoke the delivered system end-to-end via a tool call and emit 🔴 Execution observed: showing the primary behavioral intent is satisfied — this is the post-delivery "
        "integration observation; if it fails, open a new gap cycle before producing the final report. Any prose that summarizes implemented behavior, lists acceptance criteria met, "
        "describes files created or modified, or names gaps as resolved is a final report regardless of its heading; a final report may not appear unless ✅ Manifest exhausted "
        "immediately precedes it in the transcript. if no currently-false behavior remains, emit ✅ Manifest exhausted. After emitting ✅ Manifest exhausted, produce a final report "
        "containing prose, criteria, and formal notation for each thread in order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the "
