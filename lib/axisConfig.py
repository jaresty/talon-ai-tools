...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "evidence → emit completion sentinel; this applies at session, thread, rung, and file-edit levels), Derive file-edit justification: every file edit must serve the intent of its "
        "rung; explain how each file satisfies the current rung's requirements; make the reasoning VISIBLE in the conversation as evidence of derivation from intent (P1), not arbitrary "
        "action; justification is evidence, not just documentation; Session observation loop (when to observe vs descend, what observation means vs does NOT mean), ladder derivation "
        "format (table columns, columns meaning), rung-specific behaviors (when to emit each sentinel, gate blocks sentinel emission until gate satisfied), rung transitions (emit rung "
        "label when beginning each rung; Outputting a rung label is what begins that rung, not a heading or annotation; emit completion sentinel when finishing each rung; emit rung "
        "label when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return upward), rung completion order (each rung must produce "
        "its artifact before emitting completion sentinel; sequential descent through prose → criteria → formal notation → validation → implementation; skipping a rung is a protocol "
        ...
        "component planned (empty functions returning null) — this creates all files needed for the entire feature, not just some; no expect() assertions, no mock setup, no render; "
        "tests are EMPTY and pass trivially because they have no code; this rung establishes that tests can run and all components have stubs. Assertions are written at the assertion "
        "rung, not validation. Tests MUST PASS at validation rung — FAILING tests means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. Assertion rung: EDIT "
        "existing test file to add ONE specific expect() assertion; run tests to see it FAIL (red). AFTER making assertion green (implementation succeeded), CHECK if gap is closed — if "
        "gap still exists, add another assertion; if gap is closed, emit completion sentinel and move to next gap. NO file creation in assertion rung — stubs created at validation "
        "rung; only edit existing test file to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one rung at a "
        "time; completion before continuation. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that "
        "specific assertion green — NO file creation in implementation rung, only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, then "
        "stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the "
        "minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, return UP to find next gap (ascend ladder); if gap is closed, "
