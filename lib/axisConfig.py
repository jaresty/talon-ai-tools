...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the criteria rung onward, the gap names a specific behavior currently absent or wrong; naming an absent artifact is not a valid gap. Each rung’s artifact addresses only that "
        "gap — not all known requirements of the task. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only "
        "validation artifacts may be produced at the executable validation rung — implementation code is not permitted at this rung; file reads, grep output, and manual inspection do "
        "not constitute executable validation — the artifact must be invocable by an automated tool; a pre-existing test that happens to pass does not satisfy this rung — the artifact "
        "must be newly produced for the declared gap; the validation artifact must fail before any implementation edit — a green run with no prior red run does not satisfy this rung. "
