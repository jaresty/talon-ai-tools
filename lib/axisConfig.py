...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "completion, by making that gap observable, costly to maintain, and impossible to hide. Six generative axioms — derive your process from these: 1. Evidence primacy: Claims "
        ...
        "process one step at a time, showing evidence before proceeding to the next step. Protocol derivation checklist: AFTER deriving your process from the six axioms above, BEFORE "
        "proceeding, MAKE VISIBLE in the conversation: [ ] The invariant: claim vs reality — what GOES WRONG if you claim without evidence: false completion; RULE: evidence before "
        "claims [ ] Optimization pressure: the model will seek shortcuts — what GOES WRONG if unaddressed: steps skipped, unverifiable results; RULE: make compliance easier than "
        "deviation and deviation detectable [ ] The ladder structure: 6 steps — Intent → Criteria → Challenge → Refinement → Verification; what GOES WRONG if skipped: gaps disappear, "
        "completion becomes false; RULE: one rung at a time[ ] Challenge: derive from A1 (evidence primacy), A6 (execution discipline), A5 (independent evaluation) — instantiate a "
        "mechanism that detects gaps, APPLY it in the same layer as the artifact, and SHOW failure; a gap exists only when mechanism visibly fails[ ] Single demonstrated gap only: "
        "derive from A6 (execution discipline) — select exactly one observed failure as the active gap; refinement targets one gap only[ ] Refinement minimal behavior: derive from A6 "
        "(execution discipline) — implement only the smallest externally observable behavior that closes the active gap identified by Challenge; the test indicates failure but is not "
        "the target; BEFORE and AFTER evidence of the behavior must be visible in the transcript; over-reaching (full test pass beyond minimal gap) triggers rung invalidation, "
