...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "behavior. An assertion fires when execution reaches the assertion statement and evaluates it. Any error that terminates execution before reaching the assertion statement is a "
        "precondition failure, not a FAIL observation — the assertion has not fired and has detected nothing about the behavior. Precondition failures include missing module, missing "
        "file, missing symbol, failed setup or fixture, and any equivalent that prevents the subject from being constructed, in any language or runtime. Principle 1 derives that "
        "precondition failures cannot satisfy gate: the absence of the behavior remains unobserved. The required action is to introduce a minimal subject that satisfies all preconditions "
        "(a stub) so that execution can reach the assertion and produce a behavioral FAIL. Principle 2: coverage is per-assertion, not per-test — a test that FAILs establishes coverage "
        "only for assertions that fired and produced FAIL output; assertions not reached or that passed carry zero coverage. Principle 3: an observation requires executing the test — "
        "reasoning about what a test would produce is not an observation. Principle 4: some assertions have no natural FAIL state — an assertion checking that behavior does NOT produce X "
        "passes trivially when the behavior is absent, so the behavior's absence is indistinguishable from the behavior being correctly absent; such an assertion requires a coverage "
        "artifact produced by running it against a state where the behavior exists but is deliberately wrong (perturbation). Principle 5: Assertion type must match behavior type — a "
        "static check cannot govern executable behavior; a compiler-enforced constraint needs no additional assertion. Principle 6: in multi-layer work, each layer must have an "
        "independently-failing assertion that produces a meaningful FAIL when its own contract is violated, regardless of other layers. Principle 7: assertions must be as close as "
        "possible to the behavior they govern — prefer the assertion layer closest to the behavior that can observe a FAIL. An assertion that fires only through multiple layers of "
        "indirection cannot isolate which layer caused the failure; the further the assertion from the behavior, the weaker the evidence that it can detect the absence of that behavior "
        "(Principle 1 corollary). Unit tests are preferred when the behavior has a directly testable surface; integration or end-to-end assertions are valid only when no closer assertion "
        "layer exists. A passing integration test does not substitute for a missing unit assertion. Principle 8: a FAIL observation must be correlated to the intent of the assertion that "
        "produced it — given only the FAIL output, it must be possible to identify which behavioral contract was violated. A FAIL that could be produced by any number of unrelated causes "
        "is not correlated; it establishes that something is wrong without attributing the failure to the governed behavior. A FAIL attributable to an incidental property (e.g. a "
        "TypeError from the function returning None when the assertion governs tuple correctness) may fire but does not confirm the governed behavior was observed — it confirms only that "
        "execution failed before reaching it. Correlation requires the FAIL output to name the specific value, condition, or contract the assertion governs, not a side-effect of its "
        "absence. Precision derives from correlation — a vague FAIL cannot be attributed to a specific contract. PHASE 1 — classification (text artifact, required before any process "
        "derivation): produce a classification block for each assertion using this exact structure: Assertion: <name or description> | Principle: <which principle governs it> | Natural "
        "FAIL state: <yes, or no — answer no if the assertion checks that something does NOT occur, because such assertions pass trivially when the behavior is absent> | Coverage "
        "artifact: <if Natural FAIL state is no, describe the specific incorrect behavior to introduce to produce a FAIL tool result; otherwise n/a>. Every assertion must have its own "
        "block. Phase 1 is complete when every assertion has a block. PHASE 2 — coverage artifact (tool result required before implementation): every assertion requires a FAIL tool "
        "result before any implementation step — Principle 1 applies to all assertions without exception. For natural-FAIL-state assertions (Natural FAIL state: yes), the FAIL comes from "
        "the initial red-state test run. For no-FAIL-state assertions (Natural FAIL state: no), the FAIL requires perturbation. Before any implementation step, produce a Phase 2 status "
        "line for every assertion using exactly one of these two forms — COMPLETE: Phase 2 [assertion name] — <verbatim FAIL output from tool result> or NOT YET PRODUCED: Phase 2 "
        "[assertion name] — running now. The NOT YET PRODUCED form names a missing predecessor. The implementation step's correctness criterion is: change the system from the observed "
        "wrong state to the correct state. Without the FAIL tool result, the wrong state has not been observed — the correctness criterion is undefined, not merely unmet. Chain requires "
        "every step to reproduce its predecessor before proceeding, and the predecessor named here does not yet exist as a tool result. The agent derives from this that no implementation "
        "step is definable until the tool call is made and the COMPLETE line can be written from actual output. There is no path between NOT YET PRODUCED and implementation that does not "
        "require fabricating a predecessor — and fabrication is not reproduction. The COMPLETE form requires the quoted FAIL output to be present verbatim in a prior tool result in this "
        "conversation — a fabricated or paraphrased output in a COMPLETE line is a chain violation. A compilation error is not a valid FAIL artifact for any assertion — "
        "natural-FAIL-state or otherwise. Principle 5 derives this: a compile error is a build failure, not a behavioral observation; the assertion has not run and has not observed the "
        "wrong state. When a test does not compile, the required action is to write a stub — a minimal implementation with the correct signatures and no-op behavior — so the test "
        "compiles and runs, then collect the behavioral FAIL from the running test. The behavioral FAIL (e.g. 'key missing', 'unexpected value') is the only valid predecessor for an "
        "implementation step. For no-FAIL-state assertions, the FAIL requires perturbation. Phase 2 is complete when every assertion has a COMPLETE status line backed by a real tool "
        "result. Then derive the process these six principles require and follow it. The derivation must be visible. Principle 1 requires a FAIL observation. A FAIL observation requires "
        "an executable artifact whose state differs when the behavior is absent. The structural floor fallback applies only when no such artifact exists. An artifact exists whenever the "
        "behavior produces any observable output — a file, a parsed structure, a command exit code. The fallback is therefore unavailable whenever the behavior produces observable "
        "output, which is the common case for file, data structure, and command-based tasks. Structural floor assertions (file existence, path resolution, identifier presence) are "
