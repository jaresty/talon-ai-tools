...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "behavior is an executed-artifact result in which (a) precedes (c) and the failure line contains a substring from the governed assertion call expression. The response "
        "establishes, before any governed artifact-producing action, that the named governing artifact has been observed to detect the absence of each governed behavior. A named "
        "governing artifact is one whose file path — the path passed as an argument to the executor invocation, not a package name or directory — appears as a literal string in the "
        "named executor invocation that produces a result from it; an inline inspection whose output is not produced by invoking a named artifact does not satisfy this requirement. A "
        "governed artifact-producing action is any action that produces or modifies a persistent artifact detectable by a named executor invocation; an action for which no named "
        "executor exists is not a governed artifact-producing action. If no governing artifact exists for a governed behavior, writing that artifact is the first governed "
        "artifact-producing action; the assertion text and the governed symbol must reside in separate artifacts — a governed artifact-producing action that produces both in the same "
        "action does not satisfy the creation-step exception for either. Derivation phase — complete only when a literal block labeled 'Falsify derivation:' containing entries for (a) "
        "through (g) and (f2) appears in the transcript; prose does not satisfy this requirement: Name — (a) the assertion-failure marker: the literal string in an executed-artifact "
        "result signaling the artifact detected behavior absence; (b) the literal string signaling the governed behavior is present; (c) the literal string identifying each governed "
        "behavior in an executed-artifact result; (d) the governed symbol — fixed at derivation time, must appear as a substring of the failure line; (e) the layer classification — "
        "the layer is 'unit' if the governed symbol name appears as a token in a call expression in the test body; the layer is 'integration' if the governed symbol name does not "
        "appear in the test body; (f) the executor class — a governing artifact is disposable if removing the governed symbol from the governed artifact — without modifying the named "
        "governing artifact — produces an executor result in which (c) does not appear; a non-disposable artifact does not satisfy (f); (f2) the serialization gate — the assertion "
        "target is a behavioral value: a value whose content is determined by whether the governed behavior is correctly implemented, not by how its output is represented; if the "
        "assertion target is the return value of a function named after the governed behavior or a named property of the governed symbol's state, it is behavioral; if it is the string "
        "or byte representation of the governed artifact's output, it is a serialization value; when the distinction is ambiguous, the asserter names the behavioral property the "
        "assertion targets; (g) the observed FAIL result: the content of an executed-artifact result block in which (a) precedes (c), the named executor produced the result, and the "
        "failure line contains a substring from the governed assertion call expression — a failure line that contains no such substring does not satisfy (g); (a) must not be separated "
        "from (c) in (g) by any line whose content contains (c) for a different governed behavior. Exception: (g) is not required for the single action that produces the governing "
        "artifact's assertion text, per governed behavior — a creation step is that action only if (c) is absent before the action and present after it, and a named executor "
        "invocation against the governing artifact immediately following the action produces a tool-executed result containing (c); (g) is required for every subsequent governed "
        "artifact-producing action. A governed symbol renamed by a refactor is a new governed symbol requiring a new derivation block before the rename action. If no named executor "
