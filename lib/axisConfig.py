...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "(1) emit 'Observing: property [N] \"<verbatim property definition>\" — <what the artifact observes>'; before proceeding to step (2), verify this line appears in the current "
        "response for property [N] — if absent, emit it now; a property cycle that reaches step (2) without a preceding Observing: line does not satisfy this token; (2) write a "
        "concrete automatic regression guard for property [N] via a tool call and wait for the tool-result block confirming it was written; planning or describing a guard does not "
        "satisfy this token; for every executable assertion in the written guard emit exactly one 'Property [N] assertion [M]: <quoted assertion text>' line — every executable "
        "assertion must appear exactly once and only once; omitting, merging, or inventing an assertion does not satisfy this token; after the assertion inventory emit 'Assertion "
        "inventory: complete — <count> assertions for property [N]' or 'Assertion inventory: incomplete — <reason>'; if incomplete, revise the guard or inventory and repeat the "
        "assertion inventory; no subsequent step may occur until 'Assertion inventory: complete' has been emitted; then emit 'Quoted test: <smallest contiguous verbatim executable "
