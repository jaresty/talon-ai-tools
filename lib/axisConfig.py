...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "executor class — the executor must produce output that could only exist if the governed assertion reached execution at the layer named in (e); (g) the observed FAIL result: "
        "the content of a tool call result block appearing in the transcript after the artifact was applied to the subject and before the first governed action that modifies the "
        "implementation, where (a) precedes (c), the executor named in (f) produced the result, and the failure line contains a string present in the assertion body. Layer gate: if "
        "(e) is 'unit', the governed symbol name must appear in the executor invocation for (g); if (e) is 'integration', it must not. A result satisfies this token if and only if: "
        "(g) is present at the required position; (a) is not separated from (c) in (g) by any line whose content contains (c) for a different governed behavior; and a subsequent "
        "executed-artifact result contains (b) at the position where (a) was absent. A result satisfying (g) must contain a failure line whose content includes a substring from the "
        "assert statement of the governed assertion; a failure line that contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) in its output. "
        "Exception: (g) is not required for the single action that produces the governing artifact's assertion text, per governed behavior — the creation step is that action only; (g) "
        "is required for every action that modifies the implementation, including in the same session — a creation step is that action only if (c) is absent before the action and "
        "present after it, and an executor invocation against the governing artifact immediately following the action produces a tool-executed result containing (c) and not (a). "
        "Semantically-equivalent refactor exception: (g) is also satisfied when a tool call result block containing (a) before (c) appears in the transcript before the file-modifying "
        "call, produced by an executor invocation after the refactored form was modified to violate the invariant the governing assertion checks. The layer gate applies to this "
        "perturbation executor invocation. Revert is complete when the pre-perturbation executor run result is quoted verbatim in the transcript before the revert executor invocation, "
        "and a subsequent executor invocation produces (b) at the position where (a) was absent.",
