...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "observed: [verbatim tool output — content composed without running the tool is invalid]' then 'Gap: [what the verbatim output reveals — gaps inferred from reading source or "
        "test descriptions without running the tool are invalid]' on their own lines — these sentinels gate the immediately following rung only; they do not permit skipping intervening "
        "rungs; a rung manifest or verbal description of expected behavior is never a valid Execution observed; for executable rungs, observation means invoking the artifact as a "
        "running process and recording the terminal output — writing a test file is not observation; the test runner must be invoked; required regardless of whether tests pass or fail; "
        "if tests do not run, fix the rung before descending; if any implementation artifact is produced without valid Execution observed + Gap sentinels immediately preceding it, "
        "discard the implementation and run the tool first. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; stopping mid-thread is only "
        ...
        "descent; when a thread reaches observed running behavior, return to the top for the next thread. Ground applies regardless of perceived task complexity. When a rung reveals "
        "the rung above needs correction — a constraint conflict, a pivot, or new information — correct the rung above to restore faithfulness to I, then re-derive the current rung; "
        "this is correction, not enrichment; if the rung requiring correction is I, surface as a revision signal and stop the current thread. When beginning mid-ladder, first locate "
        "the highest already-instantiated rung and update it to reflect the intended change, then descend. For code contexts, R4 instantiates as: prose → criteria → formal notation → "
        ...
        "declaration; if tests skip, fix the executable validation rung first.) When the lowest V is complete, output 'Validation artifact V complete' on its own line before producing "
        "O. Non-executable rungs appear inline as labeled log entries; executable rungs are placed where they can be executed — in an existing file where a natural home exists, or a "
        "new file where none does.",
