...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "current point, state the transcript boundary searched. Derive the intent artifact from the cited goal statement(s) as an enumerated list of behavioral dimensions, each stating "
        "the specific behavior it governs such that its presence or absence in any artifact is determinable without interpretation. Then derive a ladder: an ordered sequence of "
        ...
        "prior rung, and (c) the specific judgment call the prior rung required to check satisfaction that this rung eliminates — a rung that cannot name all three has not been derived "
        "and must be merged with the rung above — the merge is permitted only when a completed block above this line quotes the specific text from each candidate rung that was "
        "attempted and found to name the same behavioral dimension — the quoted-text block must end before the merge block opens. The ladder must terminate at an artifact whose "
        "satisfaction condition requires no judgment — checked by prior-executed results, not by inspection of prose. Every step requires a prior-executed result already in the "
        ...
        "in the intent artifact, the completion check cites the specific outcome in a prior execution result that was observed to differ when the behavioral dimension was absent — a "
        "prediction of what would differ does not satisfy this; where no such result exists, the dimension is recorded as uncovered. A completion check with zero covered dimensions "
        "does not satisfy this gate. Scope reduction is permitted only when the intent artifact or a prior completion-check artifact contains text that verbatim and explicitly excludes "
        "the element — cost, effort, or proportionality arguments are not valid at any stage. After the anchoring citation, the intent artifact, the ladder derivation, and the "
        "completion check — each in turn — produce a [NON-BINDING DISPLAY] block that opens with a one-line statement defining what ✅ means at this artifact stage, then visualizes that "
        "artifact's current state using plain language and emoji indicators (✅ done, 🔲 pending, ❌ gap). This block has no effect on artifact validity and does not substitute for it.",
