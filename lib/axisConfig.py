...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "else until you have provided justification; write NO code, make NO tool calls, issue NO commands until you have explained why this change satisfies the current rung "
        "requirements; if you cannot justify why it satisfies the rung, ITERATE until it does; file changes without justification are protocol violations. JUSTIFICATION MUST reference "
        "the specific rung requirements being satisfied — vague explanations are insufficient. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement "
        "ONLY what's STRICTLY NEEDED to make that specific assertion green — NO file creation in implementation rung, only edit existing stubs; implement just enough to pass that one "
        ...
        "minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, return UP to find next "
        "gap (ascend ladder); if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion "
