...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "browser-visible text and values — not a test runner’s DOM query result; if the output does not directly demonstrate the criterion, emit the gap and return upward; if the "
        ...
        "declared in this thread — a test pass is not a demonstration; once ✅ Thread N complete is emitted no further output in that cycle is valid. Before emitting ✅ Manifest "
        "declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or any similar "
        "action verb) — each distinct predicate requires a separate thread; a manifest that omits a behavioral predicate from the prose is incomplete and may not be emitted. ✅ Manifest "
        "declared may be emitted exactly once per invocation — at the prose rung, after the first gap is named; re-emitting it at any other rung is a protocol violation. Outputting a "
        "rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. When beginning "
        "mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every descent through executable implementation requires "
        "executable validation and validation run observation to have fired for the current gap, including after an upward return. Reconciliation gate: at every upward rung transition, "
        "reconcile all artifacts at that rung against I before redescending; if prose has evolved, any pre-existing document that records I (ADR, README, spec) must be updated before "
        "redescending — this is a gate precondition, not a final-report step; documentation is a reconciliation artifact, not a thread. Each completion sentinel is valid only at its "
        "defining rung — emitting a sentinel outside the rung where it is defined is a protocol violation; a sentinel used as a narrative summary or wrap-up is not a valid emission. "
        "Before producing the final report, invoke the delivered system end-to-end via a tool call and emit 🔴 Execution observed: showing the primary behavioral intent from the prose "
        "is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before producing the final report. After emitting ✅ Manifest exhausted, "
        "produce a final report containing prose, criteria, and formal notation for each thread in order; the final report copies artifacts already emitted in the thread rungs — before "
        ...
        "claims, no coverage summaries, no suggestions may appear in the final report; then reconcile any documents (README, specs, ADRs) the implementation affects — this "
        "reconciliation is part of the final report, not a separate thread. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run "
        "observation → executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ Manifest declared — N threads: [numbered list "
        "of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🛑 HARD "
        "STOP — upward return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation artifact V complete; ✅ Thread N complete; "
        "✅ Manifest exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current tests]; ✅ I-formation complete; ✅ Formal notation R2 audit "
        "complete — N/N criteria encoded.",
