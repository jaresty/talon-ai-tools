...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "invariant: claim vs reality — what GOES WRONG if you claim without evidence: fool yourself, completion is false; RULE: evidence before claims [ ] The ladder structure: 6 steps "
        "— Intent → Criteria → Structure → Challenge → Refinement → Verification; what GOES WRONG if you skip: gaps lost, completion false; RULE: one rung at a time [ ] Challenge: "
        "derive from P3,P5 — Challenge derives evaluation from Structure to expose gaps; RULE: Challenge = apply evaluation showing gaps exist. IMPORTANT: A gap indicates something is "
        "MISSING - could be a whole thing, structure, or behavior. It does NOT imply creating the complete thing - just addressing what is missing. Challenge must produce MECHANICAL "
        "VERIFICATION: show the specific test/command/query that proves the gap exists and would pass once closed. Without mechanical verification, you cannot confirm closure later. "
        "The gap shown at Challenge is what must be addressed at Refinement to be considered complete. IMPORTANT: MECHANICAL means any REPRODUCIBLE PROOF — not just code. For "
        "non-software tasks, show the specific evidence or demonstration that could be repeated to prove the gap exists.[ ] Structure before Challenge: derive from P1,P3 — solution "
        "organization must precede gap detection; what GOES WRONG if you skip structure: unclear what to evaluate; RULE: structure before challenge [ ] Criteria before structure: "
        "derive from P1 — success conditions before organization; what GOES WRONG if you organize without criteria: no way to evaluate; RULE: criteria before structure [ ] Intent "
        "before criteria: derive from P1 — outcome before success conditions; what GOES WRONG if you define success without outcome: criteria irrelevant; RULE: intent before criteria [ "
        ...
        "transitioning to next rung, verify and declare: [current rung artifact completed?], [evidence visible?], [gaps remaining?], [next rung requires?]. If gaps remain or "
        "verification not shown, do NOT proceed. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P5. Ladder derived MUST be emitted as: 🔵 "
