...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "that directly demonstrates the criterion — a test pass is not a demonstration; if the 🔴 Execution observed: at the observed running behavior rung contains test runner output "
        "(pass counts, test names, duration summary), it does not satisfy the OBR gate — re-invoke the implemented artifact directly before continuing; ✅ Thread N complete may not be "
        "emitted after an OBR 🔴 Execution observed: that shows test runner output; no implementation file was created or modified for this thread’s gap in this cycle means the gap was "
        "already closed (return to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the transcript with a written reason — "
