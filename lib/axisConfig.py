...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "planned artifacts, rung labels, and derivation sources. Gate: output 'Rung manifest complete' on its own line before any artifact. Boundary: no artifact appears before this "
        "phrase. I is the declared intent (the fixed governing authority); V is a constraint artifact derived from I through the ladder; O is the output evaluated against V. Three "
        "rules govern every thread. R1 (I is fixed): every artifact derives faithfully from I and the prior rung — the form changes, the intent does not. R2 (rung criterion): an "
        "artifact is a rung iff upward-faithful (derived from the prior rung without adding unstated constraints) and downward-sufficient (self-contained to evaluate the next artifact "
        "without consulting I); domain phases are not downward-sufficient — writing tests for 'API layer' requires knowing what the API must do, which comes from I, not the phase name. "
        "R3 (observation terminates): every thread descends to observed running behavior; anticipated outcomes, asserted expectations, or conversational acknowledgment never satisfy "
        "the execution gate; only tool-executed output with a declared gap does; skipped tests are not observations. Execution gate (R3): before each rung descent output 'Execution "
        "observed: [actual tool output]' then 'Gap: [what the observation reveals about what the next rung must produce]' on their own lines — these sentinels gate the immediately "
        "following rung only; they do not permit skipping intervening rungs; for executable rungs, observation means invoking the artifact as a running process and recording the "
        "terminal output — writing a test file is not observation; the test runner must be invoked; required regardless of whether tests pass or fail; if tests do not run, fix the rung "
        "before descending. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; stopping mid-thread is only permitted when the next rung is not "
        "achievable (the domain provides no standard artifact type for it); the active completeness token governs how many threads are batched per descent; when a thread reaches "
        "observed running behavior, return to the top for the next thread. Ground applies regardless of perceived task complexity. When a rung reveals the rung above needs correction — "
        "a constraint conflict, a pivot, or new information — correct the rung above to restore faithfulness to I, then re-derive the current rung; this is correction, not enrichment; "
        "if the rung requiring correction is I, surface as a revision signal and stop the current thread. When beginning mid-ladder, first locate the highest already-instantiated rung "
        "and update it to reflect the intended change, then descend. For code contexts, R4 instantiates as: prose → criteria → formal notation → executable validation (tests, types, "
        "contracts, schemas, property checks, passing validation runs) → validation run observation → executable implementation → observed running behavior. (Validation run observation "
        "records the actual output of the executable validation rung and declares the gap — tests must reveal the gap — tests covering behavior that does not yet exist must fail; if "
        "all tests pass before new implementation has been written, either the intended behavior is already implemented (verify this; the thread may be complete) or the tests are not "
        "testing the intended behavior and must be corrected; the executable implementation rung is blocked until the gap has been observed; the executable implementation rung is "
        "scoped to the declared gap — implement only what closing the observed gap requires; anticipated future behavior belongs to a future gap declaration; if tests skip, fix the "
        "executable validation rung first.) When the lowest V is complete, output 'Validation artifact V complete' on its own line before producing O. Non-executable rungs appear "
        "inline as labeled log entries; executable rungs are placed where they can be executed — in an existing file where a natural home exists, or a new file where none does.",
