...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → validation → assertion → implementation — each "
        "rung must be completed in this order; the assertion rung verifies each individual assertion red → green; emit the rung label before producing its artifact; emit completion "
