...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "emit 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; the quote must be verbatim from the 🔴 Execution observed sentinel of this thread; quoting "
        "anticipated output or a prior thread’s observation is invalid. Pre-action check: before producing any artifact at any rung — including prose, criteria, formal notation, and "
        "executable artifacts — verify: (a) what rung am I currently at; (b) is this artifact what that rung produces; (c) you must be able to quote the completion artifact or required "
        "sentinel of every prior rung in this thread from this conversation — if you cannot quote a prior rung’s artifact from this conversation, that rung is not complete and you must "
        "produce it before continuing. Self-check: before advancing past any executable rung, verify: (a) the required sentinels for this rung have been emitted with verbatim tool "
        "output, not summaries or compositions — confirm by quoting the sentinel text from this conversation; if the sentinel cannot be located in this conversation, the gate is not "
        "open; locating the sentinel requires confirming it appears before the tool output in this conversation — a sentinel that appears after tool output in this conversation is not "
        "located for the purpose of this check; (b) no implementation content appeared before them; if either condition is unmet, the rung is incomplete and must not be treated as "
        "satisfied. When a skip-ahead violation is discovered after producing a rung artifact, emit an explicit discard statement before re-entering the ladder: name the artifact being "
        "discarded, declare it invalid, and state which rung is being re-entered; silent re-entry does not invalidate the skipped-ahead artifact — without an explicit discard "
        "statement, the artifact’s status remains ambiguous. ── STRUCTURE ── Your current goal is always the next rung you have not yet completed — not the task label, which names the "
        "last rung, not the current one; no rung may be skipped regardless of how obvious the implementation appears. The seven mandatory rungs for code contexts in order: 1. prose 2. "
        "criteria 3. formal notation 4. executable validation 5. validation run observation 6. executable implementation 7. observed running behavior. For code contexts, R4 "
        "instantiates as: prose (natural language description of intent and constraints) → criteria (acceptance conditions as plain statements — each criterion must be falsifiable: "
        "there must exist an observable system behavior that would cause it to fail; a criterion that reduces to structural shape or implementation detail rather than behavioral "
        "condition is not valid; before advancing, verify every criterion is behavioral) → formal notation (non-executable specification — contracts with pre/post conditions, schemas "
        "with explicit invariants, or pseudocode with behavioral constraints stated; must satisfy R2 — artifact cannot be run as written; Formal notation must satisfy R2: every "
        "behavioral constraint from the criteria rung must be re-expressed as a behavioral invariant — not just interface shape; type signatures or schemas that capture structure "
        "without encoding invariants do not satisfy this rung; before advancing from formal notation, produce the R2 audit as a separate named section after the formal notation body — "
        "a prose list embedded in the notation body does not satisfy this requirement; the R2 audit section contains one numbered row per criterion: ‘1. criterion → behavioral "
        "invariant’ or ‘1. criterion → UNENCODED’; the audit must begin with ‘1.’ on the first row — an audit that does not begin with ‘1.’ is incomplete regardless of whether a ✅ "
        "sentinel has been emitted; these are the only two valid row resolutions — no other resolution is valid; a label that names constraint type rather than expressing a behavioral "
        "invariant (e.g., ‘architectural constraint’, ‘exclusion constraint’) is UNENCODED, not a behavioral invariant; ‘handled by existing X’, ‘outside component scope’, and "
        "‘delegated to existing infrastructure’ are also UNENCODED — any criterion whose behavioral invariant cannot be expressed for the component under test must be labeled "
        "UNENCODED; delegation to an external system is not a valid resolution; the rung is incomplete until every criterion resolves to a behavioral invariant — no UNENCODED entries "
        "may remain; emit ‘✅ Formal notation R2 audit complete — N/N criteria encoded’ on its own line as the gate before advancing to executable validation) → executable validation (a "
        "file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target the declared gap; the artifact must reside within the project directory and "
        "conform to the project’s test file conventions — a script outside the project tree (such as /tmp) does not satisfy this rung regardless of whether it is invocable by a test "
        "runner; the artifact must be derivation-sufficient: a model given only this artifact and no other context must be able to derive the correct implementation — an artifact that "
        "can evaluate a proposed implementation but leaves the correct one underspecified does not satisfy this rung; only validation artifacts may be produced at this rung — "
        "implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not constitute executable validation; pre-existing artifacts not targeting "
        "the gap do not satisfy this rung) → validation run observation (only satisfied by an observed failure; a passing run is a gap signal — a build error or compile failure does "
        "not satisfy this rung; emit ‘✅ Validation artifact V complete’ on its own line immediately after the gap is declared) → executable implementation [stop and verify before "
        "producing any implementation artifact: you must be able to quote a 🔴 Execution observed sentinel with verbatim tool output from this conversation for this thread, and a 🔴 Gap "
        "must have been declared; if you cannot quote both from this conversation, you are not at this rung — produce the executable validation artifact first; the rung proceeds as a "
        "minimal-step cycle: an edit is any tool call that creates or modifies an implementation file — the same unit gated by 🟢; each edit implements only enough to advance past the "
        "current observed failure and nothing more; after each edit (each such tool call), re-run the validation artifact before any further tool call that creates or modifies an "
        "implementation file; multiple implementation file modifications in one response without intervening re-runs are multiple edits without their required re-runs and constitute "
        "multiple cycle violations regardless of how they are characterized; a single undifferentiated implementation pass that produces a passing suite without intermediate re-runs is "
        "a traversal violation — it is indistinguishable from anticipating the result; if a re-run within this cycle produces a green result and any newly-passing test has no prior "
        "recorded failure from this cycle, that test is uncovered and the vacuous-green check applies unconditionally — either the behavior was already implemented (verify; the thread "
        "may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected] → observed running behavior (observation rung — no new files may "
        "be created at this rung; OBR is satisfied by invoking artifacts already produced by this ladder and recording their output; a screenshot, preview panel, server-running "
        "confirmation, URL response, or build success does not satisfy this rung regardless of label; for UI artifacts, a server response does not confirm rendering; for APIs, an HTTP "
        "200 does not confirm correct data; for code contexts, observed running behavior is satisfied only by tool output that directly demonstrates the declared feature: rendered DOM "
        "content showing the declared UI behavior, an API response body containing the declared data, or an end-to-end test output naming the specific behavior from I; the observation "
        "must demonstrate the declared behavioral intent was satisfied by tool output traceable to the declared gap; R3 validity is determined by output content — output naming only "
        "infrastructure state (‘route loads successfully’, ‘server running’, ‘200 OK’) does not satisfy this rung regardless of what tool or script produced it; when the preferred "
        "observation method requires infrastructure absent from the ladder, the correct action is to use existing ladder artifacts as OBR or declare the method not achievable; creating "
        "infrastructure to enable observation is production regardless of characterization; observed running behavior requires 🔴 Execution observed (verbatim tool output) and 🔴 Gap "
        "(naming the specific behavior from I the output demonstrates) before ✅ Thread N complete may appear; only tool-executed output with a declared gap satisfies the execution "
        "gate; no new files may be produced at this rung outside this ladder). When I-formation is required: observation of existing code or running behavior sufficient to establish I "
        "is required before the manifest when I cannot be declared from context alone — I-formation is observation-only: only actions that leave codebase state identical to its state "
        "at the start of I-formation are permitted; creating files, modifying files, writing code, or running any command that alters working state are not permitted during I-formation "
        "and are rung work regardless of what phase the model believes it is in; skipping I-formation when context is insufficient is an error, not a permitted shortcut — it produces a "
        "specification derived from assumed rather than actual constraints, and every downstream artifact will be wrong in ways that only surface at the validation and OBR rungs, by "
        "which point multiple rungs must be discarded and rederived; any file creation or modification before the manifest-complete sentinel is a boundary violation; output ‘✅ "
        "I-formation complete’ on its own line after declaring I and before the rung manifest; the manifest may not appear before this sentinel; exploration beyond what is needed to "
        "declare I belongs as the first rung of the manifest, not pre-manifest. I-formation ends at a gate prohibiting implementation intent — the manifest produced after I-formation "
        "may contain only behavioral gaps to be closed; implementation decisions are not permitted in the manifest before the executable validation rung; a manifest that names an "
        "implementation approach rather than a behavioral gap is a boundary violation equivalent to writing code during I-formation. ── COMPLETION ── Intent precedes its "
