...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "split may be accepted. Step 3 — Evaluate split validity. Immediately after the canonical provisional definitions, emit exactly one split test: '§ split test: property [Na]: "
        '"<verbatim canonical definition>" — falsified by <construction>; property [Nb]: "<verbatim canonical definition>" — falsified by <construction>; these falsifiers are '
        "independent.' The quoted definitions must match the immediately preceding canonical provisional definitions verbatim. A split succeeds only if: the conjunction of the accepted "
        "sub-properties has identical coverage to the original property; each quoted falsifier violates exactly its quoted property while satisfying the other; both falsifiers are "
        "admissible constructions within the chosen interpretation. Step 4 — Recurse or mark atomic. If the split succeeds, replace the original property with the accepted canonical "
        ...
        "doing so does not reduce coverage of the explicit request constraints. If a merge or removal changes the retained property set, repeat the atomicity and completeness "
        ...
        "they collectively cover every explicit request constraint; they are observationally independent; they introduce no out-of-scope constraints. When all conditions are met, emit "
