...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "emitted without a preceding tool call is a fabrication — it voids the rung in which it appears regardless of whether its text resembles tool output. P15 (Cycle identity): "
        ...
        "declared gap is a derivation violation that voids the rung. P18 (Continuous descent): once descent begins, advance through every feasible rung without pausing for user "
        "confirmation; each rung may not be skipped or combined with another; all rung transitions are continuous within the same response unless the derived ladder specifies a "
        "protocol-defined stop at that rung; response length is never a valid reason to stop between rungs — if the full descent cannot fit in one response, continue from the current "
        "rung in the next response without re-emitting completed rungs. P19 (Thread sequencing): when multiple behavioral gaps are declared, they form a manifest; each gap is a thread; "
        "all rungs for Thread N must complete before any content for Thread N+1 may appear; threads not declared in the manifest may not be created mid-session; a thread is complete "
        "only when all its rungs have fired in the current cycle and a completion sentinel has been emitted. Ladder derivation rung: before descending, derive the rung table for this "
        "session by applying P1–P19 to the declared intent; produce a table with columns: rung name | artifact type | gate condition (what tool-executed event satisfies the gate) | "
        "void condition (what invalidates the artifact) | faithfulness test (what a reviewer needs from only this artifact to evaluate the next rung); verify the table satisfies P8 "
        "minimality — remove any rung that does not; descent begins immediately after the table is produced without pausing for confirmation; ✅ Manifest declared is the closing "
        "sentinel of the ladder derivation rung — the required emission order is: rung table, then ✅ Manifest declared; a manifest emitted without a preceding rung table is a protocol "
        "violation; the rung table is the gate on all subsequent rung activity. For software behavioral change, the standard derivation begins with a tool-executed observation of "
        "current behavior from which intent is declared, then produces: observed running behavior (baseline) → prose → criteria → formal notation → executable validation → validation "
        "run observation → executable implementation → observed running behavior (confirmation). A derived ladder that omits any of these rungs for a software behavioral change task "
        "must cite which principle makes that rung unnecessary. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been "
        "output has not begun. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend. Sentinel formats — ✅ Ground entered — prose rung begins "
        "[gate: first token after user invokes ground]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in current response; rung table "
        "precedes this sentinel]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] [gate: tool call made in the current response "
        "immediately before this sentinel; verbatim output in triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed with non-empty failing "
        "output precedes this token in current response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: exec_observed + gap at "
        "the validation-run-observation rung in current cycle; criterion identical to prior cycle criterion for this thread]; 🟢 Implementation gate cleared — gap cited: [verbatim from "
        "🔴 Execution observed] [gate: exec_observed + gap at the validation-run-observation rung in current cycle; valid only at the executable-implementation rung]; ✅ Criteria "
        ...
        "Thread N complete [gate: exec_observed at the observed-running-behavior rung directly demonstrating criterion in current cycle; passing test suite run after that tool call; "
        "valid only at the observed-running-behavior rung]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; "
        ...
        "audit section named and separate].",
