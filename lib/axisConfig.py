...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "a time [ ] Challenge: derive from P3,P5 — Challenge derives evaluation from Structure to expose gaps; RULE: Challenge = apply evaluation showing gaps exist. IMPORTANT: A gap "
        "indicates something is MISSING - could be a whole thing, structure, or behavior. It does NOT imply creating the complete thing - just addressing what is missing. Challenge "
        "must produce MECHANICAL VERIFICATION: show the specific test/command/query that proves the gap exists and would pass once closed. Without mechanical verification, you cannot "
        "confirm closure later. The gap shown at Challenge is what must be addressed at Refinement to be considered complete. IMPORTANT: MECHANICAL means any REPRODUCIBLE PROOF — not "
        "just code. For non-software tasks, show the specific evidence or demonstration that could be repeated to prove the gap exists.[ ] Structure before Challenge: derive from P1,P3 "
        "— solution organization must precede gap detection; what GOES WRONG if you skip structure: unclear what to evaluate; RULE: structure before challenge [ ] Criteria before "
        "structure: derive from P1 — success conditions before organization; what GOES WRONG if you organize without criteria: no way to evaluate; RULE: criteria before structure [ ] "
        "Intent before criteria: derive from P1 — outcome before success conditions; what GOES WRONG if you define success without outcome: criteria irrelevant; RULE: intent before "
        "criteria [ ] Evidence at proximity: derive from P3 — evidence must be temporally close to result; what GOES WRONG if delayed: proof weakens; RULE: evidence immediately after "
        "result[ ] Evidence echoed: derive from P3,P4 — explain how evidence proves rules satisfied; what GOES WRONG if unexplained: description indistinguishable from compliance; "
        "RULE: explain evidence connection [ ] No contradiction in completion: derive from P3,P5 — claiming complete while acknowledging gaps is void; what GOES WRONG if you write "
        "'complete' with 'gaps', 'caveats', 'needs': claim invalid; RULE: scan ENTIRE message for contradiction before 'complete' [ ] Rule citation at action: derive from P1,P3,P4 — "
        "cite specific derived rule before each action; what GOES WRONG if uncited: protocol compliance invisible; RULE: cite rule before action This checklist must appear BEFORE the "
        "first ladder rung begins. The derivation results must be in the chat, not just in your head. Before starting each rung, re-emit key rules from your derivation relevant to that "
        "rung. BEFORE transitioning to next rung, verify and declare: [current rung artifact completed?], [evidence visible?], [gaps remaining?], [next rung requires?]. If gaps remain "
        "or verification not shown, do NOT proceed. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P5. Ladder derived MUST be emitted as: 🔵 "
        ...
        "Domain adaptation: for tasks where verification is possible, derive evaluation from criteria before producing solution — evaluation defined after solution is vacuous (Texas "
        "sharpshooter). Challenge exposes gaps; Refinement adds only enough to satisfy each; Verification confirms completion. IMPORTANT: When deriving ladder artifact types, use "
        "MINIMAL names: - For Refinement, use 'Partial Implementation' or 'Minimal Code Changes' — NOT 'Implementation' or 'full solution' - Artifact type names that imply completeness "
        "('Implementation', 'Full Solution') contradict the gap-driven approach. If output diverges from protocol intent (e.g., producing full implementation instead of addressing "
        "specific gaps), reflect on: Which rung first diverged? What artifact type name drove this? What would a minimal change look like? Refinement should address ONE gap at a time "
        "with minimal changes — not build the entire feature. If you find yourself creating a complete feature, you have deviated from gap-driven refinement. for writing tasks, it maps "
        "to intent→criteria→structure→challenge→refinement→verification; for decision-making, it maps to intent→criteria→analysis→challenge→refinement→verification. The model should "
