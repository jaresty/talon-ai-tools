...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rung; add a criterion asserting the element’s content or effect (the row displays the value from the data source); the criterion for thread N must name the same behavior as "
        "the gap declared for thread N in ✅ Manifest declared — narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is "
        "a protocol violation; if the declared gap cannot be satisfied, return to the prose rung and revise the manifest. Each criterion must be written in two parts: (a) observable "
        "behavior — what a non-technical observer of the running system would see; (b) falsifying condition — what behavioral gap would cause this criterion to fail; before advancing, "
        "verify both parts are present; a criterion without an explicit falsifying condition is incomplete at time of production and must not be advanced from; if the falsifying "
        "condition requires knowledge of implementation internals, the criterion is structural, not behavioral, and must be rewritten. Formal notation encodes only the criteria "
        "declared at the criteria rung — no additional invariants, no anticipated cases; it produces only type signatures, interfaces, pseudocode, and structural invariants — the test "
        "of valid formal notation is that it cannot be directly compiled or executed without modification; if placing the notation verbatim in the source tree would produce a runnable "
        "artifact, it is implementation code and voids the rung; annotation-decorated classes, repository interfaces, and any construct that compiles as-is are implementation code "
        "regardless of whether they contain imperative logic. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ "
        "Validation artifact V complete; the artifact must be invocable by an automated tool; before writing the test, re-read the formal notation and assert each structural constraint "
        "it encodes; each test function contains exactly one assertion — one assertion per test function; a pre-existing test that happens to pass does not satisfy this rung — add to "
        "an existing test file covering the same module if one exists, otherwise create a new file; when the validation artifact is modified — tests added, rewritten, or mocks changed "
        "— emit a carry-forward statement before any implementation artifact: carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution "
        "observed sentinel] covers current test [verbatim test name as written in the artifact]’ — one row per current test; each prior-failure name must be quotable verbatim from a 🔴 "
        "Execution observed sentinel in this conversation; any current test whose name does not appear in a carry-forward row is unconditionally uncovered; modification without "
        ...
        "the test is vacuous and must be rewritten; implementation edits may not begin until a red run exists in the transcript for the current cycle — if the validation artifact "
        "passes before any implementation edit, either the test is vacuous (rewrite it) or the gap is already closed (return to criteria); ✅ Validation artifact V complete may not be "
        "emitted unless a 🔴 Execution observed: showing the test failing exists in the transcript for the current cycle — write the test, run it, confirm it fails, then emit "
        "V-complete; emitting V-complete before seeing a red run is a protocol violation regardless of whether perturbation was attempted. ✅ Validation artifact V complete must be "
        "emitted at the executable validation rung before the validation run observation label is written. Before writing the validation run observation rung label, invoke the "
        "validation artifact via a tool call — predicting what the run would show does not satisfy this gate regardless of accuracy; a green 🔴 Execution observed: is only valid as the "
        "post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared for this thread showing the test "
        "logic failing — a harness error (import failure, syntax error, missing file) is not a red run; the test must have been loadable, its assertions must have run, and they must "
        "have failed; fix the harness error before treating any run as a red witness — do not proceed to the implementation gate until a loadable run with failing assertions exists in "
        "the transcript for the current cycle. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming "
        ...
        "token in the response — any other content before the criteria label is a protocol violation; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution "
        "observed: and 🔴 Gap: have been emitted at the validation run observation rung in the current cycle — emitting it after a criteria label, after a formal notation label, or at "
        "any other position is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. "
        ...
        "formal notation; observation cannot be produced is a prose-description failure, not an implementation failure — return to the prose rung and update it. One edit per re-run "
        "cycle; after each edit re-run the validation artifact before any further edit. After every upward return, a new ✅ Validation artifact V complete must be emitted for the "
        "current cycle’s gap before the implementation gate may fire — an artifact written in a prior cycle does not satisfy this gate even if it covers the same path. Upon writing the "
        ...
        "cited rung must be quotable from this conversation; a tool call with no citable provenance is creating a new artifact regardless of characterization and is prohibited at this "
        "rung; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the observed running behavior label has "
        "been written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it and the OBS output directly "
        "demonstrates each criterion declared in this thread — a test pass is not a demonstration; no implementation file was created or modified for this thread’s gap in this cycle "
        "means the gap was already closed (return to criteria) or the test is vacuous (rewrite it) — Thread N complete may not be emitted when no implementation edit was made in this "
        "cycle; before emitting ✅ Thread N complete or ✅ Manifest exhausted, run the full test suite; every failing test must be fixed or explicitly acknowledged in the transcript with "
        "a written reason — dismissing a failing test as unrelated without an explicit written acknowledgment is a protocol violation; once ✅ Thread N complete is emitted no further "
        "output in that cycle is valid. Each sentence in the prose containing a behavioral predicate — any action verb attributing behavior to the system (fetches, requires, displays, "
        "filters, renders, validates, authenticates, loads, saves, or similar) — must be followed inline by a bracketed thread marker: [T: gap-name]; these thread markers become the "
        "authoritative enumeration for the manifest and the Thread N complete check; a prose rung with unmarked behavioral predicates is incomplete at time of production. Before "
        ...
        "saves, or any similar action verb) — each distinct predicate requires a separate thread; a manifest that omits a behavioral predicate from the prose is incomplete and may not "
        "be emitted. ✅ Manifest declared may be emitted exactly once per invocation — at the prose rung, after the first gap is named; re-emitting it at any other rung is a protocol "
        "violation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it "
        "may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every descent through executable "
        "implementation requires executable validation and validation run observation to have fired for the current gap, including after an upward return. Reconciliation gate: at every "
        "upward rung transition, reconcile all artifacts at that rung against I before redescending; if prose has evolved, any pre-existing document that records I (ADR, README, spec) "
        "must be updated before redescending — this is a gate precondition, not a final-report step; documentation is a reconciliation artifact, not a thread. Each completion sentinel "
        "is valid only at its defining rung — emitting a sentinel outside the rung where it is defined is a protocol violation; a sentinel used as a narrative summary or wrap-up is not "
        "a valid emission. Before producing the final report, invoke the delivered system end-to-end via a tool call and emit 🔴 Execution observed: showing the primary behavioral "
        "intent from the prose is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle before producing the final report. After emitting ✅ "
        "Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order; the final report copies artifacts already emitted in the "
        "thread rungs — before writing each section, locate the artifact in the prior transcript; if it cannot be found verbatim in the transcript, it may not appear in the final "
        "report; no new behavioral claims, no coverage summaries, no suggestions may appear in the final report; then reconcile any documents (README, specs, ADRs) the implementation "
        "affects — this reconciliation is part of the final report, not a separate thread. Seven rungs in order: prose → criteria → formal notation → executable validation → validation "
        "run observation → executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ Manifest declared — N threads: [numbered "
        "list of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🛑 "
        "HARD STOP — upward return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation artifact V complete; ✅ Thread N "
        "complete; ✅ Manifest exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current tests]; ✅ I-formation complete; ✅ Formal notation R2 "
        "audit complete — N/N criteria encoded.",
