...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rejects properties whose primary subject is internal and not explicitly required — not merely properties that happen to be inaccessible. Step 4 — Recurse or mark atomic. If "
        "the split succeeds, replace the original property with the accepted canonical sub-properties and apply this same recursive decomposition procedure (Steps 1–4) independently to "
        "each accepted sub-property. If no valid split exists, emit: '§ split test: property [N]: \"<verbatim canonical definition>\" — atomic, no valid split: <reason>.' The quoted "
        "definition must match the retained canonical property verbatim. Apply this procedure to every retained property until all have reached Step 4 with no valid split. After all "
        "retained properties are atomic, emit a completeness check: '§ completeness check: \"<request constraints verbatim>\" / <P1 expression> / <P2 expression> / ...' — the request "
        "constraints must be quoted verbatim from the explicit constraints of the request as clarified by the chosen interpretation; interpretive assumptions made to resolve ambiguity "
        "and observation-boundary definitions are not themselves request constraints unless they were explicit constraints of the original request; every retained atomic property "
        "expression must appear exactly once, separated by ' / '; a '§ completeness check:' line that omits the request constraints or omits any retained property expression does not "
        "satisfy this requirement. Immediately after the completeness check, attempt to identify one explicit request constraint that is not logically entailed by the conjunction of "
        "the retained atomic property expressions. If such a constraint exists, emit '§ properties complete? no' — add one new property covering that explicit constraint, perform the "
        "required split testing, and repeat the completeness check. Only when every explicit request constraint is logically entailed by the conjunction of the retained atomic property "
        "expressions may the response emit '§ properties complete? yes' — a '§ properties complete? yes' line without a preceding completeness check does not satisfy this protocol. "
        "After completeness has been established, verify observational independence. A retained property is observationally independent if there exists at least one admissible "
        "construction that violates that property while satisfying every other retained property. Any retained property that is not observationally independent is observationally "
        "redundant and must be removed or merged, provided doing so does not reduce coverage of the explicit request constraints. A proposed merge does not remove its source properties "
        "until the merged property has successfully completed canonicalization, scope validation, recursive decomposition, and revalidation of completeness and observational "
        "independence over the resulting retained set. If the merged property cannot be retained, the source properties remain in the retained set unchanged. The merged property is a "
        "new property and must enter the recursive decomposition procedure (Steps 1–4) as a newly derived property before being added to the retained set. If a merge or removal changes "
        "the retained property set, the current completeness and independence results are void; re-run the recursive decomposition procedure for every newly introduced property, then "
        "re-establish completeness and observational independence over the resulting retained set. The Ground properties block is complete only when all retained properties "
        "simultaneously satisfy: they are derived from the chosen interpretation; they are atomic; their conjunction logically entails every explicit request constraint; they are "
        "observationally independent; they introduce no out-of-scope constraints, including internal mechanism constraints not explicitly required by the chosen interpretation. When "
        "all conditions are met, emit '§ ground complete' as the final line of the Ground properties block — this sentinel may not appear before '§ properties complete? yes' in the "
        "transcript. Any test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does not satisfy this protocol; if task reasoning begins and '§ "
        "ground complete' has not yet appeared, emit the Ground properties block in full and '§ ground complete' before continuing.",
