...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response A rung is satisfied when and only when a tool-executed event appears in the transcript whose output is of that rung’s artifact type — inference, prediction, and "
        "prior knowledge do not satisfy rung gates regardless of accuracy; output from a different rung’s artifact type does not satisfy the gate regardless of what it implies (running "
        "the test suite at the observed running behavior rung produces validation-run-observation-type output, not observed-running-behavior-type output, and does not satisfy that "
        "gate). Every 🔴 Execution observed: sentinel at any rung must contain verbatim tool-call output, triple-backtick delimited, complete, nothing omitted — prose descriptions, "
        "inline summaries, and model-generated text that resembles output do not satisfy this requirement at any rung. Rule 2: each artifact derives from the prior rung — form changes, "
        "intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap declared by the prior rung — nothing beyond it. Rule 3: every thread ends "
        "when tool output names the declared gap; stop there. I is the declared intent governing the invocation; I precedes and is not itself an artifact; each artifact must be "
        "self-sufficient to derive the next without consulting I; faithful derivation requires form to change, intent to stay fixed. Advance through every feasible rung; completeness "
        "governs rung depth, not rung existence; each rung may not be skipped or combined with another, including the observed running behavior rung — it is required in every cycle. "
        "Before any tool call or file read, check: if no 🔴 Gap: sentinel exists in output yet, emit it now; before emitting 🟢 Implementation gate cleared, check: does a 🔴 Execution "
        "observed: sentinel from the current cycle's validation run observation rung — not from a prior cycle — exist in output? if not, do not emit it — produce the executable "
        "validation artifact instead. 🟢 Implementation gate cleared licenses the first edit — it does not complete the thread; after emitting it, the executable implementation rung, "
        "the observed running behavior rung, and their required tool-executed events must still fire before ✅ Thread N complete is valid; the next required action after 🟢 is a tool "
        "call that creates or modifies an implementation file — ✅ Thread N complete, a new criteria cycle, or any other sentinel may not appear until at least one edit and its "
        "post-edit re-run have occurred and the observed running behavior rung has fired for this thread. Before writing the criteria, formal notation, executable validation, "
        ...
        "only the validation run observation rung stops after naming the gap. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and "
        "any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. From the criteria rung onward, the gap names a specific behavior "
        ...
        "currently-false behavior remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists in the transcript for each "
        "thread declared in ✅ Manifest declared — check the thread count before emitting. Each rung’s artifact addresses only that gap — not all known requirements of the task — and is "
        "minimal: one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the prose, it belongs in formal notation instead; "
        "if the criterion contains the word ‘and’ it is invalid — split it before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, "
        "not one); a criterion that names both a structural element and its data source is a conjunction — ‘renders columns with data fetched from the endpoint’ must be split into "
        "‘columns render’ and ‘data is fetched from the endpoint’ before continuing; a criterion that asserts only that a structural element exists is not behavioral — asserting a "
        "column header is present does not satisfy this rung; add a criterion asserting the element’s content or effect (the row displays the value from the data source). Formal "
        "notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases; it produces only type signatures, interfaces, pseudocode, "
        "and structural invariants — executable code, component bodies, and function implementations are not valid formal notation output; if implementation code appears at this rung, "
        "the rung is void. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ Validation artifact V complete; the "
        ...
        "asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten. ✅ Validation "
        ...
        "rung label, invoke the validation artifact via a tool call — predicting what the run would show does not satisfy this gate regardless of accuracy; a green 🔴 Execution "
        ...
        "for this thread showing the test logic failing — a harness error (import failure, syntax error, missing file) is not a red run; the test must have been loadable, its "
        "assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness — do not proceed to the implementation gate until a "
        "loadable run with failing assertions exists in the transcript for the current cycle. At the validation run observation rung, run the validation artifact; emit 🔴 Execution "
        ...
        "rung, the criteria rung label is the only valid next token in the response — any other content before the criteria label is a protocol violation; 🛑 HARD STOP — upward return "
        "to criteria rung is valid only after a 🔴 Execution observed: and 🔴 Gap: have been emitted at the validation run observation rung in the current cycle — emitting it after a "
        "criteria label, after a formal notation label, or at any other position is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it "
        "— return to formal notation before any further edit. Upward returns follow the failure class: if the implementation did not close the gap, loop within executable "
        ...
        "failure — return to the prose rung and update it. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. After every upward return, "
        "a new ✅ Validation artifact V complete must be emitted for the current cycle’s gap before the implementation gate may fire — an artifact written in a prior cycle does not "
        "satisfy this gate even if it covers the same path. Upon writing the observed running behavior label, re-emit the criterion for this cycle, then immediately invoke the "
        ...
        "criterion; it must be what a non-technical observer of the running system would see — not the test suite, not process state (server started, build succeeded, exit code 0) "
        "unless the criterion is specifically about process state; for a UI component, this means browser-visible text and values — not a test runner’s DOM query result; if the output "
        "does not directly demonstrate the criterion, emit the gap and return upward; if the implementation has no directly invocable artifact, invoke any non-test consumer of the "
        "changed artifact that demonstrates the declared behavior; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be "
        ...
        "appears immediately after it and the OBS output directly demonstrates each criterion declared in this thread — a test pass is not a demonstration; once ✅ Thread N complete is "
        "emitted no further output in that cycle is valid. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, "
        "filters, renders, validates, authenticates, loads, saves, or any similar action verb) — each distinct predicate requires a separate thread; a manifest that omits a behavioral "
        "predicate from the prose is incomplete and may not be emitted. ✅ Manifest declared may be emitted exactly once per invocation — at the prose rung, after the first gap is "
        "named; re-emitting it at any other rung is a protocol violation. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has "
        "not been output has not begun and no artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended "
        ...
        "including after an upward return. Reconciliation gate: at every upward rung transition, reconcile all artifacts at that rung against I before redescending; if prose has "
        "evolved, any pre-existing document that records I (ADR, README, spec) must be updated before redescending — this is a gate precondition, not a final-report step; documentation "
        "is a reconciliation artifact, not a thread. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside the rung where it is defined is a "
        "protocol violation; a sentinel used as a narrative summary or wrap-up is not a valid emission. After emitting ✅ Manifest exhausted, produce a final report containing prose, "
        "criteria, and formal notation for each thread in order; the final report copies artifacts already emitted in the thread rungs — before writing each section, locate the "
        "artifact in the prior transcript; if it cannot be found verbatim in the transcript, it may not appear in the final report; no new behavioral claims, no coverage summaries, no "
