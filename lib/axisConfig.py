...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "block does not satisfy §1; immediately below the heading, derive a means-test: write 'The goal [text] could be achieved by: [alternative means not listed in §0]' — if no "
        ...
        "candidates exist, write both and mark one '[selected]' — the selected goal is the one whose means-test names a more concrete alternative than those listed in §0; when goal and "
        "means-test are both present, write '§1 goal derived'; Path B — (0) write the literal string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — "
        "this declaration satisfies §0; (1) derive the governing goal: if the user's message states the goal verbatim, '## Governing goal: [text]' where [text] appears verbatim in the "
        "user's message; if the goal must be derived from a scenario description, '## Governing goal: [derived: text]' where [derived: text] is a goal derived from the scenario "
        "description without adding claims absent from the user's message — a '[derived: text]' value that adds a claim absent from the user's message does not satisfy §1; immediately "
        "below the heading, derive a means-test: write 'The goal [text] could be achieved by: [alternative means]'; when goal and means-test are both present, write '§1 goal derived'; "
        "(Both paths continue from §2 onward:) (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1 goal derived' in the transcript; derive at least "
        ...
        "other listed dimension; each dimension quotes a verbatim substring of the '[text]' string from §1, or names a response property whose absence would leave a reader who accepted "
        "all other dimensions still uncertain whether '[text]' was achieved; each dimension ends with '[observable: <string>]' where '<string>' is a literal string or structural "
