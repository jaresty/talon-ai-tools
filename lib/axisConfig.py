...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "verified'; every assistant turn after '§ implementation permitted [N]' has appeared in the transcript that does not contain a file-modifying tool call must contain '## "
        "Completion check' — a turn that does not contain a file-modifying tool call and does not contain '## Completion check' does not satisfy ground; '## Completion check' is "
        ...
        "this requirement and the governed turn must continue to the next derived step rather than ending; a turn whose final non-blank content line is '§4 coverage verified' is exempt "
        "from the resume phrase requirement; (5) enumerate escape paths: '## Path enumeration' must not appear before '## Enforcement sequence' in the transcript; enumerate every path "
        "by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each path, name the literal string whose "
        "presence closes it; write '§5 enumeration complete' when no further open path remains; any referent named in the '[text]' from §1 not already present as a literal string in "
        "the transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted [N]' where N is "
        "the 1-based index of the file-modifying tool call this sentinel governs must appear in the transcript after '§5 enumeration complete'; no file-modifying tool call may appear "
