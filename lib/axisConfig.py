...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "After the edit, produce a post-edit run result by tool call, then verify: (a) the governed failure is absent from the new result; (b) no new failures appear whose governing "
        "artifact produced no failure before the edit — any such failure requires revert. An edit that introduces a failure absent from the pre-edit result must be reverted regardless "
        "of interpretation.",
