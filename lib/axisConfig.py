...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "invariants, no anticipated cases. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ Validation artifact V "
        "complete; the artifact must be invocable by an automated tool; before writing the test, re-read the formal notation and assert each structural constraint it encodes; each test "
        "function contains exactly one assertion — one assertion per test function; a pre-existing test that happens to pass does not satisfy this rung — add to an existing test file "
        "covering the same module if one exists, otherwise create a new file; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not "
        ...
        "implementation failure — return to the prose rung and update it. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upon "
        "writing the observed running behavior label, re-emit the criterion for this cycle, then immediately invoke the implemented artifact via a tool call and emit 🔴 Execution "
        ...
        "the running system would see — not the test suite, not process state (server started, build succeeded, exit code 0) unless the criterion is specifically about process state; "
        "for a UI component, this means browser-visible text and values — not a test runner’s DOM query result; if the output does not directly demonstrate the criterion, emit the gap "
        "and return upward; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that demonstrates the declared behavior; "
        ...
        "written after the most recent 🟢 Implementation gate cleared in this thread and a 🔴 Execution observed: sentinel appears immediately after it and the OBS output directly "
        "demonstrates each criterion declared in this thread — a test pass is not a demonstration; once ✅ Thread N complete is emitted no further output in that cycle is valid. For "
        "each sentence in the prose that contains a behavioral predicate (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or any similar action "
        "verb), check that a completed gap cycle exists whose criterion directly names that behavior — implicit or incidental coverage does not satisfy this check; if any behavioral "
        "sentence has no directly-named cycle, emit a new 🔴 Gap: for it before emitting ✅ Thread N complete. Outputting a rung label is what begins that rung — it is not a heading or "
        "annotation; a rung whose label has not been output has not begun and no artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, "
        "update it to reflect the intended change, then descend; every descent through executable implementation requires executable validation and validation run observation to have "
        "fired for the current gap, including after an upward return. Reconciliation gate: at every upward rung transition, reconcile all artifacts at that rung against I before "
        "redescending; if prose has evolved, any pre-existing document that records I (ADR, README, spec) must be updated before redescending — this is a gate precondition, not a "
        "final-report step; documentation is a reconciliation artifact, not a thread. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside the rung "
        "where it is defined is a protocol violation; a sentinel used as a narrative summary or wrap-up is not a valid emission. After emitting ✅ Manifest exhausted, produce a final "
        "report containing prose, criteria, and formal notation for each thread in order; the final report copies artifacts already emitted in the thread rungs — no new behavioral "
