...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "block; a property enters the retained set only after canonicalization and observability validation. Every property that can enter the retained set — including each initial "
        "property [N], each property added during the completeness phase, and each property produced by a merge — must be observable under the chosen interpretation after "
        "canonicalization, unless the chosen interpretation explicitly requires an internal mechanism, architecture, algorithm, or representation as its subject; a property that "
        "constrains an internal subject without that explicit requirement is out of scope and must not appear in the retained set. Observability classification is applied to the "
        "canonical definition produced by Step 1, not to the provisional surface form. A property is atomic when the recursive decomposition procedure reaches Step 4 and no valid split "
        "exists. Step 1 — Canonicalize the property. Emit: '§ formality check: property [N]: \"<verbatim definition>\" — [formal | informal: <reason>]'. If informal, rewrite as a typed "
        "mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, and emit '§ rewritten: property [N]: \"<new definition>\"', then repeat the "
        "formality check on the rewritten definition. Continue until the formality check concludes formal. The definition confirmed formal in the final check is the canonical "
        "definition for this property — no subsequent step may quote a definition not yet confirmed formal. After canonicalization, verify that the canonical definition is observable "
        "under the chosen interpretation. If the canonical definition constrains an internal mechanism, architecture, algorithm, or representation without that subject being explicitly "
        "required by the chosen interpretation, reject the property as out of scope and do not retain it. Step 2 — Introduce provisional sub-properties. Introduce provisional candidate "
