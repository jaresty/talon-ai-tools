...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "each collapsed step is ambiguity that was not closed, and ambiguity that was not closed remains available as an escape route. §0 is a starting gate, not a bypass — "
        "description, analysis, and planning tasks run §1–§5 in full after writing '§0: [scenario]'; the declaration opens the ladder, it does not close it; every rung sentinel is "
        "satisfied by the literal string appearing in the response text when no executable artifact is present — no tool call is required or expected to satisfy any rung when §0 "
        "observed is absent. Ladder depth: a ladder is complete only when its final pre-implementation rung contains at least one expression of the form: a typed function signature, a "
        "logical predicate (∀/∃ notation), or a concrete input/output example (f(v) = w or equivalent); other domains use the equivalent terminal form — a notation where every term has "
        "an unambiguous denotation such that two independent agents given only that rung would produce functionally equivalent outputs. After writing each rung, apply the ambiguity "
        "test: quote one phrase from the rung and ask whether that phrase could be satisfied by two non-equivalent outputs. If such a phrase exists, that phrase names the subject of "
        "the next rung. The ladder is complete only when the ambiguity test finds no such phrase. A ladder that reaches implementation before passing the ambiguity test has collapsed a "
        "step and must be re-derived. Rung-type homogeneity: rung types must stay homogeneous across the ladder — all rungs before the behavioral observation must be successive "
        "precision-narrowings of the same governed behavior; introducing a new kind of artifact, deliverable, preflight check, or task category as a rung is not a narrowing and does "
        "not satisfy the ladder. A ladder that begins with intent → criteria → specification and then inserts a preflight check, implementation step, or presenter-support deliverable "
        "as a rung has switched dimensions — those items belong outside the ladder as governed actions following it, not inside it as rungs. Intent cannot be read directly from a "
        "request — every governing goal must be derived, because the request describes what was asked for, not what would satisfy the underlying need. The response applies a "
        "meta-process discipline before any implementation action, deriving and enforcing its own correctness conditions as transcript-inspectable strings. A rung is satisfied when and "
        "only when a transcript-visible event matching its definition appears — when §0 observed is present, rungs require tool-executed events; when §0 observed is absent, rungs are "
        "satisfied by the literal sentinel string appearing in the response text; inference, prediction, and prior knowledge do not satisfy rung gates regardless of accuracy; the "
        "absence of tool calls does not make rungs unsatisfiable — it makes them prose-satisfiable by the sentinel strings written in the response. All compliance headings must be "
        "markdown '## ' headings — a line not beginning with '## ' does not satisfy any heading requirement. Invocation scope: a sentinel written in a prior ground invocation does not "
        "satisfy any rung requirement for the current invocation — a sentinel satisfies its rung only when it appears in the transcript after the most recent '=== TOKENS' section. Each "
        "rung derives exclusively from the immediately preceding rung's output; no rung has a fixed step count. A rung that references content from any rung earlier than its immediate "
        "predecessor does not satisfy this requirement — isolation is enforced at each step; backward reference to earlier rungs is not permitted even when the content appears in the "
        "transcript. When the execution environment supports literal isolation — a fresh invocation, a separate agent context, or a new session — prefer actual isolation over "
        "in-transcript constraint enforcement. Derivation phase: (0) after 'Derived stance complete.', immediately write '§0: [scenario]' where [scenario] names the subject from the "
        "conversation — this declaration satisfies §0 and opens the ladder; if an artifact that executes the subject system and returns its live output appears verbatim in the "
        "conversation above, invoke it as a tool call immediately after writing '§0: [scenario]'; after that tool-result block appears and reflects live system execution, immediately "
        "write '§0 observed' — a tool-result block produced by invoking the named artifact as a tool call satisfies this requirement; a result produced by a Read, search, or file-write "
        "tool call does not; an artifact that returns document content does not qualify — a tool result consisting of a GitHub issue body, pull request body, specification document, "
        "README, or requirements text does not satisfy §0 regardless of invocation form; goal-source classification (required when '§0 observed' is present): examine whether any line "
        "in the §0 tool-result block appears verbatim as a substring of any user message visible above §0 in the transcript; if no such line exists, write '§0 status-only'; after '§0 "
        ...
        "derive formalized properties: after '§ formalization complete', immediately write the label line 'Formalized properties:' — this block must appear before any artifact, "
        "implementation action, or '§ implementation permitted' line; decompose each §2 dimension into its formal atomic claims; each claim must appear on its own line beginning with "
        "the literal text 'property [N]:' where N is a 1-based integer — a line beginning with '**property' or any markdown-formatted variant does not satisfy this requirement; each "
        "'property [N]:' line must contain at least one formal expression — a logical predicate (∀/∃ notation), a typed function signature, or a concrete input/output example (f(v)=w "
        "or equivalent) — as a literal substring; a 'property [N]:' line whose content is prose only, with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} present as a "
        "substring, does not satisfy this requirement; after writing each 'property [N]:' line, immediately write '§ split test: [quoted sub-expression]' where [quoted sub-expression] "
