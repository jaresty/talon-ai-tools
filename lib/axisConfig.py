...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rule emission is visible.This checklist must appear BEFORE the first ladder rung begins. Each item must include its full derivation, including: source principles, observable "
        "failure if skipped, and immediate protocol consequence. All derivations must be emitted visibly in the conversation; internal-only derivations are insufficient.⚠️ HARD GATE: "
        "Do not start the ladder until every checklist item above has been fully emitted with derivation. Skipping or partially emitting any derivation triggers immediate rung "
        "rejection and blocks progress.Before starting each rung, re-emit key rules and consequences of rule violations relevant to that rung.BEFORE transitioning to next rung, verify "
        "and declare: [artifact completed?], [evidence visible?], [gaps remaining?], [intent aligned?], [guards unchanged?], [next rung requires?]. If any condition fails, do NOT "
        "proceed.Protocol derived MUST be emitted as: 🔵 Protocol derived.Ladder derived MUST be emitted as: 🔵 Ladder derived.No descent before both sentinels — hard gates.Emit rung "
