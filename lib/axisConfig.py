...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "separate agent context, or a new session — prefer actual isolation over in-transcript constraint enforcement. Derivation phase: (0) after 'Derived stance complete.', "
        "immediately write '§0: [scenario]' where [scenario] names the subject from the conversation — this declaration satisfies §0 and opens the ladder; if an artifact that executes "
        "the subject system and returns its live output appears verbatim in the conversation above, invoke it as a tool call immediately after writing '§0: [scenario]'; after that "
        ...
        "of content already present in the transcript above it does not satisfy this requirement; (1) derive the governing goal: after '§0: [scenario]', write '## Governing goal: "
        "[text]' — if '§0 observed' is present, '[text]' must appear verbatim as a substring of the §0 tool-result block above; if '§0 observed' is absent, '[text]' is derived from the "
        "scenario description — what would satisfy the underlying need; a '[text]' value that adds a claim absent from the scenario does not satisfy §1; immediately below the governing "
        ...
        "write '§1a check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the labeled fields written above in this rung — a '§1a check:' line whose "
        "quoted phrase does not appear verbatim in the labeled fields above it does not satisfy this requirement; after a valid '§1a check:' line has appeared, immediately write '§1a "
        "decomposed'; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]' — when '§0 observed' is present, "
        "alternative means must not duplicate approaches already listed in §0; if only one concrete alternative can be named, write '[hypothetical]: [speculated alternative]' before "
        "closing §1; if two non-subsuming goal candidates exist, write both and mark one '[selected]' — the selected goal is the one whose means-test names a more concrete alternative; "
        "when ≥2 alternatives (concrete or hypothetical) are present, write '§1 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the alternative "
        "means lines written above in this rung — a '§1 check:' line whose quoted phrase does not appear verbatim in the means-test above it does not satisfy this requirement; after a "
        "valid '§1 check:' line has appeared, immediately write '§1 goal derived'; immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in "
        "the means-test, write one candidate beginning with '- Candidate:' — a response property that would be absent in a response achieving '[text]' by a different means; when one "
