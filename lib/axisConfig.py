...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "— the creation action itself satisfies the creation-step exception below. Derivation phase (required before any governed artifact-producing action — complete only when a "
        "literal block labeled 'Falsify derivation:' containing entries for (a) through (g) and (f2) appears in the transcript; prose does not satisfy this requirement): Name — (a) "
        "the assertion-failure marker: the literal string in an executed-artifact result signaling the artifact detected behavior absence; (b) the literal string signaling the "
        "governed behavior is present; (c) the literal string identifying each governed behavior in an executed-artifact result; (d) the governed symbol: the literal string naming the "
        "governed behavior, which must appear as a substring of the failure line in (g); (e) the layer classification — if the governed symbol name appears as a token in a call "
        ...
        "contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) in its output. Exception: (g) is not required for the single action that produces the "
