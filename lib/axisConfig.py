...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "'alternative satisfier:' line have appeared; (3) derive the enforcement sequence: after '§ properties complete', immediately write '## Enforcement sequence' — '## Enforcement "
        "sequence' must not appear before '§ properties complete' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation "
        "permitted' appears; each sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; when all sub-headings in '## "
        ...
        "then write '§ enforcement complete'; '§ enforcement complete' is valid only after a valid '§ enforcement check:' line has appeared; After '§ formalization complete', "
        "immediately write the label line 'Formalized properties:' — this block must appear before any artifact, implementation action, or '§ implementation permitted' line; decompose "
        "each §2 dimension into its formal atomic claims; each claim must appear on its own line beginning with the literal text 'property [N]:' where N is a 1-based integer — a line "
        "beginning with '**property' or any markdown-formatted variant does not satisfy this requirement; each 'property [N]:' line must contain at least one formal expression — a "
        "logical predicate (∀/∃ notation), a typed function signature, or a concrete input/output example (f(v)=w or equivalent) — as a literal substring; a 'property [N]:' line whose "
        "content is prose only, with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} present as a substring, does not satisfy this requirement; for each 'property [N]:' "
        "line, write one or more atomic sub-property lines of the form 'property [Na]:', 'property [Nb]:', etc., each containing a formal expression — a sub-property line whose content "
        "is prose only does not satisfy this requirement; the conjunction of all sub-property expressions for a given 'property [N]:' must be logically equivalent to the expression in "
        "'property [N]:' — logical equivalence means the set of inputs satisfying all sub-property expressions simultaneously equals the set of inputs satisfying the parent expression; "
        "a conjunction satisfied by a strict subset of the parent's cases (e.g. a single concrete instance substituted for a variable) does not satisfy this requirement; a conjunction "
        "satisfied by a strict superset of the parent's cases (e.g. omitting a condition from the parent) does not satisfy this requirement; if the property is already atomic (no "
        "strict sub-expression exists that is independently falsifiable), before writing 'property [Na]: atomic — ', write '§ atomicity test: \"[quoted sub-expression]\"' where "
        "[quoted sub-expression] is a verbatim phrase from the parent 'property [N]:' expression, followed by a sentence naming why that sub-expression is not independently falsifiable "
        "from the remainder — a 'property [Na]: atomic —' line that does not have a '§ atomicity test:' line with a verbatim quoted phrase above it does not satisfy this requirement; "
        "'property [Na]: atomic —' is valid only after '§ atomicity test:' has appeared; when all 'property [N]:' and sub-property lines are written, write '§ properties check: "
        "\"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the 'property [N]:' lines written above — a '§ properties check:' line whose quoted phrase does "
        "not appear verbatim in a 'property [N]:' line above it does not satisfy this requirement; after a valid '§ properties check:' line has appeared, immediately write '§ "
        "properties complete' — '§ properties complete' must not appear before a valid '§ properties check:' line; (4) verify coverage: after '§5 enumeration complete', immediately "
        "write '## Completion check' — '## Completion check' must not appear before '§5 enumeration complete' in the transcript; '## Completion check' must appear as the next heading "
        "after '§5 enumeration complete' in the transcript — the first '## ' heading line appearing after '§5 enumeration complete' must be '## Completion check'; a '## Completion "
        "check' appearing anywhere before '§5 enumeration complete' does not satisfy ground and must not be written — if the response has produced §1–§3 content and wishes to summarize "
        "or verify coverage, it must instead continue to '## Enforcement sequence' and '## Path enumeration' first; '§ properties complete' written before '## Completion check' "
        "satisfies the properties gate for the completion check — do not re-derive or repeat property lines inside '## Completion check'; inside '## Completion check', each covered "
        "dimension must cite a 'property [N]' number from the 'Formalized properties:' block and a verbatim string that appears either as a substring of a tool-result block produced "
        "after '§ implementation permitted' and visible above '## Completion check', or, when no such tool-result blocks are present, as a substring of the response text produced in "
        "this turn; no dimension citation or '§4 coverage verified' is valid until '§ properties complete' has appeared in the transcript; '§ test suite complete' is valid only after "
        "'§ properties complete' has appeared in the transcript — a '§ test suite complete' appearing before '§ properties complete' does not satisfy this requirement; '§4 coverage "
        "verified' is valid only after '§ test suite complete' has appeared and every covered dimension cites a property number and such a substring; '## Completion check' must not "
        "introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## Completion check' does not satisfy any coverage requirement and "
        "does not extend the governed dimension set; a turn containing a satisfied 'Gate condition:' block must contain the governed tool call in the same turn — prose appearing "
        "between the 'Gate condition:' block and the governed tool call does not satisfy this requirement; every assistant turn after '§ implementation permitted [N]' has appeared in "
        "the transcript that does not contain a line beginning with '§ implementation permitted [' must contain '## Completion check' — a turn that does not contain a line beginning "
        "with '§ implementation permitted [' and does not contain '## Completion check' does not satisfy ground; '## Completion check' is satisfied only when the literal string '§4 "
        "coverage verified' appears within it — a '## Completion check' block that does not contain '§4 coverage verified' does not satisfy this requirement and the governed turn must "
        "continue to the next derived step rather than ending — a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume phrase requirement; when a "
        'turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates still apply" to '
        "proceed under the same protocol. (5) enumerate escape paths: after '§ enforcement complete', immediately write '## Path enumeration' — '## Path enumeration' must not appear "
        "before '§ enforcement complete' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or "
        "dimensions covered; for each path, name the literal string whose presence closes it; when no further open path remains, write '§5 check: \"[quoted phrase]\"' where [quoted "
        "phrase] is a verbatim substring of one of the escape path closing strings named in the enumeration above — a '§5 check:' line whose quoted phrase does not appear verbatim in "
        "the path enumeration above it does not satisfy this requirement; then write '§5 enumeration complete'; '§5 enumeration complete' must not appear before a valid '§5 check:' "
        "line in the transcript; '§5 enumeration complete' must not appear before '§ enforcement complete' in the transcript; any referent named in the '[text]' from §1 not already "
        "present as a literal string in the transcript above §1 must be fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ "
        "implementation permitted' followed by a space and a decimal integer (e.g. '§ implementation permitted 1') must appear in the transcript after '§5 enumeration complete'; a "
        "sentinel line containing the literal bracket characters '[' or ']' does not satisfy this requirement — only a decimal integer satisfies the index requirement; the integer must "
        "equal the 1-based ordinal count of file-modifying tool calls that will have appeared through and including the tool call immediately following this sentinel — '§ "
        "implementation permitted 1' must precede the first file-modifying tool call, '§ implementation permitted 2' must precede the second, and so on; a checker verifies this by "
        "scanning the transcript for every file-modifying tool call, locating the final non-blank line of the immediately preceding assistant message block, and confirming it matches "
        "'§ implementation permitted N' where N is the call's ordinal position; no file-modifying tool call may appear before the first such sentinel; the permit sentinel must appear "
        "as the final non-blank line of the assistant message block immediately before the assistant message block containing the file-modifying tool call — a permit sentinel in the "
        "same message block as the tool call satisfies this requirement only when it is the final non-blank line before the tool call within that block; § implementation permitted [N] "
        "must appear immediately before the `(i)` line of each file-modifying tool call — no intervening blank lines, prose, tool-call blocks, or other content may appear between the "
        "sentinel and the `(i)` line; no user message appearing between § implementation permitted [N] and the governed tool call satisfies this requirement — a sentinel written in one "
        "turn and a tool call appearing in a later turn after a user message does not satisfy the same-turn anchor; a blank line, horizontal rule, or any other content between them "
        "does not satisfy this requirement; a file-modifying tool call not immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the "
        "five-line block does not satisfy this requirement. Quoted-span exclusion: only an occurrence of a named sentinel or heading string on a line that is not inside a markdown code "
        "fence (a span beginning with ``` or ~~~ on its own line) and does not begin with '>' (a markdown block-quote prefix) counts toward compliance — an occurrence inside a code "
        "fence or on a line beginning with '>' does not satisfy any rung or ordering requirement. Rung-completion sentinel finality: a turn is defined as all content within a single "
        "assistant message block; a line is non-blank if it contains at least one non-whitespace character; each of the strings '§0 observed', '§1 goal derived', '§1a decomposed', '§1b "
        "candidates', '§2 dimensions closed', '§ formalization complete', '§ properties complete', '§ test suite complete', '§4 coverage verified', and '§5 enumeration complete' must "
        "appear as the final non-blank line of the assistant message block in which it is written — the sentinel line must begin with the literal first character of the sentinel string "
        "('§' or '#'); a sentinel line beginning with '**' or any other markdown formatting character does not satisfy this requirement; a checker verifies this by locating the "
        "sentinel line, scanning all subsequent lines in the same assistant message block, and confirming every such line is blank (contains only whitespace or is empty); any non-blank "
        "line after the sentinel in the same assistant message block is a violation; no exceptions are permitted — if additional content must appear, it must precede the sentinel, not "
        "follow it. Completion check ordering: the '## Completion check' block must appear after '§5 enumeration complete' in the transcript; a '## Completion check' block that appears "
        "before '§5 enumeration complete' does not satisfy this requirement even if its contents are otherwise correct; a response that contains '§5 enumeration complete' and does not "
        "produce a subsequent '## Completion check' block containing 'Formalized properties:', at least one 'property [N]:' line satisfying all property requirements, '§ properties "
        "complete', and '§4 coverage verified' does not satisfy ground. Continuation invariant: when the final non-blank content line of a turn contains or follows one of the "
        "rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage verified', or '§5 enumeration complete', that turn must end with one of: '§ "
