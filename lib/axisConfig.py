...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "in that order — read-only only, no writes. Rung table — name | artifact type | gate condition | void condition: prose: artifact=“natural language description of intent and "
        "constraints”, gate=“I declared”, voids_if=“skipped”; criteria: artifact=“falsifiable behavioral acceptance conditions”, gate=“prose complete”, voids_if=“criterion is "
        "structural/implementation rather than behavioral”; formal notation: artifact=“non-executable specification with behavioral invariants + R2 audit”, gate=“criteria complete; R2 "
        "audit 0 UNENCODED entries”, voids_if=“UNENCODED entries remain; audit not a separate named section”; executable validation: artifact=“test file invocable by an automated tool, "
        "within project tree”, gate=“formal notation R2 audit complete”, voids_if=“implementation code included; outside project tree; pre-existing artifact”; validation run "
        "observation: artifact=“exec_observed sentinel with verbatim failure naming declared gap”, gate=“executable validation artifact runs”, voids_if=“build/compile error; green run "
        "without prior failure; infrastructure-only output”; executable implementation: artifact=“implementation source edits; one edit per re-run cycle”, gate=“exec_observed + gap "
        "declared; impl_gate sentinel emitted”, voids_if=“multiple edits without intervening re-run; newly-passing test has no prior failure”; observed running behavior: "
        ...
        "only infrastructure state; file read used as evidence; test runner output — a test-suite pass is validation-run-observation-type output, not observed-running-behavior-type "
        "output, and voids this rung”. 🔴 Execution observed: requires: (1) a preceding tool call in this response, (2) verbatim tool output in a triple-backtick block — no editorial "
        ...
        "the N declared in ✅ Manifest declared — emitting it before all declared threads are complete is a protocol violation; Each completion sentinel is valid only at its defining "
        "rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The gap names a specific behavior currently absent or wrong, phrased as a "
        "currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest "
        "exhausted — one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the prose, it belongs in formal notation "
        "instead; if the criterion contains the word ‘and’ it is a conjunction — split before continuing (‘the table renders Display Name’ and ‘the search bar filters results’ are two "
        "criteria, not one); a criterion that names both a structural element and its data source is a conjunction — split before continuing; asserting a column header is present does "
        "not satisfy this rung — add a criterion asserting its content or effect; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap "
        "text for this thread verbatim — a criterion written without a citable manifest excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the "
        "criterion to match an easier implementation is a protocol violation; each criterion states (a) one observable behavior and (b) a falsifying condition referencing no "
        "implementation internals (no file names, function names, or variable names); this is a content gate, not a self-check; exactly one criterion may be written per thread per "
        "cycle — after the first criterion is written, the only valid next token is the formal notation rung label; a second criterion for the same thread is a protocol violation — "
        "stop, return to the prose rung, and split the thread before descending again; the criteria rung label is per-thread — writing criteria for Thread 2 or any subsequent thread "
        "before ✅ Thread 1 complete is emitted is a protocol violation regardless of whether they share a criteria rung label; batch-collecting criteria for multiple threads under one "
        "criteria label bypasses sequential descent and is a protocol violation. The formal notation rung separates behavioral specification from explanation. Formal notation (type "
        "signatures, interfaces, invariants, pre/postconditions) encodes what must be true — the executable or testable part. Natural language labels, introduces, and explains the "
        "specification — prose adds context but the behavioral constraints must still be encoded in notation.it must encode all structural constraints the criterion implies — the "
        "permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe what notation looks like, not what it may omit; selecting a subset of permitted forms "
        "that leaves constraints unencodable is insufficient; it produces only type signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is "
        "that it cannot be directly compiled or executed without modification; any construct that compiles or runs as-is is implementation code and voids the rung; formal notation "
        "describes what must be true, not how to implement it — permitted: type signatures without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: "
        "complete function bodies, variable assignments, constant declarations (const, let, var assignments) regardless of what they assign — the presence of an assignment operator "
        "outside a type definition indicates implementation code, working logic sequences; labeling prohibited content as pseudocode does not exempt it from this prohibition; if the "
        "criterion is already a complete behavioral specification — naming observable behavior and a falsifying condition without referencing function names, field names, or "
        "implementation internals — the formal notation artifact may restate it verbatim with an explicit declaration that no elaboration is needed; this is not a skipped rung; the "
        "artifact must appear in the transcript and the audit section still applies; the audit section is separate and named; the rung is incomplete until all criteria are encoded. "
        "Natural language may appear as section labels or introductions (e.g., ‘This invariant captures the rendering constraint:’) but may not substitute for encoding a constraint in "
        ...
        "multiple assertions are permitted only when they jointly constitute a single indivisible check (e.g., asserting a value is both non-null and of the expected type); before "
        "writing the test, re-read the formal notation and assert each structural constraint it encodes; confirm via tool call that the criteria artifact for this cycle was produced in "
        "this cycle's transcript; if the criteria artifact is identical to a prior cycle’s criteria artifact, the criterion has not changed — return to the prose rung and re-emit with "
        "an updated gap before descending again; before writing the test, check for an existing test file covering the same module — add there if so; when the artifact is modified, "
        "invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in "
        "a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]’ — each prior-failure name must be quotable verbatim from a 🔴 Execution "
        "observed sentinel; modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until carry-forward has been emitted. "
        "when the declared intent is to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the validation artifact must be a "
        "meta-test — an executable artifact that fails when the test-under-modification does not exhibit the criterion; before emitting ✅ Validation artifact V complete, the artifact "
        "content must appear in the current response — emitting the sentinel without quoting the artifact in the transcript is a protocol violation; the validation artifact must reside "
        "within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; "
        "if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten. "
        "Before writing the validation run observation rung label, invoke the validation artifact via a test runner; a harness error (import failure, syntax error, missing file) is not "
        "a red run — the test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness; a green "
        "🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation "
        "gate cleared showing the test logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: "
        "naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after a red VRO that is not a harness error, the criterion re-statement is an anchor "
        "only — re-running VRO without an implementation edit is a protocol violation; re-writing formal notation or the EV artifact without an intervening implementation edit is also "
        "a protocol violation; after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token — this shortcut applies only when a red VRO already exists in the "
        ...
        "as a harness-error exec_observed at the EV rung. Before emitting 🛑 HARD STOP, locate the criterion written in the most recent prior HARD STOP cycle for this thread — if it is "
        "textually identical to the current criterion, HARD STOP is a protocol violation: an identical criterion means the upward return accomplished nothing; if the implementation did "
        "not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation before issuing HARD STOP. if the observed gap "
        "matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ "
        "Manifest declared are not re-emitted; the next token is the criteria rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP "
        "returns. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists. If no invocable process exists, open a gap "
        "cycle to make the artifact directly invocable. if the 🔴 Execution observed: does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply "
        "the upward-return failure-class rules. A full test suite run result is required after the most recent OBR tool call — the only valid next action if no such result exists is "
        "the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason. Once ✅ Thread N complete is emitted no further "
