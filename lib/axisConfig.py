...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "implementation files so tests can RUN (imports resolve) — NO implementation whatsoever at validation rung, just wiring (empty functions returning null); tests may RENDER "
        "(setup, teardown, DOM creation allowed) but must have NO actual expect() assertions — writing actual test assertions (expect() calls) at validation rung is a protocol "
        "violation; tests should be EMPTY skeletons that pass because they have no assertions. Assertions are written at the assertion rung, not validation. Tests MUST PASS at "
