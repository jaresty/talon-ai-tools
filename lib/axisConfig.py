...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "invariant (claim vs reality) — what GOES WRONG: false completion; RULE: evidence before claims - Optimization pressure — what GOES WRONG: steps skipped, unverifiable results; "
        "RULE: make compliance easier than deviation - Ladder structure — what GOES WRONG: gaps disappear, completion becomes false; RULE: one rung at a time - Evidence at proximity — "
        "evidence must appear IMMEDIATELY after result; WHAT GOES WRONG: gaps hidden, traceability lost; RULE: cite source immediately after - Rule citation at action — cite governing "
        "rule before each action; WHAT GOES WRONG: actions without justification; RULE: state rule before acting - Challenge mechanism — instantiate mechanism that detects gaps, APPLY "
        "it, EXECUTE IT, SHOW ACTUAL FAILURE OUTPUT; the challenge is NOT analysis or conjecture — it MUST create an EXECUTABLE ARTIFACT (test file, script, runnable check) that when "
        ...
        "BEHAVIORAL GAPS, VERIFICATION MUST EXECUTE — assertions without running code are insufficient ⚠️ HARD GATE: Protocol derivation is MANDATORY FIRST STEP — you MUST derive all "
        "protocol rules with their consequences and enforcement mechanisms BEFORE emitting 🔵 Protocol derived. For EACH rule, you MUST explicitly state: (1) THE RULE — what must be "
