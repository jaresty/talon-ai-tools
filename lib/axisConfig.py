...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "prior cycle's gap, the edit did not address it — return to formal notation before any further edit. One edit per re-run cycle; after each edit re-run the validation artifact "
        "before any further edit. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no "
        "artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend. Reconciliation "
        ...
        "artifacts at that rung against I before redescending; documentation is a reconciliation artifact, not a thread. Seven rungs in order: prose → criteria → formal notation → "
