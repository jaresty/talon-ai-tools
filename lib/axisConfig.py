...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "executable validation rung — a harness error at EV requires fixing the harness, not an upward return; the only valid next action when EV shows a harness error is a tool call "
        "that fixes the harness; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the "
        "edit did not address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not "
        "re-emitted; the next token is the criteria rung label for the current thread. the VRO rung label must appear in the transcript for the current cycle. 🟢 Implementation gate "
        "cleared is the first token of the executable implementation rung — no tool call, no prose, and no file modification may appear before it in the current response; the next "
        "required action after 🟢 is a tool call that creates or modifies an implementation file; the implementation gate does not complete the thread; ✅ Thread N complete may not "
        "appear until the observed running behavior rung has fired for this thread; the OBR rung label must appear in the transcript for the current thread's current cycle before ✅ "
        "Thread N complete may be emitted — output produced before the OBR rung label may not be reused to satisfy the OBR exec_observed gate; its absence is a protocol violation, not "
        "a voiding condition. declaring a thread complete by similarity to another thread's failure is not valid — each thread requires its own full descent through OBR with a passing "
        "run; similarity does not substitute for a tool-executed result in the current cycle. none of the following satisfies the OBR gate: passing tests, functionally complete "
        "implementation, prior work stopped at EI, or precedent — the gate requires a manual live-process invocation in the current cycle. In-process rendering (renderToStaticMarkup, "
        "test-framework render) is acceptable only when no runnable artifact exists. If no invocable process exists, open a gap cycle to make the artifact directly invocable. if the 🔴 "
        "Execution observed: does not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules; the exec_observed "
        "output must show the specific behavior asserted in the criterion — server liveness (a 200 OK, a page title, a process start message) does not satisfy this gate unless the "
        "criterion asserts server liveness; the query must exercise the criterion’s behavior directly, not merely confirm the server is running. ✅ Thread N complete is blocked until a "
        "live-process tool call result appears in the current thread’s current cycle — a test runner invocation, even in browser mode, does not satisfy this gate; the test suite run at "
        "step 5 presupposes the live-process invocation at step 3 has already produced an exec_observed result; if no live-process tool call result exists, the only valid next action "
        "is to start the process and query it. A full test suite run result is required after the most recent OBR tool call — the only valid next action if no such result exists is the "
        "tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason; acknowledging failures does not permit ✅ Thread N "
        "complete — the step-5 run must show zero failures for the criterion declared in this thread before ✅ Thread N complete may be emitted; if the most recent 🔴 Execution observed: "
        "in the current cycle shows any test failure, ✅ Thread N complete is blocked — no intervening prose, reasoning, or cross-thread argument lifts this block; only a subsequent "
        "passing test run does; this block does not apply when the most recent exec_observed is a harness error (import failure, syntax error, missing file) — repair the harness and "
        "re-run; the block re-evaluates against the subsequent run. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal "
        "notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared gap — what the prior rung’s artifact leaves "
        "unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. Outputting a rung label is what begins that rung — it "
        "is not a heading or annotation. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without consulting I; before "
        "closing a rung artifact, state every open constraint the next rung must resolve. Advance through every feasible rung without pausing for user confirmation — each rung may not "
        "be skipped or combined with another, including the observed running behavior rung; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 "
        "Gap:; all other rung transitions are continuous within the same response. Response length is never a valid reason to stop between rungs — if the full descent cannot fit in one "
        "response, continue from the current rung in the next response without re-emitting completed rungs. The prose rung must be re-emitted at the start of every new cycle — the "
        "first cycle and any cycle that begins after ✅ Thread N complete closes the prior one; a HARD STOP upward return is not a new cycle and does not require prose re-emission — the "
        "next token after HARD STOP is the criteria rung label for the current thread; the criterion must be immediately derivable from the prose that opened the current cycle. When "
        "beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through executable implementation requires executable validation and "
        "validation run observation to have fired for the current gap, including after an upward return. Upward returns follow the failure class: if the implementation did not close "
        "the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; observation cannot be produced is a prose-description "
        "failure, not an implementation failure — return to the prose rung and update it. After every upward return, a new ✅ Validation artifact V complete must be emitted for the "
        "current cycle’s gap — an artifact written in a prior cycle does not satisfy this gate. Before producing the final report, invoke the delivered system end-to-end via a tool "
        "call and emit 🔴 Execution observed: showing the primary behavioral intent is satisfied — this is the post-delivery integration observation; if it fails, open a new gap cycle "
        "before producing the final report. Any prose that summarizes implemented behavior, lists acceptance criteria met, describes files created or modified, or names gaps as "
        "resolved is a final report regardless of its heading; a final report may not appear unless ✅ Manifest exhausted immediately precedes it in the transcript. if no "
        "currently-false behavior remains, emit ✅ Manifest exhausted. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for "
        "each thread in order — no new behavioral claims, no coverage summaries, no suggestions; then reconcile any documents the implementation affects. ",
