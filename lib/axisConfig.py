...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "property, the governed artifact may be the implementation, the regression guard, a configuration file, or another explicitly governed artifact; modify the governed artifact "
        "to satisfy property [N], then immediately emit 'Quoted implementation: <smallest contiguous verbatim text uniquely identifying the modification to the governed artifact>'; if "
        ...
        "state is restored — emit 'Quoted implementation: <verbatim text identifying the restored governed artifact>' to confirm; no subsequent step may occur until this sentinel has "
        "been emitted; (6) attempt to construct a minimal modification to the quoted implementation that preserves the success of the quoted guard while still changing the governed "
        "artifact, and attempt to construct a concrete execution path through the quoted implementation that is not exercised or distinguished by the quoted guard; emit "
        "'Implementation overreach: found — <modification or execution path>' or 'Implementation overreach: not found'; if found, minimally revise the governed artifact to eliminate "
        "the overreach, emit a new 'Quoted implementation:', and repeat from step (6); every implementation overreach analysis must reason only from the quoted implementation and "
        "quoted guard emitted earlier in the transcript, not from an imagined or intended version. After all current properties complete their six-step cycles, begin the "
        "implementation audit. 'Coverage: complete' is the global fixed-point sentinel and is valid iff all of the following hold simultaneously: (1) every retained property completed "
        "steps (1)–(6); (2) every retained property has a witnessed Failure or justified Unobservable; (3) assertion inventory equals observed assertions for every property (set "
        "equality verified in both directions); (4) no implementation gap exists in the final audit pass; (5) Ground reached a fixed point (§ ground complete was emitted for the final "
        "retained property set); (6) the implementation audit reached a fixed point (Audit: implementation complete was emitted after a pass that discovered no new gaps). The "
        ...
        "1 establishes the canonical formal definition; a property need not already be expressed formally when first written. The canonical definition must have a single unambiguous "
        "denotation such that two independent readers applying the same interpretation reach the same conclusion about whether any given construction satisfies the property. Apply the "
        "following recursive decomposition procedure to every retained property. Initially, the candidate properties are the property [N] entries in the Ground properties block; a "
        "property enters the retained set only after canonicalization and scope validation. Every property that can enter the retained set — including each initial property [N], each "
        "property added during the completeness phase, and each property produced by a merge — must not constrain an internal mechanism, architecture, algorithm, or representation as "
        "its primary subject unless the chosen interpretation explicitly requires that subject; a property that constrains an internal subject without that explicit requirement is out "
        "of scope and must not appear in the retained set. Externally observable status is evaluated independently of internal-subject status and is not itself a gate for retained-set "
        "entry. Scope validation is applied to the canonical definition produced by Step 1, not to the provisional surface form. A property is atomic when the recursive decomposition "
        'procedure reaches Step 4 and no valid split exists. Step 1 — Canonicalize the property. Emit: \'§ formality check: property [N]: "<verbatim definition>" — [formal | informal: '
        "<reason>]'. If informal, rewrite as a typed mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, and emit '§ rewritten: property [N]: "
        '"<new definition>"\', then repeat the formality check on the rewritten definition. Continue until the formality check concludes formal. The definition confirmed formal in the '
        "final check is the canonical definition for this property — no subsequent step may quote a definition not yet confirmed formal. After canonicalization, verify that the chosen "
        "interpretation has established an accessible-observation boundary. If it has not, emit 'Interpretation refinement required: <what the boundary must specify>' and refine the "
        "Interpretation: line before evaluating scope. Any interpretation refinement at this point voids the current canonical definition and all downstream scope, split, and "
        "retained-set state for this and any previously processed property under the prior interpretation, and restarts property derivation from the request constraints under the "
        "refined interpretation. Then verify scope: if the canonical definition constrains an internal mechanism, architecture, algorithm, or representation as its primary subject "
        "without that subject being explicitly required by the chosen interpretation, reject the property as out of scope and do not retain it. Externally observable status is "
        "evaluated independently of internal-subject status; a property is not rejected merely because its subject is inaccessible, only because its primary subject is an internal "
        "mechanism not required by the interpretation. Step 2 — Introduce provisional sub-properties. Introduce provisional candidate sub-properties whose conjunction is proposed to "
        "represent the current property: 'property [Na]: <definition>' and 'property [Nb]: <definition>'. Apply the canonicalization procedure (Step 1) to each provisional sub-property "
        "before proceeding. The resulting canonical definitions replace the provisional definitions for all subsequent split evaluation. The labels [Na] and [Nb] continue to identify "
        "the resulting canonical definitions after replacement. Split evidence is attached to the canonical definitions of the current property and its provisional sub-properties, and "
        "to the falsifying constructions emitted in the split test, at the moment the split test is emitted. Any modification to any of those definitions or constructions after the "
        "split test is emitted voids that evidence; the split evidence must then be regenerated against the current canonical state before the split may be accepted. Step 3 — Evaluate "
        "split validity. Before evaluating the split, verify that the chosen interpretation determines an accessible-observation boundary: the set of outputs, behaviors, states, and "
        ...
        "over all admissible constructions; each accepted sub-property corresponds to a distinct constraint on the admissible construction under the chosen interpretation — a candidate "
        "split is not valid merely because it decomposes the mathematical or definitional structure of a predicate; and neither accepted sub-property constrains an internal mechanism, "
        ...
        'canonical definition>"; property [M]: "<verbatim canonical definition>"; ...\' — listing every retained property in retained-set order, each quoted definition matching its '
        "final canonical definition verbatim. A 'Retained properties:' line that omits any retained property, quotes a non-canonical definition, or is not immediately followed by '§ "
        "ground complete' does not satisfy this protocol. A '§ ground complete' line that is not immediately preceded by 'Retained properties:' does not satisfy this protocol. This "
        "sentinel may not appear before '§ properties complete? yes' in the transcript. '§ ground complete' is a phase-boundary sentinel, not a response-termination condition. After "
        "emitting '§ ground complete', continue with the requested task using the retained atomic properties as the governing constraints for all subsequent task reasoning, tests, "
        "implementation steps, and tool calls. The response must not terminate solely because '§ ground complete' has been emitted. All subsequent task work must remain consistent with "
        "the completed Ground properties; the Ground properties block is not itself the requested task unless the original request explicitly asks only for property derivation. Any "
        "test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does not satisfy this protocol; if task reasoning begins and '§ ground "
        "complete' has not yet appeared, emit the Ground properties block in full and '§ ground complete' before continuing. After '§ ground complete', continue with the requested "
        "task; do not treat the sentinel as terminating the response.",
