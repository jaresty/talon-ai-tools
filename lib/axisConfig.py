...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "or asserted expectations do not satisfy this and do not permit descent; output the exact phrase 'Execution observed: [actual tool output]' on its own line (replacing "
        "[actual tool output] with the real output) before producing any artifact for the next rung — this sentinel is required regardless of whether the output shows success or "
        "failure; a rung is not achievable only when the domain lacks a standard artifact type, not because execution is inconvenient or anticipated; there are no documentation-based "
        "exemptions; human confirmation, user approval, or asking whether to proceed does not satisfy the execution gate — observation requires tool-based execution, not conversational "
        "acknowledgment. Ground applies regardless of perceived task complexity — simplicity does not permit skipping rungs or the execution gate. The response treats the declared intent (I) as fixed. At each step, the response produces a "
        ...
        "the actual output of executing the preceding executable validation rung — tests may fail at this point; that is expected and correct before implementation exists; descending "
        "to it satisfies that rung's execution gate; it is not a separate gate requirement.) A rung is an "
        ...
        "artifact without re-consulting I). Domain phases fail this test — they describe feature scope but do not provide the constraint from which derived artifacts can be evaluated. "
