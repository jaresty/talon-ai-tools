...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "valid; “need to implement a landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted. Each rung’s artifact addresses only that gap — not all "
        "known requirements of the task — and is minimal: one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the "
        "prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is invalid — split it before continuing (‘the table renders Display Name’ and ‘the "
        "search bar filters results’ are two criteria, not one); a criterion that asserts only that a structural element exists is not behavioral — asserting a column header is present "
        "does not satisfy this rung; add a criterion asserting the element’s content or effect (the row displays the value from the data source). Formal notation encodes only the "
        "criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only validation artifacts may be produced at the executable validation rung — no other "
        "content is permitted before ✅ Validation artifact V complete; the artifact must be invocable by an automated tool; before writing the test, re-read the formal notation and "
        "assert each structural constraint it encodes; each test function contains exactly one assertion — one assertion per test function; a pre-existing test that happens to pass "
        "does not satisfy this rung — add to an existing test file covering the same module if one exists, otherwise create a new file; before emitting ✅ Validation artifact V "
        ...
        "the test is vacuous and must be rewritten. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is "
        "written. Before writing the validation run observation rung label, invoke the validation artifact via a tool call — predicting what the run would show does not satisfy this "
        "gate regardless of accuracy; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after "
        "the most recent 🟢 Implementation gate cleared for this thread showing the test logic failing — a harness error (import failure, syntax error, missing file) is not a red run; "
        "the test must have been loadable, its assertions must have run, and they must have failed. At the validation run observation rung, run the validation artifact; emit 🔴 "
        ...
        "observation rung, the criteria rung label is the only valid next token in the response — any other content before the criteria label is a protocol violation; if the observed "
        "gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Upward returns follow the failure class: if the "
        ...
        "artifact before any further edit. Upon writing the observed running behavior label, re-emit the criterion for this cycle, then immediately invoke the implemented artifact via "
        ...
        "what a non-technical observer of the running system would see — not the test suite, not process state (server started, build succeeded, exit code 0) unless the criterion is "
        "specifically about process state; for a UI component, this means browser-visible text and values — not a test runner’s DOM query result; if the output does not directly "
        "demonstrate the criterion, emit the gap and return upward; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed artifact that "
        "demonstrates the declared behavior; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be emitted unless the "
        ...
        "after it and the OBS output directly demonstrates each criterion declared in this thread — a test pass is not a demonstration; once ✅ Thread N complete is emitted no further "
        "output in that cycle is valid. For each sentence in the prose that contains a behavioral predicate (fetches, requires, displays, filters, renders, validates, authenticates, "
        ...
        "and validation run observation to have fired for the current gap, including after an upward return. Reconciliation gate: at every upward rung transition, reconcile all "
        "artifacts at that rung against I before redescending; if prose has evolved, any pre-existing document that records I (ADR, README, spec) must be updated before redescending — "
        "this is a gate precondition, not a final-report step; documentation is a reconciliation artifact, not a thread. Each completion sentinel is valid only at its defining rung — "
