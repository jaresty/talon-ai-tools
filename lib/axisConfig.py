...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "failure signal text appears verbatim; a Smaller line is valid only when the most recent tool-executed result appearing above it in the transcript was produced by applying the "
        "smaller change and running the test suite, and that result contains the governing failure signal text verbatim — a tool-executed result produced before the smaller change was "
        "applied does not satisfy this gate regardless of its content; no other form of argument — including prose reasoning about other tests or failure modes — constitutes valid "
        "insufficiency evidence; second, in a separate entry below, quote the specific text from the governing failure signal that shows the governed behavior absent — the quote must "
        "appear in a tool-executed run result above the first entry; a candidate that is larger than the smallest change that resolves the quoted failure text is not permitted "
        "regardless of what else it also resolves; a failure not yet observed in a prior tool-executed run result has not been shown to govern, and a change that fixes it cannot be "
        "derived from the current governing artifact; (3) line manifest listing every line to be added; for each line, quote the specific assertion string from the governing failure "
        "signal that would be absent from the run result when the line is present and would appear in the run result when the line is absent — an entry whose quoted string would remain "
        "in the run result when the line is present does not satisfy this requirement. Immediately before each tool call that modifies a file, produce: 'Scope: <verbatim quoted failure "
        "signal> | Candidate: <named candidate change> | Smaller: <named smaller change whose insufficiency is demonstrated above> | Lines: <count of lines in manifest>' — if this line "
        "does not appear immediately above the tool call that modifies a file in the transcript, the edit is not permitted; this line is not permitted unless a tool-executed result "
        "showing the smaller change applied appears above it in the transcript — a Scope line that appears without such a result above it does not satisfy this requirement. After the "
        "edit, produce a post-edit run result by tool call, then verify: (a) the assertion text from the scope declaration is absent from the new run result; (b) no assertion text "
