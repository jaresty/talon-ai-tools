...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "conversation: [ ] How to handle files: what creates at validation, what edits at assertion/implementation, how to JUSTIFY each file change [ ] How to handle the ladder: what "
        "each rung does and when to move to next [ ] How to verify gap closure: when to check and when to emit completion [ ] Gap cycle: derive from P1 (intent primacy), P3 (evidence "
        "required), P5 (derivation chain) — why must each gap have its own assertion→implementation cycle; what goes wrong if you batch multiple gaps in one implementation; how the "
        "sentinels enforce one gap at a time This checklist must appear BEFORE the first ladder rung begins. The derivation results must be in the chat, not just in your head. derive: "
        ...
        "difficulty, failure, or constraint pressure at a lower rung is not a valid upward return trigger; GAP CYCLE RULE: for EACH gap, you MUST follow this sequence: (1) assertion "
        "rung: add failing assertion for the specific gap; (2) implementation rung: implement ONLY enough to make that specific assertion green; (3) verify assertion is green; (4) emit "
        "✅ Implementation complete; (5) if OTHER gaps remain, return to assertion rung (NOT implementation) to add next failing assertion; (6) repeat until no gaps remain; NEVER "
        "implement multiple gaps in sequence without returning to assertion between them; batching implementations violates protocol; the sentinel 'Implementation complete' means the "
        "current gap is done, not that the entire task is done), upward gap scan (ALWAYS after completing the final rung (implementation), scan upward to verify all criteria are "
        "addressed; this is NOT optional — even if you believe no gaps remain, you MUST produce the checklist to prove verification; scan upward to find the nearest rung above that "
        "still has a gap to close; return to that rung rather than restarting from the top; when returning upward, FIRST emit completion sentinel for current rung, THEN emit the rung "
        "label for the rung you are returning to — you cannot be at two rungs at once; continue descending from that rung to close the remaining gap; after all gaps are closed, return "
        "to the observation loop above the ladder to verify closure and emit Ground complete; UPWARD SCAN MUST examine ALL prior rungs (prose, criteria, formal notation, validation) "
        "for unaddressed criteria — not just the immediately prior rung; each criterion from earlier rungs must have corresponding assertions and implementation; if criteria from "
        "prose/criteria/formal notation rungs are not satisfied by assertions+implementation, the gap remains open and must be closed before emitting Ground complete; UPWARD SCAN MUST "
        "produce an explicit checklist with each criterion marked: ✅ addressed or ❌ gap remains — missing this checklist is a protocol violation), observation loop corollary (if "
        "opening observation shows no gap exists, emit Ground complete immediately — no ladder descent needed), upward return conditions (what triggers return, what does NOT trigger "
        "return and why excluded), evidential scoping (evidence is scoped to current cycle and gap, not prior cycles), scope preservation (scope does not expand between rungs), "
        "derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); evidence over theory (a model's description of what it would do is "
        "indistinguishable from having done it — only visible execution output counts as evidence; every behavioral change and execution must be observable in the log, not just "
        ...
        "fix issues at validation rung BEFORE moving to assertion rung. At validation rung, you MUST explicitly list all files being created in the conversation — this is your file "
        "manifest. Every implementation stub created at validation rung MUST have a corresponding test file created at validation rung — the test file validates that implementation; "
        "list both implementation files AND their test files in the validation file manifest. Assertion rung: EDIT existing script to add ONE specific check (expect() assertion or "
        "shell script check); run to see it FAIL (red). NO implementation in this rung. NO file creation — edit existing test files only; before editing, declare 'no new files' as "
        "verification that no new files are being created; AFTER check turns green (implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ Assertion "
        "complete (staying in assertion rung), THEN add another assertion for the same gap; the completion sentinel MUST explicitly declare gap status: ✅ Assertion complete — gaps "
        "remaining: [none/which criteria still unsatisfied] — evidence: [for each unsatisfied criterion, cite the RAW failing assertion text]; If gap IS closed: emit ✅ Thread N "
        "complete — all gaps closed; move to next gap; NO file creation in assertion rung — edit existing test files if they exist; stubs created at validation rung; only edit existing "
        "scripts to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one rung at a time; completion before "
        "continuation. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO "
        "assertions added here; NO file creation; only edit existing stubs; before editing, declare 'no new files' as verification that no new files are being created; implement just "
        "enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot "
        "explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if gap is closed — If gap still exists: FIRST emit ✅ "
        "Implementation complete (return UP to assertion rung to add next assertion); if gaps remain after implementation, the completion sentinel MUST explicitly declare: ✅ "
        "Implementation complete — gaps remaining: [list which prose/criteria/formal notation criteria are NOT yet satisfied] — evidence of gap verification: [reference the specific "
        "criteria checked, or if gaps listed, explain why return upward is needed]; if gap IS closed: emit ✅ Thread N complete — all gaps closed. If gap IS closed: emit ✅ Thread N "
