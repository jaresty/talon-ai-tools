...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "and at least one smaller change; the smaller change must be the minimal stub — a version of the candidate with the correct type signature that returns a value of the correct "
        "type without performing the governed transformation; a change that produces a type error, a crash, or a failure mode different from the governing failure signal at the call "
        "site is not a valid smaller change; then run the smaller change and produce a tool-executed result — if the governing failure signal text is absent from that result, the "
        "smaller change is sufficient and must become the candidate; if the governing failure signal text is present, the smaller change is insufficient and the candidate is permitted; "
        "an insufficiency claim is not permitted until a tool-executed result showing the smaller change appears above it in the transcript; an insufficiency argument that names or "
        ...
        "signal text alone; second, in a separate entry below, quote the specific text from the governing failure signal that shows the governed behavior absent — the quote must appear "
        "in a tool-executed run result above the first entry; a candidate that is larger than the smallest change that resolves the quoted failure text is not permitted regardless of "
        "what else it also resolves; a failure not yet observed in a prior tool-executed run result has not been shown to govern, and a change that fixes it cannot be derived from the "
        "current governing artifact; (3) line manifest listing every line to be added, each entry quoting the specific text in the governing failure signal that names the behavior the "
        "line implements. Immediately before each tool call that modifies a file, produce: 'Scope: <verbatim quoted failure signal> | Candidate: <named candidate change> | Smaller: "
        "<named smaller change whose insufficiency is demonstrated above> | Lines: <count of lines in manifest>' — if this line does not appear immediately above the tool call that "
        "modifies a file in the transcript, the edit is not permitted; this line is not permitted unless a tool-executed result showing the smaller change applied appears above it in "
        "the transcript — a Scope line that appears without such a result above it does not satisfy this requirement. After the edit, produce a post-edit run result by tool call, then "
        "verify: (a) the assertion text from the scope declaration is absent from the new run result; (b) no assertion text appears in the new run result that was absent from all "
        "tool-executed run results appearing above this edit in the transcript — any such new assertion text requires revert. An edit that introduces a failure absent from the pre-edit "
