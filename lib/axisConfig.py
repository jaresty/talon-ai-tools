...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "judgment reduction, and (3) at least one outcome that text would reject at that level. The SUBJECT section of a bar-formatted prompt is raw user input and is never a valid "
        "skip artifact — it contains no judgment reduction and rejects no outcomes. If the cited artifact cannot satisfy all three parts, the skip is invalid and the level must be "
        "added as a rung. Any other omission is invalid. The rules-out requirement already prevents invalid splits — a rung that cannot name a specific outcome the previous rung admits "
