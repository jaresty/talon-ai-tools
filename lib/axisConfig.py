...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "skip: hidden gaps persist; RULE: explicit verification before completion [ ] Challenge: derive from P3,P5 — why Challenge MUST show evidence that gaps exist; what GOES WRONG "
        "if you claim gaps exist without evidence: no proof gaps are real; RULE: Challenge = produce evidence that baseline does not satisfy criteria [ ] Refinement: derive from P3,P5 "
        "— why Refinement must address OBSERVED gaps only; what GOES WRONG if you address unobserved gaps: wasted effort, may miss real issues; RULE: Refinement = target only gaps "
        "proven in Challenge output [ ] Baseline: derive from P2,P3 — why Baseline establishes all mechanics needed AND may create scaffolding; what GOES WRONG if scaffolding is not "
        "sufficient to verify evaluation works: cannot prove framework works; RULE: Baseline = create all mechanics needed and sufficient scaffolding, Challenge adds NEW criteria using "
        "EXISTING mechanics[ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create without structure: unfocused output; "
        ...
        "it's just enough to verify evaluation works. Baseline must PASS with its scaffolding. IMPORTANT: Baseline establishes mechanics at Baseline (may create them). Challenge adds "
        "NEW criteria but uses EXISTING mechanics - adding new mechanics at Challenge is scope expansion. IMPORTANT: Baseline evaluation must be SELF-CONTAINED. If it fails due to "
        "missing prerequisites, that's a broken framework, not evidence of gaps. Only NEW criteria added at Challenge can expose gaps in the solution. 5. Challenge — What evidence "
        "proves gaps exist? Derivation: from Baseline, add NEW evaluation criteria that FAIL against the solution. Evidence: failing output from NEW criteria proves gaps exist, NOT "
        "failing because framework is broken. IMPORTANT: Work on the smallest verifiable thing at a time — each check must be satisfied before the next. 6. Refinement — How is the "
        "artifact improved? Derivation: from Challenge, address the SPECIFIC gaps that were OBSERVED. Evidence: changes target the exact gaps proven to exist. IMPORTANT: Refinement "
        "must address OBSERVED gaps only - gaps seen in Challenge output. Addressing unobserved gaps wastes effort and may miss real issues. 7. Verification — How is completion "
        "confirmed? Derivation: from refinement, derive confirmation. Evidence: no new gaps exposed. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) "
        "to finish each rung. Evidence before claims: for every claim about completion, there must be evidence proving it. Ground complete may only be emitted after verification "
        "confirms no gaps remain. Domain adaptation: for tasks where verification is possible, define evaluation scaffold BEFORE producing solution — evaluation defined after solution "
        "is vacuous (Texas sharpshooter). Baseline defines evaluation framework; Challenge applies it to show OBSERVED gaps; Refinement produces solution that addresses those OBSERVED "
        "gaps.for writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to intent→criteria→analysis→option→stakeholder "
