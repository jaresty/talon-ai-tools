...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "symbol: the literal string naming the governed behavior, which must appear as a substring of the failure line in (g); (e) the layer classification — if the governed symbol "
        ...
        "appears in the executor result after removal — a disposable artifact does not satisfy (f); (g) the observed FAIL result: the content of an executor result block appearing in "
        "the transcript after the artifact was applied to the subject and before the first governed artifact-producing action for this behavior, where (a) precedes (c), the named "
        "executor produced the result, and the failure line contains (d) as a substring. A result satisfies this token if and only if: (g) is present at the required position; (a) is "
        "not separated from (c) in (g) by any line whose content contains (c) for a different governed behavior; and a subsequent executed-artifact result contains (b) at the position "
        ...
        "satisfy (g) regardless of whether (a) precedes (c) in its output. Exception: (g) is not required for the single action that produces the governing artifact's assertion text, "
        "per governed behavior — the creation step is that action only; (g) is required for every governed artifact-producing action in the current session — a creation step is that "
        "action only if (c) is absent before the action and present after it, and a named executor invocation against the governing artifact immediately following the action produces "
        "a tool-executed result containing (c) and not (a); immediacy is defined as: no executor result block appears between that action and the named executor invocation — "
        "non-executor actions do not break immediacy. Artifact identifier must differ: the artifact identifier of the named governing artifact must differ from the artifact identifier "
        ...
        "treated as a blocking condition on the current action. Artifact gap: when the Falsify derivation block contains the line '(g) observed FAIL result: unavailable' and no "
        "tool-result block in this transcript produced by a named executor invocation contains (c) as a substring, the derivation is incomplete. A derivation is complete only when a "
        "line beginning 'Gap artifact:' appears in the transcript immediately above the Falsify derivation block, naming the executor invocation designated to produce a result "
        ...
        "appear before the tool call that produces the named artifact.",
