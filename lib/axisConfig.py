...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "proven in Challenge output [ ] Baseline: derive from P2,P3 — why Baseline must establish ALL evaluation mechanics needed; what GOES WRONG if you add new mechanics at "
        "Challenge: scope expands beyond intent; RULE: Baseline = all evaluation mechanics in place, Challenge extends existing mechanics with NEW criteria [ ] Structure before "
        "baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create without structure: unfocused output; RULE: structure before baseline [ ] "
        ...
        "BEFORE the first ladder rung begins. The derivation results must be in the chat, not just in your head. Before starting each rung, re-emit key rules from your derivation "
        "relevant to that rung. BEFORE transitioning to next rung, verify and declare: [current rung artifact completed?], [evidence visible?], [gaps remaining?], [next rung "
        "requires?]. If gaps remain, do NOT proceed — close gap first. Protocol derived MUST be emitted as: 🔵 Protocol derived — AFTER deriving all rules from P1-P5. Ladder derived "
        "MUST be emitted as: 🔵 Ladder derived — AFTER producing the ladder table with columns: rung name | artifact type | derivation source | evidence form. No descent before both 🔵 "
        "Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent work. The 7-rung ladder: 1. Intent — What outcome is being aimed for? Derivation: from "
        "task, derive specific outcome. Evidence: clear statement of goal. 2. Criteria — What conditions must be true for success? Derivation: from intent, derive conditions that "
        ...
        "work. IMPORTANT: Baseline evaluation must PROVE it works - not just describe it. Applying and seeing it pass is the evidence. Declaring complete without applying proves "
        "nothing. IMPORTANT: Baseline establishes the evaluation MECHANICS needed for the task. All evaluation structures, validation mechanisms, or verification frameworks required "
        "must be in place BEFORE Challenge can add new criteria. New criteria added at Challenge must use the SAME mechanics, not require NEW mechanics. IMPORTANT: Baseline evaluation "
        "must be SELF-CONTAINED. If it fails due to missing prerequisites, that's a broken framework, not evidence of gaps. Only NEW criteria added at Challenge can expose gaps in the "
        ...
        "next. 6. Refinement — How is the artifact improved? Derivation: from Challenge, address the SPECIFIC gaps that were OBSERVED. Evidence: changes target the exact gaps proven to "
        "exist. IMPORTANT: Refinement must address OBSERVED gaps only - gaps seen in Challenge output. Addressing unobserved gaps wastes effort and may miss real issues. 7. "
