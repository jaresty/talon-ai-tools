...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "made by the same context-rich reasoning shoshin exists to escape, so an ambient frame can make its own alternatives look too implausible to trigger a pass. Two tiers address "
        "this. Preferred (structural): when an isolating topology is available, delegate trigger detection and the fresh pass to it; the delegation is taken only when a result block "
        "produced by that separate window or subprocess appears in the transcript and the reconcile step cites it — a claim of delegation with no such result block does not satisfy "
        "this tier, and the declared fallback below is then required. A delegate never received this reasoning's accumulated frame and so cannot veto its own alternatives, but it "
        "carries its own priors, so this is the strongest available lever, not a completeness guarantee. Fallback (declared): unless a delegation result block from an isolating "
        "topology appears at an earlier transcript position, before concluding that no point triggers, enumerate the answer-shaping frames the current analysis assumes — the ambient "
        "defaults that determine what the answer is about — naming each as 'Assumed frame: <frame>', then apply the divergence test to each named frame. The enumeration fires no pass "
        "by itself; it makes unnamed priors auditable so that a single trigger found cannot be an unexamined artifact of the frame-finder running inside the frame. A shoshin-eligible "
        "response produced without a delegation result block and without any 'Assumed frame:' enumeration has not established that no trigger was missed. (2) Inadmissible: lists the "
        "specific prior conclusions or framings withheld for this pass; this line must appear at an earlier transcript position than the pass it governs. (3) A fresh-pass block "
        "reasons about the subject using only context not listed in Inadmissible — most often withholding inherited interpretation to reason as if encountering the problem fresh, or "
        "equally injecting a normally-excluded context (adversarial, outsider, or alternative-ontology framing). (4) Reconcile: compares the fresh-pass result against the context-rich "
        "view and either quotes a specific claim that differs between them or states no claim differs. Before Reconcile, a leak check runs: for each conclusion in the fresh-pass "
        "block, attempt to identify an item named in Inadmissible that the conclusion depended on; emit 'Leak: found' with the item and redo the pass excluding that item, or 'Leak: "
        "not found'; iteration terminates when Leak: not found. The withheld context is not erased — it is declared inadmissible, and a fresh-pass conclusion that reuses an "
        "Inadmissible item without a resolved leak check does not satisfy this token. The admissibility change may be enforced by an isolating topology when available (a separate "
        "window or subprocess that genuinely cannot see the withheld context), in which case Leak: not found is guaranteed by construction rather than checked. This move is "
        "re-entrant: it may be invoked any number of times in a single response, each invocation self-contained with its own four parts. A response in which no point meets the trigger "
        "emits no invocations and is compliant; excluding context never licenses contradicting an established fact.",
