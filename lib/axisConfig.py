...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response applies a meta-process discipline before any implementation action. Derivation phase (required before any file-modifying tool call): name the root criterion — (1) "
        "the governing goal must appear under a literal heading in the transcript before any implementation action, identified as the goal no other stated goal subsumes; (2) behavioral "
        "dimensions must appear under a literal heading, each naming as a literal substring the specific clause in the governing goal it derives from; (3) each artifact in the "
        "enforcement sequence must appear under a literal heading naming, as a literal substring, what the prior artifact's passing state admits that this artifact rejects; (4) each "
        "behavioral dimension must be covered by a verbatim string from a tool-executed result appearing under a literal completion-check heading. Then enumerate every path by which a "
        "transcript could satisfy these heading requirements without the governing goal having actually been identified, dimensions actually derived, or dimensions actually covered: "
        "for each path, name the literal string in the transcript that closes it — a path is closed only when naming that string makes the non-compliant transcript distinguishable from "
        "the compliant one without assessing intent; a path that cannot be closed by naming a string must be eliminated by ensuring the required structural property is present before "
        "the heading is written. The enumeration is complete when no remaining open path exists. The literal strings named during enumeration as path closures are binding — a heading "
        "or structural property that does not match the committed string verbatim does not satisfy the corresponding requirement. Proceed to implementation only when the derivation and "
        "enumeration are present as text above this point in the transcript. Any noun phrase in the governing goal that names a file path, URL, ticket number, or document title not "
        "already present in the transcript must be fetched before the governing goal heading is written — derivation is blocked until the referent is resolved. No file-modifying tool "
        "call may appear before the completion-check heading. The response is not permitted to end until the completion-check heading appears in the transcript and at least one "
        "dimension is covered by a verbatim string from a tool-executed result above it.",
