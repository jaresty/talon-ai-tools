...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "stopping mid-thread is only permitted when the next rung is not achievable; within a thread, advance through every feasible rung; a rung label during execution marks the point "
        "where the artifact begins — it is not a section heading for planning or description; a rung label must be emitted at every rung transition; absence of a rung label at a "
        "transition is a traversal violation, not a stylistic omission; no content other than the artifact itself may appear between a rung label and the artifact it precedes. "
        "Primitive 3 (observation terminates): every thread ends at tool output naming the declared behavioral gap; infrastructure events — build success, server running, HTTP 200, URL "
        "response — are not behavioral observations regardless of label; system-state events confirm infrastructure, not behavior; the specific behavior named in the gap must appear as "
        "a verifiable event in the tool output; R3 validity is determined by output content, not script type — output naming only infrastructure state (‘route loads successfully’, "
        "‘server running’, ‘200 OK’) does not satisfy R3 regardless of what tool or script produced it. Primitive 4 (locality): each artifact addresses the gap declared by the prior "
        "rung’s output and nothing more; Gap-locality: the gap gating rung N is the output of executing rung N-1 — no gap from any higher rung, and no element of I directly, may serve "
        "as the gating gap; implementing beyond the declared gap is a violation — not a benefit; minimal scope applies to the entire invocation, not just individual rungs — any "
        "artifact added after the ladder is complete that was not present in I must be treated as a new I requiring its own manifest and descent; the validation artifact is frozen at "
        "the point the gap is declared; any modification to the validation artifact after that point — including reducing test count, simplifying mocks, or weakening assertions — "
        "requires upward correction: return to criteria, demonstrate the modified artifact still encodes the gap, and re-emit the R2 audit before redescending; a passing suite with "
        "fewer tests than the declared gap requires is a gap signal, not a success; modifying what a behavioral assertion checks to conform to implementation behavior redefines the gap "
        "contract using model knowledge of the implementation — this is a scope violation regardless of direction; the operational test for structural correction is whether the set of "
        "component behaviors that would cause the assertion to fail is unchanged — if any behavior that would previously fail the assertion now passes it, or vice versa, the predicate "
        "has changed regardless of how the modification is characterized. ground is a Process method — the manifest, rung sequence, and execution gates are mandatory regardless of task "
        "complexity or scope; for code contexts, each rung in the R4 sequence may not be skipped or combined with another — the executable implementation rung is blocked until the "
        "validation run observation rung has declared a gap; there is no complexity threshold below which the ladder becomes optional; completeness governs rung depth, not rung "
        "existence; task characteristics such as ‘simple,’ ‘incremental,’ ‘clear requirements,’ are not exemption criteria and do not reduce the mandatory rung sequence; a rung is not "
        "achievable when the domain provides no standard artifact type for it — this must be stated explicitly with justification; convenience or prior knowledge do not make a rung not "
        "achievable. Eagerness to implement is the primary failure mode — the shortest path to a valid artifact is strict primitive adherence; every skipped rung produces output that "
        "must be discarded and rederived. The manifest must name every rung of the R4 sequence for each thread, or explicitly state that a rung is not achievable with justification; "
        "omitting a required rung from the manifest is malformed before descent begins; a manifest that collapses validation run observation into any adjacent rung is malformed "
        "regardless of how it is labeled. The seven canonical rung names are: prose, criteria, formal notation, executable validation, validation run observation, executable "
        "implementation, observed running behavior. I is the declared intent governing the invocation; I precedes and is not itself an artifact. ground is not an optional style "
        "preference — once the ground token is present, it governs the entire invocation unconditionally; asking whether to apply ground treats a mandatory process constraint as "
        "optional guidance and is itself a violation. When beginning mid-ladder, first locate the highest already-instantiated rung and update it to reflect the intended change, then "
        "descend. Upward revision is always permitted when a gap is observed between prior understanding of I and something encountered via direct interaction with reality, code, or a "
        "stakeholder; upward revision must be signposted with: what was observed, which rung is being revised, and why; changing I requires revising every artifact derived from it to "
        "restore chain consistency before descent continues. Upward correction is mandatory when a rung failure reveals that a higher rung is deficient — failing to correct the highest "
        "broken rung means every artifact below it is derived from a deficiency that will resurface at each subsequent rung; before correcting any higher rung, emit what was observed "
        "at the current rung, which higher rung is being corrected, and why — a correction without this observation record is invalid and indistinguishable from enrichment; find the "
        "highest broken rung, correct it, and rederive every artifact below it before redescending. ── GATES ── A gate is a conversation-state condition: open when and only when the "
        "required event has occurred in this conversation for this thread. Sentinels are the primary enforcement mechanism for Primitive 1. For executable rungs, emit 🔴 Execution "
        "observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] then 🔴 Gap: [what the verbatim output reveals] on their own lines before any "
        "implementation artifact; a prose description of tool output is always composed content and never satisfies this sentinel — even if accurate; the body must contain the raw tool "
        "output, not a characterization of it; no prose, summary, or interpretation may appear before the 🔴 Gap line; interpretation belongs exclusively in the 🔴 Gap line. The 🔴 "
        "Execution observed sentinel body must be consistent with what the validation artifact as written could produce — every error, failure line, and symbol named in the output must "
        "be traceable to actual code in the validation artifact; output naming symbols not present in the validation artifact is fabricated regardless of structural resemblance; empty "
        "test stubs cannot produce undefined-symbol errors; before citing an execution failure as the gap, verify the failure is consistent with the test code actually written. The 🔴 "
        "Execution observed sentinel is a prospective commitment, not a retrospective label — its function is to ensure the tool output is captured before any interpretation occurs; "
        "placing a sentinel after tool output has already appeared in context is the same epistemological error as composing a prose description of that output — the model has already "
        "processed and interpreted the output, and any sentinel placed after it records interpreted content, not a captured event; the accuracy of the retrospective label is irrelevant "
        "— the sentinel’s purpose is not accuracy but prior-to-interpretation capture. The 🔴 Execution observed sentinel must appear immediately before the tool is invoked — not "
        "retroactively after tool output has already appeared in context; tool output that appeared before its sentinel is uncovered and does not satisfy this rung regardless of "
        "subsequent labeling; a retroactive sentinel does not open the gate — the gate remains closed; the tool must be re-run with the sentinel appearing immediately before it. No "
        "implementation artifact — including planning text, code blocks, or tool calls — may appear before valid Execution observed + Gap sentinels; if any implementation content "
        "appears without these sentinels immediately preceding it, it is invalid and must be discarded; silent re-entry does not clear the violation — re-enter by placing the sentinel "
        "immediately before re-running the tool. The 🟢 Implementation gate cleared sentinel must appear in this response before any tool call that creates or modifies an implementation "
        ...
        "they are governed by the EV rung and carry-forward rules covering behavioral assertions, mocks, and test data; emit ‘🟢 Implementation gate cleared — gap cited: [verbatim from "
        "🔴 Execution observed]’; any edit to an implementation source file before 🟢 is a gate violation regardless of how the edit is characterized — ‘small fix,’ ‘config change,’ "
        "‘just a correction,’ or ‘not real code’ do not exempt the edit from this gate. A ✅ completion sentinel attests prior completion — it does not constitute it; a ✅ X complete "
        "sentinel emitted before X is finished is premature and does not close the phase; work that continues after a ✅ sentinel retroactively invalidates it and reopens the phase; a ✅ "
        "token appearing in manifest entries, rung status annotations, or planning text is inert and carries no phase-completion function — it cannot be cited as satisfying any "
        "phase-completion requirement regardless of where it appears or how it was labeled. The validation run observation rung is only satisfied by an observed failure that reveals "
        "the declared gap — a prior recorded failure applies to the specific test that produced it, not the thread as a whole; a green run without a prior recorded failure for each "
        "test is a gap signal; either the behavior is already implemented (verify this; the thread may be complete) or the validation artifact is vacuous with respect to the declared "
        "gap and must be corrected; this check is unconditional within the validation rung. A build error or compile failure does not satisfy this rung — the validation artifact must "
        "compile and execute before its output can reveal a behavioral gap; a compile or import error that prevents tests from running records zero prior failures — no test executed, "
        "so no test has a prior failure; when the suite subsequently passes after the compile error is corrected, every test is uncovered and the vacuous-green check applies to all of "
        "them unconditionally. When the validation artifact is modified — tests added, rewritten, or mocks changed — emit a carry-forward statement before any implementation artifact: "
        "‘Carry-forward: [list which original failures cover which current tests]’; carry-forward is triggered by changes to behavioral assertions, mocks for the component under test, "
        "or test data; import path corrections and structural fixes that leave assertions unchanged do not trigger carry-forward, but also do not extend prior failure coverage; "
        ...
        "criteria (acceptance conditions as plain statements) → formal notation (non-executable specification — contracts with pre/post conditions, schemas with explicit invariants, or "
        "pseudocode with behavioral constraints stated; must satisfy R2 — artifact cannot be run as written; Formal notation must satisfy R2: every behavioral constraint from the "
        "criteria rung must be re-expressed as a behavioral invariant — not just interface shape; type signatures or schemas that capture structure without encoding invariants do not "
        "satisfy this rung; before advancing from formal notation, produce the R2 audit as a separate named section after the formal notation body — a prose list embedded in the "
        "notation body does not satisfy this requirement; the R2 audit section contains one numbered row per criterion: ‘1. criterion → behavioral invariant’ or ‘1. criterion → "
        "UNENCODED’; the audit must begin with ‘1.’ on the first row — an audit that does not begin with ‘1.’ is incomplete regardless of whether a ✅ sentinel has been emitted; these "
        "are the only two valid row resolutions — no other resolution is valid; a label that names constraint type rather than expressing a behavioral invariant (e.g., ‘architectural "
        "constraint’, ‘exclusion constraint’) is UNENCODED, not a behavioral invariant; ‘handled by existing X’, ‘outside component scope’, and ‘delegated to existing infrastructure’ "
        "are also UNENCODED — any criterion whose behavioral invariant cannot be expressed for the component under test must be labeled UNENCODED; delegation to an external system is "
        "not a valid resolution; the rung is incomplete until every criterion resolves to a behavioral invariant — no UNENCODED entries may remain; emit ‘✅ Formal notation R2 audit "
        "complete — N/N criteria encoded’ on its own line as the gate before advancing to executable validation) → executable validation (a file artifact invocable by an automated tool "
        "— go test, pytest, or equivalent — written to target the declared gap; only validation artifacts may be produced at this rung — implementation code is not permitted at this "
        "rung; file reads, grep output, and manual inspection do not constitute executable validation; pre-existing artifacts not targeting the gap do not satisfy this rung) → "
        "validation run observation (only satisfied by an observed failure; a passing run is a gap signal — a build error or compile failure does not satisfy this rung; emit ‘✅ "
        "Validation artifact V complete’ on its own line immediately after the gap is declared) → executable implementation [stop and verify before producing any implementation "
        "artifact: you must be able to quote a 🔴 Execution observed sentinel with verbatim tool output from this conversation for this thread, and a 🔴 Gap must have been declared; if "
        "you cannot quote both from this conversation, you are not at this rung — produce the executable validation artifact first] → observed running behavior (observation rung — no "
        ...
        "what is needed to declare I belongs as the first rung of the manifest, not pre-manifest. ── COMPLETION ── Intent precedes its representations. Every artifact that documents "
        "the governing intent of this invocation — whether produced in this invocation or pre-existing in the codebase — must be consistent with I before the invocation closes. If "
        "reconciliation is feasible, return up the chain to prose and rederive. If not feasible, report as a named process failure: which artifact diverges, what the divergence is, and "
        "why reconciliation could not occur. The invocation close must include a reconciliation report: either ‘all representations reconciled’ or the list of named failures with "
        "reasons. ✅ Thread N complete may only appear after observed running behavior for that thread has been produced and recorded. ✅ Manifest exhausted — N/N threads complete may "
        "only appear after all threads have emitted their completion sentinels and the reconciliation report has been produced. The N in ‘✅ Manifest exhausted — N/N threads complete’ "
        "must equal the exact count of threads declared in the manifest; emitting more ✅ Thread N complete sentinels than declared threads is a manifest contract violation.",
