...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "files created; output names only infrastructure state; file read used as evidence”. 🔴 Execution observed: requires: (1) a preceding tool call in this response, (2) verbatim "
        "tool output in a triple-backtick block — complete, nothing omitted, no editorial content. Any deviation voids the sentinel; fix by re-invoking the tool and re-emitting the "
        "block. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current gap before any other "
        "content — ground protocol does not terminate at ✅ Manifest exhausted; it remains active until the session ends or the user explicitly exits. Each completion sentinel is valid "
        "only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. The gap names a specific behavior currently absent or "
        "wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; if no currently-false behavior "
        "remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists for each thread declared in ✅ Manifest declared — "
        "before emitting ✅ Manifest exhausted, locate the N in the ✅ Manifest declared sentinel and count ✅ Thread N complete sentinels — if the count does not equal the declared N, "
        "the manifest is not exhausted. one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the prose, it belongs in "
        ...
        "is present does not satisfy this rung — add a criterion asserting its content or effect; before writing the criteria artifact, locate ✅ Manifest declared in the transcript and "
        ...
        "no implementation internals (no file names, function names, or variable names); this is a content gate, not a self-check. The formal notation rung separates behavioral "
        ...
        "directly demonstrate the specific behavior named in the criterion. A file read never satisfies this rung — the block must contain output produced by running the artifact. If "
        "the artifact requires a start step before querying (e.g., a web server), two tool calls are required in sequence: process-start then query — the 🔴 Execution observed: sentinel "
        "may not be emitted until both complete. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists. If no invocable "
        "process exists, open a gap cycle to make the artifact directly invocable. Before any tool call at this rung, emit a provenance statement: ‘Invoking [artifact] produced at "
        "[rung name] in this thread’ — the provenance statement does not replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not be "
        "emitted unless all of the following exist in this cycle's transcript: (1) a tool call exists in the transcript after the observed running behavior label in the current cycle; "
        "(2) observed running behavior label has been written after the most recent 🟢 Implementation gate cleared in this thread; (3) non-empty verbatim output appears in the "
        ...
        "tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason. Once ✅ Thread N complete is emitted no further "
        ...
        "behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the "
        "prose — the manifest thread list must contain exactly one entry per distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol "
        "violation. The prose rung must be re-emitted at the start of every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must "
        "be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — "
        "not from a prior cycle — to exist in output; the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate "
        "regardless of whether it produced a failure. Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, "
        "filters, renders, validates, authenticates, loads, saves, or similar) — each distinct predicate requires a separate thread; re-read the prose rung output via a tool call to "
        ...
        "after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. When beginning mid-ladder, locate the highest already-instantiated rung, update it, "
        "then descend; every descent through executable implementation requires executable validation and validation run observation to have fired for the current gap, including after "
        "an upward return. Upward returns follow the failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did not model the "
