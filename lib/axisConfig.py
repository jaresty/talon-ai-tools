...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "here; if no such claim can be named, the trigger is not met and no further parts are emitted. Trigger detection is subject to a paradox: the plausible-divergence judgment is "
        "made by the same context-rich reasoning shoshin exists to escape, so an ambient frame can make its own alternatives look too implausible to trigger a pass. Resolve it by "
        "routing to an action, not by checking pre-existing transcript state. Preferred: invoke a subagent-spawning tool now to run trigger detection and the fresh pass in a context "
        "that does not inherit this reasoning's accumulated frame — the invocation appears as a tool-use block and the reconcile step cites the result block it returns; a frame the "
        "delegate never received cannot veto its own alternatives, so its triggers are trustworthy signal, though the delegate carries its own priors and is the strongest available "
        "lever rather than a completeness guarantee. If no subagent-spawning tool can be invoked, instead enumerate the answer-shaping frames the current analysis assumes — the "
        "ambient defaults that determine what the answer is about — naming each as 'Assumed frame: <frame>', then apply the divergence test to each; this in-thread enumeration makes "
        "unnamed priors auditable but does not escape the paradox. Which branch applies is decided by separability: delegation escapes a frame only when the context to withhold is "
        "separable from the minimal statement of the task — accumulated reasoning built up over prior turns (earlier conclusions, interpretations, or framings) that can simply be left "
        "unforwarded, so the delegate receives the task without it. When the frame to escape is instead inseparable from the task's own minimal statement — the task cannot be stated "
        "without stating the frame — briefing the delegate re-imports the frame, so delegation cannot strip it and in-thread enumeration is used instead. Decide by stating the task in "
        "its minimal form and checking whether the suspected frame still appears in that minimal statement: if the minimal statement omits the frame the context is separable and "
        "delegation is preferred; if the minimal statement still contains it the frame is inseparable and in-thread enumeration is used. Delegation opens a boundary the leak check "
        "must also cover: whatever is placed in the delegate's prompt can re-import the frame the delegation was meant to escape, because a fact stated as a neutral requirement may be "
        "a withheld conclusion in disguise. So before invoking the subagent-spawning tool, declare the material to be forwarded: emit one 'Forwarded: <item> — [requirement | "
        "inherited-framing]' line per item to be placed in the delegate's prompt, classifying each as a genuine requirement of the task or an interpretation inherited from this "
        "reasoning's accumulated context. The delegate's prompt is the content of the subagent tool-use block's input; an item classified inherited-framing must either not appear in "
        "that input or appear there only as an explicit question for the delegate to challenge, and an inherited-framing item appearing in the input in declarative form is passed as "
        "an assumption. The leak check then runs against both lists: a fresh-pass conclusion that depended on an item named in 'Inadmissible:', and any inherited-framing item that "
        "appears as an assumption in the delegate's tool-use input, are both leaks. A delegation with no 'Forwarded:' declaration has not established that the delegate's brief did not "
        "re-import the frame. A shoshin-eligible response that produced neither a delegation tool-use block nor an 'Assumed frame:' enumeration has not established that no trigger was "
        "missed. (2) Inadmissible: lists the specific prior conclusions or framings withheld for this pass; this line must appear at an earlier transcript position than the pass it "
        "governs. (3) A fresh-pass block reasons about the subject using only context not listed in Inadmissible — most often withholding inherited interpretation to reason as if "
        "encountering the problem fresh, or equally injecting a normally-excluded context (adversarial, outsider, or alternative-ontology framing). (4) Reconcile: compares the "
        "fresh-pass result against the context-rich view and either quotes a specific claim that differs between them or states no claim differs. Before Reconcile, a leak check runs: "
        "for each conclusion in the fresh-pass block, attempt to identify an item named in Inadmissible that the conclusion depended on; emit 'Leak: found' with the item and redo the "
        "pass excluding that item, or 'Leak: not found'; iteration terminates when Leak: not found. The withheld context is not erased — it is declared inadmissible, and a fresh-pass "
        "conclusion that reuses an Inadmissible item without a resolved leak check does not satisfy this token. The admissibility change may be enforced by an isolating topology when "
        "available (a separate window or subprocess that genuinely cannot see the withheld context), in which case Leak: not found is guaranteed by construction rather than checked. "
        "This move is re-entrant: it may be invoked any number of times in a single response, each invocation self-contained with its own four parts. A response in which no point "
        "meets the trigger emits no invocations and is compliant; excluding context never licenses contradicting an established fact.",
