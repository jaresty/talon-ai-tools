...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "guard as written by constructing a concrete scenario in which property [N] is absent while the quoted guard nevertheless succeeds (e.g. a guard that matches output text "
        "rather than executing the governed behavior succeeds whenever the text appears, even if the behavior is absent); emit 'Test blind-spot: found — <construction>' or 'Test "
        "blind-spot: not found'; if found, fix the guard and repeat from step (2); (4) execute the quoted guard in a state where property [N] is violated and emit 'Failure: property "
        "[N] — <verbatim quoted failure line from a tool-result block>' or 'Unobservable: property [N] — <reason>'; emit Unobservable only when no executable guard, assertion, "
        "constraint, or rule could ever distinguish satisfaction from violation regardless of what tests are written; if a guard can exist, execute it against a violating state and "
        "use the resulting failure line instead; before writing any implementation, a verbatim failure line from a tool-result block must appear; if the failure line does not "
        "specifically witness the violation of property [N], fix the violating state or implementation and repeat from step (4); (5) write the implementation, then immediately emit "
        "'Quoted implementation: <smallest contiguous verbatim text uniquely identifying the implementation>'; no subsequent step may occur until this sentinel has been emitted; (6) "
        "attempt to construct a concrete scenario in which the quoted implementation can be changed while the quoted guard continues to succeed; emit 'Implementation overreach: found "
        "— <construction>' or 'Implementation overreach: not found'; if found, minimally revise the implementation so the described change is no longer possible without the quoted "
        "guard detecting it, then emit a new 'Quoted implementation:' and repeat from step (6). Every overreach analysis must reason only from the quoted implementation and quoted "
        ...
        "resolved, or 'Coverage: gap — property [N]' for any unresolved property. A response that omits any required sentinel, performs a step before emitting its required quoted "
        "artifact, writes an implementation before a quoted failure line appears, or leaves a property without an automatic regression guard does not satisfy this token.",
