...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "no rung may be skipped on grounds of perceived simplicity, prior knowledge, other active constraint tokens, or content described in or implied by the manifest. I is the "
        "declared intent (the fixed governing authority); V is a constraint artifact derived from I through the ladder; O is the output evaluated against V. Three rules govern every "
        "thread. R1 (I is fixed): every artifact derives faithfully from I and the prior rung — the form changes, the intent does not. R2 (rung criterion): an artifact is a rung iff "
        ...
        "terminates): every thread descends to observed running behavior; anticipated outcomes, asserted expectations, or conversational acknowledgment never satisfy the execution "
        "gate; only tool-executed output with a declared gap does; skipped tests are not observations. Execution gate (R3): one principle governs all gates — every rung advancement "
        "requires an externally received result; nothing self-produced by the model (planning text, anticipated output, composed code blocks, or inline expansions) counts as permission "
        "to advance. Before each rung descent output '🔴 Execution observed: [verbatim tool output — content composed without running the tool is invalid]' then '🔴 Gap: [what the "
        "verbatim output reveals — gaps inferred from reading source or test descriptions without running the tool are invalid]' on their own lines — these sentinels gate the "
        "immediately following rung only; they do not permit skipping intervening rungs; a rung manifest, verbal description, or code block containing a command and its expected output "
        ...
        "tests pass or fail; if tests do not run, fix the rung before descending; if any implementation artifact is produced without valid Execution observed + Gap sentinels "
