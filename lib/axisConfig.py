...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "this step — they may not be cited to expand the candidate and become the governing failures for subsequent steps. Before any edit, produce three artifacts as separate "
        "completed blocks — each block must end before the next opens, and no tool call that modifies a file may appear in the same block as any artifact: (1) scope declaration quoting "
        "verbatim the failure signal from the prior run result — the quoted text must appear in a tool result above this line; (2) derivation block as two entries: first, name the "
        "candidate change and at least one smaller change; the smaller change must be the minimal stub — the smallest change that satisfies the structural constraints of the candidate "
        "without implementing the governed behavior; a minimal stub is a function with the correct type signature and no other statements — its body contains only a return of the zero "
        "value of the return type; a change that produces a failure mode different from the governing failure signal is not a valid smaller change; then apply the smaller change and "
        "execute the governing artifact to produce a tool-executed result — if the governing failure signal text is absent from that result, the smaller change is sufficient and must "
        "become the candidate; if the governing failure signal text is present, the smaller change is insufficient and the candidate is permitted; the only permitted evidence that the "
        "smaller change is insufficient is a tool-executed run result in which the governing failure signal text appears verbatim; a Smaller line is valid only when the file-modifying "
        "tool call that applied the smaller change must appear in the transcript above the run result — a Smaller line that appears without such a tool call above it does not satisfy "
        "this gate; and the most recent tool-executed result appearing above it in the transcript must contain the governing failure signal text verbatim — a tool-executed result "
        "produced before the smaller change was applied does not satisfy this gate regardless of its content; no other form of argument — including prose reasoning about other tests or "
        "failure modes — constitutes valid insufficiency evidence; second, in a separate entry below, quote the specific text from the governing failure signal that shows the governed "
        "behavior absent — the quote must appear in a tool-executed run result above the first entry; a candidate that is larger than the smallest change that resolves the quoted "
        "failure text is not permitted regardless of what else it also resolves; a failure not yet observed in a prior tool-executed run result has not been shown to govern, and a "
