...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "each accepted sub-property. If no candidate split can be constructed, or if the candidate split test is rejected, emit: '§ split test: property [N]: \"<verbatim canonical "
        "definition>\" — atomic, no valid split: <reason>.' The quoted definition must match the retained canonical property verbatim. Apply this procedure to every retained property "
        "until all have reached Step 4 with no valid split. After all retained properties are atomic, emit a completeness check: '§ completeness check: \"<request constraints "
        "verbatim>\" / <P1 expression> / <P2 expression> / ...' — the request constraints must be quoted verbatim from the explicit constraints of the original request; the chosen "
        "interpretation determines the meaning of those constraints where the request is ambiguous but does not add, remove, or alter explicit constraints; interpretive assumptions "
        "made to resolve ambiguity and observation-boundary definitions are not themselves request constraints unless they were explicit constraints of the original request; every "
        "retained atomic property expression must appear exactly once, separated by ' / '; a '§ completeness check:' line that omits the request constraints or omits any retained "
        "property expression does not satisfy this requirement. Immediately after the completeness check, attempt to identify one explicit request constraint that is not logically "
        "entailed by the conjunction of the retained atomic property expressions. If such a constraint exists, emit '§ properties complete? no' — add one new property covering that "
        "explicit constraint, perform the required split testing, and repeat the completeness check. Only when every explicit request constraint is logically entailed by the "
        "conjunction of the retained atomic property expressions may the response emit '§ properties complete? yes' — a '§ properties complete? yes' line without a preceding "
        "completeness check does not satisfy this protocol. The completeness phase is concerned only with coverage of explicit request constraints; redundancy introduced during this "
        "phase is resolved by the subsequent observational-independence phase. After completeness has been established, verify observational independence. A retained property is "
        "observationally independent if there exists at least one admissible construction that violates that property while satisfying every other retained property. Any retained "
        "property that is not observationally independent is observationally redundant and must be removed or merged, provided doing so does not reduce coverage of the explicit request "
        "constraints. A proposed merge creates a candidate retained set by replacing its source properties with the merged property; the source properties remain in the current "
        "retained set until the merged property has successfully completed canonicalization, scope validation, and recursive decomposition and the candidate retained set has "
        "re-established completeness and observational independence. The merged property is a new property and must enter the recursive decomposition procedure (Steps 1–4) before it "
        "can be added to the candidate retained set. If the candidate retained set satisfies all Ground completion conditions, commit the replacement; otherwise discard the merge and "
        "leave the source properties unchanged. If a removal changes the retained property set, the current completeness and independence results are void; re-establish completeness "
        "and observational independence over the resulting retained set. The Ground properties block is complete only when all retained properties simultaneously satisfy: they are "
