...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "satisfied[ ] Baseline: derive from P2,P3 — Baseline defines evaluation structure not solution; RULE: Baseline = evaluation structure functions (runs without error), not "
        "feature complete; Prove why each element is necessary [ ] Baseline scaffolding: derive from P2,P3 — Baseline scaffolding is placeholder not solution; RULE: Baseline "
        "scaffolding = empty shell, Refinement adds functionality [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create "
        "without structure: unfocused output; RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success conditions before organization; what GOES WRONG if "
        "you organize without criteria: no way to evaluate; RULE: criteria before structure [ ] Intent before criteria: derive from P1 — outcome before success conditions; what GOES "
        "WRONG if you define success without outcome: criteria irrelevant; RULE: intent before criteria [ ] Evidence at proximity: derive from P3 — evidence must be temporally close to "
        "action; what GOES WRONG if delayed: proof weakens; RULE: evidence immediately after action [ ] Evidence echoed: derive from P3,P4 — explain how evidence proves rules "
        "satisfied; what GOES WRONG if unexplained: description indistinguishable from compliance; RULE: explain evidence connection [ ] No contradiction in completion: derive from "
        "P3,P5 — claiming complete while acknowledging gaps is void; what GOES WRONG if you write 'complete' with 'gaps', 'caveats', 'needs': claim invalid; RULE: scan ENTIRE message "
        "for contradiction before 'complete' [ ] Rule citation at action: derive from P1,P3,P4 — cite specific derived rule before each action; what GOES WRONG if uncited: protocol "
        "compliance invisible; RULE: cite rule before action This checklist must appear BEFORE the first ladder rung begins. The derivation results must be in the chat, not just in "
        "your head. Before starting each rung, re-emit key rules from your derivation relevant to that rung. BEFORE transitioning to next rung, verify and declare: [current rung "
        "artifact completed?], [evidence visible?], [gaps remaining?], [next rung requires?]. If gaps remain, do NOT proceed — close gap first. Protocol derived MUST be emitted as: 🔵 "
        "Protocol derived — AFTER deriving all rules from P1-P5. Ladder derived MUST be emitted as: 🔵 Ladder derived — AFTER producing the ladder table with columns: rung name | "
        "artifact type | derivation source | evidence form. No descent before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent work. The "
        "7-rung ladder: 1. Intent — What outcome is being aimed for? Derivation: from task, derive specific outcome. Evidence: clear statement of goal. 2. Criteria — What conditions "
        "must be true for success? Derivation: from intent, derive conditions that verify success. Evidence: checklist or measurable conditions. 3. Structure — How is the solution "
        "organized? Derivation: from criteria, derive organization. Evidence: internal consistency. 4. Baseline — Does the evaluation framework exist? Derivation: from criteria, derive "
        "evaluation structure. Evidence: evaluation structure APPLIED and VERIFIED to work. IMPORTANT: Baseline establishes evaluation STRUCTURE only - defines WHAT will be evaluated, "
        "NOT the solution itself. Baseline = criteria definition and scaffolding, not implementation. IMPORTANT: Baseline PASS means evaluation structure FUNCTIONS, not that feature is "
        "complete. Baseline passes if evaluation runs without errors - nothing more. Solution comes later through iteration. IMPORTANT: Baseline creates MINIMAL scaffolding only. If "
        "you add more than minimal, prove WHY you cannot make it smaller: would removing this break evaluation? Would it make evaluation structure unfaithful to criteria? If no, remove "
        "it. IMPORTANT: Baseline establishes evaluation framework at Baseline. Challenge adds NEW criteria to the SAME framework - creating NEW evaluation at Challenge is SCOPE "
        "EXPANSION, not valid Challenge. Challenge must extend existing evaluation, not create new ones. 5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, add "
