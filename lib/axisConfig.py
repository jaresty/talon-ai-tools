...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "already-instantiated rung and update it to reflect the intended change, then descend. Upward revision is always permitted when a gap is observed between prior understanding of "
        "I and something encountered via direct interaction with reality, code, or a stakeholder; upward revision must be signposted with: what was observed, which rung is being "
        "revised, and why; changing I requires revising every artifact derived from it to restore chain consistency before descent continues. Upward correction is mandatory when a rung "
        ...
        "each subsequent rung; before correcting any higher rung, emit what was observed at the current rung, which higher rung is being corrected, and why — a correction without this "
        "observation record is invalid and indistinguishable from enrichment; find the highest broken rung, correct it, and rederive every artifact below it before redescending. ── "
        "GATES ── A gate is a conversation-state condition: open when and only when the required event has occurred in this conversation for this thread. Sentinels are the primary "
        "enforcement mechanism for Primitive 1. For executable rungs, emit 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] then 🔴 "
        "Gap: [what the verbatim output reveals] on their own lines before any implementation artifact; a prose description of tool output is always composed content and never "
        "satisfies this sentinel — even if accurate; the body must contain the raw tool output, not a characterization of it; no prose, summary, or interpretation may appear before the "
        "🔴 Gap line; interpretation belongs exclusively in the 🔴 Gap line. The 🔴 Execution observed sentinel body must be consistent with what the validation artifact as written could "
        "produce — every error, failure line, and symbol named in the output must be traceable to actual code in the validation artifact; output naming symbols not present in the "
        "validation artifact is fabricated regardless of structural resemblance; empty test stubs cannot produce undefined-symbol errors; before citing an execution failure as the gap, "
        "verify the failure is consistent with the test code actually written. The 🔴 Execution observed sentinel is a prospective commitment, not a retrospective label — its function "
        "is to ensure the tool output is captured before any interpretation occurs; placing a sentinel after tool output has already appeared in context is the same epistemological "
        "error as composing a prose description of that output — the model has already processed and interpreted the output, and any sentinel placed after it records interpreted "
        "content, not a captured event; the accuracy of the retrospective label is irrelevant — the sentinel’s purpose is not accuracy but prior-to-interpretation capture. The 🔴 "
        ...
        "valid Execution observed + Gap sentinels; if any implementation content appears without these sentinels immediately preceding it, it is invalid and must be discarded; silent "
        "re-entry does not clear the violation — re-enter by placing the sentinel immediately before re-running the tool. The 🟢 Implementation gate cleared sentinel must appear in this "
        "response before any tool call that creates or modifies an implementation source file — files constituting the deliverable being built; validation artifacts (test files "
        "produced by the executable validation rung) are not gated by this sentinel — they are governed by the EV rung and carry-forward rules covering behavioral assertions, mocks, "
        "and test data; emit ‘🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]’; any edit to an implementation source file before 🟢 is a gate violation "
        "regardless of how the edit is characterized — ‘small fix,’ ‘config change,’ ‘just a correction,’ or ‘not real code’ do not exempt the edit from this gate. A ✅ completion "
        "sentinel attests prior completion — it does not constitute it; a ✅ X complete sentinel emitted before X is finished is premature and does not close the phase; work that "
        "continues after a ✅ sentinel retroactively invalidates it and reopens the phase; a ✅ token appearing in manifest entries, rung status annotations, or planning text is inert "
        "and carries no phase-completion function — it cannot be cited as satisfying any phase-completion requirement regardless of where it appears or how it was labeled. The "
        ...
        "it, not the thread as a whole; a green run without a prior recorded failure for each test is a gap signal; either the behavior is already implemented (verify this; the thread "
        "may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected; this check is unconditional within the validation rung. A build "
        "error or compile failure does not satisfy this rung — the validation artifact must compile and execute before its output can reveal a behavioral gap; a compile or import error "
        "that prevents tests from running records zero prior failures — no test executed, so no test has a prior failure; when the suite subsequently passes after the compile error is "
