...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "a ✅ Thread N complete sentinel exists for each thread declared in ✅ Manifest declared — check the thread count before emitting. From the criteria rung onward, the gap names a "
        "specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is "
        "not. Each rung’s artifact addresses only that gap — not all known requirements of the task — and is minimal: one independently testable behavior derived from the prose alone — "
        "if stating it requires names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is invalid — split it before "
        ...
        "gate. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases; it must encode all structural constraints the "
        ...
        "pre/postconditions; prohibited: complete function bodies, variable assignments, working logic sequences; labeling prohibited content as pseudocode does not exempt it from this "
        "prohibition; if the criterion is already a complete behavioral specification — naming observable behavior and a falsifying condition without referencing function names, field "
        "names, or implementation internals — the formal notation artifact may restate it verbatim with an explicit declaration that no elaboration is needed; this is not a skipped "
        "rung; the artifact must appear in the transcript and the audit section still applies; the audit section is separate and named; the rung is incomplete until all criteria are "
        "encoded. Only validation artifacts may be produced at the executable validation rung — no other content is permitted before ✅ Validation artifact V complete; each test "
        "function asserts exactly one behavioral property — one behavioral property per test function; multiple assertions are permitted only when they jointly constitute a single "
        ...
        "before writing the test, check whether an existing test file covers the same module and add there if so; when the validation artifact is modified, invoke a tool call to read "
        "the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution observed "
        ...
        "modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until carry-forward has been emitted. before emitting ✅ "
        ...
        "tool-executed result in the transcript; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it "
        "before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it "
        "still passes after perturbation, the test is vacuous and must be rewritten; implementation edits may not begin until a red run exists in the transcript for the current cycle; "
        "✅ Validation artifact V complete may not be emitted unless a 🔴 Execution observed: showing the test failing exists in the transcript for the current cycle; emitting V-complete "
        "before seeing a red run is a protocol violation regardless of whether perturbation was attempted. ✅ Validation artifact V complete must be emitted at the executable validation "
        "rung before the validation run observation label is written. Before writing the validation run observation rung label, invoke the validation artifact via a tool call; a "
        ...
        "the harness error before treating any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior 🔴 Execution observed: for "
        "the same artifact appeared after the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation rung, run the validation "
        "artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 "
        "Gap: at the observation rung, the criteria rung label is the only valid next token; 🛑 HARD STOP — upward return to criteria rung is valid only after a 🔴 Execution observed: "
        "and 🔴 Gap: have been emitted at the validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; if the observed gap "
        "matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. Upon writing the observed running behavior label, re-emit the "
        "criterion for this cycle, then immediately invoke the implemented artifact via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must "
        ...
        "state unless the criterion is specifically about process state; a build output, bundle manifest, or compilation result never satisfies this gate regardless of what it names; "
        "for a UI component, this means rendered browser-visible text and values; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed "
        "artifact that demonstrates the declared behavior; a 🔴 Execution observed: whose verbatim block is empty or blank does not satisfy this gate; if the declared gap names a real "
        "network endpoint, the OBS must include verbatim output showing a real HTTP request or response — a test run whose network calls are mocked does not satisfy this gate; before "
        "any tool call touching the filesystem at this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the rung label cannot be "
        "located in the transcript, the tool call is prohibited; implementation edits, new files, and code changes are not permitted at this rung. ✅ Thread N complete may not be "
        ...
        "appears immediately after it that directly demonstrates the criterion — a test pass is not a demonstration; no implementation file was created or modified for this thread’s "
        "gap in this cycle means the gap was already closed (return to criteria) or the test is vacuous (rewrite it); every failing test must be fixed or explicitly acknowledged in the "
        ...
        "verb attributing behavior to the system — must be followed inline by a bracketed thread marker: [T: gap-name]. The prose rung must be re-emitted at the start of every cycle — "
        "the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate "
        "cleared requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — not from a prior cycle — to exist in output. Before emitting ✅ Manifest "
        "declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each "
        "distinct predicate requires a separate thread; re-read the prose rung output via a tool call to confirm every predicate is covered; each manifest entry is a short gap label — "
        "a noun phrase naming the gap, not a behavioral assertion; a manifest entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion "
        "for a gap is produced for the first time at the criteria rung, not the manifest; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is "
        ...
        "rung must resolve; faithful derivation requires form to change, intent to stay fixed. Advance through every feasible rung without pausing for user confirmation between rungs — "
