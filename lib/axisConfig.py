...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "edit did not address it — return to formal notation before any further edit. Before each edit, check: does a 🔴 Execution observed: sentinel exist for the current gap? If not, "
        "run the validation artifact now before editing. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upon writing the observed "
        "running behavior label, immediately invoke a tool and emit 🔴 Execution observed: with its output — prose description of tool output does not satisfy this rung. ✅ Thread N "
        "complete may not be emitted unless the observed running behavior label has been written in the current cycle and a 🔴 Execution observed: sentinel appears immediately after it; "
        "once ✅ Thread N complete is emitted no further output in that cycle is valid. Before emitting ✅ Thread N complete, check: does the declared intent have any behaviors not yet "
        "observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and begin the next cycle at the criteria rung. Outputting a rung label is what begins that rung — it "
        ...
        "documentation is a reconciliation artifact, not a thread. After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each "
        "thread in order; then reconcile any documents (README, specs, ADRs) the implementation affects — this reconciliation is part of the final report, not a separate thread. Seven "
        ...
        "formats — 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🟢 Implementation gate "
