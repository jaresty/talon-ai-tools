...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the subject of the next rung and ## Formalization must not close; when no phrase in the expression admits two non-equivalent outputs, write '§ formalization complete'; '## "
        "Enforcement sequence' must not appear before '§ formalization complete' in the transcript; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear "
        "before '§2 dimensions closed' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each "
        ...
        "'§4 coverage verified'; '## Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## "
        ...
        "write '§5 enumeration complete' when no further open path remains; any referent named in the '[text]' from §1 not already present as a literal string in the transcript above "
        "§1 must be fetched before §1 is written; the string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted' followed by a space and a decimal "
        "integer (e.g. '§ implementation permitted 1') where the integer is the 1-based index of the file-modifying tool call this sentinel governs must appear in the transcript after "
        "'§5 enumeration complete'; a sentinel line of the form '§ implementation permitted [N]' containing the literal bracket characters '[' and ']' does not satisfy this requirement "
        "— only a decimal integer satisfies the index requirement; no file-modifying tool call may appear before the first such sentinel; the literal string '§ implementation "
        "permitted' followed by a space and a decimal integer with the integer matching the 1-based index of the governed tool call must appear immediately before the `(i)` line of the "
        "five-line block with no intervening blank lines or other content between the permit sentinel and the `(i)` line — a blank line, horizontal rule, or any other content between "
        "them does not satisfy this requirement; in the assistant text block immediately before each file-modifying tool call, in the same response turn as that tool call — a user "
        ...
        "Rung-completion sentinel finality: each of the strings '§0 observed', '§1 goal derived', '§1a decomposed', '§1b candidates', '§2 dimensions closed', '§ formalization "
        "complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank line of the turn in which it is written — no prose, heading, or "
        "non-sentinel content may follow a rung-completion sentinel in the same turn; a turn that contains a rung-completion sentinel followed by any non-blank content that is not "
        "itself a rung-completion sentinel or exit string does not satisfy ground. Continuation invariant: when the final non-blank content line of a turn contains or follows one of "
