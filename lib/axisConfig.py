...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "if you claim gaps exist without evidence: no proof gaps are real; RULE: Challenge = produce evidence that baseline does not satisfy criteria [ ] Baseline: derive from P2,P3 — "
        "why Baseline must establish ALL evaluation mechanics needed; what GOES WRONG if you add new mechanics at Challenge: scope expands beyond intent; RULE: Baseline = all "
        "evaluation mechanics in place, Challenge extends existing mechanics with NEW criteria [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; "
        "what GOES WRONG if you create without structure: unfocused output; RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success conditions before "
        "organization; what GOES WRONG if you organize without criteria: no way to evaluate; RULE: criteria before structure [ ] Intent before criteria: derive from P1 — outcome before "
        "success conditions; what GOES WRONG if you define success without outcome: criteria irrelevant; RULE: intent before criteria [ ] Evidence at proximity: derive from P3 — "
        ...
        "criteria, derive evaluation structure. Evidence: evaluation structure APPLIED and VERIFIED to work. IMPORTANT: Baseline evaluation must PROVE it works - not just describe it. "
        "Applying and seeing it pass is the evidence. Declaring complete without applying proves nothing. IMPORTANT: Baseline establishes the evaluation MECHANICS needed for the task. "
        "All evaluation structures, validation mechanisms, or verification frameworks required must be in place BEFORE Challenge can add new criteria. New criteria added at Challenge "
        "must use the SAME mechanics, not require NEW mechanics. IMPORTANT: Baseline evaluation must be SELF-CONTAINED. If it fails due to missing prerequisites, that's a broken "
        "framework, not evidence of gaps. Only NEW criteria added at Challenge can expose gaps in the solution. 5. Challenge — What evidence proves gaps exist? Derivation: from "
        "Baseline, add NEW evaluation criteria that FAIL against the solution. Evidence: failing output from NEW criteria proves gaps exist, NOT failing because framework is broken. "
        ...
        "challenge, derive targeted changes. Evidence: addresses specific gaps. 7. Verification — How is completion confirmed? Derivation: from refinement, derive confirmation. "
        ...
        "framework; Challenge applies it to show gaps exist; Refinement produces solution that passes evaluation. for writing tasks, it maps to "
