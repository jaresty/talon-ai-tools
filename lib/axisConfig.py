...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rung produces validation-run-observation-type output, not observed-running-behavior-type output, and does not satisfy that gate). A3: each descent through the ladder is a new "
        "evidential context — artifacts, test results, and artifact names from prior cycles have no standing in the current cycle. R2: each artifact derives from the prior rung — form "
        "changes, intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap declared by the prior rung, nothing beyond it. P1 (Evidential "
        "boundary): a rung gate is satisfied if and only if a tool-executed event appears in the current-cycle transcript whose output is classified as that rung’s artifact type; no "
        "other event — inference, file read, test runner output, static analysis, prior-cycle output — satisfies any gate regardless of its content or accuracy. P2 (Forward-only "
        "discipline): a rung label may not be emitted until all preconditions for that rung are present in the current-cycle transcript; the precondition list for each rung is the rung "
        "table’s gate column; no content at a rung is valid before the label; no label is valid before its gate conditions. P3 (Scope discipline): each rung artifact addresses exactly "
        "the gap declared by the prior rung — nothing beyond it; one gap per cycle; one criterion per thread per cycle; one edit per re-run; After ✅ Manifest declared, the only valid "
        "next token is the criteria rung label for Thread 1 — no criteria for other threads, no planning narration, no other content; all seven rungs must complete for Thread N before "
        "any rung content for Thread N+1 may appear. no anticipation of future gaps, no additional invariants, no coverage beyond the declared gap; one edit means exactly one tool call "
        "that creates or modifies a file — narrating a change without a file-write tool call is not an edit. Rung table — name | artifact type | gate condition | void condition: prose: "
        ...
        "tool call that the artifact path does not pre-exist or is already failing — V-complete may not be emitted without this tool-executed result in the transcript; asserting "
        ...
        "STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the edit did not address it — "
        ...
        "criteria rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running behavior label, "
        "re-emit the criterion for this cycle — criterion re-emission is required: emitting the OBR label without immediately following it with the criterion text is a protocol "
        "violation; the tool call is the only valid next action after criterion re-emission; planning statements, diagnostic narration, and commentary between criterion re-emission and "
        "the tool call are protocol violations; ✅ Thread N complete may not appear until a tool call exists in the transcript after the observed running behavior label in the current "
        "cycle; then immediately invoke the implementation as a live running process via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must "
        "directly demonstrate the specific behavior named in the criterion. a test runner invocation does not satisfy this gate — running tests produces validation-run-observation-type "
        "output, not observed-running-behavior-type output; the invocation target must be the implementation artifact itself, not a test harness wrapping it; A file read never "
        "satisfies this rung — the block must contain output produced by running the artifact. If the artifact requires a start step before querying (e.g., a web server), two tool "
        "calls are required in sequence: process-start then query — the 🔴 Execution observed: sentinel may not be emitted until both complete. In-process rendering "
        ...
        "directly invocable. Before any tool call at this rung, emit a provenance statement naming the invocation target: ‘Invoking [artifact] produced at [rung name] in this thread’ — "
        "if the target is a test runner, stop: the invocation target must be the implementation artifact itself running as a live process, not a test harness; the provenance statement "
        "does not replace the tool call — the tool call must follow it in the same response; ✅ Thread N complete may not be emitted unless all of the following exist in this cycle's "
        "transcript: (1) a tool call exists in the transcript after the observed running behavior label in the current cycle; (2) observed running behavior label has been written after "
        "the most recent 🟢 Implementation gate cleared in this thread; (3) non-empty verbatim output appears in the transcript after the observed running behavior label in the current "
        "cycle — a provenance statement with no following 🔴 Execution observed: is a protocol violation; (4) the 🔴 Execution observed: directly demonstrates the criterion — if it does "
        "not directly demonstrate the criterion, emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules; (5) a full test suite run result exists "
        ...
        "acknowledged in the transcript with a written reason. ✅ Thread N complete immediately follows 🔴 Execution observed: at the OBR rung — not before it; emitting ✅ Thread N "
        "complete before the OBR rung tool call and 🔴 Execution observed: sentinel exist in this cycle is a protocol violation. Once ✅ Thread N complete is emitted no further output in "
        "that cycle is valid. Before writing the criteria, formal notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the "
        "declared gap — what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 "
        ...
        "every cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 "
        ...
        "before closing a rung artifact, state every open constraint the next rung must resolve; faithful derivation requires form to change, intent to stay fixed. Advance through "
        "every feasible rung without pausing for user confirmation between rungs — completeness governs rung depth, not rung existence; each rung may not be skipped or combined with "
        "another, including the observed running behavior rung — it is required in every cycle; the only protocol-defined stop is at the validation run observation rung after emitting "
        "🔴 Gap:; stopping between rungs at any other point is a protocol violation — waiting for user confirmation between rungs is a protocol violation; all other rung transitions are "
        ...
        "coverage summaries, no suggestions; then reconcile any documents the implementation affects. Rung-entry gate: before producing content at any rung, state (a) the rung name, "
        "(b) the current gap as a currently-false behavioral assertion, (c) the artifact type this rung requires, and (d) whether a 🔴 Execution observed: sentinel exists in the current "
        "cycle and, if so, whether its output is of type (c) — if no exec_observed exists yet in this cycle, state that explicitly rather than treating prior-cycle output as satisfying "
        "this check; if any of (a)–(d) cannot be stated from the current-cycle transcript, produce it before any other content at this rung; if (d) reveals that no valid exec_observed "
        "exists in the current cycle, the only permitted next token is a tool call — producing any other content when (d) is false is a protocol violation. Seven rungs in order: prose "
