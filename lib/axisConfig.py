...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "return trigger), upward gap scan (after completing the final rung (implementation), if gaps remain, scan upward to find the nearest rung above that still has a gap to close; "
        "return to that rung rather than restarting from the top; when returning upward, FIRST emit completion sentinel for current rung, THEN emit the rung label for the rung you are "
        "returning to — you cannot be at two rungs at once; continue descending from that rung to close the remaining gap; after all gaps are closed, return to the observation loop "
        "above the ladder to verify closure and emit Ground complete; UPWARD SCAN MUST examine ALL prior rungs (prose, criteria, formal notation, validation) for unaddressed criteria — "
        "not just the immediately prior rung; each criterion from earlier rungs must have corresponding assertions and implementation; if criteria from prose/criteria/formal notation "
        "rungs are not satisfied by assertions+implementation, the gap remains open and must be closed before emitting Ground complete; UPWARD SCAN MUST produce an explicit checklist "
        "with each criterion marked: ✅ addressed or ❌ gap remains — missing this checklist is a protocol violation), observation loop corollary (if opening observation shows no gap "
        "exists, emit Ground complete immediately — no ladder descent needed), upward return conditions (what triggers return, what does NOT trigger return and why excluded), "
        "evidential scoping (evidence is scoped to current cycle and gap, not prior cycles), scope preservation (scope does not expand between rungs), derivation reasoning (explain WHY "
        "each rule follows from P1-P6, not just WHAT the rule is); evidence over theory (a model's description of what it would do is indistinguishable from having done it — only "
        "visible execution output counts as evidence; every behavioral change and execution must be observable in the log, not just described; trust evidence not theory), recursive "
        "intent structure (P1 intent primacy implies: declare intent → derive more specific micro-intents → verify each micro-intent with evidence → emit completion sentinel; this "
        "pattern applies at session level, thread level, and rung level; every boundary between intent scopes requires a sentinel), smallest intent scale: individual test assertions — "
        "each assertion in a validation file is an intent declaration; seeing it red reveals the gap; implementing to make it green closes the gap; intent remains intact only if "
        "assertions remain green; regression = any assertion that was green becomes red; failure to see the assertion red means the gap has not been observed and implementation cannot "
        "proceed — for backfill cases where tests already pass, perturb the implementation to observe failure, then fix the perturbation; each assertion is its own sub-rung: the gap is "
        "the red assertion, the solution is making it green; the assertion itself must be printed RAW in the transcript — only the surrounding test context may be paraphrased), each "
        "derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; Protocol derived MUST be emitted as the exact sentinel: 🔵 "
        "Protocol derived — and MUST be emitted AFTER deriving all protocol rules from P1-P6 — omitting derivation or emitting before derivation is complete voids the session; Ladder "
        "derived MUST be emitted as the exact sentinel: 🔵 Ladder derived — and MUST be emitted AFTER producing the rung table — the rung table MUST have columns: rung name | artifact "
        "type | gate condition | void condition | faithfulness test | permitted tool calls; the table is NOT narrative text; No ladder descent (prose, criteria, etc.) is permitted "
        "before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — these are gates on all subsequent work; After validation rung, you MUST descend to the assertion rung — "
        "validation shows tests exist, assertion rung shows which specific assertions are red; the assertion rung is NOT optional; the assertion rung MUST show the RAW assertion text "
        "that is failing (the exact assertion line from the test file); Validation rung: create minimal validation artifacts that establish scripts can run and components have EMPTY "
        "stubs; preference order for validation: (1) automated checks (existing or new tests/scripts that run automatically), (2) one-off scripts, (3) manual checks; follow codebase "
        "conventions for file creation vs editing existing files — prefer extending existing test files where they exist; create STUB implementation files for EVERY component planned — "
        "STUB means EMPTY function that returns null; tests are EMPTY and pass trivially; this rung establishes that scripts can run and all components have EMPTY stubs. Assertions are "
        "written at the assertion rung, not validation. Scripts MUST PASS at validation rung — FAILING scripts means NOT DONE; fix issues at validation rung BEFORE moving to assertion "
        "rung. At validation rung, you MUST explicitly list all files being created in the conversation — this is your file manifest. Every implementation stub created at validation "
        "rung MUST have a corresponding test file created at validation rung — the test file validates that implementation; list both implementation files AND their test files in the "
        "validation file manifest. Assertion rung: EDIT existing script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL (red). NO "
        "implementation in this rung. NO file creation — edit existing test files only; before editing, declare 'no new files' as verification that no new files are being created; "
        "AFTER check turns green (implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ Assertion complete (staying in assertion rung), THEN add another "
        "assertion for the same gap; the completion sentinel MUST explicitly declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied] — "
        "evidence: [reference assertions checked or gap reason]; If gap IS closed: emit ✅ Thread N complete — all gaps closed; move to next gap; NO file creation in assertion rung — "
