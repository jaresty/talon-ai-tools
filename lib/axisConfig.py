...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "rung: (1) pre-existence or pre-failure check, (2) validation artifact file-writes, (3) test runner — in that order — writing implementation files at the EV rung is a protocol "
        "violation; VRO rung: test runner only; EI rung: implementation file-writes only — writing a file that served as the EV artifact in any prior cycle of this thread is a protocol "
        "violation unless the meta-test pattern is in effect; OBR rung: (1) criterion re-emission, (2) provenance statement, (3) live-process invocation of the implementation artifact "
        "— for artifacts requiring a start step before querying, live-process invocation consists of process-start followed by query, both tool calls required; a valid query is an HTTP "
        "request or browser navigation to the running server’s URL — a test runner invocation is not a valid query regardless of whether a dev server was started first; test runner "
        "after dev server start produces VRO-type output and voids the OBR rung exactly as if no server had been started — (4) exec_observed sentinel with verbatim output, (5) test "
        ...
        "prose rung, re-emit with the revised gap, and descend; a test expectation update made without re-emitting prose is a protocol violation regardless of whether the prior "
        "expectation was incorrect. before emitting ✅ Validation artifact V complete, the artifact content must appear in the current response; emitting the sentinel without artifact "
        "content is a protocol violation; if no P4 EV step (1) pre-existence or pre-failure check tool-call result exists in the transcript, ✅ Validation artifact V complete may not be "
        "emitted — asserting pre-existence without a tool call result does not satisfy this gate; a file-write tool call result showing the validation artifact was written to the "
        "project tree must appear in the current response before ✅ Validation artifact V complete may be emitted — artifact content appearing in prose without a file-write tool call "
        "does not satisfy this gate; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit "
        "— if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if it still passes "
        "after perturbation, the test is vacuous and must be rewritten. Before writing the validation run observation rung label, invoke the validation artifact via a test runner; a "
        "harness error (import failure, syntax error, missing file) is not a red run — the test must have been loadable, its assertions must have run, and they must have failed; fix "
        "the harness error before treating any run as a red witness. when VRO exec_observed is a harness error caused by a missing implementation file: 🔴 Gap: may not be emitted; the "
        "only valid next token is the executable implementation rung label; HARD STOP is not valid. when VRO exec_observed is a harness error caused by a test file error (syntax error, "
        "import of a non-existent component in the test itself): 🔴 Gap: may not be emitted; the only valid next token is a tool call that repairs the test file at the EV rung — not "
        "HARD STOP. when VRO exec_observed is a test-interaction failure — the tests loaded and ran but could not interact with the component under test (e.g., pointer-events: none, "
        "role not found, element not accessible) — 🔴 Gap: may not be emitted and 🟢 Implementation gate cleared is not valid; the only valid next token is a tool call that repairs the "
        "component's accessibility or the test's interaction strategy at the EV rung; this is a harness error class, not a behavioral failure. a green 🔴 Execution observed: is only "
        "valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared showing the test "
        "logic failing. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop "
        "there — do not enumerate multiple failures or plan fixes. re-running VRO without an implementation edit is a protocol violation — every re-run of the validation artifact must "
        "be preceded by an EI rung edit. after a red VRO that is not a harness error, after the criterion re-statement, 🟢 Implementation gate cleared is the next valid token — this "
        "shortcut applies only when a red VRO already exists in the current cycle for the current criterion; a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO "
        "must fire before impl_gate; skipping EV and VRO for a new criterion introduced by HARD STOP is a protocol violation. a green exec_observed without a prior red run means the "
        "test is vacuous — return to the executable validation rung and rewrite the test to fail. a valid 🔴 Gap: at the VRO rung must cite a failing assertion from the exec_observed "
        "output. 🛑 HARD STOP — upward return to criteria rung is valid in exactly one case: the current criterion for this thread is identical to the criterion from the most recent "
        "prior cycle for this thread, confirming the criterion is underspecified (P5 condition); all other failure classes have defined routes — missing implementation file routes to "
        "EI directly; test-interaction failure routes to EV repair; implementation gap that changes between cycles loops within EI; spec gap returns to formal notation; using HARD STOP "
        "for any failure class other than criterion underspecification is a protocol violation. 🛑 HARD STOP position gate: valid only after a 🔴 Execution observed: and 🔴 Gap: have been "
        "emitted at the validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; prose-only exec_observed sentinel does not "
        "satisfy this gate — the exec_observed block must contain non-empty verbatim tool output in a triple-backtick block; HARD STOP may not be emitted at the executable validation "
        "rung — a harness error at EV requires fixing the harness, not an upward return; the only valid next action when EV shows a harness error is a tool call that fixes the harness; "
        "HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, the edit did not address it — "
        "return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not re-emitted; the next token is the "
        "criteria rung label for the current thread. the VRO rung label must appear in the transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate "
        "regardless of whether it produced a failure. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no tool call, no prose, and no file "
        "modification may appear before it in the current response; the next required action after 🟢 is a tool call that creates or modifies an implementation file; the implementation "
        "gate does not complete the thread — EI and OBR rungs must still fire before ✅ Thread N complete; ✅ Thread N complete may not appear until the observed running behavior rung "
        "has fired for this thread; the OBR rung label must appear in the transcript for the current thread's current cycle before ✅ Thread N complete may be emitted — its absence is a "
        "protocol violation, not a voiding condition; the void conditions for OBR handle the case where OBR fires with invalid output; this rule handles the case where OBR never fires "
        "at all. declaring a thread complete by similarity to another thread's failure is not valid — each thread requires its own full descent through OBR with a passing run; "
        "cross-thread similarity reasoning does not substitute for a tool-executed result in the current thread's current cycle. In-process rendering (renderToStaticMarkup, "
        ...
        "live-process tool call result appears in the current thread’s current cycle — for UI components this means a dev server has been started and queried via HTTP or browser "
        "navigation and the query result demonstrates the specific behavior asserted in the criterion, not merely server liveness; a test runner invocation, even in browser mode, does "
        "not satisfy this gate; the test suite run at step 5 presupposes the live-process invocation at step 3 has already produced an exec_observed result; if no live-process tool "
        "call result exists, the only valid next action is to start the process and query it. A full test suite run result is required after the most recent OBR tool call — the only "
        "valid next action if no such result exists is the tool call that runs the suite; every failing test must be explicitly acknowledged in the transcript with a written reason; "
