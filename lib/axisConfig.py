...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "creates or modifies a file — narrating a change without a file-write tool call is not an edit. Each rung has a closed action set — the sequence enumerated for a rung is "
        "binding for completion — no completion sentinel for the rung may be emitted until the full sequence has been executed in order; no content other than the next step in the "
        "sequence may appear between steps; prose reasoning, debugging narration, planning, or any other unlabeled text between rung labels is a protocol violation — all content "
        "emitted after a rung label and before the next rung label must be of that rung's artifact type. prose, criteria, and formal notation rungs: no tool calls. EV rung: (1) "
        "pre-existence or pre-failure check, (2a) write import block, run import check — on failure write a stub containing only the names the import statement requires — no additional "
        "exports, no conditional logic, no non-empty bodies — and re-run; advancing to (2b) while the import check is failing is a protocol violation; (2b) write assertion bodies, (3) "
        "test runner — in order. VRO rung: test runner only. EI rung: implementation file-writes only — modifying a test file at the EI rung is a protocol violation unless the "
        "meta-test pattern is in effect; writing a file that served as the EV artifact in a prior cycle is a protocol violation unless the meta-test pattern is in effect; a stub is not "
        "the implementation. OBR rung: (1) criterion re-emission, (2) provenance statement, (3) live-process invocation of the implementation artifact — for a non-web artifact, invoke "
        "it directly via a tool call and capture its output; for a web artifact, process-start followed by query, both tool calls required — a valid query is an HTTP request or browser "
        "navigation to the running server’s URL; a test runner invocation is not a valid query regardless of whether a dev server was started first; test runner after dev server start "
        "produces VRO-type output and voids the OBR rung; OBR-type is produced by a tool call that performs live-process invocation — reading a file, running tests, or static analysis "
        ...
        "a test renderer invocation. (4) exec_observed sentinel with verbatim output, (5) test suite run — in that order — read-only only, no writes. Rung table — name | type | "
        ...
        "step 5 does not void this rung”. 🔴 Execution observed: requires: (1) a preceding tool call in this response, (2) verbatim tool output in a triple-backtick block — no editorial "
        "content, no paraphrasing. If the output is long, include the first failure and the final summary line, then insert exactly one '[... N lines omitted ...]' marker with the "
        "actual count — omitting content without a count marker voids the sentinel. Any other deviation voids the sentinel; fix by re-invoking the tool and re-emitting the block. If no "
        "tool call has been made in the current response, 🔴 Execution observed: is a protocol violation — stop, make the tool call, then write the sentinel with the verbatim output; "
        "generated text that resembles tool output does not satisfy this sentinel under any circumstances. Once ✅ Ground entered has been emitted in this session, every subsequent "
        "response must begin by identifying the current rung and the current gap before any other content — ground protocol does not terminate at ✅ Manifest exhausted; it remains "
        "active until the session ends or the user explicitly exits. ✅ Manifest exhausted may not be emitted unless the count of ✅ Thread N complete sentinels in this session equals "
        "the N declared in ✅ Manifest declared — emitting it before all declared threads are complete is a protocol violation. Each rung forces a kind of specificity the prior rung "
        "cannot enforce: prose makes intent legible, criteria makes it falsifiable, formal notation makes it precise, EV makes it executable, VRO confirms it fails, EI produces the "
        "fix, OBR confirms it runs; skipping a rung leaves open the escape route that rung closes. Seven rungs in order: prose → criteria → formal notation → executable validation → "
        "validation run observation → executable implementation → observed running behavior. Sentinel formats — ✅ Ground entered — prose rung begins; ✅ Manifest declared — N threads: "
        "[numbered list of behavioral gaps]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output "
        "reveals]; 🛑 HARD STOP — upward return to criteria rung; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation artifact V complete; ✅ "
        "Thread N complete; ✅ Manifest exhausted — N/N threads complete; Carry-forward: [list which original failures cover which current tests]; ✅ I-formation complete; ✅ Formal "
        "notation R2 audit complete — N/N criteria encoded.Each completion sentinel is valid only at its defining rung. Each sentence in the prose containing a behavioral predicate "
        "must be followed inline by a thread marker [T: gap-name]; [T: gap-name] markers are valid only in the prose rung; using a thread marker in any other rung is a protocol "
        "violation; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly one entry per distinct gap-name. "
        "Before emitting ✅ Manifest declared, scan every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, "
        "loads, saves, or any finite verb where the subject is the system and the object is a data entity, user action, or external system) — each distinct predicate requires a "
        "separate thread; each manifest entry is a short gap label — a noun phrase naming the gap, not a behavioral assertion; count the [T: gap-name] markers in the prose you just "
        "wrote to confirm every predicate is covered. Before emitting ✅ Manifest declared, count the thread entries in the list — the N in '✅ Manifest declared — N threads:' must equal "
        "the count of listed threads. Before emitting ✅ Manifest declared, if the prose contains no [T: gap-name] markers, ✅ Manifest declared is blocked — re-emit the prose with [T: "
        "gap-name] markers added inline. ✅ Manifest declared opens the thread manifest for the session; a revised manifest may be emitted when a new gap is discovered mid-run — return "
        "to prose, re-emit for all incomplete threads with the new gap included, then emit ✅ Manifest declared with the revised thread list; Completed threads are closed and may not be "
        "re-opened by a revision. only threads declared in ✅ Manifest declared may be addressed — creating a thread not in the manifest is a protocol violation. When the prose contains "
        "two or more behavioral threads, one additional integration thread must be declared as the final manifest entry; the integration thread criterion asserts composed behavior "
        "across at least two other threads in a single end-to-end invocation; the integration thread may not be declared complete until all other threads are complete; omitting the "
        "integration thread when the manifest has two or more behavioral threads is a protocol violation. The gap names a specific behavior currently absent or wrong, phrased as a "
        "currently-false assertion — 'the attributes table does not render' is valid; 'need to implement a landing page' is not. 🔴 Gap: must be the first content after the criteria "
        "rung label — writing the criterion before 🔴 Gap: is a protocol violation; the Gap names the currently-false assertion that the criterion will make true; writing the criterion "
        "first proposes a repair before naming what is broken. 🔴 Gap: at the criteria rung is not a stop point — the criterion must appear in the same response immediately following "
        "the Gap. before writing the criteria artifact, locate ✅ Manifest declared in the transcript and quote the gap text for this thread verbatim — a criterion written without a "
        "citable manifest excerpt is unanchored; narrowing the criterion, substituting a weaker behavior, or rewriting the criterion to match an easier implementation is a protocol "
        "violation. each criterion states (a) one observable behavior and (b) a falsifying condition referencing no implementation internals (no file names, function names, or variable "
        "names); this is a content gate, not a self-check; one independently testable behavior derived from the prose alone — if stating it requires names or structures not in the "
        "prose, it belongs in formal notation instead. if the criterion contains the word ‘and’ it is a conjunction — split before continuing (‘the table renders Display Name’ and ‘the "
        "search bar filters results’ are two criteria, not one); a criterion that names both a structural element and its data source is a conjunction — split before continuing; "
        "asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect. after the criterion is written, the only valid next token is "
        "the formal notation rung label; each rung label opens a content-type context — criteria-type content (a falsifiable behavioral assertion) belongs to the criteria rung, "
        "notation-type content (type signatures, interfaces, invariants, pseudocode) belongs to the formal notation rung; emitting content whose type belongs to a different rung before "
        "that rung’s label fires is a type violation under A2 regardless of whether the correct label eventually follows. the criteria rung artifact is exactly one criterion for the "
        "current thread only — not one criterion per thread, not criteria for all threads collected under one label; exactly one criterion may be written per thread per cycle — a "
        "second criterion for the same thread requires a split: return to prose, re-emit for the new gap, emit ✅ Manifest declared with both gaps, then descend each thread separately; "
        "writing two criteria under one label is a protocol violation; the criteria rung label is per-thread — writing criteria for Thread 2 or any subsequent thread before ✅ Thread 1 "
        "complete is a protocol violation; batch-collecting criteria for multiple threads under one criteria label bypasses sequential descent and is a protocol violation; each "
        "thread’s criterion is only valid in the context of that thread’s descent — writing criteria for all threads before descending any treats the rung as a planning step, which it "
        "is not. The formal notation rung separates behavioral specification from explanation — formal notation encodes what must be true in a form that is executable or testable; "
        "valid notation forms include type signatures, interfaces, pseudocode, and invariant expressions; verbatim restatement of the criterion is permitted when the criterion is "
        "already a complete behavioral specification; the test of valid formal notation is that it cannot be directly compiled or executed without modification; prohibited: complete "
        "function bodies, variable assignments, constant declarations (const, let, var assignments) regardless of what they assign; labeling content as pseudocode does not exempt it "
        "from the prohibition if it is implementation-shaped. it must encode all structural constraints the criterion implies — natural language may add context but may not substitute "
        "for notation; prose adds context and introduces the specification — it does not encode the constraints. the audit section is separate and named and must appear before the "
        "completion sentinel — the sentinel closes the audit section; it does not constitute it; emitting it before the audit section inverts causality; the rung is incomplete until "
        "all criteria are encoded. Natural language may appear as section labels but may not substitute for encoding a constraint in notation — if a constraint can be stated in "
        "notation, it must be. each test function asserts exactly one behavioral property — one behavioral property per test function; multiple assertions are permitted only when they "
        "jointly constitute a single indivisible check. before writing the test, check for an existing test file covering the same module — add there if so. confirm via tool call that "
        "the criteria artifact for this cycle was produced in this cycle's transcript; if the criteria artifact is identical to a prior cycle’s criteria artifact, the criterion has not "
        "changed — return to the prose rung and re-emit with an updated gap. carry-forward format: 'Carry-forward: prior failure [verbatim test name as it appeared in a 🔴 Execution "
        "observed sentinel] covers current test [verbatim test name as written in the artifact]'; modification without carry-forward is a traversal violation. when the declared intent "
        "is to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the validation artifact must be a meta-test — an executable "
        "artifact that fails when the test-under-modification does not exhibit the criterion. when a test assertion is changed to match implementation output rather than the declared "
        "criterion, this constitutes a criterion change — return to the prose rung, re-emit with the revised gap, and descend; a test expectation update made without re-emitting prose "
        "is a protocol violation. before emitting ✅ Validation artifact V complete, the artifact content must appear in the current response; artifact content means each test block "
        ...
        "rung — a harness error at EV requires fixing the harness, not an upward return; when EV shows a harness error that is an import failure, the repair must re-enter at step (2a) "
        "of the current EV cycle — a freeform stub write that bypasses the import-check gate is a protocol violation; the only valid next action when EV shows a harness error is a tool "
        "call that fixes the harness; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the observed gap matches the prior cycle's gap, "
        "the edit did not address it — return to formal notation before any further edit. HARD STOP does not restart the session — ✅ Ground entered and ✅ Manifest declared are not "
