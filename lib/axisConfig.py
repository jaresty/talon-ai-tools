...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "this behavior but is takes longer to run locally — if no such slower check exists above this line, the candidate is not justified as minimal; second, name the candidate check "
        "— the specific artifact to run, naming its relative speed compared to alternatives; third, name at least one insufficient check — a faster check that would not catch this "
        "behavior — and quote the specific output from a prior run result showing the insufficient check passes when the behavior is absent; if no such insufficient check output "
        "exists above this line in the transcript, the candidate is not derived and a different check must be chosen. Then run the candidate check. The FAIL must be produced by "
        "running an artifact that (a) is written to assert that specific behavior and (b) executes when the same event triggers evaluation of the artifact being changed — name the "
        "triggering event that causes both to execute; if no such shared trigger can be named, the artifact does not govern this change. A FAIL produced from a state with additional "
        "absences beyond the named behavior is not valid. Before the tool call, name: the specific behavior being removed and at least one other behavior that remains intact in the "
        "test state; if omitting less than the named behavior would produce the same FAIL, the named behavior is not the governed cause and must be rederived. Immediately before each "
