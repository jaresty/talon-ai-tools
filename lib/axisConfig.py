...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "gap — what the prior rung’s artifact leaves unresolved; immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. 🟢 "
        ...
        "protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the same response. When beginning "
        ...
        "resolved is a final report regardless of its heading; a final report may not appear unless ✅ Manifest exhausted immediately precedes it in the transcript — if no ✅ Manifest "
        "exhausted exists, stop: emit ✅ Manifest exhausted if all threads are complete, or open the next gap cycle if they are not. After emitting ✅ Manifest exhausted, produce a final "
        ...
        "the implementation affects. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → "
        ...
        "complete; Carry-forward: [list which original failures cover which current tests]; ✅ I-formation complete; ✅ Formal notation R2 audit complete — N/N criteria encoded.",
