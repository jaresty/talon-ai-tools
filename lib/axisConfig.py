...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "identifier from condition (1) as a substring. Bootstrap exception: when no tool-result block in the transcript before the Falsify derivation block contains the governed "
        "symbol name as a verbatim substring, a single governed artifact-producing action may introduce an artifact containing the governed symbol name — this action may not produce a "
        "result in which (b) appears; the named executor must be invoked immediately after this action (no executor result block between the action and the invocation), and the result "
        "must contain (a) before (c) with (d) appearing as a substring of the failure line; no further governed artifact-producing action is permitted until this executor result "
        "appears in the transcript. (e) the layer classification — if the governed symbol name appears as a token in a call expression in the test body — where the call expression is "
        "not nested inside another call expression that names a different symbol — the layer is 'unit'; if the governed symbol name does not appear in the test body, the layer is "
        "'integration'; (f) the executor class — the executor must produce output that could only exist if the governed assertion reached execution at the layer named in (e); a "
        "governing artifact is disposable if removing the governed symbol from the governed artifact — without modifying the named governing artifact — produces an executor result in "
        "which (c) does not appear; a governing artifact is non-disposable only when (c) appears in the executor result after removal — a disposable artifact does not satisfy (f); "
