...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "same gap; If gap IS closed: emit ✅ Thread N complete and move to next gap; NO file creation in assertion rung — edit existing test files if they exist; stubs created at "
        "validation rung; only edit existing scripts to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one "
        ...
        "that specific assertion green — NO assertions added here; NO file creation; only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, "
        ...
        "assertion); If gap IS closed: emit ✅ Thread N complete. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. "
        "ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. Only test assertions with expect() count as assertions. Rung completion MUST be "
        "emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; Every rung must emit its label "
        "BEFORE producing its artifact and emit its completion sentinel AFTER. No implementation is permitted before the assertion rung shows red assertions AND the specific RAW "
        "assertion text is displayed; the exec_observed sentinel must include the verbatim failing assertion text; Evidence before claims: for every claim about code being correct, "
        "there must be tool execution output proving it; describing what you would do is not evidence; ground complete may only be emitted as the outcome of the observation loop — "
        "emitting it outside the loop is a protocol violation. Rung validity test: a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. "
        "Standard ladder for software behavioral change: prose → criteria → formal notation → validation → assertion → implementation — the assertion rung is REQUIRED in this order; "
        "the assertion rung verifies each individual assertion red → green; emit the rung label before producing its artifact; emit completion sentinel after artifact is produced and "
        "gate is satisfied. EV rung: edits only validation files. EI rung: edits only implementation files. An executable validation artifact is a file whose sole purpose is to assert "
        "behavioral properties of another artifact — it contains no behavior of its own; it must be invocable by an automated tool; validation files may not be imported by "
