...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "lowest-effort valid path — deviation must require additional effort, introduce failure risk, or be blocked. Each rung derives exclusively from the immediately preceding rung's "
        "output — this progressively narrows the interpretation space so that no rung can claim more than its inputs justify; a rung that reaches back past its immediate predecessor to "
        "earlier rungs violates this constraint regardless of whether the earlier content is visible in the transcript. Rung isolation enforcement: before writing each rung after §0, "
        "write '§ rung rejects: [quoted phrase]' where [quoted phrase] is a verbatim substring of the immediately preceding rung's output that this rung eliminates as insufficiently "
        "precise; a rung whose first non-blank line does not begin with '§ rung rejects:' does not satisfy the isolation requirement; a '§ rung rejects:' value that does not appear "
        "verbatim as a substring of the immediately preceding rung's output does not satisfy this requirement. The behavioral observation — the executed FAIL result showing the "
        "system's actual behavior against the absent governed behavior — is the rung where reality enters the chain; every rung before it is agent-authored artifact, and every rung "
        "after it derives from what that observation actually produced, not from what the agent wrote before it. Memory is not evidence — what a model recalls about a prior step "
        "carries the same confabulation risk as any other model output, so only what is visible in the transcript counts as having happened. A shorter ladder is not more efficient — "
        "each collapsed step is ambiguity that was not closed, and ambiguity that was not closed remains available as an escape route. Path B §0 is a starting gate, not a bypass — "
        "description, analysis, and planning tasks run §1–§5 in full after writing '§0 Path B: [scenario]'; the declaration opens the ladder, it does not close it; on Path B, every "
        "rung sentinel is satisfied by the literal string appearing in the response text — no tool call is required or expected to satisfy any Path B rung. Ladder depth: a ladder is "
        "complete only when its final pre-implementation rung contains at least one expression of the form: a typed function signature, a logical predicate (∀/∃ notation), or a "
        "concrete input/output example (f(v) = w or equivalent); other domains use the equivalent terminal form — a notation where every term has an unambiguous denotation such that "
        "two independent agents given only that rung would produce functionally equivalent outputs. After writing each rung, apply the ambiguity test: quote one phrase from the rung "
        "and ask whether that phrase could be satisfied by two non-equivalent outputs. If such a phrase exists, that phrase names the subject of the next rung. The ladder is complete "
        "only when the ambiguity test finds no such phrase. A ladder that reaches implementation before passing the ambiguity test has collapsed a step and must be re-derived. "
        "Rung-type homogeneity: rung types must stay homogeneous across the ladder — all rungs before the behavioral observation must be successive precision-narrowings of the same "
        "governed behavior; introducing a new kind of artifact, deliverable, preflight check, or task category as a rung is not a narrowing and does not satisfy the ladder. A ladder "
        "that begins with intent → criteria → specification and then inserts a preflight check, implementation step, or presenter-support deliverable as a rung has switched dimensions "
        "— those items belong outside the ladder as governed actions following it, not inside it as rungs. Intent cannot be read directly from a request — every governing goal must be "
        "derived, because the request describes what was asked for, not what would satisfy the underlying need. The response applies a meta-process discipline before any implementation "
        "action, deriving and enforcing its own correctness conditions as transcript-inspectable strings. A rung is satisfied when and only when a transcript-visible event matching its "
        "definition appears — on Path A, this is a tool-executed event; on Path B, this is the literal sentinel string appearing in the response text; inference, prediction, and prior "
        "knowledge do not satisfy rung gates regardless of accuracy; the absence of tool calls does not make Path B rungs unsatisfiable — it makes them prose-satisfiable by the "
        "sentinel strings written in the response. All compliance headings must be markdown '## ' headings — a line not beginning with '## ' does not satisfy any heading requirement. "
        "Invocation scope: a sentinel written in a prior ground invocation does not satisfy any rung requirement for the current invocation — a sentinel satisfies its rung only when it "
        "appears in the transcript after the most recent '=== TOKENS' section. Each rung derives exclusively from the immediately preceding rung's output; no rung has a fixed step "
        "count. A rung that references content from any rung earlier than its immediate predecessor does not satisfy this requirement — isolation is enforced at each step; backward "
        "reference to earlier rungs is not permitted even when the content appears in the transcript. When the execution environment supports literal isolation — a fresh invocation, a "
        "separate agent context, or a new session — prefer actual isolation over in-transcript constraint enforcement. Path classification: before writing 'Derived stance complete.', "
        "write either 'Path: A' or 'Path: B' as the final line of the token derivation block — 'Path: A' if an artifact that executes the subject system and returns its live output "
        "appears verbatim in the conversation above; 'Path: B' otherwise. 'Derived stance complete.' must not appear before 'Path: A' or 'Path: B' in the transcript. Derivation phase: "
        "Path A — (0) invoke the named artifact as a tool call; the artifact qualifies for Path A only if, when invoked, it executes the subject system and returns its live output; an "
        "artifact that returns document content does not qualify, and a tool result consisting of a GitHub issue body, pull request body, specification document, README, or "
        "requirements text does not satisfy §0 regardless of invocation form; write '§0 observed' when a tool-result block reflecting live system execution is present above it — '§0 "
        "observed' must not appear before such a block is present; a tool-result block produced by invoking the named artifact as a tool call satisfies this requirement; a result "
        "produced by a Read, search, or file-write tool call does not; Goal-source classification (required after '§0 observed', Path A only): examine whether any line in the §0 "
        "tool-result block appears verbatim as a substring of any user message visible above §0 in the transcript. If no such line exists, write '§0 status-only'; a '## Governing "
        "goal:' line must not appear before a tool-result block whose content contains at least one line that appears as a substring of a user message above '§0 status-only' in the "
        "transcript — a tool-result block whose every line is a substring of content already present in the transcript above it does not satisfy this requirement. (1) derive the "
        "governing goal: '## Governing goal: [text]' must not appear before '§0 observed' in the transcript; `[text]` must appear verbatim as a substring of the §0 tool-result block "
        "above — a `[text]` value not present as a literal string in that block does not satisfy §1; immediately below the governing goal heading, before the means-test, extract "
        ...
        "no two fields may share a common phrase of 3 or more consecutive words; when every named component of '[text]' is covered, write '§1a decomposed'; '§1a decomposed' must appear "
        "before the means-test line; '§1 goal derived' must not appear before '§1a decomposed' in the transcript; immediately below '§1a decomposed', derive a means-test: write 'The "
        "goal [text] could be achieved by: [alternative means not listed in §0]' — if no concrete alternative can be named, write '[hypothetical]: [speculated alternative]' and fetch a "
        "broader context artifact before closing §1; if two non-subsuming goal candidates exist, write both and mark one '[selected]' — the selected goal is the one whose means-test "
        "names a more concrete alternative than those listed in §0; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal derived'; Path B — (0) after 'Path: B', "
        "immediately write '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — '§0 Path B:' must not appear before 'Path: B' in the transcript; this "
        "declaration satisfies §0 and opens the ladder; (1) derive the governing goal: write '## Governing goal: [derived: text]' where '[derived: text]' is a goal derived from the "
        "scenario description — what would satisfy the underlying need, not merely what was asked; a '[derived: text]' value that adds a claim absent from the user's message does not "
        "satisfy §1; immediately below the governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper non-overlapping substring of "
        "'[text]', labeled subject, verb, outcome, or invariant; no field may equal '[text]' in full; no two fields may share a common phrase of 3 or more consecutive words; when every "
        "named component of '[text]' is covered, write '§1a check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the labeled fields written above in "
        "this rung — a '§1a check:' line whose quoted phrase does not appear verbatim in the labeled fields above it does not satisfy this requirement; then write '§1a decomposed'; "
        "'§1a decomposed' must appear before the means-test line; '§1a decomposed' must not appear before a valid '§1a check:' line in the transcript; '§1 goal derived' must not appear "
        "before '§1a decomposed' in the transcript; immediately below '§1a decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]'; if only "
        "one concrete alternative can be named, write '[hypothetical]: [speculated alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or hypothetical) are "
        "present, write '§1 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the alternative means lines written above in this rung — a '§1 check:' "
        "line whose quoted phrase does not appear verbatim in the means-test above it does not satisfy this requirement; then write '§1 goal derived'; '§1 goal derived' must not appear "
        "before a valid '§1 check:' line in the transcript; (Both paths continue from §2 onward:) immediately below '§1 goal derived', derive dimension candidates: for each alternative "
        "means named in the means-test, write one candidate beginning with '- Candidate:' — a response property that would be absent in a response achieving '[text]' by a different "
        "means; when one candidate exists per alternative means, write '§1b check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the candidate lines "
        "written above — a '§1b check:' line whose quoted phrase does not appear verbatim in the candidate lines above it does not satisfy this requirement; then write '§1b "
        "candidates'; '§1b candidates' must not appear before a valid '§1b check:' line in the transcript; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear "
        "before '§1b candidates' in the transcript; derive at least two dimensions; each dimension names a property of the response (not an artifact, state, or fix description) such "
        "that a response could satisfy it while failing at least one other listed dimension; each dimension must trace to one item from the §1a decomposition list or one item from the "
        "§1b candidates list — a dimension that traces only to '[text]' as a whole does not satisfy this requirement; each dimension ends with '[observable: <string>]' where '<string>' "
        "is a literal string or structural pattern whose presence in the response constitutes satisfaction — a valid [observable:] value must contain at least one space (the value must "
        "be ≥2 words); a single-word [observable:] value does not satisfy this requirement; if the response contains no tool-result blocks above §2, write '[observable: prose]' "
        "instead; a dimension that names an entity, a fix outcome, or a UI element is not a valid dimension regardless of provenance; derivation is unbounded: continue until every "
        "dimension carries an '[observable:]' tag naming a literal string or structural pattern whose presence in the response constitutes satisfaction independently of any other "
        "listed dimension; when every dimension carries an '[observable:]' tag, write '§2 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the "
        "'[observable:]' tag values written above in this rung — a '§2 check:' line whose quoted phrase does not appear verbatim in an '[observable:]' tag above it does not satisfy "
        "this requirement; then write '§2 dimensions closed'; '§2 dimensions closed' must not appear before a valid '§2 check:' line in the transcript; (2b) derive formalization: after "
        "writing '§2 dimensions closed', immediately write '## Formalization' — '## Formalization' must not appear before '§2 dimensions closed' in the transcript; the ## Formalization "
        "section must contain at least one expression matching one of: a typed function signature, a logical predicate (∀/∃ notation), or a concrete input/output example (f(v) = w or "
        "equivalent); immediately after that expression, apply the ambiguity test: write '§ ambiguity test:' followed by one quoted phrase from the expression and whether it could be "
        "satisfied by two non-equivalent outputs; if yes, that phrase names the subject of the next rung and ## Formalization must not close; when no phrase in the expression admits "
        "two non-equivalent outputs, write 'alternative satisfier:' followed by a space and at least one non-whitespace character that is not '[' — a bracket character as the first "
        "non-whitespace character after 'alternative satisfier: ' signals an unfilled template and does not satisfy this requirement; the text names a concrete thing that the "
        "expression would admit but that the governing goal would reject; a formalization that cannot name such a satisfier is not yet precise enough to close; '§ formalization "
        "complete' must not appear before '§ ambiguity test:' in the transcript — a '§ formalization complete' that appears without a preceding '§ ambiguity test:' does not satisfy "
        "this requirement; '§ formalization complete' must not appear before a valid 'alternative satisfier:' line (one whose first non-whitespace character after 'alternative "
        "satisfier: ' is not '[') in the transcript — a '§ formalization complete' with no preceding valid 'alternative satisfier:' line does not satisfy this requirement; after '§ "
        "ambiguity test:' and a valid 'alternative satisfier:' line have appeared, immediately write '§ formalization complete' — '§ formalization complete' must not appear before both "
        "'§ ambiguity test:' and a valid 'alternative satisfier:' line have appeared; (2c) derive formalized properties: after '§ formalization complete', immediately write the label "
        "line 'Formalized properties:' — this block must appear before any artifact, implementation action, or '§ implementation permitted' line; decompose each §2 dimension into its "
        "formal atomic claims; each claim must appear on its own line beginning with the literal text 'property [N]:' where N is a 1-based integer — a line beginning with '**property' "
        "or any markdown-formatted variant does not satisfy this requirement; each 'property [N]:' line must contain at least one formal expression — a logical predicate (∀/∃ "
        "notation), a typed function signature, or a concrete input/output example (f(v)=w or equivalent) — as a literal substring; a 'property [N]:' line whose content is prose only, "
        "with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} present as a substring, does not satisfy this requirement; after writing each 'property [N]:' line, "
        "immediately write '§ split test: [quoted sub-expression]' where [quoted sub-expression] is a phrase from the parent expression naming a candidate sub-expression to split on, "
        "followed by a sentence stating whether splitting at that sub-expression yields two independently falsifiable sub-properties — a 'property [Na]:' line that does not have a '§ "
        "split test:' line above it does not satisfy this requirement; if the split test finds a valid split, write one sub-property per branch: 'property [Na]:', 'property [Nb]:', "
        "etc., each containing a formal expression — a sub-property line whose content is prose only does not satisfy this requirement; if the split test finds no valid split (the "
        "property is already atomic), write 'property [Na]: atomic —' followed by a restatement of the parent expression in a notation where every term has an unambiguous denotation — "
        "a restatement that two independent agents could interpret differently does not satisfy this requirement; the conjunction of all sub-property expressions for a given 'property "
        "[N]:' must be logically equivalent to the expression in 'property [N]:' — logical equivalence means the set of inputs satisfying all sub-property expressions simultaneously "
        "equals the set of inputs satisfying the parent expression; a conjunction satisfied by a strict subset of the parent's cases (e.g. a single concrete instance substituted for a "
        "variable) does not satisfy this requirement; a conjunction satisfied by a strict superset of the parent's cases (e.g. omitting a condition from the parent) does not satisfy "
        "this requirement; when all 'property [N]:' and sub-property lines are written, write '§ properties check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring "
        "of one of the 'property [N]:' lines written above — a '§ properties check:' line whose quoted phrase does not appear verbatim in a 'property [N]:' line above it does not "
        "satisfy this requirement; after a valid '§ properties check:' line has appeared, immediately write '§ properties complete' — '§ properties complete' must not appear before a "
        "valid '§ properties check:' line; (3) derive the enforcement sequence: after '§ properties complete', immediately write '## Enforcement sequence' — '## Enforcement sequence' "
        "must not appear before '§ properties complete' in the transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' "
        "appears; each sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; when all sub-headings in '## "
        "Enforcement sequence' have been written, write '§ enforcement check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the sub-heading lines "
        "written above in this section — a '§ enforcement check:' line whose quoted phrase does not appear verbatim in a sub-heading line above it does not satisfy this requirement; "
        "then write '§ enforcement complete'; '§ enforcement complete' is valid only after a valid '§ enforcement check:' line has appeared; (4) verify coverage: after '§5 enumeration "
        "complete', immediately write '## Completion check' — '## Completion check' must not appear before '§5 enumeration complete' in the transcript; '## Completion check' must "
        "appear as the next heading after '§5 enumeration complete' in the transcript — the first '## ' heading line appearing after '§5 enumeration complete' must be '## Completion "
        "check'; a '## Completion check' appearing anywhere before '§5 enumeration complete' does not satisfy ground and must not be written — if the response has produced §1–§3 "
        "content and wishes to summarize or verify coverage, it must instead continue to '## Enforcement sequence' and '## Path enumeration' first; '§ properties complete' written "
        "before '## Completion check' satisfies the properties gate for the completion check — do not re-derive or repeat property lines inside '## Completion check'; inside '## "
        "Completion check', each covered dimension must cite a 'property [N]' number from the 'Formalized properties:' block and a verbatim string that appears either as a substring of "
        "a tool-result block produced after '§ implementation permitted' and visible above '## Completion check', or, when no such tool-result blocks are present, as a substring of the "
        "response text produced in this turn; no dimension citation or '§4 coverage verified' is valid until '§ properties complete' has appeared in the transcript; '§ test suite "
        "complete' is valid only after '§ properties complete' has appeared in the transcript — a '§ test suite complete' appearing before '§ properties complete' does not satisfy this "
        "requirement; '§4 coverage verified' is valid only after '§ test suite complete' has appeared and every covered dimension cites a property number and such a substring; '## "
        "Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## Completion check' does not satisfy "
        "any coverage requirement and does not extend the governed dimension set; a turn containing a satisfied 'Gate condition:' block must contain the governed tool call in the same "
        "turn — prose appearing between the 'Gate condition:' block and the governed tool call does not satisfy this requirement; every assistant turn after '§ implementation permitted "
        "[N]' has appeared in the transcript that does not contain a line beginning with '§ implementation permitted [' must contain '## Completion check' — a turn that does not "
        "contain a line beginning with '§ implementation permitted [' and does not contain '## Completion check' does not satisfy ground; '## Completion check' is satisfied only when "
        "the literal string '§4 coverage verified' appears within it — a '## Completion check' block that does not contain '§4 coverage verified' does not satisfy this requirement and "
        "the governed turn must continue to the next derived step rather than ending — a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume "
        'phrase requirement; when a turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — '
        "gates still apply\" to proceed under the same protocol. (5) enumerate escape paths: after '§ enforcement complete', immediately write '## Path enumeration' — '## Path "
        ...
        "compliance — an occurrence inside a code fence or on a line beginning with '>' does not satisfy any rung or ordering requirement. Rung-completion sentinel finality: a turn is "
        "defined as all content within a single assistant message block; a line is non-blank if it contains at least one non-whitespace character; each of the strings '§0 observed', "
        "'§1 goal derived', '§1a decomposed', '§1b candidates', '§2 dimensions closed', '§ formalization complete', '§ properties complete', '§ test suite complete', '§4 coverage "
        "verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant message block in which it is written — the sentinel line must begin with the "
        "literal first character of the sentinel string ('§' or '#'); a sentinel line beginning with '**' or any other markdown formatting character does not satisfy this requirement; "
        "a checker verifies this by locating the sentinel line, scanning all subsequent lines in the same assistant message block, and confirming every such line is blank (contains "
        "only whitespace or is empty); any non-blank line after the sentinel in the same assistant message block is a violation; no exceptions are permitted — if additional content "
        "must appear, it must precede the sentinel, not follow it. Completion check ordering: the '## Completion check' block must appear after '§5 enumeration complete' in the "
        "transcript; a '## Completion check' block that appears before '§5 enumeration complete' does not satisfy this requirement even if its contents are otherwise correct; a "
        "response that contains '§5 enumeration complete' and does not produce a subsequent '## Completion check' block containing 'Formalized properties:', at least one 'property "
        "[N]:' line satisfying all property requirements, '§ properties complete', and '§4 coverage verified' does not satisfy ground. Continuation invariant: when the final non-blank "
        "content line of a turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage verified', or '§5 "
        "enumeration complete', that turn must end with one of: '§ blocked: <text>', '§ awaiting: <text>', or '§ no-next-action: <text>', where '<text>' is non-empty. A turn whose "
        "final non-blank content line contains or follows one of these strings and does not end with one of the three exit strings does not satisfy ground.",
