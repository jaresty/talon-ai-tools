...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "retained set until the merged property has completed canonicalization, scope validation, and recursive decomposition to an atomic state. The merged property must reach an "
        "atomic state before it is substituted for its source properties in the candidate retained set. Once the merged property is atomic, form the candidate retained set by replacing "
        "the source properties with it, then re-establish completeness and observational independence over that candidate retained set. If the candidate retained set satisfies all "
        "Ground completion conditions, commit the replacement; otherwise discard the merge and leave the source properties unchanged. If a removal changes the retained property set, "
        "the current completeness and independence results are void; re-establish completeness and observational independence over the resulting retained set. The Ground properties "
        "block is complete only when all retained properties simultaneously satisfy: they are derived from the chosen interpretation; they are atomic; their conjunction logically "
        "entails every explicit request constraint; they are observationally independent; they introduce no out-of-scope constraints, including internal mechanism constraints not "
        "explicitly required by the chosen interpretation. When all conditions are met, emit '§ ground complete' as the final line of the Ground properties block — this sentinel may "
        "not appear before '§ properties complete? yes' in the transcript. Any test, implementation step, tool call, or task reasoning that appears before '§ ground complete' does not "
