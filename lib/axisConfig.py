...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "sentinel, gate blocks sentinel emission until gate satisfied), evidence chain (every assertion must go red then green; show red output before implementing, show green output "
        "after implementing; describing what you would do is not evidence; every assertion and its red→green transition must appear in the log for independent verification), rung "
        "transitions (emit rung label when beginning each rung; emit completion sentinel when finishing each rung; emit rung label when returning upward to a prior rung; you are at "
        "rung N until you emit completion sentinel for rung N or return upward), rung completion order (each rung must produce its artifact before emitting completion sentinel; "
        "sequential descent through prose → criteria → formal notation → validation → implementation; skipping a rung is a protocol violation), P5 implies sequential descent (since "
        "each artifact derives from prior artifact per P5, you must produce them in order — treating the ladder as a planning document rather than strict sequence violates derivation), "
        "ladder is strict execution sequence (emit rung label, produce artifact, emit completion sentinel — repeat for each rung in order; you may return upward freely, but only when "
        "higher rung artifact is discovered to be malformed — NOT to make lower rung implementation easier; difficulty, failure, or constraint pressure at a lower rung is not a valid "
        "upward return trigger), observation loop returns to start (after manifest exhausted, return to observation loop to verify gap is closed; re-observe running behavior; if gap "
        "still exists, descend ladder again; if gap is closed, emit Ground complete), observation loop corollary (if opening observation shows no gap exists, emit Ground complete "
        "immediately — no ladder descent needed), upward return conditions (what triggers return, what does NOT trigger return and why excluded), evidential scoping (evidence is scoped "
        "to current cycle and gap, not prior cycles), scope preservation (scope does not expand between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just "
        "WHAT the rule is); each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; emit Protocol derived after deriving "
        "all rules; emit Ladder derived after producing the rung table; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a "
        ...
        "change: prose → criteria → formal notation → executable validation → executable implementation — each rung must be completed in this order; emit the rung label before "
        "producing its artifact; emit completion sentinel after artifact is produced and gate is satisfied. EV rung: edits only validation files. EI rung: edits only implementation "
        "files. An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another artifact — it contains no behavior of its own; it must be "
        "invocable by an automated tool; validation files may not be imported by implementation files. An executable implementation artifact is a file that produces behavior directly — "
        "it contains no assertions about other artifacts; implementation files may not contain assertions. A file that both asserts and implements is a type-discipline violation that "
        "voids the rung; classification is determined by file content, not file path or naming convention — path and naming are evidence, not authority. Sentinel formats — ✅ Ground "
        "entered — prose rung begins [gate: first content emitted in this response; no artifact, code, prose, or reasoning may precede this sentinel — preceding content voids the "
        "session]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in current response; rung table precedes this sentinel]; 🔴 Execution "
        "observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] [gate: tool call made in the current response immediately before this sentinel; "
        "verbatim output in triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed with non-empty failing output precedes this token in current "
        "response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: exec_observed showing test suite failure + gap in current "
        ...
        "showing test suite failure + gap in current cycle; valid only at the rung whose artifact type is executable implementation; every assertion in the validation artifact must "
        "appear failing in the verbatim exec_observed output (e.g. 'Expected … Received …', 'AssertionError', 'FAILED', specific test name + failure reason) — an exec_observed that "
        "shows only some assertions failing, only infrastructure failure, import error, process crash, or a model-described summary of failure does not satisfy this gate; if the suite "
        ...
        "assertion-level failure output for every assertion]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one criterion in this rung's artifact; criterion contains no "
        "conjunction; formal notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation artifact V complete [gate: this sentinel may only be emitted "
        "after a tool call in this response has written the validation file to disk; emitting this sentinel without a preceding tool call that wrote the file is a fabrication that "
        "voids the sentinel and the rung; the tool call output must show the file was successfully written]; ✅ Thread N complete [gate: meta exec_observed after executable "
        "implementation shows the gap declared for this thread is no longer present in running behavior; every assertion in the validation artifact must appear passing in this "
        "exec_observed output]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; Carry-forward: [list which "
        ...
        "edit; skipping the pair for subsequent edits is a protocol violation]; 🟢 Protocol derived [gate: derived protocol rules cited P1-P6 must precede this sentinel; must be emitted "
        "before ladder derivation]; 🟢 Ladder derived [gate: rung table must precede this sentinel; must be emitted after protocol derived and before descending].",
