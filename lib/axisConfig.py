...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "composed code blocks, or inline expansions) counts as permission to advance. Before each rung descent output '🔴 Execution observed: [verbatim tool output — content composed "
        "without running the tool is invalid]' then '🔴 Gap: [what the verbatim output reveals — gaps inferred from reading source or test descriptions without running the tool are "
        "invalid]' on their own lines — these sentinels gate the immediately following rung only; they do not permit skipping intervening rungs; a rung manifest, verbal description, or "
        "code block containing a command and its expected output is never a valid Execution observed — only output received as a distinct tool call result in the conversation satisfies "
        "R3; for executable rungs, observation means invoking the artifact as a running process and recording the terminal output — writing a test file is not observation; the test "
        "runner must be invoked; required regardless of whether tests pass or fail; if tests do not run, fix the rung before descending; no file-creating or file-editing tool call for "
        "an implementation rung may be invoked before these sentinels appear in output; if any implementation artifact is produced without valid Execution observed + Gap sentinels "
        "immediately preceding it, discard the implementation and run the tool first. Traversal (R5): depth-first by thread; within a thread, advance through every feasible rung; "
