...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "regardless of intent — a tool-result block in this transcript in which (a) precedes (c) and the failure line contains a substring from the governed assertion call expression "
        "is the only evidence that the artifact encodes the governed behavior. The response establishes, before any governed artifact-producing action, that the named governing "
        "artifact has been observed to detect the absence of each governed behavior. A governed artifact-producing action is any action that produces or modifies a persistent artifact "
        "— including file writes, document edits, configuration changes, and policy commits — whose absence of a governed behavior can be detected by a named executor invocation; an "
        "action whose output does not persist beyond the current session, or for which no named executor exists, is not a governed artifact-producing action. A named governing "
        "artifact is one whose file path — the path passed as an argument to the executor invocation, not a package name or directory — appears as a literal string in the named "
        ...
        "governed symbol: the literal string naming the governed behavior, which must appear as a substring of the failure line in (g); (e) the layer classification — if the governed "
        ...
        "implementation would produce the same assertion target value if their outputs serialize identically. When the assertion's direct argument is an attribute access rather than a "
        "call expression: attribute accesses that return a whole-object text representation are serialization values and do not satisfy (f2); attribute accesses that return a named "
        "behavioral property of the governed behavior are behavioral values and satisfy (f2). The Falsify derivation block must contain the literal line '(f2) serialization gate: "
        ...
        "substring; a failure line that contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) in its output. Exception: (g) is not required for the "
        ...
        "established; unguarded behaviors require separate governing FAILs in subsequent steps and must not be treated as a blocking condition on the current action. Artifact gap: "
        "when the Falsify derivation block contains the line '(g) observed FAIL result: unavailable' and no tool-result block in this transcript produced by a named executor "
        "invocation contains (c) as a substring, the derivation is incomplete. A derivation is complete only when a line beginning 'Gap artifact:' appears in the transcript "
        ...
        "tool call that produces that artifact satisfies the creation-step exception; no file-modifying tool call may appear before the tool call that produces the named artifact.",
