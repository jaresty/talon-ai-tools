...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "conversation: [ ] How to handle files: what creates vs edits vs justifies at each rung [ ] How to handle the ladder: what each rung does and when to move to next [ ] How to "
        "verify gap closure: when to check and when to emit completion This checklist must appear BEFORE the first ladder rung begins. The derivation results must be in the chat, not "
        "just in your head. derive: Sentinels from recursive intent structure (declare intent → derive micro-intents → verify with evidence → emit completion sentinel; this applies at "
        "session, thread, rung, and file-edit levels), Derive file-edit justification: every file edit must serve the intent of its rung; explain how each file satisfies the current "
        "rung's requirements; make the reasoning VISIBLE in the conversation as evidence of derivation from intent (P1), not arbitrary action; justification is evidence, not just "
        "documentation; if you cannot explain why a file change satisfies the current rung, do NOT make that change. Session observation loop (when to observe vs descend, what "
        "observation means vs does NOT mean), ladder derivation format (table columns, columns meaning), rung-specific behaviors (when to emit each sentinel, gate blocks sentinel "
        "emission until gate satisfied), rung transitions (emit rung label when beginning each rung; Outputting a rung label is what begins that rung, not a heading or annotation; emit "
        "completion sentinel when finishing each rung; emit rung label when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return "
        "upward), rung completion order (each rung must produce its artifact before emitting completion sentinel; sequential descent through prose → criteria → formal notation → "
        ...
        "create STUB implementation files for EVERY component planned — STUB means EMPTY function that returns null, like `export default function EditDialog() { return null; }`; STUB "
        "is NOT implementation — it is literally no code; tests are EMPTY and pass trivially because they have no code; this rung establishes that tests can run and all components have "
        "EMPTY stubs. Assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung — FAILING tests means NOT DONE; fix issues at validation rung "
        "BEFORE moving to assertion rung. Assertion rung: EDIT existing test file to add ONE specific expect() assertion; run tests to see it FAIL (red). AFTER making assertion green "
        ...
        "BEFORE emitting the next rung's start label; one rung at a time; completion before continuation. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file "
        "to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO file creation in implementation rung, only edit existing stubs; implement just enough to "
        ...
        "this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, return UP to "
        "find next gap (ascend ladder); if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit "
        "completion sentinel. ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. Only test assertions with expect() count as assertions. "
        "Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; Every rung "
        "must emit its label BEFORE producing its artifact and emit its completion sentinel AFTER. No implementation is permitted before the assertion rung shows red assertions AND the "
