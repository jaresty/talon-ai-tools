...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the first, at least one specific outcome that would satisfy the previous rung but be rejected by this rung, naming both (a) the specific artifact state that satisfies the "
        "previous rung's satisfaction condition and (b) the behavioral dimension that state fails to constrain; a rejects entry that names only a dimension label or boolean property "
        "without naming a specific satisfying artifact state is not valid. If no such outcome can be named, the proposed rung is not a refinement — it is a parallel task and must be "
