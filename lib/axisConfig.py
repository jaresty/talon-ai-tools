...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the chosen interpretation falsifies the property, followed by re-running the formality check on the weakened definition; or '§ minimality: property [N]: minimal — no "
        "admissible construction satisfying the interpretation falsifies it' when none does. The minimality check runs on every property without exception — including properties whose "
        "primary constraint appears obviously general, properties already in formal notation, and properties introduced during the completeness phase or produced by a merge; no "
        "property may be exempted on the ground that it is evidently minimal. A property that reaches Step 2 without an emitted '§ minimality:' line for that property, in one of these "
        "two forms, does not satisfy this protocol; if the '§ minimality:' line for a property has not appeared, emit it before introducing any provisional sub-property. Step 2 — "
        "Introduce provisional sub-properties. Introduce provisional candidate sub-properties whose conjunction is proposed to represent the current property: 'property [Na]: "
