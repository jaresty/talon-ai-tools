...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "representation as its primary subject unless the chosen interpretation explicitly requires that subject. Admission principle: a property is admissible into the Ground property "
        "set only if it constrains the state of the requested artifact as that artifact is observed at the accessible-observation boundary — the condition the delivering artifact must "
        "satisfy for the party the request is for to consider the request met — and its truth-value remains invariant as the production, delivery, and verification process is varied "
        "while the observed delivered artifact is held fixed. This admission test is stated as what a property must be, not what it must not be: run it by holding the observed "
