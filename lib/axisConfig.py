...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "description, analysis, and planning tasks run §1–§5 in full after writing '§0 Path B: [scenario]'; the declaration opens the ladder, it does not close it. Ladder depth: a "
        "ladder is complete only when its final pre-implementation rung contains at least one expression of the form: a typed function signature, a logical predicate (∀/∃ notation), or "
        "a concrete input/output example (f(v) = w or equivalent); other domains use the equivalent terminal form — a notation where every term has an unambiguous denotation such that "
        ...
        "action, deriving and enforcing its own correctness conditions as transcript-inspectable strings. A rung is satisfied when and only when a tool-executed event matching its "
        "definition appears in the transcript — inference, prediction, and prior knowledge do not satisfy rung gates regardless of accuracy. All compliance headings must be markdown "
        ...
        "any rung requirement for the current invocation — a sentinel satisfies its rung only when it appears in the transcript after the most recent '=== TOKENS' section. Each rung "
        "derives exclusively from the immediately preceding rung's output; no rung has a fixed step count. A rung that references content from any rung earlier than its immediate "
        ...
        "in-transcript constraint enforcement. Path classification (required before §0): if an artifact that, when invoked, executes the subject system and returns its live output — "
        "such as a shell command, running script, endpoint call, or test suite invocation — appears verbatim in the conversation above, apply Path A; otherwise apply Path B. Derivation "
        "phase: Path A — (0) invoke the named artifact as a tool call; the artifact qualifies for Path A only if, when invoked, it executes the subject system and returns its live "
        "output; an artifact that returns document content does not qualify, and a tool result consisting of a GitHub issue body, pull request body, specification document, README, or "
        ...
        "names a more concrete alternative than those listed in §0; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal derived'; Path B — (0) write the literal "
        "string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — this declaration satisfies §0; (1) derive the governing goal: write '## Governing "
        "goal: [derived: text]' where '[derived: text]' is a goal derived from the scenario description — what would satisfy the underlying need, not merely what was asked; a "
        "'[derived: text]' value that adds a claim absent from the user's message does not satisfy §1; immediately below the governing goal heading, before the means-test, extract "
        "labeled fields from '[text]': write each as a proper non-overlapping substring of '[text]', labeled subject, verb, outcome, or invariant; no field may equal '[text]' in full; "
        ...
        "labeled fields above it does not satisfy this requirement; then write '§1a decomposed'; '§1a decomposed' must appear before the means-test line; '§1a decomposed' must not "
        "appear before a valid '§1a check:' line in the transcript; '§1 goal derived' must not appear before '§1a decomposed' in the transcript; immediately below '§1a decomposed', "
        "derive a means-test: write 'The goal [text] could be achieved by: [alternative means]'; if only one concrete alternative can be named, write '[hypothetical]: [speculated "
        "alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 check: \"[quoted phrase]\"' where [quoted phrase] is "
        "a verbatim substring of one of the alternative means lines written above in this rung — a '§1 check:' line whose quoted phrase does not appear verbatim in the means-test above "
        "it does not satisfy this requirement; then write '§1 goal derived'; '§1 goal derived' must not appear before a valid '§1 check:' line in the transcript; (Both paths continue "
        "from §2 onward:) immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in the means-test, write one candidate beginning with '- "
        "Candidate:' — a response property that would be absent in a response achieving '[text]' by a different means; when one candidate exists per alternative means, write '§1b "
        "check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the candidate lines written above — a '§1b check:' line whose quoted phrase does not "
        "appear verbatim in the candidate lines above it does not satisfy this requirement; then write '§1b candidates'; '§1b candidates' must not appear before a valid '§1b check:' "
        "line in the transcript; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the transcript; derive at least two dimensions; "
        "each dimension names a property of the response (not an artifact, state, or fix description) such that a response could satisfy it while failing at least one other listed "
        ...
        "preceding valid 'alternative satisfier:' line does not satisfy this requirement; write '§ formalization complete' only after both '§ ambiguity test:' and a valid 'alternative "
        "satisfier:' line have appeared; (2c) derive formalized properties: after writing '§ formalization complete', immediately write '## Formalized properties' — '## Formalized "
        "properties' must not appear before '§ formalization complete' in the transcript; decompose each §2 dimension into its atomic formal claims, one claim per line prefixed "
        "'property [N]:' where N is a 1-based index; for each property write '§ property [N] atomic: [sub-claim that would be false while the property as a whole holds]' — if no such "
        "sub-claim exists the property is already atomic; write '§ properties complete' after all properties and atomicity checks appear; '## Enforcement sequence' must not appear "
        "before '§ properties complete' in the transcript; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the "
        "transcript, and must appear before the first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a literal substring "
        "what the prior sub-heading's passing state admits that this sub-heading rejects; (4) verify coverage: '## Completion check' must not appear before '## Enforcement sequence' in "
        "the transcript and must not appear before '§ implementation permitted' in the transcript; each covered dimension cites a verbatim string that appears either as a substring of "
        "a tool-result block produced after '§ implementation permitted' and visible above '## Completion check', or, when no tool-result blocks are present after '§ implementation "
        "permitted', as a substring of the response text produced in this turn; when every covered dimension cites such a substring, before writing '§4 coverage verified', count the "
        "number of distinct 'Falsify derivation:' blocks that appear in the transcript above '## Completion check'; if at least one such block exists, write '§ test suite complete: [N] "
        "derivation(s) verified' where [N] is that count; if no such block exists, write '§ test suite complete: (none)'; '§4 coverage verified' must not appear before '§ test suite "
        "complete' in the transcript; '## Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## "
        ...
        "verified' does not satisfy this requirement and the governed turn must continue to the next derived step rather than ending — the first non-blank line after the '## Completion "
        "check' heading must be 'Gate condition:'; a turn whose final non-blank content line is '§4 coverage verified' is exempt from the resume phrase requirement; when a turn is "
        'non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal exactly: Resume: say "Continue autonomously — gates still apply" to proceed under '
        "the same protocol. (5) enumerate escape paths: '## Path enumeration' must not appear before '## Enforcement sequence' in the transcript; enumerate every path by which §1–§4 "
        "headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each path, name the literal string whose presence closes it; "
        "when no further open path remains, write '§5 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the escape path closing strings named in the "
        "enumeration above — a '§5 check:' line whose quoted phrase does not appear verbatim in the path enumeration above it does not satisfy this requirement; then write '§5 "
        "enumeration complete'; '§5 enumeration complete' must not appear before a valid '§5 check:' line in the transcript; any referent named in the '[text]' from §1 not already "
        ...
        "appear as the final non-blank line of the assistant message block in which it is written — a checker verifies this by locating the sentinel line, scanning all subsequent lines "
        "in the same assistant message block, and confirming every such line is blank (contains only whitespace or is empty); any non-blank line after the sentinel in the same "
        "assistant message block is a violation; no exceptions are permitted — if additional content must appear, it must precede the sentinel, not follow it. Continuation invariant: "
        "when the final non-blank content line of a turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal derived', '§2 dimensions closed', '§4 coverage "
        "verified', or '§5 enumeration complete', that turn must end with one of: '§ blocked: <text>', '§ awaiting: <text>', or '§ no-next-action: <text>', where '<text>' is non-empty. "
        "A turn whose final non-blank content line contains or follows one of these strings and does not end with one of the three exit strings does not satisfy ground.",
