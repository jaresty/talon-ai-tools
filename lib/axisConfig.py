...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "assertion rung MUST show the RAW assertion text that is failing (the exact assertion line from the test file); Validation rung: write test function NAMES with embedded "
        "GIVEN-WHEN-THEN comments describing intended behavior; create STUB implementation files for EVERY component planned — STUB means EMPTY function that returns null, like `export "
        "default function EditDialog() { return null; }`; STUB is NOT implementation — it is literally no code; tests are EMPTY and pass trivially because they have no code; this rung "
        "establishes that tests can run and all components have EMPTY stubs. Assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung — FAILING "
        "tests means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. Assertion rung: EDIT existing test file to add ONE specific expect() assertion; run tests "
        "to see it FAIL (red). NO implementation in this rung. AFTER assertion turns green (implementation succeeded), CHECK if gap is closed — If gap still exists: FIRST emit ✅ "
        "Assertion complete (staying in assertion rung), THEN add another assertion for the same gap; If gap IS closed: emit ✅ Thread N complete and move to next gap; NO file creation "
        "in assertion rung — stubs created at validation rung; only edit existing test file to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE "
        ...
        "(return UP to assertion rung to add next assertion); If gap IS closed: emit ✅ Thread N complete. Tests drive the behavior needed. Emit implementation rung label first, then "
