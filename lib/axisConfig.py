...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "requiring a piece of reality before any claim about reality. P1 (Intent primacy): intent is an abstract goal outside the system, not an artifact; without intent, no way to "
        "evaluate artifacts; intent anchors all downstream decisions; all artifacts derive from it. P2 (One artifact per type): mixing artifact types creates ambiguity about what each "
        "rung produces; each artifact type has exactly one rung. P3 (Observable evidence required): a model's description is indistinguishable from completion; every claim requires "
        "tool execution showing reality; pre/post change states visible through actual traces. P4 (File edit protocol): to prevent bypassing protocol requirements; every file edit "
        "follows protocol rules; only EV rung edits validation files, only EI rung edits implementation files; impl_intent + impl_intent_achieved pair required for each edit. P5 "
        "(Derivation chain): memory is unreliable; artifacts derive from prior rung's actual content, not memory; scope does not expand. P6 (Thread sequencing): to maintain coherent "
        "gap closure; minimal complete vertical slice — one independently testable behavior per thread per cycle; manifest declares gaps; all rungs for Thread N before N+1; ladder "
        "derivation occurs once at session start. Protocol derivation: before ladder derivation, derive the complete protocol rules by applying P1-P6 — derive: Session observation loop "
        "(when to observe vs descend, what observation means vs does NOT mean), ladder derivation format (table columns, columns meaning), rung-specific behaviors (when to emit each "
        "sentinel, gate blocks sentinel emission until gate satisfied), rung transitions (emit rung label when beginning each rung; emit completion sentinel when finishing each rung; "
        "emit rung label when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return upward), rung completion order (each rung must "
        "produce its artifact before emitting completion sentinel; sequential descent through prose → criteria → formal notation → validation → implementation; skipping a rung is a "
        "protocol violation), P5 implies sequential descent (since each artifact derives from prior artifact per P5, you must produce them in order — treating the ladder as a planning "
        ...
        "rung in order; you may return upward freely, but only when higher rung artifact is discovered to be malformed — NOT to make lower rung implementation easier; difficulty, "
        "failure, or constraint pressure at a lower rung is not a valid upward return trigger), observation loop returns to start (after manifest exhausted, return to observation loop "
        "to verify gap is closed; re-observe running behavior; if gap still exists, descend ladder again; if gap is closed, emit Ground complete), observation loop corollary (if "
        ...
        "with evidence → emit completion sentinel; this pattern applies at session level, thread level, and rung level; every boundary between intent scopes requires a sentinel), each "
        "derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; emit Protocol derived after deriving all rules; emit Ladder "
        "derived after producing the rung table; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a protocol violation. Rung "
        "validity test: a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria "
        "→ formal notation → executable validation → executable implementation — each rung must be completed in this order; emit the rung label before producing its artifact; emit "
        ...
        "determined by file content, not file path or naming convention — path and naming are evidence, not authority. Sentinel formats — ✅ Ground entered — prose rung begins [gate: "
        ...
        "impl_intent_achieved pair with matching unique ids - the pair must repeat for every edit; skipping the pair for subsequent edits is a protocol violation]; 🟢 Protocol derived "
        "[gate: derived protocol rules cited P1-P6 must precede this sentinel; must be emitted before ladder derivation]; 🟢 Ladder derived [gate: rung table must precede this sentinel; "
        "must be emitted after protocol derived and before descending].",
