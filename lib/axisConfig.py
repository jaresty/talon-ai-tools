...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "return trigger; GAP CYCLE RULE: for EACH gap, you MUST follow this sequence: (1) assertion rung: add failing assertion for the specific gap; (2) implementation rung: implement "
        "ONLY enough to make that specific assertion green; (3) verify assertion is green; (4) emit ✅ Implementation complete; (5) if OTHER gaps remain, return to assertion rung (NOT "
        "implementation) to add next failing assertion; (6) repeat until no gaps remain; NEVER implement multiple gaps in sequence without returning to assertion between them; batching "
        "implementations violates protocol; the sentinel 'Implementation complete' means the current gap is done, not that the entire task is done), upward gap scan (ALWAYS after "
        "completing the final rung (implementation), scan upward to verify all criteria are addressed; this is NOT optional — even if you believe no gaps remain, you MUST produce the "
        "checklist to prove verification; scan upward to find the nearest rung above that still has a gap to close; return to that rung rather than restarting from the top; when "
        "returning upward, FIRST emit completion sentinel for current rung, THEN emit the rung label for the rung you are returning to — you cannot be at two rungs at once; continue "
        "descending from that rung to close the remaining gap; after all gaps are closed, return to the observation loop above the ladder to verify closure and emit Ground complete; "
        "UPWARD SCAN MUST examine ALL prior rungs (prose, criteria, formal notation, validation) for unaddressed criteria — not just the immediately prior rung; each criterion from "
        "earlier rungs must have corresponding assertions and implementation; if criteria from prose/criteria/formal notation rungs are not satisfied by assertions+implementation, the "
        "gap remains open and must be closed before emitting Ground complete; UPWARD SCAN MUST produce an explicit checklist with each criterion marked: ✅ addressed or ❌ gap remains — "
        "missing this checklist is a protocol violation), observation loop corollary (if opening observation shows no gap exists, emit Ground complete immediately — no ladder descent "
        "needed), upward return conditions (what triggers return, what does NOT trigger return and why excluded), evidential scoping (evidence is scoped to current cycle and gap, not "
        "prior cycles), scope preservation (scope does not expand between rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); evidence "
        "over theory (a model's description of what it would do is indistinguishable from having done it — only visible execution output counts as evidence; every behavioral change and "
        "execution must be observable in the log, not just described; trust evidence not theory), recursive intent structure (P1 intent primacy implies: declare intent → derive more "
        "specific micro-intents → verify each micro-intent with evidence → emit completion sentinel; this pattern applies at session level, thread level, and rung level; every boundary "
        "between intent scopes requires a sentinel), smallest intent scale: individual test assertions — each assertion in a validation file is an intent declaration; seeing it red "
        "reveals the gap; implementing to make it green closes the gap; intent remains intact only if assertions remain green; regression = any assertion that was green becomes red; "
        "failure to see the assertion red means the gap has not been observed and implementation cannot proceed — for backfill cases where tests already pass, perturb the "
        "implementation to observe failure, then fix the perturbation; each assertion is its own sub-rung: the gap is the red assertion, the solution is making it green; the assertion "
        "itself must be printed RAW in the transcript — only the surrounding test context may be paraphrased), each derived rule must cite its source principle(s) from P1-P6; rules "
        "that cannot be derived are protocol violations; Protocol derived MUST be emitted as the exact sentinel: 🔵 Protocol derived — and MUST be emitted AFTER deriving all protocol "
        "rules from P1-P6 — omitting derivation or emitting before derivation is complete voids the session; Ladder derived MUST be emitted as the exact sentinel: 🔵 Ladder derived — "
        "and MUST be emitted AFTER producing the rung table — the rung table MUST have columns: rung name | artifact type | gate condition | void condition | faithfulness test | "
        "permitted tool calls; the table is NOT narrative text; No ladder descent (prose, criteria, etc.) is permitted before both 🔵 Protocol derived and 🔵 Ladder derived sentinels — "
        "these are gates on all subsequent work; After validation rung, you MUST descend to the assertion rung — validation shows tests exist, assertion rung shows which specific "
        "assertions are red; the assertion rung is NOT optional; the assertion rung MUST show the RAW assertion text that is failing (the exact assertion line from the test file); "
        "Validation rung: create minimal validation artifacts that establish scripts can run and components have EMPTY stubs; preference order for validation: (1) automated checks "
        "(existing or new tests/scripts that run automatically), (2) one-off scripts, (3) manual checks; follow codebase conventions for file creation vs editing existing files — "
        "prefer extending existing test files where they exist; create STUB implementation files for EVERY component planned — STUB means EMPTY function that returns null; tests are "
        "EMPTY and pass trivially; this rung establishes that scripts can run and all components have EMPTY stubs. Assertions are written at the assertion rung, not validation. Scripts "
        "MUST PASS at validation rung — FAILING scripts means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. At validation rung, you MUST explicitly list all "
        "files being created in the conversation — this is your file manifest. Every implementation stub created at validation rung MUST have a corresponding test file created at "
        "validation rung — the test file validates that implementation; list both implementation files AND their test files in the validation file manifest. Assertion rung: EDIT "
        "existing script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL (red). NO implementation in this rung. NO file creation — edit "
        "existing test files only; before editing, declare 'no new files' as verification that no new files are being created; AFTER check turns green (implementation succeeded), CHECK "
        "if gap is closed — If gap still exists: FIRST emit ✅ Assertion complete (staying in assertion rung), THEN add another assertion for the same gap; the completion sentinel MUST "
        "explicitly declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied] — evidence: [for each unsatisfied criterion, cite the RAW failing "
        "assertion text]; If gap IS closed: emit ✅ Thread N complete — all gaps closed; move to next gap; NO file creation in assertion rung — edit existing test files if they exist; "
        "stubs created at validation rung; only edit existing scripts to add assertions. Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's "
        "start label; one rung at a time; completion before continuation. Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY "
        "NEEDED to make that specific assertion green — NO assertions added here; NO file creation; only edit existing stubs; before editing, declare 'no new files' as verification "
        "that no new files are being created; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the "
        ...
        "sentinel MUST explicitly declare: ✅ Implementation complete — gaps remaining: [list which prose/criteria/formal notation criteria are NOT yet satisfied] — evidence of gap "
        "verification: [reference the specific criteria checked, or if gaps listed, explain why return upward is needed]; if gap IS closed: emit ✅ Thread N complete — all gaps closed. "
        "If gap IS closed: emit ✅ Thread N complete — all gaps closed. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion "
        "sentinel. ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. Only test assertions with expect() count as assertions. Rung "
        "completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; Every rung must "
        "emit its label BEFORE producing its artifact and emit its completion sentinel AFTER. No implementation is permitted before the assertion rung shows red assertions AND the "
        "specific RAW assertion text is displayed; the exec_observed sentinel must include the verbatim failing assertion text; Evidence before claims: for every claim about code being "
        "correct, there must be tool execution output proving it; describing what you would do is not evidence; ground complete may only be emitted as the outcome of the observation "
        "loop — emitting it outside the loop is a protocol violation. Rung validity test: a rung is valid iff a human reviewer with only that rung's artifact can evaluate the next "
        "rung. Standard ladder for software behavioral change: prose → criteria → formal notation → validation → assertion → implementation — the assertion rung is REQUIRED in this "
        "order; the assertion rung verifies each individual assertion red → green; emit the rung label before producing its artifact; emit completion sentinel after artifact is "
        "produced and gate is satisfied. EV rung: edits only validation files. EI rung: edits only implementation files. An executable validation artifact is a file whose sole purpose "
        "is to assert behavioral properties of another artifact — it contains no behavior of its own; it must be invocable by an automated tool; validation files may not be imported by "
