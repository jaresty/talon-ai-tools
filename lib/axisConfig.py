...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "falsify": "The response observes each gap between intent and current state before it is closed, and leaves a mechanism that will detect regression without human initiation. Test and "
        "implementation may be written in any order; falsify does not constrain their sequence. Correctness is established retrospectively by two gates over the established guards. "
        "Before acting, observe the gap: emit 'Observing gap: <what is currently absent or failing>'. A guard is established by a tool call whose result block contains the guard's "
        "executable text verbatim; every assertion referenced later must be copied verbatim from such a result block. A Failure observation is valid only when it is produced by "
        "executing a committed, re-runnable guard artifact — a persisted test or check that re-runs without human initiation — never from an inline or ephemeral command; an inline "
        "shell command, a one-off script, or any guard not committed to a location that re-runs on its own cannot produce a valid Failure observation, because it is absent on the next "
        "run while the property still holds and so cannot detect a later regression. Gate 1 — Minimization. Construct a minimal modification to the governed artifact that preserves "
        "the success of every established guard while still changing the artifact, and construct a concrete execution path through the artifact not exercised or distinguished by any "
        "guard. If either exists, emit 'Overreach: found — <modification or path>', prove it by perturbing whatever is necessary — implementation, input, or state — and observing in a "
        "tool-result block that a guard's outcome is unchanged, then trim the artifact to the boundary the guards require and repeat. When neither exists, emit 'Overreach: not found'. "
        "Gate 1 reasons only from the guard text and artifact text present verbatim in the transcript, never from an intended version. Gate 3 — Observed failure. Before observing any "
        "failure, enumerate the executable assertions: emit one 'Assertion:' line for each executable assertion of the established guard, each quoted verbatim from the established "
        "guard's tool-result — 'Assertion: <verbatim assertion text>'. This enumeration makes the assertion set directly checkable; it does not carry a count or completeness sentinel, "
        "and a self-declared count does not satisfy it — the verbatim lines themselves are the observable. For every executable assertion in the established guards, if it has not "
        "already been observed failing in a prior tool-result block, construct an admissible state in which the property it guards is genuinely absent — perturbing whatever is "
        "necessary, a violating state preferred over a violating stub — and execute the guard against that state via a tool call. A Failure observation for an assertion is valid only "
        "when the state satisfies all of: the governed symbol is present and executes; that specific assertion's property is violated; every other retained assertion's property could "
        "simultaneously hold; and the observed failure cause is attributable to this assertion alone, distinct from the cause observed for any other assertion. This is the positive "
        "test each assertion must pass — a failure that does not meet it (including one in which the same observed cause is shared across more than one assertion, so it is not "
        "attributable to any one of them) does not witness that assertion and does not satisfy Gate 3, and each assertion must be so discriminated before Coverage: complete. When tool "
        "calls are available, the 'Failure: assertion' observation must immediately follow the tool-result of executing the guard against the constructed present-but-wrong state — the "
        "execution that discriminates this assertion, not merely any prior tool-result. A successful compilation, a build succeeding, an unrelated verification, or a whole-symbol or "
        "undefined error does not satisfy Gate 3 for that assertion, because such an outcome fails identically regardless of which property is absent; the violating state must exhibit "
        "the governed symbol present and executing, failing solely to witness that this assertion's property is absent. If the governed artifact already satisfies the property, "
        "construct a temporary violating modification to the artifact, execute the guard against it, emit the Failure observation from that tool-result, then restore the original "
        "state. For every executable assertion, the observed tool-result must produce an outcome attributable to that assertion alone — identifying that assertion specifically and a "
        "failure cause distinct from every other assertion's. A whole-guard execution that does not yield per-assertion outcomes is insufficient regardless of whether the model "
        "recognizes the insufficiency; the sufficiency is judged from the tool-result, not from the model's assessment of it. A guard whose framework already reports per-assertion "
        "results satisfies this directly; when the ordinary execution does not yield per-assertion outcomes, any execution mechanism that does — for example a temporary observation "
        "harness executing each enumerated assertion — may be used, and each Failure observation is derived from that tool-result. A guard is a blind spot whenever an admissible state "
        "exists in which its pass-or-fail outcome is not equivalent to whether the property actually holds, in either direction: the guard passes while the property is absent, or the "
        "guard fails or is absent while the property holds. Three named constructions must each be checked as instances of this: (a) proxy or non-exercising guard — the guard checks a "
        "surrogate rather than executing the governed behavior, so it can pass while the property is absent; (b) durability — a guard less durable than the property it governs, gone "
        "at the moment it would need to fire; (c) ephemeral guard — a guard not committed to a location that re-runs without human initiation. Emit 'Failure: assertion \"<verbatim "
        'assertion>" — "<verbatim failure text from the tool-result>"\' for each observed failure, derived exclusively from the tool-result. An assertion that no admissible '
        "perturbation can make fail is a guard defect: strengthen the guard and repeat. An assertion may be recorded 'Unobservable: assertion \"<verbatim assertion>\" — structural' "
        "only when the property it guards has the artifact's own text or structure as its primary subject, so that no behavioral violating state exists; Unobservable is never "
        "satisfied by having searched for a violating state and found none — the justification is the structural nature of the guarded property, not the failure of a search. "
        "Verdict-follows-execution: when tool calls are available, each gate verdict — 'Overreach: found'/'Overreach: not found' and each 'Failure:' observation — is valid only when "
        "it immediately follows a tool-result block that mechanically produces it: the 'Overreach:' verdict must follow an executed probe that ran the modified artifact against the "
        "guard, and the 'Failure:' observation must follow an executed run of the guard against the constructed violating state. A verdict emitted from description or analysis alone — "
        "reasoning about what a probe or run would show, without a preceding tool-result block containing that result — does not satisfy this token; a structural Unobservable "
        "assertion is exempt, because a property about the artifact's own text or structure has no execution to anchor to. 'Coverage: complete' is valid only when every executable "
        "assertion of every established guard has either an observed 'Failure:' line witnessed by a verbatim tool-result excerpt or a 'Unobservable: … — structural' line, and no "
        "'Overreach: found' remains open.",
