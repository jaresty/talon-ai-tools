...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "scope: a sentinel satisfies its rung only when it appears in the current response — a sentinel from a prior response does not satisfy any rung in this response. Each "
        "derivation-phase rung (§0 through § properties complete) derives exclusively from the immediately preceding rung's output; no rung has a fixed step count. A derivation-phase "
        "rung that references content from any rung earlier than its immediate predecessor does not satisfy this requirement — isolation is enforced at each derivation step; backward "
        "reference to earlier derivation rungs is not permitted even when the content appears in the transcript. Verification phases (## Enforcement sequence, ## Path enumeration, ## "
        "Completion check) are not derivation rungs — they are permitted and required to reference earlier derivation outputs by property number or sentinel name. When the execution "
        "environment supports literal isolation — a fresh invocation, a separate agent context, or a new session — prefer actual isolation over in-transcript constraint enforcement. "
        "Derivation phase: (0) '§0: [scenario]' is the opening sentinel of the derivation, where [scenario] names the subject from the conversation — the first sentinel appearing in "
        "the response must be '§0:'; a response whose first sentinel is not '§0:' does not satisfy this requirement; when tools are available in this execution environment, invoke the "
        "governing test artifact as a tool call immediately after writing '§0: [scenario]' — '§0 observed' is required when tools are available and is not optional; after that "
        "tool-result block appears and reflects live system execution, immediately write '§0 observed' — a tool-result block produced by invoking the named artifact as a tool call "
        "satisfies this requirement; a result produced by a Read, search, or file-write tool call does not; an artifact that returns document content does not qualify — a tool result "
        "consisting of a GitHub issue body, pull request body, specification document, README, or requirements text does not satisfy §0 regardless of invocation form; goal-source "
        "classification (required when '§0 observed' is present): examine whether any line in the §0 tool-result block appears verbatim as a substring of any user message visible above "
        "§0 in the transcript; if no such line exists, write '§0 status-only'; after '§0 status-only', fetch a broader context artifact before writing '## Governing goal:' — the fetch "
        "tool-result block must contain at least one line that appears as a substring of a user message above it in the transcript; a tool-result block whose every line is a substring "
        "of content already present in the transcript above it does not satisfy this requirement; (1) derive the governing goal: '## Governing goal:' is the first '## ' heading in the "
        "transcript after '§0:' — a '## ' heading other than '## Governing goal:' appearing first after '§0:' does not satisfy this requirement; if '§0 observed' is present, '[text]' "
