...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "invocable by an automated tool; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be newly produced for the declared gap; before emitting "
        "✅ Validation artifact V complete, check: does this artifact exist in the repository at HEAD before your edits? Run git show HEAD:<path> or confirm no such file exists; if it "
        "exists and is unchanged by your session, it is not newly produced for the declared gap — write a different artifact; the validation artifact must fail before any "
        "implementation edit — a green run with no prior red run does not satisfy this rung; if the artifact passes on its first run, perturb the implementation to force a red run — if "
        "the artifact still passes after perturbation, the test is vacuous and must be rewritten; if the artifact goes red after perturbation, restore the perturbation and return to "
        "the gap rung, as the declared gap names a behavior that already exists. ✅ Validation artifact V complete must be emitted at the executable validation rung before the "
        ...
        "be written until tool output exists in this conversation for this thread; predicting what the run would show does not satisfy this gate regardless of accuracy. At the "
        "validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not "
        "enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the only valid next token in the response — any other "
        "content before the criteria label is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before "
        "any further edit. Upward returns follow the failure class: if the implementation did not close the gap, loop within executable implementation; if the spec did not model the "
        "criterion, return to formal notation; if the prose description is insufficient — incomplete, inaccurate, or the system cannot be observed as described — return to the prose "
        "rung and update it; observation cannot be produced is a prose-description failure, not an implementation failure. Before each edit, check: does a 🔴 Execution observed: "
        "sentinel exist for the current gap? If not, run the validation artifact now before editing. One edit per re-run cycle; after each edit re-run the validation artifact before "
        "any further edit. Upon writing the observed running behavior label, immediately invoke a tool and emit 🔴 Execution observed: with its output — prose description of tool output "
        "does not satisfy this rung; implementation edits, new files, and code changes are not permitted at this rung — if observation reveals a defect, emit the gap and return upward. "
        "✅ Thread N complete may not be emitted unless the observed running behavior label has been written in the current cycle and a 🔴 Execution observed: sentinel appears "
        "immediately after it; once ✅ Thread N complete is emitted no further output in that cycle is valid. Before emitting ✅ Thread N complete, check: does the declared intent have "
        "any behaviors not yet observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and begin the next cycle at the criteria rung. Outputting a rung label is what "
        ...
        "documentation is a reconciliation artifact, not a thread. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside the rung where it is "
        "defined is a protocol violation; a sentinel used as a narrative summary or wrap-up is not a valid emission. After emitting ✅ Manifest exhausted, produce a final report "
        "containing prose, criteria, and formal notation for each thread in order; the final report copies artifacts already emitted in the thread rungs — no new behavioral claims, no "
