...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "why Baseline evaluation must APPLY and PASS to prove it works; what GOES WRONG if you declare complete without applying: no proof evaluation works; RULE: Baseline = apply "
        "evaluation and show passing output as evidence [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create without "
        "structure: unfocused output; RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success conditions before organization; what GOES WRONG if you "
        "organize without criteria: no way to evaluate; RULE: criteria before structure [ ] Intent before criteria: derive from P1 — outcome before success conditions; what GOES WRONG "
        "if you define success without outcome: criteria irrelevant; RULE: intent before criteria [ ] Evidence at proximity: derive from P3 — evidence must be temporally close to "
        ...
        "evaluation structure. Evidence: evaluation structure APPLIED and VERIFIED to work. IMPORTANT: Baseline evaluation must PROVE it works - not just describe it. Applying and "
        "seeing it pass is the evidence. Declaring complete without applying proves nothing. IMPORTANT: Baseline establishes the STRUCTURE for evaluation - this may include minimal "
        "scaffolding to hold criteria. The scaffolding is NOT the solution - it's just where criteria live. IMPORTANT: Baseline evaluation must be SELF-CONTAINED. If it fails due to "
        ...
        "artifact improved? Derivation: from challenge, derive targeted changes. Evidence: addresses specific gaps. 7. Verification — How is completion confirmed? Derivation: from "
        ...
        "sharpshooter). Baseline defines evaluation framework; Challenge applies it to show gaps exist; Refinement produces solution that passes evaluation. for writing tasks, it maps "
        "to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to intent→criteria→analysis→option→stakeholder review→recommendation→decision. The model "
