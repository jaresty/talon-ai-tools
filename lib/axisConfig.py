...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The planning phase is a state machine execution trace, not a summary or explanation. A planning trace is valid only if every state transition is explicitly emitted as a "
        "separate state. Skipping intermediate states is a syntax error, not an abbreviation. During planning, no prose, rationale, or explanation is permitted — any text outside the "
        "trace grammar before 'Planning complete.' is invalid. Node types: Request | InterpretationPair | Interpretation | Selected | Discarded | AmbiguityCheck | FormalismChoice | "
        "UnambiguousRequest | SplitDecision | SplitCandidate | Property | TerminalProperty | PlanningComplete. Type dispatch table — each type has exactly one permitted transition; any "
        "other transition applied to that type is a syntax error: Request → Interpret. InterpretationPair → Expand. Interpretation → Select (if chosen) or Discard (if not chosen). "
        "Selected → GroundCheck. Discarded → Consume. AmbiguityCheck → GroundResolve. FormalismChoice → Formalize. UnambiguousRequest → SplitCheck. SplitDecision → AnalyzeSplit. "
        "SplitCandidate → Materialize. Property → SplitCheck. TerminalProperty → Terminal. PlanningComplete → emit 'Planning complete.' Pending is a FIFO queue. The head of Pending is "
        "the only valid next Active node — processing any other node is a syntax error. Trace grammar — every state has exactly three lines:   Line 1: Active: <id>:<type>    Line 2: "
        "<transition line> — must immediately follow Active:; nothing may appear between them    Line 3: Pending: <id:type list in queue order, or 'empty'>  Any text inside a state "
        "other than these three lines is a syntax error. Any text between states (between a Pending: line and the next Active: line) is a syntax error. Nodes are opaque: during "
        ...
        "language; enqueue U:UnambiguousRequest. SplitCheck(N:UnambiguousRequest or Property): write 'SplitCheck: N → D'; enqueue D:SplitDecision. SplitCheck never produces terminal — "
        "it always produces exactly one SplitDecision. AnalyzeSplit(D:SplitDecision): inspect the expression text of D's source (U.expression for UnambiguousRequest, or the "
        ...
        "T:TerminalProperty. Terminal(T:TerminalProperty): write 'Terminal: T'; enqueue nothing. Materialize(SC:SplitCandidate): write 'Materialize: SC → P'; enqueue P:Property. When "
        "Pending is empty: enqueue PC:PlanningComplete. PlanningComplete: write 'Planning complete.'; enqueue nothing. Task content begins after. Validity: every identifier introduced "
        "exactly once; every Active: names the head of Pending; every state is exactly Active:/transition/Pending: (plus U.expression= or SC.expression= payload lines where specified); "
        "'Planning complete.' only from PlanningComplete; SC1.expression and SC2.expression are contiguous substrings of U.expression or the parent SC.expression.",
