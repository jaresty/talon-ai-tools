...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "assertion flips from red to green, revert and try a different approach. Test code is not subject to this requirement. Outputting a rung label is what begins that rung — it is "
        "not a heading or annotation; a rung whose label has not been output has not begun; the rung label must be emitted before any artifact content for that rung — producing "
        "artifact content before the rung label is a protocol violation that voids that artifact and all rungs below it. When beginning mid-ladder, locate the highest "
        "already-instantiated rung, update it, then descend. Sentinel formats — ✅ Ground entered — prose rung begins [gate: first content emitted in this response; no artifact, code, "
        "prose, or reasoning may precede this sentinel — preceding content voids the session]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table "
        "produced in current response; rung table precedes this sentinel]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] [gate: "
        "tool call made in the current response immediately before this sentinel; verbatim output in triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: "
        "exec_observed with non-empty failing output precedes this token in current response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to "
        "criteria rung [gate: exec_observed showing test suite failure + gap in current cycle; criterion identical to prior cycle criterion for this thread]; 🟢 Implementation gate "
        "cleared — gap cited: [verbatim from 🔴 Execution observed] [gate: exec_observed showing test suite failure + gap in current cycle; valid only at the rung whose artifact type is "
        "executable implementation; every assertion in the validation artifact must appear failing in the verbatim exec_observed output (e.g. 'Expected … Received …', 'AssertionError', "
        "'FAILED', specific test name + failure reason) — an exec_observed that shows only some assertions failing, only infrastructure failure, import error, process crash, or a "
        "model-described summary of failure does not satisfy this gate; if the suite cannot reach assertions, the implementation artifact for that cycle resolves only the "
        "infrastructure gap, and this gate cannot be emitted until a subsequent cycle produces assertion-level failure output for every assertion]; ✅ Criteria complete — 1 criterion: "
        ...
        "meta exec_observed after executable implementation shows the gap declared for this thread is no longer present in running behavior; every assertion in the validation artifact "
        "must appear passing in this exec_observed output]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; "
        ...
        "audit section named and separate]; ✅ Ground complete — intent achieved: [what the observation shows] [gate: 🔵 Closing observation must appear in the current response before "
        "this sentinel; meta exec_observed shows no gap between observed running behavior and declared intent; all manifest threads complete]; 🔵 Closing observation — [what the "
        "tool-executed observation shows] [gate: the tool call immediately before this sentinel must directly invoke the behavior named in the session intent and its output must show "
        "that behavior is present; this sentinel must appear after ✅ Manifest exhausted and before ✅ Ground complete in the same response; emitting ✅ Ground complete without a "
        "preceding 🔵 Closing observation in the current response is a protocol violation]; 🔵 Write authorized — rung: [rung name] | artifact type: [type] | file: [path] [gate: "
        "currently open rung (most recently emitted rung label with no completion sentinel yet) has this file's artifact type in its permitted-tool-calls column; no open rung → gate "
        "unsatisfied; artifact type mismatch → gate unsatisfied; the cited artifact type must match the artifact type of the named file as determined by the rung table's artifact-type "
        "column — a cited artifact type that does not match the file being written is a fabrication that voids the sentinel and the rung regardless of whether the open rung permits "
        "that type; a file write without a preceding 🔵 Write authorized voids the write and the rung in which it appears; a 🔵 Write authorized whose cited rung is not currently open is "
        "a fabrication that voids the write and the rung; this sentinel must appear immediately before the file-write tool call — intervening content between this sentinel and the tool "
        "call voids both].",
