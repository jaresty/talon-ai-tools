...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "layer exists. A passing integration test does not substitute for a missing unit assertion. PHASE 1 — classification (text artifact, required before any process derivation): "
        "produce a classification block for each assertion using this exact structure: Assertion: <name or description> | Principle: <which principle governs it> | Natural FAIL state: "
        "<yes, or no — answer no if the assertion checks that something does NOT occur, because such assertions pass trivially when the behavior is absent> | Coverage artifact: <if "
        "Natural FAIL state is no, describe the specific incorrect behavior to introduce to produce a FAIL tool result; otherwise n/a>. Every assertion must have its own block. Phase 1 "
        "is complete when every assertion has a block. PHASE 2 — coverage artifact (tool result required before implementation): every assertion requires a FAIL tool result before any "
        "implementation step — Principle 1 applies to all assertions without exception. For natural-FAIL-state assertions (Natural FAIL state: yes), the FAIL comes from the initial "
        "red-state test run. For no-FAIL-state assertions (Natural FAIL state: no), the FAIL requires perturbation. Before any implementation step, produce a Phase 2 status line for "
        "every assertion using exactly one of these two forms — COMPLETE: Phase 2 [assertion name] — <verbatim FAIL output from tool result> or NOT YET PRODUCED: Phase 2 [assertion name] "
        "— running now. The NOT YET PRODUCED form names a missing predecessor. The implementation step's correctness criterion is: change the system from the observed wrong state to the "
        "correct state. Without the FAIL tool result, the wrong state has not been observed — the correctness criterion is undefined, not merely unmet. Chain requires every step to "
        "reproduce its predecessor before proceeding, and the predecessor named here does not yet exist as a tool result. The agent derives from this that no implementation step is "
        "definable until the tool call is made and the COMPLETE line can be written from actual output. There is no path between NOT YET PRODUCED and implementation that does not require "
        "fabricating a predecessor — and fabrication is not reproduction. The COMPLETE form requires the quoted FAIL output to be present verbatim in a prior tool result in this "
