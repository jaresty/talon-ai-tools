...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "source that produces output by retrieval rather than by execution; (3) line manifest listing every line to be added, each entry quoting the specific text in the governing "
        "failure signal that names the behavior the line implements. Immediately before each tool call that modifies a file, produce: 'Scope: <verbatim quoted failure signal> | "
        "Candidate: <named candidate change> | Smaller: <named smaller change whose insufficiency is demonstrated above> | Lines: <count of lines in manifest>' — if this line does not "
        "appear immediately above the tool call that modifies a file in the transcript, the edit is not permitted. After the edit, produce a post-edit run result by tool call, then "
        ...
        "check; name what specific test or assertion in that check covers this behavior — the named test must be capable of catching this specific behavior; a description that states "
        "or implies the test does not cover the governed behavior — including qualifications such as 'but not [behavior]', 'does not cover', or 'cannot distinguish' — is self-refuting "
        "and invalidates the nomination regardless of the test's actual capability; name a different test whose description affirms coverage without qualification; a claim that the "
        "named test cannot catch this behavior must be grounded in a prior execution result showing the test passing while the behavior was absent — quote that result; a reasoned "
        "claim of incapability without such a quote is not valid and the slower check stands as derived; if no capable test can be named and no such quote can be produced, the slower "
        "check is not derived and the candidate check must not be named — choose a different candidate for which a nameable test exists; second, name the candidate check — the "
        "specific artifact to run, naming its relative speed compared to alternatives; third, name at least one insufficient check — a faster check that exercises the same triggering "
        "event as the candidate check but at a resolution that does not distinguish the governed behavior from its absence; name the specific shared triggering event and why the "
        "resolution is coarser; a check that does not share the triggering event is irrelevant, not insufficient, and must not be named here; then, before closing this entry, quote "
        "the specific output from a prior run result showing the insufficient check passes when the behavior is absent — the quoted output must be from a prior execution of the named "
        "insufficient check specifically, and the quoted text must contain the check's name, command, or a test identifier from it so the source is verifiable by inspection; a quote "
        "whose text does not identify the named check is not valid — this entry is not complete until that quote appears; if no such prior result exists above this line, the candidate "
        "is not derived and a different check must be chosen. Then run the candidate check. The FAIL must be produced by running an artifact that (a) is written to assert that "
        "specific behavior and (b) executes when the same event triggers evaluation of the artifact being changed — name the triggering event that causes both to execute; if no such "
        "shared trigger can be named, the artifact does not govern this change. A FAIL produced from a state with additional absences beyond the named behavior is not valid. Before "
        "the tool call, name: the specific behavior being removed and at least one other behavior that remains intact in the test state; if omitting less than the named behavior would "
        "produce the same FAIL, the named behavior is not the governed cause and must be rederived. Immediately before each implementation tool call, produce: 'Governing artifact: "
