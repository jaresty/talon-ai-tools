...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "explicit verification before completion [ ] Gap cycle: derive from P1,P3,P5 — one gap at a time ensures clear signal; what GOES WRONG if you batch: unclear what changed; RULE: "
        "challenge → refine → verify → next gap [ ] Baseline: derive from P2,P3 — why baseline creates scaffold not complete solution; what GOES WRONG if you produce full solution at "
        "baseline: no gap to expose in challenge rung, evaluation criteria become vacuous; RULE: baseline = minimal artifact that demonstrates approach, NOT complete solution [ ] "
        ...
        "consistency. 4. Baseline — Does a minimal version exist that demonstrates the approach? Derivation: from structure, derive minimal artifact. Evidence: minimal viable example "
        "that satisfies structure. IMPORTANT: Evaluation criteria are defined BEFORE the full solution — criteria written after are vacuous. 5. Challenge — What checks expose gaps? "
        "Derivation: from baseline, derive evaluation criteria that expose gaps. Evidence: failing checks are evidence of gaps. 6. Refinement — How is the artifact improved? "
        "Derivation: from challenge, derive targeted changes. Evidence: addresses specific gaps. 7. Verification — How is completion confirmed? Derivation: from refinement, derive "
        ...
        "tasks where verification is possible, define evaluation criteria BEFORE producing the complete solution — criteria written AFTER producing the solution are VACUOUS (Texas "
        "sharpshooter fallacy). Baseline creates scaffold; Challenge applies criteria to expose gaps; Refinement produces complete solution that satisfies criteria. for writing tasks, "
        "it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to intent→criteria→analysis→option→stakeholder review→recommendation→decision. The "
