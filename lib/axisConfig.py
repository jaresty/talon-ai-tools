...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "re-run the validation artifact before any further edit. Upon writing the observed running behavior label, immediately invoke a tool and emit 🔴 Execution observed: with its "
        "output — prose description of tool output does not satisfy this rung; invoke the implemented artifact directly — the binary, command, script, or rendered output produced by "
        "the implementation as a user or caller would observe it — not the test suite; running tests at this rung is a protocol violation; if the implementation has no directly "
        "invocable artifact, invoke any consumer of the changed artifact that demonstrates the declared behavior — not the test that gates it; implementation edits, new files, and code "
        "changes are not permitted at this rung — if observation reveals a defect, emit the gap and return upward. ✅ Thread N complete may not be emitted unless the observed running "
        "behavior label has been written in the current cycle and a 🔴 Execution observed: sentinel appears immediately after it; once ✅ Thread N complete is emitted no further output "
        "in that cycle is valid. Before emitting ✅ Thread N complete, check: does the declared intent have any behaviors not yet observed running? If so, emit a new 🔴 Gap: naming the "
        "next unmet behavior and begin the next cycle at the criteria rung. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has "
