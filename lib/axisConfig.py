...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "through every feasible rung; completeness governs rung depth, not rung existence; each rung may not be skipped or combined with another. Before any tool call or file read, "
        "check: if no 🔴 Gap: sentinel exists in output yet, emit it now; before emitting 🟢 Implementation gate cleared, check: does a 🔴 Execution observed: sentinel from the current "
        "cycle's validation run observation rung — not from a prior cycle — exist in output? if not, do not emit it — produce the executable validation artifact instead. Before "
        "beginning each rung, emit 🔴 Gap: naming what the prior rung’s artifact leaves unresolved, then immediately produce the rung artifact — do not pause after the gap sentinel; "
        "only the validation run observation rung stops after naming the gap. From the criteria rung onward, the gap names a specific behavior currently absent or wrong, phrased as a "
        "currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing page” is not; naming an absent artifact is not a valid gap. Each "
        "rung’s artifact addresses only that gap — not all known requirements of the task — and is minimal: one criterion (one independently testable behavior, not a conjunction of "
        "behaviors joined with ‘and’ — ‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one), one notation, one test, one edit; further "
        ...
        "anticipated cases. Only validation artifacts may be produced at the executable validation rung — implementation code is not permitted at this rung; file reads, grep output, "
        "and manual inspection do not constitute executable validation — the artifact must be invocable by an automated tool; a pre-existing test that happens to pass does not satisfy "
        "this rung — the artifact must be newly produced for the declared gap; the validation artifact must fail before any implementation edit — a green run with no prior red run does "
        "not satisfy this rung. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is written. At the "
        ...
        "Before each edit, check: does a 🔴 Execution observed: sentinel exist for the current gap? If not, run the validation artifact now before editing. One edit per re-run cycle; "
        "after each edit re-run the validation artifact before any further edit. Upon writing the observed running behavior label, immediately invoke a tool and emit 🔴 Execution "
        "observed: with its output — prose description of tool output does not satisfy this rung. ✅ Thread N complete may not be emitted unless a 🔴 Execution observed: sentinel appears "
        "after the observed running behavior label in the current cycle; once ✅ Thread N complete is emitted no further output in that cycle is valid. Before emitting ✅ Thread N "
        "complete, check: does the declared intent have any behaviors not yet observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and begin the next cycle at the "
        "criteria rung. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for "
        "it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend; every descent through executable "
        ...
        "upward rung transition — whether the scheduled formal-notation → executable-validation gate or any upward return triggered by new information — reconcile all artifacts at that "
        "rung against I before redescending; documentation is a reconciliation artifact, not a thread. Seven rungs in order: prose → criteria → formal notation → executable validation "
        "→ validation run observation → executable implementation → observed running behavior. Sentinel formats — 🔴 Execution observed: [verbatim tool output — triple-backtick "
        "delimited, complete, nothing omitted]; 🔴 Gap: [what the verbatim output reveals]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed]; ✅ Validation "
