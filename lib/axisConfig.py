...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "committed unless a new '§ completeness check:' is followed by '§ properties complete? yes' and the candidate set satisfies observational independence. A candidate retained set "
        "is committed only if it itself reaches the Ground fixed point — passing both '§ properties complete? yes' and the observational-independence procedure — before it replaces the "
        "source retained set; otherwise the candidate is discarded and the source retained set remains authoritative. If the candidate is committed and the committed change produces "
        "any further redundant property, restart the fixed-point procedure with a new completeness check on the resulting retained set. Otherwise discard the candidate and leave the "
        "source retained set unchanged, then attempt another valid independence action for the unresolved redundant property. Admissible constructions used as falsifiers or "
        "independence witnesses are evidence about the truth conditions of properties and do not thereby make the construction's internal mechanism, architecture, algorithm, or "
        "representation the primary subject of the property. Repeat until a retained set simultaneously satisfies completeness and observational independence; when no retained property "
