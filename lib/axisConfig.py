...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "must do, which comes from I, not the phase name. R3 (observation terminates): every thread descends to observed running behavior; anticipated outcomes, asserted expectations, "
        "or conversational acknowledgment never satisfy the execution gate; only tool-executed output with a declared gap does; skipped tests are not observations. Single-step derivation: each rung "
        "addresses only the gap declared by the immediately preceding rung; using gaps or intent from any rung higher than the prior rung to drive the current rung is an error — the "
        "implementation rung closes the gap the failing run declared, not the gap I implies; this is how the ladder localizes interpretation. Foundational constraint: a symbol is not "
        ...
        "without running the tool is invalid]' then '🔴 Gap: [what the verbatim output reveals — gaps inferred from reading source or test descriptions without running the "
        "tool are invalid]' on their own lines; non-executable rung observations appear inline as prose — the 🔴 sentinel format is reserved exclusively for executable rung gates and must not be used as a general "
        ...
        "the artifact as a running process and recording the terminal output — writing a test file is not observation; the test runner must be invoked; required regardless of whether "
