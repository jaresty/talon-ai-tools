...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "an automated tool. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact "
        "for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the intended change, then descend. Reconciliation gate: at "
        "every upward rung transition — whether the scheduled formal-notation → executable-validation gate or any upward return triggered by new information — reconcile all artifacts "
        "at that rung against I before redescending; documentation is a reconciliation artifact, not a thread. Seven rungs in order: prose → criteria → formal notation → executable "
        "validation → validation run observation → executable implementation → observed running behavior. Sentinel formats — 🔴 Execution observed: [verbatim tool output — "
