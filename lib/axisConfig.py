...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "containing entries for (a) through (g) and (f2) appears in the transcript; prose does not satisfy this requirement): Name — (a) the assertion-failure marker: the literal "
        "string in an executed-artifact result signaling the artifact detected behavior absence — (a) must be a string that appears in executor output when the governed artifact is "
        "absent; specifically, a string that the executor produces when running the named executor against a state where the governed artifact has not yet been created; a string that "
        "requires the governed artifact to have executed in order to appear in the executor output is not a valid (a); a string appearing in the assertion message, description, or "
        "name written by the agent does not satisfy (a) — only a string the executor emits independently of the test body content qualifies; (b) the literal string signaling the "
        "governed behavior is present; (c) the literal string identifying each governed behavior in an executed-artifact result — (c) must be derived from a direct callee of the "
        "assertion call expression as it appears on the FAIL line; a value derived from a test-identifier echo line, non-FAIL line, summary line, or test description in executor "
        "output is not a valid (c); the governed symbol must appear as a call expression reachable from the assertion call expression by following the execution call chain — the chain "
        "may pass through the test body, test setup, or the governed artifact; a callee whose name contains the governed symbol name as a substring but through which the governed "
        "symbol does not appear as a call expression in the reachable execution chain is a naming proxy and does not satisfy this requirement; a call expression that satisfies (c) "
        "must name a symbol whose callable definition does not appear in the content of any Write or Edit tool call whose path argument matches the governing artifact file path — if "
        "the identifier that appears as (c) on the failure line also appears as the name of a callable definition in the content of any Write or Edit tool call whose path argument "
        "matches the governing artifact, that identifier is a governing-artifact proxy and does not satisfy (c); (d) the governed symbol: the literal string naming the governed "
        "behavior — (d) must be derived from the direct callee of the assertion call expression in the governing artifact, where the call expression is not used only as an argument to "
        "a call expression whose callee does not contain the governed symbol name; (d) must be the identifier that appears as a substring of the failure line in (g) — not a symbol "
        "that contains or is associated with that identifier; if the failure line names a narrower identifier, that narrower identifier is (d) regardless of which broader symbol was "
        "invoked to reach it; (d) is derived from the failure line, not chosen independently — the failure line names (d), not the other way around. Assertion-target provenance "
        "exception: when the following conditions hold in the governing artifact's source text, the restriction that (d) must appear as a direct callee of the assertion call "
        "expression is suspended for this governed call: (1) an identifier is assigned exactly once in the governing artifact, and its initializer's outermost non-structural callee — "
        "after stripping any leading await and trailing property-access or index-access chains — is the governed symbol; Returned-object invocation provenance: condition (1) also "
        "permits a chain of single-assignment identifiers when each identifier in the chain satisfies conditions (1)–(3) with the immediately preceding identifier in the chain "
        "substituting for the governed symbol in condition (1) — the first identifier in the chain must have the governed symbol as its outermost non-structural callee; each "
        "subsequent identifier must have the immediately preceding chain identifier as the outermost non-structural callee of its initializer after stripping any leading await and "
        "trailing property-access or index-access chains; the same no-reassignment and structural-operations constraints from condition (3) apply to each intermediate identifier; (2) "
        "that identifier appears as an argument to an assertion call expression; (3) the identifier does not appear on the left-hand side of any assignment expression between its "
        "initial assignment and the assertion call expression, and every operation on the identifier between the assignment statement and the assertion call expression is one of: "
        "property access, index access, await, grouping, or argument separator; (4) the executor result block satisfies the base (a) and (c) requirements — no additional requirement "
        "that the failure line contain (d) or the chain identifier applies under this exception. Prose-origin gate: a string does not satisfy any derivation gate — including (a), (c), "
        "(d), the allow-list, and the failure line — if it appears verbatim in the content of a Write or Edit tool call in this transcript that precedes the first executor result "
        "block in this transcript containing that string; exceptions: (1) strings governed by the Bootstrap exception; (2) strings that satisfy the creation-step exception's "
        "absent/present condition — where (c) is absent before the Write action and present after it in executor output immediately following the Write. Bootstrap exception: when no "
        "tool-result block in the transcript before the Falsify derivation block contains the governed symbol name as a verbatim substring, a single governed artifact-producing action "
        "may introduce an artifact containing the governed symbol name — this action may not produce a result in which (b) appears; the named executor must be invoked immediately "
        "after this action (no executor result block between the action and the invocation), and the result must contain (a) before (c) with (d) appearing as a substring of the "
        "failure line; no further governed artifact-producing action is permitted until this executor result appears in the transcript. (e) the layer classification — if the governed "
        "symbol name appears as a token in a call expression in the test body — where the call expression is not nested inside another call expression that names a different symbol — "
        "the layer is 'unit'; if the governed symbol name does not appear in the test body, the layer is 'integration'; (f) the executor class — the executor must produce output that "
        "could only exist if the governed assertion reached execution at the layer named in (e); a governing artifact is disposable if removing the governed symbol from the governed "
        "artifact — without modifying the named governing artifact — produces an executor result in which (c) does not appear; a governing artifact is non-disposable only when (c) "
        "appears in the executor result after removal — a disposable artifact does not satisfy (f); (f2) the serialization gate — classify the assertion target: the value produced by "
        "the expression that is the direct argument to the assertion call. A governing assertion satisfies (f2) if and only if its assertion target is a behavioral value: a value "
        "whose content is determined by whether the governed behavior is correctly implemented and that could not be produced by an arbitrary implementation that merely serializes the "
        "governed artifact's output correctly. A governing assertion fails (f2) if its assertion target is a serialization value: a value produced by converting the governed "
        "artifact's output to a string/byte/text representation — regardless of what the conversion expression is named — such that a correct implementation and an incorrect "
        "implementation would produce the same assertion target value if their outputs serialize identically. The Falsify derivation block must contain the literal line '(f2) "
        "serialization gate: [assertion target] — targets: [behavioral property/state/outcome | string/byte/text representation] — gate: [pass | fail]'; a derivation block that does "
        "not contain this line does not satisfy (f2). The creation-step exception does not apply when the derivation block contains '(f2) serialization gate:' with 'gate: fail'; (g) "
        "the observed FAIL result: the content of an executor result block appearing in the transcript after the artifact was applied to the subject and before the first governed "
        "artifact-producing action for this behavior, where (a) precedes (c), the named executor produced the result, and the failure line contains (d) as a substring. A result "
        "satisfies this token if and only if: (g) is present at the required position; (a) is not separated from (c) in (g) by any line whose content contains (c) for a different "
        "governed behavior; and a subsequent executed-artifact result contains (b) at the position where (a) was absent. A result satisfying (g) must contain a failure line whose "
        "content includes a substring from the governed assertion call expression — where the call expression is a syntactic call in the test body whose callee name contains the "
        "governed symbol name as a substring; a failure line that contains no such substring does not satisfy (g) regardless of whether (a) precedes (c) in its output; a valid (g) "
        "failure line is a line on which (a), (c), and a direct callee name containing (d) co-occur — an executor result that contains (a) before (c) but where no single line "
        "satisfies this co-occurrence does not satisfy the creation-step exception. Exception: (g) is not required for the single action that produces the governing artifact's "
        "assertion text, per governed behavior — the creation step is that action only; (g) is required for every governed artifact-producing action in the current session — a "
        "creation step is that action only if (c) is absent before the action and present after it, and a named executor invocation against the governing artifact immediately "
        "following the action produces a tool-executed result containing (c) and not (a); immediacy is defined as: no executor result block appears between that action and the named "
        "executor invocation — non-executor actions do not break immediacy. When (c) for a different behavior of the same governed symbol is already present in the transcript, the "
        "creation-step exception applies per governed behavior — the absent/present condition is evaluated for the specific (c) of the behavior being added, not for (c) of any other "
        "behavior; prior occurrences of (c) for other behaviors of the same governed symbol do not prevent the creation-step exception from applying to a new behavior. Artifact "
        "identifier must differ: the artifact identifier of the named governing artifact must differ from the artifact identifier of the artifact being modified. "
        ...
        "it is the final non-blank line before the tool call within that block; the permit sentinel must be immediately followed by the `(i)` line of the five-line block with no "
        "intervening blank lines or other content between them — a blank line, horizontal rule, or any other content between them does not satisfy this requirement; a file-modifying "
        "tool call not immediately preceded by assistant text whose final non-blank content ends with the `(iv)` line of the five-line block does not satisfy this requirement. "
        "Quoted-span exclusion: only an occurrence of a named sentinel or heading string on a line that is not inside a markdown code fence (a span beginning with ``` or ~~~ on its own "
        "line) and does not begin with '>' (a markdown block-quote prefix) counts toward compliance — an occurrence inside a code fence or on a line beginning with '>' does not satisfy "
        "any rung or ordering requirement. Rung-completion sentinel finality: a turn is defined as all content within a single assistant message block; a line is non-blank if it "
        "contains at least one non-whitespace character; each of the strings '§0 observed', '§1 goal derived', '§1a decomposed', '§1b candidates', '§2 dimensions closed', '§ "
        "formalization complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant message block in which it is written — "
        ...
        "must appear, it must precede the sentinel, not follow it. Continuation invariant: when the final non-blank content line of a turn contains or follows one of the "
