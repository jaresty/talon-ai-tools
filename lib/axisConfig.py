...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response ── PROTOCOL ── Rule 0 (epistemic grounding): no claim about system state, test outcome, or artifact completeness is valid unless a tool-executed event in this "
        "conversation produced it; model knowledge, anticipation, and reasoning cannot satisfy any gate regardless of accuracy. Underlying all compliance failures is one "
        "epistemological error: substituting model knowledge for conversation events — prior knowledge, anticipation, and model reasoning cannot satisfy any gate regardless of "
        "accuracy. Four primitives govern every thread and derive all downstream rules: Primitive 1 (event gate): declare the sentinel before running the tool — a sentinel not "
        "immediately followed by verbatim tool output is inert and carries no gate function; it cannot be cited, pointed to, or treated as satisfying any gate requirement regardless of "
        "where it appears or how it was labeled; retroactive sentinels do not open gates; the tool must be re-run with the sentinel appearing immediately before it. Primitive 2 "
        "(derivation chain): each artifact derives from the prior artifact — form changes, intent does not (R1); a skipped rung voids all artifacts below it; an artifact is a rung iff "
        "upward-faithful (derived from the prior rung without adding unstated constraints) and downward-sufficient (self-contained such that the correct next artifact is derivable "
        "without consulting I — not merely sufficient to evaluate a proposed artifact, but sufficient to construct the correct one); stopping mid-thread is only permitted when the next "
        "rung is not achievable; within a thread, advance through every feasible rung; a rung label during execution marks the point where the artifact begins — it is not a section "
        "heading for planning or description; a rung label must be emitted at every rung transition; absence of a rung label at a transition is a traversal violation, not a stylistic "
        "omission; no content other than the artifact itself may appear between a rung label and the artifact it precedes. Primitive 3 (observation terminates): every thread ends at "
        "tool output naming the declared behavioral gap; infrastructure events — build success, server running, HTTP 200, URL response — are not behavioral observations regardless of "
        "label; system-state events confirm infrastructure, not behavior; the specific behavior named in the gap must appear as a verifiable event in the tool output; R3 validity is "
        "determined by output content, not script type — output naming only infrastructure state (‘route loads successfully’, ‘server running’, ‘200 OK’) does not satisfy R3 regardless "
        "of what tool or script produced it. Primitive 4 (locality): each artifact addresses the gap declared by the prior rung’s output and nothing more; Gap-locality: the gap gating "
        "rung N is the output of executing rung N-1 — no gap from any higher rung, and no element of I directly, may serve as the gating gap; implementing beyond the declared gap is a "
        "violation — not a benefit; minimal scope applies to the entire invocation, not just individual rungs — any artifact added after the ladder is complete that was not present in "
        "I must be treated as a new I requiring its own manifest and descent; the validation artifact is frozen at the point the gap is declared; any modification to the validation "
        "artifact after that point — including reducing test count, simplifying mocks, or weakening assertions — requires upward correction: return to criteria, demonstrate the "
        "modified artifact still encodes the gap, and re-emit the R2 audit before redescending; a passing suite with fewer tests than the declared gap requires is a gap signal, not a "
        "success; modifying what a behavioral assertion checks to conform to implementation behavior redefines the gap contract using model knowledge of the implementation — this is a "
        "scope violation regardless of direction; the operational test for structural correction is whether the set of component behaviors that would cause the assertion to fail is "
        "unchanged — if any behavior that would previously fail the assertion now passes it, or vice versa, the predicate has changed regardless of how the modification is "
        "characterized. Locality constrains downward motion — it bounds what a forward step may address; upward correction governs the opposite direction and re-scopes all artifacts "
        "below the corrected rung; the two constraints do not conflict because they apply in opposite directions of chain traversal. ground is a Process method — the manifest, rung "
        "sequence, and execution gates are mandatory regardless of task complexity or scope; for code contexts, each rung in the R4 sequence may not be skipped or combined with another "
        "— the executable implementation rung is blocked until the validation run observation rung has declared a gap; there is no complexity threshold below which the ladder becomes "
        "optional; completeness governs rung depth, not rung existence; task characteristics such as ‘simple,’ ‘incremental,’ ‘clear requirements,’ are not exemption criteria and do "
        "not reduce the mandatory rung sequence; a rung is not achievable when the domain provides no standard artifact type for it — this must be stated explicitly with justification; "
        "convenience or prior knowledge do not make a rung not achievable. Eagerness to implement is the primary failure mode — the shortest path to a valid artifact is strict "
        "primitive adherence; every skipped rung produces output that must be discarded and rederived. The manifest must name every rung of the R4 sequence for each thread, or "
        "explicitly state that a rung is not achievable with justification; omitting a required rung from the manifest is malformed before descent begins; a manifest that collapses "
        "validation run observation into any adjacent rung is malformed regardless of how it is labeled. Manifest entries must name behavioral gaps to be closed, not implementation "
        "decisions — a manifest entry that names an implementation approach, data structure, or algorithm rather than a behavioral condition is a boundary violation; test names "
        "declared in the manifest must derive from criteria behavioral gap statements, not implementation decisions; a test name that encodes an implementation choice (a field name, "
        "keybinding, data path, or algorithm) rather than a behavioral condition is a manifest violation. The seven canonical rung names are: prose, criteria, formal notation, "
        "executable validation, validation run observation, executable implementation, observed running behavior. I is the declared intent governing the invocation; I precedes and is "
        "not itself an artifact. ground is not an optional style preference — once the ground token is present, it governs the entire invocation unconditionally; asking whether to "
        "apply ground treats a mandatory process constraint as optional guidance and is itself a violation. When beginning mid-ladder, first locate the highest already-instantiated "
        "rung and update it to reflect the intended change, then descend. Upward revision is always permitted when a gap is observed between prior understanding of I and something "
        "encountered via direct interaction with reality, code, or a stakeholder; upward revision must be signposted with: what was observed, which rung is being revised, and why; "
        "changing I requires revising every artifact derived from it to restore chain consistency before descent continues. Upward correction is mandatory when a rung failure reveals "
        "that a higher rung is deficient — failing to correct the highest broken rung means every artifact below it is derived from a deficiency that will resurface at each subsequent "
        "rung; before correcting any higher rung, emit what was observed at the current rung, which higher rung is being corrected, and why — a correction without this observation "
        "record is invalid and indistinguishable from enrichment; find the highest broken rung, correct it, and rederive every artifact below it before redescending. ── GATES ── A gate "
        "is a conversation-state condition: open when and only when the required event has occurred in this conversation for this thread. Sentinels are the primary enforcement "
        "mechanism for Primitive 1. For executable rungs, emit 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] then 🔴 Gap: [what the "
        "verbatim output reveals] on their own lines before any implementation artifact; a prose description of tool output is always composed content and never satisfies this sentinel "
        "— even if accurate; the body must contain the raw tool output, not a characterization of it; no prose, summary, or interpretation may appear before the 🔴 Gap line; "
        "interpretation belongs exclusively in the 🔴 Gap line. The 🔴 Execution observed sentinel body must be consistent with what the validation artifact as written could produce — "
        "every error, failure line, and symbol named in the output must be traceable to actual code in the validation artifact; output naming symbols not present in the validation "
        "artifact is fabricated regardless of structural resemblance; empty test stubs cannot produce undefined-symbol errors; before citing an execution failure as the gap, verify the "
        "failure is consistent with the test code actually written. The 🔴 Execution observed sentinel is a prospective commitment, not a retrospective label — its function is to ensure "
        "the tool output is captured before any interpretation occurs; placing a sentinel after tool output has already appeared in context is the same epistemological error as "
        "composing a prose description of that output — the model has already processed and interpreted the output, and any sentinel placed after it records interpreted content, not a "
        "captured event; the accuracy of the retrospective label is irrelevant — the sentinel’s purpose is not accuracy but prior-to-interpretation capture. The 🔴 Execution observed "
        "sentinel must appear immediately before the tool is invoked — not retroactively after tool output has already appeared in context; tool output that appeared before its "
        "sentinel is uncovered and does not satisfy this rung regardless of subsequent labeling; a retroactive sentinel does not open the gate — the gate remains closed; the tool must "
        "be re-run with the sentinel appearing immediately before it. No implementation artifact — including planning text, code blocks, or tool calls — may appear before valid "
        "Execution observed + Gap sentinels; if any implementation content appears without these sentinels immediately preceding it, it is invalid and must be discarded; silent "
        ...
        "statement, the artifact’s status remains ambiguous. ── STRUCTURE ── For code contexts, R4 instantiates as: prose (natural language description of intent and constraints) → "
        "criteria (acceptance conditions as plain statements — each criterion must be falsifiable: there must exist an observable system behavior that would cause it to fail; a "
        "criterion that reduces to structural shape or implementation detail rather than behavioral condition is not valid; before advancing, verify every criterion is behavioral) → "
        ...
        "satisfy R2 — artifact cannot be run as written; Formal notation must satisfy R2: every behavioral constraint from the criteria rung must be re-expressed as a behavioral "
        "invariant — not just interface shape; type signatures or schemas that capture structure without encoding invariants do not satisfy this rung; before advancing from formal "
        "notation, produce the R2 audit as a separate named section after the formal notation body — a prose list embedded in the notation body does not satisfy this requirement; the "
        ...
        "invariant cannot be expressed for the component under test must be labeled UNENCODED; delegation to an external system is not a valid resolution; the rung is incomplete until "
        "every criterion resolves to a behavioral invariant — no UNENCODED entries may remain; emit ‘✅ Formal notation R2 audit complete — N/N criteria encoded’ on its own line as the "
        "gate before advancing to executable validation) → executable validation (a file artifact invocable by an automated tool — go test, pytest, or equivalent — written to target "
        "the declared gap; the artifact must reside within the project directory and conform to the project’s test file conventions — a script outside the project tree (such as /tmp) "
        "does not satisfy this rung regardless of whether it is invocable by a test runner; the artifact must be derivation-sufficient: a model given only this artifact and no other "
        "context must be able to derive the correct implementation — an artifact that can evaluate a proposed implementation but leaves the correct one underspecified does not satisfy "
        "this rung; only validation artifacts may be produced at this rung — implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not "
        "constitute executable validation; pre-existing artifacts not targeting the gap do not satisfy this rung) → validation run observation (only satisfied by an observed failure; a "
        "passing run is a gap signal — a build error or compile failure does not satisfy this rung; emit ‘✅ Validation artifact V complete’ on its own line immediately after the gap is "
        "declared) → executable implementation [stop and verify before producing any implementation artifact: you must be able to quote a 🔴 Execution observed sentinel with verbatim "
        "tool output from this conversation for this thread, and a 🔴 Gap must have been declared; if you cannot quote both from this conversation, you are not at this rung — produce "
        "the executable validation artifact first; the rung proceeds as a minimal-step cycle: each edit implements only enough to advance past the current observed failure and nothing "
        "more; after each edit, re-run the validation artifact before producing any further implementation change; a single undifferentiated implementation pass that produces a passing "
        "suite without intermediate re-runs is a traversal violation — it is indistinguishable from anticipating the result; if a re-run within this cycle produces a green result and "
        "any newly-passing test has no prior recorded failure from this cycle, that test is uncovered and the vacuous-green check applies unconditionally — either the behavior was "
        "already implemented (verify; the thread may be complete) or the validation artifact is vacuous with respect to the declared gap and must be corrected] → observed running "
        "behavior (observation rung — no new files may be created at this rung; OBR is satisfied by invoking artifacts already produced by this ladder and recording their output; a "
        "screenshot, preview panel, server-running confirmation, URL response, or build success does not satisfy this rung regardless of label; for UI artifacts, a server response does "
        "not confirm rendering; for APIs, an HTTP 200 does not confirm correct data; for code contexts, observed running behavior is satisfied only by tool output that directly "
        "demonstrates the declared feature: rendered DOM content showing the declared UI behavior, an API response body containing the declared data, or an end-to-end test output "
        "naming the specific behavior from I; the observation must demonstrate the declared behavioral intent was satisfied by tool output traceable to the declared gap; R3 validity is "
        "determined by output content — output naming only infrastructure state (‘route loads successfully’, ‘server running’, ‘200 OK’) does not satisfy this rung regardless of what "
        "tool or script produced it; when the preferred observation method requires infrastructure absent from the ladder, the correct action is to use existing ladder artifacts as OBR "
        "or declare the method not achievable; creating infrastructure to enable observation is production regardless of characterization; observed running behavior requires 🔴 "
        "Execution observed (verbatim tool output) and 🔴 Gap (naming the specific behavior from I the output demonstrates) before ✅ Thread N complete may appear; only tool-executed "
        "output with a declared gap satisfies the execution gate; no new files may be produced at this rung outside this ladder). When I-formation is required: observation of existing "
        "code or running behavior sufficient to establish I is required before the manifest when I cannot be declared from context alone — I-formation is observation-only: only actions "
        "that leave codebase state identical to its state at the start of I-formation are permitted; creating files, modifying files, writing code, or running any command that alters "
        "working state are not permitted during I-formation and are rung work regardless of what phase the model believes it is in; skipping I-formation when context is insufficient is "
        "an error, not a permitted shortcut — it produces a specification derived from assumed rather than actual constraints, and every downstream artifact will be wrong in ways that "
        "only surface at the validation and OBR rungs, by which point multiple rungs must be discarded and rederived; any file creation or modification before the manifest-complete "
        "sentinel is a boundary violation; output ‘✅ I-formation complete’ on its own line after declaring I and before the rung manifest; the manifest may not appear before this "
        "sentinel; exploration beyond what is needed to declare I belongs as the first rung of the manifest, not pre-manifest. I-formation ends at a gate prohibiting implementation "
        "intent — the manifest produced after I-formation may contain only behavioral gaps to be closed; implementation decisions are not permitted in the manifest before the "
        "executable validation rung; a manifest that names an implementation approach rather than a behavioral gap is a boundary violation equivalent to writing code during "
        "I-formation. ── COMPLETION ── Intent precedes its representations. Every artifact that documents the governing intent of this invocation — whether produced in this invocation "
        ...
        "reconciliation report has been produced. The N in ‘✅ Manifest exhausted — N/N threads complete’ must equal the exact count of threads declared in the manifest; emitting more ✅ "
        "Thread N complete sentinels than declared threads is a manifest contract violation.",
