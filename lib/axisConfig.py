...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "additional invariants, no coverage beyond the declared gap. Rung table — name | artifact type | gate condition | void condition: prose: artifact=“natural language description "
        ...
        "observed running behavior: artifact=“live-process output — output produced by a tool call that starts or queries a running process; reading any file does not satisfy this type "
        "regardless of content”, gate=“implementation complete; exec_observed + gap for behavior from I”, voids_if=“new files created; output names only infrastructure state; file read "
        "used as evidence”. Every 🔴 Execution observed: sentinel at any rung must contain verbatim tool-call output, triple-backtick delimited, complete, nothing omitted — prose "
        "descriptions, inline summaries, model-generated text that resembles output, elision markers (…, [truncated], [output continues]), reformatted text, and paraphrase are "
        "prohibited inside the block; if tool output is too long, the block ends at the last character the tool produced — no editorial closing; the only valid sequence is: tool call → "
        "🔴 Execution observed: → triple-backtick block — the sentinel may not be written before the tool call exists in the transcript — a sentinel written before its tool call records "
        "anticipated rather than observed state, and is void regardless of accuracy; a sentinel with prose content or no delimited block is void — fix by invoking the tool and "
        "re-emitting. Once ✅ Ground entered has been emitted in this session, every subsequent response must begin by identifying the current rung and the current gap before any other "
        ...
        "rewriting the criterion to match an easier implementation is a protocol violation; each criterion must state (a) observable behavior and (b) a falsifying condition statable "
        "without source files, function names, or variable names — if part (b) references implementation internals, the criterion is structural and must be rewritten before this rung "
        "closes; this is a content gate, not a self-check. The formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, "
        "invariants, pre/postconditions) encodes what must be true — the executable or testable part. Natural language labels, introduces, and explains the specification — prose adds "
        "context but the behavioral constraints must still be encoded in notation.it must encode all structural constraints the criterion implies — the permitted forms (type "
        ...
        "re-read the formal notation and assert each structural constraint it encodes; before writing the test, check for an existing test file covering the same module — add there if "
        "so; when the artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure "
        "[verbatim test name as it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]’ — each prior-failure name must be "
        "quotable verbatim from a 🔴 Execution observed sentinel; modification without carry-forward is a traversal violation; no implementation artifact may appear after modification "
        "until carry-forward has been emitted. when the declared intent is to modify a test artifact, the test-under-modification is the implementation artifact for this cycle and the "
        ...
        "the transcript; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static state, run it before any edit — if it "
        ...
        "have been emitted at the validation run observation rung in the current cycle — emitting it at any other position is a protocol violation; HARD STOP may not be emitted at the "
        "executable validation rung — a harness error at EV requires fixing the harness, not an upward return; if the observed gap matches the prior cycle's gap, the edit did not "
        ...
        "token is the criteria rung label for the current thread; the prose re-emission rule applies to new gap cycles, not to HARD STOP returns. Upon writing the observed running "
        ...
        "protocol violation; the tool call is the only valid next action after criterion re-emission; ✅ Thread N complete may not appear until a tool call exists in the transcript "
        "after the observed running behavior label in the current cycle; then immediately start or query the implemented artifact as a live running process via a tool call and emit 🔴 "
        "Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion; it must be what a non-technical "
        "observer of the running system would see — not the test suite, not process state unless the criterion is specifically about process state; a build output or compilation result "
        "never satisfies this gate regardless of what it names; prefer the most realistic invocation — for a web UI, start the dev server and emit the HTML response body (e.g., via "
        "curl or browser); for a CLI, invoke it directly and emit its output; in-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable "
        "artifact exists; if the implementation has no directly invocable artifact, invoke any non-test consumer that starts or queries a live process and demonstrates the declared "
        "behavior — reading build artifacts, compiled output, generated HTML, or any static file is not a valid fallback regardless of what the file contains; if no live process can be "
        "started or queried, open a new gap cycle to make the artifact directly invocable before continuing; if the declared gap names a real network endpoint, the OBS must include "
        "verbatim output showing a real HTTP request or response — a test run whose network calls are mocked does not satisfy this gate; before any tool call touching the filesystem at "
        "this rung, emit a provenance statement: ‘Invoking [artifact] produced at [rung name] in this thread’ — if the rung label cannot be located in the transcript, the tool call is "
        ...
        "observation rung stops after naming the gap. 🟢 Implementation gate cleared licenses the first edit — it does not complete the thread; after emitting it, the executable "
        ...
        "gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly one entry per distinct gap-name; "
        "a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at the start of every cycle — the first "
        "cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 Implementation gate cleared "
        "requires a 🔴 Execution observed: from the current cycle’s validation run observation rung — not from a prior cycle — to exist in output; the VRO rung label must appear in the "
        "transcript for the current cycle — a run at the EV rung does not satisfy the VRO gate regardless of whether it produced a failure. Before emitting ✅ Manifest declared, scan "
        "every sentence in the prose for behavioral predicates (fetches, requires, displays, filters, renders, validates, authenticates, loads, saves, or similar) — each distinct "
        "predicate requires a separate thread; re-read the prose rung output via a tool call to confirm every predicate is covered; each manifest entry is a short gap label — a noun "
        "phrase naming the gap, not a behavioral assertion; a manifest entry containing a verb or phrased as a currently-false assertion is malformed; the behavioral assertion for a "
        "gap is produced for the first time at the criteria rung, not the manifest; ✅ Manifest declared may be emitted exactly once per invocation. Outputting a rung label is what "
        "begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. I is the declared intent governing "
        "the invocation; each artifact must be self-sufficient to derive the next without consulting I; before closing a rung artifact, state every open constraint the next rung must "
        ...
        "every cycle; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; all other rung transitions are continuous within the same "
        "response. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through executable implementation requires executable "
        ...
        "this rung. Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running "
