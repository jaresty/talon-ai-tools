...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "that rung and valid transitions from your derived protocol: review your derivation to recall what that rung requires, what files are permitted, and when you may transition to "
        "other rungs (proceed downward after completing current rung, return upward when gaps found in prior rungs, one rung at a time). derive: Sentinels from recursive intent "
        "structure (declare intent → derive micro-intents → verify with evidence → emit completion sentinel; this applies at session, thread, rung, and file-edit levels), Derive "
