...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "coverage artifact (tool result required before implementation): every assertion requires a FAIL tool result before any implementation step — Principle 1 applies to all "
        "assertions without exception. For natural-FAIL-state assertions (Natural FAIL state: yes), the FAIL comes from the initial red-state test run. For no-FAIL-state assertions "
        "(Natural FAIL state: no), the FAIL requires perturbation. Before any implementation step, produce a Phase 2 status line for every assertion using exactly one of these two forms "
        "— COMPLETE: Phase 2 [assertion name] — <verbatim FAIL output from tool result> or NOT YET PRODUCED: Phase 2 [assertion name] — running now. The NOT YET PRODUCED form names a "
        "missing predecessor. The implementation step's correctness criterion is: change the system from the observed wrong state to the correct state. Without the FAIL tool result, the "
        "wrong state has not been observed — the correctness criterion is undefined, not merely unmet. Chain requires every step to reproduce its predecessor before proceeding, and the "
        "predecessor named here does not yet exist as a tool result. The agent derives from this that no implementation step is definable until the tool call is made and the COMPLETE "
        "line can be written from actual output. There is no path between NOT YET PRODUCED and implementation that does not require fabricating a predecessor — and fabrication is not "
        ...
        "COMPLETE line is a chain violation. A compilation error is not a valid FAIL artifact for any assertion — natural-FAIL-state or otherwise. Principle 5 derives this: a compile "
        "error is a build failure, not a behavioral observation; the assertion has not run and has not observed the wrong state. When a test does not compile, the required action is to "
        "write a stub — a minimal implementation with the correct signatures and no-op behavior — so the test compiles and runs, then collect the behavioral FAIL from the running test. "
        "The behavioral FAIL (e.g. 'key missing', 'unexpected value') is the only valid predecessor for an implementation step. For no-FAIL-state assertions, the FAIL requires "
        "perturbation. Phase 2 is complete when every assertion has a COMPLETE status line backed by a real tool result. Then derive the process these six principles require and follow "
        "it. The derivation must be visible. Principle 1 requires a FAIL observation. A FAIL observation requires an executable artifact whose state differs when the behavior is absent. "
        "The structural floor fallback applies only when no such artifact exists. An artifact exists whenever the behavior produces any observable output — a file, a parsed structure, a "
        "command exit code. The fallback is therefore unavailable whenever the behavior produces observable output, which is the common case for file, data structure, and command-based "
        "tasks. Structural floor assertions (file existence, path resolution, identifier presence) are required when the fallback genuinely applies but do not satisfy gate — a manual "
        "verification protocol must still be declared: who verifies, by what procedure, and what binary pass/fail condition is defined in advance and checkable by someone other than the "
        "author. When the task is exploratory, governing assertions must be observability assertions — claims about what can be detected or measured — not outcome assertions that encode "
        "a direction before exploration has occurred. An assertion's durability must match the durability of the behavior it governs — an assertion that exists only for the duration of a "
        "session cannot govern behavior that persists beyond it. A FAIL tool result produced by a session-only script satisfies the immediate Phase 2 gate but does not constitute a "
        "durable assertion; the assertion must be committed to the test suite before the implementation step closes.",
