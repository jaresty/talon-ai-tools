...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "conjunction of all sub-property expressions for a given 'property [N]:' must be logically equivalent to the expression in 'property [N]:' — logical equivalence means the set "
        "of inputs satisfying all sub-property expressions simultaneously equals the set of inputs satisfying the parent expression; a conjunction satisfied by a strict subset of the "
        "parent's cases (e.g. a single concrete instance substituted for a variable) does not satisfy this requirement; a conjunction satisfied by a strict superset of the parent's "
        "cases (e.g. omitting a condition from the parent) does not satisfy this requirement; if the property is already atomic (no strict sub-expression exists that is independently "
        "falsifiable), before writing 'property [Na]: atomic — ', write '§ atomicity test: \"[quoted sub-expression]\"' where [quoted sub-expression] is a verbatim phrase from the "
        "parent 'property [N]:' expression, followed by a sentence naming why that sub-expression is not independently falsifiable from the remainder — a 'property [Na]: atomic —' line "
        "that does not have a '§ atomicity test:' line with a verbatim quoted phrase above it does not satisfy this requirement; 'property [Na]: atomic —' is valid only after '§ "
        "atomicity test:' has appeared; when all 'property [N]:' and sub-property lines are written, write '§ properties check: \"[quoted phrase]\"' where [quoted phrase] is a "
        "verbatim substring of one of the 'property [N]:' lines written above — a '§ properties check:' line whose quoted phrase does not appear verbatim in a 'property [N]:' line "
        "above it does not satisfy this requirement; then write '§ properties complete'; '§ properties complete' is valid only after a valid '§ properties check:' line has appeared; "
        "(4) verify coverage: '## Completion check' must not appear before '§5 enumeration complete' in the transcript; '## Completion check' must appear as the next heading after '§5 "
        ...
        "proceed under the same protocol. (5) enumerate escape paths: '## Path enumeration' must not appear before '§ enforcement complete' in the transcript; enumerate every path by "
        "which §1–§4 headings could be present without the governing goal identified, dimensions derived, or dimensions covered; for each path, name the literal string whose presence "
        "closes it; when no further open path remains, write '§5 check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the escape path closing strings "
        "named in the enumeration above — a '§5 check:' line whose quoted phrase does not appear verbatim in the path enumeration above it does not satisfy this requirement; then write "
        "'§5 enumeration complete'; '§5 enumeration complete' must not appear before a valid '§5 check:' line in the transcript; '§5 enumeration complete' must not appear before '§ "
        ...
        "or ordering requirement. Rung-completion sentinel finality: a turn is defined as all content within a single assistant message block; a line is non-blank if it contains at "
        "least one non-whitespace character; each of the strings '§0 observed', '§1 goal derived', '§1a decomposed', '§1b candidates', '§2 dimensions closed', '§ formalization "
        "complete', '§ properties complete', '§ test suite complete', '§4 coverage verified', and '§5 enumeration complete' must appear as the final non-blank line of the assistant "
        "message block in which it is written — the sentinel line must begin with the literal first character of the sentinel string ('§' or '#'); a sentinel line beginning with '**' "
        "or any other markdown formatting character does not satisfy this requirement; a checker verifies this by locating the sentinel line, scanning all subsequent lines in the same "
        "assistant message block, and confirming every such line is blank (contains only whitespace or is empty); any non-blank line after the sentinel in the same assistant message "
        "block is a violation; no exceptions are permitted — if additional content must appear, it must precede the sentinel, not follow it. Completion check ordering: the '## "
        "Completion check' block must appear after '§5 enumeration complete' in the transcript; a '## Completion check' block that appears before '§5 enumeration complete' does not "
        "satisfy this requirement even if its contents are otherwise correct; a response that contains '§5 enumeration complete' and does not produce a subsequent '## Completion check' "
        "block containing 'Formalized properties:', at least one 'property [N]:' line satisfying all property requirements, '§ properties complete', and '§4 coverage verified' does not "
        "satisfy ground. Continuation invariant: when the final non-blank content line of a turn contains or follows one of the rung-completion strings '§0 observed', '§1 goal "
