...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "gate": "The goal is complete assertion coverage: every behavior that exists must have a corresponding assertion that would catch it breaking. No behavior should exist without an "
        "assertion that demands it. The cycle this token enforces is the mechanism that makes that goal more likely to hold — by ensuring each behavior is demanded by an observed failure "
        "before it is produced, uncovered behavior becomes structurally harder to introduce. The response enforces evaluation precedence: before producing any output, name the evaluation "
        "artifact that will govern it. If no natural evaluation artifact exists for this task — no testable criteria that can be verified independently of the output — state that "
        "explicitly and do not proceed. An evaluation produced by the same reasoning that generates the output is not structurally separate from it. When no artifact is possible, flag "
        "the mismatch and suggest verify or rigor instead. When an artifact is possible: produce it before the implementation artifact it evaluates. An evaluation written after the fact "
        "is shaped by what it evaluates and cannot expose gaps the implementation was designed around. Self-certification is impossible — the evaluation must be structurally separate "
        "from the artifact and must have existed before the implementation began. Naming or acknowledging a deviation does not satisfy the gate. Any statement that labels, acknowledges, "
        "or apologizes for skipping the artifact without producing it is not a discharge — the artifact must exist before proceeding and must govern whether implementation proceeds. An "
        "artifact that exists but is not used as a decision point is not a gate. The ratio constraint: each implementation artifact requires its own governing evaluation artifact. More "
        "evaluation than implementation is acceptable; any implementation artifact without a corresponding evaluation artifact is a gate violation regardless of how many other evaluation "
        "artifacts exist. A governing artifact must authorize exactly one behavior — one observable outcome, one criterion, one claim. An artifact that covers multiple behaviors cannot "
        "serve as a gate because it cannot produce a single failure that authorizes a single step. This is structural, not policy: a governing artifact can only show one failure at a "
        "time, so it can only authorize one implementation step at a time. Complex behavior must emerge through multiple cycles, each observing one failure individually. Implementation "
        "of multiple behaviors in one step is a gate violation regardless of how many behaviors the governing artifact declares.",
        ...
        "The primary form this takes is collapsing intermediate reasoning into a readable narrative: internal steps are simulated but not surfaced, making the output appear "
        "method-compliant without being verifiably so. Internal correctness and visible correctness are not the same thing. Every constraint in this protocol is a direct response to "
