...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "make it green closes the gap; intent remains intact only if assertions remain green; regression = any assertion that was green becomes red; failure to see the test red means "
        "the gap has not been observed and implementation cannot proceed — for backfill cases where tests already pass, perturb the implementation to observe failure, then fix the "
        "perturbation), each derived rule must cite its source principle(s) from P1-P6; rules that cannot be derived are protocol violations; emit Protocol derived after deriving all "
        "rules; emit Ladder derived after producing the rung table; ground complete may only be emitted as the outcome of the observation loop — emitting it outside the loop is a "
