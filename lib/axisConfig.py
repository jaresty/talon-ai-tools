...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "control flow; if any witness is not mechanically derivable, temporarily instrument the regression guard or construct an equivalent observation harness that executes the same "
        "assertions while emitting observable per-assertion outcomes; execute the instrumented guard or harness against the same violating state via a tool call, wait for the "
        "tool-result block, then derive every witness classification exclusively from that tool-result; the temporary instrumentation exists solely to observe execution and must not "
        "become part of the regression guard or governed artifact — discard it after witness generation; the text immediately following the Failure: line (or the observation harness "
        "tool-result block if instrumentation was required) must be the first witness line — no narrative, causal argument, prose explanation, or other content may appear before the "
        "first 'Property [N] assertion [M] witness:' line; emit exactly one witness line for every previously inventoried executable assertion, in ascending assertion inventory order "
        "without omission or reordering; each witness line takes one of these forms, derived exclusively from the tool-result block: 'Property [N] assertion [M] witness: Failure — "
        'assertion: "<verbatim assertion text>" — observed: "<verbatim tool-result text identifying this assertion as the failure>"\' if and only if the tool-result uniquely '
        'identifies this assertion as a failing outcome; \'Property [N] assertion [M] witness: Success — assertion: "<verbatim assertion text>" — observed: "<verbatim tool-result text '
        "confirming this assertion passed>\"' if the tool-result contains a directly observed passing outcome for this assertion; if an assertion cannot be classified Success or "
        "Failure from the current tool-result, it is not yet observable — refine the observation harness or violating state to make it observable and repeat the instrumented "
        "execution; only after no executable instrumentation or violating state modification can make the assertion observable may 'Property [N] assertion [M] witness: Unobservable — "
        'assertion: "<verbatim assertion text>" — <observed reason quoted from tool-result>\' be emitted; Unobservable is the last resort and must be justified by a verbatim '
        "tool-result excerpt; an Unobservable witness is treated as a guard defect — revise the guard to make the assertion observable and repeat from step (2); no assertion may "
        "permanently carry Unobservable status; at least one inventoried assertion must receive witness classification Failure — a guard execution in which no inventoried assertion is "
        "classified Failure does not satisfy step (4), regardless of whether a Failure: line was produced; a Failure witness whose observed text is not present verbatim in the "
        "corresponding tool-result block does not satisfy this token; if no inventoried assertion can be attributed to the observed failure, revise the violating state or the "
        "regression guard and repeat from step (2); after all assertions are witnessed, emit 'Assertion witnesses: complete — <count> assertions witnessed' where <count> must equal "
        "the count from 'Assertion inventory: complete'; a witness count that differs from the inventory count does not satisfy this token; emit 'Assertion witnesses: incomplete — "
        "<reason>' if any assertion lacks a witness; if incomplete, revise the violating state, observation harness, or guard and repeat from step (4); no 'Quoted implementation:', "
        "implementation modification, or 'Implementation overreach:' may appear before 'Assertion witnesses: complete' has been emitted; no subsequent step may occur until 'Assertion "
        "witnesses: complete' has been emitted; (5) identify the governed artifact for property [N] — the artifact whose modification satisfies property [N]; depending on the "
        "property, this may be the implementation, the regression guard, a configuration file, or another explicitly governed artifact; modify the governed artifact to satisfy "
        "property [N], then immediately emit 'Quoted implementation: <smallest contiguous verbatim text uniquely identifying the modification to the governed artifact>'; if step (4) "
        "required constructing a temporary violating state because the governed artifact already satisfied property [N], step (5) is satisfied by confirming the original state is "
        "restored — emit 'Quoted implementation: <verbatim text identifying the restored governed artifact>' to confirm; no subsequent step may occur until this sentinel has been "
        "emitted; (6) attempt to construct a minimal modification to the quoted implementation that preserves the success of the quoted guard while still changing the governed "
        ...
        "quoted guard emitted earlier in the transcript, not from an imagined or intended version. After all properties complete these steps, emit 'Coverage: complete' if all "
        "properties are resolved, or 'Coverage: gap — property [N] \"<verbatim property definition>\"' for any unresolved property; if any gap exists, return to step (1) for each "
        "ungoverned property and complete all six steps before re-emitting Coverage:; Coverage: complete may not be emitted while any property [N] lacks a witnessed Failure: or "
        "Unobservable: line. After all per-property cycles complete, perform an implementation audit before emitting 'Coverage: complete': enumerate the observable partitions of the "
        "governed artifact — distinct execution paths, call sites, externally visible interfaces, state transitions, outputs, and consumers of those outputs; for each partition "
        "determine whether at least one retained property governs its externally observable behavior; emit 'Audit: implementation gap — <description>' for any partition not governed "
        "by a retained property, execute the full Ground protocol for the new properties — including atomicity and completeness checks concluding with 'Ground complete' — then execute "
        "the full six-step falsify cycle for each new property, then repeat the audit; emit 'Audit: implementation complete' only when no ungoverned partition remains; 'Coverage: "
        "complete' may not be emitted before 'Audit: implementation complete' has been emitted. A response that omits any required sentinel, performs a step before emitting its "
        "required quoted artifact, modifies the governed artifact to satisfy property [N] before a quoted Failure line appears, omits any executable assertion from the assertion "
