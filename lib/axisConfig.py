...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "a witness is mechanically derivable only if the tool-result uniquely determines its classification without semantic reasoning about execution order, language semantics, or "
        "control flow; if any witness is not mechanically derivable, temporarily instrument the regression guard or construct an equivalent observation harness that executes exactly "
        "the inventoried assertions without strengthening, weakening, removing, or reordering them, while emitting observable per-assertion outcomes; execute the instrumented guard or "
        "harness against the same violating state via a tool call, wait for the tool-result block, then derive every witness classification exclusively from that tool-result; the "
        "temporary instrumentation exists solely to observe execution and must not become part of the regression guard or governed artifact — discard it after witness generation; the "
        "text immediately following the Failure: line (or the observation harness tool-result block if instrumentation was required) must be the first witness line — no narrative, "
        "causal argument, prose explanation, or other content may appear before the first 'Property [N] assertion [M] witness:' line; emit exactly one witness line for every "
        "previously inventoried executable assertion, in ascending assertion inventory order without omission or reordering; each witness line takes one of these forms, derived "
        'exclusively from the tool-result block: \'Property [N] assertion [M] witness: Failure — assertion: "<verbatim assertion text>" — observed: "<verbatim tool-result text '
        "uniquely identifying this assertion as the failure by assertion text, source location, or execution identifier>\"' if and only if the tool-result uniquely identifies this "
        "assertion as a failing outcome — when identical assertion texts appear more than once, the failure must identify the specific occurrence by source location or execution "
        'identifier; \'Property [N] assertion [M] witness: Success — assertion: "<verbatim assertion text>" — observed: "<verbatim tool-result text confirming this assertion '
        "passed>\"' if the tool-result contains a directly observed passing outcome for this assertion; if an assertion cannot be classified Success or Failure from the current "
        "tool-result, it is not yet observable — before accepting that conclusion, determine whether it is due to dead code, unreachability, mis-inventory, or harness omission; refine "
        "the observation harness or violating state to resolve the cause and repeat the instrumented execution; only after no executable instrumentation or violating state "
        'modification can make the assertion observable may \'Property [N] assertion [M] witness: Unobservable — assertion: "<verbatim assertion text>" — <observed reason quoted from '
        "tool-result>' be emitted; Unobservable is the last resort and must be justified by a verbatim tool-result excerpt; an Unobservable witness is treated as a guard defect — "
        ...
        "verify that every assertion observed in the harness or execution output appears exactly once in the assertion inventory — if any observed assertion is absent from the "
        "inventory, emit 'Assertion inventory: incomplete — missing assertion: <text>' and repeat from step (2); at least one inventoried assertion must receive witness classification "
        "Failure — a guard execution in which no inventoried assertion is classified Failure does not satisfy step (4), regardless of whether a Failure: line was produced; a Failure "
        "witness whose observed text is not present verbatim in the corresponding tool-result block does not satisfy this token; if no inventoried assertion can be attributed to the "
        "observed failure, revise the violating state or the regression guard and repeat from step (2); after all assertions are witnessed and the inventory is verified complete, emit "
        "'Assertion witnesses: complete — <count> assertions witnessed' where <count> must equal the count from 'Assertion inventory: complete'; a witness count that differs from the "
        "inventory count does not satisfy this token; emit 'Assertion witnesses: incomplete — <reason>' if any assertion lacks a witness; if incomplete, revise the violating state, "
        "observation harness, or guard and repeat from step (4); no 'Quoted implementation:', implementation modification, or 'Implementation overreach:' may appear before 'Assertion "
        "witnesses: complete' has been emitted; no subsequent step may occur until 'Assertion witnesses: complete' has been emitted; (5) identify the governed artifact for property "
        "[N] — the artifact whose modification satisfies property [N]; depending on the property, this may be the implementation, the regression guard, a configuration file, or "
        "another explicitly governed artifact; modify the governed artifact to satisfy property [N], then immediately emit 'Quoted implementation: <smallest contiguous verbatim text "
        "uniquely identifying the modification to the governed artifact>'; if step (4) required constructing a temporary violating state because the governed artifact already "
        "satisfied property [N], step (5) is satisfied by confirming the original state is restored — emit 'Quoted implementation: <verbatim text identifying the restored governed "
        "artifact>' to confirm; no subsequent step may occur until this sentinel has been emitted; (6) attempt to construct a minimal modification to the quoted implementation that "
        "preserves the success of the quoted guard while still changing the governed artifact, and attempt to construct a concrete execution path through the quoted implementation "
        "that is not exercised or distinguished by the quoted guard; emit 'Implementation overreach: found — <modification or execution path>' or 'Implementation overreach: not "
        "found'; if found, minimally revise the governed artifact to eliminate the overreach, emit a new 'Quoted implementation:', and repeat from step (6); every implementation "
        "overreach analysis must reason only from the quoted implementation and quoted guard emitted earlier in the transcript, not from an imagined or intended version. After all "
        "current properties complete their six-step cycles, begin the implementation audit. 'Coverage: complete' is the global fixed-point sentinel and may be emitted only after every "
        "property has completed, the implementation audit has converged, and no new properties were derived in the final audit pass. The implementation audit is a convergence loop: "
        "repeat — (a) enumerate every modified artifact; (b) enumerate every externally callable entry point of the governed artifact; (c) enumerate every distinct execution branch; "
        "(d) enumerate every caller or consumer whose observable output changed; (e) enumerate every externally visible output or state transition; for the first observable behavior "
        "not governed by any retained property, emit 'Audit: implementation gap — <description>', then restart the Ground protocol from the full retained property set incorporating "
        "the newly discovered properties — including atomicity, completeness, and observational independence checks until a new '§ ground complete' is reached — then execute the full "
        "six-step falsify cycle for every newly retained property — until no audit pass discovers an ungoverned observable behavior, then emit 'Audit: implementation complete'; "
        "'Coverage: complete' may not be emitted before 'Audit: implementation complete'. A response that omits any required sentinel, performs a step before emitting its required "
        "quoted artifact, modifies the governed artifact to satisfy property [N] before a quoted Failure line appears, omits any executable assertion from the assertion inventory, or "
        "leaves a property without an automatic regression guard does not satisfy this token.",
