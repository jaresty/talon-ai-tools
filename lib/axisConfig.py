...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "measurement, a trial, or a comparison are all instances. A guard is established by an observation-producing act whose result record contains the guard's procedure verbatim; "
        "every assertion referenced later must be copied verbatim from such a result record. Because a guard detects regression only if it re-applies on its own, a Failure observation "
        "is valid only when it is produced by applying a committed, re-runnable guard artifact — a procedure persisted to a location that re-applies without human initiation — never "
        "from an inline or ephemeral one-off. Repeatable is not the same as deterministic: a guard whose observation depends on external or changing state (a live lookup, a "
        "measurement, a poll) is still a valid witness at the moment it is applied, but its observation and provenance must be retained rather than treated as reproducible on demand. "
        "Capability is not evidence: what witnesses a behavioral observation is an available qualifying observation record, not the mere fact that an act did or did not occur this "
        "turn. A qualifying observation record is a transcript-present record produced by the committed guard artifact that carries the guard's procedure identity, the assertion's "
        "identity, and the relevant A-result. A fresh application of the guard produces one; a prior result record already present in the transcript is one; model prose is never one. "
        "When the observation capability is available the response must apply the committed guard to obtain new observations; when it is unavailable it may cite qualifying observation "
        "records already present but must not synthesize, infer, or narrate an observation. A behavioral assertion for which no qualifying observation record exists is neither Failure "
        "nor structural Unobservable — emit 'Unwitnessed: assertion \"<verbatim assertion>\" — observation unavailable'; it does not contribute to 'Coverage: complete', and it is a "
        "witnessing gap, not a structural exemption. Before acting, observe the gap: emit 'Observing gap: <what is currently absent or failing>'. Shrink the artifact to the guard "
        "(Gate 1). Always attempt the simpler artifact: construct a smaller modification to the artifact and apply the guards to it. 'Overreach: not found' is a claim that no such "
        "reduction exists, so it is valid only after an attempted reduction — a qualifying observation record showing a smaller artifact that the guards reject (or a smaller artifact "
        "they accept, which you then keep). If a reduction the guards still accept exists, or a path through the artifact is not exercised or distinguished by any guard, emit "
        "'Overreach: found — <modification or path>', trim to the boundary the guards require, and repeat. A bare 'Overreach: not found' with no attempted reduction does not satisfy "
        "this token. Both 'Overreach: found' and 'Overreach: not found' are observation-dependent; when no qualifying observation is available, emit 'Overreach: untested — observation "
        "unavailable' rather than either verdict — untested is never 'not found'. Witness every assertion (Gate 2). Enumerate the assertions: emit one 'Assertion: <verbatim assertion "
        "text>' line for each assertion of the established guard, quoted verbatim from the guard's result record. The verbatim lines are the observable; no count or completeness "
        "sentinel is used, and a self-declared count does not satisfy it. Every enumerated 'Assertion:' line must then receive its own 'Failure: assertion \"<verbatim assertion>\" — "
        "\"<verbatim failure text>\"' or 'Unobservable: assertion \"<verbatim assertion>\" — structural' before 'Coverage: complete'; the binding is by verbatim identity — the "
        "assertion text in the Failure or Unobservable line must match an enumerated Assertion line exactly, and an enumerated assertion with no matching outcome line leaves coverage "
        "incomplete. After each Failure or Unobservable outcome, if any enumerated 'Assertion:' line still lacks a verbatim-matching outcome, return to the witness step for the next "
        "unwitnessed assertion and repeat; do not leave Gate 2 until every enumerated assertion has a matching outcome. Each enumerated assertion is either behavioral or structural. A "
        "behavioral assertion is witnessed only by a pair of A-observations: an application in which A's own guard-defined result is a failure, and one in which A's own result is a "
        "pass. An A-observation is an application of the committed guard in which assertion A is evaluated and produces its own guard-defined result. The guard defines the observable "
        "result for A: an A-pass is the guard's reported pass result for A and an A-fail is the guard's reported failure result for A. These results are observations of the guard, not "
        "independent claims about the truth of any property — whether A's result corresponds to the property A is intended to govern is not established by this token; that "
        "correspondence is established separately by the adequacy check. What this token establishes is only that A distinguishes its own two results: an application in which A is "
        "evaluated and reports failure, and one in which A is evaluated and reports pass. The failure (A-fail) application must immediately precede the 'Failure:' line (it is the "
        "producer); the passing (A-pass) application is the contrast evidence and may appear elsewhere in the transcript. Any application that does not produce an A result is not an "
        ...
        "typed as prose that does not appear verbatim in a cited qualifying observation record is not evidence and does not witness A. If the artifact already passes A, construct a "
        "temporary modification that makes A alone fail while every other assertion still passes, apply the guard, emit the Failure from that result record, then restore; a "
        "modification that fails every assertion at once discriminates none. Emit 'Failure: assertion \"<verbatim assertion>\" — <A-fail result> / <A-pass result>' quoting both "
        "observed results from their result records; a 'witness:' line, if added, only projects those two results and is not itself evidence. A structural assertion — one whose "
        "property has the artifact's own text or structure as its primary subject, so no behavioral failing application exists — is witnessed instead by 'Unobservable: assertion "
        ...
        "the moment it would need to fire; (c) ephemeral guard not committed to a location that re-applies. Verdict-follows-observation governs both gates: when the observation "
        "capability is available, every 'Overreach:' verdict and every 'Failure:' observation is valid only when it immediately follows a result record that mechanically produces it; "
        "a verdict emitted from description or analysis alone does not satisfy this token. A structural Unobservable assertion is exempt, having no application to anchor to. "
        "'Coverage: complete' is valid only when every enumerated assertion has either a 'Failure:' line backed by its A-fail/A-pass application pair or an 'Unobservable: … — "
        "structural' line, and no 'Overreach: found' remains open; an 'Unwitnessed: … — observation unavailable' assertion leaves coverage incomplete and does not satisfy this token. "
        "When qualifying observation is unavailable for one or more behavioral assertions, the honest terminal state is 'Coverage: incomplete — observation unavailable', not a claimed "
        "completion; the protocol may construct guards, perturbations, and expected outcomes and identify what remains unverified, but it may not represent any observation-dependent "
        "verdict as observed.",
