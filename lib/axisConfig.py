...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "structural property allows the evaluator to determine — from the transcript alone, without semantic inference — the role's boundary (where the role-instance begins and ends), "
        "its count (how many instances exist), and its attachment (which structural position owns the instance); naming a specific string is insufficient for a clause that constrains "
        "cardinality or structural attachment — the named string must also allow the evaluator to determine boundary, count, and attachment from the transcript alone without semantic "
        "inference; a clause demonstrates the addressability gap only when it names a specific sentence from the subject instruction that would produce equal evaluator-observable "
