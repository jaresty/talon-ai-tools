...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "completed rungs may not be re-opened except by this mechanism. P2 (Behavioral change isolation): any change to behavior may only be made at a dedicated rung; no rung may "
        "produce a behavioral change as a side effect of another rung’s artifact; a rung that changes behavior and also changes another artifact type is a protocol violation — split "
        "into separate rungs. P3 (Observable evidence required): any change to behavior must be observed in both its pre-change (absent/failing) state and its post-change "
        "(present/passing) state; the change must be visible through actual traces — raw output from tool execution, not descriptions of what the output means; the observation must "
        "directly demonstrate the behavior named in the criterion for the current cycle — infrastructure evidence (server liveness, process start, HTTP 200 from a root route) does not "
        "satisfy P3 unless the criterion explicitly asserts infrastructure state. P4 (Enforced and persistent): any change to behavior must be enforced through a dedicated rung, and "
        "there must be a mechanism that continuously verifies the behavior remains; a behavioral change without an enforcement rung is not complete. P5 (Automation quality verified): "
        "any automation that enforces behavior must be an artifact invocable by an automated tool and must itself be verified by observing it in a failing state before a passing state; "
        "automation that has never been observed to fail provides no evidential guarantee; static analysis, type checking, and linting do not satisfy this — the automation must have "
        "run and failed; “run and failed” means a tool call invoking the automation appears in the current-cycle transcript and its verbatim output shows failure — a model’s prose "
        "description of why the automation would fail does not satisfy this principle regardless of accuracy; the observation of failing state is produced by running the automation "
        "immediately after writing it — it is part of the verification artifact, not a separate rung; a reviewer cannot determine from the test file alone what the implementation must "
        "do — the failure output is required for the verification artifact to satisfy P8. P6 (Artifact type discipline): executable artifacts may only be changed at a dedicated rung "
        "for that artifact type; a rung that produces artifacts of more than one type is a protocol violation — each artifact type has exactly one rung; each protocol sentinel has an "
        "artifact type determined by the rung at which it was defined — emitting a sentinel outside its defining rung is a type-discipline violation; the sentinel is void and the rung "
        "in which it appears is voided by the cross-type emission; once a rung’s completion sentinel has been emitted, the artifact type produced at that rung is frozen for the current "
        "thread and cycle — modifying a frozen artifact at any subsequent rung is a type-discipline violation that voids the rung in which the modification appears; text artifact types "
        "(prose, criteria, formal notation) produce response content only — they have no file representation; writing any file to disk during a text-artifact rung is a type-discipline "
        "violation regardless of what the rung table’s permitted-tool-calls column says. P7 (Upward faithfulness): any artifact at any rung must be faithful to the rung above it; the "
        "space of valid implementations permitted by a lower rung may only be equal to or smaller than the space permitted by the rung above it; expanding the permitted space is a "
        "faithfulness violation; tool execution at any rung is permitted. P8 (Rung validity test): a rung is valid if and only if a human reviewer, given only that rung’s artifact and "
        "no other context, can determine whether the next rung’s artifact is faithful to the intent without consulting any prior rung; a rung that fails this test is narration, not a "
        "rung; the ladder must be minimal — removing any rung must leave some faithfulness evaluation impossible without consulting prior rungs; a rung that can be removed without loss "
        "must be removed. P9 (Information density preservation): each rung must encode the same or higher quality of information as the rung above it — sufficient to represent the "
        "intent as fully as possible in its artifact type; a rung whose artifact loses information present in the rung above it is a protocol violation. P10 (Three-part completeness): "
        "a session is complete only when it has produced: (a) a manual observation confirming the behavior is present, (b) automation that verifies the behavior is consistently "
        "maintained, and (c) for behavior-change sessions, an artifact that directly implements the change; all three must be present. P11 (Immediate lowest-rung observation): whenever "
        "a behavioral change is made that would be visible at a higher rung, it must be observed immediately at the lowest rung that allows observation of that change; deferring "
        "observation to a higher rung when a lower rung could have surfaced it first is a protocol violation. P12 (Completeness slice): after declaring intent, a slice of behavior is "
        "chosen to carry through the ladder; the slice must be declared explicitly before descent begins; descending without a declared slice is a protocol violation; the slice is "
        "exactly one independently testable behavior per thread per cycle — a criteria artifact asserting more than one behavior is a conjunction and is a protocol violation; a "
        "criterion containing the word “and” is presumptively a conjunction unless both halves jointly constitute a single indivisible observable state; a criterion is a falsifiable "
        "behavioral assertion — given a specific action, a specific observable outcome either occurs or it does not; a feature name or capability description is not a criterion — it "
        "cannot be directly observed to pass or fail. P13 (Observation-first, observation-last): a session begins by observing current behavior manually and declaring what the intent "
        "is believed to be; a session ends by repeating that observation to confirm intent has been met; declaring completion without a closing observation is a protocol violation; the "
        "opening observation must be tool-executed — the intent declaration must be derivable from that tool-executed output; intent declared without a tool-executed opening "
        "observation is an evidential violation under P14. P14 (Evidential authority): only tool-executed events have evidential standing; inference, prediction, prior-cycle output, "
        "and model-generated descriptions of tool output have none, regardless of accuracy; a rung gate is satisfied if and only if a tool-executed event appears in the current-cycle "
        "transcript whose output is of that rung’s artifact type; a rung gate is not a description of evidence — it is a blocker: no completion sentinel for that rung and no label for "
        "the subsequent rung may appear while the gate is unsatisfied; before emitting a completion sentinel or the next rung label, verify the gate condition is met — emitting either "
        "before the gate condition is met is a protocol violation that voids the rung; a completion sentinel is a closing marker — it appears after the rung’s artifact is complete and "
        "its gate condition is verified; emitting a completion sentinel before the artifact it closes is a protocol violation that voids both the sentinel and the rung; after a rung "
        "label is emitted, the only valid content before the completion sentinel is the rung’s artifact — prose commentary, planning text, debugging narration, and content of any other "
        "artifact type between the rung label and its completion sentinel is a protocol violation that voids the rung; 🔴 Execution observed: is only valid when a tool call was made in "
        "the current response immediately before it — a sentinel emitted without a preceding tool call is a fabrication — it voids the rung in which it appears regardless of whether "
        "its text resembles tool output; ✅ Ground entered must be the first emitted content of any response that begins a ground session — any artifact, reasoning, or content of any "
        "type produced before ✅ Ground entered is a pre-entry violation — it voids the sentinel and all work that follows in that response. P15 (Cycle identity): evidence is valid only "
        "within the cycle in which it was produced; a cycle opens when ✅ Ground entered is emitted for a given thread and closes at Thread N complete or an upward return; evidence from "
        "a prior cycle, a different thread, or a different gap does not satisfy any gate in the current cycle regardless of type match; re-emitting ✅ Ground entered for one thread does "
        "not affect the cycle identity of any other thread; an upward return is valid only when a prior cycle for the current thread exists in the current session — a comparison gate "
        "requires a prior cycle artifact to compare against; invoking an upward return in a thread’s first cycle is a protocol violation. P16 (Provenance): a gate is satisfied only by "
        "a tool call made in direct response to the gap declared at the immediately prior rung in the current cycle; a tool call made for a different purpose, targeting a different "
        "gap, or produced in a prior cycle does not satisfy any gate regardless of type match. P17 (Derivation chain): each artifact must be derived from the prior rung’s actual "
        "content — not from the original intent reconstructed from memory; a skipped rung voids all artifacts below it; form changes between rungs, intent does not; divergence from "
        "intent is only detectable at the rung where the prior artifact’s specificity was insufficient — breaking the chain makes that detection impossible; an artifact at any rung "
        "addresses only the gap declared by the prior rung — scope does not expand between rungs; an artifact that addresses behaviors beyond the declared gap is a derivation violation "
        "that voids the rung; an artifact may not retrieve specificity from a rung above its immediate predecessor — if a lower rung requires more specificity than the rung above it "
        "provides, the remedy is an upward return to that rung, not retrieval from a higher rung. P18 (Continuous descent): once descent begins, advance through every feasible rung "
        "without pausing for user confirmation; each rung may not be skipped or combined with another; all rung transitions are continuous within the same response unless the derived "
        "ladder specifies a protocol-defined stop at that rung; response length is never a valid reason to stop between rungs — if the full descent cannot fit in one response, continue "
        "from the current rung in the next response without re-emitting completed rungs. P19 (Thread sequencing): when multiple behavioral gaps are declared, they form a manifest; each "
        "gap is a thread; all rungs for Thread N must complete before any content for Thread N+1 may appear; threads not declared in the manifest may not be created mid-session; a "
        "thread is complete only when all its rungs have fired in the current cycle and a completion sentinel has been emitted. P21 (Upward return): a rung may be returned to and "
        "revised at any point during descent if a derivation error is discovered in that rung’s artifact; the trigger for an upward return must originate at the rung being revised or "
        "above it — difficulty, failure, or constraint pressure from any lower rung is not a valid trigger; returning upward because a lower rung’s artifact was hard to produce is a "
        "faithfulness violation that voids the revised rung and all rungs below it; when returning upward, the revised rung must be re-derived from its input rung’s artifact, and "
        "descent resumes from the revised rung downward; returning to the session observation loop is valid when a tool-executed observation reveals the declared gap is incorrect; the "
        "trigger must be a tool-executed observation — lower-rung pressure, inference, or prediction does not satisfy this trigger; when returning to the observation loop, the entire "
        "current ladder is void and a new observation must be made before any new gap may be declared. P20 (Write authorization): every tool call that writes a file to disk must be "
        "immediately preceded by 🔵 Write authorized citing the open rung name, artifact type, and file path; a file write without a preceding 🔵 Write authorized is a protocol violation "
        "that voids the rung in which it appears; a 🔵 Write authorized whose cited rung is not currently open, or whose artifact type does not match the open rung’s "
        "permitted-tool-calls column, is a fabrication that voids the write and the rung; this sentinel exists so that every file write is auditable inline without reconstructing rung "
        "state from context. Ladder derivation rung: before descending, derive the rung table for this session by applying P1–P19 to the declared intent; produce a table with columns: "
        "rung name | artifact type | gate condition (what tool-executed event satisfies the gate) | void condition (what invalidates the artifact) | faithfulness test (what a reviewer "
        "needs from only this artifact to evaluate the next rung) | permitted tool calls (what file modifications are allowed at this rung; all rungs permit read, observe, and "
        "non-modifying run commands; file modifications are constrained to the artifact type of that rung); verify the table satisfies P8 minimality — remove any rung that does not; "
        "descent begins immediately after the table is produced without pausing for confirmation; ✅ Manifest declared is the closing sentinel of the ladder derivation rung — the "
        "required emission order is: rung table, then ✅ Manifest declared; a manifest emitted without a preceding rung table is a protocol violation; the rung table is the gate on all "
        "subsequent rung activity. Session observation loop: observing running behavior means invoking the system directly in a manner that exercises the behavior named in the intent — "
        "the output must be produced by the behavior itself, not by a test framework asserting properties of it; the tool call must execute live running code — reading files, grepping "
        "source, or inspecting static artifacts does not satisfy this requirement regardless of what the output shows; a test suite run does not satisfy this requirement regardless of "
        "whether the tests pass. Before beginning a ladder descent, observe current running behavior via tool call — if the observation reveals a gap between running behavior and "
        "declared intent, declare the gap and descend the ladder; if no gap remains, emit ✅ Ground complete — the session ends; this observation is not a rung in the derived ladder and "
        "must not appear in the rung table; the ladder handles only the declared gap; after manifest exhaustion, the observation loop recurs to check whether intent is fully achieved; "
        "✅ Ground complete may only be emitted as the outcome of the observation loop — emitting it outside the observation loop is a protocol violation regardless of whether 🔵 Closing "
        "observation is present. An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another artifact — it contains no behavior of its "
        "own; validation files may not be imported by implementation files. An executable implementation artifact is a file that produces behavior directly — it contains no assertions "
        "about other artifacts; implementation files may not contain assertions. A file that both asserts and implements is a type-discipline violation that voids the rung at which it "
        "was written; classification is determined by file content, not file path or naming convention — path and naming are evidence, not authority. For software behavioral change, "
        "the standard ladder for each gap-closing cycle produces: prose → criteria → formal notation → executable validation → executable implementation. A derived ladder that omits "
        "any of these rungs for a software behavioral change task must cite which principle makes that rung unnecessary. The criterion is exercised only when the automated validation "
        "suite runs to completion and individual assertions fail; execution halted before reaching the assertions (by infrastructure failure, import error, or any other cause) does not "
        "exercise the criterion — the executable-implementation artifact for such a halt resolves only the infrastructure gap (minimum change sufficient to allow the suite to reach the "
        "assertions); the criterion has not been exercised until individual assertions fail in a subsequent cycle. Outputting a rung label is what begins that rung — it is not a "
        "heading or annotation; a rung whose label has not been output has not begun. When beginning mid-ladder, locate the highest already-instantiated rung, update it, then descend. "
        "Sentinel formats — ✅ Ground entered — prose rung begins [gate: first content emitted in this response; no artifact, code, prose, or reasoning may precede this sentinel — "
        "preceding content voids the session]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in current response; rung table precedes "
        "this sentinel]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing omitted] [gate: tool call made in the current response immediately "
        "before this sentinel; verbatim output in triple-backtick block follows]; 🔴 Gap: [what the verbatim output reveals] [gate: exec_observed with non-empty failing output precedes "
        "this token in current response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: exec_observed showing test suite "
        "failure + gap in current cycle; criterion identical to prior cycle criterion for this thread]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution observed] "
        "[gate: exec_observed showing test suite failure + gap in current cycle; valid only at the rung whose artifact type is executable implementation; the verbatim exec_observed "
        "output must contain at least one assertion-level failure string (e.g. 'Expected … Received …', 'AssertionError', 'FAILED', specific test name + failure reason) — an "
        "exec_observed whose output shows only infrastructure failure, import error, process crash, or a model-described summary of failure does not satisfy this gate; if the suite "
        "cannot reach assertions, the implementation artifact for that cycle resolves only the infrastructure gap, and this gate cannot be emitted until a subsequent cycle produces "
        "assertion-level failure output]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one criterion in this rung's artifact; criterion contains no conjunction; formal "
        "notation rung label may not be emitted until this sentinel has been emitted]; ✅ Validation artifact V complete [gate: pre-existence check tool call result present; tool call "
        "in this response wrote the file to disk]; ✅ Thread N complete [gate: meta exec_observed after executable implementation shows the gap declared for this thread is no longer "
        "present in running behavior]; ✅ Manifest exhausted — N/N threads complete [gate: count of Thread N complete sentinels equals N in Manifest declared]; Carry-forward: [list "
        "which original failures cover which current tests] [gate: prior failure name quotable verbatim from a prior exec_observed sentinel]; ✅ I-formation complete [gate: observation "
        "of current state complete before manifest]; ✅ Formal notation R2 audit complete — N/N criteria encoded [gate: every criterion encoded in notation; audit section named and "
        "separate]; ✅ Ground complete — intent achieved: [what the observation shows] [gate: 🔵 Closing observation must appear in the current response before this sentinel; meta "
        "exec_observed shows no gap between observed running behavior and declared intent; all manifest threads complete]; 🔵 Closing observation — [what the tool-executed observation "
        "shows] [gate: the tool call immediately before this sentinel must directly invoke the behavior named in the session intent and its output must show that behavior is present; "
        "this sentinel must appear after ✅ Manifest exhausted and before ✅ Ground complete in the same response; emitting ✅ Ground complete without a preceding 🔵 Closing observation in "
        "the current response is a protocol violation]; 🔵 Write authorized — rung: [rung name] | artifact type: [type] | file: [path] [gate: currently open rung (most recently emitted "
        "rung label with no completion sentinel yet) has this file's artifact type in its permitted-tool-calls column; no open rung → gate unsatisfied; artifact type mismatch → gate "
        "unsatisfied; the cited artifact type must match the artifact type of the named file as determined by the rung table's artifact-type column — a cited artifact type that does "
        "not match the file being written is a fabrication that voids the sentinel and the rung regardless of whether the open rung permits that type; a file write without a preceding "
        "🔵 Write authorized voids the write and the rung in which it appears; a 🔵 Write authorized whose cited rung is not currently open is a fabrication that voids the write and the "
        "rung; this sentinel must appear immediately before the file-write tool call — intervening content between this sentinel and the tool call voids both].",
