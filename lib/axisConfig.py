...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "evidence: no proof gaps are real; RULE: Challenge = produce evidence that baseline does not satisfy criteria [ ] Baseline: derive from P2,P3 — why Baseline defines evaluation "
        "framework not solution; what GOES WRONG if Baseline IS the solution: no evaluation framework exists to prove gaps; RULE: Baseline = evaluation scaffold (tests, criteria "
        "structure), NOT the solution itself [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create without structure: "
        ...
        "framework (tests, validation logic). Evidence: evaluation structure exists. IMPORTANT: Baseline is the EVALUATION scaffold, not the solution. Baseline defines HOW the solution "
        "will be evaluated. 5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, run evaluation and produce evidence that criteria are not satisfied. Evidence: "
        "failing evaluation output proves gaps. 6. Refinement — How is the artifact improved? Derivation: from challenge, derive targeted changes. Evidence: addresses specific gaps. 7. "
        ...
        "evaluation defined after solution is vacuous (Texas sharpshooter). Baseline defines evaluation framework; Challenge applies it to show gaps exist; Refinement produces solution "
        "that passes evaluation. for writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to "
