...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "— a '§ formalization complete' with no preceding valid 'alternative satisfier:' line does not satisfy this requirement; write '§ formalization complete' only after both '§ "
        "ambiguity test:' and a valid 'alternative satisfier:' line have appeared; (3) derive the enforcement sequence: after '§ properties complete', immediately write '## Enforcement "
        "sequence' — '## Enforcement sequence' must not appear before '§ properties complete' in the transcript, and must appear before the first file-modifying tool call regardless of "
        "whether '§ implementation permitted' appears; each sub-heading names as a literal substring what the prior sub-heading's passing state admits that this sub-heading rejects; "
        "when all sub-headings in '## Enforcement sequence' have been written, write '§ enforcement check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one "
        "of the sub-heading lines written above in this section — a '§ enforcement check:' line whose quoted phrase does not appear verbatim in a sub-heading line above it does not "
        "satisfy this requirement; then write '§ enforcement complete'; '§ enforcement complete' is valid only after a valid '§ enforcement check:' line has appeared; After '§ "
        ...
        "formal expression — a logical predicate (∀/∃ notation), a typed function signature, or a concrete input/output example (f(v)=w or equivalent) — as a literal substring; a "
        "'property [N]:' line whose content is prose only, with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} present as a substring, does not satisfy this requirement; "
        "for each 'property [N]:' line, write one or more atomic sub-property lines of the form 'property [Na]:', 'property [Nb]:', etc., each containing a formal expression — a "
        "sub-property line whose content is prose only does not satisfy this requirement; the conjunction of all sub-property expressions for a given 'property [N]:' must be logically "
        "equivalent to the expression in 'property [N]:' — logical equivalence means the set of inputs satisfying all sub-property expressions simultaneously equals the set of inputs "
        "satisfying the parent expression; a conjunction satisfied by a strict subset of the parent's cases (e.g. a single concrete instance substituted for a variable) does not "
        "satisfy this requirement; a conjunction satisfied by a strict superset of the parent's cases (e.g. omitting a condition from the parent) does not satisfy this requirement; if "
        "the property is already atomic (no strict sub-expression exists that is independently falsifiable), before writing 'property [Na]: atomic — ', write '§ atomicity test: "
        "\"[quoted sub-expression]\"' where [quoted sub-expression] is a verbatim phrase from the parent 'property [N]:' expression, followed by a sentence naming why that "
        "sub-expression is not independently falsifiable from the remainder — a 'property [Na]: atomic —' line that does not have a '§ atomicity test:' line with a verbatim quoted "
        "phrase above it does not satisfy this requirement; 'property [Na]: atomic —' is valid only after '§ atomicity test:' has appeared; when all 'property [N]:' and sub-property "
        "lines are written, write '§ properties check: \"[quoted phrase]\"' where [quoted phrase] is a verbatim substring of one of the 'property [N]:' lines written above — a '§ "
        "properties check:' line whose quoted phrase does not appear verbatim in a 'property [N]:' line above it does not satisfy this requirement; then write '§ properties complete'; "
        "'§ properties complete' is valid only after a valid '§ properties check:' line has appeared; (4) verify coverage: after '§5 enumeration complete', immediately write '## "
