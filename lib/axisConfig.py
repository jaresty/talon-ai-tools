...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "form": {
        ...
        "rather than elicits — path (b) is closed globally by the Predict: requirement; paths (a) and (c) require a per-question check — before each question, write 'Path (a) check: "
        "[reason the phrasing does not reveal the answer]' and 'Path (c) check: [what the question demands that has not yet appeared in the transcript]'; proceed to writing the question "
        "only after both check strings appear above it. Before writing any question, name the concept that will carry the Misconception:/Why: pair in this quiz — this commitment must "
        "appear in the transcript before the first question is written; if no concept naturally carries a common misconception, choose the concept whose incorrect prediction would be most "
        "damaging. For each concept after the first, begin with a Following from: line containing (1) a verbatim quoted phrase from the preceding answer and (2) a one-sentence restatement "
        "of what that reveal established, making the line self-contained without requiring the reader to scroll back. Then write the question, require a Predict: line (the reader names "
        "their expectation before the answer is shown). An answer is permitted in the same response turn as a Predict: line only when an output-exclusive channel token is present in the "
        "constraints governing this response; in that case the answer follows immediately. When not in channel mode, write the answer in the response turn after a Predict: line appears in "
        "a human message turn — a Predict: line appearing inside a tool result block does not satisfy this gate. After each answer: a gap-reveal sentence comparing what the Predict: line "
        "stated against what is correct or complete; then for every concept except the first and the last, a Hook: line quoting a specific phrase from the current answer and naming the "
        "implication it leaves unresolved — the question the next concept answers; a Hook: line is compliant only when it appears after the gap-reveal sentence for this concept and before "
        "the Following from: line of the next concept (evaluator check: the quoted phrase appears verbatim in the current answer); a Hook: line must not appear after the last concept's "
        "answer; then a Misconception: line and Why: line — compliant only when they appear in the answer block of that named concept (the concept named in the pre-quiz commitment); a "
        "Misconception: line names the specific incorrect belief that would lead a reader to predict the wrong answer; a Why: line states the structural reason that belief is wrong; then "
        "an association sentence — before writing it, name the numbers already used in prior association sentences — using the format '#N (Name): [relationship]' where N is the "
        "opening-list number of a prior concept whose number has not appeared in any earlier association sentence in this quiz and Name is that concept's name from the opening list (for "
        "the first concept, use the format '#N (Name): [relationship]' naming the opening-list number and name of the first concept whose understanding it enables). emit '✓ [concept name]' "
        "only when the Predict: line states the complete and correct answer with no correction, addition, or nuance needed — if any gap exists between the prediction and the answer, write "
        "a gap-reveal sentence instead of '✓'. Continue until every concept in the opening list has either a '✓ [concept name]' marker or a gap-reveal sentence in the transcript, or the "
        "user explicitly declines to continue; then close with a terminal declaration containing the verbatim name of every concept from the opening list — an evaluator determines "
        "completeness by comparing the terminal declaration against the opening list and confirming every name appears. With an output-exclusive channel, structure the output as a quiz "
