...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "session — that appears in the transcript before any tool call that modifies a file whose modification would change the outcome of any execution result in the transcript — not "
        "asserted in prose. A file is governed if its modification changes any such result, regardless of whether the change is an addition, deletion, or rename; classifying a file as "
        "ungoverned based on change type rather than execution-result impact is not valid. A FAIL that appears after any such modification is not valid regardless of its position "
        "relative to the governing artifact citation. Before the run tool call, produce a derivation block as three entries: first, name at least one slower check that would catch "
        "this behavior but takes longer to run locally — name it by file path or command; do not run this check; name what specific test or assertion in that check covers this "
        "behavior — the named test must be capable of catching this specific behavior; a description that states or implies the test does not cover the governed behavior — including "
        "qualifications such as 'but not [behavior]', 'does not cover', or 'cannot distinguish' — is self-refuting and invalidates the nomination regardless of the test's actual "
        "capability; name a different test whose description affirms coverage without qualification; a claim that the named test cannot catch this behavior must be grounded in a prior "
        "execution result showing the test passing while the behavior was absent — quote that result; a reasoned claim of incapability without such a quote is not valid and the slower "
        "check stands as derived; if no capable test can be named and no such quote can be produced, the slower check is not derived and the candidate check must not be named — choose "
        "a different candidate for which a nameable test exists; second, name the candidate check — the specific artifact to run, naming its relative speed compared to alternatives; "
