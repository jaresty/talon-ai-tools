...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "appear immediately before each file-write tool call to implementation code; include a unique id for this edit that will be matched by impl_intent_achieved; the file must be of "
        "the artifact type permitted at the current rung - writing a test file at the implementation rung is a type-discipline violation; if a file of a different artifact type needs "
        "editing, emit an upward return to the appropriate rung first; the target must be a specific test assertion name and the evidence must be verbatim output from running the "
        "validation file showing that test assertion failing in the current state; emitting this sentinel without a preceding exec_observed showing the target test assertion failing "
        "voids the intent and the subsequent write; fabrication of red evidence (output that does not come from actual test execution) voids the sentinel and the rung; for backfill "
        "cases where tests already pass and cannot be seen red, perturb the implementation (introduce a controlled fault) to observe the test fail, then fix the perturbation]; 🔵 Intent "
        "achieved — id: [unique id] | file: [path] | target: [assertion name] | evidence: [verbatim green output showing passing] [gate: this sentinel must appear immediately after a "
        "tool call in this response that runs the validation file to show the target test assertion passing; the unique id must match the id from impl_intent in this response, the "
        "target test assertion name must match the target from impl_intent and the evidence must be verbatim output from running the validation file showing that test assertion passing "
        "in the current state; emitting this sentinel without a preceding tool call that runs the validation file is fabrication that voids the sentinel and the rung; emitting this "
        "sentinel without a matching impl_intent in the same response voids the intent; fabrication of green evidence (output that does not come from actual test execution) voids the "
        "sentinel and the rung; each file-write to implementation code requires its own impl_intent + impl_intent_achieved pair with matching unique ids - the pair must repeat for "
        "every edit at the implementation rung; skipping the pair for subsequent edits is a protocol violation].",
