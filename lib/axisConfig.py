...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "satisfy rung gates regardless of accuracy. Every 🔴 Execution observed: sentinel at any rung must contain verbatim tool-call output, triple-backtick delimited, complete, "
        "nothing omitted — prose descriptions, inline summaries, and model-generated text that resembles output do not satisfy this requirement at any rung. Rule 2: each artifact "
        "derives from the prior rung — form changes, intent does not; a skipped rung voids all artifacts below it; each artifact addresses only the gap declared by the prior rung — "
        "nothing beyond it. Rule 3: every thread ends when tool output names the declared gap; stop there. I is the declared intent governing the invocation; I precedes and is not "
        "itself an artifact; each artifact must be self-sufficient to derive the next without consulting I; faithful derivation requires form to change, intent to stay fixed. Advance "
        "through every feasible rung; completeness governs rung depth, not rung existence; each rung may not be skipped or combined with another, including the observed running "
        "behavior rung — it is required in every cycle. Before any tool call or file read, check: if no 🔴 Gap: sentinel exists in output yet, emit it now; before emitting 🟢 "
        "Implementation gate cleared, check: does a 🔴 Execution observed: sentinel from the current cycle's validation run observation rung — not from a prior cycle — exist in output? "
        "if not, do not emit it — produce the executable validation artifact instead. 🟢 Implementation gate cleared licenses the first edit — it does not complete the thread; after "
        ...
        "valid; the next required action after 🟢 is a tool call that creates or modifies an implementation file — ✅ Thread N complete, a new criteria cycle, or any other sentinel may "
        "not appear until at least one edit and its post-edit re-run have occurred and the observed running behavior rung has fired for this thread. Before writing the criteria, formal "
        "notation, executable validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming what the prior rung’s artifact leaves unresolved; "
        "immediately produce the rung artifact — only the validation run observation rung stops after naming the gap. The prose rung must be re-emitted at the start of every cycle — "
        "the first cycle, any subsequent cycle, and any cycle following an upward return; the criteria rung label is not valid until the prose rung label for that cycle has been "
        "emitted; re-emitting prose is not optional on upward returns — the criterion must be immediately derivable from the re-emitted prose. From the criteria rung onward, the gap "
        "names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a landing "
        "page” is not; naming an absent artifact is not a valid gap; if no currently-false behavior remains, emit ✅ Manifest exhausted — do not emit 🔴 Gap:. Each rung’s artifact "
        "addresses only that gap — not all known requirements of the task — and is minimal: one criterion (one independently testable behavior derived from the prose alone — if stating "
        "it requires knowledge of function names, hook names, API endpoints, or data structures not in the prose, it belongs in the formal notation rung instead; ‘the table displays "
        "attribute data’ derives from the prose, ‘the useFoo hook calls /bar’ does not; if the criterion contains the word ‘and’ it is invalid — split it before continuing; ‘the table "
        "renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); the criteria rung artifact is exactly one criterion — a list of N behavioral assertions "
        "requires N separate gap cycles; if the criteria block contains more than one assertion, split before advancing to formal notation; one notation, one test, one edit; further "
        "artifacts emerge from subsequent gaps after each cycle completes. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no "
        "anticipated cases. Only validation artifacts may be produced at the executable validation rung — no other content is permitted at the executable validation rung before ✅ "
        "Validation artifact V complete: not implementation notes, not planning prose, not file reads, not ‘let me fix’ or ‘I need to’; implementation code is not permitted at this "
        "rung; file reads, grep output, and manual inspection do not constitute executable validation — the artifact must be invocable by an automated tool; before writing the test, "
        "re-read the formal notation and identify each structural constraint it encodes — value, position, ordering, co-presence — the test must assert each of those constraints, not "
        "only the criterion’s surface text; each test function contains exactly one assertion — one assertion per test function — this enforces per-assertion red witness structurally: "
        "a single-assertion function that passes before any edit is unambiguously vacuous; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be "
        "newly produced for the declared gap; the unit of production is the test function, not the file — before writing the validation artifact, check whether an existing test file in "
        "the project covers the same module or concern (by filename or class name); if one exists, add the test function there rather than creating a new file; create a new file only "
        "when no existing file covers the concern or when the concern warrants explicit isolation; before emitting ✅ Validation artifact V complete, read the artifact path via a tool "
        "call to check whether it already contains content — use the context-appropriate mechanism (read file, git show HEAD:<path>, cat, or equivalent); emit the result in the "
        "transcript; if the path already contains content, the artifact pre-exists; a pre-existing artifact must be confirmed failing before V-complete is valid; ✅ Validation artifact "
        "V complete may not be emitted without this tool-executed result appearing in the transcript; the validation artifact must reside within the project’s version-controlled file "
        "tree — artifacts written to /tmp, system temp directories, or any path outside the repository do not satisfy this rung regardless of freshness; the validation artifact must "
        "fail before any implementation edit — a green run with no prior red run does not satisfy this rung; if the artifact asserts static state (file contents, schema structure, "
        "attribute presence, file existence, or any property checkable without executing the implementation), run it against the current codebase before any edit — if it passes before "
        "any edit, the artifact is vacuous and must be rewritten with a target that is currently absent; if the artifact asserts runtime behavior, perturb the implementation to force a "
        "red run — if the artifact still passes after perturbation, the test is vacuous and must be rewritten; if the artifact goes red after perturbation, restore the perturbation and "
        "return to the gap rung, as the declared gap names a behavior that already exists. ✅ Validation artifact V complete must be emitted at the executable validation rung before the "
        "validation run observation label is written. Before writing the validation run observation rung label, invoke the validation artifact via a tool call — the rung label may not "
        "be written until tool output exists in this conversation for this thread; predicting what the run would show does not satisfy this gate regardless of accuracy; a green 🔴 "
        "Execution observed: at the validation run observation rung is only valid as the post-implementation green if a prior 🔴 Execution observed: for the same artifact appeared in "
        "the current cycle showing the test logic failing — a harness error (import failure, syntax error, missing file) is not a red run; the test must have been loadable, its "
        "assertions must have run, and they must have failed. At the validation run observation rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then "
        "🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or plan fixes; after emitting 🔴 Gap: at the observation rung, the criteria rung label is the "
        "only valid next token in the response — any other content before the criteria label is a protocol violation; if the observed gap matches the prior cycle's gap, the edit did "
        "not address it — return to formal notation before any further edit. Upward returns follow the failure class: if the implementation did not close the gap, loop within "
        "executable implementation; if the spec did not model the criterion, return to formal notation; if the prose description is insufficient — incomplete, inaccurate, or the system "
        "cannot be observed as described — return to the prose rung and update it; observation cannot be produced is a prose-description failure, not an implementation failure. One "
        "edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upon writing the observed running behavior label, re-emit the criterion for this "
        "cycle, then immediately invoke the implemented artifact via a tool call and emit 🔴 Execution observed: with its verbatim output — the criterion re-emission makes satisfaction "
        "locally auditable: the verbatim output must directly demonstrate the specific behavior named in the criterion; the verbatim output at this rung must be what a non-technical "
        "observer of the running system would see — rendered DOM content, an API response body, CLI output, or equivalent direct artifact output; a test pass/fail report is not valid "
        "OBS output regardless of what it implies about the implementation; the output must speak for itself without requiring the reader to infer that the behavior works; process "
        "state output (server started, build succeeded, exit code 0) does not satisfy this rung unless the criterion is specifically about process state; if the output does not "
        "directly demonstrate the criterion, emit the gap and return upward; invoke the implemented artifact directly — the binary, command, script, or rendered output produced by the "
        "implementation as a user or caller would observe it — not the test suite; if the implementation has no directly invocable artifact, invoke any non-test consumer of the changed "
        "artifact that demonstrates the declared behavior — tests are never a valid consumer at this rung regardless of whether they are the gating test; implementation edits, new "
        "files, and code changes are not permitted at this rung — if observation reveals a defect, emit the gap and return upward. ✅ Thread N complete may not be emitted unless the "
        "observed running behavior label has been written in the current cycle and a 🔴 Execution observed: sentinel appears immediately after it; once ✅ Thread N complete is emitted no "
        "further output in that cycle is valid. For each sentence in the prose that contains a behavioral predicate (fetches, requires, displays, filters, renders, validates, "
        "authenticates, loads, saves, or any similar action verb), check that a completed gap cycle exists whose criterion directly names that behavior — implicit or incidental "
        "coverage does not satisfy this check; if any behavioral sentence has no directly-named cycle, emit a new 🔴 Gap: for it before emitting ✅ Thread N complete. Outputting a rung "
        "label is what begins that rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. When beginning "
        ...
        "— whether the scheduled formal-notation → executable-validation gate or any upward return triggered by new information — reconcile all artifacts at that rung against I before "
        "redescending; if prose has evolved on this upward return, any pre-existing document that records I (ADR, README, spec) must be updated to match the evolved prose before "
        ...
        "After emitting ✅ Manifest exhausted, produce a final report containing prose, criteria, and formal notation for each thread in order; the final report copies artifacts already "
        "emitted in the thread rungs — no new behavioral claims, no coverage summaries, no suggestions may appear in the final report; then reconcile any documents (README, specs, "
        "ADRs) the implementation affects — this reconciliation is part of the final report, not a separate thread. Seven rungs in order: prose → criteria → formal notation → "
