...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "trust. P15 (Failure-first guard trust): a guard is not trusted until it has visibly failed under a controlled violation and then visibly passed under correction; observing "
        "only success weakens confidence in both guard and result. P16 (Observable-minimal gap): a gap exists only when the smallest externally observable behavior fails under applied "
        "evaluation; internal absence or unstated expectation does not constitute a valid gap. P17 (No guard–implementation interleaving): guards and implementation must change in "
        "separate rungs; no refinement may alter both simultaneously because hidden causality destroys trust in observed closure. P18 (Consequence Materialization Principle): When "
        "derivation identifies what goes wrong under violation, the protocol must immediately materialize that consequence as an active cost, invalidation, blockage, or trust reduction "
        "at the point of violation; a failure mode that produces no present consequence will not reliably shape behavior. P19 — Prerequisite Minimality Principle: When a challenge "
        "fails because a prerequisite artifact is absent, refinement may add only the smallest form necessary to allow the original guard to execute; prerequisite creation must not "
        "satisfy downstream behavior beyond restoring evaluability.Protocol derivation: before work begins, derive protocol rules by applying P1-P19 — observe at every feasible rung "
        "before proceeding. Protocol derivation checklist: AFTER deriving rules from P1-P19, BEFORE proceeding, MAKE VISIBLE in the conversation: [ ] The invariant: claim vs reality — "
        "what GOES WRONG if you claim without evidence: false completion; RULE: evidence before claims [ ] Optimization pressure: the model will seek shortcuts — what GOES WRONG if "
        ...
        "/ minimal stub) and must immediately rerun the same challenge before any behavioral implementation This checklist must appear BEFORE the first ladder rung begins. The "
