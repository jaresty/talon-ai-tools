...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "evaluate the next rung. Standard ladder: prose → criteria → formal notation → executable validation → executable implementation — each rung must be completed in this order; "
        ...
        "authority. Sentinel formats — ✅ Ground entered — prose rung begins [gate: first content emitted in this response; no artifact, code, prose, or reasoning may precede this "
        "sentinel — preceding content voids the session]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] [gate: rung table produced in current response; rung table "
        ...
        "output precedes this token in current response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward return to criteria rung [gate: exec_observed showing "
        "test suite failure + gap in current cycle; criterion identical to prior cycle criterion for this thread]; 🟢 Implementation gate cleared — gap cited: [verbatim from 🔴 Execution "
        "observed] [gate: exec_observed showing test suite failure + gap in current cycle; valid only at the rung whose artifact type is executable implementation; every assertion in "
        "the validation artifact must appear failing in the verbatim exec_observed output (e.g. 'Expected … Received …', 'AssertionError', 'FAILED', specific test name + failure "
        "reason) — an exec_observed that shows only some assertions failing, only infrastructure failure, import error, process crash, or a model-described summary of failure does not "
        ...
        "a subsequent cycle produces assertion-level failure output for every assertion]; ✅ Criteria complete — 1 criterion: [gap-name] [gate: exactly one criterion in this rung's "
