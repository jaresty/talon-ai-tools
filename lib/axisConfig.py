...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "tool-result block must appear; if the failure line does not specifically witness the violation of property [N], modify the violating state and repeat from step (4); the text "
        "immediately following the Failure: line must be the first witness line — no narrative, causal argument, prose explanation, or other content may appear between the Failure: "
        "line and the first 'Property [N] assertion [M] witness:' line; emit exactly one witness line for every previously inventoried executable assertion, in ascending assertion "
        'inventory order without omission or reordering; each witness line takes the form: \'Property [N] assertion [M] witness: Failure — assertion: "<verbatim assertion text>" — '
        "failure: \"<verbatim tool-result failure text>\"' if and only if the assertion's verbatim assertion text, or a uniquely identifying contiguous substring of that assertion "
        "text, appears in the quoted tool-result failure text, or the failure uniquely identifies the assertion by file, line number, or framework-specific assertion identifier; "
        "'Property [N] assertion [M] witness: Success — assertion: \"<verbatim assertion text>\"' if the assertion passed; an assertion may be classified 'Unreached' only after "
        "constructing a concrete execution intended to reach that assertion while preserving the same violating state — execute that attempt via a tool call and wait for the "
        "tool-result block; the text immediately following that tool-result block must be one of: 'Property [N] assertion [M] witness: Failure ...', 'Property [N] assertion [M] "
        "witness: Success ...', or 'Property [N] assertion [M] witness: Unreached — assertion: \"<verbatim assertion text>\" — <observed reason quoted directly from the execution "
        "tool-result block>'; an Unreached classification whose quoted reason does not appear verbatim in the preceding tool-result block does not satisfy this token; if the execution "
        "can be modified to reach the assertion while preserving the violating state, do so and repeat; Unreached is permitted only after every reachable execution preserving the "
        "violating state still fails to reach the assertion; the verbatim assertion text in each witness line must match the assertion text from the corresponding inventory line "
        "exactly; no witness may be emitted for an assertion not present in the assertion inventory, and no inventoried assertion may lack a witness; at least one inventoried "
        "assertion must receive witness classification 'Failure' — a guard execution in which no inventoried assertion is classified 'Failure' does not satisfy step (4), regardless of "
        "whether a Failure: line was produced; a 'Failure' witness whose verbatim failure text is not present in the corresponding Failure: tool-result block does not satisfy this "
        ...
        "assertions are witnessed, emit 'Assertion witnesses: complete — <count> assertions witnessed' where <count> must equal the count from 'Assertion inventory: complete' and "
        "every Unreached classification must be the result of an attempted reachability execution whose observed reason is quoted from a tool-result block; a witness count that "
        "differs from the inventory count does not satisfy this token; emit 'Assertion witnesses: incomplete — <reason>' if any assertion lacks a witness; if incomplete, revise the "
        "violating state or guard and repeat from step (4); no 'Quoted implementation:', implementation modification, or 'Implementation overreach:' may appear before 'Assertion "
        ...
        "properties complete these steps, emit 'Coverage: complete' if all properties are resolved, or 'Coverage: gap — property [N] \"<verbatim property definition>\"' for any "
        "unresolved property; if any gap exists, return to step (1) for each ungoverned property and complete all six steps before re-emitting Coverage:; Coverage: complete may not be "
        "emitted while any property [N] lacks a witnessed Failure: or Unobservable: line. After all per-property cycles complete, perform an implementation audit before emitting "
        "'Coverage: complete': enumerate the observable partitions of the governed artifact — distinct execution paths, call sites, externally visible interfaces, state transitions, "
        "outputs, and consumers of those outputs; for each partition determine whether at least one retained property governs its externally observable behavior; emit 'Audit: "
        "implementation gap — <description>' for any partition not governed by a retained property, execute the full Ground protocol for the new properties — including atomicity and "
        "completeness checks concluding with 'Ground complete' — then execute the full six-step falsify cycle for each new property, then repeat the audit; emit 'Audit: implementation "
        "complete' only when no ungoverned partition remains; 'Coverage: complete' may not be emitted before 'Audit: implementation complete' has been emitted. A response that omits "
        "any required sentinel, performs a step before emitting its required quoted artifact, modifies the governed artifact to satisfy property [N] before a quoted Failure line "
        "appears, omits any executable assertion from the assertion inventory, or leaves a property without an automatic regression guard does not satisfy this token.",
