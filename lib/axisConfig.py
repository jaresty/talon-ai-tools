...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "inverts causality; the rung is incomplete until all criteria are encoded. each test function asserts exactly one behavioral property — one behavioral property per test "
        ...
        "VRO already exists in the current cycle for the current criterion; a new criterion introduced by HARD STOP has no red VRO yet — EV and VRO must fire before impl_gate; a green "
        "exec_observed without a prior red run means the test is vacuous — return to the executable validation rung and rewrite the test to fail. a valid 🔴 Gap: at the VRO rung must "
        "cite a failing assertion from the exec_observed output. 🛑 HARD STOP — upward return to criteria rung is valid in exactly one case: the current criterion for this thread is "
        "identical to the criterion from the most recent prior cycle for this thread, confirming the criterion is underspecified (P5 condition); all other failure classes have defined "
        "routes — implementation gap that changes between cycles loops within EI; spec gap returns to formal notation; using HARD STOP for any failure class other than criterion "
        ...
        "the harness, not an upward return; when EV shows a harness error that is an import failure, the repair must re-enter at step (2a) of the current EV cycle — a freeform stub "
        "write that bypasses the import-check gate is a protocol violation; HARD STOP may not appear in the same response as a harness-error exec_observed at the EV rung. if the "
        ...
        "current cycle. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no tool call, no prose, and no file modification may appear before it "
        "in the current response; the next required action after 🟢 is a tool call that creates or modifies an implementation file; the implementation gate does not complete the thread; "
        "✅ Thread N complete may not appear until the observed running behavior rung has fired for this thread; the OBR rung label must appear in the transcript for the current "
        "thread's current cycle before ✅ Thread N complete may be emitted — output produced before the OBR rung label may not be reused to satisfy the OBR exec_observed gate; its "
        "absence is a protocol violation, not a voiding condition. declaring a thread complete by similarity to another thread's failure is not valid — each thread requires its own "
        "full descent through OBR with a passing run; similarity does not substitute for a tool-executed result in the current cycle. prior evidence — passing tests, complete "
        "implementation, or prior cycles — does not satisfy the OBR gate; only a live-process invocation in the current cycle does. In-process rendering (renderToStaticMarkup, "
