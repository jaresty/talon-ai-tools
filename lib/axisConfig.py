...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "user explicitly exits. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its rung or as a narrative summary is a protocol violation. "
        "The gap names a specific behavior currently absent or wrong, phrased as a currently-false assertion — “the attributes table does not render” is valid; “need to implement a "
        "landing page” is not; if no currently-false behavior remains, emit ✅ Manifest exhausted — ✅ Manifest exhausted may not be emitted unless a ✅ Thread N complete sentinel exists "
        "for each thread declared in ✅ Manifest declared — before emitting ✅ Manifest exhausted, locate the N in the ✅ Manifest declared sentinel and count ✅ Thread N complete "
        "sentinels — if the count does not equal the declared N, the manifest is not exhausted. one independently testable behavior derived from the prose alone — if stating it "
        "requires names or structures not in the prose, it belongs in formal notation instead; if the criterion contains the word ‘and’ it is a conjunction — split before continuing "
        "(‘the table renders Display Name’ and ‘the search bar filters results’ are two criteria, not one); a criterion that names both a structural element and its data source is a "
        "conjunction — split before continuing; asserting a column header is present does not satisfy this rung — add a criterion asserting its content or effect; before writing the "
        ...
        "gate, not a self-check; exactly one criterion may be written per thread per cycle — after the first criterion is written, the only valid next token is the formal notation rung "
        "label; a second criterion for the same thread is a protocol violation — stop, return to the prose rung, and split the thread before descending again; the criteria rung label "
        "is per-thread — writing criteria for Thread 2 or any subsequent thread before ✅ Thread 1 complete is emitted is a protocol violation regardless of whether they share a "
        "criteria rung label; batch-collecting criteria for multiple threads under one criteria label bypasses sequential descent and is a protocol violation. The formal notation rung "
        "separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) encodes what must be true — the executable "
        "or testable part. Natural language labels, introduces, and explains the specification — prose adds context but the behavioral constraints must still be encoded in notation.it "
        "must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, structural invariants) describe what notation "
        "looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it produces only type signatures, interfaces, "
        "pseudocode, and structural invariants — the test of valid formal notation is that it cannot be directly compiled or executed without modification; any construct that compiles "
        "or runs as-is is implementation code and voids the rung; formal notation describes what must be true, not how to implement it — permitted: type signatures without bodies, "
        "interface shapes, structural invariants, pre/postconditions; prohibited: complete function bodies, variable assignments, constant declarations (const, let, var assignments) "
        "regardless of what they assign — the presence of an assignment operator outside a type definition indicates implementation code, working logic sequences; labeling prohibited "
        "content as pseudocode does not exempt it from this prohibition; if the criterion is already a complete behavioral specification — naming observable behavior and a falsifying "
        "condition without referencing function names, field names, or implementation internals — the formal notation artifact may restate it verbatim with an explicit declaration that "
        "no elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript and the audit section still applies; the audit section is separate and named; "
        "the rung is incomplete until all criteria are encoded. Natural language may appear as section labels or introductions (e.g., ‘This invariant captures the rendering "
        "constraint:’) but may not substitute for encoding a constraint in notation — if a constraint can be stated in notation, it must be. each test function asserts exactly one "
        "behavioral property — one behavioral property per test function; multiple assertions are permitted only when they jointly constitute a single indivisible check (e.g., "
        "asserting a value is both non-null and of the expected type); before writing the test, re-read the formal notation and assert each structural constraint it encodes; confirm "
        "via tool call that the criteria artifact for this cycle was produced in this cycle's transcript; if the criteria artifact is identical to a prior cycle’s criteria artifact, "
        "the criterion has not changed — return to the prose rung and re-emit with an updated gap before descending again; before writing the test, check for an existing test file "
        "covering the same module — add there if so; when the artifact is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward "
        ...
        "exhibit the criterion; before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — V-complete may "
        "not be emitted without this tool-executed result in the transcript; asserting confirmation without showing the exec_observed block does not satisfy this gate — the "
        "exec_observed block must appear in the current response; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts static "
        "state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to force a "
        "red run — if it still passes after perturbation, the test is vacuous and must be rewritten. Before writing the validation run observation rung label, invoke the validation "
        "artifact via a test runner; a harness error (import failure, syntax error, missing file) is not a red run — the test must have been loadable, its assertions must have run, and "
        "they must have failed; fix the harness error before treating any run as a red witness; a green 🔴 Execution observed: is only valid as the post-implementation green if a prior "
        "🔴 Execution observed: for the same artifact appeared after the most recent 🟢 Implementation gate cleared showing the test logic failing. At the validation run observation "
        "rung, run the validation artifact; emit 🔴 Execution observed: with verbatim output then 🔴 Gap: naming the first failure; stop there — do not enumerate multiple failures or "
        "plan fixes; after a red VRO that is not a harness error, the criterion re-statement is an anchor only — re-running VRO without an implementation edit is a protocol violation; "
        ...
        "to HARD STOP returns. Upon writing the observed running behavior label, re-emit the criterion for this cycle — criterion re-emission is required: emitting the OBR label "
        "without immediately following it with the criterion text is a protocol violation; the tool call is the only valid next action after criterion re-emission; planning statements, "
        "diagnostic narration, and commentary between criterion re-emission and the tool call are protocol violations; ✅ Thread N complete may not appear until a tool call exists in "
        "the transcript after the observed running behavior label in the current cycle; then immediately invoke the implementation as a live running process via a tool call and emit 🔴 "
        "Execution observed: with its verbatim output — the verbatim output must directly demonstrate the specific behavior named in the criterion. A file read never satisfies this "
        "rung — the block must contain output produced by running the artifact. If the artifact requires a start step before querying (e.g., a web server), two tool calls are required "
        "in sequence: process-start then query — the 🔴 Execution observed: sentinel may not be emitted until both complete. In-process rendering (renderToStaticMarkup, test-framework "
