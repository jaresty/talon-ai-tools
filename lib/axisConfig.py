...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "after implementing; describing what you would do is not evidence), rung transitions (emit rung label when beginning each rung; emit completion sentinel when finishing each "
        "rung; emit rung label when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return upward), rung completion order (each "
        "rung must produce its artifact before emitting completion sentinel; sequential descent through prose → criteria → formal notation → validation → implementation; skipping a "
        "rung is a protocol violation), P5 implies sequential descent (since each artifact derives from prior artifact per P5, you must produce them in order — treating the ladder as a "
        "planning document rather than strict sequence violates derivation), ladder is strict execution sequence (emit rung label, produce artifact, emit completion sentinel — repeat "
        "for each rung in order; you may return upward freely, but only when higher rung artifact is discovered to be malformed — NOT to make lower rung implementation easier; "
        "difficulty, failure, or constraint pressure at a lower rung is not a valid upward return trigger), observation loop returns to start (after manifest exhausted, return to "
        "observation loop to verify gap is closed; re-observe running behavior; if gap still exists, descend ladder again; if gap is closed, emit Ground complete), observation loop "
        "corollary (if opening observation shows no gap exists, emit Ground complete immediately — no ladder descent needed), upward return conditions (what triggers return, what does "
        "NOT trigger return and why excluded), evidential scoping (evidence is scoped to current cycle and gap, not prior cycles), scope preservation (scope does not expand between "
        "rungs), derivation reasoning (explain WHY each rule follows from P1-P6, not just WHAT the rule is); each derived rule must cite its source principle(s) from P1-P6; rules that "
        "cannot be derived are protocol violations; emit Protocol derived after deriving all rules; emit Ladder derived after producing the rung table; ground complete may only be "
        ...
        "rung's artifact can evaluate the next rung. Standard ladder for software behavioral change: prose → criteria → formal notation → executable validation → executable "
        "implementation — each rung must be completed in this order; emit the rung label before producing its artifact; emit completion sentinel after artifact is produced and gate is "
        "satisfied. EV rung: edits only validation files. EI rung: edits only implementation files. An executable validation artifact is a file whose sole purpose is to assert "
        ...
        "path or naming convention — path and naming are evidence, not authority. Sentinel formats — ✅ Ground entered — prose rung begins [gate: first content emitted in this response; "
        "no artifact, code, prose, or reasoning may precede this sentinel — preceding content voids the session]; ✅ Manifest declared — N threads: [numbered list of behavioral gaps] "
        "[gate: rung table produced in current response; rung table precedes this sentinel]; 🔴 Execution observed: [verbatim tool output — triple-backtick delimited, complete, nothing "
        "omitted] [gate: tool call made in the current response immediately before this sentinel; verbatim output in triple-backtick block follows]; 🔴 Gap: [what the verbatim output "
        "reveals] [gate: exec_observed with non-empty failing output precedes this token in current response; gap text is a currently-false behavioral assertion]; 🛑 HARD STOP — upward "
        "return to criteria rung [gate: exec_observed showing test suite failure + gap in current cycle; criterion identical to prior cycle criterion for this thread]; 🟢 Implementation "
        "gate cleared — gap cited: [verbatim from 🔴 Execution observed] [gate: exec_observed showing test suite failure + gap in current cycle; valid only at the rung whose artifact "
        "type is executable implementation; every assertion in the validation artifact must appear failing in the verbatim exec_observed output (e.g. 'Expected … Received …', "
        "'AssertionError', 'FAILED', specific test name + failure reason) — an exec_observed that shows only some assertions failing, only infrastructure failure, import error, process "
        "crash, or a model-described summary of failure does not satisfy this gate; if the suite cannot reach assertions, the implementation artifact for that cycle resolves only the "
