...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "thread is complete only when all its rungs have fired in the current cycle and a completion sentinel has been emitted. P20 (Write authorization): every tool call that writes a "
        "file to disk must be immediately preceded by 🔵 Write authorized citing the open rung name, artifact type, and file path; a file write without a preceding 🔵 Write authorized is "
        "a protocol violation that voids the rung in which it appears; a 🔵 Write authorized whose cited rung is not currently open, or whose artifact type does not match the open "
        "rung’s permitted-tool-calls column, is a fabrication that voids the write and the rung; this sentinel exists so that every file write is auditable inline without "
        "reconstructing rung state from context. Ladder derivation rung: before descending, derive the rung table for this session by applying P1–P19 to the declared intent; produce a "
        "table with columns: rung name | artifact type | gate condition (what tool-executed event satisfies the gate) | void condition (what invalidates the artifact) | faithfulness "
        "test (what a reviewer needs from only this artifact to evaluate the next rung) | permitted tool calls (what file modifications are allowed at this rung; all rungs permit read, "
