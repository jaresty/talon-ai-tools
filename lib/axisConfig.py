...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "that is failing (the exact assertion line from the test file); Validation rung: write test file with test function names that describe expected behavior AND include actual "
        "assertions that will fail; these assertions define the intent that implementation must satisfy. Infrastructure MUST be resolved before leaving validation rung — import errors, "
        "missing files, syntax errors must all be fixed BEFORE proceeding to assertion rung; tests must be able to RUN (not necessarily pass) before assertion rung can begin. The "
        "assertions in validation rung tests are FIXED intent declarations — they cannot be changed in later rungs; implementation must make them green, not make tests match "
        "implementation. Assertion rung is per assertion: each individual assertion is its own rung. Implement ONLY enough to make THAT specific assertion green. Tests are IMMUTABLE "
        "once written at validation rung — implementation must satisfy tests, never modify tests to match implementation. If tests are wrong, return UP to validation rung to fix them; "
        "modifying tests in later rungs voids the rung. Infrastructure failures (import errors, missing files, syntax errors) are NOT assertion failures. If infrastructure fails, "
        "return UP to validation rung to fix the infrastructure issue before continuing. A test that cannot even START due to import error has NOT yet reached the assertion phase. The "
        "assertion rung only begins AFTER tests run successfully to the point of failing assertions - not import errors. ASSERTIONS are test code that verifies behavior, NOT import "
        "errors or infrastructure failures. An import error means the file doesn't exist - fix the file first, not the test. Only test assertions with expect() count as assertions. "
        "Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; No "
        "implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the exec_observed sentinel must include the "
