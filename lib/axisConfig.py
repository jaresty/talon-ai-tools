...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "to see it FAIL (red). AFTER making assertion green (implementation succeeded), CHECK if gap is closed — if gap still exists, add another assertion; if gap is closed, emit "
        "completion sentinel and move to next gap. NO file creation in assertion rung — stubs created at validation rung; only edit existing test file to add assertions. Rung "
