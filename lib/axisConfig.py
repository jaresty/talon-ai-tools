...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "specification — prose adds context but does not substitute for formal encoding where precision matters. Formal notation encodes only the criteria declared at the criteria rung "
        "— no additional invariants, no anticipated cases; it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, "
        "pseudocode, structural invariants) describe what notation looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is "
        "insufficient; it produces only type signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is that it cannot be directly compiled or "
        "executed without modification; any construct that compiles or runs as-is is implementation code and voids the rung; formal notation describes what must be true, not how to "
        "implement it — permitted: type signatures without bodies, interface shapes, structural invariants, pre/postconditions; prohibited: complete function bodies, variable "
        "assignments, constant declarations (const, let, var assignments) regardless of what they assign — the presence of an assignment operator outside a type definition indicates "
        "implementation code, working logic sequences; labeling prohibited content as pseudocode does not exempt it from this prohibition; if the criterion is already a complete "
        "behavioral specification — naming observable behavior and a falsifying condition without referencing function names, field names, or implementation internals — the formal "
        "notation artifact may restate it verbatim with an explicit declaration that no elaboration is needed; this is not a skipped rung; the artifact must appear in the transcript "
        "and the audit section still applies; the audit section is separate and named; the rung is incomplete until all criteria are encoded. Natural language may appear as section "
        ...
        "notation and assert each structural constraint it encodes; a pre-existing test that happens to pass does not satisfy this rung; file reads, grep output, and manual inspection "
        "do not constitute executable validation — the artifact must be invocable by an automated tool; when the declared intent is to modify a test artifact, the "
        ...
        "test-under-modification does not exhibit the criterion; before writing the test, check for an existing test file covering the same module — add there if so; when the artifact "
        "is modified, invoke a tool call to read the current test file before emitting carry-forward rows — carry-forward format: ‘Carry-forward: prior failure [verbatim test name as "
        "it appeared in a 🔴 Execution observed sentinel] covers current test [verbatim test name as written in the artifact]’ — each prior-failure name must be quotable verbatim from a "
        "🔴 Execution observed sentinel; modification without carry-forward is a traversal violation; no implementation artifact may appear after modification until carry-forward has "
        "been emitted. before emitting ✅ Validation artifact V complete, confirm via tool call that the artifact path does not pre-exist or is already failing — V-complete may not be "
        "emitted without this tool-executed result in the transcript; the validation artifact must reside within the project’s version-controlled file tree; if the artifact asserts "
        "static state, run it before any edit — if it passes, the artifact is vacuous and must be rewritten; if the artifact asserts runtime behavior, perturb the implementation to "
        "force a red run — if it still passes after perturbation, the test is vacuous and must be rewritten; implementation edits may not begin until a red run exists in the transcript "
        "for the current cycle; ✅ Validation artifact V complete may not be emitted unless a 🔴 Execution observed: showing the test failing exists in the transcript for the current "
        "cycle; emitting V-complete before seeing a red run is a protocol violation regardless of whether perturbation was attempted. ✅ Validation artifact V complete must be emitted "
        "at the executable validation rung before the validation run observation label is written; if the observed running behavior label is written without a ✅ Validation artifact V "
        "complete appearing in the transcript for the current cycle, the OBR rung is void — return to the executable validation rung and produce the artifact. Before writing the "
        "validation run observation rung label, invoke the validation artifact via a test runner — file reads, grep, directory listings, and static analysis produce "
        "static-analysis-type output, not validation-run-observation-type output, and do not satisfy this gate; a harness error (import failure, syntax error, missing file) is not a "
        "red run — the test must have been loadable, its assertions must have run, and they must have failed; fix the harness error before treating any run as a red witness; a green 🔴 "
        ...
        "current cycle — emitting it at any other position is a protocol violation; HARD STOP may not be emitted at the executable validation rung — a harness error at EV requires "
        "fixing the harness, not an upward return; if the observed gap matches the prior cycle's gap, the edit did not address it — return to formal notation before any further edit. "
        ...
        "after criterion re-emission; ✅ Thread N complete may not appear until a tool call exists in the transcript after the observed running behavior label in the current cycle; then "
        "immediately invoke the implemented artifact via a tool call and emit 🔴 Execution observed: with its verbatim output — the verbatim output must directly demonstrate the "
        "specific behavior named in the criterion; it must be what a non-technical observer of the running system would see — not the test suite, not process state unless the criterion "
        "is specifically about process state; a build output or compilation result never satisfies this gate regardless of what it names; prefer the most realistic invocation — for a "
        "web UI, start the dev server and emit the HTML response body (e.g., via curl or browser); for a CLI, invoke it directly and emit its output; in-process rendering "
        "(renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact exists; if the implementation has no directly invocable artifact, invoke any "
        "non-test consumer of the changed artifact that demonstrates the declared behavior; a 🔴 Execution observed: block that is empty or blank, or contains only static-analysis "
