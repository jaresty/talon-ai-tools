...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "invocable by an automated tool; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be newly produced for the declared gap; the unit of "
        ...
        "when the concern warrants explicit isolation; before emitting ✅ Validation artifact V complete, check: does this artifact exist in the repository at HEAD before your edits? "
        "Run git show HEAD:<path> or confirm no such file exists; if it exists and is unchanged by your session, it is not newly produced for the declared gap — write a different "
        "artifact; the validation artifact must reside within the project’s version-controlled file tree — artifacts written to /tmp, system temp directories, or any path outside the "
        "repository do not satisfy this rung regardless of freshness; the validation artifact must fail before any implementation edit — a green run with no prior red run does not "
        "satisfy this rung; if the artifact asserts static state (file contents, schema structure, attribute presence, file existence, or any property checkable without executing the "
        "implementation), run it against the current codebase before any edit — if it passes before any edit, the artifact is vacuous and must be rewritten with a target that is "
        "currently absent; if the artifact asserts runtime behavior, perturb the implementation to force a red run — if the artifact still passes after perturbation, the test is "
        "vacuous and must be rewritten; if the artifact goes red after perturbation, restore the perturbation and return to the gap rung, as the declared gap names a behavior that "
        "already exists. ✅ Validation artifact V complete must be emitted at the executable validation rung before the validation run observation label is written. Before writing the "
        "validation run observation rung label, invoke the validation artifact via a tool call — the rung label may not be written until tool output exists in this conversation for "
        "this thread; predicting what the run would show does not satisfy this gate regardless of accuracy. At the validation run observation rung, run the validation artifact; emit 🔴 "
        ...
        "implementation did not close the gap, loop within executable implementation; if the spec did not model the criterion, return to formal notation; if the prose description is "
        "insufficient — incomplete, inaccurate, or the system cannot be observed as described — return to the prose rung and update it; observation cannot be produced is a "
        "prose-description failure, not an implementation failure. Before each edit, check: does a 🔴 Execution observed: sentinel exist for the current gap? If not, run the validation "
        "artifact now before editing. One edit per re-run cycle; after each edit re-run the validation artifact before any further edit. Upon writing the observed running behavior "
        "label, immediately invoke a tool and emit 🔴 Execution observed: with its output — prose description of tool output does not satisfy this rung; invoke the implemented artifact "
        "directly — the binary, command, script, or rendered output produced by the implementation as a user or caller would observe it — not the test suite; running tests at this rung "
        "is a protocol violation; if the implementation has no directly invocable artifact, invoke any consumer of the changed artifact that demonstrates the declared behavior — not "
        "the test that gates it; implementation edits, new files, and code changes are not permitted at this rung — if observation reveals a defect, emit the gap and return upward. ✅ "
        "Thread N complete may not be emitted unless the observed running behavior label has been written in the current cycle and a 🔴 Execution observed: sentinel appears immediately "
        "after it; once ✅ Thread N complete is emitted no further output in that cycle is valid. Before emitting ✅ Thread N complete, check: does the declared intent have any behaviors "
        "not yet observed running? If so, emit a new 🔴 Gap: naming the next unmet behavior and begin the next cycle at the criteria rung. Outputting a rung label is what begins that "
        "rung — it is not a heading or annotation; a rung whose label has not been output has not begun and no artifact for it may exist. When beginning mid-ladder, locate the highest "
