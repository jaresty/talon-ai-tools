...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "costly to maintain. Before acting, scan the transcript for the highest-level goal statement and produce a one-line anchoring citation naming its location and quoting its text. "
        "If no goal statement exists above the current point, state the transcript boundary searched. Derive the intent artifact from the cited goal statement as an enumerated list of "
        "behavioral dimensions, each stating the specific behavior it governs such that its presence or absence in any artifact is determinable without interpretation. Then derive a "
        "ladder: an ordered sequence of artifacts where each rung names (a) the specific outcome the prior rung admits that this rung rejects, and (b) the behavioral dimension that "
        "outcome leaves unconstrained in the prior rung — a rung that cannot name both has not been derived and must be merged with the rung above. The ladder must terminate at an "
        "artifact whose satisfaction condition is checked by prior-executed results, not by inspection of prose. Every step requires a prior-executed result already in the transcript "
        "as its governing artifact — a step whose governing artifact has not produced a result above it in the transcript has no authorization to proceed. The response is not permitted "
        "to end until the completion check artifact exists in the transcript: for each item in the intent artifact, it cites the specific outcome in a prior execution result that was "
        "observed to differ when the behavioral dimension was absent — a prediction of what would differ does not satisfy this; where no such result exists, the dimension is recorded "
        "as uncovered. Scope reduction is permitted only when the intent artifact or a prior completion-check artifact contains text that verbatim and explicitly excludes the element — "
        "cost, effort, or proportionality arguments are not valid at any stage.",
