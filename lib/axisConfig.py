...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "the explicit request constraints — a candidate split is not valid merely because it decomposes the mathematical or definitional structure of a predicate, and it is not valid "
        "merely because each sub-property is independently testable, assertable, or useful as verification evidence; and neither accepted sub-property constrains an internal mechanism, "
        "architecture, algorithm, or representation as the primary subject of the constraint, unless that mechanism is explicitly required by the chosen interpretation. A sub-property "
        "constrains an internal subject when its canonical definition makes an internal mechanism, architecture, algorithm, or representation the primary subject of the constraint — "
        "this classification is independent of whether that subject happens to be externally observable. A sub-property is externally observable when its truth value can be determined "
        "solely from the accessible-observation boundary of the chosen interpretation, without inspecting the implementation. These two criteria are distinct: a property can concern an "
        "internal subject yet still be externally observable (for example, when the interpretation explicitly exposes that subject through its output contract), and a property can "
        "concern an external subject yet still be inaccessible if the interpretation does not expose the relevant behavior. The split condition rejects properties whose primary subject "
        "is internal and not explicitly required — not merely properties that happen to be inaccessible. A candidate split can be constructed only if canonical definitions for [Na] and "
        "[Nb] have been established and admissible falsifying constructions satisfying the stated falsifier conditions can be produced for both sub-properties. A candidate split test "
        "is accepted when all split-success conditions are satisfied for that non-void candidate; otherwise the candidate split is rejected. Step 4 — Accept or mark atomic. A property "
        "may be marked atomic only after Step 2 has introduced provisional sub-properties, Step 1 has canonicalized them, and Step 3 has attempted candidate-split construction against "
        ...
        '"<verbatim canonical definition>" — candidate split rejected: <reason>.\' A property that represents a single independently meaningful constraint under the chosen '
        "interpretation is atomic even if its mathematical or logical expression admits formal decomposition; the nontriviality and distinct-constraint conditions in Step 3 enforce "
        "this: a split into sub-properties that merely reformulate the mathematical definition of the predicate — rather than separating independently meaningful request constraints — "
        "is invalid and must be rejected as not nontrivial or not corresponding to distinct constraints. The quoted definition must match the current canonical definition verbatim. "
        "Apply this procedure to every retained property until all have reached Step 4 and emitted '§ atomic:'. After all retained properties are atomic, emit a completeness check: '§ "
