...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "shrinks three things into agreement: the implementation shrinks to exactly what the guards require, the guards shrink or expand to exactly the properties, and each assertion "
        "is witnessed failing when its property is absent. Test and implementation may be written in any order. A guard is established by a tool call whose result block contains the "
        "guard's executable text verbatim; every assertion referenced later must be copied verbatim from such a result block. Because a guard detects regression only if it re-runs on "
        "its own, a Failure observation is valid only when it is produced by executing a committed, re-runnable guard artifact — a persisted test or check that re-runs without human "
        "initiation — never from an inline or ephemeral command. Capability is not evidence: what witnesses a behavioral observation is an available qualifying execution record, not "
        "the mere fact that a tool call did or did not occur this turn. A qualifying execution record is a transcript-present record produced by the committed guard artifact that "
        "carries the guard's executable identity, the assertion's identity, and the relevant A-result. A fresh tool call produces one; a prior tool-result already present in the "
        "transcript is one; model prose is never one. When execution tools are available the response must execute the committed guard to obtain new observations; when they are "
        "unavailable it may cite qualifying execution records already present but must not synthesize, infer, or narrate an execution result. A behavioral assertion for which no "
        "qualifying execution record exists is neither Failure nor structural Unobservable — emit 'Unwitnessed: assertion \"<verbatim assertion>\" — execution unavailable'; it does "
        "not contribute to 'Coverage: complete', and it is a witnessing gap, not a structural exemption. Before acting, observe the gap: emit 'Observing gap: <what is currently absent "
        "or failing>'. Shrink the implementation to the guard (Gate 1). Always attempt the simpler implementation: construct a smaller modification to the artifact and execute the "
        "guards against it. 'Overreach: not found' is a claim that no such reduction exists, so it is valid only after an executed attempt — a qualifying execution record showing a "
        "smaller artifact that the guards reject (or a smaller artifact they accept, which you then keep). If a reduction the guards still accept exists, or an execution path through "
        "the artifact is not exercised or distinguished by any guard, emit 'Overreach: found — <modification or path>', trim to the boundary the guards require, and repeat. A bare "
        "'Overreach: not found' with no executed reduction attempt does not satisfy this token. Both 'Overreach: found' and 'Overreach: not found' are execution-dependent; when no "
        "qualifying execution evidence is available, emit 'Overreach: untested — execution unavailable' rather than either verdict — untested is never 'not found'. Witness every "
        "assertion (Gate 2). Enumerate the executable assertions: emit one 'Assertion: <verbatim assertion text>' line for each executable assertion of the established guard, quoted "
        "verbatim from the guard's tool-result. The verbatim lines are the observable; no count or completeness sentinel is used, and a self-declared count does not satisfy it. Every "
        "enumerated 'Assertion:' line must then receive its own 'Failure: assertion \"<verbatim assertion>\" — \"<verbatim failure text>\"' or 'Unobservable: assertion \"<verbatim "
        "assertion>\" — structural' before 'Coverage: complete'; the binding is by verbatim identity — the assertion text in the Failure or Unobservable line must match an "
        "enumerated Assertion line exactly, and an enumerated assertion with no matching outcome line leaves coverage incomplete. After each Failure or Unobservable outcome, if any "
        ...
        "A-observations: an execution in which A's own guard-defined result is a failure, and one in which A's own result is a pass. An A-observation is an execution of the committed "
        "guard in which assertion A is evaluated and produces its own guard-defined result. The guard defines the observable result for A: an A-pass is the guard's reported pass "
        "result for A and an A-fail is the guard's reported failure result for A. These results are observations of the guard, not independent claims about the truth of any property — "
        "whether A's result corresponds to the property A is intended to govern is not established by this token; that correspondence is established separately by the adequacy check. "
        "What this token establishes is only that A distinguishes its own two results: an execution in which A is evaluated and reports failure, and one in which A is evaluated and "
        "reports pass. The failure (A-fail) execution must immediately precede the 'Failure:' line (it is the producer); the passing (A-pass) execution is the contrast evidence and "
        "may appear elsewhere in the transcript. Any execution that does not produce an A result is not an observation of A and cannot witness A, regardless of the cause or form of "
        "the execution's outcome — this covers execution errors, unrelated assertions failing, a harness that exits without reporting A, and any other run in which A itself was not "
        "evaluated; only an A-fail result can supply the violating observation, and only an A-pass result the satisfying contrast. In particular — as an illustration of this rule, not "
        "an exhaustive list — when the governed symbol does not yet exist, a build failure, compilation error, import error, or undefined-symbol error is tempting to read as 'the "
        "property is absent, so this is the failure'; it is not an A-fail, because A never ran — the run disqualified itself as an observation of A and fails identically no matter "
        "which property is absent. The A-fail you need is A's own assertion executing and reporting failure against a present-but-wrong artifact, which the symbol's absence cannot "
        "produce. Both A-observations must be produced by the committed guard artifact, and the guard's output must identify A (by test name, subtest path, or assertion message) so "
        "the pair is bound to this enumerated assertion rather than another. Provenance closure: any transcript reference that functions as evidence — the assertion identity, the "
        "A-fail result, and the A-pass result — must quote verbatim the text of the qualifying execution record that establishes it, and counts as evidence only if that exact text "
        "occurs in a referenced qualifying execution record; a result or identity typed as prose that does not appear verbatim in a cited qualifying execution record is not evidence "
        "and does not witness A. If the artifact already passes A, construct a temporary modification that makes A alone fail while every other assertion still passes, execute, emit "
        'the Failure from that tool-result, then restore; a modification that fails every assertion at once discriminates none. Emit \'Failure: assertion "<verbatim assertion>" — '
        "<A-fail result> / <A-pass result>' quoting both observed results from their tool-results; a 'witness:' line, if added, only projects those two results and is not itself "
        "evidence. A structural assertion — one whose property has the artifact's own text or structure as its primary subject, so no behavioral failing execution exists — is "
        "witnessed instead by 'Unobservable: assertion \"<verbatim assertion>\" — structural'; Unobservable is never satisfied by having searched for a failing execution and found "
        "none. An assertion that is behavioral yet no admissible modification can make fail is a guard defect: strengthen the guard and repeat. A guard is a blind spot whenever an "
        "admissible state exists in which its pass-or-fail outcome is not equivalent to whether the property holds — check three: (a) proxy or non-exercising guard checking a "
        "surrogate; (b) durability — a guard gone at the moment it would need to fire; (c) ephemeral guard not committed to a location that re-runs. Verdict-follows-execution governs "
        "both gates: when tool calls are available, every 'Overreach:' verdict and every 'Failure:' observation is valid only when it immediately follows a tool-result block that "
        "mechanically produces it; a verdict emitted from description or analysis alone does not satisfy this token. A structural Unobservable assertion is exempt, having no execution "
        "to anchor to. 'Coverage: complete' is valid only when every enumerated assertion has either a 'Failure:' line backed by its A-fail/A-pass execution pair or an 'Unobservable: "
        "… — structural' line, and no 'Overreach: found' remains open; an 'Unwitnessed: … — execution unavailable' assertion leaves coverage incomplete and does not satisfy this "
        "token. When qualifying execution evidence is unavailable for one or more behavioral assertions, the honest terminal state is 'Coverage: incomplete — execution unavailable', "
        "not a claimed completion; the protocol may construct guards, perturbations, and expected outcomes and identify what remains unverified, but it may not represent any "
        "execution-dependent verdict as observed.",
