...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "only after '## Enforcement sequence' and '§ implementation permitted' have appeared in the transcript; immediately after '## Completion check', write the label line "
        "'Formalized properties:' — this label line is valid only after '§ formalization complete' has appeared in the transcript; '§ properties complete' is valid only after "
        "'Formalized properties:' has appeared in the current '## Completion check' block; decompose each §2 dimension into its atomic formal claims; each claim must appear on its own "
        "line beginning with the literal text 'property [N]:' where N is a 1-based integer — a line beginning with '**property' or any markdown-formatted variant does not satisfy this "
        "requirement; each 'property [N]:' line must contain at least one formal expression — a logical predicate (∀/∃ notation), a typed function signature, or a concrete input/output "
        "example (f(v)=w or equivalent) — as a literal substring; a 'property [N]:' line whose content is prose only, with none of {∀, ∃, →, a typed parameter list, or f(v)=w pattern} "
        "present as a substring, does not satisfy this requirement; for each property write '§ property [N] atomic: [sub-claim that would be false while the property as a whole holds]' "
        "— if no such sub-claim exists the property is already atomic; when all 'property [N]:' lines and atomicity checks are written, write '§ properties check: \"[quoted "
        "phrase]\"' where [quoted phrase] is a verbatim substring of one of the 'property [N]:' lines written above — a '§ properties check:' line whose quoted phrase does not "
        "appear verbatim in a 'property [N]:' line above it does not satisfy this requirement; then write '§ properties complete'; '§ properties complete' is valid only after a valid "
        "'§ properties check:' line has appeared; no dimension citation, '§ test suite complete', or '§4 coverage verified' is valid until '§ properties complete' has appeared in the "
        "current '## Completion check' block; dimension citations are valid only after '§ properties complete' has appeared; each covered dimension cites a verbatim string that appears "
        "either as a substring of a tool-result block produced after '§ implementation permitted' and visible above '## Completion check', or, when no such tool-result blocks are "
        "present, as a substring of the response text produced in this turn; '§ test suite complete' is valid only after '§ properties complete' has appeared in the current '## "
        "Completion check' block — a '§ test suite complete' appearing before '§ properties complete' or before '## Completion check' does not satisfy this requirement; '§4 coverage "
        "verified' is valid only after '§ properties complete' has appeared in the current '## Completion check' block, '§ test suite complete' has appeared, and every covered "
        "dimension cites such a substring; '## Completion check' must not introduce a dimension absent from '## Behavioral dimensions' — a dimension named for the first time inside '## "
        "Completion check' does not satisfy any coverage requirement and does not extend the governed dimension set; a turn containing a satisfied 'Gate condition:' block must contain "
        "the governed tool call in the same turn — prose appearing between the 'Gate condition:' block and the governed tool call does not satisfy this requirement; every assistant "
        "turn after '§ implementation permitted [N]' has appeared in the transcript that does not contain a line beginning with '§ implementation permitted [' must contain '## "
        "Completion check' — a turn that does not contain a line beginning with '§ implementation permitted [' and does not contain '## Completion check' does not satisfy ground; '## "
        "Completion check' is satisfied only when the literal string '§4 coverage verified' appears within it — a '## Completion check' block that does not contain '§4 coverage "
        "verified' does not satisfy this requirement and the governed turn must continue to the next derived step rather than ending — a turn whose final non-blank content line is '§4 "
        "coverage verified' is exempt from the resume phrase requirement; when a turn is non-exempt, its final non-blank content line, after trimming trailing whitespace, must equal "
        "exactly: Resume: say \"Continue autonomously — gates still apply\" to proceed under the same protocol. (5) enumerate escape paths: '## Path enumeration' must not appear before "
        "'## Enforcement sequence' in the transcript; enumerate every path by which §1–§4 headings could be present without the governing goal identified, dimensions derived, or "
        ...
        "line in the transcript; any referent named in the '[text]' from §1 not already present as a literal string in the transcript above §1 must be fetched before §1 is written; the "
        "string '§5 enumeration complete' appears exactly once; the string '§ implementation permitted' followed by a space and a decimal integer (e.g. '§ implementation permitted 1') "
        "must appear in the transcript after '§5 enumeration complete'; a sentinel line containing the literal bracket characters '[' or ']' does not satisfy this requirement — only a "
        "decimal integer satisfies the index requirement; the integer must equal the 1-based ordinal count of file-modifying tool calls that will have appeared through and including "
        "the tool call immediately following this sentinel — '§ implementation permitted 1' must precede the first file-modifying tool call, '§ implementation permitted 2' must precede "
        "the second, and so on; a checker verifies this by scanning the transcript for every file-modifying tool call, locating the final non-blank line of the immediately preceding "
        ...
        "content must appear, it must precede the sentinel, not follow it. Continuation invariant: when the final non-blank content line of a turn contains or follows one of the "
