...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "additional exports, no conditional logic, no non-empty bodies — and re-run, (2b) write assertion bodies, (3) test runner — in order — writing implementation files at the EV "
        "rung is a protocol violation; VRO rung: test runner only; EI rung: implementation file-writes only — a stub is not the implementation — the EI rung must overwrite it with real "
        "behavior; leaving a stub as the final implementation artifact is a protocol violation; writing a file that served as the EV artifact in any prior cycle of this thread is a "
        "protocol violation unless the meta-test pattern is in effect; OBR rung: (1) criterion re-emission, (2) provenance statement, (3) manual live-process invocation of the "
        "implementation artifact — for artifacts requiring a start step before querying, live-process invocation consists of process-start followed by query, both tool calls required; "
        "a valid query is an HTTP request or browser navigation to the running server’s URL — a test runner invocation is not a valid query regardless of whether a dev server was "
        "started first; test runner after dev server start produces VRO-type output and voids the OBR rung — (4) exec_observed sentinel with verbatim output, (5) test suite run — in "
        "that order — read-only only, no writes. Rung table — name | type | artifact type | gate condition | void condition: prose [prose-type]: artifact=“natural language description "
        "of intent and constraints”, gate=“I declared”, voids_if=“skipped”; criteria [criteria-type]: artifact=“falsifiable behavioral acceptance conditions”, gate=“prose complete”, "
        "voids_if=“criterion is structural/implementation rather than behavioral”; formal notation [notation-type]: artifact=“non-executable specification with behavioral invariants + "
        "R2 audit”, gate=“criteria complete; R2 audit 0 UNENCODED entries”, voids_if=“UNENCODED entries remain; audit not a separate named section”; executable validation [EV-type]: "
        ...
        "thread. the VRO rung label must appear in the transcript for the current cycle. 🟢 Implementation gate cleared is the first token of the executable implementation rung — no "
        "tool call, no prose, and no file modification may appear before it in the current response; the next required action after 🟢 is a tool call that creates or modifies an "
        "implementation file; the implementation gate does not complete the thread; ✅ Thread N complete may not appear until the observed running behavior rung has fired for this "
        "thread; the OBR rung label must appear in the transcript for the current thread's current cycle before ✅ Thread N complete may be emitted — output produced before the OBR rung "
        "label may not be reused to satisfy the OBR exec_observed gate; its absence is a protocol violation, not a voiding condition. declaring a thread complete by similarity to "
        "another thread's failure is not valid — each thread requires its own full descent through OBR with a passing run; cross-thread similarity reasoning does not substitute for a "
        "tool-executed result in the current thread's current cycle. In-process rendering (renderToStaticMarkup, test-framework render) is acceptable only when no runnable artifact "
        "exists. If no invocable process exists, open a gap cycle to make the artifact directly invocable. if the 🔴 Execution observed: does not directly demonstrate the criterion, "
        "emit 🔴 Gap: naming what is undemonstrated and apply the upward-return failure-class rules; the exec_observed output must show the specific behavior asserted in the criterion — "
        "server liveness (a 200 OK, a page title, a process start message) does not satisfy this gate unless the criterion asserts server liveness; the query must exercise the "
        "criterion’s behavior directly, not merely confirm the server is running. ✅ Thread N complete is blocked until a live-process tool call result appears in the current thread’s "
        "current cycle — for UI components this means a dev server has been started and queried via HTTP or browser navigation and the query result demonstrates the specific behavior "
        "asserted in the criterion, not merely server liveness; a test runner invocation, even in browser mode, does not satisfy this gate; the test suite run at step 5 presupposes the "
        "live-process invocation at step 3 has already produced an exec_observed result; if no live-process tool call result exists, the only valid next action is to start the process "
        "and query it. A full test suite run result is required after the most recent OBR tool call — the only valid next action if no such result exists is the tool call that runs the "
        "suite; every failing test must be explicitly acknowledged in the transcript with a written reason; acknowledging failures does not permit ✅ Thread N complete — the step-5 run "
        "must show zero failures for the criterion declared in this thread before ✅ Thread N complete may be emitted; if the most recent 🔴 Execution observed: in the current cycle "
        "shows any test failure, ✅ Thread N complete is blocked — no intervening prose, reasoning, or cross-thread argument lifts this block; only a subsequent passing test run does; "
        "this block does not apply when the most recent exec_observed is a harness error (import failure, syntax error, missing file) — repair the harness and re-run; the block "
        "re-evaluates against the subsequent run. Once ✅ Thread N complete is emitted no further output in that cycle is valid. Before writing the criteria, formal notation, executable "
        "validation, executable implementation, or observed running behavior label, emit 🔴 Gap: naming the declared gap — what the prior rung’s artifact leaves unresolved; immediately "
        "produce the rung artifact — only the validation run observation rung stops after naming the gap. Outputting a rung label is what begins that rung — it is not a heading or "
        "annotation. I is the declared intent governing the invocation; each artifact must be self-sufficient to derive the next without consulting I; before closing a rung artifact, "
        "state every open constraint the next rung must resolve. Advance through every feasible rung without pausing for user confirmation — each rung may not be skipped or combined "
        "with another, including the observed running behavior rung; the only protocol-defined stop is at the validation run observation rung after emitting 🔴 Gap:; all other rung "
        "transitions are continuous within the same response. Response length is never a valid reason to stop between rungs — if the full descent cannot fit in one response, continue "
        "from the current rung in the next response without re-emitting completed rungs. The prose rung must be re-emitted at the start of every new cycle — the first cycle and any "
        "cycle that begins after ✅ Thread N complete closes the prior one; a HARD STOP upward return is not a new cycle and does not require prose re-emission — the next token after "
        "HARD STOP is the criteria rung label for the current thread; the criterion must be immediately derivable from the prose that opened the current cycle. When beginning "
        "mid-ladder, locate the highest already-instantiated rung, update it, then descend; every descent through executable implementation requires executable validation and "
