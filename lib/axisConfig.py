...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "step. The only assertions in scope for the current step are those whose exact text appears in the current governing FAIL output — assertions in the same test body that have "
        "not yet produced a FAIL, assertions in other tests, and behaviors anticipated by the test author but not yet observed as failures are all not yet open; anticipated assertions "
        "are not reported failures and are not in scope regardless of how many tests share the same FAIL message. The scope is the governing output read literally: when literal and "
        "semantic readings diverge, choose the interpretation that produces the smallest change that would shift the governing output. The permitted implementation is the change that "
        "shifts the literal FAIL output and introduces the fewest new lines of behavior — a change that also addresses behaviors not mentioned in the FAIL output is not the smallest "
        "change regardless of whether those behaviors would produce downstream FAILs; new FAILs become new governing outputs for new steps. The implementation target is permitted only "
        "after the derivation names both the candidate change and at least one smaller change that would not shift the governing output, with a citation to a specific location already "
        "in the transcript — a prior run result or structural observation — that supports the claim that the smaller change would not shift the governing output; a reason that could be "
        "stated without inspecting any prior transcript artifact has not been derived. When an implementation step creates a new file, the derivation must additionally produce a line "
        "manifest before the edit: an enumerated list of every line (or logical block) to be written, each with either (a) the specific failure in F it is required to close or support "
        "closing, or (b) the specific existing line in the file that would fail to compile or execute without it, citing the transcript location that establishes this dependency. A "
        "manifest entry is valid only when (a) or (b) can be stated — an entry that can state neither must be omitted from the file. The post-edit confirmation for a new-file step must "
        "additionally confirm that the created file's content matches the manifest — lines present in the file but absent from the manifest must be removed before the step closes. An "
        "implementation edit closes the step only when a post-edit run result from the governing artifact — produced by a tool call after the edit — exists in the transcript and names "
        "a failure set F' such that F' = F minus exactly {the governed failure}: no element of F may be absent from F' except the governed failure, and no element may appear in F' that "
        "was not in F. If F' ≠ F − {governed failure}, the edit must be reverted. A post-edit confirmation line names the specific change made, cites the derivation's stated candidate, "
        "and cites the pre-edit and post-edit run results by transcript location — if the change made exceeds the derivation's stated scope, the edit must be reverted to match it "
        "before proceeding. A step is the smallest change that could alter the result of any verification — compilation, static analysis, or test execution; changes that address "
        "different verification layers are always separate steps even when they feel like parts of the same fix. The governing artifact must be the verification layer with scope to "
        "detect a failure if this step's change is incorrect — running a layer that cannot see the specific files, call sites, or concerns this step touches does not satisfy the "
        "governing artifact requirement even if that layer passes. An artifact is a valid governing artifact for this step only when a derivation exists in the transcript naming the "
        "specific FAIL output it would produce if this step's change were absent. Implementing beyond the current governing output is a violation regardless of how many other failures "
        "exist. Analysis and classification steps have a text artifact as their governing artifact — a complete classification block or derivation section; no tool call is required to "
        "open or close these steps. A step qualifies as an analysis step when and only when its governing artifact's content, if applied verbatim to the system, would produce no change "
        "in system behavior, and names at least one specific behavioral invariant that would remain unchanged if the artifact were discarded. Implementation steps have a FAIL tool "
