...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "proven in Challenge output [ ] Baseline: derive from P2,P3 — why Baseline must establish ALL evaluation mechanics needed (may create them); what GOES WRONG if you add new "
        "mechanics at Challenge: scope expands beyond intent; RULE: Baseline = create all mechanics needed, Challenge adds NEW criteria using EXISTING mechanics[ ] Structure before "
        ...
        "nothing. IMPORTANT: Baseline establishes ALL evaluation mechanics needed for the task. These may be created at Baseline. Challenge adds NEW criteria but uses EXISTING "
        "mechanics - adding new mechanics at Challenge is scope expansion. IMPORTANT: Baseline evaluation must be SELF-CONTAINED. If it fails due to missing prerequisites, that's a "
        "broken framework, not evidence of gaps. Only NEW criteria added at Challenge can expose gaps in the solution. IMPORTANT: Baseline evaluation must be SELF-CONTAINED. If it "
        "fails due to missing prerequisites, that's a broken framework, not evidence of gaps. Only NEW criteria added at Challenge can expose gaps in the solution. 5. Challenge — What "
        "evidence proves gaps exist? Derivation: from Baseline, add NEW evaluation criteria that FAIL against the solution. Evidence: failing output from NEW criteria proves gaps "
        "exist, NOT failing because framework is broken. IMPORTANT: Work on the smallest verifiable thing at a time — each check must be satisfied before the next. 6. Refinement — How "
        "is the artifact improved? Derivation: from Challenge, address the SPECIFIC gaps that were OBSERVED. Evidence: changes target the exact gaps proven to exist. IMPORTANT: "
        "Refinement must address OBSERVED gaps only - gaps seen in Challenge output. Addressing unobserved gaps wastes effort and may miss real issues. 7. Verification — How is "
        ...
        "verification confirms no gaps remain. Domain adaptation: for tasks where verification is possible, define evaluation scaffold BEFORE producing solution — evaluation defined "
        "after solution is vacuous (Texas sharpshooter). Baseline defines evaluation framework; Challenge applies it to show OBSERVED gaps; Refinement produces solution that addresses "
        "those OBSERVED gaps.for writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to "
