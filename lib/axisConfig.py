...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "file read used as evidence”. 🔴 Execution observed: requires: (1) a preceding tool call in this response, (2) verbatim tool output in a triple-backtick block — complete, "
        "nothing omitted, no editorial content. Any deviation voids the sentinel; fix by re-invoking the tool and re-emitting the block. Once ✅ Ground entered has been emitted in this "
        "session, every subsequent response must begin by identifying the current rung and the current gap before any other content — ground protocol does not terminate at ✅ Manifest "
        "exhausted; it remains active until the session ends or the user explicitly exits. Each completion sentinel is valid only at its defining rung — emitting a sentinel outside its "
        ...
        "descending again. The formal notation rung separates behavioral specification from explanation. Formal notation (type signatures, interfaces, invariants, pre/postconditions) "
        "encodes what must be true — the executable or testable part. Natural language labels, introduces, and explains the specification — prose adds context but the behavioral "
        "constraints must still be encoded in notation.it must encode all structural constraints the criterion implies — the permitted forms (type signatures, interfaces, pseudocode, "
        "structural invariants) describe what notation looks like, not what it may omit; selecting a subset of permitted forms that leaves constraints unencodable is insufficient; it "
        "produces only type signatures, interfaces, pseudocode, and structural invariants — the test of valid formal notation is that it cannot be directly compiled or executed without "
        ...
        "enumerate multiple failures or plan fixes; a green exec_observed at the VRO rung means there is no failing assertion — 🔴 Gap: may not be emitted after a green exec_observed; a "
        ...
        "thread marker: [T: gap-name]; before emitting ✅ Manifest declared, count every [T: gap-name] marker in the prose — the manifest thread list must contain exactly one entry per "
        "distinct gap-name; a [T: gap-name] marker in the prose with no corresponding manifest entry is a protocol violation. The prose rung must be re-emitted at the start of every "
        "cycle — the first cycle, any subsequent cycle, and any cycle following an upward return; the criterion must be immediately derivable from the re-emitted prose. 🟢 "
