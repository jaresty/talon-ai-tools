...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "lowest-effort valid path — deviation must require additional effort, introduce failure risk, or be blocked. Each rung derives from what the prior rung actually produced — this "
        "progressively narrows the interpretation space so that no rung can claim more than its inputs justify. Memory is not evidence — what a model recalls about a prior step carries "
        "the same confabulation risk as any other model output, so only what is visible in the transcript counts as having happened. A shorter ladder is not more efficient — each "
        "collapsed step is ambiguity that was not closed, and ambiguity that was not closed remains available as an escape route. Path B §0 is a starting gate, not a bypass — "
        ...
        "Each rung derives from the prior rung's output; no rung has a fixed step count. Path classification (required before §0): if an artifact that, when invoked, executes the "
        "subject system and returns its live output — such as a shell command, running script, endpoint call, or test suite invocation — appears verbatim in the conversation above, "
        "apply Path A; otherwise apply Path B. Derivation phase: Path A — (0) invoke the named artifact as a tool call; the artifact qualifies for Path A only if, when invoked, it "
        "executes the subject system and returns its live output; an artifact that returns document content does not qualify, and a tool result consisting of a GitHub issue body, pull "
        "request body, specification document, README, or requirements text does not satisfy §0 regardless of invocation form; write '§0 observed' when a tool-result block reflecting "
        "live system execution is present above it — '§0 observed' must not appear before such a block is present; a tool-result block produced by invoking the named artifact as a tool "
        "call satisfies this requirement; a result produced by a Read, search, or file-write tool call does not; Goal-source classification (required after '§0 observed', Path A only): "
        ...
        "names the subject from the conversation — this declaration satisfies §0; (1) derive the governing goal: write '## Governing goal: [derived: text]' where '[derived: text]' is a "
        "goal derived from the scenario description — what would satisfy the underlying need, not merely what was asked; a '[derived: text]' value that adds a claim absent from the "
        "user's message does not satisfy §1; immediately below the governing goal heading, before the means-test, extract labeled fields from '[text]': write each as a proper "
        ...
        "consecutive words; when every named component of '[text]' is covered, write '§1a decomposed'; '§1a decomposed' must appear before the means-test line; immediately below '§1a "
        "decomposed', derive a means-test: write 'The goal [text] could be achieved by: [alternative means]'; if only one concrete alternative can be named, write '[hypothetical]: "
        "[speculated alternative]' before writing '§1 goal derived'; when ≥2 alternatives (concrete or hypothetical) are present, write '§1 goal derived'; (Both paths continue from §2 "
        "onward:) immediately below '§1 goal derived', derive dimension candidates: for each alternative means named in the means-test, write one candidate beginning with '- "
        ...
        "candidates'; (2) derive behavioral dimensions: '## Behavioral dimensions' must not appear before '§1b candidates' in the transcript; derive at least two dimensions; each "
        ...
        "dimensions closed'; (3) derive the enforcement sequence: '## Enforcement sequence' must not appear before '§2 dimensions closed' in the transcript, and must appear before the "
        "first file-modifying tool call regardless of whether '§ implementation permitted' appears; each sub-heading names as a literal substring what the prior sub-heading's passing "
        "state admits that this sub-heading rejects; (4) verify coverage: '## Completion check' must not appear before '## Enforcement sequence' in the transcript and must not appear "
        "before '§ implementation permitted' in the transcript; each covered dimension cites a verbatim string that appears either as a substring of a tool-result block produced after "
