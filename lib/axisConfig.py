...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "valid; “need to implement a landing page” is not; naming an absent artifact is not a valid gap. The criteria rung produces exactly one criterion — the minimal condition whose "
        "satisfaction closes the declared gap; further criteria emerge from subsequent gaps after each cycle completes. Each rung’s artifact addresses only that gap — not all known "
        "requirements of the task. Formal notation encodes only the criteria declared at the criteria rung — no additional invariants, no anticipated cases. Only validation artifacts "
        "may be produced at the executable validation rung — implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not constitute "
        "executable validation — the artifact must be invocable by an automated tool; a pre-existing test that happens to pass does not satisfy this rung — the artifact must be newly "
        "produced for the declared gap; the validation artifact must fail before any implementation edit — a green run with no prior red run does not satisfy this rung. At the "
        ...
        "after each edit re-run the validation artifact before any further edit. Outputting a rung label is what begins that rung — it is not a heading or annotation; a rung whose "
        "label has not been output has not begun and no artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to reflect the "
        "intended change, then descend. Reconciliation gate: at every upward rung transition — whether the scheduled formal-notation → executable-validation gate or any upward return "
        "triggered by new information — reconcile all artifacts at that rung against I before redescending; documentation is a reconciliation artifact, not a thread. Seven rungs in "
        "order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running behavior. Sentinel formats — 🔴 "
