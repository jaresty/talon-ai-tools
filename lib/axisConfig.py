...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "this rung — add a criterion asserting its content or effect. after the criterion is written, the only valid next token is the formal notation rung label; no formal notation "
        "content may appear until the formal notation rung label has been emitted — emitting notation content before the formal notation rung label is a protocol violation even if the "
        "rung label eventually follows. the criteria rung artifact is exactly one criterion for the current thread only — not one criterion per thread, not criteria for all threads "
