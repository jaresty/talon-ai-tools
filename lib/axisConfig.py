...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "another's next pass. Emit one line 'Message <N>: <from-label> to <to-label> — <content>' per exchange, at a later transcript position than the receiving context's disposition "
        "block and at an earlier position than that context's next '## shoshin pass'/'context' header, so every forwarded item has a disposition to be audited against. Each Message is "
        "Forwarded:-audited exactly as a delegate briefing is: one 'Forwarded: <item> — [requirement | inadmissible-frame]' line per item in the message; an inadmissible-frame item "
        "must be absent from the message or appear only as an explicit question. A dialogue emits 'Terminal: [converged | deadlock | divergence]' at a later position than its last "
        "Message line and at an earlier position than Reconcile; a Terminal line is valid only when at least one Message line appears at an earlier position. converged = the contexts "
        "reached one claim; deadlock = they hold incompatible claims neither drops; divergence = they produced different claims reported as the finding. Convergence is not the goal "
        "and is not required; a productive deadlock or a reported divergence is a valid, often preferred terminal state — the value of the exchange is in the differences it surfaces, "
        "not in agreement. Isolation mode (parallel and dialogue), block decides — the same observable that resolves delegate: if a subagent tool-use result block is present for a "
        "context, that context's isolation is ENFORCED and its 'Leak: not found' for inadmissible frames is guaranteed by construction; if no such block is present, the context is a "
        "DECLARED in-thread simulation (reasoning under withheld frames without a separate process) and isolation rests on the leak check alone, with the same honest ceiling as "
        "single-thread delegation — declared, not enforced. Separability governs parallel and dialogue as it governs delegate: a context is isolated only when the frames withheld from "
        "it are separable from the minimal statement of its task; a frame that re-enters through a message is a leak, caught by that message's Forwarded: audit and by the leak check. "
        "Every item entering an isolated context — the brief that spawns a delegate, parallel, or dialogue context, and every subsequent dialogue message — is Forwarded:-audited "
        "before that context's tool-use call: emit one 'Forwarded: <item> — [requirement | inadmissible-frame]' line per item, each at an earlier transcript position than the tool-use "
        "block it enters; an inadmissible-frame item must be absent from that input or appear only as an explicit question. The leak check runs over every context's conclusions AND "
        "every Message's forwarded content.  (3) Reason: reason about the subject using the default and admit frames, excluding the inadmissible frames.  (4) Reconcile: compare the "
        "selected-frame result against the current-frame view and either quote a specific differing claim or state no claim differs. Before Reconcile, run the leak check: for each "
        "conclusion, identify any inadmissible frame it relied on — reused in-thread or passed as an assumption in a delegate's tool-use input; emit 'Leak: found' with the frame and "
        "redo excluding it, or 'Leak: not found'; iteration terminates at 'Leak: not found'. When the pass ran in a genuinely isolating subagent that never received the inadmissible "
        "frames, 'Leak: not found' for those is guaranteed by construction. A shoshin-eligible response that produced no disposition lines has not established that its frame set was "
        "chosen rather than inherited; a response containing a delegation tool-use block with no disposition lines at an earlier transcript position does not satisfy this token; and a "
        "response containing a delegation tool-use block with no 'Forwarded:' line at an earlier transcript position than that block does not satisfy this token, since the items "
        "entering the delegate's prompt were not audited for inherited framing before the handoff.",
