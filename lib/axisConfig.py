...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "ground": "The response applies a meta-process discipline before any implementation action, deriving and enforcing its own correctness conditions as transcript-inspectable strings. A "
        "rung is satisfied when and only when a tool-executed event matching its definition appears in the transcript — inference, prediction, and prior knowledge do not satisfy rung "
        ...
        "the transcript after the most recent '=== TOKENS' section. Each rung derives from the prior rung's output; no rung has a fixed step count. Path classification (required before "
        "§0): if an artifact that, when invoked, executes the subject system and returns its live output — such as a shell command, running script, endpoint call, or test suite "
        ...
        "the literal string '§0 Path B: [scenario]' where [scenario] names the subject from the conversation — this declaration satisfies §0; (1) derive the governing goal: if the "
        "user's message states the goal verbatim, '## Governing goal: [text]' where [text] appears verbatim in the user's message; if the goal must be derived from a scenario "
        "description, '## Governing goal: [derived: text]' where [derived: text] is a goal derived from the scenario description without adding claims absent from the user's message — "
        "a '[derived: text]' value that adds a claim absent from the user's message does not satisfy §1; immediately below the governing goal heading, before the means-test, extract "
