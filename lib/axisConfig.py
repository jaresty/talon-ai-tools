...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "coverage to the original property; (2) each part admits at least one admissible construction that falsifies that part without falsifying the other. To test atomicity, "
        "provisional candidate sub-properties may be introduced immediately before the split test using: 'property [Na]: <definition>' and 'property [Nb]: <definition>' — these "
        "provisional lines exist only for evaluating the proposed split and are not members of the property set unless the split succeeds. Immediately after the provisional lines, emit "
        'exactly one split test: \'§ split test: property [Na]: "<verbatim definition>" — falsified by <construction>; property [Nb]: "<verbatim definition>" — falsified by '
        "<construction>; these falsifiers are independent.' The quoted definitions must match the immediately preceding provisional definitions verbatim. A split succeeds only if: the "
        ...
        "other; both falsifiers are admissible constructions within the chosen interpretation. If the split succeeds, replace the original property with the accepted sub-properties and "
        'continue recursively testing each accepted property. If no valid split exists, emit: \'§ split test: property [N]: "<verbatim definition>" — atomic, no valid split: '
        "<reason>.' The quoted definition must match the retained property verbatim. Continue recursively until every retained property has a split test concluding 'atomic, no valid "
        "split.' After all retained properties are atomic, emit a completeness check: '§ completeness check: \"<request constraints verbatim>\" / <P1 expression> / <P2 expression> / "
