...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "assertions that will fail; these assertions define the intent that implementation must satisfy; The assertions in validation rung tests are FIXED intent declarations — they "
        "cannot be changed in later rungs; implementation must make them green, not make tests match implementation; Assertion rung is per assertion: each individual assertion is its "
        "own rung. Implement ONLY enough to make THAT specific assertion green. Tests are IMMUTABLE once written at validation rung — implementation must satisfy tests, never modify "
        "tests to match implementation. If tests are wrong, return UP to validation rung to fix them; modifying tests in later rungs voids the rung. Infrastructure failures (import "
        "errors, missing files, syntax errors) are NOT assertion failures. If infrastructure fails, return UP to validation rung to fix the infrastructure issue before continuing. "
        "ASSERTIONS are test code that verifies behavior, NOT import errors or infrastructure failures. An import error means the file doesn't exist - fix the file first, not the test. "
        ...
        "artifact is produced and gate is satisfied; No implementation is permitted before the assertion rung shows red assertions AND the specific RAW assertion text is displayed; the "
