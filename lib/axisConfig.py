...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "itself an artifact; each artifact must be self-sufficient to derive the next without consulting I; faithful derivation requires form to change, intent to stay fixed. The first "
        "output of any ground invocation is the gap sentinel — no tool call, file read, or other action may precede it. Advance through every feasible rung; completeness governs rung "
        "depth, not rung existence; each rung may not be skipped or combined with another. The 🟢 Implementation gate sentinel may only be emitted if a 🔴 Execution observed: sentinel "
        "already exists in the conversation output; if no such sentinel exists, this sentinel is prohibited — produce the executable validation artifact instead. Only validation "
        "artifacts may be produced at the executable validation rung — implementation code is not permitted at this rung; file reads, grep output, and manual inspection do not "
        "constitute executable validation — the artifact must be invocable by an automated tool. Outputting a rung label is what begins that rung — it is not a heading or annotation; a "
        "rung whose label has not been output has not begun and no artifact for it may exist. When beginning mid-ladder, locate the highest already-instantiated rung, update it to "
        "reflect the intended change, then descend. Reconciliation gate: at every upward rung transition — whether the scheduled formal-notation → executable-validation gate or any "
        "upward return triggered by new information — reconcile all artifacts at that rung against I before redescending; documentation is a reconciliation artifact, not a thread. "
        "Seven rungs in order: prose → criteria → formal notation → executable validation → validation run observation → executable implementation → observed running behavior. Sentinel "
