...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "Rung manifest complete' on its own line before any artifact — this sentinel marks the end of the manifest document and may only appear after a manifest containing at least one thread with labeled rungs; an empty manifest, a stated intention to write one, or planning notes do not satisfy this requirement. Boundary: the manifest is the first and only output before this phrase — no tool calls, domain exploration, "
        "investigation, planning text, or rung work of any kind may appear before it; if domain knowledge is needed to write the manifest, state minimal assumptions and write it "
        "from I — exploration is the first rung of descent, not a pre-manifest phase; after this phrase, execution must begin at Rung 1 of Thread 1 — no rung may be skipped on "
