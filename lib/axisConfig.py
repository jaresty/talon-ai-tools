...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        'required Failure: line from the tool-result block, then restore the original state before continuing; emit \'Failure: property [N] "<verbatim property definition>" — '
        "<verbatim quoted failure line from a tool-result block>'; emit 'Unobservable: property [N] \"<verbatim property definition>\" — <reason>' only after demonstrating that no "
        "temporary violating state can be constructed within the governed artifact on which the guard could distinguish satisfaction from violation — Unobservable: is the last resort, "
        "not the first; successful compilation, build output, or unrelated verification does not satisfy this step — the violating state must exist solely to witness that the guard "
        "detects the absence of property [N]; before modifying the governed artifact to satisfy property [N], a verbatim Failure line from a tool-result block must appear; if the "
        "failure line does not specifically witness the violation of property [N], modify the violating state and repeat from step (4); after the Failure: line has been emitted, emit "
        "exactly one witness line for every previously inventoried executable assertion, in inventory order; each witness line takes the form: 'Property [N] assertion [M] witness: "
        'Failure — assertion: "<verbatim assertion text>" — failure: "<verbatim tool-result failure text>"\' if the assertion produced the observed failure, \'Property [N] assertion '
        '[M] witness: Success — assertion: "<verbatim assertion text>"\' if the assertion passed, or \'Property [N] assertion [M] witness: Unreached — assertion: "<verbatim assertion '
        "text>\" — <reason execution did not reach this assertion>' if execution could not reach the assertion; the verbatim assertion text in each witness line must match the "
        "assertion text from the corresponding inventory line exactly; no witness may be emitted for an assertion not present in the assertion inventory, and no inventoried assertion "
        "may lack a witness; at least one inventoried assertion must receive witness classification 'Failure' — a guard execution in which no inventoried assertion is classified "
        "'Failure' does not satisfy step (4), regardless of whether a Failure: line was produced; a 'Failure' witness whose verbatim failure text is not present in the corresponding "
        "Failure: tool-result block does not satisfy this token; an 'Unreached' witness without a stated reason does not satisfy this token; if no inventoried assertion can be "
        "attributed to the observed failure, revise the violating state or the regression guard and repeat from step (2); after all assertions are witnessed, emit 'Assertion "
        ...
        "count does not satisfy this token; emit 'Assertion witnesses: incomplete — <reason>' if any assertion lacks a witness; if incomplete, revise the violating state or guard and "
        "repeat from step (4); no subsequent step may occur until 'Assertion witnesses: complete' has been emitted; (5) identify the governed artifact for property [N] — the artifact "
        "whose modification satisfies property [N]; depending on the property, this may be the implementation, the regression guard, a configuration file, or another explicitly "
        "governed artifact; modify the governed artifact to satisfy property [N], then immediately emit 'Quoted implementation: <smallest contiguous verbatim text uniquely identifying "
        "the modification to the governed artifact>'; if step (4) required constructing a temporary violating state because the governed artifact already satisfied property [N], step "
        "(5) is satisfied by confirming the original state is restored — emit 'Quoted implementation: <verbatim text identifying the restored governed artifact>' to confirm; no "
        "subsequent step may occur until this sentinel has been emitted; (6) attempt to construct a minimal modification to the quoted implementation that preserves the success of the "
        "quoted guard while still changing the governed artifact, and attempt to construct a concrete execution path through the quoted implementation that is not exercised or "
        "distinguished by the quoted guard; emit 'Implementation overreach: found — <modification or execution path>' or 'Implementation overreach: not found'; if found, minimally "
        "revise the governed artifact to eliminate the overreach, emit a new 'Quoted implementation:', and repeat from step (6); every implementation overreach analysis must reason "
        "only from the quoted implementation and quoted guard emitted earlier in the transcript, not from an imagined or intended version. After all properties complete these steps, "
        "emit 'Coverage: complete' if all properties are resolved, or 'Coverage: gap — property [N] \"<verbatim property definition>\"' for any unresolved property; if any gap "
        "exists, return to step (1) for each ungoverned property and complete all six steps before re-emitting Coverage:; Coverage: complete may not be emitted while any property [N] "
        "lacks a witnessed Failure: or Unobservable: line. A response that omits any required sentinel, performs a step before emitting its required quoted artifact, modifies the "
        "governed artifact to satisfy property [N] before a quoted Failure line appears, omits any executable assertion from the assertion inventory, or leaves a property without an "
        "automatic regression guard does not satisfy this token.",
