...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "confirm intent has been met; declaring completion without a closing observation is a protocol violation. P14 (Evidential authority): only tool-executed events have evidential "
        "standing; inference, prediction, prior-cycle output, and model-generated descriptions of tool output have none, regardless of accuracy; a rung gate is satisfied if and only if "
        "a tool-executed event appears in the current-cycle transcript whose output is of that rung’s artifact type; a rung gate is not a description of evidence — it is a blocker: no "
        ...
        "verify the gate condition is met — emitting either before the gate condition is met is a protocol violation that voids the rung; 🔴 Execution observed: is only valid when a "
        "tool call was made in the current response immediately before it — a sentinel emitted without a preceding tool call is a fabrication — it voids the rung in which it appears "
        "regardless of whether its text resembles tool output. P15 (Cycle identity): evidence is valid only within the cycle in which it was produced; a cycle opens at the prose rung "
        "emission for a given thread and closes at Thread N complete or an upward return; evidence from a prior cycle, a different thread, or a different gap does not satisfy any gate "
        "in the current cycle regardless of type match; prose re-emission for one thread does not affect the cycle identity of any other thread. P16 (Provenance): a gate is satisfied "
        "only by a tool call made in direct response to the gap declared at the immediately prior rung in the current cycle; a tool call made for a different purpose, targeting a "
        "different gap, or produced in a prior cycle does not satisfy any gate regardless of type match. P17 (Derivation chain): each artifact must be derived from the prior rung’s "
        "actual content — not from the original intent reconstructed from memory; a skipped rung voids all artifacts below it; form changes between rungs, intent does not; divergence "
        "from intent is only detectable at the rung where the prior artifact’s specificity was insufficient — breaking the chain makes that detection impossible. P18 (Continuous "
        ...
        "the table is produced without pausing for confirmation; ✅ Manifest declared is blocked until the rung table artifact has been produced in the current response — a manifest "
        "emitted without a preceding rung table is a protocol violation; the rung table is the gate on all subsequent rung activity. For software behavioral change, the standard "
