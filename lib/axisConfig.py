...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "atomic": "The response enforces one governing output per step: take the first failure from a tool-executed run result appearing above this line in the transcript as the step's scope — "
        "the candidate is limited to the change that resolves the first failure only; other failures visible in the same result are not in scope for this step — they may not be cited "
        "to expand the candidate and become the governing failures for subsequent steps. Before any edit, produce three artifacts as separate completed blocks — each block must end "
        "before the next opens, and no tool call that modifies a file may appear in the same block as any artifact: (1) scope declaration quoting verbatim the failure signal from the "
        "prior run result — the quoted text must appear in a tool result above this line; (2) derivation block as two entries: first, name the candidate change and at least one smaller "
        "change; the smaller change must be the minimal stub — a version of the candidate with the correct type signature that returns a value of the correct type without performing "
        "the governed transformation; a change that produces a type error, a crash, or a failure mode different from the governing failure signal at the call site is not a valid "
        "smaller change; then run the smaller change and produce a tool-executed result — if the governing failure signal text is absent from that result, the smaller change is "
        "sufficient and must become the candidate; if the governing failure signal text is present, the smaller change is insufficient and the candidate is permitted; an insufficiency "
        "claim is not permitted until a tool-executed result showing the smaller change appears above it in the transcript; an insufficiency argument that names or references any "
        "failure string other than the governing failure signal text does not satisfy this requirement — the insufficiency must be derivable from the governing failure signal text "
        "alone; second, in a separate entry below, quote the specific text from the governing failure signal that shows the governed behavior absent — the quote must appear in a "
        "tool-executed run result above the first entry; a candidate that is larger than the smallest change that resolves the quoted failure text is not permitted regardless of what "
        "else it also resolves; a failure not yet observed in a prior tool-executed run result has not been shown to govern, and a change that fixes it cannot be derived from the "
