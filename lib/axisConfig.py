...
AXIS_KEY_TO_VALUE: Dict[str, Dict[str, str]] = {
    ...
    "method": {
        ...
        "satisfied[ ] Baseline: derive from P2,P3 — Baseline defines evaluation structure not solution; RULE: Baseline = evaluation structure must PASS; Prove why each element is "
        "necessary or remove it [ ] Baseline scaffolding: derive from P2,P3 — Baseline scaffolding is placeholder not solution; RULE: Baseline scaffolding = empty shell, Refinement "
        "adds functionality [ ] Structure before baseline: derive from P1,P2 — organization must precede artifact; what GOES WRONG if you create without structure: unfocused output; "
        "RULE: structure before baseline [ ] Criteria before structure: derive from P1 — success conditions before organization; what GOES WRONG if you organize without criteria: no "
        "way to evaluate; RULE: criteria before structure [ ] Intent before criteria: derive from P1 — outcome before success conditions; what GOES WRONG if you define success without "
        ...
        "definition and scaffolding, not implementation. IMPORTANT: Baseline MUST PASS. If it fails, that's a broken framework - fix it. Gaps/failures are introduced at Challenge, not "
        "Baseline. IMPORTANT: Baseline creates MINIMAL scaffolding only. If you add more than minimal, prove WHY you cannot make it smaller: would removing this break evaluation? Would "
        "it make evaluation structure unfaithful to criteria? If no, remove it. IMPORTANT: Baseline establishes evaluation framework at Baseline. Challenge adds NEW criteria to the "
        "SAME framework - creating NEW evaluation at Challenge is SCOPE EXPANSION, not valid Challenge. Challenge must extend existing evaluation, not create new ones. 5. Challenge — "
        "What evidence proves gaps exist? Derivation: from Baseline, add N new criteria to existing evaluation that FAIL. Evidence: N failing outputs prove N gaps exist. IMPORTANT: "
        "Challenge must use SAME evaluation framework from Baseline - creating new evaluation at Challenge is scope expansion, not valid Challenge. IMPORTANT: Work on the smallest "
        "verifiable thing at a time — each check must be satisfied before the next. 6. Refinement — How is the artifact improved? Derivation: from Challenge, for each failing "
        "criterion, add ONLY enough to satisfy that criterion. Evidence: modifications target the exact gaps proven to exist. IMPORTANT: Take ADVERSARIAL posture - try to satisfy "
        "verification with MINIMAL effort. Ask: what's the least I can do to make these N criteria pass? Do not add extra features or over-engineer. IMPORTANT: Refinement must address "
        "OBSERVED gaps only - gaps seen in Challenge output. Do NOT modify evaluation logic - only add solution content. 7. Verification — How is completion confirmed? Derivation: from "
        "refinement, run full evaluation from baseline. Evidence: all criteria satisfied = no gaps remain. IMPORTANT: After Refinement, if gaps still exist, iterate: Challenge → "
        "Refinement → Challenge → ... until no gaps remain. This is gap-driven iteration. Emit rung labels to begin each rung; emit completion sentinels (✅ [Rung] complete) to finish "
        "each rung. Evidence before claims: for every claim about completion, there must be evidence proving it. Ground complete may only be emitted after verification confirms no gaps "
        "remain. Domain adaptation: for tasks where verification is possible, define evaluation scaffold BEFORE producing solution — evaluation defined after solution is vacuous (Texas "
        "sharpshooter). Baseline defines evaluation framework; Challenge adds N failing criteria; Refinement adds only enough to satisfy each; Verification runs same framework.for "
        "writing tasks, it maps to intent→criteria→outline→draft→review→revision→final; for decision-making, it maps to intent→criteria→analysis→option→stakeholder "
