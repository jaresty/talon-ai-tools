...
COMPOSITIONS: list[dict[str, Any]] = [
    {
        ...
        "Adequacy is tested by attempted counterexample construction, not by asserted equivalence: for each retained property P and the assertions tagged to it, attempt to construct an admissible state on the wrong side of the property boundary while staying on the right side of the guard boundary, and execute the guard against it. The retained property supplies the observable criterion for whether P holds — P's own observable evaluation, not a model judgment of the state, exactly as an assertion's own result determines A-fail versus A-pass. If an executed construction shows P false while every assertion tagged to P passes, the guards do not govern P: emit 'Adequacy gap: <P> — <state>' and strengthen the guard so an assertion detects that state, then re-run the token's Gate 1 and Gate 2 for the affected assertions. If an executed construction shows P true while any assertion tagged to P fails, the guards over-govern P: emit 'Adequacy overconstraint: <P> — <state>' and weaken the guard to match P exactly, then re-run Gate 1. If an executed construction attempt produces no such state, adequacy is unrefuted by that attempt — this is never promoted to proof that the guards govern P; a bare adequacy claim with no executed construction attempt does not satisfy this gate. "
