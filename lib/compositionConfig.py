...
COMPOSITIONS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "prose": (
            ...
            "Test rung ladder (inserted between §2 dimensions closed and §3 enforcement sequence): "
            "after '§2 dimensions closed' appears, write '## Formalized properties' — each §2 "
            "dimension decomposed into its atomic formal claims, one claim per line, each prefixed "
            "'property [N]:' where N is a 1-based index; a property is atomic when no sub-claim "
            "can be false while the property as a whole is true; for each property write "
            "'§ property [N] atomic: [sub-claim that would be false while property holds]' — if no "
            "such sub-claim exists the property is already atomic; if one exists, split the property "
            "before proceeding. Then, for each property [N], a Falsify derivation block with an "
            "observed FAIL must appear in the transcript; the sentinel '§ test rung rejects: "
            "property [N]' is valid only when the immediately preceding labeled block in the "
            "transcript is a 'Falsify derivation:' block containing '(g) observed FAIL'; a "
            "'§ test rung rejects:' sentinel not immediately preceded by such a block does not "
            "satisfy this requirement. Write '§ test suite complete: [N] properties covered' only "
            "after every property [N] has a corresponding '§ test rung rejects: property [N]' line "
            "in the transcript, where [N] in the sentinel matches the count of 'property [N]:' "
            "lines in '## Formalized properties' — a count mismatch does not satisfy this "
            "requirement. '§ formalization complete' must not appear before "
            "'§ test suite complete' in the transcript."
        ),
    },
    {
        "name": "gate+atomic",
