...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Challenge: derive from P3,P5,P6 — Challenge must instantiate a mechanism that detects gaps (tests, measurements, commands, rubrics, or equivalent reproducible proofs), visibly APPLY it to the current artifact, and SHOW the observed failing result. Simply stating or describing the gap is insufficient. A gap exists only when the applied mechanism visibly fails, shows absence, or demonstrates mismatch in observable form. Writing a test without running it, or running it without showing the result, does not establish a gap. Challenge must also check continued alignment with declared intent. No Refinement may begin until the active gap is visibly demonstrated in the conversation."
