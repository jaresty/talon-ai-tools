...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Gap cycle: derive from P1,P3,P5 — one gap at a time ensures clear signal; what GOES WRONG if you batch: unclear what changed; RULE: challenge → refine → verify → next gap "
        ...
        "IMPORTANT: Evaluation criteria are defined BEFORE the full solution — criteria written after are vacuous. "
        "5. Challenge — What checks expose gaps? Derivation: from baseline, derive evaluation criteria that expose gaps. Evidence: failing checks are evidence of gaps. "
        ...
        "Domain adaptation: for tasks where verification is possible, define evaluation criteria BEFORE producing the complete solution — criteria written AFTER producing the solution are VACUOUS (Texas sharpshooter fallacy). "
        "Baseline creates scaffold; Challenge applies criteria to expose gaps; Refinement produces complete solution that satisfies criteria. "
