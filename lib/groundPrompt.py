...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Challenge: derive from P3,P5 — why Challenge MUST extend existing evaluation not create new; what GOES WRONG if you create new evaluation: scope expansion; RULE: Challenge = add N new criteria to baseline that FAIL show N failures "
        ...
        "[ ] Baseline: derive from P2,P3 — why Baseline MUST work not just exist; what GOES WRONG if you rationalize failure: broken framework confused with gaps; RULE: Baseline = fix whatever makes evaluation fail do not proceed until it passes ADVERSARIAL = minimal scaffolding to make evaluation run "
        ...
        "IMPORTANT: Baseline creates MINIMAL scaffolding only - NOT the full solution. Full solution comes at Refinement. Baseline scaffolding is just enough for evaluation to run. ADVERSARIAL = minimal scaffolding to make evaluation run, nothing more. "
