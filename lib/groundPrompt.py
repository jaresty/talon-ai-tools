...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "P5 (Gap-driven iteration): gaps are exposed before claiming completion; refinement addresses specific gaps. "
        ...
        "[ ] Baseline: derive from P2,P3 — why baseline creates scaffold not complete solution; what GOES WRONG if you produce full solution at baseline: no gap to expose in challenge rung, evaluation criteria become vacuous; RULE: baseline = minimal artifact that demonstrates approach, NOT complete solution "
        ...
        "4. Baseline — Does a minimal version exist that demonstrates the approach? Derivation: from structure, derive minimal artifact. Evidence: minimal viable example that satisfies structure. "
        "IMPORTANT: Evaluation criteria must be APPLIED to baseline BEFORE refinement — external verification proves gaps exist. Criteria that pass against baseline before refinement proves nothing (Texas sharpshooter fallacy). "
        "5. Challenge — What evidence proves gaps exist? Derivation: from baseline, produce evidence that baseline does not satisfy criteria. Evidence: external verification showing gap exists (not just assertion). "
        "6. Refinement — How is the artifact improved? Derivation: from challenge, derive targeted changes. Evidence: addresses specific gaps. "
        ...
        "Domain adaptation: for tasks where verification is possible, evaluation criteria must be APPLIED to baseline BEFORE producing the complete solution — external verification proves gaps exist. Criteria that pass against baseline before refinement proves nothing. "
        "Challenge rung MUST produce external evidence showing gaps exist. "
        "Baseline creates scaffold; Challenge applies criteria to produce gap evidence; Refinement produces complete solution that satisfies criteria. "
