...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Challenge: derive from P3,P5 — Challenge derives evaluation from Structure to expose gaps AND independently test continued alignment to declared intent; RULE: Challenge = instantiate evaluation before refinement. Challenge must PRODUCE the actual evaluative artifact itself (test, command, query, rubric application, comparison, measurement, or equivalent reproducible proof), not merely describe what evaluation would occur. A gap exists only if the produced evaluation artifact demonstrates failure or absence in observable form. Without a produced evaluation artifact, no gap is established."
        ...
        "Challenge produces the evaluative artifact that demonstrates the gap; Refinement adds only enough to make that already-produced artifact pass without introducing new drift. "
