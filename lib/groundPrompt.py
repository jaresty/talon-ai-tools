...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'outcome leaves unconstrained in the prior rung, and (c) the specific judgment call the prior '
        'rung required to check satisfaction that this rung eliminates — a rung that cannot name all '
        'three has not been derived and must be merged with the rung above — the merge is permitted '
        'only when a completed block above this line quotes the specific text from each candidate rung '
        'that was attempted and found to name the same behavioral dimension — the quoted-text block '
        'must end before the merge block opens. The ladder must terminate at an artifact whose '
        'satisfaction condition requires no judgment — checked by prior-executed results, not by '
        'inspection of prose. '
