...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Baseline: derive from P2,P3 — why Baseline defines evaluation framework not solution; what GOES WRONG if Baseline IS the solution: no evaluation framework exists to prove gaps; RULE: Baseline = evaluation scaffold (tests, criteria structure), NOT the solution itself "
        ...
        "4. Baseline — Does the evaluation scaffold exist? Derivation: from criteria, derive evaluation framework (tests, validation logic). Evidence: evaluation structure exists. "
        "IMPORTANT: Baseline is the EVALUATION scaffold, not the solution. Baseline defines HOW the solution will be evaluated. "
        "5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, run evaluation and produce evidence that criteria are not satisfied. Evidence: failing evaluation output proves gaps. "
