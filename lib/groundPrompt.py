...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test file with test function names that describe expected behavior AND include actual assertions that will fail; these assertions define the intent that implementation must satisfy. Create STUB implementation files so tests can RUN (imports resolve) but assertions FAIL because implementation is stubbed. Tests must PASS at validation rung (no failing assertions) — only infrastructure should be resolved. The assertions in validation rung tests are FIXED intent declarations — they cannot be changed in later rungs; implementation must make them green, not make tests match implementation. "
