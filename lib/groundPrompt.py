...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Ladder derived MUST be emitted as the exact sentinel: 🔵 Ladder derived — and MUST be emitted AFTER producing the rung table — emitting before rung table is complete voids the session; "
        ...
        "No implementation is permitted before validation shows red assertions — the exec_observed sentinel must appear with verbatim test output showing failure BEFORE any implementation code is written; the gap is the red assertion, the solution is making it green; "
