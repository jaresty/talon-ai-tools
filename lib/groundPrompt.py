...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "- Challenge mechanism — instantiate mechanism that detects gaps, APPLY it, EXECUTE IT, SHOW ACTUAL FAILURE OUTPUT; the challenge is NOT analysis or conjecture — it MUST create an EXECUTABLE ARTIFACT (test file, script, runnable check) that when EXECUTED produces evidence of the gap; running grep, reading files, or describing what might be wrong is NOT sufficient; ALWAYS START WITH A SKELETON — write minimal scaffolding that asserts one thing, run it, verify it fails, then elaborate; WHAT GOES WRONG: diagnostic commands replace actual tests, over-engineered tests solve too much; RULE: write the test, run it, show the failing output "
        "- Single gap rule — one observed failure as active gap; RULE: one gap per refinement "
        "- Minimal refinement — smallest observable behavior that exposes the next failure in the sequence; ALWAYS START AS SKELETON — implementation should be minimal scaffolding sufficient to reveal the next failure, not a complete solution; do NOT try to close the gap entirely — only produce an artifact sufficient to see the next failure; WHAT GOES WRONG: over-engineering, solving too much at once; RULE: minimal skeleton to reveal next failure point "
        ...
        "- Enforcement validity — every guard must fail visibly before pass; approach implementation adversarially — your job is to ENSURE the test fails first; seeing each assertion fail is essential to trust in the process; WHAT GOES WRONG: tests that pass on first run indicate weak assertions, not robust implementation; RULE: run test, verify it FAILS, then implement to make it pass; passing tests without seeing failure erodes confidence in the test suite "
