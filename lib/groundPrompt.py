...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'Before acting, produce a goal-search artifact: a numbered list of every goal-level statement '
        'in the transcript above the current point, each with its transcript location quoted verbatim. '
        'A goal-level statement is any statement that describes a desired end state rather than a specific '
        'action to take. Mark the highest-level goal statement in the list. '
        'The goal-search artifact is the only valid basis for the anchoring citation that follows. '
        'After producing the goal-search artifact, produce a one-line anchoring citation: '
        '"Intent artifact anchored to: [transcript location] — [quoted text]." '
        'The anchoring citation must name a location that appears in the goal-search artifact\'s list. '
        'A citation whose listed location was produced in the same generative act as the citation has not been derived. '
        'If no higher-level goal statement exists, the goal-search artifact must name the specific '
        'transcript boundary above which the search found none. '
        'The intent artifact must be derived from the highest-level goal statement identified in the '
        'goal-search artifact, not from the immediate sub-task description. A valid intent artifact states '
        'each behavioral dimension required for that goal to be satisfied — not only the dimensions visible '
        'in the current sub-task. '
        ...
        'The completion check is the final required step of any enforcement process and its output is a '
        'required closing artifact. A response is not permitted to end until the completion check artifact '
        'exists in the transcript. The completion check artifact is valid only when it names each item in '
        'the intent artifact by reference, and for each item: (a) the named covering artifact is a '
        'prior-executed result already present in the transcript — not a description of what an artifact '
        'would do, but a record of what it did — and the entry cites the specific outcome in that '
        'execution result that was observed to be different when the behavioral dimension was absent, '
        'named precisely enough that a reader could locate it in the transcript by inspecting the '
        'execution result; a covering claim that predicts what an artifact would do if the dimension '
        'were absent, rather than citing an observed failure when it was absent, has not been derived; '
        'and (b) where no execution result showing the dimension absent exists in the transcript, '
        'the entry records that dimension as uncovered — a passing result alone does not constitute '
        'coverage, because it does not demonstrate the artifact fires when the dimension is absent. '
        'A closing statement, summary, or synthesis that appears before the completion check artifact is '
        'present has no authorization to proceed. The completion check artifact is not satisfied by a '
        'description of having run it — only by the enumerated output above appearing in the transcript. '
