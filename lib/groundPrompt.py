...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "  Line 2: <transition> — must immediately follow Active:; nothing may appear between them  "
        "  Line 3: Pending: <id:type list, or 'empty'>  "
        "Any text inside a state other than these three lines is a syntax error. "
        "Any text between states (between Pending: and the next Active:) is a syntax error. "
        "Nodes are opaque: during planning, nodes have no descriptions, claims, meanings, attributes, or annotations. "
        "Only node identifiers may appear in the trace. "
        ...
        "Transitions: "
        "Interpret(R:Request): write 'Interpret: R → IP0'; add IP0:InterpretationPair. "
        "Expand(IP:InterpretationPair): write 'Expand: IP → I1 / I2'; add I1:Interpretation, I2:Interpretation. "
        "Select(I:Interpretation): write 'Select: I → S'; add S:Selected. "
        "  (Select may also write 'Discard: <sibling> → X' and add X:Discarded in the same transition line, "
        "   because the sibling's fate is determined by selection — this is the only permitted two-output exception.) "
        "Consume(X:Discarded): write 'Consume: X'; add nothing. "
        "GroundCheck(S:Selected): write 'GroundCheck: S → C'; add C:AmbiguityCheck. "
        "GroundResolve(C:AmbiguityCheck): if ambiguity remains, write 'GroundResolve: C → R\\'' and add R\\':Request; "
        "otherwise write 'GroundResolve: C → F' and add F:FormalismChoice. "
        "Formalize(F:FormalismChoice): write 'Formalism: <language>' on one line "
        "(first-order predicate logic | set-builder notation | lambda calculus | typed function signatures); "
        "write 'Formalize: F → U' where U is the formal expression — no natural language; add U:UnambiguousRequest. "
        "SplitCheck(N:UnambiguousRequest or Property): write 'SplitCheck: N → D'; add D:SplitDecision. "
        "AnalyzeSplit(D:SplitDecision): parse D's source expression as an abstract syntax tree; "
        "if the AST root has two child expressions E1 and E2, "
        "write 'AnalyzeSplit: D → SC1 / SC2' then 'SC1.expression=<E1>' then 'SC2.expression=<E2>' — "
        "E1 and E2 must be contiguous substrings of D's source formal text, not descriptions or steps; "
        "add SC1:SplitCandidate, SC2:SplitCandidate. "
        "Otherwise write 'AnalyzeSplit: D → terminal'; add nothing. "
        "Materialize(SC:SplitCandidate): write 'Materialize: SC → P'; add P:Property. "
        "P is a pointer to SC — no second line, no P.source line, no content written for P during planning. "
        "When Pending becomes empty: add PC:PlanningComplete. "
        "PlanningComplete: write 'Planning complete.'; add nothing. "
        "Task content and explanation of properties begin only after 'Planning complete.' "

        "Validity: every identifier introduced exactly once; "
        "every Active: names a node currently in Pending; "
        "every state is exactly three lines (Active: / transition / Pending:); "
        "'Planning complete.' emitted only by PlanningComplete; "
        "SC1.expression and SC2.expression are contiguous substrings of D's source formal text."
    )
