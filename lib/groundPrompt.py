...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "the P4 EV step (1) pre-existence or pre-failure check must have produced a tool-executed result "
        "in the transcript \u2014 asserting pre-existence without a tool call result does not satisfy this gate; "
        ...
        "a harness error at VRO caused by a missing implementation file means the EI artifact does not yet exist \u2014 "
        "HARD STOP is not valid; the only valid next action is to proceed to the EI rung and create the file; "
        "a harness error at VRO caused by a test file error (syntax, import of a non-existent component) "
        "requires fixing the test file at the EV rung \u2014 not HARD STOP; "
        ...
        "no content, confirmation, or pause between them is valid. "
        ...
        "stopping mid-descent is a protocol violation regardless of response length. "
