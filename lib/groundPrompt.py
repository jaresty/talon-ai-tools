...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'would do, but a record of what it did — and the entry states the specific outcome in that '
        'execution result that would be different if the behavioral dimension were absent, named precisely '
        'enough that a reader could locate it in the transcript by inspecting the execution result; a '
        'covering claim that names an artifact without citing a specific outcome in an execution result '
        'of that artifact has not been derived; and (b) where no execution result exists for a given '
        'dimension, the entry records that dimension as uncovered — asserting coverage from an artifact '
        'that has not been executed is not permitted. '
