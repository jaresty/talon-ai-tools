...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior; create STUB implementation files for EVERY component planned (empty functions returning null) — this creates all files needed for the entire feature, not just some; no expect() assertions, no mock setup, no render; tests are EMPTY and pass trivially because they have no code; this rung establishes that tests can run and all components have stubs. Assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung — FAILING tests means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. "
        "Assertion rung: EDIT existing test file to add ONE specific expect() assertion; run tests to see it FAIL (red). AFTER making assertion green (implementation succeeded), CHECK if gap is closed — if gap still exists, add another assertion; if gap is closed, emit completion sentinel and move to next gap. NO file creation in assertion rung — stubs created at validation rung; only edit existing test file to add assertions. "
        ...
        "Every file change requires justification: AFTER each file edit, do NOTHING else until you have provided justification IN THE CONVERSATION; write NO code, make NO tool calls, issue NO commands until you have explained in the chat why this change satisfies the current rung requirements; if you cannot justify why it satisfies the rung, ITERATE until it does; file changes without justification are protocol violations. JUSTIFICATION MUST reference the specific rung requirements being satisfied — vague explanations are insufficient. "
        "Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO file creation in implementation rung, only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, return UP to find next gap (ascend ladder); if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. "
