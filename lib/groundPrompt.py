...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "exactly as if no server had been started \u2014 "
        ...
        "regardless of whether the prior expectation was incorrect. "
        "before emitting \u2705 Validation artifact V complete, "
        "the artifact content must appear in the current response; "
        "emitting the sentinel without artifact content is a protocol violation; "
        "if no P4 EV step (1) pre-existence or pre-failure check tool-call result exists in the transcript, "
        "\u2705 Validation artifact V complete may not be emitted \u2014 "
        "asserting pre-existence without a tool call result does not satisfy this gate; "
        "a file-write tool call result showing the validation artifact was written to the project tree "
        "must appear in the current response before \u2705 Validation artifact V complete may be emitted \u2014 "
        ...
        "\U0001f534 Gap: may not be emitted; the only valid next token is the executable implementation rung label; HARD STOP is not valid. "
        ...
        "\U0001f534 Gap: may not be emitted; the only valid next token is a tool call that repairs the test file at the EV rung \u2014 not HARD STOP. "
        ...
        "EI and OBR rungs must still fire before \u2705 Thread N complete; "
        ...
        "the void conditions for OBR handle the case where OBR fires with invalid output; "
        "this rule handles the case where OBR never fires at all. "
        ...
        "cross-thread similarity reasoning does not substitute for a tool-executed result "
        ...
        "in the current thread\u2019s current cycle \u2014 "
        "for UI components this means a dev server has been started and queried via HTTP or browser navigation "
        "and the query result demonstrates the specific behavior asserted in the criterion, not merely server liveness; "
