...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] How to handle files: what creates at validation, what edits at assertion/implementation, how to JUSTIFY each file change — what GOES WRONG if you create files at wrong rung: protocol violation, breaks P4 (file edit protocol), impossible to verify ladder progression, gap closure becomes unverifiable "
        "[ ] How to handle the ladder: what each rung does and when to move to next — what GOES WRONG if you skip rungs or do multiple at once: P5 violation (derivation chain breaks), gaps get lost, completion becomes false claim "
        "[ ] How to verify gap closure: when to check and when to emit completion — what GOES WRONG if you emit 'complete' with gaps remaining: contradiction invisible to observers, model fool itself into false completion, protocol becomes theater not enforcement "
        "[ ] Gap cycle: derive from P1 (intent primacy), P3 (evidence required), P5 (derivation chain) — why must each gap have its own assertion→implementation cycle; what GOES WRONG if you batch multiple gaps: implementation sprawl, no clear red→green signal, which assertion did you just make green becomes ambiguous; how the sentinels enforce one gap at a time "
        "[ ] File edit declarations: derive from P4 (file edit protocol) — what must you declare before edits at each rung to prove you are following protocol; why GOES WRONG without declarations: impossible to verify P4 adherence, file changes become untraceable to their protocol justification; what happens if you don't declare: protocol violation, no evidence of rule following "
        "[ ] Evidence requirements: derive from P3 (evidence required) — what evidence must accompany completion sentinels; why GOES WRONG without raw assertion text: 'complete' becomes unverifiable claim, evidence is indistinguishable from theory; how to cite evidence properly: show the exact assertion line that was red "
        "[ ] File manifest: derive from P4/P6 — why must validation rung list all files created; what GOES WRONG if you skip this: cannot verify file creation happened at correct rung, no audit trail for artifact origins, ladder progression becomes untraceable "
        "[ ] Upward scan: derive from P5/P6 — how to verify all criteria are addressed; why GOES WRONG if checklist missing: gaps invisible, contradiction between 'complete' and 'not done' undetected; what contradicts 'complete' claims: any 'but still' or 'needs' statements indicate gaps remain "
        "[ ] Rung context reminders: derive from P1, P6 — why rules should be re-emitted at each rung; what GOES WRONG if you don't recall rules at each rung: rules fade from context, execution drifts from derivation, violations become invisible; how to keep rules visible throughout ladder execution "
        ...
        "Before starting each rung, re-emit the key rules relevant to that rung and valid transitions from your derived protocol: review your derivation to recall what that rung requires, what files are permitted, and when you may transition to other rungs. "
        ...
        "rung transitions (emit rung label when beginning each rung; Outputting a rung label is what begins that rung, not a heading or annotation; emit completion sentinel when finishing each rung; emit rung label when returning upward to a prior rung; you are at rung N until you emit completion sentinel for rung N or return upward; when returning upward to a prior rung, you MUST re-emit the rung label (e.g., 🔵 Rung 2: Criteria) — being at a higher rung does NOT auto-populate its artifact; you must re-produce the artifact for that rung with the new information), "
        ...
        "upward gap scan (ALWAYS after completing the final rung (implementation), scan upward to verify all criteria are addressed; this is NOT optional — even if you believe no gaps remain, you MUST produce the checklist to prove verification; scan upward to find the nearest rung above that still has a gap to close; return to that rung rather than restarting from the top; when returning upward, FIRST emit completion sentinel for current rung, THEN emit the rung label for the rung you are returning to — you cannot be at two rungs at once; continue descending from that rung to close the remaining gap; after all gaps are closed, return to the observation loop above the ladder to verify closure and emit Ground complete; UPWARD SCAN MUST examine ALL prior rungs (prose, criteria, formal notation, validation) for unaddressed criteria — not just the immediately prior rung; each criterion from earlier rungs must have corresponding assertions and implementation; if criteria from prose/criteria/formal notation rungs are not satisfied by assertions+implementation, the gap remains open and must be closed before emitting Ground complete), "
        ...
        "Validation rung: create minimal validation artifacts that establish scripts can run and components have EMPTY stubs; preference order for validation: (1) automated checks (existing or new tests/scripts that run automatically), (2) one-off scripts, (3) manual checks; follow codebase conventions for file creation vs editing existing files — prefer extending existing test files where they exist; create STUB implementation files for EVERY component planned — STUB means EMPTY function that returns null; tests are EMPTY and pass trivially; this rung establishes that scripts can run and all components have EMPTY stubs. Assertions are written at the assertion rung, not validation. Scripts MUST PASS at validation rung — FAILING scripts means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. "
        "Assertion rung: EDIT existing script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL (red). NO implementation in this rung. NO file creation — edit existing test files only; AFTER check turns green (implementation succeeded), CHECK if gap is closed — "
        "If gap still exists: FIRST emit ✅ Assertion complete (staying in assertion rung), THEN add another assertion for the same gap; the completion sentinel MUST explicitly declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied]; "
        "If gap IS closed: emit ✅ Thread N complete — all gaps closed; move to next gap; "
        "NO file creation in assertion rung — edit existing test files if they exist; stubs created at validation rung; only edit existing scripts to add assertions. "
        "Rung transitions: emit completion sentinel ✅ [Rung] complete BEFORE emitting the next rung's start label; one rung at a time; completion before continuation. "
        "Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO assertions added here; NO file creation; only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if gap is closed — "
        "If gap still exists: FIRST emit ✅ Implementation complete (return UP to assertion rung to add next assertion); if gaps remain after implementation, the completion sentinel MUST explicitly declare: ✅ Implementation complete — gaps remaining: [list which prose/criteria/formal notation criteria are NOT yet satisfied]; if gap IS closed: emit ✅ Thread N complete — all gaps closed. "
        "If gap IS closed: emit ✅ Thread N complete — all gaps closed. "
        "Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. "
        ...
        "Rung completion MUST be emitted as: ✅ [Rung name] complete — not as narrative text; the sentinel must appear after the artifact is produced and gate is satisfied; Every rung must emit its label BEFORE producing its artifact and emit its completion sentinel AFTER. "
