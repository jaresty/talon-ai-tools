...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'between the cited floor artifact and rung 1 that is absent from the ladder must either '
        'be covered by a prior artifact already present in the transcript above the ladder — cited '
        'by location — or added as a rung. An intermediate level may not be skipped by assertion '
        'or open-ended justification — only by citation of a specific existing transcript artifact '
        'that provides that level\'s judgment reduction. The rules-out '
