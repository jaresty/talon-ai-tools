...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Baseline: derive from P2,P3 — Baseline defines evaluation structure not solution; RULE: Baseline = evaluation structure functions (runs without error), not feature complete; Prove why each element is necessary "
        ...
        "IMPORTANT: Baseline PASS means evaluation structure FUNCTIONS, not that feature is complete. Baseline passes if evaluation runs without errors - nothing more. Solution comes later through iteration. "
        "IMPORTANT: Baseline creates MINIMAL scaffolding only. If you add more than minimal, prove WHY you cannot make it smaller: would removing this break evaluation? Would it make evaluation structure unfaithful to criteria? If no, remove it. "
