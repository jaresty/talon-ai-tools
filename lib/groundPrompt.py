...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Challenge: derive from P3,P5 — Challenge derives evaluation from Structure to expose gaps AND independently test continued alignment to declared intent; RULE: Challenge = apply evaluation showing (1) the smallest observable gap exists and (2) the current artifact still matches intent. IMPORTANT: A gap indicates something is MISSING - could be a whole thing, structure, or behavior. It does NOT imply creating the complete thing - just addressing what is missing. Challenge must produce MECHANICAL VERIFICATION for both: show the specific test/command/query/evidence that proves the gap exists, AND show the specific reproducible proof that the current artifact still aligns to intent. Without both, closure cannot be confirmed later."
        ...
        "Challenge exposes gaps and tests intent alignment; Refinement adds only enough to satisfy each proven gap without breaking alignment; Verification confirms both gap closure and preserved intent. "
