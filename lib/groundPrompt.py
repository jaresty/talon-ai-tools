...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: create minimal validation artifacts that establish scripts can run and components have EMPTY stubs; preference order for validation: (1) automated checks (existing or new tests/scripts that run automatically), (2) one-off scripts, (3) manual checks; follow codebase conventions for file creation vs editing existing files — prefer extending existing test files where they exist; create STUB implementation files for EVERY component planned — STUB means EMPTY function that returns null; tests are EMPTY and pass trivially; this rung establishes that scripts can run and all components have EMPTY stubs. Assertions are written at the assertion rung, not validation. Scripts MUST PASS at validation rung — FAILING scripts means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. At validation rung, you MUST explicitly list all files being created in the conversation — this is your file manifest. Every implementation stub created at validation rung MUST have a corresponding test file created at validation rung — the test file validates that implementation; list both implementation files AND their test files in the validation file manifest. "
        "Assertion rung: EDIT existing script to add ONE specific check (expect() assertion or shell script check); run to see it FAIL (red). NO implementation in this rung. NO file creation — edit existing test files only; before editing, declare 'no new files' as verification that no new files are being created; AFTER check turns green (implementation succeeded), CHECK if gap is closed — "
        "If gap still exists: FIRST emit ✅ Assertion complete (staying in assertion rung), THEN add another assertion for the same gap; the completion sentinel MUST explicitly declare gap status: ✅ Assertion complete — gaps remaining: [none/which criteria still unsatisfied] — evidence: [for each unsatisfied criterion, cite the RAW failing assertion text]; "
        ...
        "Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO assertions added here; NO file creation; only edit existing stubs; before editing, declare 'no new files' as verification that no new files are being created; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if gap is closed — "
        "If gap still exists: FIRST emit ✅ Implementation complete (return UP to assertion rung to add next assertion); if gaps remain after implementation, the completion sentinel MUST explicitly declare: ✅ Implementation complete — gaps remaining: [list which prose/criteria/formal notation criteria are NOT yet satisfied] — evidence of gap verification: [reference the specific criteria checked, or if gaps listed, explain why return upward is needed]; if gap IS closed: emit ✅ Thread N complete — all gaps closed. "
        "Implementation gap cycle: for EACH gap, you MUST follow this sequence: (1) assertion rung: add failing assertion for the specific gap; (2) implementation rung: implement ONLY enough to make that specific assertion green; (3) verify assertion is green; (4) emit ✅ Implementation complete; (5) if OTHER gaps remain, return to assertion rung (NOT implementation) to add next failing assertion; (6) repeat until no gaps remain. NEVER implement multiple gaps in sequence without returning to assertion between them — each gap requires its own assertion→implementation cycle; batching implementations violates protocol. The sentinel 'Implementation complete' means the implementation for the CURRENT gap is done, not that the entire task is done. "
