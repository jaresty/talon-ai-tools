...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior — this documents the intent without writing assertions; no expect() assertions, no mock setup, no render; stub implementation files return null; tests are EMPTY and pass trivially because they have no code; this rung establishes only that tests can be imported and run without errors. Assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung to move to assertion rung. "
        "Assertion rung: write ONE specific assertion using expect(), run tests to see it FAIL (red). NO implementation in assertion rung — only write assertion, run tests, observe failure. Then move to implementation rung. "
