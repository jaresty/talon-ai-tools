...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Before starting each rung, re-emit the key rules relevant to that rung and valid transitions: at validation: only create files here, list file manifest, create test files + implementation stubs; at assertion: edit only existing test files, add failing assertions one at a time, NO new files, NO implementation; at implementation: edit only existing stub files, NO new files, NO assertions, implement only enough for current assertion; valid transitions: proceed downward only after completing current rung, return upward when gaps found in prior rungs, one rung at a time. "
