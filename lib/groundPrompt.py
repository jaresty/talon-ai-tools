...
GROUND_PARTS: dict[str, str] = {
    "derivation_structure": (
        ...
        "before advancing from formal notation, enumerate each criterion from the criteria rung "
        "and confirm it is encoded as a behavioral invariant \u2014 "
        ...
        "when I cannot be declared from context alone, observation of existing code or running "
        "behavior sufficient to establish I is required before the manifest \u2014 "
        "this is I-formation, not rung work; skipping I-formation when context is insufficient "
        "is an error, not a permitted shortcut \u2014 it produces a specification derived from "
        "assumed rather than actual constraints, and every downstream artifact will be wrong in "
        "ways that only surface at executable validation or observed running behavior, by which "
        "point multiple rungs must be discarded and rederived; "
        "when I-formation is required, output \u2018\u2705 I-formation complete\u2019 on its own line "
        "after declaring I and before the rung manifest \u2014 "
        "the manifest may not appear before this sentinel; "
        "exploration beyond what is needed to declare I "
        ...
        "a rung label during execution must be emitted at every rung transition; "
        "absence of a rung label at a transition is a traversal violation, not a stylistic omission; "
        "the label marks the point where the artifact begins \u2014 "
        ...
        "a passing run is a gap signal \u2014 see gate_validity) \u2192 "
    ...
    "gate_validity": (
        ...
        "The validation run observation rung is only satisfied by an observed failure that reveals "
        "the declared gap \u2014 a passing run before gap-targeted implementation has been written "
        "is a gap signal, not permission to descend: either the behavior is already implemented "
        "(verify this; the thread may be complete) or the validation artifact is vacuous with "
        "respect to the declared gap and must be corrected before descent continues. "
        ...
        "on its own line immediately after the validation run observation rung has declared a gap "
        "and before the executable implementation rung begins \u2014 "
    ...
    "derivation_discipline": (
        ...
        "that will resurface at each subsequent rung \u2014 correction is not a detour but the only path to a "
        "valid artifact at the current rung; "
