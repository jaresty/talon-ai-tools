...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        '(2c) derive formalized properties: after writing \'§ formalization complete\', immediately write \'## Formalized properties\' — '
        '\'## Formalized properties\' must not appear before \'§ formalization complete\' in the transcript; '
        'decompose each §2 dimension into its atomic formal claims, one claim per line prefixed \'property [N]:\' where N is a 1-based index; '
        ...
        'write \'§ properties complete\' after all properties and atomicity checks appear; '
        ...
        'when every covered dimension cites such a substring, '
        'before writing \'§4 coverage verified\', count the number of distinct \'Falsify '
        'derivation:\' blocks that appear in the transcript above \'## Completion check\'; '
        'if at least one such block exists, write \'§ test suite complete: [N] derivation(s) '
        'verified\' where [N] is that count; if no such block exists, write \'§ test suite '
        'complete: (none)\'; \'§4 coverage verified\' must not appear before \'§ test suite '
