...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "P5 (Automation quality verified): Automation must fail before passing; the failure output is part of the verification artifact, not a separate rung; a reviewer cannot determine from the test file alone what the implementation must do \u2014 the failure output is required. "
        "P6 (Artifact type discipline): One rung per type; text rungs produce no files; frozen artifacts may not be modified at subsequent rungs. "
        "An executable validation artifact is a file whose sole purpose is to assert behavioral properties of another artifact \u2014 it contains no behavior of its own; validation files may not be imported by implementation files. "
        ...
        "P11 (Immediate lowest-rung observation): Observe at lowest feasible rung; deferring is a protocol violation. "
