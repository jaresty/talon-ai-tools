...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'a necessary condition for achieving G1. Cite the goal no other goal subsumes as the '
        'anchoring citation, quoting its text and location. If multiple unsubsumed goals exist, '
        'cite all of them. If no goal statement exists above the current point, state the transcript '
        'boundary searched. '
        ...
        'derived and must be merged with the rung above. The ladder must terminate at an artifact '
        ...
        'for completion. '
        'The response is not permitted to end until the completion check artifact exists in the '
        'transcript: for each item in the intent artifact, it cites the specific outcome in a '
        'prior execution result that was observed to differ when the behavioral dimension was absent — '
        'a prediction of what would differ does not satisfy this; where no such result exists, '
