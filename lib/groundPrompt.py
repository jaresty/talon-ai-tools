...
GROUND_PARTS: dict[str, str] = {
    "derivation_structure": (
        ...
        "observed running behavior must be traceable to the behavioral gap declared in the validation run observation \u2014 "
        "verifying that a process started or a URL responded is not sufficient; "
        "for UI artifacts, a server response does not confirm rendering; "
        "for APIs, an HTTP 200 does not confirm correct data; "
        "the observation must demonstrate the declared behavioral intent was satisfied. "
    ...
    "gate_validity": (
        ...
        "Output \u2018\u2705 Validation artifact V complete\u2019 on its own line "
        "immediately after the validation run observation rung has declared a gap and before the executable implementation rung begins \u2014 "
        "this sentinel may not appear after tests pass, after implementation, or after observed running behavior; "
