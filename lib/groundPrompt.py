...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write executable script (test file or shell script) with test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior; if existing test files exist, add to those; create STUB implementation files for EVERY component planned — STUB means EMPTY function that returns null; tests are EMPTY and pass trivially; this rung establishes that scripts can run and all components have EMPTY stubs. Assertions are written at the assertion rung, not validation. Scripts MUST PASS at validation rung — FAILING scripts means NOT DONE; fix issues at validation rung BEFORE moving to assertion rung. "
