...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test file with test function names that describe expected behavior; create STUB implementation files so tests can RUN (imports resolve); include COMMENTS or formal notation describing the assertions that will be tested — do NOT include actual expect() assertions at this rung; assertions are written at the assertion rung, not validation. Tests should PASS at validation rung because implementations are stubs. "
