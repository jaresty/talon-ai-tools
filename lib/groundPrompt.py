...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'When the task touches multiple layers or systems, the derivation must identify a governing artifact per layer — '
        'declaring one layer\'s verification tool as the governing artifact while modifying another layer is a structural '
        'cheap path that silently excludes that layer from coverage and must be explicitly named and closed. '
        'The completion check is the final required step of this process and must be included in the derivation: '
        'return to the original stated intent, and for each item, produce visible evidence that the behavior '
        'satisfies it — an assertion is not evidence, and a criterion that has only ever passed '
        'provides no coverage guarantee; only a criterion that has been verified to fail when '
        'the behavior is absent constitutes evidence. '
