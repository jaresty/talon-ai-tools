...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "Pending is an ordered FIFO sequence, not a set. "
        "The leftmost identifier in the Pending line is the queue head. "
        "The next Active line MUST name exactly the leftmost identifier from the previous Pending line — "
        "naming any other identifier is a queue violation and a syntax error. "
        "The Pending line is authoritative: it defines the only legal next transition. "
        "Removing any identifier other than the leftmost Pending identifier is a queue violation. "
        "A parent transition may not depend on its descendants being processed immediately; "
        "sibling nodes will be dequeued before children (breadth-first). "

        "Trace grammar — every state has exactly three lines: "
        "  Line 1: Active: <id>:<type>  "
        "  Line 2: <transition line> — must immediately follow Active:; nothing may appear between them  "
        "  Line 3: Pending: <id:type list in exact dequeue order, or 'empty'>  "
        "Any text inside a state other than these three lines is a syntax error. "
        "Any text between states (between a Pending: line and the next Active: line) is a syntax error. "
        "Nodes are opaque: during planning, nodes have no descriptions, claims, meanings, attributes, or annotations. "
        "Only node identifiers, transition keywords, and the two permitted payload lines (U.expression= and SC.expression=) may appear in the trace. "
        "A node cannot be explained or described until after 'Planning complete.' "
        "A newly created node is inert until it appears in a future Active: line. "
        ...
        "When Pending is empty after completing a Terminal or Consume transition: "
        "this is the one permitted meta-transition outside the node dispatch table — "
        "enqueue PC:PlanningComplete. "
        "PlanningComplete: write 'Planning complete.'; enqueue nothing. Task content begins after. "

        "Validity: every identifier introduced exactly once; "
        "every Active: names the head of Pending; "
        "every state is exactly Active:/transition/Pending: (plus U.expression= or SC.expression= payload lines where specified); "
