...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        '(4) verify coverage: \'## Completion check\' must appear as the next heading after \'§5 enumeration complete\' in the transcript — the first \'## \' heading line appearing after \'§5 enumeration complete\' must be \'## Completion check\'; a \'## Completion check\' appearing anywhere before \'§5 enumeration complete\' does not satisfy ground and must not be written — if the response has produced §1–§3 content and wishes to summarize or verify coverage, it must instead continue to \'## Enforcement sequence\' and \'## Path enumeration\' first; '
