...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "Decision nodes (AmbiguityCheck, SplitDecision) never create task-domain nodes directly — "
        "only non-decision transitions may create Request, UnambiguousRequest, SplitCandidate, or Property nodes. "
        ...
        "SplitCheck(UnambiguousRequest U or Property P) [→1]: write 'SplitCheck: <id> → D' "
        "where D records its source as the consumed node; add D (SplitDecision) to Pending. "
        "Every UnambiguousRequest and every Property node must enter Pending and then be consumed by SplitCheck — "
        "there is no other valid transition for these node types; implicit closure, early termination, "
        "or any transition other than SplitCheck consuming a Property or UnambiguousRequest node is a validity violation. "
        "AnalyzeSplit(SplitDecision D) [→0 or 2]: examine the formal expression of D.source; "
        "if it contains two independent syntactic sub-expressions, "
        "write 'AnalyzeSplit: D → A / B' where A and B are the exact formal sub-expressions of D.source — "
        "A and B must be quoted from the formal notation, not described in prose; add A, B (SplitCandidate) to Pending; "
        "otherwise write 'AnalyzeSplit: D → terminal' — nothing is added to Pending, D.source is consumed. "
        "ResolveSplit(SplitCandidate A or B) [→1]: write 'Divide: <id> → P' "
        "where P captures the formal expression of its SplitCandidate source; add P (Property) to Pending. "
        "When Pending becomes empty after all Property nodes have been consumed by SplitCheck and reached terminal, "
        "add a single PlanningComplete node to Pending. "
        ...
        "A and B in AnalyzeSplit must be verbatim sub-expressions of D.source's formal notation — "
        "prose descriptions of what the sub-expressions mean do not satisfy this requirement."
    )
