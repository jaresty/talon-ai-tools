...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test function NAMES with embedded GIVEN-WHEN-THEN comments describing intended behavior; create STUB implementation files for EVERY component planned (empty functions returning null) — this creates all files needed for the entire feature, not just some; no expect() assertions, no mock setup, no render; tests are EMPTY and pass trivially because they have no code; this rung establishes that tests can run and all components have stubs. Assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung to move to assertion rung. "
