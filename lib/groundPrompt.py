...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test file with test function names that describe expected behavior; create STUB implementation files so tests can RUN (imports resolve) — NO implementation whatsoever at validation rung, just wiring (empty functions returning null); tests may RENDER and MOCK dependencies but must have NO actual expect() assertions — writing actual test assertions (expect() calls) at validation rung is a protocol violation; tests should be EMPTY skeletons that pass because they have no assertions. FIX mock issues at validation rung, don't skip them. Assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung to move to assertion rung. "
