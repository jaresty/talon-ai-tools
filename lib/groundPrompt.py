...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: create stub test structure that encodes intent from formal notation — write test file with test function names that describe expected behavior but the assertions are not yet implemented (placeholder assertions that will fail); create stub implementation files with function signatures but no implementation details; these stubs provide the structure that the assertion rung will exercise; "
        "Assertion rung is per assertion: each individual assertion is its own rung. Implement ONLY enough to make THAT specific assertion green. DO NOT change tests in implementation rung — tests define the intent, implementation must satisfy tests, not the other way around. "
