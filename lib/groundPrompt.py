...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'Prefer structural enforcement over procedural enforcement: a procedural constraint names what must happen '
        'and can be satisfied by describing it; a structural constraint routes a later step through the output of an '
        'earlier step so that skipping the earlier step makes the later step impossible or produces a detectable '
        'failure — not merely a violation of instructions. When designing constraints, ask: does this constraint '
        'make skipping it structurally impossible, or only prohibited? '
