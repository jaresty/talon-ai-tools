...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "if the \U0001F534 Execution observed: sentinel is not immediately followed by a triple-backtick delimited block "
        "containing verbatim tool output, the sentinel is void \u2014 "
        "emitting the sentinel with prose content or no delimited block is a protocol violation "
        "equivalent to not emitting the sentinel; fix by invoking the tool and re-emitting. "
        ...
        "intervening content converts the sentinel to retroactive \u2014 re-emit and re-run. "
        # Session persistence
        ...
        "before writing the test, check whether an existing test file covers the same module and add there if so; "
        "when the validation artifact is modified, invoke a tool call to read the current test file "
        "before emitting carry-forward rows \u2014 "
        ...
        "only a post-VRO Gap justifies HARD STOP; "
        ...
        "for a React or web UI component, the OBR invocation renders the component to DOM "
        "using the project\u2019s render utility (e.g., renderToStaticMarkup, or the testing "
        "framework\u2019s render function reading container.innerHTML) and emits the resulting "
        "HTML or text fragment showing browser-visible content \u2014 "
        "this is not a test assertion; it is direct DOM output "
        "showing the criterion\u2019s named content is present in the rendered tree; "
        "starting a dev server does not satisfy this rung \u2014 "
        "server process state is not rendered content; "
        ...
        "the provenance statement does not replace the tool call \u2014 "
        "it gates the tool call; the tool call must follow the provenance statement in the same response; "
