...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'admits two non-equivalent outputs, write \'alternative satisfier: [text]\' where [text] names '
        'a concrete thing that the expression would admit but that the governing goal would reject — '
        ...
        '\'§ formalization complete\' must not appear before \'alternative satisfier:\' followed by non-empty text '
        'in the transcript — a \'§ formalization complete\' with no preceding \'alternative satisfier:\' disclosure does not satisfy this requirement; '
        'write \'§ formalization complete\' only after both \'§ ambiguity test:\' and \'alternative satisfier:\' with content have appeared; '
        ...
        'where the integer is the 1-based index of the file-modifying tool call this sentinel governs '
        'must appear in the transcript after \'§5 enumeration complete\'; '
        'a sentinel line of the form \'§ implementation permitted [N]\' containing the literal bracket characters \'[\' and \']\' '
        'does not satisfy this requirement — only a decimal integer satisfies the index requirement; '
        'no file-modifying tool call may appear before the first such sentinel; '
        'the literal string \'§ implementation permitted\' followed by a space and a decimal integer '
        'with the integer matching the 1-based index of the governed tool call '
        'must appear immediately before the `(i)` line of the five-line block '
        'with no intervening blank lines or other content between the permit sentinel and the `(i)` line — '
        ...
        'Rung-completion sentinel finality: each of the strings \'§0 observed\', \'§1 goal derived\', '
        '\'§1a decomposed\', \'§1b candidates\', \'§2 dimensions closed\', \'§ formalization complete\', '
        '\'§4 coverage verified\', and \'§5 enumeration complete\' must appear as the final non-blank line '
        'of the turn in which it is written — no prose, heading, or non-sentinel content may follow a '
        'rung-completion sentinel in the same turn; a turn that contains a rung-completion sentinel followed '
        'by any non-blank content that is not itself a rung-completion sentinel or exit string does not satisfy ground. '
