...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "AmbiguityCheck | FormalismChoice | UnambiguousRequest | SplitDecision | SplitCandidate | Property | PlanningComplete. "
        ...
        "AnalyzeSplit(D:SplitDecision): parse the U.expression or SC.expression text of D's source as an AST; "
        "if the root has two child sub-expressions E1 and E2, "
        "write 'AnalyzeSplit: D → SC1 / SC2'; write 'SC1.expression=<E1 verbatim from source text>'; "
        "write 'SC2.expression=<E2 verbatim from source text>' — "
        "E1 and E2 must be contiguous substrings of the source expression text; "
        "semantic descriptions or implementation steps are syntax errors; "
        "enqueue SC1:SplitCandidate, SC2:SplitCandidate. "
        "Otherwise write 'AnalyzeSplit: D → terminal'; enqueue nothing. "
