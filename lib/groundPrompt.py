...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "Pending is a FIFO queue — nodes are processed strictly in enqueue order (breadth-first). "
        "The head of Pending is the only valid next Active node — processing any other node is a syntax error. "
        "A parent transition may not depend on its descendants being processed immediately; "
        "sibling nodes will be dequeued before children. "
        ...
        "  Line 3: Pending: <id:type list in queue order, or 'empty'>  "
        ...
        "enqueue S:Selected then X:Discarded. "
