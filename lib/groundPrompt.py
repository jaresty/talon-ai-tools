...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Assertion rung: EDIT existing test file to add ONE specific expect() assertion; run tests to see it FAIL (red). NO file creation in assertion rung — stubs were created at validation rung; only edit existing test file to add assertions. Then move to implementation rung. "
        ...
        "Implementation rung: AFTER assertion rung shows red, implement ONLY what's STRICTLY NEEDED to make that specific assertion green — do NOT build complete components or do more than necessary; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. "
