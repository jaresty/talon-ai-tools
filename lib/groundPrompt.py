...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Challenge: derive from P3,P5 — why Challenge MUST extend existing evaluation (not create new structures); what GOES WRONG if you create new evaluation: scope expansion; RULE: Challenge = add N new criteria to baseline that FAIL, show N failures "
        "[ ] Refinement: derive from P3,P5 — why Refinement addresses OBSERVED gaps only; what GOES WRONG if you modify evaluation logic: changes what Challenge proved; RULE: Refinement = for each failing criterion, add ONLY enough to satisfy that criterion "
        "[ ] Verification: derive from P1,P3 — why Verification uses SAME evaluation framework; what GOES WRONG if you add manual verification: undermines evidence authority; RULE: Verification = run full evaluation from baseline, evidence = all criteria satisfied "
        "[ ] Baseline: derive from P2,P3 — why Baseline MUST work (not just exist); what GOES WRONG if you rationalize failure: broken framework confused with gaps; RULE: Baseline = fix whatever makes evaluation fail, do not proceed until it passes "
        "[ ] Baseline scaffolding: derive from P2,P3 — why Baseline scaffolding is placeholder not solution; what GOES WRONG if scaffolding has functionality: nothing for Challenge to expose; RULE: Baseline scaffolding = empty shell, Refinement adds functionality "
        ...
        "IMPORTANT: Baseline establishes ALL evaluation mechanics AND may create minimal scaffolding to hold them. This scaffolding is NOT the solution - it's just enough to verify evaluation works. Baseline must PASS with its scaffolding. NO PROCEEDING without proof it works. "
        "IMPORTANT: Baseline creates MINIMAL scaffolding only - NOT the full solution. Full solution comes at Refinement. Baseline scaffolding is just enough for evaluation to run. "
        "IMPORTANT: The scaffolding at Baseline should be PLACEHOLDER-LEVEL - has the structure but not the functionality. Think empty shell, not working feature. "
