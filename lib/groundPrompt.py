...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Challenge: derive from P3,P5 — Challenge derives evaluation from Structure to expose gaps; RULE: Challenge = apply evaluation showing gaps exist. IMPORTANT: A gap indicates something is MISSING - could be a whole thing, structure, or behavior. It does NOT imply creating the complete thing - just addressing what is missing. Challenge must produce MECHANICAL VERIFICATION: show the specific test/command/query that proves the gap exists and would pass once closed. Without mechanical verification, you cannot confirm closure later. The gap shown at Challenge is what must be addressed at Refinement to be considered complete. IMPORTANT: MECHANICAL means any REPRODUCIBLE PROOF — not just code. For non-software tasks, show the specific evidence or demonstration that could be repeated to prove the gap exists."
