...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Assertion rung: EDIT existing test file to add ONE specific expect() assertion; run tests to see it FAIL (red). NO implementation in this rung. AFTER assertion turns green (implementation succeeded), CHECK if gap is closed — if gap still exists, FIRST emit completion sentinel, THEN add another assertion; if gap is closed, emit completion sentinel and move to next gap. NO file creation in assertion rung — stubs created at validation rung; only edit existing test file to add assertions. "
        ...
        "Implementation rung: AFTER assertion rung shows red, EDIT existing stub file to implement ONLY what's STRICTLY NEEDED to make that specific assertion green — NO assertions added here; NO file creation; only edit existing stubs; implement just enough to pass that one assertion; verify tests go green, then stop; AFTER implementation, EXPLAIN WHY this change is the minimum necessary to satisfy the gap; if you cannot explain why this is minimal, REDUCE the implementation to the minimum. AFTER completing implementation, CHECK if all gaps for this thread are closed — if gap still exists, FIRST emit completion sentinel, THEN return UP to assertion rung to add more assertions; if gap is closed, emit completion sentinel. Tests drive the behavior needed. Emit implementation rung label first, then implement, then emit completion sentinel. "
