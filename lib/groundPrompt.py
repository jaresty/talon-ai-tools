...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "before emitting \u2705 Validation artifact V complete, the artifact content must appear in the current response \u2014 "
        "emitting the sentinel without quoting the artifact in the transcript is a protocol violation; "
        "before emitting \u2705 Validation artifact V complete, confirm via tool call that the artifact path "
        "does not pre-exist or is already failing \u2014 if that tool call has not been made in the current response, "
        "the only valid next action is that tool call \u2014 "
        "V-complete may not be emitted without this tool-executed result in the transcript; "
        ...
        "a new criterion introduced by HARD STOP has no red VRO yet \u2014 "
        "EV and VRO must fire for the new criterion before impl_gate; "
        "skipping EV and VRO after a HARD STOP that introduces a new criterion is a protocol violation; "
        ...
        "a test runner invocation does not satisfy this gate \u2014 "
        "running tests produces validation-run-observation-type output, not observed-running-behavior-type output; "
        "the invocation target must be the implementation artifact itself, not a test harness wrapping it; "
        "if the most recent tool call at this rung is a test runner, \u2705 Thread N complete is blocked \u2014 "
        "the only valid next action is a live-process invocation of the implementation artifact; "
        "the live-process invocation must directly demonstrate every criterion declared for this thread, not only the primary one \u2014 "
        "a live-process invocation that demonstrates fewer than all declared criteria is a gap, not a completion; "
        "the test suite run required by condition (5) may follow the live-process invocation but may not precede or replace it; "
        ...
        "Before any tool call at this rung, emit a provenance statement naming the invocation target: "
        "\u2018Invoking [artifact] produced at [rung name] in this thread\u2019 \u2014 "
        "if the target is a test runner, stop: the invocation target must be the implementation artifact itself "
        "running as a live process, not a test harness; "
        ...
        "\u2705 Thread N complete immediately follows \U0001f534 Execution observed: at the OBR rung \u2014 "
        "not before it; emitting \u2705 Thread N complete before the OBR rung tool call and "
        "\U0001f534 Execution observed: sentinel exist in this cycle is a protocol violation. "
