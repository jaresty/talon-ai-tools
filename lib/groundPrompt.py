...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "To test atomicity, first verify that the property definition to be split satisfies the formality requirement. "
        "Emit: '§ formality check: property [N]: \"<verbatim definition>\" — [formal | informal: <reason>]'. "
        "If the check concludes informal, rewrite the definition as a typed mathematical expression, a logical predicate (∀, ∃, ⇒, ⇔), or a formally defined relation, "
        ...
        "The definition confirmed formal in the final check is the canonical definition for all subsequent steps — "
        "a provisional line or split test that quotes a definition not yet confirmed formal does not satisfy this protocol. "
        "Provisional candidate sub-properties may then be introduced immediately before the split test using: "
