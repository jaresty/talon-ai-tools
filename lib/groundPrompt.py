...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "[ ] Baseline: derive from P2,P3 — why evaluation scaffold must be produced and verified working before it can expose gaps; what GOES WRONG if scaffold doesn't exist or doesn't work: cannot prove gaps exist; RULE: Baseline = produce evaluation scaffold and verify it works "
        ...
        "4. Baseline — Does the evaluation scaffold exist? Derivation: from criteria, derive evaluation framework. Evidence: evaluation structure produced and verified to work. "
        "IMPORTANT: Evaluation scaffold must be PRODUCED and VERIFIED before it can expose gaps. A scaffold that doesn't exist or doesn't work cannot prove gaps. "
        "5. Challenge — What evidence proves gaps exist? Derivation: from Baseline, apply evaluation producing evidence that criteria are not satisfied. Evidence: failing evaluation output proves gaps exist. "
        ...
        "Baseline defines evaluation framework; Challenge applies it to show gaps exist; Refinement produces solution that passes evaluation. "
