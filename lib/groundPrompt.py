...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "Trace grammar — every state must have exactly this form, nothing else: "
        "  Active: <id>:<type>  "
        "  <exactly one transition line for that type>  "
        "  Pending: <id:type list, or 'empty'>  "
        "Any text between Pending: and the next Active: is a syntax error. "
        "A node may not be described, explained, or elaborated until after 'Planning complete.' "
        "A newly created node is inert until it appears in a future Active: line — "
        "a transition may never inspect or process a node it just created. "
        ...
        "AnalyzeSplit(D:SplitDecision): if D's source expression splits into two independent sub-expressions E1/E2, "
        "write 'AnalyzeSplit: D → SC1 / SC2', then 'SC1.expression=<E1 verbatim>', then 'SC2.expression=<E2 verbatim>'; "
        "add SC1:SplitCandidate, SC2:SplitCandidate. "
        "Otherwise write 'AnalyzeSplit: D → terminal'; add nothing. "
        "Materialize(SC:SplitCandidate): write 'Materialize: SC → P'; write 'P.source=<SC id>'; add P:Property. "
        "  (P.source=<SC id> is the only valid content line for P — any following line before Pending: is a syntax error.) "
        ...
        "every iteration emits exactly one Active:, one transition block, one Pending:; "
        "'Planning complete.' emitted only by PlanningComplete; "
        "SC1.expression and SC2.expression are verbatim sub-expressions of D's source expression."
    )
