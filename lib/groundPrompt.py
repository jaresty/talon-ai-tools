...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Protocol derivation: before ladder derivation, derive the complete protocol rules by applying P1-P6 — observe at every feasible rung before proceeding. Derive from P2 (one artifact per type) that each artifact type requires its own rung; derive from P3 (evidence required) that validation must be executable (tool-run); derive from P5 (derivation chain) that validation artifact derives from what it validates; derive from P2/P3 that validation artifact contains NO implementation behavior — it only asserts, it does not produce; this is called 'executable validation' — a file that asserts behavioral properties without containing behavior of its own. Similarly, from P2/P3 derive 'executable implementation' — a file that produces behavior directly without asserting about other artifacts. "
        ...
        "Derive the Session observation loop from P6 (thread sequencing) — the loop that observes current state before descending into ladder; observation must complete before manifest declares gaps. "
