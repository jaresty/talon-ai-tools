...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test file with test function names that describe expected behavior AND include actual assertions that will fail; these assertions define the intent that implementation must satisfy; "
        "The assertions in validation rung tests are FIXED intent declarations — they cannot be changed in later rungs; implementation must make them green, not make tests match implementation; "
