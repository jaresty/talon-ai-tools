...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'of the label used. '
        'The only valid basis for scope reduction, rung compression, or any deviation from the derived '
        'enforcement process is the completion check against the intent artifact explicitly excluding the item. '
        'Any other basis — including commentary about cost, complexity, heaviness, resource constraints, task '
        'simplicity, or proportionality — is not valid. '
        ...
        'production. The rung 1 coverage mapping is a separate required artifact produced after '
        'the ladder is closed — producing both in the same generative act does not satisfy this '
        'requirement. The ladder must be a completed artifact in the transcript before the rung 1 '
        'coverage mapping begins. '
        'Rung 0 is the intent artifact — a representation of intent, not intent itself. Its '
        'satisfaction condition checks the representation\'s content, not intent: it names which '
        'behavioral requirements appear in this representation, with a false-evaluating state of '
        'any named behavioral requirement being absent from the representation. Dimensions of '
        'intent not captured by the representation are outside the scope of all coverage mappings '
        'derived from it — the coverage mapping is bounded by the representation, not by intent. '
        'A rung 0 satisfaction condition of "given", "present", or any phrase that does not name '
        'the representation\'s behavioral requirements has not been derived. '
        'The artifact '
        ...
        'naming both (a) the specific artifact state that satisfies the previous rung\'s '
        'satisfaction condition and (b) the behavioral dimension that state fails to constrain; a '
        'rejects entry that names only a dimension label or boolean property without naming a '
        'specific satisfying artifact state is not valid. '
