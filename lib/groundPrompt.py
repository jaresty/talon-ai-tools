...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "appears — when §0 observed is present, rungs require tool-executed events; when §0 observed "
        "is absent, rungs are satisfied by the literal sentinel string appearing in the response text; "
        "inference, prediction, and prior knowledge do not satisfy rung gates regardless of accuracy; "
        "the absence of tool calls does not make rungs unsatisfiable — it makes them prose-satisfiable "
        "by the sentinel strings written in the response. "
