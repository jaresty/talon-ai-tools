...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "Validation rung: write test file with test function names that describe expected behavior; create STUB implementation files so tests can RUN (imports resolve); include COMMENTS using GIVEN-WHEN-THEN format describing the assertions that will be tested — do NOT include actual expect() assertions at this rung; assertions are written at the assertion rung, not validation. Tests MUST PASS at validation rung to move to assertion rung — they will only pass if there are no actual assertions (only comments). "
