...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        '\'§ properties complete\' in the transcript, '
        ...
        'After \'§ formalization complete\', immediately write the label line \'Formalized properties:\' — '
        ...
        'for each \'property [N]:\' line, write one or more atomic sub-property lines of the form \'property [Na]:\', \'property [Nb]:\', etc., each containing a formal expression — a sub-property line whose content is prose only does not satisfy this requirement; the conjunction of all sub-property expressions for a given \'property [N]:\' must be logically equivalent to the expression in \'property [N]:\' — a sub-property line introducing a symbol, predicate, or clause absent from \'property [N]:\' does not satisfy this requirement; '
        'if the property is already atomic (no strict sub-expression exists that is independently falsifiable), '
        'before writing \'property [Na]: atomic — \', write \'§ atomicity test: "[quoted sub-expression]"\' '
        'where [quoted sub-expression] is a verbatim phrase from the parent \'property [N]:\' expression, '
        'followed by a sentence naming why that sub-expression is not independently falsifiable from the remainder — '
        'a \'property [Na]: atomic —\' line that does not have a \'§ atomicity test:\' line with a verbatim quoted phrase above it does not satisfy this requirement; '
        '\'property [Na]: atomic —\' is valid only after \'§ atomicity test:\' has appeared; '
        'the restatement following \'atomic —\' must use the same notation class as the parent \'property [N]:\' expression — '
        'if the parent contains \'∀\' or \'∃\', the restatement must contain \'∀\' or \'∃\'; '
        'if the parent contains a typed function signature, the restatement must contain a typed function signature; '
        'if the parent contains \'f(\' followed by \')=\', the restatement must contain \'f(\' followed by \')=\'; '
        'a restatement that changes notation class does not satisfy the same notation class requirement; '
        ...
        'then write \'§ properties complete\'; '
        '\'§ properties complete\' is valid only after a valid \'§ properties check:\' line has appeared; '
        '(4) verify coverage: \'## Completion check\' must not appear before \'§5 enumeration complete\' in the transcript; '
        ...
        '\'§ enforcement complete\' in the transcript; '
        ...
        '\'§2 dimensions closed\', \'§ formalization complete\', \'§ properties complete\', '
        '\'§ test suite complete\', \'§4 coverage verified\', and \'§5 enumeration complete\' '
        'must appear as the final non-blank line of the assistant message block in which it is written — '
        'the sentinel line must begin with the literal first character of the sentinel string (\'§\' or \'#\'); '
...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "Path B §0 is a starting gate, not a bypass — description, analysis, and planning tasks run §1–§5 in full after writing '§0 Path B: [scenario]'; the declaration opens the ladder, it does not close it; on Path B, every rung sentinel is satisfied by the literal string appearing in the response text — no tool call is required or expected to satisfy any Path B rung. "
        ...
        "appears — on Path A, this is a tool-executed event; on Path B, this is the literal "
        "sentinel string appearing in the response text; inference, prediction, and prior knowledge "
        "do not satisfy rung gates regardless of accuracy; the absence of tool calls does not make "
        "Path B rungs unsatisfiable — it makes them prose-satisfiable by the sentinel strings "
