...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        'when all \'property [N]:\' and sub-property lines are written, write \'§ completeness check: "[governing goal text]" / [P1] / [P2] / ...\' '
        'quoting the governing goal verbatim from \'## Governing goal:\' above, then quoting the atomic expression from every \'property [N]: atomic —\' line, separated by \' / \' — '
        'a \'§ completeness check:\' line that omits the governing goal text or omits any listed property expression does not satisfy this requirement; '
        'then attempt to name one thing present in the quoted governing goal that is not covered by any quoted property expression; '
        'if such a thing can be named, write \'§ properties complete? no\' — then add a new \'property [N]:\' line covering it, '
        'run the split test for that property, and repeat the \'§ completeness check:\' cycle; '
        'only when no such thing can be named, write \'§ properties complete? yes\' — '
        'a \'§ properties complete? yes\' line that appears without a preceding \'§ completeness check:\' line does not satisfy this requirement; '
        'after \'§ properties complete? yes\' has appeared, immediately write \'§ properties complete\' — '
...
def build_ground_prompt() -> str:
    ...
    return (
        ...
        "The block contains one line per property, each beginning with 'property [N]:' followed by a formal definition in mathematical notation — a typed expression, logical predicate (∀/∃), or relational formula — with a single unambiguous denotation: two independent readers must reach the same conclusion about what satisfies it. "
        "Each property must be atomic. To test atomicity, write candidate sub-properties first: write 'property [Na]: <definition of part a>' and 'property [Nb]: <definition of part b>' before the split test. "
        "Then immediately emit '§ split test: property [Na]: \"<verbatim definition>\" — falsified by <construction that violates [Na] but not [Nb]>; property [Nb]: \"<verbatim definition>\" — falsified by <construction that violates [Nb] but not [Na]>; these falsifiers are independent' — "
        "a split test whose quoted definitions do not appear verbatim in the sub-property lines immediately above it does not satisfy this requirement. "
        "If no valid independent falsifier pair exists, emit '§ split test: property [N]: \"<verbatim definition>\" — atomic, no valid split: <reason>' and retain the original property [N] line; discard the candidate sub-property lines. "
        "A split is valid only when both (a) each part can be falsified by a construction that does not falsify the other, and (b) the parts have identical coverage to the original. "
        "A property that admits a valid split is not yet atomic and must be divided before the block is complete. "
        "Continue splitting until every property [N] has a '§ split test:' line showing no valid split. The block is complete when all properties are atomic. "
        "Properties must be observationally independent: for each property [N], there must exist at least one construction that violates property [N] without violating any other property in the block. "
        "A property for which every violation also witnesses another property in the block is observationally redundant and must be removed or merged before the block is complete. "
