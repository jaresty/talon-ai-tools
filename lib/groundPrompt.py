...
SENTINEL_TEMPLATES: dict[str, str] = {
    ...
    "impl_intent_achieved": "\U0001f535 Intent achieved \u2014 target: [assertion name] | evidence: [verbatim green output showing passing]",
}
...
_SENTINEL_GATES: dict[str, str] = {
    ...
    "impl_intent": "gate: this sentinel must appear immediately before each file-write tool call to implementation code; the target assertion name must be specific and the evidence must be verbatim output showing that assertion failing in the current state; emitting this sentinel without a preceding exec_observed showing the target assertion failing voids the intent and the subsequent write; fabrication of red evidence (output that does not come from actual tool execution) voids the sentinel and the rung; for backfill cases where tests already pass and cannot be seen red, perturb the implementation (introduce a controlled fault) to observe the test fail, then fix the perturbation",
    "impl_intent_achieved": "gate: this sentinel must appear after each file-write tool call to implementation code; the target assertion name must match the target from impl_intent and the evidence must be verbatim output showing that assertion passing in the current state; emitting this sentinel without a matching impl_intent in the same response voids the intent; fabrication of green evidence (output that does not come from actual tool execution) voids the sentinel and the rung",
}
