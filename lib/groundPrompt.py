...
GROUND_PARTS_MINIMAL: dict[str, str] = {
    "core": (
        ...
        "a criterion written without a citable manifest excerpt in this cycle is unanchored "
        "and the criteria rung has not been satisfied; "
        ...
        "the falsifying condition is valid only if part (b) names a behavioral gap statable without "
        "naming source files, function names, or variable names \u2014 "
        "if part (b) references implementation internals, the criterion is structural and must be rewritten "
        "before this rung closes; this is a content gate, not a self-check. "
        ...
        "invoke a tool call to read the current test file before emitting carry-forward rows \u2014 "
        "each row must cite a prior-failure name quotable verbatim from that read result in the transcript; "
        "a carry-forward row with no citable read-tool result in the transcript for this cycle is uncovered regardless of model recall; "
        ...
        "the cited rung label must appear verbatim in this thread\u2019s prior transcript output \u2014 "
        "if the rung label cannot be located in the transcript, the tool call is prohibited; "
        "model recall of a rung that fired does not satisfy this gate; "
        ...
        "a model summary of expected suite results does not satisfy this gate; "
        ...
        "Before emitting \u2705 Manifest declared, re-read the prose rung output via a tool call; "
        "every behavioral predicate in the verbatim read result must have a [T: gap-name] marker \u2014 "
        "\u2705 Manifest declared may not be emitted until this read exists in the transcript "
        "and every predicate is covered. "
