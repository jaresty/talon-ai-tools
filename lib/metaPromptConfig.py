...
PLANNING_DIRECTIVE: str = (
    "The response contains a planning block — a structurally verifiable sequence of four named sections "
    "appearing as text output before any task content. "
    "A planning block is present when the literal strings 'SECTION 1 —', 'SECTION 2 —', 'SECTION 3 —', and 'SECTION 4 —' "
    "each appear in this order in the response, with all text between them belonging to one of the four sections — "
    "a tool call result block appearing between 'SECTION 1 —' and 'SECTION 4 —' renders the planning block non-compliant. "
    ...
    "all content between 'SECTION 1 —' and 'SECTION 4 —' is text output — "
    "all text appears after one of those four headings and before the next.\n\n"
    "SECTION 1 — TOKEN DERIVATIONS:\n"
    "For each token appearing under the METHOD 方法 heading in CONSTRAINTS, output a full block:\n"
    ...
    "Section 1 contains one ## Derive: block per method token listed under the METHOD 方法 heading — "
    "an evaluator counts the ## Derive: headings in Section 1 and confirms the count equals the number of tokens listed under METHOD 方法; "
    "when no method tokens are present, Section 1 contains only single-line token entries.\n"
    ...
    "method tokens and non-method tokens alike. "
    ...
    "Section 4 is compliant when the literal string 'Derivations complete — tokens: [list]' appears after 'SECTION 4 —', "
    "and at least one '## Derive:' string appears between 'SECTION 1 —' and 'SECTION 2 —' in the same response when method tokens are present. "
    "When method tokens are present, Section 4 contains: "
    "'Derivations complete — tokens: [list]; first derive: [verbatim opening 6 words of first ## Derive block in Section 1]' "
    "When no method tokens are present, Section 4 contains: 'Derivations complete — tokens: [list]. Beginning task work.' "
    "Task content begins after the final line of SECTION 4 — a response is non-compliant when no text appears after the SECTION 4 readiness string. "
    "After SECTION 4, proceed to task content — no user input is required between the planning block and task content."
)
