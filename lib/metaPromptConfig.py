...
PLANNING_DIRECTIVE: str = (
    ...
    "After deriving each token individually, also derive each COMPOSITION RULE that applies given the co-present tokens: "
    "restate what the rule adds beyond the individual tokens and derive what it concretely requires for this specific task. "
    "Composition rule derivations are part of the hard gate — no task work may begin until they are also visible. "
    "Finally, produce one COMBINED STANCE derivation: given all tokens together, state in a single paragraph how they interact "
    "as a unified operating mode for this specific task — how each token modifies the others, what the integrated approach looks like, "
    "and what would be different if any token were absent. This derivation is always required regardless of whether any composition rule fired.\n\n"
