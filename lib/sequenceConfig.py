                ...
                "inner": {
                    ...
                    "steps": [
                        ...
                        },
                        {
                            ...
                            "prompt_hint": "Before evaluating: confirm the abduce block from this cycle appears in the transcript above, naming at least two alternatives. A vet step with no preceding abduce block in this cycle does not satisfy this gate. Then: identify the tool call type the probe step used to produce it — a Bash tool call executing a command (live execution) satisfies this step; a Read tool call or a Bash call using cat, head, tail, or equivalent (file read) does not. If the probe step used only file reads, the evidence is invalid — reject it and require a live invocation before continuing. Otherwise: evaluate the live output against the hypothesis. End with exactly one of: 'Goal condition: met' (if the goal is fully satisfied from this frame's evidence) or 'Goal condition: unmet — [one sentence naming what further cycles must produce]'.",
                        },
                    ],
                },
            },
