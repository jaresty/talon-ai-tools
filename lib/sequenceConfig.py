...
SEQUENCES: dict[str, dict[str, Any]] = {
        ...
        "steps": [
            {
                "token": "make method:prism",
                "role": "frame enumeration",
                "prompt_hint": "Enumerate independent frames that differ structurally. A frame name contains no verb and makes no causal claim. Each frame names a structural angle and what the live system would show from that vantage point that would confirm or refute the goal condition — observable runtime behavior is what the live system does when viewed from this angle, not which commands to run or which files to read; commands and investigation methods are the agent's job during the cycle, not the frame definition's job. Each frame description must name a class of output produced only by executing a command, test run, or API call — not readable from source files, and not present in the repository as a pre-existing file, recording, or snapshot. The invocation that produces this signal must require no credentials, no running external services, and no files not present at HEAD. Before naming a signal class: (1) state the root criterion; (2) enumerate paths that could nominally satisfy without governing — naming a code artifact with runtime language (type definitions, mapping tables, switch branches, schema files, function signatures), naming a conditional prediction (would show, would appear, could be observed as), or pointing at a pre-existing output file (recording, log, snapshot) — and name the literal string closing each path; (3) proceed only when no open path remains. A valid signal class is one of: stdout lines, stderr lines, log entries, test result records, application trace. Any claimed class not in this list must demonstrate it cannot exist in the repository at rest. No backtick-wrapped text, file paths, function names, or test names in frame descriptions; those belong to the dispatched agent, not this step. Also state the goal condition from the subject as a coverage criterion under the heading `Goal condition:` — a coverage criterion names an enumerable set of cases (a plural noun phrase naming multiple items to be checked) and names what evidence for each case would look like. A conditional statement ('if X then Y') is not a valid coverage criterion. This `Goal condition:` block becomes the stop_when for each frame's experiment cycle. Only enumerate frames whose experiments are fully agent-executable via tool-executed commands, API calls, or test runs that produce new output — frames that require only static reading or human data-gathering do not belong in this sequence.",
            },
            {
                "token": "check scope:good",
                "role": "frame quality gate",
                "prompt_hint": "Evaluate each frame from the prior step against these criteria: (1) the frame name contains no verb and makes no causal claim; (2) the frame description names a class of observable runtime output — one of: stdout lines, stderr lines, log entries, test result records, application trace — whose instances cannot exist in the repository at rest; (3) the frame description contains no backtick-wrapped text, file paths, function names, or test names; (4) the frame leaves hypothesis generation and command selection entirely to the dispatched agent; (5) the frame names the type of invocation that produces its signal — one of: a test run command, a CLI command, or an API call — and that invocation requires no credentials, no pre-existing output files, and no environment state beyond what is present in a default repository checkout. A frame description that references a pre-existing file (recording, log, snapshot) as its evidence source, or that requires credentials or running external services, does not satisfy criterion (5) and must be rewritten or replaced. For any frame that fails any criterion: rewrite it in place before proceeding. A frame that cannot be made to satisfy all criteria must be replaced with a new structurally distinct frame that does. Do not proceed to dispatch until every frame passes every criterion.",
            },
            {
                "type": "dispatch",
                ...
                "inner": {
                    ...
                    "steps": [
                        ...
                        {
                            "token": "show abduce",
                            "role": "alternative explanation enumeration",
                            "prompt_hint": "Given the live output just produced: enumerate at least two alternative explanations consistent with the observed output that would not satisfy the goal condition from this frame's angle. For each alternative, name the specific additional evidence that would distinguish it from the current hypothesis.",
                        },
                        {
                            "token": "check form:vet audit adversarial",
                            "role": "evidence evaluation",
                            "prompt_hint": "Before evaluating: (1) confirm the output block from the abduce step of this cycle appears in the transcript, naming at least two alternatives — an abduce output block is absent if no block from the abduce step of this cycle precedes this vet step in the transcript; (2) confirm the last Bash tool call before this vet step is a live execution, not a file-reading operation — identify the command text of that call; if it is cat, head, tail, sed, awk, or equivalent, the gate is not satisfied and a live execution must replace it as the last call before this vet step proceeds. Enumerate every path by which this step could appear to evaluate live output without doing so: (path 1) a live call occurred in probe but subsequent file reads followed it — closed by 'last Bash tool call before this vet step'; (path 2) the abduce output block is absent — closed by gate (1); (path 3) live execution is claimed in prose without a tool-call result — closed by 'command text of that call'. When no open path remains: evaluate that result block against the hypothesis and end with exactly one of: 'Goal condition: met' (if the goal is fully satisfied from this frame's evidence) or 'Goal condition: unmet — [one sentence naming what further cycles must produce]'.",
                        },
                    ],
                },
            },
        ...
        "steps": [
            {
                "token": "make method:prism",
                "role": "frame enumeration",
                "prompt_hint": "Partition into independent investigation frames — each names a system layer or component (e.g. 'connection layer', 'query layer') that can contain multiple hypotheses. A frame name contains no verb and makes no causal claim. A frame description names a structural area. Each frame description must name a class of output produced only by executing a command, test run, or API call — not readable from source files, and not present in the repository as a pre-existing file, recording, or snapshot. The invocation that produces this signal must require no credentials, no running external services, and no files not present at HEAD. Before naming a signal class: (1) state the root criterion; (2) enumerate paths that could nominally satisfy without governing — naming a code artifact with runtime language (type definitions, mapping tables, switch branches, schema files, function signatures), naming a conditional prediction (would show, would appear, could be observed as), or pointing at a pre-existing output file (recording, log, snapshot) — and name the literal string closing each path; (3) proceed only when no open path remains. A valid signal class is one of: stdout lines, stderr lines, log entries, test result records, application trace. Any claimed class not in this list must demonstrate it cannot exist in the repository at rest. No backtick-wrapped text, file paths, function names, or test names in frame descriptions; those belong to the dispatched agent, not this step. Do not generate hypotheses yet — enumeration is the only output of this step.",
            },
            {
                "token": "check scope:good",
                "role": "frame quality gate",
            ...
            },
            {
                ...
                "inner": {
                    ...
                    "steps": [
                        ...
                        {
                            "token": "show abduce",
                            "role": "alternative root cause enumeration",
                            "prompt_hint": "Given the live output just produced: enumerate at least two alternative root causes consistent with the observed output that would point to a different fix. For each alternative, name the specific additional evidence that would distinguish it from the current hypothesis.",
                        },
                        {
                            "token": "check form:vet audit adversarial",
                            "role": "evidence evaluation",
                            "prompt_hint": "Before evaluating: (1) confirm the output block from the abduce step of this cycle appears in the transcript, naming at least two alternatives — an abduce output block is absent if no block from the abduce step of this cycle precedes this vet step in the transcript; (2) confirm the last Bash tool call before this vet step is a live execution, not a file-reading operation — identify the command text of that call; if it is cat, head, tail, sed, awk, or equivalent, the gate is not satisfied and a live execution must replace it as the last call before this vet step proceeds. Enumerate every path by which this step could appear to evaluate live output without doing so: (path 1) a live call occurred in probe but subsequent file reads followed it — closed by 'last Bash tool call before this vet step'; (path 2) the abduce output block is absent — closed by gate (1); (path 3) live execution is claimed in prose without a tool-call result — closed by 'command text of that call'. When no open path remains: evaluate that result block against the hypothesis. If rejected, state why and name the next hypothesis to investigate — a vet rejection is complete only when followed by a new bar build make form:prep verify for the next hypothesis; a vet rejection with no subsequent prep step does not satisfy this requirement. End with exactly one of: 'Root cause: confirmed' (if the root cause is identified with sufficient evidence to act) or 'Root cause: unconfirmed — [one sentence naming what further cycles must produce]'.",
                        },
                    ],
                },
            },
    ...
    "orient-and-plan": {
        "description": "Establish destination and current position, project how the overall engagement could fail before any seam is salient, then ground conclusions in retained evidence and generate candidate moves against those failures, and finally commit to a controlled next move that follows the structural grain.",
        ...
        "steps": [
            {
                "token": "task:probe intent:orient method:navigate method:mapping method:flow method:control",
            ...
            },
            {
                ...
                "role": "retained evidence and candidate generation",
                "prompt_hint": "Use this step to retrieve relevant prior work and ground the projected failures in exact, concrete observations rather than recollection. Distinguish the different kinds of evidence in play and name which claims survive scrutiny and which do not, and where the evidence chain breaks. Then enumerate candidate technical moves without committing — each candidate must name which engagement-level failure from the prior step it materially mitigates; a move that only connects existing work without mitigating a named failure is not a candidate.",
            },
            {
                "token": "task:plan method:grain method:control method:verify",
                "role": "controlled next move",
                "prompt_hint": "Use this step to select the independently executable candidate with the most engagement-relative information value, following the latent structural grain rather than forcing a direction. Name rejected alternatives and the criterion that excluded each. Apply falsification pressure to the chosen move: make its negative space explicit — name what observation would show the move was the wrong bet, such that a claim whose negative space cannot be specified is not carried forward. Name the exact stop condition, what must not follow automatically, and keep recommendations within what is actually controllable.",
            },
        ],
    },
    "frame-spike": {
