        ...
        "steps": [
            {
                "token": "make method:prism",
                "role": "frame enumeration",
                "prompt_hint": "Partition into independent investigation frames — each names a system layer or component (e.g. 'connection layer', 'query layer') that can contain multiple hypotheses. A frame name contains no verb and makes no causal claim. A frame description names a structural area and the observable runtime behavior visible from that layer when a problem is present — observable runtime behavior is what the live system exhibits when viewed from this layer, not which commands to run or which files to read; commands and investigation methods are the agent's job during hypothesis cycles, not the frame definition's job. No backtick-wrapped text, file paths, function names, or test names in frame descriptions; those belong to the dispatched agent, not this step. Do not generate hypotheses yet — enumeration is the only output of this step.",
            },
            {
                "type": "dispatch",
