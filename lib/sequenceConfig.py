    ...
    "frame-explore": {
        ...
        "steps": [
            ...
            },
            {
                ...
                "prompt_hint": "Evaluate each frame from the prior step against these criteria: (1) the frame name contains no verb and makes no causal claim; (2) the frame description names a class of observable runtime output — one of: stdout lines, stderr lines, log entries, test result records, application trace — whose instances cannot exist in the repository at rest; (3) the frame description contains no backtick-wrapped text, file paths, function names, or test names; (4) the frame leaves hypothesis generation and command selection entirely to the dispatched agent. For any frame that fails any criterion: rewrite it in place before proceeding. A frame that cannot be made to satisfy all criteria must be replaced with a new structurally distinct frame that does. Do not proceed to dispatch until every frame passes every criterion.",
            },
            {
                "type": "dispatch",
