        ...
        "steps": [
            {
                "token": "make method:prism",
                "role": "frame enumeration",
                "prompt_hint": "Partition into independent investigation frames — each names a system layer or component (e.g. 'connection layer', 'query layer') that can contain multiple hypotheses. A frame name contains no verb and makes no causal claim. A frame description names a structural area and what the live system would show from that layer when a problem is present — observable runtime behavior is what the live system exhibits when viewed from this layer, not which commands to run or which files to read; which commands to run is the agent's job — but each frame description must name what the live system would show from this layer when the problem is present; a class of output produced by running the system (e.g. stdout lines, log entries, test results, application trace (stack frames from a running process)), not a prediction or static reference; a live signal answerable without running the system is not a valid live signal. No backtick-wrapped text, file paths, function names, or test names in frame descriptions; those belong to the dispatched agent, not this step. Do not generate hypotheses yet — enumeration is the only output of this step.",
            },
            {
                "type": "dispatch",
