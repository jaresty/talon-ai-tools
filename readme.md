...
- `Ctrl+P` opens the token palette so you can browse or reset selections without leaving the session; search for “copy command” to trigger the clipboard action, and press `Ctrl+W` while the palette is open to clear the current filter instantly.
