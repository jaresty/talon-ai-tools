...
echo ""
echo "Required before recording any verdict:"
echo "  Falsification case: write a one-paragraph response sketch that satisfies A's"
echo "  constraint AND B's constraint individually, but violates the combined A+B requirement."
echo "  If no such case can be constructed, the signal is likely a false positive — mark additive."
echo ""
echo "If composition confirmed: draft prose, add to lib/compositionConfig.py, run tests"
echo "If additive:             mark pair as 'additive' in docs/composition-candidates.md"
