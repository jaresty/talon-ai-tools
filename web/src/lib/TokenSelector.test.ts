...
describe('TokenSelector — F4 keyboard focus opens D2 metadata panel', () => {
	...
	it('F5: filter input retains focus after typing pause (no auto-focus to chips)', async () => {
		vi.useFakeTimers();
		const onToggle = vi.fn();
		render(TokenSelector, {
		...
		// After a long pause, filter input should retain focus — no auto-focus to chips
