...
const grammar: Grammar = {
	...
	persona: {
		presets: {},
		axes: { voice: ['as-developer'], audience: ['engineers'], tone: ['direct'] },
		intent: { axis_tokens: { intent: ['persuade', 'inform'] } }
	}
};
...
describe('parseCommand', () => {
	...
	it('parses positional audience token', () => {
		const result = parseCommand('bar build engineers show', grammar);
		expect(result.persona.audience).toBe('engineers');
		expect(result.selected.task).toEqual(['show']);
		expect(result.unrecognized).toEqual([]);
	});

	it('parses positional tone token', () => {
		const result = parseCommand('bar build direct show', grammar);
		expect(result.persona.tone).toBe('direct');
		expect(result.selected.task).toEqual(['show']);
		expect(result.unrecognized).toEqual([]);
	});

	it('parses positional intent token', () => {
		const result = parseCommand('bar build persuade show', grammar);
		expect(result.persona.intent).toBe('persuade');
		expect(result.selected.task).toEqual(['show']);
		expect(result.unrecognized).toEqual([]);
	});

	it('round-trips positional persona command generated by SPA', () => {
		const result = parseCommand('bar build as-developer engineers show full', grammar);
		expect(result.persona.voice).toBe('as-developer');
		expect(result.persona.audience).toBe('engineers');
		expect(result.selected.task).toEqual(['show']);
		expect(result.selected.completeness).toEqual(['full']);
